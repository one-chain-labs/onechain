...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn into_effects_v1(
        ...
        self.execution_results.written_objects.iter().for_each(|(id, object)| {
        ...
        self.execution_results
            ...
            .for_each(|id| {
                unwrapped_then_deleted.push((*id, lampot_version, ObjectDigest::OBJECT_DIGEST_DELETED));
            });
    ...
    fn get_objects_to_authenticate(
        ...
        for id in &self.execution_results.modified_objects {
            ...
            let old_obj = self
                .store
                .get_object(id)
                .unwrap_or_else(|| panic!("Modified object must exist in the store: ID = {:?}", id));
            match &old_obj.owner {
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!("Generated object IDs are not recorded in ExecutionResults in sui-execution v1");
    }
}
