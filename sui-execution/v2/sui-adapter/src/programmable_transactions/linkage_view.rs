...
impl<'state> LinkageView<'state> {
    ...
    pub fn has_linkage(&self, context: ObjectID) -> bool {
        self.linkage_info.as_ref().is_some_and(|l| l.storage_id == *context)
    }
    ...
    pub fn set_linkage(&mut self, context: &MovePackage) -> Result<AccountAddress, ExecutionError> {
        ...
        for TypeOrigin { module_name, datatype_name, package: defining_id } in context.type_origin_table() {
    ...
    pub(crate) fn link_context(&self) -> AccountAddress {
        self.linkage_info.as_ref().map_or(AccountAddress::ZERO, |l| l.storage_id)
    }
    ...
    pub(crate) fn defining_module(&self, runtime_id: &ModuleId, struct_: &IdentStr) -> Result<ModuleId, SuiError> {
        ...
        for TypeOrigin { module_name, datatype_name, package } in package.move_package().type_origin_table() {
