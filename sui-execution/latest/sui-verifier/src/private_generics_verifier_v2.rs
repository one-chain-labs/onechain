...
use sui_types::{error::ExecutionError, make_invariant_violation, MOVE_STDLIB_ADDRESS, SUI_FRAMEWORK_ADDRESS};
...
// Stdlib functions
pub const MOVE_STDLIB_INTERNAL_PERMIT: FunctionIdent = (MOVE_STDLIB_ADDRESS, INTERNAL_MODULE, ident_str!("permit"));

// Event function
pub const SUI_EVENT_EMIT_EVENT: FunctionIdent = (SUI_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("emit"));
pub const SUI_EVENT_EMIT_AUTHENTICATED: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("emit_authenticated"));
pub const SUI_EVENT_NUM_EVENTS: FunctionIdent = (SUI_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("num_events"));
pub const SUI_EVENT_EVENTS_BY_TYPE: FunctionIdent = (SUI_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("events_by_type"));

// Public transfer functions
pub const SUI_TRANSFER_PUBLIC_TRANSFER: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("public_transfer"));
pub const SUI_TRANSFER_PUBLIC_FREEZE_OBJECT: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("public_freeze_object"));
pub const SUI_TRANSFER_PUBLIC_SHARE_OBJECT: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("public_share_object"));
pub const SUI_TRANSFER_PUBLIC_RECEIVE: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("public_receive"));
pub const SUI_TRANSFER_RECEIVING_OBJECT_ID: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("receiving_object_id"));
pub const SUI_TRANSFER_PUBLIC_PARTY_TRANSFER: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("public_party_transfer"));

// Private transfer functions
pub const SUI_TRANSFER_TRANSFER: FunctionIdent = (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("transfer"));
pub const SUI_TRANSFER_FREEZE_OBJECT: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("freeze_object"));
pub const SUI_TRANSFER_SHARE_OBJECT: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("share_object"));
pub const SUI_TRANSFER_RECEIVE: FunctionIdent = (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("receive"));
pub const SUI_TRANSFER_PARTY_TRANSFER: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE, ident_str!("party_transfer"));

// Coin registry functions
pub const SUI_COIN_REGISTRY_NEW_CURRENCY: FunctionIdent =
    (SUI_FRAMEWORK_ADDRESS, COIN_REGISTRY_MODULE, ident_str!("new_currency"));
...
pub const EXHAUSTIVE_MODULES: &[(AccountAddress, &IdentStr)] =
    &[(SUI_FRAMEWORK_ADDRESS, EVENT_MODULE), (SUI_FRAMEWORK_ADDRESS, TRANSFER_MODULE)];
...
/// To avoid, issues, all `su::transfer` and `one::event` functions must be configured in `INTERNAL_FUNCTIONS`.
pub fn verify_module(module: &CompiledModule, _verifier_config: &VerifierConfig) -> Result<(), ExecutionError> {
    let module_id = module.self_id();
    let module_address = *module_id.address();
    let module_name = module_id.name();

    // Skip one::test_scenario
    ...
    if EXHAUSTIVE_MODULES.contains(&(module_address, module_name)) {
        for fdef in module.function_defs.iter().filter(|fdef| fdef.visibility == Visibility::Public) {
...
fn resolve_function<'a>(module: &'a CompiledModule, callee_handle: &FunctionHandle) -> FunctionIdent<'a> {
...
pub fn help_message(callee_addr: &AccountAddress, callee_module: &IdentStr, callee_function: &IdentStr) -> String {
    if *callee_addr == SUI_FRAMEWORK_ADDRESS && callee_module == TRANSFER_MODULE {
        format!(
            " If the type has the 'store' ability, use the public variant instead: 'one::transfer::public_{}'.",
            callee_function
        )
    } else {
        String::new()
