...
pub fn add_to_accumulator_address(
    ...
    let Some(value) = args.pop_back().unwrap().value_as::<Struct>().ok() else {
        ...
        return Err(PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
            .with_message("Balance should be guaranteed under current implementation".to_owned()));
    };
    ...
    let Some([amount]): Option<[Value; 1]> = value.unpack().ok().and_then(|vs| vs.collect::<Vec<_>>().try_into().ok())
    else {
        debug_assert!(false);
        return Err(PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
            .with_message("Balance should be guaranteed under current implementation".to_owned()));
