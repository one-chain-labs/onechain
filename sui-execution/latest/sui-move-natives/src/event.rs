...
pub fn emit_authenticated_impl(
    ...
    if !get_extension!(context, ObjectRuntime)?.protocol_config.enable_authenticated_event_streams() {
        return Ok(NativeResult::err(cost, NOT_SUPPORTED));
    }

    let event_ty = ty_args.pop().unwrap();
    // This type is always one::event::EventStreamHead
    ...
    emit_impl(context, event_ty, event_value, Some(StreamRef { accumulator_id, stream_id, stream_head_ty }))
}
...
struct StreamRef {
    ...
    stream_id: Value,
    // The type of the stream head. Should always be `one::event::EventStreamHead`
    stream_head_ty: Type,
}
...
fn emit_impl(
    ...
    let event_emit_cost_params = get_extension!(context, NativesCostTable)?.event_emit_cost_params.clone();

    native_charge_gas_early_exit!(context, event_emit_cost_params.event_emit_cost_base);

    let event_value_size = abstract_size(get_extension!(context, ObjectRuntime)?.protocol_config, &event_value);
    ...
    if let Some(max_event_emit_size_total) = obj_runtime.protocol_config.max_event_emit_size_total_as_option() {
    ...
    if let Some(StreamRef { accumulator_id, stream_id, stream_head_ty: _ }) = stream_ref {
...
pub fn num_events(
    ...
    Ok(NativeResult::ok(legacy_test_cost(), smallvec![Value::u32(num_events as u32)]))
}
...
pub fn get_events_by_type(
    ...
    let matched_events = object_runtime_ref
        ...
        .events()
        .iter()
        .filter_map(|(tag, event)| if &specified_type_tag == tag { Some(event.copy_value().unwrap()) } else { None })
        .collect::<Vec<_>>();
    Ok(NativeResult::ok(legacy_test_cost(), smallvec![move_vm_types::values::Vector::pack(
        specialization,
        matched_events
    )?]))
}
