        ...
        let obj_opt = if let Some(object) = child_opt {
            ...
            match object.into_inner().data {
                ...
                Data::Move(mo @ MoveObject { .. }) => {
                    Some(((CacheInfo::Loaded(mo.contents().len()), mo), loaded_metadata))
                }
            }
        } else {
            None
...
impl<'a> ChildObjectStore<'a> {
    ...
    pub(super) fn receive_object(
        ...
        let Some(((cache_info, obj), obj_meta)) = self.inner.receive_object_from_store(parent, child, child_version)?
        else {
            return Ok(None);
        };
    ...
    pub(super) fn object_exists_and_has_type(
        ...
        if let Some(child_object) = self.store.get(&child) {
            ...
            return Ok((CacheInfo::Cached, child_object.value.exists()? && &child_object.ty == child_move_type));
        }
        Ok(self
            .inner
