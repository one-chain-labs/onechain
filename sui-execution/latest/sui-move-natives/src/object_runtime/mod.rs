...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn delete_id(&mut self, id: ObjectID) -> PartialVMResult<()> {
        ...
        if let LimitThresholdCrossed::Hard(_, lim) = check_limit_by_meter!(
            ...
            self.protocol_config.max_num_deleted_move_object_ids_system_tx(),
    ...
    pub fn transfer(
        ...
        if let LimitThresholdCrossed::Hard(_, lim) = check_limit_by_meter!(
            ...
            self.protocol_config.max_num_transferred_move_object_ids_system_tx(),
    ...
    pub fn emit_accumulator_event(
        ...
        if let MoveAccumulatorValue::U64(amount) = value {
            ...
            match action {
                MoveAccumulatorAction::Merge => {
                    let current = self.state.accumulator_merge_totals.get(&key).copied().unwrap_or(0);
                    let new_total = current + amount as u128;
                    if new_total > u64::MAX as u128 {
                        return Err(PartialVMError::new(StatusCode::ARITHMETIC_ERROR).with_message(format!(
                ...
                MoveAccumulatorAction::Split => {
                    let current = self.state.accumulator_split_totals.get(&key).copied().unwrap_or(0);
                    let new_total = current + amount as u128;
                    if new_total > u64::MAX as u128 {
                        return Err(PartialVMError::new(StatusCode::ARITHMETIC_ERROR).with_message(format!(
        ...
        let event = MoveAccumulatorEvent { accumulator_id, action, target_addr, target_ty, value };
    ...
    pub(crate) fn add_child_object(
        ...
        self.child_object_store.add_object(parent, child, child_move_type, child_value)
    }
    ...
    pub(super) fn config_setting_cache_update(
        ...
        self.child_object_store.config_setting_cache_update(config_id, name_df_id, setting_value_object_type, value)
    }
    ...
    pub fn is_transferred(&self, id: &ObjectID) -> Option<Owner> {
        self.state.transfers.get(id).map(|(owner, _, _)| owner.clone())
    }
    ...
    pub fn finish(mut self) -> Result<RuntimeResults, ExecutionError> {
        ...
        let child_effects = self
            .child_object_store
            .take_effects()
            .map_err(|e| ExecutionError::invariant_violation(format!("Failed to take child object effects: {e}")))?;
...
impl ObjectRuntimeState {
    ...
    fn apply_child_object_effects_v0(
        ...
        for (child, child_object_effect) in child_object_effects {
            let ChildObjectEffectV0 { owner: parent, ty, effect } = child_object_effect;
    ...
    fn apply_child_object_effects_v1(
        ...
        for (child, child_object_effect) in child_object_effects {
            let ChildObjectEffectV1 { owner: parent, ty, final_value, object_changed } = child_object_effect;
            ...
            if object_changed {
                ...
                match final_value {
                    None => {
                        ...
                        // transferred ==> !deleted
                        debug_assert!(!self.transfers.contains_key(&child) || !self.deleted_ids.contains(&child));
                        ...
                        debug_assert!(
                            !self.deleted_ids.contains(&child)
                                || (!self.transfers.contains_key(&child) && !self.new_ids.contains(&child))
                        );
                    }
                    Some(v) => {
                        ...
                        // loaded ==> !new
                        debug_assert!(!loaded_child_objects.contains_key(&child) || !self.new_ids.contains(&child));
                ...
                debug_assert!(loaded_child_objects.get(&child).is_none_or(|loaded_child| !loaded_child.is_modified));
            }
        }
    }
}
