    ...
    pub fn object_has_changed(
        ...
        let Some(inner) = &self.0 else {
            return Err(PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                .with_message("Object fingerprint not enabled, yet we were asked for the changes".to_string()));
        };
        Ok(match (inner, final_value) {
            (F::Empty, None) => false,
            (F::Empty, Some(_)) | (F::Preexisting { .. }, None) => true,
            (
                F::Preexisting { owner: preexisting_owner, ty: preexisting_type, value: preexisting_value },
                Some(final_value),
            ) => {
