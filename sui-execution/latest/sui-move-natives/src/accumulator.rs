...
fn emit_event(
    ...
    let recipient = args.pop_back().unwrap().value_as::<AccountAddress>().unwrap();
    let accumulator: ObjectID = args.pop_back().unwrap().value_as::<AccountAddress>().unwrap().into();

    let ty_tag = context.type_to_type_tag(&ty_args.pop().unwrap())?;

    let obj_runtime: &mut ObjectRuntime = get_extension_mut!(context)?;

    obj_runtime.emit_accumulator_event(accumulator, action, recipient, ty_tag, MoveAccumulatorValue::U64(amount))?;
