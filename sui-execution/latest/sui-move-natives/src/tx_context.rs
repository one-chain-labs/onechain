...
pub fn derive_id(
    ...
    let tx_context_derive_id_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_derive_id_cost_params.clone();
...
pub fn fresh_id(
    ...
    let tx_context_fresh_id_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_fresh_id_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_fresh_id_cost_params.tx_context_fresh_id_cost_base);
    ...
    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::address(fresh_id.into())]))
}
 ...
 **************************************************************************************************/
pub fn sender(context: &mut NativeContext, ty_args: Vec<Type>, args: VecDeque<Value>) -> PartialVMResult<NativeResult> {
    debug_assert!(ty_args.is_empty());
    debug_assert!(args.is_empty());

    let tx_context_sender_cost_params = get_extension!(context, NativesCostTable)?.tx_context_sender_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_sender_cost_params.tx_context_sender_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let sender = transaction_context.sender();

    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::address(sender.into())]))
}
 ...
 **************************************************************************************************/
pub fn epoch(context: &mut NativeContext, ty_args: Vec<Type>, args: VecDeque<Value>) -> PartialVMResult<NativeResult> {
    debug_assert!(ty_args.is_empty());
    debug_assert!(args.is_empty());

    let tx_context_epoch_cost_params = get_extension!(context, NativesCostTable)?.tx_context_epoch_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_epoch_cost_params.tx_context_epoch_cost_base);
...
pub fn epoch_timestamp_ms(
    ...
    let tx_context_epoch_timestamp_ms_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_epoch_timestamp_ms_cost_params.clone();
    ...
    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::u64(timestamp)]))
}
 ...
 **************************************************************************************************/
pub fn sponsor(context: &mut NativeContext, ty_args: Vec<Type>, args: VecDeque<Value>) -> PartialVMResult<NativeResult> {
    debug_assert!(ty_args.is_empty());
    debug_assert!(args.is_empty());

    let tx_context_sponsor_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_sponsor_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_sponsor_cost_params.tx_context_sponsor_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let sponsor = transaction_context.sponsor().map(|addr| addr.into()).into_iter();
 ...
 **************************************************************************************************/
pub fn rgp(context: &mut NativeContext, ty_args: Vec<Type>, args: VecDeque<Value>) -> PartialVMResult<NativeResult> {
    debug_assert!(ty_args.is_empty());
    debug_assert!(args.is_empty());

    let tx_context_rgp_cost_params = get_extension!(context, NativesCostTable)?.tx_context_rgp_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_rgp_cost_params.tx_context_rgp_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let rgp = transaction_context.rgp();

    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::u64(rgp)]))
}
#[derive(Clone)]
...
pub fn gas_price(
    ...
    let tx_context_gas_price_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_gas_price_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_gas_price_cost_params.tx_context_gas_price_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let gas_price = transaction_context.gas_price();

    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::u64(gas_price)]))
}
...
pub fn gas_budget(
    ...
    let tx_context_gas_budget_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_gas_budget_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_gas_budget_cost_params.tx_context_gas_budget_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let gas_budget = transaction_context.gas_budget();

    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::u64(gas_budget)]))
}
...
pub fn ids_created(
    ...
    let tx_context_ids_created_cost_params =
        get_extension!(context, NativesCostTable)?.tx_context_ids_created_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_ids_created_cost_params.tx_context_ids_created_cost_base);

    let transaction_context: &mut TransactionContext = get_extension_mut!(context)?;
    let ids_created = transaction_context.ids_created();

    Ok(NativeResult::ok(context.gas_used(), smallvec![Value::u64(ids_created)]))
}
...
pub fn replace(
    ...
    let tx_context_replace_cost_params: TxContextReplaceCostParams =
        get_extension!(context, NativesCostTable)?.tx_context_replace_cost_params.clone();
    native_charge_gas_early_exit!(context, tx_context_replace_cost_params.tx_context_replace_cost_base);
    ...
    let rgp: u64 = if args_len == 9 { pop_arg!(args, u64) } else { transaction_context.rgp() };
