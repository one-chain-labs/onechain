...
fn is_supported(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(get_extension!(context, ObjectRuntime)?.protocol_config.enable_nitro_attestation())
}

fn is_upgraded(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(get_extension!(context, ObjectRuntime)?.protocol_config.enable_nitro_attestation_upgraded_parsing())
}

fn is_all_nonzero_pcrs_included(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(get_extension!(context, ObjectRuntime)?.protocol_config.enable_nitro_attestation_all_nonzero_pcrs_parsing())
}
...
pub fn load_nitro_attestation_internal(
    ...
    let cost_params = get_extension!(context, NativesCostTable)?.nitro_attestation_cost_params.clone();
    ...
    match parse_nitro_attestation(&attestation_bytes, is_upgraded(context)?, is_all_nonzero_pcrs_included(context)?) {
        Ok((signature, signed_message, payload)) => {
            let cert_chain_length = payload.get_cert_chain_length();
            native_charge_gas_early_exit_option!(
                context,
                cost_params.verify_base_cost.and_then(|base_cost| cost_params
