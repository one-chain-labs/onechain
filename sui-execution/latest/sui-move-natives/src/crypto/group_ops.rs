...
fn is_supported(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(context.extensions().get::<ObjectRuntime>()?.protocol_config.enable_group_ops_native_functions())
}

fn is_msm_supported(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(context.extensions().get::<ObjectRuntime>()?.protocol_config.enable_group_ops_native_function_msm())
}

fn is_uncompressed_g1_supported(context: &NativeContext) -> PartialVMResult<bool> {
    Ok(context.extensions().get::<ObjectRuntime>()?.protocol_config.uncompressed_g1_group_elements())
}

fn v2_native_charge(context: &NativeContext, cost: InternalGas) -> PartialVMResult<InternalGas> {
    Ok(if context.extensions().get::<ObjectRuntime>()?.protocol_config.native_charging_v2() {
        context.gas_used()
    } else {
        cost
    })
