...
pub fn transaction(
    ...
    let inputs = inputs.into_iter().map(|arg| input(env, tx_context, arg)).collect::<Result<Vec<_>, _>>()?;
...
fn input(env: &Env, tx_context: &TxContext, arg: CallArg) -> Result<(L::InputArg, L::InputType), ExecutionError> {
    Ok(match arg {
        ...
        CallArg::FundsWithdrawal(f) => {
            let FundsWithdrawalArg { reservation, type_arg, withdraw_from } = f;
            ...
            (
                L::InputArg::FundsWithdrawal(L::FundsWithdrawalArg { amount, ty: ty.clone(), owner }),
                L::InputType::Fixed(ty),
            )
        }
    })
