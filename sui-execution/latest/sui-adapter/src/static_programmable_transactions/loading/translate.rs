...
use crate::static_programmable_transactions::{env::Env, linkage::resolved_linkage::RootedLinkage, loading::ast as L};

pub fn transaction(env: &Env, pt: P::ProgrammableTransaction) -> Result<L::Transaction, ExecutionError> {
    let P::ProgrammableTransaction { inputs, commands } = pt;
    let inputs = inputs.into_iter().map(|arg| input(env, arg)).collect::<Result<Vec<_>, _>>()?;
...
fn input(env: &Env, arg: CallArg) -> Result<(L::InputArg, L::InputType), ExecutionError> {
    Ok(match arg {
        ...
        CallArg::Object(ObjectArg::Receiving(oref)) => (L::InputArg::Receiving(oref), L::InputType::Bytes),
        CallArg::Object(ObjectArg::ImmOrOwnedObject(oref)) => {
            ...
            let arg = match obj.owner {
                ...
                Owner::ObjectOwner(_) | Owner::Shared { .. } | Owner::ConsensusAddressOwner { .. } => {
        ...
        CallArg::Object(ObjectArg::SharedObject { id, initial_shared_version, mutable }) => {
            ...
            (
                L::InputArg::Object(L::ObjectArg::SharedObject { id, initial_shared_version, mutable, kind }),
...
fn command(env: &Env, command: P::Command) -> Result<L::Command, ExecutionError> {
    Ok(match command {
        P::Command::MoveCall(pmc) => {
            let resolved_linkage = env.linkage_analysis.compute_call_linkage(&pmc, env.linkable_store)?;
            let P::ProgrammableMoveCall { package, module, function: name, type_arguments: ptype_arguments, arguments } =
                *pmc;
            ...
            L::Command::MoveCall(Box::new(L::MoveCall { function, arguments }))
        }
        P::Command::MakeMoveVec(ptype_argument, arguments) => {
            let type_argument = ptype_argument.map(|ty| env.load_type_input(0, ty)).transpose()?;
            L::Command::MakeMoveVec(type_argument, arguments)
        }
        P::Command::TransferObjects(objects, address) => L::Command::TransferObjects(objects, address),
        ...
        P::Command::Publish(items, object_ids) => {
            let resolved_linkage = env.linkage_analysis.compute_publication_linkage(&object_ids, env.linkable_store)?;
