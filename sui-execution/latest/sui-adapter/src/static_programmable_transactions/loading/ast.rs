...
use crate::static_programmable_transactions::linkage::resolved_linkage::{ResolvedLinkage, RootedLinkage};
...
pub enum ObjectArg {
    ...
    SharedObject { id: ObjectID, initial_shared_version: SequenceNumber, mutable: bool, kind: SharedObjectKind },
}
...
impl Datatype {
    pub fn qualified_ident(&self) -> (&AccountAddress, &IdentStr, &IdentStr) {
        (self.module.address(), self.module.name(), self.name.as_ident_str())
    }
...
impl TryFrom<&Datatype> for StructTag {
    ...
    fn try_from(dt: &Datatype) -> Result<Self, Self::Error> {
        let Datatype { module, name, type_arguments, .. } = dt;
        Ok(StructTag {
            address: *module.address(),
            module: module.name().to_owned(),
            name: name.to_owned(),
            type_params: type_arguments.iter().map(|t| t.clone().try_into()).collect::<Result<Vec<TypeTag>, _>>()?,
        })
    }
}
