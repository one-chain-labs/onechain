...
impl<'pc, 'vm, 'state, 'linkage> Env<'pc, 'vm, 'state, 'linkage> {
    ...
    pub fn module_definition(&self, module_id: &ModuleId) -> Result<Arc<CompiledModule>, ExecutionError> {
        let data_store = SuiDataStore::new(self.linkage_view, &[]);
        self.vm.get_runtime().load_module(module_id, &data_store).map_err(|e| self.convert_vm_error(e))
    }

    pub fn fully_annotated_layout(&self, _ty: &Type) -> Result<annotated_value::MoveTypeLayout, ExecutionError> {
        todo!("LOADING")
    }

    pub fn runtime_layout(&self, _ty: &Type) -> Result<runtime_value::MoveTypeLayout, ExecutionError> {
        todo!("LOADING")
    }
