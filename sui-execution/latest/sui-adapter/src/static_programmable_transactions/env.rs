...
use crate::{
    data_store::{cached_package_store::CachedPackageStore, linked_data_store::LinkedDataStore, PackageStore},
    ...
    static_programmable_transactions::{
        ...
        loading::ast::{self as L, Datatype, LoadedFunction, LoadedFunctionInstantiation, Type, Vector},
    },
};
...
impl<'pc, 'vm, 'state, 'linkage> Env<'pc, 'vm, 'state, 'linkage> {
    ...
    pub fn convert_type_argument_error(&self, idx: usize, e: VMError, linkage: &RootedLinkage) -> ExecutionError {
    ...
    pub fn fully_annotated_layout(&self, ty: &Type) -> Result<annotated_value::MoveTypeLayout, ExecutionError> {
        let ty = self.load_vm_type_from_adapter_type(None, ty)?;
        self.vm.get_runtime().type_to_fully_annotated_layout(&ty).map_err(|e| self.convert_vm_error(e))
    }

    pub fn runtime_layout(&self, ty: &Type) -> Result<runtime_value::MoveTypeLayout, ExecutionError> {
        let ty = self.load_vm_type_from_adapter_type(None, ty)?;
        self.vm.get_runtime().type_to_type_layout(&ty).map_err(|e| self.convert_vm_error(e))
    }
    ...
    pub fn load_function(
        ...
        let Some(original_id) = linkage.resolved_linkage.resolve_to_original_id(&package) else {
            invariant_violation!("Package ID {:?} is not found in linkage generated for that package", package);
        };
        ...
        let runtime_signature = self
            .vm
            .get_runtime()
            .load_function(&runtime_id, name.as_ident_str(), &loaded_type_arguments, &mut data_store)
            .map_err(|e| {
                if e.major_status() == StatusCode::FUNCTION_RESOLUTION_FAILURE {
                    ExecutionError::new_with_source(
                        ExecutionErrorKind::FunctionNotFound,
                        format!("Could not resolve function '{}' in module {}", name, &storage_id,),
                    )
                } else {
        ...
        let return_ = runtime_signature
            ...
            .map(|ty| self.adapter_type_from_vm_type(&ty))
            .collect::<Result<Vec<_>, _>>()?;
        let signature = LoadedFunctionInstantiation { parameters, return_ };
        Ok(LoadedFunction {
            storage_id,
            runtime_id,
            name,
    ...
    pub fn load_type_from_struct(&self, tag: &StructTag) -> Result<Type, ExecutionError> {
        let vm_type = self.load_vm_type_from_type_tag(None, &TypeTag::Struct(Box::new(tag.clone())))?;
    ...
    /// layout are needed.
    pub fn load_type_and_layout_from_struct(&self, tag: &StructTag) -> Result<(Type, MoveTypeLayout), ExecutionError> {
        let vm_type = self.load_vm_type_from_type_tag(None, &TypeTag::Struct(Box::new(tag.clone())))?;
        let type_layout = self.vm.get_runtime().type_to_type_layout(&vm_type).map_err(|e| self.convert_vm_error(e))?;
        self.adapter_type_from_vm_type(&vm_type).map(|ty| (ty, type_layout))
    }

    pub fn type_layout_for_struct(&self, tag: &StructTag) -> Result<MoveTypeLayout, ExecutionError> {
    ...
    pub fn balance_type(&self, inner_type: Type) -> Result<Type, ExecutionError> {
        ...
        Ok(Type::Datatype(Rc::new(Datatype { abilities, module, name: n.to_owned(), type_arguments: vec![inner_type] })))
    }
    ...
    pub fn vector_type(&self, element_type: Type) -> Result<Type, ExecutionError> {
        let abilities = AbilitySet::polymorphic_abilities(AbilitySet::VECTOR, [false], [element_type.abilities()])
            .map_err(|e| ExecutionError::new_with_source(ExecutionErrorKind::VMInvariantViolation, e.to_string()))?;
        Ok(Type::Vector(Rc::new(L::Vector { abilities, element_type })))
    }
    ...
    fn load_vm_type_from_adapter_type(
        ...
        let tag: TypeTag = ty
            .clone()
            .try_into()
            .map_err(|s| ExecutionError::new_with_source(ExecutionErrorKind::VMInvariantViolation, s))?;
    ...
    fn load_vm_type_from_type_tag(
        ...
        fn load_type_tag(env: &Env, type_arg_idx: Option<usize>, tag: &TypeTag) -> Result<VMR::Type, ExecutionError> {
            Ok(match tag {
                ...
                TypeTag::Vector(inner) => VMR::Type::Vector(Box::new(load_type_tag(env, type_arg_idx, inner)?)),
        ...
        fn load_struct_tag(
            ...
            let tag_linkage = ResolvedLinkage::type_linkage(&[(*address).into()], env.linkable_store)?;
            let linkage = RootedLinkage::new(*address, tag_linkage);
            let linked_store = LinkedDataStore::new(&linkage, env.linkable_store);

            let original_id = linkage.resolved_linkage.resolve_to_original_id(&(*address).into()).ok_or_else(|| {
            ...
            if type_params.is_empty() {
                ...
                Ok(VMR::Type::DatatypeInstantiation(Box::new((idx, loaded_type_params))))
            }
        }
    ...
    fn adapter_type_from_vm_type(&self, vm_type: &vm_runtime_type::Type) -> Result<Type, ExecutionError> {
        ...
        Ok(match vm_type {
            ...
            VRT::Type::Vector(inner) => {
                ...
                let abilities =
                    self.vm.get_runtime().get_type_abilities(vm_type).map_err(|e| self.convert_vm_error(e))?;
                let vector_ty = Vector { abilities, element_type };
            ...
            ty @ VRT::Type::DatatypeInstantiation(inst) => {
                ...
                let abilities = runtime.get_type_abilities(ty).map_err(|e| self.convert_vm_error(e))?;
                let module = cached_info.defining_id.clone();
                let name = cached_info.name.clone();
                let type_arguments =
                    type_arguments.iter().map(|t| self.adapter_type_from_vm_type(t)).collect::<Result<Vec<_>, _>>()?;

                Type::Datatype(Rc::new(Datatype { abilities, module, name, type_arguments }))
            }
    ...
    fn load_vm_type_from_type_input(
        ...
        fn to_type_tag_internal(env: &Env, type_arg_idx: usize, ty: TypeInput) -> Result<TypeTag, ExecutionError> {
            Ok(match ty {
                ...
                TypeInput::Struct(struct_input) => {
                    let StructInput { address, module, name, type_params } = *struct_input;

                    let pkg = env.linkable_store.get_package(&address.into()).ok().flatten().ok_or_else(|| {
                        ExecutionError::from_kind(ExecutionErrorKind::TypeArgumentError {
                            argument_idx: type_arg_idx as u16,
                            kind: TypeArgumentError::TypeNotFound,
                        })
                    })?;
                    let Some(resolved_address) = pkg.type_origin_map().get(&(module.clone(), name.clone())).cloned()
                    else {
                        return Err(ExecutionError::from_kind(ExecutionErrorKind::TypeArgumentError {
                            argument_idx: type_arg_idx as u16,
                            kind: TypeArgumentError::TypeNotFound,
                        }));
                    };
                    ...
                    let tys = type_params
                        ...
                        .collect::<Result<Vec<_>, _>>()?;
                    TypeTag::Struct(Box::new(StructTag { address: *resolved_address, module, name, type_params: tys }))
                }
            })
        }
