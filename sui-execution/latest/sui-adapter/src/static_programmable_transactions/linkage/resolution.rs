...
impl ResolutionTable {
    pub fn empty() -> Self {
        Self { resolution_table: BTreeMap::new(), all_versions_resolution_table: BTreeMap::new() }
    }
    ...
    pub fn merge(&mut self, other: &ResolutionTable) -> Result<(), ExecutionError> {
        for (object_id, resolution) in other.resolution_table.iter() {
            if let Entry::Vacant(e) = self.resolution_table.entry(*object_id) {
                ...
                let existing_unifier = self.resolution_table.get_mut(object_id).expect("Guaranteed to exist");
        ...
        for (object_id, original_pkg_id) in other.all_versions_resolution_table.iter() {
            if !self.all_versions_resolution_table.contains_key(object_id) {
                self.all_versions_resolution_table.insert(*object_id, *original_pkg_id);
            }
        }
...
impl ConflictResolution {
    ...
    pub fn unify(&self, other: &ConflictResolution) -> Result<ConflictResolution, ExecutionError> {
        match (&self, other) {
            ...
            (ConflictResolution::AtLeast(self_version, sid), ConflictResolution::AtLeast(other_version, oid)) => {
                let id = if self_version > other_version { *sid } else { *oid };

                Ok(ConflictResolution::AtLeast(*self_version.max(other_version), id))
            }
...
/// package.
pub(crate) fn get_package(object_id: &ObjectID, store: &dyn PackageStore) -> Result<Rc<MovePackage>, ExecutionError> {
    store
        .get_package(object_id)
        .map_err(|e| ExecutionError::new_with_source(ExecutionErrorKind::PublishUpgradeMissingDependency, e))?
...
pub(crate) fn add_and_unify(
    ...
    if let Entry::Vacant(e) = resolution_table.resolution_table.entry(original_pkg_id) {
        ...
        let existing_unifier = resolution_table.resolution_table.get_mut(&original_pkg_id).expect("Guaranteed to exist");
        *existing_unifier = existing_unifier.unify(&resolution)?;
    }

    if !resolution_table.all_versions_resolution_table.contains_key(object_id) {
        resolution_table.all_versions_resolution_table.insert(*object_id, original_pkg_id);
    }
