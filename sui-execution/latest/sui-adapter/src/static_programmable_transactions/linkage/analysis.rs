...
impl LinkageAnalyzer {
    pub fn new<Mode: ExecutionMode>(protocol_config: &ProtocolConfig) -> Result<Self, ExecutionError> {
    ...
    pub fn compute_call_linkage(
        ...
        Ok(ResolvedLinkage::from_resolution_table(self.compute_call_linkage_(move_call, store)?))
    }
    ...
    pub fn compute_publication_linkage(
        ...
        Ok(ResolvedLinkage::from_resolution_table(self.compute_publication_linkage_(deps, store)?))
    }
    ...
    fn compute_call_linkage_(
        ...
        for object_id in transitive_deps.iter() {
            add_and_unify(object_id, store, &mut resolution_table, VersionConstraint::exact)?;
        }
        add_and_unify(&move_call.package, store, &mut resolution_table, VersionConstraint::exact)?;
        Ok(resolution_table)
    }
