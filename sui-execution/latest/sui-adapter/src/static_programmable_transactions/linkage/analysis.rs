...
pub fn type_linkage(ids: &[ObjectID], store: &dyn PackageStore) -> Result<ResolvedLinkage, ExecutionError> {
    ...
    for id in ids {
        ...
        add_and_unify(&package_id, store, &mut resolution_table, ConflictResolution::at_least)?;
        for object_id in transitive_deps.iter() {
            add_and_unify(object_id, store, &mut resolution_table, ConflictResolution::at_least)?;
        }
    }
