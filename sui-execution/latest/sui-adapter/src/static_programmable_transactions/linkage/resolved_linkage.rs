...
use crate::static_programmable_transactions::linkage::resolution::{ConflictResolution, ResolutionTable};
...
impl ResolvedLinkage {
    ...
    pub(crate) fn from_resolution_table(resolution_table: ResolutionTable) -> Self {
        ...
        for (original_id, resolution) in resolution_table.resolution_table {
            match resolution {
                ConflictResolution::Exact(version, object_id) | ConflictResolution::AtLeast(version, object_id) => {
                    linkage.insert(original_id, object_id);
                    versions.insert(original_id, version);
                }
            }
        }
        Self { linkage, linkage_resolution: resolution_table.all_versions_resolution_table, versions }
    }
}
