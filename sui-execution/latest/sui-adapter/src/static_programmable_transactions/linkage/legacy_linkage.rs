...
use crate::{
    ...
    static_programmable_transactions::linkage::{
        ...
        resolution::{add_and_unify, get_package, ConflictResolution, ResolutionTable},
...
impl LinkageAnalysis for LegacyLinkage {
    fn compute_call_linkage(
        ...
        Ok(ResolvedLinkage::from_resolution_table(self.compute_call_linkage(move_call, store)?))
    }
    ...
    fn compute_publication_linkage(
        ...
        Ok(ResolvedLinkage::from_resolution_table(self.compute_publication_linkage(deps, store)?))
    }
...
impl LegacyLinkage {
    ...
    pub fn new(
        ...
        Ok(Self { internal: ResolutionConfig { linkage_config, binary_config } })
    }
    ...
    fn compute_call_linkage(
        ...
        let mut resolution_table = self.internal.linkage_config.resolution_table_with_native_packages(store)?;
        ...
        for object_id in transitive_deps.iter() {
            add_and_unify(object_id, store, &mut resolution_table, ConflictResolution::exact)?;
        }
        add_and_unify(&move_call.package, store, &mut resolution_table, ConflictResolution::exact)?;
        Ok(resolution_table)
    }
    ...
    pub(crate) fn compute_publication_linkage(
        ...
        for id in deps {
            ...
            resolution_table.all_versions_resolution_table.insert(pkg.id(), pkg.original_package_id());
        }
        Ok(resolution_table)
    }
}
