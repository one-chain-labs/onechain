...
pub fn meter(meter: &mut TranslationMeter, transaction: &L::Transaction) -> Result<(), ExecutionError> {
...
fn command_linkage(cmd: &L::Command) -> Option<&ResolvedLinkage> {
    match cmd {
        L::Command::Publish(_, _, linkage) | L::Command::Upgrade(_, _, _, _, linkage) => Some(linkage),
