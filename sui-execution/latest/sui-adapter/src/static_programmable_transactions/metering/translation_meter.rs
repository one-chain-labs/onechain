...
impl<'pc, 'gas> TranslationMeter<'pc, 'gas> {
    pub fn new(protocol_config: &'pc ProtocolConfig, gas_charger: &'gas mut GasCharger) -> TranslationMeter<'pc, 'gas> {
        TranslationMeter { protocol_config, charger: gas_charger }
    }

    pub fn charge_base_inputs(&mut self, num_inputs: usize) -> Result<(), ExecutionError> {
        let amount = (num_inputs as u64).max(1).saturating_mul(self.protocol_config.translation_per_input_base_charge());
        self.charge(amount)
    }

    pub fn charge_pure_input_bytes(&mut self, num_bytes: usize) -> Result<(), ExecutionError> {
        let amount =
            (num_bytes as u64).max(1).saturating_mul(self.protocol_config.translation_pure_input_per_byte_charge());
        self.charge(amount)
    }

    pub fn charge_base_command(&mut self, num_args: usize) -> Result<(), ExecutionError> {
        let amount = (num_args as u64).max(1).saturating_mul(self.protocol_config.translation_per_command_base_charge());
        self.charge(amount)
    }
    ...
    pub fn charge_num_type_nodes(&mut self, num_type_nodes: u64) -> Result<(), ExecutionError> {
        let amount = num_type_nodes.max(1).saturating_mul(self.protocol_config.translation_per_type_node_charge());
        self.charge(amount)
    }

    pub fn charge_num_type_references(&mut self, num_type_references: u64) -> Result<(), ExecutionError> {
        let amount = self.reference_cost_formula(num_type_references.max(1))?;
        let amount = amount.saturating_mul(self.protocol_config.translation_per_reference_node_charge());
        self.charge(amount)
    }

    pub fn charge_num_linkage_entries(&mut self, num_linkage_entries: usize) -> Result<(), ExecutionError> {
    ...
    fn charge(&mut self, amount: u64) -> Result<(), ExecutionError> {
        ...
        self.charger.move_gas_status_mut().deduct_gas(amount.into()).map_err(Self::gas_error)
    }
