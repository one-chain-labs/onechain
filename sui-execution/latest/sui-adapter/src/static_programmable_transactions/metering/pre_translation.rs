...
pub fn meter(meter: &mut TranslationMeter, transaction: &ProgrammableTransaction) -> Result<(), ExecutionError> {
...
fn arguments_len(cmd: &Command) -> usize {
    match cmd {
        Command::MoveCall(call) => call.type_arguments.len().saturating_add(call.arguments.len()),
        Command::TransferObjects(args, _) | Command::SplitCoins(_, args) | Command::MergeCoins(_, args) => {
        ...
        Command::Upgrade(modules, deps, _, _) => modules.len().saturating_add(deps.len()).saturating_add(2),
    }
}
