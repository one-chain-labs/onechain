...
pub struct WithdrawalInput {
    pub original_input_index: InputIndex,
    /// The full type `one::funds_accumulator::Withdrawal<T>`
    pub ty: Type,
...
pub enum Command__ {
    ...
    Upgrade(Vec<Vec<u8>>, Vec<ObjectID>, ObjectID, Argument, ResolvedLinkage),
}
...
impl Transaction {
    pub fn types(&self) -> impl Iterator<Item = &Type> {
        ...
        pure_types.chain(object_types).chain(receiving_types).chain(command_types)
    }
}
...
impl Usage {
    ...
    pub fn new_copy(location: Location) -> Usage {
        Usage::Copy { location, borrowed: OnceCell::new() }
    }
...
impl Command__ {
    pub fn arguments(&self) -> Box<dyn Iterator<Item = &Argument> + '_> {
        match self {
            ...
            Command__::TransferObjects(objs, addr) => Box::new(objs.iter().chain(std::iter::once(addr))),
            Command__::SplitCoins(_, coin, amounts) => Box::new(std::iter::once(coin).chain(amounts)),
            Command__::MergeCoins(_, target, sources) => Box::new(std::iter::once(target).chain(sources)),
    ...
    pub fn types(&self) -> Box<dyn Iterator<Item = &Type> + '_> {
        match self {
            ...
            Command__::SplitCoins(ty, arg, args) | Command__::MergeCoins(ty, arg, args) => {
                Box::new(std::iter::once(arg).chain(args.iter()).map(argument_type).chain(std::iter::once(ty)))
            }
            Command__::MakeMoveVec(ty, args) => Box::new(args.iter().map(argument_type).chain(std::iter::once(ty))),
            ...
            Command__::Upgrade(_, _, _, arg, _) => Box::new(std::iter::once(arg).map(argument_type)),
            Command__::Publish(_, _, _) => Box::new(std::iter::empty()),
        }
    }
