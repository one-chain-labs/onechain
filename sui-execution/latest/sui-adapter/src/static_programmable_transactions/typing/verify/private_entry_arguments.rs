...
impl Temperature {
    fn add(self, other: Temperature) -> Result<Temperature, ExecutionError> {
        Ok(match (self, other) {
            ...
            (Temperature::Count(a), Temperature::Count(b)) => {
                Temperature::Count(a.checked_add(b).ok_or_else(|| make_invariant_violation!("Hot count overflow"))?)
            }
        })
    }
    ...
    fn sub(self, b: usize) -> Result<Temperature, ExecutionError> {
        Ok(match self {
            ...
            Temperature::Count(a) => Temperature::Count(
                a.checked_sub(b).ok_or_else(|| make_invariant_violation!("Hot count cannot go negative"))?,
            ),
        })
    }
...
impl Cliques {
    ...
    fn merge(&mut self, clique_ids: BTreeSet<CliqueID>) -> Result<CliqueID, ExecutionError> {
        let roots: BTreeSet<CliqueID> = clique_ids.iter().map(|&id| self.root(id)).collect::<Result<_, _>>()?;
        Ok(match roots.len() {
            0 => self.next(),
    ...
    fn input_value(&mut self) -> Value {
        ...
        Value::Normal { clique, heats: false }
    }
...
impl Context {
    fn new(txn: &T::Transaction) -> Self {
        ...
        let objects = (0 .. txn.objects.len()).map(|_| Some(cliques.input_value())).collect();
        let withdrawals = (0 .. txn.withdrawals.len()).map(|_| Some(cliques.input_value())).collect();
        let pure = (0 .. txn.pure.len()).map(|_| Some(cliques.input_value())).collect();
        let receiving = (0 .. txn.receiving.len()).map(|_| Some(cliques.input_value())).collect();
        Self { tx_context, cliques, gas_coin, objects, withdrawals, pure, receiving, results: vec![] }
    }

    // Checks if all values are released and that all hot counts are zero
    fn finish(self) -> Result<(), ExecutionError> {
        let Context { mut cliques, tx_context, gas_coin, objects, withdrawals, pure, receiving, results } = self;
...
pub fn verify<Mode: ExecutionMode>(env: &Env, txn: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    for c in &txn.commands {
        let result_values = command::<Mode>(env, &mut context, c).map_err(|e| e.with_command_index(c.idx as usize))?;
        assert_invariant!(result_values.len() == c.value.result_type.len(), "result length mismatch");
...
fn command<Mode: ExecutionMode>(
    ...
    let merged_clique = context.cliques.merge(argument_cliques.into_iter().map(|(_, c)| c).collect())?;
    ...
    assert_invariant!(drop_values.len() == result_type.len(), "drop_values length mismatch");
    result_type
        .iter()
        .zip(drop_values)
        .map(|(ty, dropped)| {
...
fn argument(_env: &Env, context: &mut Context, arg: &T::Argument) -> Result<Option<CliqueID>, ExecutionError> {
...
fn move_call<Mode: ExecutionMode>(
    ...
    let T::MoveCall { function, arguments: _ } = call;
