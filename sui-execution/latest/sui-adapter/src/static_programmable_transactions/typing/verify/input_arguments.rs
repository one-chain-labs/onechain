    ...
    fn new(txn: &T::Transaction) -> Self {
        let objects = txn
            ...
            .map(|object_input| match &object_input.arg {
                ObjectArg::ImmObject(_) => ObjectUsage { allow_by_value: false, allow_by_mut_ref: false },
                ObjectArg::OwnedObject(_) => ObjectUsage { allow_by_value: true, allow_by_mut_ref: true },
                ObjectArg::SharedObject { mutable, .. } => {
                    ObjectUsage { allow_by_value: *mutable, allow_by_mut_ref: *mutable }
                }
            })
            .collect();
        Self { objects }
...
pub fn verify<Mode: ExecutionMode>(_env: &Env, txn: &T::Transaction) -> Result<(), ExecutionError> {
    let T::Transaction { bytes, objects: _, pure, receiving, commands } = txn;
    for pure in pure {
...
fn check_pure_input<Mode: ExecutionMode>(bytes: &IndexSet<Vec<u8>>, pure: &T::PureInput) -> Result<(), ExecutionError> {
    let T::PureInput { original_input_index, byte_index, ty, constraint } = pure;
    let Some(bcs_bytes) = bytes.get_index(*byte_index) else {
        invariant_violation!("Unbound byte index {} for pure input at index {}", byte_index, original_input_index.0);
    };
    let BytesConstraint { command, argument } = constraint;
    check_pure_bytes::<Mode>(*argument, bcs_bytes, ty).map_err(|e| e.with_command_index(*command as usize))
}
...
fn check_pure_bytes<Mode: ExecutionMode>(
    ...
    assert_invariant!(!matches!(constraint, Type::Reference(_, _)), "references should not be added as a constraint");
    if Mode::allow_arbitrary_values() {
        return Ok(());
    }
    let Some(layout) = primitive_serialization_layout(constraint)? else {
        let msg =
            format!("Invalid usage of `Pure` argument for a non-primitive argument type at index {command_arg_idx}.",);
        return Err(ExecutionError::new_with_source(
            ExecutionErrorKind::command_argument_error(CommandArgumentError::InvalidUsageOfPureArg, command_arg_idx),
            msg,
        ));
    };
...
fn primitive_serialization_layout(param_ty: &Type) -> Result<Option<PrimitiveArgumentLayout>, ExecutionError> {
    Ok(match param_ty {
...
fn check_receving_input(receiving: &T::ReceivingInput) -> Result<(), ExecutionError> {
    let T::ReceivingInput { original_input_index: _, object_ref: _, ty, constraint } = receiving;
    let BytesConstraint { command, argument } = constraint;
...
fn check_receiving(command_arg_idx: u16, constraint: &Type) -> Result<(), ExecutionError> {
    if is_valid_receiving(constraint) {
        ...
        Err(command_argument_error(CommandArgumentError::TypeMismatch, command_arg_idx as usize))
    }
}
...
fn check_obj_usages(context: &mut Context, arguments: &[T::Argument]) -> Result<(), ExecutionError> {
...
fn check_obj_by_mut_ref(context: &mut Context, arg_idx: u16, location: &T::Location) -> Result<(), ExecutionError> {
    match location {
        ...
        T::Location::ObjectInput(idx) => {
            if !context.objects[*idx as usize].allow_by_mut_ref {
                Err(command_argument_error(CommandArgumentError::InvalidObjectByMutRef, arg_idx as usize))
            } else {
                Ok(())
            }
        }
...
fn check_by_value(context: &mut Context, arg_idx: u16, location: &T::Location) -> Result<(), ExecutionError> {
    match location {
        T::Location::GasCoin
        ...
        T::Location::ObjectInput(idx) => {
            if !context.objects[*idx as usize].allow_by_value {
                Err(command_argument_error(CommandArgumentError::InvalidObjectByValue, arg_idx as usize))
            } else {
                Ok(())
            }
        }
...
fn check_gas_by_value_loc(idx: u16, location: &T::Location) -> Result<(), ExecutionError> {
    match location {
        T::Location::GasCoin => Err(command_argument_error(CommandArgumentError::InvalidGasCoinUsage, idx as usize)),
        T::Location::TxContext
