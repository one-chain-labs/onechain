        ...
        let inputs = inputs
            ...
            .map(|(arg, _)| {
                Some(match arg {
                    ...
                    InputArg::Object(ObjectArg::ImmObject(_)) => {
                        ObjectUsage { allow_by_value: false, allow_by_mut_ref: false }
                    }
                    InputArg::Object(ObjectArg::OwnedObject(_)) => {
                        ObjectUsage { allow_by_value: true, allow_by_mut_ref: true }
                    }
                    InputArg::Object(ObjectArg::SharedObject { mutable, .. }) => {
...
fn check_constraint<Mode: ExecutionMode>(
    ...
    match arg {
        ...
        InputArg::Object(ObjectArg::ImmObject(_) | ObjectArg::OwnedObject(_) | ObjectArg::SharedObject { .. }) => {
...
fn check_obj_usage(context: &mut Context, arg: &T::Argument) -> Result<(), ExecutionError> {
    match &arg.value.0 {
        ...
        T::Argument__::Borrow(false, _) | T::Argument__::Use(T::Usage::Copy { .. }) | T::Argument__::Read(_) => Ok(()),
    }
}
...
fn check_obj_by_mut_ref(context: &mut Context, arg_idx: u16, location: &T::Location) -> Result<(), ExecutionError> {
    match location {
        ...
        T::Location::Input(idx) => match &context.inputs[*idx as usize] {
            None | Some(ObjectUsage { allow_by_mut_ref: true, .. }) => Ok(()),
            Some(ObjectUsage { allow_by_mut_ref: false, .. }) => {
...
fn check_by_value_ref(context: &mut Context, arg_idx: u16, location: &T::Location) -> Result<(), ExecutionError> {
    match location {
        T::Location::GasCoin | T::Location::Result(_, _) => Ok(()),
        T::Location::Input(idx) => match &context.inputs[*idx as usize] {
            None | Some(ObjectUsage { allow_by_value: true, .. }) => Ok(()),
            Some(ObjectUsage { allow_by_value: false, .. }) => {
...
fn check_gas_by_value(arg: &T::Argument) -> Result<(), ExecutionError> {
    match &arg.value.0 {
        ...
        T::Argument__::Borrow(_, _) | T::Argument__::Use(T::Usage::Copy { .. }) | T::Argument__::Read(_) => Ok(()),
    }
}
