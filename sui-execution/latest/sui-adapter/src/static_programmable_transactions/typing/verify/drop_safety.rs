...
pub fn refine_and_verify<Mode: ExecutionMode>(env: &Env, ast: &mut T::Transaction) -> Result<(), ExecutionError> {
...
mod verify {
    ...
    impl Context {
        fn new(ast: &T::Transaction) -> Result<Self, ExecutionError> {
            ...
            let withdrawals = ast.withdrawals.iter().map(|_| Some(Value)).collect::<Vec<_>>();
            let pure = ast.pure.iter().map(|_| Some(Value)).collect::<Vec<_>>();
            let receiving = ast.receiving.iter().map(|_| Some(Value)).collect::<Vec<_>>();
            Ok(Self {
    ...
    pub fn transaction<Mode: ExecutionMode>(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
        ...
        for c in commands {
            ...
            assert_invariant!(result.len() == c.value.result_type.len(), "result length mismatch");
            // drop unused result values
            assert_invariant!(result.len() == c.value.drop_values.len(), "drop values length mismatch");
        ...
        let Context { tx_context, gas_coin, objects, withdrawals, pure, receiving, results } = context;
    ...
    fn drop_value<Mode: ExecutionMode>((i, j): (usize, usize), value: Value, ty: &Type) -> Result<(), ExecutionError> {
        ...
        if !abilities.has_drop() && !Mode::allow_arbitrary_values() {
            ...
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::UnusedValueWithoutDrop { result_idx: i as u16, secondary_idx: j as u16 },
                msg,
            ));
        }
        consume_value(value);
    ...
    fn move_value(context: &mut Context, l: T::Location) -> Result<Value, ExecutionError> {
        let Some(value) = context.location(l).take() else { invariant_violation!("memory safety should have failed") };
        Ok(value)
    }

    fn copy_value(context: &mut Context, l: T::Location) -> Result<Value, ExecutionError> {
        assert_invariant!(context.location(l).is_some(), "memory safety should have failed");
        Ok(Value)
    }
