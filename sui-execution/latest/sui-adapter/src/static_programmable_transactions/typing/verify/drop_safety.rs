...
mod verify {
    ...
    impl Context {
        fn new(ast: &T::Transaction) -> Result<Self, ExecutionError> {
            ...
            Ok(Self { gas_coin: Some(Value), inputs, results: Vec::with_capacity(ast.commands.len()) })
        }
    ...
    pub fn transaction(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
        ...
        let Context { gas_coin, inputs, results } = context;
    ...
    fn drop_value_opt(idx: (usize, usize), value: Option<Value>, ty: &Type) -> Result<(), ExecutionError> {
        match value {
    ...
    fn drop_value((i, j): (usize, usize), value: Value, ty: &Type) -> Result<(), ExecutionError> {
        if !ty.abilities().has_drop() {
            return Err(
                ExecutionErrorKind::UnusedValueWithoutDrop { result_idx: i as u16, secondary_idx: j as u16 }.into()
            );
        }
        consume_value(value);
        Ok(())
