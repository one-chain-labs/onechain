...
use crate::{
    ...
    sp,
    static_programmable_transactions::{env::Env, loading::ast::Type, typing::ast as T},
};
...
pub fn verify<Mode: ExecutionMode>(env: &Env, txn: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    for c in &txn.commands {
        let result_dirties = command::<Mode>(env, &mut context, c).map_err(|e| e.with_command_index(c.idx as usize))?;
        assert_invariant!(result_dirties.len() == c.value.result_type.len(), "result length mismatch");
