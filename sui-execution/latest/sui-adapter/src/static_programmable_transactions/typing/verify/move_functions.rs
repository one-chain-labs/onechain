...
impl Context {
    fn new(inputs: &T::Inputs) -> Self {
        let inputs = inputs
            ...
            .collect();
        Self { gas_coin: IsDirty::Fixed { is_dirty: false }, inputs, results: vec![] }
    }

    // check if dirty, and mark it as fixed if mutably borrowing a pure input
    fn is_dirty(&mut self, arg: &T::Argument) -> bool {
        match &arg.value.0 {
            T::Argument__::Borrow(/* mut */ true, T::Location::Input(i)) => match &mut self.inputs[*i as usize] {
    ...
    fn mark_loc_dirty(&mut self, location: T::Location) {
        match location {
            ...
            T::Location::Result(i, j) => self.results[i as usize][j as usize] = IsDirty::Fixed { is_dirty: true },
        }
    }
}
...
fn command<Mode: ExecutionMode>(
    ...
    match &command.value {
        ...
        T::Command_::SplitCoins(_, coin, amounts) => {
            ...
            context.results.push(vec![IsDirty::Fixed { is_dirty }; result.len()]);
        }
...
fn move_call<Mode: ExecutionMode>(
    ...
    let T::MoveCall { function, arguments: args } = call;
    ...
    let arg_dirties = args.iter().map(|arg| argument(env, context, arg)).collect::<Vec<_>>();
    if is_entry {
...
fn check_signature<Mode: ExecutionMode>(function: &T::LoadedFunction) -> Result<(), ExecutionError> {
    fn check_return_type<Mode: ExecutionMode>(idx: usize, return_type: &T::Type) -> Result<(), ExecutionError> {
        match return_type {
            Type::Reference(_, _) => {
