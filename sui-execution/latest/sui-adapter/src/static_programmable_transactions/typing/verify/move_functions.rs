...
use crate::{
    ...
    programmable_transactions::execution::check_private_generics_v2,
    sp,
...
fn command<Mode: ExecutionMode>(env: &Env, sp!(_, c): &T::Command) -> Result<(), ExecutionError> {
    let T::Command_ { command, result_type: _, drop_values: _, consumed_shared_objects: _ } = c;
    match command {
...
fn check_signature<Mode: ExecutionMode>(env: &Env, function: &T::LoadedFunction) -> Result<(), ExecutionError> {
...
fn check_visibility<Mode: ExecutionMode>(env: &Env, function: &T::LoadedFunction) -> Result<(), ExecutionError> {
