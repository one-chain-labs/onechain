...
impl Context {
    fn new(ast: &T::Transaction) -> Result<Self, ExecutionError> {
        ...
        let withdrawals = ast.withdrawals.iter().map(|_| Some(Value::NonRef)).collect::<Vec<_>>();
        let pure = ast.pure.iter().map(|_| Some(Value::NonRef)).collect::<Vec<_>>();
        let receiving = ast.receiving.iter().map(|_| Some(Value::NonRef)).collect::<Vec<_>>();
        let (mut graph, _locals) = Graph::new::<()>([]).map_err(graph_err)?;
        let local_root = graph.extend_by_epsilon((), std::iter::empty(), /* is_mut */ true).map_err(graph_err)?;
        Ok(Self {
            graph,
            local_root,
    ...
    fn is_location_borrowed(&self, l: T::Location) -> Result<bool, ExecutionError> {
        ...
        Ok(borrowed_by.iter().any(|(_, paths)| paths.iter().any(|path| path.starts_with(&Location(l)))))
    }
    ...
    fn extend_by_epsilon(&mut self, r: Ref, is_mut: bool) -> Result<Ref, ExecutionError> {
        let new_r = self.graph.extend_by_epsilon((), std::iter::once(r), is_mut).map_err(graph_err)?;
        Ok(new_r)
    }

    fn extend_by_label(&mut self, r: Ref, is_mut: bool, extension: T::Location) -> Result<Ref, ExecutionError> {
        let new_r =
            self.graph.extend_by_label((), std::iter::once(r), is_mut, Location(extension)).map_err(graph_err)?;
        Ok(new_r)
    }
    ...
    fn extend_by_dot_star_for_call(
        ...
        let new_refs =
            self.graph.extend_by_dot_star_for_call((), sources.iter().copied(), mutabilities).map_err(graph_err)?;
        Ok(new_refs)
    }
    ...
    fn is_writable(&self, r: Ref) -> Result<bool, ExecutionError> {
        ...
        Ok(self.borrowed_by(r)?.values().all(|paths| paths.iter().all(|path| path.is_epsilon())))
    }
...
pub fn verify(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    for c in commands {
        ...
        assert_invariant!(result.len() == c.value.result_type.len(), "result length mismatch for command. {c:?}");
        // drop unused result values
        assert_invariant!(result.len() == c.value.drop_values.len(), "drop values length mismatch for command. {c:?}");
    ...
    let Context { gas_coin, objects, pure, receiving, results, .. } = &mut context;
    ...
    assert_invariant!(context.borrowed_by(context.local_root)?.is_empty(), "reference to local root not released");
    context.release(context.local_root)?;
    assert_invariant!(context.graph.abstract_size() == 0, "reference not released");
    assert_invariant!(context.tx_context.is_some(), "tx_context should never be moved");
...
fn command(context: &mut Context, sp!(_, c): &T::Command) -> Result<Vec<Value>, ExecutionError> {
    ...
    Ok(match &c.command {
        T::Command__::MoveCall(mc) => {
            let T::MoveCall { function, arguments: args } = &**mc;
...
fn argument(context: &mut Context, x: &T::Argument) -> Result<Value, ExecutionError> {
    match &x.value.0 {
        ...
        T::Argument__::Use(T::Usage::Copy { location, borrowed }) => copy_value(context, x.idx, *location, borrowed),
        T::Argument__::Borrow(is_mut, location) => borrow_location(context, x.idx, *is_mut, *location),
        T::Argument__::Read(usage) => read_ref(context, x.idx, usage),
        T::Argument__::Freeze(usage) => freeze_ref(context, x.idx, usage),
    }
}

fn move_value(context: &mut Context, arg_idx: u16, l: T::Location) -> Result<Value, ExecutionError> {
    if context.is_location_borrowed(l)? {
        // TODO more specific error
        return Err(command_argument_error(CommandArgumentError::CannotMoveBorrowedValue, arg_idx as usize));
    }
    let Some(value) = context.location(l).take() else {
        return Err(command_argument_error(CommandArgumentError::ArgumentWithoutValue, arg_idx as usize));
    };
    Ok(value)
}
...
fn copy_value(
    ...
    borrowed.set(is_borrowed).map_err(|_| make_invariant_violation!("Copy's borrowed marker should not yet be set"))?;
...
fn borrow_location(context: &mut Context, arg_idx: u16, is_mut: bool, l: T::Location) -> Result<Value, ExecutionError> {
    // check that the location has a value
    let Some(value) = context.location(l) else {
        // TODO more specific error
        return Err(command_argument_error(CommandArgumentError::ArgumentWithoutValue, arg_idx as usize));
    };
    assert_invariant!(value.is_non_ref(), "type checking should guarantee no borrowing of references");
...
fn read_ref(context: &mut Context, arg_idx: u16, u: &T::Usage) -> Result<Value, ExecutionError> {
    ...
    assert_invariant!(value.is_ref(), "type checking should guarantee ReadRef is used on only references");
    consume_value(context, value)?;
    Ok(Value::NonRef)
}
...
fn write_ref(context: &mut Context, arg_idx: usize, value: Value) -> Result<(), ExecutionError> {
    ...
    if !context.is_writable(r)? {
        // TODO more specific error
        return Err(command_argument_error(CommandArgumentError::CannotWriteToExtendedReference, arg_idx));
    }
    consume_value(context, value)?;
    Ok(())
}
...
fn call(
    ...
    let sources = arg_values.iter().filter_map(|v| v.to_ref()).collect::<BTreeSet<_>>();
    if let Some(v) = context.find_non_transferrable(&sources)? {
        ...
        return Err(command_argument_error(CommandArgumentError::InvalidReferenceArgument, idx));
    }
    ...
    assert_invariant!(return_references.is_empty(), "return_references has more references than return_");
