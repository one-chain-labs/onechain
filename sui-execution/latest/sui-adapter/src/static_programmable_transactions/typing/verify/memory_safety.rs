...
impl Context {
    ...
    fn is_location_borrowed(&self, l: T::Location) -> Result<bool, ExecutionError> {
        ...
        Ok(borrowed_by.iter().any(|(_, paths)| paths.iter().any(|path| path.starts_with(&l))))
    }
    ...
    fn extend_by_label(&mut self, r: Ref, is_mut: bool, extension: T::Location) -> Result<Ref, ExecutionError> {
        let new_r = self.graph.extend_by_label((), std::iter::once(r), is_mut, extension).map_err(graph_err)?;
        Ok(new_r)
    }
...
pub fn verify(_env: &Env, ast: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    let Context { gas_coin, inputs, results, .. } = &mut context;
...
fn command(context: &mut Context, sp!(_, command): &T::Command) -> Result<Vec<Value>, ExecutionError> {
    Ok(match command {
...
fn move_value(context: &mut Context, arg_idx: u16, l: T::Location) -> Result<Value, ExecutionError> {
    let Some(value) = context.location(l).take() else {
        return Err(command_argument_error(CommandArgumentError::InvalidValueUsage, arg_idx as usize));
    };
    Ok(value)
}
...
fn write_ref(context: &mut Context, arg_idx: usize, value: Value) -> Result<(), ExecutionError> {
    ...
    if !context.is_writable(r)? {
        // TODO more specific error
        return Err(command_argument_error(CommandArgumentError::InvalidValueUsage, arg_idx));
    }
    consume_value(context, value)?;
    Ok(())
}

fn call(context: &mut Context, arg_values: Vec<Value>, return_: &[Type]) -> Result<Vec<Value>, ExecutionError> {
    let sources = arg_values.iter().filter_map(|v| v.to_ref()).collect::<BTreeSet<_>>();
    if let Some(v) = context.find_non_transferrable(&sources)? {
        let idx = arg_values.iter().position(|x| x.to_ref() == Some(v)).unwrap_or(arg_values.len());
        assert_invariant!(idx < arg_values.len(), "non transferrable value was not found in arguments");
        return Err(command_argument_error(CommandArgumentError::InvalidValueUsage, idx));
    }
