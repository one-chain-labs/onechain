...
impl Context {
    fn new(linputs: L::Inputs) -> Result<Self, ExecutionError> {
        ...
        let cloned_inputs = linputs.iter().map(|(arg, _)| arg.clone()).collect::<Vec<_>>();
        // - intern the bytes
        ...
        for (i, (arg, ty)) in linputs.into_iter().enumerate() {
            ...
            let kind = match (arg, ty) {
                ...
                (L::InputArg::Object(arg), L::InputType::Fixed(ty)) => {
                    let o = T::ObjectInput { original_input_index: idx, arg, ty };
                ...
                (L::InputArg::FundsWithdrawal(withdrawal), L::InputType::Fixed(input_ty)) => {
                    ...
                    let withdrawal = T::WithdrawalInput { original_input_index: idx, ty, owner, amount };
                    context.withdrawals.insert(idx, withdrawal);
                    InputKind::Withdrawal
                }
                (arg, ty) => invariant_violation!("Input arg, type mismatch. Unexpected {arg:?} with type {ty:?}"),
            };
            ...
            for (i, arg) in cloned_inputs.iter().enumerate() {
                if let L::InputArg::Pure(bytes) = &arg {
                    ...
                    if interned_bytes != bytes {
                        assert_invariant!(interned_bytes == bytes, "Interned bytes mismatch for input {i}",);
                    }
                }
            }
        }
    ...
    fn finish(self, commands: T::Commands) -> T::Transaction {
        let Self { bytes, objects, withdrawals, pure, receiving, .. } = self;
        ...
        T::Transaction { bytes, objects, withdrawals, pure, receiving, commands }
    }

    // Get the fixed type of a location. Returns `None` for Pure and Receiving inputs,
    fn fixed_type(&mut self, env: &Env, location: SplatLocation) -> Result<Option<(T::Location, Type)>, ExecutionError> {
        Ok(Some(match location {
            SplatLocation::GasCoin => (T::Location::GasCoin, env.gas_coin_type()?),
            SplatLocation::Result(i, j) => (T::Location::Result(i, j), self.results[i as usize][j as usize].clone()),
            SplatLocation::Input(i) => match &self.input_resolution[i.0 as usize] {
                InputKind::Object => {
                    ...
                    (T::Location::ObjectInput(object_index as u16), object_input.ty.clone())
                }
                InputKind::Withdrawal => {
                    let Some((withdrawal_index, _, withdrawal_input)) = self.withdrawals.get_full(&i) else {
                        invariant_violation!("Unbound withdrawal input {}", i.0)
                    };
                    (T::Location::WithdrawalInput(withdrawal_index as u16), withdrawal_input.ty.clone())
                }
    ...
    fn resolve_location(
        ...
        Ok(match location {
            ...
            SplatLocation::Input(i) => match &self.input_resolution[i.0 as usize] {
                InputKind::Object | InputKind::Withdrawal => self
                    .fixed_type(env, location)?
                    .ok_or_else(|| make_invariant_violation!("Expected fixed type for {location:?}"))?,
                InputKind::Pure => {
...
pub fn transaction<Mode: ExecutionMode>(env: &Env, lt: L::Transaction) -> Result<T::Transaction, ExecutionError> {
    ...
    let commands = commands
        ...
        .map(|(i, c)| {
            ...
            let (c_, tys) = command::<Mode>(env, &mut context, c).map_err(|e| e.with_command_index(i))?;
...
fn command<Mode: ExecutionMode>(
    ...
    Ok(match command {
        L::Command::MoveCall(lmc) => {
            let L::MoveCall { function, arguments: largs } = *lmc;
            ...
            let parameter_tys = match tx_context_kind {
                ...
                TxContextKind::Mutable | TxContextKind::Immutable => {
                    let Some(n_) = function.signature.parameters.len().checked_sub(1) else {
                        invariant_violation!("A function with a TxContext should have at least one parameter")
                    };
            ...
            (T::Command__::MoveCall(Box::new(T::MoveCall { function, arguments: args })), result)
        }
        ...
        L::Command::SplitCoins(lcoin, lamounts) => {
            ...
            let amounts = arguments(env, context, 1, amount_locs, std::iter::repeat_with(|| Type::U64))?;
        ...
        L::Command::MergeCoins(ltarget, lcoins) => {
            ...
            let coins = arguments(env, context, 1, coin_locs, std::iter::repeat_with(|| coin_type.clone()))?;
        ...
        L::Command::MakeMoveVec(Some(ty), lelems) => {
            ...
            let elems = arguments(env, context, 0, elem_locs, std::iter::repeat_with(|| ty.clone()))?;
            (T::Command__::MakeMoveVec(ty.clone(), elems), vec![env.vector_type(ty)?])
        }
        L::Command::MakeMoveVec(None, lelems) => {
            ...
            let mut elems = arguments(env, context, 1, elems_loc, std::iter::repeat_with(|| first_ty.clone()))?;
            elems.insert(0, first_arg);
            (T::Command__::MakeMoveVec(first_ty.clone(), elems), vec![env.vector_type(first_ty)?])
        }
        ...
        L::Command::Upgrade(items, object_ids, object_id, la, linkage) => {
            ...
            (T::Command__::Upgrade(items, object_ids, object_id, a, linkage), vec![res.clone()])
        }
    })
}
...
{
    fn splat_arg(context: &mut Context, res: &mut Vec<SplatLocation>, arg: L::Argument) -> Result<(), EitherError> {
        match arg {
...
fn argument_(
    ...
    let bytes_constraint = BytesConstraint { command: current_command, argument: command_arg_idx as u16 };
...
fn constrained_argument<P: FnMut(&Type) -> Result<bool, ExecutionError>>(
    ...
    constrained_argument_(env, context, command_arg_idx, location, &mut is_valid, err_case)
}
...
fn constrained_type<'a, P: FnMut(&Type) -> Result<bool, ExecutionError>>(
    ...
    Ok(if is_valid(&ty)? { Some((location, ty)) } else { None })
}
...
fn coin_mut_ref_argument(
    ...
    let arg_ = coin_mut_ref_argument_(env, context, location).map_err(|e| e.into_execution_error(command_arg_idx))?;
    Ok(sp(command_arg_idx as u16, arg_))
}
...
fn coin_mut_ref_argument_(
    ...
    Ok(match &actual_ty {
        Type::Reference(is_mut, ty) if *is_mut => {
            check_coin_type(ty)?;
            (T::Argument__::new_copy(location), Type::Reference(*is_mut, ty.clone()))
        }
        ty => {
            check_coin_type(ty)?;
            (T::Argument__::Borrow(/* mut */ true, location), Type::Reference(true, Rc::new(ty.clone())))
        }
    })
}
...
mod scope_references {
    ...
    fn argument(used: &mut BTreeSet<(u16, u16)>, sp!(_, (arg_, ty)): &mut T::Argument) {
        ...
        match (&usage, ty) {
            ...
            (T::Usage::Copy { location: T::Location::Result(i, j), .. }, Type::Reference(_, _)) => {
...
mod consumed_shared_objects {
    ...
    use crate::{
        sp,
        static_programmable_transactions::{loading::ast as L, typing::ast as T},
    };
    ...
    impl Context {
        pub fn new(ast: &T::Transaction) -> Self {
            let T::Transaction { bytes: _, objects, withdrawals: _, pure: _, receiving: _, commands: _ } = ast;
            let inputs = objects
                .iter()
                .map(|o| match &o.arg {
                    L::ObjectArg::SharedObject { id, kind: L::SharedObjectKind::Legacy, .. } => Some(*id),
                    L::ObjectArg::ImmObject(_)
                    | L::ObjectArg::OwnedObject(_)
                    | L::ObjectArg::SharedObject { kind: L::SharedObjectKind::Party, .. } => None,
                })
                .collect::<Vec<_>>();
            Self { inputs, results: vec![] }
        }
    }
    ...
    fn command(context: &mut Context, sp!(_, c): &mut T::Command) -> Result<(), ExecutionError> {
        ...
        let (consumed, result) = match &c.command {
            ...
            T::Command__::MakeMoveVec(_, _) => {
                assert_invariant!(c.result_type.len() == 1, "MakeMoveVec must return a single value");
                (vec![], vec![Some(acc)])
            }
