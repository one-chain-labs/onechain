...
enum LocationType<'context> {
    Bytes(&'context mut InputType, &'context mut BTreeMap<Type, (u16, u16)>),
    Fixed(Type),
}
...
impl Context {
    fn new(linputs: L::Inputs) -> Self {
        let mut context =
            Context { current_command: 0, gathered_input_types: BTreeMap::new(), inputs: vec![], results: vec![] };
    ...
    fn finish(self) -> Vec<(InputArg, T::InputType)> {
        let Self { mut gathered_input_types, inputs, .. } = self;
        inputs
            .into_iter()
            .enumerate()
    ...
    fn location_type<'context>(
        ...
        Ok(match location {
            ...
            T::Location::Input(i) => match &mut self.inputs[i as usize].1 {
                t @ InputType::Bytes => LocationType::Bytes(t, self.gathered_input_types.get_mut(&i).unwrap()),
                InputType::Fixed(t) => LocationType::Fixed(t.clone()),
            },
            T::Location::Result(i, j) => LocationType::Fixed(self.results[i as usize][j as usize].clone()),
        })
    }
}
...
fn command(
    ...
    Ok(match command {
        L::Command::MoveCall(lmc) => {
            ...
            (T::Command_::MoveCall(Box::new(T::MoveCall { function, arguments: args })), result)
        }
        ...
        L::Command::MakeMoveVec(Some(ty), lelems) => {
            ...
            (T::Command_::MakeMoveVec(ty.clone(), elems), vec![env.vector_type(ty)?])
        }
        L::Command::MakeMoveVec(None, lelems) => {
            ...
            let Some(first_ty) = constrained_type(env, context, first_loc, |ty| Ok(ty.abilities().has_key()))? else {
                // TODO need a new error here
                return Err(command_argument_error(CommandArgumentError::TypeMismatch, 0));
            };
            let elems_loc = locations(context, 1, lelems)?;
            let elems = arguments(env, context, 1, elems_loc, std::iter::repeat_with(|| first_ty.clone()))?;
            (T::Command_::MakeMoveVec(first_ty.clone(), elems), vec![env.vector_type(first_ty)?])
        }
        ...
        L::Command::Upgrade(items, object_ids, object_id, la) => {
            ...
            (T::Command_::Upgrade(items, object_ids, object_id, a), vec![res.clone()])
        }
    })
}

fn one_location(context: &mut Context, command_arg_idx: usize, arg: L::Argument) -> Result<T::Location, ExecutionError> {
...
{
    fn splat_arg(context: &mut Context, res: &mut Vec<T::Location>, arg: L::Argument) -> Result<(), EitherError> {
        match arg {
...
fn argument_(
    ...
    Ok(match (actual_ty, expected_ty) {
        ...
        (LocationType::Fixed(Type::Reference(a_is_mut, a)), Type::Reference(b_is_mut, b)) if !b_is_mut || a_is_mut => {
        ...
        (actual_ty, Type::Reference(is_mut, inner)) => {
            check_type_impl(command_and_arg_idx, /* fix */ *is_mut, actual_ty, inner)?;
...
fn check_type(
    ...
    check_type_impl(command_and_arg_idx, /* fix */ false, actual_ty, expected_ty)
}
...
fn check_type_impl(
    ...
    match &mut actual_ty {
        ...
        LocationType::Bytes(ty, types) => {
            types.entry(expected_ty.clone()).or_insert(command_and_arg_idx);
            if fix {
...
fn constrained_argument<P: FnMut(&Type) -> Result<bool, ExecutionError>>(
    ...
    let arg_ =
        constrained_argument_(env, context, location, is_valid).map_err(|e| e.into_execution_error(command_arg_idx))?;
    Ok(sp(command_arg_idx as u16, arg_))
}
...
fn coin_mut_ref_argument_(env: &Env, context: &mut Context, location: T::Location) -> Result<T::Argument_, EitherError> {
    let actual_ty = context.location_type(env, location)?;
