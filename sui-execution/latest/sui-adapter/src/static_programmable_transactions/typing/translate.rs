...
impl Context {
    ...
    fn finish(self, commands: T::Commands) -> T::Transaction {
        let Self { bytes, objects, pure, receiving, .. } = self;
        ...
        T::Transaction { bytes, objects, pure, receiving, commands }
    }
    ...
    Items::IntoIter: ExactSizeIterator,
{
    ...
    for (arg_idx, arg) in args.enumerate() {
        splat_arg(context, &mut res, arg).map_err(|e| e.into_execution_error(start_idx + arg_idx))?;
    }
...
fn arguments(
    ...
    locations
        ...
        .zip(expected_tys)
        .enumerate()
        .map(|(i, (location, expected_ty))| argument(env, context, start_idx + i, location, expected_ty))
        .collect()
}
...
fn constrained_arguments<P: FnMut(&Type) -> Result<bool, ExecutionError>>(
    ...
    locations
        .into_iter()
        .enumerate()
        .map(|(i, location)| constrained_argument_(env, context, start_idx + i, location, is_valid, err_case))
        .collect()
}
...
mod consumed_shared_objects {
    ...
    impl Context {
        pub fn new(ast: &T::Transaction) -> Self {
            let T::Transaction { bytes: _, objects, pure: _, receiving: _, commands: _ } = ast;
            let inputs = objects
                .iter()
                .map(|o| match &o.arg {
