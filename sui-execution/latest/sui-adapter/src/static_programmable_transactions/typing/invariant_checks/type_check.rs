...
use sui_types::{coin::RESOLVED_COIN_STRUCT, error::ExecutionError, funds_accumulator::RESOLVED_WITHDRAWAL_STRUCT};
...
impl<'txn> Context<'txn> {
    fn new(txn: &'txn T::Transaction) -> Self {
        Self {
            ...
            result_types: txn.commands.iter().map(|sp!(_, c)| c.result_type.as_slice()).collect(),
        }
    }
}
...
fn verify_<Mode: ExecutionMode>(env: &Env, txn: &T::Transaction) -> anyhow::Result<()> {
    ...
    let T::Transaction { bytes: _, objects, withdrawals, pure, receiving, commands } = txn;
    for obj in objects {
        object_input(obj)?;
    }
...
fn withdrawal_input(ty: &T::Type) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        ...
        "withdrawal input must be one::funds_accumulator::Withdrawal, got {:?}",
        dt.qualified_ident()
    );
    Ok(())
}
...
fn pure_input<Mode: ExecutionMode>(p: &T::PureInput) -> anyhow::Result<()> {
    if !Mode::allow_arbitrary_values() {
        anyhow::ensure!(input_arguments::is_valid_pure_type(&p.ty)?, "pure type must be valid");
    }
    Ok(())
}

fn receiving_input(r: &T::ReceivingInput) -> anyhow::Result<()> {
    anyhow::ensure!(input_arguments::is_valid_receiving(&r.ty), "receiving type must be valid");
    Ok(())
}

fn command<Mode: ExecutionMode>(env: &Env, context: &Context, sp!(_, c): &T::Command) -> anyhow::Result<()> {
    ...
    match &c.command {
        T::Command__::MoveCall(move_call) => {
            ...
            let L::LoadedFunctionInstantiation { parameters, return_ } = signature;
            anyhow::ensure!(
            ...
            for (actual, expected) in return_.iter().zip(result_tys) {
                anyhow::ensure!(actual == expected, "return type mismatch. Expected {expected:?}, got {actual:?}");
            }
        }
        T::Command__::TransferObjects(objs, recipient) => {
            for obj in objs {
                let ty = &obj.value.1;
                anyhow::ensure!(ty.abilities().has_key(), "transfer object type must have key, got {ty:?}");
                argument(env, context, obj, ty)?;
            }
            argument(env, context, recipient, &T::Type::Address)?;
            anyhow::ensure!(result_tys.is_empty(), "transfer objects should not return any value, got {result_tys:?}");
        }
        T::Command__::SplitCoins(ty_coin, coin, amounts) => {
            ...
            anyhow::ensure!(resolved == RESOLVED_COIN_STRUCT, "split coins should have a coin type, got {resolved:?}");
            argument(env, context, coin, &T::Type::Reference(true, Rc::new(ty_coin.clone())))?;
        ...
        T::Command__::MergeCoins(ty_coin, target, coins) => {
            ...
            let resolved = dt.qualified_ident();
            anyhow::ensure!(resolved == RESOLVED_COIN_STRUCT, "split coins should have a coin type, got {resolved:?}");
            argument(env, context, target, &T::Type::Reference(true, Rc::new(ty_coin.clone())))?;
            for coin in coins {
                argument(env, context, coin, ty_coin)?;
            }
            anyhow::ensure!(result_tys.is_empty(), "merge coins should not return any value, got {result_tys:?}");
        }
        T::Command__::MakeMoveVec(t, args) => {
            for arg in args {
                argument(env, context, arg, t)?;
            }
            anyhow::ensure!(result_tys.len() == 1, "make move vec should return exactly one vector");
            let T::Type::Vector(inner) = &result_tys[0] else {
                anyhow::bail!("make move vec should return a vector type, got {result_tys:?}");
            };
            anyhow::ensure!(t == &inner.element_type, "make move vec should return vector<{t:?}>, got {result_tys:?}");
        }
        T::Command__::Publish(_, _, _) => {
            if Mode::packages_are_predefined() {
                anyhow::ensure!(result_tys.is_empty(), "publish should not return upgrade cap for predefined packages");
            } else {
                anyhow::ensure!(result_tys.len() == 1, "publish should return exactly one upgrade cap");
                let cap = &env.upgrade_cap_type()?;
                anyhow::ensure!(cap == &result_tys[0], "publish should return {cap:?}, got {result_tys:?}",);
            }
        }
        T::Command__::Upgrade(_, _, _, arg, _) => {
            argument(env, context, arg, &env.upgrade_ticket_type()?)?;
            let receipt = &env.upgrade_receipt_type()?;
            anyhow::ensure!(result_tys.len() == 1, "upgrade should return exactly one receipt");
            anyhow::ensure!(receipt == &result_tys[0], "upgrade should return {receipt:?}, got {result_tys:?}");
        }
    }
    assert_invariant!(
...
fn argument(env: &Env, context: &Context, sp!(_, (arg__, ty)): &T::Argument, param: &T::Type) -> anyhow::Result<()> {
    anyhow::ensure!(ty == param, "argument type mismatch. Expected {param:?}, got {ty:?}");
    ...
    anyhow::ensure!(&actual == expected, "argument type mismatch. Expected {expected:?}, got {actual:?}");
    // check copy usage
    match arg__ {
        T::Argument__::Use(T::Usage::Copy { .. }) | T::Argument__::Read(_) => {
            anyhow::ensure!(param.abilities().has_copy(), "expected type does not have copy, {expected:?}");
        }
        T::Argument__::Use(T::Usage::Move(_)) | T::Argument__::Freeze(_) | T::Argument__::Borrow(_, _) => (),
    }
    Ok(())
}
...
fn usage(env: &Env, context: &Context, u: &T::Usage) -> anyhow::Result<T::Type> {
    match u {
        T::Usage::Move(l) | T::Usage::Copy { location: l, borrowed: _ } => location(env, context, *l),
    }
}
...
fn location(env: &Env, context: &Context, l: T::Location) -> anyhow::Result<T::Type> {
    Ok(match l {
        ...
        T::Location::PureInput(i) => {
            context.pure.get(i as usize).copied().ok_or_else(|| anyhow::anyhow!("pure input {i} out of bounds"))?.clone()
        }
