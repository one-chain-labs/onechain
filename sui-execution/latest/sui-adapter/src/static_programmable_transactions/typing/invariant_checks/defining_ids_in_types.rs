...
pub fn verify(env: &env::Env, tt: &T::Transaction) -> Result<(), ExecutionError> {
    ...
    tt.objects.iter().try_for_each(|object_input| check_type(&object_input.ty))?;
    tt.pure.iter().try_for_each(|pure_input| check_type(&pure_input.ty))?;
    tt.receiving.iter().try_for_each(|receiving_input| check_type(&receiving_input.ty))?;
    ...
    tt.commands.iter().try_for_each(|sp!(_, c)| {
        ...
        match &c.command {
            ...
            T::Command__::MoveCall(move_call) => {
                move_call.function.type_arguments.iter().try_for_each(check_type)?;
...
fn ensure_type_defining_id_based(env: &env::Env, ty: &T::Type) -> Result<(), ExecutionError> {
    match ty {
        ...
        T::Type::Datatype(datatype) => {
            ...
            datatype.type_arguments.iter().try_for_each(|arg| ensure_type_defining_id_based(env, arg))
        }
    }
}
