...
impl Path {
    fn initial(location: T::Location) -> Self {
        Self { root: RootLocation::Known(location), extensions: vec![] }
    }
    ...
    fn extend(&self, extension: Delta) -> Self {
        ...
        Self { root: self.root, extensions: new_extensions }
    }
}
...
impl PathSet {
    ...
    fn unknown_root(command: u16) -> Self {
        Self(IndexSet::from([Path { root: RootLocation::Unknown { command }, extensions: vec![] }]))
    }
    ...
    fn extends(&self, other: &Self, ignore_aliases: bool) -> bool {
        self.0.iter().any(|self_path| {
            other.0.iter().any(|other_path| match self_path.compare(other_path) {
    ...
    fn is_disjoint(&self, other: &Self) -> bool {
        self.0.iter().all(|self_path| {
            other.0.iter().all(|other_path| match self_path.compare(other_path) {
                PathComparison::Disjoint => true,
                PathComparison::Prefix | PathComparison::Aliases | PathComparison::Extends => false,
            })
        })
    }
...
impl Value {
    ...
    fn ref_(is_mut: bool, paths: PathSet) -> anyhow::Result<Value> {
        anyhow::ensure!(!paths.is_empty(), "Cannot create a reference with an empty path set");
        Ok(Value::Ref { is_mut, paths: Rc::new(paths) })
    }

    fn copy(&self) -> Value {
        match self {
            Value::NonRef => Value::NonRef,
            Value::Ref { is_mut, paths } => Value::Ref { is_mut: *is_mut, paths: paths.clone() },
        }
    }
    ...
    fn freeze(&mut self) -> anyhow::Result<Value> {
        ...
        match copied {
            ...
            Value::Ref { is_mut, paths } => {
                ...
                Ok(Value::Ref { is_mut: false, paths })
            }
        }
    }
}
...
impl Location {
    fn non_ref(location: T::Location) -> Self {
        Self { self_path: Rc::new(PathSet::initial(location)), value: Some(Value::NonRef) }
    }

    fn copy_value(&self) -> anyhow::Result<Value> {
        let Some(value) = self.value.as_ref() else { anyhow::bail!("Use of invalid memory location") };
        Ok(value.copy())
    }

    fn move_value(&mut self) -> anyhow::Result<Value> {
        let Some(value) = self.value.take() else { anyhow::bail!("Use of invalid memory location") };
        Ok(value)
    }
    ...
    fn borrow(&mut self, is_mut: bool) -> anyhow::Result<Value> {
        let Some(value) = self.value.as_ref() else { anyhow::bail!("Borrow of invalid memory location") };
        match value {
            Value::Ref { .. } => {
                anyhow::bail!("Cannot borrow a reference")
            }
            Value::NonRef => {
                anyhow::ensure!(!self.self_path.is_empty(), "Cannot have an empty location to borrow from");
                // a new reference that borrows from this location
                Ok(Value::Ref { is_mut, paths: self.self_path.clone() })
            }
        }
    }
}
...
impl Context {
    fn new(txn: &T::Transaction) -> Self {
        let T::Transaction { bytes: _, objects, withdrawals, pure, receiving, commands: _ } = txn;
        let tx_context = Location::non_ref(T::Location::TxContext);
        let gas = Location::non_ref(T::Location::GasCoin);
        let object_inputs =
            (0 .. objects.len()).map(|i| Location::non_ref(T::Location::ObjectInput(i as u16))).collect();
        let withdrawal_inputs =
            (0 .. withdrawals.len()).map(|i| Location::non_ref(T::Location::WithdrawalInput(i as u16))).collect();
        let pure_inputs = (0 .. pure.len()).map(|i| Location::non_ref(T::Location::PureInput(i as u16))).collect();
        let receiving_inputs =
            (0 .. receiving.len()).map(|i| Location::non_ref(T::Location::ReceivingInput(i as u16))).collect();
        Self {
            tx_context,
            gas,
            object_inputs,
    ...
    fn location(&self, loc: T::Location) -> anyhow::Result<&Location> {
        Ok(match loc {
            ...
            T::Location::PureInput(i) => {
                self.pure_inputs.get(i as usize).ok_or_else(|| anyhow::anyhow!("Pure input index out of bounds {i}"))?
            }
    ...
    fn argument(&mut self, sp!(_, (arg, _)): &T::Argument) -> anyhow::Result<Value> {
        ...
        match arg {
            T::Argument__::Use(usage) | T::Argument__::Freeze(usage) | T::Argument__::Read(usage) => {
                self.check_usage(usage, location)?
            }
    ...
    fn arguments(&mut self, args: &[T::Argument]) -> anyhow::Result<Vec<Value>> {
        args.iter().map(|arg| self.argument(arg)).collect::<anyhow::Result<Vec<_>>>()
    }
    ...
    fn any_extends(&self, paths: &PathSet, ignore_aliases: bool) -> bool {
        self.all_references().any(|other| other.extends(paths, ignore_aliases))
    }
}
...
fn verify_(txn: &T::Transaction) -> anyhow::Result<()> {
    ...
    let T::Transaction { bytes: _, objects: _, withdrawals: _, pure: _, receiving: _, commands } = txn;
    for c in commands {
...
fn command(context: &mut Context, c: &T::Command) -> anyhow::Result<()> {
    ...
    context.add_result_values(results.into_iter().zip(c.value.drop_values.iter().copied()).map(|(v, drop)| {
        if drop {
            None
        } else {
            Some(v)
...
fn command_(context: &mut Context, sp!(_, c): &T::Command) -> anyhow::Result<Vec<Value>> {
    ...
    let results = match &c.command {
        T::Command__::MoveCall(move_call) => {
            let T::MoveCall { function, arguments } = &**move_call;
...
fn call(
    ...
    anyhow::ensure!(imm_paths.is_disjoint(&mut_paths), "Mutable and immutable borrows cannot overlap");
    let command = context.current_command();
    let mut_paths = if mut_paths.is_empty() { PathSet::unknown_root(command) } else { mut_paths };
    let all_paths = if all_paths.is_empty() { PathSet::unknown_root(command) } else { all_paths };
    return_
        .iter()
        .enumerate()
        .map(|(i, ty)| {
            let delta = Delta { command, result: i as u16 };
            match ty {
                T::Type::Reference(/* is mut */ true, _) => Value::ref_(true, mut_paths.extend(delta)),
                T::Type::Reference(/* is mut */ false, _) => Value::ref_(false, all_paths.extend(delta)),
                _ => Ok(Value::NonRef),
            }
        })
