...
impl Context {
    fn new(txn: &T::Transaction) -> Self {
        let T::Transaction { bytes: _, objects, pure, receiving, commands: _ } = txn;
    ...
    fn all_references(&self) -> impl Iterator<Item = Rc<PathSet>> {
        let Self { tx_context, gas, object_inputs, pure_inputs, receiving_inputs, results, arg_roots: _ } = self;
...
fn verify_(txn: &T::Transaction) -> anyhow::Result<()> {
    ...
    let T::Transaction { bytes: _, objects: _, pure: _, receiving: _, commands } = txn;
    for c in commands {
