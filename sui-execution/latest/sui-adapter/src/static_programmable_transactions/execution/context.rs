...
use crate::{
    ...
    static_programmable_transactions::{
        better_todo,
        env::Env,
        execution::values::{ByteValue, InitialInput, InputObjectMetadata, InputValue, Inputs, Local, Locals, Value},
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    pub fn finish<Mode: ExecutionMode>(mut self) -> Result<ExecutionResults, ExecutionError> {
        ...
        let gas_object = gas.map(|g| g.into_objects()).transpose()?.unwrap_or_default();
        ...
        let Self { env, mut native_extensions, tx_context, gas_charger, new_packages, user_events, .. } = self;
    ...
    pub fn take_user_events(
        ...
        let new_events = events
            ...
            .map(|(ty, tag, value)| {
                let layout =
                    self.env.vm.get_runtime().type_to_type_layout(&ty).map_err(|e| self.env.convert_vm_error(e))?;
                let Some(bytes) = value.simple_serialize(&layout) else {
                    invariant_violation!("Failed to deserialize already serialized Move value");
                };
    ...
    fn location_value<'a>(
        ...
        location: T::Location,
        ty: Type,
    ) -> Result<(&'a mut GasCharger, &'env Env<'pc, 'vm, 'state, 'linkage>, LocationValue<'a>), ExecutionError> {
        let v = match location {
            T::Location::GasCoin => {
                let () = better_todo!("better error here? How do we handle if there is no gas coin?");
    ...
    fn location(&mut self, usage: UsageKind, location: T::Location, ty: Type) -> Result<Value, ExecutionError> {
    ...
    pub fn result(&mut self, result: Vec<CtxValue>) -> Result<(), ExecutionError> {
        self.results.push(Locals::new(result.into_iter().map(|v| v.0))?);
        Ok(())
    }
    ...
    pub fn vm_move_call(
        ...
        match function.tx_context {
            ...
            TxContextKind::Mutable | TxContextKind::Immutable => {
                args.push(CtxValue(Value::tx_context(self.tx_context.borrow().digest())?))
            }
        }
    ...
    pub fn location_updates(
        ...
        args.into_iter().filter_map(|(location, ty)| self.location_update(location, ty).transpose()).collect()
    }
...
fn load_input_arg(
    ...
    Ok(match arg {
        ...
        T::InputArg::Receiving((id, version, _)) => InitialInput::Bytes(ByteValue::Receiving { id, version }),
...
fn load_byte_value(meter: &mut GasCharger, env: &Env, value: &ByteValue, ty: Type) -> Result<Value, ExecutionError> {
