...
use crate::{
    ...
    programmable_transactions as legacy_ptb,
    sp,
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
        ...
        let gas = match gas_charger.gas_coin() {
            Some(gas_coin) => {
                ...
                let (gas_metadata, gas_value) =
                    load_object_arg_impl(gas_charger, env, &mut input_object_map, gas_coin, true, ty)?;
    ...
    pub fn finish<Mode: ExecutionMode>(mut self) -> Result<ExecutionResults, ExecutionError> {
        ...
        for (metadata, value_opt) in object_inputs.into_iter().chain(gas) {
            let InputObjectMetadata { id, is_mutable_input, owner, version, type_ } = metadata;
    ...
    fn fetch_packages(&mut self, dependency_ids: &[ObjectID]) -> Result<Vec<PackageObject>, ExecutionError> {
    ...
    pub fn upgrade(
        ...
        if self.env.protocol_config.check_for_init_during_upgrade() {
            ...
            let current_module_names: BTreeSet<&str> =
                current_package.move_package().serialized_module_map().keys().map(|s| s.as_str()).collect();
    ...
    pub fn check_shared_object_usage(&mut self, consumed_shared_objects: Vec<ObjectID>) -> Result<(), ExecutionError> {
...
fn load_object_arg(
    ...
    let (metadata, value) = load_object_arg_impl(meter, env, input_object_map, id, is_mutable_input, input.ty)?;
...
fn load_object_arg_impl(
    ...
    let object_metadata = InputObjectMetadata { id, is_mutable_input, owner: owner.clone(), version, type_: ty.clone() };
