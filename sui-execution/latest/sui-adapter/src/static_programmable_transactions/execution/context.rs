...
enum ResolvedLocation<'a> {
    Local(Local<'a>),
    Pure { bytes: &'a [u8], metadata: &'a T::PureInput, local: Local<'a> },
    Receiving { metadata: &'a T::ReceivingInput, local: Local<'a> },
}
...
impl Locations {
    ...
    fn resolve(&mut self, location: T::Location) -> Result<ResolvedLocation<'_>, ExecutionError> {
        Ok(match location {
            ...
            T::Location::WithdrawalInput(i) => ResolvedLocation::Local(self.withdrawal_inputs.local(i)?),
            ...
            T::Location::PureInput(i) => {
                ...
                let bytes = self.pure_input_bytes.get_index(metadata.byte_index).ok_or_else(|| {
                ...
                ResolvedLocation::Pure { bytes, metadata, local }
            }
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
        ...
        'pc: 'state,
    {
        ...
        let tx_context_value = Locals::new(vec![Some(Value::new_tx_context(tx_context.borrow().digest())?)])?;
        Ok(Self {
            env,
            metrics,
            native_extensions,
    ...
    pub fn finish<Mode: ExecutionMode>(mut self) -> Result<ExecutionResults, ExecutionError> {
        ...
        let mut object_inputs = std::mem::replace(&mut self.locations.object_inputs, Locals::new_invalid(0)?);
        ...
        let gas =
            gas.map(|(m, mut g)| Result::<_, ExecutionError>::Ok((m, g.local(0)?.move_if_valid()?))).transpose()?;
        ...
        for (metadata, value_opt) in object_inputs.into_iter().chain(gas) {
            let InputObjectMetadata { id, mutability, owner, version, type_ } = metadata;
            match mutability {
            ...
            if let Some(object) = value_opt {
                self.transfer_object_(owner, type_, CtxValue(object), /* end of transaction */ true)?;
        ...
        let Self { env, mut native_extensions, tx_context, gas_charger, user_events, .. } = self;
        ...
        let object_runtime: ObjectRuntime =
            native_extensions.remove().map_err(|e| env.convert_vm_error(e.finish(Location::Undefined)))?;
        ...
        for (id, (recipient, ty, value)) in writes {
            ...
            let move_object = unsafe {
                create_written_object::<Mode>(env, &loaded_runtime_objects, id, ty, has_public_transfer, bytes)?
            };
    ...
    fn location(&mut self, usage: UsageKind, location: T::Location) -> Result<Value, ExecutionError> {
        let resolved = self.locations.resolve(location)?;
        let mut local = match resolved {
            ResolvedLocation::Local(l) => l,
            ResolvedLocation::Pure { bytes, metadata, mut local } => {
                if local.is_invalid()? {
            ...
            ResolvedLocation::Receiving { metadata, mut local } => {
                if local.is_invalid()? {
        ...
        Ok(match usage {
            ...
            UsageKind::Borrow => {
                charge_gas_!(self.gas_charger, self.env, charge_simple_instr(SimpleInstruction::MutBorrowLoc))?;
                local.borrow()?
            }
        })
    }
    ...
    pub fn result(&mut self, result: Vec<Option<CtxValue>>) -> Result<(), ExecutionError> {
        self.locations.results.push(Locals::new(result.into_iter().map(|v| v.map(|v| v.0)))?);
        Ok(())
    }
    ...
    pub fn charge_command(
        ...
        let num_popped = if is_move_call {
            num_args
                .checked_add(num_return)
                .ok_or_else(|| make_invariant_violation!("usize overflow when charging gas for command",))?
        } else {
            num_args
        };
        move_gas_status
    ...
    pub fn new_coin(&mut self, amount: u64) -> Result<CtxValue, ExecutionError> {
        ...
        object_runtime_mut!(self)?.new_id(id).map_err(|e| self.env.convert_vm_error(e.finish(Location::Undefined)))?;
    ...
    pub fn upgrade_receipt(&self, upgrade_ticket: UpgradeTicket, upgraded_package_id: ObjectID) -> CtxValue {
    ...
    fn fetch_package(&mut self, dependency_id: &ObjectID) -> Result<Rc<MovePackage>, ExecutionError> {
        let [fetched_package] = self.fetch_packages(&[*dependency_id])?.try_into().map_err(|_| {
            make_invariant_violation!(
    ...
    fn fetch_packages(&mut self, dependency_ids: &[ObjectID]) -> Result<Vec<Rc<MovePackage>>, ExecutionError> {
        let mut fetched = vec![];
        let mut missing = vec![];
        ...
        for id in &dependency_ids {
            match self.env.linkable_store.get_package(id) {
                Err(e) => {
                    return Err(ExecutionError::new_with_source(ExecutionErrorKind::PublishUpgradeMissingDependency, e));
                }
                Ok(Some(inner)) => {
                    fetched.push(inner);
                }
        ...
        if missing.is_empty() {
            assert_invariant!(fetched.len() == dependency_ids.len(), "all dependencies should be fetched");
            Ok(fetched)
        } else {
            let msg = format!(
                "Missing dependencies: {}",
                missing.into_iter().map(|dep| format!("{}", dep)).collect::<Vec<_>>().join(", ")
            );
    ...
    fn publish_and_verify_modules(
        ...
        for module in modules {
            ...
            sui_verifier::verifier::sui_verify_module_unmetered(
                module,
                &BTreeMap::new(),
                &self.env.protocol_config.verifier_config(/* signing_limits */ None),
            )?;
        }
    ...
    fn init_modules(
        ...
        for module in modules {
            let Some((fdef_idx, fdef)) = module.find_function_def_by_name(INIT_FN_NAME.as_str()) else {
                continue;
            };
            let fhandle = module.function_handle_at(fdef.function);
            let fparameters = module.signature_at(fhandle.parameters);
            assert_invariant!(fparameters.0.len() <= 2, "init function should have at most 2 parameters");
            let has_otw = fparameters.0.len() == 2;
            let tx_context = self
                .location(UsageKind::Borrow, T::Location::TxContext)
                .map_err(|e| make_invariant_violation!("Failed to get tx context for init function: {}", e))?;
    ...
    pub fn upgrade(
        ...
        if self.env.protocol_config.check_for_init_during_upgrade() {
            ...
            let current_module_names: BTreeSet<&str> =
                current_move_package.serialized_module_map().keys().map(|s| s.as_str()).collect();
            let upgrade_module_names: BTreeSet<&str> =
                package.serialized_module_map().keys().map(|s| s.as_str()).collect();
            let new_module_names =
                upgrade_module_names.difference(&current_module_names).copied().collect::<BTreeSet<&str>>();
        ...
        self.env.linkable_store.push_package(storage_id, Rc::new(package))?;
        Ok(storage_id)
    }
    ...
    pub fn transfer_object(&mut self, recipient: Owner, ty: Type, object: CtxValue) -> Result<(), ExecutionError> {
    ...
    fn transfer_object_(
        ...
        let tag = TypeTag::try_from(ty).map_err(|_| make_invariant_violation!("Unable to convert Type to TypeTag"))?;
    ...
    pub fn argument_updates(
        ...
        args.into_iter().filter_map(|arg| self.argument_update(arg).transpose()).collect()
    }
    ...
    fn argument_update(
        ...
        let ty = match ty {
            ...
            ty => {
                debug_assert!(false, "Unexpected non reference type in location update: {ty:?}");
                return Ok(None);
            }
        };
        ...
        let arg = match location {
            ...
            T::Location::ObjectInput(i) => TxArgument::Input(self.locations.input_object_metadata[i as usize].0 .0),
            T::Location::WithdrawalInput(i) => {
                TxArgument::Input(self.locations.input_withdrawal_metadata[i as usize].original_input_index.0)
            }
            T::Location::PureInput(i) => {
                TxArgument::Input(self.locations.pure_input_metadata[i as usize].original_input_index.0)
            }
            T::Location::ReceivingInput(i) => {
                TxArgument::Input(self.locations.receiving_input_metadata[i as usize].original_input_index.0)
            }
        };
    ...
    pub fn tracked_results(
        ...
        assert_invariant!(results.len() == result_tys.len(), "results and result types should match");
        results.iter().zip(result_tys).map(|(v, ty)| self.tracked_result(&v.0, ty.clone())).collect()
    }

    fn tracked_result(&self, result: &Value, ty: Type) -> Result<(Vec<u8>, TypeTag), ExecutionError> {
        let inner_value;
        let (v, ty) = match ty {
...
impl CtxValue {
    pub fn vec_pack(ty: Type, values: Vec<CtxValue>) -> Result<CtxValue, ExecutionError> {
        Ok(CtxValue(Value::vec_pack(ty, values.into_iter().map(|v| v.0).collect())?))
    }
...
fn load_object_arg(
    ...
    let (metadata, value) = load_object_arg_impl(meter, env, input_object_map, id, mutability, input.ty)?;
...
fn load_object_arg_impl(
    ...
    let object_metadata = InputObjectMetadata { id, mutability, owner: owner.clone(), version, type_: ty.clone() };
    let sui_types::object::ObjectInner { data: sui_types::object::Data::Move(move_obj), .. } = obj.as_inner() else {
    ...
    let contained_uids = {
        ...
        get_all_uids(&fully_annotated_layout, move_obj.contents())
            .map_err(|e| make_invariant_violation!("Unable to retrieve UIDs for object. Got error: {e}"))?
    };
    input_object_map.insert(id, object_runtime::InputObject { contained_uids, version, owner });
...
fn load_withdrawal_arg(
    ...
    let T::WithdrawalInput { original_input_index: _, ty: _, owner, amount } = withdrawal;
...
fn assert_expected_move_object_type(actual: &Type, expected: &MoveObjectType) -> Result<(), ExecutionError> {
...
fn assert_expected_type(actual: &Type, expected: &TypeTag) -> Result<(), ExecutionError> {
    match (actual, expected) {
        ...
        (Type::Datatype(actual_dt), TypeTag::Struct(expected_st)) => assert_expected_data_type(actual_dt, expected_st),
        _ => invariant_violation!("Type mismatch between actual: {actual:?} and expected: {expected:?}"),
    }
}
/// Assert the type inferred matches the expected type. This has already been done during typing,
/// but is checked again as an invariant. This may be removed safely at a later time if needed.
fn assert_expected_data_type(actual: &Datatype, expected: &StructTag) -> Result<(), ExecutionError> {
    let (a, m, n) = actual.qualified_ident();
    assert_invariant!(
        a == &expected.address,
        "Actual address does not match expected. actual: {actual:?} vs expected: {expected:?}"
