...
impl Local<'_> {
    ...
    pub fn store(&mut self, value: Value) -> Result<(), ExecutionError> {
        self.0 .0.store_loc(self.1 as usize, value.0, /* violation check */ true).map_err(iv("store loc"))
    }
...
impl Value {
    ...
    pub fn receiving(id: ObjectID, version: SequenceNumber) -> Self {
        Self(VMValue::struct_(Struct::pack([Self::id(id.into()).0, VMValue::u64(version.into())])))
    }
    ...
    /// Constructs a `one::funds_accumulator::Withdrawal` value
    pub fn funds_accumulator_withdrawal(owner: AccountAddress, limit: U256) -> Self {
        // public struct Withdrawal has drop {
        //     owner: address,
        //     limit: u256,
        // }
        Self(VMValue::struct_(Struct::pack([VMValue::address(owner), VMValue::u256(limit)])))
    }
...
impl Value {
    pub fn upgrade_cap(cap: UpgradeCap) -> Self {
        ...
        //     policy: u8,
        // }
        let UpgradeCap { id, package, version, policy } = cap;
    ...
    pub fn upgrade_receipt(receipt: UpgradeReceipt) -> Self {
        ...
        Self(VMValue::struct_(Struct::pack([Self::id(cap.bytes.into()).0, Self::id(package.bytes.into()).0])))
    }
