...
use move_vm_types::{
    values::{self, Locals as VMLocals, Struct, VMValueCast, Value as VMValue, VectorSpecialization},
    views::ValueView,
};
use sui_types::{
...
impl Inputs {
        ...
        for (i, value) in values.enumerate() {
            match value {
                ...
                InitialInput::Object(object_metadata, value) => {
                    ...
                    locals.store_loc(i, value.0, /* violation check */ true).map_err(iv("store loc"))?;
                }
            }
        }
        Ok(Self { metadata, byte_values, locals: Locals(locals) })
    }
    ...
    pub fn fix(&mut self, index: u16, value: Value) -> Result<(), ExecutionError> {
        ...
        assert_invariant!(byte_value.is_some(), "Cannot fix a value that is not a byte value");
        self.locals.0.store_loc(index as usize, value.0, /* violation check */ true).map_err(iv("store loc"))?;
        Ok(())
    }
    ...
    pub fn into_objects(self) -> Result<Vec<(InputObjectMetadata, Option<Value>)>, ExecutionError> {
        let Self { metadata, byte_values: _, mut locals } = self;
...
impl Local<'_> {
    ...
    pub fn is_invalid(&self) -> Result<bool, ExecutionError> {
        self.0 .0.is_invalid(self.1 as usize).map_err(iv("out of bounds"))
    }
    ...
    pub fn move_(&mut self) -> Result<Value, ExecutionError> {
        ...
        Ok(Value(self.0 .0.move_loc(self.1 as usize, /* violation check */ true).map_err(iv("move loc"))?))
    }
    ...
    pub fn copy(&self) -> Result<Value, ExecutionError> {
        ...
        Ok(Value(self.0 .0.copy_loc(self.1 as usize).map_err(iv("copy loc"))?))
    }
    ...
    pub fn borrow(&self) -> Result<Value, ExecutionError> {
        ...
        Ok(Value(self.0 .0.borrow_loc(self.1 as usize).map_err(iv("borrow loc"))?))
    }
}
...
impl Value {
    pub fn uid(address: AccountAddress) -> Self {
        // UID { ID { address } }
        Self(VMValue::struct_(Struct::pack([VMValue::struct_(Struct::pack([VMValue::address(address)]))])))
    }

    pub fn receiving(id: ObjectID, version: SequenceNumber) -> Self {
        Self(VMValue::struct_(Struct::pack([Self::uid(id.into()).0, VMValue::u64(version.into())])))
    }
    ...
    pub fn coin(id: ObjectID, amount: u64) -> Self {
        Self(VMValue::struct_(Struct::pack([Self::uid(id.into()).0, Self::balance(amount).0])))
    }

    pub fn vec_pack(ty: Type, values: Vec<Self>) -> Result<Self, ExecutionError> {
        let specialization: VectorSpecialization =
            ty.try_into().map_err(|e| make_invariant_violation!("Unable to specialize vector: {e}"))?;
        let vec = values::Vector::pack(specialization, values.into_iter().map(|v| v.0)).map_err(iv("pack"))?;
        Ok(Self(vec))
    }
    ...
    pub fn tx_context(digest: TransactionDigest) -> Result<Self, ExecutionError> {
        ...
        Ok(Self(VMValue::struct_(Struct::pack([
            ...
            Self::vec_pack(Type::U8, digest.inner().iter().copied().map(|v| Value(VMValue::u8(v))).collect())?.0,
            VMValue::u64(0),
            VMValue::u64(0),
            VMValue::u64(0),
        ]))))
...
fn coin_ref_modify_balance(
    ...
    let reference: values::Reference = balance_value_ref.copy_value().map_err(iv("copy"))?.cast().map_err(iv("cast"))?;
    let balance: u64 = reference.read_ref().map_err(iv("read ref"))?.cast().map_err(iv("cast"))?;
    let new_balance = modify(balance)?;
    let reference: values::Reference = balance_value_ref.cast().map_err(iv("cast"))?;
    reference.write_ref(VMValue::u64(new_balance)).map_err(iv("write ref"))
}
