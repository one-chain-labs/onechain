...
where
    'pc: 'state,
{
    let T::Transaction { bytes, objects, pure, receiving, commands } = ast;
    let mut context = Context::new(env, metrics, tx_context, gas_charger, bytes, objects, pure, receiving)?;
    let mut mode_results = Mode::empty_results();
    for sp!(idx, c) in commands {
        let start = Instant::now();
        if let Err(err) = execute_command::<Mode>(&mut context, &mut mode_results, c, trace_builder_opt.as_mut()) {
...
fn execute_command<Mode: ExecutionMode>(
    ...
    let T::Command_ { command, result_type, drop_values, consumed_shared_objects } = c;
    ...
    let result = match command {
        ...
        T::Command__::SplitCoins(_, coin, amounts) => {
            ...
            amount_values.into_iter().map(|a| context.new_coin(a)).collect::<Result<_, _>>()?
        }
        T::Command__::MergeCoins(_, target, coins) => {
            ...
            let amounts = coins.into_iter().map(|coin| context.destroy_coin(coin)).collect::<Result<Vec<_>, _>>()?;
