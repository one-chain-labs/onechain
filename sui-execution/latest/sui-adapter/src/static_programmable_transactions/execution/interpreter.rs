...
{
    let mut timings = vec![];
    let result = execute_inner::<Mode>(&mut timings, env, metrics, tx_context, gas_charger, ast, trace_builder_opt);
    ...
    'pc: 'state,
{
    ...
    let T::Transaction { bytes, objects, withdrawals, pure, receiving, commands } = ast;
    let mut context = Context::new(env, metrics, tx_context, gas_charger, bytes, objects, withdrawals, pure, receiving)?;
    ...
    for sp!(idx, c) in commands {
        ...
        if let Err(err) = execute_command::<Mode>(&mut context, &mut mode_results, c, trace_builder_opt) {
            ...
            env.state_view.save_loaded_runtime_objects(loaded_runtime_objects);
    ...
    env.state_view.save_loaded_runtime_objects(loaded_runtime_objects);
    env.state_view.save_wrapped_object_containers(wrapped_object_containers);
    env.state_view.record_execution_results(finished?)?;
    env.state_view.record_generated_object_ids(generated_object_ids);
    Ok(mode_results)
}
...
fn execute_command<Mode: ExecutionMode>(
    ...
    let T::Command_ { command, result_type, drop_values, consumed_shared_objects: _ } = c;
    assert_invariant!(
    ...
    let result = match command {
        ...
        T::Command__::TransferObjects(objects, recipient) => {
            let object_tys = objects.iter().map(|sp!(_, (_, ty))| ty.clone()).collect::<Vec<_>>();
            let object_values: Vec<CtxValue> = context.arguments(objects)?;
            let recipient: AccountAddress = context.argument(recipient)?;
            assert_invariant!(object_values.len() == object_tys.len(), "object values and types mismatch");
        ...
        T::Command__::SplitCoins(ty, coin, amounts) => {
            ...
            let amounts = amount_values.into_iter().map(|a| context.new_coin(a)).collect::<Result<Vec<_>, _>>()?;
            trace_utils::trace_split_coins(context, trace_builder_opt, &ty, trace_values, &amounts, total)?;
        ...
        T::Command__::MergeCoins(ty, target, coins) => {
            ...
            trace_utils::trace_merge_coins(context, trace_builder_opt, &ty, trace_values, additional)?;
            vec![]
        }
        ...
        T::Command__::Publish(module_bytes, dep_ids, linkage) => {
            ...
            let modules = context.deserialize_modules(&module_bytes, /* is upgrade */ false)?;

            let runtime_id = context.publish_and_init_package::<Mode>(modules, &dep_ids, linkage, trace_builder_opt)?;
        ...
        T::Command__::Upgrade(module_bytes, dep_ids, current_package_id, upgrade_ticket, linkage) => {
            trace_utils::trace_upgrade_event(trace_builder_opt)?;
            let upgrade_ticket = context.argument::<CtxValue>(upgrade_ticket)?.into_upgrade_ticket()?;
            ...
            let computed_digest =
                MovePackage::compute_digest_for_modules_and_deps(&module_bytes, &dep_ids, /* hash_modules */ true)
                    .to_vec();
            ...
            let upgraded_package_id =
                context.upgrade(modules, &dep_ids, current_package_id, upgrade_ticket.policy, linkage)?;
    ...
    assert_invariant!(result.len() == drop_values.len(), "result values and drop values mismatch");
