...
use sui_types::{
    ...
    ptb_trace::{ExtMoveValue, ExtMoveValueInfo, ExternalEvent, PTBCommandInfo, PTBEvent, SummaryEvent},
};
...
pub fn trace_move_call_start(trace_builder_opt: &mut Option<MoveTraceBuilder>) {
    if let Some(trace_builder) = trace_builder_opt {
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::MoveCallStart))));
    }
}
...
pub fn trace_move_call_end(trace_builder_opt: &mut Option<MoveTraceBuilder>) {
    if let Some(trace_builder) = trace_builder_opt {
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::MoveCallEnd))));
    }
}
...
pub fn trace_transfer(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        for (idx, (v, ty)) in values.iter().zip(tys).enumerate() {
            ...
            to_transfer
                .push(ExtMoveValue::Single { name: format!("obj{idx}"), info: ExtMoveValueInfo { type_: tag, value } });
        }
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
        ...
        let events = commands
            ...
            .map(|c| match &c.value.command {
                Command__::MoveCall(move_call) => {
                    let pkg = move_call.function.storage_id.address().to_canonical_string(/*with_prefix*/ true);
                    let module = move_call.function.storage_id.name().to_string();
                    let function = move_call.function.name.to_string();
                    Ok(vec![PTBCommandInfo::MoveCall { pkg, module, function }])
                }
                Command__::TransferObjects(..) => Ok(vec![PTBCommandInfo::ExternalEvent("TransferObjects".to_string())]),
                Command__::SplitCoins(..) => Ok(vec![PTBCommandInfo::ExternalEvent("SplitCoins".to_string())]),
                Command__::MergeCoins(..) => Ok(vec![PTBCommandInfo::ExternalEvent("MergeCoins".to_string())]),
                ...
                Command__::MakeMoveVec(..) => Ok(vec![PTBCommandInfo::ExternalEvent("MakeMoveVec".to_string())]),
                Command__::Upgrade(..) => Ok(vec![PTBCommandInfo::ExternalEvent("Upgrade".to_string())]),
            })
            .collect::<Result<Vec<Vec<PTBCommandInfo>>, ExecutionError>>()?
            .into_iter()
            .flatten()
            .collect();
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::Summary(SummaryEvent {
...
pub fn trace_split_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        update_coin_balance(&mut input, |current_balance| current_balance.saturating_sub(total_split_value))?;

        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
                description: "SplitCoins: input => result".to_string(),
                name: "SplitCoins".to_string(),
                values: vec![ExtMoveValue::Single { name: "input".to_string(), info: input }, ExtMoveValue::Vector {
...
pub fn trace_merge_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        let mut values =
            vec![ExtMoveValue::Single { name: "merge_target".to_string(), info: target_coin_starting_state }];
        for (idx, input_value) in trace_values.into_iter().enumerate() {
            values.push(ExtMoveValue::Single { name: format!("coin{idx}"), info: input_value });
        }
        values.push(ExtMoveValue::Single { name: "merge_result".to_string(), info: target_coin_resulting_state });
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
...
/// its body is (and must be) enclosed in an if statement checking if tracing is enabled.
pub fn trace_publish_event(trace_builder_opt: &mut Option<MoveTraceBuilder>) -> Result<(), ExecutionError> {
    if let Some(trace_builder) = trace_builder_opt {
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent { description: "Publish: ()".to_string(), name: "Publish".to_string(), values: vec![] }
        )))));
    }
    Ok(())
}
...
/// its body is (and must be) enclosed in an if statement checking if tracing is enabled.
pub fn trace_upgrade_event(trace_builder_opt: &mut Option<MoveTraceBuilder>) -> Result<(), ExecutionError> {
    if let Some(trace_builder) = trace_builder_opt {
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent { description: "Upgrade: ()".to_string(), name: "Upgrade".to_string(), values: vec![] }
        )))));
    }
    Ok(())
}
...
fn annotated_type_layout_for_adapter_ty(
    ...
    let ty = context.env.load_vm_type_argument_from_adapter_type(0, type_)?;
    context
        .env
        .vm
        .get_runtime()
        .type_to_fully_annotated_layout(&ty)
        .map_err(|e| make_invariant_violation!("Failed to get annotated type layout for adapter type: {}", e))
}
...
fn serializable_move_value_from_ctx_value(
    ...
    VMValue::as_annotated_move_value_for_tracing_only(value.inner_for_tracing().inner_for_tracing(), annotated_layout)
        .ok_or_else(|| make_invariant_violation!("Failed to convert Move value to `SerializableMoveValue` for tracing"))
        .map(SerializableMoveValue::from)
}

fn update_coin_balance(coin: &mut ExtMoveValueInfo, balance_update: impl Fn(u64) -> u64) -> Result<(), ExecutionError> {
    ...
    Ok(match type_ {
        Type::Reference(mutable, inner_ty) => {
            ...
            let inner_type: TypeTag = Type::try_into((**inner_ty).clone()).map_err(|e| {
                make_invariant_violation!("Failed to convert adapter type to type tag for tracing: {}", e)
            })?;
            TypeTagWithRefs { type_: inner_type, ref_type: Some(ref_type) }
        }
        ty => {
            ...
            TypeTagWithRefs { type_, ref_type: None }
        }
    })
}
