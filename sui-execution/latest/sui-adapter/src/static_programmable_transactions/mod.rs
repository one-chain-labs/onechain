...
pub fn execute<Mode: ExecutionMode>(
    ...
    let linkage_analysis =
        linkage::analysis::linkage_analysis_for_protocol_config::<Mode>(protocol_config, &txn, &package_store)
            .map_err(|e| (e, vec![]))?;

    let mut env = Env::new(protocol_config, vm, state_view, &package_store, linkage_analysis.as_ref());
