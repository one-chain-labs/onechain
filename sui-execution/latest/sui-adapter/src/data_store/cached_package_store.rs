...
impl CachedTypeOriginMap {
    pub fn new() -> Self {
        Self { cached_type_origins: BTreeSet::new(), type_origin_map: TypeOriginMap::new() }
    }
}
...
impl<'state> CachedPackageStore<'state> {
    ...
    pub fn push_package(&self, id: ObjectID, package: Rc<MovePackage>) -> Result<(), ExecutionError> {
        ...
        if self.new_packages.borrow_mut().insert(id, package).is_some() {
            invariant_violation!("Package with ID {} already exists in the new packages. This should never happen.", id);
        }
    ...
    pub fn pop_package(&self, id: ObjectID) -> Result<Rc<MovePackage>, ExecutionError> {
        if self.new_packages.borrow().last().is_none_or(|(pkg_id, _)| *pkg_id != id) {
        ...
        let Some((pkg_id, pkg)) = self.new_packages.borrow_mut().pop() else {
            unreachable!("We just checked that new packages is not empty, so this should never happen.");
        };
        assert_eq!(
            pkg_id, id,
    ...
    pub fn to_new_packages(&self) -> Vec<MovePackage> {
        self.new_packages.borrow().iter().map(|(_, pkg)| pkg.as_ref().clone()).collect()
    }
    ...
    pub fn get_package(&self, object_id: &ObjectID) -> SuiResult<Option<Rc<MovePackage>>> {
        ...
        if !self.type_origin_cache.borrow().cached_type_origins.contains(&package_id) {
            let cached_type_origin_map = &mut self.type_origin_cache.borrow_mut();
            cached_type_origin_map.cached_type_origins.insert(package_id);
            let original_package_id = pkg.original_package_id();
            let package_types = cached_type_origin_map.type_origin_map.entry(original_package_id).or_default();
            for ((module_name, type_name), defining_id) in pkg.type_origin_map().into_iter() {
                if let Some(other) = package_types.insert((module_name.to_string(), type_name.to_string()), defining_id)
                {
                    assert_eq!(other, defining_id, "type origin map should never have conflicting entries");
                }
            }
        }
        Ok(Some(pkg))
    ...
    fn fetch_package(&self, id: &ObjectID) -> SuiResult<Option<Rc<MovePackage>>> {
        ...
        let pkg = self.package_store.get_package_object(id)?.map(|pkg_obj| Rc::new(pkg_obj.move_package().clone()));
...
impl PackageStore for CachedPackageStore<'_> {
    ...
    fn resolve_type_to_defining_id(
        ...
        Ok(self
            ...
            .and_then(|module_map| module_map.get(&(module_name.to_string(), type_name.to_string())).copied()))
    }
}
