...
impl<'state> LinkageView<'state> {
    ...
    pub fn has_linkage(&self, context: ObjectID) -> Result<bool, ExecutionError> {
        ...
        Ok(linkage_info.as_ref().is_some_and(|l| l.storage_id == *context))
    }
    ...
    pub(crate) fn link_context(&self) -> Result<AccountAddress, ExecutionError> {
        ...
        Ok(linkage_info.as_ref().map_or(AccountAddress::ZERO, |l| l.storage_id))
    }
    ...
    pub(crate) fn defining_module(&self, runtime_id: &ModuleId, struct_: &IdentStr) -> Result<ModuleId, SuiError> {
        let Ok(linkage_info) = self.linkage_info.try_borrow() else {
            invariant_violation!("Unable to borrow linkage info")
        };
        ...
        for TypeOrigin { module_name, datatype_name: struct_name, package } in package.type_origin_table() {
...
impl ModuleResolver for LinkageView<'_> {
    ...
    fn get_module(&self, id: &ModuleId) -> Result<Option<Vec<u8>>, Self::Error> {
        Ok(self
            ...
            .and_then(|package| package.serialized_module_map().get(id.name().as_str()).cloned()))
    }
}
...
impl PackageStore for LinkageView<'_> {
    ...
    fn resolve_type_to_defining_id(
        ...
        unimplemented!("resolve_type_to_defining_id is not implemented for LinkageView and should never be called")
    }
}
