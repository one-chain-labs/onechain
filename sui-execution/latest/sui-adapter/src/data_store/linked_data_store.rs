...
use crate::{data_store::PackageStore, static_programmable_transactions::linkage::resolved_linkage::RootedLinkage};
        ...
        self.linkage
            ...
            .ok_or_else(|| {
                PartialVMError::new(StatusCode::LINKER_ERROR)
                    .with_message(format!("Error relocating {module_id} -- could not find linkage"))
            })
    }

    fn defining_module(&self, module_id: &ModuleId, struct_: &IdentStr) -> PartialVMResult<ModuleId> {
        self.store
            .resolve_type_to_defining_id(ObjectID::from(*module_id.address()), module_id.name(), struct_)
            .ok()
            .flatten()
            .map(|obj_id| ModuleId::new(*obj_id, module_id.name().to_owned()))
            .ok_or_else(|| {
...
impl LinkageResolver for LinkedDataStore<'_> {
    ...
    fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, Self::Error> {
        DataStore::relocate(self, module_id)
            .map_err(|err| make_invariant_violation!("Error relocating {}: {:?}", module_id, err).into())
    }
    ...
    fn defining_module(&self, runtime_id: &ModuleId, struct_: &IdentStr) -> Result<ModuleId, Self::Error> {
        DataStore::defining_module(self, runtime_id, struct_).map_err(|err| {
            make_invariant_violation!("Error finding defining module for {}::{}: {:?}", runtime_id, struct_, err).into()
        })
    }
}
