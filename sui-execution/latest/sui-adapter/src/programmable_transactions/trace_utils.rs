        ...
        let events = commands
            ...
            .map(|c| match c {
                Command::MoveCall(move_call) => {
                    ...
                    PTBCommandInfo::MoveCall { pkg, module, function }
                }
                Command::TransferObjects(..) => PTBCommandInfo::ExternalEvent("TransferObjects".to_string()),
                ...
                Command::MakeMoveVec(..) => PTBCommandInfo::ExternalEvent("MakeMoveVec".to_string()),
...
pub fn trace_split_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        for coin_val in split_coin_values {
            ...
            split_coin_move_values
                .push(coin_obj_info(type_tag_with_refs.clone(), *coin.id.object_id(), coin.balance.value())?.value);
        }

        let input = coin_obj_info(type_tag_with_refs.clone(), *input_coin.id.object_id(), input_coin.value())?;
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
...
fn obj_info_from_obj_value(
    ...
    match &obj_val.contents {
        ObjectContents::Coin(coin) => coin_obj_info(type_tag_with_refs, *coin.id.object_id(), coin.value()),
