...
pub fn trace_transfer(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        for (idx, v) in obj_values.iter().enumerate() {
            ...
            to_transfer.push(ExtMoveValue::Single { name: format!("obj{idx}"), info: obj_info });
        }
    ...
    if let Some(trace_builder) = trace_builder_opt {
        let events = commands
            ...
            .map(|c| match c {
                ...
                Command::TransferObjects(..) => Ok(vec![PTBCommandInfo::ExternalEvent("TransferObjects".to_string())]),
                Command::SplitCoins(..) => Ok(vec![PTBCommandInfo::ExternalEvent("SplitCoins".to_string())]),
                Command::MergeCoins(..) => Ok(vec![PTBCommandInfo::ExternalEvent("MergeCoins".to_string())]),
                ...
                Command::MakeMoveVec(..) => Ok(vec![PTBCommandInfo::ExternalEvent("MakeMoveVec".to_string())]),
                Command::Upgrade(..) => Ok(vec![PTBCommandInfo::ExternalEvent("Upgrade".to_string())]),
            })
...
pub fn trace_split_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        for coin_val in split_coin_values {
            let Value::Object(ObjectValue { contents: ObjectContents::Coin(coin), .. }) = coin_val else {
                invariant_violation!("Expected result of split coins PTB command to be a coin");
            };
            split_coin_move_values.push(
                coin_move_value_info(type_tag_with_refs.clone(), *coin.id.object_id(), coin.balance.value())?.value,
            );
        }

        let input = coin_move_value_info(type_tag_with_refs.clone(), *input_coin.id.object_id(), input_coin.value())?;
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
...
pub fn trace_merge_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        for (balance, id) in input_infos {
            input_coin_move_values.push(coin_move_value_info(type_tag_with_refs.clone(), *id, *balance)?);
            to_merge += balance;
        }
        ...
        let mut values = vec![ExtMoveValue::Single { name: "merge_target".to_string(), info: merge_target }];
        ...
        let merge_result =
            coin_move_value_info(type_tag_with_refs.clone(), *target_coin.id.object_id(), target_coin.value())?;
        values.push(ExtMoveValue::Single { name: "merge_result".to_string(), info: merge_result });
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
        ...
        trace_builder.push_event(TraceEvent::External(Box::new(serde_json::json!(PTBEvent::ExternalEvent(
            ExternalEvent {
                ...
                values: vec![ExtMoveValue::Vector {
                    ...
                    value: move_values.into_iter().map(|move_value| move_value.value).collect(),
                }],
            }
        )))));
    }
...
fn move_value_info_from_raw_bytes(
    ...
    Ok(ExtMoveValueInfo { type_: type_tag_with_refs, value: serialized_move_value })
}
...
fn move_value_info_from_obj_value(
    ...
    match &obj_val.contents {
        ObjectContents::Coin(coin) => coin_move_value_info(type_tag_with_refs, *coin.id.object_id(), coin.value()),
        ObjectContents::Raw(bytes) => move_value_info_from_raw_bytes(context, &obj_val.type_, bytes),
    }
}
...
fn coin_move_value_info(
    ...
    Ok(ExtMoveValueInfo { type_: type_tag_with_refs, value: SerializableMoveValue::Struct(coin_obj) })
}
