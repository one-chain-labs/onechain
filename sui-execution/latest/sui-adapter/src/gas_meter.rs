...
impl GasMeter for SuiGasMeter<'_> {
    ...
    fn charge_simple_instr(&mut self, instr: SimpleInstruction) -> PartialVMResult<()> {
        ...
        self.0.charge(1, pushes, pops, push_size.into(), pop_size.into())
    }

    fn charge_pop(&mut self, popped_val: impl ValueView) -> PartialVMResult<()> {
        self.0.charge(1, 0, 1, 0, abstract_memory_size(self.0, popped_val).into())
    }
    ...
    fn charge_native_function(
        ...
        let size_increase = ret_vals
            .map(|ret_vals| {
                ret_vals.fold(AbstractMemorySize::zero(), |acc, elem| acc + abstract_memory_size(self.0, elem))
            })
        ...
        if native_function_threshold_exceeded(self.0.gas_model_version, self.0.num_native_calls) {
            ...
            self.0.charge(amount.into(), pushes, 0, size_increase.into(), 0)
        } else {
            // Charge for the stack operations. We don't count this as an "instruction" since we
            // already accounted for the `Call` instruction in the
            // `charge_native_function_before_execution` call.
    ...
    fn charge_native_function_before_execution(
        ...
        let stack_reduction_size =
            args.fold(AbstractMemorySize::new(pops), |acc, elem| acc + abstract_memory_size(self.0, elem));
    ...
    fn charge_call(
        ...
        let stack_reduction_size =
            args.fold(AbstractMemorySize::new(0), |acc, elem| acc + abstract_memory_size(self.0, elem));
        self.0.charge(1, 0, pops, 0, stack_reduction_size.into())
    }
    ...
    fn charge_copy_loc(&mut self, val: impl ValueView) -> PartialVMResult<()> {
        ...
        self.0.charge(1, 1, 0, abstract_memory_size(self.0, val).into(), 0)
    }
    ...
    fn charge_store_loc(&mut self, val: impl ValueView) -> PartialVMResult<()> {
        ...
        // terms of memory.
        self.0.charge(1, 0, 1, 0, abstract_memory_size(self.0, val).into())
    }
    ...
    fn charge_read_ref(&mut self, ref_val: impl ValueView) -> PartialVMResult<()> {
        ...
        // reference.
        self.0.charge(1, 1, 1, abstract_memory_size(self.0, ref_val).into(), REFERENCE_SIZE.into())
    }
    ...
    fn charge_eq(&mut self, lhs: impl ValueView, rhs: impl ValueView) -> PartialVMResult<()> {
        let size_reduction =
            abstract_memory_size_with_traversal(self.0, lhs) + abstract_memory_size_with_traversal(self.0, rhs);
        self.0.charge(1, 1, 2, (Type::Bool.size() + size_reduction).into(), size_reduction.into())
    }
    ...
    fn charge_neq(&mut self, lhs: impl ValueView, rhs: impl ValueView) -> PartialVMResult<()> {
        ...
        self.0.charge(1, 1, 2, size_increase.into(), size_reduction.into())
    }
    ...
    fn charge_vec_len(&mut self, _ty: impl TypeView) -> PartialVMResult<()> {
        self.0.charge(1, 1, 1, Type::U64.size().into(), REFERENCE_SIZE.into())
    }

    fn charge_vec_borrow(&mut self, _is_mut: bool, _ty: impl TypeView, _is_success: bool) -> PartialVMResult<()> {
        self.0.charge(1, 1, 2, REFERENCE_SIZE.into(), (REFERENCE_SIZE + Type::U64.size()).into())
    }
...
fn abstract_memory_size_with_traversal(status: &GasStatus, val: impl ValueView) -> AbstractMemorySize {
...
fn size_config_for_gas_model_version(gas_model_version: u64, should_traverse_refs: bool) -> SizeConfig {
    if use_legacy_abstract_size(gas_model_version) {
        SizeConfig { traverse_references: false, include_vector_size: false }
    } else if should_traverse_refs {
        SizeConfig { traverse_references: enable_traverse_refs(gas_model_version), include_vector_size: true }
    } else {
        SizeConfig { traverse_references: false, include_vector_size: true }
    }
}
