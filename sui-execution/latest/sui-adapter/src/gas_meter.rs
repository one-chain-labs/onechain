...
impl GasMeter for SuiGasMeter<'_> {
    ...
    fn charge_write_ref(&mut self, new_val: impl ValueView, old_val: impl ValueView) -> PartialVMResult<()> {
        ...
        // stack size.
        let (pushes, pops) = if reduce_stack_size(self.0.gas_model_version) { (0, 2) } else { (1, 2) };
        self.0.charge(
            1,
            pushes,
            pops,
    ...
    fn charge_vec_swap(&mut self, _ty: impl TypeView) -> PartialVMResult<()> {
        ...
        let (pushes, pops) = if reduce_stack_size(self.0.gas_model_version) { (0, 3) } else { (1, 1) };
...
fn size_config_for_gas_model_version(gas_model_version: u64, should_traverse_refs: bool) -> SizeConfig {
    if use_legacy_abstract_size(gas_model_version) {
        SizeConfig { traverse_references: false, include_vector_size: false, fine_grained_value_size: false }
