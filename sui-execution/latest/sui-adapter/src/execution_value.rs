...
pub enum InputObjectMetadata {
    ...
    InputObject { id: ObjectID, mutability: Mutability, owner: Owner, version: SequenceNumber },
}
...
impl InputValue {
    pub fn new_object(object_metadata: InputObjectMetadata, value: ObjectValue) -> Self {
        ...
        if let InputObjectMetadata::InputObject { id, owner: Owner::Shared { .. }, .. } = &object_metadata {
            inner.shared_object_ids.insert(*id);
        }
        InputValue { object_metadata: Some(object_metadata), inner }
    }
    ...
    pub fn withdrawal(withdrawal_ty: RawValueType, owner: SuiAddress, limit: U256) -> Self {
        let value = Value::Raw(withdrawal_ty, bcs::to_bytes(&Withdrawal::new(owner, limit)).unwrap());
        InputValue {
            object_metadata: None,
            inner: ResultValue { last_usage_kind: None, value: Some(value), shared_object_ids: BTreeSet::new() },
        }
    }
}
...
impl ResultValue {
    pub fn new(value: Value) -> Self {
        Self { last_usage_kind: None, value: Some(value), shared_object_ids: BTreeSet::new() }
    }
}
...
impl Value {
    ...
    pub fn write_bcs_bytes(&self, buf: &mut Vec<u8>, bound: Option<SizeBound>) -> Result<(), ExecutionError> {
        match self {
...
pub fn ensure_serialized_size(size: u64, bound: SizeBound) -> Result<(), ExecutionError> {
    let bound_size = match bound {
        SizeBound::Object(bound_size) | SizeBound::VectorElem(bound_size) | SizeBound::Raw(bound_size) => bound_size,
    };
    if size > bound_size {
        let e = match bound {
            SizeBound::Object(_) => {
                ExecutionErrorKind::MoveObjectTooBig { object_size: size, max_object_size: bound_size }
            }
            SizeBound::VectorElem(_) => {
                ExecutionErrorKind::MoveVectorElemTooBig { value_size: size, max_scaled_size: bound_size }
            }
            SizeBound::Raw(_) => {
                ExecutionErrorKind::MoveRawValueTooBig { value_size: size, max_scaled_size: bound_size }
            }
        };
