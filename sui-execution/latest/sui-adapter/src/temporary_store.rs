...
use sui_types::{
    ...
    effects::{AccumulatorWriteV1, EffectsObjectChange, TransactionEffects, TransactionEvents},
    error::{ExecutionError, ExecutionErrorKind, SuiResult},
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn merge_accumulator_events(
        ...
        accumulator_events
            .iter()
            .fold(BTreeMap::<ObjectID, Vec<AccumulatorWriteV1>>::new(), |mut map, event| {
                map.entry(*event.accumulator_obj.inner()).or_default().push(event.write.clone());
                map
            })
            .into_iter()
            .map(|(obj_id, writes)| {
                (obj_id, EffectsObjectChange::new_from_accumulator_write(AccumulatorWriteV1::merge(writes)))
            })
    }
...
impl TemporaryStore<'_> {
    ...
    pub fn check_ownership_invariants(
        ...
        let mut objects_to_authenticate = self.execution_results.modified_objects.iter().copied().collect::<Vec<_>>();
        ...
        while let Some(to_authenticate) = objects_to_authenticate.pop() {
            ...
            let parent = if let Some(container_id) = self.wrapped_object_containers.get(&to_authenticate) {
                ...
                *container_id
                ...
                match &old_obj.owner {
                    ...
                    Owner::AddressOwner(parent) | Owner::ConsensusAddressOwner { owner: parent, .. } => {
...
impl TemporaryStore<'_> {
    ...
    pub fn check_sui_conserved_expensive(
        ...
        // settling.
        total_input_oct += self.execution_results.settlement_input_sui;
        total_output_oct += self.execution_results.settlement_output_sui;

        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_oct += self.get_input_oct(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
...
fn was_object_mutated(object: &Object, original: &Object) -> bool {
    ...
    let owner_equal = match (&object.owner, &original.owner) {
        ...
        (Owner::ConsensusAddressOwner { owner: a, .. }, Owner::ConsensusAddressOwner { owner: b, .. }) => a == b,
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) -> Result<(), ExecutionError> {
        ...
        for (id, original) in &self.non_exclusive_input_original_versions {
            ...
            if results.written_objects.get(id).map(|obj| was_object_mutated(obj, original)).unwrap_or(true) {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> SuiResult<Option<PackageObject>> {
        ...
        if let Some(obj) = self.execution_results.written_objects.get(package_id) {
            ...
            self.store.get_package_object(package_id).inspect(|obj| {
                ...
                if let Some(v) = obj
                    && !self.runtime_packages_loaded_from_db.read().contains_key(package_id)
                {
