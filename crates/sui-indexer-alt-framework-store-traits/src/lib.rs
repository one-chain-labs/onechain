...
pub trait Connection: Send {
    ...
    async fn reader_watermark(&mut self, pipeline: &'static str) -> anyhow::Result<Option<ReaderWatermark>>;
...
impl CommitterWatermark {
    ...
    pub fn new_for_testing(checkpoint_hi_inclusive: u64) -> Self {
        CommitterWatermark { epoch_hi_inclusive: 0, checkpoint_hi_inclusive, tx_hi: 0, timestamp_ms_hi_inclusive: 0 }
    }
}
...
pub fn pipeline_task<S: Store>(pipeline_name: &'static str, task_name: Option<&str>) -> Result<String> {
    if pipeline_name.contains(S::DELIMITER) {
        anyhow::bail!("Pipeline name '{}' contains invalid delimiter '{}'", pipeline_name, S::DELIMITER);
    }
