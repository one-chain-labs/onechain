...
pub async fn start_indexer_jsonrpc_for_testing(
    ...
    let handle = {
        ...
        tokio::spawn(async move { Indexer::start_reader(&config, &registry, pool, token_clone).await })
    };
...
pub async fn start_indexer_writer_for_testing_with_mvr_mode(
    ...
    let token = cancel.unwrap_or_default();
    let snapshot_config = snapshot_config.unwrap_or(SnapshotLagConfig { snapshot_min_lag: 5, sleep_duration: 0 });
    ...
    let handle = {
        ...
        let mut ingestion_config = IngestionConfig { start_checkpoint, end_checkpoint, ..Default::default() };
...
pub async fn set_up_on_mvr_mode(
    ...
    let server_handle = tokio::spawn(async move {
        sui_rpc_api::RpcService::new_without_version(sim).start_service(server_url).await;
    });
    // Starts indexer
...
pub async fn wait_for_objects_snapshot(
    ...
    tokio::time::timeout(Duration::from_secs(30), async {
        while {
            let cp_opt = pg_store.get_latest_object_snapshot_checkpoint_sequence_number().await.unwrap();
            cp_opt.is_none() || (cp_opt.unwrap() < checkpoint_sequence_number)
        } {
            tokio::time::sleep(Duration::from_millis(100)).await;
        }
