...
use crate::{
    ...
    handlers::{pruner::PrunableTable, CommitterWatermark, EpochToCommit, TransactionObjectChangesToCommit},
...
macro_rules! chunk {
    ($data: expr, $size: expr) => {{
        $data.into_iter().chunks($size).into_iter().map(|c| c.collect()).collect::<Vec<Vec<_>>>()
    }};
}
...
impl PgIndexerStore {
    pub fn new(pool: ConnectionPool, upload_options: UploadOptions, metrics: IndexerMetrics) -> Self {
        ...
        let parallel_objects_chunk_size = std::env::var("PG_COMMIT_OBJECTS_PARALLEL_CHUNK_SIZE")
            ...
            .parse::<usize>()
            .unwrap();
        let partition_manager = PgPartitionManager::new(pool.clone()).expect("Failed to initialize partition manager");
        ...
        Self { pool, metrics, partition_manager, config }
    }
    ...
    async fn get_available_checkpoint_range(&self) -> Result<(u64, u64), IndexerError> {
        ...
        checkpoints::table
            .select((min(checkpoints::sequence_number), max(checkpoints::sequence_number)))
            .first::<(Option<i64>, Option<i64>)>(&mut connection)
            .await
            .map_err(Into::into)
            .map(|(min, max)| (min.unwrap_or_default() as u64, max.unwrap_or_default() as u64))
    ...
    pub async fn get_checkpoint_range_for_epoch(&self, epoch: u64) -> Result<(u64, Option<u64>), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    pub async fn get_transaction_range_for_checkpoint(&self, checkpoint: u64) -> Result<(u64, u64), IndexerError> {
        use diesel_async::RunQueryDsl;

        let mut connection = self.pool.get().await?;

        pruner_cp_watermark::table
            .select((pruner_cp_watermark::min_tx_sequence_number, pruner_cp_watermark::max_tx_sequence_number))
    ...
    pub async fn get_latest_object_snapshot_checkpoint_sequence_number(&self) -> Result<Option<u64>, IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn persist_display_updates(&self, display_updates: Vec<StoredDisplay>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn persist_object_mutation_chunk(
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_objects_chunks.start_timer();
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
            ...
            async {
                diesel::delete(
                    objects::table.filter(
                        objects::object_id
                            .eq_any(deleted_objects_chunk.iter().map(|o| o.object_id.clone()).collect::<Vec<_>>()),
                    ),
                )
                .execute(conn)
                .await
    ...
    async fn persist_object_snapshot_mutation_chunk(
        ...
        use diesel_async::RunQueryDsl;
        let guard = self.metrics.checkpoint_db_commit_latency_objects_snapshot_chunks.start_timer();
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for mutation_chunk in objects_snapshot_mutations.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    diesel::insert_into(objects_snapshot::table)
                        .values(mutation_chunk)
                        .on_conflict(objects_snapshot::object_id)
                        .do_update()
                        .set((
                            objects_snapshot::object_version.eq(excluded(objects_snapshot::object_version)),
                            objects_snapshot::object_status.eq(excluded(objects_snapshot::object_status)),
                            objects_snapshot::object_digest.eq(excluded(objects_snapshot::object_digest)),
                            ...
                            objects_snapshot::object_type_package.eq(excluded(objects_snapshot::object_type_package)),
                            objects_snapshot::object_type_module.eq(excluded(objects_snapshot::object_type_module)),
                            objects_snapshot::object_type_name.eq(excluded(objects_snapshot::object_type_name)),
                            objects_snapshot::object_type.eq(excluded(objects_snapshot::object_type)),
                            objects_snapshot::serialized_object.eq(excluded(objects_snapshot::serialized_object)),
    ...
    async fn persist_object_snapshot_deletion_chunk(
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for deletion_chunk in objects_snapshot_deletions.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    diesel::delete(
                        objects_snapshot::table.filter(
                            objects_snapshot::object_id
                                .eq_any(deletion_chunk.iter().map(|o| o.object_id.clone()).collect::<Vec<_>>()),
                        ),
                    )
                    .execute(conn)
                    .await
        ...
        .tap_ok(|_| {
            ...
            info!(elapsed, "Deleted {} chunked object snapshots", objects_snapshot_deletions.len(),);
        })
        .tap_err(|e| {
            tracing::error!("Failed to persist object snapshot deletions with error: {}", e);
        })
    }
    ...
    async fn persist_objects_history_chunk(
        ...
        use diesel_async::RunQueryDsl;
        let guard = self.metrics.checkpoint_db_commit_latency_objects_history_chunks.start_timer();
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for stored_objects_history_chunk in stored_objects_history.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    let error_message = concat!("Failed to write to ", stringify!((objects_history::table)), " DB");
    ...
    async fn persist_full_objects_history_chunk(
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_full_objects_history_chunks.start_timer();
        ...
        .tap_ok(|_| {
            let elapsed = guard.stop_and_record();
            info!(elapsed, "Persisted {} chunked full objects history", objects.len(),);
        })
        .tap_err(|e| {
    ...
    async fn persist_objects_version_chunk(
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_objects_version_chunks.start_timer();
        ...
        .tap_ok(|_| {
            let elapsed = guard.stop_and_record();
            info!(elapsed, "Persisted {} chunked object versions", object_versions.len(),);
        })
        .tap_err(|e| {
    ...
    async fn persist_raw_checkpoints_impl(&self, raw_checkpoints: &[StoredRawCheckpoint]) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn persist_checkpoints(&self, checkpoints: Vec<IndexedCheckpoint>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        if first_checkpoint.sequence_number == 0 {
            ...
            self.persist_protocol_configs_and_feature_flags(checkpoint_digest.clone()).await?;
            self.persist_chain_identifier(checkpoint_digest).await?;
        }
        let guard = self.metrics.checkpoint_db_commit_latency_checkpoints.start_timer();
        ...
        .await
        .tap_ok(|_| {
            info!("Persisted {} pruner_cp_watermark rows.", stored_cp_txs.len(),);
        })
        .tap_err(|e| {
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for stored_checkpoint_chunk in stored_checkpoints.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    ...
                    for stored_checkpoint in stored_checkpoint_chunk {
                        self.metrics.db_commit_lag_ms.set(time_now_ms - stored_checkpoint.timestamp_ms);
                        self.metrics.max_committed_checkpoint_sequence_number.set(stored_checkpoint.sequence_number);
                        self.metrics.committed_checkpoint_timestamp_ms.set(stored_checkpoint.timestamp_ms);
                    }
                    ...
                    for stored_checkpoint in stored_checkpoint_chunk {
                        info!(
                            ...
                            stored_checkpoint.sequence_number, time_now_ms, stored_checkpoint.timestamp_ms
                        );
                    }
                }
                Ok::<(), IndexerError>(())
        ...
        .tap_ok(|_| {
            let elapsed = guard.stop_and_record();
            info!(elapsed, "Persisted {} checkpoints", stored_checkpoints.len());
        })
        .tap_err(|e| {
    ...
    async fn persist_transactions_chunk(&self, transactions: Vec<IndexedTransaction>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
        let guard = self.metrics.checkpoint_db_commit_latency_transactions_chunks.start_timer();
        let transformation_guard =
            self.metrics.checkpoint_db_commit_latency_transactions_chunks_transformation.start_timer();
        let transactions = transactions.iter().map(StoredTransaction::from).collect::<Vec<_>>();
        drop(transformation_guard);

        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for transaction_chunk in transactions.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    let error_message = concat!("Failed to write to ", stringify!((transactions::table)), " DB");
    ...
    async fn persist_events_chunk(&self, events: Vec<IndexedEvent>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
        let guard = self.metrics.checkpoint_db_commit_latency_events_chunks.start_timer();
        let len = events.len();
        let events = events.into_iter().map(StoredEvent::from).collect::<Vec<_>>();

        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for event_chunk in events.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    let error_message = concat!("Failed to write to ", stringify!((events::table)), " DB");
    ...
    async fn persist_packages(&self, packages: Vec<IndexedPackage>) -> Result<(), IndexerError> {
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_packages.start_timer();
        let packages = packages.into_iter().map(StoredPackage::from).collect::<Vec<_>>();
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for packages_chunk in packages.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                    diesel::insert_into(packages::table)
                        ...
                        .set((
                            ...
                            packages::checkpoint_sequence_number.eq(excluded(packages::checkpoint_sequence_number)),
                        ))
                        .execute(conn)
                        .await?;
                }
    ...
    async fn persist_event_indices_chunk(&self, indices: Vec<EventIndex>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;

        let guard = self.metrics.checkpoint_db_commit_latency_event_indices_chunks.start_timer();
        let len = indices.len();
        let (
            event_emit_packages,
            event_emit_modules,
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for event_emit_packages_chunk in event_emit_packages.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for event_emit_modules_chunk in event_emit_modules.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for event_struct_packages_chunk in event_struct_packages.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for event_struct_modules_chunk in event_struct_modules.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for event_struct_names_chunk in event_struct_names.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
    ...
    async fn persist_tx_indices_chunk(&self, indices: Vec<TxIndex>) -> Result<(), IndexerError> {
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_tx_indices_chunks.start_timer();
        let len = indices.len();
        let (affected_addresses, affected_objects, input_objects, changed_objects, pkgs, mods, funs, digests, kinds) =
            indices.into_iter().map(|i| i.split()).fold(
                (
                    Vec::new(),
                    Vec::new(),
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for affected_addresses_chunk in affected_addresses.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for affected_objects_chunk in affected_objects.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
                ...
                for changed_objects_chunk in changed_objects.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
    ...
    async fn persist_epoch(&self, epoch: EpochToCommit) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
        let guard = self.metrics.checkpoint_db_commit_latency_epoch.start_timer();
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                ...
                let error_message = concat!("Failed to write to ", stringify!((epochs::table)), " DB");
    ...
    async fn advance_epoch(&self, epoch_to_commit: EpochToCommit) -> Result<(), IndexerError> {
        ...
        if let Some(last_epoch_id) = last_epoch_id {
            ...
            if let Some(last_epoch) = last_db_epoch {
                let epoch_partition_data = EpochPartitionData::compose_data(epoch_to_commit, last_epoch);
                ...
                for (table, (_, last_partition)) in table_partitions {
                    ...
                    if !self.partition_manager.get_strategy(&table).is_epoch_partitioned() {
                        continue;
                    }
                    let guard = self.metrics.advance_epoch_latency.start_timer();
                    self.partition_manager.advance_epoch(table.clone(), last_partition, &epoch_partition_data).await?;
                    let elapsed = guard.stop_and_record();
                    info!(elapsed, "Advanced epoch partition {} for table {}", last_partition, table.clone());
                }
            } else {
    ...
    async fn prune_checkpoints_table(&self, cp: u64) -> Result<(), IndexerError> {
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                diesel::delete(checkpoints::table.filter(checkpoints::sequence_number.eq(cp as i64)))
                    .execute(conn)
                    .await
                    .map_err(IndexerError::from)
    ...
    async fn prune_event_indices_table(&self, min_tx: u64, max_tx: u64) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                diesel::delete(
                    event_emit_module::table.filter(event_emit_module::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    event_emit_package::table.filter(event_emit_package::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(event_senders::table.filter(event_senders::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;

                diesel::delete(
                    event_struct_instantiation::table
                        .filter(event_struct_instantiation::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    event_struct_module::table.filter(event_struct_module::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    event_struct_name::table.filter(event_struct_name::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    event_struct_package::table.filter(event_struct_package::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;
    ...
    async fn prune_tx_indices_table(&self, min_tx: u64, max_tx: u64) -> Result<(), IndexerError> {
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                ...
                diesel::delete(
                    tx_affected_objects::table.filter(tx_affected_objects::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    tx_input_objects::table.filter(tx_input_objects::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(
                    tx_changed_objects::table.filter(tx_changed_objects::tx_sequence_number.between(min_tx, max_tx)),
                )
                .execute(conn)
                .await?;

                diesel::delete(tx_calls_pkg::table.filter(tx_calls_pkg::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;

                diesel::delete(tx_calls_mod::table.filter(tx_calls_mod::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;

                diesel::delete(tx_calls_fun::table.filter(tx_calls_fun::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;

                diesel::delete(tx_digests::table.filter(tx_digests::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;
    ...
    async fn prune_cp_tx_table(&self, cp: u64) -> Result<(), IndexerError> {
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                diesel::delete(
                    pruner_cp_watermark::table.filter(pruner_cp_watermark::checkpoint_sequence_number.eq(cp as i64)),
                )
                .execute(conn)
                .await
                .map_err(IndexerError::from)
    ...
    async fn get_network_total_transactions_by_end_of_epoch(&self, epoch: u64) -> Result<Option<u64>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        E::Iterator: Iterator<Item: AsRef<str>>,
    {
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_watermarks.start_timer();
            ...
            async {
                diesel::insert_into(watermarks::table)
                    ...
                    .set((
                        ...
                        watermarks::checkpoint_hi_inclusive.eq(excluded(watermarks::checkpoint_hi_inclusive)),
    ...
    async fn map_epochs_to_cp_tx(&self, epochs: &[u64]) -> Result<HashMap<u64, (u64, u64)>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        let results: Vec<(i64, i64, Option<i64>)> = epochs::table
            ...
            .select((epochs::epoch, epochs::first_checkpoint_id, epochs::first_tx_sequence_number))
        ...
        Ok(results
            .into_iter()
            .map(|(epoch, checkpoint, tx)| (epoch as u64, (checkpoint as u64, tx.unwrap_or_default() as u64)))
            .collect())
    }

    async fn update_watermarks_lower_bound(&self, watermarks: Vec<(PrunableTable, u64)>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
...
impl IndexerStore for PgIndexerStore {
    ...
    async fn get_latest_object_snapshot_checkpoint_sequence_number(&self) -> Result<Option<u64>, IndexerError> {
        self.get_latest_object_snapshot_checkpoint_sequence_number().await
    }

    async fn persist_objects(&self, object_changes: Vec<TransactionObjectChangesToCommit>) -> Result<(), IndexerError> {
        if object_changes.is_empty() {
            return Ok(());
        }
        let guard = self.metrics.checkpoint_db_commit_latency_objects.start_timer();
        let (indexed_mutations, indexed_deletions) = retain_latest_indexed_objects(object_changes);
        let object_mutations = indexed_mutations.into_iter().map(StoredObject::from).collect::<Vec<_>>();
        let object_deletions = indexed_deletions.into_iter().map(StoredDeletedObject::from).collect::<Vec<_>>();
        let mutation_len = object_mutations.len();
        let deletion_len = object_deletions.len();

        let object_mutation_chunks = chunk!(object_mutations, self.config.parallel_objects_chunk_size);
        let object_deletion_chunks = chunk!(object_deletions, self.config.parallel_objects_chunk_size);
        let mutation_futures =
            object_mutation_chunks.into_iter().map(|c| self.persist_object_mutation_chunk(c)).map(Either::Left);
        let deletion_futures =
            object_deletion_chunks.into_iter().map(|c| self.persist_object_deletion_chunk(c)).map(Either::Right);
        let all_futures = mutation_futures.chain(deletion_futures).collect::<Vec<_>>();

        futures::future::join_all(all_futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
        ...
        let elapsed = guard.stop_and_record();
        info!(elapsed, "Persisted {} objects mutations and {} deletions", mutation_len, deletion_len);
        Ok(())
    }
    ...
    async fn persist_objects_snapshot(
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_objects_snapshot.start_timer();
        let (indexed_mutations, indexed_deletions) = retain_latest_indexed_objects(object_changes);
        let object_snapshot_mutations: Vec<StoredObjectSnapshot> =
            indexed_mutations.into_iter().map(StoredObjectSnapshot::from).collect();
        let object_snapshot_deletions: Vec<StoredObjectSnapshot> =
            indexed_deletions.into_iter().map(StoredObjectSnapshot::from).collect();
        let mutation_len = object_snapshot_mutations.len();
        let deletion_len = object_snapshot_deletions.len();
        let object_snapshot_mutation_chunks = chunk!(object_snapshot_mutations, self.config.parallel_objects_chunk_size);
        let object_snapshot_deletion_chunks = chunk!(object_snapshot_deletions, self.config.parallel_objects_chunk_size);
        ...
        let deletion_futures = object_snapshot_deletion_chunks
            ...
            .map(Either::Right)
            .collect::<Vec<_>>();
        let all_futures = mutation_futures.into_iter().chain(deletion_futures).collect::<Vec<_>>();
        futures::future::join_all(all_futures)
            ...
            .tap_ok(|_| {
                let elapsed = guard.stop_and_record();
                info!(elapsed, "Persisted {} objects snapshot mutations and {} deletions", mutation_len, deletion_len);
            })
            .tap_err(|e| tracing::error!("Failed to persist object snapshot mutation or deletion chunks: {:?}", e))?;
        Ok(())
    }
    ...
    async fn persist_object_history(
        ...
        let skip_history =
            std::env::var("SKIP_OBJECT_HISTORY").map(|val| val.eq_ignore_ascii_case("true")).unwrap_or(false);
        if skip_history {
        ...
        let guard = self.metrics.checkpoint_db_commit_latency_objects_history.start_timer();

        let len = objects.len();
        let chunks = chunk!(objects, self.config.parallel_objects_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_objects_history_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all objects history chunks: {:?}", e))
        })?;
        let elapsed = guard.stop_and_record();
    ...
    async fn persist_full_objects_history(
        ...
        let objects: Vec<StoredFullHistoryObject> = object_changes
            .into_iter()
            .flat_map(|c| {
                let TransactionObjectChangesToCommit { changed_objects, deleted_objects } = c;
                changed_objects.into_iter().map(|o| o.into()).chain(deleted_objects.into_iter().map(|o| o.into()))
            })
            .collect();
        let guard = self.metrics.checkpoint_db_commit_latency_full_objects_history.start_timer();

        let len = objects.len();
        let chunks = chunk!(objects, self.config.parallel_objects_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_full_objects_history_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all full objects history chunks: {:?}", e))
        })?;
        let elapsed = guard.stop_and_record();
    ...
    async fn persist_objects_version(&self, object_versions: Vec<StoredObjectVersion>) -> Result<(), IndexerError> {
        if object_versions.is_empty() {
            return Ok(());
        }

        let guard = self.metrics.checkpoint_db_commit_latency_objects_version.start_timer();

        let len = object_versions.len();
        let chunks = chunk!(object_versions, self.config.parallel_objects_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_objects_version_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all objects version chunks: {:?}", e))
        })?;
    ...
    async fn persist_transactions(&self, transactions: Vec<IndexedTransaction>) -> Result<(), IndexerError> {
        let guard = self.metrics.checkpoint_db_commit_latency_transactions.start_timer();
        let len = transactions.len();

        let chunks = chunk!(transactions, self.config.parallel_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_transactions_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all transactions chunks: {:?}", e))
        })?;
        let elapsed = guard.stop_and_record();
    ...
    async fn persist_events(&self, events: Vec<IndexedEvent>) -> Result<(), IndexerError> {
        ...
        let len = events.len();
        let guard = self.metrics.checkpoint_db_commit_latency_events.start_timer();
        let chunks = chunk!(events, self.config.parallel_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_events_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| IndexerError::PostgresWriteError(format!("Failed to persist all events chunks: {:?}", e)))?;
        let elapsed = guard.stop_and_record();
        info!(elapsed, "Persisted {} events", len);
        Ok(())
    }

    async fn persist_displays(&self, display_updates: BTreeMap<String, StoredDisplay>) -> Result<(), IndexerError> {
        if display_updates.is_empty() {
            return Ok(());
        }
        self.persist_display_updates(display_updates.values().cloned().collect::<Vec<_>>()).await
    }
    ...
    async fn persist_event_indices(&self, indices: Vec<EventIndex>) -> Result<(), IndexerError> {
        ...
        let len = indices.len();
        let guard = self.metrics.checkpoint_db_commit_latency_event_indices.start_timer();
        let chunks = chunk!(indices, self.config.parallel_chunk_size);

        let futures = chunks.into_iter().map(|chunk| self.persist_event_indices_chunk(chunk)).collect::<Vec<_>>();
        futures::future::join_all(futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| {
                IndexerError::PostgresWriteError(format!("Failed to persist all event_indices chunks: {:?}", e))
            })
            .tap_ok(|_| {
                let elapsed = guard.stop_and_record();
    ...
    async fn persist_tx_indices(&self, indices: Vec<TxIndex>) -> Result<(), IndexerError> {
        ...
        let len = indices.len();
        let guard = self.metrics.checkpoint_db_commit_latency_tx_indices.start_timer();
        let chunks = chunk!(indices, self.config.parallel_chunk_size);

        let futures = chunks.into_iter().map(|chunk| self.persist_tx_indices_chunk(chunk)).collect::<Vec<_>>();
        futures::future::join_all(futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| IndexerError::PostgresWriteError(format!("Failed to persist all tx_indices chunks: {:?}", e)))
            .tap_ok(|_| {
                let elapsed = guard.stop_and_record();
    ...
    async fn prune_epoch(&self, epoch: u64) -> Result<(), IndexerError> {
        let (mut min_cp, max_cp) = match self.get_checkpoint_range_for_epoch(epoch).await? {
            ...
            _ => Err(IndexerError::PostgresReadError(format!("Failed to get checkpoint range for epoch {}", epoch))),
        }?;
        ...
        for cp in min_cp..=max_cp {
            ...
            info!("Pruning checkpoint {} of epoch {} (min_prunable_cp: {})", cp, epoch, min_prunable_cp);

            let (min_tx, max_tx) = self.get_transaction_range_for_checkpoint(cp).await?;
            self.prune_tx_indices_table(min_tx, max_tx).await?;
            info!("Pruned transactions for checkpoint {} from tx {} to tx {}", cp, min_tx, max_tx);
            self.prune_event_indices_table(min_tx, max_tx).await?;
            info!("Pruned events of transactions for checkpoint {} from tx {} to tx {}", cp, min_tx, max_tx);
    ...
    async fn upload_display(&self, epoch_number: u64) -> Result<(), IndexerError> {
        ...
        let mut buffer = Cursor::new(Vec::new());
        {
            ...
            writer
                ...
                .map_err(|_| IndexerError::GcsError("Failed to write display to csv".to_string()))?;
        ...
        if let (Some(cred_path), Some(bucket)) =
            (self.config.gcs_cred_path.clone(), self.config.gcs_display_bucket.clone())
        {
            ...
            let remote_store = remote_store_config
                .make()
                .map_err(|e| IndexerError::GcsError(format!("Failed to make GCS remote store: {}", e)))?;
    ...
    async fn restore_display(&self, bytes: bytes::Bytes) -> Result<(), IndexerError> {
        ...
        let displays = csv_reader
            ...
            .map_err(|e| IndexerError::GcsError(format!("Failed to deserialize display records: {}", e)))?;
        self.persist_display_updates(displays).await
    }

    async fn get_network_total_transactions_by_end_of_epoch(&self, epoch: u64) -> Result<Option<u64>, IndexerError> {
        self.get_network_total_transactions_by_end_of_epoch(epoch).await
    }
    ...
    async fn persist_protocol_configs_and_feature_flags(&self, chain_id: Vec<u8>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;

        let chain_id = ChainIdentifier::from(CheckpointDigest::try_from(chain_id).expect("Unable to convert chain id"));
        ...
        for version in start_version..=end_version {
            ...
            let feature_flags = protocol_configs
                .feature_map()
                .into_iter()
                .map(|(k, v)| StoredFeatureFlag { protocol_version: version, flag_name: k, flag_value: v })
                .collect::<Vec<_>>();
    ...
    async fn persist_chain_identifier(&self, checkpoint_digest: Vec<u8>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn persist_raw_checkpoints(&self, checkpoints: Vec<StoredRawCheckpoint>) -> Result<(), IndexerError> {
...
fn make_objects_history_to_commit(tx_object_changes: Vec<TransactionObjectChangesToCommit>) -> Vec<StoredHistoryObject> {
    let deleted_objects: Vec<StoredHistoryObject> =
        tx_object_changes.clone().into_iter().flat_map(|changes| changes.deleted_objects).map(|o| o.into()).collect();
    let mutated_objects: Vec<StoredHistoryObject> =
        tx_object_changes.into_iter().flat_map(|changes| changes.changed_objects).map(|o| o.into()).collect();
...
fn retain_latest_indexed_objects(
    ...
    (mutations.into_values().collect(), deletions.into_values().collect())
}
