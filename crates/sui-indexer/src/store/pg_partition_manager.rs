...
impl EpochPartitionData {
    pub fn compose_data(epoch: EpochToCommit, last_db_epoch: StoredEpochInfo) -> Self {
        ...
        let last_epoch_start_tx = next_epoch_start_tx - epoch.last_epoch_total_transactions().unwrap();
...
impl PgPartitionManager {
    pub fn new(pool: ConnectionPool) -> Result<Self, IndexerError> {
        ...
        let manager = Self { pool, partition_strategies };
        Ok(manager)
    }
    ...
    pub async fn get_table_partitions(&self) -> Result<BTreeMap<String, (u64, u64)>, IndexerError> {
        ...
        Ok(diesel_async::RunQueryDsl::load(diesel::sql_query(GET_PARTITION_SQL), &mut connection)
            .await?
            .into_iter()
            .map(|table: PartitionedTable| {
                (table.table_name, (table.first_partition as u64, table.last_partition as u64))
            })
            .collect())
    }
    ...
    pub fn get_strategy(&self, table_name: &str) -> PgPartitionStrategy {
        self.partition_strategies.get(table_name).copied().unwrap_or(PgPartitionStrategy::CheckpointSequenceNumber)
    }

    pub fn determine_epoch_partition_range(&self, table_name: &str, data: &EpochPartitionData) -> Option<(u64, u64)> {
        match self.get_strategy(table_name) {
            PgPartitionStrategy::CheckpointSequenceNumber => Some((data.last_epoch_start_cp, data.next_epoch_start_cp)),
            PgPartitionStrategy::TxSequenceNumber => Some((data.last_epoch_start_tx, data.next_epoch_start_tx)),
    ...
    pub async fn advance_epoch(
        ...
        if last_partition == data.last_epoch {
            ...
            error!("Epoch partition for table {} is not in sync with the last epoch {}.", table, data.last_epoch);
        } else {
            info!("Epoch has been advanced to {} already, skipping.", data.next_epoch);
        }
        Ok(())
    }

    pub async fn drop_table_partition(&self, table: String, partition: u64) -> Result<(), IndexerError> {
        transaction_with_retry(&self.pool, Duration::from_secs(10), |conn| {
            async {
                diesel_async::RunQueryDsl::execute(
