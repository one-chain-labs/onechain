...
impl<T: IngestionBackfillTrait + 'static> IngestionBackfillTask<T> {
    pub async fn new(remote_store_url: String, start_checkpoint: CheckpointSequenceNumber) -> Self {
        ...
        let adapter: Adapter<T> = Adapter { ready_checkpoints: ready_checkpoints.clone(), notify: notify.clone() };
        let reader_options = ReaderOptions { batch_size: 200, ..Default::default() };
        let (executor, _exit_sender) =
            setup_single_workflow(adapter, remote_store_url, start_checkpoint, 200, Some(reader_options)).await.unwrap();
        tokio::task::spawn(async move {
            executor.await.unwrap();
        });
        Self { ready_checkpoints, notify, _exit_sender }
    }
}
...
impl<T: IngestionBackfillTrait> Worker for Adapter<T> {
    ...
    async fn process_checkpoint(&self, checkpoint: &CheckpointData) -> anyhow::Result<()> {
        ...
        self.ready_checkpoints.insert(checkpoint.checkpoint_summary.sequence_number, processed);
...
impl<T: IngestionBackfillTrait> BackfillTask for IngestionBackfillTask<T> {
    async fn backfill_range(&self, pool: ConnectionPool, range: &RangeInclusive<usize>) {
        ...
        loop {
            while start <= end {
                if let Some((_, processed)) = self.ready_checkpoints.remove(&(start as CheckpointSequenceNumber)) {
