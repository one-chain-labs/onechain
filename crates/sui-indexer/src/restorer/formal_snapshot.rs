...
use crate::{
    ...
    restorer::archives::{read_restore_checkpoint_info, RestoreCheckpointInfo},
...
impl IndexerFormalSnapshotRestorer {
    pub async fn new(store: PgIndexerStore, restore_config: RestoreConfig) -> Result<Self, IndexerError> {
        ...
        info!("Initialized formal snapshot reader at epoch {}", restore_config.start_epoch);

        Ok(Self { store, reader, restore_config: restore_config.clone() })
    }
    ...
    pub async fn restore(&mut self) -> Result<(), IndexerError> {
        ...
        let (input_files, epoch_dir, remote_object_store, _concurrency) = self.reader.export_metadata().await?;
    ...
    async fn restore_move_objects(
        ...
        let move_object_progress_bar = Arc::new(
            self.reader.get_multi_progress().add(
        ...
        Abortable::new(
            async move {
                let sema_limit = Arc::new(Semaphore::new(self.restore_config.object_store_concurrent_limit));
                ...
                for (bucket, (part_num, file_metadata)) in input_files.into_iter() {
                    ...
                    let restore_task = task::spawn(async move {
                        ...
                        .await;
                        info!("Finished downloading move object file {:?}", object_file_path);
                                ...
                                for object in obj_iter {
                                    match object {
                                        LiveObject::Normal(obj) => {
                                            ...
                                            let indexed_object = IndexedObject::from_object(0, obj, None);
                        ...
                        let objects_snapshot_changes =
                            TransactionObjectChangesToCommit { changed_objects: move_objects, deleted_objects: vec![] };
                        store_clone
    ...
    async fn restore_cp_watermark_and_chain_id(&self) -> Result<(), IndexerError> {
        ...
        .await?;
        let RestoreCheckpointInfo { next_checkpoint_after_epoch, chain_identifier } = restore_checkpoint_info;
        self.store.persist_chain_identifier(chain_identifier.into_inner().to_vec()).await?;
        ...
        let last_cp = IndexedCheckpoint { sequence_number: next_checkpoint_after_epoch - 1, ..Default::default() };
