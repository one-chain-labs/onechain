...
use crate::{
    ...
    models::{
        ...
        transactions::{
            stored_events_to_events,
            tx_events_to_sui_tx_events,
            StoredTransaction,
...
impl IndexerReader {
    pub fn new(pool: ConnectionPool) -> Self {
        ...
        Self { pool, package_resolver }
    }

    pub async fn new_with_config<T: Into<String>>(db_url: T, config: ConnectionPoolConfig) -> Result<Self> {
...
impl IndexerReader {
    async fn get_object_from_db(
        ...
        let mut query = objects::table.filter(objects::object_id.eq(object_id.to_vec())).into_boxed();
        if let Some(version) = version {
            query = query.filter(objects::object_version.eq(version.value() as i64))
        }

        query.first::<StoredObject>(&mut connection).await.optional().map_err(Into::into)
    }
    ...
    pub async fn get_object_read(&self, object_id: ObjectID) -> Result<ObjectRead, IndexerError> {
        ...
        if let Some(object) = stored_object {
            object.try_into_object_read(self.package_resolver.clone()).await
        } else {
    ...
    pub async fn get_package(&self, package_id: ObjectID) -> Result<Package, IndexerError> {
        ...
        let pkg = store
            ...
            .await
            .map_err(|e| {
                IndexerError::PostgresReadError(format!("Fail to fetch package from package store with error {:?}", e))
            })?
            .as_ref()
            .clone();
        Ok(pkg)
    ...
    async fn get_epoch_info_from_db(&self, epoch: Option<EpochId>) -> Result<Option<StoredEpochInfo>, IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    pub async fn get_latest_epoch_info_from_db(&self) -> Result<StoredEpochInfo, IndexerError> {
        ...
        let stored_epoch =
            epochs::table.order_by(epochs::epoch.desc()).first::<StoredEpochInfo>(&mut connection).await?;

        Ok(stored_epoch)
    }

    pub async fn get_epoch_info(&self, epoch: Option<EpochId>) -> Result<Option<EpochInfo>, IndexerError> {
    ...
    async fn get_epochs_from_db(
        ...
        query.limit(limit as i64).load(&mut connection).await.map_err(Into::into)
    }
    ...
    pub async fn get_latest_sui_system_state(&self) -> Result<SuiSystemStateSummary, IndexerError> {
        ...
        let system_state =
            tokio::task::spawn_blocking(move || sui_types::sui_system_state::get_sui_system_state(&object_store))
                .await
                .unwrap()?
    ...
    ) -> Result<sui_types::sui_system_state::sui_system_state_summary::SuiValidatorSummary, IndexerError> {
        let object_store = ConnectionAsObjectStore::from_pool(&self.pool)
            .await
            .map_err(|e| IndexerError::PgPoolConnectionError(e.to_string()))?;
    ...
    async fn get_checkpoint_from_db(
        ...
        let stored_checkpoint = checkpoints::table
            ...
            .pipe(|query| match checkpoint_id {
                CheckpointId::SequenceNumber(seq) => query.filter(checkpoints::sequence_number.eq(seq as i64)),
    ...
    pub async fn get_latest_checkpoint(&self) -> Result<sui_json_rpc_types::Checkpoint, IndexerError> {
    ...
    async fn get_checkpoints_from_db(
        ...
        query.limit(limit as i64).load::<StoredCheckpoint>(&mut connection).await.map_err(Into::into)
    }
    ...
    async fn multi_get_transactions(
        ...
        let digests = digests.iter().map(|digest| digest.inner().to_vec()).collect::<Vec<_>>();

        transactions::table
            .inner_join(tx_digests::table.on(transactions::tx_sequence_number.eq(tx_digests::tx_sequence_number)))
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        for stored_tx in stored_txes {
            ...
            tx_block_responses_futures.push(tokio::task::spawn(
                stored_tx.try_into_sui_transaction_block_response(options_clone, package_resolver_clone),
            ));
        }
    ...
    async fn multi_get_transactions_with_sequence_numbers(
        ...
        let mut query =
            transactions::table.filter(transactions::tx_sequence_number.eq_any(tx_sequence_numbers)).into_boxed();
        match is_descending {
            Some(true) => {
        ...
        query.load::<StoredTransaction>(&mut connection).await.map_err(Into::into)
    }
    ...
    pub async fn get_owned_objects(
            ...
            match filter {
                ...
                SuiObjectDataFilter::MatchNone(filters) => {
                    for filter in filters {
                        if let SuiObjectDataFilter::StructType(struct_tag) = filter {
                            let object_type = struct_tag.to_canonical_string(/* with_prefix */ true);
                            query = query.filter(objects::object_type.not_like(format!("{}%", object_type)));
                        } else {
                            return Err(IndexerError::InvalidArgumentError(
                                    "Invalid filter type. Only struct, MatchAny and MatchNone of struct filters are supported.".into(),
                                ));
                ...
                _ => {
                    return Err(IndexerError::InvalidArgumentError(
                        "Invalid filter type. Only struct, MatchAny and MatchNone of struct filters are supported."
                            .into(),
                    ));
                }
            }
        ...
        query.load::<StoredObject>(&mut connection).await.map_err(|e| IndexerError::PostgresReadError(e.to_string()))
    }

    pub async fn multi_get_objects(&self, object_ids: Vec<ObjectID>) -> Result<Vec<StoredObject>, IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn query_transaction_blocks_by_checkpoint(
        ...
        let mut query =
            transactions::table.filter(transactions::tx_sequence_number.between(tx_range.0, tx_range.1)).into_boxed();
        ...
        let stored_txes = query.limit(limit as i64).load::<StoredTransaction>(&mut connection).await?;
        self.stored_transaction_to_transaction_block(stored_txes, options).await
    }
    ...
    pub async fn query_transaction_blocks(
        ...
        let (table_name, main_where_clause) = match filter {
            ...
            Some(TransactionFilter::Checkpoint(seq)) => {
                return self
                    .query_transaction_blocks_by_checkpoint(seq, options, cursor_tx_seq, limit, is_descending)
                    .await
            }
            ...
            Some(TransactionFilter::MoveFunction { package, module, function }) => {
                ...
                match (module, function) {
                    (Some(module), Some(function)) => (
                        ...
                        format!("package = '\\x{package}'::bytea AND module = '{module}' AND func = '{function}'",),
                    ),
                    (Some(module), None) => {
                        ("tx_calls_mod".to_owned(), format!("package = '\\x{package}'::bytea AND module = '{module}'",))
                    }
                    (None, Some(_)) => {
                        return Err(IndexerError::InvalidArgumentError(
                    ...
                    (None, None) => ("tx_calls_pkg".to_owned(), format!("package = '\\x{package}'::bytea")),
                }
            }
            Some(TransactionFilter::AffectedObject(object_id)) => {
                let object_id = Hex::encode(object_id.to_vec());
                ("tx_affected_objects".to_owned(), format!("affected = '\\x{object_id}'::bytea"))
            }
            ...
            Some(TransactionFilter::FromOrToAddress { addr }) => {
                ...
                ("tx_affected_addresses".to_owned(), format!("affected = '\\x{address}'::bytea"))
            }
            Some(TransactionFilter::TransactionKind(_) | TransactionFilter::TransactionKindIn(_)) => {
                return Err(IndexerError::NotSupportedError("TransactionKind filter is not supported.".into()));
            }
        ...
        let tx_sequence_numbers = diesel::sql_query(query.clone())
            ...
            .collect::<Vec<i64>>();
        self.multi_get_transaction_block_response_by_sequence_numbers(tx_sequence_numbers, options, Some(is_descending))
            .await
    }
    ...
    async fn multi_get_transaction_block_response_by_sequence_numbers(
        ...
        let stored_txes: Vec<StoredTransaction> =
            self.multi_get_transactions_with_sequence_numbers(tx_sequence_numbers, is_descending).await?;
        self.stored_transaction_to_transaction_block(stored_txes, options).await
    }
    ...
    pub async fn multi_get_transaction_block_response_in_blocking_task(
        ...
        self.multi_get_transaction_block_response_in_blocking_task_impl(&digests, options).await
    }
    ...
    pub async fn get_transaction_events(
        ...
        let (timestamp_ms, serialized_events) = transactions::table
            .filter(
                transactions::tx_sequence_number.nullable().eq(tx_digests::table
                    .select(tx_digests::tx_sequence_number)
        ...
        let sui_tx_events =
            tx_events_to_sui_tx_events(tx_events, self.package_resolver(), digest, timestamp_ms as u64).await?;
    ...
    async fn query_events_by_tx_digest(
        ...
        let stored_events = query.limit(limit as i64).load::<StoredEvent>(&mut connection).await?;

        let mut sui_event_futures = vec![];
        for stored_event in stored_events {
            sui_event_futures.push(tokio::task::spawn(stored_event.try_into_sui_event(self.package_resolver.clone())));
        }
    ...
    pub async fn query_events(
        ...
        let (tx_seq, event_seq) = if let Some(cursor) = cursor {
            let EventID { tx_digest, event_seq } = cursor;
        ...
        let query = if let EventFilter::Sender(sender) = &filter {
            ...
            return self.query_events_by_tx_digest(tx_digest, cursor, tx_seq, limit, descending_order).await;
        } else {
            let main_where_clause = match filter {
                ...
                EventFilter::MoveModule { package, module } => {
                    format!("package = '\\x{}'::bytea AND module = '{}'", package.to_hex(), module,)
                }
                EventFilter::MoveEventType(struct_tag) => {
                    format!("event_type = '{}'", struct_tag.to_canonical_display(/* with_prefix */ true),)
                }
        ...
        let stored_events = diesel::sql_query(query).load::<StoredEvent>(&mut connection).await?;
    ...
    pub async fn get_dynamic_fields(
        ...
        let stored_objects = self.get_dynamic_fields_raw(parent_object_id, cursor, limit).await?;
        ...
        for stored_object in stored_objects {
            ...
            df_futures.push(tokio::task::spawn(async move {
                indexer_reader_arc_clone.try_create_dynamic_field_info(stored_object).await
            }));
        }
    ...
    pub async fn get_dynamic_fields_raw(
        ...
        query.load::<StoredObject>(&mut connection).await.map_err(Into::into)
    }
    ...
    async fn try_create_dynamic_field_info(
        ...
        let move_object = match object.data.try_as_move().cloned() {
            ...
            None => {
                return Err(IndexerError::ResolveMoveStructError("Object is not a MoveObject".to_string()));
            }
        };
        let type_tag: TypeTag = move_object.type_().clone().into();
        let layout = self.package_resolver.type_layout(type_tag.clone()).await.map_err(|e| {
    ...
    pub async fn bcs_name_from_dynamic_field_name(&self, name: &DynamicFieldName) -> Result<Vec<u8>, IndexerError> {
        let move_type_layout = self.package_resolver().type_layout(name.type_.clone()).await.map_err(|e| {
            IndexerError::ResolveMoveStructError(format!("Failed to get type layout for type {}: {}", name.type_, e))
        })?;
    ...
    pub async fn get_coin_balances(
        ...
        let coin_type_filter =
            if let Some(coin_type) = coin_type { format!("= '{}'", coin_type) } else { "IS NOT NULL".to_string() };
    ...
    pub async fn get_coin_metadata(&self, coin_struct: StructTag) -> Result<Option<SuiCoinMetadata>, IndexerError> {
        let coin_metadata_type = CoinMetadata::type_(coin_struct);

        self.get_singleton_object(&coin_metadata_type).await?.and_then(|o| SuiCoinMetadata::try_from(o).ok()).pipe(Ok)
    }
...
struct ConnectionAsObjectStore {
    inner: std::sync::Mutex<
        diesel_async::async_connection_wrapper::AsyncConnectionWrapper<crate::database::Connection<'static>>,
    >,
}

impl ConnectionAsObjectStore {
    async fn from_pool(pool: &ConnectionPool) -> Result<Self, diesel_async::pooled_connection::PoolError> {
    ...
    fn get_object_from_objects(
        ...
        let connection: &mut diesel_async::async_connection_wrapper::AsyncConnectionWrapper<_> = &mut guard;
        ...
        query.first::<StoredObject>(connection).optional().map_err(Into::into)
    }
    ...
    fn get_object_from_history(
        ...
        let mut history_query =
            objects_history::table.filter(objects_history::dsl::object_id.eq(object_id.to_vec())).into_boxed();

        if let Some(version) = version {
            history_query = history_query.filter(objects_history::dsl::object_version.eq(version.value() as i64));
        }
        ...
        let mut snapshot_query =
            objects_snapshot::table.filter(objects_snapshot::dsl::object_id.eq(object_id.to_vec())).into_boxed();
...
impl sui_types::storage::ObjectStore for ConnectionAsObjectStore {
    fn get_object(&self, object_id: &ObjectID) -> Option<sui_types::object::Object> {
        self.get_object(object_id, None).expect("Failed to get object")
    }
    ...
    fn get_object_by_key(
        ...
        self.get_object(object_id, Some(version)).expect("Failed to get object")
    }
}
