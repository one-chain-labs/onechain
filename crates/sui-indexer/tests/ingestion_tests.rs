...
use sui_indexer::{
    ...
    schema::{checkpoints, objects, objects_snapshot, transactions},
...
pub async fn test_epoch_boundary() -> Result<(), IndexerError> {
    ...
    sim.advance_epoch(AdvanceEpochConfig { create_random_state: true, ..Default::default() }); // checkpoint 2 and epoch 1
