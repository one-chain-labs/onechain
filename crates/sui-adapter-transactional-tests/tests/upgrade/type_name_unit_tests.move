...
module A2::m {
    ...
    fun dt_name(str: vector<u8>): vector<u8> {
        ...
        str
            .skip(one::address::length() * 2 + 2 + 1 + 2)
            .take_while!(|c| c != &lt)
    }
    ...
    macro fun case<$T>($original: address, $defining: address, $name: vector<u8>) {
        ...
        assert!(
            one::address::from_ascii_bytes(og.address_string().as_bytes()) ==
        ...
        assert!(
            one::address::from_ascii_bytes(def.address_string().as_bytes()) ==
    ...
    // peels the leading address and parses it using `one::address::from_ascii_bytes`
    fun leading_address(str: vector<u8>): address {
        one::address::from_ascii_bytes(&str.take(one::address::length() * 2))
    }
