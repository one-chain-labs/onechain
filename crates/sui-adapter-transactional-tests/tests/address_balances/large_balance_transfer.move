...
//# programmable --sender A --inputs withdraw<one::balance::Balance<test::large_balance::MARKER>>(18446744073709551614) @B
//> 0: one::balance::redeem_funds<test::large_balance::MARKER>(Input(0));
//> 1: one::balance::send_funds<test::large_balance::MARKER>(Result(0), Input(1));
...
//# programmable --sender A --inputs withdraw<one::balance::Balance<test::large_balance::MARKER>>(18446744073709551614) withdraw<one::balance::Balance<test::large_balance::MARKER>>(18446744073709551614) @B
//> 0: one::balance::redeem_funds<test::large_balance::MARKER>(Input(0));
//> 1: one::balance::redeem_funds<test::large_balance::MARKER>(Input(1));
//> 2: one::balance::join<test::large_balance::MARKER>(Result(0), Result(1));
//> 3: one::balance::send_funds<test::large_balance::MARKER>(Result(0), Input(2));
