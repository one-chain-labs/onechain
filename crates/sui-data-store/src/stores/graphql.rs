...
impl DataStore {
    ...
    pub(crate) async fn run_query<T, V>(&self, operation: &Operation<T, V>) -> Result<GraphQlResponse<T>, Error>
    where
    ...
    async fn transaction(&self, digest: &str) -> Result<Option<(TransactionData, TransactionEffects, u64)>, Error> {
