...
impl FileSystemStore {
    ...
    pub fn base_path() -> Result<PathBuf, Error> {
        let home_dir = std::env::var("SUI_DATA_STORE")
            ...
            .map_err(|_| anyhow!("Cannot determine home directory. Define a SUI_DATA_STORE environment variable"))?;
    ...
    fn get_chain_id_for_node(&self) -> Result<String, Error> {
        ...
        Err(anyhow!("No mapping found for node '{}' in mapping file {}", node_key, mapping_file.display()))
    }
    ...
    fn root_versions_path(&self, object_id: &ObjectID) -> Result<PathBuf, Error> {
        Ok(self.objects_dir()?.join(object_id.to_string()).join(ROOT_VERSIONS_FILE))
    }

    fn checkpoint_versions_path(&self, object_id: &ObjectID) -> Result<PathBuf, Error> {
        Ok(self.objects_dir()?.join(object_id.to_string()).join(CHECKPOINT_VERSIONS_FILE))
    }
    ...
    fn write_full_version_mapping(&self, path: &Path, mapping: &BTreeMap<u64, u64>) -> Result<(), Error> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).with_context(|| format!("Failed to create directory: {}", parent.display()))?;
        }
    ...
    fn update_root_mapping(&self, object_id: &ObjectID, key: u64, version: u64) -> Result<(), Error> {
    ...
    fn update_checkpoint_mapping(&self, object_id: &ObjectID, key: u64, version: u64) -> Result<(), Error> {
    ...
    fn get_object_by_version(&self, object_id: &ObjectID, version: u64) -> Result<Option<(Object, u64)>, Error> {
    ...
    fn get_object_by_checkpoint(&self, object_id: &ObjectID, checkpoint: u64) -> Result<Option<(Object, u64)>, Error> {
        self.load_checkpoint_mapping(object_id)?;
...
impl TransactionStore for FileSystemStore {
    fn transaction_data_and_effects(&self, tx_digest: &str) -> Result<Option<TransactionInfo>, Error> {
...
impl TransactionStoreWriter for FileSystemStore {
    fn write_transaction(&self, tx_digest: &str, transaction_info: TransactionInfo) -> Result<(), Error> {
        let file_path = self.transaction_dir()?.join(tx_digest);
