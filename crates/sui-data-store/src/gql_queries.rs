...
use crate::{stores::DataStore, EpochData};
...
pub(crate) mod epoch_query {
    ...
    pub(crate) async fn query(epoch_id: u64, data_store: &DataStore) -> Result<Option<EpochData>, Error> {
        ...
        Ok(Some(EpochData {
            ...
            protocol_version: epoch
                ...
                .ok_or(anyhow!(format!("Canot find epoch info for epoch {}", epoch_id)))?,
            rgp: epoch
                .reference_gas_price
                .map(|rgp| rgp.0)
                .ok_or(anyhow!(format!("Canot find epoch info for epoch {}", epoch_id)))?
    ...
    pub(crate) async fn query(
        ...
        let txn_data: TransactionData = bcs::from_bytes(
            &CryptoBase64::decode(
                &transaction
                    .transaction_bcs
                    .ok_or_else(|| anyhow!(format!("Transaction data not available (None) for digest: {}", digest),))?
                    .0,
            )
        ...
        let effect_frag = transaction.effects.ok_or_else(|| anyhow!("Missing effects in transaction data response"))?;
        let effects: sui_types::effects::TransactionEffects = bcs::from_bytes(
            &CryptoBase64::decode(
                &effect_frag.effects_bcs.ok_or_else(|| anyhow!("Missing effects bcs in transaction data response"))?.0,
            )
...
pub(crate) mod object_query {
    ...
    pub(crate) async fn query(
        ...
        let mut keys = keys.iter().cloned().map(ObjectKey::from).collect::<Vec<_>>();
        ...
        for keys in key_chunks {
            ...
            let list = if let Some(data) = response.data {
                ...
                return Err(anyhow!("Missing data in transaction query response. Errors: {:?}", response.errors,));
            };
            ...
            let chunk = list
                ...
                .map(|frag| match frag {
                    Some(frag) => {
                        let b64 = frag.object_bcs.ok_or_else(|| anyhow!("Object bcs is None for object"))?.0;
                        let bytes = CryptoBase64::decode(&b64)?;
                        let obj: Object = bcs::from_bytes(&bytes)?;
                        let version = frag.version.ok_or_else(|| anyhow!("Object version is None for object"))?;
