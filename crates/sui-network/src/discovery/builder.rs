...
impl Builder {
    ...
    pub fn new(trusted_peer_change_rx: watch::Receiver<TrustedPeerChangeEvent>) -> Self {
        Self { config: None, metrics: None, trusted_peer_change_rx }
    }
    ...
    pub fn build(self) -> (UnstartedDiscovery, DiscoveryServer<impl Discovery>) {
        let discovery_config = self.config.clone().and_then(|config| config.discovery).unwrap_or_default();
        ...
        if let Some(limit) = discovery_config.get_known_peers_rate_limit {
            discovery_server = discovery_server.add_layer_for_get_known_peers_v2(InboundRequestLayer::new(
    ...
    pub(super) fn build_internal(self) -> (UnstartedDiscovery, Server) {
        let Builder { config, metrics, trusted_peer_change_rx } = self;
        let config = config.unwrap();
        let metrics = metrics.unwrap_or_else(Metrics::disabled);
        let (sender, receiver) = oneshot::channel();

        let handle = Handle { _shutdown_handle: Arc::new(sender) };

        let state = State { our_info: None, connected_peers: HashMap::default(), known_peers: HashMap::default() }
            .pipe(RwLock::new)
            .pipe(Arc::new);

        let server = Server { state: state.clone() };

        (
            UnstartedDiscovery { handle, config, shutdown_handle: receiver, state, trusted_peer_change_rx, metrics },
            server,
        )
    }
}
...
impl UnstartedDiscovery {
    pub(super) fn build(self, network: anemo::Network, keypair: NetworkKeyPair) -> (DiscoveryEventLoop, Handle) {
        let Self { handle, config, shutdown_handle, state, trusted_peer_change_rx, metrics } = self;
            ...
            discovery_config
                ...
                .chain(
                    config
                        .seed_peers
                        .iter()
                        .filter_map(|peer| peer.peer_id.map(|peer_id| (peer_id, Some(peer.address.clone())))),
                )
                .collect::<HashMap<_, _>>(),
        );
        (
