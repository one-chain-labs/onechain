...
use std::{collections::HashMap, io::Write, num::NonZeroUsize, time::Duration};

use anemo::{PeerId, Request};
use anyhow::anyhow;
use sui_config::{node::ArchiveReaderConfig, object_storage_config::ObjectStoreConfig};
...
async fn server_push_checkpoint() {
    ...
    let (ordered_checkpoints, _, _sequence_number_to_digest, _checkpoints) = committee.make_empty_checkpoints(2, None);
    ...
    let (UnstartedStateSync { handle: _handle, mut mailbox, peer_heights, .. }, server) =
        Builder::new().store(store).build_internal();
    let peer_id = PeerId([9; 32]); // fake PeerId

    peer_heights.write().unwrap().peers.insert(peer_id, PeerStateSyncInfo {
    ...
    assert_eq!(
        peer_heights.read().unwrap().unprocessed_checkpoints.get(checkpoint.digest()).unwrap().data(),
        checkpoint.data(),
    );
    assert_eq!(peer_heights.read().unwrap().highest_known_checkpoint().unwrap().data(), checkpoint.data(),);
    assert!(matches!(mailbox.try_recv().unwrap(), StateSyncMessage::StartSyncJob));
}

#[tokio::test]
async fn server_get_checkpoint() {
    let committee = CommitteeFixture::generate(rand::rngs::OsRng, 0, 4);
    let (ordered_checkpoints, _, _sequence_number_to_digest, _checkpoints) = committee.make_empty_checkpoints(3, None);

    let (builder, server) = Builder::new().store(SharedInMemoryStore::default()).build_internal();
    ...
    let response =
        server.get_checkpoint_summary(Request::new(GetCheckpointSummaryRequest::Latest)).await.unwrap().into_inner();
    assert_eq!(response.unwrap().data(), ordered_checkpoints.first().unwrap().data(),);
    ...
    for request in requests {
        let response = server.get_checkpoint_summary(Request::new(request)).await.unwrap().into_inner();
    ...
    builder.store.inner_mut().update_highest_synced_checkpoint(&latest);

    let request = Request::new(GetCheckpointSummaryRequest::Latest);
    let response = server.get_checkpoint_summary(request).await.unwrap().into_inner().unwrap();
    ...
    for checkpoint in ordered_checkpoints {
        ...
        let request = Request::new(GetCheckpointSummaryRequest::BySequenceNumber(*checkpoint.sequence_number()));
        let response = server.get_checkpoint_summary(request).await.unwrap().into_inner().unwrap();
        assert_eq!(response.data(), checkpoint.data());
    }
}
...
async fn isolated_sync_job() {
    ...
    event_loop_1.peer_heights.write().unwrap().peers.insert(network_2.peer_id(), PeerStateSyncInfo {
        genesis_checkpoint_digest: *ordered_checkpoints[0].digest(),
        on_same_chain_as_us: true,
    ...
    assert_eq!(
        ...
        Some(event_loop_1.store.get_highest_verified_checkpoint().unwrap().data())
    );

    {
        let store = event_loop_1.store.inner();
        let expected = checkpoints.iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
        let actual = store.checkpoints().iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
        assert_eq!(actual, expected);
        assert_eq!(store.checkpoint_sequence_number_to_digest(), &sequence_number_to_digest);
    }
}
...
async fn test_state_sync_using_archive() -> anyhow::Result<()> {
    ...
    let (builder, server) =
        Builder::new().store(SharedInMemoryStore::default()).archive_config(Some(archive_reader_config)).build();
    let network_1 = build_network(|router| router.add_rpc_service(server));
    ...
    // Prune first 10 checkpoint contents from Node 2
    {
        ...
        assert_eq!(store.get_lowest_available_checkpoint(), oldest_checkpoint_to_keep);
        assert_eq!(
            store.get_highest_synced_checkpoint().unwrap().sequence_number,
            ordered_checkpoints.last().unwrap().sequence_number
        );
        assert_eq!(
            store.get_highest_verified_checkpoint().unwrap().sequence_number,
            ordered_checkpoints.last().unwrap().sequence_number
        );
    }
    ...
    loop {
        {
            ...
            if let Some(highest_synced_checkpoint) = store.get_highest_synced_checkpoint() {
                if highest_synced_checkpoint.sequence_number == ordered_checkpoints.last().unwrap().sequence_number {
                    // Node 1 is fully synced to the latest checkpoint on Node 2
                    let expected = checkpoints.iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
                    let actual =
                        store.checkpoints().iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
                    assert_eq!(actual, expected);
                    assert_eq!(store.checkpoint_sequence_number_to_digest(), &sequence_number_to_digest);
                    break;
                }
...
async fn sync_with_checkpoints_being_inserted() {
    ...
    store_1.insert_checkpoint_contents(&checkpoint, empty_contents()).unwrap();
    store_1.insert_certified_checkpoint(&checkpoint);
    handle_1.send_checkpoint(checkpoint).await;

    timeout(Duration::from_secs(1), async {
        assert_eq!(subscriber_1.recv().await.unwrap().data(), ordered_checkpoints[1].data(),);
        assert_eq!(subscriber_2.recv().await.unwrap().data(), ordered_checkpoints[1].data());
    })
    .await
    .unwrap();
    ...
    assert_eq!(
        ...
        store_1.get_highest_verified_checkpoint().as_ref().map(|x| x.digest())
    );
    assert_eq!(
        ordered_checkpoints.last().map(|x| x.digest()),
        store_2.get_highest_verified_checkpoint().as_ref().map(|x| x.digest())
    );

    let expected = checkpoints.iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
    let actual_1 = store_1.checkpoints().iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
    assert_eq!(actual_1, expected);
    assert_eq!(store_1.checkpoint_sequence_number_to_digest(), &sequence_number_to_digest);

    let actual_2 = store_2.checkpoints().iter().map(|(key, value)| (key, value.data())).collect::<HashMap<_, _>>();
    assert_eq!(actual_2, expected);
    assert_eq!(store_2.checkpoint_sequence_number_to_digest(), &sequence_number_to_digest);
}
...
async fn sync_with_checkpoints_watermark() {
    ...
    let last_checkpoint_seq = *ordered_checkpoints.last().cloned().unwrap().sequence_number();
    // Build and connect two nodes
    let (builder, server) = Builder::new().store(SharedInMemoryStore::default()).build();
    let network_1 = build_network(|router| router.add_rpc_service(server));
    let (event_loop_1, handle_1) = builder.build(network_1.clone());
    ...
    peer_heights_1.write().unwrap().set_wait_interval_when_no_peer_to_sync_content(Duration::from_secs(1));
    peer_heights_2.write().unwrap().set_wait_interval_when_no_peer_to_sync_content(Duration::from_secs(1));
    ...
    store_1.insert_checkpoint_contents(&checkpoint_1, contents_1.clone()).unwrap();
    ...
    assert_eq!(store_1.get_highest_synced_checkpoint().unwrap().sequence_number(), checkpoint_seq);
    assert_eq!(store_2.get_highest_synced_checkpoint().unwrap().sequence_number(), checkpoint_seq);
    assert_eq!(store_1.get_highest_verified_checkpoint().unwrap().sequence_number(), &1);
    assert_eq!(store_2.get_highest_verified_checkpoint().unwrap().sequence_number(), &1);

    // So far so good.
    // Now we increase Peer 1's low watermark to a high number.
    let a_very_high_checkpoint_seq = 1000;
    store_1.inner_mut().set_lowest_available_checkpoint(a_very_high_checkpoint_seq);
    ...
    for (checkpoint, contents) in checkpoint_iter.zip(contents_iter) {
        store_1.insert_checkpoint_contents(&checkpoint, contents).unwrap();
        store_1.insert_certified_checkpoint(&checkpoint);
        handle_1.send_checkpoint(checkpoint).await;
    }
    ...
    timeout(Duration::from_secs(1), async {
        for (checkpoint, contents) in ordered_checkpoints[2 ..].iter().zip(contents.clone().into_iter().skip(2)) {
            ...
            store_1.get_full_checkpoint_contents(None, &content_digest).unwrap();
            assert_eq!(store_2.get_full_checkpoint_contents(None, &content_digest), None);
        }
    })
    .await
    .unwrap();
    ...
    assert_eq!(
        store_1.get_highest_synced_checkpoint().unwrap().sequence_number(),
        ordered_checkpoints.last().unwrap().sequence_number()
    );
    assert_eq!(
        store_2.get_highest_synced_checkpoint().unwrap().sequence_number(),
        ordered_checkpoints[1].sequence_number()
    );

    assert_eq!(store_1.get_highest_verified_checkpoint().unwrap().sequence_number(), &last_checkpoint_seq);
    ...
    peer_heights_3.write().unwrap().set_wait_interval_when_no_peer_to_sync_content(Duration::from_secs(1));
    ...
    timeout(Duration::from_secs(1), async {
        assert_eq!(subscriber_3.recv().await.unwrap().data(), ordered_checkpoints[1].data());
        let content_digest = contents[1].clone().into_checkpoint_contents_digest();
        store_3.get_full_checkpoint_contents(None, &content_digest).unwrap();
    })
    .await
    .unwrap();
    ...
    assert_eq!(
        store_3.get_highest_synced_checkpoint().unwrap().sequence_number(),
        ordered_checkpoints[1].sequence_number(),
    );
    ...
    timeout(Duration::from_secs(6), async {
        for (checkpoint, contents) in ordered_checkpoints[2 ..].iter().zip(contents.clone().into_iter().skip(2)) {
            ...
            let content_digest = contents.into_checkpoint_contents_digest();
            store_2.get_full_checkpoint_contents(None, &content_digest).unwrap();
            store_3.get_full_checkpoint_contents(None, &content_digest).unwrap();
        }
    })
    .await
    .unwrap();
    assert_eq!(store_2.get_highest_synced_checkpoint().unwrap().sequence_number(), &last_checkpoint_seq);
    ...
    peer_heights_4.write().unwrap().set_wait_interval_when_no_peer_to_sync_content(Duration::from_secs(1));
    ...
    timeout(Duration::from_secs(3), async {
        for (checkpoint, contents) in ordered_checkpoints[1 ..].iter().zip(contents.clone().into_iter().skip(1)) {
            ...
            let content_digest = contents.into_checkpoint_contents_digest();
            store_4.get_full_checkpoint_contents(None, &content_digest).unwrap();
        }
    })
    .await
    .unwrap();
    assert_eq!(store_4.get_highest_synced_checkpoint().unwrap().sequence_number(), &last_checkpoint_seq);
}
