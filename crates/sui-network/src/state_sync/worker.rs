...
impl<S: WriteStore + Clone + Send + Sync + 'static> Worker for StateSyncWorker<S> {
    ...
    async fn process_checkpoint(&self, checkpoint: &CheckpointData) -> anyhow::Result<()> {
        let verified_checkpoint =
            get_or_insert_verified_checkpoint(&self.0, checkpoint.checkpoint_summary.clone(), true)?;
    ...
    S: WriteStore + Clone,
{
    store
        ...
        .unwrap_or_else(|| {
            ...
            store.insert_checkpoint(&verified_checkpoint).map_err(|e| anyhow!("Failed to insert checkpoint: {e}"))?;
