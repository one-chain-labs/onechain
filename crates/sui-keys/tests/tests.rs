...
async fn alias_exists_test() {
    ...
    keystore.generate(Some("my_alias_test".to_string()), GenerateOptions::default()).await.unwrap();
...
async fn create_alias_keystore_file_test() {
    ...
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
    keystore.generate(None, GenerateOptions::default()).await.unwrap();
...
async fn check_reading_aliases_file_correctly() {
    ...
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
    let GeneratedKey { address, .. } = keystore.generate(None, GenerateOptions::default()).await.unwrap();
    keystore_path.set_extension(ALIASES_FILE_EXTENSION);
    assert!(keystore_path.exists());

    let new_keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path_keep).unwrap());
...
async fn create_alias_if_not_exists_test() {
    ...
    keystore.generate(alias.clone(), GenerateOptions::default()).await.unwrap();
    // test error first
...
async fn update_alias_test() {
    ...
    let _ = keystore.update_alias("my_alias_test", Some("new_alias")).await;
    let aliases = alias_names(keystore.aliases());
    ...
    assert!(keystore.update_alias("new_alias", Some("  o ")).await.is_ok());
    ...
    assert!(keystore.update_alias("my_alias_test", Some(&update)).await.is_err());
}
...
async fn mnemonic_test() {
    let temp_dir = TempDir::new().unwrap();
    let (address, _keypair, scheme, phrase) = generate_new_key(SignatureScheme::ED25519, None, None).unwrap();
...
async fn sui_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    ...
    keystore.import_from_mnemonic(phrase, SignatureScheme::ED25519, None, None).await.unwrap();
...
async fn get_alias_by_address_test() {
    ...
    let keypair = keystore.generate(Some(alias.clone()), GenerateOptions::default()).await.unwrap();
...
async fn remove_key_test() {
    ...
    let GeneratedKey { address, .. } =
        keystore.generate(Some("test_key".to_string()), GenerateOptions::default()).await.unwrap();
...
fn alias_names(aliases: Vec<&Alias>) -> Vec<&str> {
    aliases.into_iter().map(|alias| alias.alias.as_str()).collect()
}
