...
use sui_types::{
    ...
    crypto::{PublicKey, Signature, SuiKeyPair, SuiSignature, SuiSignatureInner},
};
use tokio::process::Command;

use crate::{
    keystore::{validate_alias, AccountKeystore, Alias, GenerateOptions, GeneratedKey, ALIASES_FILE_EXTENSION},
...
impl CommandRunner for StdCommandRunner {
    async fn run(&self, command: &str, method: &str, args: JsonValue) -> Result<JsonValue, Error> {
        let mut cmd = Command::new(command).arg("call").stdin(Stdio::piped()).stdout(Stdio::piped()).spawn()?;

        let mut endpoint = Endpoint::new(cmd.stdout.take().expect("No stdout"), cmd.stdin.take().expect("No stdin"));
        ...
        let output = cmd.wait_with_output().await.map_err(|e| anyhow!("Failed to wait for command to finish: {}", e))?;

        if !output.status.success() {
            return Err(Error::msg(format!("Command failed with status: {}", output.status)));
        }
...
impl External {
    ...
    pub fn load_or_create(path: &PathBuf) -> Result<Self, Error> {
        ...
        let aliases: BTreeMap<SuiAddress, Alias> = if aliases_store_directory.exists() {
            ...
            serde_json::from_str(&aliases_store).map_err(|e| anyhow!("Failed to parse aliases file: {}", e))?
        } else {
            BTreeMap::default()
        };

        let keys: BTreeMap<SuiAddress, StoredKey> = if path.exists() {
            let keys_store: String =
                std::fs::read_to_string(path).map_err(|e| anyhow!("Failed to read keys file: {}", e))?;
            serde_json::from_str(&keys_store).map_err(|e| anyhow!("Failed to parse keys file: {}", e))?
        } else {
            BTreeMap::default()
        };

        Ok(Self { aliases, keys, command_runner: Box::new(StdCommandRunner), path: Some(path.clone()) })
    }
    ...
    pub fn new_for_test(command_runner: Box<dyn CommandRunner>, path: Option<PathBuf>) -> Self {
        Self { aliases: BTreeMap::default(), keys: BTreeMap::default(), command_runner, path }
    }

    /// Execute a command against the command runner
    pub async fn exec(&self, command: &str, method: &str, args: JsonValue) -> Result<JsonValue, Error> {
        self.command_runner.run(command, method, args).await
    }

    /// Add a Key ID from the given an external signer to the Sui CLI index
    pub async fn add_existing(&mut self, ext_signer: String, key_id: String) -> Result<StoredKey, Error> {
        let keys = self.signer_available_keys(ext_signer.clone()).await?;

        let key: StoredKey = keys
            .into_iter()
            .find(|k| k.key_id == key_id)
            .ok_or_else(|| anyhow!("Key with id {} not found for external signer {}", key_id, ext_signer))?;
    ...
    pub async fn signer_available_keys(&self, ext_signer: String) -> Result<Vec<StoredKey>, Error> {
        ...
        let keys_response: KeysResponse =
            serde_json::from_value(result).map_err(|e| anyhow!("Failed to parse keys response: {}", e))?;
    ...
    pub async fn save_aliases(&self) -> Result<(), Error> {
        if let Some(path) = &self.path {
            let aliases_store: String =
                serde_json::to_string_pretty(&self.aliases).map_err(|e| anyhow!("Serialization error: {}", e))?;

            let mut aliases_path = path.clone();
            aliases_path.set_extension(ALIASES_FILE_EXTENSION);
            tokio::task::spawn_blocking(move || std::fs::write(aliases_path, aliases_store)).await?.with_context(
                || format!("Cannot write aliases to file: {}", path.with_extension(ALIASES_FILE_EXTENSION).display()),
            )?;
            Ok(())
        } else {
    ...
    pub async fn save_keystore(&self) -> Result<(), Error> {
        if let Some(path) = &self.path {
            let store: String =
                serde_json::to_string_pretty(&self.keys).map_err(|e| anyhow!("Serialization error: {}", e))?;
...
impl Serialize for External {
    ...
    where
        S: Serializer,
    {
        serializer.serialize_str(self.path.as_ref().unwrap_or(&PathBuf::default()).to_str().unwrap_or(""))
    }
}
...
impl<'de> Deserialize<'de> for External {
    ...
    {
        use serde::de::Error;
        External::load_or_create(&PathBuf::from(String::deserialize(deserializer)?)).map_err(D::Error::custom)
    }
}

#[async_trait]
impl AccountKeystore for External {
    async fn generate(&mut self, alias: Option<String>, opts: GenerateOptions) -> Result<GeneratedKey, Error> {
        ...
        let ExternalKey { key_id, public_key } =
            serde_json::from_value(res).map_err(|e| anyhow!("Failed to parse key response: {}", e))?;
        let address: SuiAddress = (&public_key).into();

        self.keys.insert(address, StoredKey { public_key: public_key.clone(), ext_signer, key_id: key_id.to_string() });

        self.aliases
            .insert(address, Alias { alias: self.create_alias(alias)?, public_key_base64: public_key.encode_base64() });
    ...
    async fn sign_hashed(&self, address: &SuiAddress, msg: &[u8]) -> Result<Signature, signature::Error> {
        let StoredKey { key_id, ext_signer, public_key } = self
            .keys
            .get(address)
            .ok_or_else(|| signature::Error::from_source(anyhow!("Key corresponding to {address} not found")))?
            .clone();

        let sign_request: SignRequest = SignRequest { key_id, msg: general_purpose::STANDARD.encode(msg), intent: None };
        let result = self
            .exec(
                &ext_signer,
                "sign_hashed",
                serde_json::to_value(&sign_request)
                    .map_err(|e| signature::Error::from_source(anyhow!("Failed to serialize sign request: {}", e)))?,
            )
            .await
            .map_err(signature::Error::from_source)?;

        let result: SignResponse = serde_json::from_value(result)
            .map_err(|e| signature::Error::from_source(anyhow!("Failed to parse sign response: {}", e)))?;

        let address = SuiAddress::from(&public_key);
        match &result.signature {
            Signature::Ed25519SuiSignature(s) => {
                let (sig, pk) = s
                    .get_verification_inputs()
                    .map_err(|e| signature::Error::from_source(anyhow!("Failed to get verification inputs: {}", e)))?;
                ...
                pk.verify(msg, &sig)
                    .map_err(|e| signature::Error::from_source(anyhow!("Signature verification failed: {}", e)))?;
            }
    ...
    async fn sign_secure<T>(&self, address: &SuiAddress, msg: &T, intent: Intent) -> Result<Signature, signature::Error>
    where
        T: Serialize + Sync,
    {
        let StoredKey { key_id, ext_signer, public_key } = self
        ...
        let msg_bcs = general_purpose::STANDARD.encode(
            &bcs::to_bytes(&msg)
                .map_err(|e| signature::Error::from_source(anyhow!("Failed to serialize message: {}", e)))?,
        );

        let sign_request = SignRequest { key_id, msg: msg_bcs, intent: Some(intent) };
        ...
        if let Err(e) = result.signature.verify_secure(&intent_message, *address, public_key.scheme()) {
            return Err(signature::Error::from_source(anyhow!("Signature verification failed: {}", e)));
        }
    ...
    fn create_alias(&self, alias: Option<String>) -> Result<String, Error> {
        match alias {
            ...
            None => Ok(random_name(&self.aliases().into_iter().map(|x| x.alias.to_string()).collect::<HashSet<_>>())),
        }
    }

    async fn update_alias(&mut self, old_alias: &str, new_alias: Option<&str>) -> Result<String, Error> {
...
mod tests {
    ...
    use crate::{
        ...
        keystore::{AccountKeystore, GenerateOptions, GeneratedKey},
    };
    ...
    async fn test_serialize() {
        ...
        // Add key
        external.keys.insert(SuiAddress::from_str(ADDRESS).unwrap(), StoredKey {
            public_key: PublicKey::decode_base64(PUBLIC_KEY).unwrap(),
            ext_signer: "signer".to_string(),
            key_id: "key-123".to_string(),
        });
        // Add alias
        external.aliases.insert(SuiAddress::from_str(ADDRESS).unwrap(), crate::keystore::Alias {
        ...
        // malformed keystore
        let external_bad_keystore_path = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap())
            .join("src")
            .join("unit_tests")
            .join("fixtures")
            .join("external_config")
        ...
        assert!(external_bad.is_err(), "Expected error when loading from bad keystore path");
        ...
        assert!(external_bad_aliases.is_err(), "Expected error when loading from bad aliases path");
    }
    ...
    async fn test_exec() {
        ...
        mock.expect_run()
            .with(eq("sui-key-tool"), eq("test_method"), eq(json!(["arg1", "arg2"])))
            .times(1)
            .returning(|_, _, _| Ok(JsonValue::Null));

        let external = External::new_for_test(Box::new(mock), None);
        let args = json!(["arg1", "arg2"]);
        assert!(external.exec("sui-key-tool", "test_method", args).await.is_ok());
    }
    ...
    async fn test_create_key_success() {
        let mut mock = MockCommandRunner::new();
        let key_id = "key-123";
        mock.expect_run().with(eq("signer"), eq("create_key"), eq(json![null])).returning(move |_, _, _| {
            Ok(json!({
                "key_id": key_id,
                "public_key": {
        ...
        let result = external.generate(None, GenerateOptions::ExternalSigner("signer".to_string())).await;
        assert!(result.is_ok());
        let GeneratedKey { address, public_key, scheme } = result.unwrap();
        ...
        let secp256k1_key = SuiKeyPair::Secp256k1(Secp256k1KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    ...
    async fn test_add_existing_key() {
        ...
        external.save().await.unwrap();
        external.add_existing("signer".to_string(), key_id.to_string()).await.unwrap();
    ...
    async fn test_add_keypair_not_supported() {
        ...
        let result = external.import(None, SuiKeyPair::Ed25519(ed25519_keypair)).await;
    ...
    async fn test_add_existing_key_not_found() {
        let mut mock = MockCommandRunner::new();
        mock.expect_run().returning(|_, _, _| Ok(json!({"keys": []})));
        let mut external = External::new_for_test(Box::new(mock), None);
        let result = external.add_existing("signer".to_string(), "missing-key-id".to_string()).await;
        assert!(result.is_err());
    }
    ...
    async fn test_keys_parsing() {
        ...
        let external = External::new_for_test(Box::new(mock), None);
        let keys = external.signer_available_keys("signer".to_string()).await.unwrap();
    ...
    async fn test_sign_hashed() {
        ...
        let stored_key = StoredKey { public_key, ext_signer: "signer".to_string(), key_id: "id".to_string() };
    ...
    async fn test_update_alias() {
        ...
        let updated_alias = external.update_alias(&old_alias, Some(&new_alias)).await.unwrap();
