...
use crate::{benchmark_context::BenchmarkContext, mock_account::Account, tx_generator::TxGenerator};
...
impl PackagePublishTxGenerator {
    pub async fn new(ctx: &mut BenchmarkContext, manifest_path: PathBuf) -> Self {
        ...
        let PackageDependencyManifest { dependencies, root_package } = manifest;
        let mut dep_map = BTreeMap::new();
        for dependency in dependencies {
            let Package { name, path, is_source_code } = dependency;

            info!("Publishing dependent package {}", name);
            let target_path = dir.join(&path);
            let module_bytes = if is_source_code {
                let compiled_package =
                    BuildConfig::new_for_testing_replace_addresses(vec![(name.clone(), ObjectID::ZERO)])
                        .build(&target_path)
                        .unwrap();
                ...
                let module_dir = target_path.join("build").join(package_name).join("bytecode_modules");
            ...
            let package_id = ctx.publish_package(PublishData::ModuleBytes(module_bytes)).await.0;
            info!("Published dependent package {}", package_id);
            dep_map.insert(Symbol::from(name), package_id);
        }

        let Package { name, path, is_source_code } = root_package;

        info!("Compiling root package {}", name);
        assert!(is_source_code, "Only support building root package from source code");
        ...
        let mut compiled_package =
            BuildConfig::new_for_testing_replace_addresses(dep_map.into_iter().map(|(k, v)| (k.to_string(), v)))
                .build(&target_path)
                .unwrap();
...
fn load_manifest_json(file_path: &PathBuf) -> PackageDependencyManifest {
    let data = fs::read_to_string(file_path).unwrap_or_else(|_| panic!("Unable to read file at: {:?}", file_path));
    let parsed_data: PackageDependencyManifest =
        serde_json::from_str(&data).unwrap_or_else(|_| panic!("Unable to parse json from file at: {:?}", file_path));
