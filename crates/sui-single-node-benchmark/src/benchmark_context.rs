        ...
        if is_consensus_tx {
            ...
            let tasks: FuturesUnordered<_> = transactions
                ...
                .map(|tx| {
                    ...
                    tokio::spawn(async move {
                        validator.execute_certificate(tx, &AssignedVersions::non_withdraw(vec![]), component).await
                    })
                })
                .collect();
            let results: Vec<_> = tasks.collect().await;
        ...
        if is_consensus_tx {
            ...
            let tasks: FuturesUnordered<_> = transactions
                ...
                .map(|tx| {
                    ...
                    let validator = self.validator();
                    tokio::spawn(async move {
                        validator.execute_transaction_in_memory(store, tx, &AssignedVersions::non_withdraw(vec![])).await
                    })
                })
                .collect();
            let results: Vec<_> = tasks.collect().await;
