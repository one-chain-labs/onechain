...
use sui_core::{
    authority::shared_object_version_manager::{AssignedTxAndVersions, AssignedVersions},
...
impl BenchmarkContext {
    pub(crate) async fn new(workload: Workload, benchmark_component: Component, print_sample_tx: bool) -> Self {
        ...
        info!("Creating {} accounts and {} gas objects", num_accounts, total);
        ...
        Self { validator, user_accounts, admin_account, benchmark_component }
    }
    ...
    pub(crate) async fn publish_package(&mut self, publish_data: PublishData) -> ObjectRef {
        ...
        let (package, updated_gas) = self
            .validator
            .publish_package(publish_data, self.admin_account.sender, &self.admin_account.keypair, gas_objects[0])
            .await;
    ...
    pub(crate) async fn preparing_dynamic_fields(
        ...
        let root_object_create_transactions = self
            .generate_transactions(Arc::new(RootObjectCreateTxGenerator::new(move_package, num_dynamic_fields)))
            .await;
        ...
        for effects in results {
            let batch = cache_commit.build_db_batch(effects.executed_epoch(), &[*effects.transaction_digest()]);

            cache_commit.commit_transaction_outputs(effects.executed_epoch(), batch, &[*effects.transaction_digest()]);

            let (owner, root_object) = effects
                .created()
                .into_iter()
                .filter_map(|(oref, owner)| owner.get_address_owner_address().ok().map(|owner| (owner, oref)))
                .next()
                .unwrap();
    ...
    pub(crate) async fn prepare_shared_objects(
        ...
        if matches!(self.benchmark_component, Component::ValidatorWithoutConsensus) {
            warn!(
        ...
        let shared_object_create_transactions: Vec<_> = self
            ...
            .collect();
        let results = self.execute_raw_transactions(shared_object_create_transactions).await;
        let mut new_gas_objects = HashMap::new();
        ...
        for effects in results {
            let shared_object = effects
                .created()
                .into_iter()
                .filter_map(|(oref, owner)| if owner.is_shared() { Some((oref.0, oref.1)) } else { None })
                .next()
                .unwrap();
    ...
    pub(crate) async fn generate_transactions(&self, tx_generator: Arc<dyn TxGenerator>) -> Vec<Transaction> {
        info!("{}: Creating {} transactions", tx_generator.name(), self.user_accounts.len());
            ...
            .map(|tx| {
                ...
                tokio::spawn(async move {
                    ...
                    let sig = if skip_signing {
                        SignedTransaction::sign(0, &tx, &*validator_state.secret, validator_state.name)
                    } else {
    ...
    pub(crate) async fn benchmark_transaction_execution(
        ...
        if print_sample_tx {
            ...
            self.execute_sample_transaction(transactions[0].clone()).await;
        }

        let tx_count = transactions.len();
        let start_time = std::time::Instant::now();
        info!("Started executing {} transactions. You can now attach a profiler", transactions.len());
        ...
        if is_consensus_tx {
            ...
            for transaction in transactions {
                ...
                self.validator
                    .execute_certificate(transaction, assigned_versions.get(&key).unwrap(), self.benchmark_component)
                    .await;
            }
        } else {
            let tasks: FuturesUnordered<_> = transactions
                ...
                .map(|tx| {
                    ...
                    tokio::spawn(async move {
                        validator.execute_certificate(tx, &AssignedVersions::new(vec![], None), component).await
                    })
                })
                .collect();
            let results: Vec<_> = tasks.collect().await;
        ...
        info!("Execution finished in {}s, TPS={}", elapsed, tx_count as f64 / elapsed);
    }
    ...
    pub(crate) async fn benchmark_transaction_execution_in_memory(
        ...
        self.execute_transactions_in_memory(in_memory_store.clone(), transactions, assigned_versions).await;
    ...
    async fn execute_sample_transaction(&self, sample_transaction: CertifiedTransaction) {
        info!("Sample transaction digest={:?}: {:?}", sample_transaction.digest(), sample_transaction.data());
        let effects = self.validator().execute_dry_run(sample_transaction.into_unsigned()).await;
        info!("Sample effects: {:?}\n\n", effects);
        assert!(effects.status().is_ok());
    }

    /// Benchmark parallel signing a vector of transactions and measure the TPS.
    pub(crate) async fn benchmark_transaction_signing(&self, transactions: Vec<Transaction>, print_sample_tx: bool) {
        if print_sample_tx {
        ...
        let elapsed = start_time.elapsed().as_millis() as f64 / 1000f64;
        info!("Transaction signing finished in {}s, TPS={}.", elapsed, tx_count as f64 / elapsed,);
    }
    ...
    pub(crate) async fn benchmark_checkpoint_executor(
        ...
        let effects: BTreeMap<_, _> = self
            .execute_transactions_in_memory(in_memory_store.clone(), transactions.clone(), assigned_versions)
            .await
            .into_iter()
    ...
    async fn execute_raw_transactions(&self, transactions: Vec<Transaction>) -> Vec<TransactionEffects> {
        let tasks: FuturesUnordered<_> = transactions
            .into_iter()
            .map(|tx| {
                let validator = self.validator();
    ...
    async fn execute_transactions_in_memory(
        ...
        if is_consensus_tx {
            ...
            for transaction in transactions {
                ...
                effects.push(
                    self.validator.execute_transaction_in_memory(store.clone(), transaction, assigned_versions).await,
                );
            }
            effects
        } else {
            let tasks: FuturesUnordered<_> = transactions
                ...
                .map(|tx| {
                    ...
                    let validator = self.validator();
                    tokio::spawn(async move {
                        validator.execute_transaction_in_memory(store, tx, &AssignedVersions::new(vec![], None)).await
                    })
                })
                .collect();
            let results: Vec<_> = tasks.collect().await;
    ...
    fn refresh_gas_objects(&mut self, mut new_gas_objects: HashMap<ObjectID, ObjectRef>) {
        ...
        for account in self.user_accounts.values_mut() {
            let refreshed_gas_objects: Vec<_> = account
                .gas_objects
                .iter()
                .map(|oref| if let Some(new_oref) = new_gas_objects.remove(&oref.0) { new_oref } else { *oref })
                .collect();
    ...
    pub(crate) async fn validator_sign_transactions(
        ...
        info!("Started signing {} transactions. You can now attach a profiler", transactions.len(),);
        let tasks: FuturesUnordered<_> = transactions
            .into_iter()
            .map(|tx| {
                let validator = self.validator();
