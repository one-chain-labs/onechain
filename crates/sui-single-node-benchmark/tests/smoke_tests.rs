...
async fn benchmark_publish_from_source() {
    ...
    path.extend(["tests", "data", "package_publish_from_source", "manifest.json"]);
    for component in Component::iter() {
        run_benchmark(
            Workload::new(10, WorkloadKind::Publish { manifest_file: path.clone() }),
            component,
            1000,
            false,
            false,
...
async fn benchmark_publish_from_bytecode() {
    // This test makes sure that the benchmark runs.
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.extend(["tests", "data", "package_publish_from_bytecode", "manifest.json"]);
    for component in Component::iter() {
        run_benchmark(
            Workload::new(10, WorkloadKind::Publish { manifest_file: path.clone() }),
            component,
