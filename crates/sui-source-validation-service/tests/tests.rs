...
use sui_sdk::{
    rpc_types::{OwnedObjectRef, SuiObjectDataOptions, SuiObjectResponseQuery, SuiTransactionBlockEffectsV1},
...
async fn test_end_to_end() -> anyhow::Result<()> {
    ...
    let mut test_cluster = TestClusterBuilder::new().with_fullnode_rpc_port(LOCALNET_PORT).build().await;
    ...
    let app_state = Arc::new(RwLock::new(AppState { sources, metrics: None, sources_list }));
    let app_state_ref = app_state.clone();
    let (tx, rx) = oneshot::channel();
    tokio::spawn(async move { watch_for_upgrades(config.packages, app_state, Network::Localnet, Some(tx)).await });
    ...
    let upgrade_pkg_path = copy_with_published_at_manifest(&package_path, &tmp_dir.path().to_path_buf(), package_id);
    // Run the upgrade.
    run_upgrade(upgrade_pkg_path, cap, context, gas_obj_id, rgp).await?;

    // Test expects to observe an upgrade transaction.
    let Ok(SuiTransactionBlockEffects::V1(effects)) = rx.await else { panic!("No upgrade transaction observed") };
    ...
    let truncated_error_message =
        &result.unwrap_err().to_string().lines().take(3).map(|s| s.into()).collect::<Vec<String>>().join("\n");
    let expected = expect![
        r#"
...
fn copy_with_published_at_manifest(source_path: &PathBuf, dest_path: &PathBuf, package_id: ObjectID) -> PathBuf {
    fs_extra::dir::copy(source_path, dest_path, &fs_extra::dir::CopyOptions::default()).unwrap();
    ...
    lines.insert(idx + 1, format!("published-at = \"{}\"", package_id.to_hex_uncompressed()));
    let new = lines.join("\n");
...
async fn test_api_route() -> anyhow::Result<()> {
    ...
    source_lookup
        .insert(Symbol::from(module), SourceInfo { path: source_path, source: Some("module address {...}".to_owned()) });
    ...
    let json = client
        .get(format!("http://{}/api?address={address}&module={module}&network=localnet", host_port()))
        .send()
        .await
    ...
    // check /list route
    let response = client.get(format!("http://{}/api/list", host_port())).send().await?.text().await?;
    ...
    let expected = expect!["Unsupported version 'bogus' specified in header x-sui-source-validation-version"];
...
async fn test_metrics_route() -> anyhow::Result<()> {
    ...
    let client = Client::new();
    let response =
        client.get(format!("http://{METRICS_HOST_PORT}/metrics")).send().await.expect("Request failed.").text().await?;
...
fn test_clone_command() -> anyhow::Result<()> {
    let source = RepositorySource {
        ...
        branches: vec![Branch {
            branch: "main".into(),
            paths: vec![Package { path: "a".into(), watch: None }, Package { path: "b".into(), watch: None }],
        }],
        network: Some(Network::Localnet),
    };

    let command = CloneCommand::new(&source, &source.branches[0], PathBuf::from("/foo").as_path())?;
    let expect = expect![
        r#"CloneCommand {
    args: [
        [
