...
impl IndexerProgressStore for PgBridgePersistent {
    async fn load_progress(&self, task_name: String) -> anyhow::Result<u64> {
        let mut conn = self.pool.get().await?;
        let cp =
            dsl::progress_store.find(&task_name).select(ProgressStore::as_select()).first(&mut conn).await.optional()?;
    ...
    async fn register_live_task(&mut self, task_name: String, start_checkpoint: u64) -> Result<(), Error> {
        let mut conn = self.pool.get().await?;
        diesel::insert_into(schema::progress_store::table)
            .values(ProgressStore {
                task_name,
    ...
    async fn update_task(&mut self, task: Task) -> Result<(), anyhow::Error> {
        let mut conn = self.pool.get().await?;
        diesel::update(QueryDsl::filter(dsl::progress_store, columns::task_name.eq(task.task_name)))
            .set((
