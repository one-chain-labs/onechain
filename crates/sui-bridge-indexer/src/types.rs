...
use sui_json_rpc_types::{SuiTransactionBlockEffects, SuiTransactionBlockEvents, SuiTransactionBlockResponse};
...
impl TryFrom<SuiTransactionBlockResponse> for RetrievedTransaction {
    ...
    fn try_from(response: SuiTransactionBlockResponse) -> Result<Self, Self::Error> {
        Ok(RetrievedTransaction {
            ...
            events: response.events.ok_or(anyhow::anyhow!("missing events in responses"))?,
            checkpoint: response.checkpoint.ok_or(anyhow::anyhow!("missing checkpoint in responses"))?,
            timestamp_ms: response.timestamp_ms.ok_or(anyhow::anyhow!("missing timestamp_ms in responses"))?,
