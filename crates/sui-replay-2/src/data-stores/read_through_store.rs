    ...
    S: ReadDataStore,
{
    fn transaction_data_and_effects(&self, tx_digest: &str) -> Result<Option<TransactionInfo>, anyhow::Error> {
        match self.primary.transaction_data_and_effects(tx_digest)? {
            ...
            None => self.secondary.transaction_data_and_effects(tx_digest)?.map_or(Ok(None), |info| {
    ...
    S: ReadDataStore,
{
    fn get_objects(&self, keys: &[ObjectKey]) -> Result<Vec<Option<(Object, u64)>>, anyhow::Error> {
        ...
        if !keys_to_fetch.is_empty() {
            ...
            for ((idx, key), fetched) in none_object_idx.iter().zip(keys_to_fetch.iter()).zip(fetched_objects.iter()) {
                ...
                if let Some((object, actual_version)) = fetched {
                    self.primary.write_object(key, object.clone(), *actual_version)?;
