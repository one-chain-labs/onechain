        ...
        for key in keys.iter() {
            let (object_and_version, hit_ctr, miss_ctr) = match &key.version_query {
                VersionQuery::Version(version) => {
                    let res = inner.object_cache.get(&(key.object_id, *version)).cloned().map(|obj| (obj, *version));
                ...
                VersionQuery::RootVersion(max_version) => {
                    ...
                    let res = actual_version
                        .and_then(|v| inner.object_cache.get(&(key.object_id, v)).cloned().map(|obj| (obj, v)));
...
impl TransactionStoreWriter for LruMemoryStore {
    fn write_transaction(&self, tx_digest: &str, transaction_info: TransactionInfo) -> Result<(), anyhow::Error> {
        self.0.write().unwrap().transaction_cache.put(tx_digest.to_string(), transaction_info);
        Ok(())
    }
}
...
impl EpochStoreWriter for LruMemoryStore {
    fn write_epoch_info(&self, epoch: u64, epoch_data: EpochData) -> Result<(), anyhow::Error> {
        self.0.write().unwrap().epoch_data_cache.put(epoch, epoch_data);
        Ok(())
    }
}
...
impl ObjectStoreWriter for LruMemoryStore {
    fn write_object(&self, key: &ObjectKey, object: Object, actual_version: u64) -> Result<(), anyhow::Error> {
        ...
        inner.object_cache.put((key.object_id, actual_version), object);

        // Handle version mappings based on query type
        match &key.version_query {
            VersionQuery::Version(_) => {}
            VersionQuery::RootVersion(max_version) => {
                inner.root_version_cache.put((key.object_id, *max_version), actual_version);
            }
            VersionQuery::AtCheckpoint(checkpoint) => {
                inner.checkpoint_cache.put((key.object_id, *checkpoint), actual_version);
            }
        }
