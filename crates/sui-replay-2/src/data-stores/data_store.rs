...
impl TransactionStore for DataStore {
    fn transaction_data_and_effects(&self, digest: &str) -> Result<Option<TransactionInfo>, anyhow::Error> {
        match block_on!(self.transaction(digest)) {
            Ok(Some((data, effects, checkpoint))) => {
                self.metrics.txn_hit.fetch_add(1, Ordering::Relaxed);
                Ok(Some(TransactionInfo { data, effects, checkpoint }))
            }
            Ok(None) => {
...
impl EpochStore for DataStore {
    fn epoch_info(&self, epoch: u64) -> Result<Option<EpochData>, anyhow::Error> {
        ...
        match block_on!(self.epoch(epoch)) {
            Ok(Some(epoch_data)) => {
                self.epoch_map.write().unwrap().insert(epoch, epoch_data.clone());
                self.metrics.epoch_hit.fetch_add(1, Ordering::Relaxed);
                Ok(Some(epoch_data))
            }
            Ok(None) => {
    ...
    fn protocol_config(&self, epoch: u64) -> Result<Option<ProtocolConfig>, anyhow::Error> {
        match self.epoch_info(epoch) {
            Ok(Some(epoch)) => {
                ...
                Ok(Some(ProtocolConfig::get_for_version(ProtocolVersion::new(epoch.protocol_version), self.chain())))
            }
            Ok(None) => {
        ...
        match block_on!(self.objects(keys)) {
                    ...
                    match key.version_query {
                        VersionQuery::Version(_) => {
                            if res.is_some() {
                                ...
                                self.metrics.obj_version_miss.fetch_add(1, Ordering::Relaxed);
                            }
                        }
                        ...
                        VersionQuery::AtCheckpoint(_) => {
                            if res.is_some() {
                                self.metrics.obj_checkpoint_hit.fetch_add(1, Ordering::Relaxed);
                            } else {
                                self.metrics.obj_checkpoint_miss.fetch_add(1, Ordering::Relaxed);
                            }
                        }
                    }
                }
            ...
            Err(e) => {
                ...
                for key in keys.iter() {
                    match key.version_query {
                        VersionQuery::Version(_) => {
                            self.metrics.obj_version_error.fetch_add(1, Ordering::Relaxed);
                        }
                        VersionQuery::RootVersion(_) => {
                            self.metrics.obj_root_error.fetch_add(1, Ordering::Relaxed);
                        }
                        VersionQuery::AtCheckpoint(_) => {
                            self.metrics.obj_checkpoint_error.fetch_add(1, Ordering::Relaxed);
                        }
                    }
                }
                Err(e)
...
impl DataStore {
    pub fn new(node: Node, version: &str) -> Result<Self, anyhow::Error> {
        ...
        let rpc = reqwest::Url::parse(url).context(format!("Failed to parse GQL RPC URL {}", url))?;
        let epoch_map = RwLock::new(BTreeMap::new());
        debug!("End stores creation");

        Ok(Self { client, node, epoch_map, rpc, version: version.to_string(), metrics: DataStoreMetrics::default() })
    }
    ...
    pub(crate) async fn run_query<T, V>(&self, operation: &Operation<T, V>) -> Result<GraphQlResponse<T>, anyhow::Error>
    where
    ...
    async fn transaction(
        ...
        debug!(op = "txn_query", phase = "end", elapsed_ms = elapsed, "transaction query");
        data
    }
    ...
    async fn epoch(&self, epoch_id: u64) -> Result<Option<EpochData>, anyhow::Error> {
        ...
        debug!(op = "epoch_query", phase = "end", elapsed_ms = elapsed, "epoch query");
        data
    }

    async fn objects(&self, keys: &[ObjectKey]) -> Result<Vec<Option<(Object, u64)>>, anyhow::Error> {
        ...
        debug!(op = "objects_query", phase = "end", elapsed_ms = elapsed, "objects query");
        Ok(data)
    }
}
...
impl StoreSummary for DataStore {
    fn summary<W: std::io::Write>(&self, w: &mut W) -> anyhow::Result<()> {
        ...
        writeln!(w, "    Epochs: {} entries", self.epoch_map.read().unwrap().len())?;
        ...
        writeln!(w, "      Version:     hit={} miss={}", obj_v_hit, obj_v_miss)?;
        writeln!(w, "      RootVersion: hit={} miss={}", obj_r_hit, obj_r_miss)?;
        writeln!(w, "      Checkpoint:  hit={} miss={}", obj_c_hit, obj_c_miss)?;
        Ok(())
    }
}
