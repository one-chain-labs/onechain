...
impl InMemoryStore {
    ...
    pub fn cache_stats(&self) -> CacheStats {
        ...
        let object_cache_size = inner.object_cache.values().map(|versions| versions.len()).sum();
    ...
    pub fn add_transaction_data(&self, tx_digest: String, transaction_info: TransactionInfo) {
        self.0.write().unwrap().transaction_cache.insert(tx_digest, transaction_info);
    }

    /// Add epoch data to the cache
    pub fn add_epoch_data(&self, epoch: u64, epoch_data: EpochData) {
        self.0.write().unwrap().epoch_data_cache.insert(epoch, epoch_data);
    }

    /// Add object data to the cache
    pub fn add_object_data(&self, object_id: ObjectID, version: u64, object: Object) {
        self.0.write().unwrap().object_cache.entry(object_id).or_default().insert(version, object);
    }
}
            ...
            let (object_and_version, hit_ctr, miss_ctr) = match &key.version_query {
                VersionQuery::Version(version) => {
                    let res = if let Some(versions_map) = inner.object_cache.get(&key.object_id) {
                        versions_map.get(version).cloned().map(|obj| (obj, *version))
                    } else {
                        None
                    };
                    (res, &inner.metrics.obj_version_hit, &inner.metrics.obj_version_miss)
                }
                VersionQuery::RootVersion(max_version) => {
                    let actual_version = inner.root_version_cache.get(&(key.object_id, *max_version)).copied();
                    ...
                    (res, &inner.metrics.obj_root_hit, &inner.metrics.obj_root_miss)
                }
                VersionQuery::AtCheckpoint(checkpoint) => {
                    let actual_version = inner.checkpoint_cache.get(&(key.object_id, *checkpoint)).copied();
                    let res = actual_version.and_then(|actual_version| {
                        inner
                            .object_cache
                            .get(&key.object_id)
                    ...
                    (res, &inner.metrics.obj_checkpoint_hit, &inner.metrics.obj_checkpoint_miss)
                }
            };
...
impl TransactionStoreWriter for InMemoryStore {
    fn write_transaction(&self, tx_digest: &str, transaction_info: TransactionInfo) -> Result<(), anyhow::Error> {
        self.0.write().unwrap().transaction_cache.insert(tx_digest.to_string(), transaction_info);
        Ok(())
    }
}
...
impl ObjectStoreWriter for InMemoryStore {
    fn write_object(&self, key: &ObjectKey, object: Object, actual_version: u64) -> Result<(), anyhow::Error> {
        ...
        inner.object_cache.entry(key.object_id).or_default().insert(actual_version, object);
        ...
        match &key.version_query {
            ...
            VersionQuery::RootVersion(max_version) => {
                inner.root_version_cache.insert((key.object_id, *max_version), actual_version);
            }
            VersionQuery::AtCheckpoint(checkpoint) => {
                inner.checkpoint_cache.insert((key.object_id, *checkpoint), actual_version);
            }
        }
...
impl StoreSummary for InMemoryStore {
    fn summary<W: std::io::Write>(&self, w: &mut W) -> anyhow::Result<()> {
        ...
        writeln!(w, "    Transactions: {} entries", stats.transaction_cache_size)?;
        ...
        writeln!(w, "    Checkpoint map: {} entries", stats.checkpoint_cache_size)?;
