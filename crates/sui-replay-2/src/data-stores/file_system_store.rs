...
impl FileSystemStore {
    pub fn new(node: Node) -> Result<Self, anyhow::Error> {
        let home_dir = std::env::var("REPLAY_STORE")
            ...
            .map_err(|_| anyhow!("Cannot determine home directory. Define a REPLAY_STORE environment variable"))?;
    ...
    fn get_chain_id_for_node(&self) -> Result<String, anyhow::Error> {
        ...
        let file = fs::File::open(&mapping_file)
            .with_context(|| format!("Failed to open node mapping file: {}", mapping_file.display()))?;
        ...
        for result in rdr.records() {
            let record =
                result.with_context(|| format!("Failed to read CSV record from: {}", mapping_file.display()))?;
        ...
        Err(anyhow::anyhow!("No mapping found for node '{}' in mapping file {}", node_key, mapping_file.display()))
    }
    ...
    fn root_versions_path(&self, object_id: &ObjectID) -> Result<PathBuf, anyhow::Error> {
        Ok(self.objects_dir()?.join(object_id.to_string()).join("root_versions"))
    }

    fn checkpoint_versions_path(&self, object_id: &ObjectID) -> Result<PathBuf, anyhow::Error> {
        Ok(self.objects_dir()?.join(object_id.to_string()).join("checkpoint_versions"))
    }

    fn read_bcs_file<T: serde::de::DeserializeOwned>(&self, path: &Path) -> Result<T, anyhow::Error> {
        let bytes = fs::read(path).with_context(|| format!("Failed to read file: {}", path.display()))?;
        bcs::from_bytes(&bytes).with_context(|| format!("Failed to deserialize BCS data from: {}", path.display()))
    }

    fn write_bcs_file<T: serde::Serialize>(&self, path: &Path, data: &T) -> Result<(), anyhow::Error> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).with_context(|| format!("Failed to create directory: {}", parent.display()))?;
        }
        let bytes =
            bcs::to_bytes(data).with_context(|| format!("Failed to serialize BCS data for: {}", path.display()))?;
    ...
    fn read_version_mapping(&self, path: &Path) -> Result<BTreeMap<u64, u64>, anyhow::Error> {
        ...
        let file =
            fs::File::open(path).with_context(|| format!("Failed to open version mapping file: {}", path.display()))?;

        let mut rdr = csv::ReaderBuilder::new().has_headers(false).trim(csv::Trim::All).from_reader(file);

        let mut mapping = BTreeMap::new();
        for result in rdr.records() {
            let record = result.with_context(|| format!("Failed to read CSV record from: {}", path.display()))?;
            ...
            let key: u64 = record[0]
                .parse()
                .with_context(|| format!("Failed to parse key '{}' in {}", &record[0], path.display()))?;
            let version: u64 = record[1]
                .parse()
                .with_context(|| format!("Failed to parse version '{}' in {}", &record[1], path.display()))?;
    ...
    fn write_full_version_mapping(&self, path: &Path, mapping: &BTreeMap<u64, u64>) -> Result<(), anyhow::Error> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).with_context(|| format!("Failed to create directory: {}", parent.display()))?;
        }
        ...
        fs::write(path, content).with_context(|| format!("Failed to write version mapping file: {}", path.display()))?;
        Ok(())
    }

    fn load_root_mapping(&self, object_id: &ObjectID) -> Result<(), anyhow::Error> {
        if self.root_versions_map.read().unwrap().contains_key(object_id) {
            return Ok(());
        }
        let path = self.root_versions_path(object_id)?;
        let mapping = self.read_version_mapping(&path)?;
        self.root_versions_map.write().unwrap().insert(*object_id, mapping);
        Ok(())
    }

    fn load_checkpoint_mapping(&self, object_id: &ObjectID) -> Result<(), anyhow::Error> {
        if self.checkpoint_versions_map.read().unwrap().contains_key(object_id) {
            return Ok(());
        }
        let path = self.checkpoint_versions_path(object_id)?;
        let mapping = self.read_version_mapping(&path)?;
        self.checkpoint_versions_map.write().unwrap().insert(*object_id, mapping);
        Ok(())
    }

    fn update_root_mapping(&self, object_id: &ObjectID, key: u64, version: u64) -> Result<(), anyhow::Error> {
    ...
    fn update_checkpoint_mapping(&self, object_id: &ObjectID, key: u64, version: u64) -> Result<(), anyhow::Error> {
    ...
    fn get_object_by_version(&self, object_id: &ObjectID, version: u64) -> Result<Option<(Object, u64)>, anyhow::Error> {
        let object_dir = self.objects_dir()?.join(object_id.to_string());
        let version_file = object_dir.join(version.to_string());
        if !version_file.exists() {
            return Ok(None);
        }
        self.read_bcs_file(&version_file).map(|obj| Some((obj, version)))
    }
...
impl TransactionStore for FileSystemStore {
    fn transaction_data_and_effects(&self, tx_digest: &str) -> Result<Option<TransactionInfo>, anyhow::Error> {
        ...
        Ok(Some(TransactionInfo { data: txn_data.data, effects: txn_data.effects, checkpoint: txn_data.checkpoint }))
    }
}
...
impl TransactionStoreWriter for FileSystemStore {
    fn write_transaction(&self, tx_digest: &str, transaction_info: TransactionInfo) -> Result<(), anyhow::Error> {
        let file_path = self.transaction_dir()?.join(tx_digest);
...
impl ObjectStoreWriter for FileSystemStore {
    fn write_object(&self, key: &ObjectKey, object: Object, actual_version: u64) -> Result<(), anyhow::Error> {
...
impl StoreSummary for FileSystemStore {
    fn summary<W: std::io::Write>(&self, w: &mut W) -> anyhow::Result<()> {
        ...
        writeln!(w, "  Overall: hit={} miss={} error={}", total_hit, total_miss, total_err)?;
        writeln!(w, "  Transaction: hit={} miss={} error={}", txn_hit, txn_miss, txn_err)?;
        writeln!(w, "  Epoch:       hit={} miss={} error={}", epoch_hit, epoch_miss, epoch_err)?;
        writeln!(w, "  Protocol:    hit={} miss={} error={}", proto_hit, proto_miss, proto_err)?;
        writeln!(w, "  Objects (all): hit={} miss={} error={}", obj_total_hit, obj_total_miss, obj_total_err)?;
        writeln!(w, "  Objects:")?;
        writeln!(w, "    Version:     hit={} miss={} error={}", obj_v_hit, obj_v_miss, obj_v_err)?;
        writeln!(w, "    RootVersion: hit={} miss={} error={}", obj_r_hit, obj_r_miss, obj_r_err)?;
        writeln!(w, "    Checkpoint:  hit={} miss={} error={}", obj_c_hit, obj_c_miss, obj_c_err)?;
        Ok(())
    }
}
...
impl FileSystemStore {
    fn write_chain_identifier(&self, chain_id: String) -> Result<(), anyhow::Error> {
        ...
        if mapping_file.exists() {
            let file = fs::File::open(&mapping_file)
                .with_context(|| format!("Failed to open existing mapping file: {}", mapping_file.display()))?;
        ...
        let file = fs::File::create(&mapping_file)
            .with_context(|| format!("Failed to create mapping file: {}", mapping_file.display()))?;

        let mut wtr = csv::WriterBuilder::new().has_headers(false).from_writer(file);

        for (node, chain_id) in mappings {
            wtr.write_record([&node, &chain_id])
                .with_context(|| format!("Failed to write mapping record: {} -> {}", node, chain_id))?;
        }

        wtr.flush().with_context(|| "Failed to flush mapping file")?;
