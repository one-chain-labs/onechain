...
pub(crate) mod epoch_query {
    ...
    pub(crate) async fn query(epoch_id: u64, data_store: &DataStore) -> Result<Option<EpochData>, anyhow::Error> {
...
pub(crate) mod object_query {
    ...
    #[derive(cynic::QueryFragment)]
    #[cynic(graphql_type = "Object", schema_module = "crate::data_stores::gql_queries::schema")]
    ...
    pub(crate) async fn query(
        ...
        for keys in key_chunks {
            ...
            let chunk = list
                ...
                .map(|frag| match frag {
                    Some(frag) => {
                        let b64 = frag.object_bcs.ok_or_else(|| anyhow::anyhow!("Object bcs is None for object"))?.0;
                        let bytes = CryptoBase64::decode(&b64)?;
                        let obj: Object = bcs::from_bytes(&bytes)?;
                        let version =
                            frag.version.ok_or_else(|| anyhow::anyhow!("Object version is None for object"))?;
