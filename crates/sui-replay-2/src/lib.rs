...
#[clap(name = "Sui Replay Tool", about = "Replay executed transactions.", rename_all = "kebab-case")]
pub struct Config {
    #[command(flatten)]
...
impl Default for ReplayConfigExperimental {
    fn default() -> Self {
        Self { node: Node::Mainnet, verbose: false, store_mode: StoreMode::GqlOnly }
    }
}
...
pub fn load_config_file() -> anyhow::Result<ReplayConfigStable> {
    ...
    let content = fs::read_to_string(&config_file_path)
        .map_err(|e| anyhow!("Failed to read replay config file '{:?}': {}", config_file_path, e))?;

    let config: TOMLConfig = toml::from_str(&content)
        .map_err(|e| anyhow!("Failed to parse replay config file '{:?}': {}", config_file_path, e))?;

    config.flags.ok_or_else(|| anyhow!("No flags section found in replay config file '{:?}'", config_file_path))
}
...
pub fn merge_configs(cli_config: ReplayConfigStable, file_config: ReplayConfigStable) -> ReplayConfigStableInternal {
    ...
    ReplayConfigStableInternal {
        ...
        trace: cli_config.trace.or(file_config.trace).unwrap_or(default_config.trace),

        output_dir: cli_config.output_dir.or(file_config.output_dir),

        show_effects: cli_config.show_effects.or(file_config.show_effects).unwrap_or(default_config.show_effects),

        overwrite: cli_config.overwrite.or(file_config.overwrite).unwrap_or(default_config.overwrite),
    }
}
...
pub async fn handle_replay_config(
    ...
    let ReplayConfigExperimental { node, verbose, store_mode } = experimental_config;
    ...
    match store_mode {
        StoreMode::GqlOnly => {
            let gql_store =
                DataStore::new(node.clone(), version).map_err(|e| anyhow!("Failed to create data store: {:?}", e))?;
            run_replay(&gql_store, &output_root_dir, &digests, *overwrite_existing, *trace, *verbose, terminate_early)
                .await?;
        }
        StoreMode::FsThenGql => {
            ...
            run_replay(&store, &output_root_dir, &digests, *overwrite_existing, *trace, *verbose, terminate_early)
                .await?;
        }
        StoreMode::FsOnly => {
            let fs_store = FileSystemStore::new(node.clone())
                .map_err(|e| anyhow!("Failed to create file system store: {:?}", e))?;
            run_replay(&fs_store, &output_root_dir, &digests, *overwrite_existing, *trace, *verbose, terminate_early)
                .await?;
        }
    ...
    S: ReadDataStore + StoreSummary + SetupStore,
{
    ...
    for tx_digest in digests {
        ...
        let result = replay_transaction(&artifact_manager, tx_digest, data_store, trace).instrument(span).await;
        match result {
...
pub fn print_effects_or_fork<W: Write>(
    ...
    if manager.member(Artifact::ForkedTransactionEffects).exists() {
        ...
        let forked_effects =
            manager.member(Artifact::ForkedTransactionEffects).try_get_transaction_effects().transpose()?.unwrap();
        let expected_effects =
            manager.member(Artifact::TransactionEffects).try_get_transaction_effects().transpose()?.unwrap();
        writeln!(w, "Forked Transaction Effects for {digest}\n{}", diff_effects(&expected_effects, &forked_effects))?;
        ...
        manager
            ...
            .try_get_gas_report()
            .transpose()?
            .map(|report| writeln!(w, "Transaction Gas Report for {digest}\n{}", Pretty(&report)).unwrap())
            .unwrap_or_else(|| {
...
pub fn diff_effects(expected_effect: &TransactionEffects, txn_effects: &TransactionEffects) -> String {
