...
impl EpochStore for DataStore {
    ...
    fn protocol_config(&self, epoch: u64) -> Result<ProtocolConfig, anyhow::Error> {
        ...
        Ok(ProtocolConfig::get_for_version(ProtocolVersion::new(epoch.protocol_version), self.chain()))
    }
}
...
impl DataStore {
    pub fn new(node: Node) -> Result<Self, anyhow::Error> {
        ...
        Ok(Self { client, rt, node, epoch_map, rpc })
    }
    ...
    async fn transaction(&self, digest: &str) -> Result<(TransactionData, TransactionEffects, u64), anyhow::Error> {
