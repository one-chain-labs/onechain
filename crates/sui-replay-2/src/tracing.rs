...
pub fn save_trace_output(
    ...
    fs::create_dir_all(&txn_output_path)
        .context(format!("Failed to create trace output directory for transaction {}", digest,))?;
    ...
    fs::write(&trace_file_path, json).context(format!("Failed to write trace output to {:?}", trace_file_path,))?;
    let bcode_dir = txn_output_path.join(BCODE_DIR);
    fs::create_dir(&bcode_dir).context(format!("Failed to create bytecode output directory '{:?}'", bcode_dir,))?;
    let objects: Vec<Object> = objects.into_values().flat_map(|versions| versions.into_values()).collect();
        ...
        if let Data::Package(pkg) = &obj.data {
            ...
            for (mod_name, serialized_mod) in pkg.serialized_module_map() {
                let compiled_mod = CompiledModule::deserialize_with_defaults(serialized_mod)
                    .context(format!("Failed to deserialize module {:?} in package {}", mod_name, &pkg_addr,))?;
                let d = Disassembler::from_module(&compiled_mod, Spanned::unsafe_no_loc(()).loc).context(format!(
                    "Failed to create disassembler for module {:?} in package {}",
                    mod_name, &pkg_addr,
                ))?;
                let (disassemble_string, bcode_map) = d
                    .disassemble_with_source_map()
                    .context(format!("Failed to disassemble module {:?} in package {}", mod_name, &pkg_addr,))?;
                ...
                fs::write(bcode_pkg_dir.join(format!("{}.{}", mod_name, MOVE_BYTECODE_EXTENSION)), disassemble_string)
                    .context(format!(
                    "Failed to write disassembled bytecode for module {:?} in package {}",
                    mod_name, &pkg_addr,
                ))?;
                fs::write(bcode_pkg_dir.join(format!("{}.json", mod_name)), bcode_map_json).context(format!(
    ...
    fs::create_dir(&src_dir).context(format!("Failed to create source output directory '{:?}'", src_dir,))?;
