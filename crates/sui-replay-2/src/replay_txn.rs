...
impl ReplayTransaction {
    ...
    pub fn load(
        ...
        let protocol_config =
            epoch_store.protocol_config(epoch).unwrap_or_else(|e| panic!("Failed to get protocol config: {:?}", e));
        let executor = ReplayExecutor::new(protocol_config, None).unwrap_or_else(|e| panic!("{:?}", e));
...
fn get_effects_ids(effects: &TransactionEffects) -> Result<BTreeSet<ObjectKey>, anyhow::Error> {
    let mut object_keys = effects
        ...
        .map(|input_shared_object| match input_shared_object {
            ...
            | InputSharedObject::ReadOnly((object_id, version, _digest)) => {
                ObjectKey { object_id: *object_id, version_query: VersionQuery::Version(version.value()) }
            }
        })
        .collect::<BTreeSet<_>>();
    effects.unchanged_shared_objects().iter().for_each(|(obj_id, kind)| match kind {
