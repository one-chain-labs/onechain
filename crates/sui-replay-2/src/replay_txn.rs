...
use sui_data_store::{EpochStore, ObjectKey, ObjectStore, ReadDataStore, TransactionStore, VersionQuery};
use sui_types::{
    base_types::{ObjectID, SequenceNumber, SuiAddress},
    digests::TransactionDigest,
    effects::{InputConsensusObject, TransactionEffects, TransactionEffectsAPI, UnchangedConsensusKind},
    gas::SuiGasStatusAPI,
    object::Object,
    transaction::{
        CallArg,
...
impl ExecutorProvider {
    pub fn new(cache_enabled: bool) -> Self {
        Self { cache: BTreeMap::new(), cache_enabled }
    }
    ...
    pub fn get_or_create(&mut self, epoch: u64, epoch_store: &dyn EpochStore) -> anyhow::Result<ReplayExecutor> {
        let protocol_config =
            epoch_store.protocol_config(epoch)?.ok_or_else(|| anyhow!("Protocol config missing for epoch {}", epoch))?;
...
pub(crate) async fn replay_transaction<S: ReadDataStore>(
    ...
    let replay_txn = match ReplayTransaction::load(tx_digest, data_store, data_store, data_store, executor_provider) {
    ...
    if let Some(trace_builder) = trace_builder_opt {
        save_trace_output(artifact_manager, trace_builder, &context_and_effects)
            .map_err(|e| anyhow!("transaction {} failed to build a trace output path -> {:?}", tx_digest, e))?;
    }
    ...
    artifact_manager.member(Artifact::ReplayCacheSummary).serialize_artifact(&cache_summary).transpose()?.unwrap();

    // Save move call info if the transaction is a ProgrammableTransaction
    if let sui_types::transaction::TransactionKind::ProgrammableTransaction(ptb) = context_and_effects.txn_data.kind() {
        ...
        match MoveCallInfo::from_transaction(ptb, &context_and_effects.object_cache) {
            Ok(move_call_info) => {
                let successful_extractions = move_call_info.command_signatures.iter().filter(|s| s.is_some()).count();
...
fn verify_txn_and_save_effects(
    ...
    if effects != expected_effects {
        ...
        artifact_manager.member(Artifact::ForkedTransactionEffects).serialize_artifact(effects).transpose()?.unwrap();
        artifact_manager.member(Artifact::TransactionEffects).serialize_artifact(expected_effects).transpose()?.unwrap();
    } else {
        artifact_manager.member(Artifact::TransactionEffects).serialize_artifact(effects).transpose()?.unwrap();
        artifact_manager.member(Artifact::ForkedTransactionEffects).try_remove_artifact()?;
    }
    Ok(())
}
...
impl ReplayTransaction {
    ...
    pub fn load(
        ...
        let digest = tx_digest.parse().context(format!("Transaction digest malformed: {}", tx_digest))?;
        ...
        Ok(Self { digest, checkpoint, txn_data, effects, executor, object_cache })
    }
    ...
    pub fn get_input_objects_for_replay(&self) -> Result<InputObjects, anyhow::Error> {
        let _deleted_shared_info_map: BTreeMap<ObjectID, (TransactionDigest, SequenceNumber)> = BTreeMap::new();
        let mut resolved_input_objs = vec![];
        let input_objects_kind = self
            .txn_data
            .input_objects()
            .context(format!("Failed to get input objects from transaction {}", self.digest,))?;
            ...
            match kind {
                InputObjectKind::MovePackage(pkg_id) => {
                    self.object_cache
                        .get(pkg_id)
                        .map(|pkgs| {
                            debug_assert!(pkgs.len() == 1, "Expected only one version for package {}", pkg_id);
                ...
                InputObjectKind::ImmOrOwnedMoveObject((obj_id, version, _digest)) => {
                    ...
                    let input_object_kind = InputObjectKind::ImmOrOwnedMoveObject(object.compute_object_reference());
                    resolved_input_objs.push(ObjectReadResult {
                        input_object_kind,
                        object: ObjectReadResultKind::Object(object.clone()),
                    });
                }
                InputObjectKind::SharedMoveObject { id, initial_shared_version, mutability } => {
                    let input_object_kind = InputObjectKind::SharedMoveObject {
                        id: *id,
                        initial_shared_version: *initial_shared_version,
                        mutability: *mutability,
                    };
                    let versions = self.object_cache.get(id).ok_or_else(|| {
                        anyhow::anyhow!(format!(
                            "Shared Object id {} not found in transaction cache. Should have been loaded already",
                            id,
                        ))
                    })?;
                    debug_assert!(versions.len() == 1, "Expected only one version for shared object {}", id);
                    let (_version, obj) = versions.iter().next().unwrap();
                    resolved_input_objs
                        .push(ObjectReadResult { input_object_kind, object: ObjectReadResultKind::Object(obj.clone()) });
                }
            }
        }
            ...
            match cmd {
                Command::MoveCall(move_call) => {
                    ...
                    for type_input in move_call.type_arguments.iter() {
                        let typ =
                            type_input.to_type_tag().context(format!("Failed to read TypeTag: {:?}", type_input))?;
                        packages_from_type_tag(&typ, &mut packages);
                    }
                }
                Command::MakeMoveVec(type_input, _) => {
                    if let Some(t) = type_input {
                        let typ = t.to_type_tag().context(format!("Failed to read TypeTag: {:?}", type_input))?;
                        packages_from_type_tag(&typ, &mut packages);
                    }
                }
...
> {
    ...
    for object in objects.into_iter() {
        ...
        object_cache.entry(object_id).or_default().insert(version, object);
    }
...
fn load_packages(
    ...
    let pkg_object_keys = packages
        .iter()
        .map(|pkg_id| ObjectKey { object_id: *pkg_id, version_query: VersionQuery::AtCheckpoint(checkpoint) })
        .collect::<Vec<_>>();
...
fn get_txn_object_keys(txn_data: &TransactionData, effects: &TransactionEffects) -> Result<Vec<ObjectKey>, Error> {
    ...
    let mut effect_ids =
        effects_object_ids.into_iter().map(|input| (input.object_id, input.version_query)).collect::<BTreeMap<_, _>>();
    for input_object in input_object_ids.into_iter() {
        effect_ids.entry(input_object.object_id).or_insert(input_object.version_query);
    }
    Ok(effect_ids
        .into_iter()
        .map(|(object_id, version_query)| ObjectKey { object_id, version_query })
...
fn get_input_ids(txn_data: &TransactionData) -> Result<BTreeSet<ObjectKey>, Error> {
    ...
    let mut object_keys: BTreeSet<ObjectKey> = txn_data
        ...
        .payment
        .iter()
        .map(|(id, seq_num, _)| ObjectKey { object_id: *id, version_query: VersionQuery::Version(seq_num.value()) })
        .collect();
                ...
                if let CallArg::Object(call_arg) = input {
                    match call_arg {
                        ObjectArg::ImmOrOwnedObject((id, seq_num, _digest)) => {
                            Some(ObjectKey { object_id: *id, version_query: VersionQuery::Version(seq_num.value()) })
                        }
...
fn get_effects_ids(effects: &TransactionEffects) -> Result<BTreeSet<ObjectKey>, Error> {
    let mut object_keys = effects
        ...
        .map(|input_consensus_object| match input_consensus_object {
            ...
            | InputConsensusObject::Cancelled(object_id, version) => {
                ObjectKey { object_id: *object_id, version_query: VersionQuery::Version(version.value()) }
            }
            InputConsensusObject::Mutate((object_id, version, _digest))
            | InputConsensusObject::ReadOnly((object_id, version, _digest)) => {
                ObjectKey { object_id: *object_id, version_query: VersionQuery::Version(version.value()) }
            }
        })
        .collect::<BTreeSet<_>>();
    effects.unchanged_consensus_objects().iter().for_each(|(obj_id, kind)| match kind {
        UnchangedConsensusKind::ReadOnlyRoot((ver, _digest)) => {
            object_keys.insert(ObjectKey { object_id: *obj_id, version_query: VersionQuery::Version(ver.value()) });
        }
