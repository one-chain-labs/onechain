...
use crate::{
    ...
    replay_interface::{EpochStore, ObjectKey, ObjectStore, ReadDataStore, TransactionStore, VersionQuery},
    summary_metrics::{log_replay_metrics, tx_metrics_reset},
...
fn verify_txn_and_save_forked_effects(
    ...
    if effects != expected_effects {
        ...
        bail!("Transaction effects do not match expected effects for transaction {}", effects.transaction_digest());
    } else {
        Ok(())
    }
}
...
impl ReplayTransaction {
    ...
    pub fn load(
        ...
        let transaction_info = txn_store
            ...
            .ok_or_else(|| anyhow::anyhow!(format!("Transaction not found for digest: {}", tx_digest)))?;
...
pub fn get_input_objects_for_replay(
    ...
    let input_objects_kind =
        txn.input_objects().context(format!("Failed to get input objects from transaction {}", tx_digest))?;
    for kind in input_objects_kind.iter() {
        match kind {
            ...
            InputObjectKind::ImmOrOwnedMoveObject((obj_id, version, _digest)) => {
                ...
                resolved_input_objs
                    .push(ObjectReadResult { input_object_kind, object: ObjectReadResultKind::Object(object.clone()) });
            }
            InputObjectKind::SharedMoveObject { id, initial_shared_version, mutable } => {
                ...
                let versions = object_cache.get(id).ok_or_else(|| {
                    anyhow::anyhow!(format!(
