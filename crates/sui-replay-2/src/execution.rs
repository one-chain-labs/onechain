...
> {
    ...
    let ReplayTransaction { digest, checkpoint: _, txn_data, effects: expected_effects, executor, object_cache } = txn;

    let protocol_config = &executor.protocol_config;
    let epoch_data = epoch_store.epoch_info(epoch)?.ok_or_else(|| anyhow!(format!("Epoch {} not found", epoch)))?;
    ...
    for (object_ref, _owner) in effects.created() {
        ...
        if let Some(object) = inner_store.written.get(&object_id) {
            object_cache.entry(object_id).or_default().insert(object.version().value(), object.clone());
        } else {
            // Return error if created object is not found in inner_store
            return Err(anyhow!("Created object {} not found in inner_store written objects", object_id));
        }
    }
...
impl BackingPackageStore for ReplayStore<'_> {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> SuiResult<Option<PackageObject>> {
        ...
        let package = self.store.get_objects(&[object_key]).map_err(|e| SuiErrorKind::Storage(e.to_string()))?;
...
impl sui_types::storage::ObjectStore for ReplayStore<'_> {
    ...
    fn get_object(&self, object_id: &ObjectID) -> Option<Object> {
        ...
        cache.entry(*object_id).or_default().insert(fetched_object.version().value(), fetched_object.clone());
...
impl ChildObjectResolver for ReplayStore<'_> {
    ...
    fn read_child_object(
        ...
        let object = self.store.get_objects(&[object_key]).map_err(|e| SuiErrorKind::Storage(e.to_string()))?;
