...
impl ReplayCacheSummary {
    ...
    pub fn from_cache(epoch_id: u64, checkpoint: u64, object_cache: &BTreeMap<ObjectID, BTreeMap<u64, Object>>) -> Self {
        ...
        for (object_id, versions) in object_cache {
            for (version, object) in versions {
                let object_type = if object.is_package() {
                    ...
                    let package = object.data.try_as_package().expect("Package object should have package data");
                    ...
                    let struct_tag = object.struct_tag().expect("Move object should have struct tag");
                    ObjectType::MoveObject(struct_tag)
                };

                cache_entries.push(CacheEntry { object_id: *object_id, version: *version, object_type });
            }
        }

        Self { epoch_id, checkpoint, cache_entries }
    }
}
...
> {
    ...
    let ReplayTransaction { digest, checkpoint, txn_data, effects: expected_effects, executor, object_cache } = txn;
    ...
    let epoch_data =
        epoch_store.epoch_info(epoch)?.ok_or_else(|| anyhow::anyhow!(format!("Epoch {} not found", epoch)))?;
    let epoch_start_timestamp = epoch_data.start_timestamp;
    let gas_status = if txn_data.kind().is_system_tx() {
        SuiGasStatus::new_unmetered()
    } else {
        SuiGasStatus::new(txn_data.gas_data().budget, txn_data.gas_data().price, epoch_data.rgp, protocol_config)
            .expect("Failed to create gas status")
    };
    let store: ReplayStore<'_> =
        ReplayStore { checkpoint, store: object_store, object_cache: RefCell::new(object_cache) };
    ...
    let ReplayStore { object_cache, checkpoint: _, store: _ } = store;
    let object_cache = object_cache.into_inner();
    debug!(op = "execute_tx", phase = "end", "execution");
    Ok((result, TxnContextAndEffects {
...
impl ReplayExecutor {
    pub fn new(protocol_config: ProtocolConfig) -> Result<Self, anyhow::Error> {
        ...
        Ok(Self { protocol_config, executor, metrics })
    }
}
...
impl ReplayStore<'_> {
    ...
    fn get_object_at_version(&self, object_id: &ObjectID, version: VersionNumber) -> Option<Object> {
        // look up in the cache
        if let Some(object) =
            self.object_cache.borrow().get(object_id).and_then(|versions| versions.get(&version.value()).cloned())
        {
            return Some(object);
        }
        ...
        let object = self
            .store
            .get_objects(&[ObjectKey { object_id: *object_id, version_query: VersionQuery::Version(version.value()) }])
        ...
        if let Some(obj) = &object {
            self.object_cache.borrow_mut().entry(obj.id()).or_default().insert(obj.version().value(), obj.clone());
        }
...
impl BackingPackageStore for ReplayStore<'_> {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> SuiResult<Option<PackageObject>> {
        ...
        if let Some(versions) = self.object_cache.borrow().get(package_id) {
            debug_assert!(versions.len() == 1, "Expected only one version in cache for package object {}", package_id);
            return Ok(Some(PackageObject::new(versions.values().next().unwrap().clone())));
        }
        ...
        let package = self.store.get_objects(&[object_key]).map_err(|e| SuiError::Storage(e.to_string()))?;
        debug_assert!(package.len() == 1, "Expected one package object for {}", package_id);
...
impl ChildObjectResolver for ReplayStore<'_> {
    ...
    fn read_child_object(
        ...
        trace!("read_child_object({}, {}, {})", _parent, child, child_version_upper_bound,);
        let object_key =
            ObjectKey { object_id: *child, version_query: VersionQuery::RootVersion(child_version_upper_bound.value()) };
        let object = self.store.get_objects(&[object_key]).map_err(|e| SuiError::Storage(e.to_string()))?;
        debug_assert!(object.len() == 1, "Expected one object for {}", child,);
        let object = object.into_iter().next().unwrap().map(|(obj, _version)| obj);
        Ok(object)
    }
