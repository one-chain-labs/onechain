...
use sui_types::{base_types::ObjectID, object::Object, transaction::ProgrammableTransaction, type_input::TypeInput};
...
impl MoveCallInfo {
    ...
    fn extract_function_signature(
        ...
        let move_package =
            package_obj.data.try_as_package().ok_or_else(|| anyhow!("Object {} is not a package", package_id))?;
        ...
        let (_, function_def) = compiled_module
            ...
            .ok_or_else(|| anyhow!("Function {} not found in module {}::{}", function_name, package_id, module_name))?;
        ...
        let type_arguments_as_move_types: Vec<MoveType> =
            move_call.type_arguments.iter().map(Self::type_input_to_move_type).collect();

        // Convert SignatureTokens to MoveTypes
        let parameters = param_signature
            .0
            .iter()
            .map(|token| Self::signature_token_to_move_type(token, &compiled_module, &type_arguments_as_move_types))
    ...
    fn type_input_to_move_type(type_input: &TypeInput) -> MoveType {
        match type_input {
            ...
            TypeInput::Vector(element) => MoveType::Vector(Box::new(Self::type_input_to_move_type(element))),
            TypeInput::Struct(struct_input) => {
                let type_params: Vec<MoveType> =
                    struct_input.type_params.iter().map(Self::type_input_to_move_type).collect();
    ...
    fn signature_token_to_move_type(
        ...
        match token {
            ...
            SignatureToken::Vector(element_type) => {
                let element = Self::signature_token_to_move_type(element_type, module, type_arguments)?;
            ...
            SignatureToken::DatatypeInstantiation(instantiation) => {
                ...
                Ok(MoveType::DatatypeInstantiation(Box::new((datatype, resolved_type_args))))
            }
