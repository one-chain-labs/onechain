...
impl<'b> ArtifactManager<'b> {
    ...
    pub fn new(base_path: &'b Path, overrides_allowed: bool) -> Result<Self> {
        std::fs::create_dir_all(base_path)
            .map_err(|e| anyhow!("Failed to create base path for replay artifacts at {}: {e}", base_path.display()))?;
        Ok(ArtifactManager { base_path, overrides_allowed })
    }
    ...
    pub fn member<'a>(&'a self, artifact: Artifact) -> ArtifactMember<'a, 'b> {
        ArtifactMember { artifact_path: self.base_path.join(artifact.as_file()), manager: self, artifact_type: artifact }
    }
}
...
impl ArtifactMember<'_, '_> {
        ...
        if self.artifact_type == Artifact::Trace {
            Some(
                std::fs::File::open(&self.artifact_path)
                    ...
                    .map_err(|e| anyhow!("Failed to open trace file {}: {e}", self.artifact_path.display())),
            )
        } else {
            None
        }
    ...
    pub fn try_get_transaction_effects(&self) -> Option<Result<TransactionEffects>> {
        if matches!(self.artifact_type, Artifact::TransactionEffects | Artifact::ForkedTransactionEffects) {
            Some(self.get_json().and_then(|json| {
                serde_json::from_value::<TransactionEffects>(json).map_err(|e| {
                    anyhow!("Failed to deserialize transaction effects from {}: {e}", self.artifact_path.display())
                })
            }))
        } else {
            None
        ...
        if self.artifact_type == Artifact::TransactionGasReport {
            Some(self.get_json().and_then(|json| {
                serde_json::from_value::<GasUsageReport>(json).map_err(|e| {
                    anyhow!("Failed to deserialize gas usage report from {}: {e}", self.artifact_path.display())
                })
            }))
        } else {
            None
        ...
        if self.artifact_type == Artifact::ReplayCacheSummary {
            Some(self.get_json().and_then(|json| {
                serde_json::from_value::<ReplayCacheSummary>(json).map_err(|e| {
                    anyhow!("Failed to deserialize replay cache summary from {}: {e}", self.artifact_path.display())
                })
            }))
        } else {
            None
        ...
        if self.artifact_type == Artifact::MoveCallInfo {
            Some(self.get_json().and_then(|json| {
                serde_json::from_value::<MoveCallInfo>(json).map_err(|e| {
                    anyhow!("Failed to deserialize move call info from {}: {e}", self.artifact_path.display())
                })
            }))
        } else {
            None
...
impl ArtifactMember<'_, '_> {
    pub fn serialize_move_trace(&self, trace: MoveTrace) -> Option<Result<()>> {
        ...
        if !self.manager.overrides_allowed && self.artifact_path.exists() {
            return Some(Err(anyhow!("Trace file already exists at {}", self.artifact_path.display())));
        }

        Some(
            std::fs::write(&self.artifact_path, trace.into_compressed_json_bytes())
                .map_err(|e| anyhow!("Failed to write trace to {}: {e}", self.artifact_path.display())),
        )
    }
    ...
    pub fn serialize_artifact(&self, data: &impl serde::Serialize) -> Option<Result<()>> {
        ...
        let mut file = match std::fs::File::create(&self.artifact_path)
            .map_err(|e| anyhow!("Failed to create file {}: {e}", self.artifact_path.display()))
        {
            Ok(file) => file,
        ...
        Some(match self.artifact_type.encoding_type() {
            EncodingType::Json => serde_json::to_writer(file, &data).map_err(|e| anyhow!("Failed to write JSON: {e}")),
            EncodingType::JsonCompressed => {
                let mut buf = Vec::new();
                let mut compressed_buf = Vec::new();
                if let Err(e) = serde_json::to_writer(&mut buf, &data).map_err(|e| anyhow!("Failed to write JSON: {e}"))
                {
                    return Some(Err(e));
                }
                zstd::bulk::compress_to_buffer(&buf, &mut compressed_buf, 0)
                    ...
                    .and_then(|_| {
                        file.write_all(&buf).map_err(|e| {
                            anyhow!("Failed to write compressed JSON to {}: {e}", self.artifact_path.display())
                        })
                    })
            }
        })
    ...
    pub fn try_remove_artifact(&self) -> Result<()> {
        ...
        {
            bail!("Failed to remove file {}: {:?}", self.artifact_path.display(), e);
        }
        Ok(())
    }
}
