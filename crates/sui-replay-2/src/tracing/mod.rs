...
pub fn save_trace_output(
    ...
    trace_member.serialize_move_trace(trace).transpose()?.unwrap();
    ...
    let mut pkgs = object_cache
        ...
        .filter_map(|obj| if let Data::Package(pkg) = &obj.data { Some(pkg) } else { None })
        .collect::<Vec<_>>();
    ...
    for pkg in pkgs {
        ...
        fs::create_dir_all(&bcode_pkg_dir)
            .context(format!("Failed to create bytecode output directory '{:?}'", bcode_pkg_dir,))?;
        fs::create_dir_all(&bcode_pkg_dir).context("Failed to create bytecode package directory")?;
        for (mod_name, serialized_mod) in pkg.serialized_module_map() {
            ...
            let d = Disassembler::from_module(&compiled_mod, Spanned::unsafe_no_loc(()).loc)
                .context(format!("Failed to create disassembler for module {:?} in package {}", mod_name, &pkg_addr,))?;
            let (disassemble_string, mut bcode_map) = d
        ...
        fs::create_dir_all(&src_pkg_dir)
            .context(format!("Failed to create source output directory '{:?}'", src_pkg_dir,))?;
    }
