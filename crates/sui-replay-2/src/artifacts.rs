...
impl<'b> ArtifactManager<'b> {
    ...
    pub fn new(base_path: &'b Path, overrides_allowed: bool) -> anyhow::Result<Self> {
        std::fs::create_dir_all(base_path).map_err(|e| {
            anyhow::anyhow!("Failed to create base path for replay artifacts at {}: {e}", base_path.display())
        })?;
        ...
        if self.artifact_type == Artifact::Trace {
            Some(
                std::fs::File::open(&self.artifact_path)
                    ...
                    .map_err(|e| anyhow::anyhow!("Failed to open trace file {}: {e}", self.artifact_path.display())),
            )
        } else {
            None
        }
        ...
        if self.artifact_type == Artifact::TransactionGasReport {
            Some(self.get_json().and_then(|json| {
                serde_json::from_value::<GasUsageReport>(json).map_err(|e| {
                    anyhow::anyhow!("Failed to deserialize gas usage report from {}: {e}", self.artifact_path.display())
                })
            }))
        } else {
            None
...
impl ArtifactMember<'_, '_> {
    pub fn serialize_move_trace(&self, trace: MoveTrace) -> Option<anyhow::Result<()>> {
        ...
        if !self.manager.overrides_allowed && self.artifact_path.exists() {
            return Some(Err(anyhow::anyhow!("Trace file already exists at {}", self.artifact_path.display())));
        }

        Some(
            std::fs::write(&self.artifact_path, trace.into_compressed_json_bytes())
                .map_err(|e| anyhow::anyhow!("Failed to write trace to {}: {e}", self.artifact_path.display())),
        )
    }
    ...
    pub fn serialize_artifact(&self, data: &impl serde::Serialize) -> Option<anyhow::Result<()>> {
        ...
        let mut file = match std::fs::File::create(&self.artifact_path)
            .map_err(|e| anyhow::anyhow!("Failed to create file {}: {e}", self.artifact_path.display()))
        {
            Ok(file) => file,
        ...
        Some(match self.artifact_type.encoding_type() {
            EncodingType::Json => {
                serde_json::to_writer(file, &data).map_err(|e| anyhow::anyhow!("Failed to write JSON: {e}"))
            }
            EncodingType::JsonCompressed => {
                let mut buf = Vec::new();
                let mut compressed_buf = Vec::new();
                if let Err(e) =
                    serde_json::to_writer(&mut buf, &data).map_err(|e| anyhow::anyhow!("Failed to write JSON: {e}"))
                {
                    return Some(Err(e));
                }
                zstd::bulk::compress_to_buffer(&buf, &mut compressed_buf, 0)
                    ...
                    .and_then(|_| {
                        file.write_all(&buf).map_err(|e| {
                            anyhow::anyhow!("Failed to write compressed JSON to {}: {e}", self.artifact_path.display())
                        })
                    })
            }
        })
