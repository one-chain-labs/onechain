...
fn main() {
    ...
    let ReplayConfig { node, digest, digests_path, show_effects, verify, trace } = config;
    ...
    let data_store = DataStore::new(node).unwrap_or_else(|e| panic!("Failed to create data store: {:?}", e));
...
fn replay_transaction(
    ...
    let (result, effects, gas_status, expected_effects, object_cache) =
        match execute_transaction_to_effects(replay_txn, data_store, data_store, &mut trace_builder_opt) {
