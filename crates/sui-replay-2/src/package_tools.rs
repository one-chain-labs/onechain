...
impl PackageInfo {
    ...
    fn extract_original_package(&self) -> Result<(Object, u64)> {
        ...
        let key = ObjectKey { object_id: self.package_id, version_query: VersionQuery::Version(version) };
    ...
    fn pkg_dir_path(&self) -> Result<PathBuf> {
        ...
        let chain_id = get_chain_id_from_mapping(&self.node).context("Failed to get chain ID for node")?;

        Ok(FileSystemStore::base_path()?.join(chain_id).join(OBJECTS_DIR).join(self.package_id.to_string()))
    }
    ...
    fn pkg_version_path(&self) -> Result<(PathBuf, u64)> {
        ...
        if !pkg_dir.exists() {
            bail!("Package {} not found in cache at {:?}", self.package_id, pkg_dir);
        }
        ...
        if version_files.len() > 1 {
            bail!("Expected exactly one version file in package directory {:?}", pkg_dir,);
        }
    ...
    pub fn save_package_to_cache(&self, object: &Object, version: SequenceNumber) -> Result<PathBuf> {
        let pkg_file = self.pkg_dir_path()?.join(version.value().to_string());

        if !pkg_file.exists() {
            bail!("Package {} version {} not found in cache", self.package_id, version);
        }
...
fn get_chain_id_from_mapping(node: &Node) -> Result<String> {
    ...
    let file = fs::File::open(&mapping_file).context(format!("Failed to open {}", NODE_MAPPING_FILE))?;

    let mut rdr = csv::ReaderBuilder::new().has_headers(false).trim(csv::Trim::All).from_reader(file);

    let node_name = node.network_name();

    // Read the file and find the matching node
    for result in rdr.records() {
        let record = result.context(format!("Failed to read CSV record from {}", NODE_MAPPING_FILE))?;
...
impl PackageRebuilder {
    pub fn new(node: Node, package_id: ObjectID, source_path: PathBuf, output_path: Option<PathBuf>) -> Self {
        Self { package_info: PackageInfo::new(node, package_id), source_path, output_path }
    }
    ...
    pub fn rebuild(&self) -> Result<()> {
        ...
        let (original_package, tx_digest, version) = self.extract_package_metadata(&original_object)?;

        if extracted_version != version.value() {
            bail!("Extracted version {} does not actual package match version {}", extracted_version, version);
        }
        ...
        match &self.output_path {
            Some(output_path) => {
                ...
                println!("Package rebuilt successfully and saved to: {:?}", output_path);
            }
            None => {
                // Replace in cache
                let cache_path = self.package_info.save_package_to_cache(&rebuilt_object, version)?;
    ...
    fn extract_package_metadata(&self, object: &Object) -> Result<(MovePackage, TransactionDigest, SequenceNumber)> {
    ...
    fn compile_package_with_id(&self) -> Result<Vec<CompiledModule>> {
        // Register SuiPackageHooks to recognize Sui-specific manifest fields (matching one move build)
        ...
        let mut config =
            resolve_lock_file_path(build_config, Some(&self.source_path)).context("Failed to resolve lock file path")?;
        ...
        let compiled_package = config.build(&self.source_path).context("Failed to build package")?;
    ...
    fn update_module_addresses(&self, modules: &mut [CompiledModule], target_id: ObjectID) -> Result<()> {
        ...
        for module in modules.iter_mut() {
            ...
            let self_handle =
                module.module_handles.get(self_handle_idx).ok_or_else(|| anyhow!("Invalid self module handle index"))?;
    ...
    fn build_merged_type_origin_table(
        ...
        for module in compiled_modules {
            ...
            for struct_def in module.struct_defs() {
                ...
                let origin_package = existing_table.remove(&type_key).unwrap_or(self.package_info.package_id);
                new_table.push(TypeOrigin { module_name, datatype_name: struct_name, package: origin_package });
            }
            ...
            for enum_def in module.enum_defs() {
                ...
                // If type exists in original, preserve its origin; otherwise use current package ID
                let origin_package = existing_table.remove(&type_key).unwrap_or(self.package_info.package_id);
                new_table.push(TypeOrigin { module_name, datatype_name: enum_name, package: origin_package });
            }
        }

        // Check if any types were removed (not allowed for compatibility)
        if !existing_table.is_empty() {
            let removed_types: Vec<String> =
                existing_table.keys().map(|(module, datatype)| format!("{}::{}", module, datatype)).collect();
            bail!(
    ...
    fn compare_dependencies(&self, new_modules: &[CompiledModule], original_package: &MovePackage) -> Result<()> {
        ...
        if !added_deps.is_empty() || !removed_deps.is_empty() {
            let mut error_msg = String::from("Dependencies have changed, which is not supported for package rebuild:\n");

            if !added_deps.is_empty() {
                error_msg.push_str(&format!(
                    "  Added dependencies: {}\n",
                    added_deps.iter().map(|d| format!("0x{}", d)).collect::<Vec<_>>().join(", ")
                ));
            }
            ...
            if !removed_deps.is_empty() {
                error_msg.push_str(&format!(
                    ...
                    removed_deps.iter().map(|d| format!("0x{}", d)).collect::<Vec<_>>().join(", ")
                ));
            }
    ...
    fn build_new_linkage_table(
        ...
        self.compare_dependencies(modules, original_package).context("Dependency validation failed")?;
        ...
        if !original_linkage.is_empty() {
            println!("Reusing original linkage table with {} entries", original_linkage.len());
        }
    ...
    fn rebuild_package(
        ...
        for module in &compiled_modules {
            ...
            module.serialize_with_version(module.version, &mut module_bytes).context("Failed to serialize module")?;
    ...
    fn verify_rebuild(&self, original: &Object, rebuilt: &Object) -> Result<()> {
        ...
        if original_bytes == rebuilt_bytes {
            ...
            if let (Data::Package(orig_pkg), Data::Package(rebuilt_pkg)) = (&original.data, &rebuilt.data) {
                println!("  Original type origins: {} entries", orig_pkg.type_origin_table().len());
                println!("  Rebuilt type origins: {} entries", rebuilt_pkg.type_origin_table().len());
                println!("  Original linkage table: {} entries", orig_pkg.linkage_table().len());
                println!("  Rebuilt linkage table: {} entries", rebuilt_pkg.linkage_table().len());
            }
        }
...
pub fn extract_package(node: Node, package_id: ObjectID, output_path: PathBuf) -> Result<()> {
    ...
    let (object, version) = package_info.extract_original_package().context("Failed to extract package from cache")?;

    // Save extracted package to the specified output path
    PackageInfo::save_package_to_file(&object, &output_path).context("Failed to save extracted package")?;

    println!("Successfully extracted package {} (version {}) to {}", package_id, version, output_path.display());
...
pub fn overwrite_package(node: Node, package_id: ObjectID, package_path: PathBuf) -> Result<()> {
    ...
    let package_bytes = fs::read(&package_path).context(format!("Failed to read package file: {:?}", package_path))?;

    // Validate it's a valid BCS-encoded Object
    let _object: Object =
        bcs::from_bytes(&package_bytes).context("Invalid package file: not a valid BCS-encoded Object")?;
    ...
    fs::write(&version_path, &package_bytes).context(format!("Failed to write package to cache: {:?}", version_path))?;

    println!("Package {} successfully overwritten in cache at: {:?}", package_id, version_path);
