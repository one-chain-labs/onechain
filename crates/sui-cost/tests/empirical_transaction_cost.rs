...
impl CommonTransactionCosts {
    pub fn is_shared_object_tx(&self) -> bool {
        matches!(self, CommonTransactionCosts::SharedCounterAssertValue | CommonTransactionCosts::SharedCounterIncrement)
    }
}
...
async fn split_n_tx(n: u64, coin: ObjectRef, gas: ObjectRef, gas_price: u64, sender: SuiAddress) -> TransactionData {
...
async fn create_txes(test_cluster: &TestCluster) -> BTreeMap<CommonTransactionCosts, TransactionData> {
    ...
    let (counter_package, counter) = publish_basics_package_and_make_counter(&test_cluster.wallet).await;
    ...
    let publish_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price).publish(package_path).build();
    ...
    ret.insert(CommonTransactionCosts::SharedCounterAssertValue, transaction);

    // Make a transaction to increment the counter.
    let transaction = TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
        .call_counter_increment(counter_package_id, counter_id, counter_initial_shared_version)
        .build();
...
async fn run_actual_costs() -> Result<BTreeMap<CommonTransactionCosts, GasCostSummary>, anyhow::Error> {
    let mut ret = BTreeMap::new();
    let test_cluster = TestClusterBuilder::new()
        .with_accounts(vec![AccountConfig { gas_amounts: vec![DEFAULT_GAS_AMOUNT; 30], address: None }])
        .build()
        .await;

    let tx_map = create_txes(&test_cluster).await;
    for (tx_type, tx) in tx_map {
        let gas_used = test_cluster.sign_and_execute_transaction(&tx).await.effects.unwrap().gas_cost_summary().clone();
