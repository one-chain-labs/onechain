...
pub fn gen_object_ref() -> impl Strategy<Value = ObjectRef> {
    (any::<AccountAddress>(), any::<SequenceNumber>(), any::<[u8; 32]>())
        .prop_map(move |(addr, seq, seed)| (ObjectID::from_address(addr), seq, ObjectDigest::new(seed)))
}
...
impl<K: Strategy<Value = TransactionKind>, G: Strategy<Value = GasData>, E: Strategy<Value = TransactionExpiration>>
    TransactionDataGenBuilder<K, G, E>
{
    pub fn new(sender: SuiAddress) -> Self {
        Self { kind: None, sender, gas_data: None, expiration: None }
    }
    ...
    pub fn finish(self) -> impl Strategy<Value = TransactionData> {
        (
            ...
            .prop_map(|(kind, sender, gas_data, expiration)| TransactionDataV1 { kind, sender, gas_data, expiration })
