    ...
    S: Send + Clone + Sync + 'static,
{
    ...
    Router::new().route("/health", get(Json(HealthResponse::new(app_name, app_version))))
}
...
pub async fn serve(app: Router) -> Result<()> {
    ...
    let listener = tokio::net::TcpListener::bind(&format!("0.0.0.0:{}", DEFAULT_PORT)).await.unwrap();
