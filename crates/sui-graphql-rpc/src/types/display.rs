...
impl Display {
    ...
    pub(crate) fn render(&self, bytes: &[u8], layout: &A::MoveTypeLayout) -> Result<Vec<DisplayEntry>, Error> {
        let fields = self.stored.to_display_update_event().map_err(|e| Error::Internal(e.to_string()))?.fields;
