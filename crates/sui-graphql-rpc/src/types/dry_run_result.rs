    ...
    fn try_from(result: SuiExecutionResult) -> Result<Self, Self::Error> {
        let mutated_references = result
            ...
            .map(|(argument, bcs, type_)| {
                ...
                Ok(DryRunMutation { input: (*argument).into(), type_: tag.into(), bcs: bcs.into() })
            })
            .collect::<Result<Vec<_>, anyhow::Error>>()
            .map_err(|e| Error::Internal(format!("Failed to parse results returned from dev inspect: {:?}", e)))?;
        let return_values = result
            .return_values
            .iter()
            .map(|(bcs, type_)| {
                let tag: TypeTag = type_.clone().try_into()?;
                Ok(DryRunReturn { type_: tag.into(), bcs: bcs.into() })
            })
            .collect::<Result<Vec<_>, anyhow::Error>>()
            .map_err(|e| Error::Internal(format!("Failed to parse results returned from dev inspect: {:?}", e)))?;
        Ok(Self { mutated_references: Some(mutated_references), return_values: Some(return_values) })
    }
}
...
impl TryFrom<DevInspectResults> for DryRunResult {
    ...
    fn try_from(dev_inspect_results: DevInspectResults) -> Result<Self, Self::Error> {
        ...
        let results = if let Some(results) = dev_inspect_results.results {
            Some(results.into_iter().map(DryRunEffect::try_from).collect::<Result<Vec<_>, Error>>()?)
        } else {
            None
        };
        let events = dev_inspect_results.events.data.into_iter().map(|e| e.into()).collect();
        ...
        let transaction = Some(TransactionBlock {
            inner: TransactionBlockInner::DryRun { tx_data, effects, events },
        ...
        Ok(Self { error: dev_inspect_results.error, results, transaction })
    }
}
