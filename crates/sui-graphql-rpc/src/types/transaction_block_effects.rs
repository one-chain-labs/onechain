...
impl TransactionBlockEffects {
    ...
    async fn abort_code(&self, ctx: &Context<'_>) -> Result<Option<BigInt>> {
        let resolver: &PackageResolver = ctx.data_unchecked();
        let status = self.resolve_native_status_impl(resolver).await?;
        let NativeExecutionStatus::Failure { error: ExecutionFailureStatus::MoveAbort(loc, code), .. } = status else {
            return Ok(None);
        };

        let Some(CleverError { error_code: Some(error_code), .. }) =
            resolver.resolve_clever_error(loc.module.clone(), code).await
        else {
    ...
    async fn unchanged_consensus_objects(
        ...
        let Some((prev, next, _, cs)) =
            page.paginate_consistent_indices(input_consensus_objects.len(), self.checkpoint_viewed_at)?
        else {
            return Ok(connection);
        };
        ...
        for c in cs {
            let result = UnchangedConsensusObject::try_from(input_consensus_objects[c.ix].clone(), c.c);
            match result {
                Ok(unchanged_consensus_object) => {
                    connection.edges.push(Edge::new(c.encode_cursor(), unchanged_consensus_object));
                }
