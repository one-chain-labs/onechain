...
impl UnchangedConsensusObject {
    pub fn try_from(
        ...
        match input {
            ...
            I::ReadOnly(oref) => {
                Ok(U::Read(ConsensusObjectRead { read: ObjectRead { native: oref, checkpoint_viewed_at } }))
            }

            I::ReadConsensusStreamEnded(id, v) => Ok(U::ConsensusStreamEnded(ConsensusObjectStreamEnded {
                address: id.into(),
                version: v.value().into(),
                mutable: false,
            })),

            I::MutateConsensusStreamEnded(id, v) => Ok(U::ConsensusStreamEnded(ConsensusObjectStreamEnded {
                address: id.into(),
                version: v.value().into(),
                mutable: true,
            })),
            ...
            I::Cancelled(id, v) => {
                Ok(U::Cancelled(ConsensusObjectCancelled { address: id.into(), version: v.value().into() }))
            }
        }
    }
}
