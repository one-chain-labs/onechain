...
impl ExtensionFactory for Timeout {
    fn create(&self) -> Arc<dyn Extension> {
        Arc::new(TimeoutExt { query: Mutex::new(None), is_mutation: AtomicBool::new(false) })
    }
}
...
impl Extension for TimeoutExt {
    async fn parse_query(
        ...
        *self.query.lock().unwrap() = Some(ctx.stringify_execute_doc(&document, variables));
        ...
        let is_mutation = document.operations.iter().any(|(_, operation)| operation.node.ty == OperationType::Mutation);
    ...
    async fn execute(
        ...
        let cfg: &ServiceConfig = ctx.data().expect("No service config provided in schema data");
        ...
        timeout(request_timeout, next.run(ctx, operation_name)).await.unwrap_or_else(|_| {
            ...
            let error_msg = if is_mutation {
                format!("Mutation request timed out. Limit: {}s", request_timeout.as_secs_f32())
            } else {
                format!("Query request timed out. Limit: {}s", request_timeout.as_secs_f32())
            };
