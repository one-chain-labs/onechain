...
async fn test_simple_client_validator_cluster() {
    ...
    let query = r#"
        ...
        "#;
    let res = cluster.graphql_client.execute(query.to_string(), vec![]).await.unwrap();
...
async fn test_simple_client_simulator_cluster() {
    ...
    let genesis_checkpoint_digest1 = *sim.store().get_checkpoint_by_sequence_number(0).unwrap().digest();
...
async fn test_graphql_client_response() {
    ...
    let query = r#"
        ...
        "#;
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, vec![], vec![]).await.unwrap();
...
async fn test_graphql_client_variables() {
    ...
    let variables = vec![
        GraphqlQueryVariable { name: "framework_addr".to_string(), ty: "SuiAddress!".to_string(), value: json!("0x2") },
    ...
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, variables, vec![]).await.unwrap();

    assert!(res.errors().is_empty());
    let data = res.response_body().data.clone().into_json().unwrap();
    data.get("obj1").unwrap().get("address").unwrap();
    assert_eq!(
        data.get("obj1").unwrap().get("address").unwrap().as_str().unwrap(),
        SUI_FRAMEWORK_ADDRESS.to_canonical_string(true)
    );
    assert_eq!(
        data.get("obj2").unwrap().get("address").unwrap().as_str().unwrap(),
    ...
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, bad_variables, vec![]).await;
    ...
    let bad_variables = vec![
        GraphqlQueryVariable { name: "framework addr".to_string(), ty: "SuiAddress!".to_string(), value: json!("0x2") },
        GraphqlQueryVariable {
            name: " deepbook_addr".to_string(),
            ty: "SuiAddress!".to_string(),
            value: json!("0xdee9"),
        ...
        GraphqlQueryVariable { name: "".to_string(), ty: "SuiAddress!".to_string(), value: json!("0xdee9") },
        GraphqlQueryVariable { name: " ".to_string(), ty: "SuiAddress!".to_string(), value: json!("0xdee9") },
    ];

    for var in bad_variables {
        let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, vec![var.clone()], vec![]).await;

        assert!(res.is_err());
        assert!(
            res.unwrap_err().to_string() == ClientError::InvalidVariableName { var_name: var.name.clone() }.to_string()
        );
    }
}
...
async fn test_transaction_execution() {
    ...
    let variables = vec![
        GraphqlQueryVariable { name: "tx".to_string(), ty: "String!".to_string(), value: json!(tx_bytes) },
        GraphqlQueryVariable { name: "sigs".to_string(), ty: "[String!]!".to_string(), value: json!(sigs) },
    ];
    let res = cluster.graphql_client.execute_mutation_to_graphql(mutation.to_string(), variables).await.unwrap();
    let binding = res.response_body().data.clone().into_json().unwrap();
    let res = binding.get("executeTransactionBlock").unwrap();

    let digest = res.get("effects").unwrap().get("transactionBlock").unwrap().get("digest").unwrap().as_str().unwrap();
    ...
    let variables =
        vec![GraphqlQueryVariable { name: "dig".to_string(), ty: "String!".to_string(), value: json!(digest) }];
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, variables, vec![]).await.unwrap();

    let binding = res.response_body().data.clone().into_json().unwrap();
    let sender_read =
        binding.get("transactionBlock").unwrap().get("sender").unwrap().get("address").unwrap().as_str().unwrap();
...
async fn test_zklogin_sig_verify() {
    ...
    let variables = vec![
        GraphqlQueryVariable { name: "bytes".to_string(), ty: "String!".to_string(), value: json!(bytes) },
        GraphqlQueryVariable { name: "signature".to_string(), ty: "String!".to_string(), value: json!(signature) },
        GraphqlQueryVariable {
            name: "intent_scope".to_string(),
            ty: "ZkLoginIntentScope!".to_string(),
            value: json!(intent_scope),
        },
        GraphqlQueryVariable { name: "author".to_string(), ty: "SuiAddress!".to_string(), value: json!(author) },
    ];
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, variables, vec![]).await.unwrap();
    ...
    let res =
        cluster.graphql_client.execute_to_graphql(query.to_string(), true, incorrect_variables, vec![]).await.unwrap();
    let binding = res.response_body().data.clone().into_json().unwrap();
    let res = binding.get("verifyZkloginSignature").unwrap();
...
async fn test_transaction_dry_run() {
    ...
    let variables =
        vec![GraphqlQueryVariable { name: "tx".to_string(), ty: "String!".to_string(), value: json!(tx_bytes) }];
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, variables, vec![]).await.unwrap();
    let binding = res.response_body().data.clone().into_json().unwrap();
    ...
    let sender_read = res.get("transaction").unwrap().get("sender").unwrap().get("address").unwrap().as_str().unwrap();
    assert_eq!(sender_read, sender.to_string());
...
async fn test_transaction_dry_run_with_kind() {
    ...
    let variables =
        vec![GraphqlQueryVariable { name: "tx".to_string(), ty: "String!".to_string(), value: json!(tx_kind_bytes) }];
    let res = cluster.graphql_client.execute_to_graphql(query.to_string(), true, variables, vec![]).await.unwrap();
    let binding = res.response_body().data.clone().into_json().unwrap();
    let res = binding.get("dryRunTransactionBlock").unwrap();
...
async fn test_dry_run_failed_execution() {
    ...
    assert!(res.get("error").unwrap().as_str().unwrap().contains("UnusedValueWithoutDrop"));
}
...
async fn test_epoch_live_object_set_digest() {
    ...
    cluster.network.validator_fullnode_handle.trigger_reconfiguration().await;

    // Wait for the epoch to be indexed
    cluster.wait_for_epoch_catchup(0, Duration::from_secs(30)).await;
    ...
    assert!(!binding.get("epoch").unwrap().get("liveObjectSetDigest").unwrap().is_null());
}
...
async fn test_payload_using_vars_mutation_passes() {
    ...
    let cluster = sui_graphql_rpc::test_infra::cluster::start_cluster(ServiceConfig {
        limits: Limits { max_query_payload_size: 5000, max_tx_payload_size: 6000, ..Default::default() },
