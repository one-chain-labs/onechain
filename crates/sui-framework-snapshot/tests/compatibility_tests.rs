...
mod compatibility_tests {
    ...
    use move_package::source_package::{
        ...
        parsed_manifest::SourceManifest,
    };
    ...
    fn check_manifest_against_tomls() {
        ...
        for entry in manifest.values() {
            for package in entry.packages.iter() {
                ...
                let toml_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("..").join("..").join(&package.path);
                let package_toml: SourceManifest = parse_move_manifest_from_file(&toml_path).expect("Move.toml exists");
                ...
                let published_at_field =
                    published_at_property(&package_toml).expect("Move.toml file has published-at field");
