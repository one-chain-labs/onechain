...
async fn test_publish_primitive() {
    ...
    let (simple_oracle_id, version) = option_env!("oracle_id")
        .and_then(|id| {
            option_env!("oracle_version")
                .map(|version| (ObjectID::from_str(id).unwrap(), u64::from_str(version).unwrap().into()))
        })
    ...
    for i in 1..200 {
        ...
        let ticker = builder.input(CallArg::Pure(bcs::to_bytes(ticker.as_bytes()).unwrap())).unwrap();
        let identifier = builder.input(CallArg::Pure(bcs::to_bytes("identifier".as_bytes()).unwrap())).unwrap();

        builder.programmable_move_call(package, module.clone(), submit_data.clone(), vec![TypeTag::U64], vec![
            simple_oracle,
            clock,
            ticker,
            value,
...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        let decimal_value =
            builder.programmable_move_call(package, data_types.clone(), create_decimal.clone(), vec![], vec![
                value, decimal,
            ]);
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let value = builder.input(CallArg::Pure(bcs::to_bytes(&10000000u64).unwrap())).unwrap();
    ...
    let test_package = publish_package(sender, &keystore, &client, Path::new("tests/data/Test")).await;
    // get data
    let mut builder = ProgrammableTransactionBuilder::new();
    let simple_oracle = builder
        .input(CallArg::Object(ObjectArg::SharedObject {
    ...
    let mist_amount = builder.input(CallArg::Pure(bcs::to_bytes(&10000000u64).unwrap())).unwrap();
    builder.programmable_move_call(test_package, test_module.clone(), simple_fx_ptb, vec![], vec![data, mist_amount]);
    ...
    builder
        .programmable_move_call(test_package, test_module.clone(), simple_fx, vec![], vec![simple_oracle, mist_amount]);
...
async fn init_test_client() -> (SuiClient, Keystore, SuiAddress) {
    ...
    assert!(!gas.data.is_empty(), "No gas coin found in account, please fund [{}]", sender);

    (client, keystore, sender)
}

async fn publish_package(sender: SuiAddress, keystore: &Keystore, client: &SuiClient, path: &Path) -> ObjectID {
    ...
    let result = client
        ...
        .await
        .unwrap();
    assert_eq!(&SuiExecutionStatus::Success, result.effects.unwrap().status());
    ...
    let ObjectChange::Published { package_id, .. } = publish else { panic!("Expected published object change") };
    package_id
}
