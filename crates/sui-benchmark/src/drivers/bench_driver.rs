...
use crate::{
    ...
    workloads::{payload::Payload, workload::ExpectedFailureType, GroupID, WorkloadInfo},
    ExecutionEffects,
    ValidatorProxy,
};
     ...
     -> NextOp {
        match result {
            Ok(effects) => {
                ...
                metrics.benchmark_duration.set(time_from_start.as_secs() as i64);
                ...
                if effects.is_ok() {
                    metrics.num_success.with_label_values(&[&payload.to_string(), &client_type.to_string()]).inc();
                    metrics
                        .num_success_cmds
                        .with_label_values(&[&payload.to_string(), &client_type.to_string()])
                    ...
                    metrics
                        .num_error
                        .with_label_values(&[&payload.to_string(), "execution", &client_type.to_string()])
                        .inc();
                }

                if let Some(sig_info) = effects.quorum_sig() {
                    sig_info.authorities(&committee).for_each(|name| {
                        metrics.validators_in_effects_cert.with_label_values(&[&name.unwrap().to_string()]).inc()
                    })
                }
            ...
            Err(err) => {
                ...
                match payload.get_failure_type() {
                    Some(ExpectedFailureType::NoFailure) => {
                        panic!("Transaction {} failed unexpectedly {}", transaction.digest(), err);
                    }
                    Some(_) => {
                        metrics
                            .num_expected_error
                    ...
                    None => {
                            ...
                            .is_err()
                        {
                            ...
                            metrics
                                .num_error
                                .with_label_values(&[&payload.to_string(), "rpc", &client_type.to_string()])
                                .inc();
                            NextOp::Retry(Box::new((transaction, payload)))
                        }
                    }
