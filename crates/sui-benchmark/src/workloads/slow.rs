...
impl Payload for SlowTestPayload {
    fn make_new_payload(&mut self, effects: &ExecutionEffects) {
        debug_assert!(effects.is_ok() || effects.is_cancelled(), "Slow transactions should never abort: {effects:?}",);
    ...
    pub fn from(
        ...
        if max_ops == 0 || num_workers == 0 {
            ...
            let workload_builder =
                Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(SlowWorkloadBuilder { num_payloads: max_ops }));
...
impl Workload<dyn Payload> for SlowWorkload {
    async fn init(
        ...
        assert!(self.shared_obj_ref.0 != ObjectID::ZERO, "Dynamic field parent must be created");
