...
impl Payload for SharedCounterTestPayload {
    ...
    fn make_transaction(&mut self) -> Transaction {
        ...
        let gas_price_increment =
            if self.max_tip_amount == 0 { 0 } else { rand::thread_rng().gen_range(0..self.max_tip_amount) };
        let gas_price = rgp + gas_price_increment;
        TestTransactionBuilder::new(self.gas.1, self.gas.0, gas_price)
            .call_counter_increment(self.package_id, self.counter_id, self.counter_initial_shared_version)
...
impl SharedCounterWorkloadBuilder {
    pub fn from(
        ...
        let shared_counter_ratio = 1.0 - (std::cmp::min(shared_counter_hotness_factor, 100) as f32 / 100.0);
        let num_shared_counters =
            num_shared_counters.unwrap_or(std::cmp::max(1, (max_ops as f32 * shared_counter_ratio) as u64));
        if max_ops == 0 || num_shared_counters == 0 || num_workers == 0 {
            ...
            let workload_builder =
                Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(SharedCounterWorkloadBuilder {
...
impl Workload<dyn Payload> for SharedCounterWorkload {
    async fn init(
        ...
        let (head, tail) = self.init_gas.split_first().expect("Not enough gas to initialize shared counter workload");
    ...
    async fn make_test_payloads(
        ...
        debug!("num of gas = {:?}, {:?}", self.payload_gas.len(), self.counters.len());
        for g in self.payload_gas.iter() {
            // pick a random counter from the pool
            let counter_ref =
                self.counters.choose(&mut rand::thread_rng()).expect("Failed to get a random counter from the pool");
