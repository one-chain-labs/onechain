...
use sui_types::{
    base_types::{FullObjectID, FullObjectRef, ObjectID, SuiAddress},
    crypto::get_key_pair,
    object::Owner,
...
impl Payload for PartyTestPayload {
    fn make_new_payload(&mut self, effects: &ExecutionEffects) {
        assert!(effects.is_ok(), "Party transactions should never abort: {effects:?}",);
        ...
        for (obj, owner) in effects.mutated().into_iter().chain(effects.created()) {
            ...
            match owner {
                ...
                Owner::ConsensusAddressOwner { start_version, owner } => {
                    self.object_ref = FullObjectRef(FullObjectID::Consensus((obj.0, start_version)), obj.1, obj.2);
                    self.sender = owner;
                }
    ...
    fn create_transaction(&self) -> Transaction {
        ...
        let mut tx_builder = TestTransactionBuilder::new(self.sender, account.gas, gas_price);
        {
            ...
            let args = vec![
                builder
                    .obj(match &self.object_ref.0 {
                        FullObjectID::Fastpath(_) => ObjectArg::ImmOrOwnedObject(self.object_ref.as_object_ref()),
            ...
            builder.programmable_move_call(
                ...
                [Identifier::new("transfer_party").unwrap(), Identifier::new("transfer_fastpath").unwrap()]
                    .into_iter()
    ...
    pub fn from(
        ...
        if max_ops == 0 || num_workers == 0 {
            ...
            let workload_builder =
                Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(PartyWorkloadBuilder { num_payloads: max_ops }));
...
impl Workload<dyn Payload> for PartyWorkload {
    async fn init(
        ...
        let (first_gas, _) = self.init_gas.split_first().expect("Not enough gas to initialize party workload");

        let mut path = benchmark_move_base_dir();
        path.push("src/workloads/data/party");
        let SystemState { reference_gas_price, protocol_config: _ } = system_state_observer.state.borrow().clone();
        let transaction = TestTransactionBuilder::new(first_gas.1, first_gas.0, reference_gas_price)
            .publish(path)
    ...
    async fn make_test_payloads(
        ...
        for (gas, sender, keypair) in &self.payload_gas[.. self.payload_gas.len() / 2] {
            ...
            futures.push(async move {
                let (_, execution_result) = proxy.execute_transaction_block(transaction).await;
                let effects = execution_result.unwrap();
                let (obj_ref, Owner::ConsensusAddressOwner { start_version, owner }) = &effects.created()[0] else {
                ...
                let full_ref = FullObjectRef(FullObjectID::Consensus((obj_ref.0, *start_version)), obj_ref.1, obj_ref.2);
                let sender = *owner;
                PartyTestPayload {
