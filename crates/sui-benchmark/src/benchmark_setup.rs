...
impl Env {
    pub async fn setup(&self, barrier: Arc<Barrier>, registry: &Registry, opts: &Opts) -> Result<BenchmarkSetup> {
        match self {
            Env::Local => {
                self.setup_local_env(barrier, registry, opts.committee_size as usize, opts.num_server_threads).await
            }
            Env::Remote => {
    ...
    async fn setup_local_env(
        ...
        let (primary_gas_owner, keypair): (SuiAddress, AccountKeyPair) = deterministic_random_account_key();
        ...
        let join_handle = std::thread::spawn(move || {
            ...
            server_runtime.block_on(async move {
                ...
                let primary_gas =
                    cluster.wallet.get_one_gas_object_owned_by_address(primary_gas_owner).await.unwrap().unwrap();
                // Send genesis and primary gas object to the main thread.
                genesis_sender.send((genesis, primary_gas)).unwrap();
                barrier.wait().await;
                shutdown_recv.await.expect("Unable to wait for terminate signal");
            });
        });
    ...
    async fn setup_remote_env(
        ...
        let proxies: Vec<Arc<dyn ValidatorProxy + Send + Sync>> = if use_fullnode_for_execution {
            ...
            vec![Arc::new(
                LocalValidatorAggregatorProxy::from_genesis(genesis, registry, reconfig_fullnode_rpc_url.map(|x| &**x))
                    .await,
            )]
        };
        let proxy = proxies.choose(&mut rand::thread_rng()).context("Failed to get proxy for reconfiguration")?;
        info!("Reconfiguration - Reconfiguration to epoch {} is done", proxy.get_current_epoch(),);

        let primary_gas_owner_addr = ObjectID::from_hex_literal(primary_gas_owner_id)?;
        let keystore_path = Some(&keystore_path)
            .filter(|s| !s.is_empty())
            .map(PathBuf::from)
            .ok_or_else(|| anyhow!(format!("Failed to find keypair at path: {}", &keystore_path)))?;

        let current_gas = if use_fullnode_for_execution {
            // Go through fullnode to get the current gas object.
            let mut gas_objects = proxy.get_owned_objects(primary_gas_owner_addr.into()).await?;
            ...
            let (balance, primary_gas_obj) = gas_objects
                ...
                .context("Failed to choose the gas object with the largest amount of gas".to_string())?;

            info!("Using primary gas id: {} with balance of {balance}", primary_gas_obj.id());

            let primary_gas_account = primary_gas_obj.owner.get_owner_address()?;

            let keypair = Arc::new(get_ed25519_keypair_from_keystore(keystore_path, &primary_gas_account)?);

            (primary_gas_obj.compute_object_reference(), primary_gas_account, keypair)
        } else {
            ...
            let keypair = Arc::new(get_ed25519_keypair_from_keystore(keystore_path, &current_gas_account)?);

            info!("Using primary gas obj: {}", current_gas_object.id());

            (current_gas_object.compute_object_reference(), current_gas_account, keypair)
        };
