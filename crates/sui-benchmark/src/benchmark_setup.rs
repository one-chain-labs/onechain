...
impl BenchmarkSetup {
    pub async fn new(barrier: Arc<Barrier>, registry: &Registry, opts: &Opts) -> Result<BenchmarkSetup> {
        ...
        let proxies: Vec<Arc<dyn ValidatorProxy + Send + Sync>> = if opts.use_fullnode_for_execution {
            ...
            vec![Arc::new(
                LocalValidatorAggregatorProxy::from_genesis(genesis, registry, reconfig_fullnode_rpc_url, None).await,
            )]
        };
        ...
        let keystore_path = Some(&opts.keystore_path)
            .filter(|s| !s.is_empty())
            .map(PathBuf::from)
            .ok_or_else(|| anyhow!(format!("Failed to find keypair at path: {}", &opts.keystore_path)))?;
