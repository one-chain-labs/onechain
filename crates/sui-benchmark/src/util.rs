...
pub async fn publish_basics_package(
    ...
    for retry in 1 ..= max_retries {
        ...
        tracing::info!("Publishing basics package with tx digest {:?} (attempt {})", transaction.digest(), retry);

        let (client_type, execution_result) = proxy.execute_transaction_block(transaction).await;
        tracing::debug!("Executed publish_basics_package transaction via {:?}", client_type);
        ...
        match execution_result {
            ...
            Err(e) => {
                tracing::error!("Attempt {}: Failed to execute publish_basics_package transaction: {:?}", retry, e);
            }
        }
        ...
        match proxy.get_object(current_gas.0).await {
            Ok(gas_obj) => {
                ...
                tracing::info!("Retry {}: Using updated gas object version {:?}", retry, current_gas);
            }
            Err(e) => {
