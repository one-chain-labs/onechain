...
impl Summary {
    pub fn execute(
        ...
        self.summary.execute(path, build_config, Some(&sui_package_metadata), Some(Self::derive_ids))
    }

    fn derive_ids(resolved_graph: &mut ResolvedGraph) -> anyhow::Result<()> {
        let root_pkg = resolved_graph.package_table.get_mut(&resolved_graph.root_package()).unwrap();
        let in_use_addrs = root_pkg.resolved_table.values().cloned().collect::<BTreeSet<_>>();
        ...
        for (pkg_name, pkg) in resolved_graph.package_table.iter_mut() {
            let package_root_renaming = root_renaming.get(pkg_name).expect("Will always be present");
