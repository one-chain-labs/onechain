...
use sui_move::{self, execute_move_command, manage_package::resolve_lock_file_path, summary::PackageSummaryMetadata};
use sui_move_build::{
    check_conflicting_addresses,
    check_invalid_dependencies,
    check_unpublished_dependencies,
    implicit_deps,
    BuildConfig as SuiBuildConfig,
    SuiPackageHooks,
};
use sui_package_management::system_package_versions::latest_system_packages;
use sui_pg_db::{
    temp::{get_available_port, LocalDatabase},
    DbArgs,
};
...
use crate::{
    client_commands::{implicit_deps_for_protocol_version, pkg_tree_shake, SuiClientCommands, USER_AGENT},
    fire_drill::{run_fire_drill, FireDrill},
    genesis_ceremony::{run, Ceremony},
    keytool::KeyToolCommand,
    trace_analysis_commands::AnalyzeTraceCommand,
...
impl RpcArgs {
    pub fn for_testing() -> Self {
        Self { with_indexer: None, with_consistent_store: None, with_graphql: None }
    }
}
...
pub enum SuiCommand {
    ...
    ///
    /// By default, one start will start a local network from the genesis blob that exists in
    ...
    Start {
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `one genesis` command
    ...
    Genesis {
        ...
        #[clap(long, help = "Creates an extra faucet configuration for one persisted runs.")]
        with_faucet: bool,
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            ...
            SuiCommand::Client { config, cmd, json, accept_defaults } => {
                let config_path = config.config.unwrap_or(sui_config_dir()?.join(SUI_CLIENT_CONFIG));
            ...
            SuiCommand::Move { package_path, build_config, mut cmd, config: client_config } => {
                match cmd {
                    sui_move::Command::Summary(mut s) if s.package_id.is_some() => {
                        let (_, client) =
                            get_chain_id_and_client(client_config, "one move summary --package-id <object_id>").await?;
                        ...
                        let root_package_id =
                            s.package_id.as_ref().expect("Safe since we checked in the match statement");
                        ...
                        let package_metadata = download_package_and_deps_under(read_api, path, *root_package_id).await?;
                    ...
                    sui_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        ...
                        let (chain_id, client) = if build.ignore_chain {
                            ...
                            get_chain_id_and_client(client_config, "one move build --dump-bytecode-as-base64").await?
                        };
                ...
                if client_config.env.is_some() && matches!(cmd, sui_move::Command::Build(_)) {
                    let (chain_id, _) = get_chain_id_and_client(client_config, "one move build").await?;
            ...
            SuiCommand::AnalyzeTrace { path, output_dir, command } => command.execute(path, output_dir).await,
            SuiCommand::ReplayTransaction { config, replay_config } => {
                let config_path = config.config.unwrap_or(sui_config_dir()?.join(SUI_CLIENT_CONFIG));
                ...
                let experimental_config = SR2::ReplayConfigExperimental { node, ..Default::default() };

                let artifact_path = SR2::handle_replay_config(&stable_config, &experimental_config, USER_AGENT).await?;
...
async fn start(
    ...
    let RpcArgs { with_indexer, mut with_consistent_store, with_graphql } = rpc_args;
    ...
    let config_dir = if force_regenesis {
        ...
        // overriding the network.yaml found in the one config directry. Otherwise it is treated as
        // the one config directory for backwards compatibility with `one-test-validator`.
    ...
    if no_full_node {
        ...
        let rpc_config = sui_config::RpcConfig { enable_indexing: Some(true), ..Default::default() };
    ...
    let fullnode_rpc_url = socket_addr_to_url(fullnode_rpc_address)?.to_string().trim_end_matches("/").to_string();
    ...
    let pipelines = if let Some(ref db_url) = database_url {
        let client_args = ClientArgs {
            ingestion: IngestionClientArgs { local_ingestion_path: data_ingestion_dir.clone(), ..Default::default() },
            ..Default::default()
        };
    ...
    let consistent_store_url = if let Some(input) = with_consistent_store {
        let address =
            parse_host_port(input, DEFAULT_CONSISTENT_STORE_PORT).context("Invalid consistent store host and port")?;
        ...
        let consistent_args = ConsistentArgs { rpc_listen_address: address, ..Default::default() };
    ...
    if let Some(input) = with_graphql {
        let address = parse_host_port(input, DEFAULT_GRAPHQL_PORT).context("Invalid graphql host and port")?;

        info!("Starting the GraphQL service at {address}");

        let graphql_args = GraphQlArgs { rpc_listen_address: address, no_ide: false };

        let consistent_store_url = consistent_store_url.map(socket_addr_to_url).transpose()?;
        let consistent_reader_args = ConsistentReaderArgs { consistent_store_url, ..Default::default() };
    ...
    if let Some(input) = with_faucet {
        ...
        if force_regenesis {
            ...
            keystore.import(None, SuiKeyPair::Ed25519(kp)).await.unwrap();
        ...
        let app_state = Arc::new(AppState { faucet: local_faucet, config });
...
async fn genesis(
    ...
    let dir = sui_config_dir
        .read_dir()
        .map_err(|err| anyhow!(err).context(format!("Cannot open OneChain config dir {:?}", sui_config_dir)))?;
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            bail!(
                "Cannot run genesis with non-empty OneChain config directory {}, please use the --force/-f option to remove the existing configuration",
    ...
    let rpc = socket_addr_to_url(fullnode_config.json_rpc_address)?.to_string().trim_end_matches("/").to_string();
    client_config.add_env(SuiEnv { alias: "localnet".to_string(), rpc, ws: None, basic_auth: None, chain_id: None });
    ...
    if !wallet_conf_path.exists() {
            ...
            None => {
                ...
                if accept_defaults || matches!(read_line(), Ok(line) if line.trim().to_lowercase() == "y") {
                    let url = if accept_defaults {
                        ...
                        print!("OneChain Full node server URL (Defaults to OneChain Testnet if not specified) : ");
                        read_line()?
                    };
                    Some(if url.trim().is_empty() {
                        ...
                        SuiEnv { alias, rpc: url, ws: None, basic_auth: None, chain_id: None }
                    })
                } else {
                    None
                }
        ...
        if let Some(env) = env {
            let keystore_path = match wallet_conf_path.parent() {
                ...
                Some(parent) => parent.canonicalize().context("Could not find one config directory")?,
...
async fn get_chain_id_and_client(
    ...
    let config = client_config.config.unwrap_or(sui_config_dir()?.join(SUI_CLIENT_CONFIG));
    ...
    Ok((client.read_api().get_chain_identifier().await.ok(), Some(client)))
}
...
async fn resolve_package(reader: &ReadApi, package_id: ObjectID) -> anyhow::Result<MovePackage> {
    let object =
        reader.get_object_with_options(package_id, SuiObjectDataOptions::bcs_lossless()).await?.into_object()?;
...
async fn download_package_and_deps_under(
    ...
    for (original_id, pkg_info) in root_package.linkage_table().iter() {
        ...
        let relative_package_path = package.id().deref().to_canonical_string(/* with_prefix */ true);

        let package_path = path.join(&relative_package_path);
        fs::create_dir_all(&package_path)?;
        for (m_name, module) in package.serialized_module_map() {
            let mut file = fs::File::create(package_path.join(m_name).with_extension(MOVE_COMPILED_EXTENSION))?;
    ...
    let package_path = path.join(root_package.id().deref().to_canonical_string(/* with_prefix */ true));
    fs::create_dir_all(&package_path)?;
    for (m_name, module) in root_package.serialized_module_map() {
        let file_path = package_path.join(m_name).with_extension(MOVE_COMPILED_EXTENSION);
...
pub async fn get_replay_node(context: &WalletContext) -> Result<sui_data_store::Node, anyhow::Error> {
    ...
    let chain_id = ChainIdentifier::from_chain_short_id(&chain_id).ok_or_else(|| anyhow::anyhow!(err_msg.clone()))?;
...
fn socket_addr_to_url(addr: SocketAddr) -> Result<Url, anyhow::Error> {
    ...
    Url::parse(&format!("http://{ip}:{}", addr.port())).with_context(|| format!("Failed to parse {addr} into a Url"))
}
...
fn update_wallet_config_rpc(
    ...
    let mut wallet_context =
        create_wallet_context(FaucetConfig::default().wallet_client_timeout_secs, config_dir.clone())?;
