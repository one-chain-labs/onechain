...
impl Display for PTBPreview<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(gas_sponsor) = self.program_metadata.gas_sponsor {
            builder.push_record([
                GAS_SPONSOR,
                gas_sponsor.value.into_inner().to_canonical_string(/* with_prefix */ true).as_str(),
            ]);
        }
        ...
        if let Some(sender) = &self.program_metadata.sender {
            builder
                .push_record([SENDER, sender.value.into_inner().to_canonical_string(/* with_prefix */ true).as_str()]);
        }
