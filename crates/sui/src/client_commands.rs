    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<SuiClientCommandResult, anyhow::Error> {
        let ret = match self {
            ...
            SuiClientCommands::RemoveAddress { alias_or_address } => {
                let address: SuiAddress = match context.config.keystore.get_address_by_alias(alias_or_address.clone()) {
                    Ok(addr) => *addr,
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        ...
        let mut signatures =
            vec![context.config.keystore.sign_secure(&signer, &tx_data, Intent::sui_transaction())?.into()];

        if let Some(gas_sponsor) = gas_sponsor {
            if gas_sponsor != signer {
                signatures.push(
                    context.config.keystore.sign_secure(&gas_sponsor, &tx_data, Intent::sui_transaction())?.into(),
                );
            }
        }
