...
use move_core_types::{account_address::AccountAddress, identifier::Identifier, language_storage::TypeTag};
...
use sui_keys::{key_derive, key_identity::KeyIdentity, keystore::AccountKeystore};
...
pub(crate) static USER_AGENT: &str = concat!(env!("CARGO_PKG_NAME"), "/", env!("CARGO_PKG_VERSION"),);
...
impl SuiClientCommands {
    pub async fn execute(self, context: &mut WalletContext) -> Result<SuiClientCommandResult, anyhow::Error> {
        let ret = match self {
            ...
            SuiClientCommands::Upgrade {
                ...
                let package_path = package_path.canonicalize().map_err(|e| SuiErrorKind::ModulePublishFailure {
                ...
                let gas_payment = client.transaction_builder().input_refs(&payment.gas).await?;

                let result =
                    dry_run_or_execute_or_serialize(sender, tx_kind, context, gas_payment, gas_data, processing).await?;
            ...
            SuiClientCommands::VerifyBytecodeMeter { protocol_version, module_paths, package_path, build_config } => {
                ...
                let (pkg_name, modules) = match (module_paths, package_path) {
                    ...
                    (_, package_path) => {
                        ...
                        let package = compile_package_simple(read_api, build_config, &package_path, None).await?;
            ...
            SuiClientCommands::Call { package, module, function, type_args, args, payment, gas_data, processing } => {
                ...
                let tx_kind =
                    client.transaction_builder().move_call_tx_kind(package, &module, &function, type_args, args).await?;
            ...
            SuiClientCommands::Transfer { to, object_id, payment, gas_data, processing } => {
                ...
                dry_run_or_execute_or_serialize(signer, tx_kind, context, gas_payment, gas_data, processing).await?
            }

            SuiClientCommands::TransferOct { to, coin_object_id: object_id, amount, gas_data, processing } => {
                let signer = context.get_object_owner(&object_id).await?;
                let to = context.get_identity_address(Some(to))?;
                let client = context.get_client().await?;
                let _ = context.cache_chain_id(&client).await?;
                ...
                let gas_payment = client.transaction_builder().input_refs(&[object_id]).await?;

                dry_run_or_execute_or_serialize(signer, tx_kind, context, gas_payment, gas_data, processing).await?
            }

            SuiClientCommands::Pay { input_coins, recipients, amounts, payment, gas_data, processing } => {
            ...
            SuiClientCommands::PayOct { input_coins, recipients, amounts, gas_data, processing } => {
                ...
                let gas_payment = client.transaction_builder().input_refs(&input_coins).await?;

                dry_run_or_execute_or_serialize(signer, tx_kind, context, gas_payment, gas_data, processing).await?
            }

            SuiClientCommands::PayAllOct { input_coins, recipient, gas_data, processing } => {
            ...
            SuiClientCommands::RemoveAddress { alias_or_address } => {
                let identity =
                    KeyIdentity::from_str(&alias_or_address).map_err(|e| anyhow!("Invalid address or alias: {}", e))?;
            ...
            SuiClientCommands::SplitCoin { coin_id, amounts, count, payment, gas_data, processing } => {
            ...
            SuiClientCommands::MergeCoin { primary_coin, coin_to_merge, payment, gas_data, processing } => {
                let client = context.get_client().await?;
                let _ = context.cache_chain_id(&client).await?;
            ...
            SuiClientCommands::SerializedTx { tx_bytes, processing } => {
            ...
            SuiClientCommands::SerializedTxKind { tx_bytes, payment, gas_data, processing } => {
                let Ok(bytes) = Base64::decode(&tx_bytes) else {
                    bail!("Invalid Base64 encoding");
                };
            ...
            SuiClientCommands::NewEnv { alias, rpc, ws, basic_auth } => {
                ...
                let mut env = SuiEnv { alias, rpc, ws, basic_auth, chain_id: None };
            ...
            SuiClientCommands::ActiveEnv => {
                SuiClientCommandResult::ActiveEnv(context.get_active_env().ok().map(|e| e.alias.clone()))
            }
            ...
            SuiClientCommands::PartyTransfer { to, object_id, payment, gas_data, processing } => {
                let signer = context.get_object_owner(&object_id).await?;
                let to = context.get_identity_address(Some(to))?;
                let client = context.get_client().await?;
                let _ = context.cache_chain_id(&client).await?;
                ...
                let (full_obj_ref, object_type) = transaction_builder.get_full_object_ref_and_type(object_id).await?;
                ...
                let object_input = builder.obj(match full_obj_ref.0 {
                    FullObjectID::Fastpath(_) => ObjectArg::ImmOrOwnedObject(full_obj_ref.as_object_ref()),
    ...
    pub fn switch_env(config: &mut SuiClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
            ...
            "Environment config not found for [{env:?}], add new environment config using the `one client new-env` command."
        );
...
pub(crate) async fn upgrade_package(
    ...
    pkg_tree_shake(read_api, with_unpublished_dependencies, &mut compiled_package).await?;
...
pub(crate) async fn compile_package(
    ...
    if compiled_package.get_package_bytes(with_unpublished_dependencies).is_empty() {
        return Err(SuiErrorKind::ModulePublishFailure { error: "No modules found in the package".to_string() }.into());
    }
...
pub(crate) fn implicit_deps_for_protocol_version(version: ProtocolVersion) -> anyhow::Result<Dependencies> {
...
pub async fn execute_dry_run(
    ...
    let response = client.read_api().dry_run_transaction_block(tx_data).await.context("Dry run failed")?;
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    let GasDataArgs { gas_budget, gas_price, gas_sponsor } = gas_data;
    ...
    if dev_inspect {
        return execute_dev_inspect(context, signer, tx_kind, gas_budget, gas_price, gas_payment, gas_sponsor, None)
            .await;
    }

    if dry_run {
        return execute_dry_run(context, signer, tx_kind, gas_budget, gas_price, gas_payment.clone(), None).await;
    }
    ...
    let gas_budget = match gas_budget {
        ...
        None => {
            ...
            let budget =
                estimate_gas_budget(context, signer, tx_kind.clone(), gas_price, gas_payment.clone(), gas_sponsor)
                    .await?;
    ...
    let gas_payment = if !gas_payment.is_empty() {
        ...
        let gas_payment = client
            .transaction_builder()
            .select_gas(gas_sponsor.unwrap_or(signer), None, gas_budget, input_objects, gas_price)
            .await?;
    ...
    if serialize_unsigned_transaction {
        ...
        let mut signatures =
            vec![context.config.keystore.sign_secure(&signer, &tx_data, Intent::sui_transaction()).await?.into()];

        if let Some(gas_sponsor) = gas_sponsor
            && gas_sponsor != signer
        {
            signatures.push(
                context.config.keystore.sign_secure(&gas_sponsor, &tx_data, Intent::sui_transaction()).await?.into(),
            );
        }
...
async fn fetch_move_packages(
    ...
    let pkg_id_to_name: BTreeMap<_, _> = immediate_dep_packages.iter().map(|(name, id)| (id, name)).collect();

    let objects = read_api.multi_get_object_with_options(package_ids, SuiObjectDataOptions::bcs_lossless()).await?;

    let mut packages = Vec::with_capacity(objects.len());
    for o in objects {
        let id = o.object_id()?;
        packages.push(to_package(o).with_context(|| {
            format!("Failed to fetch package {}", pkg_id_to_name.get(&id).map_or("of unknown name", |x| x.as_str()))
        })?);
    }
...
async fn trans_deps_original_ids(
    ...
    let linkage_table = pkgs.iter().flat_map(|pkg| pkg.linkage_table().keys()).copied().collect();
...
pub(crate) async fn pkg_tree_shake(
    ...
    let immediate_dep_packages = compiled_package.find_immediate_deps_pkgs_to_keep(with_unpublished_dependencies)?;
    ...
    compiled_package.dependency_ids.published.retain(|pkg, _| {
        immediate_dep_packages.contains_key(pkg)
            || pkg_name_to_orig_id.get(pkg).is_some_and(|id| trans_deps_orig_ids.contains(id))
    });
...
fn find_faucet_url(address: SuiAddress, rpc: &str) -> anyhow::Result<String> {
    ...
    if host == devnet_host {
        return Ok("https://faucet-devnet.onelabs.cc/v1/gas".to_string());
    }

    if host == testnet_host {
        return Ok("https://faucet-testnet.onelabs.cc/v1/gas".to_string());
    }
