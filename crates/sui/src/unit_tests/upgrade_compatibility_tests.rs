...
fn test_missing_module_toml() {
    ...
    for malformed_pkg in ["emoji", "addresses_first", "starts_second_line", "package_no_name", "whitespace", "empty"] {
        let move_pkg_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("src/unit_tests/fixtures/upgrade_errors/missing_module_toml/")
            .join(malformed_pkg);

        let move_toml_contents: Arc<str> =
            fs::read_to_string(move_pkg_path.join("Move.toml")).unwrap_or_default().into();
        let move_toml_hash = FileHash::new(&move_toml_contents);

        let result =
            missing_module_diag(&Identifier::from_str("identifier").unwrap(), &move_toml_hash, &move_toml_contents);

        let move_toml: Arc<str> = fs::read_to_string(move_pkg_path.join("Move.toml")).unwrap_or_default().into();
        ...
        let output = String::from_utf8(report_diagnostics_to_buffer(&files.into(), result.unwrap(), false)).unwrap();
...
fn get_packages(name: &str) -> (Vec<CompiledModule>, CompiledPackage, PathBuf) {
    ...
    let mods_v1 = BuildConfig::new_for_testing().build(&path).unwrap().into_modules();
