...
fn test_substitute_env_variables() {
    ...
    assert_eq!(test_string_4.clone(), substitute_env_variables(test_string_4));
}
...
fn test_completer() {
        ...
        command: CommandStructure {
            ...
            children: vec![
                ...
                CommandStructure {
                    ...
                    completions: vec!["--command_2_flag1".to_string(), "--command_2_flag2".to_string()],
                    children: vec![],
                },
            ],
        },
    ...
    let (start, candidates) = helper.complete("", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(0, start);
    assert_eq!(2, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
    assert_eq!(vec!["command1", "command2"], candidates);

    let (start, candidates) = helper.complete("command", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(0, start);
    assert_eq!(2, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
    assert_eq!(vec!["command1", "command2"], candidates);

    let (start, candidates) = helper.complete("command1 ", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(9, start);
    assert_eq!(3, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
    assert_eq!(vec!["--command_1_flag1", "--command_1_flag2", "--command_1_flag3"], candidates);

    let (start, candidates) = helper.complete("command2 ", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(9, start);
    assert_eq!(2, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
...
fn test_completer_with_cache() {
    ...
    let helper = ShellHelper {
        command: CommandStructure {
            ...
            children: vec![
                CommandStructure {
                    name: "command1".to_string(),
                    completions: vec!["--address".to_string(), "--gas".to_string(), "--other".to_string()],
                    children: vec![],
                },
                CommandStructure {
                    name: "command2".to_string(),
    ...
    completion_cache.write().unwrap().insert(CacheKey::flag("--gas"), vec![
        "Gas1".to_string(),
        "Gas2".to_string(),
        "Gas3".to_string(),
    ]);

    let (_, candidates) = helper.complete("command1 --gas ", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(5, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
    assert_eq!(vec!["Gas1", "Gas2", "Gas3", "--address", "--other"], candidates);

    let (_, candidates) = helper.complete("command2 --gas ", 1, &Context::new(&History::new())).unwrap();
    ...
    completion_cache
        .write()
        .unwrap()
        .insert(CacheKey::new("command1", "--address"), vec!["Address1".to_string(), "Address2".to_string()]);
    let (_, candidates) = helper.complete("command1 --address ", 1, &Context::new(&History::new())).unwrap();
    assert_eq!(4, candidates.len());
    let candidates = candidates.iter().map(|pair| pair.display.clone()).collect::<Vec<_>>();
    assert_eq!(vec!["Address1", "Address2", "--gas", "--other"], candidates);

    let (_, candidates) = helper.complete("command2 --address ", 1, &Context::new(&History::new())).unwrap();
...
fn test_split_line() {
    ...
    assert_eq!(vec!["create-example-nft", "--name", "test 1", "--description", "t e s t 2"], result);
}
