...
async fn test_print_raw_rgp_txn() -> Result<(), anyhow::Error> {
    ...
    let keypair: &SuiKeyPair = test_cluster.swarm.config().validator_configs.first().unwrap().account_key_pair.keypair();
    let validator_address: SuiAddress = SuiAddress::from(&keypair.public());
    let mut context = test_cluster.wallet;
    let sui_client = context.get_client().await?;
    let (_, summary) = get_validator_summary(&sui_client, validator_address).await?.unwrap();
    ...
    let deserialized_data = bcs::from_bytes::<TransactionData>(&Base64::decode(&serialized_data).unwrap())?;
    let signature = Signature::new_secure(&IntentMessage::new(Intent::sui_transaction(), deserialized_data), keypair);
