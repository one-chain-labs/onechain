    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if f.alternate() {
            write!(
                f,
                "{}",
                format_param(self.type_, self.type_params.to_vec(), &mut Vec::new()).map_err(|_| fmt::Error)?,
            )
        } else {
            write!(
                f,
...
impl<'f> FormattedField<'f> {
    pub(super) fn new(f: &'f Field, type_params: &'f [SourceName]) -> Self {
        ...
        let identifier = if let Some(ix) =
            RE_POS.captures(f.name.as_str()).and_then(|c| c.get(1)).and_then(|m| m.as_str().parse::<usize>().ok())
        {
        ...
        FormattedField { identifier, type_: FormattedType { type_: &f.type_, type_params } }
    }
}
...
pub(super) fn format_param(
    ...
    Ok(match param {
        Type::TypeParameter(t) => {
            let type_param = type_params.get(*t as usize).context("Unable to get type param location")?;

            secondary.push((type_param.1, format!("Type parameter '{}' is defined here", &type_param.0)));
        ...
        Type::Struct { address, module, name, type_arguments, .. } if !type_arguments.is_empty() => {
            format!(
                "{}::{}::{}<{}>",
...
pub(super) fn format_list(
    ...
    if let Some((singular, plural)) = noun_singular_plural {
        format!("{}: {}", singular_or_plural(items.len(), singular, plural), items_string,)
    } else {
        items_string
    }
}
