...
use sui_types::{base_types::ObjectID, execution_config_utils::to_binary_config, move_package::UpgradePolicy};
...
pub(crate) enum UpgradeCompatibilityModeError {
    StructMissing { name: Identifier },
    StructAbilityMismatch { name: Identifier, old_struct: Rc<Struct>, new_struct: Rc<Struct> },
    StructTypeParamMismatch { name: Identifier, old_struct: Rc<Struct>, new_struct: Rc<Struct> },
    StructFieldMismatch { name: Identifier, old_struct: Rc<Struct>, new_struct: Rc<Struct> },
    EnumMissing { name: Identifier },
    EnumAbilityMismatch { name: Identifier, old_enum: Rc<Enum>, new_enum: Rc<Enum> },
    EnumTypeParamMismatch { name: Identifier, old_enum: Rc<Enum>, new_enum: Rc<Enum> },
    EnumNewVariant { name: Identifier, old_enum: Rc<Enum>, new_enum: Rc<Enum> },
    EnumVariantMissing { name: Identifier, old_enum: Rc<Enum>, tag: usize },
    EnumVariantMismatch { name: Identifier, old_enum: Rc<Enum>, new_enum: Rc<Enum> },
    FunctionMissingPublic { name: Identifier },
    FunctionMissingEntry { name: Identifier },
    FunctionSignatureMismatch { name: Identifier, old_function: Rc<Function>, new_function: Rc<Function> },
    FunctionLostPublicVisibility { name: Identifier },
    FunctionEntryCompatibility { name: Identifier, old_function: Rc<Function> },

    // inclusion check specific errors
    FileFormatVersionDowngrade { old_version: u32, new_version: u32 },
    StructNew { name: Identifier },
    StructChange { name: Identifier, old_struct: Rc<Struct>, new_struct: Rc<Struct> },
    EnumNew { name: Identifier },
    EnumChange { name: Identifier, new_enum: Rc<Enum> },
    FunctionNew { name: Identifier },
    FunctionChange { name: Identifier, old_func: Rc<Function>, new_func: Rc<Function> },
    FunctionMissing { name: Identifier },
    FriendNew,
    FriendMissing,
}

/// Check if an `UpgradeCompatibilityModeError` variant breaks 'compatible' upgrades.
fn breaks_compatibility(error: &UpgradeCompatibilityModeError, compatability: &Compatibility) -> bool {
    match error {
        ...
        | UpgradeCompatibilityModeError::EnumVariantMismatch { .. } => compatability.check_datatype_layout,

        UpgradeCompatibilityModeError::StructMissing { .. } | UpgradeCompatibilityModeError::EnumMissing { .. } => true,
        ...
        | UpgradeCompatibilityModeError::FunctionEntryCompatibility { .. } => compatability.check_private_entry_linking,
...
fn breaks_inclusion_check(error: &UpgradeCompatibilityModeError, inclusion_check: &InclusionCheck) -> bool {
    match error {
        UpgradeCompatibilityModeError::StructNew { .. } => {
...
impl CompatibilityMode for CliCompatibilityMode {
    ...
    fn struct_missing(&mut self, name: &Identifier, _old_struct: &Rc<Struct>) {
        self.errors.push(UpgradeCompatibilityModeError::StructMissing { name: name.clone() });
    }

    fn struct_ability_mismatch(&mut self, name: &Identifier, old_struct: &Rc<Struct>, new_struct: &Rc<Struct>) {
    ...
    fn struct_type_param_mismatch(&mut self, name: &Identifier, old_struct: &Rc<Struct>, new_struct: &Rc<Struct>) {
    ...
    fn struct_field_mismatch(&mut self, name: &Identifier, old_struct: &Rc<Struct>, new_struct: &Rc<Struct>) {
    ...
    fn enum_missing(&mut self, name: &Identifier, _old_enum: &Rc<Enum>) {
        self.errors.push(UpgradeCompatibilityModeError::EnumMissing { name: name.clone() });
    }

    fn enum_ability_mismatch(&mut self, name: &Identifier, old_enum: &Rc<Enum>, new_enum: &Rc<Enum>) {
    ...
    fn enum_type_param_mismatch(&mut self, name: &Identifier, old_enum: &Rc<Enum>, new_enum: &Rc<Enum>) {
    ...
    fn function_missing_public(&mut self, name: &Identifier, _old_function: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionMissingPublic { name: name.clone() });
    }

    fn function_missing_entry(&mut self, name: &Identifier, _old_function: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionMissingEntry { name: name.clone() });
    }
    ...
    fn function_lost_public_visibility(&mut self, name: &Identifier, _old_function: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionLostPublicVisibility { name: name.clone() });
    }
    ...
    fn finish(self, compatability: &Compatibility) -> Result<(), Self::Error> {
        let errors: Vec<UpgradeCompatibilityModeError> =
            self.errors.into_iter().filter(|e| breaks_compatibility(e, compatability)).collect();
...
impl InclusionCheckMode for CliInclusionCheckMode {
    ...
    fn file_format_version_downgrade(&mut self, old_version: u32, new_version: u32) {
        self.errors.push(UpgradeCompatibilityModeError::FileFormatVersionDowngrade { old_version, new_version });
    }

    fn struct_new(&mut self, name: &Identifier, _new_struct: &Rc<Struct>) {
        self.errors.push(UpgradeCompatibilityModeError::StructNew { name: name.clone() });
    }

    fn struct_change(&mut self, name: &Identifier, old_struct: &Rc<Struct>, new_struct: &Rc<Struct>) {
        self.errors.push(UpgradeCompatibilityModeError::StructChange {
            name: name.clone(),
            old_struct: old_struct.clone(),
            new_struct: new_struct.clone(),
        });
    ...
    fn enum_new(&mut self, name: &Identifier, _new_enum: &Rc<Enum>) {
        self.errors.push(UpgradeCompatibilityModeError::EnumNew { name: name.clone() });
    }

    fn enum_change(&mut self, name: &Identifier, new_enum: &Rc<Enum>) {
        self.errors.push(UpgradeCompatibilityModeError::EnumChange { name: name.clone(), new_enum: new_enum.clone() });
    }
    ...
    fn function_new(&mut self, name: &Identifier, _new_func: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionNew { name: name.clone() });
    }

    fn function_change(&mut self, name: &Identifier, old_func: &Rc<Function>, new_func: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionChange {
            name: name.clone(),
    ...
    fn function_missing(&mut self, name: &Identifier, _old_func: &Rc<Function>) {
        self.errors.push(UpgradeCompatibilityModeError::FunctionMissing { name: name.clone() });
    }
    ...
    fn friend_mismatch(&mut self, old_count: usize, new_count: usize) {
        if old_count < new_count {
            ...
            self.errors.push(UpgradeCompatibilityModeError::FriendMissing);
        }
    }

    fn finish(self, inclusion: &InclusionCheck) -> Result<(), Self::Error> {
        let errors: Vec<UpgradeCompatibilityModeError> =
            self.errors.into_iter().filter(|e| breaks_inclusion_check(e, inclusion)).collect();
...
fn table_index(compiled_module: &CompiledModule) -> IdentifierTableLookup {
    ...
    let struct_identifier_to_index: BTreeMap<Identifier, TableIndex> = compiled_module
        ...
        .map(|(i, d)| {
            ...
            let s_id = compiled_module.identifier_at(compiled_module.datatype_handle_at(d.struct_handle).name);
    ...
    let enum_identifier_to_index: BTreeMap<Identifier, TableIndex> = compiled_module
        ...
        .enumerate()
        .map(|(i, d)| {
            let e_id = compiled_module.identifier_at(compiled_module.datatype_handle_at(d.enum_handle).name);
    ...
    let function_identifier_to_index: BTreeMap<Identifier, TableIndex> = compiled_module
        ...
        .enumerate()
        .map(|(i, d)| {
            let f_id = compiled_module.identifier_at(compiled_module.function_handle_at(d.function).name);
            (f_id.to_owned(), i as TableIndex)
        })
        .collect();

    IdentifierTableLookup { struct_identifier_to_index, enum_identifier_to_index, function_identifier_to_index }
}
...
pub(crate) async fn check_compatibility(
    ...
    let policy = UpgradePolicy::try_from(upgrade_policy).map_err(|_| anyhow!("Invalid upgrade policy"))?;

    compare_packages(
        *existing_package.to_move_package(u64::MAX /* safe as this pkg comes from the network */)?.original_package_id(),
        existing_modules,
        new_package,
        package_path,
        policy,
...
fn compare_packages(
    ...
    let mut new_modules_map: HashMap<Identifier, CompiledModule> =
        new_package.get_modules().map(|m| (m.self_id().name().to_owned(), m.clone())).collect();

    let lookup: HashMap<Identifier, IdentifierTableLookup> =
        new_modules_map.values().map(|m| (m.self_id().name().to_owned(), table_index(m))).collect();
    ...
    let move_toml_contents =
        Arc::from(fs::read_to_string(&move_toml_path).context("Unable to read Move.toml")?.to_string());
    ...
    for existing_module in existing_modules {
        ...
        match new_modules_map.get_mut(&name) {
            Some(new_module) => {
                ...
                let compiled_unit_with_source =
                    new_package.package.get_module_by_name_from_root(name.as_str()).context("Unable to get module")?;

                let module_diags =
                    modules_into_diags(&existing_module, new_module, compiled_unit_with_source, &lookup[&name], policy)?;

                diags.extend(module_diags);
            }
            None => {
                diags.extend(missing_module_diag(&name, &move_toml_hash, &move_toml_contents)?);
            }
        }
    }
    ...
    if diags.is_empty() {
        ...
        Err(anyhow!(
            ...
            String::from_utf8(report_diagnostics_to_buffer(&new_package.package.file_map, diags, use_colors()))
...
fn modules_into_diags(
    ...
    .into_iter()
    .map(|e| {
        compatibility_diag_from_error(&e, compiled_unit_with_source, matches!(policy, UpgradePolicy::Compatible), lookup)
    })
    .collect::<Result<Vec<_>, _>>()?;
...
fn compatibility_diag_from_error(
    ...
    match error {
        ...
        UpgradeCompatibilityModeError::StructAbilityMismatch { name, old_struct, new_struct } => {
            struct_ability_mismatch_diag(name, old_struct, new_struct, is_compatible, compiled_unit_with_source, lookup)
        }
        UpgradeCompatibilityModeError::StructFieldMismatch { name, old_struct, new_struct } => {
            struct_field_mismatch_diag(name, old_struct, new_struct, is_compatible, compiled_unit_with_source, lookup)
        }
        UpgradeCompatibilityModeError::EnumMissing { name, .. } => {
            missing_definition_diag("enum", name, is_compatible, compiled_unit_with_source)
        }

        UpgradeCompatibilityModeError::StructTypeParamMismatch { name, old_struct, new_struct } => {
        ...
        UpgradeCompatibilityModeError::EnumAbilityMismatch { name, old_enum, new_enum } => {
            enum_ability_mismatch_diag(name, old_enum, new_enum, is_compatible, compiled_unit_with_source, lookup)
        }

        UpgradeCompatibilityModeError::EnumNewVariant { name, old_enum, new_enum } => enum_new_variant_diag(
            name,
            old_enum,
            new_enum,
            // *tag,
        ...
        UpgradeCompatibilityModeError::EnumVariantMissing { name, tag, old_enum } => {
            enum_variant_missing_diag(name, old_enum, *tag, compiled_unit_with_source, lookup)
        }
        UpgradeCompatibilityModeError::EnumVariantMismatch { name, old_enum, new_enum, .. } => {
            enum_variant_mismatch_diag(name, old_enum, new_enum, is_compatible, compiled_unit_with_source, lookup)
        }
        UpgradeCompatibilityModeError::EnumTypeParamMismatch { name, old_enum, new_enum } => {
            enum_type_param_mismatch(name, old_enum, new_enum, is_compatible, compiled_unit_with_source, lookup)
        }
        UpgradeCompatibilityModeError::FunctionMissingPublic { name, .. } => {
            missing_definition_diag("public function", name, is_compatible, compiled_unit_with_source)
        }
        UpgradeCompatibilityModeError::FunctionMissingEntry { name, .. } => {
            missing_definition_diag("entry function", name, is_compatible, compiled_unit_with_source)
        }
        ...
        UpgradeCompatibilityModeError::FunctionSignatureMismatch { name, old_function, new_function } => {
        ...
        UpgradeCompatibilityModeError::FunctionEntryCompatibility { name, old_function, .. } => {
            function_entry_mismatch(name, old_function, compiled_unit_with_source, lookup)
        }

        UpgradeCompatibilityModeError::StructNew { name } => struct_new_diag(name, compiled_unit_with_source, lookup),
        UpgradeCompatibilityModeError::StructChange { name, old_struct, new_struct } => {
            struct_changed_diag(name, old_struct, new_struct, compiled_unit_with_source, lookup)
        }

        UpgradeCompatibilityModeError::EnumNew { name } => enum_new_diag(name, compiled_unit_with_source),
        ...
        UpgradeCompatibilityModeError::FunctionNew { name } => function_new_diag(name, compiled_unit_with_source),
        UpgradeCompatibilityModeError::FunctionChange { name, old_func, new_func } => {
        ...
        UpgradeCompatibilityModeError::FileFormatVersionDowngrade { old_version, new_version } => {
...
fn missing_definition_diag(
    ...
    let loc = compiled_unit_with_source.unit.source_map.definition_location;

    let (code, reason_notes) = if public_visibility_related_error {
        (Declarations::PublicMissing, vec![
            format!(
...
fn function_lost_public(
    ...
    let func_index = lookup.function_identifier_to_index.get(function_name).context("Unable to get function index")?;
    ...
    diags.add(Diagnostic::new(
        ...
        (def_loc, format!("Function '{function_name}' has lost its public visibility")),
        Vec::<(Loc, String)>::new(),
        vec![
...
fn function_signature_mismatch_diag(
    ...
    if old_function.parameters.len() != new_function.parameters.len() {
        ...
        for ((i, old_param), new_param) in old_function.parameters.iter().enumerate().zip(new_function.parameters.iter())
        {
            if old_param != new_param {
                let param_loc = func_sourcemap.parameters.get(i).context("Unable to get parameter location")?.1;

                let mut secondary = Vec::new();

                let old_param = format_param(old_param, func_sourcemap.type_parameters.clone(), &mut secondary)?;
                let new_param = format_param(new_param, func_sourcemap.type_parameters.clone(), &mut Vec::new())?;

                let label = format!("Unexpected parameter '{new_param}', expected '{old_param}'");

                diags.add(Diagnostic::new(Functions_::SignatureMismatch, (param_loc, label), secondary, vec![
                    reason.to_string(),
                    format!(
    ...
    if old_function.type_parameters.len() != new_function.type_parameters.len() {
        diags.add(Diagnostic::new(
            ...
            (
                ...
                format!(
                    ...
                    singular_or_plural(old_function.type_parameters.len(), "parameter", "parameters"),
        ...
        for ((i, old_type_param), new_type_param) in
            old_function.type_parameters.iter().enumerate().zip(new_function.type_parameters.iter())
        {
            if old_type_param != new_type_param {
                let type_param_loc =
                    func_sourcemap.type_parameters.get(i).context("Unable to get type parameter location")?.1;
                ...
                diags.add(Diagnostic::new(
                    ...
                    (
                        ...
                        format!(
                            ...
                            format_list(
                                new_type_param.into_iter().map(|t| format!("'{:?}'", t).to_lowercase()),
                                Some(("constraint", "constraints"))
                            ),
                            format_list(old_type_param.into_iter().map(|t| format!("'{:?}'", t).to_lowercase()), None),
                        ),
                    ),
                    Vec::<(Loc, String)>::new(),
                    vec![
    ...
    if old_function.return_.len() != new_function.return_.len() {
        ...
        for ((i, old_return), new_return) in old_function.return_.iter().enumerate().zip(new_function.return_.iter()) {
            let return_ = func_sourcemap.returns.get(i).context("Unable to get return location")?;

            if old_return != new_return {
                let mut secondary = Vec::new();
                let old_return = format_param(old_return, func_sourcemap.type_parameters.clone(), &mut secondary)?;
                let new_return = format_param(new_return, func_sourcemap.type_parameters.clone(), &mut Vec::new())?;
                ...
                diags.add(Diagnostic::new(Functions_::SignatureMismatch, (*return_, label), secondary, vec![
                    reason.to_string(),
                    format!(
                        "Restore the original function's return \
                            {} for function '{function_name}'.",
...
fn ability_mismatch_label(
    ...
    let missing_abilities_list: Vec<String> =
        missing_abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()).collect();
    let extra_abilities_list: Vec<String> =
        extra_abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()).collect();

    match (missing_abilities != AbilitySet::EMPTY, extra_abilities != AbilitySet::EMPTY) {
        (true, true) => format!(
...
fn struct_ability_mismatch_diag(
    ...
    let struct_index = lookup.struct_identifier_to_index.get(struct_name).context("Unable to get struct index")?;
    ...
    if old_struct.abilities != new_struct.abilities {
        let old_abilities: Vec<String> =
            old_struct.abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()).collect();

        diags.add(Diagnostic::new(
            Declarations::AbilityMismatch,
            (def_loc, ability_mismatch_label(old_struct.abilities, new_struct.abilities, "ability", "abilities")),
            Vec::<(Loc, String)>::new(),
            vec![
                ...
                format!(
                    ...
                    format_list(old_struct.abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()), None),
                ),
            ],
        ));
    }
...
fn field_mismatch_message(
    ...
    Ok(match (old_field.name != new_field.name, old_field.type_ != new_field.type_) {
        (true, true) => (Declarations::FieldMismatch, format!("Mismatched field {new_ff}, expected {old_ff}.")),
        (true, false) => (Declarations::FieldMismatch, format!("Mismatched field {new_ff:#}, expected {old_ff:#}.")),
        (false, true) => {
            (Declarations::TypeMismatch, format!("Mismatched field type {}, expected {}.", new_ff.type_, old_ff.type_))
        }
...
fn struct_field_mismatch_diag(
    ...
    let old_fields: Vec<Rc<Field>> = old_struct.fields.0.values().filter(|f| !is_dummy_field(f)).cloned().collect();

    let new_fields: Vec<Rc<Field>> = new_struct.fields.0.values().filter(|f| !is_dummy_field(f)).cloned().collect();
    ...
    if old_fields.len() != new_fields.len() {
        diags.add(Diagnostic::new(
            ...
            (def_loc, format!("Incorrect number of fields: expected {}, found {}", old_fields.len(), new_fields.len())),
            Vec::<(Loc, String)>::new(),
            vec![
                reason,
                format!(
    ...
    } else if !old_fields.iter().zip(&new_fields).all(|(old_field, new_field)| old_field.equivalent(new_field)) {
        for (i, (old_field, new_field)) in old_fields.iter().zip(&new_fields).enumerate() {
            if !old_field.equivalent(new_field) {
                let field_loc = struct_sourcemap.fields.get(i).context("Unable to get field location")?;

                let (code, label) =
                    field_mismatch_message(old_field, new_field, struct_sourcemap.type_parameters.clone())?;
...
fn struct_type_param_mismatch_diag(
    ...
    let struct_index = lookup.struct_identifier_to_index.get(name).context("Unable to get struct index")?;
...
fn enum_ability_mismatch_diag(
    ...
    let enum_index = lookup.enum_identifier_to_index.get(enum_name).context("Unable to get enum index")?;
    ...
    if old_enum.abilities != new_enum.abilities {
        let old_abilities: Vec<String> =
            old_enum.abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()).collect();
        ...
        let reason = if public_visibility_related_error {
            ...
            "Enums cannot be removed or changed during an 'additive' or 'dependency only' upgrade.".to_string()
        };

        diags.add(Diagnostic::new(
            Declarations::AbilityMismatch,
            (def_loc, ability_mismatch_label(old_enum.abilities, new_enum.abilities, "ability", "abilities")),
            Vec::<(Loc, String)>::new(),
            vec![
                reason,
                format!(
                    ...
                    for enum '{enum_name}'.",
                    singular_or_plural(old_abilities.len(), "ability", "abilities"),
                    format_list(old_enum.abilities.into_iter().map(|a| format!("'{:?}'", a).to_lowercase()), None),
                ),
            ],
        ));
    }
...
fn enum_variant_field_message(
    ...
    Ok(match (old_variant.name != new_variant.name, !old_variant.fields.equivalent(&new_variant.fields)) {
        (true, true) => vec![(
            Enums::VariantMismatch.into(),
            format!("Mismatched variant '{}', expected '{}'.", new_variant.name, old_variant.name),
        )],
        (true, false) => vec![(
            Enums::VariantMismatch.into(),
            format!("Mismatched variant '{}', expected '{}'.", new_variant.name, old_variant.name),
        ...
        (false, true) => {
            ...
            for (old_field, new_field) in old_variant.fields.0.values().zip(new_variant.fields.0.values()) {
                if !old_field.equivalent(new_field) {
                    let (code, label) = field_mismatch_message(old_field, new_field, Vec::new())?;
...
fn enum_variant_mismatch_diag(
    ...
    for (i, (old_variant, new_variant)) in old_enum.variants.values().zip(new_enum.variants.values()).enumerate() {
        if !old_variant.equivalent(new_variant) {
            let variant_loc = enum_sourcemap.variants.get(i).context("Unable to get variant location")?.0 .1;
...
fn enum_new_variant_diag(
    ...
    let old_enum_map = old_enum.variants.values().map(|v| v.name.as_ident_str()).collect::<HashSet<_>>();
    ...
    for (i, new_variant) in new_enum.variants.values().enumerate() {
        if !old_enum_map.contains(new_variant.name.as_ident_str()) {
            ...
            diags.add(Diagnostic::new(
                ...
                (variant_loc, format!("New unexpected variant '{}'.", new_variant.name)),
                vec![(def_loc, "Enum definition".to_string())],
                vec![
                    "Enums are part of a module's public interface \
                    and cannot be removed or changed during an upgrade."
...
fn enum_variant_missing_diag(
    ...
    let variant_name = &old_enum.variants.get_index(tag).context("Unable to get variant")?.1.name;

    diags.add(Diagnostic::new(
        Enums::VariantMismatch,
        (enum_sourcemap.definition_location, format!("Missing variant '{variant_name}'.")),
        Vec::<(Loc, String)>::new(),
        vec![
...
fn struct_new_diag(
    ...
    diags.add(Diagnostic::new(
        ...
        (struct_sourcemap.definition_location, format!("New unexpected struct '{struct_name}'.")),
        Vec::<(Loc, String)>::new(),
        vec![
...
fn enum_new_diag(
    ...
    let def_loc = compiled_unit_with_source.unit.source_map.definition_location;
...
fn function_new_diag(
    ...
    diags.add(Diagnostic::new(
        Declarations::NewDeclaration,
        (def_loc, format!("New unexpected function '{}'.", function_name)),
        Vec::<(Loc, String)>::new(),
        vec![
...
fn type_parameter_diag(
    ...
    if old_type_parameters.len() != new_type_parameters.len() {
        ...
        for (i, (old_type_param, new_type_param)) in
            old_type_parameters.iter().zip(new_type_parameters.iter()).enumerate()
        {
            let type_param_loc = type_parameter_locs.get(i).context("Unable to get type parameter location")?;
...
fn type_param_constraint_labels(old_constraints: AbilitySet, new_constraints: AbilitySet) -> Option<(String, String)> {
    if old_constraints == new_constraints {
        return None;
    }

    let old_abilities_list: Vec<String> = old_constraints.into_iter().map(|a| format!("'{a}'").to_lowercase()).collect();

    Some((
        ability_mismatch_label(old_constraints, new_constraints, "constraint", "constraints"),
        format!(
...
fn type_param_phantom_labels(old_phantom: bool, new_phantom: bool) -> Option<(String, String)> {
    ...
    Some(if old_phantom {
        ("Missing 'phantom' modifier".to_string(), "Restore the original 'phantom' modifier".to_string())
    } else {
        ("Unexpected 'phantom' modifier".to_string(), "Remove the 'phantom' modifier".to_string())
    })
}
...
fn file_format_version_downgrade_diag(
    ...
    diags.add(Diagnostic::new(
        ...
        (def_loc, format!("Downgrading from file format version {} to {} is not supported.", old_version, new_version)),
        Vec::<(Loc, String)>::new(),
        vec![
...
fn friend_link_diag(compiled_unit_with_source: &CompiledUnitWithSource) -> Result<Diagnostics, Error> {
    let mut diags = Diagnostics::new();
