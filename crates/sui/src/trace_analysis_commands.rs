...
impl AnalyzeTraceCommand {
    pub async fn execute(self, path: PathBuf, output_dir: Option<PathBuf>) -> Result<(), anyhow::Error> {
        let trace_file = std::fs::File::open(&path)
            .map_err(|e| anyhow!("Failed to open trace file at {}: {e}", path.to_string_lossy()))?;
        let trace_reader = MoveTraceReader::new(trace_file).map_err(|e| anyhow!("Failed to read trace file: {e}"))?;

        match self {
            AnalyzeTraceCommand::GasProfile { use_long_function_name } => {
                let mut profiler = GasProfiler::init(
                    ProfilerConfig { output_dir, use_long_function_name },
