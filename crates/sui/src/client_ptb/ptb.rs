...
use crate::{
    client_commands::{dry_run_or_execute_or_serialize, GasDataArgs, SuiClientCommandResult, TxProcessingArgs},
    client_ptb::{
...
impl AddressData {
    ...
    pub fn address(&self) -> Option<AccountAddress> {
        match self {
            ...
            AddressData::MovePackage((pkg, _)) => AccountAddress::from_bytes(pkg.id().into_bytes()).ok(),
        }
    }
}
...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        let mut starting_addresses: BTreeMap<String, AddressData> = context
            ...
            .into_iter()
            .map(|(sa, alias)| (alias.alias.clone(), AddressData::AccountAddress(AccountAddress::from(*sa))))
            .collect();

        let mvr_names = program_metadata.mvr_names.clone();
        let mvr_resolver = MvrResolver { names: program_metadata.mvr_names.into_keys().collect() };
        ...
        let gas: Vec<_> = program_metadata.gas_object_ids.into_iter().flatten().map(|x| x.value).collect();
        ...
        let gas_data = GasDataArgs {
            ...
            gas_sponsor: program_metadata.gas_sponsor.map(|x| x.value.into_inner().into()),
        };
