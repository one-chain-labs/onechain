...
impl<'a> PTBBuilder<'a> {
    ...
    fn declare_possible_address_binding(&mut self, ident: String, possible_addr: &Spanned<PTBArg>) {
        match possible_addr.value {
            PTBArg::Address(addr) => {
                self.addresses.insert(ident, AddressData::AccountAddress(addr.into_inner()));
            }
    ...
    async fn resolve(
        ...
        match arg {
            ...
            PTBArg::Identifier(i) if self.addresses.contains_key(&i) => {
                ...
                let addr = self.addresses[&i].address().ok_or_else(|| err!(arg_loc, "Expected an address"))?;
    ...
    async fn handle_command_(&mut self, cmd_span: Span, command: ParsedPTBCommand) -> PTBResult<()> {
        ...
        match command {
            ...
            ParsedPTBCommand::MakeMoveVec(sp!(ty_loc, ty_arg), sp!(_, args)) => {
                let ty_arg =
                    into_type_tag(&self.addresses, ty_arg, &resolve_address).map_err(|e| err!(ty_loc, "{e}"))?;
            ...
            ParsedPTBCommand::MoveCall(
                ...
                if let Some(sp!(ty_loc, in_ty_args)) = in_ty_args {
                    for t in in_ty_args.into_iter() {
                        ty_args
                            .push(into_type_tag(&self.addresses, t, &resolve_address).map_err(|e| err!(ty_loc, "{e}"))?)
                    }
                }
            ...
            ParsedPTBCommand::Publish(sp!(pkg_loc, package_path)) => {
                ...
                if !package_path.exists() {
                    error!(pkg_loc, "Package path '{}' does not exist", package_path.display());
                }
            ...
            ParsedPTBCommand::Upgrade(sp!(path_loc, package_path), mut arg) => {
                let package_path = Path::new(&package_path);
                if !package_path.exists() {
                    error!(path_loc, "Package path '{}' does not exist", package_path.display());
                }
    ...
    let address = match fq_name.module.address {
        ParsedAddress::Named(name) if is_mvr_name(&name) => {
            ...
            let Some(AddressData::MovePackage((pkg, type_origin_map))) = addr else {
                return Err(anyhow::anyhow!("Expected that {name} is an actual Move package"));
            };
            let Some(address) = type_origin_map.get(&(fq_name.module.name.clone(), fq_name.name.clone())) else {
            ...
            AccountAddress::from_bytes(address.into_bytes())
                .map_err(|_| anyhow::anyhow!("Cannot convert ObjectID {address} into AccountAddress"))
        }
...
pub fn into_type_tag(
    ...
    Ok(match parsed_type {
        ...
        ParsedType::Vector(inner) => TypeTag::Vector(Box::new(into_type_tag(addresses, *inner, mapping)?)),
...
fn try_resolve_parsed_address(
    ...
    match &address.value {
        ParsedAddress::Named(name) => {
            let address = if let Some(addr) = addresses.get(name) { addr.address() } else { resolve_address(name) };
...
pub async fn resolve_package(reader: &ReadApi, package_id: ObjectID, loc: Span) -> Result<MovePackage, PTBError> {
    let object = reader
