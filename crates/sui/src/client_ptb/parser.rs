            ...
            match lexeme {
                ...
                L(T::Command, A::GAS_PRICE) => {
                    ...
                    if let Some(other) = self.state.gas_price.replace(price) {
                        self.state.errors.extend([
                            err!(other.span, "Multiple gas prices found. Gas price first set here.",),
                            err!(
...
impl<'a, I: Iterator<Item = &'a str>> ProgramParser<'a, I> {
    ...
    fn parse_mvr_address(&mut self, prefix: Option<Spanned<&str>>) -> PTBResult<Spanned<ParsedAddress>> {
        use Lexeme as L;
        use Token as T;
        ...
        match prefix {
            ...
            Some(sp!(_, prefix)) => {
                ...
                match self.peek() {
                    ...
                    sp!(sp, l) => {
                        error!(sp, "Expected a valid MVR domain extension ('.sui') but got {}", l)
                    }
                }
            }
            // @ prefixed MVR name
        ...
        self.state.mvr_names_with_span.entry(address.clone()).or_insert_with(|| start_sp.widen(end_sp));
        ...
        let addr = match lexeme {
            ...
            L(t @ (mvr_ident!()), contents) => {
                ...
                if let sp!(s, L(T::Dot, _)) = self.peek() {
                    self.parse_mvr_address(Some(Spanned { span: s, value: contents }))?.value
                } else {
                    match t {
