...
impl MvrResolver {
    ...
    pub async fn resolve_names(&self, read_api: &ReadApi) -> Result<ResolvedNames, Error> {
        if self.names.is_empty() {
            return Ok(ResolvedNames { resolution: BTreeMap::new() });
        }

        let request = reqwest::Client::new();
        let (url, chain) = mvr_req_url(read_api).await?;
        let json_body = json!(NamesRequest { names: self.names.clone() });
        ...
        anyhow::ensure!(
            ...
            self.names
                .difference(&resolved_addresses.resolution.keys().cloned().collect::<BTreeSet<_>>())
                .cloned()
                .collect::<Vec<_>>()
                .join(", ")
        );
