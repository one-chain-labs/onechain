...
async fn test_shell_snapshot(path: &Path) -> datatest_stable::Result<()> {
    // set up test cluster
    let cluster = if path.starts_with(TEST_NET_DIR) { Some(TestClusterBuilder::new().build().await) } else { None };
    ...
    fs_extra::dir::copy(sui_package_dir_src, sandbox, &CopyOptions::new().content_only(true))?;

    // set up command
    let mut shell = Command::new("bash");
    shell
        .env("PATH", format!("{}:{}", get_sui_bin_path(), std::env::var("PATH")?))
    ...
    let snapshot_name: String = path.strip_prefix("tests/shell_tests")?.to_string_lossy().to_string();
...
fn get_sui_bin_path() -> String {
    get_cargo_bin("sui").parent().unwrap().to_str().expect("directory name is valid UTF-8").to_owned()
}
