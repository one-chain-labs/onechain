...
async fn main() -> anyhow::Result<()> {
    ...
    let object_store: Arc<dyn object_store::ObjectStore> = if let Some(bucket) = args.s3 {
        ...
        HttpBuilder::new().with_url(endpoint.to_string()).with_client_options(client_options).build().map(Arc::new)?
    ...
    let metrics_service =
        sui_indexer_alt_metrics::MetricsService::new(args.metrics_args, registry.clone(), cancel.clone());
    ...
    indexer
        .concurrent_pipeline(CheckpointBlobPipeline { compression_level: args.compression_level }, config.clone())
        .await?;

    indexer.concurrent_pipeline(EpochsPipeline, config.clone()).await?;
