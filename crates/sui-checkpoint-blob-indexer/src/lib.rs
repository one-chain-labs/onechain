...
mod tests {
    ...
    async fn test_checkpoint_blob_handler_uncompressed() {
        ...
        let blob = CheckpointBlob { sequence_number: 100, proto_bytes: Bytes::from(vec![1, 2, 3, 4, 5]) };
    ...
    async fn test_checkpoint_blob_handler_compressed() {
        ...
        let blob = CheckpointBlob { sequence_number: 200, proto_bytes: Bytes::from(test_data.clone()) };
    ...
    async fn test_epochs_handler() {
        ...
        {
            let mut conn = store.connect().await.unwrap();
            let epoch = EpochCheckpoint { checkpoint_number: 100 };
        ...
        {
            let mut conn = store.connect().await.unwrap();
            let epoch = EpochCheckpoint { checkpoint_number: 200 };
            let pipeline = EpochsPipeline;
        ...
        {
            let mut conn = store.connect().await.unwrap();
            let epoch = EpochCheckpoint { checkpoint_number: 150 };
            let pipeline = EpochsPipeline;
            pipeline.commit(&Some(epoch), &mut conn).await.unwrap();
