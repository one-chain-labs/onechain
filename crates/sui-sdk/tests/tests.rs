...
async fn mnemonic_test() {
    ...
    let (address, _key_pair, scheme, phrase) = generate_new_key(SignatureScheme::ED25519, None, None).unwrap();

    let keystore_path_2 = temp_dir.path().join("sui2.keystore");
    let mut keystore2 = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path_2).unwrap());
    let imported_address = keystore2.import_from_mnemonic(&phrase, SignatureScheme::ED25519, None, None).await.unwrap();
...
async fn test_verify_personal_message_signature() {
    ...
    let message = b"hello";
    let intent_message = IntentMessage::new(Intent::personal_message(), PersonalMessage { message: message.to_vec() });
    ...
    let res = verify_personal_message_signature(signature, "wrong msg".as_bytes(), address, None).await;
    assert!(res.is_err());
}
...
async fn test_verify_signature_zklogin() {
    ...
    let res = verify_personal_message_signature(signature.clone(), message, user_address, Some(client.clone())).await;
    assert!(res.is_ok());

    let res =
        verify_personal_message_signature(signature, "wrong msg".as_bytes(), user_address, Some(client.clone())).await;
    assert!(res.is_err());
}
...
async fn test_verify_signature_multisig() {
    ...
    let multisig_pk = MultiSigPublicKey::new(vec![kp1.public(), kp2.public()], vec![1, 1], 2).unwrap();
    ...
    let res = verify_personal_message_signature(generic_sig, "wrong msg".as_bytes(), address, None).await;
    assert!(res.is_err());
}
...
async fn test_get_wallet_key() {
    ...
    wallet_context.get_identity_address(Some(alias_identity.clone())).unwrap();

    // try to get a non-existing alias
    wallet_context.get_identity_address(Some(KeyIdentity::Alias("".to_string()))).unwrap_err();

    // get keystore by alias
    wallet_context.get_keystore_by_identity(&alias_identity).unwrap();
    // get mutable keystore by alias
    wallet_context.get_keystore_by_identity_mut(&alias_identity).unwrap();

    let transaction_kind =
        TransactionKind::ProgrammableTransaction(ProgrammableTransaction { inputs: vec![], commands: vec![] });

    let transaction = TransactionData::new(transaction_kind, address, random_object_ref(), 1000, 1000);

    let signature = wallet_context.sign_secure(&alias_identity, &transaction, Intent::sui_transaction()).await.unwrap();

    let intent_message = IntentMessage::new(Intent::sui_transaction(), transaction.clone());

    signature.verify_secure(&intent_message, address, SignatureScheme::ED25519).unwrap();
...
async fn test_update_env_chain_id_new_chain_id() -> Result<(), anyhow::Error> {
    ...
    assert!(active_env.chain_id.is_none(), "Chain ID should not be cached initially");
    ...
    assert_eq!(reloaded_env.chain_id, Some(chain_id), "Chain ID should be persisted to config file");
...
async fn test_update_env_chain_id_overwrite_existing() -> Result<(), anyhow::Error> {
    ...
    assert_ne!(real_chain_id, "fake-chain-id", "Real chain ID should be different from fake one");

    let final_config = SuiClientConfig::load(&config_path)?;
    assert_eq!(final_config.get_active_env()?.chain_id, Some(real_chain_id), "Real chain ID should replace fake one");
...
async fn test_chain_id_persistence() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(env.chain_id, Some(chain_id), "Chain ID should persist across wallet context instances");
