...
use sui_sdk::{
    ...
    types::{programmable_transaction_builder::ProgrammableTransactionBuilder, transaction::TransactionData},
    SuiClientBuilder,
};
use sui_types::{
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let skp_determ_0 = SuiKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let _skp_determ_1 = SuiKeyPair::Secp256k1(Secp256k1KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let _skp_determ_2 = SuiKeyPair::Secp256r1(Secp256r1KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    ...
    let _skp_import_no_flag_0 = SuiKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
        &Base64::decode("1GPhHHkVlF6GrCty2IuBkM+tj/e0jn64ksJ1pc8KPoI=").map_err(|_| anyhow!("Invalid base64"))?,
    )?);
    ...
    let _skp_import_with_flag_0 = SuiKeyPair::decode_base64("ANRj4Rx5FZRehqwrctiLgZDPrY/3tI5+uJLCdaXPCj6C")
        .map_err(|_| anyhow!("Invalid base64"))?;
    let _skp_import_with_flag_1 = SuiKeyPair::decode_base64("AdRj4Rx5FZRehqwrctiLgZDPrY/3tI5+uJLCdaXPCj6C")
        .map_err(|_| anyhow!("Invalid base64"))?;
    let _skp_import_with_flag_2 = SuiKeyPair::decode_base64("AtRj4Rx5FZRehqwrctiLgZDPrY/3tI5+uJLCdaXPCj6C")
        .map_err(|_| anyhow!("Invalid base64"))?;
    ...
    let _skp_import_with_flag_0 =
        SuiKeyPair::decode("suiprivkey1qzdlfxn2qa2lj5uprl8pyhexs02sg2wrhdy7qaq50cqgnffw4c2477kg9h3")
            .map_err(|_| anyhow!("Invalid Bech32"))?;
    let _skp_import_with_flag_1 =
        SuiKeyPair::decode("suiprivkey1qqesr6xhua2dkt840v9yefely578q5ad90znnpmhhgpekfvwtxke6ef2xyg")
            .map_err(|_| anyhow!("Invalid Bech32"))?;
    let _skp_import_with_flag_2 =
        SuiKeyPair::decode("suiprivkey1qprzkcs823gcrk7n4hy8pzhntdxakpqk32qwjg9f2wyc3myj78egvtw3ecr")
            .map_err(|_| anyhow!("Invalid Bech32"))?;
    ...
    let transaction_response = sui_client
        ...
        .execute_transaction_block(
            sui_types::transaction::Transaction::from_generic_sig_data(intent_msg.value, vec![
                GenericSignature::Signature(sui_sig),
            ]),
    ...
    println!("Transaction executed. Transaction digest: {}", transaction_response.digest.base58_encode());
