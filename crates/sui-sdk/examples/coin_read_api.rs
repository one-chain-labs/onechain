...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<OCT>` which is represented as
    // "0x2::one::OCT"
    let coin_type = Some("0x2::one::OCT".to_string());
    let coins = sui
        .coin_read_api()
    ...
    let coin_metadata = sui.coin_read_api().get_coin_metadata("0x2::one::OCT".to_string()).await?;
    ...
    // Total Supply
    let total_supply = sui.coin_read_api().get_total_supply("0x2::one::OCT".to_string()).await?;
