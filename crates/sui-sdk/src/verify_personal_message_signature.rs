...
pub async fn verify_personal_message_signature(
    ...
    let intent_msg = IntentMessage::new(Intent::personal_message(), PersonalMessage { message: message.to_vec() });
    match signature {
        GenericSignature::ZkLoginAuthenticator(ref _sig) => {
            if let Some(client) = client {
                ...
                let res = client
                    .read_api()
                    .verify_zklogin_signature(bytes, sig_string, ZkLoginIntentScope::PersonalMessage, address)
                    .await?;
                if res.success {
                    Ok(())
                } else {
