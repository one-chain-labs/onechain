...
impl WalletContext {
    ...
    pub fn new_for_tests(keystore: Keystore, external: Option<Keystore>, path: Option<PathBuf>) -> Self {
        let mut config = SuiClientConfig::new(keystore).persisted(&path.unwrap_or(PathBuf::from("test_config.yaml")));
    ...
    pub fn get_identity_address(&mut self, input: Option<KeyIdentity>) -> Result<SuiAddress, anyhow::Error> {
    ...
    pub async fn get_client(&self) -> Result<SuiClient, anyhow::Error> {
        ...
        Ok(if let Some(client) = read.as_ref() {
            ...
            drop(read);
            let client =
                self.get_active_env()?.create_rpc_client(self.request_timeout, self.max_concurrent_requests).await?;
    ...
    pub async fn load_or_cache_chain_id(&self, client: &SuiClient) -> Result<String, anyhow::Error> {
    ...
    pub async fn get_full_object_ref(&self, object_id: ObjectID) -> Result<FullObjectRef, anyhow::Error> {
        let client = self.get_client().await?;
        let object = client
            .read_api()
        ...
        let owner = object.owner.expect("Owner should be present if `with_owner` is set");
    ...
    pub async fn infer_sender(&mut self, gas: &[ObjectID]) -> Result<SuiAddress, anyhow::Error> {
        ...
        ensure!(owners.iter().all(|o| o == &owner), "Cannot infer sender, not all gas objects have the same owner.");
    ...
    pub fn get_keystore_by_identity(&self, key_identity: &KeyIdentity) -> Result<&Keystore, anyhow::Error> {
    ...
    pub fn get_keystore_by_identity_mut(&mut self, key_identity: &KeyIdentity) -> Result<&mut Keystore, anyhow::Error> {
        if self.config.keystore.get_by_identity(key_identity).is_ok() {
    ...
    pub async fn sign_transaction(&self, data: &TransactionData) -> Transaction {
        let sig = self.config.keystore.sign_secure(&data.sender(), data, Intent::sui_transaction()).await.unwrap();
