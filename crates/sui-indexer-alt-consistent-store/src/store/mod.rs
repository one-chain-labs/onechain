...
use crate::db::{config::DbConfig, Db, Watermark};
...
impl<S: Schema> Store<S> {
    ...
    pub(crate) fn open(path: impl AsRef<Path>, config: DbConfig, snapshots: u64) -> anyhow::Result<Self> {
        let db_options: rocksdb::Options = config.into();
        let db = Arc::new(
            Db::open(path, db_options.clone(), snapshots as usize, S::cfs(&db_options))
                .context("Failed to open database")?,
        );

        let schema = S::open(&db).context("Failed to open schema")?;

        Ok(Self(Arc::new(Inner { db, schema, queue: OnceLock::new() })))
    }
...
impl<S: Send + Sync> store::Connection for Connection<'_, S> {
    async fn committer_watermark(&mut self, pipeline: &'static str) -> anyhow::Result<Option<CommitterWatermark>> {
...
mod tests {
    ...
    async fn test_open() {
        ...
        let _store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), 4).unwrap();
    }

    #[tokio::test]
    async fn test_no_queue() {
        let d = tempfile::tempdir().unwrap();
        let store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), 4).unwrap();
    ...
    async fn test_single_pipeline_existing() {
        ...
        let store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), snapshots).unwrap();
