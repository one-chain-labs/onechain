...
use crate::{
    db::{config::DbConfig, Db, Watermark},
    metrics::ColumnFamilyStatsCollector,
};
...
impl<S: Schema> Store<S> {
    ...
    pub(crate) fn open(
        ...
        let db =
            Arc::new(Db::open(path, db_options.clone(), snapshots as usize, cfs).context("Failed to open database")?);

        let schema = S::open(&db).context("Failed to open schema")?;

        if let Some(registry) = registry {
            registry
                .register(Box::new(ColumnFamilyStatsCollector::new(Some("rocksdb"), db.clone(), cf_names)))
                .context("Failed to register rocksdb column family stats collector")?;
        }

        Ok(Self(Arc::new(Inner { db, queue: OnceLock::new(), schema })))
    }
    ...
    pub(crate) fn sync(&self, s: Synchronizer) -> anyhow::Result<JoinHandle<()>> {
        ...
        self.0.queue.set(queue).map_err(|_| anyhow!("Store already has synchronizer"))?;
        Ok(handle)
    }
}
...
impl<S: Send + Sync + 'static> store::Store for Store<S> {
    ...
    async fn connect(&self) -> anyhow::Result<Connection<'_, S>> {
        Ok(Connection { store: self, batch: rocksdb::WriteBatch::default(), watermark: None })
    }
}
...
impl<S: Send + Sync> store::Connection for Connection<'_, S> {
    async fn committer_watermark(&mut self, pipeline_task: &str) -> anyhow::Result<Option<CommitterWatermark>> {
        Ok(self.store.0.db.commit_watermark(pipeline_task)?.map(Into::into))
    }
    ...
    async fn reader_watermark(&mut self, _pipeline: &'static str) -> anyhow::Result<Option<store::ReaderWatermark>> {
        Ok(None)
    }
    ...
    async fn set_reader_watermark(&mut self, _pipeline: &'static str, _reader_lo: u64) -> anyhow::Result<bool> {
        bail!("Pruning not supported by this store");
    }

    async fn set_pruner_watermark(&mut self, _pipeline: &'static str, _pruner_hi: u64) -> anyhow::Result<bool> {
        bail!("Pruning not supported by this store");
    }
}
...
mod tests {
    ...
    impl Schema for TestSchema {
        ...
        fn open(db: &Arc<Db>) -> anyhow::Result<Self> {
            Ok(Self { a: DbMap::new(db.clone(), "a"), b: DbMap::new(db.clone(), "b") })
        }
    }
    ...
    async fn write<M>(store: &Store<TestSchema>, pipeline: &'static str, cp: u64, mutator: M) -> anyhow::Result<()>
    where
        ...
        store
            .transaction(move |c| {
                async move {
                    ...
                    c.set_committer_watermark(pipeline, CommitterWatermark::new_for_testing(cp)).await?;
                    Ok(())
                }
                .scope_boxed()
            })
    ...
    async fn test_open() {
        ...
        let _store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), 4, None).unwrap();
    }

    #[tokio::test]
    async fn test_no_queue() {
        let d = tempfile::tempdir().unwrap();
        let store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), 4, None).unwrap();
    ...
    async fn test_single_pipeline() {
        ...
        let mut sync = Synchronizer::new(store.db().clone(), stride, buffer_size, first_checkpoint, cancel.clone());
        ...
        wait_until(|| async { has_range(&store, None, Some(0)) }).await.unwrap();
    ...
    async fn test_multiple_pipelines() {
        ...
        // been written to.
        wait_until(|| async { has_range(&store, None, None) }).await.unwrap_err();
    ...
    async fn test_single_pipeline_existing() {
...

        {
            ...
            let db = Arc::new(
                Db::open(d.path().join("db"), db_options.clone(), snapshots as usize, TestSchema::cfs(&db_options))
                    .unwrap(),
            );
            ...
            db.write("b", CommitterWatermark::new_for_testing(0).into(), batch).unwrap();
        }

        let store: Store<TestSchema> = Store::open(d.path().join("db"), DbConfig::default(), snapshots, None).unwrap();
    ...
    async fn test_multiple_pipeline_existing() {
...

        {
            ...
            db.write("a", CommitterWatermark::new_for_testing(0).into(), batch).unwrap();
    ...
    async fn test_multiple_pipeline_catchup() {
...

        {
            ...
            let db_options: rocksdb::Options = DbConfig::default().into();
            let db = Arc::new(
                Db::open(d.path().join("db"), db_options.clone(), snapshots, TestSchema::cfs(&db_options)).unwrap(),
            );
        ...
        wait_until(|| async { has_range(&store, None, Some(1)) }).await.unwrap();
    ...
    async fn test_missing_pipeline() {
        ...
        let err = write(&store, "test", 0, |_, _| Ok(())).await.unwrap_err().to_string();
    ...
    async fn test_no_pipelines() {
        ...
        let first_checkpoint = None;
        let cancel = CancellationToken::new();
        let sync = Synchronizer::new(store.db().clone(), stride, buffer_size, first_checkpoint, cancel.clone());

        // Don't register any pipelines
        let err = store.sync(sync).unwrap_err().to_string();
        assert!(err.contains("No pipelines registered with the synchronizer"), "{err}");
    }
    ...
    async fn test_first_checkpoint() {
        ...
        wait_until(|| async { has_range(&store, Some(100), Some(100)) }).await.unwrap();
    ...
    async fn test_stride() {
        ...
        wait_until(|| async { has_range(&store, Some(2), Some(8)) }).await.unwrap();
        ...
        // range request.
        assert_eq!(Some(8), d.snapshot_range(8).map(|r| r.end().checkpoint_hi_inclusive));

        // Going one checkpoint back causes the range to drop back by the stride.
        assert_eq!(Some(5), d.snapshot_range(7).map(|r| r.end().checkpoint_hi_inclusive));
        ...
        let err = store
            .transaction(|c| {
                async move {
                    ...
                    c.store.schema().a.insert("x".to_owned(), 42, &mut c.batch)?;
                    Ok(())
                }
                .scope_boxed()
            })
    ...
    async fn test_out_of_order_batch() {
        ...
        time::timeout(Duration::from_millis(500), h_sync).await.unwrap().unwrap();

        // The first write made it through, but the second one did not.
        let s = store.schema();
        let db = store.db();
        assert_eq!(db.snapshots(), 1);
        assert_eq!(db.snapshot_range(u64::MAX).map(|s| s.end().checkpoint_hi_inclusive), Some(0));
        assert_eq!(s.a.get(0, "x".to_owned()).unwrap(), Some(42));
