...
impl Synchronizer {
    ...
    pub(crate) fn register_pipeline(&mut self, pipeline: &'static str) -> anyhow::Result<()> {
        let watermark =
            self.db.commit_watermark(pipeline).with_context(|| format!("Failed to get {pipeline} initial watermark"))?;
    ...
    pub(super) fn run(self) -> anyhow::Result<(JoinHandle<()>, Queue)> {
        ...
        let Some(init_checkpoint) =
            self.last_watermarks.values().map(|w| w.map_or(self.first_checkpoint, |w| w.checkpoint_hi_inclusive)).max()
        else {
...
fn synchronizer(
    ...
    tokio::spawn(async move {
        loop {
            let next_checkpoint =
                current_watermark.as_ref().map(|w| w.checkpoint_hi_inclusive + 1).unwrap_or(first_checkpoint);
            ...
            match next_snapshot_checkpoint.cmp(&next_checkpoint) {
                ...
                Ordering::Less => {
                    error!(pipeline, next_snapshot_checkpoint, next_checkpoint, "Missed snapshot");
                    break;
                }
