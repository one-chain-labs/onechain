...
use sui_indexer_alt_consistent_api::proto::{
    self,
    rpc::consistent::v1alpha::consistent_service_server::ConsistentServiceServer,
};
...
pub async fn start_service(
    ...
    let ServiceConfig {
        ...
        rocksdb,
        committer,
        pipeline: PipelineLayer { balances, object_by_owner, object_by_type },
        rpc,
    } = config;
    ...
    let state =
        State { store: indexer.store().clone(), rpc_config: Arc::new(rpc), consistency_config: Arc::new(consistency) };
    ...
    macro_rules! add_sequential {
        ($handler:expr, $config:expr) => {
            if let Some(layer) = $config {
                indexer
                    .sequential_pipeline($handler, SequentialConfig {
