        ...
        match ReceiverStream::new(rx)
            .try_for_each_spawned(<R as Restore<S>>::FANOUT, |objects| {
                ...
                async move {
                    ...
                    metrics.total_partitions_received.with_label_values(&[R::NAME]).inc();

                    metrics.total_objects_received.with_label_values(&[R::NAME]).inc_by(objects.objects.len() as u64);

                    let _guard = metrics.worker_partition_latency.with_label_values(&[R::NAME]).start_timer();
                    ...
                    db.restore(objects.bucket, objects.partition, R::NAME, batch).with_context(|| {
                        format!("Failed to write batch for bucket {}, partition {}", objects.bucket, objects.partition)
                    })?;

                    info!(pipeline = R::NAME, bucket = objects.bucket, partition = objects.partition, "DONE",);

                    metrics.total_partitions_restored.with_label_values(&[R::NAME]).inc();

                    metrics.total_objects_restored.with_label_values(&[R::NAME]).inc_by(objects.objects.len() as u64);
