...
impl EpochManifest {
    pub(super) fn read(data: &[u8]) -> anyhow::Result<Self> {
        ...
        ensure!(data.len() >= MAGIC_LEN + DIGEST_LEN, "Epoch manifest too short",);
        ...
        ensure!(u32::from_be_bytes(magic) == EPOCH_MANIFEST_MAGIC, "Not an epoch manifest");
        ...
        ensure!(hasher.finalize().digest == digest, "Epoch manifest digest mismatch",);
...
impl LiveObjects {
    pub(super) fn read(bytes: &[u8], metadata: &FileMetadata) -> anyhow::Result<Self> {
        ...
        ensure!(u32::from_be_bytes(magic) == OBJECT_FILE_MAGIC, "Not an object file");
        ...
        while let Ok(len) = read.read_varint::<u64>() {
            ...
            let encoding =
                BlobEncoding::try_from(e[0]).with_context(|| format!("Invalid encoding in object file: {}", e[0]))?;

            let mut data = vec![0u8; len as usize];
            read.read_exact(&mut data)?;

            let object = Blob { data, encoding }.decode().context("Failed to decode object from blob")?;
        ...
        Ok(Self { bucket: metadata.bucket, partition: metadata.partition, objects })
    }
}
