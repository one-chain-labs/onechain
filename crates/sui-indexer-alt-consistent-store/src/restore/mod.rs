...
use crate::{
    ...
    handlers::{balances::Balances, object_by_owner::ObjectByOwner, object_by_type::ObjectByType},
    store::Schema,
};
...
impl<S: Schema + Send + Sync + 'static> Restorer<S> {
    ...
    async fn new(
        ...
        let RestoreArgs { object_file_concurrency, object_file_buffer_size } = restore_args;
        ...
        let schema = S::open(&db).map(Arc::new).context("Failed to open schema")?;
    ...
    fn run(mut self) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            if future::join_all(self.handles).await.into_iter().any(|r| matches!(r, Err(_) | Ok(Err(_)))) {
...
impl Default for RestoreArgs {
    fn default() -> Self {
        Self { object_file_concurrency: 25, object_file_buffer_size: 25 }
    }
}
...
pub async fn start_restorer(
    ...
    ensure!(pipelines.is_empty(), "Unknown pipelines to restore: {pipelines:?}");
