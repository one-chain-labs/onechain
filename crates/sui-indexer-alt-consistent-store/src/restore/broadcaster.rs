...
pub(super) fn broadcaster(
    ...
    tokio::spawn(async move {
        ...
        let manifest = match snapshot.manifest().await.and_then(|bs| EpochManifest::read(&bs)) {
        ...
        let metadata: Vec<_> =
            manifest.metadata().iter().filter(|m| matches!(m.file_type, FileType::Object)).cloned().collect();
        ...
        match stream::iter(metadata)
            .try_for_each_spawned(object_file_concurrency, |metadata| {
                ...
                async move {
                    let restored = db
                        .is_restored(metadata.bucket, metadata.partition, subscribers.keys().map(|p| p.as_str()))
                    ...
                    if restored.iter().all(|r| *r) {
                        info!(bucket = metadata.bucket, partition = metadata.partition, "Skipping",);
                        metrics.total_partitions_skipped.inc();
                        return Ok(());
                    } else {
                        info!(bucket = metadata.bucket, partition = metadata.partition, "Fetching",);
                    }
...
async fn fetch_objects(
    ...
    let request = || async {
        ...
        let future =
            async { snapshot.file(&metadata).await.and_then(|bs| Ok((bs.len(), LiveObjects::read(&bs, &metadata)?))) };

        match with_slow_future_monitor(future, SLOW_FETCH_THRESHOLD, || {
            warn!(attempt, bucket = metadata.bucket, partition = metadata.partition, "Fetch slow");
        })
        .await
        {
            Ok((bytes, objects)) => {
            ...
            Err(e) => {
                warn!(attempt, bucket = metadata.bucket, partition = metadata.partition, "Fetch error: {e:#}");
    ...
    let backoff = ExponentialBackoff { max_interval: MAX_RETRY_INTERVAL, max_elapsed_time: None, ..Default::default() };
