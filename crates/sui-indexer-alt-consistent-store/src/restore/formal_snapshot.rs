...
use sui_indexer_alt_framework::{
    ingestion::remote_client::RemoteIngestionClient,
...
impl FormalSnapshot {
    ...
    pub(super) async fn new(
        ...
        let root_manifest =
            RootManifest::read(store.get("MANIFEST".into()).await.context("Failed to fetch root manifest")?.as_ref())?;
        ...
        ensure!(root_manifest.contains(epoch), "Requested epoch {epoch} is not available in the snapshot store",);

        let is_complete = store.get(format!("epoch_{epoch}/_SUCCESS").into()).await.is_ok();
        ...
        let CheckpointData { checkpoint_summary, .. } = Blob::from_bytes(
            &client
        ...
        Ok(Self { source: store, watermark })
    }
    ...
    pub(super) async fn manifest(&self) -> anyhow::Result<Bytes> {
        ...
        self.source.get(path.into()).await.context("Failed to fetch epoch manifest")
    }
    ...
    pub(super) async fn file(&self, metadata: &FileMetadata) -> anyhow::Result<Bytes> {
        ...
        let path =
            format!("epoch_{}/{}_{}.{ext}", self.watermark.epoch_hi_inclusive, metadata.bucket, metadata.partition);

        self.source.get(path.into()).await.context("Failed to fetch object file")
    }
}
