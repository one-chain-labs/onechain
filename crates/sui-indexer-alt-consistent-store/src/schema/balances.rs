...
fn merge_balances(_key: &[u8], existing_val: Option<&[u8]>, operands: &rocksdb::MergeOperands) -> Option<Vec<u8>> {
    // Panic if the merge fails, to prevent us from writing out an invalid state.
    let mut sum: i128 = existing_val.map_or(0, |v| bcs::from_bytes(v).expect("failed to deserialize balance"));
