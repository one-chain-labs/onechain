...
impl Collector for ColumnFamilyStatsCollector {
    ...
    fn collect(&self) -> Vec<MetricFamily> {
        for cf_name in &self.cf_names {
            ...
            self.metrics.size_all_mem_tables.with_label_values(&[cf_name]).set(cf_metrics.size_all_mem_tables);

            self.metrics.block_cache_usage.with_label_values(&[cf_name]).set(cf_metrics.block_cache_usage);

            self.metrics.block_cache_pinned_usage.with_label_values(&[cf_name]).set(cf_metrics.block_cache_pinned_usage);
            ...
            self.metrics.num_level0_files.with_label_values(&[cf_name]).set(cf_metrics.num_level0_files);
            ...
            self.metrics.is_write_stopped.with_label_values(&[cf_name]).set(cf_metrics.is_write_stopped);

            self.metrics.num_immutable_mem_tables.with_label_values(&[cf_name]).set(cf_metrics.num_immutable_mem_tables);

            self.metrics.mem_table_flush_pending.with_label_values(&[cf_name]).set(cf_metrics.mem_table_flush_pending);

            self.metrics.compaction_pending.with_label_values(&[cf_name]).set(cf_metrics.compaction_pending);

            self.metrics.num_snapshots.with_label_values(&[cf_name]).set(cf_metrics.num_snapshots);

            self.metrics.num_running_compactions.with_label_values(&[cf_name]).set(cf_metrics.num_running_compactions);

            self.metrics.num_running_flushes.with_label_values(&[cf_name]).set(cf_metrics.num_running_flushes);
        }
