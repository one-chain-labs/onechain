...
pub(crate) fn checkpoint_output_objects(
    ...
    for tx in &checkpoint.transactions {
        let output_objects_map: BTreeMap<_, _> =
            tx.output_objects(&checkpoint.object_set).map(|obj| ((obj.id(), obj.version()), obj)).collect();
        ...
        for change in tx.effects.object_changes() {
            ...
            let (Some(version), Some(digest)) = (change.output_version, change.output_digest) else {
                continue;
            };
