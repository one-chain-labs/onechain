...
impl<S: Schema + Send + Sync + 'static> Indexer<S> {
    ...
    pub(crate) async fn new(
        ...
        let store = Store::open(path, db_config, consistency_config.snapshots, Some(registry))
    ...
    pub(crate) async fn sequential_pipeline<H>(&mut self, handler: H, config: SequentialConfig) -> anyhow::Result<()>
    where
        H: sequential::Handler<Store = Store<S>> + Send + Sync + 'static,
    {
        ...
        ensure!(!is_restoring, "Restoration in progress for pipeline {:?}", H::NAME);
...
mod tests {
    ...
    impl sequential::Handler for TestHandler {
        ...
        async fn commit<'a>(&self, _: &(), _: &mut Connection<'a, TestSchema>) -> anyhow::Result<usize> {
            Ok(0)
        }
    }
    ...
    async fn test_restore_protection() {
...

        {
            ...
            indexer.sequential_pipeline(TestHandler, SequentialConfig::default()).await.unwrap_err();
        }
...

        {
            ...
            indexer.sequential_pipeline(TestHandler, SequentialConfig::default()).await.unwrap();
        }
    }
}
