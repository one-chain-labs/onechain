...
impl<S: Schema + Send + Sync + 'static> Indexer<S> {
    ...
    pub(crate) async fn new(
        ...
        let store = Store::open(path, db_config, consistency_config.snapshots).context("Failed to create store")?;
    ...
    {
        self.sync.register_pipeline(H::NAME).context("Failed to add pipeline to synchronizer")?;
