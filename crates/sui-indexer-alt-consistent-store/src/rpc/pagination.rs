        ...
        V: Serialize + DeserializeOwned,
    {
        if self.is_from_front {
            self.paginate_from_front(
                map.prefix(checkpoint, prefix).map_err(|e| db_error(e, "failed to create forward iterator"))?,
                pred,
            )
        } else {
            self.paginate_from_back(
                map.prefix_rev(checkpoint, prefix).map_err(|e| db_error(e, "failed to create reverse iterator"))?,
                pred,
            )
        }
    }
        ...
        V: DeserializeOwned,
    {
        ...
        while results.len() < self.limit {
            ...
            if self.before.is_some_and(|b| cursor >= b) {
                return Ok(Response { has_prev: after.is_some(), has_next: true, results });
            }
        ...
        Ok(Response { has_prev: after.is_some(), has_next: iter.valid(), results })
    }
        ...
        V: DeserializeOwned,
    {
        ...
        while results.len() < self.limit {
            ...
            if self.after.is_some_and(|a| a >= cursor) {
                results.reverse();
                return Ok(Response { has_prev: true, has_next: before.is_some(), results });
            }
        ...
        results.reverse();
        Ok(Response { has_prev: iter.valid(), has_next: before.is_some(), results })
    }
}

impl Default for PaginationConfig {
    fn default() -> Self {
        Self { default_page_size: 50, max_batch_size: 200, max_page_size: 200 }
    }
}
...
mod tests {
    ...
    fn config() -> PaginationConfig {
        PaginationConfig { default_page_size: 3, max_batch_size: 5, max_page_size: 5 }
    }
    ...
    fn paginate_prefix_forward() {
        ...
        assert_eq!(
            paginate(1, 0x0001),
            vec![(key::encode(&0x0001_0000u32), 0x0001_0000, 1), (key::encode(&0x0001_0002u32), 0x0001_0002, 21),],
    ...
    fn paginate_prefix_backward() {
        ...
        assert_eq!(
            paginate(1, 0x0001),
            vec![(key::encode(&0x0001_0002u32), 0x0001_0002, 21), (key::encode(&0x0001_0000u32), 0x0001_0000, 1),],
            "Single page, at snapshot 1"
        );
    }
    ...
    fn forward_cursor_sandwich() {
        ...
        assert_eq!(
            Page::from_request(&config(), &a, &b, 5, End::Front)
                .paginate_prefix::<_, _, _, Infallible>(&map, 1, &0x0001u16)
                .unwrap(),
            Response { has_prev: false, has_next: true, results: vec![] },
        ...
        assert_eq!(
            Page::from_request(&config(), &a, &b, 5, End::Front)
                .paginate_prefix::<_, _, _, Infallible>(&map, 0, &0x0000u16)
                .unwrap(),
            Response { has_prev: true, has_next: false, results: vec![] },
        ...
        assert_eq!(
            Page::from_request(&config(), &a, &b, 5, End::Back)
                ...
                .unwrap(),
            Response { has_prev: false, has_next: false, results: vec![] },
