...
use crate::{
    config::{ConsistencyConfig, RpcConfig},
    schema::Schema,
    store::Store,
};
...
impl State {
    ...
    /// determined.
    pub(super) fn checkpoint<T>(&self, request: &tonic::Request<T>) -> Result<u64, RpcError<Error>> {
...
pub(super) fn checkpointed_response<T>(checkpoint: u64, content: T) -> Result<tonic::Response<T>, RpcError<Error>> {
    let checkpoint =
        checkpoint.to_string().parse().with_context(|| format!("Invalid checkpoint for metadata: {checkpoint}"))?;
