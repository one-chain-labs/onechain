...
impl MakeCallbackHandler for MakeMetricsHandler {
    ...
    fn make_handler(&self, request: &http::request::Parts) -> Self::Handler {
        ...
        let path = if let Some(matched_path) = request.extensions.get::<MatchedPath>() {
            if request.headers.get(&CONTENT_TYPE).is_some_and(|header| header == GRPC_CONTENT_TYPE) {
        ...
        let timer = metrics.request_latency.with_label_values(&[path.as_ref()]).start_timer();

        metrics.requests_received.with_label_values(&[path.as_ref()]).inc();

        Handler { metrics, path, timer: Some(timer) }
    }
}
...
impl ResponseHandler for Handler {
    fn on_response(&mut self, response: &http::response::Parts) {
        ...
        let (success, status) = if response.headers.get(&CONTENT_TYPE).is_some_and(|content_type| {
            content_type
                    .as_bytes()
        ...
        if success {
            self.metrics.requests_succeeded.with_label_values(&[self.path.as_ref(), status]).inc();
        } else {
            self.metrics.requests_failed.with_label_values(&[self.path.as_ref(), status]).inc();
        }
    }
...
impl Drop for Handler {
    fn drop(&mut self) {
        if self.timer.is_some() {
            self.metrics.requests_cancelled.with_label_values(&[self.path.as_ref()]).inc();
        }
    }
}
