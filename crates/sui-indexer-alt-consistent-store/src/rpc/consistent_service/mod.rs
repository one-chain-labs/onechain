...
use sui_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::{
    consistent_service_server::ConsistentService,
...
use self::{
    ...
    balances::{batch_get_balances, get_balance, list_balances},
    list_objects_by_type::list_objects_by_type,
    list_owned_objects::list_owned_objects,
...
impl ConsistentService for State {
    ...
    async fn service_config(
        ...
        service_config(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}
