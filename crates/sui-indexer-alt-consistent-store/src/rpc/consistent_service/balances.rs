...
pub(super) fn batch_get_balances(
    ...
    let keys = if request.requests.len() > config.max_batch_size as usize {
        return Err(Error::TooManyRequests(request.requests.len(), state.rpc_config.pagination.max_batch_size).into());
    } else {
        request.requests.into_iter().map(key).collect::<Result<Vec<_>, _>>()?
    };

    let index = &state.store.schema().balances;
    let values = index.multi_get(checkpoint, &keys).map_err(|e| db_error(e, "failed to batch get balances"))?;
    ...
    for (key, value) in keys.into_iter().zip(values) {
        ...
        let balance = u64::try_from(balance).with_context(|| format!("Bad balance for type {coin_type}: {balance}"))?;
...
pub(super) fn get_balance(
    ...
    let key = key(request)?;
    let index = &state.store.schema().balances;
    let balance = index.get(checkpoint, &key).map_err(|e| db_error(e, "failed to get balance"))?.unwrap_or(0);
...
pub(super) fn list_balances(
    ...
    let resp = page.paginate_filtered(index, checkpoint, &Compat(owner), |_, _, balance| *balance > 0)?;
...
fn key(request: grpc::GetBalanceRequest) -> Result<Key, Error> {
    ...
    let type_ = if request.coin_type().is_empty() {
        ...
        TypeTag::from_str(request.coin_type()).map_err(|_| Error::InvalidType(request.coin_type().to_owned()))?
    };
