...
use middleware::{metrics::MakeMetricsHandler, version::Version};
...
impl<'d> RpcService<'d> {
    pub(crate) async fn new(
        ...
        let RpcArgs { rpc_listen_address, tls } = args;

        let TlsArgs { rpc_tls_listen_address, tls_cert, tls_key } = tls;

        let tls_config = if let (Some(cert), Some(key)) = (tls_cert, tls_key) {
            Some(RustlsConfig::from_pem_file(cert, key).await.context("Failed to load TLS configuration")?)
        } else {
            None
        };
    ...
    pub(crate) fn register_encoded_file_descriptor_set(mut self, fds: &'d [u8]) -> Self {
        ...
        self.reflection_v1alpha = self.reflection_v1alpha.register_encoded_file_descriptor_set(fds);
        self
    }
    ...
    pub(crate) async fn run(self) -> anyhow::Result<JoinHandle<()>> {
        ...
        let listener =
            TcpListener::bind(rpc_listen_address).await.context("Failed to bind Consistent RPC to listen address")?;
...
impl Default for RpcArgs {
    fn default() -> Self {
        Self { rpc_listen_address: "0.0.0.0:7001".parse().unwrap(), tls: TlsArgs::default() }
    }
}
