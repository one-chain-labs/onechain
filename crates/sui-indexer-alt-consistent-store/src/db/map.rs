    ...
    V: Serialize + DeserializeOwned,
{
    ...
    pub(crate) fn new(db: Arc<Db>, cf: impl Into<String>) -> Self {
        Self { db, cf: cf.into(), _data: PhantomData }
    }
    ...
    pub(crate) fn iter(&self, checkpoint: u64, range: impl RangeBounds<K>) -> Result<iter::FwdIter<'_, K, V>, Error> {
    ...
    ///
    /// This operation can fail if the database does not have a snapshot at `checkpoint`.
    pub(crate) fn prefix(&self, checkpoint: u64, prefix: &impl Encode) -> Result<iter::FwdIter<'_, K, V>, Error> {
    ...
    ///
    /// This operation can fail if the database does not have a snapshot at `checkpoint`.
    pub(crate) fn prefix_rev(&self, checkpoint: u64, prefix: &impl Encode) -> Result<iter::RevIter<'_, K, V>, Error> {
    ...
    pub(crate) fn insert(
        ...
        batch.put_cf(&self.cf()?, key::encode(k.borrow()), bcs::to_bytes(v.borrow())?);
        Ok(())
    }
    ...
    pub(crate) fn merge(
        ...
        batch.merge_cf(&self.cf()?, key::encode(k.borrow()), bcs::to_bytes(v.borrow())?);
        Ok(())
    }
    ...
    pub(crate) fn remove(&self, k: impl Borrow<K>, batch: &mut rocksdb::WriteBatch) -> Result<(), Error> {
        batch.delete_cf(&self.cf()?, key::encode(k.borrow()));
        Ok(())
    }

    fn cf(&self) -> Result<Arc<rocksdb::BoundColumnFamily<'_>>, Error> {
        self.db.cf(&self.cf).ok_or_else(|| Error::NoColumnFamily(self.cf.clone()))
    }
}
...
mod tests {
    ...
    fn test_insert_remove() {
        ...
        // Multi-gets
        assert_eq!(map.multi_get(0, &[42, 43, 44]).unwrap().into_iter().map(|r| r.unwrap()).collect::<Vec<_>>(), vec![
            Some(43),
            None,
            Some(45)
        ],);
        ...
        // Iteration
        assert_eq!(map.iter(0, ..).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![(42, 43), (44, 45)],);

        assert_eq!(map.iter_rev(0, ..).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![(44, 45), (42, 43)],);
        ...
        // Multi-gets
        assert_eq!(map.multi_get(1, &[42, 43, 44]).unwrap().into_iter().map(|r| r.unwrap()).collect::<Vec<_>>(), vec![
            None,
            Some(42),
            Some(45)
        ],);
        ...
        // Iteration
        assert_eq!(map.iter(1, ..).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![(43, 42), (44, 45)],);

        assert_eq!(map.iter_rev(1, ..).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![(44, 45), (43, 42)],);
    }
    ...
    fn test_prefix_iter() {
        ...
        assert_eq!(map.prefix(0, &0x0000u16).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![
            (0x0000_0001, 10),
            (0x0000_0003, 30),
            (0x0000_0005, 50)
        ],);

        assert_eq!(map.prefix_rev(0, &0xffffu16).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), vec![
            (0xffff_0004, 40),
            (0xffff_0002, 20)
        ],);
    }
