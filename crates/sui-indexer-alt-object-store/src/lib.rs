...
use bytes::Bytes;
use object_store::{path::Path as ObjectPath, Error as ObjectStoreError, PutMode, PutPayload};
use serde::{Deserialize, Serialize};
use sui_indexer_alt_framework_store_traits::{
    self as framework_traits,
    Connection,
    PrunerWatermark,
    ReaderWatermark,
    Store,
...
impl Connection for ObjectStoreConnection {
    async fn committer_watermark(
        ...
        match self.object_store.get(&object_path).await {
            Ok(result) => {
                ...
                let watermark: ComitterWatermark =
                    serde_json::from_slice(&bytes).context("Failed to parse watermark from object store")?;
    ...
    async fn reader_watermark(&mut self, _pipeline: &'static str) -> anyhow::Result<Option<ReaderWatermark>> {
        Ok(None)
    }
    ...
    async fn set_committer_watermark(
        ...
        if current_watermark.is_some() {
            self.object_store
                .put_opts(&object_path, payload, PutMode::Update(object_store::UpdateVersion { e_tag, version }).into())
                .await?;
        } else {
            self.object_store.put_opts(&object_path, payload, PutMode::Create.into()).await?;
        }
...
mod tests {
    ...
    async fn test_watermark_operations() {
        ...
        let result = conn.set_committer_watermark(pipeline, initial_watermark).await.unwrap();
        ...
        let result = conn.set_committer_watermark(pipeline, updated_watermark).await.unwrap();
        assert!(result, "Watermark update with higher checkpoint should succeed");
        ...
        let result = conn.set_committer_watermark(pipeline, regressed_watermark).await.unwrap();
