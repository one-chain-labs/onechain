...
impl SuiFlavor {
    ...
    fn system_deps_names_map() -> BTreeMap<PackageName, PackageName> {
        BTreeMap::from([
            (PackageName::new("Sui").unwrap(), PackageName::new("sui").unwrap()),
            (PackageName::new("OneSystem").unwrap(), PackageName::new("sui_system").unwrap()),
            (PackageName::new("MoveStdlib").unwrap(), PackageName::new("std").unwrap()),
            (PackageName::new("Bridge").unwrap(), PackageName::new("bridge").unwrap()),
            (PackageName::new("DeepBook").unwrap(), PackageName::new("deepbook").unwrap()),
        ])
    }

    /// The default dependencies are `sui` and `std`
    fn default_system_dep_names() -> BTreeSet<PackageName> {
        BTreeSet::from([PackageName::new("sui").unwrap(), PackageName::new("std").unwrap()])
    }
}
...
impl MoveFlavor for SuiFlavor {
    ...
    fn system_dependencies(environment: EnvironmentID) -> BTreeMap<PackageName, ReplacementDependency> {
        ...
        let pkgs = packages.packages.iter().filter(|package| !deps_to_skip.contains(&package.package_name));
        ...
        for package in pkgs {
            ...
            deps.insert(
                names
                    .get(&PackageName::new(package.package_name.clone()).expect("valid package name"))
    ...
    fn default_system_dependencies(environment: EnvironmentID) -> BTreeMap<PackageName, ReplacementDependency> {
        let default_deps = Self::default_system_dep_names();

        Self::system_dependencies(environment).into_iter().filter(|(name, _)| default_deps.contains(name)).collect()
    }
}
...
mod tests {
    ...
    use move_package_alt::{
        package::{manifest::Manifest, RootPackage},
