    ...
    EP: JsonRpcClient + Send + Sync + 'static,
{
    ...
    async fn verify_sui_message(
        ...
        if record.verified_signatures.is_some() {
            return Err(BridgeError::Generic(format!("message {seq_number} already complete")));
        }
        BridgeAction::try_from_bridge_record(&record).tap_ok(|action| info!("Sui action found: {:?}", action))
    }
}
    ...
    EP: JsonRpcClient + Send + Sync + 'static,
{
    ...
    async fn handle_sui_token_transfer(
        ...
        let bridge_action = self.verify_sui_message(source_chain, message_type, bridge_seq_num).await?;
        Ok(Json(self.sign(bridge_action)))
    }
...
mod tests {
    ...
    fn test_handler(
    ...
    ) -> (BridgeRequestHandler<SuiMockClient, EthMockProvider>, SuiMockClient, EthMockProvider, EthAddress) {
        ...
        (handler, sui_client_mock, eth_mock_provider, contract_address)
    }
    ...
    async fn test_sui_verify() {
        ...
        handler.verify_sui((sui_tx_digest, sui_event_idx)).await.unwrap_err();
        ...
        handler.verify_sui((sui_tx_digest, sui_event_idx)).await.unwrap();
        handler.verify_sui((sui_tx_digest, sui_event_idx_2)).await.unwrap();
    }
    ...
    async fn test_eth_verify() {
        ...
        handler.verify_eth((eth_tx_hash, eth_event_idx)).await.unwrap();
    }
