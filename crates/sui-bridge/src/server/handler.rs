...
pub trait BridgeRequestHandlerTrait {
    ...
    async fn handle_governance_action(&self, action: BridgeAction) -> Result<Json<SignedBridgeAction>, BridgeError>;
}
    ...
    K: std::hash::Hash + Eq + Clone + Send + Sync + 'static,
{
    ...
    fn spawn(
        mut self,
        mut rx: mysten_metrics::metered_channel::Receiver<(K, oneshot::Sender<BridgeResult<SignedBridgeAction>>)>,
    ) -> tokio::task::JoinHandle<()> {
        tokio::spawn(async move {
            loop {
                let (key, tx) = rx.recv().await.unwrap_or_else(|| panic!("Server signer's channel is closed"));
    ...
    async fn get_cache_entry(&mut self, key: K) -> Arc<Mutex<Option<BridgeResult<SignedBridgeAction>>>> {
        // This mutex exists to make sure everyone gets the same entry, namely no double insert
        let _ = self.mutex.lock().await;
        self.cache.get_or_insert(key, || Arc::new(Mutex::new(None))).clone()
    }
    ...
    async fn sign(&mut self, key: K) -> BridgeResult<SignedBridgeAction> {
        ...
        if let Some(result) = &*guard {
            self.metrics.signer_with_cache_hit.with_label_values(&[verifier_name]).inc();
            return result.clone();
        }
        self.metrics.signer_with_cache_miss.with_label_values(&[verifier_name]).inc();
    ...
    #[cfg(test)]
    async fn get_testing_only(&mut self, key: K) -> Option<&Arc<Mutex<Option<BridgeResult<SignedBridgeAction>>>>> {
        let _ = self.mutex.lock().await;
        self.cache.get(&key)
    }
}
...
pub struct BridgeRequestHandler {
    ...
    eth_signer_tx:
        mysten_metrics::metered_channel::Sender<((TxHash, u16), oneshot::Sender<BridgeResult<SignedBridgeAction>>)>,
    governance_signer_tx:
        mysten_metrics::metered_channel::Sender<(BridgeAction, oneshot::Sender<BridgeResult<SignedBridgeAction>>)>,
}

impl BridgeRequestHandler {
    pub fn new<SC: SuiClientInner + Send + Sync + 'static, EP: JsonRpcClient + Send + Sync + 'static>(
        ...
        SignerWithCache::new(signer.clone(), SuiActionVerifier { sui_client }, metrics.clone()).spawn(sui_rx);
        SignerWithCache::new(signer.clone(), EthActionVerifier { eth_client }, metrics.clone()).spawn(eth_rx);
        SignerWithCache::new(
            signer.clone(),
        ...
        Self { sui_signer_tx, eth_signer_tx, governance_signer_tx }
    }
}
...
impl BridgeRequestHandlerTrait for BridgeRequestHandler {
    async fn handle_eth_tx_hash(
        ...
        let signed_action = rx.await.unwrap_or_else(|_| panic!("Server signing task's oneshot channel is dropped"))?;
    ...
    async fn handle_governance_action(&self, action: BridgeAction) -> Result<Json<SignedBridgeAction>, BridgeError> {
        ...
        let signed_action =
            rx.await.unwrap_or_else(|_| panic!("Server governance action task's oneshot channel is dropped"))?;
        Ok(Json(signed_action))
    }
}
...
mod tests {
    ...
    async fn test_sui_signer_with_cache() {
        ...
        let sui_verifier =
            SuiActionVerifier { sui_client: Arc::new(SuiClient::new_for_testing(sui_client_mock.clone())) };
        ...
        let action =
            get_test_sui_to_eth_bridge_action(Some(sui_tx_digest), Some(sui_event_idx), None, None, None, None, None);
        ...
        sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx)).await.unwrap_err();
        let entry_ = sui_signer_with_cache.get_testing_only((sui_tx_digest, sui_event_idx)).await;
        assert!(entry_.unwrap().lock().await.is_none());
        ...
        assert!(matches!(
            sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx)).await,
            Err(BridgeError::NoBridgeEventsInTxPosition)
        ));
        let entry_ = sui_signer_with_cache.get_testing_only((sui_tx_digest, sui_event_idx)).await;
        assert_eq!(entry_.unwrap().lock().await.clone().unwrap().unwrap_err(), BridgeError::NoBridgeEventsInTxPosition,);
        ...
        sui_client_mock.add_events_by_tx_digest(sui_tx_digest, vec![sui_event_1.clone(), sui_event_2.clone()]);
        let signed_1 = sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx)).await.unwrap();
        let signed_2 = sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx_2)).await.unwrap();
        ...
        sui_client_mock.add_events_by_tx_digest(sui_tx_digest, vec![]);
        assert_eq!(sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx)).await.unwrap(), signed_1);
        assert_eq!(sui_signer_with_cache.sign((sui_tx_digest, sui_event_idx_2)).await.unwrap(), signed_2);
    }
    ...
    async fn test_eth_signer_with_cache() {
        ...
        let eth_client = EthClient::new_mocked(eth_mock_provider.clone(), HashSet::from_iter(vec![contract_address]));
        let eth_verifier = EthActionVerifier { eth_client: Arc::new(eth_client) };
        let metrics = Arc::new(BridgeMetrics::new_for_testing());
        let mut eth_signer_with_cache = SignerWithCache::new(signer.clone(), eth_verifier, metrics.clone());

        // Test `get_cache_entry` creates a new entry if not exist
        let eth_tx_hash = TxHash::random();
        let eth_event_idx = 42;
        assert!(eth_signer_with_cache.get_testing_only((eth_tx_hash, eth_event_idx)).await.is_none());
        let entry = eth_signer_with_cache.get_cache_entry((eth_tx_hash, eth_event_idx)).await;
        let entry_ = eth_signer_with_cache.get_testing_only((eth_tx_hash, eth_event_idx)).await;
        ...
        let entry_ = eth_signer_with_cache.get_testing_only((eth_tx_hash, eth_event_idx)).await;
        assert_eq!(entry_.unwrap().lock().await.clone().unwrap().unwrap(), signed_action);
        ...
        eth_mock_provider
            .add_response::<[TxHash; 1], TransactionReceipt, TransactionReceipt>(
                ...
                TransactionReceipt { block_number: log.block_number, logs: vec![log.clone()], ..Default::default() },
            )
            .unwrap();
        mock_last_finalized_block(&eth_mock_provider, log.block_number.unwrap().as_u64());

        eth_signer_with_cache.sign((eth_tx_hash, eth_event_idx)).await.unwrap();
        let entry_ = eth_signer_with_cache.get_testing_only((eth_tx_hash, eth_event_idx)).await;
    ...
    async fn test_signer_with_governace_verifier() {
        ...
        assert_eq!(entry_.unwrap().lock().await.clone().unwrap().unwrap().data(), &action_1);
