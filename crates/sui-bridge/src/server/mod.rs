...
impl BridgeNodePublicMetadata {
    pub fn new(version: &'static str, metrics_pubkey: Ed25519PublicKey) -> Self {
        Self { version, metrics_pubkey: Some(metrics_pubkey.into()) }
    }

    pub fn empty_for_testing() -> Self {
        Self { version: "testing", metrics_pubkey: None }
    }
}
...
pub fn run_server(
    ...
    tokio::spawn(async move {
        ...
        axum::serve(listener, make_router(Arc::new(handler), metrics, metadata).into_make_service()).await.unwrap();
    })
}
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ASSET_PRICE_UPDATE_PATH, get(handle_asset_price_update_action))
        .route(EVM_CONTRACT_UPGRADE_PATH, get(handle_evm_contract_upgrade))
        .route(EVM_CONTRACT_UPGRADE_PATH_WITH_CALLDATA, get(handle_evm_contract_upgrade_with_calldata))
...
async fn handle_sui_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler.handle_sui_tx_digest(tx_digest_base58, event_idx).await?;
        Ok(sig)
    };
...
async fn handle_update_committee_blocklist_action(
    ...
    let future = async {
        let chain_id = BridgeChainId::try_from(chain_id)
            .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid chain id: {:?}", err)))?;
    ...
    with_metrics!(metrics.clone(), "handle_update_committee_blocklist_action", future).await
}
...
async fn handle_emergency_action(
    ...
    let future = async {
        ...
        let action = BridgeAction::EmergencyAction(EmergencyAction { chain_id, nonce, action_type });
        let sig: Json<SignedBridgeAction> = handler.handle_governance_action(action).await?;
        Ok(sig)
    };
...
async fn handle_limit_update_action(
    ...
    let future = async {
        let chain_id = BridgeChainId::try_from(chain_id)
            .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid chain id: {:?}", err)))?;
        let sending_chain_id = BridgeChainId::try_from(sending_chain_id)
            .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid chain id: {:?}", err)))?;
        let action =
            BridgeAction::LimitUpdateAction(LimitUpdateAction { chain_id, nonce, sending_chain_id, new_usd_limit });
        let sig: Json<SignedBridgeAction> = handler.handle_governance_action(action).await?;
        Ok(sig)
    };
...
async fn handle_asset_price_update_action(
    ...
    let future = async {
        let chain_id = BridgeChainId::try_from(chain_id)
            .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid chain id: {:?}", err)))?;
        let action =
            BridgeAction::AssetPriceUpdateAction(AssetPriceUpdateAction { chain_id, nonce, token_id, new_usd_price });
        let sig: Json<SignedBridgeAction> = handler.handle_governance_action(action).await?;
        Ok(sig)
    };
...
async fn handle_evm_contract_upgrade_with_calldata(
    Path((chain_id, nonce, proxy_address, new_impl_address, calldata)): Path<(u8, u64, EthAddress, EthAddress, String)>,
    State((handler, metrics, _metadata)): State<(
        Arc<impl BridgeRequestHandlerTrait + Sync + Send>,
        Arc<BridgeMetrics>,
        Arc<BridgeNodePublicMetadata>,
    ...
    let future = async {
        let chain_id = BridgeChainId::try_from(chain_id)
            .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid chain id: {:?}", err)))?;
        let call_data = Hex::decode(&calldata)
            .map_err(|e| BridgeError::InvalidBridgeClientRequest(format!("Invalid call data: {:?}", e)))?;
    ...
    with_metrics!(metrics.clone(), "handle_evm_contract_upgrade_with_calldata", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id, nonce, proxy_address, new_impl_address))]
async fn handle_evm_contract_upgrade(
    Path((chain_id, nonce, proxy_address, new_impl_address)): Path<(u8, u64, EthAddress, EthAddress)>,
    State((handler, metrics, _metadata)): State<(
        Arc<impl BridgeRequestHandlerTrait + Sync + Send>,
        Arc<BridgeMetrics>,
        Arc<BridgeNodePublicMetadata>,
...
async fn handle_add_tokens_on_sui(
    ...
    let future = async {
        ...
        let token_ids = token_ids
            ...
            .map(|s| {
                s.parse::<u8>()
                    .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid token id: {:?}", err)))
            })
            .collect::<Result<Vec<_>, _>>()?;
        let token_type_names = token_type_names
            ...
            .map(|s| {
                TypeTag::from_str(s).map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!("Invalid token type name: {:?}", err))
                })
            })
            .collect::<Result<Vec<_>, _>>()?;
        let token_prices = token_prices
            ...
            .map(|s| {
                s.parse::<u64>()
                    .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid token price: {:?}", err)))
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        let token_addresses = token_addresses
            ...
            .map(|s| {
                EthAddress::from_str(s)
                    .map_err(|err| BridgeError::InvalidBridgeClientRequest(format!("Invalid token address: {:?}", err)))
            })
            .collect::<Result<Vec<_>, _>>()?;
        let token_sui_decimals = token_sui_decimals
            ...
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!("Invalid token sui decimals: {:?}", err))
                })
            })
            .collect::<Result<Vec<_>, _>>()?;
        let token_prices = token_prices
...
mod tests {
    ...
    use crate::{
        ...
        test_utils::get_test_authorities_and_run_mock_bridge_server,
