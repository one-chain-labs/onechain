...
use sui_types::{
    ...
    bridge::{BridgeCommitteeSummary, BridgeSummary, MoveTypeBridgeRecord, MoveTypeParsedTokenTransferMessage},
...
use crate::{
    ...
    types::{BridgeAction, BridgeActionStatus, IsBridgePaused, SuiEvents},
};
...
pub struct SuiMockClient {
    ...
    events_by_tx_digest: Arc<Mutex<HashMap<TransactionDigest, Result<Vec<SuiEvent>, sui_sdk::error::Error>>>>,
    transaction_responses: Arc<Mutex<HashMap<TransactionDigest, BridgeResult<SuiTransactionBlockResponse>>>>,
...
impl SuiMockClient {
    ...
    pub fn add_event_response(&self, package: ObjectID, module: Identifier, cursor: EventID, events: EventPage) {
        self.events.lock().unwrap().insert((package, module, Some(cursor)), events);
    }

    pub fn add_events_by_tx_digest(&self, tx_digest: TransactionDigest, events: Vec<SuiEvent>) {
        self.events_by_tx_digest.lock().unwrap().insert(tx_digest, Ok(events));
    }
    ...
    pub fn add_events_by_tx_digest_error(&self, tx_digest: TransactionDigest) {
        self.events_by_tx_digest
            .lock()
            .unwrap()
            .insert(tx_digest, Err(sui_sdk::error::Error::DataError("".to_string())));
    }
    ...
    pub fn add_transaction_response(
        ...
        self.transaction_responses.lock().unwrap().insert(tx_digest, response);
    }

    pub fn set_action_onchain_status(&self, action: &BridgeAction, status: BridgeActionStatus) {
        self.onchain_status.lock().unwrap().insert((action.chain_id() as u8, action.seq_number()), status);
    }

    pub fn set_bridge_committee(&self, committee: BridgeCommitteeSummary) {
        self.bridge_committee_summary.lock().unwrap().replace(committee);
    }
    ...
    pub fn set_wildcard_transaction_response(&self, response: BridgeResult<SuiTransactionBlockResponse>) {
        *self.wildcard_transaction_response.lock().unwrap() = Some(response);
    }

    pub fn set_latest_checkpoint_sequence_number(&self, value: u64) {
        self.latest_checkpoint_sequence_number.store(value, std::sync::atomic::Ordering::Relaxed);
    }

    pub fn add_gas_object_info(&self, gas_coin: GasCoin, object_ref: ObjectRef, owner: Owner) {
        self.get_object_info.lock().unwrap().insert(object_ref.0, (gas_coin, object_ref, owner));
    }

    pub fn subscribe_to_requested_transactions(&self) -> tokio::sync::broadcast::Receiver<TransactionDigest> {
...
impl SuiClientInner for SuiMockClient {
    ...
    async fn query_events(&self, query: EventFilter, cursor: Option<EventID>) -> Result<EventPage, BridgeError> {
        ...
        match query {
            EventFilter::MoveEventModule { package, module } => {
                self.past_event_query_params.lock().unwrap().push_back((package, module.clone(), cursor));
                Ok(events.get(&(package, module.clone(), cursor)).cloned().unwrap_or_else(|| {
                    panic!(
    ...
    async fn get_events_by_tx_digest(&self, tx_digest: TransactionDigest) -> Result<SuiEvents, BridgeError> {
        ...
        match events.get(&tx_digest).unwrap_or_else(|| panic!("No preset events found for tx_digest: {:?}", tx_digest)) {
    ...
    async fn get_latest_checkpoint_sequence_number(&self) -> Result<u64, BridgeError> {
        Ok(self.latest_checkpoint_sequence_number.load(std::sync::atomic::Ordering::Relaxed))
    }
    ...
    async fn get_bridge_summary(&self) -> Result<BridgeSummary, BridgeError> {
        Ok(BridgeSummary {
            ...
            committee: self.bridge_committee_summary.lock().unwrap().clone().unwrap_or_default(),
    ...
    async fn get_gas_data_panic_if_not_gas(&self, gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner) {
        self.get_object_info
            ...
            .get(&gas_object_id)
            .cloned()
            .unwrap_or_else(|| panic!("No preset gas object info found for gas_object_id: {:?}", gas_object_id))
    }
}
