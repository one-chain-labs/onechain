...
impl BridgeOrchestratorTables {
    pub fn new(path: &Path) -> Arc<Self> {
        Arc::new(Self::open_tables_read_write(path.to_path_buf(), MetricConf::new("bridge"), None, None))
    }

    pub(crate) fn insert_pending_actions(&self, actions: &[BridgeAction]) -> BridgeResult<()> {
        let mut batch = self.pending_actions.batch();
        batch
            .insert_batch(&self.pending_actions, actions.iter().map(|a| (a.digest(), a)))
            .map_err(|e| BridgeError::StorageError(format!("Couldn't insert into pending_actions: {:?}", e)))?;
        batch.write().map_err(|e| BridgeError::StorageError(format!("Couldn't write batch: {:?}", e)))
    }

    pub(crate) fn remove_pending_actions(&self, actions: &[BridgeActionDigest]) -> BridgeResult<()> {
        let mut batch = self.pending_actions.batch();
        batch
            .delete_batch(&self.pending_actions, actions)
            .map_err(|e| BridgeError::StorageError(format!("Couldn't delete from pending_actions: {:?}", e)))?;
        batch.write().map_err(|e| BridgeError::StorageError(format!("Couldn't write batch: {:?}", e)))
    }

    pub(crate) fn update_sui_event_cursor(&self, module: Identifier, cursor: EventID) -> BridgeResult<()> {
        let mut batch = self.sui_syncer_cursors.batch();

        batch
            .insert_batch(&self.sui_syncer_cursors, [(module, cursor)])
            .map_err(|e| BridgeError::StorageError(format!("Coudln't insert into sui_syncer_cursors: {:?}", e)))?;
        batch.write().map_err(|e| BridgeError::StorageError(format!("Couldn't write batch: {:?}", e)))
    }
    ...
    pub(crate) fn update_eth_event_cursor(
        ...
        batch
            ...
            .map_err(|e| BridgeError::StorageError(format!("Coudln't insert into eth_syncer_cursors: {:?}", e)))?;
        batch.write().map_err(|e| BridgeError::StorageError(format!("Couldn't write batch: {:?}", e)))
    }
    ...
    pub fn get_sui_event_cursors(&self, identifiers: &[Identifier]) -> BridgeResult<Vec<Option<EventID>>> {
        self.sui_syncer_cursors
            .multi_get(identifiers)
            .map_err(|e| BridgeError::StorageError(format!("Couldn't get sui_syncer_cursors: {:?}", e)))
    }
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        let action2 = get_test_sui_to_eth_bridge_action(None, Some(2), Some(100), Some(10000), None, None, None);
        ...
        assert_eq!(
            actions,
            HashMap::from_iter(vec![(action1.digest(), action1.clone()), (action2.digest(), action2.clone())])
        );
        ...
        let actions = store.get_all_pending_actions();
        assert_eq!(actions, HashMap::from_iter(vec![(action1.digest(), action1.clone())]));
        ...
        store.update_eth_event_cursor(eth_contract_address, eth_block_num).unwrap();
        assert_eq!(store.get_eth_event_cursors(&[eth_contract_address]).unwrap()[0].unwrap(), eth_block_num);
