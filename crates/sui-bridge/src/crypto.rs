...
impl std::str::FromStr for BridgeAuthorityPublicKeyBytes {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes =
            Hex::decode(s).map_err(|e| FastCryptoError::GeneralError(format!("Failed to decode hex string: {}", e)))?;
...
impl BridgeAuthoritySignInfo {
    pub fn new(msg: &BridgeAction, secret: &BridgeAuthorityKeyPair) -> Self {
        let msg_bytes = msg.to_bytes().expect("Message encoding should not fail for valid actions");
        Self {
    ...
    pub fn verify(&self, msg: &BridgeAction, committee: &BridgeCommittee) -> BridgeResult<()> {
        ...
        if !committee.is_active_member(&self.authority_pub_key_bytes()) {
            return Err(BridgeError::InvalidBridgeAuthority(self.authority_pub_key_bytes()));
        }

        // 2. verify signature
        let msg_bytes = msg
            .to_bytes()
            .map_err(|e| BridgeError::Generic(format!("Failed to encode message for verification: {}", e)))?;

        self.authority_pub_key
            .verify_recoverable_with_hash::<Keccak256>(&msg_bytes, &self.signature)
            .map_err(|e| BridgeError::InvalidBridgeAuthoritySignature((self.authority_pub_key_bytes(), e.to_string())))
    }
...
pub fn verify_signed_bridge_action(
    ...
    sig.verify(signed_action.data(), committee)
        .tap_err(|e| tracing::error!("Failed to verify SignedBridgeEvent {:?}. Error {:?}", signed_action, e))?;
...
mod tests {
    ...
    fn test_sign_and_verify_bridge_event_basic() -> anyhow::Result<()> {
        ...
        let _ = verify_signed_bridge_action(&action, signed_action.clone(), &pubkey_bytes, &committee).unwrap();

        // Verification should fail - mismatched signer
        assert!(matches!(
            verify_signed_bridge_action(&action, signed_action.clone(), &pubkey_bytes2, &committee).unwrap_err(),
        ...
        assert!(matches!(
            verify_signed_bridge_action(&mismatched_action, signed_action.clone(), &pubkey_bytes2, &committee)
                .unwrap_err(),
        ...
        let _ = verify_signed_bridge_action(&action, signed_action, &pubkey_bytes, &committee).unwrap_err();
        ...
        let _ = verify_signed_bridge_action(&action, signed_action2, &pubkey_bytes_2, &committee).unwrap_err();
    ...
    fn test_bridge_sig_verification_regression_test() {
        ...
        let public_key_bytes =
            Hex::decode("026f311bcd1c2664c14277c7a80e4857c690626597064f89edc33b8f67b99c6bc0").unwrap();
        ...
        let public_key_bytes =
            Hex::decode("03a57b85771aedeb6d31c808be9a6e73194e4b70e679608f2bca68bcc684773736").unwrap();
        ...
        let committee =
            BridgeCommittee::new(vec![authority1.clone(), authority2.clone(), authority3.clone(), authority4.clone()])
                .unwrap();
        ...
        let action = BridgeAction::SuiToEthBridgeAction(SuiToEthBridgeAction {
            ...
            sui_bridge_event: EmittedSuiToEthTokenBridgeV1 {
                ...
                sui_address: SuiAddress::from_str("0x80ab1ee086210a3a37355300ca24672e81062fcdb5ced6618dab203f6a3b291c")
                    .unwrap(),
                eth_chain_id: BridgeChainId::EthSepolia,
                eth_address: EthAddress::from_str("0xb18f79Fe671db47393315fFDB377Da4Ea1B7AF96").unwrap(),
    ...
    fn test_bridge_authority_public_key_bytes_to_eth_address() {
        ...
        .unwrap();
        let addr = "0x68b43fd906c0b8f024a18c56e06744f7c6157c65".parse::<EthAddress>().unwrap();
        ...
        let pub_key_bytes = BridgeAuthorityPublicKeyBytes::from_bytes(
            &Hex::decode("0376698beebe8ee5c74d8cc50ab84ac301ee8f10af6f28d0ffd6adf4d6d3b9b762").unwrap(),
        )
        .unwrap();
        let addr = "0Ac1dF02185025F65202660F8167210A80dD5086".parse::<EthAddress>().unwrap();
        assert_eq!(pub_key_bytes.to_eth_address(), addr);
    }
}
