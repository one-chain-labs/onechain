...
use sui_bridge::{
    ...
    metrics::start_metrics_push_task,
...
async fn main() -> anyhow::Result<()> {
    ...
    // Init metrics server
    let metrics_address = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), config.metrics_port);
    ...
    start_metrics_push_task(&config.metrics, config.metrics_key_pair.copy(), registry_service.clone());
    Ok(run_bridge_node(config, metadata, prometheus_registry).await?.await?)
}
