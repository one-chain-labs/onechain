...
pub trait BridgeActionExecutorTrait {
    fn run(
        self,
    ) -> (Vec<tokio::task::JoinHandle<()>>, mysten_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>);
}
    ...
    C: SuiClientInner + 'static,
{
    ...
    fn run_inner(
        ...
        let (sender, receiver) = mysten_metrics::metered_channel::channel(
            CHANNEL_SIZE,
            &mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&["executor_signing_queue"]),
        );

        let (execution_tx, execution_rx) = mysten_metrics::metered_channel::channel(
            CHANNEL_SIZE,
            &mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&["executor_execution_queue"]),
        );
        ...
        tasks.push(spawn_logged_monitored_task!(Self::run_signature_aggregation_loop(
            client_clone,
        ...
        let metrics = self.metrics.clone();
        let execution_tx_clone = execution_tx.clone();
        tasks.push(spawn_logged_monitored_task!(Self::run_onchain_execution_loop(
    ...
    async fn run_signature_aggregation_loop(
        ...
        mut signing_queue_receiver: mysten_metrics::metered_channel::Receiver<BridgeActionExecutionWrapper>,
        execution_queue_sender: mysten_metrics::metered_channel::Sender<CertifiedBridgeActionExecutionWrapper>,
        metrics: Arc<BridgeMetrics>,
    ) {
    ...
    async fn should_proceed_signing(sui_client: &Arc<SuiClient<C>>) -> bool {
        let Ok(Ok(is_paused)) = retry_with_max_elapsed_time!(sui_client.is_bridge_paused(), Duration::from_secs(600))
        else {
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        let status = sui_client
            .get_token_transfer_action_onchain_status_until_success(action.chain_id() as u8, action.seq_number())
            .await;
        match status {
            BridgeActionStatus::Approved | BridgeActionStatus::Claimed => {
                info!("Action already approved or claimed, removing action from pending logs: {:?}", action);
    ...
    async fn request_signatures(
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(&sui_client, &action, &store, &metrics).await {
            return;
        }
        match auth_agg.load().request_committee_signatures(action.clone()).await {
            Ok(certificate) => {
                info!("Sending certificate to execution");
                execution_queue_sender.send(CertifiedBridgeActionExecutionWrapper(certificate, 0)).await.unwrap_or_else(
                    |e| {
            ...
            Err(e) => {
                ...
                signing_queue_sender.send(BridgeActionExecutionWrapper(action, attempt_times + 1)).await.unwrap_or_else(
                    |e| {
    ...
    async fn run_onchain_execution_loop(
        ...
        store: Arc<BridgeOrchestratorTables>,
        execution_queue_sender: mysten_metrics::metered_channel::Sender<CertifiedBridgeActionExecutionWrapper>,
        mut execution_queue_receiver: mysten_metrics::metered_channel::Receiver<CertifiedBridgeActionExecutionWrapper>,
        bridge_object_arg: ObjectArg,
        sui_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        bridge_pause_rx: tokio::sync::watch::Receiver<IsBridgePaused>,
        metrics: Arc<BridgeMetrics>,
    ...
    async fn handle_execution_task(
        ...
        metrics.action_executor_execution_queue_received_actions.inc();
        ...
        let sig = Signature::new_secure(&IntentMessage::new(Intent::sui_transaction(), &tx_data), sui_key);
        ...
        match sui_client.execute_transaction_block_with_effects(signed_tx).await {
            Ok(resp) => Self::handle_execution_effects(tx_digest, resp, store, action, metrics).await,

            // If the transaction did not go through, retry up to a certain times.
            Err(err) => {
                error!(?action_key, ?tx_digest, "Sui transaction failed at signing: {err:?}");
    ...
    async fn handle_execution_effects(
        ...
        let effects = response.effects.clone().expect("We requested effects but got None.");
    ...
    async fn get_gas_data_assert_ownership(
        ...
        let (gas_coin, gas_obj_ref, owner) = sui_client.get_gas_data_panic_if_not_gas(gas_object_id).await;
...
pub async fn submit_to_executor(
    ...
    tx.send(BridgeActionExecutionWrapper(action, 0)).await.map_err(|e| BridgeError::Generic(e.to_string()))
}
...
mod tests {
    ...
    async fn test_onchain_execution_loop() {
        ...
        sui_client_mock.add_gas_object_info(gas_coin.clone(), gas_object_ref, Owner::AddressOwner(sui_address));
        ...
        mock_transaction_response(
            &sui_client_mock,
            tx_digest,
            SuiExecutionStatus::Failure { error: "failure is mother of success".to_string() },
            None,
            true,
        );
        ...
        assert!(store.get_all_pending_actions().contains_key(&action.digest()));
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let id_token_map = (*sui_token_type_tags.load().clone()).clone();
        let (action_certificate, sui_tx_digest, sui_tx_event_index) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
            true,
        ...
        let action = action_certificate.data().clone();
        mock_bridge_authority_signing_errors(vec![&mock0, &mock1, &mock2], sui_tx_digest, sui_tx_event_index);
        let mut sigs =
            mock_bridge_authority_sigs(vec![&mock3], &action, vec![&secrets[3]], sui_tx_digest, sui_tx_event_index);
        ...
        loop {
            let requested_times = mock0.get_sui_token_events_requested(sui_tx_digest, sui_tx_event_index);
        ...
        assert_eq!(tx_subscription.try_recv().unwrap_err(), tokio::sync::broadcast::error::TryRecvError::Empty);
        // Still in WAL
        assert_eq!(store.get_all_pending_actions()[&action.digest()], action.clone());

        // Let authorities sign the action too. Now we are above the threshold
        let sig_from_2 =
            mock_bridge_authority_sigs(vec![&mock2], &action, vec![&secrets[2]], sui_tx_digest, sui_tx_event_index);
        sigs.extend(sig_from_2);
        let certified_action =
            CertifiedBridgeAction::new_from_data_and_sig(action.clone(), BridgeCommitteeValiditySignInfo {
                signatures: sigs,
            });
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        let tx_data =
            build_sui_transaction(sui_address, &gas_object_ref, action_certificate.clone(), arg, &id_token_map, 1000)
                .unwrap();
        let tx_digest = get_tx_digest(tx_data, &dummy_sui_key);
        mock_transaction_error(&sui_client_mock, tx_digest, BridgeError::Generic("some random error".to_string()), true);
        ...
        execution_tx.send(CertifiedBridgeActionExecutionWrapper(action_certificate, 0)).await.unwrap();
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        // Kick it (send to the execution queue, skipping the signing queue)
        execution_tx.send(CertifiedBridgeActionExecutionWrapper(action_certificate.clone(), 0)).await.unwrap();
        ...
        assert_eq!(tx_subscription.try_recv().unwrap_err(), tokio::sync::broadcast::error::TryRecvError::Empty);
        // Still in WAL
        assert_eq!(store.get_all_pending_actions()[&action_digest], action.clone());
    }
    ...
    fn mock_bridge_authority_sigs(
        ...
        for (mock, secret) in mocks.iter().zip(secrets.iter()) {
            ...
            mock.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(signed_action.clone()), None);
    ...
    fn get_bridge_authority_approved_action(
        ...
        let sigs = mock_bridge_authority_sigs(mocks, &action, secrets, sui_tx_digest, sui_tx_event_index);
        let certified_action =
            CertifiedBridgeAction::new_from_data_and_sig(action, BridgeCommitteeValiditySignInfo { signatures: sigs });
        (VerifiedCertifiedBridgeAction::new_from_verified(certified_action), sui_tx_digest, sui_tx_event_index)
    }

    fn get_tx_digest(tx_data: TransactionData, dummy_sui_key: &SuiKeyPair) -> TransactionDigest {
        let sig = Signature::new_secure(&IntentMessage::new(Intent::sui_transaction(), &tx_data), dummy_sui_key);
        let signed_tx = Transaction::from_data(tx_data, vec![sig]);
        *signed_tx.digest()
    }
    ...
    async fn setup() -> (
        ...
        let agg = Arc::new(ArcSwap::new(Arc::new(BridgeAuthorityAggregator::new_for_testing(Arc::new(committee)))));
