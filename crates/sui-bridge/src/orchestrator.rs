    ...
    C: SuiClientInner + 'static,
{
    pub fn new(
        ...
        Self { _sui_client: sui_client, sui_events_rx, eth_events_rx, store, sui_monitor_tx, eth_monitor_tx, metrics }
    }

    pub async fn run(self, bridge_action_executor: impl BridgeActionExecutorTrait) -> Vec<JoinHandle<()>> {
        ...
        for action in actions {
            submit_to_executor(&executor_sender, action).await.expect("Submit to executor should not fail");
        }
    ...
    async fn run_sui_watcher(
        ...
        while let Some((identifier, events)) = sui_events_rx.recv().await {
            ...
            metrics.sui_watcher_received_events.inc_by(events.len() as u64);
            let bridge_events = events
                ...
                .filter_map(|sui_event| {
                    match SuiBridgeEvent::try_from_sui_event(sui_event) {
                        ...
                        Err(e) => {
                            panic!("Sui Event could not be deserialzed to SuiBridgeEvent: {:?}", e);
                        }
                    }
                })
                .collect::<Vec<_>>();
            ...
            for (sui_event, opt_bridge_event) in events.iter().zip(bridge_events) {
                ...
                monitor_tx.send(bridge_event.clone()).await.expect("Sending event to monitor channel should not fail");

                if let Some(action) =
                    bridge_event.try_into_bridge_action(sui_event.id.tx_digest, sui_event.id.event_seq as u16)
                {
            ...
            if !actions.is_empty() {
                ...
                metrics.sui_watcher_received_actions.inc_by(actions.len() as u64);
                // Write action to pending WAL
                store.insert_pending_actions(&actions).expect("Store operation should not fail");
                for action in actions {
                    submit_to_executor(&executor_tx, action).await.expect("Submit to executor should not fail");
                }
            }

            // Unwrap safe: in the beginning of the loop we checked that events is not empty
            let cursor = events.last().unwrap().id;
            store.update_sui_event_cursor(identifier, cursor).expect("Store operation should not fail");
        }
    ...
    async fn run_eth_watcher(
        store: Arc<BridgeOrchestratorTables>,
        executor_tx: mysten_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        mut eth_events_rx: mysten_metrics::metered_channel::Receiver<(ethers::types::Address, u64, Vec<EthLog>)>,
        eth_monitor_tx: mysten_metrics::metered_channel::Sender<EthBridgeEvent>,
        metrics: Arc<BridgeMetrics>,
    ) {
        ...
        while let Some((contract, end_block, logs)) = eth_events_rx.recv().await {
            ...
            metrics.eth_watcher_received_events.inc_by(logs.len() as u64);

            let bridge_events = logs.iter().map(EthBridgeEvent::try_from_eth_log).collect::<Vec<_>>();
            ...
            if !actions.is_empty() {
                ...
                metrics.eth_watcher_received_actions.inc_by(actions.len() as u64);
                // Write action to pending WAL
                store.insert_pending_actions(&actions).expect("Store operation should not fail");
...
mod tests {
    ...
    async fn test_sui_watcher_task() {
        ...
        sui_events_tx.send((identifier.clone(), vec![sui_event.clone()])).await.unwrap();

        let start = std::time::Instant::now();
        // Executor should have received the action
        assert_eq!(executor_requested_action_rx.recv().await.unwrap(), bridge_action.digest());
        loop {
            ...
            assert_eq!(store.get_sui_event_cursors(&[identifier]).unwrap()[0].unwrap(), sui_event.id,);
            break;
        }
    }
    ...
    async fn test_eth_watcher_task() {
        ...
        eth_events_tx.send((address, end_block_num, vec![eth_log.clone()])).await.unwrap();
        ...
        loop {
            ...
            let action = actions.get(&bridge_action.digest()).unwrap();
            assert_eq!(action, &bridge_action);
            assert_eq!(store.get_eth_event_cursors(&[address]).unwrap()[0].unwrap(), end_block_num,);
            break;
        }
    }
    ...
    async fn test_resume_actions_in_pending_logs() {
        ...
        let action1 = get_test_sui_to_eth_bridge_action(None, Some(0), Some(99), Some(10000), None, None, None);

        let action2 = get_test_eth_to_sui_bridge_action(None, None, None, None);
        store.insert_pending_actions(&vec![action1.clone(), action2.clone()]).unwrap();
    ...
    fn setup() -> (
        ...
        let (eth_events_tx, eth_events_rx) = mysten_metrics::metered_channel::channel(
            100,
            &mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&["unit_test_eth_events_queue"]),
        );

        let (sui_events_tx, sui_events_rx) = mysten_metrics::metered_channel::channel(
            100,
            &mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&["unit_test_sui_events_queue"]),
        );
    ...
    impl MockExecutor {
        fn new() -> (Self, tokio::sync::broadcast::Receiver<BridgeActionDigest>) {
            ...
            (Self { requested_transactions_tx: tx }, rx)
        }
    }
    ...
    impl BridgeActionExecutorTrait for MockExecutor {
        ...
        {
            let (tx, mut rx) = mysten_metrics::metered_channel::channel::<BridgeActionExecutionWrapper>(
                100,
                &mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&["unit_test_mock_executor"]),
            );

            let handles = tokio::spawn(async move {
                while let Some(action) = rx.recv().await {
                    self.requested_transactions_tx.send(action.0.digest()).unwrap();
                }
            });
            (vec![handles], tx)
        }
