...
use sui_types::{
    ...
    object::Owner,
    parse_sui_type_tag,
    transaction::{ObjectArg, SharedObjectMutability, Transaction},
    Identifier,
    TypeTag,
    BRIDGE_PACKAGE_ID,
...
use crate::{
    ...
    types::{BridgeAction, BridgeActionStatus, BridgeAuthority, BridgeCommittee, ParsedTokenTransferMessage, SuiEvents},
};
...
impl SuiBridgeClient {
    pub async fn new(rpc_url: &str, bridge_metrics: Arc<BridgeMetrics>) -> anyhow::Result<Self> {
        let jsonrpc_client = SuiClientBuilder::default()
            .build(rpc_url)
            .await
            .map_err(|e| anyhow!("Can't establish connection with Sui Rpc {rpc_url}. Error: {e}"))?;
        let grpc_client = sui_rpc::Client::new(rpc_url)?;
        let inner = SuiClientInternal { jsonrpc_client, grpc_client };
        let self_ = Self { inner, bridge_metrics };
    ...
    P: SuiClientInner,
{
    pub fn new_for_testing(inner: P) -> Self {
        Self { inner, bridge_metrics: Arc::new(BridgeMetrics::new_for_testing()) }
    }
    ...
    async fn describe(&self) -> Result<(), BridgeError> {
        ...
        tracing::info!("SuiClient is connected to chain {chain_id}, current block number: {block_number}");
        Ok(())
    }
    ...
    pub async fn get_mutable_bridge_object_arg_must_succeed(&self) -> ObjectArg {
        ...
        *ARG.get_or_init(|| async move {
            let Ok(Ok(bridge_object_arg)) =
                retry_with_max_elapsed_time!(self.inner.get_mutable_bridge_object_arg(), Duration::from_secs(30))
            else {
    ...
    pub async fn query_events_by_module(
        ...
        let filter = EventFilter::MoveEventModule { package, module: module.clone() };
        ...
        assert!(events
            .data
            .iter()
            .all(|event| event.type_.address.as_ref() == package.as_ref() && event.type_.module == module));
        Ok(events)
    }
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        let event = events.events.get(event_idx as usize).ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
        if event.type_.address.as_ref() != BRIDGE_PACKAGE_ID.as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedSuiPackage);
        }
        let bridge_event = SuiBridgeEvent::try_from_sui_event(event)?.ok_or(BridgeError::NoBridgeEventsInTxPosition)?;

        bridge_event.try_into_bridge_action().ok_or(BridgeError::BridgeEventNotActionable)
    }
    ...
    pub async fn is_bridge_paused(&self) -> BridgeResult<bool> {
        self.get_bridge_summary().await.map(|summary| summary.is_frozen)
    }
    ...
    pub async fn get_token_id_map(&self) -> BridgeResult<HashMap<u8, TypeTag>> {
        self.get_bridge_summary()
            ...
            .into_iter()
            .map(|(id, name)| {
                parse_sui_type_tag(&format!("0x{name}")).map(|name| (id, name)).map_err(|e| {
                    BridgeError::InternalError(format!("Failed to retrieve token id mapping: {e}, type name: {name}"))
                })
            })
            .collect()
    }
        ...
        bridge_summary
            ...
            .map(|(id, type_name)| {
                bridge_summary
                    ...
                    .supported_tokens
                    .iter()
                    .find_map(|(tn, metadata)| if type_name == tn { Some((*id, metadata.notional_value)) } else { None })
                    .ok_or(BridgeError::InternalError("Error encountered when retrieving token notional values.".into()))
            })
            .collect()
    }
    ...
    pub async fn get_bridge_committee(&self) -> BridgeResult<BridgeCommittee> {
        ...
        for (_, member) in move_type_bridge_committee.members {
            let MoveTypeCommitteeMember { sui_address, bridge_pubkey_bytes, voting_power, http_rest_url, blocklisted } =
                member;
    ...
    pub async fn get_reference_gas_price_until_success(&self) -> u64 {
        loop {
            let Ok(Ok(rgp)) =
                retry_with_max_elapsed_time!(self.inner.get_reference_gas_price(), Duration::from_secs(30))
            else {
                self.bridge_metrics.sui_rpc_errors.with_label_values(&["get_reference_gas_price"]).inc();
    ...
    pub async fn get_token_transfer_action_onchain_status_until_success(
        ...
        loop {
            ...
            let Ok(Ok(status)) = retry_with_max_elapsed_time!(
                self.inner.get_token_transfer_action_onchain_status(bridge_object_arg, source_chain_id, seq_number),
                ...
                self.bridge_metrics
                    ...
                    .inc();
                error!(source_chain_id, seq_number, "Failed to get token transfer action onchain status");
                continue;
            };
            return status;
        }
    ...
    pub async fn get_token_transfer_action_onchain_signatures_until_success(
        ...
        loop {
            ...
            let Ok(Ok(sigs)) = retry_with_max_elapsed_time!(
                self.inner.get_token_transfer_action_onchain_signatures(bridge_object_arg, source_chain_id, seq_number),
                Duration::from_secs(30)
            ) else {
                self.bridge_metrics
                    .sui_rpc_errors
                    ...
                    .inc();
                error!(source_chain_id, seq_number, "Failed to get token transfer action onchain signatures");
                continue;
            };
            return sigs;
        }
    ...
    pub async fn get_parsed_token_transfer_message(
        ...
        let bridge_object_arg = self.get_mutable_bridge_object_arg_must_succeed().await;
        let message =
            self.inner.get_parsed_token_transfer_message(bridge_object_arg, source_chain_id, seq_number).await?;
        Ok(match message {
    ...
    pub async fn get_bridge_record(
        ...
        self.inner.get_bridge_record(source_chain_id, seq_number).await
    }

    pub async fn get_gas_data_panic_if_not_gas(&self, gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner) {
        self.inner.get_gas_data_panic_if_not_gas(gas_object_id).await
    }
}
...
pub trait SuiClientInner: Send + Sync {
    async fn query_events(&self, query: EventFilter, cursor: Option<EventID>) -> Result<EventPage, BridgeError>;

    async fn get_events_by_tx_digest(&self, tx_digest: TransactionDigest) -> Result<SuiEvents, BridgeError>;
    ...
    async fn get_gas_data_panic_if_not_gas(&self, gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner);
}

#[async_trait]
impl SuiClientInner for SuiSdkClient {
    async fn query_events(&self, query: EventFilter, cursor: Option<EventID>) -> Result<EventPage, BridgeError> {
        self.event_api().query_events(query, cursor, None, false).await.map_err(Into::into)
    }

    async fn get_events_by_tx_digest(&self, _tx_digest: TransactionDigest) -> Result<SuiEvents, BridgeError> {
    ...
    async fn get_gas_data_panic_if_not_gas(&self, gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner) {
        loop {
            match self
                .read_api()
                .get_object_with_options(gas_object_id, SuiObjectDataOptions::default().with_owner().with_content())
                .await
                .map(|resp| resp.data)
            {
                Ok(Some(gas_obj)) => {
...
impl SuiClientInner for sui_rpc::Client {
    async fn query_events(&self, _query: EventFilter, _cursor: Option<EventID>) -> Result<EventPage, BridgeError> {
    ...
    async fn get_events_by_tx_digest(&self, tx_digest: TransactionDigest) -> Result<SuiEvents, BridgeError> {
        ...
        let resp = client
            .ledger_client()
            .get_transaction(GetTransactionRequest::new(&(tx_digest.into())).with_read_mask(FieldMask::from_paths([
        ...
        Ok(SuiEvents {
            ...
            events: resp
                ...
                .map(|(idx, event)| {
                    Ok(SuiEvent {
                        id: EventID { tx_digest, event_seq: idx as u64 },
                        ...
                        bcs: BcsEvent::Base64 { bcs: event.contents().value().into() },
                        timestamp_ms: None,
                    })
                })
    ...
    async fn get_reference_gas_price(&self) -> Result<u64, BridgeError> {
        let mut client = self.clone();
        sui_rpc::Client::get_reference_gas_price(&mut client).await.map_err(Into::into)
    }
        ...
        let resp = client
            ...
            .get_checkpoint(
                GetCheckpointRequest::latest()
                    .with_read_mask(FieldMask::from_paths([Checkpoint::path_builder().sequence_number()])),
            )
            .await?
            .into_inner();
    ...
    async fn get_mutable_bridge_object_arg(&self) -> Result<ObjectArg, BridgeError> {
        let owner = self
            ...
            .ledger_client()
            .get_object(
                GetObjectRequest::new(&(SUI_BRIDGE_OBJECT_ID.into()))
                    .with_read_mask(FieldMask::from_paths([Object::path_builder().owner().finish()])),
            )
            .await?
            .into_inner()
            .object()
    ...
    async fn get_bridge_summary(&self) -> Result<BridgeSummary, BridgeError> {
        static BRIDGE_VERSION_ID: tokio::sync::OnceCell<Address> = tokio::sync::OnceCell::const_new();
        ...
        let bridge_version_id = BRIDGE_VERSION_ID
            .get_or_try_init::<BridgeError, _, _>(|| async {
                let bridge_wrapper_bcs = self
                    ...
                    .get_object(
                        GetObjectRequest::new(&(SUI_BRIDGE_OBJECT_ID.into()))
                            .with_read_mask(FieldMask::from_paths([Object::path_builder().contents().finish()])),
                    )
                    .await?
                    .into_inner()
                    .object()
        ...
        let bridge_inner_id =
            bridge_version_id.derive_dynamic_child_id(&sui_sdk_types::TypeTag::U64, &bcs::to_bytes(&1u64).unwrap());

        let field_bcs = self
            .clone()
            .ledger_client()
            .get_object(
                GetObjectRequest::new(&bridge_inner_id)
                    .with_read_mask(FieldMask::from_paths([Object::path_builder().contents().finish()])),
            )
            .await?
            .into_inner()
    ...
    async fn get_parsed_token_transfer_message(
        ...
        let MoveTypeBridgeMessage { message_type: _, message_version, seq_num, source_chain, payload } = record.message;
        ...
        Ok(Some(MoveTypeParsedTokenTransferMessage { message_version, seq_num, source_chain, payload, parsed_payload }))
    }
    ...
    async fn get_bridge_record(
        ...
        static BRIDGE_RECORDS_ID: tokio::sync::OnceCell<Address> = tokio::sync::OnceCell::const_new();

        let records_id = BRIDGE_RECORDS_ID
            .get_or_try_init(|| async {
                self.get_bridge_summary().await.map(|summary| summary.bridge_records_id.into())
            })
            .await?;
        ...
        let response = match self
            ...
            .ledger_client()
            .get_object(
                GetObjectRequest::new(&record_id)
                    .with_read_mask(FieldMask::from_paths([Object::path_builder().contents().finish()])),
            )
            .await
        {
    ...
    async fn get_gas_data_panic_if_not_gas(&self, _gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner) {
        todo!()
    }
}
...
impl SuiClientInner for SuiClientInternal {
    ...
    async fn get_latest_checkpoint_sequence_number(&self) -> Result<u64, BridgeError> {
        self.grpc_client.get_latest_checkpoint_sequence_number().await
    }
    ...
    async fn get_token_transfer_action_onchain_status(
        ...
        self.grpc_client.get_token_transfer_action_onchain_status(bridge_object_arg, source_chain_id, seq_number).await
    }
    ...
    async fn get_token_transfer_action_onchain_signatures(
        ...
        self.grpc_client
            .get_token_transfer_action_onchain_signatures(bridge_object_arg, source_chain_id, seq_number)
            .await
    }
    ...
    async fn execute_transaction_block_with_effects(
        ...
        self.jsonrpc_client.execute_transaction_block_with_effects(tx).await
    }
    ...
    async fn get_parsed_token_transfer_message(
        ...
        self.grpc_client.get_parsed_token_transfer_message(bridge_object_arg, source_chain_id, seq_number).await
    }
    ...
    async fn get_bridge_record(
        ...
        self.grpc_client.get_bridge_record(source_chain_id, seq_number).await
    }

    async fn get_gas_data_panic_if_not_gas(&self, gas_object_id: ObjectID) -> (GasCoin, ObjectRef, Owner) {
        self.jsonrpc_client.get_gas_data_panic_if_not_gas(gas_object_id).await
    }
}
...
mod tests {
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        assert_eq!(
            sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 0).await.unwrap(),
            expected_action,
        );
        assert_eq!(
            sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2).await.unwrap(),
            expected_action,
        );
        assert!(matches!(
            sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 1).await.unwrap_err(),
            BridgeError::NoBridgeEventsInTxPosition
        ),);
        assert!(matches!(
            sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 3).await.unwrap_err(),
            BridgeError::BridgeEventInUnrecognizedSuiPackage
        ),);
        assert!(matches!(
            sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 4).await.unwrap_err(),
            BridgeError::NoBridgeEventsInTxPosition
        ),);
        ...
        sui_client.get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2).await.unwrap_err();
    }
    ...
    async fn test_get_action_onchain_status_for_sui_to_eth_transfer() {
        ...
        let sui_client = SuiClient::new(&test_cluster.inner.fullnode_handle.rpc_url, bridge_metrics).await.unwrap();
        ...
        let status = sui_client
            .inner
            .get_token_transfer_action_onchain_status(bridge_object_arg, action.chain_id() as u8, action.seq_number())
            .await
            .unwrap();
        ...
        let action = get_test_sui_to_eth_bridge_action(None, None, Some(100), None, None, None, None);
        let status = sui_client
            .inner
            .get_token_transfer_action_onchain_status(bridge_object_arg, action.chain_id() as u8, action.seq_number())
            .await
