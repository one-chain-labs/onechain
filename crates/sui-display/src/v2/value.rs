...
use crate::v2::{error::FormatError, parser::Transform, writer::BoundedWriter};
...
impl Value<'_> {
    ...
    pub(crate) fn format(self, transform: Transform, w: &mut BoundedWriter<'_>) -> Result<(), FormatError> {
    ...
    pub(crate) fn as_u64(&self) -> Option<u64> {
        ...
        match self {
            ...
            V::Slice(Slice { layout, bytes: data }) => match layout {
            ...
            V::Address(_) | V::Bool(_) | V::Bytes(_) | V::Enum(_) | V::String(_) | V::Struct(_) | V::Vector(_) => None,
        }
    }
}
...
impl Atom<'_> {
    ...
    fn format_as_str(&self, w: &mut BoundedWriter<'_>) -> Result<(), FormatError> {
        match self {
            ...
            Atom::Bytes(bs) => {
                let s = str::from_utf8(bs).map_err(|_| FormatError::TransformInvalid("expected utf8 bytes"))?;
                write!(w, "{s}")?;
            }
        }
    ...
    fn format_as_timestamp(&self, w: &mut BoundedWriter<'_>) -> Result<(), FormatError> {
        let ts = self
            ...
            .ok_or_else(|| FormatError::TransformInvalid("expected unix timestamp in milliseconds"))?;
    ...
    fn format_as_base64(&self, e: &impl Engine, w: &mut BoundedWriter<'_>) -> Result<(), FormatError> {
...
impl Serialize for Enum<'_> {
        ...
        S: serde::Serializer,
    {
        ...
        let mut s = serializer.serialize_tuple_variant("", self.variant_index as u32, "", self.fields.len())?;
        ...
        Ok(match value {
            ...
            V::Vector(Vector { type_, elements }) => {
                ...
                let bytes: Result<Vec<_>, _> = elements
                    ...
                    .map(|e| match e {
                        V::U8(b) => Ok(b),
                        V::Slice(Slice { layout, bytes }) if layout == &L::U8 => Ok(bcs::from_bytes(bytes)?),
            ...
            V::Slice(Slice { layout, bytes }) => match layout {
                ...
                L::Vector(layout) if layout.as_ref() == &L::U8 => A::Bytes(Cow::Borrowed(bcs::from_bytes(bytes)?)),

                L::Struct(layout)
                    if [move_ascii_str_layout(), move_utf8_str_layout(), url_layout()].contains(layout.as_ref()) =>
                {
...
pub(crate) mod tests {
    ...
    use move_core_types::{
        annotated_value::{MoveEnumLayout, MoveFieldLayout, MoveStructLayout, MoveTypeLayout as L},
        identifier::Identifier,
    };
    use sui_types::{
        base_types::{STD_ASCII_MODULE_NAME, STD_ASCII_STRUCT_NAME},
    ...
    impl MockStore {
        ...
        pub(crate) fn with_dynamic_field<N: Serialize, V: Serialize>(
            ...
            let field_bytes = bcs::to_bytes(&Field { id: UID::new(df_id), name, value }).unwrap();
        ...
        pub(crate) fn with_dynamic_object_field<N: Serialize, V: Serialize>(
            ...
            let dof_id = derive_dynamic_field_id(parent, &wrap_type.clone().into(), &name_bytes).unwrap();

            let field_bytes = bcs::to_bytes(&Field { id: UID::new(dof_id), name, value: val_id }).unwrap();
    ...
    impl<'s> Store<'s> for &'s MockStore {
        async fn object(&self, id: AccountAddress) -> anyhow::Result<Option<Slice<'s>>> {
            ...
            Ok(Some(Slice { layout, bytes: bytes.as_slice() }))
        }
    }
    ...
    pub fn enum_(type_: &str, variants: Vec<(&str, Vec<(&str, MoveTypeLayout)>)>) -> MoveTypeLayout {
        let type_: StructTag = type_.parse().unwrap();
    ...
    pub fn optional_(layout: MoveTypeLayout) -> MoveTypeLayout {
        ...
        struct_(&format!("0x1::option::Option<{type_}>"), vec![("vec", vector_(layout))])
    }

    #[test]
    fn test_slice_serialize_roundtrip() {
        let bytes = &[0x01, 0x02, 0x03, 0x04];
        let slice = Slice { layout: &L::U64, bytes };
    ...
    fn test_serialize_bool() {
        assert_eq!(bcs::to_bytes(&Value::Bool(true)).unwrap(), bcs::to_bytes(&true).unwrap());
        assert_eq!(bcs::to_bytes(&Value::Bool(false)).unwrap(), bcs::to_bytes(&false).unwrap());
    }

    #[test]
    fn test_serialize_u8() {
        assert_eq!(bcs::to_bytes(&Value::U8(42)).unwrap(), bcs::to_bytes(&42u8).unwrap());
    }

    #[test]
    fn test_serialize_u16() {
        assert_eq!(bcs::to_bytes(&Value::U16(1234)).unwrap(), bcs::to_bytes(&1234u16).unwrap());
    }

    #[test]
    fn test_serialize_u32() {
        assert_eq!(bcs::to_bytes(&Value::U32(123456)).unwrap(), bcs::to_bytes(&123456u32).unwrap());
    }

    #[test]
    fn test_serialize_u64() {
        assert_eq!(bcs::to_bytes(&Value::U64(12345678901234)).unwrap(), bcs::to_bytes(&12345678901234u64).unwrap());
    }
    ...
    fn test_serialize_u256() {
        ...
        assert_eq!(bcs::to_bytes(&Value::U256(val)).unwrap(), bcs::to_bytes(&val).unwrap());
    }

    #[test]
    fn test_serialize_address() {
        let addr: AccountAddress = "0x1".parse().unwrap();
        assert_eq!(bcs::to_bytes(&Value::Address(addr)).unwrap(), bcs::to_bytes(&addr).unwrap());
    }
    ...
    fn test_serialize_bytes() {
        ...
        assert_eq!(bcs::to_bytes(&Value::Bytes(Cow::Borrowed(&bytes))).unwrap(), bcs::to_bytes(&bytes).unwrap());
    }
    ...
    fn test_serialize_positional_struct() {
        ...
        assert_eq!(bcs::to_bytes(&struct_).unwrap(), bcs::to_bytes(&(42u64, true, "test")).unwrap());
    }
    ...
    fn test_serialize_named_struct() {
        ...
        let struct_ = Value::Struct(Struct {
            type_,
            fields: Fields::Named(vec![("x", Value::U32(100)), ("y", Value::U32(200)), ("z", Value::Address(addr))]),
        });

        assert_eq!(bcs::to_bytes(&struct_).unwrap(), bcs::to_bytes(&(100u32, 200u32, addr)).unwrap());
    }
    ...
    fn test_serialize_empty_struct() {
        ...
        let positional = Value::Struct(Struct { type_, fields: Fields::Positional(vec![]) });

        let named = Value::Struct(Struct { type_, fields: Fields::Named(vec![]) });

        assert_eq!(bcs::to_bytes(&positional).unwrap(), bcs::to_bytes(&false).unwrap());

        assert_eq!(bcs::to_bytes(&named).unwrap(), bcs::to_bytes(&false).unwrap());
    }
    ...
    fn test_serialize_enum() {
        ...
        assert_eq!(bcs::to_bytes(&enum_).unwrap(), bcs::to_bytes(&E::A(42, true)).unwrap());
        ...
        assert_eq!(bcs::to_bytes(&enum_).unwrap(), bcs::to_bytes(&E::B { x: 100, y: 200 }).unwrap());
        ...
        assert_eq!(bcs::to_bytes(&enum_).unwrap(), bcs::to_bytes(&E::C).unwrap());
    }
    ...
    fn test_serialize_vector() {
        ...
        assert_eq!(bcs::to_bytes(&vec).unwrap(), bcs::to_bytes(&vec![10u64, 20, 30]).unwrap());
        ...
        assert_eq!(bcs::to_bytes(&vec).unwrap(), bcs::to_bytes(&vec!["hello", "world"]).unwrap());

        // Test empty vector
        let vec = Value::Vector(Vector { type_: Cow::Owned(TypeTag::U64), elements: vec![] });
    ...
    fn test_literal_to_atom_conversion() {
        let values = vec![
            ...
            Value::Vector(Vector {
                ...
                elements: vec![Value::U8(4), Value::U8(5), Value::Slice(Slice { layout: &L::U8, bytes: &[6] })],
            }),
        ];
    ...
    fn test_slice_to_atom_converion() {
        ...
        let values = vec![
            Value::Slice(Slice { layout: &L::Bool, bytes: &bool_bytes }),
            Value::Slice(Slice { layout: &L::U8, bytes: &u8_bytes }),
            Value::Slice(Slice { layout: &L::U16, bytes: &u16_bytes }),
            Value::Slice(Slice { layout: &L::U32, bytes: &u32_bytes }),
            Value::Slice(Slice { layout: &L::U64, bytes: &u64_bytes }),
            Value::Slice(Slice { layout: &L::U128, bytes: &u128_bytes }),
            Value::Slice(Slice { layout: &L::U256, bytes: &u256_bytes }),
            Value::Slice(Slice { layout: &L::Address, bytes: &addr_bytes }),
            Value::Slice(Slice { layout: &str_layout, bytes: &str_bytes }),
            Value::Slice(Slice { layout: &vec_layout, bytes: &vec_bytes }),
        ];
