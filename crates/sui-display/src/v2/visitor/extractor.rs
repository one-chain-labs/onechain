...
use move_core_types::{account_address::AccountAddress, annotated_value as A, annotated_visitor as AV, u256::U256};

use crate::v2::{
    error::FormatError,
    value::{Accessor, Slice, Value},
    visitor::{
        address::AddressExtractor,
        option::{is_option, OptionExtractor},
        vec_map::{is_vec_map, VecMapVisitor},
    },
};
...
impl<'v> AV::Visitor<'v, 'v> for Extractor<'v, '_> {
    ...
    fn visit_u8(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: u8) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U8(n)))
    }

    fn visit_u16(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: u16) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U16(n)))
    }

    fn visit_u32(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: u32) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U32(n)))
    }

    fn visit_u64(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: u64) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U64(n)))
    }

    fn visit_u128(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: u128) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U128(n)))
    }

    fn visit_u256(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, n: U256) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::U256(n)))
    }

    fn visit_bool(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, b: bool) -> Result<Self::Value, Self::Error> {
        Ok(self.path.is_empty().then_some(Value::Bool(b)))
    }

    fn visit_address(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, a: AccountAddress) -> Result<Self::Value, Self::Error> {
    ...
    fn visit_signer(&mut self, _: &AV::ValueDriver<'_, 'v, 'v>, _: AccountAddress) -> Result<Self::Value, Self::Error> {
        Ok(None)
    }

    fn visit_vector(&mut self, driver: &mut AV::VecDriver<'_, 'v, 'v>) -> Result<Self::Value, Self::Error> {
    ...
    fn visit_struct(&mut self, driver: &mut AV::StructDriver<'_, 'v, 'v>) -> Result<Self::Value, Self::Error> {
        let Some(accessor) = self.path.last() else {
            ...
            if is_option(driver.struct_layout()) {
                return Ok(driver.next_field(&mut OptionExtractor)?.and_then(|(_, v)| v).map(Value::Slice));
            } else {
        ...
        if matches!(accessor, Accessor::DFIndex(_) | Accessor::DOFIndex(_)) {
            return AddressExtractor.visit_struct(driver).map(|a| a.map(Value::Address));
        }
    ...
    fn visit_variant(&mut self, driver: &mut AV::VariantDriver<'_, 'v, 'v>) -> Result<Self::Value, Self::Error> {
        let Some(accessor) = self.path.pop() else {
            while driver.skip_field()?.is_some() {}
            return Ok(Some(Value::Slice(Slice {
                layout: driver.layout()?,
