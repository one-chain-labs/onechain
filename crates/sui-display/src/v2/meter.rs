...
impl Limits {
    pub(crate) fn budget(&self) -> Budget {
        Budget { nodes: self.max_nodes, loads: self.max_loads }
    }
}

impl<'b> Meter<'b> {
    pub fn new(max_depth: usize, budget: &'b mut Budget) -> Self {
        Meter { depth_budget: max_depth, budget }
    }
    ...
    pub fn nest(&mut self) -> Result<Meter<'_>, FormatError> {
        ...
        Ok(Meter { depth_budget: self.depth_budget - 1, budget: self.budget })
    }
...
impl Default for Limits {
    fn default() -> Self {
        Self { max_depth: 32, max_nodes: 32768, max_loads: 8 }
    }
}
