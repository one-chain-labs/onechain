...
impl<'s> Parser<'s> {
    ...
    fn parse_array_elements(&mut self) -> Result<Vec<Chain<'s>>, Error> {
        ...
        loop {
            elements.push(self.parse_chain().map_err(|e| e.also_tried(offset, terminated.clone()))?);
    ...
    fn parse_data(&mut self) -> Result<Literal<'s>, Error> {
        ...
        if let Match::Tried(offset, enum_) = enum_ {
            return Ok(Literal::Struct(Box::new(Struct {
                type_,
                fields: self.parse_fields().map_err(|e| e.also_tried(offset, enum_))?,
            })));
        }
    ...
    fn parse_fields(&mut self) -> Result<Fields<'s>, Error> {
        ...
        Ok(if is_named {
            ...
            loop {
                fields.push(self.parse_chain().map_err(|e| e.also_tried(offset, terminated.clone()))?);
...
fn read_u8(slice: &str, radix: u32, what: &'static str) -> Result<u8, Error> {
    u8::from_str_radix(&slice.replace('_', ""), radix).map_err(|err| Error::InvalidNumber { what, err: err.to_string() })
}

fn read_u16(slice: &str, radix: u32, what: &'static str) -> Result<u16, Error> {
    u16::from_str_radix(&slice.replace('_', ""), radix)
        .map_err(|err| Error::InvalidNumber { what, err: err.to_string() })
}
...
fn read_hex_literal(lexeme: &Lex<'_>, slice: &str) -> Result<Vec<u8>, Error> {
    ...
    for i in (0 .. slice.len()).step_by(2) {
        let byte =
            u8::from_str_radix(&slice[i .. i + 2], 16).map_err(|_| Error::InvalidHexCharacter(lexeme.detach()))?;
        output.push(byte);
    }
