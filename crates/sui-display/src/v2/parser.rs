...
use crate::v2::{
    error::{Expected, ExpectedSet, FormatError, Match},
    lexer::{Lexeme as Lex, Lexer, Token as T},
    meter::Meter,
    peek::{Peekable2, Peekable2Ext},
};
...
impl<'s> Parser<'s> {
    ...
    pub(crate) fn new(src: &'s str) -> Self {
        Self { lexer: Lexer::new(src).peekable2() }
    }

    /// Entrypoint into the parser, parsing the root non-terminal -- `format`.
    pub(crate) fn run<'b>(src: &'s str, meter: &mut Meter<'b>) -> Result<Vec<Strand<'s>>, FormatError> {
    ...
    fn try_parse_literal<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Match<Literal<'s>>, FormatError> {
    ...
    fn try_parse_accessor<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Match<Accessor<'s>>, FormatError> {
        Ok(match_token_opt! { self.lexer;
            Tok(_, T::Dot, _, _) => {
                self.lexer.next();
                match_token! { self.lexer;
    ...
    fn parse_array_elements<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Vec<Chain<'s>>, FormatError> {
        let mut elements = vec![];

        if match_token_opt! { self.lexer; Tok(_, T::LBracket, _, _) => { self.lexer.next(); } }.is_not_found() {
            return Ok(elements);
        }
        ...
        loop {
            elements.push(self.parse_chain(meter).map_err(|e| e.also_tried(offset, terminated.clone()))?);
    ...
    fn parse_data<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Literal<'s>, FormatError> {
        ...
        if let Match::Tried(offset, enum_) = enum_ {
            ...
            return Ok(Literal::Struct(Box::new(Struct {
                type_,
                fields: self.parse_fields(meter).map_err(|e| e.also_tried(offset, enum_))?,
            })));
        }
    ...
    fn parse_fields<'b>(&mut self, meter: &mut Meter<'b>) -> Result<Fields<'s>, FormatError> {
        ...
        Ok(if is_named {
            ...
            loop {
                fields.push(self.parse_chain(meter).map_err(|e| e.also_tried(offset, terminated.clone()))?);
    ...
    fn parse_datatype(&mut self, meter: &mut Meter<'_>) -> Result<StructTag, FormatError> {
        ...
        meter.alloc()?;
        Ok(StructTag { address, module, name, type_params })
    }

    fn parse_type_params(&mut self, meter: &mut Meter<'_>) -> Result<Vec<TypeTag>, FormatError> {
        let mut type_params = vec![];
        if match_token_opt! { self.lexer; Tok(_, T::LAngle, _, _) => { self.lexer.next(); } }.is_not_found() {
    ...
    fn parse_xmod(&mut self) -> Result<Base64Modifier, FormatError> {
        ...
        if match_token_opt! { self.lexer; Tok(_, T::LParen, _, _) => { self.lexer.next(); } }.is_not_found() {
            return Ok(xmod);
        }
...
fn read_string_literal(slice: &str) -> Cow<'_, str> {
    ...
    while start < slice.len() {
        ...
        let end = slice[start + 1 ..].find('\\').map_or(slice.len(), |i| start + 1 + i);
...
fn read_hex_literal(lexeme: &Lex<'_>, slice: &str) -> Result<Vec<u8>, FormatError> {
    ...
    for i in (0 .. slice.len()).step_by(2) {
        let byte =
            u8::from_str_radix(&slice[i .. i + 2], 16).map_err(|_| FormatError::InvalidHexCharacter(lexeme.detach()))?;
        output.push(byte);
    }
...
mod tests {
    ...
    fn nodes_and_loads(src: &str) -> (usize, usize, Vec<Strand<'_>>) {
        let limits = Limits { max_depth: usize::MAX, max_nodes: usize::MAX, max_loads: usize::MAX };

        let mut budget = limits.budget();
        let mut meter = Meter::new(limits.max_depth, &mut budget);

        let strands = Parser::new(src).parse_format(&mut meter).unwrap();
        (usize::MAX - budget.nodes, usize::MAX - budget.loads, strands)
    }

    fn parse_with_depth(depth: usize, src: &str) -> Result<Vec<Strand<'_>>, FormatError> {
        let limits = Limits { max_depth: depth, max_nodes: usize::MAX, max_loads: usize::MAX };
    ...
    fn test_metering_expression() {
        ...
        assert_eq!(nodes, 3);
        assert_eq!(loads, 0);
        assert_eq!(strands, vec![S::Expr(E {
            alternates: vec![C { root: None, accessors: vec![A::Field(ident_str!("foo"))] }],
            transform: None,
        })]);
    }
    ...
    fn test_metering_text_and_expr() {
        ...
        assert_eq!(strands, vec![
            S::Text("foo ".into()),
            S::Expr(E {
                alternates: vec![C { root: None, accessors: vec![A::Field(ident_str!("bar"))] }],
                transform: None,
            }),
            S::Text(" baz".into()),
        ]);
    ...
    fn test_metering_alternates() {
        ...
        assert_eq!(strands, vec![S::Expr(E {
            alternates: vec![
                C { root: None, accessors: vec![A::Field(ident_str!("foo"))] },
                C { root: None, accessors: vec![A::Field(ident_str!("bar"))] },
                C { root: None, accessors: vec![A::Field(ident_str!("baz"))] },
            ],
            transform: None,
        })]);
    }
        ...
        assert_eq!(strands, vec![S::Expr(E {
            alternates: vec![C {
                ...
                accessors: vec![
                    ...
                    A::Index(C { root: None, accessors: vec![A::Field(ident_str!("bar"))] }),
                    A::DFIndex(C { root: None, accessors: vec![A::Field(ident_str!("baz"))] }),
                ],
            }],
            transform: None,
        })]);
            ...
            alternates: vec![
                ...
                C {
                    ...
                    accessors: vec![
                        ...
                        A::DFIndex(C { root: None, accessors: vec![A::Field(ident_str!("quy"))] }),
                    ],
                },
            ],
            transform: None,
    ...
    fn test_metering_primitive_literals() {
        let (nodes, loads, strands) = nodes_and_loads("{true | @0x1234 | 5678u64 | x'abcdef' | b'hello' | 'world'}");
        assert_eq!(nodes, 13);
        assert_eq!(loads, 0);
        assert_eq!(strands, vec![S::Expr(E {
            alternates: vec![
                C { root: Some(L::Bool(true)), accessors: vec![] },
                C { root: Some(L::Address(AccountAddress::from_hex_literal("0x1234").unwrap())), accessors: vec![] },
                C { root: Some(L::U64(5678)), accessors: vec![] },
                C { root: Some(L::ByteArray(vec![0xab, 0xcd, 0xef])), accessors: vec![] },
                C { root: Some(L::ByteArray(b"hello".to_vec())), accessors: vec![] },
                C { root: Some(L::String("world".into())), accessors: vec![] },
            ],
            transform: None,
        })]);
    }
            ...
            alternates: vec![
                C {
                    root: Some(L::Vector(Box::new(Vector {
                        type_: None,
                        elements: vec![
                            C { root: Some(L::U8(1)), accessors: vec![] },
                            C { root: Some(L::U8(2)), accessors: vec![] },
                            C { root: Some(L::U8(3)), accessors: vec![] },
                        ],
                    }))),
                    accessors: vec![],
                },
                C {
                    root: Some(L::Vector(Box::new(Vector {
                        ...
                        elements: vec![C { root: Some(L::U16(4)), accessors: vec![] }, C {
                            root: Some(L::U16(5)),
                            accessors: vec![],
                        },],
                    }))),
                ...
                C {
                    root: Some(L::Vector(Box::new(Vector { type_: Some(TypeTag::U32), elements: vec![] }))),
                    accessors: vec![],
                },
                C {
                    root: Some(L::Vector(Box::new(Vector { type_: Some(TypeTag::U64), elements: vec![] }))),
                    accessors: vec![],
                },
                C {
                    root: Some(L::Vector(Box::new(Vector {
                        type_: Some(TypeTag::from_str("0x2::coin::Coin<0x2::oct::OCT>").unwrap()),
                        elements: vec![],
                    }))),
                    accessors: vec![],
                },
    ...
    fn test_metering_datatypes() {
        let (nodes, loads, strands) = nodes_and_loads(
            r#"{ 0x1::string::String(42u64, 'foo', vector[1u256, 2u256, 3u256])
               | 0x2::coin::Coin<0x2::oct::OCT>::Foo#1 { balance: 100u64 } }"#,
        );
            ...
            alternates: vec![
                C {
                    root: Some(L::Struct(Box::new(Struct {
                        ...
                        fields: Fields::Positional(vec![
                            C { root: Some(L::U64(42)), accessors: vec![] },
                            C { root: Some(L::String("foo".into())), accessors: vec![] },
                            C {
                                root: Some(L::Vector(Box::new(Vector {
                                    type_: None,
                                    elements: vec![
                                        C { root: Some(L::U256(1u64.into())), accessors: vec![] },
                                        C { root: Some(L::U256(2u64.into())), accessors: vec![] },
                                        C { root: Some(L::U256(3u64.into())), accessors: vec![] },
                                    ],
                                }))),
                                accessors: vec![],
                            },
                ...
                C {
                    root: Some(L::Enum(Box::new(Enum {
                        type_: StructTag::from_str("0x2::coin::Coin<0x2::oct::OCT>").unwrap(),
                        variant_name: Some("Foo"),
                        variant_index: 1,
                        fields: Fields::Named(vec![("balance", C { root: Some(L::U64(100)), accessors: vec![] },)]),
                    }))),
                    accessors: vec![],
                },
            ],
    ...
    fn test_hex_address_overflow() {
        assert_snapshot!(strands(r#"{@0x12345678901234567890123456789012345678901234567890123456789012345}"#));
    }
    ...
    fn test_struct_literal_positional_trailing() {
        assert_snapshot!(strands(r#"{0x1::string::String(42u64, 'foo', vector[1u64, 2u64, 3u64]"#));
    }
    ...
    fn test_enum_literal_positional_trailing() {
        assert_snapshot!(strands(r#"{0x1::option::Option<u64>::Some#1(42u64, 43u64, 44u64}"#));
    }

    #[test]
    fn test_enum_literal_named_trailing() {
        assert_snapshot!(strands(r#"{0x1::option::Option<u64>::Some#1 { value: 42u16, other: 43u32,"#,));
    }
