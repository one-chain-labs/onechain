...
impl<'s, S: Store<'s>> Interpreter<'s, S> {
    pub(crate) fn new(root: Slice<'s>, store: S, max_output_size: usize) -> Self {
        Self { root, store, max_output_size, used_output: AtomicUsize::new(0) }
    }

    /// Entrypoint to evaluate a single format string, represented as a sequence of its strands.
    pub(crate) async fn eval(&self, strands: &'s [P::Strand<'s>]) -> Result<serde_json::Value, FormatError> {
        ...
        for strand in strands {
            match strand {
                P::Strand::Text(s) => writer.write_str(s).map_err(|_| FormatError::TooMuchOutput)?,

                P::Strand::Expr(P::Expr { alternates, transform }) => {
    ...
    async fn eval_chain(&self, chain: &'s P::Chain<'s>) -> Result<Option<Value<'s>>, FormatError> {
        ...
        let root: OptionFuture<_> = chain.root.as_ref().map(|literal| self.eval_literal(literal)).into();
        ...
        let Some(mut accessors) = accessors.into_iter().collect::<Result<Option<Vec<_>>, _>>()? else {
            return Ok(None);
        };
        ...
        while let Some(accessor) = accessors.last() {
            match (root, accessor) {
                (V::Address(a), A::DFIndex(i)) => {
                    ...
                    let Some(field) =
                        self.store.object(df_id.into()).await.map_err(|e| FormatError::Store(Arc::new(e)))?
                    else {
                        return Ok(None);
                    };
                    ...
                    accessors.pop();
                    root = V::Slice(Slice { bytes: field.value_bytes, layout: field.value_layout });
                }
                ...
                (V::Address(a), A::DOFIndex(i)) => {
                    ...
                    let Some(slice) = self.store.object(id).await.map_err(|e| FormatError::Store(Arc::new(e)))? else {
                        return Ok(None);
                    };
                ...
                // numeric index.
                (V::Bytes(bs), accessor) => {
                    let Some(&b) = accessor.as_numeric_index().and_then(|i| bs.get(i as usize)) else {
                        return Ok(None);
                    };
    ...
    async fn eval_accessor(&self, acc: &'s P::Accessor<'s>) -> Result<Option<Accessor<'s>>, FormatError> {
        use Accessor as VA;
    ...
    async fn eval_literal(&self, lit: &'s P::Literal<'s>) -> Result<Option<Value<'s>>, FormatError> {
        use Value as V;
        use P::Literal as L;
        ...
        Ok(match lit {
            ...
            L::Vector(v) => self.eval_chains(&v.elements).await.and_then(|elements| {
                ...
                for e in &elements {
                    ...
                    if element_type != *type_ {
                        return Err(FormatError::VectorTypeMismatch(type_.into_owned(), element_type));
                    }
                }
            ...
            L::Struct(s) => {
                self.eval_fields(&s.fields).await?.map(|fields| V::Struct(Struct { type_: &s.type_, fields }))
            }

            L::Enum(e) => self.eval_fields(&e.fields).await?.map(|fields| {
                V::Enum(Enum { type_: &e.type_, variant_name: e.variant_name, variant_index: e.variant_index, fields })
            }),
        })
    }
    ...
    async fn eval_fields(&self, field: &'s P::Fields<'s>) -> Result<Option<Fields<'s>>, FormatError> {
        Ok(match field {
    ...
    async fn eval_chains(
        ...
        let values = chains.into_iter().map(|chain| Box::pin(self.eval_chain(chain)));
