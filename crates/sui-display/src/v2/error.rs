...
use crate::v2::{
    lexer::{Lexeme, OwnedLexeme, Token},
    peek::Peekable2Ext,
};
...
pub enum FormatError {
    ...
    UnexpectedToken { actual: OwnedLexeme, expect: ExpectedSet },
...
impl FormatError {
    ...
    pub(crate) fn also_tried(self, offset: Option<usize>, tried: ExpectedSet) -> Self {
        match (offset, self) {
            (Some(offset), FormatError::UnexpectedToken { actual, expect }) if offset == actual.2 => {
                FormatError::UnexpectedToken { actual, expect: expect.union(tried) }
            }

            (None, FormatError::UnexpectedEos { expect }) => FormatError::UnexpectedEos { expect: expect.union(tried) },
...
impl ExpectedSet {
    ...
    pub(crate) fn into_error(self, actual: Option<&Lexeme<'_>>) -> FormatError {
        if let Some(actual) = actual {
            FormatError::UnexpectedToken { actual: actual.detach(), expect: self }
        } else {
...
impl fmt::Display for ExpectedSet {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let mut prefix = if tokens.peek2().is_some() { "one of " } else { "" };

        while let Some(token) = tokens.next() {
            write!(f, "{prefix}{token}")?;
            prefix = if tokens.peek2().is_some() { ", " } else { ", or " };
        }
