    ...
    pub(crate) fn also_tried(self, offset: Option<usize>, tried: ExpectedSet) -> Self {
        match (offset, self) {
            (Some(offset), Error::UnexpectedToken { actual, expect }) if offset == actual.2 => {
                Error::UnexpectedToken { actual, expect: expect.union(tried) }
            }

            (None, Error::UnexpectedEos { expect }) => Error::UnexpectedEos { expect: expect.union(tried) },
...
impl ExpectedSet {
    ...
    pub(crate) fn into_error(self, actual: Option<&Lexeme<'_>>) -> Error {
        if let Some(actual) = actual {
            Error::UnexpectedToken { actual: actual.detach(), expect: self }
        } else {
