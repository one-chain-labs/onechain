...
/// message.
#[derive(Debug, Clone)]
pub struct OwnedLexeme(pub(crate) bool, pub(crate) Token, pub(crate) usize, pub(crate) String);
...
impl<'s> Lexer<'s> {
    ...
    fn next_expr_token(&mut self) -> Option<Lexeme<'s>> {
        ...
        Some(match bytes.first()? {
            ...
            b'0' if bytes.get(1) == Some(&b'x') && bytes.get(2).is_some_and(|b| is_valid_hex_byte(*b)) => {
                self.advance(2);
            ...
            b'a' ..= b'z' | b'A' ..= b'Z' | b'_' => self.take_until(ws, T::Ident, |c| !is_valid_identifier_byte(c)),
            ...
            _ => {
                let next_boundary =
                    (1 ..= self.src.len()).find(|&i| self.src.is_char_boundary(i)).unwrap_or(self.src.len());
    ...
    fn take_whitespace(&mut self) -> bool {
        let Lexeme(_, _, _, slice) = self.take_until(false, Token::Unexpected, |b| !b.is_ascii_whitespace());
        !slice.is_empty()
    }
