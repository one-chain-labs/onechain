...
impl<'s> Format<'s> {
    ...
    pub fn parse(limits: Limits, display_fields: &'s VecMap<String, String>) -> Result<Self, Error> {
...
mod tests {
    ...
    use move_core_types::{account_address::AccountAddress, annotated_value::MoveTypeLayout as T, u256::U256};
    use serde::Serialize;
    use sui_types::{
    ...
    use crate::v2::{
        error::FormatError,
        value::tests::{enum_, optional_, struct_, vector_, MockStore},
    };
    ...
    async fn format<'b, 'l>(
        ...
        let display = VecMap {
            contents: fields
                .into_iter()
                .map(|(key, value)| Entry { key: key.to_owned(), value: value.to_owned() })
                .collect(),
        };

        Format::parse(limits, &display)?.display(max_output_size, bytes, layout, store).await
    }
    ...
    async fn test_fields_and_scalars() {
        ...
        let formats = [
            ...
            ("lit_addr", "{@0x5455}"),
            ("lit_bool", "{false}"),
            ("lit_nums", "{56u8}, {57u16}, {58u32}, {59u64}, {60u128}, {61u256}"),
            ("lit_str", "{'goodbye'}"),
        ];

        let output =
            format(&MockStore::default(), Limits::default(), &bytes, &struct_("0x1::m::S", fields), ONE_MB, formats)
                .await
                .unwrap();
    ...
    async fn test_vector_access() {
        let bytes = bcs::to_bytes(&(vec![2u64, 1u64, 0u64], vec!["first", "second", "third"])).unwrap();

        let fields = vec![("ns", vector_(T::U64)), ("ss", vector_(T::Struct(Box::new(move_ascii_str_layout()))))];
    ...
    async fn test_enums() {
        ...
        let layout = enum_("0x1::m::Status", vec![
            ("Pending", vec![("message", T::Struct(Box::new(move_ascii_str_layout())))]),
        ...
        outputs
            .push(format(&MockStore::default(), Limits::default(), &pending, &layout, ONE_MB, formats).await.unwrap());

        let active = bcs::to_bytes(&Status::Active(42)).unwrap();
        outputs.push(format(&MockStore::default(), Limits::default(), &active, &layout, ONE_MB, formats).await.unwrap());

        let complete = bcs::to_bytes(&Status::Done(100, 999)).unwrap();
        outputs
            .push(format(&MockStore::default(), Limits::default(), &complete, &layout, ONE_MB, formats).await.unwrap());
    ...
    async fn test_nested_access() {
        let bytes = bcs::to_bytes(&((42u64, "nested"), vec![(1u32, "first"), (2u32, "second")], vec![Some((
            100u64, 200u64, 300u64,
        ))]))
        .unwrap();

        let inner =
            struct_("0x1::m::Inner", vec![("value", T::U64), ("label", T::Struct(Box::new(move_ascii_str_layout())))]);

        let item = struct_("0x1::m::Item", vec![("id", T::U32), ("name", T::Struct(Box::new(move_ascii_str_layout())))]);

        let tuple = struct_("0x1::m::Tuple", vec![("pos0", T::U64), ("pos1", T::U64), ("pos2", T::U64)]);

        let option = enum_("0x1::option::Option", vec![("None", vec![]), ("Some", vec![("pos0", tuple)])]);

        let fields = vec![("inner", inner), ("is", vector_(item)), ("ts", vector_(option))];
    ...
    async fn test_string_bytes() {
        ...
        let output = format(&MockStore::default(), Limits::default(), &bytes, &layout, ONE_MB, formats).await.unwrap();
    ...
    async fn test_alternate_optional() {
        ...
        let layout = struct_("0x1::m::S", vec![("a", optional_(T::U64)), ("b", optional_(T::U64))]);
    ...
    async fn test_dynamic_fields() {
        ...
        let layout = struct_("0x1::m::Root", vec![(
            "parent",
            struct_("0x1::m::Parent", vec![("id", T::Struct(Box::new(UID::layout())))]),
        )]);
        ...
        let output = format(&store, Limits::default(), &bytes, &layout, ONE_MB, formats).await.unwrap();
    ...
    async fn test_dynamic_object_fields() {
        ...
        let store = MockStore::default().with_dynamic_object_field(
            ...
            T::Struct(Box::new(move_utf8_str_layout())),
            (child, 100u64, 200u64),
            struct_("0x1::m::Child", vec![("id", T::Struct(Box::new(UID::layout()))), ("x", T::U64), ("y", T::U64)]),
        );
        ...
        let output = format(&store, limits, &bytes, &layout, ONE_MB, formats).await.unwrap();
    ...
    async fn test_nested_dynamic_fields() {
        ...
        let store = MockStore::default()
            .with_dynamic_object_field(
                ...
                T::Struct(Box::new(move_utf8_str_layout())),
                (child, 100u64),
                struct_("0x1::m::Child", vec![("id", T::Struct(Box::new(UID::layout()))), ("data", T::U64)]),
            )
            .with_dynamic_field(
                child,
                "L2",
        ...
        let limits = Limits { max_loads: 20, ..Limits::default() };
    ...
    async fn test_vec_map() {
        let key = struct_("0x42::m::Key", vec![("id", T::U64), ("name", T::Struct(Box::new(move_ascii_str_layout())))]);

        let val = struct_("0x42::m::Value", vec![("data", T::U32)]);

        let map = struct_("0x2::vec_map::VecMap<0x42::m::Key, 0x42::m::Value>", vec![(
            "contents",
            vector_(struct_("0x2::vec_map::Entry<0x42::m::Key, 0x42::m::Value>", vec![("key", key), ("value", val)])),
        )]);

        // Create test data: VecMap with 3 entries
        let bytes = bcs::to_bytes(&VecMap {
            contents: vec![
                Entry { key: (1u64, "first"), value: 100u32 },
                Entry { key: (2u64, "second"), value: 200u32 },
                Entry { key: (3u64, "third"), value: 300u32 },
            ],
        })
        .unwrap();
    ...
    async fn test_hex() {
        let bytes = bcs::to_bytes(&(
            ...
            U256::from_str_radix("42434445464748494a4b4c4d4e4f505152535455565758595a5b5c5d5e5f6061", 16).unwrap(),
            AccountAddress::from_str("0x41403f3e3d3c3b3a393837363534333231300f0e0d0c0b0a0908070605040302").unwrap(),
            vec![0x41u8, 0x40, 0x3a],
            "ABC",
        ))
        .unwrap();

        let layout = struct_("0x1::m::S", vec![
            ("n8", T::U8),
            ("n16", T::U16),
            ("n32", T::U32),
            ("n64", T::U64),
    ...
    async fn test_url() {
        let bytes =
            bcs::to_bytes(&(1234u32, "hello/goodbye world", "🔥", vec![0x3eu8, 0x3f, 0x40, 0x41, 0x42, 0x43])).unwrap();
        ...
        let formats = [("url", "https://example.com/?num={num:url}&str={str:url}&emoji={emoji:url}&data={bytes:url}")];
    ...
    async fn test_bcs() {
        let bytes =
            bcs::to_bytes(&(0x42u8, 0x1234u16, 0x12345678u32, 0x123456789abcdef0u64, "hello", vec![1u8, 2, 3])).unwrap();
    ...
    async fn test_bcs_modifiers() {
        ...
        let formats = [
            ...
            ("flatland", "{43920588204278303214855528440570972873796977361529388163322669436471087583698u256:bcs(url)}"),
            (
                "flatland_nopad",
    ...
    async fn test_string_hardening() {
        ...
        let formats = [("ascii", "{ascii}"), ("utf8", "{utf8}"), ("invalid", "{invalid}")];
    ...
    async fn test_field_errors() {
        ...
        let formats =
            [("parsing_error", "{42"), ("bad_transform", "{byte:invalid}"), ("too_deep", "{a[b[c[d[e[f]]]]]}")];

        let limits = Limits { max_depth: 5, ..Limits::default() };

        let output = format(&MockStore::default(), limits, &bytes, &layout, ONE_MB, formats).await.unwrap();
    ...
    async fn test_output_node_limits() {
        ...
        let res = format(&MockStore::default(), limits.clone(), &bytes, &T::U64, ONE_MB, big_field).await;
        assert!(matches!(res, Err(Error::TooBig)));

        let res = format(&MockStore::default(), limits, &bytes, &T::U64, ONE_MB, two_fields).await;
        assert!(matches!(res, Err(Error::TooBig)));
    }
    ...
    async fn test_output_size_limits() {
        ...
        let res = format(&MockStore::default(), Limits::default(), &bytes, &T::U64, 10, formats).await;
    ...
    async fn test_too_many_loads() {
        ...
        let limits = Limits { max_loads: 3, ..Limits::default() };
    ...
    async fn test_name_empty() {
        ...
        let res = format(&MockStore::default(), Limits::default(), &bytes, &T::U64, ONE_MB, formats).await;
    ...
    async fn test_duplicate_name() {
        ...
        let res = format(&MockStore::default(), Limits::default(), &bytes, &layout, ONE_MB, formats).await;
