...
impl<'s> Format<'s> {
    ...
    /// considered an error.
    pub fn parse(max_depth: usize, display_fields: &'s VecMap<String, String>) -> anyhow::Result<Self> {
        ...
        for Entry { key, value } in &display_fields.contents {
            ...
            let strands =
                parser.parse_format().with_context(|| format!("Failed to parse format for display field {name:?}"))?;
    ...
    for strand in strands {
        let res = match strand {
            ...
            Strand::Expr(path) => {
                ...
                let extracted: SuiMoveValue = MoveValue::visit_deserialize(bytes, layout, &mut extractor)
                ...
                match extracted {
                    SuiMoveValue::Vector(_) => {
                        return Err(Error::Error(anyhow!("'{strand}' is a vector, and is not supported in Display")));
                    }
