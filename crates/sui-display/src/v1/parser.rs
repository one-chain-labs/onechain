...
pub enum Error {
    ...
    UnexpectedToken { actual: OwnedLexeme, expect: TokenSet<'static> },
}
...
impl<'s> Parser<'s> {
    ...
    pub(crate) fn new(max_depth: usize, src: &'s str) -> Self {
        Self { max_depth, lexer: Lexer::new(src).peekable() }
    }
...
mod tests {
    ...
    fn test_literal_string() {
        assert_eq!(Parser::new(10, "foo bar").parse_format().unwrap(), vec![Strand::Text("foo bar".into())]);
    }

    #[test]
    fn test_field_expr() {
        assert_eq!(Parser::new(10, "{foo}").parse_format().unwrap(), vec![Strand::Expr(vec![field("foo")])]);
    }

    #[test]
    fn test_compound_expr() {
        assert_eq!(Parser::new(10, "{foo.bar.baz}").parse_format().unwrap(), vec![Strand::Expr(vec![
            field("foo"),
            field("bar"),
            field("baz"),
        ])]);
    ...
    fn test_text_with_escape() {
        assert_eq!(Parser::new(10, r#"foo \{bar\} baz"#).parse_format().unwrap(), vec![Strand::Text(
            r#"foo {bar} baz"#.into()
        )],);
    }
    ...
    fn test_escape_chain() {
        assert_eq!(Parser::new(10, r#"\\\\\\\\\"#).parse_format().unwrap(), vec![Strand::Text(r#"\\\\\"#.into())],);
    }

    #[test]
    fn test_back_to_back_exprs() {
        assert_eq!(Parser::new(10, "{foo . bar}{baz.qux}").parse_format().unwrap(), vec![
    ...
    fn test_bad_identifier() {
        assert_eq!(
            Parser::new(10, "{foo.bar.baz!}").parse_format().unwrap_err().to_string(),
    ...
    fn test_unexpected_lcurly() {
        assert_eq!(
            Parser::new(10, "{foo{}}").parse_format().unwrap_err().to_string(),
    ...
    fn test_unexpected_rcurly() {
        assert_eq!(
            Parser::new(10, "foo bar}").parse_format().unwrap_err().to_string(),
    ...
    fn test_too_deep() {
        assert_eq!(
            Parser::new(2, "{foo.bar.baz}").parse_format().unwrap_err().to_string(),
