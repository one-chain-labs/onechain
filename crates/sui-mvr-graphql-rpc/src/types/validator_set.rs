...
impl ValidatorSet {
    ...
    async fn active_validators(
        ...
        let Some((prev, next, _, cs)) = page.paginate_consistent_indices(validators.len(), self.checkpoint_viewed_at)?
        else {
            return Ok(connection);
        };
        ...
        for c in cs {
            ...
            connection.edges.push(Edge::new(c.encode_cursor(), validator));
        }
