...
impl Paginated<Cursor> for StoredEvent {
    ...
    fn filter_ge<ST, GB>(cursor: &Cursor, query: Query<ST, GB>) -> Query<ST, GB> {
        ...
        query.filter(tx.gt(cursor.tx as i64).or(tx.eq(cursor.tx as i64).and(event.ge(cursor.e as i64))))
    }

    fn filter_le<ST, GB>(cursor: &Cursor, query: Query<ST, GB>) -> Query<ST, GB> {
        use events::dsl::{event_sequence_number as event, tx_sequence_number as tx};
        query.filter(tx.lt(cursor.tx as i64).or(tx.eq(cursor.tx as i64).and(event.le(cursor.e as i64))))
    }

    fn order<ST, GB>(asc: bool, query: Query<ST, GB>) -> Query<ST, GB> {
        use events::dsl;
        if asc {
            query.order_by(dsl::tx_sequence_number.asc()).then_order_by(dsl::event_sequence_number.asc())
        } else {
            query.order_by(dsl::tx_sequence_number.desc()).then_order_by(dsl::event_sequence_number.desc())
        }
    }
}
...
impl RawPaginated<Cursor> for StoredEvent {
    fn filter_ge(cursor: &Cursor, query: RawQuery) -> RawQuery {
        filter!(query, format!("ROW(tx_sequence_number, event_sequence_number) >= ({}, {})", cursor.tx, cursor.e))
    }
...
impl Target<Cursor> for EvLookup {
    fn cursor(&self, checkpoint_viewed_at: u64) -> Cursor {
        Cursor::new(EventKey { tx: self.tx as u64, e: self.ev as u64, checkpoint_viewed_at })
    }
}
