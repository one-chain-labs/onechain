...
/// <https://github.com/one-chain-labs/onechain/blob/c3feec3ac3b626bf2fd40c668ba32be9c73e7528/crates/sui-json-rpc/src/governance_api.rs#L280>
pub(crate) fn calculate_apy(stake_subsidy_start_epoch: u64, rates: &[(u64, PoolTokenExchangeRate)]) -> f64 {
    // we start the apy calculation from the epoch when the stake subsidy starts
    let exchange_rates = rates
        .iter()
        .filter_map(|(epoch, rate)| if epoch >= &stake_subsidy_start_epoch { Some(rate) } else { None })
        .collect::<Vec<_>>();
    ...
    if exchange_rates_size >= 2 {
        ...
        let apys = er_e.zip(er_e_1).map(apy_rate).filter(|apy| *apy > 0.0 && *apy < 0.1).take(30).collect::<Vec<_>>();
...
pub(crate) fn apy_rate((rate_e, rate_e_1): (&&PoolTokenExchangeRate, &&PoolTokenExchangeRate)) -> f64 {
...
mod tests {
    ...
    fn test_apys_calculation_filter_outliers() {
        ...
        let file = std::fs::File::open("src/unit_tests_data/validator_exchange_rates.json").unwrap();
        ...
        rates.into_iter().for_each(|(validator, rates)| {
            ...
            validator_exchange_rates.insert(
                address,
                (validator, ValidatorExchangeRates { address, pool_id: ObjectID::random(), active: true, rates }),
            );
        });
