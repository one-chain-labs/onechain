...
async fn health_check(
    ...
    let checkpoint_timestamp = Duration::from_millis(watermark_lock.read().await.checkpoint_timestamp_ms);
...
pub mod tests {
    ...
    async fn prep_schema(db_url: String, service_config: Option<ServiceConfig>) -> ServerBuilder {
        ...
        let watermark = Watermark { checkpoint: 1, checkpoint_timestamp_ms: 1, epoch: 0 };
        let state = AppState::new(
