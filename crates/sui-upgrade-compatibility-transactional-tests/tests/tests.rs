...
fn normalize(pool: &mut normalized::RcPool, modules: &[CompiledModule]) -> Vec<compatibility::Module> {
    modules.iter().map(|m| compatibility::Module::new(pool, m, /* include code */ true)).collect()
}
