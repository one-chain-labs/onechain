...
impl Loader<CheckpointKey> for PgReader {
    ...
    async fn load(&self, keys: &[CheckpointKey]) -> Result<HashMap<CheckpointKey, Self::Value>, Error> {
        ...
        let checkpoints: Vec<StoredCheckpoint> =
            conn.results(c::kv_checkpoints.filter(c::sequence_number.eq_any(seqs))).await?;

        Ok(checkpoints.into_iter().map(|c| (CheckpointKey(c.sequence_number as u64), c)).collect())
    }
}
...
impl Loader<CheckpointKey> for BigtableReader {
    type Error = Error;
    type Value = (CheckpointSummary, CheckpointContents, AuthorityQuorumSignInfo<true>);
    ...
    async fn load(&self, keys: &[CheckpointKey]) -> Result<HashMap<CheckpointKey, Self::Value>, Error> {
        ...
        Ok(self
            ...
            .await?
            .into_iter()
            .map(|c| (CheckpointKey(c.summary.sequence_number), (c.summary, c.contents, c.signatures)))
            .collect())
    }
}
    ...
    async fn load(&self, keys: &[CheckpointKey]) -> Result<HashMap<CheckpointKey, Self::Value>, Error> {
        ...
        for key in keys {
            let request = proto::GetCheckpointRequest::by_sequence_number(key.0)
                .with_read_mask(FieldMask::from_paths(["summary.bcs", "signature", "contents.bcs"]));

            match self.0.clone().get_checkpoint(request).await {
                Ok(response) => {
                    let checkpoint = response.into_inner().checkpoint.context("No checkpoint returned")?;
