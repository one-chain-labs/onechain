...
impl Loader<TransactionKey> for PgReader {
    ...
    async fn load(&self, keys: &[TransactionKey]) -> Result<HashMap<TransactionKey, Self::Value>, Error> {
        ...
        let transactions: Vec<StoredTransaction> =
            conn.results(t::kv_transactions.filter(t::tx_digest.eq_any(digests))).await?;

        let digest_to_stored: HashMap<_, _> =
            transactions.into_iter().map(|stored| (stored.tx_digest.clone(), stored)).collect();
...
impl Loader<TransactionKey> for LedgerGrpcReader {
    ...
    async fn load(&self, keys: &[TransactionKey]) -> Result<HashMap<TransactionKey, Self::Value>, Error> {
        ...
        for tx_result in batch_response.transactions {
            if let Some(proto::get_transaction_result::Result::Transaction(executed)) = tx_result.result {
                let full_tx: sui_types::full_checkpoint_content::ExecutedTransaction =
                    (&executed).try_into().context("Failed to convert ExecutedTransaction from proto")?;
                ...
                results.insert(TransactionKey(transaction.transaction_data.digest()), transaction);
            }
        }
        Ok(results)
    }
