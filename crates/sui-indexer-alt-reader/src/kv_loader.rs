...
impl KvLoader {
    ...
    pub async fn load_one_object(&self, id: ObjectID, version: u64) -> Result<Option<Object>, Arc<Error>> {
        ...
        match self {
            ...
            Self::Pg(loader) => loader
                ...
                .await?
                .and_then(|stored| {
                    stored.serialized_object.map(|serialized_object| -> Result<Object, Arc<Error>> {
                        bcs::from_bytes(serialized_object.as_slice()).map_err(|e| Arc::new(Error::Serde(e.into())))
                    })
                })
                .transpose(),
        }
    ...
    pub async fn load_one_checkpoint(
        &self,
        sequence_number: u64,
    ) -> Result<Option<(CheckpointSummary, CheckpointContents, AuthorityQuorumSignInfo<true>)>, Arc<Error>> {
            ...
            Self::Pg(loader) => loader
                ...
                .map(|stored| {
                    let summary: CheckpointSummary =
                        bcs::from_bytes(&stored.checkpoint_summary).map_err(|e| Error::Serde(e.into()))?;

                    let contents: CheckpointContents =
                        bcs::from_bytes(&stored.checkpoint_contents).map_err(|e| Error::Serde(e.into()))?;

                    let signature: AuthorityQuorumSignInfo<true> =
                        bcs::from_bytes(&stored.validator_signatures).map_err(|e| Error::Serde(e.into()))?;
