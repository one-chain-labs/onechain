...
use sui_kvstore::{TransactionData as KVTransactionData, TransactionEventsData as KVTransactionEventsData};
...
impl KvLoader {
    ...
    pub async fn load_one_object(&self, id: ObjectID, version: u64) -> Result<Option<Object>, Error> {
        ...
        match self {
            ...
            Self::Pg(loader) => loader
                ...
                .await?
                .and_then(|stored| {
                    stored.serialized_object.map(|serialized_object| -> Result<Object, Error> {
                        Ok(bcs::from_bytes(serialized_object.as_slice()).context("Failed to deserialize object")?)
                    })
                })
                .transpose(),
            ...
            Self::Pg(loader) => {
                ...
                for (key, stored) in stored_objects {
                    if let Some(serialized_object) = stored.serialized_object {
                        let object =
                            bcs::from_bytes(serialized_object.as_slice()).context("Failed to deserialize object")?;
    ...
    pub async fn load_one_checkpoint(
        &self,
        sequence_number: u64,
    ) -> Result<Option<(CheckpointSummary, CheckpointContents, AuthorityQuorumSignInfo<true>)>, Error> {
    ...
    pub async fn load_one_transaction(&self, digest: TransactionDigest) -> Result<Option<TransactionContents>, Error> {
        let key = TransactionKey(digest);
        match self {
            Self::Bigtable(loader) => Ok(loader.load_one(key).await?.map(TransactionContents::Bigtable)),
            Self::Pg(loader) => Ok(loader.load_one(key).await?.map(TransactionContents::Pg)),
            Self::LedgerGrpc(loader) => Ok(loader.load_one(key).await?.map(TransactionContents::LedgerGrpc)),
        }
    }
    ...
    pub async fn load_many_transaction_events(
        ...
        let keys = digests.iter().map(|d| TransactionEventsKey(*d)).collect::<Vec<_>>();
        match self {
    ...
    pub async fn load_many_transactions(
        ...
        let keys = digests.iter().map(|d| TransactionKey(*d)).collect::<Vec<_>>();
        match self {
            Self::Bigtable(loader) => Ok(loader
                .load_many(keys)
                .await?
...
impl TransactionContents {
    ...
    pub fn data(&self) -> anyhow::Result<TransactionData> {
        match self {
            Self::Pg(stored) => {
                bcs::from_bytes(&stored.raw_transaction).context("Failed to deserialize transaction data")
            }
            Self::Bigtable(kv) => Ok(kv.transaction.data().transaction_data().clone()),
            Self::LedgerGrpc(txn) => Ok(txn.transaction_data.as_ref().clone()),
            Self::ExecutedTransaction { transaction_data, .. } => Ok(transaction_data.as_ref().clone()),
        }
    }
    ...
    pub fn digest(&self) -> anyhow::Result<TransactionDigest> {
        match self {
            Self::Pg(stored) => {
                TransactionDigest::try_from(stored.tx_digest.clone()).context("Failed to deserialize transaction digest")
            }
    ...
    pub fn signatures(&self) -> anyhow::Result<Vec<GenericSignature>> {
        match self {
            Self::Pg(stored) => bcs::from_bytes(&stored.user_signatures).context("Failed to deserialize signatures"),
    ...
    pub fn effects(&self) -> anyhow::Result<TransactionEffects> {
        match self {
            Self::Pg(stored) => bcs::from_bytes(&stored.raw_effects).context("Failed to deserialize effects"),
    ...
    pub fn events(&self) -> anyhow::Result<Vec<Event>> {
        match self {
            Self::Pg(stored) => bcs::from_bytes(&stored.events).context("Failed to deserialize events"),
    ...
    pub fn balance_changes(&self) -> Option<&[grpc::BalanceChange]> {
        match self {
            Self::ExecutedTransaction { balance_changes, .. } => Some(balance_changes),
            _ => None,
        }
    }
    ...
    pub fn raw_transaction(&self) -> anyhow::Result<Vec<u8>> {
        match self {
            ...
            Self::Bigtable(kv) => {
                bcs::to_bytes(kv.transaction.data().transaction_data()).context("Failed to serialize transaction")
            }
            Self::LedgerGrpc(txn) => {
                bcs::to_bytes(txn.transaction_data.as_ref()).context("Failed to serialize transaction")
            }
    ...
    pub fn raw_effects(&self) -> anyhow::Result<Vec<u8>> {
        match self {
            ...
            Self::LedgerGrpc(txn) => bcs::to_bytes(txn.effects.as_ref()).context("Failed to serialize effects"),
...
impl TransactionEventsContents {
    pub fn events(&self) -> anyhow::Result<Vec<Event>> {
        match self {
            Self::Serialized(stored) => bcs::from_bytes(&stored.events).context("Failed to deserialize events"),
