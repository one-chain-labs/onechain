...
impl BigtableReader {
    ...
    pub(crate) async fn checkpoints(&self, keys: &[CheckpointSequenceNumber]) -> Result<Vec<Checkpoint>, Error> {
        measure("checkpoints", &keys, self.0.clone().get_checkpoints(keys)).await
    }

    /// Multi-get transactions by transaction digest.
    pub(crate) async fn transactions(&self, keys: &[TransactionDigest]) -> Result<Vec<TransactionData>, Error> {
