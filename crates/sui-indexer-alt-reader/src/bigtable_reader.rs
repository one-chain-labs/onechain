...
use sui_kvstore::{BigTableClient, Checkpoint, KeyValueStoreReader, TransactionData, TransactionEventsData};
use sui_types::{
...
impl BigtableArgs {
    pub fn statement_timeout(&self) -> Option<Duration> {
        self.bigtable_statement_timeout_ms.map(Duration::from_millis)
    }
}
...
impl BigtableReader {
    ...
    pub async fn checkpoint_watermark(&self) -> anyhow::Result<Option<CheckpointSummary>> {
        measure("watermark", &(), self.0.clone().get_latest_checkpoint_summary()).await
    }

    /// Multi-get checkpoints by sequence number.
    pub(crate) async fn checkpoints(&self, keys: &[CheckpointSequenceNumber]) -> anyhow::Result<Vec<Checkpoint>> {
        measure("checkpoints", &keys, self.0.clone().get_checkpoints(keys)).await
    }

    /// Multi-get transactions by transaction digest.
    pub(crate) async fn transactions(&self, keys: &[TransactionDigest]) -> anyhow::Result<Vec<TransactionData>> {
    ...
    pub(crate) async fn transactions_events(
        ...
        measure("events", &keys, self.0.clone().get_events_for_transactions(keys)).await
    }
}
