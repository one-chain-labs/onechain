...
impl Loader<EpochStartKey> for PgReader {
    ...
    async fn load(&self, keys: &[EpochStartKey]) -> Result<HashMap<EpochStartKey, Self::Value>, Self::Error> {
        ...
        let epochs: Vec<StoredEpochStart> =
            conn.results(s::kv_epoch_starts.filter(s::epoch.eq_any(ids))).await.map_err(Arc::new)?;

        Ok(epochs.into_iter().map(|e| (EpochStartKey(e.epoch as u64), e)).collect())
    }
}
...
impl Loader<CheckpointBoundedEpochStartKey> for PgReader {
    ...
    async fn load(
        ...
        let cp_to_stored: BTreeMap<_, _> = stored_epochs.into_iter().map(|epoch| (epoch.cp_lo as u64, epoch)).collect();
...
impl Loader<EpochEndKey> for PgReader {
    ...
    async fn load(&self, keys: &[EpochEndKey]) -> Result<HashMap<EpochEndKey, Self::Value>, Self::Error> {
        ...
        let ids: Vec<_> = keys.iter().map(|e| e.0 as i64).collect();
        let epochs: Vec<StoredEpochEnd> =
            conn.results(e::kv_epoch_ends.filter(e::epoch.eq_any(ids))).await.map_err(Arc::new)?;

        Ok(epochs.into_iter().map(|e| (EpochEndKey(e.epoch as u64), e)).collect())
    }
}
