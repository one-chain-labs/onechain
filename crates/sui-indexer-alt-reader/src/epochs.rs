...
impl Loader<EpochStartKey> for PgReader {
    ...
    async fn load(&self, keys: &[EpochStartKey]) -> Result<HashMap<EpochStartKey, Self::Value>, Error> {
        ...
        let epochs: Vec<StoredEpochStart> = conn.results(s::kv_epoch_starts.filter(s::epoch.eq_any(ids))).await?;
...
impl Loader<EpochEndKey> for PgReader {
    ...
    async fn load(&self, keys: &[EpochEndKey]) -> Result<HashMap<EpochEndKey, Self::Value>, Error> {
        ...
        let ids: Vec<_> = keys.iter().map(|e| e.0 as i64).collect();
        let epochs: Vec<StoredEpochEnd> = conn.results(e::kv_epoch_ends.filter(e::epoch.eq_any(ids))).await?;
