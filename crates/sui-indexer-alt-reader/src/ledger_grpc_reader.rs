...
impl LedgerGrpcArgs {
    pub fn statement_timeout(&self) -> Option<std::time::Duration> {
        self.ledger_grpc_statement_timeout_ms.map(Duration::from_millis)
    }
}
...
impl LedgerGrpcReader {
    ...
    pub async fn checkpoint_watermark(&self) -> anyhow::Result<CheckpointSummary> {
        ...
        let response = self.0.clone().get_checkpoint(request).await.context("Failed to get latest checkpoint")?;
