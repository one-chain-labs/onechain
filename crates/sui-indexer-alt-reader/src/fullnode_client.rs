...
impl FullnodeClient {
    pub async fn new(
        ...
        let execution_client = if let Some(url) = &args.fullnode_rpc_url {
            let channel =
                Channel::from_shared(url.clone()).context("Failed to create channel for gRPC endpoint")?.connect_lazy();
        ...
        Ok(Self { execution_client, metrics, cancel })
    }
    ...
    pub async fn simulate_transaction(
        ...
        self.request("simulate_transaction", self.execution_client.clone(), |mut client| async move {
