...
impl FullnodeClient {
    pub async fn new(
        ...
        Ok(Self { execution_client, live_data_client, metrics, cancel })
    }
    ...
    pub async fn execute_transaction(
        ...
        self.request("execute_transaction", self.execution_client.clone(), |mut client| async move {
    ...
    pub async fn simulate_transaction(
        ...
        tx_proto.bcs = Some(
            proto::Bcs::serialize(&transaction_data)
                .map_err(|e| Error::Internal(anyhow!("Failed to serialize transaction data: {e}")))?,
        );
        ...
        self.request("simulate_transaction", self.live_data_client.clone(), |mut client| async move {
            client.simulate_transaction(request).await
        })
        .await
    }

    async fn request<C, F, Fut, R>(&self, method: &str, client: Option<C>, response: F) -> Result<R, Error>
    where
        F: FnOnce(C) -> Fut,
