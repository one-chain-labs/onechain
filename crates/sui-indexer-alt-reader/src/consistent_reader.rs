...
impl ConsistentReaderArgs {
    pub fn statement_timeout(&self) -> Option<Duration> {
        self.consistent_store_statement_timeout_ms.map(Duration::from_millis)
    }
}
...
impl ConsistentReader {
    pub async fn new(
        ...
        let client = if let Some(url) = &args.consistent_store_url {
            let mut endpoint =
                Channel::from_shared(url.to_string()).context("Failed to create channel for gRPC endpoint")?;

            if let Some(timeout) = args.statement_timeout() {
                endpoint = endpoint.timeout(timeout);
            }

            let channel = endpoint.connect().await.context("Failed to connect to gRPC endpoint")?;
        ...
        Ok(Self { client, timeout, metrics, cancel })
    }
    ...
    pub async fn get_balance(
        ...
        let response = self
            .request(
                ...
                proto::GetBalanceRequest { owner: Some(address), coin_type: Some(coin_type) },
            )
            .await?;
    ...
    pub async fn list_balances(
        ...
        let response = self
            .request(
                ...
                proto::ListBalancesRequest {
                    ...
                    end: if is_from_front { Some(End::Front.into()) } else { Some(End::Back.into()) },
                },
            )
            .await?;
        ...
        let results = response.balances.into_iter().map(TryFrom::try_from).collect::<Result<Vec<_>, _>>()?;

        Ok(Page { results, has_next_page, has_previous_page })
    }
    ...
    pub async fn list_objects_by_type(
        ...
        let results = response.objects.into_iter().map(TryFrom::try_from).collect::<Result<Vec<_>, _>>()?;
    ...
    pub async fn list_owned_objects(
        ...
        let response = self
            .request(
                ...
                ListOwnedObjectsRequest {
                    owner: Some(Owner { kind: Some(kind.into()), address }),
                    object_type,
                    page_size,
                    after_token: after_token.map(Into::into),
                    before_token: before_token.map(Into::into),
        ...
        Fut: Future<Output = Result<tonic::Response<O>, tonic::Status>>,
    {
        ...
        self.metrics.requests_received.with_label_values(&[method]).inc();

        let _timer = self.metrics.latency.with_label_values(&[method]).start_timer();
        ...
        if let Some(checkpoint) = checkpoint {
            request.metadata_mut().insert(
                CHECKPOINT_METADATA,
                checkpoint.to_string().parse().with_context(|| format!("Invalid checkpoint {checkpoint}"))?,
            );
        }
        ...
        if response.is_ok() {
            self.metrics.requests_succeeded.with_label_values(&[method]).inc();
        } else {
            self.metrics.requests_failed.with_label_values(&[method]).inc()
        }
...
impl TryFrom<Balance> for Edge<(TypeTag, u64)> {
    ...
    fn try_from(proto: Balance) -> Result<Self, Error> {
        let coin_type: TypeTag = proto.coin_type.context("coin type missing")?.parse().context("invalid coin type")?;

        let balance: u64 = proto.balance.unwrap_or(0);

        let token: Vec<u8> = proto.page_token.unwrap_or_default().into();

        Ok(Edge { token, value: (coin_type, balance) })
    }
}
...
impl TryFrom<Object> for Edge<ObjectRef> {
    ...
    fn try_from(proto: Object) -> Result<Self, Error> {
        let object_id: ObjectID = proto.object_id.context("object ID missing")?.parse().context("invalid object ID")?;

        let digest: ObjectDigest = proto.digest.context("digest missing")?.parse().context("invalid digest")?;

        let version: SequenceNumber = proto.version.context("version missing")?.into();
        let token: Vec<u8> = proto.page_token.unwrap_or_default().into();

        Ok(Edge { token, value: (object_id, version, digest) })
    }
}
