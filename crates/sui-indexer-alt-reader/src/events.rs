...
impl Loader<TransactionEventsKey> for PgReader {
    ...
    async fn load(&self, keys: &[TransactionEventsKey]) -> Result<HashMap<TransactionEventsKey, Self::Value>, Error> {
        ...
        let digest_to_stored: HashMap<_, _> =
            transactions.into_iter().map(|(tx_digest, stored)| (tx_digest.clone(), stored)).collect();
    ...
    async fn load(
        ...
        for key in keys {
            ...
            match self.0.clone().get_transaction(request).await {
                Ok(response) => {
                    let executed = response.into_inner().transaction.context("No transaction returned")?;
                    ...
                    let events = executed
                        ...
                        .map(|bcs| -> anyhow::Result<_> {
                            let tx_events: TransactionEvents =
                                bcs.deserialize().context("Failed to deserialize transaction events")?;
                            Ok(tx_events.data)
                        })
                        .transpose()?
                        .unwrap_or_default();
                    ...
                    results.insert(*key, TransactionEventsData { events, timestamp_ms });
                }
