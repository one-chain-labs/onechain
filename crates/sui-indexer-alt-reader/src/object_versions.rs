...
impl Loader<LatestObjectVersionKey> for PgReader {
    ...
    async fn load(
        ...
        let id_to_stored: HashMap<_, _> =
            obj_versions.into_iter().map(|stored| (stored.object_id.clone(), stored)).collect();
...
impl Loader<VersionBoundedObjectVersionKey> for PgReader {
    ...
    async fn load(
        ...
        for obj_version in obj_versions {
            let id = ObjectID::from_bytes(&obj_version.object_id).context("Failed to deserialize ObjectID")?;
...
impl Loader<CheckpointBoundedObjectVersionKey> for PgReader {
    ...
    async fn load(
        ...
        for obj_version in obj_versions {
            ...
            key_to_stored.insert(CheckpointBoundedObjectVersionKey(id, cp_sequence_number), obj_version);
        }
...
mod tests {
    ...
    async fn setup() -> (TempDb, Db, PgReader) {
        ...
        let reader = PgReader::new(None, Some(url.clone()), DbArgs::default(), &registry, CancellationToken::new())
            .await
            .unwrap();
    ...
    async fn test_version_bounded() {
...

        {
            ...
            diesel::insert_into(v::obj_versions)
                .values(vec![stored(o0, d0, 1, 1), stored(o1, d1, 2, 1), stored(o0, d0, 2, 2), stored(o0, d0, 4, 2)])
                .execute(&mut conn)
                .await
                .unwrap();
        }
        ...
        assert_eq!(Loader::load(&reader, &[K(o1, 1)]).await.unwrap(), HashMap::new(),);

        // No matching object.
        assert_eq!(Loader::load(&reader, &[K(o2, 1)]).await.unwrap(), HashMap::new(),);

        // Multiple requests that map to the same record.
        assert_eq!(
            Loader::load(&reader, &[K(o0, 2), K(o0, 3)]).await.unwrap(),
            HashMap::from_iter([(K(o0, 2), stored(o0, d0, 2, 2)), (K(o0, 3), stored(o0, d0, 2, 2))]),
        );
    ...
    async fn test_checkpoint_bounded() {
        ...
        assert_eq!(Loader::load(&reader, &[K(o1, 0)]).await.unwrap(), HashMap::new(),);
        ...
        assert_eq!(
            Loader::load(&reader, &[K(o0, 2), K(o0, 3)]).await.unwrap(),
            HashMap::from_iter([(K(o0, 2), stored(o0, d0, 4, 2)), (K(o0, 3), stored(o0, d0, 4, 2)),])
        );
        ...
        // All the requests in one.
        assert_eq!(
            Loader::load(&reader, &[K(o0, 0), K(o0, 1), K(o0, 2), K(o0, 3), K(o1, 0), K(o1, 1), K(o1, 2), K(o2, 1),])
                .await
                .unwrap(),
            HashMap::from_iter([
                (K(o0, 1), stored(o0, d0, 1, 1)),
