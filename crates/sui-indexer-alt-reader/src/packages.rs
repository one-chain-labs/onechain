...
impl Loader<PackageOriginalIdKey> for PgReader {
    ...
    async fn load(
        ...
        let id_to_stored: HashMap<_, _> = stored.iter().map(|package| (&package.package_id[..], package)).collect();
...
impl Loader<CheckpointBoundedOriginalPackageKey> for PgReader {
    ...
    async fn load(
        ...
        for package in stored_packages {
            let id = ObjectID::from_bytes(&package.original_id).context("Failed to deserialize ObjectID")?;

            let cp_sequence_number = package.cp_sequence_number as u64;
            key_to_stored.insert(CheckpointBoundedOriginalPackageKey(id, cp_sequence_number), package);
        }
