...
impl Loader<CoinMetadataKey> for PgReader {
    ...
    async fn load(&self, keys: &[CoinMetadataKey]) -> Result<HashMap<CoinMetadataKey, StoredObjInfo>, Self::Error> {
        ...
        let instantiations_to_stored: HashMap<_, _> =
            obj_info.iter().map(|stored| (&stored.instantiation, stored)).collect();
