...
impl Loader<VersionedObjectKey> for PgReader {
    ...
    async fn load(&self, keys: &[VersionedObjectKey]) -> Result<HashMap<VersionedObjectKey, StoredObject>, Error> {
...
impl Loader<VersionedObjectKey> for BigtableReader {
    ...
    async fn load(&self, keys: &[VersionedObjectKey]) -> Result<HashMap<VersionedObjectKey, Object>, Error> {
        if keys.is_empty() {
            return Ok(HashMap::new());
        }
