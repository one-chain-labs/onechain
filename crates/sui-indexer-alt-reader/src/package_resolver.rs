...
use sui_package_resolver::{error::Error, Package, PackageStore, PackageStoreWithLruCache, Resolver, Result};
...
impl Loader<PackageKey> for PgReader {
    ...
    async fn load(&self, keys: &[PackageKey]) -> Result<HashMap<PackageKey, Arc<Package>>> {
        ...
        let mut conn = self.connect().await.map_err(|e| Error::Store { store: STORE, error: e.to_string() })?;
        ...
        let stored_packages: Vec<SerializedPackage> =
            conn.results(query).await.map_err(|e| Error::Store { store: STORE, error: e.to_string() })?;
