...
impl RpcPackageStore {
    ...
    /// # Example
    /// ```ignore
    /// let store = RpcPackageStore::new("https://rpc-testnet.onelabs.cc:443");
    /// ```
...
impl PackageStore for RpcPackageStore {
    async fn fetch(&self, id: AccountAddress) -> Result<Arc<Package>, Error> {
        ...
        let object = self.client.clone().get_object(ObjectID::from(id)).await.map_err(|_| Error::PackageNotFound(id))?;
