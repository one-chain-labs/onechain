...
impl JsonVisitor {
    ...
    pub async fn deserialize_event<S>(event: &Event, resolver: &Resolver<S>) -> Result<Value, DeserializationError>
    where
        S: PackageStore,
    {
        ...
        S: PackageStore,
    {
        ...
        let futures = events.iter().map(|event| Self::deserialize_event(event, resolver));
...
impl<'b, 'l> AV::Visitor<'b, 'l> for JsonVisitor {
    ...
    fn visit_u8(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: u8) -> Result<Self::Value, Self::Error> {
        Ok(Value::Number(value.into()))
    }

    fn visit_u16(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: u16) -> Result<Self::Value, Self::Error> {
        Ok(Value::Number(value.into()))
    }

    fn visit_u32(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: u32) -> Result<Self::Value, Self::Error> {
        Ok(Value::Number(value.into()))
    }

    fn visit_u64(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: u64) -> Result<Self::Value, Self::Error> {
        Ok(Value::String(value.to_string()))
    }

    fn visit_u128(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: u128) -> Result<Self::Value, Self::Error> {
        Ok(Value::String(value.to_string()))
    }

    fn visit_u256(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: U256) -> Result<Self::Value, Self::Error> {
        Ok(Value::String(value.to_string()))
    }

    fn visit_bool(&mut self, _driver: &AV::ValueDriver<'_, 'b, 'l>, value: bool) -> Result<Self::Value, Self::Error> {
        Ok(Value::Bool(value))
    }
    ...
    fn visit_vector(&mut self, driver: &mut AV::VecDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        ...
        if driver.element_layout().is_type(&TypeTag::U8) {
            ...
            if let Some(bytes) = driver.bytes().get(driver.position() .. (driver.position() + driver.len() as usize)) {
    ...
    fn visit_struct(&mut self, driver: &mut AV::StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        ...
        if layout == &move_ascii_str_layout() || layout == &move_utf8_str_layout() || layout == &url_layout() {
            ...
            // HACK: Bypassing the layout to deserialize its bytes as a Rust type.
            let bytes = &driver.bytes()[lo .. hi];
            let id = AccountAddress::from_bytes(bytes).map_err(|_| AV::Error::UnexpectedEof)?.to_canonical_string(true);
            Ok(Value::String(id))
            ...
            // HACK: Bypassing the layout to deserialize its bytes as a Rust type.
            let bytes = &driver.bytes()[lo .. hi];
            let balance = bcs::from_bytes::<u64>(bytes).map_err(|_| AV::Error::UnexpectedEof)?.to_string();
    ...
    fn visit_variant(&mut self, driver: &mut AV::VariantDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        ...
        fields.insert("@variant".to_string(), Value::String(driver.variant_name().to_string()));
...
mod tests {
    ...
    fn enum_(rep: &str, variants: Vec<(&str, Vec<(&str, MoveTypeLayout)>)>) -> MoveTypeLayout {
        ...
        let variants = variants
            ...
            .map(|(tag, (name, fields))| {
                let fields =
                    fields.into_iter().map(|(name, layout)| A::MoveFieldLayout::new(ident_(name), layout)).collect();
    ...
    fn test_option() {
        ...
        let l = layout_("0x1::option::Option<u64>", vec![("vec", L::Vector(Box::new(L::U64)))]);
    ...
    #[test]
    fn test_balance() {
        let l = layout_("0x2::balance::Balance<0x2::oct::OCT>", vec![("value", L::U64)]);
    ...
    fn test_compound() {
        ...
        let layout = layout_("0x42::foo::Compound", vec![
            ("name", L::Struct(Box::new(move_utf8_str_layout()))),
            ("id", L::Struct(Box::new(ID::layout()))),
            ("balance", layout_("0x2::balance::Balance<0x2::oct::OCT>", vec![("value", L::U64)])),
            ("url", L::Struct(Box::new(url_layout()))),
            ("opt_value", layout_("0x1::option::Option<u64>", vec![("vec", L::Vector(Box::new(L::U64)))])),
        ]);

        let actual = to_json(layout, ("Test Object", address("0x42"), 1000u64, "https://example.com", Some(999u64)));
        ...
        let layout2 = layout_("0x42::foo::Compound2", vec![(
            "opt",
            layout_("0x1::option::Option<u128>", vec![("vec", L::Vector(Box::new(L::U128)))]),
        )]);
    ...
    fn test_simple_struct_to_json() {
        let layout = layout_("0x2::coin::Coin<0x2::oct::OCT>", vec![("id", L::Address), ("balance", L::U64)]);
    ...
    fn test_nested_struct_with_vector() {
        ...
        let layout =
            layout_("0x1::test::Outer", vec![("items", L::Vector(Box::new(inner_layout.clone()))), ("count", L::U64)]);
    ...
    fn test_variant() {
        ...
        let enum_layout = enum_("0x1::option::Option<u64>", vec![("None", vec![]), ("Some", vec![("value", L::U64)])]);
    ...
    fn test_byte_vector_base64() {
        let layout = layout_("0x1::test::Data", vec![
    ...
    fn test_large_numbers() {
        let layout = layout_("0x1::test::Numbers", vec![
            ("small_u64", L::U64),
            ("large_u64", L::U64),
            ("u128_value", L::U128),
            ("u256_value", L::U256),
        ...
        let json = to_json(layout, (1000u64, u64::MAX, u128::MAX, U256::from(123456789u128)));
