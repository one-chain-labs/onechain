...
async fn test_dry_run_publish_with_mocked_coin() -> Result<(), anyhow::Error> {
    ...
    let compiled_modules_bytes =
        compiled_package.get_package_base64(false).into_iter().map(|b| b.to_vec().unwrap()).collect::<Vec<_>>();
    ...
    let transaction_bytes = TransactionData::new_with_gas_coins(publish, address, vec![], 100000000, 1000);

    let result = client.read_api().dry_run_transaction_block(transaction_bytes).await;
