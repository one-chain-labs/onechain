...
async fn test_indexing_with_tto() {
    ...
    let tx = cluster.wallet.sign_transaction(&transaction_bytes.to_data().unwrap()).await;
    ...
    let gas = object_changes
        .iter()
        .find_map(|c| {
            if let ObjectChange::Mutated { object_id, version, digest, .. } = c {
    ...
    builder
        .move_call(
            ...
            vec![],
            vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(coin.object_ref()))],
        )
        .unwrap();
    let ptb = builder.finish();

    let gas_data =
        sui_types::transaction::GasData { payment: vec![gas], owner: address, price: 1000, budget: 100_000_000 };
    ...
    let parent = object_changes
        .iter()
        .find_map(|e| {
            if let ObjectChange::Created { object_id, version, digest, .. } = e {
                Some((*object_id, *version, *digest))
            } else {
                None
            }
    ...
    let parent_start_balance = http_client.get_balance(parent.0.into(), None).await.unwrap().total_balance;
    assert_eq!(http_client.get_balance(SuiAddress::ZERO, None).await.unwrap().total_balance, 0);
    ...
    builder
        .move_call(
            ...
            vec![],
            vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(parent)), CallArg::Object(ObjectArg::Receiving(coin))],
        )
        .unwrap();
    let ptb = builder.finish();
    ...
    assert_eq!(http_client.get_balance(parent.0.into(), None).await.unwrap().total_balance, 0);
    assert_eq!(http_client.get_balance(SuiAddress::ZERO, None).await.unwrap().total_balance, parent_start_balance);
}
