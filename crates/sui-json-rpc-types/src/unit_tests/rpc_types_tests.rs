...
fn test_move_type_serde() {
    use crate::{sui_move as SM, sui_move::SuiMoveNormalizedType as SNT};
    let test_types = vec![
        SNT::Bool,
        SNT::U8,
        SNT::U16,
    ...
    let s = SM::SuiMoveNormalizedStruct {
        abilities: SM::SuiMoveAbilitySet { abilities: vec![SM::SuiMoveAbility::Copy] },
        type_parameters: vec![SM::SuiMoveStructTypeParameter {
            constraints: SM::SuiMoveAbilitySet { abilities: vec![SM::SuiMoveAbility::Drop] },
            is_phantom: false,
        }],
        fields: vec![
            SM::SuiMoveNormalizedField { name: "field1".to_string(), type_: SNT::U8 },
            SM::SuiMoveNormalizedField { name: "field2".to_string(), type_: SNT::U16 },
        ],
    };
    ...
    let variant_declaration_order = variants.iter().map(|(name, _)| name.to_string()).collect::<Vec<_>>();
        ...
        .map(|(name, type_)| {
            (
                ...
                type_
                    .into_iter()
                    .enumerate()
                    .map(|(i, t)| SM::SuiMoveNormalizedField { name: format!("field{}", i), type_: t })
                    .collect(),
            )
        })
        .collect();
