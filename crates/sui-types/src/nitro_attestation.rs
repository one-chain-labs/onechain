...
pub fn parse_nitro_attestation(attestation_bytes: &[u8]) -> SuiResult<(Vec<u8>, Vec<u8>, AttestationDocument)> {
...
impl AttestationDocument {
    ...
    pub fn parse_payload(payload: &Vec<u8>) -> Result<AttestationDocument, NitroAttestationVerifyError> {
        let document_data: ciborium::value::Value = ciborium::de::from_reader(payload.as_slice()).map_err(|err| {
        ...
        let get_string = |key: &str| -> Result<String, NitroAttestationVerifyError> {
            match document_map.iter().find(|(k, _)| matches!(k, ciborium::value::Value::Text(s) if s == key)) {
                Some((_, ciborium::value::Value::Text(val))) => Ok(val.clone()),
                _ => Err(NitroAttestationVerifyError::InvalidAttestationDoc(format!("cannot parse {}", key))),
            }
        };
        ...
        let get_optional_bytes = |key: &str| -> Option<Vec<u8>> {
            document_map.iter().find(|(k, _)| matches!(k, ciborium::value::Value::Text(s) if s == key)).and_then(
                |(_, v)| match v {
            ...
            .find(|(k, _)| matches!(k, ciborium::value::Value::Text(s) if s == "timestamp"))
        {
            ...
            _ => return Err(NitroAttestationVerifyError::InvalidAttestationDoc("cannot parse timestamp".to_string())),
        };

        let public_key = get_optional_bytes("public_key");

        let pcrs = match document_map.iter().find(|(k, _)| matches!(k, ciborium::value::Value::Text(s) if s == "pcrs")) {
            Some((_, ciborium::value::Value::Map(pcr_map))) => {
                ...
                for i in [0, 1, 2, 3, 4, 8] {
                    if let Some((_, ciborium::value::Value::Bytes(val))) =
                        pcr_map.iter().find(|(k, _)| matches!(k, ciborium::value::Value::Integer(n) if *n == i.into()))
                    {
            ...
            _ => return Err(NitroAttestationVerifyError::InvalidAttestationDoc("cannot parse PCRs".to_string())),
        };
            ...
            .find(|(k, _)| matches!(k, ciborium::value::Value::Text(s) if s == "cabundle"))
        {
            Some((_, ciborium::value::Value::Array(arr))) => arr
                ...
                .map(|v| match v {
                    ...
                    _ => Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid cabundle format".to_string())),
                })
                .collect::<Result<Vec<_>, _>>()?,
            _ => return Err(NitroAttestationVerifyError::InvalidAttestationDoc("cannot parse cabundle".to_string())),
        };
...
fn verify_cert_chain(cert_chain: &[&[u8]], now_ms: u64) -> Result<(), NitroAttestationVerifyError> {
    if cert_chain.is_empty() || cert_chain.len() > MAX_CERT_CHAIN_LENGTH {
        return Err(NitroAttestationVerifyError::InvalidCertificate("invalid certificate chain length".to_string()));
    }
