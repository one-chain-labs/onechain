...
pub fn parse_nitro_attestation(
    ...
    let doc = AttestationDocument::parse_payload(&cose_sign1.payload, is_upgraded_parsing, include_all_nonzero_pcrs)?;
...
pub fn verify_nitro_attestation(
    ...
    let signature = Signature::from_slice(signature).map_err(|_| NitroAttestationVerifyError::InvalidSignature)?;
    ...
    match pk_bytes {
        PublicKey::EC(ec) => {
            let verifying_key =
                VerifyingKey::from_sec1_bytes(ec.data()).map_err(|_| NitroAttestationVerifyError::InvalidPublicKey)?;
            verifying_key
        ...
        impl<'de> Visitor<'de> for CoseSign1Visitor {
                ...
                A: SeqAccess<'de>,
            {
                ...
                let protected = match seq.next_element::<Value>()? {
                    Some(v) => extract_bytes(v).ok_or_else(|| A::Error::custom("protected header must be bytes"))?,
                ...
                let payload = match seq.next_element::<Value>()? {
                    Some(v) => extract_bytes(v).ok_or_else(|| A::Error::custom("payload must be bytes"))?,
                    None => return Err(A::Error::missing_field("payload")),
                };

                // Get signature bytes
                let signature = match seq.next_element::<Value>()? {
                    Some(v) => extract_bytes(v).ok_or_else(|| A::Error::custom("signature must be bytes"))?,
...
impl CoseSign1 {
    ...
    pub fn parse_and_validate(bytes: &[u8]) -> Result<Self, NitroAttestationVerifyError> {
        ...
        match tag {
            None | Some(18) => (),
            Some(_) => {
                return Err(NitroAttestationVerifyError::InvalidCoseSign1("invalid tag".to_string()));
            }
        }
    ...
    pub fn validate_header(&self) -> Result<(), NitroAttestationVerifyError> {
        ...
        {
            return Err(NitroAttestationVerifyError::InvalidCoseSign1("invalid cbor header".to_string()));
        }
        Ok(())
    }
    ...
    fn is_valid_protected_header(bytes: &[u8]) -> bool {
        ...
        match value {
            Value::Map(vec) => match &vec[..] {
                [(Value::Integer(key), Value::Integer(val))] => key == &expected_key && val == &expected_val,
                _ => false,
            },
            _ => false,
        }
    ...
    fn to_signed_message(&self) -> SuiResult<Vec<u8>> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes)
            .map_err(|_| SuiErrorKind::NitroAttestationFailedToVerify("cannot parse message".to_string()))?;
        Ok(bytes)
    }
}
...
impl AttestationDocument {
    ...
    fn to_map(
        ...
        let document_data: ciborium::value::Value = ciborium::de::from_reader(payload).map_err(|err| {
            NitroAttestationVerifyError::InvalidAttestationDoc(format!("cannot parse payload CBOR: {}", err))
        })?;
        ...
        let document_map: BTreeMap<String, Value> = match document_data {
            ciborium::value::Value::Map(map) => {
                ...
                let result = map
                    .into_iter()
                    .map(|(k, v)| {
                        let k = k.as_text().ok_or(NitroAttestationVerifyError::InvalidAttestationDoc(format!(
                            "invalid key type: {:?}",
                            k
                        )))?;
                        Ok((k.to_string(), v))
    ...
    fn validate_document_map(
        ...
        let module_id = document_map
            .get("module_id")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("module id not found".to_string()))?
            .as_text()
            .filter(|s| !s.is_empty())
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("invalid module id".to_string()))?
            .to_string();

        let digest = document_map
            .get("digest")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("digest not found".to_string()))?
            .as_text()
            .filter(|s| s == &"SHA384")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("invalid digest".to_string()))?
            .to_string();

        let certificate = document_map
            .get("certificate")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("certificate not found".to_string()))?
            .as_bytes()
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("invalid certificate".to_string()))?
            .to_vec();

        if certificate.is_empty() || certificate.len() > MAX_CERT_LENGTH {
            return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid certificate".to_string()));
        }

        let timestamp = document_map
            .get("timestamp")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("timestamp not found".to_string()))?
            .as_integer()
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("timestamp is not an integer".to_string()))
            .and_then(|integer| {
                u64::try_from(integer)
                    .map_err(|_| NitroAttestationVerifyError::InvalidAttestationDoc("timestamp not u64".to_string()))
            })?;

        let public_key = document_map.get("public_key").and_then(|v| v.as_bytes()).map(|bytes| bytes.to_vec());

        if let Some(data) = &public_key {
            if is_upgraded_parsing {
                if data.len() > MAX_PK_LENGTH {
                    return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid public key".to_string()));
                }
        ...
        let user_data = document_map.get("user_data").and_then(|v| v.as_bytes()).map(|bytes| bytes.to_vec());

        if let Some(data) = &user_data
            && data.len() > MAX_USER_DATA_LENGTH
        {
            return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid user data".to_string()));
        }

        let nonce = document_map.get("nonce").and_then(|v| v.as_bytes()).map(|bytes| bytes.to_vec());

        if let Some(data) = &nonce
            && data.len() > MAX_USER_DATA_LENGTH
        {
            return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid nonce".to_string()));
        }

        let (pcr_vec, pcr_map) = document_map
            .get("pcrs")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("pcrs not found".to_string()))?
            .as_map()
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("invalid pcrs format".to_string()))
            .and_then(|pairs| {
                if pairs.len() > MAX_PCRS_LENGTH {
                    return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid PCRs length".to_string()));
                }
                let mut pcr_vec = Vec::with_capacity(pairs.len());
                let mut pcr_map = BTreeMap::new();
                for (k, v) in pairs.iter() {
                    let key = k.as_integer().ok_or(NitroAttestationVerifyError::InvalidAttestationDoc(
                        "invalid PCR key format".to_string(),
                    ))?;
                    let value = v.as_bytes().ok_or(NitroAttestationVerifyError::InvalidAttestationDoc(
                    ...
                    let key_u64 = u64::try_from(key).map_err(|_| {
                        NitroAttestationVerifyError::InvalidAttestationDoc("invalid PCR index".to_string())
                    })?;
        ...
        let cabundle = document_map
            .get("cabundle")
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("cabundle not found".to_string()))?
            .as_array()
            .map(|arr| {
                ...
                for cert in arr.iter() {
                    let cert_bytes = cert.as_bytes().ok_or(NitroAttestationVerifyError::InvalidAttestationDoc(
                        "invalid certificate bytes".to_string(),
                    ))?;
                    if cert_bytes.is_empty() || cert_bytes.len() > MAX_CERT_LENGTH {
                        return Err(NitroAttestationVerifyError::InvalidAttestationDoc("invalid ca length".to_string()));
                    }
                    certs.push(cert_bytes.to_vec());
                }
                Ok(certs)
            })
            .ok_or(NitroAttestationVerifyError::InvalidAttestationDoc("invalid cabundle".to_string()))??;
...
fn verify_cert_chain(cert_chain: &[&[u8]], now_ms: u64) -> Result<(), NitroAttestationVerifyError> {
    ...
    let now_secs = ASN1Time::from_timestamp(
        (now_ms as i64)
            .checked_div(1000)
            .ok_or_else(|| NitroAttestationVerifyError::InvalidAttestationDoc("timestamp overflow".to_string()))?,
    )
    .map_err(|e| NitroAttestationVerifyError::InvalidCertificate(e.to_string()))?;
    ...
    for i in 0 .. cert_chain.len() {
        ...
        if let Ok(Some(key_usage)) = cert.key_usage() {
            ...
            return Err(NitroAttestationVerifyError::InvalidCertificate("Missing key usage extension".to_string()));
        }
        ...
        if i != 0 {
            ...
            if let Ok(Some(bc)) = cert.basic_constraints() {
                if !bc.critical || !bc.value.ca {
                    return Err(NitroAttestationVerifyError::InvalidCertificate("CA certificate invalid".to_string()));
                }
                ...
                return Err(NitroAttestationVerifyError::InvalidCertificate("missing basic constraint".to_string()));
            }
        ...
        if !cert.validity().is_valid_at(now_secs) {
            return Err(NitroAttestationVerifyError::InvalidCertificate("Certificate timestamp not valid".to_string()));
        }
        ...
        cert.verify_signature(Some(issuer_cert.public_key()))
            .map_err(|_| NitroAttestationVerifyError::InvalidCertificate("certificate fails to verify".to_string()))?;
    }
