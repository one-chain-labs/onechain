...
impl CoseSign1 {
    ...
    pub fn parse_and_validate(bytes: &[u8]) -> Result<Self, NitroAttestationVerifyError> {
        ...
        match tag {
            None | Some(18) => (),
            Some(_) => return Err(NitroAttestationVerifyError::InvalidCoseSign1("invalid tag".to_string())),
        }
    ...
    fn to_signed_message(&self) -> SuiResult<Vec<u8>> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes)
            .map_err(|_| SuiError::NitroAttestationFailedToVerify("cannot parse message".to_string()))?;
        Ok(bytes)
    }
}
