    ...
    V: Serialize,
{
    ...
    bcs::to_bytes(&field).map_err(|err| SuiError::ObjectSerializationError { error: err.to_string() })
}
    ...
    V: Serialize,
{
    ...
    pub fn into_move_object_unsafe_for_testing(self, version: SequenceNumber) -> Result<MoveObject, SuiError> {
        ...
        let field_bytes =
            bcs::to_bytes(&field).map_err(|e| SuiError::ObjectSerializationError { error: e.to_string() })?;
        Ok(unsafe {
