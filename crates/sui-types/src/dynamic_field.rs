...
use crate::{
    base_types::{MoveObjectType, ObjectDigest, SuiAddress},
    crypto::DefaultHash,
...
impl DynamicFieldInfo {
    ...
    pub fn parse_move_object(move_struct: &MoveStruct) -> SuiResult<(MoveValue, DynamicFieldType, ObjectID)> {
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            let object_id = extract_id_value(value).ok_or_else(|| SuiErrorKind::ObjectDeserializationError {
                error: format!(
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| SuiErrorKind::ObjectDeserializationError {
                error: format!(
                    "Cannot extract dynamic object's object id from \
...
where
    ...
    V: Serialize,
{
    let field = Field::<K, V> { id: id.clone(), name: name.clone(), value };

    bcs::to_bytes(&field).map_err(|err| SuiErrorKind::ObjectSerializationError { error: err.to_string() }.into())
}
    ...
    K: Serialize + fmt::Debug,
{
    ...
    pub fn into_id_with_bound(self, parent_version: SequenceNumber) -> Result<BoundedDynamicFieldID<K>, SuiError> {
        let id = self.object_id()?;
        Ok(BoundedDynamicFieldID::<K>::new(self.0.into(), id, parent_version))
    }
    ...
    {
        let id = self.object_id()?;
        let field = Field::<K, V> { id: UID::new(id), name: self.1, value };
        let type_tag = TypeTag::Struct(Box::new(DynamicFieldInfo::dynamic_field_type(self.2, V::get_type_tag())));
    ...
    V: Serialize,
{
    ...
    pub fn into_move_object_unsafe_for_testing(self, version: SequenceNumber) -> Result<MoveObject, SuiError> {
        let field = self.0;
        let type_tag = self.1;
        let TypeTag::Struct(struct_tag) = type_tag else { unreachable!() };
        // TODO(address-balances): more efficient type repr
        let move_object_type = MoveObjectType::from(*struct_tag);

        let field_bytes =
            bcs::to_bytes(&field).map_err(|e| SuiErrorKind::ObjectSerializationError { error: e.to_string() })?;
        Ok(unsafe {
    ...
    K: Serialize + std::fmt::Debug,
{
    ...
    pub fn load_object(self, object_store: &dyn ObjectStore) -> Option<DynamicFieldObject<K>> {
        object_store.get_object(&self.1).map(DynamicFieldObject::<K>::new)
    }

    /// Load the field object from the store.
    /// If the field does not exist, return an error.
    pub fn expect_object(self, key: &K, object_store: &dyn ObjectStore) -> Result<DynamicFieldObject<K>, SuiError> {
        let parent = self.0;
        let id = self.1;
    ...
    K: Serialize,
{
    ...
    pub fn new(parent_id: ObjectID, child_id: ObjectID, parent_version: SequenceNumber) -> Self {
        Self(parent_id, child_id, parent_version, std::marker::PhantomData)
    }
    ...
    pub fn load_object(
        ...
        child_object_resolver.read_child_object(&self.0, &self.1, self.2).map(|r| r.map(DynamicFieldObject::<K>::new))
    }
    ...
    K: Serialize + DeserializeOwned,
{
        ...
        V: Serialize + DeserializeOwned,
    {
        ...
        let move_object = object.data.try_as_move().ok_or_else(|| {
            SuiErrorKind::DynamicFieldReadError(format!("Dynamic field {:?} is not a Move object", object.id()))
        })?;
