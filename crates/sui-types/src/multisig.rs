...
impl AuthenticatorTrait for MultiSig {
    fn verify_user_authenticator_epoch(
        ...
        self.get_zklogin_sigs()?
            .iter()
            .try_for_each(|s| s.verify_user_authenticator_epoch(epoch_id, max_epoch_upper_bound_delta))
    }
        ...
        T: Serialize,
    {
        self.multisig_pk
            .validate()
            .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid multisig pubkey".to_string() })?;

        if SuiAddress::from(&self.multisig_pk) != multisig_address {
            return Err(SuiErrorKind::InvalidSignature { error: "Invalid address derived from pks".to_string() }.into());
        }
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) = self
                .multisig_pk
                .pk_map
                .get(i as usize)
                .ok_or(SuiErrorKind::InvalidSignature { error: "Invalid public keys index".to_string() })?;
            let res = match sig {
                CompressedSignature::Ed25519(s) => {
                    ...
                    let pk = Ed25519PublicKey::from_bytes(subsig_pubkey.as_ref())
                        .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid ed25519 pk bytes".to_string() })?;
                    pk.verify(
                        &digest,
                ...
                CompressedSignature::Secp256k1(s) => {
                    ...
                    let pk = Secp256k1PublicKey::from_bytes(subsig_pubkey.as_ref())
                        .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid k1 pk bytes".to_string() })?;
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| SuiErrorKind::InvalidSignature {
                ...
                CompressedSignature::Secp256r1(s) => {
                    ...
                    let pk = Secp256r1PublicKey::from_bytes(subsig_pubkey.as_ref())
                        .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid r1 pk bytes".to_string() })?;
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| SuiErrorKind::InvalidSignature {
                ...
                CompressedSignature::ZkLogin(z) => {
                    ...
                    let authenticator = ZkLoginAuthenticator::from_bytes(&z.0).map_err(|_| {
                        SuiErrorKind::InvalidSignature { error: "Invalid zklogin authenticator bytes".to_string() }
                    })?;
                    authenticator
                        .verify_claims(
                            value,
                ...
                CompressedSignature::Passkey(bytes) => {
                    let authenticator = PasskeyAuthenticator::from_bytes(&bytes.0).map_err(|_| {
                        SuiErrorKind::InvalidSignature { error: "Invalid passkey authenticator bytes".to_string() }
                    })?;
                    authenticator
                        .verify_claims(
                            value,
...
pub fn as_indices(bitmap: u16) -> Result<Vec<u8>, SuiError> {
    if bitmap > MAX_BITMAP_VALUE {
        return Err(SuiErrorKind::InvalidSignature { error: "Invalid bitmap".to_string() }.into());
    }
    let mut res = Vec::new();
    for i in 0 .. 10 {
        if bitmap & (1 << i) != 0 {
...
impl MultiSig {
    ...
    pub fn insecure_new(sigs: Vec<CompressedSignature>, bitmap: BitmapUnit, multisig_pk: MultiSigPublicKey) -> Self {
    ...
    pub fn combine(full_sigs: Vec<GenericSignature>, multisig_pk: MultiSigPublicKey) -> Result<Self, SuiError> {
        multisig_pk
            .validate()
        ...
        for s in full_sigs {
            ...
            if bitmap & (1 << index) != 0 {
                return Err(SuiErrorKind::InvalidSignature { error: "Duplicate public key".to_string() }.into());
            }
            bitmap |= 1 << index;
    ...
    pub fn init_and_validate(&mut self) -> Result<Self, FastCryptoError> {
        if self.sigs.len() > self.multisig_pk.pk_map.len() || self.sigs.is_empty() || self.bitmap > MAX_BITMAP_VALUE {
        ...
        authenticator_as_bytes
            ...
            .map(|z| {
                ZkLoginAuthenticator::from_bytes(&z.0).map_err(|_| {
                    SuiErrorKind::InvalidSignature { error: "Invalid zklogin authenticator bytes".to_string() }.into()
                })
            })
            .collect()
    }
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s)
            .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid base64 string".to_string() })?;
        let sig = MultiSig::from_bytes(&bytes)
            .map_err(|_| SuiErrorKind::InvalidSignature { error: "Invalid multisig bytes".to_string() })?;
        Ok(sig)
    }
}
...
impl MultiSigPublicKey {
    ...
    pub fn new(pks: Vec<PublicKey>, weights: Vec<WeightUnit>, threshold: ThresholdUnit) -> Result<Self, SuiError> {
        if pks.is_empty()
            ...
            || pks.iter().enumerate().any(|(i, pk)| pks.iter().skip(i + 1).any(|other_pk| *pk == *other_pk))
        {
            return Err(SuiErrorKind::InvalidSignature {
    ...
    pub fn validate(&self) -> Result<MultiSigPublicKey, FastCryptoError> {
        ...
        if self.threshold == 0
            ...
            || pk_map.iter().map(|(_pk, weight)| *weight as ThresholdUnit).sum::<ThresholdUnit>() < self.threshold
            || pk_map
                .iter()
                .enumerate()
