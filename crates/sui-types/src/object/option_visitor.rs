    ...
    E: From<Error> + From<AV::Error>,
{
    ...
    fn visit_struct(&mut self, driver: &mut AV::StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        if is_option(driver.struct_layout()) {
            driver.next_field(self)?.ok_or_else(|| Error.into()).map(|(_, option)| option)
        } else {
            Err(Error.into())
        }
    }
    ...
    fn visit_u8(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: u8) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_u16(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: u16) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_u32(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: u32) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_u64(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: u64) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_u128(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: u128) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_u256(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: U256) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_bool(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: bool) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_address(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_signer(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }

    fn visit_variant(&mut self, _: &mut AV::VariantDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        Err(Error.into())
    }
}
