...
use crate::{
    ...
    storage::{error::Error as StorageError, BackingPackageStore, EpochInfo, ObjectKey},
    sui_system_state::{get_sui_system_state, SuiSystemStateTrait},
    transaction::{Transaction, TransactionData, TransactionDataAPI, TransactionKind},
};
...
impl CheckpointData {
    ...
    pub fn epoch_info(&self) -> Result<Option<EpochInfo>, StorageError> {
        if self.checkpoint_summary.end_of_epoch_data.is_none() && self.checkpoint_summary.sequence_number != 0 {
            return Ok(None);
        }
        ...
        let system_state = get_sui_system_state(&transaction.output_objects.as_slice()).map_err(|e| {
            StorageError::custom(format!("Failed to find system state object output from end of epoch transaction: {e}"))
        })?;
        Ok(Some(EpochInfo {
...
impl ExecutedTransaction {
    pub fn input_objects<'a>(&self, object_set: &'a ObjectSet) -> impl Iterator<Item = &'a Object> + 'a {
    ...
    pub fn output_objects<'a>(&self, object_set: &'a ObjectSet) -> impl Iterator<Item = &'a Object> + 'a {
        self.effects.object_changes().into_iter().filter_map(move |change| {
        ...
        let transactions = value
            ...
            .map(|tx| {
                let input_objects = tx
                    ...
                    .into_iter()
                    .filter_map(|(object_id, version)| value.object_set.get(&ObjectKey(object_id, version)).cloned())
                    .collect::<Vec<_>>();
                let output_objects = tx
                    .effects
                    .all_changed_objects()
                    .into_iter()
                    .filter_map(|(object_ref, _owner, _kind)| value.object_set.get(&object_ref.into()).cloned())
                    .collect::<Vec<_>>();
            ...
            .collect();
        Self { checkpoint_summary: value.summary, checkpoint_contents: value.contents, transactions }
    }
}
...
impl From<CheckpointData> for Checkpoint {
    fn from(value: CheckpointData) -> Self {
        ...
        let transactions = value
            ...
            .collect();
        Self { summary: value.checkpoint_summary, contents: value.checkpoint_contents, transactions, object_set }
    }
}
