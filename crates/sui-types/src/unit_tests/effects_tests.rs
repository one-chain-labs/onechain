...
fn make_test_transaction(sender: SuiAddress, keypair: &AccountKeyPair, gas_object_id: ObjectID) -> Transaction {
    let pt = ProgrammableTransactionBuilder::new();
    // Create a gas object ref for the transaction
    let gas_object = (gas_object_id, SequenceNumber::from(1), ObjectDigest::random());
...
fn test_written_with_created_objects() {
    ...
    let (sender, keypair): (SuiAddress, AccountKeyPair) = get_key_pair_from_rng::<Ed25519KeyPair, _>(&mut rng);
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        .with_created_objects(vec![(obj1, Owner::AddressOwner(sender)), (obj2, Owner::AddressOwner(sender))])
        .build();
    ...
    assert!(written.iter().any(|(id, _, digest)| { *id == obj1 && digest.is_alive() }));
    assert!(written.iter().any(|(id, _, digest)| { *id == obj2 && digest.is_alive() }));
    // Check gas object is included
    assert!(written.iter().any(|(id, _, digest)| { *id == gas_object_id && digest.is_alive() }));
}
...
fn test_written_with_deleted_objects() {
    ...
    let effects =
        TestEffectsBuilder::new(tx.data()).with_deleted_objects(vec![(obj1, version1), (obj2, version2)]).build();
    ...
    assert!(written.iter().any(|(id, _, digest)| { *id == obj1 && *digest == ObjectDigest::OBJECT_DIGEST_DELETED }));
    assert!(written.iter().any(|(id, _, digest)| { *id == obj2 && *digest == ObjectDigest::OBJECT_DIGEST_DELETED }));
    // Check gas object is included
    assert!(written.iter().any(|(id, _, digest)| { *id == gas_object_id && digest.is_alive() }));
}
...
fn test_written_with_wrapped_objects() {
    ...
    let effects =
        TestEffectsBuilder::new(tx.data()).with_wrapped_objects(vec![(obj1, version1), (obj2, version2)]).build();
    ...
    assert!(written.iter().any(|(id, _, digest)| { *id == obj1 && *digest == ObjectDigest::OBJECT_DIGEST_WRAPPED }));
    assert!(written.iter().any(|(id, _, digest)| { *id == obj2 && *digest == ObjectDigest::OBJECT_DIGEST_WRAPPED }));
    // Check gas object is included
    assert!(written.iter().any(|(id, _, digest)| { *id == gas_object_id && digest.is_alive() }));
}
...
fn test_written_with_unwrapped_objects() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        .with_unwrapped_objects(vec![(obj1, Owner::AddressOwner(sender)), (obj2, Owner::AddressOwner(sender))])
        .build();
...
fn test_written_with_combination_of_all_types() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        .with_created_objects(vec![(created1, Owner::AddressOwner(sender)), (created2, Owner::AddressOwner(sender))])
        .with_mutated_objects(vec![
            (mutated1, SequenceNumber::from(5), Owner::AddressOwner(sender)),
            (mutated2, SequenceNumber::from(10), Owner::AddressOwner(sender)),
        ])
        .with_deleted_objects(vec![(deleted1, SequenceNumber::from(3)), (deleted2, SequenceNumber::from(7))])
        .with_wrapped_objects(vec![(wrapped1, SequenceNumber::from(4)), (wrapped2, SequenceNumber::from(8))])
    ...
    assert!(written.iter().any(|(id, _, digest)| { *id == created1 && digest.is_alive() }));
    assert!(written.iter().any(|(id, _, digest)| { *id == created2 && digest.is_alive() }));

    // Verify mutated objects have alive digests
    assert!(written.iter().any(|(id, _, digest)| { *id == mutated1 && digest.is_alive() }));
    assert!(written.iter().any(|(id, _, digest)| { *id == mutated2 && digest.is_alive() }));

    // Verify deleted objects have OBJECT_DIGEST_DELETED
    assert!(written.iter().any(|(id, _, digest)| { *id == deleted1 && *digest == ObjectDigest::OBJECT_DIGEST_DELETED }));
    assert!(written.iter().any(|(id, _, digest)| { *id == deleted2 && *digest == ObjectDigest::OBJECT_DIGEST_DELETED }));

    // Verify wrapped objects have OBJECT_DIGEST_WRAPPED
    assert!(written.iter().any(|(id, _, digest)| { *id == wrapped1 && *digest == ObjectDigest::OBJECT_DIGEST_WRAPPED }));
    assert!(written.iter().any(|(id, _, digest)| { *id == wrapped2 && *digest == ObjectDigest::OBJECT_DIGEST_WRAPPED }));

    // Verify unwrapped objects have alive digests
    assert!(written.iter().any(|(id, _, digest)| { *id == unwrapped1 && digest.is_alive() }));
    assert!(written.iter().any(|(id, _, digest)| { *id == unwrapped2 && digest.is_alive() }));
...
fn test_written_excludes_frozen_objects() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        ...
        .with_mutated_objects(vec![(mutated, SequenceNumber::from(5), Owner::AddressOwner(sender))])
        .build();

    let written = effects.written();
    // Should include gas object (mutated) and the mutated object, but not frozen objects
    assert_eq!(written.len(), 2);
    assert!(written.iter().any(|(id, _, digest)| { *id == mutated && digest.is_alive() }));
...
fn test_written_version_numbers() {
    ...
    let effects = TestEffectsBuilder::new(tx.data())
        ...
        .with_mutated_objects(vec![(obj2, SequenceNumber::from(5), Owner::AddressOwner(sender))])
