...
use crate::{
    ...
    transaction::{Argument::Input, CallArg, Command, ObjectArg},
};
...
fn test_builder_merge_coins_one_source() {
    ...
    builder.merge_coins(target_coin_ref, vec![coins_ref]).unwrap();
    ...
    assert_eq!(tx.commands, vec![Command::MergeCoins(Input(0), vec![Input(1)])]);
}
...
fn test_builder_merge_coins_two_sources() {
    ...
    builder.merge_coins(target_coin_ref, vec![source_coin1_ref, source_coin2_ref]).unwrap();
    ...
    assert_eq!(tx.commands, vec![Command::MergeCoins(Input(0), vec![Input(1), Input(2),])]);
}
...
fn test_builder_merge_coins_zero_source() {
    ...
    assert_eq!(tx.inputs, vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(target_coin_ref)),]);
...
fn test_builder_smash_coins_two_coins() {
    ...
    let arg = builder.smash_coins(vec![target_coin_ref, source_coin_ref]).unwrap();
...
fn test_builder_smash_coins_three_coin() {
    ...
    let arg = builder.smash_coins(vec![target_coin_ref, source_coin1_ref, source_coin2_ref]).unwrap();
