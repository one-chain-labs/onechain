...
fn create_test_transaction_data(gas_payment: Vec<ObjectRef>, expiration: TransactionExpiration) -> TransactionDataV1 {
    ...
    TransactionDataV1 {
        ...
        sender,
        gas_data: GasData { payment: gas_payment, owner: sender, price: 1000, budget: 1000000 },
        expiration,
    }
}
...
fn test_address_balance_payment_requires_accumulators_enabled() {
    ...
    let tx_data = create_test_transaction_data(vec![], TransactionExpiration::ValidDuring {
        min_epoch: Some(1),
        max_epoch: Some(1),
...
fn test_address_balance_payment_valid() {
    ...
    let result = tx_data.validity_check(&config);
    assert!(result.is_ok(), "Transaction should be valid with accumulators enabled");
}
...
fn test_address_balance_payment_missing_epochs() {
    ...
    fn assert_missing_epoch_error(config: &ProtocolConfig, expiration: TransactionExpiration, case_description: &str) {
...
fn test_regular_gas_payment_works_without_accumulators() {
    ...
    let tx_data = create_test_transaction_data(vec![random_object_ref()], TransactionExpiration::None);

    let result = tx_data.validity_check(&config);
    assert!(result.is_ok(), "Regular gas payment should work without accumulators");
}
...
fn test_regular_gas_payment_with_valid_during_expiration() {
    ...
    let tx_data = create_test_transaction_data(vec![random_object_ref()], TransactionExpiration::ValidDuring {
        min_epoch: Some(1),
        max_epoch: Some(1),
        min_timestamp_seconds: None,
        max_timestamp_seconds: None,
    ...
    let result = tx_data.validity_check(&config);
    assert!(result.is_ok(), "Regular gas payment with ValidDuring expiration should be allowed");
}
...
fn test_regular_gas_payment_with_epoch_expiration() {
    ...
    let tx_data = create_test_transaction_data(vec![random_object_ref()], TransactionExpiration::Epoch(5));

    let result = tx_data.validity_check(&config);
    assert!(result.is_ok(), "Regular gas payment with Epoch expiration should be allowed");
}
