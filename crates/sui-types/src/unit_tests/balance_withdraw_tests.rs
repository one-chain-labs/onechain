...
fn test_withdraw_max_amount() {
    ...
    let tx = TransactionData::new_programmable(sender, vec![random_object_ref()], ptb.finish(), 1, 1);
    ...
    let account_id = AccumulatorValue::get_field_id(
        sender,
        &WithdrawalTypeArg::Balance(GAS::type_tag().into()).to_type_tag().unwrap(),
    )
    .unwrap();
...
fn test_multiple_withdraws_different_accounts() {
    ...
    .unwrap();
    let account_id2 =
        AccumulatorValue::get_field_id(sender, &WithdrawalTypeArg::Balance(TypeInput::Bool).to_type_tag().unwrap())
            .unwrap();
    assert_eq!(withdraws, BTreeMap::from([(account_id1, 100), (account_id2, 200),]));
}
...
fn test_withdraw_too_many_withdraws() {
    let mut ptb = ProgrammableTransactionBuilder::new();
    for _ in 0 .. 11 {
        ptb.funds_withdrawal(FundsWithdrawalArg::balance_from_sender(100, TypeInput::from(GAS::type_tag()))).unwrap();
    }
    let sender = SuiAddress::random_for_testing_only();
    let tx = TransactionData::new_programmable(sender, vec![random_object_ref()], ptb.finish(), 1, 1);
    assert!(tx.process_funds_withdrawals().is_err());
