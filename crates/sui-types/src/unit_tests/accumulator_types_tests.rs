...
use crate::{
    accumulator_root::{AccumulatorKey, U128},
    balance::Balance,
    base_types::{MoveObjectType, SequenceNumber, SuiAddress},
    dynamic_field::{DynamicFieldInfo, DynamicFieldKey},
    gas_coin::GAS,
    object::MoveObject,
    MoveTypeTagTrait,
...
fn test_sui_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<OCT>>, U128> type
    ...
    let balance_type = move_type.balance_accumulator_field_type_maybe().expect("Should have balance type");
...
fn test_accumulator_storage_savings() {
    ...
    // Create the field struct tag for Field<Key<Balance<OCT>>, U128>
    let sui_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[sui_balance]);
    let u128_type = U128::get_type_tag();
    ...
    let field_key =
        DynamicFieldKey(SUI_ACCUMULATOR_ROOT_OBJECT_ID, key, AccumulatorKey::get_type_tag(&[GAS::type_tag()]));
    ...
    let standard_move_type = MoveObjectType(crate::base_types::MoveObjectType_::Other(field_struct_tag));
    ...
    println!("Efficient representation size: {} bytes", efficient_serialized.len());
    println!("Standard representation size: {} bytes", standard_serialized.len());
    println!("Savings: {} bytes", standard_serialized.len() - efficient_serialized.len());
    ...
    assert!(savings >= 100, "Expected at least 100 bytes of savings, got {} bytes", savings);

    // Also verify the efficient object is using the right type
    assert!(efficient_move_object.type_().is_sui_balance_accumulator_field());
}
