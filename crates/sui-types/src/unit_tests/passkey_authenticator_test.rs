...
async fn test_passkey_fails_invalid_json() {
    ...
    assert_eq!(err, SuiErrorKind::InvalidSignature { error: "Invalid client data json".to_string() });
...
async fn test_passkey_fails_invalid_challenge() {
    ...
    let res: Result<PasskeyAuthenticator, SuiError> = raw.try_into();
    let err = res.unwrap_err();
    assert_eq!(err, SuiErrorKind::InvalidSignature { error: "Invalid encoded challenge".to_string() });
}
...
async fn test_passkey_fails_wrong_client_data_type() {
    ...
    let res: Result<PasskeyAuthenticator, SuiError> = raw.try_into();
    let err = res.unwrap_err();
    assert_eq!(err, SuiErrorKind::InvalidSignature { error: "Invalid client data type".to_string() });
}
...
async fn test_real_passkey_output() {
    ...
    let address = SuiAddress::from_str("0x9c0c00e929f08431583dad0e9409b5afb20cdbae0043fa5577f2577dbe88a0db").unwrap();
