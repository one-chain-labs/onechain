...
impl ProtoVisitorBuilder {
    ...
    /// budget.
    pub fn deserialize_value(mut self, bytes: &[u8], layout: &A::MoveTypeLayout) -> Result<Value, Error> {
...
impl<'b, 'l> AV::Visitor<'b, 'l> for ProtoVisitor<'_> {
    ...
    fn visit_address(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, value: AccountAddress) -> Result<Value, Error> {
        let value = value.to_canonical_string(true);
        self.debit_string_value(&value)?;
        Ok(Value::from(value))
    }

    fn visit_signer(&mut self, _: &AV::ValueDriver<'_, 'b, 'l>, value: AccountAddress) -> Result<Value, Error> {
        let value = value.to_canonical_string(true);
        self.debit_string_value(&value)?;
        Ok(Value::from(value))
    }
    ...
    fn visit_variant(&mut self, driver: &mut AV::VariantDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        let mut map = Struct::default();
        self.debit_value()?;
