...
impl ProtoVisitorBuilder {
    ...
    /// budget.
    pub fn deserialize_value(mut self, bytes: &[u8], layout: &A::MoveTypeLayout) -> anyhow::Result<Value> {
    ...
    /// size budget.
    pub fn deserialize_struct(mut self, bytes: &[u8], layout: &A::MoveStructLayout) -> anyhow::Result<Struct> {
        let mut visitor = self.new_visitor()?;

        let Value { kind: Some(Kind::StructValue(struct_)) } =
...
impl<'b, 'l> Visitor<'b, 'l> for ProtoVisitor<'_> {
    ...
    fn visit_address(&mut self, _: &ValueDriver<'_, 'b, 'l>, value: AccountAddress) -> Result<Value, Error> {
        let value = value.to_canonical_string(true);
        self.debit_string_value(&value)?;
        Ok(Value::from(value))
    }

    fn visit_signer(&mut self, _: &ValueDriver<'_, 'b, 'l>, value: AccountAddress) -> Result<Value, Error> {
        let value = value.to_canonical_string(true);
        self.debit_string_value(&value)?;
        Ok(Value::from(value))
    }
    ...
    fn visit_vector(&mut self, driver: &mut VecDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        let value = if driver.element_layout().is_type(&TypeTag::U8) {
            ...
            while let Some(elem) = driver.next_element(&mut ProtoVisitor::new(self.bound, self.depth)?)? {
                elems.push(elem);
            }
    ...
    fn visit_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        ...
        let value = if layout == &move_ascii_str_layout() || layout == &move_utf8_str_layout() || layout == &url_layout()
        {
            ...
            // HACK: Bypassing the layout to deserialize its bytes as a Rust type.
            let bytes = &driver.bytes()[lo .. hi];
            let id = AccountAddress::from_bytes(bytes).map_err(|_| Error::UnexpectedType)?.to_canonical_string(true);
            ...
            // HACK: Bypassing the layout to deserialize its bytes as a Rust type.
            let bytes = &driver.bytes()[lo .. hi];
            let balance = bcs::from_bytes::<u64>(bytes).map_err(|_| Error::UnexpectedType)?.to_string();
            ...
            while let Some((field, elem)) = driver.next_field(&mut ProtoVisitor::new(self.bound, self.depth)?)? {
    ...
    fn visit_variant(&mut self, driver: &mut VariantDriver<'_, 'b, 'l>) -> Result<Value, Error> {
        ...
        map.fields.insert("@variant".to_owned(), driver.variant_name().as_str().into());
    ...
    T: Visitor<'b, 'l, Error = Error>,
{
    ...
    fn visit_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Error> {
        if is_option(driver.struct_layout()) {
            driver.next_field(self)?.ok_or(Error::UnexpectedType).map(|(_, option)| option)
        } else {
    ...
    fn visit_address(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Error> {
        Err(Error::UnexpectedType)
    }

    fn visit_signer(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Error> {
        Err(Error::UnexpectedType)
    }
...
pub(crate) mod tests {
    ...
    fn test_simple() {
        ...
        let deser = ProtoVisitorBuilder::new(bound).deserialize_value(&bytes, &type_layout).unwrap();

        assert_eq!(expected, proto_value_to_json_value(deser));

        ProtoVisitorBuilder::new(bound - 1).deserialize_value(&bytes, &type_layout).unwrap_err();
    }
    ...
    fn test_too_deep() {
        ...
        let deser = ProtoVisitorBuilder::new(bound).deserialize_value(&bytes, &layout).unwrap();
        ...
        let err = ProtoVisitorBuilder::new(bound).deserialize_value(&bytes, &layout).unwrap_err();
    ...
    fn proto_value_to_json_value(proto: Value) -> serde_json::Value {
        match proto.kind {
            ...
            Some(Kind::StructValue(map)) => serde_json::Value::Object(
                map.fields.into_iter().map(|(k, v)| (k, proto_value_to_json_value(v))).collect(),
            ),
            Some(Kind::ListValue(list_value)) => {
                serde_json::Value::Array(list_value.values.into_iter().map(proto_value_to_json_value).collect())
            }
        }
    }
    ...
    fn required_budget(json: &serde_json::Value) -> usize {
        size_of::<Value>()
            + match json {
                ...
                serde_json::Value::Object(map) => map.iter().map(|(k, v)| k.len() + required_budget(v)).sum(),
            }
    }
    ...
    fn json<T: serde::Serialize>(layout: A::MoveTypeLayout, data: T) -> serde_json::Value {
        ...
        let proto_value = ProtoVisitorBuilder::new(1024 * 1024).deserialize_value(&bcs, &layout).unwrap();
    ...
    #[test]
    fn json_u256() {
        let actual = json(L::U256, U256::from_str("42424242424242424242424242424242424242424").unwrap());
