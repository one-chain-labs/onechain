...
/// Rust representation of `one::coin_registry::CurrencyKey<T>`.
#[derive(Serialize, Deserialize, Copy, Clone, Default, PartialEq, Eq)]
pub struct CurrencyKey(bool);

/// Rust representation of `one::coin_registry::Currency<phantom T>`.
...
/// Rust representation of `one::coin_registry::SupplyState<phantom T>`.
#[derive(Serialize, Deserialize)]
...
/// Rust representation of `one::coin_registry::RegulatedState`.
#[derive(Serialize, Deserialize)]
pub enum RegulatedState {
    Regulated { cap: ObjectID, allow_global_pause: Option<bool>, variant: u8 },
    Unregulated,
    Unknown,
}

/// Rust representation of `one::coin_registry::MetadataCapState`.
#[derive(Serialize, Deserialize)]
...
/// Rust representation of `one::coin_registry::ExtraField`.
#[derive(Serialize, Deserialize)]
...
impl Currency {
    /// Derive the ObjectID for `one::coin_registry::Currency<$coin_type>`.
    pub fn derive_object_id(coin_type: TypeTag) -> Result<ObjectID, bcs::Error> {
        ...
        derived_object::derive_object_id(SUI_COIN_REGISTRY_OBJECT_ID, &key, &bcs::to_bytes(&CurrencyKey::default())?)
    }

    /// Is this `StructTag` a `one::coin_registry::Currency<...>`?
    pub fn is_currency(tag: &StructTag) -> bool {
        tag.address == SUI_FRAMEWORK_ADDRESS && tag.module.as_str() == "coin_registry" && tag.name.as_str() == "Currency"
    }
}
...
pub fn get_coin_registry_obj_initial_shared_version(
    ...
    Ok(object_store.get_object(&SUI_COIN_REGISTRY_OBJECT_ID).map(|obj| match obj.owner {
