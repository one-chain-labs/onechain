...
use sui_rpc::{field::FieldMaskTree, merge::Merge, proto::sui::rpc::v2beta2::*};
...
impl Merge<crate::full_checkpoint_content::CheckpointTransaction> for ExecutedTransaction {
    fn merge(&mut self, source: crate::full_checkpoint_content::CheckpointTransaction, mask: &FieldMaskTree) {
        ...
        if let Some(submask) = mask.subtree(ExecutedTransaction::SIGNATURES_FIELD) {
            self.signatures = signatures.into_iter().map(|s| UserSignature::merge_from(s, &submask)).collect();
        }
        ...
        if let Some(submask) = mask.subtree(ExecutedTransaction::EVENTS_FIELD) {
            self.events = source.events.map(|events| TransactionEvents::merge_from(events, &submask));
        }

        if let Some(submask) = mask.subtree(ExecutedTransaction::INPUT_OBJECTS_FIELD) {
            self.input_objects = source.input_objects.into_iter().map(|o| Object::merge_from(o, &submask)).collect();
        }

        if let Some(submask) = mask.subtree(ExecutedTransaction::OUTPUT_OBJECTS_FIELD) {
            self.output_objects = source.output_objects.into_iter().map(|o| Object::merge_from(o, &submask)).collect();
        }
    }
}
...
impl Merge<crate::effects::TransactionEvents> for TransactionEvents {
    fn merge(&mut self, source: crate::effects::TransactionEvents, mask: &FieldMaskTree) {
        ...
        if let Some(events_mask) = mask.subtree(Self::EVENTS_FIELD) {
            self.events = source.data.into_iter().map(|event| Event::merge_from(event, &events_mask)).collect();
        }
    }
}
...
impl TryFrom<&TransactionExpiration> for crate::transaction::TransactionExpiration {
    ...
    fn try_from(value: &TransactionExpiration) -> Result<Self, Self::Error> {
        ...
        Ok(match value.kind() {
            ...
            TransactionExpirationKind::Unknown | _ => return Err("unknown TransactionExpirationKind"),
        })
    }
}
...
impl From<crate::transaction::TransactionKind> for TransactionKind {
    fn from(value: crate::transaction::TransactionKind) -> Self {
        ...
        let kind = match value {
            ...
            K::ProgrammableSystemTransaction(ptb) => Kind::ProgrammableSystemTransaction(ptb.into()),
            K::ChangeEpoch(change_epoch) => Kind::ChangeEpoch(change_epoch.into()),
            K::Genesis(genesis) => Kind::Genesis(genesis.into()),
            K::ConsensusCommitPrologue(prologue) => Kind::ConsensusCommitPrologueV1(prologue.into()),
            ...
            K::ConsensusCommitPrologueV2(prologue) => Kind::ConsensusCommitPrologueV2(prologue.into()),
            K::ConsensusCommitPrologueV3(prologue) => Kind::ConsensusCommitPrologueV3(prologue.into()),
            K::ConsensusCommitPrologueV4(prologue) => Kind::ConsensusCommitPrologueV4(prologue.into()),
        };
...
impl From<crate::transaction::EndOfEpochTransactionKind> for EndOfEpochTransactionKind {
    fn from(value: crate::transaction::EndOfEpochTransactionKind) -> Self {
        ...
        let kind = match value {
            ...
            K::BridgeCommitteeInit(bridge_object_version) => Kind::BridgeCommitteeInit(bridge_object_version.value()),
            K::StoreExecutionTimeObservations(observations) => Kind::ExecutionTimeObservations(observations.into()),
