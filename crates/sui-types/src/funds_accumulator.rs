...
//! Rust bindings for `one::funds_accumulator`
...
use crate::{base_types::SuiAddress, SUI_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_WITHDRAWAL_STRUCT: (&AccountAddress, &IdentStr, &IdentStr) =
    (&SUI_FRAMEWORK_ADDRESS, FUNDS_ACCUMULATOR_MODULE_NAME, WITHDRAWAL_STRUCT_NAME);

/// Rust bindings for the Move struct `one::funds_accumulator::Withdrawal`.
