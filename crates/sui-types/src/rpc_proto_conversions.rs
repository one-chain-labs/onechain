            ...
            self.transactions = source
                ...
                .map(|t| {
                    ...
                    transaction.checkpoint =
                        submask.contains(ExecutedTransaction::CHECKPOINT_FIELD).then_some(sequence_number);
...
impl Merge<crate::messages_checkpoint::CheckpointSummary> for CheckpointSummary {
    fn merge(&mut self, source: crate::messages_checkpoint::CheckpointSummary, mask: &FieldMaskTree) {
...
impl Merge<crate::messages_checkpoint::CheckpointContents> for CheckpointContents {
    fn merge(&mut self, source: crate::messages_checkpoint::CheckpointContents, mask: &FieldMaskTree) {
        if mask.contains(Self::BCS_FIELD) {
            let mut bcs = Bcs::serialize(&source).unwrap();
...
impl Merge<&crate::effects::TransactionEvents> for TransactionEvents {
    fn merge(&mut self, source: &crate::effects::TransactionEvents, mask: &FieldMaskTree) {
        ...
        if let Some(events_mask) = mask.subtree(Self::EVENTS_FIELD) {
            self.events = source.data.iter().map(|event| Event::merge_from(event, &events_mask)).collect();
        }
    }
}
...
impl From<crate::sui_system_state::sui_system_state_inner_v1::SuiSystemStateInnerV1> for SystemState {
    fn from(
        ...
        let validator_report_records = validator_report_records
            ...
            .map(|entry| {
                ...
                record.reporters = entry.value.contents.iter().map(ToString::to_string).collect();
                record
            })
            .collect();
...
impl From<crate::sui_system_state::sui_system_state_inner_v2::SuiSystemStateInnerV2> for SystemState {
    fn from(
...
impl From<crate::sui_system_state::sui_system_state_inner_v1::SystemParametersV1> for SystemParameters {
    fn from(
...
impl From<crate::sui_system_state::sui_system_state_inner_v2::SystemParametersV2> for SystemParameters {
    fn from(
...
impl From<crate::sui_system_state::sui_system_state_inner_v1::ValidatorSetV1> for ValidatorSet {
    fn from(
        ...
        let at_risk_validators =
            at_risk_validators.contents.into_iter().map(|entry| (entry.key.to_string(), entry.value)).collect();
...
impl From<crate::execution_status::ExecutionFailureStatus> for ExecutionError {
    fn from(value: crate::execution_status::ExecutionFailureStatus) -> Self {
        ...
        let kind = match value {
            ...
            E::MoveObjectTooBig { object_size, max_object_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(object_size, max_object_size)));
                ExecutionErrorKind::ObjectTooBig
            }
            E::MovePackageTooBig { object_size, max_object_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(object_size, max_object_size)));
            ...
            E::VMVerificationOrDeserializationError => ExecutionErrorKind::VmVerificationOrDeserializationError,
            ...
            E::CommandArgumentError { arg_idx, kind } => {
                ...
                message.error_details = Some(ErrorDetails::CommandArgumentError(command_argument_error));
            ...
            E::TypeArgumentError { argument_idx, kind } => {
                ...
                type_argument_error.kind = Some(type_argument_error::TypeArgumentErrorKind::from(kind).into());
                message.error_details = Some(ErrorDetails::TypeArgumentError(type_argument_error));
                ExecutionErrorKind::TypeArgumentError
            }
            E::UnusedValueWithoutDrop { result_idx, secondary_idx } => {
                message.error_details =
                    Some(ErrorDetails::IndexError(index_error(result_idx.into(), Some(secondary_idx.into()))));
                ExecutionErrorKind::UnusedValueWithoutDrop
            }
            E::InvalidPublicFunctionReturnType { idx } => {
                message.error_details = Some(ErrorDetails::IndexError(index_error(idx.into(), None)));
                ExecutionErrorKind::InvalidPublicFunctionReturnType
            }
            E::InvalidTransferObject => ExecutionErrorKind::InvalidTransferObject,
            E::EffectsTooLarge { current_size, max_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(current_size, max_size)));
                ExecutionErrorKind::EffectsTooLarge
            }
            E::PublishUpgradeMissingDependency => ExecutionErrorKind::PublishUpgradeMissingDependency,
            E::PublishUpgradeDependencyDowngrade => ExecutionErrorKind::PublishUpgradeDependencyDowngrade,
            E::PackageUpgradeError { upgrade_error } => {
                message.error_details = Some(ErrorDetails::PackageUpgradeError(upgrade_error.into()));
                ExecutionErrorKind::PackageUpgradeError
            }
            E::WrittenObjectsTooLarge { current_size, max_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(current_size, max_size)));
            ...
            E::SharedObjectOperationNotAllowed => ExecutionErrorKind::ConsensusObjectOperationNotAllowed,
            ...
            E::ExecutionCancelledDueToSharedObjectCongestion { congested_objects } => {
                message.error_details = Some(ErrorDetails::CongestedObjects({
                    ...
                    message.objects = congested_objects.0.iter().map(|o| o.to_canonical_string(true)).collect();
                    message
                }));
            ...
            E::MoveVectorElemTooBig { value_size, max_scaled_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(value_size, max_scaled_size)));

                ExecutionErrorKind::MoveVectorElemTooBig
            }
            E::MoveRawValueTooBig { value_size, max_scaled_size } => {
                message.error_details = Some(ErrorDetails::SizeError(size_error(value_size, max_scaled_size)));
...
impl From<crate::execution_status::CommandArgumentError> for CommandArgumentError {
    fn from(value: crate::execution_status::CommandArgumentError) -> Self {
        ...
        let kind = match value {
            ...
            E::InvalidArgumentToPrivateEntryFunction => CommandArgumentErrorKind::InvalidArgumentToPrivateEntryFunction,
            E::IndexOutOfBounds { idx } => {
                message.index_error = Some(index_error(idx.into(), None));
                CommandArgumentErrorKind::IndexOutOfBounds
            }
            E::SecondaryIndexOutOfBounds { result_idx, secondary_idx } => {
                message.index_error = Some(index_error(result_idx.into(), Some(secondary_idx.into())));
...
impl From<crate::execution_status::TypeArgumentError> for type_argument_error::TypeArgumentErrorKind {
    ...
    fn from(value: crate::execution_status::PackageUpgradeError) -> Self {
        ...
        let kind = match value {
            ...
            E::DigestDoesNotMatch { digest } => {
                message.digest = crate::digests::Digest::try_from(digest).ok().map(|d| d.to_string());
            ...
            E::PackageIDDoesNotMatch { package_id, ticket_id } => {
                message.package_id = Some(package_id.to_canonical_string(true));
...
impl<const T: bool> From<crate::crypto::AuthorityQuorumSignInfo<T>> for ValidatorAggregatedSignature {
    fn from(value: crate::crypto::AuthorityQuorumSignInfo<T>) -> Self {
        ...
        Self::default().with_epoch(value.epoch).with_signature(value.signature.as_ref().to_vec()).with_bitmap(bitmap)
    }
}

impl<const T: bool> TryFrom<&ValidatorAggregatedSignature> for crate::crypto::AuthorityQuorumSignInfo<T> {
    type Error = TryFromProtoError;
...
impl From<&crate::zk_login_authenticator::ZkLoginAuthenticator> for ZkLoginAuthenticator {
    fn from(value: &crate::zk_login_authenticator::ZkLoginAuthenticator) -> Self {
        ...
        sui_sdk_types::ZkLoginAuthenticator::try_from(value.clone()).map(Into::into).ok().unwrap_or(message)
    }
}
...
impl From<&crate::multisig_legacy::MultiSigLegacy> for MultisigAggregatedSignature {
    fn from(value: &crate::multisig_legacy::MultiSigLegacy) -> Self {
        ...
        value.get_bitmap().serialize_into(&mut legacy_bitmap).unwrap();
...
impl TryFrom<&BalanceChange> for crate::balance_change::BalanceChange {
    ...
    fn try_from(value: &BalanceChange) -> Result<Self, Self::Error> {
        Ok(Self {
            address: value.address().parse().map_err(|e| TryFromProtoError::invalid(BalanceChange::ADDRESS_FIELD, e))?,
            coin_type: value
                .coin_type()
                .parse()
                .map_err(|e| TryFromProtoError::invalid(BalanceChange::COIN_TYPE_FIELD, e))?,
            amount: value.amount().parse().map_err(|e| TryFromProtoError::invalid(BalanceChange::AMOUNT_FIELD, e))?,
        })
    }
}
    ...
    fn merge(&mut self, source: &crate::move_package::MovePackage, mask: &FieldMaskTree) {
        ...
        if mask.contains(Self::PACKAGE_FIELD.name) {
            ...
            package.modules = source
                ...
                .collect();
            package.type_origins = source.type_origin_table().clone().into_iter().map(Into::into).collect();
            package.linkage = source
                .linkage_table()
                .iter()
                .map(|(original_id, crate::move_package::UpgradeInfo { upgraded_id, upgraded_version })| {
...
impl ObjectReferenceExt for ObjectReference {
    fn try_to_object_ref(&self) -> Result<crate::base_types::ObjectRef, anyhow::Error> {
        ...
        let object_id = self.object_id_opt().ok_or_else(|| anyhow::anyhow!("missing object_id"))?;
        ...
        let version = self.version_opt().ok_or_else(|| anyhow::anyhow!("missing version"))?;
        let version = crate::base_types::SequenceNumber::from(version);

        let digest = self.digest_opt().ok_or_else(|| anyhow::anyhow!("missing digest"))?;
        let digest = digest
...
impl From<&crate::storage::ObjectKey> for ObjectReference {
    fn from(value: &crate::storage::ObjectKey) -> Self {
        Self::default().with_object_id(value.0.to_canonical_string(true)).with_version(value.1.value())
    }
}
...
impl From<crate::object::Owner> for Owner {
    fn from(value: crate::object::Owner) -> Self {
        ...
        let kind = match value {
            ...
            O::ConsensusAddressOwner { start_version, owner } => {
...
impl From<&crate::transaction::GasData> for GasPayment {
    fn from(value: &crate::transaction::GasData) -> Self {
        let mut message = Self::default();
        message.objects = value.payment.iter().map(|obj_ref| obj_ref.to_proto()).collect();
...
impl From<crate::transaction::TransactionKind> for TransactionKind {
    fn from(value: crate::transaction::TransactionKind) -> Self {
        ...
        match value {
            K::ProgrammableTransaction(ptb) => {
                message.with_programmable_transaction(ptb).with_kind(Kind::ProgrammableTransaction)
            }
            K::ChangeEpoch(change_epoch) => message.with_change_epoch(change_epoch).with_kind(Kind::ChangeEpoch),
            K::Genesis(genesis) => message.with_genesis(genesis).with_kind(Kind::Genesis),
            K::ConsensusCommitPrologue(prologue) => {
                message.with_consensus_commit_prologue(prologue).with_kind(Kind::ConsensusCommitPrologueV1)
            }
            K::AuthenticatorStateUpdate(update) => {
                message.with_authenticator_state_update(update).with_kind(Kind::AuthenticatorStateUpdate)
            }
            ...
            K::RandomnessStateUpdate(update) => {
                message.with_randomness_state_update(update).with_kind(Kind::RandomnessStateUpdate)
            }
            K::ConsensusCommitPrologueV2(prologue) => {
                message.with_consensus_commit_prologue(prologue).with_kind(Kind::ConsensusCommitPrologueV2)
            }
            K::ConsensusCommitPrologueV3(prologue) => {
                message.with_consensus_commit_prologue(prologue).with_kind(Kind::ConsensusCommitPrologueV3)
            }
            K::ConsensusCommitPrologueV4(prologue) => {
                message.with_consensus_commit_prologue(prologue).with_kind(Kind::ConsensusCommitPrologueV4)
            }
...
impl From<crate::messages_consensus::ConsensusCommitPrologue> for ConsensusCommitPrologue {
    fn from(value: crate::messages_consensus::ConsensusCommitPrologue) -> Self {
        ...
        message.commit_timestamp = Some(sui_rpc::proto::timestamp_ms_to_proto(value.commit_timestamp_ms));
        message
    }
}
...
impl From<crate::messages_consensus::ConsensusCommitPrologueV3> for ConsensusCommitPrologue {
    fn from(value: crate::messages_consensus::ConsensusCommitPrologueV3) -> Self {
        ...
        message.consensus_determined_version_assignments = Some(value.consensus_determined_version_assignments.into());
        message
    }
}
...
impl From<crate::messages_consensus::ConsensusCommitPrologueV4> for ConsensusCommitPrologue {
    fn from(
        ...
        message.consensus_determined_version_assignments = Some(consensus_determined_version_assignments.into());
...
impl From<crate::transaction::RandomnessStateUpdate> for RandomnessStateUpdate {
    fn from(value: crate::transaction::RandomnessStateUpdate) -> Self {
        ...
        message.randomness_object_initial_shared_version = Some(value.randomness_obj_initial_shared_version.value());
        message
    }
}
...
impl From<crate::transaction::ChangeEpoch> for ChangeEpoch {
    fn from(value: crate::transaction::ChangeEpoch) -> Self {
        ...
        message.epoch_start_timestamp = Some(sui_rpc::proto::timestamp_ms_to_proto(value.epoch_start_timestamp_ms));
        message.system_packages = value
            ...
            .map(|(version, modules, dependencies)| {
                ...
                message.dependencies = dependencies.iter().map(|d| d.to_canonical_string(true)).collect();
                message
            })
            .collect();
        message
...
impl From<crate::transaction::EndOfEpochTransactionKind> for EndOfEpochTransactionKind {
    fn from(value: crate::transaction::EndOfEpochTransactionKind) -> Self {
        ...
        match value {
            ...
            K::AuthenticatorStateExpire(expire) => {
                message.with_authenticator_state_expire(expire).with_kind(Kind::AuthenticatorStateExpire)
            }
            ...
            crate::transaction::StoredExecutionTimeObservations::V1(vec) => {
                ...
                message.observations = vec
                    ...
                    .map(|(key, observation)| {
                        ...
                        let kind = match key {
                            K::MoveEntryPoint { package, module, function, type_arguments } => {
                                message.move_entry_point = Some({
                                    ...
                                    message.type_arguments =
                                        type_arguments.into_iter().map(|ty| ty.to_canonical_string(true)).collect();
                                    message
                                });
...
impl From<crate::transaction::CallArg> for Input {
    fn from(value: crate::transaction::CallArg) -> Self {
        ...
        use crate::transaction::{CallArg as I, ObjectArg as O};
...
impl From<crate::transaction::ProgrammableMoveCall> for MoveCall {
    fn from(value: crate::transaction::ProgrammableMoveCall) -> Self {
        ...
        message.type_arguments = value.type_arguments.iter().map(|t| t.to_canonical_string(true)).collect();
...
impl Merge<&crate::effects::TransactionEffectsV1> for TransactionEffects {
    fn merge(&mut self, value: &crate::effects::TransactionEffectsV1, mask: &FieldMaskTree) {
        ...
        if mask.contains(Self::DEPENDENCIES_FIELD.name) {
            self.dependencies = value.dependencies().iter().map(ToString::to_string).collect();
        }
            ...
            || mask.contains(Self::GAS_OBJECT_FIELD.name)
        {
            ...
            for (object_id, version) in value.modified_at_versions() {
                ...
                if let Some(changed_object) = changed_objects.iter_mut().find(|object| object.object_id() == object_id) {
            ...
            for (id, version, digest) in value.shared_objects() {
                ...
                if let Some(changed_object) = changed_objects.iter_mut().find(|object| object.object_id() == object_id) {
                    ...
                    unchanged_consensus_object.kind =
                        Some(unchanged_consensus_object::UnchangedConsensusObjectKind::ReadOnlyRoot.into());
            ...
            if mask.contains(Self::GAS_OBJECT_FIELD.name) {
                ...
                self.gas_object = changed_objects.iter().find(|object| object.object_id() == gas_object_id).cloned();
            }
    ...
    fn merge(
        ...
        if mask.contains(Self::GAS_OBJECT_FIELD.name) {
            self.gas_object = gas_object_index
                .map(|index| {
                    changed_objects.get(index as usize).cloned().map(|(id, change)| {
...
impl From<crate::effects::EffectsObjectChange> for ChangedObject {
    fn from(value: crate::effects::EffectsObjectChange) -> Self {
        ...
        use crate::effects::{ObjectIn, ObjectOut};
...
impl From<simulate_transaction_request::TransactionChecks> for crate::transaction_executor::TransactionChecks {
...
impl From<crate::coin_registry::MetadataCapState> for coin_metadata::MetadataCapState {
    fn from(value: crate::coin_registry::MetadataCapState) -> Self {
        match value {
            crate::coin_registry::MetadataCapState::Claimed(_) => coin_metadata::MetadataCapState::Claimed,
            crate::coin_registry::MetadataCapState::Unclaimed => coin_metadata::MetadataCapState::Unclaimed,
            crate::coin_registry::MetadataCapState::Deleted => coin_metadata::MetadataCapState::Deleted,
        }
    }
}
    ...
    fn from(value: &crate::coin_registry::Currency) -> Self {
        ...
        match &value.metadata_cap_id {
            ...
            crate::coin_registry::MetadataCapState::Unclaimed => {
                metadata.metadata_cap_state = Some(coin_metadata::MetadataCapState::Unclaimed as i32);
            }
...
impl From<&crate::coin_registry::RegulatedState> for RegulatedCoinMetadata {
    fn from(value: &crate::coin_registry::RegulatedState) -> Self {
        ...
        match value {
            crate::coin_registry::RegulatedState::Regulated { cap, allow_global_pause, variant } => {
                ...
                regulated.coin_regulated_state = Some(regulated_coin_metadata::CoinRegulatedState::Regulated as i32);
            }
            crate::coin_registry::RegulatedState::Unregulated => {
                regulated.coin_regulated_state = Some(regulated_coin_metadata::CoinRegulatedState::Unregulated as i32);
            }
            crate::coin_registry::RegulatedState::Unknown => {
                regulated.coin_regulated_state = Some(regulated_coin_metadata::CoinRegulatedState::Unknown as i32);
            }
        }
...
impl From<crate::coin::RegulatedCoinMetadata> for RegulatedCoinMetadata {
    fn from(value: crate::coin::RegulatedCoinMetadata) -> Self {
        ...
        message.coin_metadata_object = Some(sui_sdk_types::Address::from(value.coin_metadata_object.bytes).to_string());
