...
use super::{
    object_change::{AccumulatorWriteV1, ObjectIn, ObjectOut},
    EffectsObjectChange,
    IDOperation,
    ObjectChange,
};
...
impl TransactionEffectsAPI for TransactionEffectsV2 {
    ...
    fn input_consensus_objects(&self) -> Vec<InputConsensusObject> {
        self.changed_objects
            ...
            .chain(self.unchanged_consensus_objects.iter().filter_map(|(id, change_kind)| match change_kind {
                ...
                UnchangedConsensusKind::MutateConsensusStreamEnded(seqno) => {
                    Some(InputConsensusObject::MutateConsensusStreamEnded(*id, *seqno))
                }
                UnchangedConsensusKind::ReadConsensusStreamEnded(seqno) => {
                    Some(InputConsensusObject::ReadConsensusStreamEnded(*id, *seqno))
                }
                UnchangedConsensusKind::Cancelled(seqno) => Some(InputConsensusObject::Cancelled(*id, *seqno)),
    ...
    fn consensus_owner_changed(&self) -> Vec<ObjectRef> {
        self.changed_objects
            ...
            .filter_map(|(id, change)| match (&change.input_state, &change.output_state, &change.id_operation) {
                (
                    ObjectIn::Exist((_, Owner::ConsensusAddressOwner { owner: old_owner, .. })),
                    ObjectOut::ObjectWrite((object_digest, Owner::ConsensusAddressOwner { owner: new_owner, .. })),
                    IDOperation::None,
                ) if old_owner != new_owner => Some((*id, self.lamport_version, *object_digest)),
                _ => None,
            })
            .collect()
    }
        ...
        self.changed_objects
            ...
            .filter_map(|(id, change)| match &change.output_state {
                ObjectOut::AccumulatorWriteV1(write) => {
                    Some(AccumulatorEvent::new(AccumulatorObjId::new_unchecked(*id), write.clone()))
                }
                _ => None,
            })
            .collect()
    ...
    fn unsafe_add_input_consensus_object_for_testing(&mut self, kind: InputConsensusObject) {
        match kind {
            InputConsensusObject::Mutate(obj_ref) => self.changed_objects.push((obj_ref.0, EffectsObjectChange {
            ...
            InputConsensusObject::ReadOnly(obj_ref) => self
                .unchanged_consensus_objects
                .push((obj_ref.0, UnchangedConsensusKind::ReadOnlyRoot((obj_ref.1, obj_ref.2)))),
            InputConsensusObject::ReadConsensusStreamEnded(obj_id, seqno) => {
                self.unchanged_consensus_objects.push((obj_id, UnchangedConsensusKind::ReadConsensusStreamEnded(seqno)))
            }
            InputConsensusObject::MutateConsensusStreamEnded(obj_id, seqno) => self
                .unchanged_consensus_objects
                .push((obj_id, UnchangedConsensusKind::MutateConsensusStreamEnded(seqno))),
            InputConsensusObject::Cancelled(obj_id, seqno) => {
                self.unchanged_consensus_objects.push((obj_id, UnchangedConsensusKind::Cancelled(seqno)))
            }
        }
    }
...
impl TransactionEffectsV2 {
    pub fn new(
        ...
        let unchanged_consensus_objects = shared_objects
                ...
                SharedInput::ConsensusStreamEnded((id, version, mutability, _)) => {
                    ...
                    match mutability {
                        SharedObjectMutability::Mutable => {
                            Some((id, UnchangedConsensusKind::MutateConsensusStreamEnded(version)))
                        }
                        SharedObjectMutability::Immutable => {
                            Some((id, UnchangedConsensusKind::ReadConsensusStreamEnded(version)))
                        }
            ...
            .chain(loaded_per_epoch_config_objects.into_iter().map(|id| (id, UnchangedConsensusKind::PerEpochConfig)))
            .collect();
