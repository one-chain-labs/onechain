...
use crate::{
    ...
    object::Owner,
    transaction::{InputObjectKind, SenderSignedData, SharedObjectMutability, TransactionDataAPI},
};
...
impl TestEffectsBuilder {
    ...
    pub fn with_created_objects(mut self, objects: impl IntoIterator<Item = (ObjectID, Owner)>) -> Self {
    ...
    pub fn with_wrapped_objects(mut self, objects: impl IntoIterator<Item = (ObjectID, SequenceNumber)>) -> Self {
        self.wrapped_objects.extend(objects);
        self
    }

    pub fn with_unwrapped_objects(mut self, objects: impl IntoIterator<Item = (ObjectID, Owner)>) -> Self {
        self.unwrapped_objects.extend(objects);
        self
    }

    pub fn with_deleted_objects(mut self, objects: impl IntoIterator<Item = (ObjectID, SequenceNumber)>) -> Self {
    ...
    pub fn build(self) -> TransactionEffects {
        ...
        let changed_objects = self
            ...
            .chain(self.mutated_objects.into_iter().map(|(id, version, owner)| {
                (id, EffectsObjectChange {
                    input_state: ObjectIn::Exist(((version, ObjectDigest::random()), Owner::AddressOwner(sender))),
                    output_state: ObjectOut::ObjectWrite((ObjectDigest::random(), owner)),
                    id_operation: IDOperation::None,
                })
            }))
