    ...
    pub fn merge(mut writes: Vec<Self>) -> Self {
        ...
        let (merged_value, net_operation) = match &writes[0].value {
            AccumulatorValue::Integer(_) => {
                let (merge_amount, split_amount) = writes.iter().fold((0u64, 0u64), |(merge, split), w| {
                    if let AccumulatorValue::Integer(v) = w.value {
                        match w.operation {
                            AccumulatorOperation::Merge => {
                                (merge.checked_add(v).expect("validated in object runtime"), split)
                            }
                            AccumulatorOperation::Split => {
                                (merge, split.checked_add(v).expect("validated in object runtime"))
                            }
                        }
                    } else {
            ...
            AccumulatorValue::EventDigest(first_digests) => {
                ...
                (AccumulatorValue::EventDigest(event_digests), AccumulatorOperation::Merge)
            }
        };
        AccumulatorWriteV1 { address, operation: net_operation, value: merged_value }
    }
}
...
mod tests {
    ...
    fn test_accumulator_address() -> AccumulatorAddress {
        AccumulatorAddress::new(
            ...
            "0x2::balance::Balance<0x2::oct::OCT>".parse().unwrap(),
        )
    }
    ...
    fn test_merge_mismatched_types() {
        ...
        let addr1 = AccumulatorAddress::new(addr, "0x2::balance::Balance<0x2::oct::OCT>".parse().unwrap());
        let addr2 = AccumulatorAddress::new(addr, "0x2::accumulator_settlement::EventStreamHead".parse().unwrap());
