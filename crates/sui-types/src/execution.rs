...
///    took it by value.
pub type ConsensusStreamEndedInfo = (ObjectID, SequenceNumber, SharedObjectMutability, TransactionDigest);
...
impl ExecutionResultsV2 {
    ...
    pub fn merge_results(&mut self, new_results: Self) {
        ...
        self.accumulator_events.extend(new_results.accumulator_events);
        ...
        for (id, obj) in self.written_objects.iter_mut() {
            ...
            if let Owner::Shared { initial_shared_version } = &mut obj.owner {
                ...
                if reshare_at_initial_version
                    && let Some(Owner::Shared { initial_shared_version: previous_initial_shared_version }) =
...
impl std::fmt::Display for ExecutionTimeObservationKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ExecutionTimeObservationKey::MoveEntryPoint { module, function, .. } => {
