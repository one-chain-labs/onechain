...
pub fn get_early_execution_error(
    ...
    if input_objects.inner().contains_consensus_stream_ended_objects() {
    ...
    if matches!(balance_withdraw_status, BalanceWithdrawStatus::InsufficientBalance) {
...
mod tests {
    ...
    use crate::{
        base_types::ObjectID,
        transaction::{CheckedInputObjects, InputObjectKind, InputObjects, ObjectReadResult, ObjectReadResultKind},
    };
    ...
    fn test_early_execution_error_insufficient_balance() {
        ...
        assert_eq!(result, Some(ExecutionErrorKind::InsufficientBalanceForWithdraw));

        // Test with sufficient balance
        let result =
            get_early_execution_error(&tx_digest, &input_objects, &deny_set, &BalanceWithdrawStatus::SufficientBalance);
    ...
    fn test_early_execution_error_precedence() {
        ...
        let input_objects = InputObjects::new(vec![
            ...
            ObjectReadResult {
                ...
                object: ObjectReadResultKind::CancelledTransactionSharedObject(SequenceNumber::CONGESTED),
            },
        ]);
        let result = get_early_execution_error(
            &tx_digest,
        ...
        assert!(matches!(result, Some(ExecutionErrorKind::ExecutionCancelledDueToSharedObjectCongestion { .. })));
    }
}
