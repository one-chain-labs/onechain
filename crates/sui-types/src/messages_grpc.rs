...
impl SubmitTxRequest {
    ...
    pub fn tx_type(&self) -> TxType {
        if let Some(ping_type) = self.ping_type {
            return if ping_type == PingType::FastPath { TxType::SingleWriter } else { TxType::SharedObject };
        }
    ...
    pub fn into_raw(&self) -> Result<RawSubmitTxRequest, SuiError> {
        let transactions = if let Some(transaction) = &self.transaction {
            vec![bcs::to_bytes(&transaction)
                .map_err(|e| SuiErrorKind::TransactionSerializationError { error: e.to_string() })?
                .into()]
        } else {
            vec![]
        };

        let submit_type = if self.ping_type.is_some() { SubmitTxType::Ping } else { SubmitTxType::Default };
...
impl TryFrom<RawExecutedData> for ExecutedData {
    ...
    fn try_from(value: RawExecutedData) -> Result<Self, Self::Error> {
        let effects =
            bcs::from_bytes(&value.effects).map_err(|err| crate::error::SuiErrorKind::GrpcMessageDeserializeError {
                type_info: "RawExecutedData.effects".to_string(),
                error: err.to_string(),
            })?;
        let events = if let Some(events) = value.events {
            Some(bcs::from_bytes(&events).map_err(|err| crate::error::SuiErrorKind::GrpcMessageDeserializeError {
