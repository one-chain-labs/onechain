...
impl SubmitTxRequest {
    pub fn new_transaction(transaction: Transaction) -> Self {
        Self { transaction: Some(transaction), ping: None }
    }

    pub fn new_ping(ping: PingType) -> Self {
        Self { transaction: None, ping: Some(ping) }
    }

    pub fn tx_type(&self) -> TxType {
        if let Some(ping) = self.ping {
            return if ping == PingType::FastPath { TxType::SingleWriter } else { TxType::SharedObject };
        }
    ...
    pub fn into_raw(&self) -> Result<RawSubmitTxRequest, SuiError> {
        let transactions = if let Some(transaction) = &self.transaction {
            vec![bcs::to_bytes(&transaction)
                .map_err(|e| SuiError::TransactionSerializationError { error: e.to_string() })?
                .into()]
        } else {
            vec![]
        };

        let submit_type = if self.ping.is_some() { SubmitTxType::Ping } else { SubmitTxType::Default };
...
impl std::fmt::Debug for WaitForEffectsResponse {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Executed { effects_digest, fast_path, .. } => {
                f.debug_struct("Executed").field("effects_digest", effects_digest).field("fast_path", fast_path).finish()
            }
            Self::Rejected { error } => f.debug_struct("Rejected").field("error", error).finish(),
...
impl TryFrom<ExecutedData> for RawExecutedData {
    ...
    fn try_from(value: ExecutedData) -> Result<Self, Self::Error> {
        ...
        Ok(RawExecutedData { effects, events, input_objects, output_objects })
    }
}
...
impl TryFrom<RawExecutedData> for ExecutedData {
    ...
    fn try_from(value: RawExecutedData) -> Result<Self, Self::Error> {
        let effects =
            bcs::from_bytes(&value.effects).map_err(|err| crate::error::SuiError::GrpcMessageDeserializeError {
                type_info: "RawExecutedData.effects".to_string(),
                error: err.to_string(),
            })?;
        let events = if let Some(events) = value.events {
            Some(bcs::from_bytes(&events).map_err(|err| crate::error::SuiError::GrpcMessageDeserializeError {
        ...
        Ok(ExecutedData { effects, events, input_objects, output_objects })
    }
}
    ...
    fn try_from(value: RawSubmitTxResult) -> Result<Self, Self::Error> {
        match value.inner {
            Some(RawValidatorSubmitStatus::Submitted(consensus_position)) => {
                Ok(SubmitTxResult::Submitted { consensus_position: consensus_position.as_ref().try_into()? })
            }
...
fn try_from_raw_executed_status(
    executed: RawExecutedStatus,
) -> Result<(crate::digests::TransactionEffectsDigest, Option<Box<ExecutedData>>, bool), crate::error::SuiError> {
    ...
    let executed_data = if let Some(details) = executed.details { Some(Box::new(details.try_into()?)) } else { None };
...
fn try_from_raw_rejected_status(
    ...
    match rejected.error {
        Some(error_bytes) => {
            let error =
                bcs::from_bytes(&error_bytes).map_err(|err| crate::error::SuiError::GrpcMessageDeserializeError {
...
impl TryFrom<RawWaitForEffectsRequest> for WaitForEffectsRequest {
    ...
    fn try_from(value: RawWaitForEffectsRequest) -> Result<Self, Self::Error> {
        let transaction_digest = match value.transaction_digest {
            Some(digest) => {
                Some(bcs::from_bytes(&digest).map_err(|err| crate::error::SuiError::GrpcMessageDeserializeError {
        ...
        let ping = value
            ...
            .transpose()?;
        Ok(Self { consensus_position, transaction_digest, include_details: value.include_details, ping })
    }
}
...
impl TryFrom<WaitForEffectsRequest> for RawWaitForEffectsRequest {
    ...
    fn try_from(value: WaitForEffectsRequest) -> Result<Self, Self::Error> {
        ...
        Ok(Self { consensus_position, transaction_digest, include_details: value.include_details, ping_type })
    }
}
...
mod tests {
    ...
    async fn test_submit_tx_request_into_raw() {
        ...
        println!("Case 2. SubmitTxRequest::new_transaction should be converted to RawSubmitTxRequest with submit_type set to Default.");
        {
            ...
            let tx_data = TransactionData::new_transfer_oct(recipient, sender, None, gas_object_ref, 1000, 1000);
