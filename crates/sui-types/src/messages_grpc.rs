...
impl SubmitTxResponse {
    pub fn from_bytes(
        ...
        Ok(Self {
            effects: bcs::from_bytes(&effects).map_err(|e| SuiError::GrpcMessageDeserializeError {
            ...
            events: if include_events {
                events
                    .map(|events| {
                        bcs::from_bytes(&events).map_err(|e| SuiError::GrpcMessageDeserializeError {
            ...
            input_objects: if include_input_objects {
                Some(
                    input_objects
                        .into_iter()
                        .map(|object| {
                            bcs::from_bytes(&object).map_err(|e| SuiError::GrpcMessageDeserializeError {
            ...
            auxiliary_data: if include_auxiliary_data { auxiliary_data.map(|data| data.to_vec()) } else { None },
        })
    }
}
