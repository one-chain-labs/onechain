...
impl AccumulatorValue {
    pub fn get_field_id(owner: SuiAddress, type_: &TypeTag) -> SuiResult<AccumulatorObjId> {
        if !Balance::is_balance_type(type_) {
            return Err(SuiError::TypeError { error: "only Balance<T> is supported".to_string() });
        }

        let key = AccumulatorKey { owner };
        Ok(AccumulatorObjId(
            DynamicFieldKey(SUI_ACCUMULATOR_ROOT_OBJECT_ID, key, AccumulatorKey::get_type_tag(&[type_.clone()]))
                .into_unbounded_id()?
                .as_object_id(),
        ))
    }
    ...
    pub fn create_for_testing(owner: SuiAddress, type_tag: TypeTag, balance: u64) -> Object {
        ...
        let field_key =
            DynamicFieldKey(SUI_ACCUMULATOR_ROOT_OBJECT_ID, key, AccumulatorKey::get_type_tag(&[type_tag.clone()]));
...
impl TryFrom<&MoveObject> for AccumulatorValue {
    ...
    fn try_from(value: &MoveObject) -> Result<Self, Self::Error> {
        value
            ...
            .map(Self::U128)
            .ok_or_else(|| {
                SuiError::DynamicFieldReadError(format!("Dynamic field {:?} is not a AccumulatorValue", value.id()))
            })
    }
}
