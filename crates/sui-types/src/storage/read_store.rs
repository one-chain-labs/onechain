...
use crate::{
    ...
    base_types::{EpochId, ObjectID, ObjectType, SequenceNumber, SuiAddress},
    committee::Committee,
...
pub type PackageVersionsIterator<'a> = Box<dyn Iterator<Item = Result<(u64, ObjectID), TypedStoreError>> + 'a>;
...
pub trait ReadStore: ObjectStore {
    ...
    fn get_checkpoint_by_sequence_number(&self, sequence_number: CheckpointSequenceNumber)
    ...
    fn multi_get_transactions(&self, tx_digests: &[TransactionDigest]) -> Vec<Option<Arc<VerifiedTransaction>>> {
    ...
    fn multi_get_events(&self, event_digests: &[TransactionDigest]) -> Vec<Option<TransactionEvents>> {
        event_digests.iter().map(|digest| self.get_events(digest)).collect()
    }
    ...
    fn get_checkpoint_data(
        ...
        use crate::{effects::TransactionEffectsAPI, storage::error::Error};
        ...
        let txns = self
            ...
            .into_iter()
            .map(|maybe_transaction| maybe_transaction.ok_or_else(|| Error::missing("missing transaction")))
        ...
        for (tx, fx) in txns.into_iter().zip(effects) {
            let events = fx.events_digest().map(|_event_digest| {
                events.remove(fx.transaction_digest()).expect("event was already checked to be present")
            });
        ...
        let object_set = {
            ...
            for (idx, object) in objects.into_iter().enumerate() {
                object_set
                    .insert(object.ok_or_else(|| Error::missing(format!("unable to load object {:?}", refs[idx])))?);
            }
            object_set
        };

        let checkpoint_data =
            Checkpoint { summary: checkpoint.into(), contents: checkpoint_contents, transactions, object_set };
    ...
    fn get_type_layout(&self, type_tag: &TypeTag) -> Result<Option<MoveTypeLayout>> {
        match type_tag {
            ...
            TypeTag::Vector(type_tag) => {
                Ok(self.get_type_layout(type_tag)?.map(|layout| MoveTypeLayout::Vector(Box::new(layout))))
            }
...
pub trait RpcIndexes: Send + Sync {
    ...
    fn package_versions_iter(&self, original_id: ObjectID, cursor: Option<u64>) -> Result<PackageVersionsIterator<'_>>;
...
impl DynamicFieldKey {
    pub fn new<P: Into<ObjectID>>(parent: P, field_id: ObjectID) -> Self {
        Self { parent: parent.into(), field_id }
    }
}
...
impl TransactionInfo {
    pub fn new(
        ...
        let object_types =
            input_objects.iter().chain(output_objects).map(|object| (object.id(), ObjectType::from(object))).collect();
