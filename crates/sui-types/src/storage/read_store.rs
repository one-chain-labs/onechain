...
pub trait ReadStore: ObjectStore {
    ...
    fn get_checkpoint_data(
        ...
        let transactions = self
            ...
            .into_iter()
            .map(|maybe_transaction| maybe_transaction.ok_or_else(|| anyhow::anyhow!("missing transaction")))
        ...
        for (tx, fx) in transactions.into_iter().zip(effects) {
            let events = fx.events_digest().map(|_event_digest| {
                events.get(fx.transaction_digest()).cloned().expect("event was already checked to be present")
            });
