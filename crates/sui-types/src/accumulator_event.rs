...
impl AccumulatorEvent {
    pub fn new(accumulator_obj: AccumulatorObjId, write: AccumulatorWriteV1) -> Self {
        if in_test_configuration()
            && let Ok(expected_obj) =
                crate::accumulator_root::AccumulatorValue::get_field_id(write.address.address, &write.address.ty)
        {
            debug_assert_eq!(
        ...
        Self { accumulator_obj, write }
    }

    pub fn from_balance_change(address: SuiAddress, balance_type: TypeTag, net_change: i64) -> Result<Self, SuiError> {
        ...
        let accumulator_obj = crate::accumulator_root::AccumulatorValue::get_field_id(address, &balance_type)?;
        ...
        let accumulator_write =
            AccumulatorWriteV1 { address: accumulator_address, operation, value: AccumulatorValue::Integer(amount) };

        Ok(Self::new(accumulator_obj, accumulator_write))
    }

    pub fn total_oct_in_event(&self) -> (u64 /* input */, u64 /* output */) {
        let Self { write: AccumulatorWriteV1 { address: AccumulatorAddress { ty, .. }, operation, value }, .. } = self;
