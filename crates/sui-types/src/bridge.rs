...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, SuiError> {
    ...
    let move_object = wrapper
        .data
        .try_as_move()
        .ok_or_else(|| SuiErrorKind::SuiBridgeReadError("BridgeWrapper object must be a Move object".to_owned()))?;
...
pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, SuiError> {
    ...
    match version {
        ...
        _ => Err(SuiErrorKind::SuiBridgeReadError(format!("Unsupported SuiBridge version: {}", version)).into()),
    }
}
...
impl BridgeTrait for BridgeInnerV1 {
    ...
    fn try_into_bridge_summary(self) -> SuiResult<BridgeSummary> {
        let transfer_limit = self
            ...
            .into_iter()
            .map(|e| {
                let source = BridgeChainId::try_from(e.key.source).map_err(|_e| SuiErrorKind::GenericBridgeError {
                    error: format!("Unrecognized chain id: {}", e.key.source),
                })?;
                let destination = BridgeChainId::try_from(e.key.destination).map_err(|_e| {
                    SuiErrorKind::GenericBridgeError { error: format!("Unrecognized chain id: {}", e.key.destination) }
                })?;
