...
use sui_types::{base_types::ObjectDigest, global_state_hash::GlobalStateHash};
...
fn global_state_hash_benchmark(c: &mut Criterion) {
    {
        ...
        group.bench_function("accumulate_digests", |b| b.iter(|| state_hash.insert_all(&digests)));
    }

    {
        ...
        group.bench_function("sum_global_state_hashes", |b| b.iter(|| state_hash.union(&point)));
        group.bench_function("serialize_global_state_hashes", |b| b.iter(|| bcs::to_bytes(&state_hash).unwrap()));
