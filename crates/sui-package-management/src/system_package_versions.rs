...
pub fn latest_system_packages() -> &'static SystemPackagesVersion {
    VERSION_TABLE.last_key_value().expect("known system package version table should be nonempty").1
}
...
pub fn system_packages_for_protocol(
    ...
    let (protocol, system_packages) =
        VERSION_TABLE.range(..= version).next_back().context(format!("Unrecognized protocol version {version:?}"))?;
...
fn test_exact_version() {
    ...
    assert_eq!(system_packages.git_revision, "f5d26f1b3ae89f68cb66f3a007e90065e5286905");
    assert_eq!(protocol, 4.into());
    assert!(system_packages.packages.iter().any(|p| p.package_name == "Sui"));
}
...
fn test_gap_version() {
    ...
    assert_eq!(system_packages_for_protocol(56.into()).unwrap(), system_packages_for_protocol(55.into()).unwrap(),);
    assert_eq!(system_packages_for_protocol(57.into()).unwrap(), system_packages_for_protocol(55.into()).unwrap(),);
    // version 58 is present though!
    assert_ne!(system_packages_for_protocol(58.into()).unwrap(), system_packages_for_protocol(55.into()).unwrap(),);
}

#[test]
/// we get the correct hash for the latest known protocol version
fn test_version_latest() {
    assert_eq!(system_packages_for_protocol(ProtocolVersion::MAX).unwrap().0, latest_system_packages());
    assert_eq!(system_packages_for_protocol(ProtocolVersion::MAX + 1).unwrap().0, latest_system_packages());
}
