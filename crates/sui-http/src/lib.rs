...
impl Builder {
    ...
    pub fn tls_single_cert(
        ...
        let tls_config = rustls::ServerConfig::builder().with_no_client_auth().with_single_cert(certs, private_key)?;
    ...
    pub fn serve<A, S, ResponseBody>(self, addr: A, service: S) -> Result<ServerHandle<std::net::SocketAddr>, BoxError>
    where
    ...
    {
        let listener = listener::TcpListenerWithOptions::new(addr, self.config.tcp_nodelay, self.config.tcp_keepalive)?;

        Self::serve_with_listener(self, listener, service)
    }

    fn serve_with_listener<L, S, ResponseBody>(self, listener: L, service: S) -> Result<ServerHandle<L::Addr>, BoxError>
    where
        L: Listener,
        S: Service<Request<BoxBody>, Response = Response<ResponseBody>, Error: Into<BoxError>, Future: Send>
            + Clone
        ...
        ResponseBody: http_body::Body<Data = bytes::Bytes, Error: Into<BoxError>> + Send + 'static,
    {
        ...
        let handle =
            ServerHandle(Arc::new(HandleInner { local_addr, connections, graceful_shutdown_token, watch_sender }));
...
impl<A> ServerHandle<A> {
    ...
    pub fn connections(&self) -> std::sync::RwLockReadGuard<'_, HashMap<ConnectionId, ConnectionInfo<A>>> {
    ...
    L: Listener,
{
    ...
    fn handle_connection(&mut self, io: ServerIo<L::Io>, remote_addr: L::Addr) {
        ...
        let connection_info = ConnectionInfo::new(remote_addr, io.peer_certs(), connection_shutdown_token.clone());
    ...
    async fn shutdown(mut self) {
        ...
        let graceful_shutdown = async { while self.connection_handlers.join_next().await.is_some() {} };
