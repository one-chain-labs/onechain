...
impl SaveAfterDurationPolicy {
    pub fn new(duration: tokio::time::Duration) -> Self {
        Self { duration, last_save_time: Arc::new(Mutex::new(HashMap::new())) }
    }
}
        ...
        match self {
            ...
            ProgressSavingPolicy::OutOfOrderSaveAfterDuration(policy) => {
                let mut next_to_fill = {
                    ...
                    (*next_to_fill_guard.entry(task_name.clone()).or_insert(Some(start_height))).unwrap()
                };
                let old_next_to_fill = next_to_fill;
                {
                    let mut seen_guard = policy.seen.lock().unwrap();
                    let seen = seen_guard.entry(task_name.clone()).or_insert(HashSet::new());
                ...
                if old_next_to_fill != next_to_fill {
                    policy.next_to_fill.lock().unwrap().insert(task_name.clone(), Some(next_to_fill));
                }

                let mut last_save_time_guard = policy.last_save_time.lock().unwrap();
                let last_save_time = last_save_time_guard.entry(task_name.clone()).or_insert(None);
...
mod tests {
    ...
    async fn test_save_after_duration_policy() {
        ...
        let mut policy = ProgressSavingPolicy::SaveAfterDuration(SaveAfterDurationPolicy::new(duration));
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[1]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[2]), Some(2));
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[3]), Some(3));

        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[4]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[5, 6]), Some(6));
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[8, 7]), Some(8));
    }
    ...
    async fn test_out_of_order_save_after_duration_policy() {
        let duration = tokio::time::Duration::from_millis(100);
        let mut policy =
            ProgressSavingPolicy::OutOfOrderSaveAfterDuration(OutOfOrderSaveAfterDurationPolicy::new(duration));

        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[0]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[1]), Some(1));
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[3]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[4]), Some(1));
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task1", 0, 100), &[2]), Some(4));

        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[0]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[1]), Some(1));
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[2]), Some(2));
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[3]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[4]), Some(4));

        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[6, 7, 8]), None);
        tokio::time::sleep(duration).await;
        assert_eq!(policy.cache_progress(&new_test_task("task2", 0, 100), &[5, 9]), Some(9));
    }
