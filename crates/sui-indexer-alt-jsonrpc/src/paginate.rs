...
impl<C: Cursor> Page<C> {
    ...
    pub(crate) fn from_params<E: From<Error> + std::error::Error>(
        ...
        let cursor = cursor.map(|c| C::decode(&c)).transpose().map_err(|e| invalid_params(E::from(e)))?;

        let limit = limit.unwrap_or(default_page_size);
        if limit > max_page_size {
            return Err(invalid_params(E::from(Error::ExceededMaxPageSize { requested: limit, max: max_page_size })));
        }

        Ok(Page { cursor, limit: limit as i64, descending: descending.unwrap_or(false) })
    }
}
