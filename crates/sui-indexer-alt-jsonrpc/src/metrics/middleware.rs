...
impl MetricsLayer {
    ...
    pub fn new(metrics: Arc<RpcMetrics>, methods: HashSet<String>, slow_request_threshold: Duration) -> Self {
        Self { metrics, methods: Arc::new(methods), slow_request_threshold }
    }
}
...
impl<S> Layer<S> for MetricsLayer {
    ...
    fn layer(&self, inner: S) -> Self::Service {
        MetricsService { layer: self.clone(), inner }
    }
}
    ...
    S: RpcServiceT<'a>,
{
    ...
    fn call(&self, request: Request<'a>) -> Self::Future {
        let method =
            if self.layer.methods.contains(request.method_name()) { request.method.clone() } else { "<UNKNOWN>".into() };

        self.layer.metrics.requests_received.with_label_values(&[method.as_ref()]).inc();

        let timer = self.layer.metrics.request_latency.with_label_values(&[method.as_ref()]).start_timer();
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if let Some(code) = resp.as_error_code() {
            ...
            if code == INTERNAL_ERROR_CODE {
                error!(method, params, code, response, elapsed_ms, "Request failed with internal error");
            } else if tracing::enabled!(tracing::Level::DEBUG) {
                debug!(method, params, code, response, elapsed_ms, "Request failed with non-internal error");
            } else {
    ...
    fn drop(self: Pin<&mut Self>) {
        ...
        if let Some(metrics) = this.metrics.take() {
            ...
            if elapsed_ms > this.slow_request_threshold.as_millis() as f64 {
                let params = this.params.as_ref().map(|p| p.get()).unwrap_or("[]");
                warn!(method, params, elapsed_ms, "Cancelled request took a long time");
            }
        }
    }
}
