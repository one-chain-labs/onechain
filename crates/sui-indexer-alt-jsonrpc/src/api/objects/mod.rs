...
impl ObjectsApiServer for Objects {
    async fn get_object(
        ...
        Ok(response::live_object(ctx, object_id, &options)
            .await
            .with_internal_context(|| format!("Failed to get object {object_id} at latest version"))?)
    }
    ...
    async fn multi_get_objects(
        ...
        Ok(future::join_all(obj_futures)
            ...
            .into_iter()
            .zip(object_ids)
            .map(|(r, o)| r.with_internal_context(|| format!("Failed to get object {o} at latest version")))
    ...
    async fn try_get_past_object(
        ...
        Ok(response::past_object(ctx, object_id, version, &options)
            .await
            .with_internal_context(|| format!("Failed to get object {object_id} at version {}", version.value()))?)
    }
    ...
    async fn try_multi_get_past_objects(
        ...
        let obj_futures =
            past_objects.iter().map(|obj| response::past_object(ctx, obj.object_id, obj.version, &options));
...
impl QueryObjectsApiServer for QueryObjects {
    async fn get_owned_objects(
        ...
        let Page { data: object_ids, next_cursor, has_next_page } =
        ...
        let data = future::join_all(obj_futures)
            ...
            .into_iter()
            .zip(object_ids)
            .map(|(r, id)| r.with_internal_context(|| format!("Failed to get object {id} at latest version")))
            .collect::<Result<Vec<_>, _>>()?;
