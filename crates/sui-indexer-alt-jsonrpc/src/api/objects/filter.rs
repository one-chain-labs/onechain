...
enum RawFilter {
    ...
    Type { package: Vec<u8>, module: Option<String>, name: Option<String>, instantiation: Option<Vec<u8>> },
}
    ...
    fn to_raw(&self, ctx: &Context) -> Result<RawFilter, RpcError<Error>> {
        ...
        fn convert(
            ...
            if !matches!(filter, F::MatchNone(_)) {
                if *type_filters == 0 {
                    return Err(invalid_params(Error::FilterTooBig { max: max_type_filters }));
                } else {
                    *type_filters -= 1;
                }
            }
            ...
            Ok(match filter {
                ...
                F::Package(object_id) => {
                    R::Type { package: object_id.to_vec(), module: None, name: None, instantiation: None }
                }
...
impl RawFilter {
    ...
    fn matches(&self, info: &StoredObjInfo) -> bool {
        ...
        let (package_filter, module_filter, name_filter, instantiation_filter) = match self {
            ...
            R::Type { package, module, name, instantiation } => {
                (package, module.as_ref(), name.as_ref(), instantiation.as_ref())
            }
        };
...
pub(super) async fn owned_objects(
    ...
    match filter {
        Some(f) if f.is_compound() => by_sequential_scan(ctx, owner, &f.to_raw(ctx)?, cursor, limit).await,
        filter => by_type_indices(ctx, owner, StoredOwnerKind::Address, filter, cursor, limit).await,
    }
}
...
async fn by_sequential_scan(
    ...
    let page: Page<Cursor> = Page::from_params(config.default_page_size, config.max_page_size, cursor, limit, None)?;
    ...
    loop {
        ...
        cursor =
            Some(ObjectCursor { object_id: last.object_id.clone(), cp_sequence_number: last.cp_sequence_number as u64 });
    }

    ctx.metrics().owned_objects_filter_scans.observe(scans as f64);
    ...
    let next_cursor = results
        .last()
        .map(|(o, c)| BcsCursor(ObjectCursor { object_id: o.clone(), cp_sequence_number: *c }).encode())
        .transpose()
    ...
    Ok(PageResponse { data, has_next_page, next_cursor })
}
...
async fn owned_obj_info(
    ...
    let mut query = candidates
        .select(candidates!(object_id, cp_sequence_number, owner_kind, owner_id, package, module, name, instantiation,))
        .left_join(
...
async fn by_type_indices(
    ...
    let next_cursor = results
        .last()
        .map(|(o, c)| BcsCursor(ObjectCursor { object_id: o.clone(), cp_sequence_number: *c as u64 }).encode())
        .transpose()
        .context("Failed to encode next cursor")?;
