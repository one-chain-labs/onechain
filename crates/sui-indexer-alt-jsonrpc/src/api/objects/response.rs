...
pub(super) async fn live_object(
    ...
    let Some(object) = load_live(ctx, object_id).await.context("Failed to load latest object")? else {
        return Ok(SuiObjectResponse::new_with_error(SuiObjectResponseError::NotExists { object_id }));
    };

    Ok(SuiObjectResponse::new_with_data(object_data_with_options(ctx, object, options).await?))
}
...
pub(super) async fn past_object(
    ...
    let Some(object) =
        ctx.kv_loader().load_one_object(object_id, version.value()).await.context("Failed to load object from store")?
    else {
        return Ok(SuiPastObjectResponse::VersionNotFound(object_id, version));
    };

    Ok(SuiPastObjectResponse::VersionFound(object_data_with_options(ctx, object, options).await?))
}
...
pub(crate) async fn object_data_with_options(
    ...
    let previous_transaction = options.show_previous_transaction.then(|| object.previous_transaction);

    let storage_rebate = options.show_storage_rebate.then(|| object.storage_rebate);

    let content: OptionFuture<_> = options.show_content.then(|| object_data::<SuiParsedData>(ctx, &object)).into();

    let bcs: OptionFuture<_> = options.show_bcs.then(|| object_data::<SuiRawData>(ctx, &object)).into();
    ...
    let content = content.transpose().internal_context("Failed to deserialize object content")?;

    let bcs = bcs.transpose().internal_context("Failed to deserialize object to BCS")?;
...
async fn object_data<D: SuiData>(ctx: &Context, object: &Object) -> Result<D, RpcError> {
    Ok(match object.data.clone() {
        ...
        Data::Move(move_object) => {
            ...
            let MoveTypeLayout::Struct(layout) =
                ctx.package_resolver().type_layout(type_.clone()).await.with_context(|| {
                    format!("Failed to resolve type layout for {}", type_.to_canonical_display(/*with_prefix */ true))
                })?
            else {
                rpc_bail!("Type {} is not a struct", type_.to_canonical_display(/*with_prefix */ true));
            };
...
async fn display(ctx: &Context, object: &Object) -> DisplayFieldsResponse {
    let fields = match display_fields(ctx, object).await {
        ...
        Err(e) => {
            return DisplayFieldsResponse {
                data: None,
                error: Some(SuiObjectResponseError::DisplayError { error: format!("{e:#}") }),
            };
        }
    };
    ...
    DisplayFieldsResponse {
        ...
        error: (!field_errors.is_empty()).then_some(SuiObjectResponseError::DisplayError { error: field_errors }),
    }
}
...
async fn display_fields(ctx: &Context, object: &Object) -> anyhow::Result<BTreeMap<String, anyhow::Result<String>>> {
    ...
    let Some(stored) = display.context("Failed to load Display format")? else {
        bail!("Display format not found for {}", type_.to_canonical_display(/*with_prefix */ true));
    };
