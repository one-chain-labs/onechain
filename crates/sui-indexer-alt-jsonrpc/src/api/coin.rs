...
use crate::{
    context::Context,
    data::{objects::load_latest, singleton_object::load_singleton_object_id},
...
impl CoinsApiServer for Coins {
    ...
    async fn get_all_balances(&self, owner: SuiAddress) -> RpcResult<Vec<Balance>> {
        ...
        let coin_futures = coin_ids.data.iter().map(|id| object_with_coin_data(ctx, *id));
        let coins = future::join_all(coin_futures)
            .await
            .into_iter()
            .zip(coin_ids.data)
...
async fn coin_metadata_object_id(ctx: &Context, coin_type: String) -> Result<Option<ObjectID>, RpcError<Error>> {
    let coin_type_param =
        StructTag::from_str(&coin_type).map_err(|e| invalid_params(Error::BadType(coin_type.clone(), e)))?;

    let full_coin_type = sui_types::coin::CoinMetadata::type_(coin_type_param);

    Ok(load_singleton_object_id(ctx.pg_reader(), full_coin_type).await.context("Failed to load singleton object id")?)
}
...
async fn coin_metadata_response(ctx: &Context, id: ObjectID) -> Result<SuiCoinMetadata, RpcError<Error>> {
    let object = load_latest(ctx, id).await?.context("Latest object not found")?;

    let coin_metadata = object.try_into().context("Failed to convert object to coin metadata")?;
    Ok(coin_metadata)
}

async fn object_with_coin_data(ctx: &Context, id: ObjectID) -> Result<(Object, String, u64), RpcError<Error>> {
    let object = load_latest(ctx, id).await?.ok_or_else(|| anyhow::anyhow!("Failed to load latest object {}", id))?;
