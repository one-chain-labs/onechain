...
use sui_indexer_alt_schema::{objects::StoredCoinOwnerKind, schema::coin_balance_buckets};
...
impl CoinsApiServer for Coins {
    async fn get_coins(
        ...
        let coin_type_tag = if let Some(coin_type) = coin_type {
            sui_types::parse_sui_type_tag(&coin_type).map_err(|e| invalid_params(Error::BadType(coin_type, e)))?
        } else {
            GAS::type_tag()
        };
        ...
        let page: Page<Cursor> =
            Page::from_params::<Error>(config.default_page_size, config.max_page_size, cursor, limit, None)?;
...
impl DelegationCoinsApiServer for DelegationCoins {
    async fn get_all_balances(&self, owner: SuiAddress) -> RpcResult<Vec<Balance>> {
        ...
        client.get_all_balances(owner).await.map_err(client_error_to_error_object)
    }

    async fn get_balance(&self, owner: SuiAddress, coin_type: Option<String>) -> RpcResult<Balance> {
        let Self(client) = self;

        client.get_balance(owner, coin_type).await.map_err(client_error_to_error_object)
    }
}
...
async fn filter_coins(
    ...
    let mut conn = ctx.pg_reader().connect().await.context("Failed to connect to database")?;

    // We use two aliases of coin_balance_buckets to make the query more readable.
    let (candidates, newer) = diesel::alias!(coin_balance_buckets as candidates, coin_balance_buckets as newer);
    ...
    if let Some(coin_type_tag) = coin_type_tag {
        let serialized_coin_type = bcs::to_bytes(&coin_type_tag).context("Failed to serialize coin type tag")?;
    ...
    let mut buckets: Vec<(Vec<u8>, i64, i16)> = conn.results(query).await.context("Failed to query coins")?;
    ...
    Ok(PageResponse { data: ids, next_cursor, has_next_page })
}
...
async fn coin_response(ctx: &Context, id: ObjectID) -> Result<Coin, RpcError<Error>> {
    ...
    Ok(Coin { coin_type, coin_object_id, version, digest, balance, previous_transaction })
}

async fn coin_registry_response(ctx: &Context, coin_type: &str) -> Result<Option<SuiCoinMetadata>, RpcError<Error>> {
    let coin_type = TypeTag::from_str(coin_type).map_err(|e| invalid_params(Error::BadType(coin_type.to_owned(), e)))?;

    let currency_id =
        Currency::derive_object_id(coin_type).context("Failed to derive object id for coin registry Currency")?;

    let Some(object) = load_live(ctx, currency_id).await.context("Failed to load Currency object")? else {
        return Ok(None);
    };
    ...
    let currency: Currency = bcs::from_bytes(move_object.contents()).context("Failed to parse Currency object")?;

    Ok(Some(currency.into()))
}

async fn coin_metadata_response(ctx: &Context, coin_type: &str) -> Result<Option<SuiCoinMetadata>, RpcError<Error>> {
    let coin_type =
        StructTag::from_str(coin_type).map_err(|e| invalid_params(Error::BadType(coin_type.to_owned(), e)))?;

    let Some(stored) =
        ctx.pg_loader().load_one(CoinMetadataKey(coin_type)).await.context("Failed to load info for CoinMetadata")?
    else {
        return Ok(None);
    };
    ...
    let Some(object) = load_live(ctx, id).await.context("Failed to load latest version of CoinMetadata")? else {
        return Ok(None);
    };
...
async fn object_with_coin_data(ctx: &Context, id: ObjectID) -> Result<(Object, String, u64), RpcError<Error>> {
    let object = load_live(ctx, id).await?.with_context(|| format!("Failed to load latest object {id}"))?;

    let coin = object.as_coin_maybe().context("Object is expected to be a coin")?;
