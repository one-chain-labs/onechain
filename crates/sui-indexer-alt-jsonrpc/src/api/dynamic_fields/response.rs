...
use sui_json_rpc_types::{
    BcsName,
    DynamicFieldInfo as DynamicFieldInfoResponse,
    SuiMoveValue,
...
pub(super) async fn dynamic_field_object(
    ...
    let (df, dof) =
        try_join!(df, dof).with_context(|| format!("Failed to fetch dynamic field on {parent_object_id}"))?;

    let Some(object) = df.or(dof) else {
        return Ok(SuiObjectResponse::new_with_error(SuiObjectResponseError::DynamicFieldNotFound { parent_object_id }));
    };
    ...
    Ok(SuiObjectResponse::new_with_data(
        objects::response::object_data_with_options(ctx, object, &options).await.map_err(|e| match e {
...
pub(super) async fn dynamic_field_info(
    ...
    let bcs_name = BcsName::Base64 { bcs_name: field.name_bytes.to_owned() };
    ...
    let value_metadata = field.value_metadata().context("Failed to extract dynamic field value metadata")?;
...
async fn resolve_type(ctx: &Context, type_: &TypeTag) -> Result<MoveTypeLayout, RpcError<Error>> {
    ctx.package_resolver().type_layout(type_.clone()).await.map_err(|e| {
        ...
        match &e {
            ...
            PRE::TooManyTypeNodes(_, _) | PRE::TooManyTypeParams(_, _) | PRE::TypeParamNesting(_, _) => {
                invalid_params(Error::BadType(type_.clone(), e))
            }
            ...
            | PRE::ValueNesting(_) => RpcError::from(anyhow!(e).context("Failed to resolve type layout")),
        }
    })
}
...
async fn load_df(
    ...
    let id = derive_dynamic_field_id(parent_id, type_, value).context("Failed to derive dynamic field ID")?;

    Ok(load_live(ctx, id).await.context("Failed to load dynamic field")?)
}
...
async fn load_dof(
    ...
    let id = derive_dynamic_field_id(parent_id, &wrapper, name).context("Failed to derive dynamic object field ID")?;

    let Some(object) = load_live(ctx, id).await.context("Failed to load dynamic object field")? else {
        return Ok(None);
    };
    ...
    Ok(load_live(ctx, value).await.context("Failed to load dynamic field object")?)
}
