...
pub(super) async fn resolved_address(ctx: &Context, name: &str) -> Result<Option<SuiAddress>, RpcError<Error>> {
    use Error as E;

    let domain: Domain = name.parse().map_err(|e| invalid_params(E::NameService(e)))?;
    ...
    let parent_object: OptionFuture<_> = domain.is_subdomain().then(|| load_live(ctx, parent_record_id)).into();
    ...
    // parent's.
    let (timestamp_ms, domain_object, parent_object) = join!(latest_timestamp_ms(ctx), domain_object, parent_object);
    ...
    let domain_record = NameRecord::try_from(domain_object).context("Failed to deserialize domain record")?;
    ...
    if !domain_record.is_leaf_record() {
        return if !domain_record.is_node_expired(timestamp_ms) {
            ...
            return Err(invalid_params(E::NameService(NameServiceError::NameExpired)));
        };
    }
    ...
    let Some(parent_object) = parent_object.transpose().context("Failed to fetch parent record")?.flatten() else {
    ...
    let parent_record = NameRecord::try_from(parent_object).context("Failed to deserialize parent record")?;

    if parent_record.is_valid_leaf_parent(&domain_record) && !parent_record.is_node_expired(timestamp_ms) {
        Ok(domain_record.target_address)
    } else {
        Err(invalid_params(E::NameService(NameServiceError::NameExpired)))
    }
}
...
pub(super) async fn resolved_name(ctx: &Context, address: SuiAddress) -> Result<Option<String>, RpcError<Error>> {
    let config = &ctx.config().name_service;

    let reverse_record_id = config.reverse_record_field_id(address.as_ref());
    let Some(reverse_record_object) =
        load_live(ctx, reverse_record_id).await.context("Failed to fetch reverse record")?
    else {
        return Ok(None);
    };

    let reverse_record: Field<SuiAddress, Domain> = bcs::from_bytes(
        reverse_record_object.data.try_as_move().context("Reverse record not a Move object")?.contents(),
    )
...
async fn latest_timestamp_ms(ctx: &Context) -> Result<u64, RpcError<Error>> {
    ...
    let query = w::watermarks.select(w::timestamp_ms_hi_inclusive).filter(w::pipeline.eq("obj_info"));

    let timestamp_ms: i64 = conn.first(query).await.context("Failed to fetch latest timestamp")?;
