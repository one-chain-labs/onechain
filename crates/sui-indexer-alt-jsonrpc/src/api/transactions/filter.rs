...
use sui_indexer_alt_schema::{
    schema::{tx_affected_addresses, tx_affected_objects, tx_calls, tx_digests},
    transactions::StoredTxDigest,
};
...
pub(super) async fn transactions(
    ...
    let page: Page<Cursor> =
        Page::from_params(config.default_page_size, config.max_page_size, cursor, limit, descending_order)?;
    ...
    match filter {
        ...
        Some(F::MoveFunction { package, module, function }) => {
...
async fn all_transactions(ctx: &Context, page: &Page<Cursor>) -> Result<Digests, RpcError<Error>> {
    ...
    let query = d::tx_digests.select((d::tx_sequence_number, d::tx_digest)).into_boxed();
...
async fn by_checkpoint(ctx: &Context, page: &Page<Cursor>, checkpoint: u64) -> Result<Digests, RpcError<Error>> {
    let Some((summary, contents, _)) =
        ctx.kv_loader().load_one_checkpoint(checkpoint).await.context("Failed to load checkpoint")?
    else {
    ...
    let Page { cursor, limit, descending } = page;
    ...
    if *descending {
        ...
        pg_lo = cursor.as_ref().map(|c| c.0.saturating_add(1)).map_or(cp_lo, |c| c.max(cp_lo));
    ...
    for tx in pg_lo .. pg_hi {
        ...
        let digest = digests.get(ix).context("Transaction out of bounds in checkpoint")?.transaction.inner().to_vec();
...
async fn tx_calls(
    ...
    if let (None, Some(function)) = (module, function) {
        return Err(invalid_params(Error::MissingModule { function: function.clone() }));
    }

    let mut query = c::tx_calls.select(c::tx_sequence_number).filter(c::package.eq(package.as_slice())).into_boxed();
...
/// or wrapped it).
async fn tx_affected_objects(ctx: &Context, page: &Page<Cursor>, object: ObjectID) -> Result<Digests, RpcError<Error>> {
    use tx_affected_objects::dsl as o;

    let query =
        o::tx_affected_objects.select(o::tx_sequence_number).filter(o::affected.eq(object.as_slice())).into_boxed();
    ...
    let results: Vec<i64> = ctx
        ...
        .await
        .context("Failed to connect to the database")?
        .results(paginate(page, "tx_affected_objects", o::tx_sequence_number, query))
        .await
        .context("Failed to fetch transaction sequence numbers")?;
...
async fn tx_affected_addresses(
    ...
    let mut query =
        a::tx_affected_addresses.select(a::tx_sequence_number).filter(a::affected.eq(to.to_inner())).into_boxed();
    ...
    let results: Vec<i64> = ctx
        ...
        .await
        .context("Failed to connect to the database")?
        .results(paginate(page, "tx_affected_addresses", a::tx_sequence_number, query))
        .await
        .context("Failed to fetch transaction sequence numbers")?;
...
async fn from_sequence_numbers(ctx: &Context, limit: i64, mut rows: Vec<i64>) -> Result<Digests, RpcError<Error>> {
    ...
    let next_cursor =
        rows.last().map(|last| JsonCursor(*last).encode()).transpose().context("Failed to encode next cursor")?;

    // Get digests from the table and retry any of the digests that are not found.
    let config = &ctx.config().transactions;
    let max_retries = config.tx_retry_count;
    let mut retry_interval = tokio::time::interval(Duration::from_millis(config.tx_retry_interval_ms));
    ...
    for _ in 0 ..= max_retries {
        ...
        digests.extend(ctx.pg_loader().load_many(keys.clone()).await.context("Failed to load transaction digests")?);
        ...
        retries += 1;
        ctx.metrics().read_retries.with_label_values(&["tx_digest"]).inc();
    }

    ctx.metrics().read_retries_per_request.with_label_values(&["tx_digest"]).observe(retries as f64);
    ...
    for seq in rows {
        ...
        let digest = TransactionDigest::try_from(bytes).context("Failed to deserialize transaction digests")?;

        data.push(digest);
    }

    Ok(PageResponse { data, next_cursor, has_next_page })
}
...
fn from_digests(limit: i64, mut rows: Vec<(i64, Vec<u8>)>) -> Result<Digests, RpcError<Error>> {
    ...
    let next_cursor =
        rows.last().map(|(last, _)| JsonCursor(*last).encode()).transpose().context("Failed to encode next cursor")?;
