...
use sui_json_rpc_types::{
    ...
    ObjectChange as SuiObjectChange,
    SuiEvent,
    SuiTransactionBlock,
...
pub(super) async fn transaction(
    ...
    let stored_bc: OptionFuture<_> =
        options.show_balance_changes.then(|| ctx.pg_loader().load_one(TxBalanceChangeKey(digest))).into();

    let (tx, stored_bc) = join!(tx, stored_bc);

    let tx =
        tx.context("Failed to fetch transaction from store")?.ok_or_else(|| invalid_params(Error::NotFound(digest)))?;
...
async fn input(ctx: &Context, tx: &TransactionContents) -> Result<SuiTransactionBlock, RpcError<Error>> {
...
fn effects(tx: &TransactionContents) -> Result<SuiTransactionBlockEffects, RpcError<Error>> {
    ...
    Ok(effects.try_into().context("Failed to convert Effects into response")?)
}
...
async fn events(
    ...
    for (ix, event) in events.into_iter().enumerate() {
        let layout = match ctx.package_resolver().type_layout(event.type_.clone().into()).await.with_context(|| {
            format!("Failed to resolve layout for {}", event.type_.to_canonical_display(/* with_prefix */ true))
        })? {
...
fn balance_changes(balance_changes: StoredTxBalanceChange) -> Result<Vec<SuiBalanceChange>, RpcError<Error>> {
    let balance_changes: Vec<BalanceChange> =
        bcs::from_bytes(&balance_changes.balance_changes).context("Failed to deserialize BalanceChanges")?;
    let mut response = Vec::with_capacity(balance_changes.len());

    for BalanceChange::V1 { owner, coin_type, amount } in balance_changes {
        let coin_type = TypeTag::from_str(&coin_type).with_context(|| format!("Invalid coin type: {coin_type:?}"))?;

        response.push(SuiBalanceChange { owner, coin_type, amount });
    }
...
async fn object_changes(
    ...
    let objects = ctx.kv_loader().load_many_objects(keys).await.context("Failed to fetch object contents")?;
    ...
    let fetch_object = |id: ObjectID,
     ...
     -> Result<Option<(Object, ObjectDigest)>, RpcError<Error>> {
        ...
        let o =
            objects.get(&VersionedObjectKey(id, v)).ok_or_else(|| invalid_params(Error::PrunedObject(digest, id, v)))?;
    ...
    for change in native_changes {
        ...
        changes.push(match (id_operation, input, output) {
            ...
            (ID::Created, _, Some((o, d))) => SuiObjectChange::Created {
                ...
                owner: o.owner().clone(),
                object_type: o.struct_tag().with_context(|| format!("No type for object {object_id}"))?,
                object_id,
                version: o.version(),
                digest: d,
            },

            (ID::None, Some((i, _)), Some((o, od))) if i.owner() != o.owner() => SuiObjectChange::Transferred {
                sender: tx_data.sender(),
            ...
            (ID::None, Some((i, _)), None) => SuiObjectChange::Wrapped {
                sender: tx_data.sender(),
                object_type: i.struct_tag().with_context(|| format!("No type for object {object_id}"))?,
                object_id,
