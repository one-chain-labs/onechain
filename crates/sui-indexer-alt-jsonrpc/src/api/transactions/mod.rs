...
impl QueryTransactionsApiServer for QueryTransactions {
    async fn query_transaction_blocks(
        ...
        let Page { data: digests, next_cursor, has_next_page } =
            filter::transactions(ctx, &query.filter, cursor.clone(), limit, descending_order).await?;
        ...
        let tx_futures = digests.iter().map(|d| {
            async {
                ...
                let mut interval = tokio::time::interval(std::time::Duration::from_millis(config.tx_retry_interval_ms));
                ...
                for _ in 0 .. config.tx_retry_count {
                    ...
                    if let Err(RpcError::InvalidParams(_e @ (Error::BalanceChangesNotFound(_) | Error::NotFound(_)))) =
                        tx
                    {
                        interval.tick().await;
                        retries += 1;
                        tx = response::transaction(ctx, *d, &options).await;
                        ctx.metrics().read_retries.with_label_values(&["tx_response"]).inc();
                    } else {
                        break;
                    }
                }

                ctx.metrics().read_retries_per_request.with_label_values(&["tx_response"]).observe(retries as f64);
                tx
            }
        });
        ...
        Ok(Page { data, next_cursor: next_cursor.or(cursor), has_next_page })
    }
}
