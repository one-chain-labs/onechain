...
pub(super) async fn function(
    ...
    let sig = ctx.package_resolver().function_signature(*package, module, name).await.map_err(|e| {
        ...
        match &e {
            ...
            PRE::TooManyTypeNodes(_, _) | PRE::TooManyTypeParams(_, _) | PRE::TypeParamNesting(_, _) => {
                invalid_params(E::ResolutionLimit(e))
            }
...
fn ability_set(a: AbilitySet) -> SuiMoveAbilitySet {
    SuiMoveAbilitySet { abilities: a.into_iter().map(ability).collect() }
}
