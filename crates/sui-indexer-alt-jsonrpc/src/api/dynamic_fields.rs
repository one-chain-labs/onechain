...
async fn dynamic_field_object_response(
    ...
    let layout = ctx.package_resolver().type_layout(name.type_.clone()).await.map_err(|e| {
        ...
        match &e {
            ...
            | PRE::TypeArityMismatch(_, _) => invalid_params(Error::BadType(name.type_.clone(), e)),
...
async fn load_df(
    ...
    Ok(load_latest(ctx, id).await.context("Failed to load dynamic field")?)
}
...
async fn load_dof(
    ...
    let Some(object) = load_latest(ctx, id).await.context("Failed to load dynamic object field")? else {
        return Ok(None);
    };
    ...
    Ok(load_latest(ctx, value).await.context("Failed to load dynamic field object")?)
}
