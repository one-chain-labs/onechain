...
impl ObjectsLayer {
    pub fn finish(self, base: ObjectsConfig) -> ObjectsConfig {
        ObjectsConfig {
            max_multi_get_objects: self.max_multi_get_objects.unwrap_or(base.max_multi_get_objects),
            ...
            obj_retry_interval_ms: self.obj_retry_interval_ms.unwrap_or(base.obj_retry_interval_ms),
        }
    }
}
...
impl TransactionsLayer {
    pub fn finish(self, base: TransactionsConfig) -> TransactionsConfig {
        TransactionsConfig {
            ...
            tx_retry_interval_ms: self.tx_retry_interval_ms.unwrap_or(base.tx_retry_interval_ms),
        }
    }
}
...
impl NodeConfig {
    pub fn client(&self, fullnode_rpc_url: url::Url) -> anyhow::Result<HttpClient> {
        ...
        headers.insert(CLIENT_SDK_TYPE_HEADER, HeaderValue::from_str(&self.header_value)?);
...
impl Default for DynamicFieldsConfig {
    fn default() -> Self {
        Self { default_page_size: 50, max_page_size: 100 }
    }
}

impl Default for TransactionsConfig {
    fn default() -> Self {
        Self { default_page_size: 50, max_page_size: 100, tx_retry_count: 5, tx_retry_interval_ms: 100 }
    }
}
...
impl From<DynamicFieldsConfig> for DynamicFieldsLayer {
    fn from(config: DynamicFieldsConfig) -> Self {
        Self { default_page_size: Some(config.default_page_size), max_page_size: Some(config.max_page_size) }
    }
}
...
impl From<NodeConfig> for NodeLayer {
    fn from(config: NodeConfig) -> Self {
        Self { header_value: Some(config.header_value), max_request_size: Some(config.max_request_size) }
    }
}
