...
pub(crate) async fn load_singleton_object_id(
    ...
    let instantiation = bcs::to_bytes(&struct_tag.type_params).map_err(|e| Arc::new(Error::Serde(e.into())))?;
    ...
    stored.map(|b| ObjectID::from_bytes(&b)).transpose().map_err(|e| Arc::new(Error::Serde(e.into())))
}
