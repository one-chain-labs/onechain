...
impl KvLoader {
    ...
    pub(crate) async fn load_one_object(&self, id: ObjectID, version: u64) -> Result<Option<Object>, Arc<Error>> {
...
impl TransactionContents {
    pub(crate) fn data(&self) -> anyhow::Result<TransactionData> {
        match self {
            Self::Pg(stored) => {
                let data: TransactionData = bcs::from_bytes(&stored.raw_transaction)
                    .map_err(|e| anyhow::anyhow!("Failed to deserialize transaction data: {}", e))?;
    ...
    pub(crate) fn digest(&self) -> anyhow::Result<TransactionDigest> {
        match self {
            Self::Pg(stored) => {
                let digest = TransactionDigest::try_from(stored.tx_digest.clone())
                    .map_err(|e| anyhow::anyhow!("Failed to deserialize transaction digest: {}", e))?;
    ...
    pub(crate) fn raw_effects(&self) -> anyhow::Result<Vec<u8>> {
        match self {
            ...
            Self::Bigtable(kv) => {
                bcs::to_bytes(&kv.effects).map_err(|e| anyhow::anyhow!("Failed to serialize effects: {}", e))
            }
        }
    }
