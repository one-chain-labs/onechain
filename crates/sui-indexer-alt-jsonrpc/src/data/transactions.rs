...
impl Loader<TransactionKey> for PgReader {
    ...
    async fn load(&self, keys: &[TransactionKey]) -> Result<HashMap<TransactionKey, Self::Value>, Self::Error> {
        ...
        let transactions: Vec<StoredTransaction> =
            conn.results(t::kv_transactions.filter(t::tx_digest.eq_any(digests))).await.map_err(Arc::new)?;
...
impl Loader<TransactionKey> for BigtableReader {
    ...
    async fn load(&self, keys: &[TransactionKey]) -> Result<HashMap<TransactionKey, Self::Value>, Self::Error> {
        ...
        let transactions =
            self.0.clone().get_transactions(&digests).await.map_err(|e| Arc::new(Error::BigtableRead(e)))?;

        Ok(transactions.into_iter().map(|t| (TransactionKey(*t.transaction.digest()), t)).collect())
    }
}
