...
impl SystemPackageTask {
    pub(crate) fn new(context: Context, args: SystemPackageTaskArgs, cancel: CancellationToken) -> Self {
        Self { context, interval: args.epoch_polling_interval(), cancel }
    }
    ...
    pub(crate) fn run(self) -> JoinHandle<()> {
        tokio::spawn(async move {
            let Self { context, interval, cancel } = self;
...
impl Default for SystemPackageTaskArgs {
    fn default() -> Self {
        Self { epoch_polling_interval_ms: 10_000 }
    }
}
