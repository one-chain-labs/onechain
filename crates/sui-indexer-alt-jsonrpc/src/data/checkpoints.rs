...
impl Loader<CheckpointKey> for PgReader {
    ...
    async fn load(&self, keys: &[CheckpointKey]) -> Result<HashMap<CheckpointKey, Self::Value>, Self::Error> {
        ...
        let checkpoints: Vec<StoredCheckpoint> =
            conn.results(c::kv_checkpoints.filter(c::sequence_number.eq_any(seqs))).await.map_err(Arc::new)?;
...
impl Loader<CheckpointKey> for BigtableReader {
    ...
    async fn load(&self, keys: &[CheckpointKey]) -> Result<HashMap<CheckpointKey, Self::Value>, Self::Error> {
        ...
        let checkpoints =
            self.0.clone().get_checkpoints(&checkpoint_keys).await.map_err(|e| Arc::new(Error::BigtableRead(e)))?;
