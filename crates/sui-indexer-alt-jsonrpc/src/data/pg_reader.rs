...
use diesel::{
    ...
    query_dsl::{methods::LimitDsl, CompatibleType},
    OptionalExtension,
};
...
impl PgReader {
    pub(crate) async fn new(db_args: db::DbArgs, metrics: Arc<RpcMetrics>, registry: &Registry) -> Result<Self, Error> {
        let db = db::Db::for_read(db_args).await.map_err(Error::PgCreate)?;

        registry
            .register(Box::new(DbConnectionStatsCollector::new(Some("rpc_db"), db.clone())))
    ...
    pub(crate) async fn connect(&self) -> Result<Connection<'_>, Error> {
        Ok(Connection { conn: self.db.connect().await.map_err(Error::PgConnect)?, metrics: self.metrics.clone() })
    }
}
