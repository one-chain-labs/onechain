...
impl Loader<VersionedObjectKey> for PgReader {
    ...
    async fn load(&self, keys: &[VersionedObjectKey]) -> Result<HashMap<VersionedObjectKey, StoredObject>, Self::Error> {
        ...
        for VersionedObjectKey(id, version) in keys {
            query = query.or_filter(o::object_id.eq(id.into_bytes()).and(o::object_version.eq(*version as i64)));
        }
...
impl Loader<VersionedObjectKey> for BigtableReader {
    ...
    async fn load(&self, keys: &[VersionedObjectKey]) -> Result<HashMap<VersionedObjectKey, Object>, Self::Error> {
        if keys.is_empty() {
            return Ok(HashMap::new());
        }

        let object_keys: Vec<ObjectKey> = keys.iter().map(|key| ObjectKey(key.0, key.1.into())).collect();

        let objects = self.0.clone().get_objects(&object_keys).await.map_err(|e| Arc::new(Error::BigtableRead(e)))?;

        Ok(objects.into_iter().map(|o| (VersionedObjectKey(o.id(), o.version().into()), o)).collect())
    }
}
...
pub(crate) async fn load_latest(ctx: &Context, object_id: ObjectID) -> Result<Option<Object>, anyhow::Error> {
    let Some(latest_version) =
        ctx.pg_loader().load_one(LatestObjectVersionKey(object_id)).await.context("Failed to load latest version")?
    else {
        return Ok(None);
    };
...
pub(crate) async fn load_latest_deserialized<T: DeserializeOwned>(
    ...
    let object = load_latest(ctx, object_id).await?.context("No data found")?;
...
pub(crate) async fn load_live(ctx: &Context, object_id: ObjectID) -> Result<Option<Object>, anyhow::Error> {
    let Some(obj_info) =
        ctx.pg_loader().load_one(LatestObjectInfoKey(object_id)).await.context("Failed to fetch object info")?
    else {
        return Ok(None);
    };
    ...
    Ok(Some(load_latest(ctx, object_id).await?.context("Failed to find content for latest version of live object")?))
}
