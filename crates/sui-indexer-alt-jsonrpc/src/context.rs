...
impl Context {
    ...
    pub(crate) async fn new(
        ...
        let kv_loader = if let Some(instance_id) = bigtable_instance {
            let bigtable_reader =
                BigtableReader::new(instance_id, "indexer-alt-jsonrpc".to_owned(), bigtable_args, registry).await?;
        ...
        let package_resolver = Arc::new(Resolver::new_with_limits(store, config.package_resolver.clone()));

        Ok(Self { pg_reader, pg_loader, kv_loader, package_resolver, metrics, config: Arc::new(config) })
    }
