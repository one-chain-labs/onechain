...
impl<S> Layer<S> for TimeoutLayer {
    ...
    fn layer(&self, inner: S) -> Self::Service {
        TimeoutService { request_timeout: self.request_timeout, inner }
    }
}
    ...
    S: RpcServiceT<'a>,
{
    ...
    fn call(&self, request: Request<'a>) -> Self::Future {
        TimeoutFuture { request: request.clone(), inner: timeout(self.request_timeout, self.inner.call(request)) }
    }
}
    ...
    F: Future<Output = MethodResponse>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let Ok(resp) = resp else {
            ...
            let params = this.request.params.as_ref().map(|p| p.get()).unwrap_or("[]");
