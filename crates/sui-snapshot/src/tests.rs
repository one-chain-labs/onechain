...
async fn test_snapshot_basic() -> Result<(), anyhow::Error> {
    ...
    let local_store_config =
        ObjectStoreConfig { object_store: Some(ObjectStoreType::File), directory: Some(local), ..Default::default() };
    let remote_store_config =
        ObjectStoreConfig { object_store: Some(ObjectStoreType::File), directory: Some(remote), ..Default::default() };
...
async fn test_snapshot_empty_db() -> Result<(), anyhow::Error> {
    ...
    let remote_store_config =
        ObjectStoreConfig { object_store: Some(ObjectStoreType::File), directory: Some(remote), ..Default::default() };
    let include_wrapped_tombstone = !ProtocolConfig::get_for_max_version_UNSAFE().simplified_unwrap_then_delete();
    let snapshot_writer = StateSnapshotWriterV1::new(
        &local_store_config,
        &remote_store_config,
        FileCompression::Zstd,
...
async fn test_archive_epoch_if_needed() -> Result<(), anyhow::Error> {
    ...
    for (filename, content) in &test_files {
        ...
        sui_storage::object_store::util::put(&store, &file_path, bytes::Bytes::from(*content)).await?;
    }

    // Should archive epoch 60 (60 % 30 == 0)
    uploader.archive_epoch_if_needed(epoch).await?;
    for (filename, expected_content) in &test_files {
        let archive_path = object_store::path::Path::from(format!("archive/epoch_{}/{}", epoch, filename));
    ...
    assert!(objects.next().await.is_none(), "Should not archive epoch 61");
