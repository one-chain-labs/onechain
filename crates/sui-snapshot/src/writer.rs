...
impl LiveObjectSetWriterV1 {
    ...
    fn finalize(&mut self) -> Result<()> {
        ...
        let file_path = self.dir_path.join(format!("{}_{}.obj", self.bucket_num, self.current_part_num));
    ...
    fn finalize_ref(&mut self) -> Result<()> {
        ...
        let file_path = self.dir_path.join(format!("{}_{}.ref", self.bucket_num, self.current_part_num));
        let file_metadata = create_file_metadata(
            &file_path,
            self.file_compression,
            FileType::Reference,
    ...
    fn cut(&mut self) -> Result<()> {
        self.finalize()?;
        let (n, f) = Self::object_file(self.dir_path.clone(), self.bucket_num, self.current_part_num + 1)?;
        self.n = n;
    ...
    fn cut_reference_file(&mut self) -> Result<()> {
        self.finalize_ref()?;
        let f = Self::ref_file(self.dir_path.clone(), self.bucket_num, self.current_part_num + 1)?;
...
impl StateSnapshotWriterV1 {
    ...
    pub async fn new(
        ...
        let local_staging_dir = local_store_config.directory.as_ref().context("No local directory specified")?.clone();
        Ok(StateSnapshotWriterV1 {
            local_staging_dir,
            file_compression,
            remote_object_store,
    ...
    pub async fn write(
        ...
        self.write_internal(epoch, include_wrapped_tombstone, perpetual_db, root_state_hash).await
    }
    ...
    pub(crate) async fn write_internal(
        ...
        write_handler.await?.context(format!("Failed to write state snapshot for epoch: {}", &epoch))?;

        upload_handle.await?.context(format!("Failed to upload state snapshot for epoch: {}", &epoch))?;

        Self::sync_file_to_remote(local_staging_dir, manifest_file_path, local_object_store, remote_object_store)
            .await?;
        Ok(())
    }
    ...
    fn start_upload(
        ...
        let join_handle = tokio::spawn(async move {
            let results: Vec<Result<(), anyhow::Error>> = ReceiverStream::new(receiver)
                ...
                .collect()
                .await;
            results.into_iter().collect::<Result<Vec<()>, anyhow::Error>>()
        });
        Ok(join_handle)
    }
        ...
        F: Fn(&LiveObject) -> u32,
    {
        ...
        for object in perpetual_db.iter_live_object_set(include_wrapped_tombstone) {
            ...
            let writer = object_writers.get_mut(&bucket_num).context("Unexpected missing bucket writer")?;
            writer.write(&object)?;
        }
        assert_eq!(ECMHLiveObjectSetDigest::from(acc.digest()), root_state_hash, "Root state hash mismatch!");
        let mut files = vec![];
    ...
    fn manifest_file(&mut self, epoch: u64) -> Result<(File, PathBuf)> {
        let manifest_file_path =
            path_to_filesystem(self.local_staging_dir.clone(), &self.epoch_dir(epoch).child("MANIFEST"))?;
        let manifest_file_tmp_path =
            path_to_filesystem(self.local_staging_dir.clone(), &self.epoch_dir(epoch).child("MANIFEST.tmp"))?;
        ...
        let mut f = OpenOptions::new().append(true).open(manifest_file_path.clone())?;
    ...
    async fn setup_epoch_dir(&self, epoch: u64) -> Result<()> {
        ...
        delete_recursively(&epoch_dir, &self.remote_object_store, NonZeroUsize::new(self.concurrency).unwrap()).await?;
