...
pub async fn setup_indexer(
    ...
    let IndexerConfig { ingestion, committer, pruner, pipeline } = indexer_config;
    ...
    let mut indexer =
        Indexer::new(store, indexer_args, client_args, ingestion, metrics_prefix, registry, cancel.clone()).await?;
        ...
        ($handler:expr, $config:expr) => {
            if let Some(layer) = $config {
                indexer
                    .concurrent_pipeline(
                        $handler,
                        layer.finish(ConcurrentConfig { committer: committer.clone(), pruner: Some(pruner.clone()) })?,
                    )
                    .await?
            }
        };
