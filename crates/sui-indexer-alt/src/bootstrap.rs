...
pub async fn bootstrap(
    ...
    let BootstrapGenesis { stored_genesis, stored_epoch_start } = match bootstrap_genesis {
        ...
        None => {
            ...
            let Checkpoint { summary: checkpoint_summary, transactions, .. } = genesis_checkpoint.as_ref();

            let Some(genesis_transaction) =
                transactions.iter().find(|tx| matches!(tx.transaction.kind(), TransactionKind::Genesis(_)))
            else {
                bail!("Could not find Genesis transaction");
            };

            let output_objects: Vec<_> =
                genesis_transaction.output_objects(&genesis_checkpoint.object_set).cloned().collect();

            let sui_system_state =
                get_sui_system_state(&output_objects.as_slice()).context("Failed to get Genesis SystemState")?;
            ...
            let stored_epoch_start = StoredEpochStart {
                ...
                system_state: bcs::to_bytes(&sui_system_state).context("Failed to serialize SystemState")?,
            };

            BootstrapGenesis { stored_genesis, stored_epoch_start }
        }
    };
