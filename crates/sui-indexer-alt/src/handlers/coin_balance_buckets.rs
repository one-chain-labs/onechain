...
use sui_indexer_alt_schema::{
    objects::{StoredCoinBalanceBucket, StoredCoinBalanceBucketDeletionReference, StoredCoinOwnerKind},
...
impl Handler for CoinBalanceBuckets {
    async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> Result<usize> {
        let stored = values.iter().map(|v| v.try_into()).collect::<Result<Vec<StoredCoinBalanceBucket>>>()?;
    ...
    async fn prune<'a>(&self, from: u64, to_exclusive: u64, conn: &mut Connection<'a>) -> anyhow::Result<usize> {
        ...
        let CountResult { deleted_coins, deleted_refs } =
            diesel::sql_query(query).get_result::<CountResult>(conn).await?;
...
impl TryInto<StoredCoinBalanceBucket> for &ProcessedCoinBalanceBucket {
    ...
    fn try_into(self) -> Result<StoredCoinBalanceBucket> {
        match &self.change {
            CoinBalanceBucketChangeKind::Upsert { owner_kind, owner_id, coin_type, balance_bucket, created: _ } => {
...
mod tests {
    ...
    fn test_get_coin_owner() {
        ...
        assert_eq!(get_coin_owner(&addr_owned), Some((StoredCoinOwnerKind::Fastpath, addr1)));
    ...
    async fn test_process_coin_balance_buckets_new_sui_coin() {
        ...
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 2);
        assert!(values.iter().any(|v| matches!(v.change, CoinBalanceBucketChangeKind::Upsert {
    ...
    async fn test_process_coin_balance_buckets_new_other_coin() {
        ...
        builder = builder.start_transaction(0).create_coin_object(0, 0, 10, coin_type.clone()).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        assert_eq!(&values[0].change, &CoinBalanceBucketChangeKind::Upsert {
            owner_kind: StoredCoinOwnerKind::Fastpath,
            balance_bucket: 1,
    ...
    async fn test_process_coin_balance_buckets_balance_change() {
        ...
        builder = builder.start_transaction(0).transfer_coin_balance(0, 1, 1, 10).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        ...
        builder = builder.start_transaction(0).transfer_coin_balance(0, 2, 1, 1).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 2);
        ...
        assert_eq!(all_balance_buckets[0], StoredCoinBalanceBucket {
        ...
        assert_eq!(all_balance_buckets[1], StoredCoinBalanceBucket {
        ...
        assert_eq!(all_balance_buckets[2], StoredCoinBalanceBucket {
    ...
    async fn test_process_coin_balance_buckets_owner_change() {
        ...
        builder = builder.start_transaction(0).transfer_object(0, 1).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].change, CoinBalanceBucketChangeKind::Upsert {
    ...
    async fn test_process_coin_balance_buckets_object_owned() {
        ...
        builder =
            builder.start_transaction(0).change_object_owner(0, Owner::ObjectOwner(dbg_addr(1))).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].change, CoinBalanceBucketChangeKind::Delete);
    ...
    async fn test_process_coin_balance_buckets_wrap_and_prune_after_unwrap() {
        ...
        builder = builder.start_transaction(0).wrap_object(0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].change, CoinBalanceBucketChangeKind::Delete);
        ...
        builder = builder.start_transaction(0).unwrap_object(0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = CoinBalanceBuckets.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].change, CoinBalanceBucketChangeKind::Upsert {
    ...
    async fn test_process_coin_balance_buckets_out_of_order_pruning() {
        ...
        let result = CoinBalanceBuckets.process(&Arc::new(checkpoint0)).await.unwrap();
        ...
        let result = CoinBalanceBuckets.process(&Arc::new(checkpoint1)).await.unwrap();
        assert_eq!(result.len(), 3);
        let rows_inserted = CoinBalanceBuckets::commit(&result, &mut conn).await.unwrap();
        ...
        let result = CoinBalanceBuckets.process(&Arc::new(checkpoint2)).await.unwrap();
        assert_eq!(result.len(), 3);
        let rows_inserted = CoinBalanceBuckets::commit(&result, &mut conn).await.unwrap();
        assert_eq!(rows_inserted, 6);
    ...
    async fn test_process_coin_balance_buckets_concurrent_pruning() {
        ...
        let results: Vec<Result<usize, anyhow::Error>> =
