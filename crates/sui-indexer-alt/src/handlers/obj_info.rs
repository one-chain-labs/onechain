    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> Result<Vec<Self::Value>> {
        ...
        for object_id in checkpoint_input_objects.keys() {
            if !latest_live_output_objects.contains_key(object_id) {
                ...
                values.insert(*object_id, ProcessedObjInfo {
                    cp_sequence_number,
        ...
        for (object_id, object) in latest_live_output_objects.iter() {
            ...
            if should_insert {
                ...
                values.insert(*object_id, ProcessedObjInfo {
                    cp_sequence_number,
                    update: ProcessedObjInfoUpdate::Upsert { object: object.clone(), created },
                });
            }
        }
...
impl Handler for ObjInfo {
    async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> Result<usize> {
        ...
        let count = diesel::insert_into(obj_info::table).values(&stored).on_conflict_do_nothing().execute(conn).await?;
    ...
    async fn prune<'a>(&self, from: u64, to_exclusive: u64, conn: &mut Connection<'a>) -> Result<usize> {
        ...
        let CountResult { deleted_objects, deleted_refs } =
...
mod tests {
    ...
    async fn test_process_basics() {
        ...
        assert!(matches!(processed.update, ProcessedObjInfoUpdate::Upsert { object: _, created: true }));
        ...
        let all_obj_info_deletion_references = get_all_obj_info_deletion_references(&mut conn).await.unwrap();
        ...
        assert!(matches!(processed.update, ProcessedObjInfoUpdate::Upsert { object: _, created: false }));
        let rows_inserted = ObjInfo::commit(&result, &mut conn).await.unwrap();
        ...
        assert!(matches!(processed.update, ProcessedObjInfoUpdate::Delete(_)));
        let rows_inserted = ObjInfo::commit(&result, &mut conn).await.unwrap();
    ...
    async fn test_process_wrap_and_prune_before_unwrap() {
        let (indexer, _db) = Indexer::new_for_testing(&MIGRATIONS).await;
        let mut conn = indexer.store().connect().await.unwrap();
        let mut builder = TestCheckpointBuilder::new(0).start_transaction(0).create_owned_object(0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let result = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        let rows_inserted = ObjInfo::commit(&result, &mut conn).await.unwrap();
        assert_eq!(rows_inserted, 1);
    ...
    async fn test_process_shared_object() {
        ...
        let mut conn = indexer.store().connect().await.unwrap();
        let mut builder =
            TestCheckpointBuilder::new(0).start_transaction(0).create_shared_object(0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let result = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(result.len(), 1);
        let processed = &result[0];
    ...
    async fn test_process_immutable_object() {
        ...
        builder = builder.start_transaction(0).change_object_owner(0, Owner::Immutable).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let result = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(result.len(), 1);
        let processed = &result[0];
    ...
    async fn test_process_object_owned_object() {
        ...
        builder =
            builder.start_transaction(0).change_object_owner(0, Owner::ObjectOwner(dbg_addr(0))).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let result = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        assert_eq!(result.len(), 1);
        let processed = &result[0];
    ...
    async fn test_obj_info_prune_with_missing_data() {
        ...
        builder = builder.start_transaction(1).transfer_object(0, 0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        ObjInfo::commit(&values, &mut conn).await.unwrap();
        ...
        builder = builder.start_transaction(2).delete_object(0).finish_transaction();
        let checkpoint = builder.build_checkpoint();
        let values = ObjInfo.process(&Arc::new(checkpoint)).await.unwrap();
        ObjInfo::commit(&values, &mut conn).await.unwrap();
    ...
    async fn test_process_unchanged_consensus_object() {
        let mut builder =
            TestCheckpointBuilder::new(0).start_transaction(0).create_shared_object(1).finish_transaction();
    ...
    async fn test_process_concurrent_pruning() {
        ...
        let final_deletion_references = get_all_obj_info_deletion_references(&mut conn).await.unwrap();
