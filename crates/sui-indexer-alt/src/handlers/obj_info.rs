        ...
        for (object_id, object) in latest_live_output_objects.iter() {
            ...
            if should_insert {
                ...
                values.insert(*object_id, StoredObjInfo {
                    ...
                    instantiation: type_.map(|t| bcs::to_bytes(&t.type_params())).transpose().map_err(|e| {
                        anyhow!(
...
impl Handler for ObjInfo {
    async fn commit(values: &[Self::Value], conn: &mut db::Connection<'_>) -> Result<usize> {
        Ok(diesel::insert_into(obj_info::table).values(values).on_conflict_do_nothing().execute(conn).await?)
    }
}
