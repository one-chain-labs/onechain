...
pub(crate) fn checkpoint_input_objects(checkpoint: &CheckpointData) -> anyhow::Result<BTreeMap<ObjectID, &Object>> {
    ...
    for tx in checkpoint.transactions.iter() {
        let input_objects_map: BTreeMap<(ObjectID, SequenceNumber), &Object> =
            tx.input_objects.iter().map(|obj| ((obj.id(), obj.version()), obj)).collect();
        ...
        for change in tx.effects.object_changes() {
            ...
            let input_obj = input_objects_map.get(&(id, version)).copied().with_context(|| {
                format!("Object {id} at version {version} referenced in effects not found in input_objects")
            })?;
...
#[cfg(test)]
mod tests {
    use sui_indexer_alt_framework::types::{object::Owner, test_checkpoint_data_builder::TestCheckpointDataBuilder};
    ...
    fn test_checkpoint_input_objects_simple() {
        ...
        let setup_versions: BTreeMap<_, _> = setup
            ...
            .iter()
            .flat_map(|tx| {
                tx.effects.object_changes().into_iter().filter_map(|c| Some((c.id, c.output_version?.value())))
            })
            .collect();

        let setup_version =
            |idx: u64| -> Option<u64> { setup_versions.get(&TestCheckpointDataBuilder::derive_object_id(idx)).copied() };

        let inputs = checkpoint_input_objects(&checkpoint).unwrap();
        let input_version = |idx: u64| -> Option<u64> {
            inputs.get(&TestCheckpointDataBuilder::derive_object_id(idx)).map(|obj| obj.version().value())
        };
        ...
        assert_eq!(input_version(SHARED_MODIFIED), setup_version(SHARED_MODIFIED));
