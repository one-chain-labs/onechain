...
pub(crate) fn checkpoint_input_objects(checkpoint: &Checkpoint) -> anyhow::Result<BTreeMap<ObjectID, &Object>> {
    ...
    for tx in checkpoint.transactions.iter() {
        let input_objects_map: BTreeMap<_, _> =
            tx.input_objects(&checkpoint.object_set).map(|obj| ((obj.id(), obj.version()), obj)).collect();
...
#[cfg(test)]
mod tests {
    use sui_indexer_alt_framework::types::{object::Owner, test_checkpoint_data_builder::TestCheckpointBuilder};
    ...
    fn test_checkpoint_input_objects_simple() {
        ...
        let setup_version =
            |idx: u64| -> Option<u64> { setup_versions.get(&TestCheckpointBuilder::derive_object_id(idx)).copied() };

        let inputs = checkpoint_input_objects(&checkpoint).unwrap();
        let input_version = |idx: u64| -> Option<u64> {
            inputs.get(&TestCheckpointBuilder::derive_object_id(idx)).map(|obj| obj.version().value())
        };
