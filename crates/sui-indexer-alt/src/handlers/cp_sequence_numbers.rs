...
impl Processor for CpSequenceNumbers {
    ...
    fn process(&self, checkpoint: &Arc<CheckpointData>) -> Result<Vec<Self::Value>> {
        ...
        let network_total_transactions = checkpoint.checkpoint_summary.network_total_transactions as i64;
...
impl Handler for CpSequenceNumbers {
    ...
    async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> Result<usize> {
        Ok(diesel::insert_into(cp_sequence_numbers::table).values(values).on_conflict_do_nothing().execute(conn).await?)
    }
}
