...
mod tests {
    use anyhow::Result;
    use sui_indexer_alt_framework::{
        types::test_checkpoint_data_builder::{AdvanceEpochConfig, TestCheckpointDataBuilder},
        Indexer,
    };
    ...
    pub async fn test_kv_epoch_ends_safe_mode() {
        ...
        let checkpoint = Arc::new(builder.advance_epoch(AdvanceEpochConfig { safe_mode: true, ..Default::default() }));
