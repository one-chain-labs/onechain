        ...
        for tx in transactions {
            for obj in tx.output_objects(&checkpoint.object_set) {
                ...
                values.push(StoredPackage {
                    ...
                    serialized_object: bcs::to_bytes(obj)
                        .with_context(|| format!("Error serializing package object {}", obj.id()))?,
                    cp_sequence_number,
                });
            }
        }
...
impl Handler for KvPackages {
    async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> Result<usize> {
        Ok(diesel::insert_into(kv_packages::table).values(values).on_conflict_do_nothing().execute(conn).await?)
    }
}
