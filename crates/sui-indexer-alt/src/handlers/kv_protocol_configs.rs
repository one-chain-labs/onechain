...
use sui_indexer_alt_schema::{checkpoints::StoredGenesis, epochs::StoredProtocolConfig, schema::kv_protocol_configs};
...
mod tests {
    ...
    async fn test_protocol_version_processing() {
        ...
        let checkpoint = Arc::new(builder.advance_epoch_and_protocol_upgrade(false, ProtocolVersion::MIN));
        ...
        let protocol_configs = KvProtocolConfigs(stored_genesis).process(&checkpoint).unwrap();

        assert!(!protocol_configs.is_empty());
        for config in protocol_configs {
            assert_eq!(config.protocol_version, ProtocolVersion::MIN.as_u64() as i64);
        }
    }
    ...
    async fn test_protocol_version_too_high() {
        let mut builder = TestCheckpointDataBuilder::new(0);
        let genesis = Arc::new(builder.build_checkpoint());
        let checkpoint = Arc::new(builder.advance_epoch_and_protocol_upgrade(false, ProtocolVersion::MAX + 1));
        ...
        KvProtocolConfigs(stored_genesis).process(&checkpoint).unwrap_err();
    }
}
