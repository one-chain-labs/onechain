...
#[cfg(test)]
mod tests {
    use sui_indexer_alt_framework::types::test_checkpoint_data_builder::{AdvanceEpochConfig, TestCheckpointBuilder};
    ...
    async fn test_protocol_version_processing() {
        ...
        let protocol_configs = KvProtocolConfigs(stored_genesis).process(&checkpoint).await.unwrap();
    ...
    async fn test_protocol_version_too_high() {
        ...
        KvProtocolConfigs(stored_genesis).process(&checkpoint).await.unwrap_err();
    }
}
