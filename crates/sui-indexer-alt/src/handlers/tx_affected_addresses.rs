...
use sui_indexer_alt_schema::{schema::tx_affected_addresses, transactions::StoredTxAffectedAddress};
...
impl Handler for TxAffectedAddresses {
    ...
    async fn prune<'a>(&self, from: u64, to_exclusive: u64, conn: &mut Connection<'a>) -> Result<usize> {
        ...
        let filter = tx_affected_addresses::table
            .filter(tx_affected_addresses::tx_sequence_number.between(from_tx as i64, to_tx as i64 - 1));
...
mod tests {
    ...
    async fn test_tx_affected_addresses_pruning() {
        ...
        TxAffectedAddresses::commit(&values, &mut conn).await.unwrap();
        ...
        let remaining_tx_affected_addresses = get_all_tx_affected_addresses(&mut conn).await.unwrap();
