...
impl Handler for TxCalls {
    ...
    async fn prune(&self, from: u64, to_exclusive: u64, conn: &mut db::Connection<'_>) -> Result<usize> {
        ...
        let filter = tx_calls::table.filter(tx_calls::tx_sequence_number.between(from_tx as i64, to_tx as i64 - 1));
...
mod tests {
    ...
    use sui_types::{base_types::ObjectID, test_checkpoint_data_builder::TestCheckpointDataBuilder};
    ...
    async fn test_tx_calls_pruning() {
        ...
        builder =
            builder.start_transaction(0).add_move_call(ObjectID::random(), "module", "function").finish_transaction();
