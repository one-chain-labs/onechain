...
impl PruningLookupTable {
    ...
    pub fn get_prune_info(&self, cp_from: u64, cp_to_exclusive: u64) -> anyhow::Result<Vec<(ObjectID, u64)>> {
        if cp_from >= cp_to_exclusive {
            bail!("No valid range to take from the lookup table: from={}, to_exclusive={}", cp_from, cp_to_exclusive);
        }

        let mut result = Vec::new();
        for cp in cp_from .. cp_to_exclusive {
            let info = self.table.get(&cp).ok_or_else(|| anyhow::anyhow!("Prune info for checkpoint {cp} not found"))?;
