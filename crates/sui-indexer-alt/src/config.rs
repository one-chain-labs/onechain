    ...
    pub fn for_test() -> Self {
        Self::example()
            .merge(IndexerConfig {
                ...
                pruner: PrunerLayer { interval_ms: Some(50), delay_ms: Some(0), ..Default::default() },
                ..Default::default()
            })
...
impl From<SequentialConfig> for SequentialLayer {
    fn from(config: SequentialConfig) -> Self {
        Self { committer: Some(config.committer.into()), checkpoint_lag: Some(config.checkpoint_lag) }
    }
}

impl From<ConcurrentConfig> for ConcurrentLayer {
    fn from(config: ConcurrentConfig) -> Self {
        Self { committer: Some(config.committer.into()), pruner: config.pruner.map(Into::into) }
    }
}
...
mod tests {
    ...
    fn finish_concurrent_unpruned_override() {
        let layer = ConcurrentLayer { committer: None, pruner: None };
        ...
        assert_matches!(layer.finish(base).unwrap(), ConcurrentConfig {
            committer: CommitterConfig { write_concurrency: 5, collect_interval_ms: 50, watermark_interval_ms: 500 },
            pruner: None,
        },);
    }
    ...
    fn detect_unrecognized_fields() {
        ...
        assert!(err.to_string().contains("i_dont_exist"), "Unexpected error: {err}");
    }
}
