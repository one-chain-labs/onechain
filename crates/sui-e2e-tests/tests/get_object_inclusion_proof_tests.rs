...
use sui_rpc_api::grpc::alpha::proof_service_proto::{
    proof_service_client::ProofServiceClient,
...
async fn get_test_object(test_cluster: &TestCluster) -> ObjectID {
    ...
    test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap().0
}
...
async fn test_feature_flag_disabled() {
    ...
    assert!(err.message().contains("Authenticated events indexing is disabled"));
}
...
async fn test_valid_request() {
    ...
    assert!(found_checkpoint.is_some(), "Object not found in any checkpoint");
    ...
    assert!(object_ref.object_id.is_some(), "object_id should be present");
    ...
    assert!(inclusion_proof.leaf_index.is_some(), "leaf_index should be present");
    assert!(inclusion_proof.tree_root.is_some(), "tree_root should be present");

    assert!(response.object_data.is_some(), "object_data should be present");
}
