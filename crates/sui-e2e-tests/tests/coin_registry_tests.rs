...
async fn test_create_coin_registry_object() {
    let _guard = sui_protocol_config::ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
    ...
    let package = framework.iter().find(|f| f.id() == SUI_FRAMEWORK_PACKAGE_ID).unwrap().data.try_as_package().unwrap();

    // Make sure that `coin_registry` does not exist on previous protocol version.
    assert!(!package.serialized_module_map().contains_key("coin_registry"));
