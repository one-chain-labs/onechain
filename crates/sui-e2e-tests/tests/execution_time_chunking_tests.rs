...
use sui_protocol_config::{ExecutionTimeEstimateParams, PerObjectCongestionControlMode, ProtocolConfig};
use sui_types::{
    base_types::{ObjectID, SequenceNumber, SuiAddress},
    dynamic_field::get_dynamic_field_from_store,
    execution::ExecutionTimeObservationChunkKey,
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    sui_system_state,
    transaction::{SharedObjectMutability, StoredExecutionTimeObservations, TransactionData},
};
...
async fn setup_test_cluster_with_chunking() -> TestCluster {
    ...
    let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut cfg| {
        cfg.set_per_object_congestion_control_mode_for_testing(PerObjectCongestionControlMode::ExecutionTimeEstimate(
...
async fn validate_chunked_execution_time_storage(test_cluster: &TestCluster) {
    ...
    let system_state = sui_system_state::get_sui_system_state(object_store.as_ref()).expect("System state must exist");

    let extra_fields_id = match &system_state {
        sui_types::sui_system_state::SuiSystemState::V2(system_state) => system_state.extra_fields.id.id.bytes,
        #[cfg(msim)]
    ...
    let old_format_result: Result<Vec<u8>, _> =
        get_dynamic_field_from_store(object_store.as_ref(), extra_fields_id, &EXTRA_FIELD_EXECUTION_TIME_ESTIMATES_KEY);
    ...
    for chunk_index in 0 .. chunk_count {
        ...
        match chunk_bytes_result {
            Ok(chunk_bytes) => {
                let _chunk: StoredExecutionTimeObservations =
                    bcs::from_bytes(&chunk_bytes).expect("Failed to deserialize stored execution time estimates chunk");
            }
            Err(_) => {
                panic!("Could not find stored execution time observation chunk {}", chunk_index);
            }
        }
    }

    assert!(chunk_count > 1, "Expected more than 1 chunk to validate chunking, got {}", chunk_count);
}
...
async fn test_chunked_execution_time() {
    ...
    let (package_id, _, _) = sui_test_transaction_builder::publish_package(&test_cluster.wallet, path).await;

    let sender = test_cluster.wallet.config.keystore.addresses()[0];

    let (counter_id, initial_shared_version) = create_shared_counter(&test_cluster, sender, package_id).await;

    send_transactions(&test_cluster, sender, package_id, counter_id, initial_shared_version).await;
...
async fn create_shared_counter(
    ...
    let created_obj = response.effects.unwrap().created().iter().find(|obj| obj.owner.is_shared()).unwrap().clone();

    (created_obj.reference.object_id, created_obj.reference.version)
}
...
async fn send_transactions(
    ...
    for i in 0 .. 6 {
        ...
        assert_eq!(res.effects.unwrap().executed_epoch(), 0, "all txns to execute in epoch 0");
    }
}
