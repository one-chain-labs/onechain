...
use sui_rpc_api::{
    proto::rpc::{
        v2alpha::{live_data_service_client::LiveDataServiceClient, ResolveTransactionRequest},
        v2beta::{
            Argument,
            Command,
            GasPayment,
...
fn build_resolve_request(transaction: Transaction, simulate: bool) -> ResolveTransactionRequest {
    let read_mask = if simulate { Some(FieldMask { paths: vec!["simulation".to_string()] }) } else { None };
    ResolveTransactionRequest { unresolved_transaction: Some(transaction), read_mask }
}
...
fn proto_to_response(
    ...
    Option<(sui_types::effects::TransactionEffects, Option<sui_types::effects::TransactionEvents>)>,
) {
    let effects_and_events = proto.simulation.map(|simulation| {
        ...
        (
            ...
            txn.events.map(|events| events.bcs.unwrap().deserialize().unwrap()),
        )
    });
    (proto.transaction.unwrap().bcs.unwrap().deserialize().unwrap(), effects_and_events)
}
...
async fn resolve_transaction_simple_transfer() {
    ...
    let unresolved_transaction = Transaction {
        kind: Some(TransactionKind::from(ProgrammableTransaction {
            inputs: vec![
                Input { object_id: Some(obj_to_send.to_canonical_string(true)), ..Default::default() },
                Input { literal: Some(Box::new(recipient.to_string().into())), ..Default::default() },
            ],
...
async fn resolve_transaction_transfer_with_sponsor() {
    ...
    let unresolved_transaction = Transaction {
        ...
        sender: Some(sender.to_string()),
        gas_payment: Some(GasPayment { owner: Some(sponsor.to_string()), ..Default::default() }),
        ..Default::default()
    };
    ...
    let sender_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &transaction, Intent::sui_transaction()).unwrap();
    let sponsor_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &transaction, Intent::sui_transaction()).unwrap();
...
async fn resolve_transaction_mutable_shared_object() {
    ...
    let unresolved_transaction = Transaction {
        kind: Some(TransactionKind::from(ProgrammableTransaction {
            inputs: vec![
                Input { object_id: Some("0x5".to_owned()), ..Default::default() },
                Input { object_id: Some(obj_to_stake.to_canonical_string(true)), ..Default::default() },
                Input { literal: Some(Box::new(validator_address.to_string().into())), ..Default::default() },
            ],
            commands: vec![Command::from(MoveCall {
                ...
                type_arguments: vec![],
                arguments: vec![Argument::new_input(0), Argument::new_input(1), Argument::new_input(2)],
            })],
        })),
        sender: Some(sender.to_string()),
        ..Default::default()
...
async fn resolve_transaction_insufficient_gas() {
    ...
    let error =
        alpha_client.resolve_transaction(build_resolve_request(unresolved_transaction, false)).await.unwrap_err();
