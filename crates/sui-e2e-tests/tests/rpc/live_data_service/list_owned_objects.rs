...
use sui_rpc_api::proto::rpc::{
    ...
    v2beta::changed_object::{IdOperation, OutputObjectState},
};
...
async fn test_indexing_with_tto() {
    ...
    let mut channel =
        tonic::transport::Channel::from_shared(cluster.rpc_url().to_owned()).unwrap().connect().await.unwrap();

    let mut client = LiveDataServiceClient::new(channel.clone());
    let address = cluster.get_address_0();

    let objects = client
        .list_owned_objects(ListOwnedObjectsRequest { owner: Some(address.to_string()), ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects;
    ...
    let gas_data = sui_types::transaction::GasData {
        payment: vec![(gas.object_id().parse().unwrap(), gas.version().into(), gas.digest().parse().unwrap())],
        owner: address,
        price: 1000,
        budget: 100_000_000,
    };
    ...
    let parent = effects
        ...
        .find_map(|o| {
            if matches!(o.id_operation(), IdOperation::Created) {
                Some((o.object_id().to_owned(), o.output_version(), o.output_digest().to_owned()))
            } else {
                None
            }
        })
    ...
    let coin = effects
        ...
        .iter()
        .find_map(|o| {
            if matches!(o.output_state(), OutputObjectState::ObjectWrite) && o.object_id() == coin.object_id() {
                Some((o.object_id().to_owned(), o.output_version(), o.output_digest().to_owned()))
            } else {
                None
            }
    ...
    assert_eq!(
        client
            .list_owned_objects(ListOwnedObjectsRequest { owner: Some(parent.0.clone()), ..Default::default() })
            .await
            .unwrap()
            .into_inner()
            .objects
    ...
    assert!(client
        .list_owned_objects(ListOwnedObjectsRequest { owner: Some("0x0".to_owned()), ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects
    ...
    builder
        .move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::Receiving((coin.0.parse().unwrap(), coin.1.into(), coin.2.parse().unwrap()))),
            ],
        )
        .unwrap();
    let ptb = builder.finish();
...
async fn test_filter_by_type() {
    ...
    let sui = "0x2::coin::Coin<0x2::oct::OCT>".parse::<TypeTag>().unwrap().to_string();
    let mut channel =
        tonic::transport::Channel::from_shared(cluster.rpc_url().to_owned()).unwrap().connect().await.unwrap();
    ...
    let trusted = format!("{package_id}::trusted_coin::TRUSTED_COIN").parse::<TypeTag>().unwrap().to_string();
    let trusted_coin = format!("0x2::coin::Coin<{trusted}>").parse::<TypeTag>().unwrap().to_string();
    let treasury_cap_type = sui_types::coin::TreasuryCap::type_(sui_types::parse_sui_struct_tag(&trusted).unwrap())
    ...
    let treasury_cap = effects
        ...
        .iter()
        .find_map(|o| {
            if matches!(o.id_operation(), IdOperation::Created) && treasury_cap_type == o.object_type() {
                Some((o.object_id().to_owned(), o.output_version(), o.output_digest().to_owned()))
            } else {
                None
            }
    ...
    let GetCoinInfoResponse { coin_type, metadata, treasury, .. } =
        client.get_coin_info(GetCoinInfoRequest { coin_type: Some(trusted.clone()) }).await.unwrap().into_inner();
    ...
    let GetCoinInfoResponse { coin_type, treasury, .. } =
        client.get_coin_info(GetCoinInfoRequest { coin_type: Some(trusted.clone()) }).await.unwrap().into_inner();
    assert_eq!(coin_type.as_ref(), Some(&trusted));
    ...
    assert_eq!(objects.iter().filter(|o| o.object_type() == trusted_coin).count(), 1);
...
async fn test_reverse_sorted_coins_by_balance() {
    ...
    for amount in amounts {
        let txn =
            sui_test_transaction_builder::make_transfer_oct_transaction(&cluster.wallet, Some(address), Some(amount))
                .await;
        crate::execute_transaction(&mut channel, &txn).await;
    }
    ...
    let balances = objects.iter().map(|o| o.balance.unwrap()).collect::<Vec<_>>();
