...
async fn transfer_coin(context: &sui_sdk::wallet_context::WalletContext) -> sui_sdk_types::TransactionDigest {
...
async fn stake_with_validator(cluster: &test_cluster::TestCluster) -> sui_sdk_types::TransactionDigest {
...
async fn execute_transaction(
    ...
    assert!(transaction.effects.as_ref().unwrap().status.as_ref().unwrap().success());
...
async fn wait_for_transaction(
    ...
    tokio::time::timeout(WAIT_FOR_LOCAL_EXECUTION_TIMEOUT, async {
        ...
        loop {
            ...
            if let Ok(poll_response) =
                client.get_transaction(GetTransactionRequest { digest: Some(digest.to_owned()), read_mask: None }).await
            {
                break poll_response;
            }
        }
