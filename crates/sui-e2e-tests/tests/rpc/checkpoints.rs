...
async fn get_checkpoint() {
    ...
    let GetCheckpointResponse { sequence_number, digest, summary, summary_bcs, signature, contents, contents_bcs } =
        grpc_client.get_checkpoint(GetCheckpointRequest::latest()).await.unwrap().into_inner();
    ...
    // Request by digest
    let response =
        grpc_client.get_checkpoint(GetCheckpointRequest::by_digest(digest.clone().unwrap())).await.unwrap().into_inner();
...
async fn get_full_checkpoint() {
    ...
    let latest = core_client.get_latest_checkpoint().await.unwrap().into_data();
    let _ = core_client.get_full_checkpoint(latest.sequence_number).await.unwrap();
    ...
    let GetFullCheckpointResponse {
        ...
        .get_full_checkpoint(
            GetFullCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(FieldMask::from_paths([
                "sequence_number",
                "digest",
                "transactions.digest",
            ])),
    ...
    // Request all fields
    let response = grpc_client
        .get_full_checkpoint(GetFullCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(
    ...
    } in transactions
    {
        ...
        for FullCheckpointObject { object_id, version, digest, object, object_bcs } in
            input_objects.iter().chain(output_objects.iter())
        {
    ...
    // Request by sequence_number
    let response = grpc_client
        .get_full_checkpoint(GetFullCheckpointRequest::by_sequence_number(sequence_number.unwrap()))
        .await
        .unwrap()
        .into_inner();
    assert_eq!(response.sequence_number, sequence_number.to_owned());
...
async fn subscribe_checkpoint() {
    use sui_rpc_api::proto::node::v2alpha::{
    ...
    let mut client = SubscriptionServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();

    let request = SubscribeCheckpointsRequest {
        read_mask: Some(prost_types::FieldMask { paths: vec!["sequence_number".to_owned()] }),
    };

    let mut stream = client.subscribe_checkpoints(request).await.unwrap().into_inner();
    ...
    while let Some(item) = stream.next().await {
        ...
        assert_eq!(cursor, checkpoint.checkpoint.unwrap().sequence_number.unwrap());
