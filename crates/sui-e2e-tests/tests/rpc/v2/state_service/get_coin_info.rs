...
use sui_types::{
    ...
    transaction::{ObjectArg, SharedObjectMutability, TransactionData},
    TypeTag,
...
async fn test_get_coin_info_burnonly_coin() {
    ...
    // Publish coin package using new_currency_with_otw
    let (package_id, treasury_cap, _metadata_cap) = publish_burnonly_coin(&test_cluster, address).await;
...
async fn test_get_coin_info_regulated_coin() {
    ...
    let (package_id, _treasury_cap, _metadata_cap, deny_cap) = publish_regulated_coin(&test_cluster, address).await;
    ...
    let regulated_metadata = response.regulated_metadata.expect("Expected regulated_metadata");
    assert_eq!(
    ...
    assert_eq!(regulated_metadata.deny_cap_object.unwrap(), deny_cap.to_string());
}
...
async fn test_get_coin_info_non_otw_coin() {
    ...
    let (package_id, _treasury_cap, metadata_cap) = publish_non_otw_coin(&test_cluster, address).await;
    ...
    assert_eq!(metadata.icon_url, Some("https://example.com/non_otw.png".to_string()));
    // Check that metadata cap is claimed with the correct ID
    if metadata.metadata_cap_state == Some(MetadataCapState::Claimed as i32) {
        let cap_id = metadata.metadata_cap_id.as_ref().expect("Expected metadata_cap_id when state is Claimed");
        assert!(ObjectID::from_str(cap_id).is_ok(), "metadata_cap_state.claimed should be a valid ObjectID");
...
async fn test_get_coin_info_legacy_coin() {
    ...
    let (package_id, _treasury_cap, metadata_id) = publish_legacy_coin(&test_cluster, address).await;
    ...
    assert!(metadata.metadata_cap_state.is_none(), "Legacy coins don't have metadata caps");
...
async fn publish_registry_coin(
    ...
    let (package_id_obj, transaction) = super::super::publish_package(test_cluster, address, path).await;
    ...
    let treasury_cap = transaction
        ...
        .iter()
        .find_map(|o| {
            if o.object_type.as_ref().map(|t| t.contains("TreasuryCap")).unwrap_or(false) {
                o.object_id.as_ref().and_then(|id| ObjectID::from_str(id).ok())
            } else {
                None
            }
        })
    ...
    let metadata_cap = transaction
        ...
        .iter()
        .find_map(|o| {
            if o.object_type.as_ref().map(|t| t.contains("MetadataCap")).unwrap_or(false) {
                o.object_id.as_ref().and_then(|id| ObjectID::from_str(id).ok())
            } else {
                None
            }
...
async fn publish_regulated_coin(
    ...
    let deny_cap = transaction
        ...
        .iter()
        .find_map(|o| {
            if o.object_type.as_ref().map(|t| t.contains("DenyCapV2")).unwrap_or(false) {
                o.object_id.as_ref().and_then(|id| ObjectID::from_str(id).ok())
            } else {
                None
            }
...
async fn publish_legacy_coin(
    ...
    let metadata_id = transaction
        ...
        .iter()
        .find_map(|o| {
            if o.object_type.as_ref().map(|t| t.contains("CoinMetadata")).unwrap_or(false) {
                o.object_id.as_ref().and_then(|id| ObjectID::from_str(id).ok())
            } else {
                None
            }
...
async fn publish_non_otw_coin(
    ...
    let (package_id_obj, _transaction) = super::super::publish_package(test_cluster, address, path).await;
    let package_id = package_id_obj;
    ...
    let registry_initial_version = registry_obj_response
        ...
        .and_then(|owner| {
            if owner.kind == Some(OwnerKind::Shared as i32) {
                owner.version.map(sui_types::base_types::SequenceNumber::from)
            } else {
                None
            }
        })
    ...
    for o in effects.created() {
        let obj = test_cluster.get_object_from_fullnode_store(&o.reference.object_id).await.unwrap();
...
async fn get_grpc_client(test_cluster: &test_cluster::TestCluster) -> StateServiceClient<tonic::transport::Channel> {
    StateServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap()
}
