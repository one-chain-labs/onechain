...
async fn test_filter_by_type() {
    ...
    let GetCoinInfoResponse { coin_type, metadata, treasury, .. } = client
        .state_client()
        .get_coin_info({
    ...
    let GetCoinInfoResponse { coin_type, treasury, .. } = client
        .state_client()
        .get_coin_info({
            let mut message = GetCoinInfoRequest::default();
            message.coin_type = Some(trusted.clone());
...
async fn test_reverse_sorted_coins_by_balance() {
    ...
    let objects = client
        ...
        .list_owned_objects({
            let mut message = ListOwnedObjectsRequest::default();
            message.owner = Some(address.to_string());
            message.read_mask = Some(FieldMask::from_str("object_id,version,digest,object_type,balance"));
            message
        })
        .await
        .unwrap()
