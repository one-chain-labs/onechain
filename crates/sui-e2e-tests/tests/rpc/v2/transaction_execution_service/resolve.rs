...
async fn resolve_transaction_simple_transfer() {
    ...
    let mut alpha_client = TransactionExecutionServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();
...
async fn resolve_transaction_insufficient_gas_with_payment_objects() {
    ...
    request.read_mask = Some(FieldMask { paths: vec!["transaction".to_string(), "effects".to_string()] });
...
async fn resolve_transaction_shared_object_with_generic_type_parameter() {
    ...
    let (package, party_object) = publish_basics_package_and_make_party_object(&test_cluster.wallet).await;
    ...
    unresolved_transaction.kind = Some(TransactionKind::from({
        ...
        ptb.commands = vec![Command::from({
            ...
            message.type_arguments = vec![format!("{}::object_basics::Object", package.0.to_canonical_string(true))];
    ...
    match &inputs[0] {
        sui_types::transaction::CallArg::Object(sui_types::transaction::ObjectArg::SharedObject {
            id,
            mutability,
            ..
        }) => {
