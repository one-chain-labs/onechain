...
use sui_types::{
    ...
    transaction::{CallArg, ObjectArg, SharedObjectMutability, TransactionData, TransactionKind},
    Identifier,
};
...
async fn test_unchanged_loaded_runtime_objects() {
    ...
    let t = client
        .ledger_client()
        .get_transaction(GetTransactionRequest::new(&transaction_digest).with_read_mask(FieldMask::from_paths(["*"])))
        .await
        .unwrap()
        .into_inner()
        .transaction
    ...
    let c = client
        .ledger_client()
        .get_checkpoint(
            GetCheckpointRequest::by_sequence_number(t.checkpoint()).with_read_mask(FieldMask::from_paths(["*"])),
        )
        .await
        .unwrap()
        .into_inner()
    ...
    assert!(!c.objects().objects().iter().any(|o| o.object_type() == "package"));
...
async fn test_tto_receive_twice() {
    ...
    assert!(!t.effects().changed_objects().iter().any(|o| o.object_id() == coin.0.as_str()));
}
