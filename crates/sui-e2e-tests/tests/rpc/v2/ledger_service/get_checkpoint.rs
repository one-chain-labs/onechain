...
async fn get_checkpoint() {
    let test_cluster = TestClusterBuilder::new().disable_fullnode_pruning().build().await;
    ...
    let mut client = LedgerServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();

    // Request with no provided read_mask
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions, objects, .. } =
        client.get_checkpoint(GetCheckpointRequest::default()).await.unwrap().into_inner().checkpoint.unwrap();
    ...
    // Request all fields
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions, objects, .. } = client
        .get_checkpoint(GetCheckpointRequest::latest().with_read_mask(FieldMask::from_paths([
            "sequence_number",
            "digest",
            "summary",
            "signature",
    ...
    let response = client
        .get_checkpoint(GetCheckpointRequest::by_sequence_number(sequence_number.unwrap()))
        .await
        .unwrap()
        .into_inner()
        .checkpoint
    ...
    let checkpoint = client
        .get_transaction(
            GetTransactionRequest::new(&transaction_digest).with_read_mask(FieldMask::from_paths(["checkpoint"])),
        )
        .await
        .unwrap()
        .into_inner()
    ...
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions, objects, .. } =
        client
            .get_checkpoint(GetCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(FieldMask::from_paths(
                ["sequence_number", "digest", "summary", "signature", "contents", "transactions", "objects"],
            )))
            .await
            .unwrap()
            .into_inner()
    ...
    let mut found_transaction = false;
    for ExecutedTransaction { digest, transaction, effects, events, objects, .. } in transactions {
        assert!(digest.is_some());
        if digest == Some(transaction_digest.to_string()) {
            found_transaction = true;
    ...
    for Object { bcs, object_id, version, digest, owner, object_type, .. } in objects.unwrap().objects {
    ...
    let Checkpoint { transactions, .. } =
        client
            .get_checkpoint(GetCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(FieldMask::from_paths(
                ["sequence_number", "transactions.digest", "transactions.events.events.json"],
            )))
            .await
            .unwrap()
            .into_inner()
    ...
    for tx in transactions {
        if tx.digest == Some(transaction_digest.to_string()) {
            ...
            for event in events.events {
                let json = event.json.as_ref().expect("json field should be populated when requested in mask");

                let prost_types::value::Kind::StructValue(s) = json.kind.as_ref().expect("json should have kind") else {
                ...
                for addr_field in ["pool_id", "staker_address", "validator_address"] {
                    ...
                    assert!(addr.starts_with("0x"), "{} should start with 0x", addr_field);
                    assert_eq!(addr.len(), 66, "{} should be 66 chars (0x + 64 hex)", addr_field);
                }
            }
        }
    }
    assert!(found_transaction_with_events, "should have found transaction with events");
