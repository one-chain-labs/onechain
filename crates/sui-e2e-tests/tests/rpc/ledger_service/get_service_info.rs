...
async fn get_service_info() {
    ...
    let mut grpc_client = LedgerServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();
    ...
    let GetServiceInfoResponse {
        ...
        server_version,
    } = grpc_client.get_service_info(GetServiceInfoRequest {}).await.unwrap().into_inner();
