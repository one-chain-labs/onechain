...
async fn get_checkpoint() {
    ...
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions } =
        client.get_checkpoint(GetCheckpointRequest::default()).await.unwrap().into_inner();
    ...
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions } = client
        .get_checkpoint(GetCheckpointRequest {
            ...
            read_mask: Some(FieldMask::from_paths(["sequence_number", "digest", "transactions.digest"])),
        })
        .await
        .unwrap()
        .into_inner();
