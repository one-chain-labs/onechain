...
use sui_rpc_api::proto::rpc::{
    v2alpha::{signature_verification_service_client::SignatureVerificationServiceClient, VerifySignatureRequest},
    v2beta::{Bcs, UserSignature},
};
...
async fn test_verify_signature_zklogin() -> Result<(), anyhow::Error> {
    let test_cluster = TestClusterBuilder::new().with_epoch_duration_ms(10000).with_default_jwks().build().await;
    test_cluster.wait_for_epoch(Some(1)).await;
    test_cluster.wait_for_authenticator_state_update().await;

    let mut client = SignatureVerificationServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();

    // Construct a valid zkLogin transaction data, signature.
    let (kp, pk_zklogin, inputs) =
        &sui_types::utils::load_test_vectors("../sui-types/src/unit_tests/zklogin_test_vectors.json")[1];
    ...
    let response = client
        .verify_signature(VerifySignatureRequest {
            ...
            signature: Some(UserSignature { bcs: Some(signature.clone()), ..Default::default() }),
            ..Default::default()
        })
        .await
        .unwrap()
    ...
    assert_eq!(
        response.reason,
        Some(format!("provided address `{}` does not match derived address `{}`", wrong_address, zklogin_addr))
    );
    ...
    let response = client
        .verify_signature(VerifySignatureRequest {
            ...
            signature: Some(UserSignature { bcs: Some(signature), ..Default::default() }),
            ..Default::default()
        })
        .await
        .unwrap()
