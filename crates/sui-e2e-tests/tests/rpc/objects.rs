...
use sui_rpc_api::{
    ...
    proto::node::v2::{node_service_client::NodeServiceClient, GetObjectRequest, GetObjectResponse},
};
...
async fn get_object() {
    ...
    let GetObjectResponse { object_id, version, digest, object, object_bcs } = grpc_client
        .get_object(
            GetObjectRequest::new(id).with_version(1).with_read_mask(FieldMask::from_paths(["object_id", "version"])),
        )
        .await
        .unwrap()
        .into_inner();
