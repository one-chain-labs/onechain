...
use sui_rpc::{
    client::Client,
    field::FieldMaskUtil,
    proto::sui::rpc::v2beta2::{Bcs, ExecuteTransactionRequest, ExecutedTransaction, Transaction, UserSignature},
};
use sui_types::{
...
async fn execute_transaction(
    ...
    let transaction =
        client.execute_transaction_and_wait_for_checkpoint(request, Duration::from_secs(10)).await.unwrap();

    // Assert that the txn was successful
    let status = transaction.effects.as_ref().unwrap().status.as_ref().unwrap();
