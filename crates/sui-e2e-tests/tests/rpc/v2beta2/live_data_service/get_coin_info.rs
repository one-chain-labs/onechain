...
use sui_rpc::{
    ...
    proto::sui::rpc::v2beta2::{
        ...
        live_data_service_client::LiveDataServiceClient,
        owner::OwnerKind,
...
async fn get_coin_info_sui() {
    ...
    let mut grpc_client = LiveDataServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();
    ...
    let GetCoinInfoResponse { coin_type, metadata, treasury, regulated_metadata, .. } =
        grpc_client.get_coin_info(request).await.unwrap().into_inner();
    ...
    assert_eq!(treasury.total_supply, Some(sui_types::gas_coin::TOTAL_SUPPLY_MIST));
    assert_eq!(treasury.supply_state, Some(SupplyState::Fixed as i32), "SUI should have Fixed supply state");

    // SUI is not in CoinRegistry, so regulation state is Unknown
    let regulated_metadata = regulated_metadata.unwrap();
    assert_eq!(regulated_metadata.coin_regulated_state, Some(CoinRegulatedState::Unknown as i32));
}
...
async fn test_get_coin_info_registry_coin() {
    ...
    let (package_id, treasury_cap, metadata_cap) = publish_registry_coin(&test_cluster, address).await;
    ...
    let treasury_cap_obj = test_cluster.get_object_from_fullnode_store(&treasury_cap).await.unwrap();
    let treasury_cap_ref = treasury_cap_obj.compute_object_reference();

    // Build a transaction to mint coins
    let mut ptb = ProgrammableTransactionBuilder::new();
    let treasury_cap_arg = ptb.obj(ObjectArg::ImmOrOwnedObject(treasury_cap_ref)).unwrap();
    let amount_arg = ptb.pure(mint_amount).unwrap();
    let recipient_arg = ptb.pure(address).unwrap();

    ptb.programmable_move_call(package_id, "registry_coin".parse().unwrap(), "mint".parse().unwrap(), vec![], vec![
        treasury_cap_arg,
        amount_arg,
        recipient_arg,
    ]);
    ...
    let metadata_object_id = metadata.id.as_ref().unwrap();
    assert!(ObjectID::from_str(metadata_object_id).is_ok(), "metadata.id should be a valid ObjectID");
    ...
    assert_eq!(metadata.description, Some("Registry coin for testing GetCoinInfo with CoinRegistry".to_string()));
    assert_eq!(metadata.icon_url, Some("https://example.com/registry.png".to_string()));
    ...
    if metadata.metadata_cap_state == Some(MetadataCapState::Claimed as i32) {
        let cap_id = metadata.metadata_cap_id.as_ref().expect("Expected metadata_cap_id when state is Claimed");
        assert!(ObjectID::from_str(cap_id).is_ok(), "metadata_cap_state.claimed should be a valid ObjectID");
    ...
    assert!(ObjectID::from_str(treasury.id.as_ref().unwrap()).is_ok(), "treasury.id should be a valid ObjectID");
    ...
    assert_eq!(regulated_metadata.coin_regulated_state, Some(CoinRegulatedState::Unregulated as i32));
    ...
    let updated_treasury_cap_obj = test_cluster.get_object_from_fullnode_store(&treasury_cap).await.unwrap();
    ...
    let currency_obj = test_cluster.get_object_from_fullnode_store(&currency_id).await.unwrap();

    let initial_shared_version = match currency_obj.owner {
        sui_types::object::Owner::Shared { initial_shared_version } => initial_shared_version,
        _ => panic!("Currency object should be shared"),
    };

    // Add the Currency object at the derived address as a shared object
    let currency_arg =
        ptb.obj(ObjectArg::SharedObject { id: currency_id, initial_shared_version, mutable: true }).unwrap();

    // Add the treasury cap as an owned object with updated reference
    let treasury_cap_arg = ptb.obj(ObjectArg::ImmOrOwnedObject(updated_treasury_cap_ref)).unwrap();
...
async fn test_regulated_coin_info() {
    ...
    let (package_id, treasury_cap, metadata_cap, deny_cap) = publish_regulated_coin(&test_cluster, address).await;
    ...
    assert_eq!(metadata.description, Some("Regulated coin for testing GetCoinInfo with CoinRegistry".to_string()));
    assert_eq!(metadata.icon_url, Some("https://example.com/regulated.png".to_string()));
    // Check that metadata cap is claimed with the correct ID
    if metadata.metadata_cap_state == Some(MetadataCapState::Claimed as i32) {
        let cap_id = metadata.metadata_cap_id.as_ref().expect("Expected metadata_cap_id when state is Claimed");
        assert!(ObjectID::from_str(cap_id).is_ok(), "metadata_cap_state.claimed should be a valid ObjectID");
    ...
    let regulated = response.regulated_metadata.expect("Expected regulated_metadata");
    assert_eq!(
...
async fn test_legacy_coin_from_registry() {
    ...
    let (package_id, _metadata_id, treasury_cap_id) = publish_legacy_coin(&test_cluster, sender).await;
    let coin_type = format!("{}::legacy_coin::LEGACY_COIN", package_id);

    let mint_amount = 1_000_000_000u64;

    let treasury_cap_obj = test_cluster.get_object_from_fullnode_store(&treasury_cap_id).await.unwrap();
    let treasury_cap_ref = treasury_cap_obj.compute_object_reference();
    ...
    ptb.programmable_move_call(package_id, "legacy_coin".parse().unwrap(), "mint".parse().unwrap(), vec![], vec![
        treasury_cap_arg,
        amount_arg,
        recipient_arg,
    ]);
    ...
    let pt = ptb.finish();
    let gas_price = test_cluster.get_reference_gas_price().await;
    let gas = test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap();
    ...
    assert_eq!(metadata.description, Some("Legacy coin for testing GetCoinInfo fallback".to_string()));
    assert_eq!(metadata.icon_url, Some("https://example.com/legacy.png".to_string()));
    assert!(metadata.metadata_cap_state.is_none());
    ...
    let updated_treasury_cap_obj = test_cluster.get_object_from_fullnode_store(&treasury_cap_id).await.unwrap();
    let updated_treasury_cap_ref = updated_treasury_cap_obj.compute_object_reference();
...
async fn test_invalid_coin_type() {
    ...
    let fake_coin_type = "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef::fakecoin::FAKECOIN";
    let mut request = GetCoinInfoRequest::default();
...
async fn test_burnonly_coin_info() {
    ...
    let (package_id, treasury_cap, metadata_cap) = publish_burnonly_coin(&test_cluster, address).await;
    ...
    ptb.programmable_move_call(package_id, "burnonly_coin".parse().unwrap(), "mint".parse().unwrap(), vec![], vec![
        treasury_cap_arg,
        amount_arg,
        recipient_arg,
    ]);
    ...
    assert_eq!(
        ...
        Some("BurnOnly coin for testing GetCoinInfo with CoinRegistry BurnOnly supply state".to_string())
    );
    assert_eq!(metadata.icon_url, Some("https://example.com/burnonly.png".to_string()));
    // Check that metadata cap is claimed with the correct ID
    if metadata.metadata_cap_state == Some(MetadataCapState::Claimed as i32) {
        let cap_id = metadata.metadata_cap_id.as_ref().expect("Expected metadata_cap_id when state is Claimed");
        assert_eq!(cap_id, &metadata_cap.to_string());
    ...
    assert_eq!(regulated_metadata_after.coin_regulated_state, Some(CoinRegulatedState::Unregulated as i32));
}

async fn finalize_registration(test_cluster: &test_cluster::TestCluster, _package_id: ObjectID, coin_type: &str) {
    ...
    let registry_owned = grpc_client
        .list_owned_objects({
            ...
            request.read_mask = Some(FieldMask::from_str("object_id,version,digest,object_type,owner"));
            request
        })
        .await
        .unwrap()
                                ...
                                && struct_tag.type_params.len() == 1
                            {
                                ...
                                let matches = struct_tag
                                    ...
                                    .map(|type_param| {
                                        ...
                                        let target_str = target_coin_type.to_canonical_string(false);
...
async fn get_grpc_client(test_cluster: &test_cluster::TestCluster) -> LiveDataServiceClient<tonic::transport::Channel> {
    LiveDataServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap()
}
...
async fn publish_registry_coin(
    ...
    let (package_id, transaction) = super::super::publish_package(test_cluster, address, path).await;
...
fn find_object_by_type(transaction: &ExecutedTransaction, type_substr: &str) -> ObjectID {
    transaction
        ...
        .changed_objects
        .iter()
        .find(|obj| obj.object_type.as_ref().map(|t| t.contains(type_substr)).unwrap_or(false))
        .and_then(|obj| obj.object_id.as_ref().map(|id| id.parse().unwrap()))
        .unwrap_or_else(|| panic!("Object with type containing '{}' not found in transaction", type_substr))
}
