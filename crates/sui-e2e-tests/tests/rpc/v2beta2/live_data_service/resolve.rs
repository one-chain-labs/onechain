...
use sui_types::{
    ...
    transaction::{Command as SuiCommand, ObjectArg, TransactionData, TransactionDataAPI},
};
...
fn proto_to_response(
    ...
    let transaction = executed_transaction.transaction.unwrap().bcs.unwrap().deserialize().unwrap();
    let effects = executed_transaction.effects.unwrap().bcs.unwrap().deserialize().unwrap();
    let events = executed_transaction.events.map(|events| events.bcs.unwrap().deserialize().unwrap());
...
async fn resolve_transaction_simple_transfer() {
    ...
    let mut alpha_client = LiveDataServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();
    ...
    let effects = client.execute_transaction(&signed_transaction).await.unwrap().effects;
...
async fn resolve_transaction_transfer_with_sponsor() {
    ...
    let sender_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &transaction, Intent::sui_transaction()).await.unwrap();
    ...
    let signed_transaction = sui_types::transaction::Transaction::from_data(transaction, vec![sender_sig, sponsor_sig]);
    let effects = client.execute_transaction(&signed_transaction).await.unwrap().effects;
...
async fn resolve_transaction_mutable_shared_object() {
    ...
    unresolved_transaction.kind = Some(TransactionKind::from({
        ...
        ptb.commands = vec![Command::from({
            ...
            message.arguments = vec![Argument::new_input(0), Argument::new_input(1), Argument::new_input(2)];
            message
        })];
        ptb
    }));
...
async fn resolve_transaction_insufficient_gas_with_payment_objects() {
    ...
    let coin_arg = builder.obj(ObjectArg::ImmOrOwnedObject(coin_to_split)).unwrap();
    ...
    let split_coin = match split_result {
        sui_types::transaction::Argument::Result(idx) => sui_types::transaction::Argument::NestedResult(idx, 0),
        _ => panic!("Expected Result argument"),
    };
    builder.transfer_arg(sender, split_coin);

    let ptb = builder.finish();
    let tx_data = TransactionData::new_programmable(sender, vec![gas_for_split], ptb, 10_000_000, 1000);
    ...
    let mut client = sui_rpc::client::Client::new(test_cluster.rpc_url()).unwrap();

    let mut transaction = sui_rpc::proto::sui::rpc::v2beta2::Transaction::default();
    transaction.bcs = Some(sui_rpc::proto::sui::rpc::v2beta2::Bcs::serialize(signed_tx.transaction_data()).unwrap());

    let signatures = signed_tx
        .tx_signatures()
        .iter()
        .map(|s| {
            let mut message = sui_rpc::proto::sui::rpc::v2beta2::UserSignature::default();
            message.bcs = Some({
                let mut message = sui_rpc::proto::sui::rpc::v2beta2::Bcs::default();
    ...
    let mut request = sui_rpc::proto::sui::rpc::v2beta2::ExecuteTransactionRequest::default();
    ...
    let executed_tx =
        client.execute_transaction_and_wait_for_checkpoint(request, std::time::Duration::from_secs(10)).await.unwrap();
    ...
    let effects: sui_types::effects::TransactionEffects = effects_proto.bcs.unwrap().deserialize().unwrap();
    ...
    assert_eq!(error.code(), tonic::Code::InvalidArgument, "Expected InvalidArgument error code");
    assert_contains(error.message(), "Insufficient gas balance to cover estimated transaction cost.");
}
