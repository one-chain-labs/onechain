...
async fn get_checkpoint() {
    ...
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions, .. } =
    ...
    // Request all fields
    let Checkpoint { sequence_number, digest, summary, signature, contents, transactions, .. } = client
    ...
    let mut found_transaction = false;
    for ExecutedTransaction { digest, transaction, effects, events, input_objects, output_objects, .. } in transactions {
        assert!(digest.is_some());
        if digest == Some(transaction_digest.to_string()) {
            found_transaction = true;
        ...
        for Object { bcs, object_id, version, digest, owner, object_type, .. } in
