...
async fn get_object() {
    ...
    let Object {
        ...
        json,
        ..
    } = client.get_object(GetObjectRequest::new(&id)).await.unwrap().into_inner().object.unwrap();
    ...
    let Object {
        ...
        .get_object(
            GetObjectRequest::new(&id).with_version(1u64).with_read_mask(FieldMask::from_str("object_id,version")),
        )
        .await
        .unwrap()
        .into_inner()
...
async fn batch_get_objects() {
    ...
    assert_eq!(objects[0].object().object_id, Some("0x1".parse::<Address>().unwrap().to_string()));
    assert_eq!(objects[1].object().object_id, Some("0x2".parse::<Address>().unwrap().to_string()));
    assert_eq!(objects[2].object().object_id, Some("0x3".parse::<Address>().unwrap().to_string()));
}
