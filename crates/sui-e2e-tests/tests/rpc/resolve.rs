...
fn build_resolve_request(transaction: &unresolved::Transaction, simulate: bool) -> ResolveTransactionRequest {
    let read_mask = if simulate { Some(FieldMask { paths: vec!["simulation".to_string()] }) } else { None };
    ResolveTransactionRequest { unresolved_transaction: Some(serde_json::to_string(transaction).unwrap()), read_mask }
}
...
fn proto_to_response(
    ...
    ResolveTransactionResponse {
        ...
        simulation: proto.simulation.map(|simulation| TransactionSimulationResponse {
            effects: simulation.effects_bcs.unwrap().deserialize().unwrap(),
            events: simulation.events_bcs.map(|events_bcs| events_bcs.deserialize().unwrap()),
...
async fn resolve_transaction_simple_transfer() {
    ...
    let mut alpha_client = sui_rpc_api::proto::node::v2alpha::node_service_client::NodeServiceClient::connect(
