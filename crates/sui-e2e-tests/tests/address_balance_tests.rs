...
async fn get_sender_and_all_gas(context: &mut WalletContext) -> (SuiAddress, Vec<ObjectRef>) {
    ...
    let gas = context.gas_objects(sender).await.unwrap().into_iter().map(|(_, obj)| obj.object_ref()).collect();
...
fn create_transaction_with_expiration(
    ...
    TransactionData::V1(TransactionDataV1 {
        kind: tx,
        sender,
        gas_data: GasData { payment: vec![gas_coin], owner: sender, price: rgp, budget: 10000000 },
...
async fn test_accumulators_root_created() {
    ...
    test_cluster.fullnode_handle.sui_node.with(|node| {
        ...
        assert!(!state.load_epoch_store_one_call_per_task().accumulator_root_exists());
    });
    ...
    test_cluster.fullnode_handle.sui_node.with(|node| {
        let state = node.state();
        assert!(state.load_epoch_store_one_call_per_task().accumulator_root_exists());
        assert_eq!(state.load_epoch_store_one_call_per_task().protocol_config().version, ProtocolVersion::MAX);
    });
    ...
    test_cluster.fullnode_handle.sui_node.with(|node| {
        let state = node.state();
        assert_eq!(state.load_epoch_store_one_call_per_task().protocol_config().version, ProtocolVersion::MAX_ALLOWED);
    });
}
...
async fn test_accumulators_disabled() {
    ...
    let withdraw_tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(1000), vec![(1000, dbg_addr(2))])
        .build();
    let withdraw_tx = test_cluster.wallet.sign_transaction(&withdraw_tx).await;
    test_cluster.wallet.execute_transaction_may_fail(withdraw_tx).await.unwrap_err();

    // Transfer fails at execution time
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas), vec![(1000, recipient)])
        .build();

    let signed = test_cluster.wallet.sign_transaction(&tx).await;
    let effects = test_cluster.wallet.execute_transaction_may_fail(signed).await.unwrap().effects.unwrap();
    ...
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas), vec![(1000, sender)])
        .build();

    let gas =
        test_cluster.sign_and_execute_transaction(&tx).await.effects.unwrap().gas_object().reference.to_object_ref();
    ...
    let withdraw_tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(1000), vec![(1000, dbg_addr(2))])
        .build();
    test_cluster.sign_and_execute_transaction(&withdraw_tx).await;
...
async fn test_multiple_settlement_txns() {
    ...
    let amounts_and_recipients = (0 .. 20).map(|_| (1u64, SuiAddress::random_for_testing_only())).collect::<Vec<_>>();

    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas), amounts_and_recipients.clone())
        .build();
...
fn get_balance(child_object_resolver: &dyn ChildObjectResolver, owner: SuiAddress) -> u64 {
    ...
    let accumulator_value =
        AccumulatorValue::load(child_object_resolver, None, owner, &sui_coin_type).expect("read cannot fail");
...
fn verify_accumulator_exists(child_object_resolver: &dyn ChildObjectResolver, owner: SuiAddress, expected_balance: u64) {
    let sui_coin_type = Balance::type_tag(GAS::type_tag());
    ...
    let accumulator_object = AccumulatorValue::load_object(child_object_resolver, None, owner, &sui_coin_type)
        .expect("read cannot fail")
        .expect("accumulator should exist");

    assert!(accumulator_object.data.try_as_move().unwrap().type_().is_efficient_representation());

    let accumulator_value = AccumulatorValue::load(child_object_resolver, None, owner, &sui_coin_type)
        .expect("read cannot fail")
        .expect("accumulator should exist");

    assert_eq!(
        accumulator_value,
        AccumulatorValue::U128(U128 { value: expected_balance as u128 }),
    ...
    let owner =
        AccumulatorOwner::load(child_object_resolver, None, owner).expect("read cannot fail").expect("owner must exist");

    assert!(
        owner.metadata_exists(child_object_resolver, None, &sui_coin_type).unwrap(),
        "Metadata object should have been created"
    );

    let _metadata = owner.load_metadata(child_object_resolver, None, &sui_coin_type).unwrap();
}
...
async fn test_deposit_and_withdraw_with_larger_reservation() {
    ...
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(1000), vec![(800, dbg_addr(2))])
        .build();
    test_cluster.sign_and_execute_transaction(&tx).await;
...
async fn test_withdraw_non_existent_balance() {
    ...
    let mut test_cluster = TestClusterBuilder::new().with_num_validators(1).build().await;
    let rgp = test_cluster.get_reference_gas_price().await;
    let context = &mut test_cluster.wallet;
    ...
    let err = test_cluster.wallet.execute_transaction_may_fail(signed_tx).await.unwrap_err();
...
async fn test_withdraw_insufficient_balance() {
    ...
    let tx = TestTransactionBuilder::new(sender, gas1, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas1), vec![(1000, sender)])
        .build();
    let res = test_cluster.sign_and_execute_transaction(&tx).await;
    ...
    let tx = TestTransactionBuilder::new(sender, gas1, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(1001), vec![(1001, dbg_addr(2))])
        .build();
    let signed_tx = test_cluster.sign_transaction(&tx).await;
    let err = test_cluster.wallet.execute_transaction_may_fail(signed_tx).await.unwrap_err();
    ...
    let tx1 = TestTransactionBuilder::new(sender, gas1, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(500), vec![(500, dbg_addr(2))])
        .build();
    let tx2 = TestTransactionBuilder::new(sender, gas2, rgp)
        .transfer_oct_to_address_balance(FundSource::address_fund_with_reservation(501), vec![(501, dbg_addr(2))])
        .build();

    let mut effects = test_cluster.sign_and_execute_txns_in_soft_bundle(&[tx1, tx2]).await.unwrap();
    ...
    assert!(effects2.status().is_err(), "Expected transaction to fail due to insufficient balance");
...
async fn test_address_balance_gas() {
    ...
    let mut test_cluster = TestClusterBuilder::new().build().await;
    let (sender, gas_package_id) = setup_address_balance_account(&mut test_cluster, 10_000_000).await;
    ...
    {
        let mut tx = create_storage_test_transaction_address_balance(sender, gas_package_id, rgp, chain_id, None, 0);
    ...
    let tx = create_storage_test_transaction_address_balance(sender, gas_package_id, rgp, chain_id, None, 0);
    ...
    assert!(gas_used > 0, "Gas used should be greater than 0 with Move function calls, got: {}", gas_used);
...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let sender_gas = test_cluster.wallet.gas_objects(sender).await.unwrap().pop().unwrap().1.object_ref();
    let deposit_tx_sender = TestTransactionBuilder::new(sender, sender_gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(sender_gas), vec![(100_000_000, sender)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx_sender).await;

    let sponsor_gas = test_cluster.wallet.gas_objects(sponsor).await.unwrap().pop().unwrap().1.object_ref();
    let deposit_tx_sponsor = TestTransactionBuilder::new(sponsor, sponsor_gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(sponsor_gas), vec![(100_000_000, sponsor)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx_sponsor).await;

    let create_txn =
        create_storage_test_transaction_address_balance(sender, gas_test_package_id, rgp, chain_id, Some(sponsor), 0);

    let sender_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &create_txn, Intent::sui_transaction()).await.unwrap();
    let sponsor_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &create_txn, Intent::sui_transaction()).await.unwrap();
    ...
    assert!(gas_used > 0, "Gas should be charged for sponsored transaction, got: {}", gas_used);
    ...
    test_cluster.fullnode_handle.sui_node.with(|node| {
        ...
        assert_eq!(sender_actual, 100_000_000, "Sender balance should remain at 100_000_000, got: {}", sender_actual);
    });

    let created_objects: Vec<_> = create_effects.created().iter().collect();
    assert_eq!(created_objects.len(), 1, "Should have created exactly one object");
    ...
    let sender_delete_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &delete_txn, Intent::sui_transaction()).await.unwrap();
    let sponsor_delete_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &delete_txn, Intent::sui_transaction()).await.unwrap();

    let signed_delete_txn = Transaction::from_data(delete_txn, vec![sender_delete_sig, sponsor_delete_sig]);
    ...
    test_cluster.fullnode_handle.sui_node.with(|node| {
        ...
        assert_eq!(sender_final, 100_000_000, "Sender balance should remain unchanged at 100_000_000");
        assert_ne!(sponsor_final, 100_000_000, "Sponsor balance should have changed from 100_000_000");
    });
...
async fn setup_test_package(context: &mut WalletContext) -> ObjectID {
    ...
    let txn = context
        .sign_transaction(
            &TestTransactionBuilder::new(sender, gas_object, gas_price).publish(move_test_code_path).build(),
        )
        .await;
...
async fn setup_address_balance_account(test_cluster: &mut TestCluster, deposit_amount: u64) -> (SuiAddress, ObjectID) {
    let rgp = test_cluster.get_reference_gas_price().await;
    let context = &mut test_cluster.wallet;
    ...
    let deposit_tx = TestTransactionBuilder::new(sender, gas_objects[1], rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas_objects[1]), vec![(deposit_amount, sender)])
        .build();
...
fn create_abort_test_transaction_kind(gas_test_package_id: ObjectID, should_abort: bool) -> TransactionKind {
    let mut builder = ProgrammableTransactionBuilder::new();
...
fn create_address_balance_transaction_with_sponsor(
    ...
    TransactionData::V1(TransactionDataV1 {
        kind: tx_kind,
        sender,
        gas_data: GasData { payment: vec![], owner: gas_owner, price: rgp, budget },
        expiration: TransactionExpiration::ValidDuring {
            min_epoch: Some(0),
            max_epoch: Some(0),
            min_timestamp_seconds: None,
...
fn create_storage_test_transaction_address_balance(
    ...
    TransactionData::V1(TransactionDataV1 {
        kind: tx,
        sender,
        gas_data: GasData { payment: vec![], owner: gas_owner, price: rgp, budget: 10000000 },
        expiration: TransactionExpiration::ValidDuring {
            min_epoch: Some(0),
            max_epoch: Some(0),
            min_timestamp_seconds: None,
...
fn create_delete_test_transaction_kind(gas_test_package_id: ObjectID, object_to_delete: ObjectRef) -> TransactionKind {
    let mut builder = ProgrammableTransactionBuilder::new();
    let object_arg = builder.obj(sui_types::transaction::ObjectArg::ImmOrOwnedObject(object_to_delete)).unwrap();
    builder.programmable_move_call(
        gas_test_package_id,
        Identifier::new("gas_test").unwrap(),
...
fn create_withdraw_balance_transaction(
    ...
    let split_withdraw_arg = builder.programmable_move_call(
        ...
        vec!["0x2::balance::Balance<0x2::oct::OCT>".parse().unwrap()],
    ...
    TransactionData::V1(TransactionDataV1 {
        kind: tx,
        sender,
        gas_data: GasData { payment: vec![], owner: sender, price: rgp, budget: 10000000 },
        expiration: TransactionExpiration::ValidDuring {
            min_epoch: Some(0),
            max_epoch: Some(0),
            min_timestamp_seconds: None,
...
async fn test_regular_gas_payment_with_valid_during_current_epoch() {
    ...
    let tx = create_regular_gas_transaction_with_current_epoch(sender, gas_coin, rgp, current_epoch, chain_id);

    let signed_tx = test_cluster.sign_transaction(&tx).await;
    let (effects, _) = test_cluster.execute_transaction_return_raw_effects(signed_tx).await.unwrap();

    assert!(effects.status().is_ok(), "Transaction should execute successfully. Error: {:?}", effects.status());
}
...
async fn test_transaction_expired_too_early() {
    ...
    let tx = create_regular_gas_transaction_with_current_epoch(sender, gas_coin, rgp, future_epoch, chain_id);

    let signed_tx = test_cluster.sign_transaction(&tx).await;
    let result = test_cluster.execute_transaction_return_raw_effects(signed_tx).await;

    match result {
        Err(err) => {
            let err_str = format!("{:?}", err);
            assert!(err_str.contains("TransactionExpired"), "Expected TransactionExpired error, got: {:?}", err);
        }
...
async fn test_transaction_expired_too_late() {
    ...
    let tx = create_regular_gas_transaction_with_current_epoch(sender, gas_coin, rgp, past_epoch, chain_id);
...
async fn test_transaction_invalid_chain_id() {
    ...
    let tx = create_regular_gas_transaction_with_current_epoch(sender, gas_coin, rgp, current_epoch, wrong_chain_id);
    ...
    match result {
        Err(err) => {
            let err_str = format!("{:?}", err);
            assert!(err_str.contains("InvalidChainId"), "Expected InvalidChainId error, got: {:?}", err);
        }
...
async fn test_transaction_expiration_min_none_max_some() {
    ...
    let tx = create_transaction_with_expiration(sender, gas_coin, rgp, None, Some(current_epoch + 5), chain_id, 12345);
...
async fn test_transaction_expiration_edge_cases() {
    ...
    let result1 = test_cluster.execute_transaction_return_raw_effects(test_cluster.sign_transaction(&tx1).await).await;
    assert!(result1.is_ok(), "Single epoch window transaction should succeed");
    ...
    let result2 = test_cluster.execute_transaction_return_raw_effects(test_cluster.sign_transaction(&tx2).await).await;
    ...
    let result3 = test_cluster.execute_transaction_return_raw_effects(test_cluster.sign_transaction(&tx3).await).await;
    match result3 {
        Err(err) => {
            let err_str = format!("{:?}", err);
            assert!(
...
async fn test_address_balance_gas_cost_parity() {
    ...
    let mut test_cluster = TestClusterBuilder::new().with_num_validators(1).build().await;
    let (sender, gas_test_package_id) = setup_address_balance_account(&mut test_cluster, 100_000_000).await;

    let rgp = test_cluster.get_reference_gas_price().await;
    let chain_id = test_cluster.get_chain_identifier();

    let gas_coin = test_cluster.wallet.get_one_gas_object().await.unwrap().unwrap().1;
    ...
    let address_balance_tx =
        create_storage_test_transaction_address_balance(sender, gas_test_package_id, rgp, chain_id, None, 0);
    let address_balance_resp = test_cluster.sign_and_execute_transaction(&address_balance_tx).await;
    ...
    assert!(coin_gas_summary.storage_cost > 0, "Coin payment should have storage costs for object creation");
    assert!(
    ...
    let address_balance_created_objects: Vec<_> = address_balance_effects.created().iter().collect();

    assert_eq!(coin_created_objects.len(), 1, "Should have created exactly one object with coin payment");
    assert_eq!(
    ...
    let gas_coin_for_delete = test_cluster.wallet.get_one_gas_object().await.unwrap().unwrap().1;

    let coin_delete_tx =
        create_delete_transaction_gas(sender, gas_test_package_id, coin_created_obj, gas_coin_for_delete, rgp);
    let coin_delete_resp = test_cluster.sign_and_execute_transaction(&coin_delete_tx).await;
    ...
    let address_balance_delete_resp = test_cluster.sign_and_execute_transaction(&address_balance_delete_tx).await;
    ...
    assert!(coin_delete_effects.status().is_ok(), "Coin deletion should succeed");
    assert!(address_balance_delete_effects.status().is_ok(), "Address balance deletion should succeed");

    assert!(coin_delete_gas_summary.storage_rebate > 0, "Coin deletion should provide non-zero storage rebate");
    assert!(
    ...
    assert_eq!(
        coin_delete_gas_summary.computation_cost, address_balance_delete_gas_summary.computation_cost,
...
async fn test_address_balance_gas_charged_on_move_abort() {
    ...
    let mut test_cluster = TestClusterBuilder::new().with_num_validators(1).build().await;
    let (sender, gas_test_package_id) = setup_address_balance_account(&mut test_cluster, 10_000_000).await;
    ...
    let abort_tx = create_abort_test_transaction_address_balance(sender, gas_test_package_id, rgp, chain_id, true, None);
    ...
    assert!(effects.status().is_err(), "Expected transaction to fail due to Move abort");

    let gas_summary = effects.gas_cost_summary();
    let gas_used = calculate_total_gas_cost(gas_summary);

    assert!(gas_used > 0, "Gas should still be charged even on Move abort, got: {}", gas_used);
    assert!(gas_summary.computation_cost > 0, "Computation cost should be charged for work done before abort");
...
async fn test_explicit_sponsor_withdrawal_banned() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let withdrawal =
        FundsWithdrawalArg::balance_from_sponsor(1000, sui_types::type_input::TypeInput::from(GAS::type_tag()));
    ...
    let tx = TransactionData::V1(TransactionDataV1 {
        ...
        sender,
        gas_data: GasData { payment: vec![], owner: sponsor, price: rgp, budget: 10000000 },
        expiration: TransactionExpiration::ValidDuring {
            min_epoch: Some(0),
            max_epoch: Some(0),
            min_timestamp_seconds: None,
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let sponsor_gas = test_cluster.wallet.gas_objects(sponsor).await.unwrap().pop().unwrap().1.object_ref();
    let deposit_tx_sponsor = TestTransactionBuilder::new(sponsor, sponsor_gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(sponsor_gas), vec![(sponsor_initial_balance, sponsor)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx_sponsor).await;
    ...
    let tx1 =
        create_storage_test_transaction_address_balance(sender, gas_test_package_id, rgp, chain_id, Some(sponsor), 0);
    let tx2 =
        create_storage_test_transaction_address_balance(sender, gas_test_package_id, rgp, chain_id, Some(sponsor), 1);

    let sender_sig1 =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &tx1, Intent::sui_transaction()).await.unwrap();
    let sponsor_sig1 =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &tx1, Intent::sui_transaction()).await.unwrap();
    let signed_tx1 = Transaction::from_data(tx1, vec![sender_sig1, sponsor_sig1]);

    let sender_sig2 =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &tx2, Intent::sui_transaction()).await.unwrap();
    let sponsor_sig2 =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &tx2, Intent::sui_transaction()).await.unwrap();
    ...
    assert_ne!(tx1_digest, tx2_digest, "Transaction digests should be different");

    let mut effects = test_cluster.execute_signed_txns_in_soft_bundle(&[signed_tx1, signed_tx2]).await.unwrap();

    test_cluster.wait_for_tx_settlement(&[tx1_digest, tx2_digest]).await;
    ...
    let (succeeded_effects, succeeded_gas, failed_effects, failed_gas) = if tx1_effects.status().is_ok() {
        (&tx1_effects, tx1_total_gas, &tx2_effects, tx2_total_gas)
    } else {
        (&tx2_effects, tx2_total_gas, &tx1_effects, tx1_total_gas)
    };

    assert!(succeeded_effects.status().is_ok(), "One transaction should succeed");
    assert!(!failed_effects.status().is_ok(), "One transaction should fail");
    assert!(succeeded_gas > 0, "Successful transaction should have gas charged");
    assert_eq!(failed_gas, 0, "Failed transaction should have zero gas charged");
    ...
    assert_eq!(final_sender_balance, 100_000_000, "Sender balance should remain unchanged");
...
async fn test_insufficient_balance_charges_zero_gas() {
    ...
    let deposit_tx = TestTransactionBuilder::new(sender, gas_for_deposit, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas_for_deposit), vec![(initial_balance, sender)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx).await;
...
async fn test_soft_bundle_different_gas_payers() {
    ...
    let sender1_gas = test_cluster.wallet.gas_objects(sender1).await.unwrap().pop().unwrap().1.object_ref();
    let deposit_tx1 = TestTransactionBuilder::new(sender1, sender1_gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(sender1_gas), vec![(10_000_000, sender1)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx1).await;

    let sender2_gas = test_cluster.wallet.gas_objects(sender2).await.unwrap().pop().unwrap().1.object_ref();
    let deposit_tx2 = TestTransactionBuilder::new(sender2, sender2_gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(sender2_gas), vec![(10_000_000, sender2)])
        .build();
    test_cluster.sign_and_execute_transaction(&deposit_tx2).await;

    let tx1 = create_storage_test_transaction_address_balance(sender1, gas_test_package_id, rgp, chain_id, None, 0);

    let tx2 = create_storage_test_transaction_address_balance(sender2, gas_test_package_id, rgp, chain_id, None, 1);
...
async fn test_multiple_deposits_merged_in_effects() {
    ...
    let initial_deposit = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_oct_to_address_balance(FundSource::coin(gas), vec![(10000, recipient)])
        .build();
    let res = test_cluster.sign_and_execute_transaction(&initial_deposit).await;
    let gas = res.effects.unwrap().gas_object().reference.to_object_ref();
    ...
    for amount in &withdraw_amounts {
        ...
        let amount_arg = builder.pure(move_core_types::u256::U256::from(*amount)).unwrap();
    ...
    assert!(effects.status().is_ok(), "Transaction should succeed, got: {:?}", effects.status());

    let acc_events = effects.accumulator_events();
    assert_eq!(acc_events.len(), 1, "Should have exactly 1 accumulator event (merged), got: {}", acc_events.len());

    let event = &acc_events[0];
    assert_eq!(event.write.address.address, recipient, "Accumulator event should be for the recipient");
...
async fn test_address_balance_gas_budget_enforcement_with_storage() {
    ...
    let mut test_cluster = TestClusterBuilder::new().with_num_validators(1).build().await;
    let (sender, gas_package_id) = setup_address_balance_account(&mut test_cluster, 100_000_000).await;
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let obj_arg = builder.obj(ObjectArg::ImmOrOwnedObject(created_obj)).unwrap();
    ...
    assert!(
        matches!(error_kind, sui_types::execution_status::ExecutionFailureStatus::InsufficientGas),
...
async fn test_address_balance_large_rebate() {
    ...
    assert!(effects.status().is_ok(), "Creating object should succeed, got: {:?}", effects.status());
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let object_arg = builder.obj(sui_types::transaction::ObjectArg::ImmOrOwnedObject(created_object_ref)).unwrap();
    builder.programmable_move_call(
        gas_package_id,
        Identifier::new("gas_test").unwrap(),
        Identifier::new("delete_object").unwrap(),
    ...
    assert!(effects.status().is_ok(), "Deleting object should succeed, got: {:?}", effects.status());

    let gas_summary = effects.gas_cost_summary();
    let net_gas = (gas_summary.computation_cost + gas_summary.storage_cost) as i128 - gas_summary.storage_rebate as i128;
    ...
    assert!(net_gas < 0, "Net gas ({}) should be negative when rebate exceeds total gas cost", net_gas);
...
async fn test_sponsored_address_balance_storage_oog() {
    ...
    let tx =
        create_address_balance_transaction_with_sponsor(tx_kind, sender, Some(sponsor), sponsor_budget, rgp, chain_id);

    let sender_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sender, &tx, Intent::sui_transaction()).await.unwrap();
    let sponsor_sig =
        test_cluster.wallet.config.keystore.sign_secure(&sponsor, &tx, Intent::sui_transaction()).await.unwrap();
    ...
    let gas_summary = effects.gas_cost_summary();
    let total_gas_cost = gas_summary.computation_cost + gas_summary.storage_cost - gas_summary.storage_rebate;
