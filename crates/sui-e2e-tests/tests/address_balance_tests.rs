...
async fn get_sender_and_gas(context: &mut WalletContext) -> (SuiAddress, ObjectRef) {
    ...
    let gas = context.gas_objects(sender).await.unwrap().pop().unwrap().1.object_ref();
...
async fn test_withdraw_non_existent_balance() {
    ...
    assert!(effects.status().is_err(), "Expected transaction to fail due to underflow");
...
fn withdraw_from_balance_tx(amount: u64, sender: SuiAddress, gas: ObjectRef, rgp: u64) -> TransactionData {
