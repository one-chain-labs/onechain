...
async fn test_passkey_fails_mismatched_challenge() {
    ...
    assert_eq!(err, SuiErrorKind::InvalidSignature { error: "Invalid challenge".to_string() });
...
async fn test_passkey_fails_to_verify_sig() {
    ...
    let res = execute_tx(tx, &test_cluster).await;
    let err = res.unwrap_err();
    assert_eq!(err, SuiErrorKind::InvalidSignature { error: "Fails to verify".to_string() });
