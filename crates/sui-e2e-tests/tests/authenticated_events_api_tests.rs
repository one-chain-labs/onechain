...
use sui_light_client::{
    mmr::apply_stream_updates,
    proof::{
        base::{Proof, ProofContents, ProofTarget, ProofVerifier},
        committee::extract_new_committee_info,
        ocs::{OCSInclusionProof, OCSProof, OCSTarget},
    },
};
...
use sui_rpc_api::grpc::alpha::{
    event_service_proto::{
        ...
        AuthenticatedEvent,
        ListAuthenticatedEventsRequest,
    },
    proof_service_proto::{proof_service_client::ProofServiceClient, GetObjectInclusionProofRequest},
};
use sui_sdk_types::ValidatorCommittee;
use sui_types::{
    accumulator_root as ar,
    accumulator_root::EventCommitment,
    base_types::{ObjectID, SequenceNumber, SuiAddress},
    committee::Committee,
    digests::{Digest, ObjectDigest},
    dynamic_field::{DynamicFieldKey, Field},
    object::Object,
...
async fn publish_test_package(test_cluster: &TestCluster) -> ObjectID {
    ...
    let (sender, gas_object) = test_cluster.wallet.get_one_gas_object().await.unwrap().unwrap();
    let txn = test_cluster
        ...
        .await;
    let resp = test_cluster.wallet.execute_transaction_must_succeed(txn).await;
    resp.get_new_package_obj().unwrap().0
}

async fn emit_test_event(test_cluster: &TestCluster, package_id: ObjectID, sender: SuiAddress, value: u64) {
    ...
    let gas_object = test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap();
...
async fn query_authenticated_events(
...
) -> Result<sui_rpc_api::grpc::alpha::event_service_proto::ListAuthenticatedEventsResponse, tonic::Status> {
    let mut client = EventServiceClient::connect(rpc_url.to_owned()).await.unwrap();
    ...
    client.list_authenticated_events(req).await.map(|r| r.into_inner())
}
...
async fn list_authenticated_events(
    ...
    let mut event_client = EventServiceClient::connect(rpc_url.to_owned()).await.unwrap();
    ...
    loop {
        ...
        let response = event_client.list_authenticated_events(req).await.unwrap().into_inner();

        let event_count = response.events.len();
        all_events.extend(response.events);

        if response.next_page_token.is_none() || response.next_page_token.as_ref().unwrap().is_empty() {
            break;
        }
...
async fn verify_events_with_stream_head(
    ...
    let mut proof_client = ProofServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();

    let mut ledger_client = LedgerServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();

    let current_epoch = test_cluster.fullnode_handle.sui_node.state().epoch_store_for_testing().epoch();
    ...
    let events_by_accum_version: Vec<Vec<EventCommitment>> = events
        ...
        .map(|event| {
            let commitment = convert_grpc_event_to_commitment(event).expect("should convert event to commitment");
            let accumulator_version = event.accumulator_version.expect("Missing accumulator_version");
    ...
    let calculated_stream_head = apply_stream_updates(&first_stream_head.value, events_by_accum_version);
...
fn proto_object_ref_to_sui_object_ref(
    ...
    let object_id_str = object_ref_proto.object_id.as_ref().ok_or("Missing object_id")?;
    let object_id = ObjectID::from_str(object_id_str).map_err(|e| format!("Invalid object_id: {}", e))?;
    ...
    let digest = ObjectDigest::from_str(digest_str).map_err(|e| format!("Invalid digest: {}", e))?;
...
fn proto_bytes_to_digest(bytes: &[u8]) -> Result<Digest, String> {
    let digest: [u8; 32] =
        bytes.try_into().map_err(|_| format!("Invalid digest length: expected 32, got {}", bytes.len()))?;
...
fn proto_ocs_inclusion_proof_to_light_client_proof(
    ...
    let merkle_proof_bytes = grpc_proof.merkle_proof.as_ref().ok_or("Missing merkle_proof")?;
    let merkle_proof =
        bcs::from_bytes(merkle_proof_bytes).map_err(|e| format!("Failed to deserialize merkle_proof: {}", e))?;
    ...
    Ok(OCSInclusionProof { merkle_proof, leaf_index, tree_root })
}

fn convert_grpc_event_to_commitment(auth_event: &AuthenticatedEvent) -> Result<EventCommitment, String> {
    ...
    let sender =
        sui_types::base_types::SuiAddress::from_str(sender).map_err(|e| format!("Failed to parse sender: {}", e))?;
    let type_tag: move_core_types::language_storage::StructTag =
        event_type.parse().map_err(|e| format!("Failed to parse event_type: {}", e))?;
...
async fn get_committee_for_epoch_via_api(
    ...
    let proto_committee =
        response.epoch.ok_or("Missing epoch in response")?.committee.ok_or("Missing committee in epoch response")?;
...
async fn get_last_checkpoint_of_epoch(
    ...
    let next_epoch_response = ledger_client
        .get_epoch(GetEpochRequest::new(epoch + 1).with_read_mask(FieldMask::from_paths(["first_checkpoint"])))
        .await
        .map_err(|e| format!("Failed to get epoch {} from API: {}", epoch + 1, e))?
        .into_inner();

    let next_epoch = next_epoch_response.epoch.ok_or_else(|| format!("Missing epoch {} in response", epoch + 1))?;

    let first_checkpoint =
        next_epoch.first_checkpoint.ok_or_else(|| format!("Missing first_checkpoint for epoch {}", epoch + 1))?;
        ...
        self.committees
            ...
            .ok_or_else(|| {
                format!(
                    ...
                    checkpoint_seq,
                    self.committees.iter().map(|(start, end, _)| (start, end)).collect::<Vec<_>>()
                )
            })
    }
}
...
async fn build_epoch_cache(
    ...
    for epoch in 0 .. current_epoch {
        ...
        committees.push((prev_epoch_end_checkpoint, end_of_epoch_checkpoint_seq, current_committee.clone()));
        ...
        let checkpoint_response = ledger_client
            ...
            .await
            .map_err(|e| format!("Failed to fetch checkpoint {}: {}", end_of_epoch_checkpoint_seq, e))?
            .into_inner();

        let proto_checkpoint = checkpoint_response.checkpoint.ok_or("Missing checkpoint in response")?;

        let checkpoint: sui_types::full_checkpoint_content::Checkpoint =
            (&proto_checkpoint).try_into().map_err(|e| format!("Failed to convert checkpoint: {:?}", e))?;
        ...
        let next_committee = extract_new_committee_info(&checkpoint.summary).map_err(|e| {
            format!("Failed to extract committee from checkpoint {}: {}", end_of_epoch_checkpoint_seq, e)
        })?;
...
async fn verify_ocs_inclusion_proof(
    ...
    let proof = Proof {
        ...
        proof_contents: ProofContents::ObjectCheckpointStateProof(OCSProof::Inclusion(ocs_inclusion_proof)),
    };

    let committee = epoch_cache.get_committee_for_checkpoint(checkpoint_seq)?;

    proof.verify(committee).map_err(|e| format!("Proof verification failed: {:?}", e))?;
...
async fn fetch_and_verify_event_stream_head(
    ...
    let response = proof_client.get_object_inclusion_proof(req).await.unwrap().into_inner();

    let object_ref = response.object_ref.expect("object_ref should be present");

    assert!(object_ref.object_id.is_some(), "object_id should be present in object_ref");
    assert!(object_ref.version.is_some(), "version should be present in object_ref");
    assert!(object_ref.digest.is_some(), "digest should be present in object_ref");

    let inclusion_proof = response.inclusion_proof.expect("inclusion_proof should be present");

    assert!(inclusion_proof.merkle_proof.is_some(), "merkle_proof should be present");
    assert!(inclusion_proof.tree_root.is_some(), "tree_root should be present");

    let object_data_bytes = response.object_data.expect("object_data should be present");

    let object: Object = bcs::from_bytes(&object_data_bytes).expect("should deserialize object from BCS");

    let move_obj = object.data.try_as_move().expect("should be move object");
    let stream_head: Field<ar::AccumulatorKey, ar::EventStreamHead> =
        move_obj.to_rust().expect("should deserialize to EventStreamHead");
    ...
    let checkpoint_response = ledger_client
        .get_checkpoint(GetCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(FieldMask::from_paths([
            "summary",
            "signature",
            "contents",
        ])))
    ...
    let proto_checkpoint = checkpoint_response.checkpoint.expect("Missing checkpoint in response");

    let checkpoint_data: sui_types::full_checkpoint_content::Checkpoint =
        (&proto_checkpoint).try_into().expect("Failed to convert checkpoint");

    verify_ocs_inclusion_proof(epoch_cache, &checkpoint_data.summary, &object_ref, &inclusion_proof, checkpoint)
        .await
...
async fn list_authenticated_events_end_to_end() {
    ...
    let all_events = list_authenticated_events(test_cluster.rpc_url(), &package_id.to_string(), 0, None).await;
    ...
    let found = all_events.iter().any(|event| match &event.event {
        Some(Event { contents: Some(bcs), .. }) => bcs.value.as_ref().is_some_and(|v| !v.is_empty()),
        _ => false,
    });
...
async fn list_authenticated_events_page_size_validation() {
    ...
    let test_cluster = test_cluster::TestClusterBuilder::new().with_rpc_config(rpc_config).build().await;
    let sender = test_cluster.wallet.config.keystore.addresses()[0];

    let response = query_authenticated_events(test_cluster.rpc_url(), &sender.to_string(), 0, Some(1500)).await.unwrap();
...
async fn list_authenticated_events_start_beyond_highest() {
    ...
    let probe_response =
        query_authenticated_events(test_cluster.rpc_url(), &sender.to_string(), 0, Some(1)).await.unwrap();
    let highest = probe_response.highest_indexed_checkpoint.unwrap_or(0);

    let response =
        query_authenticated_events(test_cluster.rpc_url(), &sender.to_string(), highest + 1000, Some(10)).await.unwrap();
...
async fn list_authenticated_events_pruned_checkpoint_error() {
    ...
    let response = query_authenticated_events(test_cluster.rpc_url(), &sender.to_string(), 0, Some(10)).await.unwrap();
...
async fn authenticated_events_disabled_test() {
    ...
    let result = query_authenticated_events(test_cluster.rpc_url(), &sender.to_string(), 0, Some(10)).await;

    assert!(result.is_err(), "Expected error when authenticated events indexing is disabled");

    let error = result.unwrap_err();
    assert_eq!(error.code(), tonic::Code::Unimplemented);
    assert!(error.message().contains("Authenticated events indexing is disabled"));
}
...
async fn authenticated_events_backfill_test() {
    ...
    let mut test_cluster =
        TestClusterBuilder::new().disable_fullnode_pruning().with_rpc_config(rpc_config_without_indexing).build().await;
    ...
    let rpc_url_with_indexing = {
        let mut new_fullnode_config =
            test_cluster.fullnode_config_builder().build(&mut rand::rngs::OsRng, test_cluster.swarm.config());
        ...
        let new_fullnode_handle = test_cluster.start_fullnode_from_config(new_fullnode_config).await;
    ...
    let response = loop {
        let response =
            query_authenticated_events(&rpc_url_with_indexing, &package_id.to_string(), 0, None).await.unwrap();
        ...
        if start.elapsed() > tokio::time::Duration::from_secs(30) {
            panic!("Timeout waiting for backfill to complete. Found {} events, expected 5", response.events.len());
        }
    ...
    assert_eq!(count, 5, "expected 5 authenticated events after backfill, got {count}");
}
...
async fn authenticated_events_multiple_events_per_transaction() {
    ...
    let test_cluster = TestClusterBuilder::new().disable_fullnode_pruning().with_rpc_config(rpc_config).build().await;
    ...
    let mut event_client = EventServiceClient::connect(test_cluster.rpc_url().to_owned()).await.unwrap();
    ...
    let count = response.events.len();
    assert_eq!(count, 3, "expected 3 authenticated events (all from one transaction), got {count}");
    ...
    let values: Vec<u64> = response
        ...
        .filter_map(|event| {
            event.event.as_ref().and_then(|e| {
                e.contents
                    .as_ref()
                    .and_then(|c| c.value.as_ref().and_then(|v| bcs::from_bytes::<E>(v).ok().map(|e| e.value)))
            })
        })
        .collect();
    ...
    let tx_indices: std::collections::HashSet<u32> =
        response.events.iter().filter_map(|event| event.transaction_idx).collect();

    assert_eq!(tx_indices.len(), 1, "all events should be from the same transaction");
}
...
async fn test_pagination() {
    ...
    let all_events = list_authenticated_events(test_cluster.rpc_url(), &package_id.to_string(), 0, Some(7)).await;

    assert_eq!(all_events.len(), 25, "expected 25 total events across all pages, got {}", all_events.len());
...
async fn test_object_inclusion_proof_error_code() {
    ...
    let highest_checkpoint = test_cluster
        .fullnode_handle
        .sui_node
        .with(|node| node.state().get_checkpoint_store().get_highest_executed_checkpoint_seq_number().unwrap().unwrap());
    ...
    assert!(result.is_err(), "Should fail to get inclusion proof at checkpoint where object was not modified");
...
async fn test_size_based_pagination() {
    ...
    let first_response = event_client.list_authenticated_events(req).await.unwrap().into_inner();
    ...
    assert!(!first_response.events.is_empty(), "Should get at least 1 event (forward progress guarantee)");

    assert!(first_response.next_page_token.is_some(), "Should have next_page_token since not all events were returned");

    let all_events = list_authenticated_events(test_cluster.rpc_url(), &package_id.to_string(), 0, Some(10)).await;

    assert_eq!(all_events.len(), 4, "expected 4 total events across all pages, got {}", all_events.len());
...
async fn authenticated_events_multiple_commits_per_checkpoint() {
    ...
    let gas_for_deposit = test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap();
    ...
    let coin = deposit_builder.command(Command::SplitCoins(Argument::GasCoin, vec![deposit_amount]));
    ...
    let tx_data_vec: Vec<_> = (0 .. event_count)
        .map(|i| {
            ...
            sui_types::transaction::TransactionData::V1(sui_types::transaction::TransactionDataV1 {
                kind: sui_types::transaction::TransactionKind::ProgrammableTransaction(ptb.finish()),
                sender,
    ...
    let bundle_tasks: Vec<_> =
        tx_data_vec.chunks(5).map(|bundle| test_cluster.sign_and_execute_txns_in_soft_bundle(bundle)).collect();
    futures::future::try_join_all(bundle_tasks).await.unwrap();

    let all_events = list_authenticated_events(test_cluster.rpc_url(), &package_id.to_string(), 0, None).await;

    assert_eq!(all_events.len(), event_count as usize, "expected all events, got {}", all_events.len());

    let mut events_by_checkpoint: std::collections::HashMap<u64, std::collections::HashSet<u64>> =
        std::collections::HashMap::new();
    for event in &all_events {
        if let (Some(checkpoint), Some(accumulator_version)) = (event.checkpoint, event.accumulator_version) {
            events_by_checkpoint.entry(checkpoint).or_default().insert(accumulator_version);
        }
    }

    let checkpoint_with_multiple_versions = events_by_checkpoint.iter().find(|(_, versions)| versions.len() > 1);
    ...
    verify_events_with_stream_head(&test_cluster, package_id, &all_events, all_events.len() as u64).await;
}
