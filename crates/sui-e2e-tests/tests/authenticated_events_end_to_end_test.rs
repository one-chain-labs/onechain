...
use sui_types::{
    accumulator_root::{self as ar, EventCommitment, EventStreamHead},
...
async fn try_emit_authenticated_event(
    ...
    let tx_data = TransactionData::new(
        ...
        sender,
        {
            ...
            wallet.gas_objects(sender).await.unwrap().pop().unwrap().1.object_ref()
        },
        10_000_000,
        rgp,
    );
        ...
        for acc_event in accumulator_events.iter() {
            if let AccumulatorValue::EventDigest(event_digests) = &acc_event.write.value {
                for (event_idx, digest) in event_digests {
                    let event_commitment = EventCommitment::new(checkpoint_seq, idx as u64, *event_idx, *digest);
...
async fn authenticated_events_single_event_test() {
    ...
    assert!(effects.status().is_ok(), "Transaction effects should be successful");

    let acc_events = effects.accumulator_events();
    assert_eq!(acc_events.len(), 1, "Expected 1 accumulator event");

    let event = &acc_events[0];
    assert_eq!(event.address, SuiAddress::from(AccountAddress::from(package_id)));
...
async fn authenticated_events_multiple_events_test() {
    ...
    for i in 0 .. 10 {
        ...
        assert_eq!(acc_events[0].address, SuiAddress::from(AccountAddress::from(package_id)));
...
async fn authenticated_events_disabled_test() {
    ...
    assert!(effects.status().is_err(), "Transaction should have failed when authenticated events are disabled");

    let acc_events = effects.accumulator_events();
    assert_eq!(acc_events.len(), 0, "No accumulator events should be generated when feature is disabled");

    let error_str = format!("{:?}", effects.status());
    assert!(error_str.contains("0"), "Error should contain abort code 0 (NOT_SUPPORTED): {}", error_str);
}
...
async fn authenticated_events_mmr_validation_test() {
    ...
    assert!(checkpoint_x > start_checkpoint, "Checkpoint should have advanced");
    ...
    for cp_seq in (start_checkpoint + 1) ..= checkpoint_x {
        let cp_events =
            build_event_commitments_from_checkpoint(&state, cp_seq).expect("Should be able to build event commitments");
    ...
    assert_eq!(calculated_stream_head_x.mmr, sui_stream_head_x.mmr, "MMR should match at checkpoint X");
    ...
    assert!(checkpoint_x_prime > checkpoint_x, "Checkpoint should have advanced further");
    ...
    let actual_stream_head_x_prime = load_event_stream_head_by_object_id(&state, accumulator_obj_id)
        .await
        .expect("Should be able to load event stream head at checkpoint X'");
