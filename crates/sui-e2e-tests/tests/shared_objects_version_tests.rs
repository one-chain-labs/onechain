...
async fn objects_transitioning_to_shared_remember_their_previous_version() {
    ...
    let ExecutionFailureStatus::MoveAbort(location, code) = env.share_counter(counter).await.unwrap_err() else {
        panic!()
    };
...
async fn shared_object_owner_doesnt_change_on_write() {
    ...
    let ExecutionFailureStatus::MoveAbort(location, code) = env.share_counter(inc_counter).await.unwrap_err() else {
        panic!()
    };
    assert_eq!(location.module.address(), &SUI_FRAMEWORK_ADDRESS);
    assert_eq!(location.module.name().as_str(), "transfer");
...
async fn shared_object_not_found() {
    ...
    assert!(env.increment_shared_counter(nonexistent_id, initial_shared_seq).await.is_err());
}
...
impl TestEnvironment {
    ...
    async fn create_shared_counter(&self) -> (ObjectRef, Owner) {
        let (fx, _) = self.move_call("create_shared_counter", vec![]).await.unwrap();
        assert!(fx.status().is_ok());

        fx.created().iter().find(|(_, owner)| owner.is_shared()).cloned().expect("Shared object created")
    }

    async fn share_counter(&self, counter: ObjectRef) -> Result<(ObjectRef, Owner), ExecutionFailureStatus> {
        let (fx, _) =
            self.move_call("share_counter", vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(counter))]).await.unwrap();

        if let ExecutionStatus::Failure { error, .. } = fx.status() {
            return Err(error.clone());
        }

        Ok(fx.mutated().iter().find(|(obj, _)| obj.0 == counter.0).cloned().expect("Counter mutated"))
    }

    async fn increment_owned_counter(&self, counter: ObjectRef) -> (ObjectRef, Owner) {
        let (fx, _) = self
            .move_call("increment_counter", vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(counter))])
            .await
            .unwrap();

        fx.mutated().iter().find(|(obj, _)| obj.0 == counter.0).cloned().expect("Counter modified")
    }
    ...
    async fn increment_shared_counter(
        ...
        Ok(fx.mutated().iter().find(|(obj, _)| obj.0 == counter).cloned().expect("Counter modified"))
    }
}
