...
use sui_types::{
    base_types::{FullObjectRef, SuiAddress},
...
async fn party_object_grpc() {
    ...
    let resp = ledger_service_client
        .get_object({
            ...
            message.read_mask = Some(FieldMask::from_paths(["object_id", "version", "digest", "owner", "object_type"]));
            message
        })
        .await
        .unwrap()
...
async fn party_coin_grpc() {
    ...
    builder.transfer_object(recipient, FullObjectRef::from_fastpath_ref(owned_coin)).unwrap();
    ...
    let resolved = live_data_service_client
        .simulate_transaction(SimulateTransactionRequest::new(unresolved_transaction).with_do_gas_selection(true))
        .await
        .unwrap()
        .into_inner();

    // Assert that the simulation was successful
    assert!(resolved.transaction.unwrap().effects.unwrap().status.unwrap().success.unwrap());
}
