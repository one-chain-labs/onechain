...
async fn party_object_deletion() {
    ...
    let (package, object) = publish_basics_package_and_make_party_object(&test_cluster.wallet).await;
    ...
    let transaction = test_cluster
        ...
        .await
        .call_object_delete(package_id, ObjectArg::SharedObject {
            id: object_id,
...
async fn party_object_transfer() {
    ...
    assert_eq!(mutated_party.owner, Owner::ConsensusAddressOwner {
...
async fn party_object_transfer_multiple_times() {
    ...
    let mut test_cluster = TestClusterBuilder::new()
        .with_accounts(vec![AccountConfig { address: None, gas_amounts: vec![DEFAULT_GAS_AMOUNT; num_transfers] }])
        .build()
        .await;
...
async fn party_object_transfer_multi_certs() {
    ...
    let _ = test_cluster.create_certificate(xfer_tx.clone(), Some(client_ip)).await.unwrap();
    let _ = test_cluster.create_certificate(repeat_tx_a.clone(), Some(client_ip)).await.unwrap();
    let _ = test_cluster.create_certificate(repeat_tx_b.clone(), Some(client_ip)).await.unwrap();

    let validators = test_cluster.get_validator_pubkeys();

    // transfer obj on all validators, await effects
    test_cluster.submit_transaction_to_validators(xfer_tx, &validators).await.unwrap();

    // now submit remaining txns simultaneously
    futures::join!(
        async { test_cluster.submit_transaction_to_validators(repeat_tx_a, &validators).await.unwrap() },
        async { test_cluster.submit_transaction_to_validators(repeat_tx_b, &validators).await.unwrap() }
    );
...
async fn party_object_read() {
    ...
    for gas_coin in gas_coins_account1.iter().take(num_reads / 2) {
        let transaction = test_cluster
            ...
            .await
            .move_call(package_id, "object_basics", "get_value", vec![CallArg::Object(ObjectArg::SharedObject {
                id: object_id,
                initial_shared_version: object_initial_shared_version,
                mutable: false,
            })])
    ...
    let client_ip = SocketAddr::new([127, 0, 0, 1].into(), 0);
    test_cluster.create_certificate(signed_transfer.clone(), Some(client_ip)).await.unwrap();

    let validators = test_cluster.get_validator_pubkeys();
    let (transfer_effects, _) =
        test_cluster.submit_transaction_to_validators(signed_transfer.clone(), &validators).await.unwrap();
    ...
    // transactions.
    let fullnode = test_cluster.spawn_new_fullnode().await.sui_node;
    let effects = fullnode.state().get_transaction_cache_reader().notify_read_executed_effects(&all_digests).await;
...
async fn party_object_grpc() {
    use sui_rpc_api::{
        field_mask::{FieldMask, FieldMaskUtil},
        proto::rpc::{
            v2alpha::{live_data_service_client::LiveDataServiceClient, ListOwnedObjectsRequest},
            v2beta::{ledger_service_client::LedgerServiceClient, owner::OwnerKind, GetObjectRequest},
        },
    };
    ...
    let channel =
        tonic::transport::Channel::from_shared(test_cluster.rpc_url().to_owned()).unwrap().connect().await.unwrap();
    ...
    let resp = ledger_service_client
        .get_object(GetObjectRequest {
            ...
            read_mask: Some(FieldMask::from_paths(["object_id", "version", "digest", "owner", "object_type"])),
    ...
    let objects = live_data_service_client
        .list_owned_objects(ListOwnedObjectsRequest { owner: original_owner.address.clone(), ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects;
    ...
    let transaction = test_cluster
        ...
        .build();
    test_cluster.sign_and_execute_transaction(&transaction).await.effects.unwrap();
    ...
    let objects = live_data_service_client
        .list_owned_objects(ListOwnedObjectsRequest { owner: original_owner.address, ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects;
    ...
    let objects = live_data_service_client
        .list_owned_objects(ListOwnedObjectsRequest { owner: new_owner.address, ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects;
...
async fn party_coin_grpc() {
    ...
    let channel = tonic::transport::Channel::from_shared(cluster.rpc_url().to_owned()).unwrap().connect().await.unwrap();
    ...
    let recipient_arg = builder.input(CallArg::Pure(bcs::to_bytes(&recipient).unwrap())).unwrap();
    ...
    let party_coin_arg = builder.input(CallArg::Object(ObjectArg::ImmOrOwnedObject(party_coin))).unwrap();
    builder.programmable_move_call(
        "0x2".parse().unwrap(),
        Identifier::new("transfer").unwrap(),
        Identifier::new("public_party_transfer").unwrap(),
        vec!["0x2::coin::Coin<0x2::oct::OCT>".parse().unwrap()],
    ...
    let gas_data =
        sui_types::transaction::GasData { payment: vec![gas_coin], owner: sender, price: 1000, budget: 100_000_000 };
    ...
    cluster.sign_and_execute_transaction(&tx_data).await.effects.unwrap();
    ...
    let objects = live_data_service_client
        .list_owned_objects(ListOwnedObjectsRequest { owner: Some(recipient.to_string()), ..Default::default() })
        .await
        .unwrap()
        .into_inner()
        .objects;
    ...
    assert!(objects.iter().any(|o| o.object_id() == party_coin.0.to_canonical_string(true)
    ...
    assert!(objects.iter().any(|o| o.object_id() == owned_coin.0.to_canonical_string(true)
        && o.owner.as_ref().is_some_and(|owner| {
            owner.kind() == OwnerKind::Address && owner.address() == recipient.to_string() && owner.version.is_none()
        })));
    ...
    let unresolved_transaction = Transaction {
        kind: Some(TransactionKind::from(ProgrammableTransaction {
            inputs: vec![Input { object_id: Some("0x6".to_owned()), ..Default::default() }],
    ...
    assert!(resolved.simulation.unwrap().transaction.unwrap().effects.unwrap().status.unwrap().success.unwrap());
}
...
async fn party_object_jsonrpc() {
    ...
    let object = client
        .read_api()
        .get_object_with_options(object_id, sui_json_rpc_types::SuiObjectDataOptions::new().with_owner())
        .await
        .unwrap()
        .data
        .unwrap();
    let original_owner = object.owner.unwrap();
    assert!(matches!(original_owner, Owner::ConsensusAddressOwner { .. }));
    let original_owner_address = original_owner.get_owner_address().unwrap();

    let objects = client.read_api().get_owned_objects(original_owner_address, None, None, None).await.unwrap().data;

    assert!(objects.into_iter().any(|o| o.data.unwrap().object_id == object_id));
    ...
    let objects = client.read_api().get_owned_objects(new_owner_address, None, None, None).await.unwrap().data;
