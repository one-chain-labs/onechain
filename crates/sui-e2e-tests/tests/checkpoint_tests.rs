...
async fn test_checkpoint_split_brain() {
    ...
    register_debug_fatal_handler!("Split brain detected in checkpoint signature aggregation", move || {
    ...
    register_fail_point_arg("simulate_fork_during_execution", || {
        Some((
            std::sync::Arc::new(std::sync::Mutex::new(std::collections::HashSet::<AuthorityName>::new())),
            true, // full_halt
            std::sync::Arc::new(std::sync::Mutex::new(std::collections::BTreeMap::<String, String>::new())),
...
async fn test_checkpoint_timestamps_non_decreasing() {
    ...
    let test_cluster =
        TestClusterBuilder::new().with_epoch_duration_ms(epoch_duration_ms).disable_fullnode_pruning().build().await;

    sleep(Duration::from_millis(epoch_duration_ms * num_epochs_to_run + epoch_duration_ms / 2)).await;

    // Retrieve checkpoints and verify timestamps from the first full node.
    let full_node = test_cluster.swarm.fullnodes().next().expect("No full node is found");

    let checkpoint_store = full_node.get_node_handle().unwrap().state().checkpoint_store.clone();
    ...
    assert!(highest_executed_checkpoint.epoch() > 0, "Test did not run long enough to cross epochs");
...
async fn test_checkpoint_fork_detection_storage() {
    ...
    let validator_handle = test_cluster.swarm.validator_node_handles().into_iter().next().expect("No validator found");
