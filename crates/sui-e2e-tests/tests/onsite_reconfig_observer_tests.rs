...
async fn test_onsite_reconfig_observer_basic() {
    ...
    let qd = fullnode.with(|node| node.transaction_orchestrator().unwrap().clone_quorum_driver());
    ...
    assert_eq!(fullnode.with(|node| node.clone_authority_aggregator().unwrap().committee.epoch), 1);
