...
mod sim_only_tests {
    ...
    use sui_types::{
        base_types::{ConciseableName, FullObjectID, FullObjectRef, ObjectID, ObjectRef, SequenceNumber, SuiAddress},
    ...
    async fn transfer_obj(cluster: &TestCluster, recipient: SuiAddress, obj: FullObjectRef) -> ObjectRef {
        execute(cluster, {
            let mut builder = ProgrammableTransactionBuilder::new();
    ...
    async fn execute_creating(cluster: &TestCluster, ptb: ProgrammableTransaction) -> Vec<FullObjectRef> {
        execute(cluster, ptb)
            .await
            .created()
            .iter()
            .map(|oref| FullObjectRef::from_object_ref_and_owner(oref.reference.to_object_ref(), &oref.owner))
            .collect()
    }
    ...
    async fn has_public_transfer(cluster: &TestCluster, object_id: &FullObjectID) -> bool {
        get_object(&cluster, &object_id.id()).await.data.try_as_move().unwrap().has_public_transfer()
    }
