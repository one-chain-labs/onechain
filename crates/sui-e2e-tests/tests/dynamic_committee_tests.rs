...
trait StatePredicate {
    ...
    async fn pre_epoch_post_condition(&mut self, runner: &mut StressTestRunner, effects: &TransactionEffects);
    #[allow(unused)]
...
impl StressTestRunner {
    ...
    pub async fn run(&self, sender: SuiAddress, pt: ProgrammableTransaction) -> TransactionEffects {
        ...
        let transaction = self.test_cluster.wallet.sign_transaction(&transaction_data).await;
...
mod add_stake {
    ...
    impl StatePredicate for RequestAddStake {
        ...
        async fn pre_epoch_post_condition(&mut self, runner: &mut StressTestRunner, effects: &TransactionEffects) {
            ...
            let staked_oct: StakedOct = bcs::from_bytes(object.data.try_as_move().unwrap().contents()).unwrap();
            ...
            runner.delegations.insert(object.id(), (self.sender, object.id(), object.digest(), object.version()));
...
mod remove_stake {
    ...
    impl GenStateChange for RequestWithdrawStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let (owner, object_id, digest, version) = runner.delegations.remove(&delegation_object_id).unwrap();

            RequestWithdrawStake { object_id, digest, owner, version }
        }
    }
    ...
    impl StatePredicate for RequestWithdrawStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::SUI_SYSTEM_MUT).unwrap();
                let staked_oct =
                    builder.obj(ObjectArg::ImmOrOwnedObject((self.object_id, self.version, self.digest))).unwrap();
        ...
        async fn pre_epoch_post_condition(&mut self, _runner: &mut StressTestRunner, _effects: &TransactionEffects) {
...
async fn fuzz_dynamic_committee() {
    ...
    let mut initial_committee =
        runner.system_state().active_validators.iter().map(|v| (v.sui_address, v.voting_power)).collect::<Vec<_>>();
    ...
    let total_stake = active_validators.iter().fold(0, |acc, v| acc + v.staking_pool_oct_balance);
    ...
    active_validators.iter().for_each(|v| {
        ...
        let calculated_power = ((v.staking_pool_oct_balance as u128 * 10_000) / total_stake as u128).min(1_000) as u64;
