...
impl SnapshotRestorer {
    ...
    pub async fn restore(&mut self) -> Result<(), Error> {
        info!(epoch = self.restore_args.start_epoch, "Starting snapshot restore");
        let (sha3_digests, num_part_files) = self.snapshot_reader.compute_checksum().await?;
        let (input_files, epoch_dir, remote_object_store, _concurrency) = self.snapshot_reader.export_metadata().await?;
        ...
        self.restore_object_infos(owned_input_files, epoch_dir, remote_object_store, sha3_digests, num_part_files)
            .await?;
        info!(epoch = self.restore_args.start_epoch, "Finished snapshot restore");
        Ok(())
    }
    ...
    async fn restore_object_infos(
        ...
        let move_object_progress_bar = Arc::new(
            self.snapshot_reader.get_multi_progress().add(
        ...
        futures::stream::iter(input_files)
            .try_for_each_spawned(self.restore_args.concurrency, |(bucket, (part_num, file_metadata))| {
                ...
                async move {
                    debug!(bucket = bucket, part_num = part_num, "Start downloading move object file");
                    ...
                    .await;
                    debug!(bucket = bucket, part_num = part_num, "Finished downloading move object file");
                    let object_infos = LiveObjectIter::new(&file_metadata, bytes.clone())?
                        .filter_map(|object| match object {
                            LiveObject::Normal(obj) => Some(StoredObjInfo::from_object(&obj, next_cp as i64)),
