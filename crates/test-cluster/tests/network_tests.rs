...
async fn test_additional_objects() {
    ...
    let cluster = TestClusterBuilder::new().with_objects([Object::immutable_with_id_for_testing(id)]).build().await;
...
async fn test_package_override() {
    ...
    let framework_ref = {
        ...
        let obj = client.get_object(SUI_SYSTEM_PACKAGE_ID, None).await.unwrap();
    ...
    let modified_ref = {
        let mut framework_modules = BuiltInFramework::get_package_by_id(&SUI_SYSTEM_PACKAGE_ID).modules().to_vec();
        ...
        let package_override =
            Object::new_package_for_testing(&framework_modules, TransactionDigest::genesis_marker(), [
        ...
        let modified_cluster = TestClusterBuilder::new().with_objects([package_override]).build().await;
