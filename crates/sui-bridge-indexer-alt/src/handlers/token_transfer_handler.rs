...
use sui_bridge_schema::{
    models::{BridgeDataSource, TokenTransfer, TokenTransferStatus},
...
use sui_indexer_alt_framework::{
    ...
    postgres::Db,
    store::Store,
    types::{effects::TransactionEffectsAPI, full_checkpoint_content::CheckpointData, BRIDGE_ADDRESS},
};
use tracing::info;

use crate::{
    handlers::{is_bridge_txn, BRIDGE, TOKEN_DEPOSITED_EVENT, TOKEN_TRANSFER_APPROVED, TOKEN_TRANSFER_CLAIMED},
        ...
        for tx in &checkpoint.transactions {
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                let (chain_id, nonce) = if self.deposited_event_type == ev.type_ {
                    ...
                    self.metrics.bridge_events_total.with_label_values(&["token_deposited", "sui"]).inc();
                    self.metrics
                    ...
                    self.metrics.bridge_events_total.with_label_values(&["transfer_approved", "sui"]).inc();
                    self.metrics.token_transfers_total.with_label_values(&["eth_to_sui", "approved", "unknown"]).inc();

                    (event.message_key.source_chain, event.message_key.bridge_seq_num)
                    ...
                    self.metrics.bridge_events_total.with_label_values(&["transfer_claimed", "sui"]).inc();
                    self.metrics.token_transfers_total.with_label_values(&["eth_to_sui", "claimed", "unknown"]).inc();
...
impl Handler for TokenTransferHandler {
    ...
    async fn commit<'a>(
        ...
        Ok(diesel::insert_into(token_transfer::table).values(values).on_conflict_do_nothing().execute(conn).await?)
    }
}
