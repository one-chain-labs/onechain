...
use sui_indexer_alt_framework::types::{full_checkpoint_content::CheckpointTransaction, SUI_BRIDGE_OBJECT_ID};
...
macro_rules! struct_tag {
    ($address:ident, $module:ident, $name:ident) => {{
        StructTag { address: $address, module: $module.into(), name: $name.into(), type_params: vec![] }
    }};
}

pub fn is_bridge_txn(txn: &CheckpointTransaction) -> bool {
    txn.input_objects.iter().any(|obj| obj.id() == SUI_BRIDGE_OBJECT_ID)
}
