...
use sui_bridge_schema::{models::TokenTransferData, schema::token_transfer_data};
use sui_indexer_alt_framework::{
    pipeline::Processor,
    postgres::{handler::Handler, Connection},
    types::{full_checkpoint_content::Checkpoint, BRIDGE_ADDRESS},
};
use tracing::info;

use crate::{
    handlers::{is_bridge_txn, BRIDGE, TOKEN_DEPOSITED_EVENT},
    struct_tag,
};
...
impl Default for TokenTransferDataHandler {
    fn default() -> Self {
        Self { deposited_event_type: struct_tag!(BRIDGE_ADDRESS, BRIDGE, TOKEN_DEPOSITED_EVENT) }
    }
}
...
impl Handler for TokenTransferDataHandler {
    async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> sui_indexer_alt_framework::Result<usize> {
        Ok(diesel::insert_into(token_transfer_data::table).values(values).on_conflict_do_nothing().execute(conn).await?)
    }
}
