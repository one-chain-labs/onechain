...
use sui_indexer_alt_framework::{
    ...
    types::{full_checkpoint_content::Checkpoint, transaction::TransactionDataAPI, BRIDGE_ADDRESS},
};
use tracing::info;
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                ...
                let (action, data) = match &ev.type_ {
                    t if t == &self.update_limit_event_type => {
                        ...
                        self.metrics.governance_actions_total.with_label_values(&["update_bridge_limit", "sui"]).inc();
                        self.metrics.bridge_events_total.with_label_values(&["update_route_limit", "sui"]).inc();
                    ...
                    t if t == &self.emergency_op_event_type => {
                        ...
                        self.metrics
                            ...
                            .inc();
                        self.metrics.governance_actions_total.with_label_values(&["emergency_operation", "sui"]).inc();
                        self.metrics.bridge_pause_status.with_label_values(&["bridge_main"]).set(if event.frozen {
                            0
                        } else {
                            1
                        });
