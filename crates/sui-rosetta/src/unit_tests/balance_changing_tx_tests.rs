...
async fn test_transfer_object() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_object(recipient, FullObjectRef::from_fastpath_ref(object_ref)).unwrap();
        builder.finish()
    };
    test_transaction(
        &client,
...
async fn test_publish_and_move_call() {
    ...
    let package = object_changes
        .iter()
        .find_map(|change| if let ObjectChange::Published { package_id, .. } = change { Some(package_id) } else { None })
        .unwrap();
    ...
    test_transaction(&client, keystore, vec![], sender, pt, vec![], rgp * TEST_ONLY_GAS_UNIT_FOR_GENERIC, rgp, false)
        .await;
}
...
async fn test_split_coin() {
    ...
    let tx = client.transaction_builder().split_coin(sender, coin.0, vec![100000], None, 10000).await.unwrap();
    let pt = match tx.into_kind() {
        TransactionKind::ProgrammableTransaction(pt) => pt,
        _ => unreachable!(),
    };
    test_transaction(&client, keystore, vec![], sender, pt, vec![], rgp * TEST_ONLY_GAS_UNIT_FOR_SPLIT_COIN, rgp, false)
        .await;
}
...
async fn test_merge_coin() {
    ...
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let coin = get_random_sui(&client, sender, vec![]).await.object_ref();
    let coin2 = get_random_sui(&client, sender, vec![coin.0]).await.object_ref();
    let tx = client.transaction_builder().merge_coins(sender, coin.0, coin2.0, None, 10000).await.unwrap();
    let pt = match tx.into_kind() {
        TransactionKind::ProgrammableTransaction(pt) => pt,
        _ => unreachable!(),
    };
...
async fn test_pay() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay(vec![coin], vec![recipient], vec![100000]).unwrap();
        builder.finish()
    };
    test_transaction(
        &client,
...
async fn test_pay_multiple_coin_multiple_recipient() {
    ...
    let coin2 = get_random_sui(&client, sender, vec![coin1.0]).await.object_ref();
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay(vec![coin1, coin2], vec![recipient1, recipient2], vec![100000, 200000]).unwrap();
        builder.finish()
    };
    test_transaction(
        &client,
...
async fn test_stake_sui() {
    ...
    let coin2 = get_random_sui(&client, sender, vec![coin1.0]).await.object_ref();
    let validator =
        client.governance_api().get_latest_sui_system_state().await.unwrap().active_validators[0].sui_address;
    let tx = client
        .transaction_builder()
        .request_add_stake(sender, vec![coin1.0, coin2.0], Some(1000000000), validator, None, 10_000_000)
        .await
        .unwrap();
    let pt = match tx.into_kind() {
        TransactionKind::ProgrammableTransaction(pt) => pt,
    ...
    test_transaction(&client, keystore, vec![], sender, pt, vec![], rgp * TEST_ONLY_GAS_UNIT_FOR_STAKING, rgp, false)
        .await;
}
...
async fn test_stake_sui_with_none_amount() {
    ...
    let tx = client
        .transaction_builder()
        .request_add_stake(sender, vec![coin1.0, coin2.0], None, validator, None, rgp * TEST_ONLY_GAS_UNIT_FOR_STAKING)
        .await
        .unwrap();
    let pt = match tx.into_kind() {
        TransactionKind::ProgrammableTransaction(pt) => pt,
...
fn find_module_object(changes: &[ObjectChange], type_pred: impl Fn(&StructTag) -> bool) -> OwnedObjectRef {
    let mut results: Vec<_> = changes
        .iter()
        .filter_map(|change| {
            if let ObjectChange::Created { object_id, object_type, owner, version, digest, .. } = change {
                if type_pred(object_type) {
                    return Some(OwnedObjectRef {
                        owner: owner.clone(),
                        reference: SuiObjectRef { object_id: *object_id, version: *version, digest: *digest },
                    });
                }
            };
            None
...
async fn test_transaction(
    ...
    let gas = if !gas.is_empty() {
        ...
        let input_objects = tx
            ...
            .collect::<Vec<_>>();
        vec![get_random_sui(client, sender, input_objects).await.object_ref()]
    };
    ...
    if !expect_fail {
        assert_eq!(SuiExecutionStatus::Success, *effects.status(), "TX execution failed for {:#?}", data);
    } else {
    ...
    let ops = Operations::try_from_response(response.clone(), &coin_cache).await.unwrap();
    ...
    assert_eq!(actual_balance_change, balances_from_ops, "balance check failed for tx: {}\neffect:{:#?}", tx, effects);
    response
}

fn extract_balance_changes_from_ops(ops: Operations) -> HashMap<SuiAddress, i128> {
    ops.into_iter().fold(HashMap::<SuiAddress, i128>::new(), |mut changes, op| {
...
fn get_random_address(addresses: &[SuiAddress], except: Vec<SuiAddress>) -> SuiAddress {
    *addresses.iter().filter(|addr| !except.contains(*addr)).choose(&mut OsRng).unwrap()
}
    ...
    for coin in coins {
        ...
        if obj.is_gas_coin() {
            let object = client
                ...
                .await
                .unwrap();
            let coin: GasCoin = object.into_object().unwrap().bcs.unwrap().try_as_move().unwrap().deserialize().unwrap();
            balance += coin.value()
        }
    }
    balance
