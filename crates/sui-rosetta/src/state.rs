...
impl OnlineServerContext {
    pub fn new(
        ...
        Self { client, block_provider, coin_metadata_cache }
    }
...
impl BlockProvider for CheckpointBlockProvider {
    async fn get_block_by_index(&self, index: u64) -> Result<BlockResponse, Error> {
        let request = GetCheckpointRequest::by_sequence_number(index).with_read_mask(FieldMask::from_paths([
            "sequence_number",
            "digest",
        ...
        let checkpoint = response.checkpoint.ok_or_else(|| Error::DataError("Checkpoint not found".to_string()))?;
    ...
    async fn get_block_by_hash(&self, hash: BlockHash) -> Result<BlockResponse, Error> {
        ...
        request.checkpoint_id = Some(get_checkpoint_request::CheckpointId::Digest(hash.to_string()));

        let mut client = self.client.clone();
        let response = client.ledger_client().get_checkpoint(request).await?.into_inner();
        let checkpoint = response.checkpoint.ok_or_else(|| Error::DataError("Checkpoint not found".to_string()))?;
    ...
    async fn current_block(&self) -> Result<BlockResponse, Error> {
        let request = GetCheckpointRequest::latest().with_read_mask(FieldMask::from_paths(["sequence_number"]));
    ...
    async fn current_block_identifier(&self) -> Result<BlockIdentifier, Error> {
        ...
        let response = self.client.clone().ledger_client().get_checkpoint(request).await?.into_inner();

        let checkpoint = response.checkpoint.ok_or_else(|| Error::DataError("Missing checkpoint".to_string()))?;
...
impl CheckpointBlockProvider {
    ...
    async fn create_block_response(&self, checkpoint: Checkpoint) -> Result<BlockResponse, Error> {
        ...
        let previous_hash = if index == 0 { hash } else { CheckpointDigest::from_str(summary.previous_digest())? };
        ...
        let parent_block_identifier = if index == 0 {
            ...
            BlockIdentifier { index: index - 1, hash: previous_hash }
        };
    ...
    async fn create_block_identifier(&self, seq_number: CheckpointSequenceNumber) -> Result<BlockIdentifier, Error> {
        ...
        let mut client = self.client.clone();
        let response = client.ledger_client().get_checkpoint(grpc_request).await?.into_inner();

        let checkpoint = response.checkpoint();
        let index = checkpoint.sequence_number();
        let hash = checkpoint.digest();

        Ok(BlockIdentifier { index, hash: CheckpointDigest::from_str(hash)? })
    }
}
