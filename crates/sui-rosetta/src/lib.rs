...
pub static SUI: Lazy<Currency> = Lazy::new(|| Currency {
    ...
    decimals: 9,
    metadata: CurrencyMetadata { coin_type: SDKTypeTag::from(StructTag::sui()).to_string() },
});
...
impl CoinMetadataCache {
    ...
    pub async fn get_currency(&self, type_tag: &TypeTag) -> Result<Currency, Error> {
        ...
        if !cache.contains(type_tag) {
            ...
            let response = client.state_client().get_coin_info(request).await?.into_inner();

            let (symbol, decimals) =
                response.metadata.and_then(|m| Some((m.symbol?, m.decimals?))).ok_or(MissingMetadata)?;
