...
async fn get_checkpoint(ctx: &mut OnlineServerContext) -> Result<CheckpointSequenceNumber, Error> {
    ...
    Ok(ctx.client.ledger_client().get_checkpoint(request).await?.into_inner().checkpoint().sequence_number())
}
...
async fn get_balances(
    ...
    if let Some(sub_account) = &request.account_identifier.sub_account {
        ...
        let balance_futures = currencies.0.iter().map(|currency| {
            ...
            async move { (currency.clone(), get_account_balances(&mut client, address, &coin_type).await) }
        });
...
async fn get_account_balances(client: &mut Client, address: SuiAddress, coin_type: &str) -> Result<i128, Error> {
    let request = GetBalanceRequest::default().with_owner(address.to_string()).with_coin_type(coin_type.to_string());
...
pub async fn get_current_epoch(client: &mut Client) -> Result<u64, Error> {
    ...
    Ok(client.ledger_client().get_epoch(request).await?.into_inner().epoch().epoch())
}
