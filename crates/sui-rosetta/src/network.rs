...
pub async fn status(
    ...
    let mut client = context.client.clone();
    let checkpoint_request = GetCheckpointRequest::latest().with_read_mask(FieldMask::from_paths(["sequence_number"]));

    let checkpoint_response = client.ledger_client().get_checkpoint(checkpoint_request).await?.into_inner();
