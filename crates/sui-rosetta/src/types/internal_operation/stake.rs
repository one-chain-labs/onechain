...
use crate::{errors::Error, types::internal_operation::MAX_GAS_COINS};
...
impl TryConstructTransaction for Stake {
    async fn try_fetch_needed_objects(
        ...
        let Self { sender, validator, amount } = self;

        if amount.is_none() {
            let all_coins = client.coin_read_api().get_coins_stream(sender, None).collect::<Vec<_>>().await;
            ...
            let budget = match budget {
                Some(budget) => budget,
                None => {
                    let pt = stake_pt(validator, total_oct_balance as u64, true, &extra_gas_coins)?;
        ...
        if let Some(budget) = budget {
            let all_coins = client.coin_read_api().select_coins(sender, None, (amount + budget) as u128, vec![]).await?;
            let total_oct_balance = all_coins.iter().map(|c| c.balance).sum::<u64>() as i128;
        ...
        let stake_pt = |extra_gas_coins: &[ObjectRef]| stake_pt(validator, amount, false, extra_gas_coins);
