...
impl TryConstructTransaction for Stake {
    async fn try_fetch_needed_objects(
        ...
        let pt = stake_pt(validator, simulation_amount, false, &extra_gas_coins, &extra_party_coins)?;
        let (budget, _) = simulate_transaction(client, pt, sender, gas_coins.clone(), gas_price, budget).await?;
