...
use sui_types::{
    ...
    governance::WITHDRAW_STAKE_FUN_NAME,
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    rpc_proto_conversions::ObjectReferenceExt,
    sui_system_state::SUI_SYSTEM_MODULE_NAME,
    transaction::{CallArg, Command, ObjectArg, ProgrammableTransaction},
    ...
    async fn try_fetch_needed_objects(
        ...
        let stake_ids = if withdraw_all {
            ...
            let list_request = ListOwnedObjectsRequest::default()
                ...
                .with_object_type("0x3::staking_pool::StakedOct".to_string())
        ...
        for stake_id in &stake_ids {
            ...
            let request = GetObjectRequest::new(&stake_address).with_read_mask(FieldMask::from_paths([
                "object_id",
                "version",
                "digest",
            ]));
            ...
            if let Some(object) = response.object {
                let object_id =
                    object.object_id.ok_or_else(|| Error::InvalidInput("Object ID missing in response".to_string()))?;
                let version =
                    object.version.ok_or_else(|| Error::InvalidInput("Version missing in response".to_string()))?;
                let digest =
                    object.digest.ok_or_else(|| Error::InvalidInput("Digest missing in response".to_string()))?;

                stake_refs.push((
                    ObjectID::from_str(&object_id)
                        .map_err(|e| Error::InvalidInput(format!("Failed to parse object ID: {}", e)))?,
                    version.into(),
                    digest.parse().map_err(|e| Error::InvalidInput(format!("Failed to parse digest: {}", e)))?,
                ));
            } else {
                return Err(Error::InvalidInput(format!("Stake object {} not found", stake_id)));
            }
        }

        let pt = withdraw_stake_pt(stake_refs.clone(), withdraw_all)?;
        let (budget, gas_coin_objs) = simulate_transaction(client, pt, sender, vec![], gas_price, budget).await?;

        let total_oct_balance = gas_coin_objs.iter().map(|c| c.balance()).sum::<u64>() as i128;
        let gas_coins =
            gas_coin_objs.iter().map(|obj| obj.object_reference().try_to_object_ref()).collect::<Result<Vec<_>, _>>()?;

        Ok(TransactionObjectData { gas_coins, objects: stake_refs, party_objects: vec![], total_oct_balance, budget })
    }
}

pub fn withdraw_stake_pt(stake_objs: Vec<ObjectRef>, withdraw_all: bool) -> anyhow::Result<ProgrammableTransaction> {
