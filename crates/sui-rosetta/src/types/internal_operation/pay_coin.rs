...
use super::{simulate_transaction, TransactionObjectData, TryConstructTransaction, MAX_COMMAND_ARGS};
...
impl TryConstructTransaction for PayCoin {
    async fn try_fetch_needed_objects(
        ...
        let Self { sender, recipients, amounts, currency } = self;
        ...
        let all_coins = client.select_coins(&Address::from(sender), &coin_type, amount, &[]).await?;
        ...
        Ok(TransactionObjectData { gas_coins, objects: coins, party_objects: party_coins, total_oct_balance, budget })
    }
}
...
pub fn pay_coin_pt(
    ...
    let all_chunks: Vec<Vec<ObjectArg>> = coins
        ...
        .map(|chunk| chunk.iter().map(|&o| ObjectArg::ImmOrOwnedObject(o)).collect::<Vec<_>>())
    ...
    let mut merged: Vec<Argument> = all_chunks
        ...
        .map(|chunk| -> anyhow::Result<Argument> {
            let mut to_merge: Vec<Argument> =
                chunk.into_iter().map(|o| builder.obj(o)).collect::<Result<Vec<Argument>, anyhow::Error>>()?;
            let merge_into = to_merge.pop().expect("chunks() guarantees non-empty chunks");
    ...
    let single_coin = merged.pop().expect("At least one of coins or party_coins is non-empty");
    if !merged.is_empty() {
    ...
    let amount_args = amounts.into_iter().map(|v| builder.pure(v)).collect::<Result<Vec<_>, anyhow::Error>>()?;
    let split_command = commands;
    builder.command(Command::SplitCoins(single_coin, amount_args));

    recipients.into_iter().enumerate().for_each(|(i, recipient)| {
