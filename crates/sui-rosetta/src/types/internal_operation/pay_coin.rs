...
use super::{budget_from_dry_run, TransactionObjectData, TryConstructTransaction, MAX_COMMAND_ARGS};
use crate::{errors::Error, types::internal_operation::MAX_GAS_COINS, Currency};
...
impl TryConstructTransaction for PayCoin {
    async fn try_fetch_needed_objects(
        ...
        let coin_objs: Vec<ObjectRef> = client
            .coin_read_api()
            .select_coins(sender, Some(currency.metadata.coin_type.clone()), amount.into(), vec![])
            .await?
            .iter()
        ...
        let gas_coins = client.coin_read_api().select_coins(sender, None, budget as u128, vec![]).await?;
        ...
        let total_oct_balance = gas_coins_iter.clone().map(|c| c.balance).sum::<u64>() as i128;
        let gas_coins = gas_coins_iter.map(|c| c.object_ref()).collect();

        Ok(TransactionObjectData { gas_coins, extra_gas_coins: vec![], objects: coin_objs, total_oct_balance, budget })
    }
}
...
pub fn pay_coin_pt(
    ...
    let single_coin = merged.pop().expect("Already checked for non-zero coins above");
    if !merged.is_empty() {
