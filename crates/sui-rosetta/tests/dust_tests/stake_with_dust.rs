...
async fn test_stake_with_many_small_coins() -> Result<()> {
    ...
    let mut all_coins = client.coin_read_api().get_all_coins(sender, None, None).await?.data;
...
async fn test_stake_with_multiple_merges() -> Result<()> {
    ...
    let amount_to_stake: i128 = (coin_to_split.balance as i128 * 255 + 511 + 100) / (255 + 511 + 101); // Almost all coin to go for staking
