...
use crate::{
    ...
    test_utils::{execute_transaction, get_all_coins, get_object_ref, wait_for_transaction},
};
...
async fn test_stake_with_many_small_coins() -> Result<()> {
    ...
    assert!(num_coins > 255, "Test requires more than 255 coins to test multiple merge commands");
    ...
    for coin in all_coins_iter {
        ...
        assert!(effects.status().success(), "Something went wrong sending coins");
    }
    ...
    let coin_to_split = coin_to_split_obj.bcs().deserialize::<sui_types::object::Object>()?;
    let gas_for_split_tx = gas_for_split_tx_obj.bcs().deserialize::<sui_types::object::Object>()?;
    ...
    let response = client.clone().ledger_client().get_epoch(request).await.unwrap().into_inner();
...
async fn test_stake_with_multiple_merges() -> Result<()> {
    ...
    assert!(num_coins > 255 + 511, "Test requires more than 255 + 511 coins to test multiple merge operations");
...
async fn test_stake_with_coin_limit() -> Result<()> {
    ...
    let request = GetEpochRequest::latest().with_read_mask(FieldMask::from_paths(["system_state"]));
    let response = client.clone().ledger_client().get_epoch(request).await?.into_inner();
    let system_state = response.epoch.and_then(|epoch| epoch.system_state).unwrap();
    let validator = system_state.validators.unwrap().active_validators[0]
        .address()
        .parse::<sui_types::base_types::SuiAddress>()
