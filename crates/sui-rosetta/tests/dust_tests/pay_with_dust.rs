...
use sui_rosetta::{operations::Operations, types::PreprocessMetadata, CoinMetadataCache};
use sui_rpc::{
...
use super::{
    rosetta_client::{start_rosetta_test_server, RosettaError},
    split_coin::{make_change, DEFAULT_GAS_BUDGET},
};
use crate::test_utils::{execute_transaction, get_all_coins, get_coin_value, get_object_ref, wait_for_transaction};
...
async fn test_pay_with_many_small_coins() -> Result<()> {
    ...
    let test_cluster = TestClusterBuilder::new().with_epoch_duration_ms(36000000).with_accounts(accounts).build().await;
    ...
    for coin in all_coins_sender.iter().skip(2) {
        ...
        let sig = keystore.sign_secure(&sender, &tx_data, Intent::sui_transaction()).await?;
        ...
        gas_for_transfers = get_object_ref(&mut client.clone(), all_coins_sender[0].id()).await?.as_object_ref();
    }
    ...
    let batch_response = client.ledger_client().batch_get_objects(batch_request).await?.into_inner();
    ...
    let coin_to_split = first_coin_obj.bcs().deserialize::<sui_types::object::Object>()?;
    let gas_for_split_tx = second_coin_obj.bcs().deserialize::<sui_types::object::Object>()?;
    let new_coins = 300;
    ...
    let gas_object = test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await?.unwrap();

    let mut ptb = ProgrammableTransactionBuilder::new();
    ptb.transfer_object(recipient, get_object_ref(&mut client.clone(), gas_for_split_tx.id()).await?)?;
    let tx_data =
        TransactionData::new_programmable(sender, vec![gas_object], ptb.finish(), DEFAULT_GAS_BUDGET, gas_price);
    let sig = keystore.sign_secure(&sender, &tx_data, Intent::sui_transaction()).await?;
    let signed_transaction = Transaction::from_data(tx_data, vec![sig]);
    let _resp = execute_transaction(&mut client.clone(), &signed_transaction).await?;
    ...
    grpc_request.read_mask =
        Some(FieldMask::from_paths(["digest", "transaction", "effects", "balance_changes", "events"]));
    ...
    let grpc_response = client_mut.ledger_client().get_transaction(grpc_request).await.unwrap().into_inner();

    let coin_cache = CoinMetadataCache::new(client_copy.clone(), NonZeroUsize::new(2).unwrap());
    let executed_tx = grpc_response.transaction.expect("Response transaction should not be empty");
    let ops2 = Operations::try_from_executed_transaction(executed_tx, &coin_cache).await.unwrap();
    assert!(
        ops2.contains(&ops),
...
async fn test_pay_with_many_small_coins_with_budget() -> Result<()> {
    ...
    let response = rosetta_client
        .rosetta_flow(&ops, keystore, Some(PreprocessMetadata { budget: Some(budget) }))
        .await
        .submit
        .unwrap()
        .unwrap();
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_budget_none() -> Result<()> {
    ...
    assert!(all_coins_sender.len() == 2, "Should have exactly 2 coins by now.");

    // Keep one small coin
    let coin_to_send = all_coins_sender.pop().unwrap();
    let new_coins = 300;
    let split_amount = 50_000_000;
    assert!(new_coins > 255, "Test requires more than 255 coins to test multiple merge commands");
    let send_amount_to_decrease_balance = get_coin_value(&coin_to_send) - new_coins * split_amount;

    let mut ptb = ProgrammableTransactionBuilder::new();
    ptb.pay_oct(vec![recipient], vec![send_amount_to_decrease_balance])?;
    let tx_data = TransactionData::new_programmable(
        sender,
    ...
    let (coin_to_split, gas_for_split_tx) = match all_coins_sender.as_mut_slice() {
        ...
        [_, _] => (all_coins_sender.pop().unwrap(), all_coins_sender.pop().unwrap()),
    ...
    for resp in resps {
        assert!(resp.effects().status().success(), "Something went wrong splitting coins to change");
    ...
    let mut ptb = ProgrammableTransactionBuilder::new();
    ptb.transfer_object(recipient, get_object_ref(&mut client.clone(), gas_for_split_tx.id()).await?)?;
    let tx_data = TransactionData::new_programmable(sender, vec![gas_object], ptb.finish(), 10_000_000, gas_price);
    let sig = keystore.sign_secure(&sender, &tx_data, Intent::sui_transaction()).await?;
    let signed_transaction = Transaction::from_data(tx_data, vec![sig]);
    let resp = execute_transaction(&mut client.clone(), &signed_transaction).await?;
    ...
    let details =
        Some(json!({ "error": "ExecutionError: Kind: INSUFFICIENT_COIN_BALANCE, Description: No description" }));
    assert_eq!(err, RosettaError {
        code: 11,
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_with_budget() -> Result<()> {
    ...
    let Some(Err(e)) = resps.metadata else { panic!("Expected metadata to exist and error") };
    assert_eq!(e, RosettaError {
        code: 11,
        message: "Transaction dry run error".to_string(),
        description: None,
        retriable: false,
...
async fn test_pay_with_many_small_coins_fail_insufficient_budget() -> Result<()> {
    ...
    let rosetta_resp =
        rosetta_client.rosetta_flow(&ops, keystore, Some(PreprocessMetadata { budget: Some(budget as u64) })).await;
    ...
    assert_eq!(err, RosettaError {
        ...
        description: None,
        retriable: false,
        details: Some(json!({"error": "ExecutionError: Kind: INSUFFICIENT_GAS, Description: No description"}))
    });
    Ok(())
}
