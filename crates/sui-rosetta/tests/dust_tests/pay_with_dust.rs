...
async fn test_pay_with_many_small_coins() -> Result<()> {
    ...
    let all_coins_sender = client.coin_read_api().get_all_coins(sender, None, None).await?;
    assert!(!all_coins_sender.has_next_page, "Multiple pages for sender coins not implemented");

    // Send rest of the coins to recipient first
    for coin in all_coins_sender.data.iter().skip(2) {
        let tx_data = client
            .transaction_builder()
            .transfer_object(sender, coin.coin_object_id, None, DEFAULT_GAS_BUDGET, recipient)
            .await?;
        ...
        let resp = client
            ...
            .await?;
        assert!(resp.status_ok().is_some() && resp.status_ok().unwrap(), "Something went wrong sending coins");
    }

    // First two coins were probably been used for gas already so update coins:
    let all_coins_sender = client.coin_read_api().get_all_coins(sender, None, None).await?;
    assert!(all_coins_sender.data.len() == 2, "Should have exactly 2 coins by now.");
    ...
    let resps =
        make_change(&client, keystore, sender, coin_to_split, Some(gas_for_split_tx.object_ref()), split_amount).await?;
    ...
    let tx_data = client
        .transaction_builder()
        .transfer_object(sender, gas_for_split_tx.coin_object_id, None, DEFAULT_GAS_BUDGET, recipient)
        .await?;
    let sig = keystore.sign_secure(&sender, &tx_data, Intent::sui_transaction()).await?;
    let resp = client
        .quorum_driver_api()
...
async fn test_pay_with_many_small_coins_with_budget() -> Result<()> {
    ...
    assert_eq!(&SuiExecutionStatus::Success, tx.effects.as_ref().unwrap().status());
    assert_eq!(tx.transaction.as_ref().unwrap().data.gas_data().budget, budget);
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_budget_none() -> Result<()> {
    ...
    // First two coins were probably been used for gas already so update coins:
    let mut all_coins_sender = client.coin_read_api().get_all_coins(sender, None, None).await?;
    assert!(all_coins_sender.data.len() == 2, "Should have exactly 2 coins by now.");
    ...
    let mut all_coins_sender = client.coin_read_api().get_all_coins(sender, None, None).await?.data;
    ...
    let tx_data = client
        .transaction_builder()
        .transfer_object(sender, gas_for_split_tx.coin_object_id, None, 2_000_000, recipient)
        .await?;
    let sig = keystore.sign_secure(&sender, &tx_data, Intent::sui_transaction()).await?;
    let resp = client
        .quorum_driver_api()
    ...
    println!("{}", serde_json::to_string_pretty(&resps.preprocess.as_ref().unwrap().as_ref().unwrap())?);
