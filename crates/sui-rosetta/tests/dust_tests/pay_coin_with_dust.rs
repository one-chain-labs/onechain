...
async fn test_pay_custom_coin_with_multiple_coins() -> anyhow::Result<()> {
    ...
    let submit = rosetta_client.rosetta_flow(&ops, keystore, None).await.submit.unwrap().unwrap();

    wait_for_transaction(&mut client, &submit.transaction_identifier.hash.to_string()).await.unwrap();
    ...
    let ops2 = Operations::try_from_executed_transaction(tx, &coin_cache).await.unwrap();
    assert!(
        ops2.contains(&ops),
...
async fn test_pay_custom_coin_with_multiple_merge_chunks() -> anyhow::Result<()> {
    ...
    let mut init_ret =
        init_package(&test_cluster, &mut client, keystore, sender, Path::new("tests/custom_coins/test_coin"))
            .await
            .unwrap();
    let coin_type = init_ret.coin_tag.to_canonical_string(true);
    ...
    let balances_to = vec![(coin_balance, sender); n_coins];
    // Create 10 coins to transfer later at once
    let mint_res0 = mint(&test_cluster, &mut client, keystore, init_ret.clone(), balances_to.clone()).await.unwrap();
    let t_cap_obj = mint_res0
        .effects()
        .changed_objects
        .iter()
        .find(|obj| obj.object_type_opt().map(|t| t.contains("TreasuryCap")).unwrap_or(false))
