...
pub async fn make_change(
    ...
    for amounts in amounts_vec.into_iter() {
        let resp = split_coins(client, keystore, sender, coin_ref, &amounts, gas_ref, Some(ref_gas_price), None).await?;
        progress += 1;
        if progress % 4 == 0 {
        ...
        gas_ref = match gas_ref {
            Some(_) => {
                let SuiObjectRef { object_id, version, digest } =
                    resp.effects.as_ref().ok_or(anyhow!("Expected balance_changes"))?.gas_object().reference;
        ...
        client
            .read_api()
            .get_transaction_with_options(resp.digest, SuiTransactionBlockResponseOptions::default())
            .await?;
        responses.push(resp);
    }
    Ok(responses)
...
async fn test_make_change_exact_div() -> Result<()> {
    ...
    assert!(coin.balance == DEFAULT_INIT_COIN_BALANCE, "Coin did not match INIT_COIN_BALANCE");

    let split_resp = split_coins(&client, keystore, sender, coin.object_ref(), &[SUI_100], None, None, None).await?;

    let tx_digest = split_resp.digest;
    if split_resp.effects.as_ref().ok_or(anyhow!("Expected effects"))?.status().clone() != SuiExecutionStatus::Success {
        return Err(anyhow!("Transaction failed!: {:#?}", split_resp));
    }

    // let digest = (split_resp.digest).clone();
    let (initial, splitted) = match split_resp.object_changes.ok_or(anyhow!("Expected object_changes"))?.as_slice() {
        [coin0, coin1] if coin0.object_id() == coin.coin_object_id => (coin0.object_ref(), coin1.object_ref()),
...
async fn test_make_change_remainder_div() -> Result<()> {
    ...
    let coins_with_data =
        client.read_api().multi_get_object_with_options(all_coins, SuiObjectDataOptions::full_content()).await?;

    let (mut twelve_count, mut four_count) = (0, 0);
    for coin in coins_with_data {
        let SuiParsedData::MoveObject(object) =
            coin.data.ok_or(anyhow!("No data in coin"))?.content.ok_or(anyhow!("No coin.data.content"))?
        else {
            return Err(anyhow!("Coin should be a MoveObject"));
        };
        let SuiMoveValue::String(balance) =
            object.fields.field_value("balance").ok_or(anyhow!("No field coin.balance"))?
        else {
