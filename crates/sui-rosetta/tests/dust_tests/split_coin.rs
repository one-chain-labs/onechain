...
use sui_types::{
    ...
    transaction::{Argument, Command, ObjectArg, Transaction, TransactionData, TransactionDataAPI},
};
use test_cluster::TestClusterBuilder;

use crate::test_utils::{execute_transaction, extract_object_ref_from_changed_objects, get_all_coins, get_coin_value};
...
pub async fn split_coins(
    ...
    let amounts = amounts.iter().map(|amount| ptb.pure(amount)).collect::<Result<Vec<_>>>()?;
    ...
    let results = (0 .. amounts_len).map(|i| Argument::NestedResult(0, i as u16)).collect::<Vec<_>>();
    ptb.command(Command::TransferObjects(results, sender_arg));
    let builder = ptb.finish();

    // Sign transaction
    let tx_data = TransactionData::new_programmable(sender, vec![gas], builder, budget, reference_gas_price);
...
pub async fn make_change(
    ...
    for amounts in amounts_vec.into_iter() {
        let resp = split_coins(keystore, sender, coin_ref, &amounts, gas_ref, Some(ref_gas_price), None, client).await?;
        ...
        let fresh_coin_ref = extract_object_ref_from_changed_objects(changed_objects, coin_object.id())?;
        ...
        gas_ref = match gas_ref {
            Some(old_gas_ref) => {
                ...
                let fresh_gas_ref = extract_object_ref_from_changed_objects(changed_objects, old_gas_ref.0)?;
...
async fn test_make_change_exact_div() -> Result<()> {
    ...
    let coin_object =
        get_all_coins(&mut client.clone(), sender).await?.into_iter().next().ok_or(anyhow!("Expected 1 coin"))?;

    let coin_value = get_coin_value(&coin_object);
    assert!(coin_value == DEFAULT_INIT_COIN_BALANCE, "Coin did not match INIT_COIN_BALANCE");

    let split_resp =
        split_coins(keystore, sender, coin_object.compute_object_reference(), &[SUI_100], None, None, None, &mut client)
            .await?;
    ...
    let new_coin_obj = changed_objects
        ...
        .ok_or(anyhow!("Could not find newly created coin in changed_objects"))?;

    let new_coin_id = ObjectID::from_hex_literal(new_coin_obj.object_id_opt().ok_or(anyhow!("Missing object_id"))?)?;

    // Get the updated reference for the original coin
    let original_coin_ref = extract_object_ref_from_changed_objects(changed_objects, coin_object.id())?;
    ...
    let new_coin_response = client.ledger_client().get_object(new_coin_request).await?.into_inner();
    ...
    if get_coin_value(&new_coin) != SUI_100 {
        return Err(anyhow!("New coin has unexpected value: {} (expected {})", get_coin_value(&new_coin), SUI_100));
    }

    let splitted_object = &new_coin;

    let txs = make_change(&mut client.clone(), keystore, sender, splitted_object, Some(initial), SUI_10).await?;
    ...
    let new_coins_count =
        changed_objects.iter().filter(|obj| obj.id_operation == Some(IdOperation::Created as i32)).count();
...
async fn test_make_change_remainder_div() -> Result<()> {
    ...
    let txs = make_change(&mut client.clone(), keystore, sender, splitted_object, Some(initial), SUI_12).await?;
    ...
    let new_coins: Vec<_> =
        changed_objects.iter().filter(|obj| obj.id_operation == Some(IdOperation::Created as i32)).collect();
    ...
    let mut all_coins: Vec<ObjectID> =
        new_coins.into_iter().map(|c| ObjectID::from_hex_literal(c.object_id()).unwrap()).collect();
    all_coins.push(splitted_id.into());

    let requests: Vec<GetObjectRequest> =
        all_coins.iter().map(|id| GetObjectRequest::default().with_object_id(id.to_string())).collect();

    let batch_request =
        BatchGetObjectsRequest::default().with_requests(requests).with_read_mask(FieldMask::from_paths(["balance"]));

    let batch_response = client.clone().ledger_client().batch_get_objects(batch_request).await?.into_inner();
    ...
    for result in batch_response.objects {
        ...
        let b = object.balance.ok_or(anyhow!("No balance field in object"))?;
        match b {
            SUI_12 => {
                twelve_count += 1;
            }
            ...
            b => {
                return Err(anyhow!("Did not expect anything else other than 12 or 4 SUI. Found {b}"));
            }
        }
    }
