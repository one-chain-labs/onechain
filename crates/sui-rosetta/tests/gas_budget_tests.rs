...
use rosetta_client::{start_rosetta_test_server, FlowResponses, RosettaError};
use serde_json::json;
...
async fn pay_with_gas_budget(budget: u64) -> anyhow::Result<FlowResponses> {
    ...
    let (unsigned_transaction, signing_payload) = match payloads_result {
        Ok(resp) => {
            let signing_payload = resp.payloads.first().ok_or_else(|| anyhow::anyhow!("No payloads"))?.clone();
    ...
    let submit_result: Result<TransactionIdentifierResponse, RosettaError> = rosetta_client
        .call(RosettaEndpoint::Submit, &ConstructionSubmitRequest { network_identifier, signed_transaction })
        .await;
...
async fn test_pay_with_gas_budget() {
    ...
    let flow_responses = pay_with_gas_budget(TX_BUDGET_PASS).await.expect("Should not error during test setup");

    assert!(flow_responses.preprocess.as_ref().unwrap().is_ok(), "Preprocess should succeed");
    assert!(flow_responses.metadata.as_ref().unwrap().is_ok(), "Metadata should succeed");
    assert!(flow_responses.payloads.as_ref().unwrap().is_ok(), "Payloads should succeed");
    assert!(flow_responses.combine.as_ref().unwrap().is_ok(), "Combine should succeed");
    assert!(flow_responses.submit.as_ref().unwrap().is_ok(), "Submit should succeed");
}

#[tokio::test]
async fn test_pay_with_gas_budget_fail() {
    const TX_BUDGET_FAIL: u64 = 1_100_000;
    let flow_responses = pay_with_gas_budget(TX_BUDGET_FAIL).await.expect("Should not error during test setup");

    assert!(flow_responses.preprocess.as_ref().unwrap().is_ok(), "Preprocess should succeed");

    assert!(flow_responses.metadata.is_some(), "Metadata should be attempted");

    match &flow_responses.metadata.as_ref().unwrap() {
        Err(rosetta_error) => {
            assert_eq!(rosetta_error.code, 11, "Expected error code 11 for dry run error");
    ...
    assert!(flow_responses.payloads.is_none(), "Payloads should not be attempted after metadata failure");
    assert!(flow_responses.combine.is_none(), "Combine should not be attempted after metadata failure");
    assert!(flow_responses.submit.is_none(), "Submit should not be attempted after metadata failure");
}
