...
async fn test_mint_with_gas_coin_transfer() -> anyhow::Result<()> {
    ...
    let mut gas_coins = client.coin_read_api().select_coins(sender, None, LARGE_GAS_BUDGET, vec![]).await?;
    assert!(gas_coins.len() == 1, "Expected 1 large gas-coin to satisfy the budget");
    ...
    let coin_cache = CoinMetadataCache::new(client, NonZeroUsize::new(2).unwrap());
    let ops = Operations::try_from_response(mint_res, &coin_cache).await.unwrap();
