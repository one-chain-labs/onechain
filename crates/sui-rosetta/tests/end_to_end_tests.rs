...
use sui_types::{
    ...
    transaction::{Argument, InputObjectKind, Transaction, TransactionData, TEST_ONLY_GAS_UNIT_FOR_TRANSFER},
...
async fn test_get_staked_oct() {
    ...
    let coins = client.coin_read_api().get_coins(address, None, None, None).await.unwrap().data;
    let delegation_tx = client
        .transaction_builder()
        .request_add_stake(address, vec![coins[0].coin_object_id], Some(1_000_000_000), validator, None, 1_000_000_000)
        .await
        .unwrap();
...
async fn test_transfer_single_gas_coin() {
    ...
    let coin_cache = CoinMetadataCache::new(client, NonZeroUsize::new(2).unwrap());
    let operations = Operations::try_from_response(response, &coin_cache).await.unwrap();
