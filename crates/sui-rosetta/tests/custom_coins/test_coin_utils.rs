...
pub async fn select_gas(
    ...
    if let Some(gas) = input_gas {
        ...
        return Ok(GasRet { object, budget, price });
    }
    ...
    for obj in gas_objs {
        ...
        if !exclude_objects.contains(&obj_ref.0) && gas.value() >= budget {
            return Ok(GasRet { object: obj_ref, budget, price });
        }
    }
...
pub async fn init_package(client: &SuiClient, keystore: &Keystore, sender: SuiAddress, path: &Path) -> Result<InitRet> {
    ...
    let res = client
        ...
        .execute_transaction_block(
            ...
            SuiTransactionBlockResponseOptions::new().with_effects().with_object_changes().with_input(),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;

    let treasury_cap = res.object_changes.unwrap().into_iter().find_map(|change| {
        if let ObjectChange::Created { object_type, object_id, version, digest, owner, .. } = change {
            if object_type.to_string().contains("2::coin::TreasuryCap") {
                ...
                return Some(InitRet { owner, treasury_cap: (object_id, version, digest), coin_tag });
            }
        }
        None
    });
