...
pub async fn init_package(
    ...
    let path_buf = path.canonicalize().map_err(|e| anyhow!("Failed to canonicalize path {}: {}", path.display(), e))?;
    ...
    let pt = {
        ...
        builder.publish_immutable(modules_bytes, vec![
    ...
    let (_, gas_object_data) = test_cluster.wallet.gas_for_owner_budget(sender, budget, Default::default()).await?;
    ...
    assert!(effects.status().success(), "Transaction failed: {:?}", effects.status().error());
...
pub async fn mint(
    ...
    let (_gas_balance, gas_object_data) =
        test_cluster.wallet.gas_for_owner_budget(treasury_cap_owner, budget, forbidden_objects).await?;
    let gas_object = gas_object_data.object_ref();
    ...
    let tx_data = TransactionData::new_programmable(treasury_cap_owner, vec![gas_object], builder, budget, price);
