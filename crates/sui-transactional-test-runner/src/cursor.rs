...
/// - Struct tags (e.g., `0x2::coin::Coin<0x2::oct::OCT>>`)
...
fn value(input: &mut &str) -> Result<Value> {
    ...
    preceded(multispace0, alt((struct_, id, number, string, bytes, tuple, list))).parse_next(input)
}
...
fn struct_tag(input: &mut &str) -> Result<StructTag> {
    let (address, _, module, _, name, type_params) =
        seq!(address, "::", identifier, "::", identifier, opt(type_params)).parse_next(input)?;

    Ok(StructTag { address, module, name, type_params: type_params.unwrap_or_default() })
}
...
fn type_params(input: &mut &str) -> Result<Vec<TypeTag>> {
    Ok(seq!(
        "<",
        separated(0 .., preceded(multispace0, type_tag), preceded(multispace0, ",")),
...
fn vector(input: &mut &str) -> Result<TypeTag> {
    let (_, _, inner, _) =
        seq!("vector", preceded(multispace0, "<"), preceded(multispace0, type_tag), preceded(multispace0, ">"))
            .parse_next(input)?;
...
fn string(input: &mut &str) -> Result<Value> {
    let (_, content, _) = seq!("'", repeat(0 .., alt(("\\'".map(|_| '\''), "\\\\".map(|_| '\\'), none_of('\'')))), "'")
        .parse_next(input)?;
...
mod tests {
    ...
    #[test]
    fn test_struct_tag() {
        let tag = StructTag::from_str("0x2::table::Table<address, 0x2::coin::Coin<0x2::oct::OCT>>").unwrap();
        let (bcs, bin) = expect(tag);

        assert_eq!(parse("bcs(0x2::table::Table<address, 0x2::coin::Coin<0x2::oct::OCT>>)").unwrap(), bcs);
        assert_eq!(parse("bin(0x2::table::Table<address, 0x2::coin::Coin<0x2::oct::OCT>>)").unwrap(), bin);
    }
}
