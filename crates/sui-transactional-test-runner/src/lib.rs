...
pub async fn run_ptb_v2_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    options.suffix("v2");
    run_test_impl::<SuiTestAdapter>(path, Some(std::sync::Arc::new(PRE_COMPILED.clone())), Some(options)).await?;
    Ok(())
}
...
impl TransactionalAdapter for ValidatorWithFullnode {
    ...
    async fn dev_inspect_transaction_block(
        ...
        self.fullnode
            .dev_inspect_transaction_block(sender, transaction_kind, gas_price, None, None, None, None, None)
            .await
    }
    ...
    async fn query_tx_events_asc(&self, tx_digest: &TransactionDigest, limit: usize) -> SuiResult<Vec<Event>> {
        Ok(self
            .validator
            .query_events(&self.kv_store, EventFilter::Transaction(*tx_digest), None, limit, false)
            .await
            .unwrap_or_default()
            .into_iter()
    ...
    async fn advance_clock(&mut self, _duration: std::time::Duration) -> anyhow::Result<TransactionEffects> {
    ...
    async fn request_gas(&mut self, _address: SuiAddress, _amount: u64) -> anyhow::Result<TransactionEffects> {
...
impl ReadStore for ValidatorWithFullnode {
    fn get_committee(&self, _epoch: sui_types::committee::EpochId) -> Option<Arc<sui_types::committee::Committee>> {
        todo!()
    }
    ...
    fn get_latest_checkpoint(&self) -> sui_types::storage::error::Result<VerifiedCheckpoint> {
        let sequence_number = self.validator.get_latest_checkpoint_sequence_number().unwrap();
        Ok(self.get_checkpoint_by_sequence_number(sequence_number).unwrap())
    }
    ...
    fn get_checkpoint_by_sequence_number(
        ...
        self.validator.get_checkpoint_store().get_checkpoint_by_sequence_number(sequence_number).expect("db error")
    }
    ...
    fn get_checkpoint_contents_by_digest(
        ...
        self.validator.get_checkpoint_store().get_checkpoint_contents(digest).expect("db error")
    }
    ...
    fn get_transaction(
        ...
        self.validator.get_transaction_cache_reader().get_transaction_block(tx_digest)
    }
...
impl ObjectStore for ValidatorWithFullnode {
    ...
    fn get_object_by_key(&self, object_id: &ObjectID, version: VersionNumber) -> Option<Object> {
        self.validator.get_object_store().get_object_by_key(object_id, version)
    }
}
