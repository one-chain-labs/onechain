...
pub fn command<W: fmt::Write>(
    ...
    match command {
        Command::MoveCall(m) => {
            let ProgrammableMoveCall { package, module, function, type_arguments: tys, arguments: args } = &**m;
            write!(buf, "{}::{}::{}", address_str(named_addresses, **package), module, function)?;
...
pub fn struct_type_<W: fmt::Write>(
    ...
    write!(buf, "{}::{}::{}", address_str(named_addresses, *address), module, name)?;
...
pub fn address_str(named_addresses: &BTreeMap<AccountAddress, String>, address: AccountAddress) -> String {
    match address {
