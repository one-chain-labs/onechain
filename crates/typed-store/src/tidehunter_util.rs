...
use tidehunter::{
    config::Config,
    db::Db,
    iterators::db_iterator::DbIterator,
    key_shape::{KeyShape, KeySpace},
    metrics::Metrics,
};
pub use tidehunter::{
...
pub fn open(path: &Path, key_shape: KeyShape, db_name: String) -> Arc<Db> {
    ...
    let db = Db::open(path, key_shape, Arc::new(thdb_config()), metrics).expect("failed to open tidehunter db");
...
fn thdb_config() -> Config {
    let frag_size = if let Ok(frag_size) = env::var("TH_FRAG_SIZE") {
        ...
        assert!(frag_size % (4 * 1024) == 0, "TH_FRAG_SIZE must be page aligned({frag_size} given)");
...
pub(crate) fn apply_range_bounds(iterator: &mut DbIterator, lower_bound: Option<Vec<u8>>, upper_bound: Option<Vec<u8>>) {
...
pub(crate) fn transform_th_iterator<'a, K, V>(
    iterator: impl Iterator<Item = Result<(tidehunter::minibytes::Bytes, tidehunter::minibytes::Bytes), tidehunter::db::DbError>>
        + 'a,
    ...
    V: DeserializeOwned,
{
    ...
    iterator.map(move |item| {
        item.map_err(|e| TypedStoreError::RocksDBError(format!("tidehunter error {:?}", e))).and_then(
            |(raw_key, raw_value)| {
                let _timer = &timer;
...
impl ThConfig {
    ...
    pub fn new_with_indexing(key_indexing: KeyIndexing, mutexes: usize, key_type: KeyType) -> Self {
        Self { key_indexing, mutexes, key_type, config: KeySpaceConfig::default(), prefix: None }
    }
    ...
    pub fn new_with_config_indexing(
        ...
        Self { key_indexing, mutexes, key_type, config, prefix: None }
    }

    pub fn new_with_config(key_size: usize, mutexes: usize, key_type: KeyType, config: KeySpaceConfig) -> Self {
    ...
    pub fn new_with_rm_prefix(
        ...
        Self::new_with_rm_prefix_indexing(KeyIndexing::fixed(key_size), mutexes, key_type, config, prefix)
    }
    ...
    pub fn new_with_rm_prefix_indexing(
        ...
        Self { key_indexing, mutexes, key_type, config, prefix: Some(prefix) }
    }
