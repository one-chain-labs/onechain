    ...
    V: Serialize + DeserializeOwned,
{
    ...
    fn delete_file_in_range(&self, from: &K, to: &K) -> Result<(), TypedStoreError> {
        let mut locked = self.rows.write().unwrap();
        locked.retain(|k, _| k < &be_fix_int_ser(from).unwrap() || k >= &be_fix_int_ser(to).unwrap());
        Ok(())
    }
...
impl TestDBWriteBatch {
        ...
        dbs.iter().for_each(|(db, name)| {
            if !db_locks.contains_key(name) {
                db_locks.insert(
                    ...
                    DBLockedBuilder {
                        db: db.clone(),
                        db_guard_builder: |db: &Arc<RwLock<BTreeMap<Vec<u8>, Vec<u8>>>>| db.write().unwrap(),
                    }
                    .build(),
                );
            }
    ...
    pub fn delete_range<K: Serialize, V>(&mut self, db: &TestDB<K, V>, from: &K, to: &K) -> Result<(), TypedStoreError> {
        let raw_from = be_fix_int_ser(from).unwrap();
        let raw_to = be_fix_int_ser(to).unwrap();
        self.ops.push_back(WriteBatchOp::DeleteRange((db.rows.clone(), db.name.clone(), (raw_from, raw_to))));
        Ok(())
    }
    ...
    pub fn insert_batch<J: Borrow<K>, K: Serialize, U: Borrow<V>, V: Serialize>(
        ...
        self.ops.push_back(WriteBatchOp::InsertBatch((
            ...
            new_vals
                .into_iter()
                .map(|(key, value)| (be_fix_int_ser(&key.borrow()).unwrap(), bcs::to_bytes(&value.borrow()).unwrap()))
                .collect(),
        )));
        Ok(())
    }
