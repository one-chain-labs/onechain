...
fn get_range_iter<'a, K, V>(db: &'a DBMap<K, V>, range: impl RangeBounds<K> + 'a) -> impl Iterator<Item = (K, V)> + 'a
where
    K: Serialize + DeserializeOwned,
    V: Serialize + DeserializeOwned,
{
...
async fn test_delete_range() {
    ...
    let db: DBMap<i32, String> =
        DBMap::reopen(&open_rocksdb(temp_dir(), &[rocksdb::DEFAULT_COLUMN_FAMILY_NAME]), None, &options, false).unwrap();
...
async fn open_as_secondary_test() {
    ...
    let secondary_store =
        open_cf_opts_secondary(primary_path.clone(), None, None, MetricConf::default(), &[("table", opt.clone())])
            .unwrap();
    let secondary_db =
...
fn open_map<P: AsRef<Path>, K, V>(path: P, opt_cf: Option<&str>) -> DBMap<K, V> {
    ...
    DBMap::<K, V>::reopen(&open_rocksdb(path, &[cf_key]), opt_cf, &ReadWriteOptions::default(), false)
        .expect("failed to open rocksdb")
}

fn open_rocksdb<P: AsRef<Path>>(path: P, opt_cfs: &[&str]) -> Arc<Database> {
    let opts = rocksdb::Options::default();
    open_cf_opts(path, None, MetricConf::default(), &opt_cfs.iter().map(|cf| (*cf, opts.clone())).collect::<Vec<_>>())
        .expect("failed to open rocksdb")
}
