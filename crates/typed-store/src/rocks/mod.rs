...
use crate::tidehunter_util::{
    apply_range_bounds,
    transform_th_iterator,
    transform_th_key,
    typed_store_error_from_th_error,
};
use crate::{
...
impl Database {
    pub fn new(storage: Storage, metric_conf: MetricConf) -> Self {
        ...
        Self { storage, metric_conf }
    }

    /// Flush all memtables to SST files on disk.
    pub fn flush(&self) -> Result<(), TypedStoreError> {
        match &self.storage {
            Storage::Rocks(rocks_db) => rocks_db
                .underlying
                .flush()
                .map_err(|e| TypedStoreError::RocksDBError(format!("Failed to flush database: {}", e))),
            Storage::InMemory(_) => {
        ...
        K: AsRef<[u8]>,
    {
        match (&self.storage, cf) {
            ...
            (Storage::InMemory(db), ColumnFamily::InMemory(cf_name)) => {
                db.multi_get(cf_name, keys).into_iter().map(|r| Ok(r.map(GetResult::InMemory))).collect()
            }
            #[cfg(tidehunter)]
            (Storage::TideHunter(db), ColumnFamily::TideHunter((ks, prefix))) => {
                let res = keys.into_iter().map(|k| {
                    db.get(*ks, &transform_th_key(k.as_ref(), prefix)).map_err(typed_store_error_from_th_error)
                });
                res.into_iter().map(|r| r.map(|item| item.map(GetResult::TideHunter))).collect()
            }
    ...
    fn delete_cf<K: AsRef<[u8]>>(&self, cf: &ColumnFamily, key: K) -> Result<(), TypedStoreError> {
        ...
        let ret = match (&self.storage, cf) {
            (Storage::Rocks(db), ColumnFamily::Rocks(_)) => {
                db.underlying.delete_cf(&cf.rocks_cf(db), key).map_err(typed_store_err_from_rocks_err)
            }
            (Storage::InMemory(db), ColumnFamily::InMemory(cf_name)) => {
            ...
            #[cfg(tidehunter)]
            (Storage::TideHunter(db), ColumnFamily::TideHunter((ks, prefix))) => {
                db.remove(*ks, transform_th_key(key.as_ref(), prefix)).map_err(typed_store_error_from_th_error)
            }
            _ => Err(TypedStoreError::RocksDBError("typed store invariant violation".to_string())),
        };
    ...
    fn put_cf(&self, cf: &ColumnFamily, key: Vec<u8>, value: Vec<u8>) -> Result<(), TypedStoreError> {
        fail_point!("put-cf-before");
        let ret = match (&self.storage, cf) {
            (Storage::Rocks(db), ColumnFamily::Rocks(_)) => {
                db.underlying.put_cf(&cf.rocks_cf(db), key, value).map_err(typed_store_err_from_rocks_err)
            }
            (Storage::InMemory(db), ColumnFamily::InMemory(cf_name)) => {
            ...
            #[cfg(tidehunter)]
            (Storage::TideHunter(db), ColumnFamily::TideHunter((ks, prefix))) => {
                db.insert(*ks, transform_th_key(&key, prefix), value).map_err(typed_store_error_from_th_error)
            }
            _ => Err(TypedStoreError::RocksDBError("typed store invariant violation".to_string())),
        };
    ...
    pub fn key_may_exist_cf<K: AsRef<[u8]>>(&self, cf_name: &str, key: K, readopts: &ReadOptions) -> bool {
        match &self.storage {
            ...
            Storage::Rocks(rocks) => rocks.underlying.key_may_exist_cf_opt(&rocks_cf(rocks, cf_name), key, readopts),
            _ => true,
        }
    }
    ...
    pub fn write_opt(
        ...
        let ret = match (&self.storage, batch) {
            (Storage::Rocks(rocks), StorageWriteBatch::Rocks(batch)) => {
                rocks.underlying.write_opt(batch, write_options).map_err(typed_store_err_from_rocks_err)
            }
            ...
            (Storage::TideHunter(db), StorageWriteBatch::TideHunter(batch)) => {
                ...
                db.write_batch(batch).map_err(typed_store_error_from_th_error)
            }
    ...
    #[cfg(tidehunter)]
    pub fn drop_cells_in_range(&self, ks: KeySpace, from_inclusive: &[u8], to_inclusive: &[u8]) -> anyhow::Result<()> {
        if let Storage::TideHunter(db) = &self.storage {
            db.drop_cells_in_range(ks, from_inclusive, to_inclusive).map_err(|e| anyhow::anyhow!("{:?}", e))?;
        } else {
    ...
    pub fn compact_range_cf<K: AsRef<[u8]>>(&self, cf_name: &str, start: Option<K>, end: Option<K>) {
    ...
    pub fn checkpoint(&self, path: &Path) -> Result<(), TypedStoreError> {
        ...
        if let Storage::Rocks(rocks) = &self.storage {
            let checkpoint = Checkpoint::new(&rocks.underlying).map_err(typed_store_err_from_rocks_err)?;
...
fn rocks_cf<'a>(rocks_db: &'a RocksDB, cf_name: &str) -> Arc<rocksdb::BoundColumnFamily<'a>> {
    rocks_db.underlying.cf_handle(cf_name).expect("Map-keying column family should have been checked at DB creation")
}
...
impl<K, V> DBMap<K, V> {
    ...
    pub fn reopen(
        ...
        Ok(DBMap::new(db.clone(), rw_options, &cf_key, ColumnFamily::Rocks(cf_key.to_string()), is_deprecated))
    }

    #[cfg(tidehunter)]
    pub fn reopen_th(db: Arc<Database>, cf_name: &str, ks: KeySpace, prefix: Option<Vec<u8>>) -> Self {
        DBMap::new(db, &ReadWriteOptions::default(), cf_name, ColumnFamily::TideHunter((ks, prefix.clone())), false)
    }
    ...
    pub fn batch(&self) -> DBBatch {
        let batch = match &self.db.storage {
            ...
            #[cfg(tidehunter)]
            Storage::TideHunter(_) => StorageWriteBatch::TideHunter(tidehunter::batch::WriteBatch::new()),
        };
        DBBatch::new(&self.db, batch, &self.db_metrics, &self.write_sample_interval)
    }
    ...
    #[cfg(tidehunter)]
    pub fn drop_cells_in_range<J: Serialize>(&self, from_inclusive: &J, to_inclusive: &J) -> Result<(), TypedStoreError>
    where
        K: Serialize,
    {
        ...
        K: Serialize,
    {
        ...
        let results: Result<Vec<_>, TypedStoreError> =
            self.db.multi_get(&self.column_family, keys_bytes, &self.opts.readopts()).into_iter().collect();
        let entries = results?;
    ...
    fn report_rocksdb_metrics(database: &Arc<Database>, cf_name: &str, db_metrics: &Arc<DBMetrics>) {
        let Storage::Rocks(rocksdb) = &database.storage else {
            return;
        };

        let Some(cf) = rocksdb.underlying.cf_handle(cf_name) else {
            tracing::warn!("unable to report metrics for cf {cf_name:?} in db {:?}", database.db_name());
            return;
        };

        db_metrics.cf_metrics.rocksdb_total_sst_files_size.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::TOTAL_SST_FILES_SIZE).unwrap_or(METRICS_ERROR),
        );
        db_metrics.cf_metrics.rocksdb_total_blob_files_size.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, ROCKSDB_PROPERTY_TOTAL_BLOB_FILES_SIZE)
        ...
        let total_num_files: i64 = (0 ..= 6)
            .map(|level| {
                Self::get_rocksdb_int_property(rocksdb, &cf, &num_files_at_level(level)).unwrap_or(METRICS_ERROR)
            })
            .sum();
        ...
        db_metrics
            ...
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, &num_files_at_level(0)).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_current_size_active_mem_tables.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::CUR_SIZE_ACTIVE_MEM_TABLE).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_size_all_mem_tables
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::SIZE_ALL_MEM_TABLES).unwrap_or(METRICS_ERROR));
        db_metrics
            .cf_metrics
            .rocksdb_num_snapshots
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::NUM_SNAPSHOTS).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_oldest_snapshot_time.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::OLDEST_SNAPSHOT_TIME).unwrap_or(METRICS_ERROR),
        );
        db_metrics.cf_metrics.rocksdb_actual_delayed_write_rate.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::ACTUAL_DELAYED_WRITE_RATE).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_is_write_stopped
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::IS_WRITE_STOPPED).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_block_cache_capacity.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::BLOCK_CACHE_CAPACITY).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_block_cache_usage
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::BLOCK_CACHE_USAGE).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_block_cache_pinned_usage.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::BLOCK_CACHE_PINNED_USAGE).unwrap_or(METRICS_ERROR),
        );
        db_metrics.cf_metrics.rocksdb_estimate_table_readers_mem.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::ESTIMATE_TABLE_READERS_MEM)
                .unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            ...
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::ESTIMATE_NUM_KEYS).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_num_immutable_mem_tables.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::NUM_IMMUTABLE_MEM_TABLE).unwrap_or(METRICS_ERROR),
        );
        db_metrics.cf_metrics.rocksdb_mem_table_flush_pending.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::MEM_TABLE_FLUSH_PENDING).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_compaction_pending
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::COMPACTION_PENDING).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_estimate_pending_compaction_bytes.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::ESTIMATE_PENDING_COMPACTION_BYTES)
                .unwrap_or(METRICS_ERROR),
        );
        db_metrics.cf_metrics.rocksdb_num_running_compactions.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::NUM_RUNNING_COMPACTIONS).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_num_running_flushes
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::NUM_RUNNING_FLUSHES).unwrap_or(METRICS_ERROR));
        db_metrics.cf_metrics.rocksdb_estimate_oldest_key_time.with_label_values(&[cf_name]).set(
            Self::get_rocksdb_int_property(rocksdb, &cf, properties::ESTIMATE_OLDEST_KEY_TIME).unwrap_or(METRICS_ERROR),
        );
        db_metrics
            .cf_metrics
            .rocksdb_background_errors
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::BACKGROUND_ERRORS).unwrap_or(METRICS_ERROR));
        db_metrics
            .cf_metrics
            .rocksdb_base_level
            .with_label_values(&[cf_name])
            .set(Self::get_rocksdb_int_property(rocksdb, &cf, properties::BASE_LEVEL).unwrap_or(METRICS_ERROR));
    }
    ...
    fn start_iter_timer(&self) -> HistogramTimer {
        self.db_metrics.op_metrics.rocksdb_iter_latency_seconds.with_label_values(&[&self.cf]).start_timer()
    }
        ...
        V: Serialize + DeserializeOwned,
    {
        ...
        match &self.db.storage {
            Storage::Rocks(db) => {
                let readopts = rocks_util::apply_range_bounds(self.opts.readopts(), it_lower_bound, it_upper_bound);
            ...
            Storage::InMemory(db) => Ok(db.iterator(&self.cf, it_lower_bound, it_upper_bound, true)),
            #[cfg(tidehunter)]
            Storage::TideHunter(db) => match &self.column_family {
                ColumnFamily::TideHunter((ks, prefix)) => {
                    ...
                    iter.reverse();
                    Ok(Box::new(transform_th_iterator(iter, prefix, self.start_iter_timer())))
                }
    ...
    pub fn delete_batch<J: Borrow<K>, K: Serialize, V>(
        ...
        purged_vals.into_iter().try_for_each::<_, Result<_, TypedStoreError>>(|k| {
            ...
            match (&mut self.batch, &db.column_family) {
                ...
                (StorageWriteBatch::InMemory(b), ColumnFamily::InMemory(name)) => b.delete_cf(name, k_buf),
                #[cfg(tidehunter)]
    ...
    pub fn insert_batch<J: Borrow<K>, K: Serialize, U: Borrow<V>, V: Serialize>(
        ...
        new_vals.into_iter().try_for_each::<_, Result<_, TypedStoreError>>(|(k, v)| {
            ...
            if db.opts.log_value_hash {
                ...
                debug!("Insert to DB table: {:?}, key_hash: {:?}, value_hash: {:?}", db.cf_name(), key_hash, value_hash);
            }
            match (&mut self.batch, &db.column_family) {
                (StorageWriteBatch::Rocks(b), ColumnFamily::Rocks(name)) => {
                    b.put_cf(&rocks_cf_from_db(&self.database, name)?, k_buf, v_buf)
                }
                (StorageWriteBatch::InMemory(b), ColumnFamily::InMemory(name)) => b.put_cf(name, k_buf, v_buf),
                #[cfg(tidehunter)]
                (StorageWriteBatch::TideHunter(b), ColumnFamily::TideHunter((ks, prefix))) => {
    ...
    V: Serialize + DeserializeOwned,
{
    ...
    fn safe_iter(&'a self) -> DbIterator<'a, (K, V)> {
        match &self.db.storage {
            Storage::Rocks(db) => {
                let db_iter = db.underlying.raw_iterator_cf_opt(&rocks_cf(db, &self.cf), self.opts.readopts());
                let (_timer, bytes_scanned, keys_scanned, _perf_ctx) = self.create_iter_context();
            ...
            Storage::TideHunter(db) => match &self.column_family {
                ColumnFamily::TideHunter((ks, prefix)) => {
                    Box::new(transform_th_iterator(db.iterator(*ks), prefix, self.start_iter_timer()))
                }
                _ => unreachable!("storage backend invariant violation"),
            },
        }
...
pub fn open_cf_opts<P: AsRef<Path>>(
    ...
    nondeterministic!({
        ...
        Ok(Arc::new(Database::new(Storage::Rocks(RocksDB { underlying: rocksdb }), metric_conf)))
    })
}
...
pub async fn safe_drop_db(path: PathBuf, timeout: Duration) -> Result<(), rocksdb::Error> {
    let mut backoff = backoff::ExponentialBackoff { max_elapsed_time: Some(timeout), ..Default::default() };
    loop {
