    ...
    pub fn write(&self, batch: StorageWriteBatch) -> Result<(), TypedStoreError> {
        ...
        let ret = match (&self.storage, batch) {
            (Storage::Rocks(rocks), StorageWriteBatch::Rocks(batch)) => {
                rocks.underlying.write(batch).map_err(typed_store_err_from_rocks_err)
            }
    ...
    pub fn partial_merge_batch<J: Borrow<K>, K: Serialize, V: Serialize, B: AsRef<[u8]>>(
        ...
        new_vals.into_iter().try_for_each::<_, Result<_, TypedStoreError>>(|(k, v)| {
            ...
            match &mut self.batch {
                StorageWriteBatch::Rocks(b) => b.merge_cf(&rocks_cf_from_db(&self.database, db.cf_name())?, k_buf, v),
