    ...
    V: Serialize + DeserializeOwned,
{
    ...
    fn schedule_delete_all(&self) -> Result<(), TypedStoreError> {
        ...
        let last_key = self.reversed_safe_iter_with_bounds(None, None)?.next().transpose()?.map(|(k, _v)| k);
...
impl Default for ReadWriteOptions {
    fn default() -> Self {
        Self { ignore_range_deletions: true, sync_to_disk: std::env::var("SUI_DB_SYNC_TO_DISK").is_ok_and(|v| v != "0") }
    }
}
