...
pub(crate) fn iterator_bounds<K>(lower_bound: Option<K>, upper_bound: Option<K>) -> (Option<Vec<u8>>, Option<Vec<u8>>)
where
    K: Serialize,
{
...
pub(crate) fn iterator_bounds_with_range<K>(range: impl RangeBounds<K>) -> (Option<Vec<u8>>, Option<Vec<u8>>)
where
    K: Serialize,
{
