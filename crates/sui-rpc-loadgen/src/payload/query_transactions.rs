...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(&'a self, op: &'a QueryTransactionBlocks, _signer_info: &Option<SignerInfo>) -> Result<()> {
        ...
        let filters = {
            let mut from: Vec<Option<TransactionFilter>> =
                op.addresses.iter().map(|address| Some(TransactionFilter::FromAddress(*address))).collect();

            let mut to = op.addresses.iter().map(|address| Some(TransactionFilter::ToAddress(*address))).collect();
        ...
        for query in queries {
            ...
            while results.is_empty() || results.iter().any(|r| r.has_next_page) {
                ...
                results = join_all(clients.iter().map(|client| {
                    ...
                    async move { query_transaction_blocks(client, with_query, cursor, None).await.unwrap() }
                }))
                .await;
...
async fn query_transaction_blocks(
    ...
    let transactions = client.read_api().query_transaction_blocks(query, cursor, limit, true).await.unwrap();
    Ok(transactions)
}
