...
impl<'a> ProcessPayload<'a, &'a PayOct> for RpcCommandProcessor {
    async fn process(&'a self, _op: &'a PayOct, signer_info: &Option<SignerInfo>) -> anyhow::Result<()> {
        ...
        let keypair = SuiKeyPair::decode_base64(&encoded_keypair).expect("Decoding keypair should not fail");

        debug!("Transfer Sui {} time to {recipient} with {amount} MIST with {gas_payments:?}", gas_payments.len());
        ...
        join_all(gas_payments.iter().map(|gas| async {
            ...
            self.sign_and_execute(client, &keypair, tx, ExecuteTransactionRequestType::WaitForEffectsCert).await
        }))
        .await;
