...
static MIDDLEWARE_OPS: Lazy<CounterVec> = Lazy::new(|| {
    register_counter_vec!("middleware_operations", "Operations counters and status for axum middleware.", &[
        "operation",
        "status"
    ])
    .unwrap()
...
pub async fn expect_mysten_proxy_header(
    ...
    match format!("{content_type}").as_str() {
        ...
        ct => {
            ...
            MIDDLEWARE_OPS.with_label_values(&["expect_mysten_proxy_header", "invalid-content-type"]).inc();
...
pub async fn expect_valid_public_key(
    ...
    let Some(public_key) = tls_connect_info.public_key() else {
        ...
        MIDDLEWARE_OPS.with_label_values(&["expect_valid_public_key", "missing-public-key"]).inc();
    ...
    let Some(peer) = allower.get(public_key) else {
        ...
        MIDDLEWARE_OPS.with_label_values(&["expect_valid_public_key", "unknown-validator-connection-attempt"]).inc();
        return Err((StatusCode::FORBIDDEN, "unknown clients are not allowed"));
    };
    ...
    S: Send + Sync,
{
    ...
    async fn from_request(req: Request<axum::body::Body>, state: &S) -> Result<Self, Self::Rejection> {
        let should_be_snappy = req.headers().get(CONTENT_ENCODING).map(|v| v.as_bytes() == b"snappy").unwrap_or(false);

        let body = Bytes::from_request(req, state).await.map_err(|e| {
            let msg = format!("error extracting bytes; {e}");
            error!(msg);
            MIDDLEWARE_OPS.with_label_values(&["LenDelimProtobuf_from_request", "unable-to-extract-bytes"]).inc();
            (e.status(), msg)
        })?;
        ...
        let intermediate = if should_be_snappy {
            ...
            let decompressed = s.decompress_vec(&body).map_err(|e| {
                ...
                error!(msg);
                MIDDLEWARE_OPS.with_label_values(&["LenDelimProtobuf_decompress_vec", "unable-to-decode-snappy"]).inc();
        ...
        let decoded = decoder.parse::<MetricFamily>().map_err(|e| {
            ...
            error!(msg);
            MIDDLEWARE_OPS.with_label_values(&["LenDelimProtobuf_from_request", "unable-to-decode-protobufs"]).inc();
            (StatusCode::BAD_REQUEST, msg)
        })?;
        Ok(Self(decoded))
    }
