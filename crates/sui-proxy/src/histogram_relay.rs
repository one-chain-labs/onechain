...
static RELAY_DURATION: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        ...
        vec![
            ...
            1.75, 2.0, 4.0, 8.0, 10.0, 12.5, 15.0
        ],
    )
    .unwrap()
});
...
pub fn start_prometheus_server(listener: TcpListener) -> HistogramRelay {
    ...
    let app = Router::new().route(METRICS_ROUTE, get(metrics)).layer(Extension(relay.clone())).layer(
...
async fn metrics(Extension(relay): Extension<HistogramRelay>) -> (StatusCode, String) {
    let Ok(expformat) = relay.export() else {
        return (StatusCode::INTERNAL_SERVER_ERROR, "unable to pop metrics from HistogramRelay".into());
    };
...
impl HistogramRelay {
    ...
    pub fn submit(&self, data: Vec<MetricFamily>) {
        ...
        let timestamp_secs = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;
        let mut queue = self.0.lock().expect("couldn't get mut lock on HistogramRelay");
        queue.retain(|v| {
        ...
        RELAY_PRESSURE.with_label_values(&["submitted"]).inc_by(data.len() as f64);
    ...
    pub fn export(&self) -> Result<String> {
        ...
        info!("histogram queue drained {} items; remaining count {}", histograms.len(), queue.len());
        ...
        RELAY_PRESSURE.with_label_values(&["exported"]).inc_by(histograms.len() as f64);
        timer.observe_duration();
        Ok(string)
    }
}
...
fn extract_histograms(data: Vec<MetricFamily>) -> impl Iterator<Item = MetricFamily> {
    data.into_iter().filter_map(|mf| {
        let metrics = mf.get_metric().iter().filter_map(|m| {
            ...
            v.set_histogram(m.histogram.clone().unwrap());
