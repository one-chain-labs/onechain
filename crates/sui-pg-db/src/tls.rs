...
impl ServerCertVerifier for SkipServerCertCheck {
    ...
    fn supported_verify_schemes(&self) -> Vec<rustls::SignatureScheme> {
        rustls::client::WebPkiServerVerifier::builder(Arc::new(root_certs())).build().unwrap().supported_verify_schemes()
    }
}
...
pub(crate) async fn establish_tls_connection(
    ...
    let (client, conn) =
        tokio_postgres::connect(database_url, tls).await.map_err(|e| ConnectionError::BadConnection(e.to_string()))?;
...
pub(crate) fn build_tls_config(
    ...
    if let Some(ca_cert_path) = &tls_ca_cert_path {
        let ca_cert_bytes = std::fs::read(ca_cert_path)
            .with_context(|| format!("Failed to read CA certificate from {}", ca_cert_path.display()))?;

        let certs = if ca_cert_bytes.starts_with(b"-----BEGIN CERTIFICATE-----") {
            rustls_pemfile::certs(&mut ca_cert_bytes.as_slice())
                .collect::<Result<Vec<_>, _>>()
                .with_context(|| format!("Failed to parse PEM certificates from {}", ca_cert_path.display()))?
        } else {
        ...
        for cert in certs {
            root_certs.add(cert).with_context(|| "Failed to add CA certificate to root store".to_string())?;
        }
    }

    Ok(ClientConfig::builder().with_root_certificates(root_certs).with_no_client_auth())
}

fn root_certs() -> RootCertStore {
    RootCertStore { roots: TLS_SERVER_ROOTS.to_vec() }
}
