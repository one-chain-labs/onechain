...
impl<'f> Query<'f> {
    ...
    pub fn new(sql: impl AsRef<str>) -> Self {
        Self { parts: vec![Part::Sql(sql.as_ref().to_owned())] }
    }
    ...
    where
        ...
        ST: Send + 'f,
    {
        self.parts.push(Part::Bind(Box::new(Bind { value, _data: PhantomData })));
...
mod tests {
    ...
    fn query_extended() {
        ...
        assert_eq!(diesel::debug_query(&q).to_string(), "SELECT 1 WHERE id = 1 -- binds: []");
    }
    ...
    fn query_with_binds() {
        let q =
            Query::new("SELECT 1 WHERE ").sql("id = ").bind::<BigInt, _>(42).sql(" AND name = ").bind::<Text, _>("foo");

        assert_eq!(diesel::debug_query(&q).to_string(), "SELECT 1 WHERE id = $1 AND name = $2 -- binds: [42, \"foo\"]");
    }
