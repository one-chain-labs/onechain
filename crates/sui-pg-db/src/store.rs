...
use diesel::{prelude::*, sql_types::BigInt, ExpressionMethods, OptionalExtension};
...
use crate::{model::StoredWatermark, schema::watermarks, Connection, Db};

#[async_trait]
impl store::Connection for Connection<'_> {
    async fn committer_watermark(&mut self, pipeline_task: &str) -> anyhow::Result<Option<store::CommitterWatermark>> {
    ...
    async fn reader_watermark(&mut self, pipeline: &'static str) -> anyhow::Result<Option<store::ReaderWatermark>> {
    ...
    async fn set_reader_watermark(&mut self, pipeline: &'static str, reader_lo: u64) -> anyhow::Result<bool> {
        Ok(diesel::update(watermarks::table)
            .set((watermarks::reader_lo.eq(reader_lo as i64), watermarks::pruner_timestamp.eq(diesel::dsl::now)))
            .filter(watermarks::pipeline.eq(pipeline))
