...
fn extract_from_message(message: &str) -> Option<LogEntry> {
    ...
    let headers_json_str = headers_str.trim_start_matches("headers=").split_once(" body=")?.0;
    let headers: Value = serde_json::from_str(headers_json_str).ok()?;
    let host = headers.get("host").and_then(|h| h.as_str()).unwrap_or("unknown_host").to_string();
        ...
        && let Some(peer_type_start) = message.find(" peer_type=")
    {
        ...
        if raw_body.starts_with('b') {
            ...
            if let Ok(parsed) = serde_json::from_str::<Value>(&unescaped) {
                let method = parsed.get("method").and_then(|m| m.as_str()).unwrap_or("unknown_method").to_string();
                return Some(LogEntry { timestamp, host, method, body: unescaped });
            }
        }
    }
    None
...
fn extract_from_proxy_message(message: &str) -> Option<LogEntry> {
    ...
    if request_split.len() == 2 {
        ...
        let request_body = format!(r#"{{"jsonrpc":"2.0","id":{},"method":"{}","params":{}}}"#, id, method, params);

        // Host is not in this format, use a placeholder
        let host = "unknown".to_string();

        return Some(LogEntry { timestamp, host, method, body: request_body });
    }
...
async fn run() -> Result<(), Box<dyn Error>> {
    let grafana_url =
        env::var("GRAFANA_LOGS_URL").unwrap_or_else(|_| "https://metrics.sui.io/loki/api/v1/query_range".to_string());
    ...
    let default_substring = if net == "mainnet-proxy" { "Request:" } else { "Sampled read request" };
    ...
    loop {
        ...
        let response = fetch_logs(&client, &grafana_url, &query, &start, &end, chunk_limit, offset).await?;

        let batch: Vec<_> = response
            .data
            .result
            .into_iter()
            .flat_map(|result| result.values.into_iter().map(|(_, message)| GrafanaLog { message }))
            .collect();
