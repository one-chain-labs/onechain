...
async fn fetch_logs(
    ...
    let resp = client.get(url).header(ACCEPT, "application/json").query(&params).send().await?;
...
async fn run() -> Result<(), Box<dyn Error>> {
    ...
    let query = format!(r#"{{namespace="{}", container="sui-edge-proxy-mysten"}} |= "{}""#, namespace, substring);
    debug!("Query: {}", query);
