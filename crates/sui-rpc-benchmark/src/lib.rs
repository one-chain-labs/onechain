...
use crate::{
    ...
    direct::{
        ...
        query_template_generator::QueryTemplateGenerator,
    },
};

#[derive(Parser)]
#[clap(name = "sui-rpc-benchmark", about = "Benchmark tool for comparing Sui RPC access methods")]
pub struct Opts {
    #[clap(subcommand)]
    pub command: Command,
}
...
pub async fn run_benchmarks() -> Result<(), anyhow::Error> {
    ...
    match opts.command {
        Command::DirectQuery { db_url, concurrency, duration_secs } => {
            ...
            info!("Enriched {} queries with sample data", enriched_queries.len());
        ...
        Command::JsonRpc { endpoint, concurrency, duration_secs, requests_file, methods_to_skip } => {
            info!(concurrency, ?duration_secs, requests_file, "Running JSON RPC benchmark against {endpoint}");
