...
pub async fn run_benchmarks() -> Result<(), anyhow::Error> {
    ...
    match opts.command {
        Command::DirectQuery { db_url, concurrency, duration_secs } => {
            ...
            let config =
                BenchmarkConfig { concurrency, duration: Duration::from_secs(duration_secs), json_rpc_file_path: None };
        ...
        Command::JsonRpc { endpoint, concurrency, duration_secs, requests_file } => {
