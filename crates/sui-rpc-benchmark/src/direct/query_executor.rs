...
use crate::{
    ...
    direct::{
        metrics::{BenchmarkResult, MetricsCollector},
        query_enricher::{EnrichedBenchmarkQuery, SqlValue},
    },
};
...
impl QueryExecutor {
    pub async fn new(
        ...
        Ok(Self { pool, enriched_queries, config, metrics: MetricsCollector::default() })
    }
    ...
    async fn worker_task(
        ...
        while Instant::now() < deadline {
            let enriched = enriched_queries.choose(&mut rng).ok_or_else(|| anyhow::anyhow!("No queries available"))?;
    ...
    pub async fn run(&self) -> Result<BenchmarkResult> {
        info!("Running benchmark with {} concurrent clients", self.config.concurrency);

        let start = Instant::now();
        let deadline = start + self.config.duration;
        let (concurrency, metrics, pool, queries) =
            (self.config.concurrency, self.metrics.clone(), self.pool.clone(), self.enriched_queries.clone());
        futures::stream::iter(
            queries.into_iter().map(move |query| (pool.clone(), vec![query], metrics.clone(), deadline)),
        )
        .try_for_each_spawned(concurrency, |(pool, queries, metrics, deadline)| async move {
