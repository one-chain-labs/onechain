...
impl QueryEnricher {
    ...
    pub async fn enrich_queries(&self, queries: Vec<QueryTemplate>) -> Result<Vec<EnrichedBenchmarkQuery>> {
            ...
            .try_for_each_spawned(10, move |query| {
                ...
                async move {
                    ...
                    let Some(first_row) = rows.first() else {
                        warn!(table = query.table_name, "No sample data found for query on table, table is empty.");
                    ...
                    let types = first_row.columns().iter().map(|c| c.type_().clone()).collect();
                    let raw_rows = rows.iter().map(Self::row_to_values).collect();

                    let enriched = EnrichedBenchmarkQuery { query: query.clone(), rows: raw_rows, types };
                    enriched_queries.lock().push(enriched);
                    Ok::<(), anyhow::Error>(())
                }
            })
        ...
        Ok(Arc::try_unwrap(enriched_queries).map_err(|_| anyhow::anyhow!("Failed to try_unwrap Arc"))?.into_inner())
    }
}
