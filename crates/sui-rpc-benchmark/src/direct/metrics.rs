...
impl MetricsCollector {
    ...
    pub fn record_query(&self, query_template: QueryTemplate, latency: Duration, is_error: bool) {
        let mut entry = self.metrics.entry(query_template.table_name.to_string()).or_default();
    ...
    pub fn generate_report(&self) -> BenchmarkResult {
        ...
        for entry in self.metrics.iter() {
            ...
            let avg_latency =
                if successful > 0 { metrics.latency_ms.iter().sum::<f64>() / successful as f64 } else { 0.0 };
        ...
        BenchmarkResult {
            total_queries,
            total_errors,
            avg_latency_ms: if total_successful > 0 { total_latency / total_successful as f64 } else { 0.0 },
            table_stats,
        }
    }
}
