...
impl QueryTemplateGenerator {
    ...
    pub async fn generate_query_templates(&self) -> Result<Vec<QueryTemplate>, anyhow::Error> {
        ...
        let tables: Vec<String> = rows
            ...
            .into_iter()
            .collect();
        info!("Found {} active tables in database: {:?}", tables.len(), tables);
        ...
        for (i, query) in queries.iter().enumerate() {
            debug!("  {}. Table: {}, Template: {}", i + 1, query.table_name, query.query_template);
        }
    ...
    fn create_pk_benchmark_query(&self, table: &str, columns: &[String]) -> QueryTemplate {
        let conditions =
            columns.iter().enumerate().map(|(i, col)| format!("{} = ${}", col, i + 1)).collect::<Vec<_>>().join(" AND ");
