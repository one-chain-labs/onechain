...
pub async fn run_queries(
    ...
    let stream = futures::stream::iter(requests.into_iter().map(move |request_line| {
        ...
        async move {
            let now = Instant::now();
            let res =
                timeout(std::time::Duration::from_secs(10), client.post(&endpoint).json(&request_line.body_json).send())
                    .await;

            let elapsed_ms = now.elapsed().as_millis() as f64;
            let is_error = !matches!(res, Ok(Ok(ref resp)) if resp.status().is_success());

            let mut stats = task_stats.lock().map_err(|e| anyhow::anyhow!("Failed to acquire stats lock: {}", e))?;
            stats.record_request(&request_line.method, elapsed_ms, is_error);
            Ok::<(), anyhow::Error>(())
        }
    }));

    timeout(config.duration, stream.try_for_each_spawned(concurrency, |fut| fut)).await.unwrap_or(Ok(()))?;
