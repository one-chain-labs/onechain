...
impl PaginationCursorState {
    ...
    fn get_method_key(method: &str, params: &[Value]) -> Result<(String, Vec<Value>), anyhow::Error> {
        ...
        if let Some(param_to_modify) = key_params.get_mut(*cursor_idx) {
            *param_to_modify = Value::Null;
            ...
            let method_length =
                METHOD_LENGTHS.get(method).with_context(|| format!("method {} not found in method lengths", method))?;
    ...
    fn update_params_cursor(
        ...
        let param_to_modify = params_array.get_mut(cursor_idx).with_context(|| {
            format!("Failed to access cursor parameter at index {} for method {}", cursor_idx, method)
        })?;
...
pub async fn run_queries(
    ...
    debug!("Starting benchmark with {} requests at concurrency {}", total_requests, concurrency);
    ...
    let process_requests = async {
        ...
        let result = futures::stream::iter(requests.into_iter())
            .try_for_each_spawned(concurrency, |mut request_line| {
                ...
                let params =
                    request_line.body_json.get("params").and_then(|v| v.as_array()).map(|a| a.to_vec()).unwrap_or_else(
                        || {
                ...
                async move {
                    ...
                    let res = client.post(&endpoint).json(&request_line.body_json).send().await;
                    ...
                    if let Ok(resp) = res {
                        ...
                        if status.is_success() {
                            let supports_pagination =
                                PaginationCursorState::get_method_cursor_index(&request_line.method).is_some();
                            if supports_pagination {
                                ...
                                debug!(
                                    "Parsed response for method: {}, result: {:?}",
                                    request_line.method, parse_result
                                );
                                if let Ok(Body { result }) = parse_result {
                                    let method_key =
                                        match PaginationCursorState::get_method_key(&request_line.method, &params) {
                                            Ok(key) => key,
                                            Err(e) => return Err(BenchmarkError::Other(e)),
                                        };
                                    if result.has_next_page {
                                        debug!(
                                            "Updated pagination cursor for method: {}, has_next_page: true",
                                            request_line.method
                                        );
                    ...
                    let mut stats = task_stats.lock().expect("Thread holding stats lock panicked");
    ...
    let final_stats = shared_stats.lock().expect("Thread holding stats lock panicked").clone();
    info!(
        "Benchmark completed in {:?}. Total requests: {}, errors: {}, avg latency: {:.2}ms",
        elapsed,
        final_stats.total_sent,
        final_stats.total_errors,
        if final_stats.total_sent > 0 { final_stats.total_latency_ms / final_stats.total_sent as f64 } else { 0.0 }
    );
