...
use sui_rpc::{
    ...
    merge::Merge,
    proto::sui::rpc::v2beta2::{Epoch, GetEpochRequest, GetEpochResponse, ProtocolConfig as RpcProtocolConfig},
};
use sui_rpc_api::{
    proto::{google::rpc::bad_request::FieldViolation, timestamp_ms_to_proto},
    protocol_config_to_proto,
    ErrorReason,
};
...
pub async fn get_epoch(
    ...
    let maybe_epoch_info =
        if let Some(epoch) = request.epoch { client.get_epoch(epoch).await? } else { client.get_latest_epoch().await? };
    ...
    if let (Some(submask), Some(version)) =
        (read_mask.subtree(Epoch::PROTOCOL_CONFIG_FIELD.name), epoch_info.protocol_version)
    {
        let protocol_config = ProtocolConfig::get_for_version_if_supported(version.into(), chain);
        message.protocol_config =
            protocol_config.map(|config| RpcProtocolConfig::merge_from(protocol_config_to_proto(config), &submask));
    }
    if read_mask.contains(Epoch::COMMITTEE_FIELD.name) {
        message.committee = epoch_info.system_state.map(|system_state| {
            let committee: ValidatorCommittee = system_state.get_current_epoch_committee().committee().clone().into();
            committee.into()
        });
    }
