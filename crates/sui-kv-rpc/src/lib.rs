...
use sui_rpc_api::{proto::timestamp_ms_to_proto, CheckpointNotFoundError, RpcError, ServerVersion};
use sui_sdk_types::Digest;
use sui_types::{digests::ChainIdentifier, message_envelope::Message};
use tokio::{
    sync::RwLock,
    time::{sleep, Duration},
};
...
impl KvRpcServer {
    pub async fn new(
        ...
        .await?;
        let genesis =
            client.get_checkpoints(&[0]).await?.pop().expect("failed to fetch genesis checkpoint from the KV store");
        let chain_id = ChainIdentifier::from(genesis.summary.digest());
        let cache = Arc::new(RwLock::new(None));

        let server = Self { chain_id, client, server_version, checkpoint_bucket, cache };
...
impl LedgerService for KvRpcServer {
    async fn get_service_info(
        ...
        get_service_info(self.client.clone(), self.chain_id, self.server_version.clone())
            .await
    ...
    async fn get_checkpoint(
        ...
        get_checkpoint::get_checkpoint(self.client.clone(), request.into_inner(), self.checkpoint_bucket.clone())
            .await
            .map(tonic::Response::new)
            .map_err(Into::into)
    }
    ...
    async fn get_epoch(
        ...
        get_epoch::get_epoch(self.client.clone(), request.into_inner(), self.chain_id.chain())
            .await
            .map(tonic::Response::new)
            .map_err(Into::into)
    }
