...
pub async fn get_transaction(
    ...
    let transaction_digest = request
        .digest
        .ok_or_else(|| {
            FieldViolation::new("digest").with_description("missing digest").with_reason(ErrorReason::FieldMissing)
        })?
    ...
    let read_mask = {
        let read_mask =
            request.read_mask.unwrap_or_else(|| FieldMask::from_str(GetTransactionRequest::READ_MASK_DEFAULT));
    ...
    let transaction = response.pop().ok_or(TransactionNotFoundError(transaction_digest.into()))?;
...
pub async fn batch_get_transactions(
    ...
    let read_mask = {
        let read_mask = read_mask.unwrap_or_else(|| FieldMask::from_str(BatchGetTransactionsRequest::READ_MASK_DEFAULT));
        read_mask.validate::<ExecutedTransaction>().map_err(|path| {
            FieldViolation::new("read_mask")
                .with_description(format!("invalid read_mask path: {path}"))
                .with_reason(ErrorReason::FieldInvalid)
    ...
    let digests = digests.iter().map(|digest| TransactionDigest::from_str(digest)).collect::<Result<Vec<_>, _>>()?;
...
fn transaction_to_response(source: TransactionData, mask: &FieldMaskTree) -> Result<ExecutedTransaction, RpcError> {
    ...
    if let Some(submask) = mask.subtree(ExecutedTransaction::TRANSACTION_FIELD.name) {
        let transaction = sui_sdk_types::Transaction::try_from(source.transaction.transaction_data().clone())?;
    ...
    if let Some(submask) = mask.subtree(ExecutedTransaction::SIGNATURES_FIELD.name) {
        message.signatures = source
            ...
            .tx_signatures()
            .iter()
            .map(|s| sui_sdk_types::UserSignature::try_from(s.clone()).map(|s| UserSignature::merge_from(s, &submask)))
            .collect::<Result<_, _>>()?;
    }

    if let Some(submask) = mask.subtree(ExecutedTransaction::EFFECTS_FIELD.name) {
        let effects =
            TransactionEffects::merge_from(&sui_sdk_types::TransactionEffects::try_from(source.effects)?, &submask);
    ...
    if let Some(submask) = mask.subtree(ExecutedTransaction::EVENTS_FIELD.name) {
        if let Some(events) = source.events {
            message.events =
                Some(TransactionEvents::merge_from(sui_sdk_types::TransactionEvents::try_from(events)?, &submask));
