...
pub(crate) async fn get_object(
    ...
    let object = match version {
        ...
        None => client.get_latest_object(&object_id.into()).await?.ok_or_else(|| ObjectNotFoundError::new(object_id))?,
    };
...
pub(crate) async fn batch_get_objects(
    ...
    let object_keys: Vec<_> = requests
        .into_iter()
        .map(|(object_id, version)| ObjectKey(object_id.into(), version.expect("invariant's already checked").into()))
        .collect();
    let response: HashMap<_, _> =
        client.get_objects(&object_keys).await?.into_iter().map(|obj| ((obj.id(), obj.version()), obj)).collect();
    ...
    let objects = object_keys
        ...
        .map(|object_key| {
            ...
            let err: RpcError = ObjectNotFoundError::new_with_version(object_key.0.into(), object_key.1.into()).into();
