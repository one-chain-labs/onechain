...
pub async fn get_transaction(
    ...
    Ok(GetTransactionResponse::new(transaction_to_response(transaction, &read_mask)?))
}
...
pub async fn batch_get_transactions(
    ...
    let response: HashMap<_, _> =
        client.get_transactions(&digests).await?.into_iter().map(|tx| (*tx.transaction.digest(), tx)).collect();
