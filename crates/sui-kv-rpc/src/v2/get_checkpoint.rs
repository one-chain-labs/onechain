...
use sui_rpc_api::{proto::google::rpc::bad_request::FieldViolation, CheckpointNotFoundError, ErrorReason, RpcError};
...
pub async fn get_checkpoint(
    ...
    let checkpoint =
        match request.checkpoint_id {
            Some(CheckpointId::Digest(digest)) => {
                ...
                client.get_checkpoint_by_digest(digest).await?.ok_or(CheckpointNotFoundError::digest(digest.into()))?
            }
    ...
    if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
        message.merge(sui_sdk_types::CheckpointContents::try_from(checkpoint.contents)?, &read_mask);
    }

    if (read_mask.contains(Checkpoint::TRANSACTIONS_FIELD) || read_mask.contains(Checkpoint::OBJECTS_FIELD))
        && let Some(url) = checkpoint_bucket
    {
        let client = create_remote_store_client(url, vec![], 60)?;
        let (checkpoint_data, _) = CheckpointReader::fetch_from_object_store(&client, sequence_number).await?;
        let checkpoint =
            sui_types::full_checkpoint_content::Checkpoint::from(std::sync::Arc::into_inner(checkpoint_data).unwrap());
