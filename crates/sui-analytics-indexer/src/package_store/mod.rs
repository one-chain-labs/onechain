...
impl LazyPackageCache {
    ...
    pub fn initialize_or_get_cache(&mut self) -> Arc<PackageCache> {
        ...
        self.cache.as_ref().unwrap().get_or_init(|| (self.constructor)()).clone()
    }
...
impl From<Error> for ResolverError {
    fn from(e: Error) -> Self {
        ResolverError::Store { store: STORE, error: e.to_string() }
    }
}
...
impl LocalDBPackageStore {
    pub fn new(path: &Path, rest_url: &str) -> Self {
        Self { tables: PackageStoreTables::new(path), client: Client::new(rest_url).expect("invalid REST URL") }
    }
    ...
    where
        I: IntoIterator<Item = &'a Object>,
    {
        let filtered = objects.into_iter().filter(|o| o.data.try_as_package().is_some());
    ...
    async fn get(&self, id: AccountAddress) -> StdResult<Object, Error> {
        ...
        let o = self.client.clone().get_object(ObjectID::from(id)).await.map_err(|_| Error::PackageNotFound(id))?;
        self.update(&o)?;
        Ok(o)
    }
    ...
    pub async fn get_original_package_id(&self, id: AccountAddress) -> StdResult<ObjectID, Error> {
        ...
        let Data::Package(p) = &o.data else {
            return Err(Error::TypedStore(TypedStoreError::SerializationError("not a package".into())));
        };
...
impl PackageCache {
    pub fn new(path: &Path, rest_url: &str) -> Self {
        ...
        Self {
            ...
            base_store,
            global_cache,
            epochs: Arc::new(Mutex::new(LruCache::new(NonZeroUsize::new(MAX_EPOCH_CACHES).unwrap()))),
