...
impl<S: Serialize + ParquetSchema> AnalyticsWriter<S> for ParquetWriter {
            ...
            if let Some(first_row) = row_iter.peek() {
                for col_idx in 0 .. S::schema().len() {
                    ...
                    self.builders.push(match value {
                        ParquetValue::U64(_) | ParquetValue::OptionU64(_) => ColumnBuilder::U64(UInt64Builder::new()),
                        ParquetValue::I64(_) => ColumnBuilder::I64(Int64Builder::new()),
                        ParquetValue::Bool(_) => ColumnBuilder::Bool(BooleanBuilder::new()),
                        ParquetValue::Str(_) | ParquetValue::OptionStr(_) => ColumnBuilder::Str(StrBuilder::new()),
                    });
                }
            }
        }
    ...
    fn flush(&mut self, end_checkpoint_seq_num: u64) -> Result<bool> {
        ...
        if self.builders.is_empty() || self.builders.iter_mut().all(|b| b.as_any_builder().is_empty()) {
            return Ok(false);
        }
        ...
        let arrays: Vec<ArrayRef> = std::mem::take(&mut self.builders).into_iter().map(|b| b.finish()).collect();

        let batch = RecordBatch::try_from_iter(S::schema().iter().zip(arrays))?;

        let propertiess = WriterProperties::builder().set_compression(Compression::SNAPPY).build();
