...
use handlers::{package_bcs_handler::PackageBCSHandler, transaction_bcs_handler::TransactionBCSHandler};
...
pub static TRANSACTION_CONCURRENCY_LIMIT: Lazy<usize> = Lazy::new(|| {
    let async_transactions_opt = std::env::var(TRANSACTION_CONCURRENCY_LIMIT_VAR_NAME).ok().and_then(|s| s.parse().ok());
...
impl JobConfig {
    pub async fn create_checkpoint_processors(
        ...
        let lazy_package_cache =
            Arc::new(Mutex::new(LazyPackageCache::new(self.package_cache_path.clone(), self.rest_url.clone())));
        ...
        let package_cache = lazy_package_cache.lock().unwrap().get_cache_if_initialized();
...
impl TaskConfig {
    pub fn remote_store_path_prefix(&self) -> Result<Option<Path>> {
        self.remote_store_path_prefix.as_ref().map(|pb| Ok(Path::from(pb.as_str()))).transpose()
    }
}
...
impl TaskContext {
    ...
    pub async fn create_analytics_processor(self) -> Result<Processor> {
        match &self.config.file_type {
            FileType::Checkpoint => self.create_processor_for_handler(Box::new(CheckpointHandler::new())).await,
            FileType::Object => {
                let package_id_filter = self.config.package_id_filter.clone();
                let package_cache = self.lazy_package_cache.lock().unwrap().initialize_or_get_cache();
            ...
            FileType::Transaction => self.create_processor_for_handler(Box::new(TransactionHandler::new())).await,
            FileType::TransactionBCS => self.create_processor_for_handler(Box::new(TransactionBCSHandler::new())).await,
            FileType::Event => {
                let package_cache = self.lazy_package_cache.lock().unwrap().initialize_or_get_cache();
                self.create_processor_for_handler(Box::new(EventHandler::new(package_cache))).await
            }
            FileType::TransactionObjects => {
                self.create_processor_for_handler(Box::new(TransactionObjectsHandler::new())).await
            }
            FileType::MoveCall => self.create_processor_for_handler(Box::new(MoveCallHandler::new())).await,
            FileType::MovePackage => self.create_processor_for_handler(Box::new(PackageHandler::new())).await,
            FileType::MovePackageBCS => self.create_processor_for_handler(Box::new(PackageBCSHandler::new())).await,
            FileType::DynamicField => {
                let package_cache = self.lazy_package_cache.lock().unwrap().initialize_or_get_cache();
                self.create_processor_for_handler(Box::new(DynamicFieldHandler::new(package_cache))).await
            }
            FileType::WrappedObject => {
                let package_cache = self.lazy_package_cache.lock().unwrap().initialize_or_get_cache();
                let metrics = self.metrics.clone();
                self.create_processor_for_handler(Box::new(WrappedObjectHandler::new(package_cache, metrics))).await
            }
        }
    }
    ...
    async fn create_processor_for_handler<T: Serialize + Clone + ParquetSchema + Send + Sync + 'static>(
        ...
        Processor::new::<T>(handler, writer, max_checkpoint_reader, starting_checkpoint_seq_num, self).await
    }
    ...
    async fn get_starting_checkpoint_seq_num(&self) -> Result<u64> {
        ...
        Ok(self.config.starting_checkpoint_seq_num.map_or(remote_latest, |start| start.max(remote_latest)))
    }
    ...
    fn make_writer<S: Serialize + ParquetSchema>(
        ...
        Ok(match self.config.file_format {
            FileFormat::CSV => {
                Box::new(CSVWriter::new(self.checkpoint_dir_path(), self.config.file_type, starting_checkpoint_seq_num)?)
            }
        ...
        let res: Box<dyn MaxCheckpointReader> = if self.config.report_bq_max_table_checkpoint {
            Box::new(
                BQMaxCheckpointReader::new(
                    self.job_config.bq_service_account_key_file.as_ref().ok_or(anyhow!("Missing gcp key file"))?,
                    self.job_config.bq_project_id.as_ref().ok_or(anyhow!("Missing big query project id"))?,
                    self.job_config.bq_dataset_id.as_ref().ok_or(anyhow!("Missing big query dataset id"))?,
                    self.config.bq_table_id.as_ref().ok_or(anyhow!("Missing big query table id"))?,
                    self.config.bq_checkpoint_col_id.as_ref().ok_or(anyhow!("Missing big query checkpoint col id"))?,
                )
                .await?,
            )
            ...
            Box::new(
                SnowflakeMaxCheckpointReader::new(
                    self.job_config.sf_account_identifier.as_ref().ok_or(anyhow!("Missing sf account identifier"))?,
                    self.job_config.sf_warehouse.as_ref().ok_or(anyhow!("Missing sf warehouse"))?,
                    self.job_config.sf_database.as_ref().ok_or(anyhow!("Missing sf database"))?,
                    self.job_config.sf_schema.as_ref().ok_or(anyhow!("Missing sf schema"))?,
                    self.job_config.sf_username.as_ref().ok_or(anyhow!("Missing sf username"))?,
                    self.job_config.sf_role.as_ref().ok_or(anyhow!("Missing sf role"))?,
                    &load_password(self.job_config.sf_password_file.as_ref().ok_or(anyhow!("Missing sf password"))?)?,
                    self.config.sf_table_id.as_ref().ok_or(anyhow!("Missing sf table id"))?,
                    self.config.sf_checkpoint_col_id.as_ref().ok_or(anyhow!("Missing sf checkpoint col id"))?,
                )
                .await?,
            )
        } else {
...
#[derive(Copy, Clone, Debug, Eq, PartialEq, Serialize, Deserialize, TryFromPrimitive, IntoPrimitive, EnumIter)]
#[repr(u8)]
pub enum FileType {
    Checkpoint = 0,
    Object,
...
impl Processor {
    pub async fn new<S: Serialize + ParquetSchema + Send + Sync + 'static>(
        ...
        let processor = Box::new(
            AnalyticsProcessor::new(handler, writer, max_checkpoint_reader, starting_checkpoint_seq_num, task).await?,
        );

        Ok(Processor { processor, starting_checkpoint_seq_num, task_name, file_type })
    }
