...
async fn main() -> Result<()> {
    ...
    let mut min_watermark = processors.iter().peekable().peek().map(|p| p.starting_checkpoint_seq_num).unwrap_or(0);
    for processor in processors.iter() {
        let watermark = processor.last_committed_checkpoint().map(|seq_num| seq_num + 1).unwrap_or(0);
    ...
    let mut executor = IndexerExecutor::new(progress_store, num_workers, DataIngestionMetrics::new(&registry));
    if let Some(package_cache) = maybe_package_cache {
        let worker = PackageCacheWorker::new(package_cache);
        executor.register(WorkerPool::new(worker, PACKAGE_CACHE_WORKER_NAME.to_string(), 1)).await?;
    }
    ...
    let reader_options = ReaderOptions { batch_size, data_limit, timeout_secs, ..Default::default() };
