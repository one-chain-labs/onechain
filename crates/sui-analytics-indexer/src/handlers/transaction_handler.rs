...
use crate::{
    handlers::{process_transactions, AnalyticsHandler, TransactionProcessor},
    tables::TransactionEntry,
    FileType,
};
...
impl TransactionProcessor<TransactionEntry> for TransactionHandler {
    async fn process_transaction(
        ...
        let packages: BTreeSet<_> =
            move_calls_vec.iter().map(|(package, _, _)| package.to_canonical_string(/* with_prefix */ false)).collect();
        let packages = packages.iter().map(|s| s.as_str()).collect::<Vec<_>>().join("-");
        let transaction_digest = transaction.digest().base58_encode();
        let events_digest = checkpoint_transaction.events.as_ref().map(|events| events.digest().base58_encode());

        let transaction_position = *transaction_positions
            .get(transaction.digest())
            .expect("Expect transaction to exist in checkpoint_contents.") as u64;

        let transaction_data_bcs_length = bcs::to_bytes(&txn_data).unwrap().len() as u64;
        let effects_bcs_length = bcs::to_bytes(&checkpoint_transaction.effects).unwrap().len() as u64;
        ...
        let signatures_bcs_length =
            bcs::to_bytes(&checkpoint_transaction.transaction.data().tx_signatures()).unwrap().len() as u64;
        ...
        let entry = TransactionEntry {
            ...
            input: txn_data.input_objects().expect("Input objects must be valid").len() as u64,
            ...
            deleted: (effects.deleted().len() + effects.unwrapped_then_deleted().len() + effects.wrapped().len()) as u64,
            transfers,
            split_coins,
            merge_coins,
            publish,
...
fn compute_transaction_positions(checkpoint_contents: &CheckpointContents) -> HashMap<TransactionDigest, usize> {
...
mod tests {
    ...
    use crate::handlers::{transaction_handler::TransactionHandler, AnalyticsHandler};
    ...
    pub async fn test_transaction_handler() -> anyhow::Result<()> {
        ...
        let checkpoint_data: sui_types::full_checkpoint_content::CheckpointData = sim
            .get_checkpoint_data(
                checkpoint.clone(),
                sim.get_checkpoint_contents_by_digest(&checkpoint.content_digest).unwrap(),
            )?
            .into();
        let txn_handler = TransactionHandler::new();
        let transaction_entries: Vec<_> = txn_handler.process_checkpoint(&Arc::new(checkpoint_data)).await?.collect();
        ...
        assert_eq!(
            ...
            bcs::to_bytes(&transaction.transaction_data()).unwrap().len() as u64
        );
