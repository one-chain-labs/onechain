...
use sui_types::{base_types::ObjectID, full_checkpoint_content::CheckpointData, object::Object, TypeTag};
...
impl ObjectHandler {
    pub fn new(package_cache: Arc<PackageCache>, package_filter: &Option<String>, metrics: AnalyticsMetrics) -> Self {
        Self {
    ...
    async fn check_type_hierarchy(&self, type_tag: &TypeTag, original_package_id: ObjectID) -> Result<bool> {
    ...
    async fn process_object(
        ...
        {
            match get_move_struct(&tag, contents, &self.package_cache.resolver_for_epoch(epoch)).await {
                Ok(move_struct) => Some(move_struct),
                Err(err)
                    if err
                        .downcast_ref::<sui_types::object::bounded_visitor::Error>()
                        .filter(|e| matches!(e, sui_types::object::bounded_visitor::Error::OutOfBudget))
                        .is_some() =>
                {
                    self.metrics.total_too_large_to_deserialize.with_label_values(&[NAME]).inc();
                    tracing::warn!("Skipping struct with type {} because it was too large.", tag);
                    None
                }
        ...
        let is_match = if let Some(package_id) = self.package_filter {
            if let Some(object_type) = object_type {
                let original_package_id = self.package_cache.get_original_package_id(package_id.into()).await?;

                // Check if any type parameter matches the package filter
                let type_tag: TypeTag = object_type.clone().into();
                self.check_type_hierarchy(&type_tag, original_package_id).await?
            } else {
                false
            }
        } else {
...
impl TransactionProcessor<ObjectEntry> for ObjectHandler {
    async fn process_transaction(
        ...
        let mut vec = Vec::new();
        for object in checkpoint_transaction.output_objects.iter() {
            if let Some(object_entry) =
                self.process_object(epoch, checkpoint, timestamp_ms, object, &object_status_tracker).await?
            {
...
mod tests {
    ...
    async fn test_check_type_hierarchy() {
        ...
        assert!(handler.check_type_hierarchy(&type_tag, ObjectID::from_hex_literal("0xabc").unwrap()).await.unwrap());
