    ...
    async fn process_object(
        ...
        let is_match = if let Some(package_id) = self.package_filter {
            if let Some(object_type) = object_type {
                let original_package_id = state.package_store.get_original_package_id(package_id.into()).await?;

                // Check if any type parameter matches the package filter
                let type_tag: TypeTag = object_type.clone().into();
                self.check_type_hierarchy(&type_tag, original_package_id, state).await?
            } else {
                false
            }
        } else {
...
mod tests {
    ...
    async fn test_check_type_hierarchy() {
        ...
        let handler = ObjectHandler::new(temp_dir.path(), "http://localhost:9000", &Some("0xabc".to_string()));
        let mut state = handler.state.lock().await;

        // 1. Direct match
        let type_tag = create_struct_tag("0xabc", "module", "Type", vec![]);
        assert!(handler
            .check_type_hierarchy(&type_tag, ObjectID::from_hex_literal("0xabc").unwrap(), &mut state)
            .await
            .unwrap());
