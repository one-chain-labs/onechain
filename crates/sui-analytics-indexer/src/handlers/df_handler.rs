...
impl DynamicFieldHandler {
    ...
    async fn process_dynamic_field(
        ...
        let layout = state.resolver.type_layout(move_object.type_().clone().into()).await?;
        ...
        let name_value = BoundedVisitor::deserialize_value(field.name_bytes, field.name_layout).tap_err(|e| {
            warn!("{e}");
        })?;
        ...
        let entry = match type_ {
            ...
            DynamicFieldType::DynamicObject => {
                let object = all_written_objects.get(&object_id).ok_or(IndexerError::UncategorizedError(
    ...
    async fn process_transaction(
        ...
        let all_objects: HashMap<_, _> =
            checkpoint_transaction.output_objects.iter().map(|x| (x.id(), x.clone())).collect();
        for object in checkpoint_transaction.output_objects.iter() {
            self.process_dynamic_field(epoch, checkpoint, timestamp_ms, object, &all_objects, state).await?;
        }
        Ok(())
    }
}
