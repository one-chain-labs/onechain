...
async fn high_low_prices_24h(
    ...
    let results: Vec<(String, Option<i64>, Option<i64>)> = schema::order_fills::table
        ...
        .select((schema::order_fills::pool_id, max(schema::order_fills::price), min(schema::order_fills::price)))
        .load(connection)
        .await?;
...
async fn price_change_24h(
    ...
    for (pool_name, (pool_id, (base_decimals, quote_decimals))) in pool_metadata.iter() {
        ...
        let earliest_trade_24h = schema::order_fills::table
            .filter(schema::order_fills::checkpoint_timestamp_ms.between(timestamp_48h_ago, timestamp_24h_ago))
...
async fn order_updates(
    ...
    let limit = params.get("limit").and_then(|v| v.parse::<i64>().ok()).unwrap_or(1);
        ...
        .map(
            |(
                ...
                HashMap::from([
                    ...
                    ("original_quantity".to_string(), Value::from(original_quantity as f64 / base_factor as f64)),
                    ("remaining_quantity".to_string(), Value::from(quantity as f64 / base_factor as f64)),
                    ("filled_quantity".to_string(), Value::from(filled_quantity as f64 / base_factor as f64)),
