...
impl SubscriptionService {
    pub fn build(registry: &prometheus::Registry) -> (mpsc::Sender<CheckpointData>, SubscriptionServiceHandle) {
        ...
        tokio::spawn(Self { checkpoint_mailbox, mailbox, subscribers: Vec::new(), metrics }.start());
    ...
    fn handle_message(&mut self, request: SubscriptionRequest) {
        ...
        if self.subscribers.len() >= MAX_SUBSCRIBERS {
            trace!("failed to register new subscriber: hit maximum number of subscribers {}", MAX_SUBSCRIBERS);
            return;
        }
