...
impl TryFrom<&BalanceChange> for sui_sdk_types::BalanceChange {
    ...
    fn try_from(value: &BalanceChange) -> Result<Self, Self::Error> {
        Ok(Self {
            address: value.address().parse().map_err(TryFromProtoError::from_error)?,
            coin_type: value.coin_type().parse().map_err(TryFromProtoError::from_error)?,
            amount: value.amount().parse().map_err(TryFromProtoError::from_error)?,
        })
    }
}
