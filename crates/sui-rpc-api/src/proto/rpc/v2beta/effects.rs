...
impl TransactionEffects {
    pub const AUXILIARY_DATA_DIGEST_FIELD: &'static MessageField = &MessageField::new("auxiliary_data_digest");
    ...
    pub const TRANSACTION_DIGEST_FIELD: &'static MessageField = &MessageField::new("transaction_digest");
    pub const UNCHANGED_SHARED_OBJECTS_FIELD: &'static MessageField = &MessageField::new("unchanged_shared_objects");
...
impl MessageMerge<&sui_sdk_types::TransactionEffects> for TransactionEffects {
    fn merge(&mut self, source: &sui_sdk_types::TransactionEffects, mask: &crate::field_mask::FieldMaskTree) {
...
impl MessageMerge<&sui_sdk_types::TransactionEffectsV1> for TransactionEffects {
    fn merge(
        ...
        use super::{ChangedObject, UnchangedSharedObject};
            ...
            for object in shared_objects {
                ...
                if let Some(changed_object) = changed_objects.iter_mut().find(|object| object.object_id() == object_id) {
                    ...
                    let unchanged_shared_object = UnchangedSharedObject {
                        kind: Some(super::unchanged_shared_object::UnchangedSharedObjectKind::ReadOnlyRoot.into()),
...
impl MessageMerge<&sui_sdk_types::TransactionEffectsV2> for TransactionEffects {
    fn merge(
        ...
        if mask.contains(Self::GAS_OBJECT_FIELD.name) {
            self.gas_object =
                gas_object_index.map(|index| changed_objects.get(index as usize).cloned().map(Into::into)).flatten();
        }
        ...
        if mask.contains(Self::UNCHANGED_SHARED_OBJECTS_FIELD.name) {
            self.unchanged_shared_objects = unchanged_shared_objects.clone().into_iter().map(Into::into).collect();
        }
...
impl From<sui_sdk_types::ChangedObject> for super::ChangedObject {
    fn from(value: sui_sdk_types::ChangedObject) -> Self {
        use super::changed_object::{InputObjectState, OutputObjectState};

        let mut message = Self { object_id: Some(value.object_id.to_string()), ..Default::default() };

        // Input State
        let input_state = match value.input_state {
            sui_sdk_types::ObjectIn::NotExist => InputObjectState::DoesNotExist,
            sui_sdk_types::ObjectIn::Exist { version, digest, owner } => {
...
impl TryFrom<&super::ChangedObject> for sui_sdk_types::ChangedObject {
    ...
    fn try_from(value: &super::ChangedObject) -> Result<Self, Self::Error> {
        ...
        let input_state = match value.input_state() {
            InputObjectState::Unknown => return Err(TryFromProtoError::from_error("unknown InputObjectState")),
            InputObjectState::DoesNotExist => sui_sdk_types::ObjectIn::NotExist,
            InputObjectState::Exists => sui_sdk_types::ObjectIn::Exist {
                version: value.input_version.ok_or_else(|| TryFromProtoError::missing("version"))?,
                digest: value
                    .input_digest
                    .as_ref()
                    ...
                    .parse()
                    .map_err(TryFromProtoError::from_error)?,
                owner: value.input_owner.as_ref().ok_or_else(|| TryFromProtoError::missing("owner"))?.try_into()?,
            },
        };

        let output_state = match value.output_state() {
            OutputObjectState::Unknown => return Err(TryFromProtoError::from_error("unknown OutputObjectState")),
            ...
            OutputObjectState::ObjectWrite => sui_sdk_types::ObjectOut::ObjectWrite {
                ...
                owner: value.output_owner.as_ref().ok_or_else(|| TryFromProtoError::missing("owner"))?.try_into()?,
            },
            OutputObjectState::PackageWrite => sui_sdk_types::ObjectOut::PackageWrite {
                version: value.output_version.ok_or_else(|| TryFromProtoError::missing("version"))?,
                digest: value
                    .output_digest
                    .as_ref()
                    .ok_or_else(|| TryFromProtoError::missing("digest"))?
...
impl TryFrom<super::changed_object::IdOperation> for sui_sdk_types::IdOperation {
    ...
    fn try_from(value: super::changed_object::IdOperation) -> Result<Self, Self::Error> {
        ...
        match value {
            IdOperation::Unknown => return Err(TryFromProtoError::from_error("unknown IdOperation")),
...
impl TryFrom<&super::UnchangedSharedObject> for sui_sdk_types::UnchangedSharedObject {
    ...
    fn try_from(value: &super::UnchangedSharedObject) -> Result<Self, Self::Error> {
        ...
        let kind = match value.kind() {
            UnchangedSharedObjectKind::Unknown => return Err(TryFromProtoError::from_error("unknown InputKind")),
