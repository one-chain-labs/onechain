...
impl Bcs {
    pub fn serialize<T: serde::Serialize>(value: &T) -> Result<Self, bcs::Error> {
        bcs::to_bytes(value).map(|bcs| Self { name: None, value: Some(bcs.into()) })
    }
...
impl From<Vec<u8>> for Bcs {
    fn from(value: Vec<u8>) -> Self {
        Self { name: None, value: Some(value.into()) }
    }
}

impl From<&Bcs> for Vec<u8> {
    fn from(value: &Bcs) -> Self {
        value.value.as_ref().map(|bytes| bytes.to_vec()).unwrap_or_default()
    }
}
