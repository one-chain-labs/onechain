...
impl From<sui_sdk_types::ExecutionStatus> for super::ExecutionStatus {
    fn from(value: sui_sdk_types::ExecutionStatus) -> Self {
        match value {
            sui_sdk_types::ExecutionStatus::Success => Self { success: Some(true), error: None },
            sui_sdk_types::ExecutionStatus::Failure { error, command } => {
                let mut error = super::ExecutionError::from(error);
                error.command = command;
                Self { success: Some(false), error: Some(error) }
            }
        }
    }
}
...
impl TryFrom<&super::ExecutionStatus> for sui_sdk_types::ExecutionStatus {
    ...
    fn try_from(value: &super::ExecutionStatus) -> Result<Self, Self::Error> {
        ...
        match (success, &value.error) {
            ...
            (false, Some(error)) => Self::Failure { error: error.try_into()?, command: error.command },
    ...
    fn from(value: sui_sdk_types::ExecutionError) -> Self {
        ...
        let kind = match value {
            ...
            ObjectTooBig { object_size, max_object_size } => {
                message.size_error = Some(super::SizeError { size: Some(object_size), max_size: Some(max_object_size) });
            ...
            VmVerificationOrDeserializationError => ExecutionErrorKind::VmVerificationOrDeserializationError,
            ...
            TypeArgumentError { type_argument, kind } => {
                message.type_argument_error = Some(super::TypeArgumentError {
                    type_argument: Some(type_argument.into()),
                    kind: Some(super::type_argument_error::TypeArgumentErrorKind::from(kind).into()),
                });
            ...
            EffectsTooLarge { current_size, max_size } => {
                message.size_error = Some(super::SizeError { size: Some(current_size), max_size: Some(max_size) });
                ExecutionErrorKind::EffectsTooLarge
            }
            PublishUpgradeMissingDependency => ExecutionErrorKind::PublishUpgradeMissingDependency,
            PublishUpgradeDependencyDowngrade => ExecutionErrorKind::PublishUpgradeDependencyDowngrade,
            PackageUpgradeError { kind } => {
                message.package_upgrade_error = Some(kind.into());
                ExecutionErrorKind::PackageUpgradeError
            }
            WrittenObjectsTooLarge { object_size, max_object_size } => {
                message.size_error = Some(super::SizeError { size: Some(object_size), max_size: Some(max_object_size) });
            ...
            ExecutionCanceledDueToSharedObjectCongestion { congested_objects } => {
                message.congested_objects = congested_objects.iter().map(ToString::to_string).collect();
            ...
            MoveVectorElemTooBig { value_size, max_scaled_size } => {
                message.size_error = Some(super::SizeError { size: Some(value_size), max_size: Some(max_scaled_size) });

                ExecutionErrorKind::MoveVectorElemTooBig
            }
            MoveRawValueTooBig { value_size, max_scaled_size } => {
                message.size_error = Some(super::SizeError { size: Some(value_size), max_size: Some(max_scaled_size) });
...
impl TryFrom<&super::ExecutionError> for sui_sdk_types::ExecutionError {
    ...
    fn try_from(value: &super::ExecutionError) -> Result<Self, Self::Error> {
        ...
        match value.kind() {
            ...
            ObjectTooBig => {
                let super::SizeError { size, max_size } =
                    value.size_error.as_ref().ok_or_else(|| TryFromProtoError::missing("size_error"))?;
                Self::ObjectTooBig {
            ...
            MovePrimitiveRuntimeError => {
                Self::MovePrimitiveRuntimeError { location: value.location.as_ref().map(TryInto::try_into).transpose()? }
            }
            MoveAbort => Self::MoveAbort {
                location: value.location.as_ref().ok_or_else(|| TryFromProtoError::missing("location"))?.try_into()?,
                code: value.abort_code.ok_or_else(|| TryFromProtoError::missing("abort_code"))?,
            },
            ...
            UnusedValueWithoutDrop => Self::UnusedValueWithoutDrop {
                result: value.index.ok_or_else(|| TryFromProtoError::missing("result"))?.try_into()?,
                subresult: value.subresult.ok_or_else(|| TryFromProtoError::missing("subresult"))?.try_into()?,
            },
            InvalidPublicFunctionReturnType => Self::InvalidPublicFunctionReturnType {
                index: value.index.ok_or_else(|| TryFromProtoError::missing("index"))?.try_into()?,
            },
            ...
            ExecutionCanceledDueToSharedObjectCongestion => Self::ExecutionCanceledDueToSharedObjectCongestion {
            ...
            AddressDeniedForCoin => Self::AddressDeniedForCoin {
                address: value
                    ...
                    .parse()
                    .map_err(TryFromProtoError::from_error)?,
                coin_type: value.coin_type.as_ref().ok_or_else(|| TryFromProtoError::missing("coin_type"))?.to_owned(),
            },
            CoinTypeGlobalPause => Self::CoinTypeGlobalPause {
                coin_type: value.coin_type.as_ref().ok_or_else(|| TryFromProtoError::missing("coin_type"))?.to_owned(),
            },
            ExecutionCanceledDueToRandomnessUnavailable => Self::ExecutionCanceledDueToRandomnessUnavailable,
            ...
            MoveVectorElemTooBig => {
                ...
                Self::MoveVectorElemTooBig {
                    ...
                    max_scaled_size: max_size.ok_or_else(|| TryFromProtoError::missing("max_size"))?,
                }
            }
            MoveRawValueTooBig => {
                let super::SizeError { size, max_size } =
...
impl From<sui_sdk_types::CommandArgumentError> for super::CommandArgumentError {
    fn from(value: sui_sdk_types::CommandArgumentError) -> Self {
        ...
        let kind = match value {
            ...
            InvalidArgumentToPrivateEntryFunction => CommandArgumentErrorKind::InvalidArgumentToPrivateEntryFunction,
            ...
            SharedObjectOperationNotAllowed => CommandArgumentErrorKind::SharedObjectOperationNotAllowed,
...
impl TryFrom<&super::CommandArgumentError> for sui_sdk_types::CommandArgumentError {
    ...
    fn try_from(value: &super::CommandArgumentError) -> Result<Self, Self::Error> {
        ...
        match value.kind() {
            Unknown => return Err(TryFromProtoError::from_error("unknown CommandArgumentErrorKind")),
            ...
            SecondaryIndexOutOfBounds => Self::SecondaryIndexOutOfBounds {
                result: value.index.ok_or_else(|| TryFromProtoError::missing("index"))?.try_into()?,
                subresult: value.subresult.ok_or_else(|| TryFromProtoError::missing("subresult"))?.try_into()?,
            },
...
impl TryFrom<super::type_argument_error::TypeArgumentErrorKind> for sui_sdk_types::TypeArgumentError {
    type Error = TryFromProtoError;

    fn try_from(value: super::type_argument_error::TypeArgumentErrorKind) -> Result<Self, Self::Error> {
        use super::type_argument_error::TypeArgumentErrorKind::*;

        match value {
            Unknown => return Err(TryFromProtoError::from_error("unknown TypeArgumentErrorKind")),
            TypeNotFound => Self::TypeNotFound,
            ConstraintNotSatisfied => Self::ConstraintNotSatisfied,
        }
        .pipe(Ok)
...
impl TryFrom<&super::PackageUpgradeError> for sui_sdk_types::PackageUpgradeError {
    ...
    fn try_from(value: &super::PackageUpgradeError) -> Result<Self, Self::Error> {
        ...
        match value.kind() {
            Unknown => return Err(TryFromProtoError::from_error("unknown PackageUpgradeErrorKind")),
            ...
            UnknownUpgradePolicy => Self::UnknownUpgradePolicy {
                policy: value.policy.ok_or_else(|| TryFromProtoError::missing("policy"))?.try_into()?,
            },
...
impl TryFrom<&super::MoveLocation> for sui_sdk_types::MoveLocation {
    ...
    fn try_from(value: &super::MoveLocation) -> Result<Self, Self::Error> {
        ...
        let function_name =
            value.function_name.as_ref().map(|name| name.parse().map_err(TryFromProtoError::from_error)).transpose()?;
