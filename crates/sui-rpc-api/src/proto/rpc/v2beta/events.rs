...
use super::{Bcs, Event, TransactionEvents};
use crate::{
...
impl MessageMerge<sui_sdk_types::Event> for Event {
    fn merge(&mut self, source: sui_sdk_types::Event, mask: &crate::field_mask::FieldMaskTree) {
        ...
        if mask.contains(Self::CONTENTS_FIELD.name) {
            self.contents = Some(Bcs { name: Some(source.type_.to_string()), value: Some(source.contents.into()) });
        }
    }
}
...
impl MessageMerge<&Event> for Event {
    fn merge(&mut self, source: &Event, mask: &crate::field_mask::FieldMaskTree) {
        let Event { package_id, module, sender, event_type, contents, json } = source;
...
impl TransactionEvents {
    ...
    pub const EVENTS_FIELD: &'static MessageField = &MessageField::new("events").with_message_fields(Event::FIELDS);
}

impl MessageFields for TransactionEvents {
    const FIELDS: &'static [&'static MessageField] = &[Self::BCS_FIELD, Self::DIGEST_FIELD, Self::EVENTS_FIELD];
}
...
impl MessageMerge<sui_sdk_types::TransactionEvents> for TransactionEvents {
    fn merge(&mut self, source: sui_sdk_types::TransactionEvents, mask: &crate::field_mask::FieldMaskTree) {
        ...
        if let Some(events_mask) = mask.subtree(Self::EVENTS_FIELD.name) {
            self.events = source.0.into_iter().map(|event| Event::merge_from(event, &events_mask)).collect();
        }
    }
}
...
impl MessageMerge<&TransactionEvents> for TransactionEvents {
    fn merge(&mut self, source: &TransactionEvents, mask: &crate::field_mask::FieldMaskTree) {
        let TransactionEvents { bcs, digest, events } = source;
        ...
        if let Some(events_mask) = mask.subtree(Self::EVENTS_FIELD.name) {
            self.events = events.iter().map(|event| Event::merge_from(event, &events_mask)).collect();
        }
    }
}
