...
use super::{Bcs, Checkpoint, CheckpointContents, CheckpointSummary, ExecutedTransaction};
use crate::{
    message::{MessageField, MessageFields, MessageMerge, MessageMergeFrom},
...
impl CheckpointSummary {
    const BCS_FIELD: &'static MessageField = &MessageField::new("bcs").with_message_fields(Bcs::FIELDS);
    ...
    const TOTAL_NETWORK_TRANSACTIONS_FIELD: &'static MessageField = &MessageField::new("total_network_transactions");
    const VERSION_SPECIFIC_DATA_FIELD: &'static MessageField = &MessageField::new("version_specific_data");
}
...
impl MessageMerge<sui_sdk_types::CheckpointSummary> for CheckpointSummary {
    fn merge(&mut self, source: sui_sdk_types::CheckpointSummary, mask: &crate::field_mask::FieldMaskTree) {
...
impl TryFrom<&CheckpointSummary> for sui_sdk_types::CheckpointSummary {
    ...
    fn try_from(
        ...
        let previous_digest =
            previous_digest.as_ref().map(|s| s.parse().map_err(TryFromProtoError::from_error)).transpose()?;
    ...
    fn try_from(value: &super::CheckpointCommitment) -> Result<Self, Self::Error> {
        ...
        match value.kind() {
            CheckpointCommitmentKind::Unknown => {
                return Err(TryFromProtoError::from_error("unknown CheckpointCommitmentKind"))
            }
            CheckpointCommitmentKind::EcmhLiveObjectSet => {
                Self::EcmhLiveObjectSet { digest: value.digest().parse().map_err(TryFromProtoError::from_error)? }
            }
        }
        .pipe(Ok)
    }
...
impl MessageFields for CheckpointContents {
    const FIELDS: &'static [&'static MessageField] =
        &[Self::BCS_FIELD, Self::DIGEST_FIELD, Self::VERSION_FIELD, Self::TRANSACTIONS_FIELD];
}
...
impl MessageMerge<sui_sdk_types::CheckpointContents> for CheckpointContents {
    fn merge(&mut self, source: sui_sdk_types::CheckpointContents, mask: &crate::field_mask::FieldMaskTree) {
        if mask.contains(Self::BCS_FIELD.name) {
            let mut bcs = Bcs::serialize(&source).unwrap();
...
impl MessageMerge<&CheckpointContents> for CheckpointContents {
    fn merge(&mut self, source: &CheckpointContents, mask: &crate::field_mask::FieldMaskTree) {
        let CheckpointContents { bcs, digest, version, transactions } = source;
...
impl MessageMerge<sui_sdk_types::ValidatorAggregatedSignature> for Checkpoint {
    fn merge(&mut self, source: sui_sdk_types::ValidatorAggregatedSignature, mask: &crate::field_mask::FieldMaskTree) {
...
impl MessageMerge<&Checkpoint> for Checkpoint {
    fn merge(&mut self, source: &Checkpoint, mask: &crate::field_mask::FieldMaskTree) {
        let Checkpoint { sequence_number, digest, summary, signature, contents, transactions } = source;
        ...
        if let Some(submask) = mask.subtree(Self::SUMMARY_FIELD.name) {
            self.summary = summary.as_ref().map(|summary| CheckpointSummary::merge_from(summary, &submask));
        }
        ...
        if let Some(submask) = mask.subtree(Self::CONTENTS_FIELD.name) {
            self.contents = contents.as_ref().map(|contents| CheckpointContents::merge_from(contents, &submask));
        }

        if let Some(submask) = mask.subtree(Self::TRANSACTIONS_FIELD.name) {
            self.transactions =
                transactions.iter().map(|transaction| ExecutedTransaction::merge_from(transaction, &submask)).collect();
        }
    }
}
