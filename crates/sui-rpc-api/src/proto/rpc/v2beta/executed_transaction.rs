...
use super::{ExecutedTransaction, Object, Transaction, TransactionEffects, TransactionEvents, UserSignature};
use crate::message::{MessageField, MessageFields, MessageMerge, MessageMergeFrom};
...
impl ExecutedTransaction {
    ...
    //.with_message_fields(UserSignature::FIELDS);
    ...
    pub const SIGNATURES_FIELD: &'static MessageField = &MessageField::new("signatures");
...
impl MessageMerge<&ExecutedTransaction> for ExecutedTransaction {
    fn merge(&mut self, source: &ExecutedTransaction, mask: &crate::field_mask::FieldMaskTree) {
        ...
        if let Some(submask) = mask.subtree(Self::EVENTS_FIELD.name) {
            self.events = events.as_ref().map(|events| TransactionEvents::merge_from(events, &submask));
        }
        ...
        if let Some(submask) = mask.subtree(Self::INPUT_OBJECTS_FIELD.name) {
            self.input_objects = input_objects.iter().map(|object| Object::merge_from(object, &submask)).collect();
        }

        if let Some(submask) = mask.subtree(Self::OUTPUT_OBJECTS_FIELD.name) {
            self.output_objects = output_objects.iter().map(|object| Object::merge_from(object, &submask)).collect();
        }
    }
}
