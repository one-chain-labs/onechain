...
use super::{Epoch, ProtocolConfig};
...
impl Epoch {
    ...
    pub const FIRST_CHECKPOINT_FIELD: &'static MessageField = &MessageField::new("first_checkpoint");
    ...
    pub const REFERENCE_GAS_PRICE_FIELD: &'static MessageField = &MessageField::new("reference_gas_price");
...
impl MessageMerge<&Epoch> for Epoch {
    fn merge(&mut self, source: &Epoch, mask: &crate::field_mask::FieldMaskTree) {
        ...
        if let Some(submask) = mask.subtree(Self::PROTOCOL_CONFIG_FIELD.name) {
            self.protocol_config = protocol_config.as_ref().map(|config| ProtocolConfig::merge_from(config, &submask));
        }
    }
}
...
impl ProtocolConfig {
    ...
    pub const PROTOCOL_VERSION_FIELD: &'static MessageField = &MessageField::new("protocol_version");
}

impl MessageFields for ProtocolConfig {
    const FIELDS: &'static [&'static MessageField] =
        &[Self::PROTOCOL_VERSION_FIELD, Self::FEATURE_FLAGS_FIELD, Self::ATTRIBUTES_FIELD];
}
