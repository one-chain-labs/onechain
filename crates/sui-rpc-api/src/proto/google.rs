...
pub mod rpc {
    ...
    impl bad_request::FieldViolation {
        ...
        pub fn nested<T: Into<String>>(mut self, field: T) -> Self {
            ...
            if !self.field.is_empty() {
                write!(&mut field, "{}{}", crate::field_mask::FIELD_SEPARATOR, self.field)
    ...
    impl BadRequest {
        pub fn nested<T: AsRef<str>>(mut self, field: T) -> Self {
            ...
            self.field_violations = self.field_violations.into_iter().map(|violation| violation.nested(field)).collect();
            self
        }
        ...
        pub fn nested_at<T: AsRef<str>>(mut self, field: T, index: usize) -> Self {
            ...
            self.field_violations =
                self.field_violations.into_iter().map(|violation| violation.nested_at(field, index)).collect();
            self
        }
    }
