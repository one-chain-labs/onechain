...
impl TryFrom<&super::ExecutionStatus> for sui_sdk_types::types::ExecutionStatus {
    ...
    fn try_from(value: &super::ExecutionStatus) -> Result<Self, Self::Error> {
        ...
        match (success, &value.status) {
            ...
            (false, Some(super::FailureStatus { command, execution_error })) => Self::Failure {
            ...
            (true, Some(_)) | (false, None) => return Err(TryFromProtoError::from_error("invalid execution status")),
        }
        .pipe(Ok)
    }
}
...
impl From<sui_sdk_types::types::ExecutionError> for super::failure_status::ExecutionError {
    fn from(value: sui_sdk_types::types::ExecutionError) -> Self {
        ...
        match value {
            ...
            ObjectTooBig { object_size, max_object_size } => {
                Self::ObjectTooBig(super::SizeError { size: Some(object_size), max_size: Some(max_object_size) })
            }
            PackageTooBig { object_size, max_object_size } => {
                Self::PackageTooBig(super::SizeError { size: Some(object_size), max_size: Some(max_object_size) })
            }
            ...
            MovePrimitiveRuntimeError { location } => Self::MovePrimitiveRuntimeError(super::MoveError {
                location: location.map(Into::into),
                abort_code: None,
            }),
            MoveAbort { location, code } => {
                Self::MoveAbort(super::MoveError { location: Some(location.into()), abort_code: Some(code) })
            }
            ...
            CommandArgumentError { argument, kind } => Self::CommandArgumentError(super::CommandArgumentError {
                argument: Some(argument.into()),
                kind: Some(kind.into()),
            }),
            TypeArgumentError { type_argument, kind } => Self::TypeArgumentError(super::TypeArgumentError {
                type_argument: Some(type_argument.into()),
                kind: Some(kind.into()),
            }),
            UnusedValueWithoutDrop { result, subresult } => Self::UnusedValueWithoutDrop(super::NestedResult {
                result: Some(result.into()),
                subresult: Some(subresult.into()),
            }),
            InvalidPublicFunctionReturnType { index } => Self::InvalidPublicFunctionReturnType(index.into()),
            InvalidTransferObject => Self::InvalidTransferObject(()),
            EffectsTooLarge { current_size, max_size } => {
                Self::EffectsTooLarge(super::SizeError { size: Some(current_size), max_size: Some(max_size) })
            }
            ...
            PackageUpgradeError { kind } => {
                Self::PackageUpgradeError(super::PackageUpgradeError { kind: Some(kind.into()) })
            }
            WrittenObjectsTooLarge { object_size, max_object_size } => Self::WrittenObjectsTooLarge(super::SizeError {
            ...
            ExecutionCancelledDueToRandomnessUnavailable => Self::ExecutionCancelledDueToRandomnessUnavailable(()),
        }
    }
}
...
impl TryFrom<&super::failure_status::ExecutionError> for sui_sdk_types::types::ExecutionError {
    ...
    fn try_from(value: &super::failure_status::ExecutionError) -> Result<Self, Self::Error> {
        ...
        match value {
            ...
            CircularObjectOwnership(object) => Self::CircularObjectOwnership { object: object.try_into()? },
            InsufficientCoinBalance(()) => Self::InsufficientCoinBalance,
            CoinBalanceOverflow(()) => Self::CoinBalanceOverflow,
            PublishErrorNonZeroAddress(()) => Self::PublishErrorNonZeroAddress,
            SuiMoveVerificationError(()) => Self::SuiMoveVerificationError,
            MovePrimitiveRuntimeError(super::MoveError { location, abort_code: _ }) => {
                Self::MovePrimitiveRuntimeError { location: location.as_ref().map(TryInto::try_into).transpose()? }
            }
            MoveAbort(super::MoveError { location: Some(location), abort_code: Some(abort_code) }) => {
                Self::MoveAbort { location: location.try_into()?, code: *abort_code }
            }
            ...
            CommandArgumentError(super::CommandArgumentError { argument, kind }) => Self::CommandArgumentError {
                argument: argument.ok_or_else(|| TryFromProtoError::missing("argument"))?.try_into()?,
                kind: kind.as_ref().ok_or_else(|| TryFromProtoError::missing("kind"))?.try_into()?,
            },
            TypeArgumentError(super::TypeArgumentError { type_argument, kind }) => Self::TypeArgumentError {
                type_argument: type_argument.ok_or_else(|| TryFromProtoError::missing("type_argument"))?.try_into()?,
                kind: kind.as_ref().ok_or_else(|| TryFromProtoError::missing("kind"))?.try_into()?,
            },
            UnusedValueWithoutDrop(super::NestedResult { result, subresult }) => Self::UnusedValueWithoutDrop {
                result: result.ok_or_else(|| TryFromProtoError::missing("result"))?.try_into()?,
                subresult: subresult.ok_or_else(|| TryFromProtoError::missing("subresult"))?.try_into()?,
            },
            InvalidPublicFunctionReturnType(index) => {
                Self::InvalidPublicFunctionReturnType { index: (*index).try_into()? }
            }
            InvalidTransferObject(()) => Self::InvalidTransferObject,
            ...
            WrittenObjectsTooLarge(super::SizeError { size, max_size }) => Self::WrittenObjectsTooLarge {
                object_size: size.ok_or_else(|| TryFromProtoError::missing("size"))?,
                max_object_size: max_size.ok_or_else(|| TryFromProtoError::missing("max_size"))?,
            },
            CertificateDenied(()) => Self::CertificateDenied,
            ...
            SharedObjectOperationNotAllowed(()) => Self::SharedObjectOperationNotAllowed,
            InputObjectDeleted(()) => Self::InputObjectDeleted,
            ExecutionCancelledDueToSharedObjectCongestion(super::CongestedObjectsError { congested_objects }) => {
                Self::ExecutionCancelledDueToSharedObjectCongestion {
                    congested_objects: congested_objects.iter().map(TryInto::try_into).collect::<Result<_, _>>()?,
                }
            }
            AddressDeniedForCoin(super::AddressDeniedForCoinError { address, coin_type }) => {
                Self::AddressDeniedForCoin {
                    address: address.as_ref().ok_or_else(|| TryFromProtoError::missing("address"))?.try_into()?,
                    coin_type: coin_type.as_ref().ok_or_else(|| TryFromProtoError::missing("coin_type"))?.to_owned(),
                }
            }
            CoinTypeGlobalPause(coin_type) => Self::CoinTypeGlobalPause { coin_type: coin_type.to_owned() },
            ExecutionCancelledDueToRandomnessUnavailable(()) => Self::ExecutionCancelledDueToRandomnessUnavailable,
        }
        .pipe(Ok)
    }
...
impl From<sui_sdk_types::types::CommandArgumentError> for super::command_argument_error::Kind {
    fn from(value: sui_sdk_types::types::CommandArgumentError) -> Self {
        ...
        match value {
            ...
            SecondaryIndexOutOfBounds { result, subresult } => Self::SecondaryIndexOutOfBounds(super::NestedResult {
                result: Some(result.into()),
                subresult: Some(subresult.into()),
            }),
...
impl TryFrom<&super::command_argument_error::Kind> for sui_sdk_types::types::CommandArgumentError {
    ...
    fn try_from(value: &super::command_argument_error::Kind) -> Result<Self, Self::Error> {
        use super::{command_argument_error::Kind::*, NestedResult};
        ...
        match value {
            ...
            InvalidUsageOfPureArgument(()) => Self::InvalidUsageOfPureArgument,
            InvalidArgumentToPrivateEntryFunction(()) => Self::InvalidArgumentToPrivateEntryFunction,
            IndexOutOfBounds(index) => Self::IndexOutOfBounds { index: (*index).try_into()? },
            SecondaryIndexOutOfBounds(NestedResult { result, subresult }) => Self::SecondaryIndexOutOfBounds {
                result: result.ok_or_else(|| TryFromProtoError::missing("result"))?.try_into()?,
                subresult: subresult.ok_or_else(|| TryFromProtoError::missing("subresult"))?.try_into()?,
            },
            InvalidResultArity(result) => Self::InvalidResultArity { result: (*result).try_into()? },
            InvalidGasCoinUsage(()) => Self::InvalidGasCoinUsage,
            InvalidValueUsage(()) => Self::InvalidValueUsage,
            InvalidObjectByValue(()) => Self::InvalidObjectByValue,
            InvalidObjectByMutRef(()) => Self::InvalidObjectByMutRef,
...
impl TryFrom<&super::package_upgrade_error::Kind> for sui_sdk_types::types::PackageUpgradeError {
    ...
    fn try_from(value: &super::package_upgrade_error::Kind) -> Result<Self, Self::Error> {
        ...
        match value {
            UnableToFetchPackage(package_id) => Self::UnableToFetchPackage { package_id: package_id.try_into()? },
            NotAPackage(object_id) => Self::NotAPackage { object_id: object_id.try_into()? },
            IncompatibleUpgrade(()) => Self::IncompatibleUpgrade,
            DigetsDoesNotMatch(digest) => Self::DigestDoesNotMatch { digest: digest.try_into()? },
...
impl TryFrom<&super::MoveLocation> for sui_sdk_types::types::MoveLocation {
    ...
    fn try_from(value: &super::MoveLocation) -> Result<Self, Self::Error> {
        ...
        let function = value.function.ok_or_else(|| TryFromProtoError::missing("function"))?.try_into()?;
        let instruction = value.instruction.ok_or_else(|| TryFromProtoError::missing("instruction"))?.try_into()?;
        let function_name = value.function_name.as_ref().map(TryFrom::try_from).transpose()?;

        Ok(Self { package, module, function, instruction, function_name })
    }
}
