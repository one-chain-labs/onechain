...
impl From<RpcError> for tonic::Status {
    fn from(value: RpcError) -> Self {
        ...
        let status = crate::proto::google::rpc::Status {
            ...
            details: value.details.map(ErrorDetails::into_status_details).unwrap_or_default(),
        };
    ...
    fn from(error: sui_types::quorum_driver_types::QuorumDriverError) -> Self {
        ...
        match error {
            ...
            TxAlreadyFinalizedWithDifferentUserSignatures => {
                RpcError::new(Code::Aborted, "The transaction is already finalized but with different user signatures")
            }
...
impl ErrorDetails {
    ...
    fn into_status_details(self: Box<Self>) -> Vec<prost_types::Any> {
        ...
        if let Some(error_info) = &self.error_info {
            details.push(prost_types::Any::from_msg(error_info).expect("Message encoding cannot fail"));
        }

        if let Some(bad_request) = &self.bad_request {
            details.push(prost_types::Any::from_msg(bad_request).expect("Message encoding cannot fail"));
        }

        if let Some(retry_info) = &self.retry_info {
            details.push(prost_types::Any::from_msg(retry_info).expect("Message encoding cannot fail"));
        }
        details
    }
}
...
impl ObjectNotFoundError {
    ...
    pub fn new_with_version(object_id: sui_sdk_types::ObjectId, version: sui_sdk_types::Version) -> Self {
