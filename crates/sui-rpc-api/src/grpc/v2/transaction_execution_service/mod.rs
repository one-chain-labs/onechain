...
impl TransactionExecutionService for RpcService {
    ...
    async fn simulate_transaction(
        ...
        simulate::simulate_transaction(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}
...
pub async fn execute_transaction(
    ...
    let sui_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        effects: sui_types::quorum_driver_types::FinalizedEffects { effects, finality_info: _ },
        events,
        input_objects,
        output_objects,
        auxiliary_data: _,
    ...
    let executed_transaction = if let Some(mask) = read_mask.subtree(ExecuteTransactionResponse::TRANSACTION_FIELD.name)
    {
        let events = mask
            ...
            .and_then(|mask| events.as_ref().map(|e| TransactionEvents::merge_from(e, &mask)));
        ...
        message.objects = mask.subtree(ExecutedTransaction::path_builder().objects().objects().finish()).map(|mask| {
            let set: std::collections::BTreeMap<_, _> = input_objects
                ...
                .collect();
            ObjectSet::default().with_objects(set.into_values().map(|o| Object::merge_from(o, &mask)).collect())
        });
        Some(message)
    } else {
        None
