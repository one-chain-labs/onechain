...
use sui_types::{
    balance_change::derive_balance_changes,
...
pub fn simulate_transaction(
    ...
    let SimulateTransactionResult { input_objects, output_objects, events, effects, execution_result, mock_gas_id: _ } =
            ...
            submask.subtree(ExecutedTransaction::EFFECTS_FIELD).map(|mask| {
                ...
                if mask.contains(TransactionEffects::CHANGED_OBJECTS_FIELD.name) {
                    for changed_object in effects.changed_objects.iter_mut() {
                        ...
                        if let Some(object) = input_objects.iter().chain(&output_objects).find(|o| o.id() == object_id) {
