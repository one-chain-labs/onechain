...
use crate::{reader::StateReader, ErrorReason, Result, RpcError, RpcService};
...
pub fn simulate_transaction(
    ...
    let read_mask =
        request.read_mask.as_ref().map(FieldMaskTree::from_field_mask).unwrap_or_else(FieldMaskTree::new_wildcard);
    ...
    let mut transaction = match sui_sdk_types::Transaction::try_from(transaction_proto) {
        ...
        _ => resolve::resolve_transaction(service, transaction_proto, reference_gas_price, &protocol_config)?,
    };
    ...
    if request.do_gas_selection() && checks.enabled() {
        ...
        if request.transaction().gas_payment().budget.is_none() && request.transaction().bcs_opt().is_none() {
            ...
            if !simulation_result.effects.status().is_ok() {
                return Err(RpcError::new(
                    ...
                    format!("Budget estimation failed with status: {:?}.", simulation_result.effects.status()),
                ));
            }
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        message.balance_changes = if submask.contains(ExecutedTransaction::BALANCE_CHANGES_FIELD.name) {
            ...
            submask.subtree(ExecutedTransaction::EFFECTS_FIELD).map(|mask| {
                ...
                if submask.contains(TransactionEffects::UNCHANGED_LOADED_RUNTIME_OBJECTS_FIELD) {
                    effects.unchanged_loaded_runtime_objects =
                        unchanged_loaded_runtime_objects.iter().map(Into::into).collect();
                }
                ...
                if mask.contains(TransactionEffects::UNCHANGED_CONSENSUS_OBJECTS_FIELD.name) {
                    for unchanged_consensus_object in effects.unchanged_consensus_objects.iter_mut() {
                        let Ok(object_id) = unchanged_consensus_object.object_id().parse::<ObjectID>() else {
                            continue;
                        };

                        if let Some(object) = objects.iter().find(|o| o.id() == object_id) {
                            unchanged_consensus_object.object_type = Some(match object.struct_tag() {
                                Some(struct_tag) => struct_tag.to_canonical_string(true),
                                None => "package".to_owned(),
                            });
                        }
        ...
        message.objects =
            submask.subtree(ExecutedTransaction::path_builder().objects().objects().finish()).map(|mask| {
                ObjectSet::default().with_objects(objects.iter().map(|o| Object::merge_from(o, &mask)).collect())
            });
    ...
    let outputs = if read_mask.contains(SimulateTransactionResponse::COMMAND_OUTPUTS_FIELD) {
        execution_result
            ...
            .map(|(reference_outputs, return_values)| {
                ...
                message.return_values =
                    return_values.into_iter().map(|(bcs, ty)| to_command_output(service, None, bcs, ty)).collect();
...
fn to_command_output(
    ...
    let json = service.reader.inner().get_type_layout(&ty).ok().flatten().and_then(|layout| {
...
fn estimate_gas_budget_from_gas_cost(
    ...
    let base_estimate_mist = gas_cost_summary.computation_cost.max(if gas_usage < 0 { 0 } else { gas_usage as u64 });
    ...
    let rounded_gas_loading_cost_units = if let Some(step) = protocol_config.gas_rounding_step_as_option() {
    ...
    let estimate_mist = base_estimate_mist.saturating_add(gas_loading_cost_mist).saturating_add(safe_overhead_mist);
