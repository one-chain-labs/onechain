...
pub fn get_balance(service: &RpcService, request: GetBalanceRequest) -> Result<GetBalanceResponse> {
    ...
    let coin_type = request
        ...
        .as_ref()
        .ok_or_else(|| {
            FieldViolation::new("coin_type").with_description("missing coin_type").with_reason(ErrorReason::FieldMissing)
        })?
        .parse::<StructTag>()
        .map_err(|e| {
    ...
    if coin_info.is_none() {
        return Err(RpcError::new(tonic::Code::InvalidArgument, format!("coin type does not exist: {}", coin_type)));
    }

    let balance_info = indexes.get_balance(&owner, &core_coin_type)?.unwrap_or_default(); // Use default (zero) if no balance found
