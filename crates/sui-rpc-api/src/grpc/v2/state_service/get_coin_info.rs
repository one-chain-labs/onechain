...
use sui_types::{
    ...
    coin_registry::{Currency, RegulatedState as CurrencyRegulatedState, SupplyState},
    object::Owner::{AddressOwner, ConsensusAddressOwner, Immutable},
...
fn get_coin_info_from_registry(
    ...
    let regulated_metadata = if !matches!(&currency.regulated, CurrencyRegulatedState::Unknown) {
        ...
        let coin_info = indexes.get_coin_info(core_coin_type).map_err(|e| {
            RpcError::new(tonic::Code::Internal, format!("Failed to get coin info for {}: {}", core_coin_type, e))
        })?;
