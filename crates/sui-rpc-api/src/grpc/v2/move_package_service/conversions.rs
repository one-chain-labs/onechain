...
use move_binary_format::file_format::{
    Ability as MoveAbility,
    AbilitySet as MoveAbilitySet,
    DatatypeTyParameter,
...
pub(crate) fn convert_datatype(
    ...
    let type_parameters = data_def.type_params.iter().map(convert_type_parameter).collect();
    ...
    let (kind, fields, variants) = match &data_def.data {
        ...
        MoveData::Enum(variants) => {
            let proto_variants =
                variants.iter().enumerate().map(|(pos, variant)| convert_variant(pos, variant)).collect();
            (DatatypeKind::Enum, vec![], proto_variants)
        }
    };

    let mut message = DatatypeDescriptor::default();
    message.type_name = Some(format!("{}::{}::{}", package_id.to_canonical_string(true), module_name, datatype_name));
...
pub(crate) fn convert_module(
    ...
    for datatype_name in resolved_module.datatypes(None, None) {
        let data_def = resolved_module
            ...
            .unwrap_or_else(|| panic!("datatype {} does not have a data_def. This shouldn't happen.", datatype_name));
    ...
    for func_name in resolved_module.functions(None, None) {
        let func_def = resolved_module
            ...
            .map_err(convert_error)?
            .unwrap_or_else(|| panic!("function {} does not have a function_def. This shouldn't happen.", func_name));
...
pub(crate) fn convert_function(function_name: &str, func_def: &FunctionDef) -> FunctionDescriptor {
    ...
    let parameters = func_def.parameters.iter().map(convert_open_signature).collect();
    let returns = func_def.return_.iter().map(convert_open_signature).collect();
...
fn convert_open_signature_body(body: &sui_package_resolver::OpenSignatureBody) -> OpenSignatureBody {
    let (type_enum, type_name, type_params, type_param_idx) = match body {
        ...
        sui_package_resolver::OpenSignatureBody::Address => (SignatureType::Address, None, vec![], None),
        ...
        sui_package_resolver::OpenSignatureBody::Datatype(key, args) => {
            let type_name = format!("{}::{}::{}", key.package.to_canonical_string(true), key.module, key.name);
