...
impl NameService for RpcService {
    async fn lookup_name(
        ...
        lookup_name(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
    ...
    async fn reverse_lookup_name(
        ...
        reverse_lookup_name(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}
...
fn name_service_config(service: &RpcService) -> Result<NameServiceConfig> {
    match service.chain_id.chain() {
        ...
        sui_protocol_config::Chain::Unknown => {
            Err(RpcError::new(tonic::Code::Unimplemented, "SuiNS not configured for this network"))
        }
    }
}
...
fn lookup_name(service: &RpcService, request: LookupNameRequest) -> Result<LookupNameResponse> {
    ...
    let domain = request
        .name
        .ok_or_else(|| FieldViolation::new(LookupNameRequest::NAME_FIELD.name).with_reason(ErrorReason::FieldMissing))?
        .parse::<Domain>()
        .map_err(|e| {
    ...
    let name_record =
        NameRecord::try_from(record_object).map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
    ...
    let is_valid = if !name_record.is_leaf_record() {
        ...
        if let Some(parent_object) = service.reader.inner().get_object(&parent_record_id) {
            let parent_name_record =
                NameRecord::try_from(parent_object).map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
    ...
    if is_valid {
        let mut record = sui_rpc::proto::sui::rpc::v2::NameRecord::default();
        ...
        record.expiration_timestamp = Some(timestamp_ms_to_proto(name_record.expiration_timestamp_ms));
        record.target_address = name_record.target_address.map(|address| address.to_string());
        record.data = name_record.data.contents.into_iter().map(|entry| (entry.key, entry.value)).collect();

        let mut response = LookupNameResponse::default();
        response.record = Some(record);
        Ok(response)
    } else {
        Err(RpcError::new(tonic::Code::ResourceExhausted, "name has expired"))
    }
}

#[tracing::instrument(skip(service))]
fn reverse_lookup_name(service: &RpcService, request: ReverseLookupNameRequest) -> Result<ReverseLookupNameResponse> {
    let name_service_config = name_service_config(service)?;

    let address = request
        .address
        .ok_or_else(|| {
            FieldViolation::new(ReverseLookupNameRequest::ADDRESS_FIELD.name).with_reason(ErrorReason::FieldMissing)
        })?
        .parse::<Address>()
        .map_err(|e| {
    ...
    let Some(field_reverse_record_object) = service.reader.inner().get_object(&reverse_record_id) else {
        return Err(RpcError::not_found());
    };

    let domain = field_reverse_record_object
        .to_rust::<sui_types::dynamic_field::Field<Address, Domain>>()
        .ok_or_else(|| RpcError::new(tonic::Code::Internal, format!("Malformed Object {reverse_record_id}")))?
        .value;
