...
impl SignatureVerificationService for RpcService {
    async fn verify_signature(
        ...
        verify_signature(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}

#[tracing::instrument(skip(service))]
fn verify_signature(service: &RpcService, request: VerifySignatureRequest) -> Result<VerifySignatureResponse> {
        ...
        })?
    {
        ...
        let derived_addresses = match &signature {
            sui_sdk_types::UserSignature::Simple(simple_signature) => match simple_signature {
                ...
                _ => return Err(RpcError::new(tonic::Code::Internal, "unknown signature scheme")),
            },
