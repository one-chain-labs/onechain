    ...
    if let Some(submask) = mask.subtree(ExecutedTransaction::EFFECTS_FIELD.name) {
        ...
        if submask.contains(TransactionEffects::UNCHANGED_LOADED_RUNTIME_OBJECTS_FIELD) {
            effects.unchanged_loaded_runtime_objects =
                source.unchanged_loaded_runtime_objects.unwrap_or_default().iter().map(Into::into).collect();
        }
        ...
        message.events = source.events.map(|events| {
                ...
                && event_mask.contains(Event::JSON_FIELD.name)
            {
                for (message, event) in message.events.iter_mut().zip(&events.0) {
                    message.json = struct_tag_sdk_to_core(event.type_.clone()).ok().and_then(|struct_tag| {
                        crate::grpc::v2::render_json(service, &struct_tag, &event.contents).map(Box::new)
                    });
                }
            }
...
pub(crate) fn render_clever_error(service: &RpcService, effects: &mut TransactionEffects) {
    use sui_rpc::proto::sui::rpc::v2::{clever_error, execution_error::ErrorDetails, CleverError, MoveAbort};

    let Some(move_abort) = effects.status.as_mut().and_then(|status| status.error.as_mut()).and_then(|error| {
        match &mut error.error_details {
    ...
    fn render(service: &RpcService, move_abort: &MoveAbort) -> Option<CleverError> {
        ...
        match clever_error.error_info {
            ...
            sui_package_resolver::ErrorConstants::Rendered { identifier, constant } => {
