...
use crate::{error::CheckpointNotFoundError, ErrorReason, RpcError, RpcService};

pub const READ_MASK_DEFAULT: &str = "sequence_number,digest";

#[tracing::instrument(skip(service))]
pub fn get_checkpoint(service: &RpcService, request: GetCheckpointRequest) -> Result<GetCheckpointResponse, RpcError> {
    let read_mask = {
        ...
        || read_mask.contains(Checkpoint::OBJECTS_FIELD.name)
    {
        ...
        if read_mask.contains(Checkpoint::TRANSACTIONS_FIELD.name) || read_mask.contains(Checkpoint::OBJECTS_FIELD.name)
        {
                ...
                checkpoint.transactions = checkpoint_data
                    ...
                    .map(|t| {
                        let balance_changes = submask
                            ...
                            .then(|| {
                                service.reader.get_transaction_info(&t.transaction.digest()).map(|info| {
                        ...
                        if let Some(events_mask) = submask.subtree(ExecutedTransaction::EVENTS_FIELD.name)
                            && let Some(event_mask) = events_mask.subtree(TransactionEvents::EVENTS_FIELD.name)
                            ...
                            && let Some(sdk_events) = &t.events
                        {
                            for (message, event) in events.events.iter_mut().zip(&sdk_events.data) {
                                message.json =
                                    crate::grpc::v2::render_json(service, &event.type_, &event.contents).map(Box::new);
                            }
                        }
