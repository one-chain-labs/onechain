...
use bcs;
use fastcrypto::{hash::Blake2b256, merkle::MerkleTree};
use sui_types::{
...
use crate::{
    grpc::alpha::proof_service_proto::{
        proof_service_server::ProofService,
        GetObjectInclusionProofRequest,
        GetObjectInclusionProofResponse,
        OcsInclusionProof,
    },
    RpcError,
    RpcService,
...
impl ProofService for ProofServiceImpl {
    async fn get_object_inclusion_proof(
        ...
        let response =
            get_object_inclusion_proof_impl(&self.service, request.into_inner()).map_err(tonic::Status::from)?;
...
fn build_ocs_inclusion_proof(
    ...
    let effects_refs: Vec<&_> = checkpoint.transactions.iter().map(|tx| &tx.effects).collect();

    let checkpoint_artifacts = CheckpointArtifacts::from(effects_refs.as_slice());

    let object_states =
        checkpoint_artifacts.object_states().map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;

    let object_ref_from_checkpoint = object_states.get(&object_id).ok_or_else(|| {
        RpcError::new(
            tonic::Code::FailedPrecondition,
            format!("Object {} was not written at checkpoint {}", object_id, checkpoint_seq),
        )
    })?;

    let object_ref = (object_id, object_ref_from_checkpoint.0, object_ref_from_checkpoint.1);

    let leaves: Vec<ObjectRef> = object_states.iter().map(|(id, (seq, digest))| (*id, *seq, *digest)).collect();

    let leaf_index = leaves.iter().position(|r| *r == object_ref).ok_or_else(|| {
        RpcError::new(tonic::Code::Internal, format!("Object {} not found in checkpoint", object_ref.0))
    })?;

    let tree = MerkleTree::<Blake2b256>::build_from_unserialized(leaves.iter())
        .map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;

    let merkle_proof = tree.get_proof(leaf_index).map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;

    let tree_root = Digest::new(tree.root().bytes());

    let merkle_proof_bytes =
        bcs::to_bytes(&merkle_proof).map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
...
fn get_object_inclusion_proof_impl(
    ...
    if !service.config.authenticated_events_indexing() {
        return Err(RpcError::new(tonic::Code::Unimplemented, "Authenticated events indexing is disabled".to_string()));
    }

    let object_id_str =
        request.object_id.ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "missing object_id".to_string()))?;

    if object_id_str.trim().is_empty() {
        return Err(RpcError::new(tonic::Code::InvalidArgument, "object_id cannot be empty".to_string()));
    }

    let object_id = ObjectID::from_str(&object_id_str)
        .map_err(|e| RpcError::new(tonic::Code::InvalidArgument, format!("invalid object_id: {e}")))?;

    let checkpoint_seq = request
        .checkpoint
        .ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "missing checkpoint".to_string()))?;
    ...
    let checkpoint = reader
        ...
        .ok_or_else(|| RpcError::new(tonic::Code::NotFound, format!("checkpoint {} not found", checkpoint_seq)))?;

    let checkpoint_contents = reader.get_checkpoint_contents_by_sequence_number(checkpoint_seq).ok_or_else(|| {
        RpcError::new(tonic::Code::NotFound, format!("checkpoint contents for {} not found", checkpoint_seq))
    })?;
    ...
    let (proto_inclusion_proof, object_ref) = build_ocs_inclusion_proof(&checkpoint_data, object_id, checkpoint_seq)?;

    let object = reader.get_object_by_key(&object_id, object_ref.1).ok_or_else(|| {
        RpcError::new(tonic::Code::NotFound, format!("Object {} not found at version {}", object_id, object_ref.1))
    })?;
    ...
    let object_data_bytes = bcs::to_bytes(&object).map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
