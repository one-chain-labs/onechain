...
use crate::{
    grpc::alpha::event_service_proto::{
        AuthenticatedEvent,
...
fn decode_page_token(page_token: &[u8]) -> Result<PageToken, RpcError> {
    bcs::from_bytes(page_token)
        .map_err(|_| RpcError::new(tonic::Code::InvalidArgument, "invalid page_token".to_string()))
}
...
pub fn list_authenticated_events(
    ...
    let stream_id =
        request.stream_id.ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "missing stream_id".to_string()))?;

    if stream_id.trim().is_empty() {
        return Err(RpcError::new(tonic::Code::InvalidArgument, "stream_id cannot be empty".to_string()));
    }

    let stream_addr = SuiAddress::from_str(&stream_id)
        .map_err(|e| RpcError::new(tonic::Code::InvalidArgument, format!("invalid stream_id: {e}")))?;

    let page_size = request.page_size.map(|s| s.clamp(1, MAX_PAGE_SIZE)).unwrap_or(DEFAULT_PAGE_SIZE);

    let page_token = request.page_token.as_ref().map(|token| decode_page_token(token)).transpose()?;

    if let Some(token) = &page_token
        && token.stream_id != stream_addr
    {
        return Err(RpcError::new(tonic::Code::InvalidArgument, "page_token stream_id mismatch".to_string()));
    }
    ...
    for (i, event_result) in iter.enumerate() {
        ...
        let authenticated_event =
            to_authenticated_event(&stream_id, cp, accumulator_version, transaction_idx, event_idx, &ev);
