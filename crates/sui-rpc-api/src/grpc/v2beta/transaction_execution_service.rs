...
use sui_types::{balance_change::derive_balance_changes, transaction_executor::TransactionExecutor};
use tap::Pipe;
...
impl TransactionExecutionService for RpcService {
    async fn execute_transaction(
        ...
        let executor = self.executor.as_ref().ok_or_else(|| tonic::Status::unimplemented("no transaction executor"))?;

        execute_transaction(executor, request.into_inner()).await.map(tonic::Response::new).map_err(Into::into)
    }
}
...
pub async fn execute_transaction(
    ...
    let signed_transaction =
        sui_sdk_types::SignedTransaction { transaction: transaction.clone(), signatures: signatures.clone() };

    let read_mask = {
        let read_mask =
            request.read_mask.unwrap_or_else(|| FieldMask::from_str(ExecuteTransactionRequest::READ_MASK_DEFAULT));
        read_mask.validate::<ExecuteTransactionResponse>().map_err(|path| {
    ...
    let request = {
        let mask = read_mask.subtree(ExecuteTransactionResponse::TRANSACTION_FIELD.name).unwrap_or_default();
    ...
    let sui_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        effects: sui_types::quorum_driver_types::FinalizedEffects { effects, finality_info },
        events,
        input_objects,
        output_objects,
        auxiliary_data: _,
    ...
    let finality = {
        let finality = match finality_info {
            ...
            sui_types::quorum_driver_types::EffectsFinalityInfo::QuorumExecuted(_) => Finality::QuorumExecuted(()),
        };
        crate::proto::rpc::v2beta::TransactionFinality { finality: Some(finality) }
    };

    let executed_transaction = if let Some(mask) = read_mask.subtree(ExecuteTransactionResponse::TRANSACTION_FIELD.name)
    {
        let events = events.map(sui_sdk_types::TransactionEvents::try_from).transpose()?;
        ...
        let balance_changes = mask
            ...
            .then(|| {
                derive_balance_changes(&effects, &input_objects, &output_objects).into_iter().map(Into::into).collect()
            })
            .unwrap_or_default();

        let input_objects =
            input_objects.into_iter().map(sui_sdk_types::Object::try_from).collect::<Result<Vec<_>, _>>()?;
        let output_objects =
            output_objects.into_iter().map(sui_sdk_types::Object::try_from).collect::<Result<Vec<_>, _>>()?;

        let effects = sui_sdk_types::TransactionEffects::try_from(effects)?;
        let effects = mask.subtree(ExecutedTransaction::EFFECTS_FIELD.name).map(|mask| {
            ...
            if mask.contains(TransactionEffects::CHANGED_OBJECTS_FIELD.name) {
                for changed_object in effects.changed_objects.iter_mut() {
                    ...
                    if let Some(object) =
                        input_objects.iter().chain(&output_objects).find(|o| o.object_id() == object_id)
                    {
                        changed_object.object_type = Some(match object.object_type() {
                            sui_sdk_types::ObjectType::Package => "package".to_owned(),
                            sui_sdk_types::ObjectType::Struct(struct_tag) => struct_tag.to_string(),
                        });
                    }
                }
            }
            ...
            if mask.contains(TransactionEffects::UNCHANGED_SHARED_OBJECTS_FIELD.name) {
                for unchanged_shared_object in effects.unchanged_shared_objects.iter_mut() {
                    let Ok(object_id) = unchanged_shared_object.object_id().parse::<ObjectId>() else {
                        continue;
                    };

                    if let Some(object) = input_objects.iter().find(|o| o.object_id() == object_id) {
                        unchanged_shared_object.object_type = Some(match object.object_type() {
                            sui_sdk_types::ObjectType::Package => "package".to_owned(),
                            sui_sdk_types::ObjectType::Struct(struct_tag) => struct_tag.to_string(),
                        });
                    }
        ...
        Some(ExecutedTransaction {
            digest: mask.contains(ExecutedTransaction::DIGEST_FIELD.name).then(|| transaction.digest().to_string()),
            transaction: mask
            ...
            signatures: mask
                ...
                .map(|mask| signatures.into_iter().map(|s| UserSignature::merge_from(s, &mask)).collect())
                .unwrap_or_default(),
            effects,
            events: mask
            ...
            input_objects: mask
                ...
                .map(|mask| input_objects.into_iter().map(|o| Object::merge_from(o, &mask)).collect())
                .unwrap_or_default(),
            output_objects: mask
                .subtree(ExecutedTransaction::OUTPUT_OBJECTS_FIELD.name)
                .map(|mask| output_objects.into_iter().map(|o| Object::merge_from(o, &mask)).collect())
                .unwrap_or_default(),
        })
    } else {
        None
    ...
    ExecuteTransactionResponse {
        finality: read_mask.contains(ExecuteTransactionResponse::FINALITY_FIELD.name).then_some(finality),
