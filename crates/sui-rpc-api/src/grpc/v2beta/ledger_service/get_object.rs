...
use crate::{
    ...
    proto::{
        ...
        rpc::v2beta::{BatchGetObjectsRequest, BatchGetObjectsResponse, GetObjectRequest, Object},
    },
    ErrorReason,
    RpcError,
    RpcService,
    ...
    let requests = requests
        ...
        .map(|(idx, (object_id, version))| {
            let object_id = object_id
                .as_ref()
                .ok_or_else(|| {
                    FieldViolation::new("object_id").with_reason(ErrorReason::FieldMissing).nested_at("requests", idx)
                })?
                .parse()
                .map_err(|e| {
...
pub fn get_object(
    ...
    let (requests, read_mask) = validate_get_object_requests(vec![(object_id, version)], read_mask)?;
...
fn get_object_impl(
    ...
    let object = if let Some(version) = version {
        ...
        service.reader.get_object(object_id)?.ok_or_else(|| ObjectNotFoundError::new(object_id))?
    };
