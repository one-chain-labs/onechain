...
use crate::{
    ...
    message::{MessageMerge, MessageMergeFrom},
    proto::{
...
pub fn get_checkpoint(service: &RpcService, request: GetCheckpointRequest) -> Result<Checkpoint, RpcError> {
    let read_mask = {
        let read_mask =
    ...
    let sui_sdk_types::SignedCheckpointSummary { checkpoint: summary, signature } =
    ...
    if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) || read_mask.contains(Checkpoint::TRANSACTIONS_FIELD.name) {
        let core_contents = service
            .reader
            .inner()
            .get_checkpoint_contents_by_sequence_number(summary.sequence_number)
            .ok_or(CheckpointNotFoundError::sequence_number(summary.sequence_number))?;

        if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
            checkpoint.merge(sui_sdk_types::CheckpointContents::try_from(core_contents.clone())?, &read_mask);
        }

        if let Some(submask) = read_mask.subtree(Checkpoint::TRANSACTIONS_FIELD.name) {
            let checkpoint_data = service.reader.inner().get_checkpoint_data(verified_summary, core_contents)?;
...
pub(crate) fn checkpoint_data_to_checkpoint_proto(
    ...
    if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
        checkpoint.merge(sui_sdk_types::CheckpointContents::try_from(checkpoint_data.checkpoint_contents)?, read_mask);
    }
    ...
    if let Some(submask) = read_mask.subtree(Checkpoint::TRANSACTIONS_FIELD.name) {
        checkpoint.transactions = checkpoint_data
            .transactions
            .into_iter()
            .map(|t| core_transaction_to_executed_transaction_proto(t, sequence_number, timestamp_ms, &submask))
            .collect::<Result<_, _>>()?;
    }
...
fn core_transaction_to_executed_transaction_proto(
    ...
    let digest = read_mask
        ...
        .then(|| sui_sdk_types::TransactionDigest::from(transaction.digest().to_owned()).to_string());
    ...
    let signatures = read_mask
        ...
        .map(|mask| {
            signatures
                .into_iter()
                .map(|s| sui_sdk_types::UserSignature::try_from(s).map(|s| UserSignature::merge_from(s, &mask)))
    ...
    Ok(ExecutedTransaction {
        ...
        effects,
        events,
        checkpoint: read_mask.contains(ExecutedTransaction::CHECKPOINT_FIELD.name).then_some(checkpoint),
        timestamp: read_mask
