...
use crate::{
    ...
    proto::{
        ...
        rpc::v2beta::{Epoch, GetEpochRequest, ProtocolConfig},
...
pub fn get_epoch(service: &RpcService, request: GetEpochRequest) -> Result<Epoch> {
    let read_mask = {
        let read_mask = request.read_mask.unwrap_or_else(|| FieldMask::from_str(GetEpochRequest::READ_MASK_DEFAULT));
    ...
    let epoch =
        if let Some(epoch) = request.epoch { epoch } else { service.reader.inner().get_latest_checkpoint()?.epoch() };
    ...
    if let Some(epoch_info) =
        service.reader.inner().indexes().and_then(|indexes| indexes.get_epoch_info(epoch).ok().flatten())
    {
        ...
        if let Some(submask) = read_mask.subtree(Epoch::PROTOCOL_CONFIG_FIELD.name) {
            let protocol_config =
                epoch_info.protocol_version.map(|version| service.get_protocol_config(Some(version))).transpose()?;

            message.protocol_config = protocol_config.map(|config| ProtocolConfig::merge_from(config, &submask));
        }
    }

    if read_mask.contains(Epoch::COMMITTEE_FIELD.name) {
        message.committee =
            Some(service.reader.get_committee(epoch).ok_or_else(|| CommitteeNotFoundError::new(epoch))?.into());
    }
