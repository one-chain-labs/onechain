...
use sui_types::{base_types::ObjectID, sui_sdk_types_conversions::struct_tag_sdk_to_core};
...
pub fn get_transaction(service: &RpcService, request: GetTransactionRequest) -> Result<ExecutedTransaction, RpcError> {
    ...
    Ok(transaction_to_response(service, transaction_read, &read_mask))
}
    ...
    let transactions = digests
        ...
        .map(|(idx, digest)| {
            ...
            service
                ...
                .map(|transaction_read| transaction_to_response(service, transaction_read, &read_mask))
        })
        .collect::<Result<_, _>>()?;
...
fn transaction_to_response(
    ...
    if let Some(submask) = mask.subtree(ExecutedTransaction::SIGNATURES_FIELD.name) {
        message.signatures = source.signatures.into_iter().map(|s| UserSignature::merge_from(s, &submask)).collect();
    }
                ...
                if event_mask.contains(Event::JSON_FIELD.name) {
                    for (message, event) in message.events.iter_mut().zip(&events.0) {
                        message.json = struct_tag_sdk_to_core(event.type_.clone())
                            .ok()
                            .and_then(|struct_tag| {
                                let layout = service.reader.inner().get_struct_layout(&struct_tag).ok().flatten()?;
