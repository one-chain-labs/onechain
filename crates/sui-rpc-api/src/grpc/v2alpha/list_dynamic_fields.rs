...
pub fn list_dynamic_fields(
    ...
    let next_page_token = if dynamic_fields.len() > page_size {
        ...
        // gaurenteed to be >= 1.
        dynamic_fields.pop().unwrap().field_id.unwrap().parse::<ObjectId>().unwrap().pipe(encode_page_token).pipe(Some)
    } else {
        None
    };

    Ok(ListDynamicFieldsResponse { dynamic_fields, next_page_token })
}
