...
use sui_sdk_types::{Jwk, JwkId};
use tap::Pipe;

use crate::{
    proto::{
        google::rpc::bad_request::FieldViolation,
        rpc::v2alpha::{VerifySignatureRequest, VerifySignatureResponse},
    },
    ErrorReason,
    Result,
    RpcService,
...
pub fn verify_signature(service: &RpcService, request: VerifySignatureRequest) -> Result<VerifySignatureResponse> {
    ...
    let signing_digest = {
        let bcs =
            request.message.ok_or_else(|| FieldViolation::new("message").with_reason(ErrorReason::FieldMissing))?;

        match bcs.name() {
            "TransactionData" => bcs.deserialize::<sui_sdk_types::Transaction>()?.signing_digest(),
            "PersonalMessage" => {
                bcs.deserialize::<&[u8]>().map(|slice| sui_sdk_types::PersonalMessage(slice.into()))?.signing_digest()
            }
            _ => {
                if let Ok(personal_message) =
                    bcs.deserialize::<&[u8]>().map(|slice| sui_sdk_types::PersonalMessage(slice.into()))
                {
    ...
    if let Some(address) =
        request.address.map(|address| address.parse::<sui_sdk_types::Address>()).transpose().map_err(|e| {
        ...
        })?
    {
        ...
        let derived_addresses = match &signature {
            sui_sdk_types::UserSignature::Simple(simple_signature) => match simple_signature {
                sui_sdk_types::SimpleSignature::Ed25519 { public_key, .. } => [Some(public_key.derive_address()), None],
            ...
            sui_sdk_types::UserSignature::Multisig(multisig) => [Some(multisig.committee().derive_address()), None],
            sui_sdk_types::UserSignature::ZkLogin(z) => {
                ...
                [Some(id.derive_address_padded()), Some(id.derive_address_unpadded())]
            }
            sui_sdk_types::UserSignature::Passkey(p) => [Some(p.public_key().derive_address()), None],
        };
        ...
        if !derived_addresses.into_iter().flatten().any(|derived_address| derived_address == address) {
    ...
    *zklogin_verifier.jwks_mut() = jwks;
    ...
    match verifier.verify(&signing_digest, &signature) {
        Ok(()) => VerifySignatureResponse { is_valid: Some(true), reason: None },
        Err(error) => VerifySignatureResponse { is_valid: Some(false), reason: Some(error.to_string()) },
    }
    .pipe(Ok)
}
