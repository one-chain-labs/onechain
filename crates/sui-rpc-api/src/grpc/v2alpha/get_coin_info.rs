...
use crate::{
    proto::rpc::v2alpha::{CoinMetadata, CoinTreasury, GetCoinInfoRequest, GetCoinInfoResponse, RegulatedCoinMetadata},
    Result,
    RpcError,
    RpcService,
};
...
const SUI_COIN_TREASURY: CoinTreasury =
    CoinTreasury { id: None, total_supply: Some(sui_types::gas_coin::TOTAL_SUPPLY_MIST) };
...
pub fn get_coin_info(service: &RpcService, request: GetCoinInfoRequest) -> Result<GetCoinInfoResponse> {
    ...
    let sui_types::storage::CoinInfo { coin_metadata_object_id, treasury_object_id, regulated_coin_metadata_object_id } =
    ...
    let regulated_metadata = if let Some(regulated_coin_metadata_object_id) = regulated_coin_metadata_object_id {
        service
            .reader
            .inner()
            ...
            .map(|value| RegulatedCoinMetadata {
                id: Some(ObjectId::from(value.id.id.bytes).to_string()),
                coin_metadata_object: Some(ObjectId::from(value.coin_metadata_object.bytes).to_string()),
    ...
    Ok(GetCoinInfoResponse { coin_type: Some(coin_type.to_string()), metadata, treasury, regulated_metadata })
}
