...
use sui_types::{
    ...
    transaction_executor::{SimulateTransactionResult, TransactionExecutor},
};
use tap::Pipe;
...
use crate::{
    ...
    proto::{
        ...
        rpc::{
            v2alpha::{SimulateTransactionRequest, SimulateTransactionResponse},
            v2beta::{ExecutedTransaction, Transaction, TransactionEffects, TransactionEvents},
        },
    },
    ErrorReason,
    Result,
...
pub fn simulate_transaction_impl(
    ...
    let SimulateTransactionResult { input_objects, output_objects, events, effects, mock_gas_id: _ } =
        executor.simulate_transaction(transaction.clone().try_into()?).map_err(anyhow::Error::from)?;
