    ...
    for (command, idx) in super::find_arg_uses(arg_idx, commands) {
        match (command, idx) {
            ...
            | (Command::SplitCoins(_), None) => {
                return Err(RpcError::new(tonic::Code::InvalidArgument, "invalid use of literal"));
            }

            // bug in find_arg_uses
            (Command::MakeMoveVector(_), None) | (Command::Publish(_), _) | (Command::MoveCall(_), None) => {
                return Err(RpcError::new(tonic::Code::Internal, "error determining type of literal"));
            }
        }
    }

    literal_type.ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "unable to determine type of literal"))
}
...
fn resolve_literal_to_type(buf: &mut Vec<u8>, type_: &Type, value: &Value) -> Result<()> {
    match type_ {
        ...
        {
            let ty = dt.type_arguments.first().expect("length of type_arguments is 1");
        ...
        Type::Signer | Type::Datatype(_) | Type::TypeParameter(_) | Type::Reference(_, _) => {
            Err(RpcError::new(tonic::Code::InvalidArgument, format!("literal cannot be resolved into type {type_}")))
        }
    }
}
...
fn resolve_as_bool(buf: &mut Vec<u8>, value: &Value) -> Result<()> {
    let b: bool = match &value.kind {
        ...
        Some(Kind::StringValue(s)) => s.parse().map_err(|e| {
            RpcError::new(tonic::Code::InvalidArgument, format!("literal cannot be resolved as bool: {e}"))
        })?,
        _ => return Err(RpcError::new(tonic::Code::InvalidArgument, "literal cannot be resolved into type bool")),
    };
...
fn resolve_as_address(buf: &mut Vec<u8>, value: &Value) -> Result<()> {
    let address = match &value.kind {
        ...
        _ => return Err(RpcError::new(tonic::Code::InvalidArgument, "literal cannot be resolved into type address")),
    };
...
fn resolve_as_string(buf: &mut Vec<u8>, value: &Value) -> Result<()> {
    match &value.kind {
        ...
        _ => return Err(RpcError::new(tonic::Code::InvalidArgument, "literal cannot be resolved into string")),
    };
...
fn resolve_as_option(buf: &mut Vec<u8>, type_: &Type, value: &Value) -> Result<()> {
    match &value.kind {
        ...
        _ => return Err(RpcError::new(tonic::Code::InvalidArgument, "literal cannot be resolved into Option")),
    }
...
mod test {
    ...
    fn resolve_bool() {
        let test_cases = [
            ...
            (Type::Bool, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
    ...
    fn resolve_number() {
        let test_cases = [
            // U8 Successful cases
            (Type::U8, Kind::NumberValue(u8::MAX.into()), Some(bcs::to_bytes(&u8::MAX).unwrap())),
            (Type::U8, Kind::NumberValue(u8::MIN.into()), Some(bcs::to_bytes(&u8::MIN).unwrap())),
            (Type::U8, Kind::StringValue(u8::MAX.to_string()), Some(bcs::to_bytes(&u8::MAX).unwrap())),
            (Type::U8, Kind::StringValue(u8::MIN.to_string()), Some(bcs::to_bytes(&u8::MIN).unwrap())),
            // U8 failure cases
            (Type::U8, Kind::BoolValue(true), None),
            (Type::U8, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            ...
            (Type::U16, Kind::NumberValue(u16::MAX.into()), Some(bcs::to_bytes(&u16::MAX).unwrap())),
            (Type::U16, Kind::NumberValue(u16::MIN.into()), Some(bcs::to_bytes(&u16::MIN).unwrap())),
            (Type::U16, Kind::StringValue(u16::MAX.to_string()), Some(bcs::to_bytes(&u16::MAX).unwrap())),
            (Type::U16, Kind::StringValue(u16::MIN.to_string()), Some(bcs::to_bytes(&u16::MIN).unwrap())),
            // U16 failure cases
            (Type::U16, Kind::BoolValue(true), None),
            (Type::U16, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            ...
            (Type::U32, Kind::NumberValue(u32::MAX.into()), Some(bcs::to_bytes(&u32::MAX).unwrap())),
            (Type::U32, Kind::NumberValue(u32::MIN.into()), Some(bcs::to_bytes(&u32::MIN).unwrap())),
            (Type::U32, Kind::StringValue(u32::MAX.to_string()), Some(bcs::to_bytes(&u32::MAX).unwrap())),
            (Type::U32, Kind::StringValue(u32::MIN.to_string()), Some(bcs::to_bytes(&u32::MIN).unwrap())),
            // U32 failure cases
            (Type::U32, Kind::BoolValue(true), None),
            (Type::U32, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            ...
            (Type::U64, Kind::NumberValue(u64::MAX as _), Some(bcs::to_bytes(&u64::MAX).unwrap())),
            (Type::U64, Kind::NumberValue(u64::MIN as _), Some(bcs::to_bytes(&u64::MIN).unwrap())),
            (Type::U64, Kind::StringValue(u64::MAX.to_string()), Some(bcs::to_bytes(&u64::MAX).unwrap())),
            (Type::U64, Kind::StringValue(u64::MIN.to_string()), Some(bcs::to_bytes(&u64::MIN).unwrap())),
            // U64 failure cases
            (Type::U64, Kind::BoolValue(true), None),
            (Type::U64, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            ...
            (Type::U128, Kind::NumberValue(u64::MAX as _), Some(bcs::to_bytes(&u128::from(u64::MAX)).unwrap())),
            (Type::U128, Kind::NumberValue(u64::MIN as _), Some(bcs::to_bytes(&u128::MIN).unwrap())),
            (Type::U128, Kind::StringValue(u128::MAX.to_string()), Some(bcs::to_bytes(&u128::MAX).unwrap())),
            (Type::U128, Kind::StringValue(u128::MIN.to_string()), Some(bcs::to_bytes(&u128::MIN).unwrap())),
            // U128 failure cases
            (Type::U128, Kind::BoolValue(true), None),
            (Type::U128, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            (Type::U128, Kind::NullValue(0), None),
            (Type::U128, Kind::StringValue("foo".into()), None),
            (Type::U128, Kind::StringValue(U256::max_value().to_string()), None),
            // U256 Successful cases
            (Type::U256, Kind::NumberValue(u64::MAX as _), Some(bcs::to_bytes(&U256::from(u64::MAX)).unwrap())),
            (Type::U256, Kind::NumberValue(u64::MIN as _), Some(bcs::to_bytes(&U256::zero()).unwrap())),
            (
                Type::U256,
                Kind::StringValue(U256::max_value().to_string()),
                Some(bcs::to_bytes(&U256::max_value()).unwrap()),
            ),
            (Type::U256, Kind::StringValue(U256::zero().to_string()), Some(bcs::to_bytes(&U256::zero()).unwrap())),
            // U256 failure cases
            (Type::U256, Kind::BoolValue(true), None),
            (Type::U256, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
    ...
    fn resolve_address() {
        let test_cases = [
            ...
            (Type::Address, Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
    ...
    fn resolve_string() {
        fn utf8() -> Type {
            Type::from_struct_tag(&mut normalized::RcPool::new(), &StructTag {
        ...
        let test_cases = [
            ...
            (utf8(), Kind::StringValue("foo".into()), Some(bcs::to_bytes(&"foo").unwrap())),
            (ascii(), Kind::StringValue("foo".into()), Some(bcs::to_bytes(&"foo").unwrap())),
            (utf8(), Kind::StringValue("".into()), Some(bcs::to_bytes(&"").unwrap())),
            (ascii(), Kind::StringValue("".into()), Some(bcs::to_bytes(&"").unwrap())),
            // String failure cases
            (utf8(), Kind::BoolValue(true), None),
            (utf8(), Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
            (utf8(), Kind::NullValue(0), None),
            (utf8(), Kind::NumberValue(0 as _), None),
            (ascii(), Kind::BoolValue(true), None),
            (ascii(), Kind::ListValue(prost_types::ListValue { values: vec![] }), None),
    ...
    fn resolve_option() {
        fn option_type(t: Type) -> Type {
            ...
            Type::Datatype(Box::new(Datatype {
                module: normalized::ModuleId { address: MOVE_STDLIB_ADDRESS, name: pool.intern(STD_OPTION_MODULE_NAME) },
        ...
        let test_cases = [
            ...
            (option_type(Type::Address), Kind::NullValue(0), Some(vec![0])),
            (option_type(Type::U64), Kind::NumberValue(u64::MIN as _), Some(bcs::to_bytes(&Some(u64::MIN)).unwrap())),
            (
                option_type(Type::U64),
                Kind::StringValue(u64::MAX.to_string()),
                Some(bcs::to_bytes(&Some(u64::MAX)).unwrap()),
            ),
            (option_type(Type::Bool), Kind::BoolValue(true), Some(bcs::to_bytes(&Some(true)).unwrap())),
    ...
    fn resolve_vector() {
        ...
        let test_cases = [
            ...
            (
                vector_type(Type::U8),
                Kind::ListValue(prost_types::ListValue { values: vec![Kind::NumberValue(9 as _).into()] }),
                Some(vec![1, 9]),
            ),
            (vector_type(Type::U8), Kind::ListValue(prost_types::ListValue { values: vec![] }), Some(vec![0])),
            (
            ...
            (
                ...
                Kind::ListValue(prost_types::ListValue { values: vec![Kind::BoolValue(true).into(); 256] }),
