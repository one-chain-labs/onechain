...
use sui_sdk_types::{Argument, Command, ObjectId};
use sui_types::{
...
use crate::{
    ...
    field_mask::FieldMaskTree,
    message::MessageMergeFrom,
    proto::{
        google::rpc::bad_request::FieldViolation,
        rpc::v2alpha::{ResolveTransactionRequest, ResolveTransactionResponse},
    },
    reader::StateReader,
    ErrorReason,
    Result,
...
pub fn resolve_transaction(
    ...
    let executor =
        service.executor.as_ref().ok_or_else(|| RpcError::new(tonic::Code::Unimplemented, "no transaction executor"))?;

    let read_mask = request.read_mask.map(FieldMaskTree::from).unwrap_or_else(FieldMaskTree::new_wildcard);

    let unresolved_transaction = request
        .unresolved_transaction
        .ok_or_else(|| FieldViolation::new("unresolved_transaction").with_reason(ErrorReason::FieldMissing))?;
    ...
    let ptb = unresolved_transaction
        ...
        .and_then(|kind| {
            kind.kind.and_then(|kind| match kind {
                crate::proto::rpc::v2beta::transaction_kind::Kind::ProgrammableTransaction(ptb) => Some(ptb),
                _ => None,
            })
        })
        .ok_or_else(|| {
    ...
    let (reference_gas_price, protocol_config) = {
        ...
        .ok_or_else(|| RpcError::new(tonic::Code::Internal, "unable to get current protocol config"))?;
    ...
    ResolveTransactionResponse { transaction: Some(transaction), simulation }.pipe(Ok)
}
...
fn select_gas(
    ...
    for maybe_coin in gas_coins {
        let (object_ref, value) = maybe_coin.map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
