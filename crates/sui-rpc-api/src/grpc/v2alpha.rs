...
use crate::{
    ...
    proto::rpc::{
        v2alpha::{
            live_data_service_server::LiveDataService,
    ...
    subscription::SubscriptionServiceHandle,
    RpcService,
};
...
impl LiveDataService for RpcService {
    ...
    async fn resolve_transaction(
        ...
        resolve::resolve_transaction(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}
...
impl SignatureVerificationService for RpcService {
    async fn verify_signature(
        ...
        verify_signature::verify_signature(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
}
