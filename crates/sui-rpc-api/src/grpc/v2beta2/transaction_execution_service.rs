...
use crate::{ErrorReason, RpcError, RpcService};
...
impl TransactionExecutionService for RpcService {
    async fn execute_transaction(
        ...
        execute_transaction(self, executor, request.into_inner()).await.map(tonic::Response::new).map_err(Into::into)
    }
}
...
pub async fn execute_transaction(
    ...
    let read_mask = {
        let read_mask = request.read_mask.unwrap_or_else(|| FieldMask::from_str(EXECUTE_TRANSACTION_READ_MASK_DEFAULT));
    ...
    let executed_transaction = if let Some(mask) = read_mask.subtree(ExecuteTransactionResponse::TRANSACTION_FIELD.name)
    {
        ...
        let effects = mask.subtree(ExecutedTransaction::EFFECTS_FIELD.name).map(|mask| {
            ...
            if mask.contains(TransactionEffects::UNCHANGED_CONSENSUS_OBJECTS_FIELD.name) {
                for unchanged_consensus_object in effects.unchanged_consensus_objects.iter_mut() {
                    let Ok(object_id) = unchanged_consensus_object.object_id().parse::<Address>() else {
                        continue;
                    };

                    if let Some(object) = input_objects.iter().find(|o| o.object_id() == object_id) {
                        unchanged_consensus_object.object_type = Some(match object.object_type() {
                            sui_sdk_types::ObjectType::Package => "package".to_owned(),
                            sui_sdk_types::ObjectType::Struct(struct_tag) => struct_tag.to_string(),
                        });
                    }
        ...
        message.digest = mask.contains(ExecutedTransaction::DIGEST_FIELD.name).then(|| transaction.digest().to_string());
    ...
    message.finality = read_mask.contains(ExecuteTransactionResponse::FINALITY_FIELD.name).then_some(finality);
