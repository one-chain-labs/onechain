...
use sui_rpc::proto::{
    ...
    sui::rpc::v2beta2::{ListPackageVersionsRequest, ListPackageVersionsResponse, PackageVersion},
};
...
pub fn list_package_versions(
    ...
    let page_size = request.page_size.map(|s| (s as usize).clamp(1, 10000)).unwrap_or(1000);
    ...
    for result in iter.take(page_size + 1) {
        let (version, storage_id) = result.map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?;
    ...
    let next_page_token = if versions.len() > page_size {
        ...
        versions
            .pop()
            .unwrap()
            .pipe(|v| v.version.map(|version| encode_page_token(PageToken { original_package_id, version })))
    } else {
        None
    };
