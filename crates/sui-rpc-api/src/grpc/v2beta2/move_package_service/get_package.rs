...
use sui_rpc::proto::{
    ...
    sui::rpc::v2beta2::{GetPackageRequest, GetPackageResponse, Package},
};
...
pub fn get_package(service: &RpcService, request: GetPackageRequest) -> Result<GetPackageResponse> {
    ...
    let resolved_package = sui_package_resolver::Package::read_from_package(&package).map_err(convert_error)?;

    let modules: Vec<_> = resolved_package
        .modules()
        .iter()
        .map(|(module_name, resolver_module)| convert_module(module_name, resolver_module, &package_id))
