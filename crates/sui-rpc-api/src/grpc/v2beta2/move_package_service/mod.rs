...
impl MovePackageService for RpcService {
    async fn get_package(
        ...
        get_package::get_package(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
    ...
    async fn get_datatype(
        ...
        get_datatype::get_datatype(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
    ...
    async fn get_function(
        ...
        get_function::get_function(self, request.into_inner()).map(tonic::Response::new).map_err(Into::into)
    }
...
pub(crate) fn load_package(service: &RpcService, package_id_str: &str) -> Result<MovePackage> {
    ...
    let object = service.reader.inner().get_object(&package_id).ok_or_else(RpcError::not_found)?;

    let inner = object.into_inner();
    inner.data.try_into_package().ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "object is not a package"))
}
