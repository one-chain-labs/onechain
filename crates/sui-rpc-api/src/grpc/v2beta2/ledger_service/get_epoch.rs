...
use sui_rpc::{
    ...
    proto::{
        ...
        sui::rpc::v2beta2::{Epoch, GetEpochRequest, GetEpochResponse, ProtocolConfig},
...
pub fn get_epoch(service: &RpcService, request: GetEpochRequest) -> Result<GetEpochResponse> {
    ...
    let mut system_state = if epoch == current_epoch && read_mask.contains(Epoch::SYSTEM_STATE_FIELD.name) {
        ...
        service.reader.inner().indexes().and_then(|indexes| indexes.get_epoch_info(epoch).ok().flatten())
    {
        ...
        if let Some(submask) = read_mask.subtree(Epoch::PROTOCOL_CONFIG_FIELD.name) {
            ...
            let protocol_config =
                epoch_info.protocol_version.map(|version| get_protocol_config(version, chain)).transpose()?;
...
fn get_protocol_config(
    ...
    let config = sui_protocol_config::ProtocolConfig::get_for_version_if_supported(version.into(), chain)
