...
use crate::{error::ObjectNotFoundError, ErrorReason, RpcError, RpcService};
...
fn get_object_impl(
    ...
    let object = if let Some(version) = version {
        ...
        service.reader.inner().get_object(&object_id.into()).ok_or_else(|| ObjectNotFoundError::new(object_id))?
    };
