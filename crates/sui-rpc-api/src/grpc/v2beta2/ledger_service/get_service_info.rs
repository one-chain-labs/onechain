...
use sui_rpc::proto::{sui::rpc::v2beta2::GetServiceInfoResponse, timestamp_ms_to_proto};
...
pub fn get_service_info(service: &RpcService) -> Result<GetServiceInfoResponse, RpcError> {
    ...
    let lowest_available_checkpoint = service.reader.inner().get_lowest_available_checkpoint()?.pipe(Some);
