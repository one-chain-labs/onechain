                ...
                if event_mask.contains(Event::JSON_FIELD.name) {
                    for (message, event) in message.events.iter_mut().zip(&events.0) {
                        message.json = struct_tag_sdk_to_core(event.type_.clone()).ok().and_then(|struct_tag| {
                            crate::grpc::v2beta2::render_json(service, &struct_tag, &event.contents).map(Box::new)
                        });
                    }
                }
            }
...
pub(crate) fn render_clever_error(service: &RpcService, effects: &mut TransactionEffects) {
    use sui_rpc::proto::sui::rpc::v2beta2::{clever_error, execution_error::ErrorDetails, CleverError, MoveAbort};
