...
use sui_rpc::proto::{
    ...
    sui::rpc::v2beta2::{Balance, ListBalancesRequest, ListBalancesResponse},
};
...
pub fn list_balances(service: &RpcService, request: ListBalancesRequest) -> Result<ListBalancesResponse> {
    ...
    let owner: Address = request
        ...
        .as_ref()
        .ok_or_else(|| {
            FieldViolation::new("owner").with_description("missing owner").with_reason(ErrorReason::FieldMissing)
        })?
        .parse()
        .map_err(|e| {
    ...
    let mut balances = indexes
        .balance_iter(&owner.into(), page_token.map(|t| (owner.into(), t.coin_type)))?
        .take(page_size + 1)
    ...
    let next_page_token = if balances.len() > page_size {
        ...
        // guaranteed to be >= 1.
        balances.pop().unwrap().pipe(|(coin_type, _info)| encode_page_token(PageToken { owner, coin_type })).pipe(Some)
    } else {
        None
    };
...
fn balance_info_to_proto((coin_type, info): (move_core_types::language_storage::StructTag, BalanceInfo)) -> Balance {
