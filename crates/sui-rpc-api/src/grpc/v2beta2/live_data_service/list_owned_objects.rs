...
use sui_rpc::{
    ...
    proto::{
        ...
        sui::rpc::v2beta2::{ErrorReason, ListOwnedObjectsRequest, ListOwnedObjectsResponse, Object},
    },
};
...
pub fn list_owned_objects(service: &RpcService, request: ListOwnedObjectsRequest) -> Result<ListOwnedObjectsResponse> {
    ...
    let mut iter = indexes.owned_objects_iter(owner.into(), object_type.clone(), page_token.map(|t| t.inner))?;
        ...
        iter.next().transpose().map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))?
    {
        let object = if should_load_object {
            let Some(object) = service.reader.inner().get_object_by_key(&object_info.object_id, object_info.version)
            else {
                tracing::debug!(
