...
pub fn list_dynamic_fields(
    ...
    let parent: Address = request
        ...
        .as_ref()
        .ok_or_else(|| {
            FieldViolation::new(ListDynamicFieldsRequest::PARENT_FIELD.name).with_reason(ErrorReason::FieldMissing)
        })?
        .parse()
        .map_err(|e| {
    ...
    let page_size = request.page_size.map(|s| (s as usize).clamp(1, MAX_PAGE_SIZE)).unwrap_or(DEFAULT_PAGE_SIZE);
    ...
    let mut iter = indexes.dynamic_field_iter(parent.into(), page_token.map(|t| t.field_id.into()))?;
    let mut dynamic_fields = Vec::with_capacity(page_size);
    let mut size_bytes = 0;
    while let Some(key) = iter.next().transpose().map_err(|e| RpcError::new(tonic::Code::Internal, e.to_string()))? {
        let Some(dynamic_field) = get_dynamic_field(service, &key.parent, &key.field_id, &read_mask) else {
            continue;
        };
    ...
    let next_page_token = iter
        ...
        .map(|cursor| encode_page_token(PageToken { parent, field_id: cursor.field_id.into() }));
...
fn load_dynamic_field(
    ...
    use sui_types::dynamic_field::{visitor as DFV, DynamicFieldType};
    ...
    let layout = match service.reader.inner().get_struct_layout(&move_object.type_().clone().into()) {
        Ok(Some(layout)) => layout,
        Ok(None) => return Err(anyhow::anyhow!("unable to load layout for type `{:?}`", move_object.type_())),
        Err(e) => {
            return Err(anyhow::anyhow!("unable to load layout for type `{:?}`: {e}", move_object.type_()));
        }
    };
    ...
    if read_mask.contains(DynamicField::NAME_TYPE_FIELD) {
        message.name_type = Some(sui_types::TypeTag::from(field.name_layout).to_canonical_string(true));
    }
    ...
    match field.value_metadata()? {
        ...
        DFV::ValueMetadata::DynamicObjectField(object_id) => {
            ...
            if read_mask.contains(DynamicField::VALUE_TYPE_FIELD) || read_mask.contains(DynamicField::OBJECT_FIELD) {
                let object = service
                    .reader
                    .inner()
                    .get_object(&object_id)
                    .ok_or_else(|| anyhow::anyhow!("missing dynamic object {object_id}"))?;
                let type_tag = sui_types::TypeTag::from(
                    object.struct_tag().ok_or_else(|| anyhow::anyhow!("dynamic object field cannot be a package"))?,
                );
                if read_mask.contains(DynamicField::VALUE_TYPE_FIELD) {
                    message.value_type = Some(type_tag.to_canonical_string(true));
                }
