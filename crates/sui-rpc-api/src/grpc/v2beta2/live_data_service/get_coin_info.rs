...
use sui_rpc::proto::sui::rpc::v2beta2::{
    coin_metadata::MetadataCapState as ProtoMetadataCapState,
    coin_treasury::SupplyState as RpcSupplyState,
    regulated_coin_metadata::CoinRegulatedState as ProtoCoinRegulatedState,
    CoinMetadata,
    CoinTreasury,
    GetCoinInfoRequest,
    GetCoinInfoResponse,
...
use sui_types::{
    base_types::{ObjectID as SuiObjectID, SuiAddress},
    coin_registry::{Currency, MetadataCapState, RegulatedState as CurrencyRegulatedState, SupplyState},
    sui_sdk_types_conversions::struct_tag_sdk_to_core,
};

use crate::{Result, RpcError, RpcService};
...
pub fn get_coin_info(service: &RpcService, request: GetCoinInfoRequest) -> Result<GetCoinInfoResponse> {
    let indexes = service.reader.inner().indexes().ok_or_else(RpcError::not_found)?;

    let coin_type = request
        .coin_type()
        .parse::<StructTag>()
        .map_err(|e| RpcError::new(tonic::Code::InvalidArgument, format!("invalid coin_type: {e}")))?;
...
fn get_coin_info_from_registry(
    ...
    let currency_id = Currency::derive_object_id(core_coin_type.clone().into()).map_err(|e| {
        RpcError::new(
            tonic::Code::Internal,
            format!("Failed to derive Currency ID for coin type {}: {}", core_coin_type, e),
        )
    })?;
    ...
    let move_obj = currency_obj.data.try_as_move().ok_or_else(|| {
        RpcError::new(tonic::Code::Internal, format!("Currency for coin type {} is not a Move object", core_coin_type))
    })?;

    let currency = bcs::from_bytes::<Currency>(move_obj.contents()).map_err(|e| {
        RpcError::new(
            tonic::Code::Internal,
            format!("Failed to deserialize Currency for coin type {}: {}", core_coin_type, e),
        )
    })?;
    let metadata = {
    ...
    let treasury = if sui_types::gas_coin::GAS::is_gas(core_coin_type) {
        ...
        match &currency.supply {
            Some(SupplyState::Fixed(supply)) => {
                ...
                treasury.id = currency.treasury_cap_id.map(|id| Address::from(id).to_string());
            ...
            _ => {
                ...
                currency.treasury_cap_id.and_then(|id| get_treasury_cap_info(service, id))
            }
        }
    };

    let regulated_metadata = match &currency.regulated {
        CurrencyRegulatedState::Regulated { cap, allow_global_pause, variant } => {
...
fn get_coin_info_from_index(
    ...
    let regulated_metadata = if let Some(regulated_coin_metadata_object_id) = coin_info.regulated_coin_metadata_object_id
    {
        service
            .reader
            .inner()
            ...
            .map(|value| {
                ...
                regulated.coin_metadata_object = Some(Address::from(value.coin_metadata_object.bytes).to_string());
                regulated.deny_cap_object = Some(Address::from(value.deny_cap_object.bytes).to_string());
                regulated.coin_regulated_state = Some(ProtoCoinRegulatedState::Regulated as i32);
                regulated
            })
    } else {
...
fn get_treasury_cap_info(service: &RpcService, treasury_object_id: SuiObjectID) -> Option<CoinTreasury> {
    ...
    sui_types::coin::TreasuryCap::try_from(obj).ok().map(|treasury| {
