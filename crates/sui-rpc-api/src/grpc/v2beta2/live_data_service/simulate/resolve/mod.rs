...
use sui_rpc::proto::{google::rpc::bad_request::FieldViolation, sui::rpc::v2beta2::Transaction};
use sui_sdk_types::{Address, Argument, Command};
use sui_types::{
    base_types::ObjectRef,
    move_package::MovePackage,
    transaction::{CallArg, GasData, ObjectArg, ProgrammableTransaction, TransactionData},
};
use tap::Pipe;

use crate::{error::ObjectNotFoundError, reader::StateReader, ErrorReason, Result, RpcError, RpcService};
    ...
    let ptb = unresolved_transaction
        ...
        .and_then(|kind| {
            kind.kind.as_ref().and_then(|kind| match kind {
                sui_rpc::proto::sui::rpc::v2beta2::transaction_kind::Kind::ProgrammableTransaction(ptb) => Some(ptb),
                _ => None,
            })
        })
        .ok_or_else(|| {
            FieldViolation::new("transaction.kind.programmable_transaction").with_reason(ErrorReason::FieldMissing)
        })?;

    let commands =
        ptb.commands.iter().map(sui_sdk_types::Command::try_from).collect::<Result<Vec<_>, _>>().map_err(|e| {
...
pub(super) fn called_packages(
        ...
        )
    {
        let package = reader
            ...
            .try_as_package()
            .ok_or_else(|| {
                RpcError::new(tonic::Code::InvalidArgument, format!("object {} is not a package", move_call.package))
            })?
            .to_owned();
        ...
        let normalized_modules = package.normalize(&mut pool, &binary_config, /* include code */ true).map_err(|e| {
            RpcError::new(tonic::Code::Internal, format!("unable to normalize package {}: {e}", move_call.package))
        })?;
...
fn resolve_unresolved_transaction(
    ...
    let gas_data = if let Some(unresolved_gas_payment) = gas_payment {
        ...
        let max_gas_budget = if payment.is_empty() {
            ...
            gas_coins.iter().map(|(_, value)| *value).sum::<u64>().min(max_gas_budget)
        };
        GasData {
            payment,
        ...
        GasData { payment: vec![], owner: sender.into(), price: reference_gas_price, budget: max_gas_budget }
    };

    let expiration = expiration
        .map(sui_types::transaction::TransactionExpiration::try_from)
        .transpose()
        .map_err(|e| FieldViolation::new("expiration").with_description(e).with_reason(ErrorReason::FieldInvalid))?
...
fn resolve_gas_object_reference(
    ...
    resolve_object_reference_with_object(&object, unresolved_object_reference).map(|r| (r, gas_coin.value()))
}
...
fn resolve_object_reference_with_object(
    ...
    let UnresolvedObjectReference { object_id, version, digest } = unresolved_object_reference;
    ...
    if object_id.inner() != &id.into_bytes() {
        return Err(RpcError::new(tonic::Code::Internal, "provided object and object_id should match"));
    }
...
pub(super) fn resolve_ptb(
    ...
    ProgrammableTransaction { inputs, commands: commands.into_iter().map(Into::into).collect() }.pipe(Ok)
}
...
fn resolve_arg(
    ...
    match UnresolvedInput::from_proto(arg)? {
        ...
        UnresolvedInput {
            ...
            CallArg::Object(ObjectArg::ImmOrOwnedObject(resolve_object_reference(reader, UnresolvedObjectReference {
                object_id,
                version,
                digest,
            })?))
        ...
        UnresolvedInput {
            ...
            mutable: None,
            literal: None,
        } => CallArg::Object(ObjectArg::Receiving(resolve_object_reference(reader, UnresolvedObjectReference {
            object_id,
            version,
            digest,
        })?)),
        ...
        } => CallArg::Pure(literal::resolve_literal(called_packages, commands, arg_idx, literal)?),

        _ => return Err(RpcError::new(tonic::Code::InvalidArgument, "invalid unresolved input argument")),
    }
    .pipe(Ok)
}
...
fn resolve_object(
    ...
    match object.owner() {
        sui_types::object::Owner::Immutable => {
            resolve_object_reference_with_object(&object, UnresolvedObjectReference { object_id, version, digest })
...
fn arg_type_of_move_call_input(
    ...
    let Some(ty) = function.parameters.get(idx) else {
        return Err(RpcError::new(tonic::Code::InvalidArgument, "invalid input parameter"));
    };
    Ok(ty.clone())
}
...
fn resolve_shared_input_with_object(
    ...
    let initial_shared_version =
        ...
        | sui_types::object::Owner::ConsensusAddressOwner { start_version: initial_shared_version, .. } =
            object.owner()
        {
    ...
    for (command, idx) in find_arg_uses(arg_idx, commands) {
        match (command, idx) {
            (Command::MoveCall(move_call), Some(idx)) => {
                let arg_type = arg_type_of_move_call_input(called_packages, move_call, idx)?;
                if matches!(&*arg_type, normalized::Type::Reference(/* mut */ true, _) | normalized::Type::Datatype(_)) {
                    mutable = true;
                }
            }
...
impl TryFrom<&sui_rpc::proto::sui::rpc::v2beta2::ObjectReference> for UnresolvedObjectReference {
    ...
    fn try_from(value: &sui_rpc::proto::sui::rpc::v2beta2::ObjectReference) -> Result<Self, Self::Error> {
        ...
        let digest = value.digest.as_ref().map(|digest| digest.parse()).transpose().map_err(|e| {
            FieldViolation::new("digest")
                .with_description(format!("invalid digest: {e}"))
                .with_reason(ErrorReason::FieldInvalid)
        })?;

        Ok(Self { object_id, version, digest })
    }
}
...
impl<'a> UnresolvedInput<'a> {
    fn from_proto(input: &'a sui_rpc::proto::sui::rpc::v2beta2::Input) -> Result<Self, FieldViolation> {
        Ok(Self {
