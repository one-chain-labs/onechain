...
use sui_sdk_types::{CheckpointContents, CheckpointDigest, CheckpointSequenceNumber, SignedCheckpointSummary};
use tap::Pipe;

use crate::{
    field_mask::{FieldMaskTree, FieldMaskUtil},
    proto::{
        google::rpc::{bad_request::FieldViolation, BadRequest},
        node::v2::{
...
impl RpcService {
    pub fn get_checkpoint(&self, request: GetCheckpointRequest) -> Result<GetCheckpointResponse> {
        let checkpoint_id = CheckpointId::try_from_proto_request(request.sequence_number, request.digest)?;

        let read_mask =
            request.read_mask.unwrap_or_else(|| FieldMask::from_str(GetCheckpointRequest::READ_MASK_DEFAULT));
        ...
        let SignedCheckpointSummary { checkpoint, signature } = match checkpoint_id {
        ...
        let (contents, contents_bcs) = if read_mask.contains("contents") || read_mask.contains("contents_bcs") {
            ...
            let contents_bcs =
                read_mask.contains("contents_bcs").then(|| bcs::to_bytes(&contents)).transpose()?.map(Into::into);

            (read_mask.contains("contents").then(|| contents.into()), contents_bcs)
        } else {
            (None, None)
        };

        let summary_bcs =
            read_mask.contains("summary_bcs").then(|| bcs::to_bytes(&checkpoint)).transpose()?.map(Into::into);

        GetCheckpointResponse {
            sequence_number: read_mask.contains("sequence_number").then_some(checkpoint.sequence_number),
            digest: read_mask.contains("digest").then(|| checkpoint.digest().into()),
    ...
    pub fn get_full_checkpoint(&self, request: GetFullCheckpointRequest) -> Result<GetFullCheckpointResponse> {
        let checkpoint_id = CheckpointId::try_from_proto_request(request.sequence_number, request.digest)?;

        let read_mask =
            request.read_mask.unwrap_or_else(|| FieldMask::from_str(GetFullCheckpointRequest::READ_MASK_DEFAULT));
        ...
        let verified_summary = match checkpoint_id {
            Some(checkpoint_id @ CheckpointId::SequenceNumber(s)) => {
                if s < self.reader.inner().get_lowest_available_checkpoint()?
                    || (s < self.reader.inner().get_lowest_available_checkpoint_objects()?
        ...
        let checkpoint_contents = self
            ...
            .ok_or(CheckpointNotFoundError(CheckpointId::SequenceNumber(*verified_summary.sequence_number())))?;

        let checkpoint = self.reader.inner().get_checkpoint_data(verified_summary, checkpoint_contents)?;
...
pub(crate) fn checkpoint_data_to_full_checkpoint_response(
    ...
    let summary_bcs = read_mask.contains("summary_bcs").then(|| bcs::to_bytes(&summary)).transpose()?.map(Into::into);
    let contents_bcs =
        read_mask.contains("contents_bcs").then(|| bcs::to_bytes(&checkpoint_contents)).transpose()?.map(Into::into);
    ...
    GetFullCheckpointResponse {
        sequence_number: read_mask.contains("sequence_number").then_some(sequence_number),
...
fn transaction_to_checkpoint_transaction(
    ...
    let transaction_bcs = read_mask.contains("transaction_bcs").then(|| Bcs::serialize(&transaction)).transpose()?;
    let transaction = read_mask
        .contains("transaction")
        .then(|| sui_sdk_types::Transaction::try_from(transaction))
        .transpose()?
        .map(Into::into);
    let effects_bcs = read_mask.contains("effects_bcs").then(|| Bcs::serialize(&effects)).transpose()?;
    let effects = read_mask
        ...
        .map(Into::into);
    let events_bcs =
        read_mask.contains("events_bcs").then(|| events.as_ref().map(Bcs::serialize)).flatten().transpose()?;
    ...
    let input_objects = read_mask
        ...
        .map(|read_mask| {
            input_objects.into_iter().map(|object| object_to_object_response(object, &read_mask)).collect::<Result<_>>()
        })
        .transpose()?
        .unwrap_or_default();
    ...
    let output_objects = read_mask
        ...
        .map(|read_mask| {
            output_objects.into_iter().map(|object| object_to_object_response(object, &read_mask)).collect::<Result<_>>()
        })
        .transpose()?
        .unwrap_or_default();
...
fn object_to_object_response(
    ...
    let object_id = read_mask.contains("object_id").then(|| sui_sdk_types::ObjectId::from(object.id()).into());
    let version = read_mask.contains("version").then(|| object.version().value());
    let digest = read_mask.contains("digest").then(|| sui_sdk_types::ObjectDigest::from(object.digest()).into());

    let object_bcs = read_mask.contains("object_bcs").then(|| Bcs::serialize(&object)).transpose()?;
    let object =
        read_mask.contains("object").then(|| sui_sdk_types::Object::try_from(object)).transpose()?.map(Into::into);
