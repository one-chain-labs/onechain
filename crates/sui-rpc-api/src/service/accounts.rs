...
impl RpcService {
    pub fn list_account_objects(&self, request: ListAccountObjectsRequest) -> Result<ListAccountObjectsResponse> {
        ...
        let next_page_token = if object_info.len() > page_size {
            ...
            // gaurenteed to be >= 1.
            object_info.pop().unwrap().object_id.pipe(encode_page_token).pipe(Some)
        } else {
            None
        };

        Ok(ListAccountObjectsResponse {
            objects: object_info.into_iter().map(AccountOwnedObjectInfo::into_proto).collect(),
            next_page_token,
        })
    }
}
