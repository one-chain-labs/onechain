...
use crate::{
    ...
    proto::{
        ...
        node::v2::{GetTransactionRequest, GetTransactionResponse},
    },
    ErrorReason,
    Result,
    RpcService,
...
impl RpcService {
    pub fn get_transaction(
        ...
        let read_mask = read_mask.unwrap_or_else(|| FieldMask::from_str(GetTransactionRequest::READ_MASK_DEFAULT));
        ...
        let transaction_bcs =
            read_mask.contains("transaction_bcs").then(|| bcs::to_bytes(&transaction)).transpose()?.map(Into::into);
        ...
        let signatures_bytes = read_mask
            ...
            .then(|| signatures.iter().map(|signature| signature.to_bytes().into()).collect())
            .unwrap_or_default();

        GetTransactionResponse {
            digest: read_mask.contains("digest").then(|| digest.into()),
            transaction: read_mask.contains("transaction").then(|| transaction.into()),
            transaction_bcs,
            ...
            events_bcs,
            checkpoint: read_mask.contains("checkpoint").then_some(checkpoint).flatten(),
            timestamp: read_mask
                .contains("timestamp")
