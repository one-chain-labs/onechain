...
use crate::{
    ...
    proto::{
        ...
        node::v2::{effects_finality::Finality, EffectsFinality, ExecuteTransactionRequest, ExecuteTransactionResponse},
    },
    types::{SimulateTransactionQueryParameters, TransactionSimulationResponse},
    ErrorReason,
    Result,
    RpcError,
    RpcService,
...
impl RpcService {
    pub async fn execute_transaction(&self, request: ExecuteTransactionRequest) -> Result<ExecuteTransactionResponse> {
        ...
        let signed_transaction = SignedTransaction { transaction, signatures };
        ...
        let (effects, finality) = {
            ...
            (effects.try_into()?, EffectsFinality { finality: Some(finality) })
        };

        let effects_bcs =
            read_mask.contains("effects_bcs").then(|| bcs::to_bytes(&effects)).transpose()?.map(Into::into);
        ...
        let balance_changes = match (read_mask.contains("balance_changes"), &input_objects, &output_objects) {
        ...
        ExecuteTransactionResponse {
            ...
            effects_bcs,
            events: read_mask.contains("events").then(|| events.map(Into::into)).flatten(),
            events_bcs,
            balance_changes: balance_changes.map(|b| b.into_iter().map(Into::into).collect()).unwrap_or_default(),
        }
        .pipe(Ok)
    }
    ...
    pub fn simulate_transaction(
        ...
        if transaction.gas_payment.objects.is_empty() {
            return Err(RpcError::new(tonic::Code::InvalidArgument, "no gas payment provided"));
        }
        ...
        if mock_gas_id.is_some() {
            return Err(RpcError::new(tonic::Code::Internal, "simulate unexpectedly used a mock gas payment"));
        }
