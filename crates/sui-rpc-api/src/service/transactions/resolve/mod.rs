...
impl RpcService {
    pub fn resolve_transaction(
        ...
        let called_packages = called_packages(&self.reader, &protocol_config, &unresolved_transaction)?;
        ...
        ResolveTransactionResponse { transaction: resolved_transaction.try_into()?, simulation }.pipe(Ok)
    }
}
    ...
    match (kind, value, object_id, version, digest, mutable) {
        ...
        (Some(Pure), Some(unresolved::Value::String(v)), None, None, None, None) => {
            let value = Base64::decode(&v).map_err(|e| {
                RpcError::new(tonic::Code::InvalidArgument, format!("argument is an invalid pure argument: {e}"))
            })?;
            CallArg::Pure(value)
        }
...
fn arg_type_of_move_call_input<'a>(
    ...
    function.parameters.get(idx).ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "invalid input parameter"))
}
