...
use sui_sdk_types::{ObjectId, StructTag, TypeTag};
...
impl RpcService {
    pub fn get_coin_info(&self, request: GetCoinInfoRequest) -> Result<GetCoinInfoResponse> {
        ...
        let coin_type_proto =
            request.coin_type.ok_or_else(|| RpcError::new(tonic::Code::InvalidArgument, "missing coin_type"))?;
        let coin_type = TypeTag::try_from(&coin_type_proto)
        ...
        let coin_type = if let TypeTag::Struct(struct_tag) = coin_type {
            *struct_tag
            ...
            return Err(RpcError::new(tonic::Code::InvalidArgument, "invalid coin_type"));
        };
        ...
        let treasury =
            if let Some(treasury_object_id) = treasury_object_id {
                self.reader
                    .inner()
        ...
        Ok(GetCoinInfoResponse { coin_type: Some(coin_type_proto), metadata, treasury })
    }
}
