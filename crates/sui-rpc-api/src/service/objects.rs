...
use crate::{
    ...
    proto::{
        ...
        node::{
            v2::{GetObjectRequest, GetObjectResponse},
            v2alpha::{DynamicField, ListDynamicFieldsRequest, ListDynamicFieldsResponse},
        },
    },
    ErrorReason,
    Result,
...
impl RpcService {
    pub fn get_object(
        ...
        let read_mask = read_mask.unwrap_or_else(|| FieldMask::from_str(GetObjectRequest::READ_MASK_DEFAULT));
        ...
        let object_bcs = read_mask.contains("object_bcs").then(|| bcs::to_bytes(&object)).transpose()?.map(Into::into);

        GetObjectResponse {
            object_id: read_mask.contains("object_id").then(|| object.object_id().into()),
...
impl RpcService {
    pub fn list_dynamic_fields(&self, request: ListDynamicFieldsRequest) -> Result<ListDynamicFieldsResponse> {
        let indexes = self.reader.inner().indexes().ok_or_else(RpcError::not_found)?;
        ...
        let page_size = request.page_size.map(|s| (s as usize).clamp(1, 1000)).unwrap_or(50);
        ...
        let next_page_token = if dynamic_fields.len() > page_size {
            ...
            // gaurenteed to be >= 1.
            dynamic_fields.pop().unwrap().field_id.pipe(ObjectId::from).pipe(encode_page_token).pipe(Some)
        } else {
            None
        };

        Ok(ListDynamicFieldsResponse {
            dynamic_fields: dynamic_fields.into_iter().map(DynamicFieldInfo::into_proto).collect(),
            next_page_token,
        })
    }
}
...
impl TryFrom<(DynamicFieldKey, DynamicFieldIndexInfo)> for DynamicFieldInfo {
    ...
    fn try_from(value: (DynamicFieldKey, DynamicFieldIndexInfo)) -> Result<Self, Self::Error> {
        ...
        let DynamicFieldIndexInfo { dynamic_field_type: _, name_type, name_value, dynamic_object_id } = value.1;
