...
use sui_types::{
    ...
    full_checkpoint_content::{CheckpointData, ObjectSet},
...
impl Client {
    ...
    async fn get_checkpoint_internal(
        ...
        let (metadata, checkpoint, _extentions) = self.0.ledger_client().get_checkpoint(request).await?.into_parts();
    ...
    pub async fn get_full_checkpoint(&mut self, sequence_number: CheckpointSequenceNumber) -> Result<CheckpointData> {
    ...
    pub async fn get_object_with_version(&mut self, object_id: ObjectID, version: SequenceNumber) -> Result<Object> {
        self.get_object_internal(object_id, Some(version.value())).await
    }

    async fn get_object_internal(&mut self, object_id: ObjectID, version: Option<u64>) -> Result<Object> {
        let mut request = proto::GetObjectRequest::new(&object_id.into()).with_read_mask(FieldMask::from_paths(["bcs"]));
        request.version = version;

        let (metadata, object, _extentions) = self.0.ledger_client().get_object(request).await?.into_parts();
    ...
    pub async fn execute_transaction(&mut self, transaction: &Transaction) -> Result<TransactionExecutionResponse> {
...
fn execute_transaction_response_try_from_proto(
    ...
    let objects = executed_transaction.objects().try_into().map_err(|e| TryFromProtoError::invalid("objects.bcs", e))?;

    TransactionExecutionResponse { effects, events, balance_changes, objects }.pipe(Ok)
}
