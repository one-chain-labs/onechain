...
fn find_dependency_proto_dir(package_name: &str, subpath: &str) -> PathBuf {
    ...
    cmd.arg("metadata").arg("--format-version=1").current_dir(workspace_root);
    let output = cmd.output().expect("Failed to run cargo metadata");
    let metadata: serde_json::Value = serde_json::from_slice(&output.stdout).expect("Failed to parse cargo metadata");
    ...
    let manifest_path = package["manifest_path"].as_str().expect("manifest_path not found");
...
fn main() {
    ...
    let file_descriptors =
        protox::compile(proto_files, [sui_proto_dir, sui_rpc_proto_dir]).expect("failed to compile proto files");
