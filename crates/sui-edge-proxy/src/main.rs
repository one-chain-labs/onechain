...
use sui_edge_proxy::{
    ...
    handlers::{proxy_handler, AppState},
    metrics::AppMetrics,
};
use tracing::info;
...
struct Args {
    #[clap(long, short, default_value = "./sui-edge-proxy.yaml", help = "Specify the config file path to use")]
    config: String,
}
...
async fn main() {
    ...
    let (config, client): (ProxyConfig, Client) = load(&args.config).await.expect("Failed to load config");
    ...
    axum_server::Server::bind(config.listen_address).serve(app.into_make_service()).await.unwrap();
}
