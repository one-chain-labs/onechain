...
use crate::{
    checkpoint::{read_checkpoint, read_checkpoint_list, CheckpointsList},
    config::Config,
...
pub async fn get_verified_object(config: &Config, id: ObjectID) -> Result<Object> {
    let sui_client: Arc<sui_sdk::SuiClient> =
        Arc::new(SuiClientBuilder::default().build(config.full_node_url.as_str()).await?);
...
pub async fn get_verified_effects_and_events(
    ...
    let sui_mainnet: sui_sdk::SuiClient = SuiClientBuilder::default().build(config.full_node_url.as_str()).await?;
    ...
    let full_check_point =
        object_store.get_full_checkpoint(seq).await.map_err(|e| anyhow!(format!("Cannot get full checkpoint: {e}")))?;
...
pub async fn get_verified_checkpoint(id: ObjectID, config: &Config) -> Result<CheckpointSequenceNumber> {
    let sui_client: sui_sdk::SuiClient = SuiClientBuilder::default().build(config.full_node_url.as_str()).await?;
    let read_api = sui_client.read_api();
    let object_json =
        read_api.get_object_with_options(id, SuiObjectDataOptions::bcs_lossless()).await.expect("Cannot get object");
    let object = object_json.into_object().expect("Cannot make into object data");
    ...
    full_check_point.checkpoint_summary.verify_with_contents(&committee, Some(&full_check_point.checkpoint_contents))?;

    if full_check_point.transactions.iter().any(|t| *t.transaction.digest() == object.previous_transaction) {
        Ok(seq)
    } else {
