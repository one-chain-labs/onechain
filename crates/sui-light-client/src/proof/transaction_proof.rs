...
impl TransactionProof {
    pub fn new(tx_digest: TransactionDigest, checkpoint: &CheckpointData, add_events: bool) -> ProofResult<Self> {
        let tx = checkpoint
            .transactions
            .iter()
    ...
    fn verify_events(&self, target_events: &Vec<(EventID, Event)>, tx_digest: &TransactionDigest) -> ProofResult<()> {
...
impl ProofContentsVerifier for TransactionProof {
    fn verify(self, targets: &ProofTarget, summary: &VerifiedCheckpoint) -> ProofResult<()> {
        ...
        if !self.checkpoint_contents.enumerate_transactions(summary).any(|x| x.1 == &digests) {
