...
use sui_types::{
    ...
    digests::{CheckpointArtifactsDigest, Digest, ObjectDigest},
    full_checkpoint_content::CheckpointData,
    messages_checkpoint::{CheckpointArtifacts, VerifiedCheckpoint},
};
...
impl OCSTarget {
    pub fn new_non_inclusion_target(id: ObjectID) -> Self {
        Self { object_ref: Self::get_dummy_object_ref(id), target_type: OCSTargetType::NonInclusion }
    }

    pub fn new_inclusion_target(object_ref: ObjectRef) -> Self {
        Self { object_ref, target_type: OCSTargetType::Inclusion }
    }
...
impl ModifiedObjectTree {
    pub fn new(artifacts: &CheckpointArtifacts) -> ProofResult<Self> {
        ...
        let object_states = artifacts.object_states().map_err(|e| ProofError::GeneralError(e.to_string()))?;
        let leaves = object_states.iter().map(|(id, (seq, digest))| (*id, *seq, *digest)).collect::<Vec<_>>();
        ...
        for (i, id) in leaves.iter().map(|(id, _, _)| id).enumerate() {
            ...
            if ret.is_some() {
                return Err(ProofError::GeneralError(format!("Object ID {} appears more than once", id)));
            }
        }

        // Build the Merkle tree from the leaves.
        let tree =
            MerkleTree::build_from_unserialized(leaves.iter()).map_err(|e| ProofError::GeneralError(e.to_string()))?;
        let tree_root = Digest::new(tree.root().bytes());

        Ok(ModifiedObjectTree { leaves, object_pos_map, tree, tree_root })
    }
    ...
    pub fn get_inclusion_proof(&self, object_ref: ObjectRef) -> ProofResult<OCSInclusionProof> {
        ...
        let index =
            self.object_pos_map.get(&id).ok_or(ProofError::GeneralError(format!("Object ID {} not found", id)))?;

        // Get the Merkle proof for the object.
        let proof = self.tree.get_proof(*index).map_err(|e| ProofError::GeneralError(e.to_string()))?;
        Ok(OCSInclusionProof { merkle_proof: proof, leaf_index: *index, tree_root: self.tree_root })
    }

    pub fn get_non_inclusion_proof(&self, object_ref: ObjectRef) -> ProofResult<OCSNonInclusionProof> {
        // Sanity check: Object should not be in checkpoint.
        if self.is_object_in_checkpoint(object_ref.0) {
            return Err(ProofError::GeneralError(format!("Object ID {} is in checkpoint", object_ref.0)));
        }
        ...
        Ok(OCSNonInclusionProof { non_inclusion_proof, tree_root: self.tree_root })
    }
}
        ...
        match target {
            ProofTarget::ObjectCheckpointState(target) => {
                let actual_artifacts_digest =
                    summary.data().checkpoint_artifacts_digest().map_err(|e| ProofError::GeneralError(e.to_string()))?;

                let expected_artifacts_digest =
                    CheckpointArtifactsDigest::from_artifact_digests(vec![self.get_artifact_digest()])
                        .map_err(|e| ProofError::GeneralError(e.to_string()))?;
        ...
        match self.target_type {
            ...
            OCSTargetType::NonInclusion => {
                ...
                Ok(Proof {
                    targets: ProofTarget::ObjectCheckpointState(self.clone()),
                    checkpoint_summary: checkpoint.checkpoint_summary.clone(),
                    proof_contents: ProofContents::ObjectCheckpointStateProof(OCSProof::NonInclusion(proof)),
                })
            }
        }
    }
