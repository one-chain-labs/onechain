...
use crate::{config::Config, graphql::query_last_checkpoint_of_epoch, object_store::SuiObjectStore};
...
pub fn write_checkpoint_list(config: &Config, checkpoints_list: &CheckpointsList) -> Result<()> {
    ...
    writer.write_all(&bytes).map_err(|e| anyhow!("Unable to serialize checkpoint list: {}", e))
}

pub fn read_checkpoint(config: &Config, seq: u64) -> Result<Envelope<CheckpointSummary, AuthorityQuorumSignInfo<true>>> {
...
async fn sync_checkpoint_list_to_latest(config: &Config) -> anyhow::Result<CheckpointsList> {
    ...
    let graphql_list = if config.graphql_url.is_some() {
        match sync_checkpoint_list_to_latest_using_graphql(config).await {
            ...
            Err(e) => {
                ...
                CheckpointsList { checkpoints: vec![] }
            }
        }
    } else {
        CheckpointsList { checkpoints: vec![] }
    ...
    let archive_list =
        match sync_checkpoint_list_to_latest_using_checkpoint_bucket(config.object_store_url.clone()).await {
            Ok(list) => list,
            Err(e) => {
    ...
    if graphql_list.checkpoints.is_empty() && archive_list.checkpoints.is_empty() {
        return Err(anyhow!("Could not retrieve any checkpoints from configured sources"));
    }

    let merged_checkpoints = merge_checkpoint_lists(&graphql_list, &archive_list);
    Ok(CheckpointsList { checkpoints: merged_checkpoints })
}
...
fn merge_checkpoint_lists(list1: &CheckpointsList, list2: &CheckpointsList) -> Vec<u64> {
    ...
    let unique_checkpoints: HashSet<u64> = list1.checkpoints.iter().chain(list2.checkpoints.iter()).copied().collect();
...
async fn sync_checkpoint_list_to_latest_using_checkpoint_bucket(archive_url: String) -> anyhow::Result<CheckpointsList> {
    info!("Syncing checkpoints from Archive store");
    let checkpoints = end_of_epoch_data(archive_url, vec![], CHECKPOINT_BUCKET_TIMEOUT_SECS).await?;
...
async fn sync_checkpoint_list_to_latest_using_graphql(config: &Config) -> anyhow::Result<CheckpointsList> {
    ...
    if checkpoints_list.checkpoints.is_empty() {
        return Err(anyhow!("Empty checkpoint list and no initial checkpoint to start from"));
    }
    ...
    let summary = object_store.download_checkpoint_summary(*latest_in_list).await?;
    ...
    while last_epoch + 1 < latest.epoch() {
        ...
        info!("Last Epoch: {} Last Checkpoint: {}", target_epoch, target_last_checkpoint_number);
    }
...
pub async fn check_and_sync_checkpoints(config: &Config) -> anyhow::Result<()> {
    ...
    for ckp_id in &checkpoints_list.checkpoints {
        ...
        info!("Epoch: {} Checkpoint ID: {}", summary.epoch(), summary.digest());
...
mod tests {
    ...
    fn create_test_config() -> (Config, TempDir) {
        ...
        let config = Config { checkpoint_summary_dir: temp_dir.path().to_path_buf(), ..Default::default() };
        (config, temp_dir)
    }
    ...
    fn test_checkpoint_list_read_write() {
        ...
        let test_list = CheckpointsList { checkpoints: vec![1, 2, 3] };
    ...
    fn test_checkpoint_read_write() {
        ...
        assert_eq!(test_summary.sequence_number(), read_summary.sequence_number());
    }
}
