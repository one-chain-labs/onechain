...
use sui_types::{full_checkpoint_content::CheckpointData, messages_checkpoint::CertifiedCheckpointSummary};
use tracing::info;
use url::Url;
...
impl SuiObjectStore {
    ...
    pub async fn download_checkpoint_summary(&self, checkpoint_number: u64) -> Result<CertifiedCheckpointSummary> {
    ...
    pub async fn get_full_checkpoint(&self, checkpoint_number: u64) -> Result<CheckpointData> {
        ...
        let response =
            self.store.get(&path).await.map_err(|_| anyhow!("Cannot get full checkpoint from object store"))?;
        let bytes = response.bytes().await?;
...
pub trait ObjectStoreExt {
    async fn get_checkpoint_summary(&self, checkpoint_number: u64) -> Result<CertifiedCheckpointSummary>;
}
...
pub async fn download_checkpoint_summary(config: &Config, checkpoint_number: u64) -> Result<CertifiedCheckpointSummary> {
