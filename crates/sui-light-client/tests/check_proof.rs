...
async fn test_object_target_fail_no_data() {
    ...
    let bad_proof = Proof {
        ...
        targets: ProofTarget::Objects(ObjectsTarget { objects: vec![(sample_ref, sample_object)] }),
    };
...
async fn test_object_target_success() {
    ...
    let target = ProofTarget::Objects(ObjectsTarget { objects: vec![(sample_ref, sample_object)] });
