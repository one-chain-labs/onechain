...
fn get_all_modified_objects(checkpoint: &CheckpointData) -> Result<ModifiedObjectTree, anyhow::Error> {
...
fn test_derive_artifacts() {
    ...
    println!("Checkpoint sequence number: {:?}", checkpoint.checkpoint_summary.sequence_number());
    let artifacts = CheckpointArtifacts::from(&checkpoint);
    let artifacts_digest = artifacts.digest().unwrap();

    assert_eq!(checkpoint.checkpoint_summary.data().checkpoint_commitments.len(), 1);
    assert_eq!(
...
fn test_get_object_non_inclusion_proof() {
    ...
    for key in obj_test_cases.iter() {
        ...
        if all_objects.is_object_in_checkpoint(*key) {
            ...
            assert!(proof.is_err(), "Should not be able to get non-inclusion proof for included object");
        } else {
...
fn test_serialization_roundtrip() {
    ...
    let object_ref = all_objects.leaves.first().expect("No objects in the checkpoint");
    ...
    let bcs_deserialized: Proof = bcs::from_bytes(&bcs_serialized).expect("Should deserialize with BCS");
    ...
    if !all_objects.is_object_in_checkpoint(non_existent_id) {
        ...
        let proof = target.construct(&checkpoint).expect("Should be able to get non-inclusion proof");
