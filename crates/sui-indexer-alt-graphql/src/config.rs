...
impl HealthLayer {
    pub(crate) fn finish(self, base: HealthConfig) -> HealthConfig {
        HealthConfig {
            max_checkpoint_lag: self.max_checkpoint_lag_ms.map(Duration::from_millis).unwrap_or(base.max_checkpoint_lag),
        }
    }
}
...
impl Limits {
    ...
    pub(crate) fn pagination(&self) -> PaginationConfig {
        PaginationConfig::new(
            ...
            PageLimits { default: self.default_page_size, max: self.max_page_size },
            BTreeMap::from([
                (("TransactionEffects", "objectChanges"), PageLimits {
                    default: self.page_size_override_fx_object_changes,
                    max: self.page_size_override_fx_object_changes,
                }),
                (("Query", "packages"), PageLimits {
...
impl LimitsLayer {
    pub(crate) fn finish(self, base: Limits) -> Limits {
        Limits {
            ...
            max_query_payload_size: self.max_query_payload_size.unwrap_or(base.max_query_payload_size),
            ...
            page_size_override_packages: self.page_size_override_packages.unwrap_or(base.page_size_override_packages),
            max_type_argument_depth: self.max_type_argument_depth.unwrap_or(base.max_type_argument_depth),
            max_type_argument_width: self.max_type_argument_width.unwrap_or(base.max_type_argument_width),
            max_type_nodes: self.max_type_nodes.unwrap_or(base.max_type_nodes),
            max_move_value_depth: self.max_move_value_depth.unwrap_or(base.max_move_value_depth),
            max_move_value_bound: self.max_move_value_bound.unwrap_or(base.max_move_value_bound),
            max_display_field_depth: self.max_display_field_depth.unwrap_or(base.max_display_field_depth),
            max_display_output_size: self.max_display_output_size.unwrap_or(base.max_display_output_size),
            max_disassembled_module_size: self.max_disassembled_module_size.unwrap_or(base.max_disassembled_module_size),
        }
    }
}
...
impl ZkLoginLayer {
    pub(crate) fn finish(self, base: ZkLoginConfig) -> ZkLoginConfig {
        ZkLoginConfig {
            ...
            max_epoch_upper_bound_delta: self.max_epoch_upper_bound_delta.unwrap_or(base.max_epoch_upper_bound_delta),
        }
    }
}
...
impl From<HealthConfig> for HealthLayer {
    fn from(value: HealthConfig) -> Self {
        Self { max_checkpoint_lag_ms: Some(value.max_checkpoint_lag.as_millis() as u64) }
    }
}
...
impl From<WatermarkConfig> for WatermarkLayer {
    fn from(value: WatermarkConfig) -> Self {
        Self { watermark_polling_interval_ms: Some(value.watermark_polling_interval.as_millis() as u64) }
    }
}

impl From<ZkLoginConfig> for ZkLoginLayer {
    fn from(value: ZkLoginConfig) -> Self {
        Self { env: Some(value.env), max_epoch_upper_bound_delta: Some(value.max_epoch_upper_bound_delta) }
    }
}

impl Default for HealthConfig {
    fn default() -> Self {
        Self { max_checkpoint_lag: Duration::from_secs(300) }
    }
}

impl Default for Limits {
    fn default() -> Self {
        let max_tx_size_bytes =
            max_across_protocol(ProtocolConfig::max_tx_size_bytes_as_option).unwrap_or(u32::MAX as u64) as u32;

        let max_type_argument_depth =
            max_across_protocol(ProtocolConfig::max_type_argument_depth_as_option).unwrap_or(u32::MAX) as usize;

        let max_type_argument_width = max_across_protocol(ProtocolConfig::max_generic_instantiation_length_as_option)
            .unwrap_or(u32::MAX as u64) as usize;

        let max_type_nodes =
            max_across_protocol(ProtocolConfig::max_type_nodes_as_option).unwrap_or(u32::MAX as u64) as usize;

        let max_move_value_depth =
            max_across_protocol(ProtocolConfig::max_move_value_depth_as_option).unwrap_or(u32::MAX as u64) as usize;
...
impl Default for WatermarkConfig {
    fn default() -> Self {
        Self { watermark_polling_interval: Duration::from_millis(500) }
    }
}
...
impl Default for ZkLoginConfig {
    fn default() -> Self {
        Self {
            env: ZkLoginEnv::Prod,
            max_epoch_upper_bound_delta: max_across_protocol(ProtocolConfig::zklogin_max_epoch_upper_bound_delta),
        }
    }
}
