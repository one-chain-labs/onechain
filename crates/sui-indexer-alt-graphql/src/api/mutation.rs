...
use sui_types::{crypto::ToFromBytes, signature::GenericSignature, transaction::TransactionData};
...
impl Mutation {
    ...
    async fn execute_transaction(
        ...
        for (index, sig_base64) in signatures.iter().enumerate() {
            ...
            let signature: GenericSignature = GenericSignature::from_bytes(sig_bytes)
                .map_err(|err| bad_user_input(TransactionInputError::InvalidSignatureFormat { index, err }))?;
            parsed_signatures.push(signature);
        }

        // Execute transaction - capture gRPC errors for ExecutionResult.errors
        match fullnode_client.execute_transaction(tx_data.clone(), parsed_signatures.clone()).await {
            Ok(response) => {
                let scope = Scope::new(ctx)?;
                let effects = TransactionEffects::from_execution_response(scope, response, tx_data, parsed_signatures)
                    .map_err(upcast)?;

                Ok(ExecutionResult { effects: Some(effects), errors: None })
            }
            Err(GrpcExecutionError(status)) => {
                Ok(ExecutionResult { effects: None, errors: Some(vec![status.to_string()]) })
            }
            Err(other_error) => Err(anyhow!(other_error).context("Failed to execute transaction").into()),
        }
    }
}
