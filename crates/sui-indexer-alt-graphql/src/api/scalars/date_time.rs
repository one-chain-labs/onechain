...
impl DateTime {
    ...
    pub(crate) fn from_ms(timestamp_ms: i64) -> Result<Self, RpcError> {
        Ok(Self(
            chrono::Utc.timestamp_millis_opt(timestamp_ms).single().context("Cannot convert timestamp into DateTime")?,
        ))
    }
}
...
impl FromStr for DateTime {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        Ok(DateTime(s.parse().map_err(|e| InputValueError::custom(format!("Error parsing DateTime: {e}")))?))
    }
}
