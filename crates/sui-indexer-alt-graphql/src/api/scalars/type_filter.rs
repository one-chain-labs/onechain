...
use sui_types::{parse_sui_address, parse_sui_module_id, parse_sui_struct_tag, parse_sui_type_tag, TypeTag};
...
impl TypeFilter {
    ...
    pub(crate) fn intersect(self, other: Self) -> Option<Self> {
        ...
        match (&self, &other) {
            ...
            (F::Type(t), F::Module(p, m)) => ((p, m.as_str()) == (&t.address.into(), t.module.as_str())).then_some(self),

            (F::Type(t), F::Type(u)) if t.type_params.is_empty() => {
                ((&t.address, &t.module, &t.name) == (&u.address, &u.module, &u.name)).then_some(other)
            }

            (F::Type(t), F::Type(u)) if u.type_params.is_empty() => {
                ((&t.address, &t.module, &t.name) == (&u.address, &u.module, &u.name)).then_some(self)
            }
...
impl FromStr for TypeFilter {
    ...
    fn from_str(s: &str) -> Result<Self, TypeFilterError> {
        if let Ok(tag) = parse_sui_struct_tag(s) {
            ...
            Ok(TypeFilter::Module(SuiAddress::from(*module.address()), module.name().to_string()))
...
mod tests {
    ...
    fn test_parse_type_filter() {
        ...
        let filter = TypeFilter::from_str("0x2::coin::Coin<0x2::oct::OCT>").unwrap();
        assert!(matches!(filter, TypeFilter::Type(ref t) if !t.type_params.is_empty()));
    }
    ...
    fn test_type_filter_intersect() {
        ...
        let type_with_params = TypeFilter::from_str("0x2::coin::Coin<0x2::oct::OCT>").unwrap();

        // Package intersect with module in same package
        assert!(matches!(pkg.clone().intersect(module.clone()), Some(TypeFilter::Module(_, _))));

        // Module intersect with type in same module
        assert!(matches!(module.clone().intersect(type_no_params.clone()), Some(TypeFilter::Type(_))));
