...
use super::{
    scalars::{digest::Digest, domain::Domain, sui_address::SuiAddress, type_filter::TypeInput, uint53::UInt53},
    types::{
        address::Address,
...
use crate::{
    api::{
        ...
        types::{epoch::CEpoch, simulation_result::SimulationResult, transaction_effects::TransactionEffects},
    },
...
impl Query {
    ...
    async fn checkpoint(
        ...
        Ok(Checkpoint::with_sequence_number(scope, sequence_number.map(|s| s.into())))
    }
    ...
    async fn checkpoints(
        ...
        Checkpoint::paginate(ctx, scope, page, filter).await.map(Some)
    }
    ...
    async fn epoch(&self, ctx: &Context<'_>, epoch_id: Option<UInt53>) -> Result<Option<Epoch>, RpcError> {
        let scope = self.scope(ctx)?;
    ...
    async fn events(
        ...
        Event::paginate(ctx, scope, page, filter.unwrap_or_default()).await.map(Some)
    }
    ...
    async fn multi_get_checkpoints(
        ...
        let scope = self.scope(ctx)?;
        Ok(keys.into_iter().map(|k| Checkpoint::with_sequence_number(scope.clone(), Some(k.into()))).collect())
    }
    ...
    async fn multi_get_epochs(&self, ctx: &Context<'_>, keys: Vec<UInt53>) -> Result<Vec<Option<Epoch>>, RpcError> {
        let scope = self.scope(ctx)?;
        let epochs = keys.into_iter().map(|k| Epoch::fetch(ctx, scope.clone(), Some(k)));
    ...
    async fn multi_get_objects(
        ...
        let scope = self.scope(ctx)?;
        let objects = keys.into_iter().map(|k| Object::by_key(ctx, scope.clone(), k));
    ...
    async fn multi_get_packages(
        ...
        let scope = self.scope(ctx)?;
        let packages = keys.into_iter().map(|k| MovePackage::by_key(ctx, scope.clone(), k));
    ...
    async fn multi_get_transactions(
        ...
        let scope = self.scope(ctx)?;
        let transactions = keys.into_iter().map(|d| Transaction::fetch(ctx, scope.clone(), d));
    ...
    async fn multi_get_transaction_effects(
        ...
        let scope = self.scope(ctx)?;
        let effects = keys.into_iter().map(|d| TransactionEffects::fetch(ctx, scope.clone(), d));
    ...
    async fn multi_get_types(
        ...
        let types = keys.into_iter().map(|t| async move { MoveType::canonicalize(t.into(), self.scope(ctx)?).await });
    ...
    async fn object(
        ...
        Object::by_key(ctx, self.scope(ctx)?, ObjectKey { address, version, root_version, at_checkpoint }).await
    }
    ...
    async fn objects(
        ...
        Ok(Some(Object::paginate_live(ctx, self.scope(ctx)?, page, filter).await?))
    }
    ...
    async fn object_versions(
        ...
        Ok(Some(
            Object::paginate_by_version(ctx, self.scope(ctx)?, page, address.into(), filter.unwrap_or_default()).await?,
        ))
    }
    ...
    async fn package(
        ...
        MovePackage::by_key(ctx, self.scope(ctx)?, PackageKey { address, version, at_checkpoint }).await
    }
    ...
    async fn packages(
        ...
        Ok(Some(MovePackage::paginate_by_checkpoint(ctx, self.scope(ctx)?, page, filter.unwrap_or_default()).await?))
    }
    ...
    async fn package_versions(
        ...
        Ok(Some(
            MovePackage::paginate_by_version(ctx, self.scope(ctx)?, page, address.into(), filter.unwrap_or_default())
                .await?,
        ))
    }
    ...
    async fn transaction(&self, ctx: &Context<'_>, digest: Digest) -> Result<Option<Transaction>, RpcError> {
    ...
    async fn transactions(
        ...
        let filter = filter.unwrap_or_default();
        Transaction::paginate(ctx, scope, page, filter).await.map(Some)
    }
    ...
    async fn type_(&self, ctx: &Context<'_>, type_: TypeInput) -> Result<Option<MoveType>, RpcError<move_type::Error>> {
    ...
    async fn simulate_transaction(
        ...
        let json_value: serde_json::Value =
            transaction.try_into().map_err(|err| bad_user_input(TransactionInputError::InvalidTransactionJson(err)))?;
        ...
        match fullnode_client.simulate_transaction(proto_tx).await {
            ...
            Err(GrpcExecutionError(status)) => {
                Ok(SimulationResult { effects: None, outputs: None, error: Some(status.to_string()) })
            }
            Err(other_error) => Err(anyhow!(other_error).context("Failed to simulate transaction").into()),
        }
    }
    ...
    async fn verify_zk_login_signature(
        ...
        zklogin::verify_signature(ctx, self.scope(ctx)?, bytes, signature, intent_scope, author).await
    }
}
