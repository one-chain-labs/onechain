...
impl Query {
    ...
    async fn simulate_transaction(
        ...
        let tx_data: TransactionData = {
            ...
            bcs::from_bytes(bytes).map_err(|err| bad_user_input(TransactionInputError::InvalidTransactionBcs(err)))?
        };
        ...
        match fullnode_client.simulate_transaction(tx_data.clone()).await {
            ...
            Err(GrpcExecutionError(status)) => {
                Ok(SimulationResult { effects: None, events: None, outputs: None, error: Some(status.to_string()) })
            }
