...
impl Query {
    ...
    async fn checkpoint(
        ...
        let sequence_number = sequence_number.map(|s| s.into()).unwrap_or(scope.checkpoint_viewed_at());
    ...
    async fn multi_get_checkpoints(
        ...
        let scope = self.scope(ctx)?;
        Ok(keys.into_iter().map(|k| Checkpoint::with_sequence_number(scope.clone(), k.into())).collect())
    }
