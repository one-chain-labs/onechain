...
impl SimulationResult {
    ...
    pub(crate) fn from_simulation_response(
        ...
        let executed_transaction = response.transaction.as_ref().context("No transaction in simulation response")?;

        let events_bcs = executed_transaction.events.as_ref().and_then(|events| events.bcs.as_ref());

        let transaction_events =
            events_bcs.map(|bcs| bcs.deserialize()).transpose().context("Failed to deserialize events BCS")?;
        ...
        let outputs =
            Some(response.outputs.into_iter().map(|output| CommandResult::from_proto(output, scope.clone())).collect());

        Ok(Self { effects, events, outputs, error: None })
    }
}
