...
pub(crate) async fn events_from_sequence_numbers(
    ...
    let events = digests.into_iter().map(|(tx_sequence_number, digest)| {
        let events = digest_to_events.get(&digest).context("Failed to get events")?;
...
fn tx_events_paginated<'e>(
    scope: &Scope,
    page: &Page<CEvent>,
    contents: impl Iterator<Item = anyhow::Result<(u64, TransactionDigest, &'e TransactionEventsContents)>>,
    filter: &EventFilter,
) -> Result<Vec<(EventCursor, Event)>, RpcError> {
    ...
    'outer: for events in contents {
        ...
        for ev_sequence_number in bounds {
            let event_cursor = EventCursor { tx_sequence_number, ev_sequence_number: ev_sequence_number as u64 };
