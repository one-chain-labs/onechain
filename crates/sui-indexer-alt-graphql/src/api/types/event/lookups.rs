...
pub(crate) async fn events_from_sequence_numbers(
    ...
    let tx_digest_keys: Vec<TxDigestKey> = tx_sequence_numbers.iter().map(|r| TxDigestKey(*r)).collect();

    let sequence_to_digest = pg_loader.load_many(tx_digest_keys).await.context("Failed to load transaction digests")?;
    ...
    let mut results =
        tx_events_paginated(scope, page, tx_sequence_numbers, &sequence_to_digest, &digest_to_events, filter)?;
...
fn tx_events_paginated(
    ...
    for &tx_seq_num in tx_sequence_numbers {
        ...
        let stored_tx_digest = sequence_to_digest.get(&key).context("Failed to get transaction digest")?;
        ...
        let contents = digest_to_events.get(&tx_digest).context("Failed to get events")?;
        ...
        for ev_seq_num in event_bounds {
            let event_cursor = EventCursor { tx_sequence_number: tx_seq_num, ev_sequence_number: ev_seq_num as u64 };
