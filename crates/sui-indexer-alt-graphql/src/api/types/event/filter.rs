...
use crate::{
    api::{
        scalars::{module_filter::ModuleFilter, sui_address::SuiAddress, type_filter::TypeFilter, uint53::UInt53},
...
pub(crate) struct EventFilter {
    ...
    ///
    /// Generic types can be queried by either the generic type name, e.g. `0x2::coin::Coin`, or by the full type name, such as `0x2::coin::Coin<0x2::oct::OCT>`.
...
impl EventFilter {
    ...
    pub(crate) fn query<'q>(&self) -> Result<Query<'q>, RpcError> {
        let table = match (&self.module, &self.type_) {
            (Some(_), Some(_)) => {
                return Err(feature_unavailable("Filtering by both emitting module and event type is not supported"))
            }
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if self.sender.is_some_and(|s| s != SuiAddress::from(event.sender)) {
            return false;
        }
        ...
        if let Some(type_) = &self.type_ {
            ...
            if type_.type_params().is_some_and(|p| p != event.type_.type_params.as_slice()) {
                return false;
            }
        }
...
pub(super) fn tx_ev_bounds(page: &Page<CEvent>, tx_sequence_number: u64, event_count: usize) -> Range<usize> {
