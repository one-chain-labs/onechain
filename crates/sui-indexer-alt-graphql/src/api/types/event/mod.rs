...
use sui_types::{base_types::SuiAddress as NativeSuiAddress, digests::TransactionDigest, event::Event as NativeEvent};
...
impl Event {
    ...
    async fn sender(&self) -> Option<Address> {
        ...
        Some(Address::with_address(self.scope.clone(), self.native.sender))
    }
    ...
    async fn transaction(&self) -> Option<Transaction> {
        Some(Transaction::with_id(self.scope.clone(), self.transaction_digest))
    }
    ...
    async fn transaction_module(&self) -> Option<MoveModule> {
        ...
        Some(MoveModule::with_fq_name(package, self.native.transaction_module.to_string()))
    }
}
...
impl Event {
    ...
    pub(crate) async fn paginate(
        ...
        let events = lookups::events_from_sequence_numbers(&scope, ctx, &page, &tx_sequence_numbers, &filter).await?;

        let (has_prev, has_next, edges) = page.paginate_results(events, |(cursor, _)| JsonCursor::new(*cursor));
