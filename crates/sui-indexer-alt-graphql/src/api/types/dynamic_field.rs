...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Result<Option<DynamicFieldValue>, RpcError> {
        ...
        let address: SuiAddress =
            bcs::from_bytes(&native.value_bytes).context("Failed to deserialize dynamic object field ID")?;
        ...
        Ok(Some(DynamicFieldValue::MoveObject(MoveObject::from_super(object))))
    }
}
...
impl DynamicField {
    ...
    pub(crate) fn from_super(super_: MoveObject) -> Self {
        Self { super_, native: OnceCell::new() }
    }
    ...
    pub(crate) async fn by_name(
        ...
        let field_id =
            derive_dynamic_field_id(parent, &type_, &name.bcs.0).context("Failed to derive dynamic field ID")?.into();
    ...
    pub(crate) async fn paginate(
        ...
        for edge in objects.edges {
            ...
            dynamic_fields.edges.push(Edge::new(edge.cursor, dynamic_field));
        }
    ...
    pub(crate) async fn native(&self, ctx: &Context<'_>) -> Result<&Option<NativeField>, RpcError> {
        self.native
            .get_or_try_init(async || {
                ...
                let DFV::Field { kind, name_layout, name_bytes, value_layout, value_bytes, .. } =
