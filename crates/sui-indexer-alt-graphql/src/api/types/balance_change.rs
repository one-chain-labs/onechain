...
use sui_types::{object::Owner, TypeTag};
...
impl BalanceChange {
    ...
    pub(crate) fn from_grpc(scope: Scope, grpc: &GrpcBalanceChange) -> Result<Self, RpcError> {
        ...
        let coin_type: TypeTag = grpc.coin_type().parse().context("Failed to parse coin type")?;
    ...
    pub(crate) fn from_stored(scope: Scope, stored: StoredBalanceChange) -> Result<Self, RpcError> {
        let StoredBalanceChange::V1 { owner, coin_type, amount } = stored;

        // Extract address from owner
        let address = match owner {
            Owner::AddressOwner(addr) | Owner::ObjectOwner(addr) | Owner::ConsensusAddressOwner { owner: addr, .. } => {
                Some(addr)
            }
