...
impl ServiceConfig {
    ...
    async fn max_transaction_payload_size(&self, ctx: &Context<'_>) -> Result<Option<u32>, RpcError> {
        let limits: &Limits = ctx.data()?;
    ...
    async fn default_page_size(&self, ctx: &Context<'_>, type_: String, field: String) -> Result<Option<u32>, RpcError> {
        let registry = &ctx.schema_env.registry;

        if !registry.concrete_type_by_name(&type_).and_then(|t| t.field_by_name(&field)).is_some_and(is_connection) {
            return Ok(None);
        }
    ...
    async fn max_page_size(&self, ctx: &Context<'_>, type_: String, field: String) -> Result<Option<u32>, RpcError> {
        let registry = &ctx.schema_env.registry;
    ...
    async fn max_disassembled_module_size(&self, ctx: &Context<'_>) -> Result<Option<usize>, RpcError> {
        let limits: &Limits = ctx.data()?;
    ...
    async fn available_range(
        ...
        AvailableRange::new(ctx, &self.scope, AvailableRangeKey { type_, field, filters })
    }
}
