...
use crate::{
    api::{scalars::base64::Base64, types::epoch::Epoch},
    scope::Scope,
};
...
impl ValidatorAggregatedSignature {
    ...
    async fn epoch(&self) -> Option<Epoch> {
        Some(Epoch::with_id(self.scope.clone(), self.authority_info.epoch))
    }
...
impl ValidatorAggregatedSignature {
    pub(crate) fn with_authority_info(scope: Scope, authority_info: AuthorityStrongQuorumSignInfo) -> Self {
        Self { authority_info, scope }
    }
}
