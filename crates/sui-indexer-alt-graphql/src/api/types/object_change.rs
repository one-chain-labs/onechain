...
impl ObjectChange {
    ...
    async fn input_state(&self) -> Option<Object> {
        let NativeObjectChange { id, input_version: Some(version), input_digest: Some(digest), .. } = self.native else {
            return None;
        };
    ...
    async fn output_state(&self) -> Option<Object> {
        let NativeObjectChange { id, output_version: Some(version), output_digest: Some(digest), .. } = self.native
        else {
            return None;
        };
