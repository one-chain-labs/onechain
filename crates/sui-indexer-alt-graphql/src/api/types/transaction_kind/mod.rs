    ...
    pub fn from(kind: NativeTransactionKind, scope: Scope) -> Option<Self> {
        ...
        match kind {
            ...
            K::ConsensusCommitPrologue(ccp) => {
                Some(T::ConsensusCommitPrologue(ConsensusCommitPrologueTransaction::from_v1(ccp, scope)))
            }
            K::ConsensusCommitPrologueV2(ccp) => {
                Some(T::ConsensusCommitPrologue(ConsensusCommitPrologueTransaction::from_v2(ccp, scope)))
            }
            K::ConsensusCommitPrologueV3(ccp) => {
                Some(T::ConsensusCommitPrologue(ConsensusCommitPrologueTransaction::from_v3(ccp, scope)))
            }
            K::ConsensusCommitPrologueV4(ccp) => {
                Some(T::ConsensusCommitPrologue(ConsensusCommitPrologueTransaction::from_v4(ccp, scope)))
            }
            K::ChangeEpoch(ce) => Some(T::ChangeEpoch(ChangeEpochTransaction { native: ce, scope })),
            K::RandomnessStateUpdate(rsu) => {
                Some(T::RandomnessStateUpdate(RandomnessStateUpdateTransaction { native: rsu }))
            }
            K::AuthenticatorStateUpdate(asu) => {
                Some(T::AuthenticatorStateUpdate(AuthenticatorStateUpdateTransaction { native: asu, scope }))
            }
            K::EndOfEpochTransaction(eoe) => Some(T::EndOfEpoch(EndOfEpochTransaction { native: eoe, scope })),
            K::ProgrammableTransaction(pt) => Some(T::Programmable(ProgrammableTransaction { native: pt, scope })),
            K::ProgrammableSystemTransaction(pt) => Some(T::ProgrammableSystem(ProgrammableSystemTransaction {
