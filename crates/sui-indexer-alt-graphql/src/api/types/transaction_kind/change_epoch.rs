...
impl ChangeEpochTransaction {
    ...
    async fn protocol_configs(&self) -> Option<ProtocolConfigs> {
        Some(ProtocolConfigs::with_protocol_version(self.native.protocol_version.as_u64()))
    }
    ...
    async fn epoch_start_timestamp(&self) -> Result<Option<DateTime>, RpcError> {
        Ok(Some(DateTime::from_ms(self.native.epoch_start_timestamp_ms as i64)?))
    }
    ...
    async fn system_packages(
        ...
        for edge in cursors.edges {
            ...
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), package));
        }
