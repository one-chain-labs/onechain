...
impl ProgrammableTransaction {
    ...
    async fn inputs(
        ...
        for edge in cursors.edges {
            let input = TransactionInput::from(self.native.inputs[*edge.cursor].clone(), self.scope.clone());
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), input));
        }
    ...
    async fn commands(
        ...
        for edge in cursors.edges {
            let command = Command::from(self.scope.clone(), self.native.commands[*edge.cursor].clone());

            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), command));
        }
