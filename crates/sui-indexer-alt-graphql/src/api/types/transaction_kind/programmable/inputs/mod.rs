...
impl TransactionInput {
    pub fn from(input: sui_types::transaction::CallArg, scope: Scope) -> Self {
        ...
        match input {
            CallArg::Pure(bytes) => Self::Pure(Pure { bytes: Some(Base64::from(bytes)) }),
            CallArg::Object(obj_arg) => match obj_arg {
                ObjectArg::ImmOrOwnedObject((object_id, version, digest)) => {
                    Self::OwnedOrImmutable(OwnedOrImmutable::from_object_ref(object_id, version, digest, scope))
                }
                ObjectArg::SharedObject { id, initial_shared_version, mutable } => {
                    Self::SharedInput(SharedInput::from_shared_object(id, initial_shared_version, mutable))
                }
                ObjectArg::Receiving((object_id, version, digest)) => {
                    Self::Receiving(Receiving::from_object_ref(object_id, version, digest, scope))
                }
            },
            // TODO: Handle FundsWithdrawal
            CallArg::FundsWithdrawal(_) => {
                Self::Pure(Pure { bytes: Some(Base64::from(b"TODO: FundsWithdrawal not supported".to_vec())) })
            }
        }
    }
}
