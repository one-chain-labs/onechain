...
impl Command {
    pub fn from(scope: Scope, command: NativeCommand) -> Self {
        match command {
            NativeCommand::MakeMoveVec(type_opt, elements) => Command::MakeMoveVec(MakeMoveVecCommand {
                type_: type_opt.map(|type_input| MoveType::from_input(type_input, scope.clone())),
                elements: Some(elements.into_iter().map(TransactionArgument::from).collect()),
            }),
            NativeCommand::MoveCall(call) => Command::MoveCall(MoveCallCommand { native: *call, scope }),
            ...
            NativeCommand::TransferObjects(objects, address) => Command::TransferObjects(TransferObjectsCommand {
