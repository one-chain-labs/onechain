...
impl From<NativeArgument> for TransactionArgument {
    fn from(argument: NativeArgument) -> Self {
        match argument {
            ...
            NativeArgument::Result(cmd) => TransactionArgument::Result(TxResult { cmd: Some(cmd), ix: None }),
            NativeArgument::NestedResult(cmd, ix) => {
                TransactionArgument::Result(TxResult { cmd: Some(cmd), ix: Some(ix) })
            }
        }
    }
}
...
impl TryFrom<proto::Argument> for TransactionArgument {
    ...
    fn try_from(arg: proto::Argument) -> Result<Self, Self::Error> {
        ...
        match arg.kind() {
            ...
            ArgumentKind::Input => {
                Ok(TransactionArgument::Input(Input { ix: Some(arg.input.unwrap_or_default() as u16) }))
            }
