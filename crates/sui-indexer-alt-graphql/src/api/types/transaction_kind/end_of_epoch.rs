...
impl AuthenticatorStateExpireTransaction {
    ...
    async fn authenticator_obj_initial_shared_version(&self) -> Option<UInt53> {
        Some(self.native.authenticator_obj_initial_shared_version.value().into())
    }
}
...
impl EndOfEpochTransaction {
    ...
    async fn transactions(
        ...
        page.paginate_indices(self.native.len(), |i| {
            Ok(EndOfEpochTransactionKind::from(self.native[i].clone(), self.scope.clone()))
        })
        .map(Some)
    }
}
    ...
    pub fn from(kind: NativeEndOfEpochTransactionKind, scope: Scope) -> Self {
        ...
        match kind {
            ...
            N::AuthenticatorStateExpire(expire_data) => {
                K::AuthenticatorStateExpire(AuthenticatorStateExpireTransaction { native: expire_data, scope })
            }
            ...
            N::StoreExecutionTimeObservations(_) => {
                K::StoreExecutionTimeObservations(StoreExecutionTimeObservationsTransaction { dummy: None })
            }
            N::BridgeStateCreate(chain_id) => K::BridgeStateCreate(BridgeStateCreateTransaction { native: chain_id }),
            N::BridgeCommitteeInit(bridge_version) => K::BridgeCommitteeInit(BridgeCommitteeInitTransaction {
                bridge_object_version: Some(bridge_version.value().into()),
            }),
            N::AccumulatorRootCreate => K::AccumulatorRootCreate(AccumulatorRootCreateTransaction { dummy: None }),
            N::CoinRegistryCreate => K::CoinRegistryCreate(CoinRegistryCreateTransaction { dummy: None }),
            N::DisplayRegistryCreate => K::DisplayRegistryCreate(DisplayRegistryCreateTransaction { dummy: None }),
        }
    }
}
