...
impl GasInput {
    ...
    async fn gas_sponsor(&self) -> Option<Address> {
        (self.native.owner != SuiAddress::ZERO).then(|| Address::with_address(self.scope.clone(), self.native.owner))
    }
    ...
    async fn gas_payment(
        ...
        for edge in cursors.edges {
            ...
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), object));
        }
