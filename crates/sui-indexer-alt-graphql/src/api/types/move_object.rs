...
#[graphql(
    ...
    field(
        ...
        desc = "Whether this object can be transfered using the `TransferObjects` Programmable Transaction Command or `one::transfer::public_transfer`.\n\nBoth these operations require the object to have both the `key` and `store` abilities.",
    ),
    field(
...
impl MoveObject {
    ...
    pub(crate) async fn as_coin_metadata(&self, ctx: &Context<'_>) -> Result<Option<CoinMetadata>, RpcError> {
        CoinMetadata::from_move_object(self, ctx).await
    }

    /// Attempts to convert the object into a DynamicField.
    pub(crate) async fn as_dynamic_field(&self, ctx: &Context<'_>) -> Result<Option<DynamicField>, RpcError> {
    ...
    pub(crate) async fn contents(&self, ctx: &Context<'_>) -> Result<Option<MoveValue>, RpcError> {
        ...
        let type_ = MoveType::from_native(native.type_().clone().into(), self.super_.super_.scope.clone());
    ...
    pub(crate) async fn dynamic_fields(
        ...
        let dynamic_fields =
            DynamicField::paginate(ctx, self.super_.super_.scope.clone(), self.super_.super_.address.into(), page)
                .await?;
    ...
    pub(crate) async fn has_public_transfer(&self, ctx: &Context<'_>) -> Result<Option<bool>, RpcError> {
        ...
        Ok(type_.abilities_impl().await?.map(|s| s.has_key() && s.has_store()))
    }
...
impl MoveObject {
    ...
    pub(crate) fn from_super(super_: Object) -> Self {
        Self { super_, native: Arc::new(OnceCell::new()) }
    }
    ...
    pub(crate) async fn from_object(object: &Object, ctx: &Context<'_>) -> Result<Option<Self>, RpcError> {
        ...
        Ok(Some(Self { super_: object.clone(), native: Arc::new(OnceCell::from(Some(native.clone()))) }))
    }

    /// Get the native MoveObject, loading it lazily if needed.
    pub(crate) async fn native(&self, ctx: &Context<'_>) -> Result<&Option<NativeMoveObject>, RpcError> {
        self.native
            .get_or_try_init(async || {
                ...
                let native = contents.data.try_as_move().context("Object is not a MoveObject")?;
