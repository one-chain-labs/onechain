...
impl EpochStart {
    ...
    async fn coin_deny_list(&self, ctx: &Context<'_>) -> Result<Option<Object>, RpcError<object::Error>> {
    ...
    async fn protocol_configs(&self) -> Option<ProtocolConfigs> {
        ...
        Some(ProtocolConfigs::with_protocol_version(contents.protocol_version as u64))
    }
...
impl Epoch {
    ...
    pub(crate) fn with_id(scope: Scope, epoch_id: u64) -> Self {
        Self { epoch_id, start: EpochStart::empty(scope) }
    }
    ...
    pub(crate) async fn fetch(
        ...
        let start = EpochStart::empty(scope).fetch(ctx, epoch_id.map(|id| id.into())).await?;

        let Some(contents) = &start.contents else {
            return Ok(None);
        };

        Ok(Some(Self { epoch_id: contents.epoch as u64, start }))
    }
}
...
impl EpochStart {
    ...
    async fn fetch(&self, ctx: &Context<'_>, epoch_id: Option<u64>) -> Result<Self, RpcError> {
        ...
        Ok(Self { scope: self.scope.clone(), contents: Some(Arc::new(stored)) })
    }
}
...
impl EpochEnd {
    ...
    async fn fetch(ctx: &Context<'_>, scope: &Scope, epoch_id: u64) -> Result<Self, RpcError> {
        let pg_loader: &Arc<DataLoader<PgReader>> = ctx.data()?;
        let Some(stored) =
            pg_loader.load_one(EpochEndKey(epoch_id)).await.context("Failed to fetch epoch end information")?
        else {
        ...
        Ok(Self { contents: Some(Arc::new(stored)) })
    }
}
