...
use sui_indexer_alt_schema::{
    cp_sequence_numbers::StoredCpSequenceNumbers,
...
use crate::{
    api::{
        scalars::{big_int::BigInt, cursor::JsonCursor, date_time::DateTime, uint53::UInt53},
        types::{
            safe_mode::{from_system_state, SafeMode},
            stake_subsidy::{from_stake_subsidy_v1, StakeSubsidy},
            storage_fund::StorageFund,
            system_parameters::{from_system_parameters_v1, from_system_parameters_v2, SystemParameters},
...
impl Epoch {
    ...
    async fn checkpoints(
        ...
        let Some(filter) = filter
            .unwrap_or_default()
            .intersect(CheckpointFilter { at_epoch: Some(self.epoch_id.into()), ..Default::default() })
        else {
            return Ok(Some(Connection::new(false, false)));
        };

        Ok(Some(Checkpoint::paginate(ctx, self.scope.clone(), page, filter).await?))
    }
    ...
    async fn protocol_configs(&self, ctx: &Context<'_>) -> Result<Option<ProtocolConfigs>, RpcError> {
        let Some(start) = self.start(ctx).await? else {
            return Ok(None);
        };

        Ok(Some(ProtocolConfigs::with_protocol_version(start.protocol_version as u64)))
    }
    ...
    async fn transactions(
        ...
        let Some(checkpoint_viewed_at_exclusive_bound) = self.scope.checkpoint_viewed_at_exclusive_bound() else {
            return Ok(None);
        };
        ...
        let cp_hi = end.as_ref().map_or_else(|| checkpoint_viewed_at_exclusive_bound, |end| end.cp_hi as u64);
        ...
        Ok(Some(Transaction::paginate(ctx, self.scope.clone(), page, filter).await?))
    }
    ...
    async fn system_packages(
        ...
        Ok(Some(MovePackage::paginate_system_packages(ctx, self.scope.clone(), page, contents.cp_lo as u64).await?))
    }
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>, RpcError> {
        ...
        let validator_set = ValidatorSet::from_validator_set_v1(self.scope.clone(), validator_set_v1, report_records);
    ...
    async fn total_transactions(&self, ctx: &Context<'_>) -> Result<Option<UInt53>, RpcError> {
        let (Some(cp_sequence_numbers), Some(end)) = try_join!(self.cp_sequence_numbers(ctx), self.end(ctx))? else {
            return Ok(None);
        };
    ...
    async fn total_stake_rewards(&self, ctx: &Context<'_>) -> Result<Option<BigInt>, RpcError> {
        let Some(StoredEpochEnd { total_stake_rewards_distributed, .. }) = self.end(ctx).await? else {
            return Ok(None);
        };
    ...
    async fn total_stake_subsidies(&self, ctx: &Context<'_>) -> Result<Option<BigInt>, RpcError> {
        let Some(StoredEpochEnd { stake_subsidy_amount, .. }) = self.end(ctx).await? else {
            return Ok(None);
        };
    ...
    async fn fund_size(&self, ctx: &Context<'_>) -> Result<Option<BigInt>, RpcError> {
        let Some(StoredEpochEnd { storage_fund_balance, .. }) = self.end(ctx).await? else {
            return Ok(None);
        };
    ...
    async fn net_inflow(&self, ctx: &Context<'_>) -> Result<Option<BigInt>, RpcError> {
        let Some(StoredEpochEnd { storage_charge: Some(storage_charge), storage_rebate: Some(storage_rebate), .. }) =
            self.end(ctx).await?
        else {
            return Ok(None);
        };
    ...
    async fn system_parameters(&self, ctx: &Context<'_>) -> Result<Option<SystemParameters>, RpcError> {
        let Some(system_state) = self.system_state(ctx).await? else {
            return Ok(None);
        };
    ...
    async fn system_stake_subsidy(&self, ctx: &Context<'_>) -> Result<Option<StakeSubsidy>, RpcError> {
        let Some(system_state) = self.system_state(ctx).await? else {
            return Ok(None);
        };
    ...
    async fn live_object_set_digest(&self, ctx: &Context<'_>) -> Result<Option<String>, RpcError> {
        ...
        let commitments: Vec<CheckpointCommitment> =
            bcs::from_bytes(&end.epoch_commitments).context("Failed to deserialize epoch commitments")?;
...
impl Epoch {
    ...
    pub(crate) fn with_id(scope: Scope, epoch_id: u64) -> Self {
        Self { epoch_id, scope, start: OnceCell::new(), end: OnceCell::new(), cp_sequence_numbers: OnceCell::new() }
    }
    ...
    pub(crate) async fn fetch(
        ...
        let stored = match epoch_id {
            ...
            None => pg_loader
                ...
                .await
                .context("Failed to fetch epoch start information by checkpoint bounded start key")?,
        }
    ...
    pub(crate) async fn paginate(
        ...
        page.paginate_indices(1 + latest_epoch.epoch_id as usize, |id| Ok(Epoch::with_id(scope.clone(), id as u64)))
            .map(Some)
    }
    ...
    async fn system_state(&self, ctx: &Context<'_>) -> Result<Option<SuiSystemState>, RpcError> {
        ...
        let system_state =
            bcs::from_bytes::<SuiSystemState>(&start.system_state).context("Failed to deserialize system state")?;
    ...
    async fn cp_sequence_numbers(&self, ctx: &Context<'_>) -> Result<&Option<StoredCpSequenceNumbers>, RpcError> {
        let Some(start) = self.start(ctx).await? else {
            return Ok(&None);
        };
