...
#[graphql(
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::oct::OCT`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
impl Address {
    ...
    pub(crate) async fn as_object(&self, ctx: &Context<'_>) -> Result<Option<Object>, RpcError<object::Error>> {
        Object::by_key(ctx, self.scope.clone(), ObjectKey {
    ...
    pub(crate) async fn balances(
        ...
        Balance::paginate(ctx, self.scope.clone(), self.address, page).await.map(Some)
    }
    ...
    pub(crate) async fn dynamic_field(
        ...
        DynamicField::by_name(ctx, self.scope.clone(), self.address.into(), DynamicFieldType::DynamicField, name).await
    }
    ...
    pub(crate) async fn dynamic_fields(
        ...
        let dynamic_fields = DynamicField::paginate(ctx, self.scope.clone(), self.address.into(), page).await?;
    ...
    pub(crate) async fn dynamic_object_field(
        ...
        DynamicField::by_name(ctx, self.scope.clone(), self.address.into(), DynamicFieldType::DynamicObject, name).await
    }
    ...
    pub(crate) async fn multi_get_dynamic_fields(
        ...
        try_join_all(keys.into_iter().map(|key| {
            DynamicField::by_name(ctx, self.scope.clone(), self.address.into(), DynamicFieldType::DynamicField, key)
        }))
        .await
    }
    ...
    pub(crate) async fn multi_get_dynamic_object_fields(
        ...
        try_join_all(keys.into_iter().map(|key| {
            DynamicField::by_name(ctx, self.scope.clone(), self.address.into(), DynamicFieldType::DynamicObject, key)
        }))
        .await
    }
    ...
    pub(crate) async fn transactions(
        ...
        let address_filter = match relation {
            AddressTransactionRelationship::Sent => {
                TransactionFilter { sent_address: Some(self.address.into()), ..Default::default() }
            }
            AddressTransactionRelationship::Affected => {
                TransactionFilter { affected_address: Some(self.address.into()), ..Default::default() }
            }
        };
        ...
        Transaction::paginate(ctx, self.scope.clone(), page, filter).await.map(Some)
    }
}
