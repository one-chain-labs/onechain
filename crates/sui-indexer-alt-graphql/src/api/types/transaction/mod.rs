...
impl Transaction {
    ...
    async fn kind(&self, ctx: &Context<'_>) -> Result<Option<TransactionKind>, RpcError> {
        ...
        Ok(TransactionKind::from(transaction_data.kind().clone(), contents.scope.clone()))
    }
...
impl TransactionContents {
    ...
    async fn expiration(&self) -> Result<Option<Epoch>, RpcError> {
        ...
        match transaction_data.expiration() {
            ...
            TransactionExpiration::Epoch(epoch_id) => Ok(Some(Epoch::with_id(self.scope.clone(), *epoch_id))),
        }
    }
    ...
    async fn gas_input(&self) -> Result<Option<GasInput>, RpcError> {
        ...
        let transaction_data = content.data()?;
        Ok(Some(GasInput::from_gas_data(self.scope.clone(), transaction_data.gas_data().clone())))
    }
    ...
    async fn sender(&self) -> Result<Option<Address>, RpcError> {
        ...
        Ok((sender != NativeSuiAddress::ZERO).then(|| Address::with_address(self.scope.clone(), sender)))
    }
    ...
    async fn signatures(&self) -> Result<Vec<UserSignature>, RpcError> {
        ...
        Ok(signatures.into_iter().map(UserSignature::from_generic_signature).collect())
    }
}
...
impl Transaction {
    ...
    pub(crate) fn with_id(scope: Scope, digest: TransactionDigest) -> Self {
        Self { digest, contents: TransactionContents::empty(scope) }
    }
    ...
    pub(crate) async fn fetch(ctx: &Context<'_>, scope: Scope, digest: Digest) -> Result<Option<Self>, RpcError> {
        let contents = TransactionContents::empty(scope).fetch(ctx, digest.into()).await?;
    ...
    pub(crate) async fn paginate(
        ...
        let tx_digest_keys: Vec<TxDigestKey> = results.iter().map(|(_, sq)| TxDigestKey(*sq)).collect();

        // Load the transaction digests for the paginated tx_sequence_numbers
        let pg_loader: &Arc<DataLoader<PgReader>> = ctx.data()?;
        let digest_map = pg_loader.load_many(tx_digest_keys).await.context("Failed to load transaction digests")?;
        ...
        for (cursor, tx_sequence_number) in results {
            ...
            if let Some(stored) = digest_map.get(&key) {
                ...
                conn.edges.push(Edge::new(cursor.encode_cursor(), transaction));
            }
        }
...
async fn tx_sequence_numbers(
    ...
    let wrapped_tx_sequence_numbers: Vec<TxSequenceNumber> =
        conn.results(query).await.context("Failed to execute query")?;

    let tx_sequence_numbers = if page.is_from_front() {
        wrapped_tx_sequence_numbers.iter().map(|t| t.tx_sequence_number as u64).collect()
    } else {
        wrapped_tx_sequence_numbers.iter().rev().map(|t| t.tx_sequence_number as u64).collect()
    };
...
async fn tx_unfiltered(
    ...
    let results: Vec<TxBounds> = conn.results(query).await.context("Failed to execute query")?;
...
impl From<TransactionEffects> for Transaction {
    fn from(fx: TransactionEffects) -> Self {
        ...
        Self { digest: fx.digest, contents: TransactionContents { scope, contents } }
    }
}
