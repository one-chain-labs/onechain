...
impl MoveValue {
    ...
    async fn display(&self, ctx: &Context<'_>) -> Result<Option<Display>, RpcError> {
        ...
        let (layout, display) = join!(self.type_.layout_impl(), pg_loader.load_one(DisplayKey(*type_)),);

        let (Some(layout), Some(display)) = (layout?, display.context("Failed to fetch Display")?) else {
            return Ok(None);
        };

        let event: DisplayVersionUpdatedEvent =
            bcs::from_bytes(&display.display).context("Failed to deserialize DisplayVersionUpdatedEvent")?;

        let mut output = IndexMap::new();
        let mut errors = IndexMap::new();

        for (field, value) in sui_display::v1::Format::parse(limits.max_display_field_depth, &event.fields)
...
fn proto_to_json(proto: proto::Value) -> async_graphql::Value {
    match proto.kind {
        ...
        Some(Kind::StructValue(map)) => {
            async_graphql::Value::Object(map.fields.into_iter().map(|(k, v)| (Name::new(k), proto_to_json(v))).collect())
        }

        Some(Kind::ListValue(list)) => async_graphql::Value::List(list.values.into_iter().map(proto_to_json).collect()),
    }
}
