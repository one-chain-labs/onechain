...
impl MoveModule {
    ...
    async fn datatype(&self, ctx: &Context<'_>, name: String) -> Result<Option<MoveDatatype>, RpcError> {
        let Some(contents) = self.contents(ctx).await?.as_ref() else {
            return Ok(None);
        };

        let Some(def) = contents.parsed.data_def(&name).context("Failed to get datatype definition")? else {
            return Ok(None);
        };
    ...
    async fn datatypes(
        ...
        let datatype_range =
            contents.parsed.datatypes(page.after().map(|c| c.as_ref()), page.before().map(|c| c.as_ref()));
        ...
        conn.has_previous_page =
            datatypes.first().is_some_and(|fst| contents.parsed.datatypes(None, Some(fst)).next().is_some());

        conn.has_next_page =
            datatypes.last().is_some_and(|lst| contents.parsed.datatypes(Some(lst), None).next().is_some());
    ...
    async fn enum_(&self, ctx: &Context<'_>, name: String) -> Result<Option<MoveEnum>, RpcError> {
        ...
        let Some(def) = contents.parsed.enum_def(&name).context("Failed to get enum definition")? else {
            return Ok(None);
        };
    ...
    async fn enums(
        ...
        let enum_range = contents.parsed.enums(page.after().map(|c| c.as_ref()), page.before().map(|c| c.as_ref()));
        ...
        conn.has_previous_page =
            enums.first().is_some_and(|fst| contents.parsed.enums(None, Some(fst)).next().is_some());

        conn.has_next_page = enums.last().is_some_and(|lst| contents.parsed.enums(Some(lst), None).next().is_some());
    ...
    async fn friends(
        ...
        page.paginate_indices(friends.len(), |i| {
            ...
            Ok(MoveModule::with_fq_name(self.package.clone(), friend_mod.to_string()))
        })
        .map(Some)
    }

    /// The function named `name` in this module.
    async fn function(&self, ctx: &Context<'_>, name: String) -> Result<Option<MoveFunction>, RpcError> {
        let Some(contents) = self.contents(ctx).await?.as_ref() else {
            return Ok(None);
        };

        let Some(def) = contents.parsed.function_def(&name).context("Failed to get function definition")? else {
            return Ok(None);
        };
    ...
    async fn functions(
        ...
        let function_range =
            contents.parsed.functions(page.after().map(|c| c.as_ref()), page.before().map(|c| c.as_ref()));
        ...
        conn.has_previous_page =
            functions.first().is_some_and(|fst| contents.parsed.functions(None, Some(fst)).next().is_some());

        conn.has_next_page =
            functions.last().is_some_and(|lst| contents.parsed.functions(Some(lst), None).next().is_some());
    ...
    async fn struct_(&self, ctx: &Context<'_>, name: String) -> Result<Option<MoveStruct>, RpcError> {
        let Some(contents) = self.contents(ctx).await?.as_ref() else {
            return Ok(None);
        };

        let Some(def) = contents.parsed.struct_def(&name).context("Failed to get struct definition")? else {
            return Ok(None);
        };
    ...
    async fn structs(
        ...
        let struct_range = contents.parsed.structs(page.after().map(|c| c.as_ref()), page.before().map(|c| c.as_ref()));
        ...
        conn.has_previous_page =
            structs.first().is_some_and(|fst| contents.parsed.structs(None, Some(fst)).next().is_some());

        conn.has_next_page = structs.last().is_some_and(|lst| contents.parsed.structs(Some(lst), None).next().is_some());
...
impl MoveModule {
    ...
    pub(crate) fn with_fq_name(package: MovePackage, name: String) -> Self {
        Self { package, name, contents: Arc::new(OnceCell::new()) }
    }

    /// Get the native CompiledModule, loading it lazily if needed.
    pub(crate) async fn contents(&self, ctx: &Context<'_>) -> Result<&Option<ModuleContents>, RpcError> {
        self.contents
