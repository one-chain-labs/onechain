...
pub(crate) async fn verify_signature(
    ...
    let epoch =
        Epoch::fetch(ctx, scope.clone(), None).await.map_err(upcast)?.context("Failed to fetch current Epoch")?;

    let sig @ GenericSignature::ZkLoginAuthenticator(_) =
        GenericSignature::from_bytes(&signature).map_err(|_| bad_user_input(Error::BadSignature))?
    else {
    ...
    let jwk_object = DynamicField::by_name(
        ...
        DynamicFieldName { type_: TypeInput(TypeTag::U64), bcs: Base64(bcs::to_bytes(&1u64).unwrap()) },
    )
    .await
    .map_err(upcast)?
    .context("JWK dynamic field not found")?;

    let authenticator_field =
        jwk_object.native(ctx).await.map_err(upcast)?.as_ref().context("Couldn't fetch JWK dynamic field contents")?;

    let authenticator_jwks: AuthenticatorStateInner =
        bcs::from_bytes(&authenticator_field.value_bytes).context("Failed to deserialize JWK dynamic field contents")?;
    ...
    let params = VerifyParams::new(jwks, vec![], config.env, true, true, true, config.max_epoch_upper_bound_delta, true);
    ...
    Ok(match intent_scope {
        ZkLoginIntentScope::TransactionData => verify(
            ...
            &IntentMessage::new(
                ...
                bcs::from_bytes::<TransactionData>(&bytes).map_err(|_| bad_user_input(Error::NotTransactionData))?,
            ),
            author,
            epoch.epoch_id,
            &params,
        ...
        ZkLoginIntentScope::PersonalMessage => verify(
            sig,
            &IntentMessage::new(Intent::personal_message(), PersonalMessage { message: bytes }),
            author,
            epoch.epoch_id,
            &params,
        ),
...
fn verify<T: Serialize>(
    ...
    match sig.verify_authenticator(message, author.into(), epoch_id, params, Arc::new(VerifiedDigestCache::new_empty()))
    {
        Ok(()) => ZkLoginVerifyResult { success: Some(true), error: None },

        Err(e) => ZkLoginVerifyResult { success: Some(false), error: Some(e.to_string()) },
    }
}
