...
impl ConfigContent {
    ...
    async fn config(&self, key: String) -> Option<ProtocolConfig> {
        self.0.get(&key).map(|value| ProtocolConfig { key: key.clone(), value: value.clone() })
    }

    /// List all available configurations and their values.
    async fn configs(&self) -> Vec<ProtocolConfig> {
        self.0.clone().into_iter().map(|(key, value)| ProtocolConfig { key, value }).collect()
    }
}
...
impl FlagContent {
    ...
    async fn feature_flag(&self, key: String) -> Option<FeatureFlag> {
        self.0.get(&key).map(|value| FeatureFlag { key: key.clone(), value: *value })
    }

    /// List all available feature flags and their values.
    async fn feature_flags(&self) -> Vec<FeatureFlag> {
        self.0.clone().into_iter().map(|(key, value)| FeatureFlag { key, value }).collect()
    }
}
...
impl ProtocolConfigs {
    ...
    pub(crate) async fn latest(ctx: &Context<'_>, scope: &Scope) -> Result<Option<Self>, RpcError> {
        ...
        let Some(stored) =
            pg_loader.load_one(CheckpointBoundedEpochStartKey(cp)).await.context("Failed to fetch latest epoch")?
        else {
            return Ok(None);
        };

        Ok(Some(Self { protocol_version: stored.protocol_version as u64 }))
    }
}
...
impl ConfigContent {
    async fn fetch(ctx: &Context<'_>, protocol_version: u64) -> Result<Self, RpcError> {
        ...
        Ok(Self(configs.into_iter().map(|c| (c.config_name, c.config_value)).collect()))
    }
}
...
impl FlagContent {
    async fn fetch(ctx: &Context<'_>, protocol_version: u64) -> Result<Self, RpcError> {
        ...
        Ok(Self(configs.into_iter().map(|c| (c.flag_name, c.flag_value)).collect()))
    }
}
