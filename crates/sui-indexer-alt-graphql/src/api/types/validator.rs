...
impl Validator {
    ...
    async fn operation_cap(&self, ctx: &Context<'_>) -> Result<Option<MoveObject>, RpcError<Error>> {
        let address = Address::with_address(self.contents.scope.clone(), self.validator().operation_cap_id.bytes.into());
    ...
    async fn exchange_rates_table(&self) -> Option<Address> {
        let address =
            Address::with_address(self.contents.scope.clone(), self.validator().staking_pool.exchange_rates.id.into());
        Some(address)
    }
    ...
    async fn staking_pool_activation_epoch(&self) -> Option<UInt53> {
        self.validator().staking_pool.activation_epoch.map(UInt53::from)
    }

    /// The total number of SUI tokens in this pool.
    async fn staking_pool_oct_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.oct_balance))
    }

    /// The epoch stake rewards will be added here at the end of each epoch.
    async fn rewards_pool(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.rewards_pool.value()))
    }

    /// Total number of pool tokens issued by the pool.
    async fn pool_token_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.pool_token_balance))
    }
    ...
    async fn pending_total_oct_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.pending_total_oct_withdraw))
    }

    /// Pending pool token withdrawn during the current epoch, emptied at epoch boundaries.
    async fn pending_pool_token_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.pending_pool_token_withdraw))
    }
    ...
    async fn report_records(
        ...
        let Some(report_records) = self.contents.report_records.get(&self.validator().metadata.sui_address) else {
        ...
        page.paginate_indices(report_records.len(), |i| {
            ...
            Ok(Validator { contents: Arc::clone(&self.contents), idx })
        })
        .map(Some)
    }
}
...
impl Validator {
    fn super_(&self) -> Address {
        Address::with_address(self.contents.scope.clone(), self.validator().metadata.sui_address)
    }
