...
use crate::{
    api::{
        scalars::{base64::Base64, big_int::BigInt, sui_address::SuiAddress, type_filter::TypeInput, uint53::UInt53},
        types::{
            address::Address,
            balance,
            balance::Balance,
...
impl Validator {
    ...
    async fn pending_total_oct_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(self.native.staking_pool.pending_total_oct_withdraw))
    }

    /// Pending pool token withdrawn during the current epoch, emptied at epoch boundaries.
    async fn pending_pool_token_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(self.native.staking_pool.pending_pool_token_withdraw))
    }
...
impl Validator {
    pub(crate) fn from_validator_v1(scope: Scope, native: ValidatorV1) -> Self {
        Self { super_: Address::with_address(scope, native.metadata.sui_address), native }
    }
}
