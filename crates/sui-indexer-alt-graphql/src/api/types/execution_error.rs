...
use sui_types::{
    execution_status::{ExecutionFailureStatus, ExecutionStatus as NativeExecutionStatus, MoveLocation},
...
impl ExecutionError {
    ...
    async fn abort_code(&self) -> Option<BigInt> {
        ...
        Some(BigInt::from(
            self.clever_error().await.as_ref().and_then(|err| err.error_code).map_or(*raw_code, |code| code as u64),
        ))
    }
    ...
    async fn message(&self) -> Result<String, RpcError> {
        self.format_error_message()
            .await
            .map_err(|e| anyhow::Error::from(e).context("Failed to format error message").into())
    }
}
...
impl ExecutionError {
    ...
    pub(crate) async fn from_execution_status(
        ...
        Ok(Some(Self { native: native_error, command: *command, clever: OnceCell::new(), scope: scope.clone() }))
    }
    ...
    async fn clever_error(&self) -> &Option<CleverError> {
        ...
        self.clever
            .get_or_init(|| async {
                self.scope.package_resolver().resolve_clever_error(location.module.clone(), *raw_code).await
            })
            .await
    }
    ...
    async fn format_error_message(&self) -> Result<String, std::fmt::Error> {
        match self.command {
            ...
            Some(command) => {
                ...
                let Some(CleverError { source_line_number, error_info, error_code, .. }) =
                    self.clever_error().await.as_ref()
                else {
                    // No clever error, show basic abort info
                    write!(msg, " (instruction {}), abort code: {code}", loc.instruction)?;
                    return Ok(msg);
                };
                ...
                match error_info {
                    ...
                    ErrorConstants::None => {
                        write!(msg, " (line {source_line_number}){}", match error_code {
...
async fn resolve_module_id_for_move_abort(
    ...
    use sui_types::{execution_status::MoveLocationOpt, transaction::Command};
    ...
    let module = match native_error {
        ...
        ExecutionFailureStatus::MovePrimitiveRuntimeError(MoveLocationOpt(Some(MoveLocation { module, .. }))) => module,
        _ => return Ok(()),
    };
    ...
    match scope.package_resolver().resolve_module_id(module_new, ptb_call.package.into()).await {
        Ok(resolved_module) => {
        ...
        Err(e) => {
            return Err(anyhow::Error::from(e).context("Failed to resolve module ID for execution error"));
        }
    }
