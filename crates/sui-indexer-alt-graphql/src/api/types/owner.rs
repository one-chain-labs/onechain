...
impl Owner {
    pub(crate) fn from_native(scope: Scope, native: NativeOwner) -> Self {
        ...
        match native {
            NO::AddressOwner(a) => {
                O::Address(AddressOwner { address: Some(Address::with_address(scope.without_root_version(), a)) })
            }

            NO::ObjectOwner(a) => O::Object(ObjectOwner { address: Some(Address::with_address(scope, a)) }),

            NO::Shared { initial_shared_version } => {
                O::Shared(Shared { initial_shared_version: Some(initial_shared_version.into()) })
            }

            NO::Immutable => O::Immutable(Immutable { dummy: None }),

            NO::ConsensusAddressOwner { start_version, owner } => O::ConsensusAddress(ConsensusAddressOwner {
