...
use sui_types::{
    ...
    coin::{
        CoinMetadata as NativeMetadata,
        RegulatedCoinMetadata,
        TreasuryCap,
        COIN_METADATA_STRUCT_NAME,
        COIN_MODULE_NAME,
    },
    coin_registry::{Currency as NativeCurrency, RegulatedState as NativeRegulated, SupplyState as NativeSupply},
...
use crate::{
    api::scalars::{base64::Base64, big_int::BigInt, sui_address::SuiAddress, type_filter::TypeInput, uint53::UInt53},
...
impl CoinMetadata {
    ...
    pub(crate) async fn dynamic_fields(
        ...
        self.super_.dynamic_fields(ctx, first, after, last, before).await
    }
    ...
    /// Whether this object can be transfered using the `TransferObjects` Programmable Transaction Command or `one::transfer::public_transfer`.
    ///
    /// Both these operations require the object to have both the `key` and `store` abilities.
    pub(crate) async fn has_public_transfer(&self, ctx: &Context<'_>) -> Result<Option<bool>, RpcError> {
    ...
    pub(crate) async fn move_object_bcs(&self, ctx: &Context<'_>) -> Result<Option<Base64>, RpcError> {
    ...
    #[graphql(flatten)]
    pub(crate) async fn regulated_fields(&self, ctx: &Context<'_>) -> Result<RegulatedFields, RpcError<object::Error>> {
        ...
        match &contents.native {
            NativeContents::Registry(NativeCurrency { regulated: NativeRegulated::Unregulated, .. }) => {
            ...
            NativeContents::Registry(NativeCurrency {
                regulated: NativeRegulated::Regulated { cap, allow_global_pause, variant: _ },
                ..
            }) => {
                return Ok(RegulatedFields {
        ...
        let Some(object) = Object::singleton(ctx, scope.without_root_version(), type_).await? else {
        ...
        let move_object = contents.data.try_as_move().context("Query by type returned a package")?;

        let metadata: RegulatedCoinMetadata =
            bcs::from_bytes(move_object.contents()).context("Failed to deserialize RegulatedCoinMetadata")?;
    ...
    #[graphql(flatten)]
    pub(crate) async fn supply_fields(&self, ctx: &Context<'_>) -> Result<SupplyFields, RpcError<object::Error>> {
        let Some(contents) = self.metadata_contents(ctx).await.map_err(upcast)? else {
        ...
        // return it.
        match &contents.native {
            NativeContents::Registry(NativeCurrency { supply: Some(NativeSupply::Fixed(s)), .. }) => {
                return Ok(SupplyFields { supply_state: Some(SupplyState::Fixed), supply: Some(BigInt::from(*s)) });
            }

            NativeContents::Registry(NativeCurrency { supply: Some(NativeSupply::BurnOnly(s)), .. }) => {
                return Ok(SupplyFields { supply_state: Some(SupplyState::BurnOnly), supply: Some(BigInt::from(*s)) });
            }
        ...
        let supply_state = if matches!(
            ...
            NativeOwner::Immutable
                ...
                | NativeOwner::ConsensusAddressOwner { owner: NativeAddress::ZERO, .. }
        ) {
        ...
        Ok(SupplyFields { supply_state, supply: Some(BigInt::from(treasury_cap.total_supply.value)) })
    }
...
impl CoinMetadata {
    ...
    pub(crate) fn from_super(super_: MoveObject) -> Self {
        Self { super_, contents: OnceCell::new() }
    }

    /// Create a CoinMetadata from a `MoveObject`, after checking whether it is a CoinMetadata.
    pub(crate) async fn from_move_object(move_object: &MoveObject, ctx: &Context<'_>) -> Result<Option<Self>, RpcError> {
    ...
    pub(crate) async fn by_coin_type(
        ...
        let currency_id: SuiAddress =
            NativeCurrency::derive_object_id(coin_type.clone()).context("Failed to derive Currency ID")?.into();

        if let Some(object) = Object::latest(ctx, scope.clone(), currency_id).await.map_err(upcast)? {
        ...
        Ok(Object::singleton(ctx, scope, type_).await?.map(|obj| Self::from_super(MoveObject::from_super(obj))))
    }

    /// Get the native CoinMetadata data, loading it lazily if needed.
    async fn metadata_contents(&self, ctx: &Context<'_>) -> Result<&Option<MetadataContents>, RpcError> {
        self.contents
            .get_or_try_init(async || {
                ...
                let coin_type =
                    || StructTag::from(native_move.type_().clone()).type_params.pop().context("No coin type parameter");

                Ok(Some(if native_move.type_().is_currency() {
                    MetadataContents {
                        coin_type: coin_type()?,
                        native: NativeContents::Registry(
                            bcs::from_bytes(native_move.contents()).context("Failed to deserialize Currency")?,
                        ),
                    }
                    ...
                    MetadataContents {
                        ...
                        native: NativeContents::Metadata(
                            bcs::from_bytes(native_move.contents()).context("Failed to deserialize CoinMetadata")?,
                        ),
                    }
                } else {
