...
pub(crate) struct ObjectFilter {
    ...
    /// - A type instantiation: `0x2::coin::Coin<0x2::oct::OCT>`.
    ...
    fn check(&self, filter: &ObjectFilter) -> Result<(), InputValueError<ObjectFilter>> {
        match filter {
            ObjectFilter {
                ...
                owner: None,
                type_: _,
            } => Err(InputValueError::custom(format!("{kind} owner kind requires an `owner` to be specified"))),

            ObjectFilter {
                owner_kind: Some(kind @ (OwnerKind::Shared | OwnerKind::Immutable)),
                owner: Some(_),
                type_: _,
            } => Err(InputValueError::custom(format!("Unexpected `owner` for {kind} owner kind",))),

            ObjectFilter {
                owner_kind: Some(kind @ (OwnerKind::Shared | OwnerKind::Immutable)),
                owner: None,
                type_: None,
            } => Err(InputValueError::custom(format!("{kind} owner kind requires a `type` to be specified"))),

            ObjectFilter { owner_kind: None, owner: None, type_: None } => {
                if self.allow_empty {
                    Ok(())
                } else {
            ...
            ObjectFilter { owner_kind: Some(OwnerKind::Shared | OwnerKind::Immutable), owner: None, type_: Some(_) } => {
                Ok(())
            }

            // Valid type-only filter
            ObjectFilter { owner_kind: None, owner: None, type_: Some(_) } => Ok(()),
        }
    }
}
...
mod tests {
    ...
    fn test_owner_kind_intersection() {
        let a = ObjectFilter { owner_kind: Some(OwnerKind::Address), owner: None, type_: None };

        let o = ObjectFilter { owner_kind: Some(OwnerKind::Object), owner: None, type_: None };
    ...
    fn test_owner_intersection() {
        let a1 = ObjectFilter { owner_kind: None, owner: Some("0x1".parse().unwrap()), type_: None };

        let a2 = ObjectFilter { owner_kind: None, owner: Some("0x2".parse().unwrap()), type_: None };
    ...
    fn test_type_filter_intersection() {
        let coin = ObjectFilter { owner_kind: None, owner: None, type_: Some("0x2::coin::Coin".parse().unwrap()) };

        let sui = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::oct::OCT>".parse().unwrap()),
        };

        let token = ObjectFilter { owner_kind: None, owner: None, type_: Some("0x2::token::Token".parse().unwrap()) };
