...
use crate::{api::scalars::domain::Domain, error::RpcError, scope::Scope, task::watermark::Watermarks};
...
pub(crate) async fn name_to_address(
    ...
    let parent_record: OptionFuture<_> =
        domain.is_subdomain().then_some(async move { name_record(ctx, scope.clone(), &domain.parent()).await }).into();
    ...
    if !domain_record.is_leaf_record() {
        return Ok(if domain_record.is_node_expired(timestamp_ms) {
            ...
            domain_record.target_address.map(|addr| Address::with_address(scope.clone(), addr))
        });
    }
    ...
    Ok(if parent_record.is_valid_leaf_parent(&domain_record) && !parent_record.is_node_expired(timestamp_ms) {
        domain_record.target_address.map(|addr| Address::with_address(scope.clone(), addr))
    } else {
        None
    })
...
pub(crate) async fn address_to_name(
    ...
    Ok(name_to_address(ctx, scope, &reverse_record.value).await?.is_some().then(|| reverse_record.value.to_string()))
}

/// Fetch the latest version of the name record for the given `domain`.
async fn name_record(ctx: &Context<'_>, scope: Scope, domain: &NativeDomain) -> Result<Option<NameRecord>, RpcError> {
    ...
    let record = NameRecord::try_from(object.clone()).context("Failed to deserialize name record")?;
...
async fn reverse_record(
    ...
    let record = bcs::from_bytes(move_object.contents()).context("Failed to deserialize reverse record")?;
