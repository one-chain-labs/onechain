...
impl MoveType {
    ...
    pub(crate) async fn canonicalize(input: TypeTag, scope: Scope) -> Result<Option<Self>, RpcError<Error>> {
        ...
        let canonical = match scope.package_resolver().canonical_type(input.clone()).await {
            ...
            Err(
                ...
                return Err(anyhow!(err)
                    .context(format!("Error canonicalizing type {}", input.to_canonical_display(/* with_prefix */ true)))
                    .into());
            }
        };

        Ok(Some(Self { native: canonical.into(), scope }))
    }
    ...
    pub(crate) async fn layout_impl(&self) -> Result<Option<A::MoveTypeLayout>, RpcError> {
        ...
        let layout = self.scope.package_resolver().type_layout(tag).await.map_err(|err| {
            internal_resolution_error(err, || {
                format!("Error calculating layout for {}", self.native.to_canonical_display(/* with_prefix */ true))
            })
        })?;
    ...
    pub(crate) async fn abilities_impl(&self) -> Result<Option<AbilitySet>, RpcError> {
        ...
        let set = self.scope.package_resolver().abilities(tag).await.map_err(|err| {
            internal_resolution_error(err, || {
                format!("Error calculating abilities for {}", self.native.to_canonical_display(/* with_prefix */ true))
            })
        })?;
    ...
    F: FnOnce() -> C,
{
    ...
    match &err {
        RE::TooManyTypeNodes(_, _) | RE::TooManyTypeParams(_, _) | RE::TypeParamNesting(_, _) | RE::ValueNesting(_) => {
