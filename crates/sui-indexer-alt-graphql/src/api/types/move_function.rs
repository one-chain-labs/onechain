...
impl MoveFunction {
    ...
    async fn parameters(&self, ctx: &Context<'_>) -> Result<Option<Vec<OpenMoveType>>, RpcError> {
        ...
        Ok(Some(contents.parameters.iter().cloned().map(OpenMoveType::from).collect()))
    }
    ...
    async fn return_(&self, ctx: &Context<'_>) -> Result<Option<Vec<OpenMoveType>>, RpcError> {
        ...
        Ok(Some(contents.return_.iter().cloned().map(OpenMoveType::from).collect()))
    }
    ...
    async fn type_parameters(&self, ctx: &Context<'_>) -> Result<Option<Vec<MoveFunctionTypeParameter>>, RpcError> {
        let Some(contents) = self.contents(ctx).await?.as_ref() else {
            return Ok(None);
        };

        Ok(Some(contents.type_params.iter().map(|c| MoveFunctionTypeParameter { constraints: abilities(*c) }).collect()))
    }
    ...
    async fn contents(&self, ctx: &Context<'_>) -> Result<&Option<FunctionDef>, RpcError> {
        self.contents
            .get_or_try_init(|| async {
                ...
                let def = contents.parsed.function_def(&self.name).context("Failed to get function definition")?;
