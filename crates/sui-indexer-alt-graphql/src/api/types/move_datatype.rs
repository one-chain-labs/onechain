...
#[graphql(
    name = "IMoveDatatype",
    field(name = "module", ty = "Result<&MoveModule, RpcError>", desc = "The module that this datatype is defined in",),
    field(name = "name", ty = "Result<&str, RpcError>", desc = "The datatype's unqualified name",),
    field(
        name = "abilities",
...
impl MoveDatatype {
    ...
    async fn as_move_struct(&self, ctx: &Context<'_>) -> Result<Option<MoveStruct>, RpcError> {
        ...
        Ok(matches!(def.data, MoveData::Struct(_)).then(|| MoveStruct { super_: self.clone() }))
    }
    ...
    async fn as_move_enum(&self, ctx: &Context<'_>) -> Result<Option<MoveEnum>, RpcError> {
        ...
        Ok(matches!(def.data, MoveData::Enum(_)).then(|| MoveEnum { super_: self.clone() }))
    }

    /// Constraints on the datatype's formal type parameters.
    ///
    /// Move bytecode does not name type parameters, so when they are referenced (e.g. in field types), they are identified by their index in this list.
    async fn type_parameters(&self, ctx: &Context<'_>) -> Result<Option<Vec<MoveDatatypeTypeParameter>>, RpcError> {
        let Some(def) = self.contents(ctx).await? else {
            return Ok(None);
        };
...
impl MoveEnum {
    ...
    async fn variants(&self, ctx: &Context<'_>) -> Result<Option<Vec<MoveEnumVariant<'_>>>, RpcError> {
...
impl MoveStruct {
    ...
    async fn fields(&self, ctx: &Context<'_>) -> Result<Option<Vec<MoveField<'_>>>, RpcError> {
        ...
        Ok(Some(fields.iter().map(|(name, type_)| MoveField { name: name.as_str(), type_ }).collect()))
    }
...
impl MoveEnumVariant<'_> {
    ...
    async fn fields(&self) -> Option<Vec<MoveField<'_>>> {
        Some(self.0.signatures.iter().map(|(name, type_)| MoveField { name: name.as_str(), type_ }).collect())
    }
}
    ...
    async fn contents(&self, ctx: &Context<'_>) -> Result<&Option<DataDef>, RpcError> {
        self.contents
            .get_or_try_init(|| async {
                ...
                Ok(module.parsed.data_def(&self.name).context("Failed to deserialize datatype definition")?)
            })
            .await
    }
}
...
impl MoveEnum {
    pub(crate) fn with_fq_name(module: MoveModule, name: String) -> Self {
        Self { super_: MoveDatatype::with_fq_name(module, name) }
    }

    pub(crate) fn from_def(module: MoveModule, name: String, def: DataDef) -> Self {
        Self { super_: MoveDatatype::from_def(module, name, def) }
    }
}
