...
impl Balance {
    ...
    pub(crate) async fn fetch_one(
        ...
        let (coin_type, total_balance) = consistent_reader
            .get_balance(checkpoint, address.to_string(), coin_type.to_canonical_string(true))
            .await
    ...
    pub(crate) async fn fetch_many(
        ...
        let balances = consistent_reader
            .batch_get_balances(
                checkpoint,
                address.to_string(),
                coin_types.into_iter().map(|t| t.to_canonical_string(true)).collect(),
            )
            .await
            .map_err(|e| consistent_error(checkpoint, e))?;
...
fn consistent_error(checkpoint: u64, error: consistent_reader::Error) -> RpcError<Error> {
    match error {
        consistent_reader::Error::NotConfigured => feature_unavailable("fetching balances for addresses"),

        consistent_reader::Error::OutOfRange(_) => bad_user_input(Error::OutOfRange(checkpoint)),

        consistent_reader::Error::Internal(error) => error.context("Failed to fetch balance").into(),
    }
}
