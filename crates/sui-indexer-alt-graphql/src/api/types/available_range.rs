...
impl AvailableRange {
    ...
    async fn first(&self) -> Result<Option<Checkpoint>, RpcError> {
        Ok(Checkpoint::with_sequence_number(self.scope.clone(), Some(self.first)))
    }
...
impl AvailableRange {
    ...
    pub(crate) fn new(
        ...
        Ok(Self { scope: scope.clone(), first })
    }
}
...
impl AvailableRangeKey {
    ...
    pub(crate) fn reader_lo(self, watermarks: &Watermarks) -> Result<u64, RpcError> {
        ...
        pipelines
            .iter()
            .try_fold(0, |acc, pipeline| watermarks.pipeline_lo_watermark(pipeline).map(|wm| acc.max(wm.checkpoint())))
    }
    ...
    fn validate(&self, registry: &Registry) -> Result<(), RpcError<Error>> {
        let fields = match registry.types.get(&self.type_) {
            ...
            Some(_) => {
                return Err(bad_user_input(Error::NotAnObjectOrInterface(self.type_.to_string())));
            }
            None => {
        ...
        if let Some(field_name) = &self.field {
            fields
                .get(field_name)
                .ok_or_else(|| bad_user_input(Error::FieldNotFound(self.type_.to_string(), field_name.to_string())))?;
        }
...
pub(crate) fn pipeline_unavailable(pipeline: &str) -> RpcError {
    match pipeline {
        ...
        "tx_affected_addresses" => feature_unavailable("filtering transactions by affected address"),
...
mod field_piplines_tests {
    ...
    fn test_collect_pipelines(type_: &str, field: Option<&str>, filters: BTreeSet<String>) -> BTreeSet<String> {
        let mut pipelines = BTreeSet::new();
    ...
    async fn with_registry(test_fn: fn(&Registry)) {
        ...
        impl ExtensionFactory for TestExtension {
            fn create(&self) -> Arc<dyn Extension> {
                Arc::new(TestExtensionImpl { test_fn: self.test_fn })
            }
        }
    ...
    fn type_delegation_matches_interface(registry: &Registry) {
        ...
        for (interface_name, meta_type) in registry.types.iter() {
            let MetaType::Interface { possible_types, fields, .. } = meta_type else {
                continue;
            };
    ...
    fn registry_collect_pipelines_snapshot(registry: &Registry) {
        ...
        for (type_name, meta_type) in registry.types.iter() {
            let (MetaType::Object { fields, .. } | MetaType::Interface { fields, .. }) = meta_type else {
                continue;
            };
            ...
            for (field_name, meta_field) in fields.iter() {
                ...
                let filter_fields: Vec<String> = meta_field
                    ...
                    .flat_map(|(param_name, meta_input_value)| {
                        ...
                        match registry.types.get(concrete_type) {
                            Some(MetaType::InputObject { input_fields, .. }) => input_fields.keys().cloned().collect(),
                ...
                super::collect_pipelines(type_name, Some(field_name), BTreeSet::new(), &mut unfiltered_pipelines);
                let unfiltered_output_str = formatted_output_str(type_name, field_name, &unfiltered_pipelines, None);
                ...
                for filter_field in filter_fields.iter().map(Some) {
                    ...
                    let output_str = formatted_output_str(type_name, field_name, &pipelines, filter_field);
        ...
        for pipeline in all_pipelines {
            ...
            if let RpcError::InternalError(err) = result {
                panic!("Pipeline '{}' does not have a feature mapping in pipeline_unavailable: {}", pipeline, err);
            }
        }
    ...
    fn formatted_output_str(
        ...
        let pipeline_list = pipelines.iter().map(|s| format!("\"{s}\"")).collect::<Vec<_>>().join(", ");

        let output_str = format!("{type_name}.{field_name}{filter_suffix}\n  => {{{pipeline_list}}}\n\n");
        output_str
    }
    ...
    fn should_ignore_in_snapshot(type_name: &str, field_name: &str) -> bool {
        ...
        if type_name.starts_with(INTROSPECTION_TYPE)
            || GENERATED_TYPES.iter().any(|suffix| type_name.ends_with(suffix))
                && GENERATED_FIELDS.iter().any(|suffix| field_name.ends_with(suffix))
        {
            return true;
        }
        false
