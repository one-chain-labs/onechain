...
impl EffectsContents {
    ...
    async fn timestamp(&self) -> Result<Option<DateTime>, RpcError> {
        ...
        content.timestamp_ms().map(|ms| DateTime::from_ms(ms as i64)).transpose()
    }
    ...
    async fn object_changes(
        ...
        page.paginate_indices(object_changes.len(), |i| {
            Ok(ObjectChange { scope: self.scope.clone(), native: object_changes[i].clone() })
        })
        .map(Some)
    }
    ...
    async fn unchanged_consensus_objects(
        ...
        page.paginate_indices(unchanged_consensus_objects.len(), |i| {
            Ok(UnchangedConsensusObject::from_native(self.scope.clone(), unchanged_consensus_objects[i].clone(), epoch))
        })
        .map(Some)
    }
    ...
    async fn dependencies(
        ...
        page.paginate_indices(dependencies.len(), |i| Ok(Transaction::with_id(self.scope.clone(), dependencies[i])))
            .map(Some)
    }
}
