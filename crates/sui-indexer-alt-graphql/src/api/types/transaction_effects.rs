...
use crate::{
    api::scalars::{base64::Base64, cursor::JsonCursor, date_time::DateTime, digest::Digest, uint53::UInt53},
    error::RpcError,
...
impl EffectsContents {
    ...
    async fn checkpoint(&self) -> Option<Checkpoint> {
        ...
        content.cp_sequence_number().and_then(|cp| Checkpoint::with_sequence_number(self.scope.clone(), Some(cp)))
    }
    ...
    async fn execution_error(&self) -> Result<Option<ExecutionError>, RpcError> {
        ...
        let programmable_tx = content.data().ok().and_then(|tx_data| match tx_data.into_kind() {
    ...
    async fn epoch(&self) -> Result<Option<Epoch>, RpcError> {
        ...
        let effects = content.effects()?;
        Ok(Some(Epoch::with_id(self.scope.clone(), effects.executed_epoch())))
    }
    ...
    async fn events(
        ...
        for edge in cursors.edges {
            ...
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), event));
        }
    ...
    async fn balance_changes(
        ...
        let Some(stored_balance_changes) = pg_loader.load_one(key).await.context("Failed to load balance changes")?
        else {
            return Ok(Some(Connection::new(false, false)));
        };
        ...
        let balance_changes: Vec<NativeBalanceChange> =
            bcs::from_bytes(&stored_balance_changes.balance_changes).context("Failed to deserialize balance changes")?;
        ...
        for edge in cursors.edges {
            let balance_change =
                BalanceChange { scope: self.scope.clone(), stored: balance_changes[*edge.cursor].clone() };
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), balance_change));
        }
    ...
    async fn object_changes(
        ...
        let mut conn = Connection::new(cursors.has_previous_page, cursors.has_next_page);
        for edge in cursors.edges {
            let object_change = ObjectChange { scope: self.scope.clone(), native: object_changes[*edge.cursor].clone() };

            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), object_change))
        }
    ...
    async fn dependencies(
        ...
        for edge in cursors.edges {
            ...
            conn.edges.push(Edge::new(edge.cursor.encode_cursor(), transaction));
        }
...
impl TransactionEffects {
    ...
    fn from_executed_transaction(
        ...
        let contents =
            NativeTransactionContents::from_executed_transaction(executed_transaction, transaction_data, signatures)
        ...
        let digest = contents.digest().context("Failed to get digest from transaction contents")?;
        let scope = scope.with_execution_output(objects);

        Ok(Self { digest, contents: EffectsContents { scope, contents: Some(Arc::new(contents)) } })
    }
    ...
    pub(crate) fn from_execution_response(
        ...
        let executed_transaction =
            response.transaction.as_ref().context("ExecuteTransactionResponse should have transaction")?;
    ...
    pub(crate) fn from_simulation_response(
        ...
        let executed_transaction =
            response.transaction.as_ref().context("SimulateTransactionResponse should have transaction")?;
    ...
    /// from the store).
    pub(crate) async fn fetch(ctx: &Context<'_>, scope: Scope, digest: Digest) -> Result<Option<Self>, RpcError> {
        let contents = EffectsContents::empty(scope).fetch(ctx, digest.into()).await?;

        let Some(tx) = &contents.contents else {
            return Ok(None);
        };

        Ok(Some(Self { digest: tx.digest()?, contents }))
    }
}

impl EffectsContents {
    fn empty(scope: Scope) -> Self {
        Self { scope, contents: None }
    }
    ...
    pub(crate) async fn fetch(&self, ctx: &Context<'_>, digest: TransactionDigest) -> Result<Self, RpcError> {
        ...
        let Some(transaction) =
            kv_loader.load_one_transaction(digest).await.context("Failed to fetch transaction contents")?
        else {
            return Ok(self.clone());
        };

        let cp_num =
            transaction.cp_sequence_number().context("Fetched transaction should have checkpoint sequence number")?;

        if cp_num > checkpoint_viewed_at {
            return Ok(self.clone());
        }

        Ok(Self { scope: self.scope.clone(), contents: Some(Arc::new(transaction)) })
    }
}
...
impl From<Transaction> for TransactionEffects {
    fn from(tx: Transaction) -> Self {
        ...
        Self { digest: tx.digest, contents: EffectsContents { scope, contents } }
    }
}

/// Extract input and output object contents from an ExecutedTransaction.
fn extract_objects_from_executed_transaction(executed_transaction: &ExecutedTransaction) -> Vec<NativeObject> {
