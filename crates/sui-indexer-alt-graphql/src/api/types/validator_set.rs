...
impl ValidatorSet {
    ...
    async fn pending_active_validators_id(&self) -> Option<SuiAddress> {
        Some(self.contents.native.pending_active_validators.contents.id.into())
    }
...
impl ValidatorSet {
    pub(crate) fn from_validator_set_v1(
        ...
        let address_to_index: BTreeMap<_, _> =
            native.active_validators.iter().enumerate().map(|(i, v)| (v.metadata.sui_address, i)).collect();
        let report_records = report_records
            .contents
            .into_iter()
            .map(|Entry { key, value }| (key, value.contents.into_iter().map(|v| address_to_index[&v]).collect()))
            .collect();
        Self { contents: Arc::new(ValidatorSetContents { scope, native, report_records }) }
    }
}
