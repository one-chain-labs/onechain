...
impl PerEpochConfig {
    ...
    async fn object(&self, ctx: &Context<'_>) -> Result<Option<Object>, RpcError> {
        ...
        let Some(epoch_start) =
            pg_loader.load_one(EpochStartKey(self.epoch)).await.context("Failed to fetch epoch start information")?
        else {
            return Ok(None);
        };
...
impl ConsensusObjectRead {
    ...
    async fn object(&self) -> Option<Object> {
        Some(Object::with_ref(&self.scope, self.object_id.into(), self.version, self.digest))
    }
}

impl UnchangedConsensusObject {
    pub(crate) fn from_native(scope: Scope, native: (ObjectID, NativeUnchangedConsensusKind), epoch: u64) -> Self {
        let (object_id, kind) = native;

        match kind {
            NativeUnchangedConsensusKind::ReadOnlyRoot((version, digest)) => {
                Self::Read(ConsensusObjectRead { scope, object_id, version, digest })
            }
            ...
            NativeUnchangedConsensusKind::Cancelled(sequence_number) => {
                let cancellation_reason = match sequence_number {
                    SequenceNumber::CANCELLED_READ => ConsensusObjectCancellationReason::CancelledRead,
                    SequenceNumber::CONGESTED => ConsensusObjectCancellationReason::Congested,
                    SequenceNumber::RANDOMNESS_UNAVAILABLE => ConsensusObjectCancellationReason::RandomnessUnavailable,
            ...
            NativeUnchangedConsensusKind::PerEpochConfig => {
                Self::PerEpochConfig(PerEpochConfig { scope, object_id, epoch })
            }
        }
    }
}
