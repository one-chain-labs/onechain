...
use sui_indexer_alt_reader::{
    ...
    kv_loader::KvLoader,
    object_versions::{CheckpointBoundedObjectVersionKey, VersionBoundedObjectVersionKey, VersionedObjectVersionKey},
    pg_reader::PgReader,
};
use sui_indexer_alt_schema::{objects::StoredObjVersion, schema::obj_versions};
use sui_pg_db::sql;
use sui_types::{
    base_types::{SequenceNumber, SuiAddress as NativeSuiAddress, TransactionDigest, VersionDigest},
...
#[graphql(
    ...
    field(name = "owner", ty = "Result<Option<Owner>, RpcError<Error>>", desc = "The object's owner kind."),
    field(
...
impl Object {
    ...
    pub(crate) async fn dynamic_fields(
        ...
        let dynamic_fields =
            DynamicField::paginate(ctx, self.super_.scope.clone(), self.super_.address.into(), page).await?;
    ...
    pub(crate) async fn object_at(
        ...
        let key = ObjectKey { address: self.super_.address.into(), version, root_version, at_checkpoint: checkpoint };
    ...
    pub(crate) async fn owner(&self, ctx: &Context<'_>) -> Result<Option<Owner>, RpcError> {
        ...
        Ok(Some(Owner::from_native(self.super_.scope.clone(), object.owner.clone())))
    }
    ...
    pub(crate) async fn previous_transaction(&self, ctx: &Context<'_>) -> Result<Option<Transaction>, RpcError> {
        ...
        Ok(Some(Transaction::with_id(self.super_.scope.without_root_version(), object.previous_transaction)))
    }
    ...
    pub(crate) async fn received_transactions(
        ...
        let address_filter =
            TransactionFilter { affected_address: Some(self.super_.address.into()), ..Default::default() };
        ...
        Transaction::paginate(ctx, self.super_.scope.clone(), page, filter).await.map(Some)
    }
}
...
impl Object {
    ...
    pub(crate) fn with_ref(
        ...
        Self { super_, version_digest: Some((version, digest)), contents: Arc::new(OnceCell::new()) }
    }
    ...
    pub(crate) fn with_address(scope: Scope, address: NativeSuiAddress) -> Self {
        Self { super_: Address::with_address(scope, address), version_digest: None, contents: Arc::new(OnceCell::new()) }
    }
    ...
    pub(crate) async fn by_key(
        ...
        let bounds = key.version.is_some() as u8 + key.root_version.is_some() as u8 + key.at_checkpoint.is_some() as u8;
        ...
        if bounds > 1 {
            ...
            Self::version_bounded(ctx, scope, key.address, v).await.map_err(upcast)
    ...
    pub(crate) async fn version_bounded(
        ...
        let Some(stored) = pg_loader
            .load_one(VersionBoundedObjectVersionKey(address.into(), root_version.into()))
            .await
    ...
    /// according to `scope`.
    pub(crate) async fn latest(ctx: &Context<'_>, scope: Scope, address: SuiAddress) -> Result<Option<Self>, RpcError> {
    ...
    pub(crate) async fn checkpoint_bounded(
        ...
        let Some(stored) = pg_loader
            .load_one(CheckpointBoundedObjectVersionKey(address.into(), at_checkpoint.into()))
            .await
            .context("Failed to fetch object versions")?
        else {
            return Ok(None);
    ...
    pub(crate) async fn at_version(
        ...
        let stored_version = pg_loader.load_one(VersionedObjectVersionKey(address.into(), version.into()));
        ...
        Ok(Some(Self::from_contents(scope.with_root_version(version.into()), c)))
    }
    ...
    ///
    /// Returns `None` when no checkpoint is set in scope (e.g. execution scope).
    fn from_stored_version(scope: Scope, stored: StoredObjVersion) -> Result<Option<Self>, RpcError> {
        let Some(checkpoint_viewed_at) = scope.checkpoint_viewed_at() else {
            return Ok(None);
        };
        ...
        let address = Address::with_address(
            scope,
            NativeSuiAddress::from_bytes(stored.object_id).context("Failed to deserialize SuiAddress")?,
        );
    ...
    pub(crate) async fn paginate_by_version(
        ...
        let mut results: Vec<StoredObjVersion> = c.results(query).await.context("Failed to read from database")?;

        if !page.is_from_front() {
            results.reverse();
        }

        let (prev, next, results) = page.paginate_results(results, |v| JsonCursor::new(v.object_version as u64));
    ...
    pub(crate) async fn paginate_live(
        ...
        let refs = match filter {
            ...
            ObjectFilter { owner_kind: None, owner: None, type_: Some(type_) } => {
                consistent_reader
            ...
            _ => return Err(anyhow!("Invalid ObjectFilter not caught by validation: {filter:?}").into()),
        }
        .map_err(|e| match e {
            consistent_reader::Error::NotConfigured => feature_unavailable("paginating the live object set"),

            consistent_reader::Error::OutOfRange(_) => bad_user_input(Error::OutOfRange(checkpoint)),

            consistent_reader::Error::Internal(error) => error.context("Failed to fetch live objects").into(),
        })?;
        ...
        for edge in refs.results {
            ...
            let object =
                Object { super_: address, version_digest: Some((version, digest)), contents: Arc::new(OnceCell::new()) };
            conn.edges.push(Edge::new(cursor.encode_cursor(), object));
        }
    ...
    pub(crate) async fn singleton(
        ...
        let filter = ObjectFilter { type_: Some(TypeFilter::Type(type_)), owner_kind: None, owner: None };
    ...
    pub(crate) async fn contents(&self, ctx: &Context<'_>) -> Result<&Option<NativeObject>, RpcError> {
        self.contents
            .get_or_try_init(async || {
                ...
                let version = if let Some((version, _)) = self.version_digest {
                    ...
                    let Some(stored) = pg_loader
                        .load_one(CheckpointBoundedObjectVersionKey(self.super_.address.into(), cp))
                        .await
                ...
                if let Some(cached_object) =
                    self.super_.scope.execution_output_object(self.super_.address.into(), version.into())
                {
...
impl VersionFilter {
    ...
    pub(crate) fn intersect(self, other: Self) -> Option<Self> {
        ...
        match (a.map(u64::from), b.map(u64::from)) {
            ...
            _ => Some(Self { after_version: a, before_version: b }),
        }
    }
}
