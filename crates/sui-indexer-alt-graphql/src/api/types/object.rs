...
#[graphql(
    name = "IObject",
    field(name = "version", ty = "UInt53", desc = "The version of this object that this content comes from.",),
    field(
        name = "digest",
        ty = "String",
...
impl Object {
    ...
    async fn as_move_package(&self, ctx: &Context<'_>) -> Result<Option<MovePackage>, RpcError<Error>> {
    ...
    async fn object_at(
        ...
        ObjectImpl::from(self).object_at(ctx, version, root_version, checkpoint).await
    }
    ...
    async fn object_versions_after(
        ...
        ObjectImpl::from(self).object_versions_after(ctx, first, after, last, before, filter).await
    }
    ...
    async fn object_versions_before(
        ...
        ObjectImpl::from(self).object_versions_before(ctx, first, after, last, before, filter).await
    }

    /// The transaction that created this version of the object.
    async fn previous_transaction(&self, ctx: &Context<'_>) -> Result<Option<Transaction>, RpcError<Error>> {
...
impl Object {
    ...
    pub(crate) fn with_ref(addressable: Addressable, version: SequenceNumber, digest: ObjectDigest) -> Self {
        Self { super_: addressable, version, digest, contents: None }
    }
    ...
    pub(crate) fn from_contents(scope: Scope, contents: Arc<NativeObject>) -> Self {
        ...
        Self { super_: addressable, version: contents.version(), digest: contents.digest(), contents: Some(contents) }
    }
    ...
    pub(crate) fn from_stored_version(scope: Scope, stored: StoredObjVersion) -> Result<Option<Self>, RpcError<Error>> {
    ...
    pub(crate) async fn paginate_by_version(
        ...
        query = if page.is_from_front() {
            query.order_by(v::cp_sequence_number).then_order_by(v::object_version)
        } else {
            query.order_by(v::cp_sequence_number.desc()).then_order_by(v::object_version.desc())
        };
    ...
    /// store.
    pub(crate) async fn contents(&self, ctx: &Context<'_>) -> Result<Option<Arc<NativeObject>>, RpcError<Error>> {
...
impl ObjectImpl<'_> {
    ...
    pub(crate) async fn object_at(
        ...
        let key = ObjectKey { address: self.0.super_.address.into(), version, root_version, at_checkpoint: checkpoint };

        Object::by_key(ctx, self.0.super_.scope.clone(), key).await
    }

    pub(crate) async fn object_bcs(&self, ctx: &Context<'_>) -> Result<Option<Base64>, RpcError<Error>> {
    ...
    pub(crate) async fn object_versions_after(
        ...
        let Some(filter) = filter
            .unwrap_or_default()
            .intersect(VersionFilter { after_version: Some(self.0.version.value().into()), ..VersionFilter::default() })
        else {
            return Ok(Connection::new(false, false));
        };

        Object::paginate_by_version(ctx, self.0.super_.scope.clone(), page, self.0.super_.address, filter).await
    }
    ...
    pub(crate) async fn previous_transaction(&self, ctx: &Context<'_>) -> Result<Option<Transaction>, RpcError<Error>> {
        let Some(object) = self.0.contents(ctx).await? else {
            return Ok(None);
        };

        Ok(Some(Transaction::with_id(self.0.super_.scope.clone(), object.as_ref().previous_transaction)))
    }
}
