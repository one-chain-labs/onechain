...
use sui_indexer_alt_reader::{
    packages::{CheckpointBoundedOriginalPackageKey, PackageOriginalIdKey, VersionedOriginalPackageKey},
    pg_reader::PgReader,
};
...
pub(crate) enum Error {
    ...
    #[error("At most one of a version, or a checkpoint bound can be specified when fetching a package")]
    OneBound,
}
...
impl MovePackage {
    ...
    /// Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::oct::OCT`), owned by this address.
    ///
    ...
    pub(crate) async fn default_suins_name(&self, ctx: &Context<'_>) -> Result<Option<String>, RpcError> {
        self.super_.default_suins_name(ctx).await
    }

    /// The module named `name` in this package.
    async fn module(&self, ctx: &Context<'_>, name: String) -> Result<Option<MoveModule>, RpcError> {
    ...
    async fn modules(
        ...
        conn.has_previous_page = modules
            .first()
            .is_some_and(|fst| parsed.modules().range::<String, _>((B::Unbounded, B::Excluded(fst))).next().is_some());

        conn.has_next_page = modules
            .last()
            .is_some_and(|lst| parsed.modules().range::<String, _>((B::Excluded(lst), B::Unbounded)).next().is_some());
    ...
    async fn module_bcs(&self, ctx: &Context<'_>) -> Result<Option<Base64>, RpcError> {
        ...
        let bytes = bcs::to_bytes(native.serialized_module_map()).context("Failed to serialize module map")?;
        Ok(Some(bytes.into()))
    }

    /// Fetch the total balances keyed by coin types (e.g. `0x2::oct::OCT`) owned by this address.
    ///
    ...
    pub(crate) async fn objects(
        ...
        self.super_.objects(ctx, first, after, last, before, filter).await
    }
    ...
    pub(crate) async fn object_at(
        ...
        self.super_.object_at(ctx, version, root_version, checkpoint).await
    }
    ...
    pub(crate) async fn object_versions_after(
        ...
        self.super_.object_versions_after(ctx, first, after, last, before, filter).await
    }
    ...
    pub(crate) async fn object_versions_before(
        ...
        self.super_.object_versions_before(ctx, first, after, last, before, filter).await
    }
    ...
    async fn package_at(
        ...
        MovePackage::by_key(ctx, self.super_.super_.scope.clone(), PackageKey {
    ...
    async fn package_versions_after(
        ...
        let Some(filter) = filter
            .unwrap_or_default()
            .intersect(VersionFilter { after_version: Some(version), ..VersionFilter::default() })
        else {
    ...
    async fn package_versions_before(
        ...
        let Some(filter) = filter
            .unwrap_or_default()
            .intersect(VersionFilter { before_version: Some(version), ..VersionFilter::default() })
        else {
            return Ok(Some(Connection::new(false, false)));
        };
    ...
    pub(crate) async fn previous_transaction(&self, ctx: &Context<'_>) -> Result<Option<Transaction>, RpcError> {
    ...
    async fn linkage(&self, ctx: &Context<'_>) -> Result<Option<Vec<Linkage>>, RpcError> {
        ...
        let linkage =
            native.linkage_table().iter().map(|(object_id, upgrade_info)| Linkage { object_id, upgrade_info }).collect();

        Ok(Some(linkage))
    }

    /// The SUI returned to the sponsor or sender of the transaction that modifies or deletes this object.
    pub(crate) async fn storage_rebate(&self, ctx: &Context<'_>) -> Result<Option<BigInt>, RpcError> {
    ...
    pub(crate) async fn received_transactions(
        ...
        self.super_.received_transactions(ctx, first, after, last, before, filter).await
    }
    ...
    async fn type_origins(&self, ctx: &Context<'_>) -> Result<Option<Vec<TypeOrigin>>, RpcError> {
        ...
        let type_origins = native.type_origin_table().iter().map(|native| TypeOrigin::from(native.clone())).collect();
...
impl MovePackage {
    ...
    pub(crate) fn with_address(scope: Scope, address: NativeSuiAddress) -> Self {
        ...
        Self { super_, native: Arc::new(OnceCell::new()), parsed: Arc::new(OnceCell::new()) }
    }
    ...
    pub(crate) fn from_native_object(scope: Scope, native: NativeObject) -> Option<Self> {
        ...
        Some(Self { super_, native: Arc::new(OnceCell::from(Some(package))), parsed: Arc::new(OnceCell::new()) })
    }
    ...
    pub(crate) async fn from_object(object: &Object, ctx: &Context<'_>) -> Result<Option<Self>, RpcError> {
    ...
    pub(crate) async fn by_key(
        ...
        if bounds > 1 {
            ...
            Self::at_version(ctx, scope, key.address, v).await.map_err(upcast)
        } else if let Some(cp) = key.at_checkpoint {
            let scope =
                scope.with_checkpoint_viewed_at(cp.into()).ok_or_else(|| bad_user_input(Error::Future(cp.into())))?;

            Self::checkpoint_bounded(ctx, scope, key.address, cp).await.map_err(upcast)
        } else if let Some(cp) = scope.checkpoint_viewed_at() {
            Self::checkpoint_bounded(ctx, scope, key.address, cp.into()).await.map_err(upcast)
        } else {
            Ok(None)
        }
    }
    ...
    pub(crate) async fn at_version(
        ...
        let original_id =
            ObjectID::from_bytes(&stored_original.original_id).context("Failed to deserialize ObjectID")?;
    ...
    pub(crate) async fn checkpoint_bounded(
        ...
        let Some(stored_package) = pg_loader
            .load_one(CheckpointBoundedOriginalPackageKey(original_id, at_checkpoint.into()))
            .await
            .context("Failed to load package")?
        else {
            return Ok(None);
    ...
    pub(crate) fn from_stored(scope: Scope, stored: StoredPackage) -> Result<Option<Self>, RpcError> {
        if scope.checkpoint_viewed_at().is_none_or(|cp| stored.cp_sequence_number as u64 > cp) {
            return Ok(None);
        }

        let native: NativeObject =
            bcs::from_bytes(&stored.serialized_object).context("Failed to deserialize package as object")?;
        ...
        let super_ = Object::from_contents(scope, native);
        Ok(Some(Self { super_, native: Arc::new(OnceCell::from(Some(package))), parsed: Arc::new(OnceCell::new()) }))
    }
    ...
    pub(crate) async fn paginate_by_version(
        ...
        query = if page.is_from_front() {
            query.order_by(p::cp_sequence_number).then_order_by(p::package_version)
        } else {
            query.order_by(p::cp_sequence_number.desc()).then_order_by(p::package_version.desc())
        };
        ...
        let mut c = pg_reader.connect().await.context("Failed to connect to database")?;

        let mut results: Vec<StoredPackage> = c.results(query).await.context("Failed to read from database")?;

        if !page.is_from_front() {
            results.reverse();
        }

        let (prev, next, results) = page.paginate_results(results, |p| JsonCursor::new(p.package_version as u64));
    ...
    pub(crate) async fn paginate_by_checkpoint(
        ...
        query = if page.is_from_front() {
            query.order_by(p::cp_sequence_number).then_order_by(p::original_id).then_order_by(p::package_version)
        } else {
            query
    ...
    pub(crate) async fn native(&self, ctx: &Context<'_>) -> Result<&Option<NativeMovePackage>, RpcError> {
        self.native
            .get_or_try_init(async || {
                ...
                let native = contents.data.try_as_package().context("Object is not a MovePackage")?;
    ...
    pub(crate) async fn parsed(&self, ctx: &Context<'_>) -> Result<&Option<ParsedMovePackage>, RpcError> {
        self.parsed
            .get_or_try_init(async || {
                let Some(native) = self.native(ctx).await?.as_ref() else {
                    return Ok(None);
                ...
                let parsed = ParsedMovePackage::read_from_package(native).context("Failed to parse MovePackage")?;
    ...
    pub(crate) async fn paginate_system_packages(
        ...
        let query = query!(
            ...
            checkpoint as i64,
            pagination,
            if page.is_from_front() { query!("original_id") } else { query!("original_id DESC") },
            page.limit() as i64 + 2,
            checkpoint as i64,
        );
        ...
        let (prev, next, results) = page.paginate_results(results, |p| BcsCursor::new(p.original_id.clone()));
