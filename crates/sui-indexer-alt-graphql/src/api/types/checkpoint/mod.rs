...
struct CheckpointContents {
    ...
    #[allow(unused)]
    scope: Scope,
    contents: Option<(CheckpointSummary, NativeCheckpointContents, AuthorityStrongQuorumSignInfo)>,
}
...
impl Checkpoint {
    ...
    async fn query(&self) -> Result<Option<Query>, RpcError> {
        let scope =
            Some(self.scope.with_checkpoint_viewed_at(self.sequence_number).context("Checkpoint in the future")?);
...
impl CheckpointContents {
    ...
    async fn previous_checkpoint_digest(&self) -> Result<Option<String>, RpcError> {
        ...
        Ok(summary.previous_digest.as_ref().map(|digest| digest.base58_encode()))
    }
    ...
    async fn rolling_gas_summary(&self) -> Option<GasCostSummary> {
        let (summary, _, _) = self.contents.as_ref()?;
        Some(GasCostSummary::from(summary.epoch_rolling_gas_cost_summary.clone()))
    }
    ...
    async fn summary_bcs(&self) -> Result<Option<Base64>, RpcError> {
        ...
        Ok(Some(Base64::from(bcs::to_bytes(summary).context("Failed to serialize checkpoint summary")?)))
    }
    ...
    async fn content_bcs(&self) -> Result<Option<Base64>, RpcError> {
        ...
        Ok(Some(Base64::from(bcs::to_bytes(content).context("Failed to serialize checkpoint content")?)))
    }
    ...
    async fn validator_signatures(&self) -> Result<Option<ValidatorAggregatedSignature>, RpcError> {
        ...
        Ok(Some(ValidatorAggregatedSignature::with_authority_info(self.scope.clone(), authority_info.clone())))
    }
...
impl Checkpoint {
    ...
    pub(crate) fn with_sequence_number(scope: Scope, sequence_number: Option<u64>) -> Option<Self> {
        ...
        (sequence_number <= scope_checkpoint).then_some(Self { scope, sequence_number })
    }
...
impl CheckpointContents {
    ...
    pub(crate) async fn fetch(ctx: &Context<'_>, scope: Scope, sequence_number: u64) -> Result<Self, RpcError> {
        let kv_loader: &KvLoader = ctx.data()?;
        let contents =
            kv_loader.load_one_checkpoint(sequence_number).await.context("Failed to fetch checkpoint contents")?;
