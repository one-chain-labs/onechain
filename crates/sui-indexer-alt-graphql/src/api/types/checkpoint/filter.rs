...
pub(crate) fn checkpoint_bounds(
    ...
    cp_hi.checked_sub(cp_lo).map(|_| RangeInclusive::new(cp_lo, cp_hi))
}
...
pub(super) fn cp_unfiltered(cp_bounds: &RangeInclusive<u64>, page: &Page<CCheckpoint>) -> Vec<u64> {
    ...
    if page.is_from_front() {
        (pg_lo ..= pg_hi_inclusive).take(page.limit_with_overhead()).collect()
    } else {
        ...
        let mut results: Vec<_> = (pg_lo ..= pg_hi_inclusive).rev().take(page.limit_with_overhead()).collect();
        results.reverse();
        results
    }
}
...
pub(super) async fn cp_by_epoch(
    ...
    let results: Vec<CpBounds> = conn.results(query).await.context("Failed to execute epoch checkpoint query")?;
...
mod tests {
    ...
    fn test_checkpoint_bounds_no_filters() {
        assert_eq!(checkpoint_bounds(None, None, None, 5, 200).unwrap(), 5 ..= 200);
    }

    #[test]
    fn test_checkpoint_bounds_after_checkpoint() {
        assert_eq!(checkpoint_bounds(Some(10), None, None, 5, 200).unwrap(), 11 ..= 200);
    }

    #[test]
    fn test_checkpoint_bounds_before_checkpoint() {
        assert_eq!(checkpoint_bounds(None, None, Some(100), 5, 200).unwrap(), 5 ..= 99);
    }

    #[test]
    fn test_checkpoint_bounds_at_checkpoint() {
        assert_eq!(checkpoint_bounds(None, Some(50), None, 5, 200).unwrap(), 50 ..= 50);
    }

    #[test]
    fn test_checkpoint_bounds_combined_filters() {
        assert_eq!(checkpoint_bounds(Some(10), None, Some(100), 5, 200).unwrap(), 11 ..= 99);
    }

    #[test]
    fn test_checkpoint_bounds_at_checkpoint_precedence() {
        assert_eq!(checkpoint_bounds(Some(10), Some(50), Some(100), 5, 200).unwrap(), 50 ..= 50);
    }

    #[test]
    fn test_checkpoint_bounds_boundary_equal() {
        assert_eq!(checkpoint_bounds(None, None, None, 100, 100).unwrap(), 100 ..= 100);
    }

    #[test]
    fn test_checkpoint_bounds_reader_lo_override() {
        assert_eq!(checkpoint_bounds(Some(10), None, None, 50, 200).unwrap(), 50 ..= 200);
    }

    #[test]
    fn test_checkpoint_bounds_viewed_at_override() {
        assert_eq!(checkpoint_bounds(None, None, Some(100), 5, 80).unwrap(), 5 ..= 80);
    }
