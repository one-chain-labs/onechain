...
impl<F> MetricsFuture<F> {
        ...
        F: Future<Output = Response>,
    {
        ...
        MetricsFuture { metrics: Some(RequestMetrics { timer: guard, ext: ext.clone() }), inner }
    }
}
    ...
    F: Future<Output = Response>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if resp.is_ok() {
            ...
            if codes.is_empty() {
                ext.metrics.queries_failed.with_label_values(&["<UNKNOWN>"]).inc();
            }
