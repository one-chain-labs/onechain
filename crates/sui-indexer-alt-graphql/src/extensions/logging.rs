...
impl Session {
    pub(crate) fn new(addr: SocketAddr) -> Self {
        Self { request_id: Uuid::new_v4(), client_addr: addr }
    }
}

impl ExtensionFactory for Logging {
    fn create(&self) -> Arc<dyn Extension> {
        Arc::new(LoggingExt { session: Mutex::new(None), query: Mutex::new(None), metrics: self.0.clone() })
    }
}
...
impl Extension for LoggingExt {
    async fn request(&self, ctx: &ExtensionContext<'_>, next: NextRequest<'_>) -> Response {
        ...
        let Session { request_id: uuid, client_addr: addr } = self.session.lock().unwrap().unwrap();
        ...
        if response.is_ok() {
            ...
            if codes.is_empty() {
                self.metrics.queries_failed.with_label_values(&["<UNKNOWN>"]).inc();
            }
...
fn is_loud_query(codes: &[&str]) -> bool {
    codes.is_empty() || codes.iter().any(|c| matches!(*c, code::REQUEST_TIMEOUT | code::INTERNAL_SERVER_ERROR))
}
...
mod tests {
    ...
    async fn parsing_error_code() {
        ...
        assert_eq!(metrics.queries_failed.with_label_values(&[code::GRAPHQL_PARSE_FAILED]).get(), 1);
    }
    ...
    async fn validation_error_code() {
        ...
        let request = Request::from("query ($foo: String) { op }").data(Session::new("0.0.0.0:0".parse().unwrap()));
        ...
        assert!(response.is_err());
        assert_eq!(error_codes(&response), vec![code::GRAPHQL_VALIDATION_FAILED]);
        assert_eq!(metrics.queries_received.get(), 1);
        assert_eq!(metrics.queries_failed.with_label_values(&[code::GRAPHQL_VALIDATION_FAILED]).get(), 1);
    }
}
