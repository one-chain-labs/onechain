...
impl<'r> TxPayloadRule<'r> {
    ...
    fn has_tx_payload(&self, driver: &FieldDriver<'_, 'r>) -> bool {
        ...
        self.tx_payload_args.range(&(type_, field, "") ..).next().is_some_and(|&(t, f, _)| t == type_ && f == field)
    }
        ...
        macro_rules! debit {
            ($debit: expr) => {{
                ...
                if debit > self.tx_payload_budget as usize {
                    return Err(driver.err_at(value.pos, ErrorKind::PayloadSizeTx { limit: self.max_tx_payload_size }));
                } else {
    ...
    ///
    /// Fails if there is insufficient remaining budget.
    fn check_tx_var(&mut self, driver: &FieldDriver<'_, 'r>, pos: Pos, name: &'r Name) -> Result<(), Error> {
        ...
        macro_rules! debit {
            ($debit: expr) => {{
                let debit: usize = $debit;
                if debit > self.tx_payload_budget as usize {
                    return Err(driver.err_at(pos, ErrorKind::PayloadSizeTx { limit: self.max_tx_payload_size }));
                } else {
                    // SAFETY: We know that debit <= self.tx_payload_budget, which is a u32, so
                    // the cast and subtraction are both safe.
                    self.tx_payload_budget -= debit as u32;
...
pub(super) fn check(
    ...
    Ok(Usage { query_payload_size, tx_payload_size })
}
