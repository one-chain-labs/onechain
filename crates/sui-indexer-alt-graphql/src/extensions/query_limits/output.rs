...
use async_graphql::{parser::types::ExecutableDocument, registry::Registry, ServerResult, Value, Variables};
...
impl<'r> OutputNodeRule<'r, '_> {
    ...
    fn page_size(&self, driver: &FieldDriver<'_, 'r>) -> Result<Option<u32>, Error> {
        ...
        if size > limits.max as u64 {
            return Err(driver.err(ErrorKind::PageSizeTooLarge { limit: limits.max, actual: size }));
        }
    ...
    fn multi_get_size(&self, driver: &FieldDriver<'_, 'r>) -> Result<Option<u32>, Error> {
        ...
        if let Ok(Some(Value::List(vs))) = driver.resolve_arg("keys") {
            ...
            if keys > limit as usize {
                return Err(driver.err(ErrorKind::MultiGetTooLarge { limit, actual: keys }));
            }
...
impl<'r> Visitor<'r> for OutputNodeRule<'r, '_> {
    fn visit_field(&mut self, driver: &FieldDriver<'_, 'r>) -> Result<(), Error> {
        ...
        driver.visit_selection_set(&mut OutputNodeRule { budget: self.budget, multiplicity, page_size })
    }
}
...
pub(super) fn check(
    ...
    Driver::visit_document(registry, doc, variables, &mut OutputNodeRule {
        budget: &mut budget,
        multiplicity: 1,
        page_size: None,
    })?;

    Ok(Usage { nodes: limits.max_output_nodes - budget.output_node_budget })
}
