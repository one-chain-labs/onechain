...
use async_graphql::{parser::types::OperationType, ErrorExtensions, Name, PathSegment, Pos, ServerError};
...
impl From<Error> for ServerError {
    fn from(err: Error) -> Self {
        ...
        let async_graphql::Error { message, source, extensions } = kind.extend_with(|_, ext| ext.set("code", code));

        ServerError { message, source, locations: pos.into_iter().collect(), path, extensions }
    }
}
