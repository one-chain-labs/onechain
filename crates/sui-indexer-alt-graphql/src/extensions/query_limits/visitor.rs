...
impl<'r> FieldDriver<'_, 'r> {
    ...
    pub(super) fn resolve_arg(&self, name: &str) -> Result<Option<ConstValue>, Error> {
        let Some(val) = self.field.node.arguments.iter().find_map(|(n, v)| (n.node.as_str() == name).then_some(&v.node))
        else {
            return Ok(None);
        };
    ...
    pub(super) fn resolve_val(&self, val: Value) -> Result<ConstValue, Error> {
        val.into_const_with(|name| {
            self.resolve_var(&name).ok_or_else(|| self.err(ErrorKind::VariableNotFound(name))).cloned()
        })
    }
