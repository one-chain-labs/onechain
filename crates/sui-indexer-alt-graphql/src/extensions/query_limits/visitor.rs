...
impl<'r> FieldDriver<'_, 'r> {
    ...
    pub(super) fn resolve_arg(&self, name: &str) -> Option<ConstValue> {
        self.field
            ...
            .arguments
            .iter()
            .find_map(|(n, v)| (n.node.as_str() == name).then_some(self.resolve_val(v.node.clone())))
    }
    ...
    pub(super) fn resolve_val(&self, val: Value) -> ConstValue {
        let val: Result<_, Infallible> =
            val.into_const_with(|name| Ok(self.resolve_var(&name).cloned().unwrap_or(ConstValue::Null)));
    ...
    /// if there are any.
    pub(super) fn visit_selection_set<V: Visitor<'r> + ?Sized>(&self, visitor: &mut V) -> Result<(), Error> {
        self.driver.visit_selection_set(&self.meta_field.ty, self.chain.clone(), &self.field.node.selection_set, visitor)
    }
}
...
impl<'r> Driver<'r> {
    ...
    pub(super) fn visit_document<V: Visitor<'r> + ?Sized>(
        ...
        let driver = Self { registry, fragments: &doc.fragments, variables };
        ...
        for sel in &selection_set.node.items {
            match &sel.node {
                Selection::Field(field) => {
                    ...
                    let meta_field = if name == "__typename" {
                        ...
                        meta_type.field_by_name(name.as_str()).ok_or_else(|| {
                            Error::new(
                                ErrorKind::InternalError(anyhow!("Field '{type_}.{name}' not found in schema")),
                                Chain::path(&chain),
                                field.pos,
                            )
                        })?
                ...
                Selection::InlineFragment(inline_fragment) => {
                    self.visit_selection_set(
                        inline_fragment.node.type_condition.as_ref().map_or(type_, |cond| cond.node.on.node.as_str()),
                        chain.clone(),
