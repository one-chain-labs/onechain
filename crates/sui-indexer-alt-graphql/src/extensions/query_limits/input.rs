...
pub(super) fn check(limits: &QueryLimitsConfig, doc: &ExecutableDocument) -> ServerResult<Usage> {
    ...
    while let Some((chain, next)) = next_level.first() {
        if depth_budget == 0 {
            Err(Error::new(ErrorKind::InputNesting(limits.max_query_depth), Chain::path(chain), next.pos))?
        } else {
            depth_budget -= 1;
        }
        ...
        for (pred, selection) in curr_level.drain(..) {
            if node_budget == 0 {
                Err(Error::new(ErrorKind::InputNodes(limits.max_query_nodes), Chain::path(&pred), selection.pos))?
            } else {
                node_budget -= 1;
            }
            ...
            match &selection.node {
                ...
                Selection::FragmentSpread(fs) => {
                    ...
                    let def = doc.fragments.get(name).ok_or_else(|| {
                        Error::new(ErrorKind::UnknownFragment(name.as_str().to_owned()), Chain::path(&pred), fs.pos)
                    })?;
    ...
    Ok(Usage { nodes: limits.max_query_nodes - node_budget, depth: limits.max_query_depth - depth_budget })
}
