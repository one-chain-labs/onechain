...
use async_graphql::{
    extensions::{Extension, ExtensionContext, ExtensionFactory, NextParseQuery, NextRequest, NextValidation},
...
impl QueryLimitsChecker {
    pub(crate) fn new(limits: QueryLimitsConfig, metrics: Arc<RpcMetrics>) -> Self {
        Self { limits: Arc::new(limits), metrics }
    }
}
...
impl Extension for QueryLimitsCheckerExt {
    ...
    async fn parse_query(
        ...
        *self.doc.lock().unwrap() = Some(ParsedDocument { var: variables.clone(), doc: doc.clone() });
    ...
    async fn validation(
        ...
        let payload = payload::check(self.limits.as_ref(), length, &ctx.schema_env.registry, &doc, &var)?;

        let output = output::check(self.limits.as_ref(), pagination_config, &ctx.schema_env.registry, &doc, &var)?;
        ...
        self.metrics.query_payload_size.observe(payload.query_payload_size as f64);
        self.metrics.tx_payload_size.observe(payload.tx_payload_size as f64);
        self.metrics.output_nodes.observe(output.nodes as f64);

        if let Some(ShowUsage(_)) = ctx.data_opt() {
            *self.usage.lock().unwrap() = Some(Usage { input, payload, output });
        }
    ...
    async fn request(&self, ctx: &ExtensionContext<'_>, next: NextRequest<'_>) -> Response {
        ...
        if let Some(Usage { input, payload, output }) = self.usage.lock().unwrap().take() {
...
mod tests {
    ...
    impl Query {
        ...
        async fn p(&self, _first: Option<usize>, _last: Option<usize>) -> Connection<String, Query, Extra> {
            Connection::with_additional_fields(false, false, Extra)
        }

        /// A paginated field with a nullable return type.
        async fn q(&self, _first: Option<usize>, _last: Option<usize>) -> Option<Connection<String, Query, Extra>> {
            None
        }
    ...
    fn small_page() -> PageLimits {
        PageLimits { default: 5, max: 10 }
    }

    fn big_page() -> PageLimits {
        PageLimits { default: 10, max: 20 }
    }
    ...
    fn request(query: &str) -> Request {
        Request::from(query).data(ShowUsage(HeaderValue::from_static("true"))).data(ContentLength(query.len() as u64))
    }
    ...
    async fn test_pass_limits() {
        ...
        let usage = response.extensions.get("usage").unwrap().clone().into_json().unwrap();
    ...
    async fn test_variable_optional() {
        let schema = schema(config(), page());
        let response = schema.execute(request("query ($first: Int) { p(first: $first) { nodes { z } } }")).await;
    ...
    async fn test_variable_null() {
        let schema = schema(config(), page());
        let response = schema
            .execute(request("query ($first: Int) { p(first: $first) { nodes { z } } }").variables(
    ...
    async fn test_overall_payload_size_too_small() {
        let schema = schema(QueryLimitsConfig { max_query_payload_size: 5, max_tx_payload_size: 5, ..config() }, page());
    ...
    async fn test_query_payload_too_large() {
        let schema =
            schema(QueryLimitsConfig { max_query_payload_size: 5, max_tx_payload_size: 50, ..config() }, page());
    ...
    async fn test_tx_payload_too_large() {
        let schema =
            schema(QueryLimitsConfig { max_tx_payload_size: 5, max_query_payload_size: 50, ..config() }, page());
    ...
    async fn test_tx_payload_structured_variable() {
        ...
        let request = Request::from(query)
            ...
            .variables(Variables::from_json(serde_json::to_value(variables).unwrap()));
    ...
    async fn test_output_exceeded() {
        let schema = schema(
            QueryLimitsConfig { max_output_nodes: 40, max_query_depth: 10, ..config() },
    ...
    async fn test_output_page_default() {
        let pagination = PaginationConfig::new(10, small_page(), BTreeMap::from_iter([(("Query", "q"), big_page())]));
    ...
    async fn test_output_fallback_max_page_size_exceeded() {
        let schema = schema(
            config(),
            PaginationConfig::new(10, small_page(), BTreeMap::from_iter([(("Query", "p"), big_page())])),
        );

        let response = execute(&schema, &format!("{{ q(first: {}) {{ nodes {{ z }} }} }}", small_page().max + 1)).await;
    ...
    async fn test_output_override_max_page_size_met() {
        ...
        let response = execute(&schema, &format!("{{ p(first: {}) {{ nodes {{ z }} }} }}", small_page().max + 1)).await;
    ...
    async fn test_output_max_multi_get_size_exceeded() {
        let schema = schema(config(), PaginationConfig::new(4, small_page(), Default::default()));

        let response = execute(&schema, r#"{ multiGetQ(keys: ["a", "b", "c", "d", "e"]) { z } }"#).await;
