...
impl ExtensionFactory for Timeout {
    fn create(&self) -> Arc<dyn Extension> {
        Arc::new(TimeoutExt { config: self.0.clone(), is_mutation: AtomicBool::new(false) })
    }
}
...
impl Extension for TimeoutExt {
    async fn parse_query(
        ...
        self.is_mutation
            .store(document.operations.iter().any(|(_, op)| op.node.ty == OperationType::Mutation), Ordering::Relaxed);
    ...
    async fn execute(
        ...
        let limit = if is_mutation { self.config.mutation } else { self.config.query };

        timeout(limit, next.run(ctx, operation_name)).await.unwrap_or_else(|_| {
...
mod tests {
    ...
    async fn test_query_timeout_pass() {
        ...
        let response = Schema::build(Root(delay / 2), EmptyMutation, EmptySubscription)
            .extension(Timeout::new(TimeoutConfig { query: delay, mutation: zero }))
            .finish()
            .execute("query { op }")
            .await;
    ...
    async fn test_mutation_timeout_pass() {
        ...
        let response = Schema::build(Root(zero), Root(delay / 2), EmptySubscription)
            .extension(Timeout::new(TimeoutConfig { query: zero, mutation: delay }))
            .finish()
            .execute("mutation { op }")
            .await;
    ...
    async fn test_query_timeout_fail() {
        ...
        assert_eq!(error.extensions.as_ref().unwrap().get("code"), Some(&Value::String(code::REQUEST_TIMEOUT.into())))
    }
