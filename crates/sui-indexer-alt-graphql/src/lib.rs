...
use api::types::{address::IAddressable, move_datatype::IMoveDatatype, move_object::IMoveObject, object::IObject};
use async_graphql::{
...
use sui_indexer_alt_reader::{
    ...
    pg_reader::{db::DbArgs, PgReader},
    system_package_task::{SystemPackageTask, SystemPackageTaskArgs},
};
use task::{
    chain_identifier,
...
use crate::{
    ...
    extensions::logging::{Logging, Session},
    metrics::RpcMetrics,
    ...
    S: SubscriptionType + 'static,
{
    pub fn new(
        ...
        let RpcArgs { rpc_listen_address, no_ide } = args;
        ...
        Self { rpc_listen_address, with_ide: !no_ide, version, router, schema, metrics, cancel }
    }
    ...
    where
        ...
        S: SubscriptionType + 'static,
    {
        let Self { rpc_listen_address, with_ide, version, mut router, schema, metrics: _, cancel } = self;
        ...
        router = router
            ...
            .layer(axum::middleware::from_fn_with_state(Version(version), middleware::version::set_version))
            .layer(
                cors::CorsLayer::new().allow_methods([Method::POST]).allow_origin(cors::Any).allow_headers(cors::Any),
            );

        info!("Starting GraphQL service on {rpc_listen_address}");
        let listener =
            TcpListener::bind(rpc_listen_address).await.context("Failed to bind GraphQL to listen address")?;

        let service = axum::serve(listener, router.into_make_service_with_connect_info::<SocketAddr>())
...
impl Default for RpcArgs {
    fn default() -> Self {
        Self { rpc_listen_address: "0.0.0.0:7000".parse().unwrap(), no_ide: false }
    }
}
...
pub async fn start_rpc(
    ...
    let fullnode_client =
        FullnodeClient::new(Some("graphql_fullnode"), fullnode_args, registry, cancel.child_token()).await?;

    let pg_reader =
        PgReader::new(Some("graphql_db"), database_url.clone(), db_args, registry, cancel.child_token()).await?;

    let bigtable_reader = if let Some(instance_id) = bigtable_instance {
        let reader = BigtableReader::new(instance_id, "indexer-alt-graphql".to_owned(), bigtable_args, registry).await?;
    ...
    let consistent_reader =
        ConsistentReader::new(Some("graphql_consistent"), consistent_reader_args, registry, cancel.child_token())
            .await?;
    ...
    let system_package_task =
        SystemPackageTask::new(system_package_task_args, pg_reader.clone(), package_store.clone(), cancel.child_token());

    // Fetch and cache the chain identifier from the database.
    let chain_identifier =
        chain_identifier::task(&pg_reader, config.watermark.watermark_polling_interval, cancel.child_token()).await?;
    ...
    let rpc = rpc
        ...
        .extension(QueryLimitsChecker::new(config.limits.query_limits(), metrics))
...
async fn graphql(
    ...
    let mut request =
        request.into_inner().data(content_length).data(Session::new(addr)).data(watermark.read().await.clone());
...
mod tests {
    ...
    fn test_schema_sdl_export() {
        ...
        let file = if cfg!(feature = "staging") { "staging.graphql" } else { "schema.graphql" };
