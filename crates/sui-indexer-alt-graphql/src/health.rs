...
pub(crate) async fn check(
    ...
    let max_lag = params.max_checkpoint_lag_ms.map(Duration::from_millis).unwrap_or(config.max_checkpoint_lag);

    if lag.is_some_and(|l| l > max_lag) {
        errors.push("Watermark lag is too high".to_owned());
    }

    Response { checkpoint_lag_ms: lag.map(|l| l.as_millis() as u64), errors }
}
...
async fn check_connection(url: &Option<Url>) -> anyhow::Result<()> {
    ...
    let addrs = url.socket_addrs(|| None).map_err(|_| anyhow!("Could not resolve URL"))?;

    TcpStream::connect(addrs.as_slice()).await.map_err(|_| anyhow!("Failed to establish TCP connection"))?;
...
impl IntoResponse for Response {
    fn into_response(self) -> AxumResponse {
        let status = if self.errors.is_empty() { StatusCode::OK } else { StatusCode::INTERNAL_SERVER_ERROR };
