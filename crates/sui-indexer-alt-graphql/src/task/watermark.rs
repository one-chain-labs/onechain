...
use crate::{
    api::types::available_range::pipeline_unavailable,
...
impl WatermarkTask {
    pub(crate) fn new(
        ...
        let WatermarkConfig { watermark_polling_interval } = config;
...
impl Watermarks {
    ...
    pub(crate) fn pipeline_lo_watermark(&self, pipeline: &str) -> Result<&Watermark, RpcError> {
        self.pipeline_lo.get(pipeline).ok_or_else(|| pipeline_unavailable(pipeline))
    }
    ...
    fn merge(&mut self, row: WatermarkRow) {
        ...
        self.timestamp_ms_hi_inclusive = self.timestamp_ms_hi_inclusive.min(row.timestamp_ms_hi_inclusive);

        self.pipeline_lo.insert(row.pipeline.clone(), Watermark {
            epoch: row.epoch_lo,
...
impl WatermarkRow {
    async fn read(
        ...
        let ledger_grpc: OptionFuture<_> = ledger_grpc_reader.map(watermark_from_ledger_grpc).into();
        ...
        let bigtable = bigtable.transpose().context("Failed to read watermarks from Bigtable")?;
        rows.extend(bigtable);

        let ledger_grpc = ledger_grpc.transpose().context("Failed to read watermarks from Ledger gRPC")?;
    ...
    fn record_metrics(&self, metrics: &Arc<RpcMetrics>) {
        metrics.watermark_epoch.with_label_values(&[&self.pipeline]).set(self.epoch_hi_inclusive);

        metrics.watermark_checkpoint.with_label_values(&[&self.pipeline]).set(self.checkpoint_hi_inclusive);

        metrics.watermark_transaction.with_label_values(&[&self.pipeline]).set(self.tx_hi);

        metrics.watermark_timestamp_ms.with_label_values(&[&self.pipeline]).set(self.timestamp_ms_hi_inclusive);

        metrics.watermark_reader_epoch_lo.with_label_values(&[&self.pipeline]).set(self.epoch_lo);

        metrics.watermark_reader_checkpoint_lo.with_label_values(&[&self.pipeline]).set(self.checkpoint_lo);

        metrics.watermark_reader_transaction_lo.with_label_values(&[&self.pipeline]).set(self.tx_lo);
    }
}
...
impl Default for Watermarks {
    fn default() -> Self {
        Self {
            global_hi: Watermark { epoch: i64::MAX, checkpoint: i64::MAX, transaction: i64::MAX },
...
async fn watermark_from_ledger_grpc(ledger_grpc_reader: &LedgerGrpcReader) -> anyhow::Result<WatermarkRow> {
    let summary = ledger_grpc_reader.checkpoint_watermark().await.context("Failed to get checkpoint watermark")?;
...
async fn watermarks_from_pg(pg_reader: &PgReader, pg_pipelines: &[String]) -> anyhow::Result<Vec<WatermarkRow>> {
    let mut conn = pg_reader.connect().await.context("Failed to connect to database")?;
    ...
    ensure!(!pg_pipelines.is_empty(), "Indexer not tracking any pipelines");
    ...
    ensure!(remaining_pipelines.is_empty(), "Missing watermarks for {remaining_pipelines:?}",);
