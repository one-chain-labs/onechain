...
impl Watermarks {
    ...
    pub(crate) fn pipeline_lo_watermark(&self, pipeline: &str) -> anyhow::Result<&Watermark> {
        self.pipeline_lo.get(pipeline).ok_or_else(|| anyhow!("'{pipeline}' not found in pipeline_lo watermarks"))
    }
...
impl WatermarkRow {
    async fn read(
        ...
        let last = last.transpose().context("Failed to read watermarks from Bigtable")?;
