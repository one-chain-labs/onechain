...
async fn fetch(conn: &mut Connection<'_>) -> anyhow::Result<ChainIdentifier> {
    ...
    let digest_bytes: Vec<_> =
        conn.first(g::kv_genesis.select(g::genesis_digest)).await.context("Failed to fetch digest information")?;

    let digest = CheckpointDigest::try_from(digest_bytes).context("Failed to deserialize genesis digest")?;
