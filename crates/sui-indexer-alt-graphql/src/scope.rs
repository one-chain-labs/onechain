...
use sui_rpc::proto::sui::rpc::{v2 as grpc, v2::changed_object::OutputObjectState};
use sui_types::{
...
impl Scope {
    ...
    pub(crate) fn execution_output_object(&self, object_id: ObjectID, version: SequenceNumber) -> Option<&NativeObject> {
        self.execution_objects.get(&(object_id, version)).and_then(|opt| opt.as_ref())
    }

    /// Get the latest version of an object from the execution context cache, if available.
    /// Returns None if the object doesn't exist in the cache or if it was deleted/wrapped.
    pub(crate) fn execution_output_object_latest(&self, object_id: ObjectID) -> Option<&NativeObject> {
        self.execution_objects.range(..= (object_id, SequenceNumber::MAX)).last().and_then(|(_, opt)| opt.as_ref())
    }
...
fn extract_objects_from_executed_transaction(
    ...
    if let Some(object_set) = &executed_transaction.objects {
        for obj in &object_set.objects {
            if let Some(bcs) = &obj.bcs {
                let native_obj: NativeObject = bcs.deserialize().context("Failed to deserialize object BCS")?;
    ...
    if let Some(effects) = &executed_transaction.effects {
        ...
        let lamport_version =
            SequenceNumber::from_u64(effects.lamport_version.context("Effects should have lamport_version")?);
