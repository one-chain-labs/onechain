...
impl PaginationConfig {
    pub(crate) fn new(
        ...
        Self { max_multi_get_size, fallback, overrides }
    }
    ...
    pub(crate) fn from_params(
        ...
        let page = match (first, after, last, before) {
            ...
            (limit, after, None, before) => {
                Page { after, before, limit: limit.unwrap_or(limits.default as u64), end: End::Front }
            }
        ...
        if page.limit > limits.max as u64 {
            return Err(Error::TooLarge { limit: page.limit, max: limits.max });
        }
...
impl Page<JsonCursor<usize>> {
    ...
    pub(crate) fn paginate_indices<N: OutputType, E>(
        ...
        for i in lo .. hi {
            conn.edges.push(Edge::new(JsonCursor::new(i).encode_cursor(), node(i)?));
        }
...
impl<C: CursorType + Eq + PartialEq + Clone> Page<C> {
    ...
    pub(crate) fn paginate_results<T, N: OutputType, E>(
        ...
        let (prev, next, prefix, suffix) = match (self.after(), first, last, self.before(), self.end) {
...
mod tests {
    ...
    fn test_default_page() {
        let limits = PageLimits { default: 10, max: 100 };

        assert_eq!(Page::<usize>::from_params(&limits, None, None, None, None).unwrap(), Page {
            after: None,
            before: None,
    ...
    fn test_page_forward() {
        ...
        assert_eq!(Page::from_params(&limits, None, Some(1), None, None).unwrap(), Page {
            after: Some(1),
            before: None,
            limit: limits.default as u64,
            end: End::Front
        ...
        assert_eq!(Page::from_params(&limits, None, None, None, Some(10)).unwrap(), Page {
            after: None,
            before: Some(10),
            limit: limits.default as u64,
            end: End::Front
        ...
        assert_eq!(Page::from_params(&limits, Some(5), Some(1), None, None).unwrap(), Page {
            after: Some(1),
            before: None,
            limit: 5,
            end: End::Front
    ...
    fn test_page_backward() {
        ...
        assert_eq!(Page::from_params(&limits, None, None, Some(5), Some(10)).unwrap(), Page {
            after: None,
            before: Some(10),
            limit: 5,
            end: End::Back
    ...
    fn test_page_both() {
        ...
        assert!(matches!(Page::<usize>::from_params(&limits, Some(5), None, Some(5), None), Err(Error::FirstAndLast)));
    }
    ...
    fn test_page_too_large() {
        ...
        assert!(matches!(
            ...
            Err(Error::TooLarge { limit: 1000, max: 100 })
        ));
    }

    #[test]
    fn test_paginate_results() {
        let limits = PageLimits { default: 5, max: 100 };
        ...
        let actual: Vec<(String, usize)> = conn.edges.into_iter().map(|e| (e.cursor, e.node)).collect();
    ...
    fn test_paginate_results_not_enough() {
        let limits = PageLimits { default: 5, max: 10 };
    ...
    fn test_paginate_results_backward_limited() {
        ...
        let page: Page<usize> = Page::from_params(&limits, None, None, Some(limits.default as u64), None).unwrap();
