...
impl<E: std::error::Error> From<RpcError<E>> for async_graphql::Error {
    fn from(err: RpcError<E>) -> Self {
        match err {
            ...
            RpcError::FeatureUnavailable { what } => format!("{what} not available").extend_with(|_, ext| {
            ...
            RpcError::RequestTimeout { kind, limit } => format!("{kind} timed out after {:.2}s", limit.as_secs_f64())
                .extend_with(|_, ext| {
...
impl<E: std::error::Error> From<RpcError<E>> for async_graphql::ServerError {
    fn from(err: RpcError<E>) -> Self {
        let async_graphql::Error { message, source, extensions } = async_graphql::Error::from(err);

        async_graphql::ServerError { message, source, locations: vec![], path: vec![], extensions }
    }
}
...
pub(crate) fn error_codes(response: &Response) -> Vec<&str> {
    response
        ...
        .filter_map(|code| if let Value::String(code) = code { Some(code.as_str()) } else { None })
        .collect()
}
...
mod tests {
    ...
    fn test_graphql_error() {
        let err: async_graphql::Error = RpcError::<Infallible>::from(async_graphql::Error::new("Boom!")).into();
    ...
    fn test_graphql_error_existing_code() {
        let err: async_graphql::Error = RpcError::<Infallible>::from(
            async_graphql::Error::new("Boom!").extend_with(|_, ext| ext.set("code", code::BAD_USER_INPUT)),
        )
        .into();
    ...
    fn test_internal_error() {
        let err: async_graphql::Error =
            RpcError::<Infallible>::from(anyhow!("Root cause").context("Immediate predecessor").context("Main message"))
                .into();
        ...
        let ext = err.extensions.as_ref().expect("No extensions");
        assert_eq!(ext.get("code"), Some(&code::INTERNAL_SERVER_ERROR.into()));
        assert_eq!(ext.get("chain"), Some(&vec!["Immediate predecessor", "Root cause"].into()));
    }
