...
impl JsonRpcServerBuilder {
    ...
    pub async fn to_router(&self, server_type: ServerType) -> Result<axum::Router, Error> {
        ...
        let middleware =
            ServiceBuilder::new().layer(Self::trace_layer()).map_request(move |mut request: http::Request<_>| {
        ...
        match server_type {
            ServerType::WebSocket => {
                let service = JsonRpcService(service_builder.ws_only().to_service_builder().build(module, stop_handle));
                router = router
            ...
            ServerType::Http => {
                let service =
                    JsonRpcService(service_builder.http_only().to_service_builder().build(module, stop_handle));
                router = router
            ...
            ServerType::Both => {
                let service = JsonRpcService(service_builder.to_service_builder().build(module, stop_handle));
                router = router
                    .route("/", axum::routing::post_service(service.clone()))
                    .route("/", axum::routing::get_service(service.clone()))
