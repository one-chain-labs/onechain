...
impl TransactionBuilderServer for TransactionBuilderApi {
    async fn transfer_object(
        ...
        let data =
            self.0.transfer_object(signer, object_id, gas, *gas_budget, recipient).await.map_err(crate::Error::from)?;
    ...
    async fn batch_transaction(
        ...
        Ok(TransactionBlockBytes::from_data(
            self.0.batch_transaction(signer, params, gas, *gas_budget).await.map_err(crate::Error::from)?,
        )
        .map_err(crate::Error::from)?)
    }
