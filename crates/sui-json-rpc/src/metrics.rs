...
impl Metrics {
    ...
    fn on_request(&self, request: &jsonrpsee::types::Request<'_>) {
        ...
        self.inflight_requests_by_route.with_label_values(&[method_name]).inc();
        self.requests_by_route.with_label_values(&[method_name]).inc();

        self.rpc_request_size.with_label_values(&[method_name]).observe(request.params().len_bytes() as f64);
    }

    fn on_response(&self, method_name: &str, started_at: Instant, response: &jsonrpsee::MethodResponse) {
        let method_name = self.check_spam(method_name);
        self.inflight_requests_by_route.with_label_values(&[method_name]).dec();
        let req_latency_secs = (Instant::now() - started_at).as_secs_f64();
        self.req_latency_by_route.with_label_values(&[method_name]).observe(req_latency_secs);
        ...
        if let Some(code) = response.as_error_code() {
            ...
            {
                self.server_errors_by_route.with_label_values(&[method_name]).inc();
            } else if code == TRANSIENT_ERROR_CODE {
                self.transient_errors_by_route.with_label_values(&[method_name]).inc();
            } else {
                self.client_errors_by_route.with_label_values(&[method_name]).inc();
            }
            self.errors_by_route.with_label_values(&[method_name]).inc();
        }

        self.rpc_response_size.with_label_values(&[method_name]).observe(response.as_result().len() as f64)
    }

    pub fn on_http_request(&self, headers: &axum::http::HeaderMap<axum::http::HeaderValue>) {
        let client_type = headers.get(CLIENT_SDK_TYPE_HEADER).and_then(|v| v.to_str().ok()).unwrap_or("Unknown");

        let api_version =
            headers.get(CLIENT_TARGET_API_VERSION_HEADER).and_then(|v| v.to_str().ok()).unwrap_or("Unknown");
        self.client.with_label_values(&[client_type, api_version]).inc();
    }
}
    ...
    S::Future: 'a,
{
    ...
    fn call(&self, req: jsonrpsee::types::Request<'a>) -> Self::Future {
        ...
        async move {
            match future.await {
                Ok(res) => res,
                Err(_) => jsonrpsee::MethodResponse::error(Id::Null, ErrorObject::from(ErrorCode::InternalError)),
            }
        }
        .boxed()
    }
