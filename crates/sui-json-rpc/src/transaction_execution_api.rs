...
use sui_types::{
    ...
    quorum_driver_types::{ExecuteTransactionRequestType, ExecuteTransactionRequestV3, ExecuteTransactionResponseV3},
...
impl TransactionExecutionApi {
    ...
    pub fn convert_bytes<T: serde::de::DeserializeOwned>(&self, tx_bytes: Base64) -> Result<T, SuiRpcInputError> {
    ...
    > {
        ...
        let raw_transaction = if opts.show_raw_input { bcs::to_bytes(txn.data())? } else { vec![] };
        let transaction = if opts.show_input {
            let epoch_store = self.state.load_epoch_store_one_call_per_task();
            Some(SuiTransactionBlock::try_from(txn.data().clone(), epoch_store.module_cache())?)
        } else {
            None
        };
        ...
        Ok((request, opts, sender, input_objs, txn, transaction, raw_transaction))
    }
    ...
    async fn execute_transaction_block(
        ...
        let request_type = request_type.unwrap_or(ExecuteTransactionRequestType::WaitForEffectsCert);
    ...
    async fn handle_post_orchestration(
        ...
        let events = if opts.show_events {
            ...
            let mut layout_resolver = epoch_store.executor().type_layout_resolver(Box::new(backing_package_store));
        ...
        let balance_changes = match &object_cache {
            Some(object_cache) if opts.show_balance_changes => {
                Some(get_balance_changes_from_effect(object_cache, &response.effects.effects, input_objs, None).await?)
            }
            _ => None,
        };
        ...
        let raw_effects = if opts.show_raw_effects { bcs::to_bytes(&response.effects.effects)? } else { vec![] };

        Ok(SuiTransactionBlockResponse {
            digest,
            transaction,
            raw_transaction,
            effects: opts.show_effects.then_some(response.effects.effects.try_into()?),
            events,
            object_changes,
            balance_changes,
            timestamp_ms: None,
    ...
    async fn dry_run_transaction_block(&self, tx_bytes: Base64) -> Result<DryRunTransactionBlockResponse, Error> {
        ...
        let (resp, written_objects, transaction_effects, mock_gas) =
            self.state.dry_exec_transaction(txn_data.clone(), txn_digest).await?;
        let object_cache = ObjectProviderCache::new_with_cache(self.state.clone(), written_objects);
        let balance_changes =
            get_balance_changes_from_effect(&object_cache, &transaction_effects, input_objs, mock_gas).await?;
...
impl WriteApiServer for TransactionExecutionApi {
    ...
    async fn execute_transaction_block(
        ...
        with_tracing!(Duration::from_secs(10), async move {
            self.execute_transaction_block(tx_bytes, signatures, opts, request_type).await
        })
    }
    ...
    async fn dev_inspect_transaction_block(
        ...
        with_tracing!(async move {
            let DevInspectArgs { gas_sponsor, gas_budget, gas_objects, show_raw_txn_data_and_effects, skip_checks } =
