...
use crate::{authority_state::StateRead, error::Error, with_tracing, SuiRpcModule};
...
impl BridgeReadApiServer for BridgeReadApi {
    ...
    async fn get_latest_bridge(&self) -> RpcResult<BridgeSummary> {
        with_tracing!(async move {
            self.state.get_bridge().map_err(Error::from)?.try_into_bridge_summary().map_err(Error::from)
        })
    }
    ...
    async fn get_bridge_object_initial_shared_version(&self) -> RpcResult<u64> {
        with_tracing!(async move {
            Ok(get_bridge_obj_initial_shared_version(self.state.get_object_store())?
                .ok_or(Error::UnexpectedError("Failed to find Bridge object initial version".to_string()))?
                .into())
        })
    }
}
