...
impl<S> TrafficControllerService<S> {
    pub fn new(service: S, traffic_controller: Option<Arc<TrafficController>>) -> Self {
        Self { inner: service, traffic_controller }
    }
}
...
pub fn determine_client_ip<T>(client_id_source: ClientIdSource, request: &mut axum::http::Request<T>) {
    let headers = request.headers();
    let client = match client_id_source {
        ClientIdSource::SocketAddr => request.extensions().get::<ConnectInfo<SocketAddr>>().map(|info| info.0.ip()),
