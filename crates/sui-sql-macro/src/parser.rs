...
pub(crate) enum Error {
    ...
    Unexpected { offset: usize, actual: Token, expect: Token },
...
impl<'l, 's> Parser<'l, 's> {
    ...
    fn eat(&mut self, expect: Token) -> Result<Lexeme<'s>, Error> {
        match self.peek(expect) {
            ...
            Err(Some(l)) => Err(Error::Unexpected { offset: l.1, actual: l.0, expect }),
...
mod tests {
    ...
    fn test_no_binds() {
        ...
        assert_eq!(parse("foo").unwrap(), Format { head: "foo".into(), tail: vec![] });
    }
    ...
    fn test_single_bind() {
        ...
        // string in two.
        assert_eq!(parse("foo = {Text} AND bar = 42").unwrap(), Format {
            head: "foo = ".into(),
    ...
    fn test_multiple_binds() {
        ...
        assert_eq!(parse("foo = {Text} AND (bar < {BigInt} OR bar > 5)").unwrap(), Format {
            head: "foo = ".into(),
            tail: vec![(type_("Text"), " AND (bar < ".into()), (type_("BigInt"), " OR bar > 5)".into()),],
        },);
    }
    ...
    fn test_ends_with_a_bind() {
        ...
        // binder.
        assert_eq!(parse("bar BETWEEN {BigInt} AND {BigInt}").unwrap(), Format {
            head: "bar BETWEEN ".into(),
            tail: vec![(type_("BigInt"), " AND ".into()), (type_("BigInt"), "".into()),],
        },);
    }
    ...
    fn test_escaped_curlies() {
        ...
        assert_eq!(parse("foo LIKE '{{bar%'").unwrap(), Format { head: "foo LIKE '{bar%'".into(), tail: vec![] },);
    }

    #[test]
    fn test_curly_nest() {
        // This input can be tricky to parse if the lexer treats escaped curlies as a single token.
        assert_eq!(parse("{{{Bool}}}").unwrap(), Format { head: "{".into(), tail: vec![(type_("Bool"), "}".into())] },);
    }

    #[test]
    fn test_bind_unexpected_token() {
        // Error if the binder is not properly closed.
        assert!(matches!(parse("{Bool{").unwrap_err(), Error::Unexpected {
            offset: 5,
            actual: Token::LCurl,
            expect: Token::RCurl
        },));
    ...
    fn test_bind_no_type() {
        ...
        assert_eq!(parse("foo = {}").unwrap(), Format { head: "foo = ".into(), tail: vec![(None, "".into())] });
    }

    #[test]
    fn test_bind_unfinished() {
        // Error if the binder does not contain a type.
        assert!(matches!(parse("foo = {").unwrap_err(), Error::UnexpectedEos { expect: Token::Text },));
    }

    #[test]
    fn test_bind_no_rcurly() {
        // Error if the binder ends before it is closed.
        assert!(matches!(parse("foo = {Text").unwrap_err(), Error::UnexpectedEos { expect: Token::RCurl },));
    }

    #[test]
    fn test_bind_bad_type() {
        // Failure to parse the binder as a Rust type is also an error for this parser.
        assert!(matches!(parse("foo = {not a type}").unwrap_err(), Error::TypeParse { offset: 7, .. },));
    }
}
