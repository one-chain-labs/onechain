...
impl Parse for SqlInput {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if input.is_empty() {
            return Ok(Self { return_, format_, binds: Punctuated::new() });
        }

        input.parse::<Token![,]>()?;
        let binds = Punctuated::parse_terminated(input)?;

        Ok(Self { return_, format_, binds })
    }
}
...
impl Parse for QueryInput {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if input.is_empty() {
            return Ok(Self { format_, binds: Punctuated::new() });
        }
...
pub fn sql(input: TokenStream) -> TokenStream {
    let SqlInput { return_, format_, binds, .. } = parse_macro_input!(input as SqlInput);
