...
use mysten_common::{
    ...
    fatal,
    random::get_rng,
    sync::notify_read::{NotifyRead, CHECKPOINT_BUILDER_NOTIFY_READ_TASK_NAME},
};
...
use sui_types::{
    ...
    digests::{CheckpointContentsDigest, CheckpointDigest, Digest, TransactionEffectsDigest},
    ...
    sui_system_state::{epoch_start_sui_system_state::EpochStartSystemStateTrait, SuiSystemState, SuiSystemStateTrait},
...
use typed_store::{
    rocks::{default_db_options, DBMap, DBOptions, MetricConf, ReadWriteOptions},
    DBMapUtils,
    Map,
    TypedStoreError,
};
...
pub use crate::checkpoints::{
    checkpoint_output::{LogCheckpointOutput, SendCheckpointToStateSync, SubmitCheckpointToConsensus},
    metrics::CheckpointMetrics,
};
use crate::{
    accumulators::AccumulatorSettlementTxBuilder,
    authority::{
...
pub struct CheckpointStoreTables {
    ...
    pub(crate) checkpoint_sequence_by_contents_digest: DBMap<CheckpointContentsDigest, CheckpointSequenceNumber>,
...
impl CheckpointStoreTables {
    ...
    #[cfg(tidehunter)]
    pub fn new(path: &Path, metric_name: &'static str, pruner_watermarks: Arc<PrunerWatermarks>) -> Self {
        ...
        let override_dirty_keys_config =
            KeySpaceConfig::new().with_max_dirty_keys(64_000).with_value_cache_size(default_value_cache_size());
        let config_u64 = ThConfig::new_with_config(8, mutexes, u64_sequence_key, override_dirty_keys_config.clone());
        ...
        let watermarks_config = KeySpaceConfig::new().with_value_cache_size(10).disable_unload();
        let lru_config = KeySpaceConfig::new().with_value_cache_size(default_value_cache_size());
        let configs = vec![
            (
                "checkpoint_content",
                digest_config.clone().with_config(lru_config.clone().with_relocation_filter(|_, _| Decision::Remove)),
            ),
            (
            ...
            ("watermarks", ThConfig::new_with_config(4, 1, KeyType::uniform(1), watermarks_config.clone())),
            (
        ...
        Self::open_tables_read_write(
            ...
            configs.into_iter().map(|(cf, config)| (cf.to_string(), config)).collect(),
        )
    }

    pub fn open_readonly(path: &Path) -> CheckpointStoreTablesReadOnly {
        Self::get_read_only_handle(path.to_path_buf(), None, None, MetricConf::new("checkpoint_readonly"))
    }
}
...
impl CheckpointStore {
    ...
    pub fn new_for_db_checkpoint_handler(path: &Path) -> Arc<Self> {
        let tables = CheckpointStoreTables::new(path, "db_checkpoint", Arc::new(PrunerWatermarks::default()));
        Arc::new(Self {
            tables,
            synced_checkpoint_notify_read: NotifyRead::new(),
            executed_checkpoint_notify_read: NotifyRead::new(),
    ...
    pub fn insert_genesis_checkpoint(
        ...
        assert_eq!(checkpoint.epoch(), 0, "can't call insert_genesis_checkpoint with a checkpoint not in epoch 0");
        assert_eq!(
        ...
        match self.get_checkpoint_by_sequence_number(0).unwrap() {
            ...
            None => {
                if epoch_store.epoch() == checkpoint.epoch {
                    epoch_store.put_genesis_checkpoint_in_builder(checkpoint.data(), &contents).unwrap();
                } else {
                    debug!(
    ...
    pub fn get_checkpoint_by_digest(
        ...
        self.tables.checkpoint_by_digest.get(digest).map(|maybe_checkpoint| maybe_checkpoint.map(|c| c.into()))
    }
    ...
    pub fn get_highest_verified_checkpoint(&self) -> Result<Option<VerifiedCheckpoint>, TypedStoreError> {
        let highest_verified =
            if let Some(highest_verified) = self.tables.watermarks.get(&CheckpointWatermark::HighestVerified)? {
                highest_verified
            } else {
                return Ok(None);
            };
    ...
    pub fn get_highest_synced_checkpoint(&self) -> Result<Option<VerifiedCheckpoint>, TypedStoreError> {
        let highest_synced =
            if let Some(highest_synced) = self.tables.watermarks.get(&CheckpointWatermark::HighestSynced)? {
                highest_synced
            } else {
                return Ok(None);
            };
    ...
    pub fn get_highest_executed_checkpoint_seq_number(
        ...
        if let Some(highest_executed) = self.tables.watermarks.get(&CheckpointWatermark::HighestExecuted)? {
    ...
    pub fn get_highest_pruned_checkpoint_seq_number(&self) -> Result<Option<CheckpointSequenceNumber>, TypedStoreError> {
        self.tables.watermarks.get(&CheckpointWatermark::HighestPruned).map(|watermark| watermark.map(|w| w.0))
    }
    ...
    fn prune_local_summaries(&self) -> SuiResult {
        if let Some((last_local_summary, _)) =
            self.tables.locally_computed_checkpoints.reversed_safe_iter_with_bounds(None, None)?.next().transpose()?
        {
            let mut batch = self.tables.locally_computed_checkpoints.batch();
            batch.schedule_delete_range(&self.tables.locally_computed_checkpoints, &0, &last_local_summary)?;
            batch.write()?;
    ...
    pub fn clear_locally_computed_checkpoints_from(&self, from_seq: CheckpointSequenceNumber) -> SuiResult {
        let keys: Vec<_> = self
            .tables
            .locally_computed_checkpoints
    ...
    fn check_for_checkpoint_fork(&self, local_checkpoint: &CheckpointSummary, verified_checkpoint: &VerifiedCheckpoint) {
        if local_checkpoint != verified_checkpoint.data() {
            ...
            let local_contents = self
                ...
                .map(|opt_contents| {
                    opt_contents.map(|contents| format!("{:?}", contents)).unwrap_or_else(|| {
                        format!("Local checkpoint contents not found, digest: {:?}", local_checkpoint.content_digest)
                    })
                })
                .map_err(|e| {
                    format!(
            ...
            if let Err(e) =
                self.record_checkpoint_fork_detected(*local_checkpoint.sequence_number(), local_checkpoint.digest())
            {
                error!("Failed to record checkpoint fork in database: {:?}", e);
            }

            fail_point_arg!("kill_checkpoint_fork_node", |checkpoint_overrides: std::sync::Arc<
                std::sync::Mutex<std::collections::BTreeMap<u64, String>>,
            >| {
                #[cfg(msim)]
                {
                    if let Ok(mut overrides) = checkpoint_overrides.lock() {
                        overrides.insert(local_checkpoint.sequence_number, verified_checkpoint.digest().to_string());
                    }
                    tracing::error!(
                            fatal = true,
            ...
            fatal!("Local checkpoint fork detected for sequence number: {}", local_checkpoint.sequence_number());
        }
    }
    ...
    pub fn insert_certified_checkpoint(&self, checkpoint: &VerifiedCheckpoint) -> Result<(), TypedStoreError> {
        ...
        batch
            .insert_batch(&self.tables.certified_checkpoints, [(
                checkpoint.sequence_number(),
                checkpoint.serializable_ref(),
            )])?
            .insert_batch(&self.tables.checkpoint_by_digest, [(checkpoint.digest(), checkpoint.serializable_ref())])?;
        if checkpoint.next_epoch_committee().is_some() {
            batch.insert_batch(&self.tables.epoch_last_checkpoint_map, [(
                &checkpoint.epoch(),
                checkpoint.sequence_number(),
            )])?;
        }
        ...
        if let Some(local_checkpoint) = self.tables.locally_computed_checkpoints.get(checkpoint.sequence_number())? {
    ...
    pub fn update_highest_verified_checkpoint(&self, checkpoint: &VerifiedCheckpoint) -> Result<(), TypedStoreError> {
        if Some(*checkpoint.sequence_number()) > self.get_highest_verified_checkpoint()?.map(|x| *x.sequence_number()) {
            debug!(checkpoint_seq = checkpoint.sequence_number(), "Updating highest verified checkpoint",);
            self.tables
                .watermarks
                .insert(&CheckpointWatermark::HighestVerified, &(*checkpoint.sequence_number(), *checkpoint.digest()))?;
        }
    ...
    pub fn update_highest_synced_checkpoint(&self, checkpoint: &VerifiedCheckpoint) -> Result<(), TypedStoreError> {
        ...
        self.tables.watermarks.insert(&CheckpointWatermark::HighestSynced, &(seq, *checkpoint.digest()))?;
        ...
        F: Fn() -> Option<CheckpointSequenceNumber>,
    {
        notify_read
            .read("notify_read_checkpoint_watermark", &[seq], |seqs| {
                ...
                let checkpoint =
                    self.get_checkpoint_by_sequence_number(seq).expect("db error").expect("checkpoint not found");
                vec![Some(checkpoint)]
            })
            .await
            .into_iter()
    ...
    pub fn update_highest_executed_checkpoint(&self, checkpoint: &VerifiedCheckpoint) -> Result<(), TypedStoreError> {
        ...
        self.tables.watermarks.insert(&CheckpointWatermark::HighestExecuted, &(seq, *checkpoint.digest()))?;
    ...
    pub fn update_highest_pruned_checkpoint(&self, checkpoint: &VerifiedCheckpoint) -> Result<(), TypedStoreError> {
        self.tables
            .watermarks
            .insert(&CheckpointWatermark::HighestPruned, &(*checkpoint.sequence_number(), *checkpoint.digest()))
    }
    ...
    pub fn set_highest_executed_checkpoint_subtle(
        ...
        self.tables
            .watermarks
            .insert(&CheckpointWatermark::HighestExecuted, &(*checkpoint.sequence_number(), *checkpoint.digest()))
    }
    ...
    pub fn insert_verified_checkpoint_contents(
        ...
        batch.insert_batch(&self.tables.checkpoint_sequence_by_contents_digest, [(
            &checkpoint.content_digest,
            checkpoint.sequence_number(),
        )])?;
        let full_contents = full_contents.into_inner();
        batch.insert_batch(&self.tables.full_checkpoint_content, [(checkpoint.sequence_number(), &full_contents)])?;
    ...
    pub fn get_epoch_stats(&self, epoch: EpochId, last_checkpoint: &CheckpointSummary) -> Option<EpochStats> {
        ...
        Some(EpochStats {
            ...
            transaction_count: last_checkpoint.network_total_transactions - prev_epoch_network_transactions,
            total_gas_reward: last_checkpoint.epoch_rolling_gas_cost_summary.computation_cost,
        })
    }
    ...
    pub fn delete_highest_executed_checkpoint_test_only(&self) -> Result<(), TypedStoreError> {
        ...
        wb.delete_batch(&self.tables.watermarks, std::iter::once(CheckpointWatermark::HighestExecuted))?;
        wb.write()?;
        Ok(())
    }
    ...
    pub fn record_checkpoint_fork_detected(
        ...
        self.tables.watermarks.insert(&CheckpointWatermark::CheckpointForkDetected, &(checkpoint_seq, checkpoint_digest))
    }
    ...
    pub fn record_transaction_fork_detected(
        ...
        self.tables
            .transaction_fork_detected
            .insert(&TRANSACTION_FORK_DETECTED_KEY, &(tx_digest, expected_effects_digest, actual_effects_digest))
    }

    pub fn get_transaction_fork_detected(
        &self,
    ) -> Result<Option<(TransactionDigest, TransactionEffectsDigest, TransactionEffectsDigest)>, TypedStoreError> {
...
impl<SuiError: std::error::Error + Send + Sync + 'static> From<SuiError> for CheckpointBuilderError {
...
impl CheckpointBuilder {
    ...
    async fn run(mut self, consensus_replay_waiter: Option<ReplayWaiter>) {
        ...
        loop {
            match self.maybe_build_checkpoints().await {
                Ok(()) => {}
                err @ Err(
                    CheckpointBuilderError::ChangeEpochTxAlreadyExecuted | CheckpointBuilderError::SystemPackagesMissing,
                ) => {
    ...
    async fn maybe_build_checkpoints(&mut self) -> CheckpointBuilderResult {
        ...
        let summary = self.epoch_store.last_built_checkpoint_builder_summary().expect("epoch should not have ended");
        ...
        let min_checkpoint_interval_ms =
            self.epoch_store.protocol_config().min_checkpoint_interval_ms_as_option().unwrap_or_default();
        ...
        while let Some((height, pending)) = checkpoints_iter.next() {
            ...
            debug!(
                checkpoint_commit_height_from = grouped_pending_checkpoints.first().unwrap().details().checkpoint_height,
                checkpoint_commit_height_to = last_height,
                "Making checkpoint with commit height range"
            );

            let seq = self.make_checkpoint(std::mem::take(&mut grouped_pending_checkpoints)).await?;
    ...
    async fn make_checkpoint(
        ...
        let pending_ckpt_str = pendings
            .iter()
            .map(|p| format!("height={}, commit={}", p.details().checkpoint_height, p.details().consensus_commit_ref))
            .join("; ");
        ...
        let new_checkpoints =
            self.create_checkpoints(sorted_tx_effects_included_in_checkpoint, &last_details, &all_roots).await?;
        ...
        info!("Made new checkpoint {} from pending checkpoint {}", new_ckpt_str, pending_ckpt_str);
    ...
    async fn construct_and_execute_settlement_transactions(
        ...
        let _scope = monitored_scope("CheckpointBuilder::construct_and_execute_settlement_transactions");

        let tx_key = TransactionKey::AccumulatorSettlement(self.epoch_store.epoch(), checkpoint_height);
        ...
        debug!(?settlement_digests, ?tx_key, "created settlement transactions with {num_updates} updates");

        self.epoch_store.notify_settlement_transactions_ready(tx_key, settlement_txns);
        ...
        let settlement_effects = loop {
            ...
            .await
            {
                ...
                Err(_) => {
                    debug_fatal!("Timeout waiting for settlement transactions to be executed {:?}, retrying...", tx_key);
                }
            }
        };
        ...
        for fx in settlement_effects.iter() {
            ...
            if let Some(version) =
                fx.mutated().iter().find_map(|(oref, _)| (oref.0 == SUI_ACCUMULATOR_ROOT_OBJECT_ID).then_some(oref.1))
            {
                assert!(
        ...
        self.state.execution_scheduler().settle_balances(settlements);
    ...
    async fn resolve_checkpoint_transactions(
        ...
        for pending_checkpoint in pending_checkpoints.into_iter() {
            ...
            trace!("roots for pending checkpoint {:?}: {:?}", pending.details.checkpoint_height, pending.roots,);

            let settlement_root = if self.epoch_store.accumulators_enabled() {
                let Some(settlement_root @ TransactionKey::AccumulatorSettlement(..)) = pending.roots.pop() else {
            ...
            self.metrics.checkpoint_roots_count.inc_by(roots.len() as u64);
            ...
            let root_effects = self
                .effects_store
                .notify_read_executed_effects(CHECKPOINT_BUILDER_NOTIFY_READ_TASK_NAME, &root_digests)
                ...
                .prepend_prologue_tx_in_consensus_commit_in_checkpoints()
            {
                ...
                let consensus_commit_prologue = self.extract_consensus_commit_prologue(&root_digests, &root_effects)?;
                ...
                if let Some((_ccp_digest, ccp_effects)) = &consensus_commit_prologue {
                    let unsorted_ccp =
                        self.complete_checkpoint_effects(vec![ccp_effects.clone()], &mut effects_in_current_checkpoint)?;
            ...
            let unsorted = self.complete_checkpoint_effects(root_effects, &mut effects_in_current_checkpoint)?;
            ...
            if let Some(settlement_root) = settlement_root {
                ...
                let last_checkpoint = Self::load_last_built_checkpoint_summary(&self.epoch_store, &self.store)?;
                let next_checkpoint_seq = last_checkpoint.as_ref().map(|(seq, _)| *seq).unwrap_or_default() + 1;
    ...
    async fn write_checkpoints(
        ...
        let mut all_tx_digests = Vec::with_capacity(new_checkpoints.iter().map(|(_, c)| c.size()).sum());
        ...
        for (summary, contents) in &new_checkpoints {
            ...
            self.metrics.transactions_included_in_checkpoint.inc_by(contents.size() as u64);
            let sequence_number = summary.sequence_number;
            self.metrics.last_constructed_checkpoint.set(sequence_number as i64);
        ...
        for (local_checkpoint, _) in &new_checkpoints {
            ...
            {
                self.store.check_for_checkpoint_fork(local_checkpoint, &certified_checkpoint.into());
            }
        }
    ...
    fn split_checkpoint_chunks(
            ...
            effects_and_transaction_sizes.into_iter().zip(signatures.into_iter())
        {
            ...
            let size = transaction_size + bcs::serialized_size(&effects)? + bcs::serialized_size(&signatures)?;
    ...
    async fn create_checkpoints(
        ...
        let mut last_checkpoint = Self::load_last_built_checkpoint_summary(&self.epoch_store, &self.store)?;
        ...
        let all_digests: Vec<_> = all_effects.iter().map(|effect| *effect.transaction_digest()).collect();
        let transactions_and_sizes =
            self.state.get_transaction_cache_reader().get_transactions_and_serialized_sizes(&all_digests)?;
        ...
        let mut randomness_rounds = BTreeMap::new();
        {
            ...
            debug!(?last_checkpoint_seq, "Waiting for {:?} certificates to appear in consensus", all_effects.len());

            for (effects, transaction_and_size) in all_effects.into_iter().zip(transactions_and_sizes.into_iter()) {
                let (transaction, size) =
                    transaction_and_size.unwrap_or_else(|| panic!("Could not find executed transaction {:?}", effects));
                match transaction.inner().transaction_data().kind() {
                    ...
                    | TransactionKind::EndOfEpochTransaction(_) => {
                        fatal!("unexpected transaction in checkpoint effects: {:?}", transaction);
                    }
                    TransactionKind::RandomnessStateUpdate(rsu) => {
                        randomness_rounds.insert(*effects.transaction_digest(), rsu.randomness_round);
                    }
        ...
        let signatures = self.epoch_store.user_signatures_for_checkpoint(&transactions, &all_digests);
        debug!(?last_checkpoint_seq, "Received {} checkpoint user signatures from consensus", signatures.len());
        ...
        let mut end_of_epoch_observation_keys: Option<Vec<_>> = if details.last_of_epoch {
            Some(
                transactions
                    .iter()
                    .flat_map(|tx| {
                        if let TransactionKind::ProgrammableTransaction(ptb) = tx.transaction_data().kind() {
                            itertools::Either::Left(ptb.commands.iter().map(ExecutionTimeObservationKey::from_command))
                        } else {
        ...
        debug!(?last_checkpoint_seq, "Creating {} checkpoints with {} transactions", chunks_count, total,);

        let epoch = self.epoch_store.epoch();
        for (index, transactions) in chunks.into_iter().enumerate() {
            let first_checkpoint_of_epoch =
                index == 0 && last_checkpoint.as_ref().map(|(_, c)| c.epoch != epoch).unwrap_or(true);
            ...
            let sequence_number = last_checkpoint.as_ref().map(|(_, c)| c.sequence_number + 1).unwrap_or_default();
                ...
                && last_checkpoint.timestamp_ms > timestamp_ms
            {
                ...
                if self.epoch_store.protocol_config().enforce_checkpoint_timestamp_monotonicity() {
            ...
            let end_of_epoch_data = if last_checkpoint_of_epoch {
                let system_state_obj = self
                    .augment_epoch_last_checkpoint(
                        ...
                        &mut signatures,
                        sequence_number,
                        std::mem::take(&mut end_of_epoch_observation_keys)
                            .expect("end_of_epoch_observation_keys must be populated for the last checkpoint"),
                ...
                let root_state_digest = {
                    ...
                    let acc = state_acc.accumulate_checkpoint(&effects, sequence_number, &self.epoch_store)?;

                    state_acc.wait_for_previous_running_root(&self.epoch_store, sequence_number).await?;
                ...
                let epoch_commitments = if self.epoch_store.protocol_config().check_commit_root_state_digest_supported()
                {
                ...
                Some(EndOfEpochData {
                    ...
                    next_epoch_protocol_version: ProtocolVersion::new(system_state_obj.protocol_version()),
                    epoch_commitments,
                })
            } else {
            ...
            let network_total_transactions =
                last_checkpoint.as_ref().map(|(_, c)| c.network_total_transactions + num_txns).unwrap_or(num_txns);

            let previous_digest = last_checkpoint.as_ref().map(|(_, c)| c.digest());

            let matching_randomness_rounds: Vec<_> =
                effects.iter().filter_map(|e| randomness_rounds.get(e.transaction_digest())).copied().collect();

            let checkpoint_commitments =
                if self.epoch_store.protocol_config().include_checkpoint_artifacts_digest_in_summary() {
            ...
            if last_checkpoint_of_epoch {
                info!(checkpoint_seq = sequence_number, "creating last checkpoint of epoch {}", epoch);
                if let Some(stats) = self.store.get_epoch_stats(epoch, &summary) {
                    self.epoch_store.report_epoch_metrics_at_last_checkpoint(stats);
                }
            }
    ...
    fn get_epoch_total_gas_cost(
        ...
        let (previous_epoch, previous_gas_costs) =
            last_checkpoint.map(|c| (c.epoch, c.epoch_rolling_gas_cost_summary.clone())).unwrap_or_default();
        ...
        if previous_epoch == self.epoch_store.epoch() {
            ...
            GasCostSummary::new(
                ...
                previous_gas_costs.non_refundable_storage_fee + current_gas_costs.non_refundable_storage_fee,
            )
        } else {
            current_gas_costs
        }
    ...
    fn complete_checkpoint_effects(
        ...
        loop {
            ...
            let transactions_included = self
                .epoch_store
                .builder_included_transactions_in_checkpoint(roots.iter().map(|e| e.transaction_digest()))?;
            ...
            for (effect, tx_included) in roots.into_iter().zip(transactions_included.into_iter()) {
                ...
                let existing_effects = self.epoch_store.transactions_executed_in_cur_epoch(effect.dependencies())?;

                for (dependency, effects_signature_exists) in effect.dependencies().iter().zip(existing_effects.iter()) {
    ...
    fn expensive_consensus_commit_prologue_invariants_check(
        ...
        if !self.epoch_store.protocol_config().prepend_prologue_tx_in_consensus_commit_in_checkpoints() {
            return;
        }

        // Gets all the consensus commit prologue transactions from the roots.
        let root_txs = self.state.get_transaction_cache_reader().multi_get_transaction_blocks(root_digests);
        let ccps = root_txs
            .iter()
            .filter_map(|tx| {
                if let Some(tx) = tx {
        ...
        let txs = self
            .state
            .get_transaction_cache_reader()
            .multi_get_transaction_blocks(&sorted.iter().map(|tx| tx.transaction_digest().clone()).collect::<Vec<_>>());
...
impl CheckpointAggregator {
    fn new(
        ...
        let current = None;
        Self { store: tables, epoch_store, notify, current, output, state, metrics }
    }
    ...
    fn run_inner(&mut self) -> SuiResult<Vec<CertifiedCheckpointSummary>> {
        ...
        'outer: loop {
            ...
            let current = if let Some(current) = &mut self.current {
                ...
                let Some(summary) = self.epoch_store.get_built_checkpoint_summary(next_to_certify)? else {
                    return Ok(result);
                };
                self.current = Some(CheckpointSignatureAggregator {
                    ...
                    summary,
                    signatures_by_digest: MultiStakeAggregator::new(self.epoch_store.committee().clone()),
            ...
            for item in iter {
                ...
                self.metrics
                    ...
                    .with_label_values(&[&format!("{:?}", data.summary.auth_sig().authority.concise())])
                    .inc();
                if let Ok(auth_signature) = current.try_aggregate(data) {
                    ...
                    let summary = VerifiedCheckpoint::new_unchecked(CertifiedCheckpointSummary::new_from_data_and_sig(
                        current.summary.clone(),
                        auth_signature,
                    ));

                    self.store.insert_certified_checkpoint(&summary)?;
                    self.metrics.last_certified_checkpoint.set(current.summary.sequence_number as i64);
...
impl CheckpointSignatureAggregator {
    ...
    pub fn try_aggregate(&mut self, data: CheckpointSignatureMessage) -> Result<AuthorityStrongQuorumSignInfo, ()> {
        let their_digest = *data.summary.digest();
        let (_, signature) = data.summary.into_data_and_sig();
        let author = signature.authority;
        let envelope = SignedCheckpointSummary::new_from_data_and_sig(self.summary.clone(), signature);
    ...
    fn check_for_split_brain(&self) {
        ...
        if self.signatures_by_digest.quorum_unreachable() {
            ...
            let digests_by_stake_messages = all_unique_values
                ...
                .map(|(digest, (_authorities, total_stake))| format!("{:?} (total stake: {})", digest, total_stake))
                .collect::<Vec<String>>();
            fail_point_arg!("kill_split_brain_node", |(checkpoint_overrides, forked_authorities): (
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store.epoch_start_state().get_sui_committee_with_network_metadata();
    let network_config = default_mysten_network_config();
    let network_clients = make_network_authority_clients_with_network_config(&committee, &network_config);
    ...
    let response_data = futures::future::join_all(response_futures)
        ...
        .filter_map(|(response, (digest, name))| match response {
            Ok(response) => match response {
                ...
                CheckpointResponseV2 { checkpoint: Some(CheckpointSummaryResponse::Certified(_)), contents: _ } => {
                    panic!("Expected pending checkpoint, but got certified checkpoint");
                }
                CheckpointResponseV2 { checkpoint: None, contents: _ } => {
                    error!(
                ...
                CheckpointResponseV2 { checkpoint: _, contents: None } => {
                    error!(
    ...
    let local_checkpoint_contents = tables
        ...
        .unwrap_or_else(|_| panic!("Could not find checkpoint contents for digest {:?}", local_summary.digest()))
        .unwrap_or_else(|| {
            panic!(
    ...
    let diff_patches = response_data
        ...
        .map(|(name, other_digest, other_summary, contents)| {
            ...
            let transactions_patch = create_patch(&local_transactions_text, &other_transactions_text);
...
impl CheckpointServiceState {
    fn take_unstarted(&mut self) -> (CheckpointBuilder, CheckpointAggregator, CheckpointStateHasher) {
        let mut state = CheckpointServiceState::Started;
        std::mem::swap(self, &mut state);

        match state {
            CheckpointServiceState::Unstarted((builder, aggregator, hasher)) => (builder, aggregator, hasher),
...
impl CheckpointService {
    ...
    pub fn build(
        ...
        let ckpt_state_hasher =
            CheckpointStateHasher::new(epoch_store.clone(), global_state_hasher.clone(), receive_from_builder);
        ...
        let last_signature_index =
            epoch_store.get_last_checkpoint_signature_index().expect("should not cross end of epoch");
        ...
        Arc::new(Self {
            ...
            metrics,
            state: Mutex::new(CheckpointServiceState::Unstarted((builder, aggregator, ckpt_state_hasher))),
        })
    }
    ...
    pub async fn spawn(&self, epoch_store: Arc<AuthorityPerEpochStore>, consensus_replay_waiter: Option<ReplayWaiter>) {
...
impl CheckpointServiceNotify for CheckpointService {
    fn notify_checkpoint_signature(
        ...
        {
            trace!(checkpoint_seq = sequence, "Ignore checkpoint signature from {} - already certified", signer,);
            self.metrics.last_ignored_checkpoint_signature_received.set(sequence as i64);
            return Ok(());
        }
        trace!(
        ...
        self.metrics.last_received_checkpoint_signatures.with_label_values(&[&signer.to_string()]).set(sequence as i64);
...
impl CheckpointServiceNotify for CheckpointServiceNoop {
    fn notify_checkpoint_signature(&self, _: &AuthorityPerEpochStore, _: &CheckpointSignatureMessage) -> SuiResult {
        Ok(())
    }
...
mod tests {
    ...
    use super::*;
    use crate::{authority::test_authority_builder::TestAuthorityBuilder, transaction_outputs::TransactionOutputs};
    ...
    async fn test_clear_locally_computed_checkpoints_from_deletes_inclusive_range() {
        ...
        for seq in 70u64 ..= 80u64 {
            let contents = sui_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests([
            ...
            store.tables.locally_computed_checkpoints.insert(&seq, &summary).unwrap();
        }

        store.clear_locally_computed_checkpoints_from(76).expect("clear should succeed");

        // Explicit boundary checks: 75 must remain, 76 must be deleted
        assert!(store.tables.locally_computed_checkpoints.get(&75).unwrap().is_some());
        assert!(store.tables.locally_computed_checkpoints.get(&76).unwrap().is_none());

        for seq in 70u64 .. 76u64 {
            assert!(store.tables.locally_computed_checkpoints.get(&seq).unwrap().is_some());
        }
        for seq in 76u64 ..= 80u64 {
            assert!(store.tables.locally_computed_checkpoints.get(&seq).unwrap().is_none());
        }
    }
    ...
    async fn test_fork_detection_storage() {
        ...
        store.record_checkpoint_fork_detected(seq_num, digest).unwrap();
        ...
        store.record_transaction_fork_detected(tx_digest, expected_effects, actual_effects).unwrap();
    ...
    pub async fn checkpoint_builder_test() {
        ...
        let state = TestAuthorityBuilder::new().with_protocol_config(protocol_config).build().await;

        let dummy_tx = VerifiedTransaction::new_authenticator_state_update(0, 0, vec![], SequenceNumber::new());

        let jwks = {
            let mut jwks = Vec::new();
            while bcs::to_bytes(&jwks).unwrap().len() < 40_000 {
                jwks.push(ActiveJwk {
                    jwk_id: JwkId::new("https://accounts.google.com".to_string(), "1234567890".to_string()),
                    jwk: JWK {
                        kty: "RSA".to_string(),
                        e: "AQAB".to_string(),
        ...
        let dummy_tx_with_data = VerifiedTransaction::new_authenticator_state_update(0, 1, jwks, SequenceNumber::new());
        ...
        commit_cert_for_test(&mut store, state.clone(), d(1), vec![d(2), d(3)], GasCostSummary::new(11, 12, 11, 1));
        commit_cert_for_test(&mut store, state.clone(), d(2), vec![d(3), d(4)], GasCostSummary::new(21, 22, 21, 1));
        commit_cert_for_test(&mut store, state.clone(), d(3), vec![], GasCostSummary::new(31, 32, 31, 1));
        commit_cert_for_test(&mut store, state.clone(), d(4), vec![], GasCostSummary::new(41, 42, 41, 1));
        for i in [5, 6, 7, 10, 11, 12, 13] {
            commit_cert_for_test(&mut store, state.clone(), d(i), vec![], GasCostSummary::new(41, 42, 41, 1));
        }
        for i in [15, 16, 17] {
            commit_cert_for_test(&mut store, state.clone(), d(i), vec![], GasCostSummary::new(51, 52, 51, 1));
        }
        let all_digests: Vec<_> = store.keys().copied().collect();
        for digest in all_digests {
            let signature = Signature::Ed25519SuiSignature(Default::default()).into();
            state.epoch_store_for_testing().test_insert_user_signature(digest, vec![signature]);
        }

        let (output, mut result) = mpsc::channel::<(CheckpointContents, CheckpointSummary)>(10);
        let (certified_output, mut certified_result) = mpsc::channel::<CertifiedCheckpointSummary>(10);
        let store = Arc::new(store);

        let ckpt_dir = tempfile::tempdir().unwrap();
        let checkpoint_store = CheckpointStore::new(ckpt_dir.path(), Arc::new(PrunerWatermarks::default()));
        let epoch_store = state.epoch_store_for_testing();

        let global_state_hasher =
            Arc::new(GlobalStateHasher::new_for_tests(state.get_global_state_hash_store().clone()));
        ...
        checkpoint_service.write_and_notify_checkpoint_for_testing(&epoch_store, p(0, vec![4], 0)).unwrap();
        checkpoint_service.write_and_notify_checkpoint_for_testing(&epoch_store, p(1, vec![1, 3], 2000)).unwrap();
        checkpoint_service
            .write_and_notify_checkpoint_for_testing(&epoch_store, p(2, vec![10, 11, 12, 13], 3000))
            .unwrap();
        checkpoint_service.write_and_notify_checkpoint_for_testing(&epoch_store, p(3, vec![15, 16, 17], 4000)).unwrap();
        checkpoint_service.write_and_notify_checkpoint_for_testing(&epoch_store, p(4, vec![5], 4001)).unwrap();
        checkpoint_service.write_and_notify_checkpoint_for_testing(&epoch_store, p(5, vec![6], 5000)).unwrap();
        ...
        assert_eq!(c1s.epoch_rolling_gas_cost_summary, GasCostSummary::new(41, 42, 41, 1));

        assert_eq!(c2t, vec![d(3), d(2), d(1)]);
        assert_eq!(c2s.previous_digest, Some(c1s.digest()));
        assert_eq!(c2s.sequence_number, 1);
        assert_eq!(c2s.epoch_rolling_gas_cost_summary, GasCostSummary::new(104, 108, 104, 4));
    ...
    impl TransactionCacheRead for HashMap<TransactionDigest, TransactionEffects> {
        fn notify_read_executed_effects(
            ...
            std::future::ready(digests.iter().map(|d| self.get(d).expect("effects not found").clone()).collect()).boxed()
        }
        ...
        fn notify_read_executed_effects_digests(
            ...
            std::future::ready(
                digests.iter().map(|d| self.get(d).map(|fx| fx.digest()).expect("effects not found")).collect(),
            )
            .boxed()
        }
        ...
        fn multi_get_transaction_blocks(&self, _: &[TransactionDigest]) -> Vec<Option<Arc<VerifiedTransaction>>> {
            unimplemented!()
        }

        fn multi_get_executed_effects_digests(&self, _: &[TransactionDigest]) -> Vec<Option<TransactionEffectsDigest>> {
            unimplemented!()
        }

        fn multi_get_effects(&self, _: &[TransactionEffectsDigest]) -> Vec<Option<TransactionEffects>> {
            unimplemented!()
        }
        ...
        fn get_mysticeti_fastpath_outputs(&self, _: &TransactionDigest) -> Option<Arc<TransactionOutputs>> {
            unimplemented!()
        }
    ...
    impl CertifiedCheckpointOutput for mpsc::Sender<CertifiedCheckpointSummary> {
        async fn certified_checkpoint_created(&self, summary: &CertifiedCheckpointSummary) -> SuiResult {
    ...
    fn p(i: u64, t: Vec<u8>, timestamp_ms: u64) -> PendingCheckpoint {
        PendingCheckpoint {
            roots: t.into_iter().map(|t| TransactionKey::Digest(d(t))).collect(),
            details: PendingCheckpointInfo {
                timestamp_ms,
                last_of_epoch: false,
                checkpoint_height: i,
