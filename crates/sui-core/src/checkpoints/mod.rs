...
use mysten_common::{assert_reachable, debug_fatal, fatal, random::get_rng, sync::notify_read::NotifyRead};
...
impl PendingCheckpointV2 {
    ...
    pub fn expect_v1(self) -> PendingCheckpoint {
        let v2 = self.into_v2();
        PendingCheckpoint {
            roots: v2.roots.into_iter().map(|root| *root.unwrap_digest()).collect(),
            details: v2.details,
        }
    }
...
impl CheckpointStoreTables {
    ...
    pub fn new(path: &Path, metric_name: &'static str) -> Self {
        ...
        use typed_store::tidehunter_util::{default_cells_per_mutex, KeySpaceConfig, KeyType, ThConfig};
        ...
        let configs = vec![
            ...
            ("checkpoint_sequence_by_contents_digest", digest_config.clone()),
...
impl CheckpointStore {
    ...
    /// order.
    #[instrument(level = "debug", skip_all)]
    pub async fn reexecute_local_checkpoints(&self, state: &AuthorityState, epoch_store: &Arc<AuthorityPerEpochStore>) {
        ...
        for seq in highest_executed + 1 ..= *highest_built.sequence_number() {
            ...
            let Some(checkpoint) =
                self.get_locally_computed_checkpoint(seq).expect("get_locally_computed_checkpoint should not fail")
            else {
            ...
            info!(?seq, ?tx_digests, "Re-executing transactions for locally built checkpoint");
            ...
            cache.notify_read_executed_effects_digests(&tx_digests).await;
...
impl CheckpointBuilder {
    ...
    async fn maybe_build_checkpoints(&mut self) {
        ...
        while let Some((height, pending)) = checkpoints_iter.next() {
            ...
            debug!(checkpoint_commit_height = height, "Making checkpoint at commit height");

            match self.make_checkpoint(std::mem::take(&mut grouped_pending_checkpoints)).await {
                Ok(seq) => {
    ...
    async fn make_checkpoint(&self, pendings: Vec<PendingCheckpointV2>) -> anyhow::Result<CheckpointSequenceNumber> {
        ...
        for pending_checkpoint in pendings.into_iter() {
            ...
            let (root_digests, txn_in_checkpoint) =
                self.resolve_checkpoint_transactions(pending.roots, &mut effects_in_current_checkpoint).await?;
    ...
    async fn resolve_checkpoint_transactions(
        ...
        let root_digests =
            self.epoch_store.notify_read_executed_digests(&roots).in_monitored_scope("CheckpointNotifyDigests").await?;
        ...
        let consensus_commit_prologue =
            if self.epoch_store.protocol_config().prepend_prologue_tx_in_consensus_commit_in_checkpoints() {
                ...
                let consensus_commit_prologue =
                    self.extract_consensus_commit_prologue(&root_digests, &root_effects).await?;
        ...
        let unsorted = self.complete_checkpoint_effects(root_effects, effects_in_current_checkpoint)?;
...
impl CheckpointService {
    ...
    pub async fn spawn(&self, consensus_replay_waiter: Option<ReplayWaiter>) -> JoinSet<()> {
        ...
        if tokio::time::timeout(Duration::from_secs(120), self.wait_for_rebuilt_checkpoints()).await.is_err() {
...
impl From<PendingCheckpoint> for PendingCheckpointV2 {
    fn from(value: PendingCheckpoint) -> Self {
        PendingCheckpointV2::V2(PendingCheckpointV2Contents {
            roots: value.roots.into_iter().map(TransactionKey::Digest).collect(),
...
mod tests {
    ...
    pub async fn checkpoint_builder_test() {
        ...
        let dummy_tx_with_data = VerifiedTransaction::new_genesis_transaction(vec![GenesisObject::RawObject {
    ...
    impl TransactionCacheRead for HashMap<TransactionDigest, TransactionEffects> {
        fn notify_read_executed_effects(&self, digests: &[TransactionDigest]) -> BoxFuture<'_, Vec<TransactionEffects>> {
