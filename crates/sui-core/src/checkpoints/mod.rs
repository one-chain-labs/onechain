...
use sui_types::{
    ...
    digests::{CheckpointContentsDigest, CheckpointDigest, TransactionEffectsDigest},
    ...
    pub fn new(path: &Path, metric_name: &'static str, pruner_watermarks: Arc<PrunerWatermarks>) -> Self {
        ...
        let configs = vec![
            (
                ...
                digest_config
                    .clone()
                    .with_config(KeySpaceConfig::default().with_relocation_filter(|_, _| Decision::Remove)),
            ),
            (
    ...
    async fn resolve_checkpoint_transactions(
        ...
        for pending_checkpoint in pending_checkpoints.into_iter() {
            ...
            if let Some(settlement_root) = settlement_root {
                let (tx_key, settlement_effects) =
                    self.construct_and_execute_settlement_transactions(&sorted, pending.details.checkpoint_height).await;
...
impl CheckpointSignatureAggregator {
    ...
    pub fn try_aggregate(&mut self, data: CheckpointSignatureMessage) -> Result<AuthorityStrongQuorumSignInfo, ()> {
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed { error: SuiError::StakeAggregatorRepeatedSigner { conflicting_sig: false, .. } } => {
                Err(())
            }
...
mod tests {
    ...
    fn p(i: u64, t: Vec<u8>, timestamp_ms: u64) -> PendingCheckpoint {
        PendingCheckpoint {
            ...
            details: PendingCheckpointInfo { timestamp_ms, last_of_epoch: false, checkpoint_height: i },
        }
    }
