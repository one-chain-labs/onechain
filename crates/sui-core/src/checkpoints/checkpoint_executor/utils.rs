...
pub(super) fn stream_synced_checkpoints(
    ...
    let state = State { current_seq: start_seq, checkpoint_store, warning_timeout, panic_timeout, stop_seq };
    ...
    futures::stream::unfold(Some(state), |state| async move {
        match state {
            ...
            Some(mut state) => {
                ...
                let checkpoint =
                    wait_for_checkpoint(&state.checkpoint_store, seq, state.warning_timeout, state.panic_timeout).await;
...
impl PipelineStages {
    pub fn new(starting_seq: CheckpointSequenceNumber, metrics: Arc<CheckpointExecutorMetrics>) -> Arc<Self> {
        Arc::new(Self { stages: std::array::from_fn(|_| SequenceWatch::new(starting_seq)), metrics })
    }
    ...
    async fn begin(&self, stage: PipelineStage, seq: CheckpointSequenceNumber) {
        ...
        self.metrics.stage_wait_duration_ns.with_label_values(&[stage.as_str()]).inc_by(duration.as_nanos() as u64);
    }
...
mod test {
    ...
    async fn test_skip_pipeline_stages() {
        ...
        handle.finish_stage(PipelineStage::WaitForTransactions).await;
    }
