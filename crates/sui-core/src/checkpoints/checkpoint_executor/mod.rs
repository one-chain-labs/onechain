...
use sui_types::{
    base_types::{SequenceNumber, TransactionDigest, TransactionEffectsDigest},
    ...
    pub fn new(
        ...
        for (idx, efx) in effects.iter().enumerate() {
            ...
            if let Some(acc_version) = acc_version {
                ...
                for slot in accumulator_versions.iter_mut().take(idx + 1).skip(next_update_index) {
        ...
        assert!(
            next_update_index == 0
                ...
                || (next_update_index == transactions.len() - 1
                    && transactions.last().unwrap().transaction_data().is_end_of_epoch_tx())
        );
        Self { transactions, effects, executed_fx_digests, accumulator_versions }
    }
}
...
impl CheckpointExecutor {
    ...
    fn process_checkpoint_data(
        ...
        let checkpoint =
            load_checkpoint(ckpt_data, tx_data, self.state.get_object_store(), &*self.transaction_cache_reader)
    ...
    fn schedule_transaction_execution(
        ...
        let (unexecuted_tx_digests, unexecuted_txns): (Vec<_>, Vec<_>) = itertools::multiunzip(
            ...
            .filter_map(
                |(txn, tx_digest, expected_fx_digest, effects, executed_fx_digest, accumulator_version)| {
    ...
    async fn execute_change_epoch_tx(&self, tx_data: &CheckpointTransactionData) {
        ...
        self.transaction_cache_reader
            .notify_read_executed_effects_digests("CheckpointExecutor::notify_read_advance_epoch_tx", &[
                *change_epoch_tx.digest(),
            ])
            .await;
    }
    ...
    #[instrument(level = "info", skip_all)]
    async fn commit_index_updates_and_enqueue_to_subscription_service(&self, checkpoint: Checkpoint) {
        if let Some(rpc_index) = &self.state.rpc_index {
            rpc_index
