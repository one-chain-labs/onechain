...
impl CheckpointExecutionState {
    pub fn new(data: CheckpointExecutionData, executed_fx_digests: Vec<Option<TransactionEffectsDigest>>) -> Self {
        Self { data, executed_fx_digests, accumulator: None, full_data: None }
    }
}
...
impl CheckpointExecutor {
        ...
        tokio::task::spawn_blocking({
            move || {
                ...
                cache_commit.commit_transaction_outputs(
                    ...
                    self.epoch_store.protocol_config().use_object_per_epoch_marker_table_v2_as_option().unwrap_or(false),
                );
                ...
                if let Some(randomness_reporter) = self.epoch_store.randomness_reporter() {
                    let randomness_rounds = self
                        .extract_randomness_rounds(&ckpt_state.data.checkpoint, &ckpt_state.data.checkpoint_contents);
    ...
    async fn execute_checkpoint(self: Arc<Self>, checkpoint: VerifiedCheckpoint) -> CheckpointExecutionState {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            ckpt_state.accumulator = Some(
                self.accumulator
                    .accumulate_checkpoint(&ckpt_state.data.effects, sequence_number, &self.epoch_store)
    ...
    fn process_checkpoint_data(&self, ckpt_state: &CheckpointExecutionState) -> Option<CheckpointData> {
        if !self.checkpoint_data_enabled() {
            return None;
        }

        let checkpoint_data =
            load_checkpoint_data(&ckpt_state.data, &*self.object_cache_reader, &*self.transaction_cache_reader)
    ...
    fn load_checkpoint_transactions(&self, checkpoint: VerifiedCheckpoint) -> CheckpointExecutionState {
    ...
    fn schedule_transaction_execution(&self, ckpt_state: &CheckpointExecutionState) -> Vec<TransactionDigest> {
        ...
        self.tx_manager.enqueue_with_expected_effects_digest(unexecuted_txns, &self.epoch_store);
