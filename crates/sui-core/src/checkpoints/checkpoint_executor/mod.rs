...
impl CheckpointExecutionState {
    ...
    pub fn new_with_global_state_hasher(data: CheckpointExecutionData, hasher: GlobalStateHash) -> Self {
...
impl CheckpointExecutor {
    ...
    fn get_next_to_schedule(&self) -> Option<CheckpointSequenceNumber> {
        ...
        if let Some(highest_executed) = &highest_executed {
            if self.epoch_store.epoch() == highest_executed.epoch() && highest_executed.is_last_checkpoint_of_epoch() {
    ...
    pub async fn run_epoch(self, run_with_range: Option<RunWithRange>) -> StopReason {
        ...
        self.metrics.checkpoint_exec_epoch.set(self.epoch_store.epoch() as i64);
...
impl CheckpointExecutor {
    ...
    async fn execute_checkpoint(
        ...
        checkpoint
            .report_checkpoint_age(&self.metrics.checkpoint_contents_age, &self.metrics.checkpoint_contents_age_ms);
        ...
        if checkpoint.is_last_checkpoint_of_epoch() && sequence_number > 0 {
            let _wait_for_previous_checkpoints_guard =
                mysten_metrics::monitored_scope("CheckpointExecutor::wait_for_previous_checkpoints");
        ...
        let ckpt_state = if self.state.is_fullnode(&self.epoch_store) || checkpoint.is_last_checkpoint_of_epoch() {
        ...
        let tps =
            self.tps_estimator.lock().update(Instant::now(), ckpt_state.data.checkpoint.network_total_transactions);
        self.metrics.checkpoint_exec_sync_tps.set(tps as i64);

        self.backpressure_manager.update_highest_executed_checkpoint(*ckpt_state.data.checkpoint.sequence_number());
        ...
        let batch = self.state.get_cache_commit().build_db_batch(self.epoch_store.epoch(), &ckpt_state.data.tx_digests);
        ...
        let mut ckpt_state = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                cache_commit.commit_transaction_outputs(this.epoch_store.epoch(), batch, &ckpt_state.data.tx_digests);
                ckpt_state
            }
        })
        .await
        ...
        let randomness_rounds =
            self.extract_randomness_rounds(&ckpt_state.data.checkpoint, &ckpt_state.data.checkpoint_contents);
        ...
        if self.state.is_fullnode(&self.epoch_store) {
            ...
            self.epoch_store.remove_shared_version_assignments(
                randomness_rounds.iter().map(|round| TransactionKey::RandomnessRound(epoch, *round)),
            );

            self.epoch_store.remove_shared_version_assignments(
                ckpt_state.data.tx_digests.iter().copied().map(TransactionKey::Digest),
            );
        }
        ...
        if let Some(randomness_reporter) = self.epoch_store.randomness_reporter() {
            for round in randomness_rounds {
                ...
                randomness_reporter.notify_randomness_in_checkpoint(round).expect("epoch cannot have ended");
            }
        }
    ...
    async fn verify_locally_built_checkpoint(
        ...
        assert!(!checkpoint.is_last_checkpoint_of_epoch(), "only fullnode path has end-of-epoch logic");

        let sequence_number = checkpoint.sequence_number;
        let locally_built_checkpoint =
            self.checkpoint_store.get_locally_computed_checkpoint(sequence_number).expect("db error");
        ...
        let Some(locally_built_checkpoint) = locally_built_checkpoint else {
            ...
            return self.execute_transactions_from_synced_checkpoint(checkpoint, pipeline_handle).await;
        };

        self.metrics.checkpoint_executor_validator_path.inc();

        // Check for fork
        assert_checkpoint_not_forked(&locally_built_checkpoint, &checkpoint, &self.checkpoint_store);

        // Checkpoint builder triggers accumulation of the checkpoint, so this is guaranteed to finish.
        let state_hasher = {
            let _metrics_scope = mysten_metrics::monitored_scope("CheckpointExecutor::notify_read_state_hasher");
            self.epoch_store.notify_read_checkpoint_state_hasher(&[sequence_number]).await.unwrap().pop().unwrap()
        };
        ...
        let (tx_digests, fx_digests): (Vec<_>, Vec<_>) =
            checkpoint_contents.iter().map(|digests| (digests.transaction, digests.effects)).unzip();
    ...
    async fn execute_transactions_from_synced_checkpoint(
        ...
        {
            let _metrics_scope =
                mysten_metrics::monitored_scope("CheckpointExecutor::notify_read_executed_effects_digests");
            self.transaction_cache_reader.notify_read_executed_effects_digests(&unexecuted_tx_digests).await;
        }
    ...
    fn insert_finalized_transactions(
        ...
        if self.state.is_fullnode(&self.epoch_store) {
            ...
            self.state.get_checkpoint_cache().deprecated_insert_finalized_transactions(
                tx_digests,
    ...
    fn process_checkpoint_data(
        ...
        let checkpoint_data =
            load_checkpoint_data(ckpt_data, tx_data, self.state.get_object_store(), &*self.transaction_cache_reader)
        ...
        if self.state.rpc_index.is_some() || self.config.data_ingestion_dir.is_some() {
            ...
            if let Some(rpc_index) = &self.state.rpc_index {
                let mut layout_resolver = self.epoch_store.executor().type_layout_resolver(Box::new(
                    PackageStoreWithFallback::new(self.state.get_backing_package_store(), &checkpoint_data),
                ));
            ...
            if let Some(path) = &self.config.data_ingestion_dir {
                store_checkpoint_locally(path, &checkpoint_data).expect("failed to store checkpoint locally");
            }
        }
    ...
    fn load_checkpoint_transactions(
            ...
            .tap_some(|_| debug!("loaded full checkpoint contents in bulk for sequence {seq}"))
        {
            ...
            full_contents.into_iter().zip(checkpoint_contents.iter()).for_each(|(execution_data, digests)| {
            ...
            let executed_fx_digests = self.transaction_cache_reader.multi_get_executed_effects_digests(&tx_digests);
            ...
            (
                ...
                CheckpointTransactionData { transactions, effects, executed_fx_digests },
            )
        } else {
            ...
            let (tx_digests, fx_digests): (Vec<_>, Vec<_>) = digests.iter().map(|d| (d.transaction, d.effects)).unzip();
            let transactions = self
                ...
                .enumerate()
                .map(|(i, tx)| {
                    let tx = tx.unwrap_or_else(|| fatal!("transaction not found for {:?}", tx_digests[i]));
            ...
            let effects = self
                ...
                .into_iter()
                .enumerate()
                .map(|(i, effect)| effect.unwrap_or_else(|| fatal!("checkpoint effect not found for {:?}", digests[i])))
                .collect();
    ...
    fn schedule_transaction_execution(
        ...
        let (unexecuted_tx_digests, unexecuted_txns, unexecuted_effects): (Vec<_>, Vec<_>, Vec<_>) =
            itertools::multiunzip(
                ...
                .filter_map(|(txn, tx_digest, expected_fx_digest, effects, executed_fx_digest)| {
        ...
        for ((tx, _), effects) in itertools::izip!(unexecuted_txns.iter(), unexecuted_effects) {
            if tx.contains_shared_object() {
                self.epoch_store
                    .acquire_shared_version_assignments_from_effects(tx, effects, &*self.object_cache_reader)
                    .expect("failed to acquire shared version assignments");
            }
        }

        // Enqueue unexecuted transactions with their expected effects digests
        self.execution_scheduler.enqueue_with_expected_effects_digest(unexecuted_txns, &self.epoch_store);
    ...
    async fn execute_change_epoch_tx(&self, tx_data: &CheckpointTransactionData) {
        ...
        assert!(change_epoch_tx.transaction_data().is_end_of_epoch_tx(), "final txn must be an end of epoch txn");
        ...
        self.execution_scheduler.enqueue_with_expected_effects_digest(
            vec![(change_epoch_tx.clone(), change_epoch_fx.digest())],
            &self.epoch_store,
        );

        self.transaction_cache_reader.notify_read_executed_effects_digests(&[*change_epoch_tx.digest()]).await;
    }
    ...
    #[instrument(level = "info", skip_all)]
    async fn commit_index_updates_and_enqueue_to_subscription_service(&self, checkpoint: CheckpointData) {
        if let Some(rpc_index) = &self.state.rpc_index {
            rpc_index
    ...
    fn extract_randomness_rounds(
            ...
            .expect("unable to get version_specific_data")
        {
            ...
            // dependent on it.
            assert_eq!(0, self.epoch_store.protocol_config().min_checkpoint_interval_ms_as_option().unwrap_or_default(),);
