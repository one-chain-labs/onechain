...
pub(crate) fn store_checkpoint_locally(path: impl AsRef<Path>, checkpoint_data: &CheckpointData) -> SuiResult {
    let path = path.as_ref();
    let file_name = format!("{}.chk", checkpoint_data.checkpoint_summary.sequence_number);

    std::fs::create_dir_all(path)
        .map_err(|err| SuiErrorKind::FileIOError(format!("failed to save full checkpoint content locally {:?}", err)))?;
    ...
    let object_set = {
        ...
        for (idx, object) in objects.into_iter().enumerate() {
            object_set.insert(object.ok_or_else(|| {
                sui_types::storage::error::Error::custom(format!("unabled to load object {:?}", refs[idx]))
            })?);
        }
        object_set
    };
