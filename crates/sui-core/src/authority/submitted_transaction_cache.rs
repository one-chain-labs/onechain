...
use std::{collections::BTreeSet, net::IpAddr, num::NonZeroUsize, sync::Arc};
...
use sui_types::{digests::TransactionDigest, traffic_control::Weight};
use tracing::debug;
    ...
    pub fn new(registry: &Registry) -> Self {
        Self {
            ...
            submission_count_exceeded: register_histogram_with_registry!(
                ...
                vec![1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0, 5000.0, 10000.0,],
                registry,
            )
            .unwrap(),
...
impl SubmittedTransactionCache {
    pub(crate) fn new(cache_capacity: Option<usize>, metrics: Arc<SubmittedTransactionCacheMetrics>) -> Self {
        let capacity = cache_capacity
            .and_then(NonZeroUsize::new)
            .unwrap_or_else(|| NonZeroUsize::new(DEFAULT_CACHE_CAPACITY).unwrap());

        Self { inner: RwLock::new(Inner { transactions: LruCache::new(capacity) }), metrics }
    }
    ...
    pub(crate) fn record_submitted_tx(
        ...
        if let Some(metadata) = inner.transactions.get_mut(digest) {
            ...
            let metadata = SubmissionMetadata { submission_count: 0, max_allowed_submissions, submitter_client_addrs };

            inner.transactions.put(*digest, metadata);

            self.metrics.transactions_tracked.set(inner.transactions.len() as i64);
            self.metrics.amplification_factor_distribution.observe(amplification_factor as f64);

            debug!("First submission of transaction {digest} (max_allowed: {max_allowed_submissions})",);
        }
    }
    ...
    pub(crate) fn increment_submission_count(&self, digest: &TransactionDigest) -> Option<(Weight, BTreeSet<IpAddr>)> {
        let mut inner = self.inner.write();
        ...
        if let Some(metadata) = inner.transactions.get_mut(digest) {
            ...
            if metadata.submission_count > metadata.max_allowed_submissions {
                ...
                self.metrics.submission_count_exceeded.observe(metadata.submission_count as f64);

                debug!(
                    "Transaction {} seen in consensus {} times, exceeds limit {} (spam_weight: {:?})",
                    digest, metadata.submission_count, metadata.max_allowed_submissions, spam_weight
                );
...
mod tests {
    ...
    fn test_first_submission_allowed() {
        let cache = SubmittedTransactionCache::new(None, Arc::new(SubmittedTransactionCacheMetrics::new_test()));
    ...
    fn test_lru_eviction() {
        ...
        let cache = SubmittedTransactionCache::new(Some(3), Arc::new(SubmittedTransactionCacheMetrics::new_test()));
