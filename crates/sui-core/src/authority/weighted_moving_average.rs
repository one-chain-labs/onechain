...
impl WeightedMovingAverage {
    ...
    pub fn new(default: u64, max_size: usize) -> Self {
        Self { queue: VecDeque::new(), total_weight: 0, max_size, default }
    }
    ...
    pub fn add_sample(&mut self, value: u64, weight: u64) {
        ...
        let sample = Sample { value, weight, insertion_total: self.total_weight };
        ...
        if self.is_full() {
            ...
            if let Some(min_idx) = self.queue.iter().enumerate().find(|(_, s)| s.weight < sample.weight).map(|(i, _)| i)
            {
    ...
    pub fn get_weighted_average(&self) -> u64 {
        ...
        let (weighted_sum, total_w) =
            self.queue.iter().fold((0u64, 0u64), |(sum, w_sum), s| (sum + s.value * s.weight, w_sum + s.weight));
