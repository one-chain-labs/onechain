...
use super::{shared_object_version_manager::AssignedVersions, test_authority_builder::TestAuthorityBuilder, *};
...
pub async fn send_and_confirm_transaction_(
    ...
    let (txn, effects, _execution_error_opt) =
        send_and_confirm_transaction_with_execution_error(authority, fullnode, transaction, with_shared, true).await?;
    Ok((txn, effects))
}
...
pub async fn execute_certificate_with_execution_error(
    ...
    let assigned_versions = if with_shared {
        if fake_consensus {
            ...
            let assigned_versions = authority
                ...
                .assign_shared_object_versions_for_tests(authority.get_object_cache_reader().as_ref(), &vec![
    ...
    let (result, execution_error_opt) = authority
        .try_execute_for_test(&certificate, ExecutionEnv::new().with_assigned_versions(assigned_versions.clone()))
        .await;
    ...
    if let Some(fullnode) = fullnode {
        fullnode.try_execute_for_test(&certificate, ExecutionEnv::new().with_assigned_versions(assigned_versions)).await;
    }
...
pub async fn send_and_confirm_transaction_with_execution_error(
    ...
    execute_certificate_with_execution_error(authority, fullnode, certificate, with_shared, fake_consensus).await
}
...
pub async fn init_state_with_ids_and_versions<I: IntoIterator<Item = (SuiAddress, ObjectID, SequenceNumber)>>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
    for (address, object_id, version) in objects {
        let obj = Object::with_id_owner_version_for_testing(object_id, version, Owner::AddressOwner(address));
        state.insert_genesis_object(obj).await;
    }
    state
}

pub async fn init_state_with_objects<I: IntoIterator<Item = Object>>(objects: I) -> Arc<AuthorityState> {
...
pub async fn init_state_with_object_id(address: SuiAddress, object: ObjectID) -> Arc<AuthorityState> {
    init_state_with_ids(std::iter::once((address, object))).await
}

pub async fn init_state_with_ids_and_expensive_checks<I: IntoIterator<Item = (SuiAddress, ObjectID)>>(
    objects: I,
...
pub fn init_certified_transaction(transaction: Transaction, authority_state: &AuthorityState) -> VerifiedCertificate {
    let epoch_store = authority_state.epoch_store_for_testing();
    let transaction = epoch_store.verify_transaction(transaction).unwrap();

    let vote = VerifiedSignedTransaction::new(0, transaction.clone(), authority_state.name, &*authority_state.secret);
    CertifiedTransaction::new(transaction.into_message(), vec![vote.auth_sig().clone()], epoch_store.committee())
        .unwrap()
...
pub async fn enqueue_all_and_execute_all(
    ...
    authority.execution_scheduler.enqueue(
        certificates
            .iter()
            .map(|(cert, env)| (VerifiedExecutableTransaction::new_from_certificate(cert.clone()).into(), env.clone()))
            .collect(),
...
pub async fn execute_sequenced_certificate_to_effects(
    ...
    authority.execution_scheduler.enqueue(
        vec![(VerifiedExecutableTransaction::new_from_certificate(certificate.clone()).into(), env.clone())],
        &authority.epoch_store_for_testing(),
    );
...
pub async fn send_consensus(authority: &AuthorityState, cert: &VerifiedCertificate) -> AssignedVersions {
    let assigned_versions = authority
        .epoch_store_for_testing()
        .assign_shared_object_versions_for_tests(authority.get_object_cache_reader().as_ref(), &vec![
    ...
    let assigned_versions =
        assigned_versions.into_map().get(&cert.key()).cloned().unwrap_or_else(|| AssignedVersions::new(vec![], None));
    ...
    authority.execution_scheduler().enqueue_transactions(certs, &authority.epoch_store_for_testing());

    assigned_versions
}

pub async fn send_consensus_no_execution(authority: &AuthorityState, cert: &VerifiedCertificate) -> AssignedVersions {
    ...
    C: crate::checkpoints::CheckpointServiceNotify + Send + Sync + 'static,
{
    ...
    let consensus_transactions: Vec<ConsensusTransaction> = certificates
        .iter()
        .map(|cert| ConsensusTransaction::new_certificate_message(&authority.name, cert.clone().into()))
        .collect();
    ...
    let commit = TestConsensusCommit::new(consensus_transactions, round, timestamp_ms, sub_dag_index);
