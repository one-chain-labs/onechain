...
pub async fn execute_certificate_with_execution_error(
    ...
    if with_shared {
        ...
        if let Some(fullnode) = fullnode {
            fullnode
                .epoch_store_for_testing()
                .assign_shared_object_versions_for_tests(fullnode.get_object_cache_reader().as_ref(), &vec![
                    VerifiedExecutableTransaction::new_from_certificate(certificate.clone()),
                ])
                .await?;
        }
...
pub fn init_transfer_transaction(
    ...
    let data = TransactionData::new_transfer(recipient, object_ref, sender, gas_object_ref, gas_budget, gas_price);
...
pub async fn enqueue_all_and_execute_all(
    ...
    authority.enqueue_certificates_for_execution(certificates.clone(), &authority.epoch_store_for_testing());
...
pub async fn execute_sequenced_certificate_to_effects(
    ...
    authority.enqueue_certificates_for_execution(vec![certificate.clone()], &authority.epoch_store_for_testing());
...
pub async fn send_consensus(authority: &AuthorityState, cert: &VerifiedCertificate) {
    let transaction = SequencedConsensusTransaction::new_test(ConsensusTransaction::new_certificate_message(
        &authority.name,
...
pub fn build_test_modules_with_dep_addr(
    ...
    for (addr_name, obj_id) in dep_original_addresses {
        build_config.config.additional_named_addresses.insert(addr_name.to_string(), AccountAddress::from(obj_id));
    }
    let mut package = build_config.build(path).unwrap();

    let dep_id_mapping: BTreeMap<_, _> =
        dep_ids.into_iter().map(|(dep_name, obj_id)| (Symbol::from(dep_name), obj_id)).collect();

    assert_eq!(dep_id_mapping.len(), package.dependency_ids.unpublished.len());
    for unpublished_dep in &package.dependency_ids.unpublished {
        ...
        assert!(package.dependency_ids.published.insert(*unpublished_dep, *published_id).is_none())
    }
...
pub async fn publish_package_on_single_authority(
    ...
    let txn_data =
        TransactionData::new_programmable(sender, vec![gas_payment], pt, rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH, rgp);
    ...
    let package_id = effects.data().created().iter().find(|c| c.1 == Owner::Immutable).unwrap().0 .0;
    let cap_object = effects.data().created().iter().find(|c| matches!(c.1, Owner::AddressOwner(..))).unwrap().0;
