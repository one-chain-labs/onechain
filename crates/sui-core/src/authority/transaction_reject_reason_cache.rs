...
use sui_types::{committee::EpochId, error::SuiError, messages_consensus::ConsensusPosition};
use tracing::trace;
...
impl TransactionRejectReasonCache {
    ...
    pub fn set_last_committed_leader_round(&self, round: u32) {
        let _scope = monitored_scope("TransactionRejectReasonCache::set_last_committed_leader_round");
...
mod test {
    ...
    async fn test_set_rejection_vote_reason_and_get_reason() {
        ...
        let position =
            ConsensusPosition { epoch: 1, block: BlockRef::new(1, AuthorityIndex::MAX, BlockDigest::MAX), index: 1 };
        ...
        {
            let position =
                ConsensusPosition { epoch: 1, block: BlockRef::new(1, AuthorityIndex::MAX, BlockDigest::MIN), index: 2 };
    ...
    async fn test_set_last_committed_leader_round() {
        ...
        let position = |round: Round, transaction_index: u16| ConsensusPosition {
            epoch: 1,
            block: BlockRef::new(round, AuthorityIndex::new_for_test(transaction_index as u32), BlockDigest::MAX),
        ...
        for round in 0 .. TOTAL_ROUNDS {
            for transaction_index in 0 .. 5 {
                cache.set_rejection_vote_reason(position(round, transaction_index), &SuiError::InvalidTransactionDigest);
            }
        }
        ...
        for round in 0 .. TOTAL_ROUNDS {
            for transaction_index in 0 .. 5 {
                ...
                if round <= 2 {
                    ...
                    assert_eq!(cache.get_rejection_vote_reason(position), Some(SuiError::InvalidTransactionDigest));
                }
            }
        }
