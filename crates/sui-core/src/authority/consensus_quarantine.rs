...
use sui_types::{
    ...
    messages_consensus::{AuthorityIndex, Round, TimestampMs, VersionedDkgConfirmation},
...
use crate::{
    ...
    epoch::{
        randomness::{VersionedProcessedMessage, VersionedUsedProcessedMessages, SINGLETON_KEY},
...
pub(crate) struct ConsensusCommitOutput {
    ...
    execution_time_observations:
        Vec<(AuthorityIndex, u64 /* generation */, Vec<(ExecutionTimeObservationKey, Duration)>)>,
}
...
impl ConsensusCommitOutput {
    ...
    pub fn insert_execution_time_observation(
        ...
        self.execution_time_observations.push((source, generation, estimates));
    }
    ...
    pub fn get_consensus_messages_processed(&self) -> impl Iterator<Item = &SequencedConsensusTransactionKey> {
    ...
    pub fn defer_transactions(&mut self, key: DeferralKey, transactions: Vec<VerifiedExecutableTransaction>) {
    ...
    pub fn set_congestion_control_randomness_object_debts(&mut self, object_debts: Vec<(ObjectID, u64)>) {
    ...
    pub fn write_to_batch(self, epoch_store: &AuthorityPerEpochStore, batch: &mut DBBatch) -> SuiResult {
        ...
        let consensus_commit_stats = self.consensus_commit_stats.expect("consensus_commit_stats must be set");
...
pub(crate) struct ConsensusOutputCache {
    ...
    pub(crate) deferred_transactions_v2: Mutex<BTreeMap<DeferralKey, Vec<VerifiedExecutableTransaction>>>,
    ...
    pub(crate) user_signatures_for_checkpoints: Mutex<HashMap<TransactionDigest, Vec<GenericSignature>>>,
...
impl ConsensusOutputCache {
    pub(crate) fn new(epoch_start_configuration: &EpochStartConfiguration, tables: &AuthorityEpochTables) -> Self {
        let deferred_transactions_v2 =
            tables.get_all_deferred_transactions_v2().expect("load deferred transactions cannot fail");
...
pub(crate) struct ConsensusOutputQuarantine {
    ...
    builder_checkpoint_summary: BTreeMap<CheckpointSequenceNumber, (BuilderCheckpointSummary, CheckpointContents)>,
    ...
    congestion_control_randomness_object_debts: RefCountedHashMap<ObjectID, CongestionPerObjectDebt>,
...
impl ConsensusOutputQuarantine {
    ...
    pub(crate) fn push_consensus_output(
        ...
        self.metrics.consensus_quarantine_queue_size.set(self.output_queue.len() as i64);
...
impl ConsensusOutputQuarantine {
    ...
    fn commit_with_batch(&mut self, epoch_store: &AuthorityPerEpochStore, batch: &mut DBBatch) -> SuiResult {
        ...
        while self.builder_checkpoint_summary.first_key_value().map(|(seq, _)| *seq <= self.highest_executed_checkpoint)
            == Some(true)
        {
            let (seq, (builder_summary, contents)) = self.builder_checkpoint_summary.pop_first().unwrap();
            ...
            batch.insert_batch(&tables.builder_digest_to_checkpoint, contents.iter().map(|tx| (tx.transaction, seq)))?;

            batch.insert_batch(&tables.builder_checkpoint_summary_v2, [(seq, &builder_summary)])?;
        ...
        while !self.output_queue.is_empty() {
            ...
            let Some(highest_in_commit) = self.output_queue.front().unwrap().get_highest_pending_checkpoint_height()
            else {
            ...
            if highest_in_commit <= highest_committed_height {
                info!("committing output with highest pending checkpoint height {:?}", highest_in_commit);
...
impl ConsensusOutputQuarantine {
    ...
    fn insert_congestion_control_debts(&mut self, output: &ConsensusCommitOutput) {
        ...
        for (object_id, debt) in output.congestion_control_object_debts.iter() {
            self.congestion_control_object_debts.insert(*object_id, CongestionPerObjectDebt::new(current_round, *debt));
        }

        for (object_id, debt) in output.congestion_control_randomness_object_debts.iter() {
            self.congestion_control_randomness_object_debts
                .insert(*object_id, CongestionPerObjectDebt::new(current_round, *debt));
        }
    }
    ...
    fn remove_congestion_control_debts(&mut self, output: &ConsensusCommitOutput) {
        ...
        for (object_id, _) in output.congestion_control_randomness_object_debts.iter() {
            self.congestion_control_randomness_object_debts.remove(object_id);
        }
    }
        ...
        if let Some(next_versions) = output.next_shared_object_versions.as_ref() {
            for object_id in next_versions.keys() {
                if !self.shared_object_next_versions.remove(object_id) {
                    fatal!("Shared object next version not found in quarantine: {:?}", object_id);
                }
            }
        }
    }
...
impl ConsensusOutputQuarantine {
    ...
    pub(super) fn get_built_summary(&self, sequence: CheckpointSequenceNumber) -> Option<&BuilderCheckpointSummary> {
    ...
    pub(super) fn is_consensus_message_processed(&self, key: &SequencedConsensusTransactionKey) -> bool {
    ...
    pub(super) fn get_new_jwks(&self, epoch_store: &AuthorityPerEpochStore, round: u64) -> SuiResult<Vec<ActiveJwk>> {
        ...
        for output in self.output_queue.iter().rev() {
            ...
            if round == output_round {
                return Ok(output
                    .active_jwks
                    .iter()
                    .map(|(_, (jwk_id, jwk))| ActiveJwk { jwk_id: jwk_id.clone(), jwk: jwk.clone(), epoch })
                    .collect());
            }
        }
    ...
    pub(super) fn get_randomness_last_round_timestamp(&self) -> Option<TimestampMs> {
        self.output_queue.iter().rev().filter_map(|output| output.get_randomness_last_round_timestamp()).next()
    }
    ...
    pub(crate) fn load_initial_object_debts(
        ...
        let (hash_table, db_table, per_commit_budget) = if for_randomness {
            ...
            (&self.congestion_control_object_debts, &tables.congestion_control_object_debts, default_per_commit_budget)
        };
        let mut shared_input_object_ids: Vec<_> =
            transactions.iter().flat_map(|tx| tx.shared_input_objects().map(|obj| obj.id)).collect();
