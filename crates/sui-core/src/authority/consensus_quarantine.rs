...
pub(crate) struct ConsensusCommitOutput {
    ...
    shared_object_versions: Option<(AssignedTxAndVersions, HashMap<ConsensusObjectSequenceKey, SequenceNumber>)>,
...
impl ConsensusCommitOutput {
    ...
    pub fn insert_user_signatures_for_checkpoints(&mut self, transactions: &[VerifiedExecutableTransaction]) {
    ...
    pub fn write_to_batch(self, epoch_store: &AuthorityPerEpochStore, batch: &mut DBBatch) -> SuiResult {
        ...
        if let Some((assigned_versions, next_versions)) = self.shared_object_versions {
            if epoch_store.epoch_start_config().use_version_assignment_tables_v3() {
                batch.insert_batch(&tables.assigned_shared_object_versions_v3, assigned_versions)?;
