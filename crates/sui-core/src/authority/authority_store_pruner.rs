...
use typed_store::{
    rocksdb::{compaction_filter::Decision, LiveFile},
    Map,
    TypedStoreError,
};
...
impl AuthorityStorePruner {
        ...
        for effects in &transaction_effects {
            ...
            if enable_pruning_tombstones {
                for deleted_object_key in effects.all_tombstones() {
                    object_tombstones_to_prune.push(ObjectKey(deleted_object_key.0, deleted_object_key.1));
                }
            }
        }

        metrics.num_pruned_objects.inc_by(live_object_keys_to_prune.len() as u64);
        metrics.num_pruned_tombstones.inc_by(object_tombstones_to_prune.len() as u64);
    ...
    fn prune_checkpoints(
        ...
        let transactions: Vec<_> =
            checkpoint_content_to_prune.iter().flat_map(|content| content.iter().map(|tx| tx.transaction)).collect();
        ...
        perpetual_batch.delete_batch(&perpetual_db.executed_transactions_to_checkpoint, transactions.iter())?;
        ...
        for effects in effects_to_prune {
            ...
            if effects.events_digest().is_some() {
                perpetual_batch.delete_batch(&perpetual_db.events_2, [effects.transaction_digest()])?;
            }
        }
        perpetual_batch.delete_batch(&perpetual_db.unchanged_loaded_runtime_objects, transactions.iter())?;
        ...
        let checkpoint_content_digests = checkpoint_content_to_prune.iter().map(|ckpt| ckpt.digest());
        checkpoints_batch.delete_batch(&checkpoint_db.tables.checkpoint_content, checkpoint_content_digests.clone())?;
        checkpoints_batch
            .delete_batch(&checkpoint_db.tables.checkpoint_sequence_by_contents_digest, checkpoint_content_digests)?;

        checkpoints_batch.delete_batch(&checkpoint_db.tables.checkpoint_by_digest, checkpoints_to_prune)?;

        checkpoints_batch.insert_batch(&checkpoint_db.tables.watermarks, [(
        ...
        metrics.last_pruned_effects_checkpoint.set(checkpoint_number as i64);
    ...
    pub async fn prune_objects_for_eligible_epochs(
        ...
        let (mut max_eligible_checkpoint_number, epoch_id) = checkpoint_store
            ...
            .unwrap_or_default();
        let pruned_checkpoint_number = perpetual_db.get_highest_pruned_checkpoint()?.unwrap_or_default();
    ...
    pub async fn prune_checkpoints_for_eligible_epochs(
        ...
        let pruned_checkpoint_number = checkpoint_store.get_highest_pruned_checkpoint_seq_number()?.unwrap_or(0);
        ...
        if config.num_epochs_to_retain != u64::MAX {
            max_eligible_checkpoint =
                min(max_eligible_checkpoint, perpetual_db.get_highest_pruned_checkpoint()?.unwrap_or_default());
        }
        if config.smooth
        ...
        Self::prune_for_eligible_epochs(
            ...
            config.num_epochs_to_retain_for_checkpoints().ok_or_else(|| anyhow!("config value not set"))?,
            pruned_checkpoint_number,
            max_eligible_checkpoint,
            config.clone(),
            metrics.clone(),
        ...
        if let Some(num_epochs_to_retain) = config.num_epochs_to_retain_for_checkpoints() {
            Self::update_pruning_watermarks(checkpoint_store, num_epochs_to_retain, pruner_watermarks)?;
        }
        Ok(())
    }
    ...
    pub async fn prune_for_eligible_epochs(
        ...
        let current_epoch = checkpoint_store.get_highest_executed_checkpoint()?.map(|c| c.epoch()).unwrap_or_default();
        ...
        loop {
            let Some(ckpt) = checkpoint_store.tables.certified_checkpoints.get(&(checkpoint_number + 1))? else {
                break;
            };
            ...
            let content = checkpoint_store
                ...
                .ok_or_else(|| anyhow::anyhow!("checkpoint content data is missing: {}", checkpoint.sequence_number))?;
    ...
    fn prune_indexes(
        ...
        if let (Some(mut epochs_to_retain), Some(indexes)) = (config.num_epochs_to_retain_for_indexes, indexes) {
            if epochs_to_retain < 7 {
                warn!("num_epochs_to_retain_for_indexes is too low. Reseting it to 7");
                epochs_to_retain = 7;
            }
            let now = SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis();
            if let Some(cut_time_ms) = u64::try_from(now)?.checked_sub(epochs_to_retain * epoch_duration_ms) {
                let transaction_id = indexes.prune(cut_time_ms)?;
                metrics.last_pruned_indexes_transaction.set(transaction_id as i64);
            }
        }
        Ok(())
    }
    ...
    async fn prune_executed_tx_digests(
        ...
        info!("Pruning executed_transaction_digests for epochs < {} (current epoch: {})", target_epoch, current_epoch);
    ...
    fn prune_executed_tx_digests_th(
        ...
        perpetual_db.executed_transaction_digests.drop_cells_in_range(&from_key, &to_key)?;
        Ok(())
    }
    ...
    fn update_pruning_watermarks(
        ...
        let current_epoch_id = checkpoint_store.get_highest_executed_checkpoint()?.map(|c| c.epoch).unwrap_or_default();
        if current_epoch_id < num_epochs_to_retain {
            return Ok(false);
        }
        let target_epoch_id = current_epoch_id - num_epochs_to_retain;
        let checkpoint_id = checkpoint_store.get_epoch_last_checkpoint_seq_number(target_epoch_id)?;
    ...
    fn prune_th(
        ...
        let watermark_updated =
            Self::update_pruning_watermarks(checkpoint_store, num_epochs_to_retain, &pruning_watermark)?;
    ...
    fn compact_next_sst_file(
        ...
        let mut state = last_processed.lock().expect("failed to obtain a lock for last processed SST files");
        let mut sst_file_for_compaction: Option<LiveFile> = None;
        let time_threshold = SystemTime::now() - Duration::from_secs(delay_days as u64 * 24 * 60 * 60);
    ...
    fn smoothed_max_eligible_checkpoint_number(
        ...
        let num_intervals =
            epoch_duration_ms.checked_div(Self::pruning_tick_duration_ms(epoch_duration_ms)).unwrap_or(1);
    ...
    fn setup_pruning(
        ...
        debug!("Starting object pruning service with num_epochs_to_retain={}", config.num_epochs_to_retain);

        let tick_duration = Duration::from_millis(Self::pruning_tick_duration_ms(epoch_duration_ms));
        let pruning_initial_delay = if cfg!(msim) {
            Duration::from_millis(1)
        } else {
            Duration::from_secs(config.pruning_run_delay_seconds.unwrap_or(60 * 60))
        };
        let mut objects_prune_interval = tokio::time::interval_at(Instant::now() + pruning_initial_delay, tick_duration);

        metrics.num_epochs_to_retain_for_objects.set(config.num_epochs_to_retain as i64);
        metrics
            .num_epochs_to_retain_for_checkpoints
            .set(config.num_epochs_to_retain_for_checkpoints.unwrap_or_default() as i64);
            ...
            if let Some(delay_days) = config.periodic_compaction_threshold_days {
                spawn_monitored_task!(async move {
                    ...
                    loop {
                        ...
                        let result =
                            tokio::task::spawn_blocking(move || Self::compact_next_sst_file(db, delay_days, state))
                                .await;
...
impl ObjectsCompactionFilter {
    pub fn new(db: Arc<AuthorityPrunerTables>, registry: &Registry) -> Self {
        Self { db: Arc::downgrade(&db), metrics: ObjectCompactionMetrics::new(registry) }
    }

    pub fn filter(&mut self, key: &[u8], value: &[u8]) -> anyhow::Result<Decision> {
        let ObjectKey(object_id, version) =
            bincode::DefaultOptions::new().with_big_endian().with_fixint_encoding().deserialize(key)?;
...
mod tests {
    ...
    use typed_store::{
        rocks::{default_db_options, DBMap, MetricConf, ReadWriteOptions},
        Map,
    };
    ...
    fn get_keys_after_pruning(path: &Path) -> anyhow::Result<HashSet<ObjectKey>> {
        ...
        let cfs: Vec<_> = cf_names.keys().map(|x| (x.as_str(), default_db_options().options)).collect();
        let perpetual_db =
            typed_store::rocks::open_cf_opts(perpetual_db_path, None, MetricConf::new("perpetual_pruning"), &cfs);
    ...
    fn generate_test_data(
        ...
        for id in ids {
            for (counter, seq) in (0 .. num_versions_per_object).rev().enumerate() {
                ...
                batch.insert_batch(&db.objects, [(ObjectKey(id, SequenceNumber::from(seq)), obj.clone())])?;
            }
            ...
            if num_object_versions_to_retain == 0 {
                ...
                batch.insert_batch(&db.objects, [(tombstone_key, StoreObjectWrapper::V1(StoreObject::Deleted))])?;
        ...
        assert_eq!(
            to_keep.len() as u64,
            std::cmp::min(num_object_versions_to_retain, num_versions_per_object) * total_unique_object_ids as u64
        );
        assert_eq!(
            tombstones.len() as u64,
            if num_object_versions_to_retain == 0 { total_unique_object_ids as u64 } else { 0 }
        );
    ...
    async fn run_pruner(
        ...
        let to_keep = {
            ...
            for object in to_delete {
                effects.unsafe_add_deleted_live_object_for_testing((object.0, object.1, ObjectDigest::MIN));
            }
            for object in tombstones {
                effects.unsafe_add_object_tombstone_for_testing((object.0, object.1, ObjectDigest::MIN));
            }
            AuthorityStorePruner::prune_objects(vec![effects], &db, None, 0, metrics, true).await.unwrap();
            to_keep
        };
    ...
    async fn test_pruning_objects() {
        ...
        assert_eq!(HashSet::from_iter(to_keep), get_keys_after_pruning(&path).unwrap());
    ...
    async fn test_db_size_after_compaction() -> Result<(), anyhow::Error> {
        ...
        for id in ids {
            for i in (0 .. num_versions_per_object).rev() {
                ...
                let obj = get_store_object(Object::immutable_with_id_for_testing(id));
                perpetual_db.objects.insert(&ObjectKey(id, SequenceNumber::from(i)), &obj)?;
            }
        }
        ...
        let metrics = AuthorityStorePruningMetrics::new(&registry);
        let total_pruned =
            AuthorityStorePruner::prune_objects(vec![effects], &perpetual_db, None, 0, metrics, true).await;
