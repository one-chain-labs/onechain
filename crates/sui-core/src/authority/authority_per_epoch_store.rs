...
use sui_types::{
    ...
    execution::{ExecutionTimeObservationChunkKey, ExecutionTimeObservationKey, ExecutionTiming},
...
use crate::{
    ...
    checkpoints::{BuilderCheckpointSummary, CheckpointHeight, EpochStats, PendingCheckpoint},
    consensus_handler::{
        ConsensusCommitInfo,
...
impl AuthorityEpochTables {
    ...
    fn get_all_deferred_transactions_v2(&self) -> SuiResult<BTreeMap<DeferralKey, Vec<VerifiedExecutableTransaction>>> {
        Ok(self
...
impl AuthorityPerEpochStore {
    ...
    pub fn display_registry_exists(&self) -> bool {
        self.epoch_start_configuration.display_registry_obj_initial_shared_version().is_some()
    }
        ...
        let stored_observations = if protocol_config.enable_observation_chunking() {
            if let Ok::<u64, _>(chunk_count) = get_dynamic_field_from_store(
                ...
                for chunk_index in 0 .. chunk_count {
                    ...
                    let Ok::<Vec<u8>, _>(chunk_bytes) =
                        get_dynamic_field_from_store(object_store, system_state.extra_fields.id.id.bytes, &chunk_key)
                    else {
                        debug_fatal!("Could not find stored execution time observation chunk {}", chunk_index);
                        return itertools::Either::Left(std::iter::empty());
                    };
            ...
            // This is stored as a vector<u8> in Move, so we double-deserialize to get back
            //`StoredExecutionTimeObservations`.
            let stored_observations: StoredExecutionTimeObservations = bcs::from_bytes(&stored_observations_bytes)
    ...
    pub fn get_all_deferred_transactions_for_test(&self) -> Vec<(DeferralKey, Vec<VerifiedExecutableTransaction>)> {
    ...
    pub fn deferred_transactions_empty_v2(&self) -> bool {
        self.consensus_output_cache.deferred_transactions_v2.lock().is_empty()
    }
    ...
    pub async fn transactions_executed_in_checkpoint_notify(
        ...
        Ok(self
            ...
            .read("transactions_executed_in_checkpoint_notify", &digests, |digests| {
                tables.executed_transactions_to_checkpoint.multi_get(digests).expect("db error")
            })
            .await)
    }

    pub fn has_received_end_of_publish_from(&self, authority: &AuthorityName) -> bool {
        self.end_of_publish.try_lock().expect("No contention on end_of_publish lock").contains_key(authority)
    }
    ...
    pub(crate) fn record_jwk_vote(
        ...
        info!(?round, "received jwk vote from {:?} for jwk ({:?}, {:?})", authority.concise(), id, jwk);
    ...
    pub(crate) fn process_user_signatures<'a>(&self, certificates: impl Iterator<Item = &'a Schedulable>) {
    ...
    pub(crate) fn get_reconfig_state_write_lock_guard(&self) -> RwLockWriteGuard<'_, ReconfigState> {
    ...
    pub fn assign_shared_object_versions_for_tests(
        ...
        for tx in transactions.iter() {
            if let Schedulable::Transaction(exec_tx) = tx {
                let key =
                    SequencedConsensusTransactionKey::External(ConsensusTransactionKey::Certificate(*exec_tx.digest()));
