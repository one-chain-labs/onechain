...
use super::{
    ...
    consensus_tx_status_cache::{ConsensusTxStatus, ConsensusTxStatusCache},
    epoch_start_configuration::EpochStartConfigTrait,
    execution_time_estimator::{ConsensusObservations, ExecutionTimeEstimator},
    ...
    submitted_transaction_cache::{SubmittedTransactionCache, SubmittedTransactionCacheMetrics},
    transaction_deferral::{transaction_deferral_within_limit, DeferralKey, DeferralReason},
    transaction_reject_reason_cache::TransactionRejectReasonCache,
};
use crate::{
    authority::{
        epoch_start_configuration::EpochStartConfiguration,
        execution_time_estimator::EXTRA_FIELD_EXECUTION_TIME_ESTIMATES_KEY,
    ...
    execution_cache::{cache_types::CacheResult, ObjectCacheRead},
...
use consensus_quarantine::{ConsensusCommitOutput, ConsensusOutputCache, ConsensusOutputQuarantine};
...
pub struct AuthorityEpochTables {
    ...
    pub(crate) pending_checkpoint_signatures: DBMap<(CheckpointSequenceNumber, u64), CheckpointSignatureMessage>,
    ...
    pub(crate) execution_time_observations: DBMap<(u64, AuthorityIndex), Vec<(ExecutionTimeObservationKey, Duration)>>,
}
...
impl AuthorityEpochTables {
    ...
    pub fn open(epoch: EpochId, parent_path: &Path, db_options: Option<Options>) -> Self {
        Self::open_tables_read_write(Self::path(epoch, parent_path), MetricConf::new("epoch"), db_options, None)
    }
    ...
    pub fn open(epoch: EpochId, parent_path: &Path, db_options: Option<Options>) -> Self {
        ...
        let pending_checkpoint_signatures_config =
            KeySpaceConfig::new().disable_unload().with_value_cache_size(default_value_cache_size());
        ...
        let configs = vec![
            ...
            ("last_consensus_stats".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("reconfig_state".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("end_of_publish".to_string(), ThConfig::new(104, 1, KeyType::uniform(1))),
            (
            ...
            ("authority_capabilities".to_string(), ThConfig::new(104, mutexes, uniform_key)),
            ("authority_capabilities_v2".to_string(), ThConfig::new(104, 1, KeyType::uniform(1))),
            ("override_protocol_upgrade_buffer_stake".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            (
            ...
            ("deferred_transactions".to_string(), ThConfig::new_with_indexing(KeyIndexing::Hash, mutexes, uniform_key)),
            ("dkg_processed_messages_v2".to_string(), ThConfig::new(2, 1, KeyType::uniform(1))),
            ("dkg_used_messages_v2".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("dkg_confirmations_v2".to_string(), ThConfig::new(2, 1, KeyType::uniform(1))),
            ("dkg_output".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("randomness_next_round".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("randomness_highest_completed_round".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            ("randomness_last_round_timestamp".to_string(), ThConfig::new(8, 1, KeyType::uniform(1))),
            (
                "congestion_control_object_debts".to_string(),
                ThConfig::new_with_config(32, mutexes, uniform_key, lru_bloom_config.clone()),
            ),
            ("congestion_control_randomness_object_debts".to_string(), ThConfig::new(32, mutexes, uniform_key)),
            ("execution_time_observations".to_string(), ThConfig::new(8 + 4, mutexes, uniform_key)),
        ];
    ...
    fn get_all_deferred_transactions(
        ...
        Ok(self.deferred_transactions.safe_iter().collect::<Result<_, _>>()?)
    }
}
...
impl AuthorityPerEpochStore {
    ...
    pub fn new(
        ...
        let end_of_publish = StakeAggregator::from_iter(committee.clone(), tables.end_of_publish.safe_iter())?;
        ...
        info!("initializing epoch store from chain id {:?} to chain id {:?}", chain_from_id, chain.1);
        ...
        let execution_time_observations =
            tables.execution_time_observations.safe_iter().collect::<Result<Vec<_>, _>>()?;
        let execution_time_estimator = if let PerObjectCongestionControlMode::ExecutionTimeEstimate(protocol_params) =
        ...
        let submitted_transaction_cache = SubmittedTransactionCache::new(None, submitted_transaction_cache_metrics);
    ...
    pub async fn set_randomness_manager(&self, mut randomness_manager: RandomnessManager) -> SuiResult<()> {
        ...
        if self.randomness_manager.set(tokio::sync::Mutex::new(randomness_manager)).is_err() {
            debug_fatal!("BUG: `set_randomness_manager` called more than once; this should never happen");
        }
    ...
    pub fn accumulator_root_exists(&self) -> bool {
        self.epoch_start_configuration.accumulator_root_obj_initial_shared_version().is_some()
    }
    ...
    pub fn coin_registry_exists(&self) -> bool {
        self.epoch_start_configuration.coin_registry_obj_initial_shared_version().is_some()
    }
    ...
    pub fn get_highest_running_root_state_hash(&self) -> SuiResult<Option<(CheckpointSequenceNumber, GlobalStateHash)>> {
        Ok(self.tables()?.running_root_state_hash.reversed_safe_iter_with_bounds(None, None)?.next().transpose()?)
    }
    ...
    pub fn clear_state_hashes_after_checkpoint(&self, last_committed_checkpoint: CheckpointSequenceNumber) -> SuiResult {
        let tables = self.tables()?;

        let mut keys_to_remove = Vec::new();
        for kv in tables.running_root_state_hash.safe_iter_with_bounds(Some(last_committed_checkpoint + 1), None) {
        ...
        for kv in tables.state_hash_by_checkpoint.safe_iter_with_bounds(Some(last_committed_checkpoint + 1), None) {
            let (checkpoint_seq, _) = kv?;
            if checkpoint_seq > last_committed_checkpoint {
        ...
        if !keys_to_remove.is_empty() || !checkpoint_keys_to_remove.is_empty() {
            ...
            if !keys_to_remove.is_empty() {
                batch.delete_batch(&tables.running_root_state_hash, keys_to_remove.clone()).expect("db error");
            }
            if !checkpoint_keys_to_remove.is_empty() {
                batch
                    .delete_batch(&tables.state_hash_by_checkpoint, checkpoint_keys_to_remove.clone())
                    .expect("db error");
            }
    ...
    pub fn set_local_execution_time_channels(
        ...
        if let Err(e) = self.tx_local_execution_time.set(tx_local_execution_time) {
            debug_fatal!("failed to set tx_local_execution_time channel on AuthorityPerEpochStore: {e:?}");
        }
    ...
    pub fn record_local_execution_time(
        ...
        if !ptb.inputs.iter().any(|input| matches!(input, CallArg::Object(ObjectArg::SharedObject { .. }))) {
            return;
        }

        if let Err(e) = tx_local_execution_time.try_send((ptb.clone(), timings, total_duration, tx.gas_data().price)) {
    ...
    pub fn get_stored_execution_time_observations(
    ...
    ) -> impl Iterator<Item = (AuthorityIndex, Option<u64>, ExecutionTimeObservationKey, Duration)> {
        if !matches!(
            protocol_config.per_object_congestion_control_mode(),
        ...
        let system_state = sui_system_state::get_sui_system_state(object_store).expect("System state must exist");
        let system_state = match system_state {
            ...
            #[cfg(msim)]
            SuiSystemState::SimTestV1(_) | SuiSystemState::SimTestShallowV2(_) | SuiSystemState::SimTestDeepV2(_) => {
                return itertools::Either::Left(std::iter::empty());
            }
        };
        ...
        let stored_observations: StoredExecutionTimeObservations =
            bcs::from_bytes(&stored_observations_bytes).expect("failed to deserialize stored execution time estimates");
        let stored_observations = stored_observations.unwrap_v1();

        info!("loaded stored execution time observations for {} keys", stored_observations.len());
        metrics.epoch_execution_time_observations_loaded.set(stored_observations.len() as i64);
        ...
        itertools::Either::Right(stored_observations.into_iter().flat_map(move |(key, observations)| {
            let committee = committee.clone();
            observations.into_iter().filter_map(move |(authority, duration)| {
                committee.authority_index(&authority).map(|authority_index| {
                    (
                        authority_index,
    ...
    pub fn get_end_of_epoch_execution_time_observations(&self) -> &StoredExecutionTimeObservations {
        self.end_of_epoch_execution_time_observations
            .get()
            .expect("`get_end_of_epoch_execution_time_observations` must not be called until end of epoch")
    }
    ...
    pub(crate) fn insert_tx_key(&self, tx_key: TransactionKey, tx_digest: TransactionDigest) -> SuiResult {
        let _metrics_scope = mysten_metrics::monitored_scope("AuthorityPerEpochStore::insert_tx_key");
    ...
    pub fn transactions_executed_in_cur_epoch(&self, digests: &[TransactionDigest]) -> SuiResult<Vec<bool>> {
        let tables = self.tables()?;
        Ok(do_fallback_lookup(
            ...
            |digests| tables.executed_transactions_to_checkpoint.multi_contains_keys(digests).expect("db error"),
        ))
    }
    ...
    pub(crate) fn get_input_object_keys(
        ...
        let assigned_shared_versions = assigned_versions.iter().cloned().collect::<BTreeMap<_, _>>();
        objects
            .iter()
            .map(|kind| {
                match kind {
                    ...
                    InputObjectKind::ImmOrOwnedMoveObject(objref) => {
                        InputKey::VersionedObject { id: FullObjectID::new(objref.0, None), version: objref.1 }
                    }
                }
            })
            .collect()
    ...
    pub fn get_last_consensus_stats(&self) -> SuiResult<ExecutionIndicesWithStats> {
        ...
        match self.tables()?.get_last_consensus_stats()? {
            Some(stats) => Ok(stats),
            None => {
                let indices = self.tables()?.get_last_consensus_index().map(|x| x.unwrap_or_default())?;
    ...
    pub async fn notify_read_checkpoint_state_hasher(
        ...
        Ok(self
            ...
            .read("notify_read_checkpoint_state_hasher", checkpoints, |checkpoints| {
                tables.state_hash_by_checkpoint.multi_get(checkpoints).expect("db error")
            })
            .await)
    }

    pub async fn notify_read_running_root(&self, checkpoint: CheckpointSequenceNumber) -> SuiResult<GlobalStateHash> {
    ...
    #[cfg(test)]
    pub fn get_next_object_version(&self, obj: &ObjectID, start_version: SequenceNumber) -> Option<SequenceNumber> {
        self.tables()
            .expect("test should not cross epoch boundary")
    ...
    pub fn insert_finalized_transactions(
        ...
        let _metrics_scope = mysten_metrics::monitored_scope("AuthorityPerEpochStore::insert_finalized_transactions");
    ...
    pub(crate) fn get_or_init_next_object_versions(
        ...
        let _locks = self.version_assignment_mutex_table.acquire_locks(objects_to_init.iter().map(|(id, _)| *id));
        let tables = self.tables()?;

        let next_versions =
            self.consensus_quarantine.read().get_next_shared_object_versions(&tables, objects_to_init)?;
        ...
        if uninitialized_objects.is_empty() {
            ...
            return Ok(izip!(objects_to_init.iter().cloned(), next_versions.into_iter().map(|v| v.unwrap())).collect());
        }
        ...
        tables.next_shared_object_versions_v2.multi_insert(versions_to_write)?;
    ...
    pub fn assign_shared_object_versions_idempotent<'a>(
        ...
        Ok(SharedObjVerManager::assign_versions_from_consensus(self, cache_reader, assignables, &BTreeMap::new())?
            .assigned_versions)
    }
    ...
    fn load_deferred_transactions(
        ...
        let (keys, txns) = {
            ...
            for (key, transactions) in deferred_transactions.range(min .. max) {
                debug!("Loaded {:?} deferred txn with deferral key {:?}", transactions.len(), key);
                keys.push(*key);
    ...
    pub fn get_all_deferred_transactions_for_test(
        ...
        self.consensus_output_cache.deferred_transactions.lock().iter().map(|(key, txs)| (*key, txs.clone())).collect()
    }
    ...
    fn should_defer(
        ...
        if let Some((deferral_key, congested_objects)) = shared_object_congestion_tracker
            .should_defer_due_to_object_congestion(tx_cost, cert, previously_deferred_tx_digests, commit_info)
        {
    ...
    pub fn acquire_shared_version_assignments_from_effects(
        ...
        let assigned_versions =
            SharedObjVerManager::assign_versions_from_effects(&[(certificate, effects)], self, cache_reader);
    ...
    pub fn deferred_transactions_empty(&self) -> bool {
        self.consensus_output_cache.deferred_transactions.lock().is_empty()
    }
    ...
    // non-digest keys.
    pub async fn notify_read_tx_key_to_digest(&self, keys: &[TransactionKey]) -> SuiResult<Vec<TransactionDigest>> {
    ...
    pub fn user_signatures_for_checkpoint(
        ...
        fn is_signature_expected(transaction: &VerifiedTransaction) -> bool {
            !matches!(
                ...
                TransactionKind::RandomnessStateUpdate(_) | TransactionKind::ProgrammableSystemTransaction(_)
            )
        }
    ...
    pub(crate) fn get_randomness_last_round_timestamp(&self) -> SuiResult<Option<TimestampMs>> {
        if let Some(ts) = self.consensus_quarantine.read().get_randomness_last_round_timestamp() {
            Ok(Some(ts))
        } else {
            Ok(self.tables()?.randomness_last_round_timestamp.get(&SINGLETON_KEY)?)
        }
    }

    #[cfg(test)]
    pub fn test_insert_user_signature(&self, digest: TransactionDigest, signatures: Vec<GenericSignature>) {
        self.consensus_output_cache.user_signatures_for_checkpoints.lock().insert(digest, signatures);
    ...
    pub(crate) async fn process_consensus_transactions_and_commit_boundary<C: CheckpointServiceNotify>(
        &self,
        ...
        let mut execution_time_estimator =
            self.execution_time_estimator.try_lock().expect("should only ever be called from the commit handler thread");
        // It is ok to just release lock here as functions called by this one are the
        // only place that transition reconfig state, and this function itself is always
        // executed from consensus task.
        if !self.protocol_config().ignore_execution_time_observations_after_certs_closed()
            || self.get_reconfig_state_read_lock_guard().should_accept_consensus_certs()
        {
            for (key, ExecutionTimeObservation { authority, generation, estimates }) in execution_time_observations {
        ...
        let sequenced_non_randomness_transactions: Vec<_> =
            system_transactions.into_iter().chain(sequenced_transactions).collect();
        ...
        if self.accumulators_enabled() {
            ...
            let checkpoint_height = self.calculate_pending_checkpoint_height(consensus_commit_info.round);

            let non_random_settlement = Schedulable::AccumulatorSettlement(self.epoch(), checkpoint_height);
            ...
            if randomness_round.is_some() {
                let randomness_settlement = Schedulable::AccumulatorSettlement(self.epoch(), checkpoint_height + 1);
        ...
        let (lock, final_round) =
            self.process_end_of_publish_transactions_and_reconfig(&mut output, &end_of_publish_transactions)?;

        self.process_user_signatures(
            verified_non_randomness_transactions.iter().chain(verified_randomness_transactions.iter()),
        );
        ...
        self.consensus_quarantine.write().push_consensus_output(output, self)?;
        ...
        for txn in transactions.iter() {
            ...
            match key.as_digest().and_then(|d| cancelled_txns.get(d)) {
                ...
                | Some(CancelConsensusCertificateReason::DkgFailed) => {
                    ...
                    version_assignment.push((*key.unwrap_digest(), assigned_versions.shared_object_versions));
                }
                None => {}
            }
        }
    ...
    fn process_consensus_transaction_shared_object_versions<'a>(
        ...
        let ConsensusSharedObjVerAssignment { shared_input_next_versions, assigned_versions } =
            SharedObjVerManager::assign_versions_from_consensus(self, cache_reader, all_certs, cancelled_txns)?;
    ...
    pub fn assign_shared_object_versions_for_tests(
        ...
        let transactions: Vec<_> = transactions.iter().cloned().map(Schedulable::Transaction).collect();
        let assigned_versions = self.process_consensus_transaction_shared_object_versions(
            cache_reader,
            transactions.iter(),
            std::iter::empty(),
    ...
    async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        let mut notifications = Vec::with_capacity(non_randomness_transactions.len() + randomness_transactions.len());
        ...
        let mut verified_non_randomness_certificates = VecDeque::with_capacity(non_randomness_transactions.len() + 1);
        let mut verified_randomness_certificates = VecDeque::with_capacity(randomness_transactions.len() + 1);

        if let Some(round) = randomness_round {
            verified_randomness_certificates.push_back(Schedulable::RandomnessStateUpdate(self.epoch(), round));
        }

        for entry in
            non_randomness_transactions.iter().map(Either::Left).chain(randomness_transactions.iter().map(Either::Right))
        {
            let (tx, execution_cost, verified_certificates) = match entry {
                Either::Left(tx) => {
                    (tx, &mut shared_object_congestion_tracker, &mut verified_non_randomness_certificates)
                }
                Either::Right(tx) => {
                    (tx, &mut shared_object_using_randomness_congestion_tracker, &mut verified_randomness_certificates)
                }
            };
        ...
        let object_debts = shared_object_congestion_tracker.accumulated_debts(consensus_commit_info);
        let randomness_object_debts =
            shared_object_using_randomness_congestion_tracker.accumulated_debts(consensus_commit_info);
        ...
        Ok((verified_non_randomness_certificates, verified_randomness_certificates, notifications, cancelled_txns))
    }
    ...
    pub(crate) fn should_send_end_of_publish(&self) -> bool {
        ...
        self.pending_consensus_certificates_empty()
            && self.consensus_tx_status_cache.as_ref().is_none_or(|c| c.get_num_fastpath_certified() == 0)
    }
    ...
    pub fn last_built_checkpoint_summary(&self) -> SuiResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
        if let Some(BuilderCheckpointSummary { summary, .. }) = self.consensus_quarantine.read().last_built_summary() {
    ...
    pub(crate) fn get_lowest_non_genesis_checkpoint_summary(&self) -> SuiResult<Option<CheckpointSummary>> {
        for result in self.tables()?.builder_checkpoint_summary_v2.safe_iter_with_bounds(None, None) {
            let (seq, bcs) = result?;
            if seq > 0 {
    ...
    pub fn builder_included_transactions_in_checkpoint<'a>(
        ...
        Ok(do_fallback_lookup(
            ...
            |remaining| tables.builder_digest_to_checkpoint.multi_contains_keys(remaining).expect("db error"),
        ))
    }
    ...
    pub(crate) fn set_consensus_tx_status(&self, position: ConsensusPosition, status: ConsensusTxStatus) {
    ...
    pub(crate) fn get_rejection_vote_reason(&self, position: ConsensusPosition) -> Option<SuiError> {
        if let Some(tx_reject_reason_cache) = self.tx_reject_reason_cache.as_ref() {
    ...
    pub async fn get_consensus_tx_cost_estimates(&self) -> Vec<(ExecutionTimeObservationKey, ConsensusObservations)> {
        self.execution_time_estimator
            .lock()
            .await
            .as_ref()
