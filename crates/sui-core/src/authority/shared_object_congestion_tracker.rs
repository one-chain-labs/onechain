...
use crate::{
    ...
    consensus_handler::{ConsensusCommitInfo, IndirectStateObserver},
};
    ...
    pub fn commit_budget(&self, commit_info: &ConsensusCommitInfo) -> u64 {
        match self.mode {
            PerObjectCongestionControlMode::ExecutionTimeEstimate(params) => {
                ...
                let mut budget = commit_period_micros.saturating_mul(params.target_utilization) / 100;
...
impl SharedObjectCongestionTracker {
    ...
    pub fn bump_object_execution_cost(&mut self, tx_cost: Option<u64>, cert: &VerifiedExecutableTransaction) {
        let shared_input_objects: Vec<_> = cert.shared_input_objects().collect();
        if shared_input_objects.is_empty() {
            return;
        }
...
mod object_cost_tests {
    ...
    use sui_types::{
        ...
        transaction::{CallArg, ObjectArg, SharedObjectMutability, VerifiedTransaction},
        Identifier,
    };
    ...
    fn construct_shared_input_objects(objects: &[(ObjectID, bool)]) -> Vec<SharedInputObject> {
        objects
            ...
            .map(|(id, mutable)| SharedInputObject {
                ...
                mutability: if *mutable { SharedObjectMutability::Mutable } else { SharedObjectMutability::Immutable },
            })
            .collect()
    }
    ...
    fn build_programmable_transaction(
        ...
        let mut tx_builder = TestTransactionBuilder::new(sender, gas_object, 1000).with_gas_budget(gas_budget);
        {
