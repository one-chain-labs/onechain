...
use super::{authority_per_epoch_store::AuthorityPerEpochStore, weighted_moving_average::WeightedMovingAverage};
...
impl LocalObservations {
    fn new(config: ExecutionTimeObserverConfig, default_duration: Duration) -> Self {
        ...
        Self {
            ...
            weighted_moving_average: WeightedMovingAverage::new(default_duration.as_micros() as u64, window_size),
            last_shared: None,
            config,
        }
    }
    ...
    fn add_sample(&mut self, duration: Duration, gas_price: u64) {
        ...
        self.weighted_moving_average.add_sample(duration.as_micros() as u64, gas_price);
    }
...
impl ExecutionTimeObserver {
    ...
    fn record_local_observations(
        ...
        if should_inject {
            ...
            self.record_local_observations_timing(tx, &generated_timings, generated_duration, gas_price)
        } else {
    ...
    fn record_local_observations_timing(
        ...
        for (i, timing) in timings.iter().enumerate() {
            ...
            let local_observation = self
                .local_observations
                .get_or_insert_mut(key.clone(), || LocalObservations::new(self.config.clone(), Duration::ZERO));
    ...
    fn generate_test_timings(
        ...
        let total_duration = generated_timings.iter().map(|t| t.duration()).sum::<Duration>()
    ...
    fn get_test_duration(&self, key: &ExecutionTimeObservationKey) -> Duration {
        ...
        let checkpoint_digest_used = self
            .epoch_store
            .upgrade()
            .and_then(|store| store.get_lowest_non_genesis_checkpoint_summary().ok().flatten())
...
impl ConsensusObservations {
    fn update_stake_weighted_median(&mut self, committee: &Committee, config: &ExecutionTimeEstimateParams) {
...
impl ExecutionTimeEstimator {
    pub fn new(
        committee: Arc<Committee>,
        protocol_params: ExecutionTimeEstimateParams,
        initial_observations: impl Iterator<Item = (AuthorityIndex, Option<u64>, ExecutionTimeObservationKey, Duration)>,
    ) -> Self {
        ...
        for observation in estimator.consensus_observations.values_mut() {
            observation.update_stake_weighted_median(&estimator.committee, &estimator.protocol_params);
        }
        estimator
    }
    ...
    pub fn process_observations_from_consensus(
        ...
        for (key, duration) in observations {
            self.process_observation_from_consensus(source, generation, key.to_owned(), *duration, false);
        }
    }
    ...
    fn process_observation_from_consensus(
        ...
        let observations = self.consensus_observations.entry(observation_key).or_insert_with(|| {
            ...
            ConsensusObservations {
                ...
                stake_weighted_median: if self.protocol_params.default_none_duration_for_new_keys {
                    None
                } else {
                    Some(Duration::ZERO)
                },
        ...
        let (obs_generation, obs_duration) = &mut observations.observations[TryInto::<usize>::try_into(source).unwrap()];
                ...
                .map(|(key, observations)| {
                    let observations = observations
                        ...
                        .filter_map(|(idx, (_, duration))| {
                            duration.map(|d| {
                                (self.committee.authority_by_index(idx.try_into().unwrap()).cloned().unwrap(), d)
                            })
                        })
                        .collect();
                    (key, observations)
...
mod tests {
    ...
    use sui_types::{
        ...
        transaction::{Argument, CallArg, ObjectArg, ProgrammableMoveCall, SharedObjectMutability},
    };
    ...
    async fn test_record_local_observations() {
        ...
        observer.local_observations.get_mut(&key).unwrap().last_shared =
            Some((Duration::from_millis(110), Instant::now() - Duration::from_secs(60)));
    ...
    async fn test_record_local_observations_with_multiple_commands() {
        ...
        let transfer_obs = observer.local_observations.get(&ExecutionTimeObservationKey::TransferObjects).unwrap();
        assert_eq!(
    ...
    async fn test_stake_weighted_median() {
        ...
        let params = ExecutionTimeEstimateParams { stake_weighted_median_threshold: 0, ..Default::default() };
    ...
    async fn test_stake_weighted_median_threshold() {
        ...
        let params = ExecutionTimeEstimateParams { stake_weighted_median_threshold: 5000, ..Default::default() };
    ...
    async fn test_execution_time_estimator() {
        ...
        estimator.process_observation_from_consensus(0, Some(1), move_key.clone(), Duration::from_millis(1000), false);
        estimator.process_observation_from_consensus(1, Some(1), move_key.clone(), Duration::from_millis(1000), false);
        estimator.process_observation_from_consensus(2, Some(1), move_key.clone(), Duration::from_millis(1000), false);
        ...
        estimator.process_observation_from_consensus(0, Some(2), move_key.clone(), Duration::from_millis(100), false);
        estimator.process_observation_from_consensus(1, Some(2), move_key.clone(), Duration::from_millis(200), false);
        estimator.process_observation_from_consensus(2, Some(2), move_key.clone(), Duration::from_millis(300), false);

        estimator.process_observation_from_consensus(0, Some(2), transfer_key.clone(), Duration::from_millis(50), false);
        estimator.process_observation_from_consensus(1, Some(2), transfer_key.clone(), Duration::from_millis(60), false);
        estimator.process_observation_from_consensus(2, Some(2), transfer_key.clone(), Duration::from_millis(70), false);
        ...
        assert_eq!(estimator.get_estimate(&single_move_tx), Duration::from_millis(300));
        ...
        let multi_command_tx = TransactionData::new_programmable(
            ...
            ProgrammableTransaction {
                ...
                commands: vec![
                    ...
                    Command::TransferObjects(vec![Argument::Input(1), Argument::Input(2)], Argument::Input(0)),
                ],
            },
            100,
            100,
        ...
        assert_eq!(estimator.get_estimate(&multi_command_tx), Duration::from_millis(510));
    }
    ...
    fn generate_test_inputs(
    ...
    ) -> Vec<(AuthorityIndex, Option<u64>, ExecutionTimeObservationKey, Duration)> {
        ...
        for _ in 0 .. target_samples {
            ...
            let authority_index = AuthorityIndex::try_from(rng.gen_range(0 .. num_validators)).unwrap();
    ...
    fn generate_test_transactions(seed: u64) -> Vec<(String, TransactionData)> {
        ...
        transactions.push((
            ...
            TransactionData::new_programmable(
                ...
                ProgrammableTransaction {
                    inputs: vec![],
                    commands: vec![Command::MakeMoveVec(None, (0 .. num_elements).map(Argument::Input).collect())],
                },
                rng.gen_range(100 .. 1000),
                rng.gen_range(100 .. 1000),
            ),
        ...
        transactions.push((
            ...
            TransactionData::new_programmable(
                ...
                ProgrammableTransaction {
                    inputs: vec![],
                    commands: vec![Command::Upgrade(vec![], vec![], package3, Argument::Input(0))],
                },
                rng.gen_range(100 .. 1000),
                rng.gen_range(100 .. 1000),
            ),
    ...
    fn snapshot_tests() {
        ...
        for version in test_versions {
            ...
            let initial_generation = if let PerObjectCongestionControlMode::ExecutionTimeEstimate(params) =
            ...
            let initial_observations = generate_test_inputs(0, committee.num_members(), initial_generation);
            let mut estimator = ExecutionTimeEstimator::new(
                committee.clone(),
                ExecutionTimeEstimateParams {
            ...
            for (source, generation, observation_key, duration) in test_inputs {
                estimator.process_observation_from_consensus(source, generation, observation_key, duration, false);
            }
            ...
            insta::assert_yaml_snapshot!(format!("execution_time_observer_v{}", version), snapshot_data);
        }
    }
}
