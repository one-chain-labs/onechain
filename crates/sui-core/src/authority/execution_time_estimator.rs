...
impl ExecutionTimeObserver {
    ...
    fn record_local_observations(
        ...
        for (i, timing) in timings.iter().enumerate() {
            ...
            let local_observation = self.local_observations.get_or_insert_mut(key.clone(), || LocalObservations {
...
impl ExecutionTimeEstimator {
    ...
    fn process_observation_from_consensus(
        ...
        let observations = self.consensus_observations.entry(observation_key).or_insert_with(|| {
            ...
            ConsensusObservations { observations: empty_observations, stake_weighted_median: Duration::ZERO }
        });
...
mod tests {
    ...
    async fn test_execution_time_estimator() {
        ...
        estimator.process_observation_from_consensus(0, 1, move_key.clone(), Duration::from_millis(1000), false);
        estimator.process_observation_from_consensus(1, 1, move_key.clone(), Duration::from_millis(1000), false);
        estimator.process_observation_from_consensus(2, 1, move_key.clone(), Duration::from_millis(1000), false);

        estimator.process_observation_from_consensus(0, 1, transfer_key.clone(), Duration::from_millis(500), false);
        estimator.process_observation_from_consensus(1, 1, transfer_key.clone(), Duration::from_millis(500), false);
        estimator.process_observation_from_consensus(2, 1, transfer_key.clone(), Duration::from_millis(500), false);

        // Now record newer observations that should be used
        estimator.process_observation_from_consensus(0, 2, move_key.clone(), Duration::from_millis(100), false);
        estimator.process_observation_from_consensus(1, 2, move_key.clone(), Duration::from_millis(200), false);
        estimator.process_observation_from_consensus(2, 2, move_key.clone(), Duration::from_millis(300), false);

        estimator.process_observation_from_consensus(0, 2, transfer_key.clone(), Duration::from_millis(50), false);
        estimator.process_observation_from_consensus(1, 2, transfer_key.clone(), Duration::from_millis(60), false);
        estimator.process_observation_from_consensus(2, 2, transfer_key.clone(), Duration::from_millis(70), false);
