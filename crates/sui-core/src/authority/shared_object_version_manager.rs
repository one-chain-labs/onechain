...
use sui_types::{
    ...
    transaction::{SharedInputObject, TransactionDataAPI, TransactionKey},
...
impl AssignedVersions {
    pub fn new(
        ...
        Self { shared_object_versions, withdraw_type }
    }

    pub fn non_withdraw(shared_object_versions: Vec<(ConsensusObjectSequenceKey, SequenceNumber)>) -> Self {
...
impl SharedObjVerManager {
    ...
    pub fn assign_versions_from_effects(
        ...
        let _ = get_or_init_versions(
            certs_and_effects.iter().flat_map(|(cert, _)| cert.transaction_data().shared_input_objects().into_iter()),
            epoch_store,
            cache_reader,
        );
        let mut assigned_versions = Vec::new();
    ...
    pub fn assign_versions_for_certificate(
        ...
        let withdraw_type = assignable
            .as_tx()
            .and_then(|tx| {
                if tx.transaction_data().has_funds_withdrawals() {
                    let accumulator_initial_version =
                        epoch_store.epoch_start_config().accumulator_root_obj_initial_shared_version().expect(
                            "accumulator root obj initial shared version should be set when accumulators are enabled",
                        );
...
mod tests {
    ...
    async fn test_assign_versions_from_consensus_basic() {
        ...
        let authority = TestAuthorityBuilder::new().with_starting_objects(&[shared_object.clone()]).build().await;
        let certs = vec![
        ...
        assert_eq!(assigned_versions.0, vec![
            (certs[0].key(), AssignedVersions::non_withdraw(vec![((id, init_shared_version), init_shared_version)])),
            (
                certs[1].key(),
                AssignedVersions::non_withdraw(vec![((id, init_shared_version), SequenceNumber::from_u64(4))])
            ),
            (
                certs[2].key(),
                AssignedVersions::non_withdraw(vec![((id, init_shared_version), SequenceNumber::from_u64(4))])
            ...
            (
                certs[3].key(),
                AssignedVersions::non_withdraw(vec![((id, init_shared_version), SequenceNumber::from_u64(10))])
            ),
        ]);
    }
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        assert_eq!(assigned_versions.0, vec![
            ...
            (
                certs[2].key(),
                AssignedVersions::non_withdraw(vec![((id1, init_shared_version_1), SequenceNumber::from_u64(4))])
            ),
            (
                certs[3].key(),
                AssignedVersions::non_withdraw(vec![
    ...
    impl WithdrawTestContext {
        ...
        pub fn add_withdraw_with_shared_object_transaction(&mut self) -> TransactionKey {
            ...
            if let Some(shared_obj) = self.shared_objects.first() {
                ...
                ptb_builder
                    .obj(ObjectArg::SharedObject { id, initial_shared_version: init_version, mutable: true })
                    .unwrap();
            }
