...
impl SharedObjVerManager {
    pub fn assign_versions_from_consensus<'a>(
        ...
        let mut shared_input_next_versions =
            get_or_init_versions(certificates.clone().map(|cert| cert.data()), epoch_store, cache_reader)?;
        ...
        for cert in certificates {
            ...
            let cert_assigned_versions =
                Self::assign_versions_for_certificate(cert, &mut shared_input_next_versions, cancelled_txns);
    ...
    pub fn assign_versions_from_effects(
        ...
        let _ = get_or_init_versions(certs_and_effects.iter().map(|(cert, _)| cert.data()), epoch_store, cache_reader);
        let mut assigned_versions = Vec::new();
    ...
    pub fn assign_versions_for_certificate(
        ...
        let mut input_object_keys =
            transaction_non_shared_input_object_keys(cert).expect("Transaction input should have been verified");
        ...
        trace!(?tx_digest, ?assigned_versions, ?next_version, ?txn_cancelled, "locking shared objects");
...
mod tests {
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(assigned_versions, vec![
            (certs[0].key(), vec![((SUI_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version), randomness_obj_version),]),
            (
                certs[1].key(),
