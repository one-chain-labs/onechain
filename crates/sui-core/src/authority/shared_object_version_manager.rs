...
use sui_types::{
    ...
    storage::{transaction_non_shared_input_object_keys, transaction_receiving_object_keys, ObjectKey},
    transaction::{SharedInputObject, SharedObjectMutability, TransactionDataAPI, TransactionKey},
...
use crate::{
    authority::{authority_per_epoch_store::CancelConsensusCertificateReason, AuthorityPerEpochStore},
...
impl AssignedVersions {
    pub fn new(
        ...
        Self { shared_object_versions, accumulator_version }
    }
...
impl Schedulable<&'_ VerifiedExecutableTransaction> {
    ...
    pub fn to_owned_schedulable(&self) -> Schedulable<VerifiedExecutableTransaction> {
        match self {
            ...
            Schedulable::RandomnessStateUpdate(epoch, round) => Schedulable::RandomnessStateUpdate(*epoch, *round),
...
impl<T> Schedulable<T> {
        ...
        T: AsTx,
    {
        match self {
            ...
            Schedulable::RandomnessStateUpdate(_, _) => Either::Right(std::iter::once(SharedInputObject {
            ...
            Schedulable::AccumulatorSettlement(_, _) => Either::Right(std::iter::once(SharedInputObject {
            ...
            Schedulable::ConsensusCommitPrologue(_, _, _) => Either::Right(std::iter::once(SharedInputObject {
        ...
        T: AsTx,
    {
        match self {
            ...
            Schedulable::RandomnessStateUpdate(epoch, round) => TransactionKey::RandomnessRound(*epoch, *round),
            Schedulable::AccumulatorSettlement(epoch, checkpoint_height) => {
...
impl SharedObjVerManager {
        ...
        T: AsTx + 'a,
    {
        ...
        for assignable in assignables {
            assert!(
                !matches!(assignable, Schedulable::AccumulatorSettlement(_, _)) || epoch_store.accumulators_enabled(),
    ...
    pub fn assign_versions_from_effects(
        ...
        let _ = get_or_init_versions(
            certs_and_effects.iter().flat_map(|(cert, _, _)| cert.transaction_data().shared_input_objects().into_iter()),
            epoch_store,
            cache_reader,
        );
        let mut assigned_versions = Vec::new();
        for (cert, effects, accumulator_version) in certs_and_effects {
            ...
            let cert_assigned_versions: Vec<_> = effects
                ...
                .collect();
            let tx_key = cert.key();
            trace!(?tx_key, ?cert_assigned_versions, "assigned consensus object versions from effects");
            assigned_versions.push((tx_key, AssignedVersions::new(cert_assigned_versions, *accumulator_version)));
        }
    ...
    pub fn assign_versions_for_certificate(
        ...
        let cancellation_info = tx_key.as_digest().and_then(|tx_digest| cancelled_txns.get(tx_digest));
        let congested_objects_info: Option<HashSet<_>> =
            if let Some(CancelConsensusCertificateReason::CongestionOnObjects(congested_objects)) = &cancellation_info {
        ...
        if txn_cancelled {
            ...
            for (SharedInputObject { id, initial_shared_version, mutability }, assigned_version) in shared_input_objects
                .iter()
        ...
        if !txn_cancelled {
            ...
            assigned_versions
                ...
                .filter_map(|((id, _), mutable)| if mutable { Some((*id, next_version)) } else { None })
                .for_each(|(id, version)| {
        ...
        trace!(?tx_key, ?assigned_versions, ?next_version, ?txn_cancelled, "locking shared objects");
...
fn get_or_init_versions<'a>(
    ...
    let mut shared_input_objects: Vec<_> = shared_input_objects.map(|so| so.into_id_and_version()).collect();
...
mod tests {
    ...
    use sui_types::{
        ...
        crypto::{get_account_key_pair, RandomnessRound},
        digests::ObjectDigest,
        effects::TestEffectsBuilder,
        executable_transaction::{CertificateProof, ExecutableTransaction, VerifiedExecutableTransaction},
        gas_coin::GAS,
        object::Object,
        transaction::{FundsWithdrawalArg, ObjectArg, SenderSignedData, VerifiedTransaction},
    ...
    async fn test_assign_versions_from_consensus_basic() {
        ...
        let authority =
            TestAuthorityBuilder::new().with_starting_objects(std::slice::from_ref(&shared_object)).build().await;
        let certs = vec![
        ...
        let assignables = certs.iter().map(Schedulable::Transaction).collect::<Vec<_>>();
        ...
        assert_eq!(assigned_versions.0, vec![
            (certs[0].key(), AssignedVersions::new(vec![((id, init_shared_version), init_shared_version)], None)),
            (
                certs[1].key(),
                AssignedVersions::new(vec![((id, init_shared_version), SequenceNumber::from_u64(4))], None)
            ),
            (
                certs[2].key(),
                AssignedVersions::new(vec![((id, init_shared_version), SequenceNumber::from_u64(4))], None)
            ...
            (
                certs[3].key(),
                AssignedVersions::new(vec![((id, init_shared_version), SequenceNumber::from_u64(10))], None)
            ),
        ]);
    }
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        let epoch_store = authority.epoch_store_for_testing();
        let randomness_obj_version = epoch_store.epoch_start_config().randomness_obj_initial_shared_version().unwrap();
        let certs = vec![
            ...
            generate_shared_objs_tx_with_gas_version(
                &[(SUI_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version, false)],
                5,
            ),
        ];
        let assignables = certs.iter().map(Schedulable::Transaction).collect::<Vec<_>>();
        ...
        assert_eq!(assigned_versions.0, vec![
            (
                ...
                AssignedVersions::new(
                    vec![((SUI_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version), randomness_obj_version)],
                    None
                )
            ),
            (
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        let randomness_obj_version =
            authority.epoch_store_for_testing().epoch_start_config().randomness_obj_initial_shared_version().unwrap();
        ...
        let certs = vec![
            generate_shared_objs_tx_with_gas_version(
                &[(id1, init_shared_version_1, true), (id2, init_shared_version_2, true)],
                3,
            ),
            generate_shared_objs_tx_with_gas_version(
                &[(id1, init_shared_version_1, true), (id2, init_shared_version_2, true)],
            ...
            generate_shared_objs_tx_with_gas_version(
                &[(SUI_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version, false), (id2, init_shared_version_2, true)],
                11,
            ),
        ];
        let epoch_store = authority.epoch_store_for_testing();
        ...
        let cancelled_txns: BTreeMap<TransactionDigest, CancelConsensusCertificateReason> = [
            (*certs[1].digest(), CancelConsensusCertificateReason::CongestionOnObjects(vec![id1])),
            (*certs[3].digest(), CancelConsensusCertificateReason::CongestionOnObjects(vec![id2])),
            (*certs[4].digest(), CancelConsensusCertificateReason::DkgFailed),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(assigned_versions.0, vec![
            ...
            (
                certs[2].key(),
                AssignedVersions::new(vec![((id1, init_shared_version_1), SequenceNumber::from_u64(4))], None)
            ),
            (
                certs[3].key(),
                AssignedVersions::new(
    ...
    async fn test_assign_versions_from_effects() {
        ...
        let assigned_versions = SharedObjVerManager::assign_versions_from_effects(
            certs.iter().zip(effects.iter()).map(|(cert, effect)| (cert, effect, None)).collect::<Vec<_>>().as_slice(),
            &epoch_store,
            authority.get_object_cache_reader().as_ref(),
        );
        // Check that the shared object's next version is always initialized in the epoch store.
    ...
    fn generate_shared_objs_tx_with_gas_version(
        ...
        let mut tx_builder = TestTransactionBuilder::new(
            SuiAddress::ZERO,
            (ObjectID::random(), SequenceNumber::from_u64(gas_object_version), ObjectDigest::random()),
            0,
        );
        let tx_data = {
    ...
    impl WithdrawTestContext {
        pub async fn new() -> Self {
            ...
            let authority = TestAuthorityBuilder::new()
                ...
                .build()
                .await;
            Self { authority, assignables: vec![], shared_objects }
        }
        ...
        pub fn add_withdraw_transaction(&mut self) -> TransactionKey {
            ...
            let tx_data = {
                ...
                ptb_builder
                    .funds_withdrawal(FundsWithdrawalArg::balance_from_sender(200, TypeInput::from(GAS::type_tag())))
                    .unwrap();
                tx_builder.build()
            };
    ...
    async fn test_assign_versions_from_consensus_with_withdraws_simple() {
        ...
        let acc_version = ctx.authority.get_object(&SUI_ACCUMULATOR_ROOT_OBJECT_ID).await.unwrap().version();
        ...
        assert_eq!(assigned_versions, ConsensusSharedObjVerAssignment {
            assigned_versions: AssignedTxAndVersions::new(vec![
                (withdraw_key, AssignedVersions {
                    shared_object_versions: vec![],
                    accumulator_version: Some(acc_version),
                }),
                (settlement_key, AssignedVersions {
                    shared_object_versions: vec![((SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version), acc_version)],
                    accumulator_version: Some(acc_version),
                }),
            ]),
    ...
    async fn test_assign_versions_from_consensus_with_multiple_withdraws_and_settlements() {
        ...
        assert_eq!(assigned_versions, ConsensusSharedObjVerAssignment {
            assigned_versions: AssignedTxAndVersions::new(vec![
                (withdraw_key1, AssignedVersions {
                    shared_object_versions: vec![],
                    accumulator_version: Some(acc_version),
                }),
                (settlement_key1, AssignedVersions {
                    shared_object_versions: vec![((SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version), acc_version)],
                    accumulator_version: Some(acc_version),
                }),
                (withdraw_key2, AssignedVersions {
                    shared_object_versions: vec![],
                    accumulator_version: Some(acc_version.next()),
                }),
                (settlement_key2, AssignedVersions {
                    shared_object_versions: vec![((SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version), acc_version.next())],
                    accumulator_version: Some(acc_version.next()),
                }),
                (withdraw_key3, AssignedVersions {
                    shared_object_versions: vec![],
                    accumulator_version: Some(acc_version.next().next()),
                }),
                (settlement_key3, AssignedVersions {
                    shared_object_versions: vec![(
                        (SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version),
    ...
    async fn test_assign_versions_from_consensus_with_withdraw_and_shared_object() {
        ...
        assert_eq!(assigned_versions, ConsensusSharedObjVerAssignment {
            assigned_versions: AssignedTxAndVersions::new(vec![
                (withdraw_with_shared_key, AssignedVersions {
                    shared_object_versions: vec![((shared_obj_id, shared_obj_version), shared_obj_version)],
                    accumulator_version: Some(acc_version),
                }),
                (settlement_key, AssignedVersions {
                    shared_object_versions: vec![((SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version), acc_version)],
            ...
            shared_input_next_versions: HashMap::from([
                ((SUI_ACCUMULATOR_ROOT_OBJECT_ID, acc_version), acc_version.next()),
                ((shared_obj_id, shared_obj_version), shared_obj_version.next()),
            ]),
        });
    }
}
