...
impl ConsensusTxStatusCache {
    ...
    pub fn set_transaction_status(&self, transaction_position: ConsensusTxPosition, status: ConsensusTxStatus) {
        debug!("Setting transaction status for {:?}: {:?}", transaction_position, status);
        let mut inner = self.inner.write();
        if let Some(last_committed_leader_round) = *self.last_committed_leader_round_rx.borrow() {
            if transaction_position.block.round as u64 + CONSENSUS_STATUS_RETENTION_ROUNDS < last_committed_leader_round
            {
                return;
            }
        }
        let old_status = inner.transaction_status.insert(transaction_position, status);
        ...
        if let Some(old_status) = old_status {
            if status == ConsensusTxStatus::FastpathCertified {
                ...
                inner.transaction_status.insert(transaction_position, old_status);
        ...
        self.status_notify_read.notify(&transaction_position, &status);
    }
    ...
    pub fn check_position_too_ahead(&self, position: &ConsensusTxPosition) -> SuiResult<()> {
        if let Some(last_committed_leader_round) = *self.last_committed_leader_round_rx.borrow() {
            if position.block.round as u64 > last_committed_leader_round + CONSENSUS_STATUS_RETENTION_ROUNDS {
    ...
    #[cfg(test)]
    pub fn get_transaction_status(&self, position: &ConsensusTxPosition) -> Option<ConsensusTxStatus> {
        let inner = self.inner.read();
...
mod tests {
    ...
    async fn test_status_notification() {
        ...
        let handle = tokio::spawn(async move { cache_clone.notify_read_transaction_status(tx_pos, None).await });
    ...
    async fn test_multiple_status_updates() {
        ...
        let result = cache.notify_read_transaction_status(tx_pos, Some(ConsensusTxStatus::FastpathCertified)).await;
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Status(ConsensusTxStatus::Finalized)));
    }
    ...
    async fn test_concurrent_operations() {
        ...
        for _ in 0 .. 3 {
            let cache_clone = cache.clone();
            handles.push(tokio::spawn(async move { cache_clone.notify_read_transaction_status(tx_pos, None).await }));
        }
    ...
    async fn test_out_of_order_status_updates() {
        ...
        let result = cache.notify_read_transaction_status(tx_pos, None).await;
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Status(ConsensusTxStatus::Rejected)));

        // We should not receive a new status update since the new status is older than the old status.
        cache.set_transaction_status(tx_pos, ConsensusTxStatus::FastpathCertified);
        let result = cache.notify_read_transaction_status(tx_pos, Some(ConsensusTxStatus::Rejected)).now_or_never();
        assert!(result.is_none());
        assert_eq!(cache.get_transaction_status(&tx_pos), Some(ConsensusTxStatus::Rejected));
    }
}
