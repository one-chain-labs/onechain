...
impl ConsensusTxStatusCache {
        ...
        match status_entry {
            ...
            Entry::Occupied(mut entry) => {
                ...
                match (old_status, status) {
                    ...
                    _ => {
                        panic!("Conflicting status updates for transaction {:?}: {:?} -> {:?}", pos, old_status, status);
                    }
                }
            }
        };
        ...
        let expiration_check = async {
            loop {
                if let Some(last_committed_leader_round) = *round_rx.borrow() {
                    if consensus_position.block.round + CONSENSUS_STATUS_RETENTION_ROUNDS <= last_committed_leader_round
                    {
                        return last_committed_leader_round;
                    }
                }
                // Channel closed - this should never happen in practice, so panic
                round_rx.changed().await.expect("last_committed_leader_round watch channel closed unexpectedly");
            }
        };
        tokio::select! {
    ...
    pub(crate) async fn update_last_committed_leader_round(&self, last_committed_leader_round: u32) {
        ...
        let Some(leader_round) = inner.last_committed_leader_round.replace(last_committed_leader_round) else {
        ...
        while let Some(position) = inner.fastpath_certified.first().cloned() {
            if position.block.round + self.consensus_gc_depth <= leader_round {
                ...
                self.set_transaction_status_inner(&mut inner, position, ConsensusTxStatus::Rejected);
            } else {
                break;
            }
        }
    ...
    pub(crate) fn check_position_too_ahead(&self, position: &ConsensusPosition) -> SuiResult<()> {
        if let Some(last_committed_leader_round) = *self.last_committed_leader_round_rx.borrow() {
            if position.block.round > last_committed_leader_round + CONSENSUS_STATUS_RETENTION_ROUNDS {
    ...
    #[cfg(test)]
    pub(crate) fn get_transaction_status(&self, position: &ConsensusPosition) -> Option<ConsensusTxStatus> {
        let inner = self.inner.read();
...
mod tests {
    ...
    fn create_test_tx_position(round: u64, index: u64) -> ConsensusPosition {
        ConsensusPosition {
            ...
            block: BlockRef { round: round as u32, author: Default::default(), digest: Default::default() },
    ...
    async fn test_set_and_get_transaction_status() {
        ...
        let result = cache.notify_read_transaction_status_change(tx_pos, None).await;
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Status(ConsensusTxStatus::FastpathCertified)));
    }
    ...
    async fn test_status_notification() {
        ...
        let handle = tokio::spawn(async move { cache_clone.notify_read_transaction_status_change(tx_pos, None).await });
        ...
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Status(ConsensusTxStatus::Finalized)));
    }
    ...
    async fn test_round_expiration() {
        ...
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + 1).await;
        ...
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + 2).await;

        // Try to read status - should be expired
        let result = cache.notify_read_transaction_status_change(tx_pos, None).await;
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Expired(_)));
    }
    ...
    async fn test_multiple_status_updates() {
        ...
        let result =
            cache.notify_read_transaction_status_change(tx_pos, Some(ConsensusTxStatus::FastpathCertified)).await;
        assert!(matches!(result, NotifyReadConsensusTxStatusResult::Status(ConsensusTxStatus::Finalized)));
    }
    ...
    async fn test_cleanup_expired_rounds() {
        ...
        // No rounds should be cleaned up yet since this was the initial update
        {
            ...
            let rounds = inner.transaction_status.keys().map(|p| p.block.round).collect::<Vec<_>>();
        ...
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + 3).await;
        ...
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + 4).await;
    ...
    async fn test_concurrent_operations() {
        ...
        for _ in 0 .. 3 {
            ...
            handles.push(tokio::spawn(
                async move { cache_clone.notify_read_transaction_status_change(tx_pos, None).await },
            ));
        }
    ...
    async fn test_out_of_order_status_updates() {
        ...
        let notify_read_task = tokio::spawn(async move {
            cache_clone.notify_read_transaction_status_change(tx_pos, Some(ConsensusTxStatus::Finalized)).await
        });
        ...
        assert_eq!(cache.get_transaction_status(&tx_pos), Some(ConsensusTxStatus::Finalized));
    }
    ...
    async fn test_fastpath_certified_tracking() {
        ...
        assert_eq!(cache.get_num_fastpath_certified(), 3);
        assert_eq!(cache.get_transaction_status(&tx_pos1), Some(ConsensusTxStatus::Finalized));

        // Transition another fastpath certified to rejected
        cache.set_transaction_status(tx_pos2, ConsensusTxStatus::Rejected);
        assert_eq!(cache.get_num_fastpath_certified(), 2);
        assert_eq!(cache.get_transaction_status(&tx_pos2), Some(ConsensusTxStatus::Rejected));
        ...
        assert_eq!(cache.get_num_fastpath_certified(), 2);
        assert_eq!(cache.get_transaction_status(&tx_pos3), Some(ConsensusTxStatus::FastpathCertified));
        assert_eq!(cache.get_transaction_status(&tx_pos4), Some(ConsensusTxStatus::FastpathCertified));
        ...
        assert_eq!(cache.get_num_fastpath_certified(), 1);
        assert_eq!(cache.get_transaction_status(&tx_pos3), Some(ConsensusTxStatus::Rejected));
        assert_eq!(cache.get_transaction_status(&tx_pos4), Some(ConsensusTxStatus::FastpathCertified));
        ...
        assert_eq!(cache.get_num_fastpath_certified(), 0);
        assert_eq!(cache.get_transaction_status(&tx_pos4), Some(ConsensusTxStatus::Rejected));
        ...
        assert_eq!(cache.get_num_fastpath_certified(), 0);
        assert_eq!(cache.get_transaction_status(&tx_pos6), Some(ConsensusTxStatus::Finalized));
    }
}
