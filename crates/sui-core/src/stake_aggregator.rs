...
impl<S: Clone + Eq, const STRENGTH: bool> StakeAggregator<S, STRENGTH> {
    ...
    pub fn insert_generic(&mut self, authority: AuthorityName, s: S) -> InsertResult<&HashMap<AuthorityName, S>> {
        ...
        if votes > 0 {
            ...
            InsertResult::Failed { error: SuiErrorKind::InvalidAuthenticator.into() }
        }
    }
...
impl<const STRENGTH: bool> StakeAggregator<AuthoritySignInfo, STRENGTH> {
    ...
    pub fn insert<T: Message + Serialize>(
        ...
        if self.committee.epoch != sig.epoch {
            return InsertResult::Failed {
                error: SuiErrorKind::WrongEpoch { expected_epoch: self.committee.epoch, actual_epoch: sig.epoch }.into(),
            };
        }
        match self.insert_generic(sig.authority, sig) {
            InsertResult::QuorumReached(_) => {
                match AuthorityQuorumSignInfo::<STRENGTH>::new_from_auth_sign_infos(
                    ...
                    Ok(aggregated) => {
                        match aggregated.verify_secure(&data, Intent::sui_app(T::SCOPE), self.committee()) {
                            ...
                            Err(_) => {
                                ...
                                for (name, sig) in &self.data.clone() {
                                    if let Err(err) =
                                        sig.verify_secure(&data, Intent::sui_app(T::SCOPE), self.committee())
                                    {
...
impl<K, V, const STRENGTH: bool> MultiStakeAggregator<K, V, STRENGTH> {
    ...
    pub fn total_votes(&self) -> StakeUnit {
        ...
        voted_authorities.iter().map(|k| self.committee.weight(k)).sum()
    }
}
    ...
    K: Hash + Eq,
{
    ...
    pub fn insert(&mut self, authority: AuthorityName, k: K) -> InsertResult<&HashMap<AuthorityName, ()>> {
        let agg = self.stake_maps.entry(k).or_insert_with(|| StakeAggregator::new(self.committee.clone()));
...
fn test_votes_per_authority() {
    ...
    let mut agg: GenericMultiStakeAggregator<&str, true> = GenericMultiStakeAggregator::new(Arc::new(committee));
...
mod multi_stake_aggregator_tests {
    ...
    fn test_equivocation_stake_not_double_counted() {
        ...
        let mut agg: MultiStakeAggregator<String, TestMessage, true> = MultiStakeAggregator::new(committee.clone());
        ...
        let msg1 = TestMessage { value: "value1".to_string() };
        let envelope1 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg1.clone(), key0, authority0);
        agg.insert("key1".to_string(), envelope1);

        // Second signature from same authority for "value2" (equivocation)
        let msg2 = TestMessage { value: "value2".to_string() };
        let envelope2 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg2.clone(), key0, authority0);
        agg.insert("key2".to_string(), envelope2);

        // Third signature from same authority for "value3" (more equivocation)
        let msg3 = TestMessage { value: "value3".to_string() };
        let envelope3 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg3.clone(), key0, authority0);
        ...
        let envelope4 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg1.clone(), key1, authority1);
    ...
    fn test_multistake_uncommitted_and_plurality() {
        ...
        let msg1 = TestMessage { value: "value1".to_string() };
        let envelope1 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg1.clone(), &key_pairs[0], authorities[0]);
        agg.insert("key1".to_string(), envelope1);
        ...
        let msg2 = TestMessage { value: "value2".to_string() };
        let envelope2 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg2.clone(), &key_pairs[1], authorities[1]);
        agg.insert("key2".to_string(), envelope2);

        assert_eq!(agg.uncommitted_stake(), total_stake - 2 * stake_per_authority);
        assert_eq!(agg.plurality_stake(), stake_per_authority);

        // Add third authority voting for value1 (now value1 has plurality)
        let envelope3 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg1.clone(), &key_pairs[2], authorities[2]);
        agg.insert("key1".to_string(), envelope3);

        assert_eq!(agg.uncommitted_stake(), total_stake - 3 * stake_per_authority);
    ...
    fn test_multistake_quorum_unreachable() {
        ...
        let envelope2 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg1.clone(), &key_pairs[1], authorities[1]);
        agg.insert("key1".to_string(), envelope2);

        // Two authorities vote for value2
        let envelope3 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg2.clone(), &key_pairs[2], authorities[2]);
        agg.insert("key2".to_string(), envelope3);

        let envelope4 = <Envelope<TestMessage, AuthoritySignInfo>>::new(0, msg2.clone(), &key_pairs[3], authorities[3]);
...
mod stake_aggregator_tests {
    ...
    fn test_stake_aggregator_repeated_signer() {
        ...
        let SuiErrorKind::StakeAggregatorRepeatedSigner { signer, conflicting_sig } = error.into_inner() else {
            panic!("Expected StakeAggregatorRepeatedSigner error");
        };
    ...
    fn test_stake_aggregator_from_iter() {
        ...
        let data = vec![Ok((authorities[0], ())), Ok((authorities[1], ())), Ok((authorities[2], ()))];

        let agg: StakeAggregator<(), true> = StakeAggregator::from_iter(committee.clone(), data.into_iter()).unwrap();
    ...
    fn test_stake_aggregator_from_iter_with_error() {
        ...
        let data: Vec<Result<(AuthorityName, ()), TypedStoreError>> =
            vec![Ok((authorities[0], ())), Err(TypedStoreError::RocksDBError("test error".to_string()))];
...
mod generic_multi_stake_aggregator_tests {
    ...
    fn test_has_quorum_for_key() {
        ...
        let mut agg: GenericMultiStakeAggregator<&str, true> = GenericMultiStakeAggregator::new(committee.clone());
