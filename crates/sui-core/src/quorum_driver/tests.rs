...
use sui_types::{
    ...
    quorum_driver_types::{ExecuteTransactionRequestV3, QuorumDriverError, QuorumDriverResponse, QuorumDriverResult},
...
use crate::{
    ...
    test_utils::{init_local_authorities, make_transfer_oct_transaction},
};

async fn setup() -> (AuthorityAggregator<LocalAuthorityClient>, Transaction) {
    let (sender, keypair): (_, AccountKeyPair) = get_key_pair();
    let gas_object = Object::with_owner_for_testing(sender);
    let (aggregator, authorities, genesis, _) = init_local_authorities(4, vec![gas_object.clone()]).await;
    let rgp = authorities.first().unwrap().reference_gas_price_for_testing().unwrap();
    let gas_object = genesis.objects().iter().find(|o| o.id() == gas_object.id()).unwrap();
...
async fn test_quorum_driver_submit_transaction() {
    ...
    let quorum_driver_handler = Arc::new(
        QuorumDriverHandlerBuilder::new(Arc::new(aggregator), Arc::new(QuorumDriverMetrics::new_for_tests()))
    ...
    let handle = tokio::task::spawn(async move {
        let (tx, QuorumDriverResponse { effects_cert, .. }) =
            qd_clone.subscribe_to_effects().recv().await.unwrap().unwrap();
        assert_eq!(tx.digest(), &digest);
        assert_eq!(*effects_cert.data().transaction_digest(), digest);
    });
    let ticket = quorum_driver_handler.submit_transaction(ExecuteTransactionRequestV3::new_v2(tx)).await.unwrap();
...
async fn test_quorum_driver_with_given_notify_read() {
    ...
    let ticket2 = quorum_driver_handler.submit_transaction(ExecuteTransactionRequestV3::new_v2(tx)).await.unwrap();
...
async fn test_quorum_driver_update_validators_and_max_retry_times() {
    ...
    let quorum_driver_handler = Arc::new(
        QuorumDriverHandlerBuilder::new(arc_aggregator.clone(), Arc::new(QuorumDriverMetrics::new_for_tests()))
            .with_reconfig_observer(Arc::new(DummyReconfigObserver {}))
    ...
    assert_eq!(quorum_driver_handler.clone_quorum_driver().current_epoch(), 10);
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (aggregator, authorities, genesis, _) = init_local_authorities(4, gas_objects.clone()).await;
    let rgp = authorities.first().unwrap().reference_gas_price_for_testing().unwrap();

    let mut gas_objects = gas_objects
        .into_iter()
        .map(|o| genesis.objects().iter().find(|go| go.id() == o.id()).unwrap().to_owned())
        .collect::<Vec<_>>();

    let aggregator = Arc::new(aggregator);

    let quorum_driver_handler = Arc::new(
        QuorumDriverHandlerBuilder::new(aggregator.clone(), Arc::new(QuorumDriverMetrics::new_for_tests()))
            .with_reconfig_observer(Arc::new(DummyReconfigObserver {}))
            .start(),
    );
    ...
    assert!(client0.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    assert!(client1.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    ...
    assert!(client0.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    assert!(client1.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok(),);
    assert!(client2.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok(),);
    ...
    let res = quorum_driver.submit_transaction(ExecuteTransactionRequestV3::new_v2(tx2)).await.unwrap().await.unwrap();
    ...
    assert!(client0.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    assert!(client1.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    assert!(client2.handle_transaction(tx2, Some(client_ip)).await.is_ok());

    let res = quorum_driver.submit_transaction(ExecuteTransactionRequestV3::new_v2(tx)).await.unwrap().await.unwrap();
    ...
    let tx3 = make_tx(&gas, sender, &keypair, rgp);
    assert!(client0.handle_transaction(tx.clone(), Some(client_ip)).await.is_ok());
    assert!(client1.handle_transaction(tx2.clone(), Some(client_ip)).await.is_ok());
    assert!(client2.handle_transaction(tx3.clone(), Some(client_ip)).await.is_ok());
...
async fn test_quorum_driver_handling_overload_and_retry() {
    ...
    let (sender, keypair): (_, AccountKeyPair) = get_key_pair();
    let gas_object = Object::with_owner_for_testing(sender);
    let (mut aggregator, authorities, genesis, _) = init_local_authorities(4, vec![gas_object.clone()]).await;
