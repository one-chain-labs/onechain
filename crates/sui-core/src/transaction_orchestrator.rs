...
use sui_types::{
    ...
    error::{ErrorCategory, SuiError, SuiErrorKind, SuiResult},
    messages_grpc::{SubmitTxRequest, TxType},
    quorum_driver_types::{
        EffectsFinalityInfo,
    ...
    OnsiteReconfigObserver: ReconfigObserver<A>,
{
    pub fn new(
        ...
        let client_metrics = Arc::new(crate::validator_client_monitor::ValidatorClientMetrics::new(prometheus_registry));
        ...
        let pending_tx_log =
            Arc::new(WritePathPendingTransactionLog::new(parent_path.join("fullnode_pending_transactions")));
        Inner::start_task_to_recover_txes_in_log(pending_tx_log.clone(), transaction_driver.clone());
        ...
        let enable_early_validation =
            node_config.transaction_driver_config.as_ref().map(|config| config.enable_early_validation).unwrap_or(true);
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn execute_transaction_block(
        ...
        let tx_type = if request.transaction.is_consensus_tx() { TxType::SharedObject } else { TxType::SingleWriter };
        let tx_digest = *request.transaction.digest();

        let inner = self.inner.clone();
        let (response, mut executed_locally) =
            spawn_monitored_task!(Inner::<A>::execute_transaction_with_retry(inner, request, client_addr))
                .await
        ...
        if !executed_locally {
            executed_locally = if matches!(request_type, ExecuteTransactionRequestType::WaitForLocalExecution) {
                let executed_locally = Inner::<A>::wait_for_finalized_tx_executed_locally_with_timeout(
        ...
        let QuorumTransactionResponse { effects, events, input_objects, output_objects, auxiliary_data } = response;

        let response = ExecuteTransactionResponseV3 { effects, events, input_objects, output_objects, auxiliary_data };

        self.inner
            .metrics
            .request_latency
            .with_label_values(&[tx_type.as_str(), "execute_transaction_block", executed_locally.to_string().as_str()])
    ...
    pub async fn execute_transaction_v3(
        ...
        Ok(ExecuteTransactionResponseV3 { effects, events, input_objects, output_objects, auxiliary_data })
    }
    ...
    pub fn clone_authority_aggregator(&self) -> Arc<AuthorityAggregator<A>> {
        self.inner.transaction_driver.authority_aggregator().load_full()
    }
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    async fn execute_transaction_with_retry(
            ...
            && e.is_retriable()
        {
            spawn_monitored_task!(async move {
                ...
                let backoff = backoff::ExponentialBackoff::new(Duration::from_secs(1), Duration::from_secs(300));
                ...
                for (i, delay) in backoff.enumerate() {
                    ...
                    let result =
                        inner.execute_transaction_with_effects_waiting(request.clone(), client_addr, i > 3).await;
                    match result {
                        Ok(_) => {
                            inner.metrics.background_retry_attempts.with_label_values(&["success"]).inc();
                            debug!("Background retry {i} for transaction {} succeeded", request.transaction.digest());
                            break;
                        }
                        Err(e) => {
                            if !e.is_retriable() {
                                inner.metrics.background_retry_attempts.with_label_values(&["non-retriable"]).inc();
                                debug!(
                            ...
                            inner.metrics.background_retry_attempts.with_label_values(&["retriable"]).inc();
                            debug!(
    ...
    async fn execute_transaction_with_effects_waiting(
            ...
            )
        {
            ...
            if !error_category.is_submission_retriable() {
                ...
                if !self.validator_state.is_tx_already_executed(&tx_digest) {
                    self.metrics.early_validation_rejections.with_label_values(&[e.to_variant_name()]).inc();
                    debug!(
                        error = ?e,
        ...
        let guard = TransactionSubmissionGuard::new(self.pending_tx_log.clone(), &verified_transaction);
        ...
        for i in 0 .. num_submissions {
            ...
            let future = async move {
                ...
                self.execute_transaction_impl(request, verified_transaction, client_addr, Some(finality_timeout)).await
            }
            .boxed();
        ...
        let mut local_effects_future = epoch_store
            .within_alive_epoch(self.validator_state.get_transaction_cache_reader().notify_read_executed_effects(
    ...
    async fn execute_transaction_impl(
        ...
        let timer = Instant::now();
        let tx_type = if verified_transaction.is_consensus_tx() { TxType::SharedObject } else { TxType::SingleWriter };

        let (_in_flight_metrics_guards, good_response_metrics) = self.update_metrics(&request.transaction);
        ...
        self.metrics.settlement_finality_latency.with_label_values(&[tx_type.as_str(), driver_type]).observe(elapsed);
    ...
    async fn submit_with_transaction_driver(
        ...
        let td_response = td
            ...
            .await
            .map_err(|e| match e {
                TransactionDriverError::TimeoutWithLastRetriableError { last_error, attempts, timeout } => {
                ...
                other => {
                    QuorumDriverError::TransactionFailed { category: other.categorize(), details: other.to_string() }
                }
            });
    ...
    async fn wait_for_finalized_tx_executed_locally_with_timeout(
        ...
        let _metrics_guard = scopeguard::guard(metrics.local_execution_in_flight.clone(), |in_flight| {
            in_flight.dec();
        });

        let _latency_guard = metrics.local_execution_latency.with_label_values(&[tx_type.as_str()]).start_timer();
        debug!("Waiting for finalized tx to be executed locally.");
        match timeout(
            LOCAL_EXECUTION_TIMEOUT,
            validator_state.get_transaction_cache_reader().notify_read_executed_effects_digests(
    ...
    fn update_metrics<'a>(&'a self, transaction: &Transaction) -> (impl Drop + use<A>, &'a GenericCounter<AtomicU64>) {
        let (in_flight, good_response) = if transaction.is_consensus_tx() {
            self.metrics.total_req_received_shared_object.inc();
            (self.metrics.req_in_flight_shared_object.clone(), &self.metrics.good_response_shared_object)
        } else {
            self.metrics.total_req_received_single_writer.inc();
            (self.metrics.req_in_flight_single_writer.clone(), &self.metrics.good_response_single_writer)
        };
        in_flight.inc();
        (
        ...
        spawn_logged_monitored_task!(async move {
                ...
                .map(|tx| {
                    ...
                    async move {
                        ...
                        if let Err(err) = pending_tx_log.finish_transaction(&tx_digest) {
                            warn!(?tx_digest, "Failed to clean up transaction in pending log: {err}");
                        } else {
            ...
            while recovery.next().await.is_some() {
                ...
                if num_recovered % 1000 == 0 {
                    info!("Recovered {} out of {} transactions from pending_tx_log.", num_recovered, num_pending_txes);
                }
            }
            info!(
...
impl TransactionOrchestratorMetrics {
    pub fn new(registry: &Registry) -> Self {
        ...
        let total_req_received_single_writer = total_req_received.with_label_values(&[TX_TYPE_SINGLE_WRITER_TX]);
        let total_req_received_shared_object = total_req_received.with_label_values(&[TX_TYPE_SHARED_OBJ_TX]);
        ...
        let good_response_single_writer = good_response.with_label_values(&[TX_TYPE_SINGLE_WRITER_TX]);
        ...
        let req_in_flight_single_writer = req_in_flight.with_label_values(&[TX_TYPE_SINGLE_WRITER_TX]);
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    fn simulate_transaction(
        ...
        self.inner.validator_state.simulate_transaction(transaction, checks)
    }
}
...
impl TransactionSubmissionGuard {
    pub fn new(pending_tx_log: Arc<WritePathPendingTransactionLog>, tx: &VerifiedTransaction) -> Self {
        ...
        if is_new_transaction {
            ...
            debug!(?tx_digest, "Transaction already being processed, no new submission will be made");
        };
