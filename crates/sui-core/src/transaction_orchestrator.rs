...
use mysten_common::{in_antithesis, sync::notify_read::NotifyRead};
use mysten_metrics::{
    add_server_timing,
    ...
    OnsiteReconfigObserver: ReconfigObserver<A>,
{
    pub fn new(
        ...
        let transaction_driver = if td_percentage > 0 {
            ...
            let client_metrics =
                Arc::new(crate::validator_client_monitor::ValidatorClientMetrics::new(prometheus_registry));
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn execute_transaction_v3(
        ...
        let (response, _) = self.execute_transaction_with_effects_waiting(request, client_addr).await?;
    ...
    async fn execute_transaction_with_effects_waiting(
        ...
        let is_new_transaction =
            self.pending_tx_log.write_pending_transaction_maybe(&verified_transaction).await.map_err(|e| {
    ...
    async fn execute_transaction_impl(
        ...
        let (response, driver_type) =
            if self.transaction_driver.is_some() && self.should_use_transaction_driver(epoch_store, tx_digest) {
                ...
                let resp = self
                    .submit_with_quorum_driver(epoch_store.clone(), verified_transaction.clone(), request, client_addr)
                    .await
                    .map_err(|e| {
                        warn!("QuorumDriverInternalError: {e:?}");
                        QuorumDriverError::QuorumDriverInternalError(e)
    ...
    async fn submit_with_quorum_driver(
        ...
        self.quorum_driver().submit_transaction_no_ticket(request.clone(), client_addr).await?;
        ...
        Ok(async move {
            ...
            let effects_await = epoch_store.within_alive_epoch(
                cache_reader
                    .notify_read_executed_effects("TransactionOrchestrator::notify_read_submit_with_qd", &digests),
            );
            ...
            let res = match select(ticket, effects_await.boxed()).await {
                ...
                Either::Right((_, unfinished_quorum_driver_task)) => {
                    ...
                    qd.submit_transaction_no_ticket(request, client_addr).await?;
    ...
    fn should_use_transaction_driver(
        ...
        debug!("Choosing whether to use transaction driver: {} vs {}", v, self.td_percentage);
            ...
            match effects_receiver.recv().await {
                Ok(Ok((transaction, ..))) => {
                    ...
                    if let Err(err) = pending_transaction_log.finish_transaction(tx_digest) {
                        error!(?tx_digest, "Failed to finish transaction in pending transaction log: {err}");
                    }
                }
    ...
    fn update_metrics(&'_ self, transaction: &Transaction) -> (impl Drop, &'_ GenericCounter<AtomicU64>) {
        ...
        spawn_logged_monitored_task!(async move {
            ...
            for (i, tx) in pending_txes.into_iter().enumerate() {
                ...
                if let Err(err) = quorum_driver
                    ...
                    .await
                {
                    warn!(?tx_digest, "Failed to enqueue transaction from pending_tx_log, err: {err:?}");
                } else {
