...
use sui_types::{
    ...
    transaction::{Argument, CallArg, ObjectArg, TransactionKind},
    TypeTag,
...
use crate::{
    authority::{authority_per_epoch_store::AuthorityPerEpochStore, epoch_start_configuration::EpochStartConfigTrait},
...
impl AccumulatorSettlementTxBuilder {
    pub fn new(cache: Option<&dyn TransactionCacheRead>, ckpt_effects: &[TransactionEffects]) -> Self {
        ...
        Self { updates, addresses, total_input_oct, total_output_oct }
    }
    ...
    //   have been applied.
    pub fn build_tx(self, epoch_store: &AuthorityPerEpochStore, checkpoint_height: u64) -> Vec<TransactionKind> {
        ...
        let total_input_oct_arg = builder.pure(self.total_input_oct).unwrap();
        let total_output_oct_arg = builder.pure(self.total_output_oct).unwrap();
        tracing::debug!("total_input_oct: {}", self.total_input_oct);
        tracing::debug!("total_output_oct: {}", self.total_output_oct);
        ...
        builder.programmable_move_call(
            ...
            vec![],
            vec![epoch_arg, checkpoint_height_arg, idx_arg, total_input_oct_arg, total_output_oct_arg],
        );
        ...
        vec![TransactionKind::ProgrammableSystemTransaction(builder.finish())]
    }
}
