...
use sui_types::{
    ...
    balance::{BALANCE_MODULE_NAME, BALANCE_STRUCT_NAME},
    ...
    transaction::{Argument, CallArg, ObjectArg, SharedObjectMutability, TransactionKind},
    TypeTag,
...
impl MergedValue {
    fn add_move_call(
        ...
        match (ty, merge, split) {
            (ClassifiedType::Balance, MergedValue::SumU128(merge_amount), MergedValue::SumU128(split_amount)) => {
            ...
            (_, MergedValue::EventDigest(digest, event_count), MergedValue::EventDigest(_, _)) => {
                let args = vec![
                    ...
                    builder.pure(U256::from_le_bytes(&digest.into_inner())).unwrap(),
                ...
                builder.programmable_move_call(
                    SUI_FRAMEWORK_PACKAGE_ID,
                    ACCUMULATOR_SETTLEMENT_MODULE.into(),
                    sui_types::accumulator_root::ACCUMULATOR_ROOT_SETTLEMENT_SETTLE_EVENTS_FUNC.into(),
                    vec![],
                    args,
                );
            }
...
impl MergedValueIntermediate {
    ...
    fn accumulate_into(&mut self, value: AccumulatorValue, checkpoint_seq: u64, transaction_idx: u64) {
        match (self, value) {
            ...
            (Self::Events(commitments), AccumulatorValue::EventDigest(event_digests)) => {
                for (event_idx, digest) in event_digests {
                    commitments.push(EventCommitment::new(checkpoint_seq, transaction_idx, event_idx, digest));
                }
            }
            _ => {
                fatal!("invalid merge");
...
impl AccumulatorSettlementTxBuilder {
    pub fn new(
        ...
        for (tx_index, effect) in ckpt_effects.iter().enumerate() {
            ...
            for event in events {
                ...
                let (event_input_sui, event_output_sui) = event.total_oct_in_event();

                let AccumulatorEvent { accumulator_obj, write: AccumulatorWriteV1 { operation, value, address } } =
                    event;
                ...
                let entry = updates.entry(accumulator_obj).or_insert_with(|| {
                    ...
                    Update { merge: zero.clone(), split: zero, input_sui: 0, output_sui: 0 }
                });
                ...
                match operation {
                    AccumulatorOperation::Merge => {
                        entry.merge.accumulate_into(value, checkpoint_seq, tx_index as u64 + tx_index_offset);
                    }
                    AccumulatorOperation::Split => {
                        entry.split.accumulate_into(value, checkpoint_seq, tx_index as u64 + tx_index_offset);
                    }
                }
            }
        }
    ...
    pub fn collect_accumulator_changes(&self) -> BTreeMap<AccumulatorObjId, i128> {
        self.updates
            ...
            .filter_map(|(object_id, update)| match (&update.merge, &update.split) {
                (MergedValueIntermediate::SumU128(merge), MergedValueIntermediate::SumU128(split)) => {
    ...
    pub fn build_tx(
        ...
        checkpoint_seq: u64,
    ) -> (Vec<TransactionKind> /* settlements */, TransactionKind /* barrier */) {
        ...
        let build_one_settlement_txn = |idx: u64, updates: &mut Vec<(AccumulatorObjId, Update)>| {
            let (total_input_oct, total_output_oct) =
                updates.iter().fold((0, 0), |(acc_input, acc_output), (_, update)| {
        ...
        for (obj, update) in updates.into_iter() {
            ...
            if pending_updates.len()
                == protocol_config.max_updates_per_settlement_txn_as_option().unwrap_or(u32::MAX) as usize
            {
        ...
        if !pending_updates.is_empty() {
            settlements.push(build_one_settlement_txn(settlements.len() as u64, &mut pending_updates));
        }
        ...
        Self::add_prologue(&mut builder, root, epoch, checkpoint_height, settlements.len() as u64, 0u64, 0u64);
    ...
    fn add_prologue(
        ...
        let total_input_oct_arg = builder.pure(total_input_oct).unwrap();
        let total_output_oct_arg = builder.pure(total_output_oct).unwrap();
        ...
        builder.programmable_move_call(
            ...
            vec![],
            vec![root, epoch_arg, checkpoint_height_arg, idx_arg, total_input_oct_arg, total_output_oct_arg],
        );
    }
    ...
    fn build_one_settlement_txn(
        ...
        Self::add_prologue(&mut builder, root, epoch, checkpoint_height, idx, total_input_oct, total_output_oct);
        ...
        for (accumulator_obj, update) in updates {
            ...
            MergedValue::add_move_call(merged_value, split_value, root, address, checkpoint_seq, &mut builder);
        }
