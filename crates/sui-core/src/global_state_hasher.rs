...
impl GlobalStateHasher {
    ...
    pub fn set_inconsistent_state(&self, is_inconsistent_state: bool) {
        self.metrics.inconsistent_state.set(is_inconsistent_state as i64);
    }
    ...
    pub fn accumulate_cached_live_object_set_for_testing(&self, include_wrapped_tombstone: bool) -> GlobalStateHash {
    ...
    pub fn accumulate_live_object_set(&self, include_wrapped_tombstone: bool) -> GlobalStateHash {
        Self::accumulate_live_object_set_impl(self.store.iter_live_object_set(include_wrapped_tombstone))
    }
    ...
    fn get_prior_root(
        ...
        let Some(prior_running_root) = epoch_store.get_running_root_state_hash(checkpoint_seq_num - 1)? else {
            fatal!("Running root state hasher must exist for checkpoint {}", checkpoint_seq_num - 1);
        };
    ...
    pub fn accumulate_epoch(
        ...
        self.store.insert_state_hash_for_epoch(epoch_store.epoch(), &last_checkpoint_of_epoch, &running_root)?;
        debug!(
