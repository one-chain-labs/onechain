...
use typed_store::{
    ...
    rocksdb::{MergeOperands, WriteOptions},
    traits::Map,
    DBMapUtils,
    TypedStoreError,
};
...
fn balance_table_options() -> typed_store::rocks::DBOptions {
    default_table_options().set_merge_operator_associative("balance_merge", balance_delta_merge_operator)
}
...
impl IndexStoreTables {
    ...
    fn open<P: Into<PathBuf>>(path: P) -> Self {
        IndexStoreTables::open_tables_read_write(path.into(), MetricConf::new("rpc-index"), None, None)
    }
    ...
    fn index_existing_transactions(
        ...
        checkpoint_range.into_par_iter().try_for_each(|seq| {
            let checkpoint_data = sparse_checkpoint_data_for_backfill(authority_store, checkpoint_store, seq)?;
    ...
    pub async fn new(
        ...
        let tables = {
            ...
            if tables.needs_to_do_initialization(checkpoint_store) {
                ...
                tables
                    .init(authority_store, checkpoint_store, epoch_store, package_store, batch_size_limit)
...
fn sparse_checkpoint_data_for_backfill(
    ...
    for (tx, fx) in transactions.into_iter().zip(effects) {
        ...
        let full_transaction =
            CheckpointTransaction { transaction: tx.into(), effects: fx, events: None, input_objects, output_objects };
