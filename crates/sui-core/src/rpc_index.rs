...
impl OwnerIndexInfo {
    pub fn new(object: &Object) -> Self {
        ...
        Self { version: object.version(), digest: object.digest(), start_version }
    }
}
...
impl IndexStoreTables {
                ...
                match object.owner() {
                    ...
                    Owner::ObjectOwner(parent) => {
                        if let Some(field_info) =
                            try_create_dynamic_field_info(object, resolver, &tx.output_objects.as_slice() as _)?
                        {
    ...
    > {
        ...
        let iter = self.dynamic_field.safe_iter_with_bounds(Some(lower_bound), Some(upper_bound));
        Ok(iter)
    }
    ...
    pub async fn new(
        ...
        let tables = {
            ...
            if tables.needs_to_do_initialization(checkpoint_store) {
                ...
                tables
                    .init(authority_store, checkpoint_store, epoch_store, package_store)
...
fn try_create_dynamic_field_info(
    ...
    let layout =
        resolver.get_annotated_layout(&move_object.type_().clone().into()).map_err(StorageError::custom)?.into_layout();

    let field = DFV::FieldVisitor::deserialize(move_object.contents(), &layout).map_err(StorageError::custom)?;

    let (value_type, dynamic_object_id) = match field.value_metadata().map_err(StorageError::custom)? {
...
impl<'a> ParMakeLiveObjectIndexer for RpcParLiveObjectSetIndexer<'a> {
    ...
    fn make_live_object_indexer(&self) -> Self::ObjectIndexer {
        RpcLiveObjectIndexer {
            ...
            resolver: self.epoch_store.executor().type_layout_resolver(Box::new(self.package_store)),
...
impl LiveObjectIndexer for RpcLiveObjectIndexer<'_> {
    fn index_object(&mut self, object: Object) -> Result<(), StorageError> {
        match object.owner {
            ...
            Owner::ObjectOwner(parent) => {
                if let Some(field_info) =
                    try_create_dynamic_field_info(&object, self.resolver.as_mut(), self.object_store)?
                {
                    let field_key = DynamicFieldKey::new(parent, object.id());

                    self.batch.insert_batch(&self.tables.dynamic_field, [(field_key, field_info)])?;
                }
            }
