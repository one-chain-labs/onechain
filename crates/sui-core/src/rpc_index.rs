...
use sui_types::{
    ...
    storage::{error::Error as StorageError, BackingPackageStore, DynamicFieldKey, EpochInfo, TransactionInfo},
...
use typed_store::{
    rocks::{DBMap, DBMapTableConfigMap, MetricConf},
    rocksdb::{compaction_filter::Decision, MergeOperands, WriteOptions},
    traits::Map,
    DBMapUtils,
    TypedStoreError,
};
...
impl BalanceIndexInfo {
    fn invert(self) -> Self {
        ...
        assert!(self.balance_delta != i128::MIN, "Cannot invert balance_delta: would overflow i128");
...
fn events_table_options(compaction_filter: Option<EventsCompactionFilter>) -> typed_store::rocks::DBOptions {
    let mut options = default_table_options();
    if let Some(filter) = compaction_filter {
        options.options.set_compaction_filter("events_by_stream", move |_, key, value| {
            match filter.filter(key, value) {
                Ok(decision) => decision,
                Err(e) => {
                    warn!("Failed to parse event key during compaction: {}, key: {:?}", e, key);
                    Decision::Remove
                }
            }
        });
...
fn balance_delta_merge_operator(_key: &[u8], existing_val: Option<&[u8]>, operands: &MergeOperands) -> Option<Vec<u8>> {
    ...
    Some(bcs::to_bytes(&result).expect("Failed to deserialize BalanceIndexInfo from RocksDB - data corruption."))
}
...
impl CoinIndexInfo {
    fn merge(&mut self, other: Self) {
        self.coin_metadata_object_id = self.coin_metadata_object_id.or(other.coin_metadata_object_id);
        self.regulated_coin_metadata_object_id =
            self.regulated_coin_metadata_object_id.or(other.regulated_coin_metadata_object_id);
...
impl EventsCompactionFilter {
    ...
    pub fn filter(&self, key: &[u8], _value: &[u8]) -> anyhow::Result<Decision> {
        ...
        let watermark = self.pruning_watermark.load(std::sync::atomic::Ordering::Relaxed);
...
impl IndexStoreTables {
    fn track_coin_balance_change(
        ...
        if let Some((struct_tag, value)) = get_balance_and_type_if_coin(object)? {
            let key = BalanceKey { owner: *owner, coin_type: struct_tag };
    ...
    fn extract_version_if_package(object: &Object) -> Option<(PackageVersionKey, PackageVersionInfo)> {
        if let Data::Package(package) = &object.data {
            ...
            let key = PackageVersionKey { original_package_id: original_id, version };
    ...
    fn open_with_index_options<P: Into<PathBuf>>(path: P, index_options: IndexStoreOptions) -> Self {
        let mut table_options = std::collections::BTreeMap::new();
        table_options.insert("balance".to_string(), balance_table_options());
        table_options
            .insert("events_by_stream".to_string(), events_table_options(index_options.events_compaction_filter));
    ...
    fn open_with_options<P: Into<PathBuf>>(
        ...
        IndexStoreTables::open_tables_read_write(path.into(), MetricConf::new("rpc-index"), Some(options), table_options)
    }
    ...
    fn is_indexed_watermark_out_of_date(&self, checkpoint_store: &CheckpointStore) -> bool {
        let highest_executed_checkpint = checkpoint_store.get_highest_executed_checkpoint_seq_number().ok().flatten();
    ...
    fn init(
        ...
        let highest_executed_checkpint = checkpoint_store.get_highest_executed_checkpoint_seq_number()?;
        let lowest_available_checkpoint =
            checkpoint_store.get_highest_pruned_checkpoint_seq_number()?.map(|c| c.saturating_add(1)).unwrap_or(0);
        let lowest_available_checkpoint_objects =
            authority_store.perpetual_tables.get_highest_pruned_checkpoint()?.map(|c| c.saturating_add(1)).unwrap_or(0);
        ...
        let lowest_available_checkpoint = lowest_available_checkpoint.max(lowest_available_checkpoint_objects);

        let checkpoint_range = highest_executed_checkpint
            .map(|highest_executed_checkpint| lowest_available_checkpoint ..= highest_executed_checkpint);

        if let Some(checkpoint_range) = checkpoint_range {
            self.index_existing_transactions(authority_store, checkpoint_store, checkpoint_range, rpc_config)?;
        }
        ...
        if highest_executed_checkpint.is_some() {
            ...
            let make_live_object_indexer =
                RpcParLiveObjectSetIndexer { tables: self, coin_index: &coin_index, batch_size_limit };
        ...
        self.watermark.insert(&Watermark::Indexed, &highest_executed_checkpint.unwrap_or(0))?;
    ...
    fn index_existing_transactions(
        ...
        checkpoint_range.into_par_iter().try_for_each(|seq| {
            ...
            let checkpoint_data =
                sparse_checkpoint_data_for_backfill(authority_store, checkpoint_store, seq, load_events)?;
    ...
    fn prune(
        ...
        let transactions_to_prune =
            checkpoint_contents_to_prune.iter().flat_map(|contents| contents.iter().map(|digests| digests.transaction));
    ...
    fn index_checkpoint(
        ...
        self.index_transactions(checkpoint, &mut batch, rpc_config.authenticated_events_indexing())?;
        self.index_objects(checkpoint, &mut batch)?;

        batch.insert_batch(&self.watermark, [(Watermark::Indexed, checkpoint.checkpoint_summary.sequence_number)])?;
    ...
    fn extract_accumulator_version(
        ...
        let TransactionKind::ProgrammableSystemTransaction(pt) = tx.transaction.transaction_data().kind() else {
            return None;
        };
    ...
    fn index_transaction_events(
        ...
        for acc in acc_events {
            if let AccumulatorValue::EventDigest(event_digests) = &acc.write.value {
                ...
                if let Some(stream_id) = sui_types::accumulator_root::stream_id_from_accumulator_event(&acc) {
    ...
    fn index_transactions(
        ...
        for (tx_idx, tx) in checkpoint.transactions.iter().enumerate().rev() {
            ...
            if index_events {
                ...
                self.index_transaction_events(tx, cp, tx_idx as u32, current_accumulator_version, batch)?;
            }
        }
            ...
            for removed_object in tx.removed_objects_pre_version() {
                match removed_object.owner() {
                    Owner::AddressOwner(owner) | Owner::ConsensusAddressOwner { owner, .. } => {
                        Self::track_coin_balance_change(removed_object, owner, true, &mut balance_changes)?;
            ...
            for (object, old_object) in tx.changed_objects() {
                if let Some(old_object) = old_object {
                    match old_object.owner() {
                        Owner::AddressOwner(owner) | Owner::ConsensusAddressOwner { owner, .. } => {
                            Self::track_coin_balance_change(old_object, owner, true, &mut balance_changes)?;
    ...
    ) -> Result<impl Iterator<Item = Result<(OwnerIndexKey, OwnerIndexInfo), TypedStoreError>> + '_, TypedStoreError>
    {
        // TODO can we figure out how to pass a raw byte array as a cursor?
        let lower_bound = cursor.unwrap_or_else(|| OwnerIndexKey {
            owner,
            object_type: object_type.clone().unwrap_or_else(|| "0x0::a::a".parse::<StructTag>().unwrap()),
    ...
    fn dynamic_field_iter(
        ...
        let iter =
            self.dynamic_field.safe_iter_with_bounds(Some(lower_bound), Some(upper_bound)).map_ok(|(key, ())| key);
        Ok(iter)
    }

    fn get_coin_info(&self, coin_type: &StructTag) -> Result<Option<CoinIndexInfo>, TypedStoreError> {
        let key = CoinIndexKey { coin_type: coin_type.to_owned() };
        self.coin.get(&key)
    }
    ...
    fn get_balance(
        ...
        let key = BalanceKey { owner: owner.to_owned(), coin_type: coin_type.to_owned() };
    ...
    ) -> Result<impl Iterator<Item = Result<(BalanceKey, BalanceIndexInfo), TypedStoreError>> + '_, TypedStoreError>
    {
        let lower_bound =
            cursor.unwrap_or_else(|| BalanceKey { owner, coin_type: "0x0::a::a".parse::<StructTag>().unwrap() });
    ...
    fn package_versions_iter(
        ...
        TypedStoreError,
    > {
        let lower_bound = PackageVersionKey { original_package_id: original_id, version: cursor.unwrap_or(0) };
        let upper_bound = PackageVersionKey { original_package_id: original_id, version: u64::MAX };

        Ok(self.package_version.safe_iter_with_bounds(Some(lower_bound), Some(upper_bound)))
    }
}
...
impl RpcIndexStore {
    ...
    pub async fn new(
        ...
        let index_options = IndexStoreOptions { events_compaction_filter: Some(events_filter) };
    ...
    pub async fn new_with_options(
            ...
            if tables.needs_to_do_initialization(checkpoint_store) {
                ...
                let mut tables = {
                    ...
                    let max_background_jobs =
                        if let Some(jobs) = index_config.as_ref().and_then(|c| c.max_background_jobs) {
                    ...
                    // all column families.
                    let db_buffer_size = if let Some(size) = index_config.as_ref().and_then(|c| c.db_write_buffer_size) {
                    ...
                    let (buffer_size, buffer_count) = match (
                        ...
                        index_config.as_ref().and_then(|c| c.cf_max_write_buffer_number),
                    ) {
                        (Some(size), Some(count)) => {
                            debug!("Using config overrides - buffer_size: {} bytes, buffer_count: {}", size, count);
                            (size, count)
                        }
                        (None, None) => {
                    ...
                    batch_size_limit = if let Some(limit) = index_config.as_ref().and_then(|c| c.batch_size_limit) {
                        debug!("Using config override for batch_size_limit: {} bytes", limit);
                        limit
                    } else {
                    ...
                    balance_options =
                        balance_options.set_merge_operator_associative("balance_merge", balance_delta_merge_operator);
                    ...
                    IndexStoreTables::open_with_options(&path, options, Some(DBMapTableConfigMap::new(table_config_map)))
                };

                tables
                    .init(authority_store, checkpoint_store, epoch_store, package_store, batch_size_limit, &rpc_config)
                ...
                let reopened_tables = IndexStoreTables::open_with_index_options(&path, index_options);
        ...
        Self { tables, pending_updates: Default::default(), rpc_config }
    }
    ...
    pub fn new_without_init(dir: &Path) -> Self {
        ...
        Self { tables, pending_updates: Default::default(), rpc_config: sui_config::RpcConfig::default() }
    }
    ...
    pub fn index_checkpoint(&self, checkpoint: &CheckpointData, resolver: &mut dyn LayoutResolver) {
        ...
        let batch = self.tables.index_checkpoint(checkpoint, resolver, &self.rpc_config).expect("db error");
    ...
    pub fn get_coin_info(&self, coin_type: &StructTag) -> Result<Option<CoinIndexInfo>, TypedStoreError> {
...
fn should_index_dynamic_field(object: &Object) -> bool {
    ...
    //   to object
    //
    object.data.try_as_move().is_some_and(|move_object| move_object.type_().is_dynamic_field())
}

fn try_create_coin_index_info(object: &Object) -> Option<(CoinIndexKey, CoinIndexInfo)> {
    use sui_types::coin::{CoinMetadata, RegulatedCoinMetadata, TreasuryCap};
    ...
    if let Some(coin_type) = CoinMetadata::is_coin_metadata_with_coin_type(object_type).cloned() {
        return Some((CoinIndexKey { coin_type }, CoinIndexInfo {
    ...
    if let Some(coin_type) = RegulatedCoinMetadata::is_regulated_coin_metadata_with_coin_type(object_type).cloned() {
        return Some((CoinIndexKey { coin_type }, CoinIndexInfo {
            coin_metadata_object_id: None,
            treasury_object_id: None,
...
impl LiveObjectIndexer for RpcLiveObjectIndexer<'_> {
    fn index_object(&mut self, object: Object) -> Result<(), StorageError> {
        match object.owner {
            ...
            Owner::ObjectOwner(parent) => {
                if should_index_dynamic_field(&object) {
                    let field_key = DynamicFieldKey::new(parent, object.id());
                    self.batch.insert_batch(&self.tables.dynamic_field, [(field_key, ())])?;
                }
            }
        ...
        if let Some((key, info)) = IndexStoreTables::extract_version_if_package(&object) {
            self.batch.insert_batch(&self.tables.package_version, [(key, info)])?;
        }
        ...
        if self.batch.size_in_bytes() >= self.batch_size_limit {
            std::mem::replace(&mut self.batch, self.tables.owner.batch()).write_opt(&bulk_ingestion_write_options())?;
        }
    ...
    fn finish(mut self) -> Result<(), StorageError> {
        self.batch.partial_merge_batch(&self.tables.balance, std::mem::take(&mut self.balance_changes))?;
...
fn sparse_checkpoint_data_for_backfill(
    ...
    let transaction_digests = contents.iter().map(|execution_digests| execution_digests.transaction).collect::<Vec<_>>();
    let transactions = authority_store
        .multi_get_transaction_blocks(&transaction_digests)?
        .into_iter()
        .map(|maybe_transaction| maybe_transaction.ok_or_else(|| StorageError::custom("missing transaction")))
    ...
    let events = if load_events {
        authority_store.multi_get_events(&transaction_digests).map_err(|e| StorageError::custom(e.to_string()))?
    } else {
    ...
    for ((tx, fx), ev) in transactions.into_iter().zip(effects).zip(events) {
        let input_objects = sui_types::storage::get_transaction_input_objects(authority_store, &fx)?;
        let output_objects = sui_types::storage::get_transaction_output_objects(authority_store, &fx)?;

        let full_transaction =
            CheckpointTransaction { transaction: tx.into(), effects: fx, events: ev, input_objects, output_objects };
...
fn get_balance_and_type_if_coin(object: &Object) -> Result<Option<(StructTag, u64)>, StorageError> {
    match Coin::extract_balance_if_coin(object) {
        ...
        Err(e) => {
            // Corrupted coin data
            Err(StorageError::custom(format!("Failed to deserialize coin object {}: {}", object.id(), e)))
        }
    }
}
...
mod tests {
    ...
    async fn test_events_compaction_filter() {
        ...
        let index_options = IndexStoreOptions { events_compaction_filter: Some(compaction_filter) };
        ...
        for key in &test_events {
            batch.insert_batch(&tables.events_by_stream, [(key.clone(), ())]).unwrap();
        }
        batch.write().unwrap();
        ...
        assert_eq!(events_before_compaction, 5, "Should have 5 events before compaction");
        ...
        tables.events_by_stream.compact_range(&start_key, &end_key).unwrap();
        ...
        assert!(events_after_compaction.len() >= 2, "Should have at least the events that shouldn't be pruned");
    ...
    fn test_events_compaction_filter_logic() {
        ...
        assert!(matches!(decision, Decision::Keep), "Event with checkpoint 150 should be kept when watermark is 100");
        ...
        assert!(matches!(decision, Decision::Remove), "Event with checkpoint equal to watermark should be removed");
    }
}
