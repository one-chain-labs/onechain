...
async fn test_immediate_return_canceled_shared() {
    ...
    cache.notify_read_input_objects(&[canceled_key], &receiving_keys, epoch).now_or_never().unwrap();
    ...
    cache.notify_read_input_objects(&[congested_key], &receiving_keys, epoch).now_or_never().unwrap();
    ...
    cache.notify_read_input_objects(&[randomness_unavailable_key], &receiving_keys, epoch).now_or_never().unwrap();
}
...
async fn test_immediate_return_cached_object() {
    ...
    let input_keys = vec![InputKey::VersionedObject { id: FullObjectID::new(object_id, None), version }];
    let receiving_keys = HashSet::new();
    let epoch = 0;

    // Should return immediately since object is in cache
    cache.notify_read_input_objects(&input_keys, &receiving_keys, epoch).now_or_never().unwrap();
}
...
async fn test_immediate_return_cached_package() {
    ...
    let input_keys = vec![InputKey::Package { id: SUI_FRAMEWORK_PACKAGE_ID }];
    let receiving_keys = HashSet::new();
    let epoch = 0;
...
async fn test_immediate_return_consensus_stream_ended() {
    ...
    let input_keys = vec![InputKey::VersionedObject { id: FullObjectID::new(object_id, Some(version)), version }];
    let receiving_keys = HashSet::new();
...
async fn test_wait_for_object() {
    ...
    let result =
        timeout(Duration::from_secs(3), cache.notify_read_input_objects(&input_keys, &receiving_keys, epoch)).await;
    ...
    tokio::spawn({
        ...
        async move {
            ...
            let object = Object::with_id_owner_version_for_testing(object_id, SequenceNumber::from(0), Owner::Shared {
    ...
    tokio::spawn({
        ...
        async move {
            ...
            let object = Object::with_id_owner_version_for_testing(object_id, version, Owner::Shared {
                initial_shared_version: version,
            });
            cache.write_object_entry_for_test(object);
        }
    });
    timeout(Duration::from_secs(3), cache.notify_read_input_objects(&input_keys, &receiving_keys, epoch)).await.unwrap();
}
...
async fn test_receiving_object_higher_version() {
    ...
    let object =
        Object::with_id_owner_version_for_testing(object_id, higher_version, Owner::AddressOwner(SuiAddress::default()));

    // Write higher version to cache
    cache.write_object_entry(&object_id, higher_version, ObjectEntry::Object(object));

    let input_keys =
        vec![InputKey::VersionedObject { id: FullObjectID::new(object_id, None), version: requested_version }];
