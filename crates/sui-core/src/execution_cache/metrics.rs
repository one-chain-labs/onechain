...
impl ExecutionCacheMetrics {
    ...
    pub(crate) fn record_cache_request(&self, request_type: &'static str, level: &'static str) {
        ...
        self.cache_requests.with_label_values(&[request_type, level]).inc();
    }

    pub(crate) fn record_cache_multi_request(&self, request_type: &'static str, level: &'static str, count: usize) {
        trace!(
        ...
        self.cache_requests.with_label_values(&[request_type, level]).inc_by(count as u64);
    }
    ...
    pub(crate) fn record_cache_hit(&self, request_type: &'static str, level: &'static str) {
        ...
        self.cache_hits.with_label_values(&[request_type, level]).inc();
    }
    ...
    pub(crate) fn record_cache_miss(&self, request_type: &'static str, level: &'static str) {
        ...
        self.cache_misses.with_label_values(&[request_type, level]).inc();
    }

    pub(crate) fn record_cache_negative_hit(&self, request_type: &'static str, level: &'static str) {
        trace!(target: "cache_metrics", "Cache negative hit: {} {}", request_type, level);
        self.cache_negative_hits.with_label_values(&[request_type, level]).inc();
    }
