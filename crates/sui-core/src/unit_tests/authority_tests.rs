...
use sui_types::{
    ...
    gas_coin::GasCoin,
    messages_consensus::{AuthorityCapabilitiesV2, ConsensusDeterminedVersionAssignments, ConsensusTransaction},
...
use crate::{
    ...
    consensus_test_utils::{CapturedTransactions, TestConsensusCommit},
...
async fn test_conflicting_transactions() {
    ...
    for _ in 0 .. 100 {
        ...
        assert!(matches!(err.as_inner(), SuiErrorKind::ObjectLockConflict { .. }));
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (SuiAddress, Vec<DynamicFieldInfo>) {
    ...
    let add_effects = authority_state.try_execute_for_test(&add_cert, ExecutionEnv::new()).await.0.into_message();
...
async fn test_shared_object_transaction_ok() {
    ...
    authority.try_execute_for_test(&certificate, ExecutionEnv::new().with_assigned_versions(assigned_versions)).await;
...
async fn test_consensus_commit_prologue_generation() {
    ...
    let consensus_setup = crate::consensus_test_utils::setup_consensus_handler_for_testing(&authority_state).await;
    ...
    C: CheckpointServiceNotify + Send + Sync,
{
    ...
    for cert in certificates {
        let transaction = ConsensusTransaction::new_certificate_message(&authority.name, cert.clone().into());
        transactions.push(transaction);
    }

    // Create a TestConsensusCommit with the transactions
    // Use a realistic timestamp based on the epoch start time
    let epoch_start_ms = authority.epoch_store_for_testing().epoch_start_state().epoch_start_timestamp_ms();
        ...
        if filter_prologue {
            schedulables.retain(|s| {
                if let Schedulable::Transaction(tx) = s {
                    !matches!(tx.data().transaction_data().kind(), TransactionKind::ConsensusCommitPrologueV4(..))
                } else {
                    true
                }
            });
...
async fn test_consensus_handler_congestion_control_transaction_cancellation() {
    ...
    let (scheduled_txns, assigned_versions) = process_certificates_through_consensus_handler_with_prologue(
        &mut consensus_handler,
        &authority,
        &[],
        3,
...
async fn test_insufficient_balance_for_withdraw_early_error() {
    ...
    let (effects, execution_error) =
        state.try_execute_immediately(&certificate, execution_env, &epoch_store).await.unwrap();
