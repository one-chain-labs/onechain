...
use sui_types::{
    ...
    messages_grpc::{RawSubmitTxRequest, SubmitTxRequest, SubmitTxResponse, SubmitTxResult, SubmitTxType},
    object::Object,
    transaction::{Transaction, TransactionDataAPI, TransactionExpiration, VerifiedTransaction},
...
impl TestContext {
    async fn new() -> Self {
        ...
        let authority = TestAuthorityBuilder::new().with_starting_objects(&[gas_object]).build().await;
        ...
        Self { state: authority, _server_handle: server_handle, client, sender, keypair, gas_object_ref }
    }
    ...
    fn build_submit_request(&self, transaction: Transaction) -> SubmitTxRequest {
        SubmitTxRequest { transaction: Some(transaction), ping: None }
    }
}
...
async fn test_submit_transaction_success() {
    ...
    let response = test_context.client.submit_transaction(request, None).await.unwrap();
...
async fn test_submit_ping_request() {
    ...
    println!("Case 1. Ping request cannot contain transactions.");
    {
        ...
        let response = test_context.client.client().unwrap().submit_transaction(request).await;
        assert!(response.is_err());
        assert!(matches!(response.unwrap_err().into(), SuiError::InvalidRequest { .. }));
    }
    ...
    println!("Case 2. Valid ping request.");
    {
        ...
        let request = RawSubmitTxRequest { transactions: vec![], submit_type: SubmitTxType::Ping.into() };

        let response = test_context.client.client().unwrap().submit_transaction(request).await.unwrap();
...
async fn test_submit_transaction_invalid_transaction() {
    ...
    let request = RawSubmitTxRequest { transactions: vec![vec![0xFF, 0xFF, 0xFF].into()], ..Default::default() };
...
async fn test_submit_transaction_already_executed() {
    ...
    let response1 = test_context.client.submit_transaction(request.clone(), None).await.unwrap();
    ...
    let response2 = test_context.client.submit_transaction(request, None).await.unwrap();
    // Verify we got the full effects back.
    assert_eq!(response2.results.len(), 1);
    match &response2.results[0] {
        SubmitTxResult::Executed { effects_digest, details, fast_path: _ } => {
            let details = details.as_ref().unwrap();
            assert_eq!(*effects_digest, details.effects.digest());
            assert_eq!(verified_transaction.digest(), details.effects.transaction_digest());
        }
...
async fn test_submit_transaction_wrong_epoch() {
    ...
    let tx_data = TestTransactionBuilder::new(
        ...
        test_context.state.reference_gas_price_for_testing().unwrap(),
    )
...
async fn test_submit_transaction_gas_object_validation() {
    ...
    assert!(matches!(result, SubmitTxResult::Rejected {
        error: SuiError::UserInputError { error: UserInputError::ObjectNotFound { .. } }
    }));
}
...
async fn test_submit_batched_transactions() {
    ...
    let request = RawSubmitTxRequest {
        transactions: vec![bcs::to_bytes(&tx1).unwrap().into(), bcs::to_bytes(&tx2).unwrap().into()],
        ..Default::default()
    };

    // Submit request with batched transactions, using grpc client directly.
    let raw_response = test_context.client.client().unwrap().submit_transaction(request).await.unwrap().into_inner();
...
async fn test_submit_soft_bundle_transactions_with_already_executed() {
    ...
    // Submit request with batched transactions, using grpc client directly.
    let error = test_context.client.client().unwrap().submit_transaction(request).await.unwrap_err().into();

    // Verify the error is AlreadyExecutedInSoftBundleError.
    assert!(matches!(error, SuiError::UserInputError {
