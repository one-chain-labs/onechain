...
fn test_new_initial() {
    ...
    let c_pkg = MovePackage::new_initial(&build_test_modules("Cv1"), u64::MAX, VERSION_MAX, []).unwrap();

    let b_id1 = ObjectID::from_single_byte(0xb1);
    let b_pkg = MovePackage::new_initial(&build_test_modules("B"), u64::MAX, VERSION_MAX, [&c_pkg]).unwrap();

    let a_pkg = MovePackage::new_initial(&build_test_modules("A"), u64::MAX, VERSION_MAX, [&b_pkg, &c_pkg]).unwrap();
...
fn test_depending_on_upgrade() {
    ...
    let b_pkg = MovePackage::new_initial(&build_test_modules("B"), u64::MAX, VERSION_MAX, [&c_new]).unwrap();
...
fn test_transitively_depending_on_upgrade() {
    ...
    let a_pkg = MovePackage::new_initial(&build_test_modules("A"), u64::MAX, VERSION_MAX, [&b_pkg, &c_new]).unwrap();
...
fn package_digest_changes_with_dep_upgrades_and_in_sync_with_move_package_digest() {
    let c_v1 = MovePackage::new_initial(&build_test_modules("Cv1"), u64::MAX, VERSION_MAX, []).unwrap();

    let c_id2 = ObjectID::from_single_byte(0xc2);
    let c_v2 =
        c_v1.new_upgraded(c_id2, &build_test_modules("Cv2"), &ProtocolConfig::get_for_max_version_UNSAFE(), []).unwrap();

    let b_pkg = MovePackage::new_initial(&build_test_modules("B"), u64::MAX, VERSION_MAX, [&c_v1]).unwrap();
    let b_v2 = MovePackage::new_initial(&build_test_modules("Bv2"), u64::MAX, VERSION_MAX, [&c_v2]).unwrap();
...
fn test_fail_on_missing_dep() {
    let err = MovePackage::new_initial(&build_test_modules("B"), u64::MAX, VERSION_MAX, []).unwrap_err();
...
fn test_fail_on_missing_transitive_dep() {
    ...
    let err = MovePackage::new_initial(&build_test_modules("A"), u64::MAX, VERSION_MAX, [&b_pkg]).unwrap_err();
...
fn test_fail_on_transitive_dependency_downgrade() {
    ...
    let err = MovePackage::new_initial(&build_test_modules("A"), u64::MAX, VERSION_MAX, [&b_pkg, &c_pkg]).unwrap_err();
...
fn test_fail_on_upgrade_missing_type() {
    let c_pkg = MovePackage::new_initial(&build_test_modules("Cv2"), u64::MAX, VERSION_MAX, []).unwrap();
