...
fn test_new_initial() {
    ...
    let c_pkg =
        MovePackage::new_initial(&build_test_modules("Cv1"), &ProtocolConfig::get_for_max_version_UNSAFE(), []).unwrap();

    let b_id1 = ObjectID::from_single_byte(0xb1);
    let b_pkg =
        MovePackage::new_initial(&build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_pkg])
            .unwrap();

    let a_pkg = MovePackage::new_initial(&build_test_modules("A"), &ProtocolConfig::get_for_max_version_UNSAFE(), [
        &b_pkg, &c_pkg,
    ])
    .unwrap();
    ...
    assert_eq!(c_pkg_obj.object_size_for_gas_metering() + 2, c_serialized.len());
}
...
fn test_upgraded() {
    ...
    let c_new = c_pkg
        .new_upgraded(c_id2, &build_test_modules("Cv2"), &ProtocolConfig::get_for_max_version_UNSAFE(), [])
        .unwrap();
...
fn test_depending_on_upgrade() {
    ...
    let b_pkg =
        MovePackage::new_initial(&build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_new])
            .unwrap();
...
fn test_upgrade_upgrades_linkage() {
    ...
    let b_new = b_pkg
        .new_upgraded(b_id2, &build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_new])
        .unwrap();
...
fn test_duplicate_transitive_deps() {
    ...
    let pkg =
        MovePackage::new_initial(&[basic_test_module()], &ProtocolConfig::get_for_version(88.into(), Chain::Unknown), [
            &dep_pkg, &dep_pkg,
        ]);
...
fn test_transitive_dep_downgrade() {
    ...
    let l2 = l1.new_upgraded(ObjectID::from_hex_literal("0x1").unwrap(), &[basic_test_module()], &config, []).unwrap();
    ...
    let f2 = f1.new_upgraded(ObjectID::from_hex_literal("0x3").unwrap(), &[m], &config, [&l2]).unwrap();
...
fn test_upgrade_downgrade_transitive() {
    ...
    let c_id2 = ObjectID::from_single_byte(0xc2);
    let c_v2 = c_v1.new_upgraded(c_id2, &build_test_modules("Cv2"), &config, []).unwrap();

    let b_id1 = ObjectID::from_single_byte(0xb1);
    let b_v1 = MovePackage::new_initial(&build_test_modules("B"), &config, [&c_v1]).unwrap();

    let b_id2 = ObjectID::from_single_byte(0xb2);
    let b_v2 = b_v1.new_upgraded(b_id2, &build_test_modules("Bv2"), &config, [&c_v2]).unwrap();

    let f_id = ObjectID::from_single_byte(0xf1);
    let f_pkg = MovePackage::new_initial(&build_test_modules("F"), &config, [&b_v1, &c_v1]).unwrap();
    ...
    assert_eq!(h_pkg.linkage_table(), &linkage_table! {
        c_id1 => (c_id1, OBJECT_START_VERSION),
        b_id1 => (b_id2, b_v2.version()),
        f_id => (f_id, OBJECT_START_VERSION),
    },);

    assert!(MovePackage::new_initial(&build_test_modules("H"), &config, [&f_pkg, &b_v2, &c_v1]).is_err());
}
...
fn test_upgrade_downngrades_linkage() {
    ...
    let b_id2 = ObjectID::from_single_byte(0xb2);
    let b_new = b_pkg
        .new_upgraded(b_id2, &build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_pkg])
        .unwrap();
...
fn package_digest_changes_with_dep_upgrades_and_in_sync_with_move_package_digest() {
    ...
    let c_id2 = ObjectID::from_single_byte(0xc2);
    let c_v2 =
        c_v1.new_upgraded(c_id2, &build_test_modules("Cv2"), &ProtocolConfig::get_for_max_version_UNSAFE(), []).unwrap();

    let b_pkg =
        MovePackage::new_initial(&build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_v1])
            .unwrap();
    let b_v2 =
        MovePackage::new_initial(&build_test_modules("Bv2"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&c_v2])
            .unwrap();
...
fn test_fail_on_missing_dep() {
    let err = MovePackage::new_initial(&build_test_modules("B"), &ProtocolConfig::get_for_max_version_UNSAFE(), [])
        .unwrap_err();

    assert_eq!(err.kind(), &ExecutionErrorKind::PublishUpgradeMissingDependency);
}
...
fn test_fail_on_missing_transitive_dep() {
    ...
    let err =
        MovePackage::new_initial(&build_test_modules("A"), &ProtocolConfig::get_for_max_version_UNSAFE(), [&b_pkg])
            .unwrap_err();
...
fn test_fail_on_transitive_dependency_downgrade() {
    ...
    let err = MovePackage::new_initial(&build_test_modules("A"), &ProtocolConfig::get_for_max_version_UNSAFE(), [
        &b_pkg, &c_pkg,
    ])
    .unwrap_err();

    assert_eq!(err.kind(), &ExecutionErrorKind::PublishUpgradeDependencyDowngrade);
}

#[test]
fn test_fail_on_upgrade_missing_type() {
    let c_pkg =
        MovePackage::new_initial(&build_test_modules("Cv2"), &ProtocolConfig::get_for_max_version_UNSAFE(), []).unwrap();

    let c_id2 = ObjectID::from_single_byte(0xc2);
    let err = c_pkg
        .new_upgraded(c_id2, &build_test_modules("Cv1"), &ProtocolConfig::get_for_max_version_UNSAFE(), [])
        .unwrap_err();
    ...
    let err = c_pkg
        .new_upgraded(c_id2, &build_test_modules("Cv1"), &ProtocolConfig::get_for_version(4.into(), Chain::Unknown), [])
        .unwrap_err();
...
pub fn build_test_modules(test_dir: &str) -> Vec<CompiledModule> {
    build_test_package(test_dir).get_modules().cloned().collect()
}
