...
use sui_types::{
    ...
    deny_list_v2::{check_address_denied_by_config, check_global_pause, get_per_type_coin_deny_list_v2, DenyCapV2},
...
async fn test_regulated_coin_v1_creation() {
    ...
    assert_eq!(regulated_metadata.id.id.bytes, regulated_metadata_object.id());
    assert_eq!(regulated_metadata.deny_cap_object.bytes, deny_cap_object.id());
    assert_eq!(regulated_metadata.coin_metadata_object.bytes, metadata_object.id());
}
...
async fn test_regulated_coin_v2_types() {
    ...
    .move_call_with_type_args(
        ...
        vec![
            ...
            CallArg::Object(ObjectArg::ImmOrOwnedObject(env.get_latest_object_ref(&metadata.deny_cap_id).await)),
            CallArg::Pure(bcs::to_bytes(&deny_address).unwrap()),
        ],
    )
    .build_and_sign(&env.keypair);
    let (_, effects) = send_and_confirm_transaction_(&env.authority, None, tx, true).await.unwrap();
    ...
    assert!(check_address_denied_by_config(&coin_deny_config, deny_address, &env.authority.get_object_store(), None,));
    // If no epoch is specified, we always read the latest value, and it should be denied.
    assert!(check_address_denied_by_config(&coin_deny_config, deny_address, &env.authority.get_object_store(), None,));
    ...
    // used.
    assert!(
        check_address_denied_by_config(&coin_deny_config, deny_address, &env.authority.get_object_store(), Some(1),)
    );
    // Check a different address, and it should not be denied.
    assert!(
        !check_address_denied_by_config(&coin_deny_config, dbg_addr(3), &env.authority.get_object_store(), Some(1),)
    );
    ...
    assert!(check_global_pause(&coin_deny_config, &env.authority.get_object_store(), None,));
    assert!(!check_global_pause(&coin_deny_config, &env.authority.get_object_store(), Some(0),));
    assert!(check_global_pause(&coin_deny_config, &env.authority.get_object_store(), Some(1),));
}
...
async fn test_regulated_coin_v2_funds_withdraw_deny() {
    ...
    env.authority.settle_transactions_for_testing(0, std::slice::from_ref(&env.publish_effects)).await;

    {
        ...
        .build_and_sign(&env.keypair);
        let effects = send_and_confirm_transaction_(&env.authority, None, tx, true).await.unwrap().1;
        assert!(effects.status().is_ok(), "Funding should succeed");
        env_gas_ref = effects.gas_object().0;

        env.authority.settle_transactions_for_testing(1, std::slice::from_ref(&effects)).await;
    }

    // Add the denied address to the regulated coin deny list.
    let add_tx =
        TestTransactionBuilder::new(env.sender, env_gas_ref, env.authority.reference_gas_price_for_testing().unwrap())
            .move_call_with_type_args(
                SUI_FRAMEWORK_PACKAGE_ID,
                "coin",
                "deny_list_v2_add",
            ...
            .build_and_sign(&env.keypair);
    send_and_confirm_transaction_(&env.authority, None, add_tx, true).await.unwrap();

    // Build the programmable transaction with a funds withdrawal argument for the denied coin.
    let mut builder = ProgrammableTransactionBuilder::new();
    let withdraw_arg = FundsWithdrawalArg::balance_from_sender(1, TypeInput::from(regulated_coin_type.clone()));
    ...
    match err.into_inner() {
        SuiErrorKind::UserInputError { error: UserInputError::AddressDeniedForCoin { address, coin_type } } => {
...
impl TestEnv {
    async fn get_latest_object_ref(&self, id: &ObjectID) -> ObjectRef {
        self.authority.get_object(id).await.unwrap().compute_object_reference()
    }
    ...
    async fn extract_v2_metadata(&self) -> V2Metadata {
        ...
        assert_eq!(regulated_metadata.deny_cap_object.bytes, deny_cap_object.id());
        assert_eq!(regulated_metadata.coin_metadata_object.bytes, metadata_object.id());
        V2Metadata { package_id, deny_cap_id: deny_cap_object.id() }
    }
}
...
async fn new_authority_and_publish(path: &str) -> TestEnv {
    ...
    .await;
    TestEnv { authority, sender, keypair, gas_object_id, publish_effects: effects.into_data() }
}
