...
async fn test_regulated_coin_v2_types() {
    ...
    .move_call(SUI_FRAMEWORK_PACKAGE_ID, "coin", "deny_list_v2_add", vec![
        CallArg::Object(ObjectArg::SharedObject {
            id: SUI_DENY_LIST_OBJECT_ID,
            initial_shared_version: deny_list_object_init_version,
            mutable: true,
        }),
        CallArg::Object(ObjectArg::ImmOrOwnedObject(deny_cap_object.compute_object_reference())),
    ...
    .move_call(SUI_FRAMEWORK_PACKAGE_ID, "coin", "deny_list_v2_enable_global_pause", vec![
        CallArg::Object(ObjectArg::SharedObject {
            id: SUI_DENY_LIST_OBJECT_ID,
            initial_shared_version: deny_list_object_init_version,
            mutable: true,
        }),
        CallArg::Object(ObjectArg::ImmOrOwnedObject(env.get_latest_object_ref(&deny_cap_object.id()).await)),
    ])
    .with_type_args(vec![regulated_coin_type.clone()])
    .build_and_sign(&env.keypair);
    let (_, effects) = send_and_confirm_transaction_(&env.authority, None, tx, true).await.unwrap();
