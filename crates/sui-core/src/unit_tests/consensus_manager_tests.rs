...
pub fn checkpoint_service_for_testing(state: Arc<AuthorityState>) -> Arc<CheckpointService> {
    ...
    let accumulator = Arc::new(GlobalStateHasher::new_for_tests(state.get_global_state_hash_store().clone()));
    ...
    checkpoint_service.spawn(epoch_store.clone(), None).now_or_never().unwrap();
    checkpoint_service
}
...
async fn test_consensus_manager() {
    ...
    let manager = ConsensusManager::new(config, consensus_config, &registry_service, consensus_client);
