...
async fn test_notify_read_fastpath_transaction_outputs() {
    ...
    let tx_data = TestTransactionBuilder::new(sender, random_object_ref(), 1).transfer_oct(None, sender).build();
    ...
    assert!(state.get_transaction_cache_reader().get_mysticeti_fastpath_outputs(&tx_digest).is_none());

    // Write fastpath transaction outputs
    state.get_cache_writer().write_fastpath_transaction_outputs(tx_outputs);

    // Verify that the transaction is marked as fastpath executed
    assert!(state.get_transaction_cache_reader().get_mysticeti_fastpath_outputs(&tx_digest).is_some());

    assert!(!state.get_transaction_cache_reader().is_tx_already_executed(&tx_digest));

    // Test notification and reading of fastpath transaction outputs
    let outputs = state.get_transaction_cache_reader().notify_read_fastpath_transaction_outputs(&[tx_digest]).await;
    ...
    let epoch_id = 0;
    state.get_cache_writer().write_transaction_outputs(epoch_id, outputs[0].clone());

    // Verify that the outputs are now available through regular transaction outputs
    let effects_digests =
        state.get_transaction_cache_reader().notify_read_executed_effects_digests("", &[tx_digest]).await;
...
async fn test_fast_path_execution() {
    ...
    let tx_data = TestTransactionBuilder::new(sender, gas_object_ref, state.reference_gas_price_for_testing().unwrap())
    ...
    let outputs = state.get_transaction_cache_reader().get_mysticeti_fastpath_outputs(&tx_digest).unwrap();
    ...
    let notify_read_task = tokio::spawn(async move {
        state_clone.get_transaction_cache_reader().notify_read_executed_effects_digests("", &[tx_digest]).await
    });

    state.get_cache_writer().write_transaction_outputs(0, outputs);
...
async fn test_fast_path_then_consensus_execution() {
    ...
    let tx_digest = *cert.digest();
    assert!(!state.epoch_store_for_testing().transactions_executed_in_cur_epoch(&[tx_digest]).unwrap().pop().unwrap());
...
async fn test_fast_path_then_consensus_execution_e2e() {
    ...
    state.execution_scheduler().enqueue_transactions(
        vec![(cert.clone(), ExecutionEnv::new().with_scheduling_source(SchedulingSource::MysticetiFastPath))],
        &state.epoch_store_for_testing(),
    );

    let outputs = tokio::time::timeout(
        std::time::Duration::from_secs(5),
        state.get_transaction_cache_reader().notify_read_fastpath_transaction_outputs(&[*cert.digest()]),
    )
    .await
    .unwrap()
    .pop()
    ...
    state.execution_scheduler().enqueue_transactions(
        vec![(cert.clone(), ExecutionEnv::new().with_scheduling_source(SchedulingSource::NonFastPath))],
        &state.epoch_store_for_testing(),
    );

    let effects_digest = tokio::time::timeout(
        std::time::Duration::from_secs(5),
        state.get_transaction_cache_reader().notify_read_executed_effects_digests("", &[tx_digest]),
    )
    .await
    .unwrap()
    .pop()
