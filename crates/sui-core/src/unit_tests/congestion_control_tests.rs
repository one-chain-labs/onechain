...
use sui_types::{
    ...
    transaction::{CertifiedTransaction, ObjectArg, SharedObjectMutability, VerifiedTransaction},
};

use crate::{
    authority::{
        authority_test_utils::certify_shared_obj_transaction_no_execution,
...
impl TestSetup {
    async fn new() -> Self {
        ...
        let max_accumulated_txn_cost_per_object_in_commit = TEST_ONLY_GAS_PRICE * TEST_ONLY_GAS_UNIT;
        ...
        setup_authority_state.insert_genesis_object(gas_object.clone()).await;
        ...
        Self { setup_authority_state, protocol_config, sender, sender_key, package, gas_object_id }
    }
    ...
    async fn create_genesis_objects_for_new_authority_state(&self, objects: &[ObjectID]) -> Vec<Object> {
        let mut genesis_objects = Vec::new();
        genesis_objects.push(TestSetup::convert_to_genesis_obj(
            self.setup_authority_state.get_object(&self.package.0).await.unwrap(),
        ));
        genesis_objects.push(TestSetup::convert_to_genesis_obj(
            self.setup_authority_state.get_object(&self.gas_object_id).await.unwrap(),
        ));

        for obj in objects {
            genesis_objects
                .push(TestSetup::convert_to_genesis_obj(self.setup_authority_state.get_object(obj).await.unwrap()));
        }
        genesis_objects
    }
}
...
async fn test_congestion_control_execution_cancellation() {
    ...
    let genesis_objects = test_setup
        .create_genesis_objects_for_new_authority_state(&[shared_object_1.0, shared_object_2.0, owned_object.0])
        .await;
    ...
    let authority_state = TestAuthorityBuilder::new()
        ...
        .build()
        .await;
    authority_state.insert_genesis_objects(&genesis_objects).await;
    let authority_state_2 = TestAuthorityBuilder::new()
        .with_reference_gas_price(TEST_ONLY_GAS_PRICE)
        .with_protocol_config(test_setup.protocol_config.clone())
        .build()
        .await;
    authority_state_2.insert_genesis_objects(&genesis_objects).await;
    ...
    let consensus_setup = consensus_test_utils::setup_consensus_handler_for_testing(&authority_state).await;
    ...
    let arg2 = txn_builder
        ...
        .unwrap();
    let owned_object_ref = authority_state.get_object(&owned_object.0).await.unwrap().compute_object_reference();
    let arg3 = txn_builder.obj(ObjectArg::ImmOrOwnedObject(owned_object_ref)).unwrap();
    move_call! {
        txn_builder,
    ...
    let response = authority_state_2.handle_transaction(&epoch_store_2, verified_tx_2.clone()).await.unwrap();
    ...
    let consensus_transactions =
        vec![ConsensusTransaction::new_certificate_message(&authority_state.name, cert.clone().into())];
    ...
    assert_eq!(scheduled_txns.len(), 2, "Expected prologue + cancelled transaction");
    ...
    let assigned_versions =
        assigned_tx_and_versions.into_map().get(&cert_key).expect("Transaction should have assigned versions").clone();

    let execution_env = ExecutionEnv::new().with_assigned_versions(assigned_versions);
    let (effects, execution_error) = authority_state.try_execute_for_test(&cert, execution_env).await;
    ...
    let assigned_versions = authority_state_2
        ...
        .unwrap();
    let execution_env = ExecutionEnv::new().with_assigned_versions(assigned_versions);
    let (effects_2, execution_error) = authority_state_2.try_execute_for_test(&cert, execution_env).await;
