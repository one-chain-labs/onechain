...
use crate::authority::{
    ...
    move_integration_tests::{
        ...
        build_multi_upgrade_txns,
        build_package,
        collect_packages_and_upgrade_caps,
        run_multi_txns,
        UpgradeData,
    },
...
fn build_upgrade_test_modules_with_overlay(base_pkg: &str, overlay: FileOverlay<'_>) -> (Vec<u8>, Vec<Vec<u8>>) {
    ...
    match overlay {
        ...
        FileOverlay::Add { file_name, contents } => {
...
pub fn build_upgrade_txn(
    ...
    builder.obj(ObjectArg::ImmOrOwnedObject(upgrade_cap)).unwrap();
...
impl UpgradeStateRunner {
    pub async fn new(base_package_name: &str) -> Self {
        ...
        Self { sender, sender_key, gas_object_id, authority_state, package, upgrade_cap, rgp }
    }

    pub async fn publish(&mut self, modules: Vec<Vec<u8>>, dep_ids: Vec<ObjectID>) -> (ObjectRef, ObjectRef) {
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
        ...
        let cap = effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::AddressOwner(_))).unwrap();
    ...
    pub async fn upgrade(
        ...
        let pt = {
            ...
            let cap = builder.obj(ObjectArg::ImmOrOwnedObject(self.upgrade_cap)).unwrap();
...
async fn test_upgrade_package_happy_path() {
    ...
    runner.upgrade(UpgradePolicy::COMPATIBLE, digest, modules, vec![]).await;
    ...
    assert!(normalized_modules["new_module"]
        .functions
        .contains_key(ident_str!("i_can_call_funs_in_other_modules_that_already_existed")));
...
async fn test_upgrade_introduces_type_then_uses_it() {
    ...
    let effects = runner
        .upgrade(UpgradePolicy::COMPATIBLE, digest, modules, vec![SUI_FRAMEWORK_PACKAGE_ID, MOVE_STDLIB_PACKAGE_ID])
        .await;
    ...
    let b = runner.authority_state.get_object_store().get_object_by_key(&created.0, created.1).unwrap();
...
async fn test_upgrade_package_incorrect_digest() {
    ...
    let effects = runner.upgrade(UpgradePolicy::COMPATIBLE, bad_digest, modules, vec![]).await;
...
async fn test_upgrade_package_compatibility_too_permissive() {
    ...
    assert!(matches!(effects.into_status().unwrap_err().0, ExecutionFailureStatus::MoveAbort(_, 1)));
}
...
async fn test_upgrade_package_compatible_in_dep_only_mode() {
    ...
    let (digest, modules) = build_upgrade_test_modules("stage1_basic_compatibility_valid");
    let effects = runner.upgrade(UpgradePolicy::DEP_ONLY, digest, modules, vec![]).await;
...
async fn test_invalid_dep_only_upgrades() {
    ...
    assert_valid_dep_only_upgrade(&mut runner, base_pkg).await;
    let overlays = [
        FileOverlay::Add { file_name: "new_module.move", contents: "module base_addr::new_module;" },
        FileOverlay::Add {
            file_name: "new_friend_module.move",
            contents: r#"
module base_addr::new_friend_module; 
...
async fn test_upgrade_package_compatible_in_additive_mode() {
    ...
    let (digest, modules) = build_upgrade_test_modules("stage1_basic_compatibility_valid");
    let effects = runner.upgrade(UpgradePolicy::ADDITIVE, digest, modules, vec![]).await;
...
async fn test_upgrade_package_not_a_ticket() {
    ...
    assert_eq!(effects.into_status().unwrap_err().0, ExecutionFailureStatus::CommandArgumentError {
        arg_idx: 0,
...
async fn test_upgrade_ticket_doesnt_match() {
    ...
    let pt = {
        ...
        let (digest, modules) = build_upgrade_test_modules("stage1_basic_compatibility_valid");
        // We take as input the upgrade runner.upgrade_cap
        builder.obj(ObjectArg::ImmOrOwnedObject(runner.upgrade_cap)).unwrap();
        // Create the upgrade ticket
        let upgrade_arg = builder.pure(UpgradePolicy::COMPATIBLE).unwrap();
        let digest_arg = builder.pure(digest).unwrap();
        let upgrade_ticket = move_call! {
...
async fn test_multiple_upgrades(
    ...
    let package_v2 = effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::Immutable)).unwrap().0 .0;
    ...
    let effects = runner
        .upgrade(
            ...
            digest,
            modules,
            if use_empty_deps { vec![] } else { vec![SUI_FRAMEWORK_PACKAGE_ID, MOVE_STDLIB_PACKAGE_ID] },
        )
        .await;
    (package_v2, effects)
}
...
async fn test_interleaved_upgrades() {
    ...
    let dep_v2_package = effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::Immutable)).unwrap().0;
    ...
    let pt2 = {
        ...
        // We take as input the upgrade cap
        builder.obj(ObjectArg::ImmOrOwnedObject(depender_cap)).unwrap();
...
async fn test_publish_override_happy_path() {
    ...
    //                   base(v2)
    let pt1 = build_upgrade_txn(runner.package.0, "stage1_basic_compatibility_valid", runner.upgrade_cap);
    ...
    let (_, modules, dep_ids) = build_upgrade_test_modules_with_dep_addr(
        "dep_on_dep",
        [("base_addr", dep_v2_package.0), ("dep_on_upgrading_package", depender_package.0)],
        [("package_upgrade_base", dep_v2_package.0), ("dep_on_upgrading_package", depender_package.0)],
    );

    let (new_package, _) = runner.publish(modules, dep_ids).await;

    let package = runner.authority_state.get_object_cache_reader().get_package_object(&new_package.0).unwrap().unwrap();

    // Make sure the linkage table points to the correct versions!
    let dep_ids_in_linkage_table: BTreeSet<_> =
        package.move_package().linkage_table().values().map(|up| up.upgraded_id).collect();
...
async fn test_publish_transitive_happy_path() {
    ...
    let (_, module_bytes, dep_ids) = build_upgrade_test_modules_with_dep_addr(
        ...
        [("base_addr", runner.package.0), ("dep_on_upgrading_package", ObjectID::ZERO)],
        [("package_upgrade_base", runner.package.0)],
    );
    // Dependency graph: base <-- dep_on_upgrading_package
    let (depender_package, _) = runner.publish(module_bytes, dep_ids).await;
    ...
    let (_, root_module_bytes, root_dep_ids) = build_upgrade_test_modules_with_dep_addr(
        ...
        [("base_addr", runner.package.0), ("dep_on_upgrading_package", depender_package.0)],
        [("package_upgrade_base", runner.package.0), ("dep_on_upgrading_package", depender_package.0)],
    );
    // Dependency graph: base(v1)  <-- dep_on_upgrading_package
    ...
    let root_move_package =
        runner.authority_state.get_object_cache_reader().get_package_object(&root_package.0).unwrap().unwrap();

    // Make sure the linkage table points to the correct versions!
    let dep_ids_in_linkage_table: BTreeSet<_> =
        root_move_package.move_package().linkage_table().values().map(|up| up.upgraded_id).collect();
...
async fn test_publish_transitive_override_happy_path() {
    ...
    let base_v2_package = effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::Immutable)).unwrap().0;
    ...
    let (_, root_module_bytes, root_dep_ids) = build_upgrade_test_modules_with_dep_addr(
        "dep_on_upgrading_package_transitive",
        [("base_addr", runner.package.0), ("dep_on_upgrading_package", depender_package.0)],
        [("package_upgrade_base", base_v2_package.0), ("dep_on_upgrading_package", depender_package.0)],
    );
    // Dependency graph: base(v1)  <-- dep_on_upgrading_package
...
async fn test_different_versions_across_calls() {
    ...
    let package_v3 = effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::Immutable)).unwrap().0 .0;
...
async fn test_conflicting_versions_across_calls() {
    ...
    let dependent_v2_package =
        effects.created().into_iter().find(|(_, owner)| matches!(owner, Owner::Immutable)).unwrap().0;
...
async fn test_upgrade_max_packages() {
    ...
    let max_pub_cmd =
        authority.epoch_store_for_testing().protocol_config().max_publish_or_upgrade_per_ptb_as_option().unwrap_or(0);
    ...
    let result = run_multi_txns(&authority, sender, &sender_key, &gas_object_id, builder).await.unwrap().1;
