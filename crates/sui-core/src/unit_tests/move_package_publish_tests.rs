...
async fn test_publishing_with_unpublished_deps() {
    ...
    let ObjectRead::Exists(read_ref, package_obj, _) = authority.get_object_read(&package.0).unwrap() else {
        panic!("Can't read package")
    };

    assert_eq!(package, read_ref);
    let Data::Package(move_package) = package_obj.into_inner().data else { panic!("Not a package") };

    // Check that the published package includes its depended upon module.
    assert_eq!(
        move_package.serialized_module_map().keys().map(String::as_str).collect::<HashSet<_>>(),
        HashSet::from(["depends_on_basics", "object_basics"]),
    );

    let effects =
        call_move(&authority, &gas, &sender, &sender_key, &package.0, "depends_on_basics", "delegate", vec![], vec![])
            .await
            .unwrap();
...
async fn test_publish_empty_package() {
    ...
    let transaction = to_sender_signed_transaction(data, &sender_key);
    let result = send_and_confirm_transaction(&authority, transaction).await.unwrap().1;
...
async fn test_custom_property_parse_published_at() {
    ...
    build_config.build(&path).expect("Move package did not build");
    let manifest = manifest_parser::parse_move_manifest_from_file(path.as_path()).expect("Could not parse Move.toml");
    let properties =
        manifest.package.custom_properties.iter().map(|(k, v)| (k.to_string(), v.to_string())).collect::<Vec<_>>();
...
async fn test_custom_property_check_unpublished_dependencies() {
    let build_config = BuildConfig::new_for_testing();
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.extend(["src", "unit_tests", "data", "custom_properties_in_manifest_ensure_published_at"]);
...
async fn test_publish_extraneous_bytes_modules() {
    ...
    let correct_modules = build_test_package("object_owner", /* with_unpublished_deps */ false);
    ...
    let new_bytes = {
        ...
        m.metadata.push(move_core_types::metadata::Metadata { key: vec![0], value: vec![1] });
        let mut buf = vec![];
