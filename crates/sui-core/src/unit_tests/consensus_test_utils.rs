...
use sui_types::{
    ...
    messages_consensus::{AuthorityIndex, ConsensusPosition, ConsensusTransaction, ConsensusTransactionKind},
...
use crate::{
    authority::{
        authority_per_epoch_store::{AuthorityPerEpochStore, ExecutionIndicesWithStats},
...
pub(crate) type CapturedTransactions = Arc<Mutex<Vec<(Vec<Schedulable>, AssignedTxAndVersions, SchedulingSource)>>>;
...
impl TestConsensusCommit {
    pub fn new(transactions: Vec<ConsensusTransaction>, round: u64, timestamp_ms: u64, sub_dag_index: u64) -> Self {
        Self { transactions, round, timestamp_ms, sub_dag_index }
    }

    pub fn empty(round: u64, timestamp_ms: u64, sub_dag_index: u64) -> Self {
        Self { transactions: vec![], round, timestamp_ms, sub_dag_index }
    }
}
...
impl ConsensusCommitAPI for TestConsensusCommit {
    ...
    fn transactions(&self) -> Vec<(BlockRef, Vec<ParsedTransaction>)> {
        ...
        let parsed_txs: Vec<ParsedTransaction> = self
            .transactions
            .iter()
            .map(|tx| ParsedTransaction { transaction: tx.clone(), rejected: false, serialized_len: 0 })
            .collect();
...
pub fn make_consensus_adapter_for_test(
    ...
    impl ConsensusClient for SubmitDirectly {
        async fn submit(
            ...
            let keys = sequenced_transactions.iter().map(|tx| tx.key()).collect::<Vec<_>>();
                ...
                for tx in sequenced_transactions {
                    if let Some(transaction_digest) = tx.transaction.executable_transaction_digest() {
                        ...
                        let executable_tx = match &tx.transaction {
                            ...
                            SequencedConsensusTransactionKind::System(sys_tx) => Some(sys_tx.clone()),
                        };
                        ...
                        if let Some(exec_tx) = executable_tx {
                            ...
                            let assigned_version =
                                versions.into_map().into_iter().next().map(|(_, v)| v).unwrap_or_default();
            ...
            Ok((consensus_positions, self.mock_block_status_receivers.lock().remove(0)))
        }
    }
    ...
    C: Send + Sync + 'static,
{
    ...
    let consensus_adapter = make_consensus_adapter_for_test(authority.clone(), HashSet::new(), false, vec![]);

    let last_consensus_stats = ExecutionIndicesWithStats {
        stats: crate::authority::authority_per_epoch_store::ConsensusStats::new(consensus_committee.size()),
        ..Default::default()
    };

    // Create a test ExecutionSchedulerSender that captures transactions
    let captured_transactions =
        Arc::new(Mutex::new(Vec::<(Vec<Schedulable>, AssignedTxAndVersions, SchedulingSource)>::new()));
    let captured_tx_clone = captured_transactions.clone();

    // Create a channel to capture sent transactions
    let (tx_sender, mut receiver) = mysten_metrics::monitored_mpsc::unbounded_channel("test_execution_scheduler");
