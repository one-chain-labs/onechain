...
async fn test_authority_txn_signing_pushback() {
    ...
    let error: SuiError = validator_service.handle_transaction_for_benchmarking(tx.clone()).await.err().unwrap().into();
    assert!(matches!(error.into_inner(), SuiErrorKind::ValidatorOverloadedRetryAfter { .. }));
    ...
    let error: SuiError = validator_service.handle_transaction_for_benchmarking(tx2).await.err().unwrap().into();
    assert!(matches!(error.into_inner(), SuiErrorKind::ObjectLockConflict { .. }));
...
async fn test_authority_txn_execution_pushback() {
    ...
    let error: SuiError = validator_service.execute_certificate_for_testing(cert.clone()).await.err().unwrap().into();
    assert!(matches!(error.into_inner(), SuiErrorKind::ValidatorOverloadedRetryAfter { .. }));
