    ...
    impl ConsensusClient for SubmitDirectly {
        async fn submit(
            ...
            let transactions = transactions
                .into_iter()
                .map(|tx| {
                    let assigned_versions = assigned_versions.get(&tx.key()).cloned().unwrap_or_default();
                    (tx, ExecutionEnv::new().with_assigned_versions(assigned_versions))
                })
                .collect();
...
async fn submit_transaction_to_consensus_adapter() {
    ...
    let waiter = adapter
        .submit(transaction.clone(), Some(&epoch_store.get_reconfig_state_read_lock_guard()), &epoch_store, None, None)
        .unwrap();
    waiter.await.unwrap();
}
...
async fn submit_multiple_transactions_to_consensus_adapter() {
    ...
    let waiter = adapter
        .submit_batch(&transactions, Some(&epoch_store.get_reconfig_state_read_lock_guard()), &epoch_store, None, None)
        .unwrap();
    waiter.await.unwrap();
}
...
async fn submit_checkpoint_signature_to_consensus_adapter() {
    ...
    // Make a new consensus adapter instance.
    let adapter = make_consensus_adapter_for_test(state.clone(), HashSet::new(), false, vec![with_block_status(
        BlockStatus::Sequenced(BlockRef::MIN),
    )]);
    ...
    let signed_checkpoint_summary =
        SignedCheckpointSummary::new(committee.epoch, checkpoint_summary.clone(), authority_key, authority);
    ...
    let t1 = tokio::spawn({
        ...
        async move {
            ...
            sleep(delay).await;
            state.checkpoint_store.insert_verified_checkpoint(&verified_checkpoint_summary).unwrap();
            state.checkpoint_store.update_highest_synced_checkpoint(&verified_checkpoint_summary).unwrap();
        }
    });
...
async fn submit_empty_array_of_transactions_to_consensus_adapter() {
    ...
    assert_eq!(consensus_position, vec![ConsensusPosition {
        epoch: epoch_store.epoch(),
        block: BlockRef::MIN,
