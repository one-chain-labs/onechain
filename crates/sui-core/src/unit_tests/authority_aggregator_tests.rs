...
use crate::{
    ...
    test_authority_clients::{
        HandleTransactionTestAuthorityClient,
        LocalAuthorityClient,
        LocalAuthorityClientFaultConfig,
        MockAuthorityApi,
    },
    test_utils::{init_local_authorities, make_transfer_object_transaction, make_transfer_oct_transaction},
};
...
pub fn set_local_client_config(
    ...
    clients_values_mut.next().unwrap().authority_client_mut().fault_config = config;
...
pub fn set_object_move_transaction(
    ...
    let args =
        vec![CallArg::Object(ObjectArg::ImmOrOwnedObject(object_ref)), CallArg::Pure(bcs::to_bytes(&value).unwrap())];
...
{
    authority.handle_transaction(transaction.clone(), Some(make_socket_addr())).await.unwrap();
}
    ...
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    for authority in authorities {
        let response = authority
            .handle_transaction_info_request(TransactionInfoRequest { transaction_digest: *transaction_digest })
            .await;
        match response {
            Ok(PlainTransactionInfoResponse::Signed(signed)) => {
                ...
                if let Some(inner_transaction) = tx_data {
                    assert_eq!(inner_transaction.intent_message().value, data.intent_message().value);
                }
                tx_data = Some(data);
            }
...
pub async fn do_cert<A>(authority: &SafeClient<A>, cert: &CertifiedTransaction) -> TransactionEffects
where
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    authority.handle_certificate_v2(cert.clone(), Some(make_socket_addr())).await.unwrap().signed_effects.into_data()
}
...
where
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    let result = authority.handle_certificate_v2(cert.clone(), Some(make_socket_addr())).await;
    if result.is_err() {
...
async fn execute_transaction_with_fault_configs(
    ...
    authorities.process_certificate(request, Some(client_ip)).await.is_ok()
}
...
fn reference_gas_price(authorities: &AuthorityAggregator<LocalAuthorityClient>) -> u64 {
    authorities
        .authority_clients
        .values()
        .find_map(|client| client.authority_client().state.reference_gas_price_for_testing().ok())
        .unwrap()
}
...
async fn test_quorum_map_and_reduce_timeout() {
    ...
    let client_ip = make_socket_addr();
    let modules: Vec<_> = build_config.build(&path).unwrap().get_modules().cloned().collect();
    ...
    let certified_tx = authorities.process_transaction(tx.clone(), Some(client_ip)).await;
    ...
    let certified_effects = authorities.process_certificate(request, Some(client_ip)).await;
    // Ensure it is an error
    assert!(certified_effects.is_err());
    assert!(matches!(certified_effects, Err(AggregatorProcessCertificateError::RetryableExecuteCertificate { .. })));
    tokio::time::sleep(tokio::time::Duration::from_secs(10)).await;
    let tx_info = TransactionInfoRequest { transaction_digest: *tx.digest() };
    for (_, client) in authorities.authority_clients.iter() {
        let resp = client.handle_transaction_info_request(tx_info.clone()).await;
        ...
        |_name, _client| {
            Box::pin(async move {
                let res: Result<usize, SuiError> =
                    Err(SuiError::TooManyIncorrectAuthorities { errors: vec![], action: "".to_string() });
                res
            })
        },
        |mut accumulated_state, _authority_name, _authority_weight, result| {
            Box::pin(async move {
                assert!(matches!(result, Err(SuiError::TooManyIncorrectAuthorities { .. })));
                accumulated_state += 1;
...
async fn test_process_transaction_fault_fail() {
    ...
    let fail_before_process_transaction_config =
        LocalAuthorityClientFaultConfig { fail_before_handle_transaction: true, ..Default::default() };
    assert!(
        !execute_transaction_with_fault_configs(
            &[(0, fail_before_process_transaction_config), (1, fail_before_process_transaction_config),],
            &[],
        )
        .await
    );
...
async fn test_process_certificate_fault_fail() {
    ...
    let fail_before_process_certificate_config =
        LocalAuthorityClientFaultConfig { fail_before_handle_confirmation: true, ..Default::default() };
    assert!(
        !execute_transaction_with_fault_configs(&[], &[
        ...
        agg.quorum_once_with_timeout(
            ...
            |_name, client| {
                ...
                Box::pin(async move {
                    ...
                    let res = client
                        .handle_transaction_info_request(TransactionInfoRequest { transaction_digest: digest })
                        .await;
                    match res {
                        Ok(_) => Ok(()),
...
fn get_authorities(
    count: Arc<Mutex<u32>>,
    committee_size: u64,
) -> (BTreeMap<AuthorityName, StakeUnit>, Vec<(AuthorityName, StakeUnit)>, BTreeMap<AuthorityName, MockAuthorityApi>) {
...
fn get_genesis_agg<A: Clone>(
    ...
    let timeouts_config =
        TimeoutConfig { serial_authority_request_interval: Duration::from_millis(50), ..Default::default() };
    ...
    A: Clone,
{
    ...
    agg.committee_store.insert_new_committee(&committee).unwrap();
...
async fn test_handle_transaction_fork() {
    ...
    let cert_epoch_0 = agg.process_transaction(tx.clone(), Some(client_ip)).await.unwrap().into_cert_for_testing();

    // Validators 2 and 3 return successful effects
    let effects = effects_with_tx(*cert_epoch_0.digest());
    set_tx_info_response_with_cert_and_effects(&mut clients, authority_keys.iter(), Some(&cert_epoch_0), effects, 1);

    // Validator 0 and 1 return failed effects
    let effects = TestEffectsBuilder::new(cert_epoch_0.data())
        .with_status(ExecutionStatus::Failure { error: ExecutionFailureStatus::InsufficientGas, command: None })
        .build();
    ...
    let err = agg.process_transaction(tx.clone(), Some(client_ip)).await.unwrap_err();
    assert!(matches!(err, AggregatorProcessTransactionError::FatalTransaction { .. }));
}
#[tokio::test]
async fn test_handle_certificate_response() {
    ...
    let cert_epoch_0 =
        agg.process_transaction(tx.clone().into(), Some(client_ip)).await.unwrap().into_cert_for_testing();
    ...
    let committee_1 = Committee::new_for_testing_with_normalized_voting_power(1, authorities.clone());
    agg.committee_store.insert_new_committee(&committee_1).unwrap();
    ...
    let err = agg.process_certificate(request, Some(client_ip)).await.unwrap_err();
    assert_matches!(
        err,
...
async fn test_handle_transaction_response() {
    ...
    let package_not_found_error =
        SuiError::UserInputError { error: UserInputError::DependentPackageNotFound { package_id: gas_object.0 } };
    let object_not_found_error = SuiError::UserInputError {
        error: UserInputError::ObjectNotFound { object_id: gas_object.0, version: Some(gas_object.1) },
    };
    ...
    assert_resp_err(
        &agg,
        tx.clone().into(),
        |e| matches!(e, AggregatorProcessTransactionError::FatalTransaction { .. }),
    ...
    clients.get_mut(&authority_keys[0].0).unwrap().set_tx_info_response(resp);
    ...
    agg.process_transaction(tx.clone().into(), Some(client_ip)).await.unwrap();
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                SuiError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } | SuiError::RpcError(..)
            )
        },
    )
    .await;
    ...
    let resp = HandleTransactionResponse {
        status: TransactionStatus::Executed(
            None,
            SignedTransactionEffects::new(1, effects.clone(), &authority_keys[1].1, authority_keys[1].0),
            TransactionEvents { data: vec![] },
        ),
    };
    clients.get_mut(&authority_keys[1].0).unwrap().set_tx_info_response(resp);

    // Validators 3 returns different tx-effects without cert for epoch 1 (simulating byzantine behavior)
    let effects = TestEffectsBuilder::new(cert_epoch_0.data())
        .with_status(ExecutionStatus::Failure { error: ExecutionFailureStatus::InvalidGasObject, command: None })
        .build();

    let resp = HandleTransactionResponse {
        status: TransactionStatus::Executed(
            None,
            SignedTransactionEffects::new(1, effects.clone(), &authority_keys[2].1, authority_keys[2].0),
            TransactionEvents { data: vec![] },
        ),
    };
    clients.get_mut(&authority_keys[2].0).unwrap().set_tx_info_response(resp);
    ...
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    let cert_epoch_0_2 =
        agg.process_transaction(tx2.clone().into(), Some(client_ip)).await.unwrap().into_cert_for_testing();
    ...
    set_tx_info_response_with_cert_and_effects(&mut clients, authority_keys.iter(), Some(&cert_epoch_1), effects, 1);
    let mut agg = get_genesis_agg(authorities.clone(), clients.clone());
    ...
    for (name, _) in authority_keys.iter().skip(2) {
        clients.get_mut(name).unwrap().set_tx_info_response_error(object_not_found_error.clone());
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
    ...
    for (name, _) in authority_keys.iter().skip(2) {
        clients.get_mut(name).unwrap().set_tx_info_response_error(package_not_found_error.clone());
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
    ...
    clients.get_mut(&authority_keys[2].0).unwrap().set_tx_info_response_error(package_not_found_error.clone());
    clients.get_mut(&authority_keys[3].0).unwrap().set_tx_info_response_error(object_not_found_error.clone());
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
        tx.clone().into(),
    ...
    for (name, _) in authority_keys.iter().skip(2) {
        clients.get_mut(name).unwrap().set_tx_info_response_error(SuiError::EpochEnded(0));
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
    ...
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    let cert = agg.process_transaction(tx.clone().into(), Some(client_ip)).await.unwrap();
    ...
    // < 2f+1 object + package not found errors
    clients.get_mut(&authority_keys[1].0).unwrap().set_tx_info_response_error(object_not_found_error.clone());
    clients.get_mut(&authority_keys[2].0).unwrap().set_tx_info_response_error(package_not_found_error.clone());
    clients.get_mut(&authority_keys[3].0).unwrap().set_tx_info_response_error(object_not_found_error.clone());
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let conflicting_error =
        SuiError::ObjectLockConflict { obj_ref: conflicting_object, pending_transaction: *conflicting_tx2.digest() };
    let retryable_error = SuiError::RpcError("RPC".into(), "Error".into());
    let non_retryable_error =
        SuiError::ByzantineAuthoritySuspicion { authority: authority_keys[0].0, reason: "Faulty".into() };
    let object_not_found_error = SuiError::UserInputError {
        error: UserInputError::ObjectNotFound { object_id: conflicting_object.0, version: Some(conflicting_object.1) },
    };
    ...
    clients.get_mut(&authority_keys[0].0).unwrap().set_tx_info_response_error(conflicting_error.clone());
    // Val 1 returns retryable error.
    clients.get_mut(&authority_keys[1].0).unwrap().set_tx_info_response_error(retryable_error.clone());
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
        tx1.clone().into(),
    ...
    println!("Case 1 - Retryable Transaction, state is still retryable so ignore conflicting transaction");
    // Only Val 3 returns conflicting Tx2 and Tx1 is still in retryable state.
    set_retryable_tx_info_response_error(&mut clients, &authority_keys);
    for (name, _) in authority_keys.iter().skip(3) {
        clients.get_mut(name).unwrap().set_tx_info_response_error(conflicting_error.clone());
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
    ...
    let conflicting_error_2 =
        SuiError::ObjectLockConflict { obj_ref: conflicting_object, pending_transaction: *conflicting_tx3.digest() };
    clients.get_mut(&authority_keys[2].0).unwrap().set_tx_info_response_error(conflicting_error_2.clone());
    // Validator 4 returns a nonretryable error
    clients.get_mut(&authority_keys[3].0).unwrap().set_tx_info_response_error(non_retryable_error.clone());
    ...
    clients.get_mut(&authority_keys[2].0).unwrap().set_tx_info_response_error(non_retryable_error.clone());
    // Validator 4 returns a ObjectNotFound error
    clients.get_mut(&authority_keys[3].0).unwrap().set_tx_info_response_error(object_not_found_error.clone());
    ...
    let client_ip = make_socket_addr();
    let cert_epoch_0 =
        agg.process_transaction(tx1.clone().into(), Some(client_ip)).await.unwrap().into_cert_for_testing();

    // Val-1 returns conflicting tx2; Val2-3 returns retryable error
    set_retryable_tx_info_response_error(&mut clients, &authority_keys);
    let (name_1, _) = &authority_keys[1];
    clients.get_mut(name_1).unwrap().set_tx_info_response_error(conflicting_error.clone());
    ...
    set_tx_info_response_with_cert_and_effects(&mut clients, authority_keys.iter(), None, effects, 2);
...
async fn test_handle_overload_response() {
    ...
    let overload_error = SuiError::TooManyTransactionsPendingExecution { queue_len: 100, threshold: 100 };
    ...
    assert_resp_err(
        ...
        |e| matches!(e, SuiError::TooManyTransactionsPendingExecution { .. } | SuiError::RpcError(..)),
    )
    .await;
    ...
    // should be returned.
    clients.get_mut(&authority_keys[1].0).unwrap().set_tx_info_response_error(rpc_error);
...
async fn test_handle_overload_retry_response() {
    ...
    for (index, (name, _)) in authority_keys.iter().enumerate() {
        clients
            .get_mut(name)
            .unwrap()
            .set_tx_info_response_error(SuiError::ValidatorOverloadedRetryAfter { retry_after_secs: index as u64 });
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    ...
    assert_resp_err(
        ...
        |e| matches!(e, SuiError::ValidatorOverloadedRetryAfter { .. } | SuiError::RpcError(..)),
    )
    .await;
    ...
    clients.get_mut(&authority_keys[0].0).unwrap().set_tx_info_response_error(rpc_error.clone());
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
        txn.clone(),
...
async fn test_early_exit_with_too_many_conflicts() {
    ...
    set_tx_info_response_with_error(&mut clients, authority_keys.iter().take(1), SuiError::ObjectLockConflict {
        obj_ref: random_object_ref(),
        pending_transaction: TransactionDigest::random(),
    });
    set_tx_info_response_with_error(&mut clients, authority_keys.iter().skip(1).take(1), SuiError::ObjectLockConflict {
        obj_ref: random_object_ref(),
        pending_transaction: TransactionDigest::random(),
    });
    set_tx_info_response_with_error(&mut clients, authority_keys.iter().skip(2).take(1), SuiError::ObjectLockConflict {
        obj_ref: random_object_ref(),
        pending_transaction: TransactionDigest::random(),
    });
    set_tx_info_response_with_error(
        ...
        SuiError::TooManyTransactionsPendingExecution { queue_len: 100, threshold: 100 },
    );
...
async fn test_process_transaction_again() {
    ...
    let cert = agg.process_transaction(tx.clone(), client_ip).await.unwrap();
    let certificate = match cert {
        ProcessTransactionResult::Certified { certificate, newly_formed } => {
            assert!(newly_formed);
            certificate
        }
        _ => {
...
async fn run_aggregator(
    ...
    for (name, secret) in &authority_keys {
        ...
        let resp = HandleTransactionResponse { status: TransactionStatus::Signed(auth_signature) };
        clients.get_mut(name).unwrap().set_tx_info_response(resp);
    }

    let agg = get_agg_at_epoch(authorities.clone(), clients.clone(), 0);
    agg.process_transaction(tx.clone(), Some(make_socket_addr())).await
}
...
async fn process_with_cert(
    ...
    let agg = get_agg_at_epoch(authorities.clone(), clients.clone(), 0);
    let client_ip = make_socket_addr();
    let cert = agg.process_transaction(tx.clone(), Some(client_ip)).await.unwrap().into_cert_for_testing();
    ...
    for (name, secret) in &authority_keys {
        let auth_signature = if byzantines.contains(name) {
            ...
            AuthoritySignInfo::new(0, &effects.clone(), Intent::sui_app(IntentScope::TransactionEffects), *name, secret)
        };
...
fn set_tx_info_response_with_signed_tx(
    ...
    for (name, secret) in authority_keys {
        ...
        let resp = HandleTransactionResponse { status: TransactionStatus::Signed(signed_tx.into_sig()) };
        clients.get_mut(name).unwrap().set_tx_info_response(resp);
    }
}
...
fn set_tx_info_response_with_error<'a>(
    ...
    for (name, _) in authority_keys {
        clients.get_mut(name).unwrap().set_tx_info_response_error(error.clone());
    }
}
