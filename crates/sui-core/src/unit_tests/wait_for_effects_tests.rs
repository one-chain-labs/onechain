...
use sui_types::{
    ...
    messages_grpc::{PingType, WaitForEffectsRequest, WaitForEffectsResponse},
    object::Object,
...
use crate::{
    authority::{
        consensus_tx_status_cache::{ConsensusTxStatus, CONSENSUS_STATUS_RETENTION_ROUNDS},
...
impl TestContext {
    async fn new() -> Self {
        ...
        let server_handle = AuthorityServer::new_for_test(state.clone()).spawn_for_test().await.unwrap();
        ...
        Self { state, _server_handle: server_handle, client, sender, keypair, gas_object_ref }
    }
...
async fn test_wait_for_effects_position_mismatch() {
    ...
    let tx_position1 = ConsensusPosition { epoch: EpochId::MIN, block: BlockRef::MIN, index: TransactionIndex::MIN };
    let tx_position2 = ConsensusPosition { epoch: EpochId::MIN, block: BlockRef::MIN, index: TransactionIndex::MIN + 1 };
...
async fn test_wait_for_effects_consensus_rejected_validator_accepted() {
    ...
    let transaction = test_context.build_test_transaction();
    let tx_digest = *transaction.digest();
    let tx_position = ConsensusPosition { epoch: EpochId::MIN, block: BlockRef::MIN, index: TransactionIndex::MIN };
    ...
    let response = test_context.client.wait_for_effects(request, None).await.unwrap();
...
async fn test_wait_for_effects_consensus_rejected_validator_rejected() {
    ...
    tokio::spawn(async move {
        ...
        let epoch_store = state_clone.epoch_store_for_testing();
        epoch_store.set_consensus_tx_status(tx_position, ConsensusTxStatus::Rejected);
        epoch_store.set_rejection_vote_reason(tx_position, &SuiError::UserInputError {
            error: UserInputError::TransactionDenied { error: "object denied".to_string() },
        });
    });
...
async fn test_wait_for_effects_fastpath_certified_only() {
    ...
    match response {
        WaitForEffectsResponse::Executed { details, effects_digest, fast_path: _ } => {
...
async fn test_wait_for_effects_expired() {
    ...
    let tx_position = ConsensusPosition {
        epoch: EpochId::MIN,
        block: BlockRef { round: block_round, ..BlockRef::MIN },
    ...
    tokio::spawn(async move {
        ...
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + block_round).await;

        // Update that will actually trigger expiration using the leader round, CONSENSUS_STATUS_RETENTION_ROUNDS + block_round
        cache.update_last_committed_leader_round(CONSENSUS_STATUS_RETENTION_ROUNDS + block_round + 1).await;
...
async fn test_wait_for_effects_ping() {
    ...
    {
        let tx_position = ConsensusPosition { epoch: EpochId::MIN, block: BlockRef::MIN, index: PING_TRANSACTION_INDEX };
    ...
    println!("Case 2. Send a Consensus ping request. The end point should wait for the transaction is finalised via Consensus.");
    {
        ...
        block.round = 5;
        let tx_position = ConsensusPosition { epoch: EpochId::MIN, block, index: TransactionIndex::MIN };
        ...
        match response {
            WaitForEffectsResponse::Executed { effects_digest, details, fast_path } => {
                assert!(details.is_none());
                assert_eq!(effects_digest, TransactionEffectsDigest::ZERO);
                assert!(!fast_path, "This is Consensus ping request, so fast_path should be false");
            }
    ...
    );
    {
        ...
        tokio::spawn(async move {
            ...
            consensus_tx_status_cache.update_last_committed_leader_round(leader_round).await;
            // The second time we update the last committed leader round will kick of a clean up - the first one doesn't.
            consensus_tx_status_cache.update_last_committed_leader_round(leader_round + 1).await;
        });
