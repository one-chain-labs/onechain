...
use crate::{
    ...
    wait_for_effects_request::{ConsensusTxPosition, RejectReason, WaitForEffectsRequest, WaitForEffectsResponse},
};
...
async fn test_wait_for_effects_position_mismatch() {
    ...
    let tx_position1 = ConsensusTxPosition { block: BlockRef::MIN, index: TransactionIndex::MIN };
    let tx_position2 = ConsensusTxPosition { block: BlockRef::MIN, index: TransactionIndex::MIN + 1 };
    ...
    tokio::spawn(async move {
        ...
        state_clone.try_execute_immediately(&transaction, None, &epoch_store).await.unwrap().0
    });
...
async fn test_wait_for_effects_post_commit_rejected() {
    ...
    let transaction = test_context.build_test_transaction();
    let tx_digest = *transaction.digest();
    let tx_position = ConsensusTxPosition { block: BlockRef::MIN, index: TransactionIndex::MIN };
    ...
    let response = test_context.client.wait_for_effects(request, None).await.unwrap().try_into().unwrap();
...
async fn test_wait_for_effects_fastpath_certified() {
    ...
    match response {
        WaitForEffectsResponse::Executed { details, effects_digest } => {
