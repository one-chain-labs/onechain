...
async fn test_notify_read_executed_transactions_to_checkpoint() {
    ...
    let txes_to_be_notified =
        vec![TransactionDigest::random(), TransactionDigest::random(), TransactionDigest::random()];

    // Insert only the first transaction already
    store
        .insert_finalized_transactions(vec![txes_to_be_notified[0]].as_slice(), checkpoint_sequence_1)
    ...
    let result = store.multi_get_transaction_checkpoint(txes_to_be_notified.as_slice()).expect("Should not fail");
