...
async fn test_pay_sui_success_one_input_coin() -> anyhow::Result<()> {
    ...
    assert_eq!(GasCoin::try_from(&gas_object)?.value(), coin_amount - 100 - 200 - 300 - gas_used,);
...
async fn test_pay_sui_success_multiple_input_coins() -> anyhow::Result<()> {
    ...
    .await;
    let recipient_amount_map: HashMap<_, u64> = HashMap::from([(recipient1, 500), (recipient2, 1500)]);
    let effects = res.txn_result.unwrap().into_data();
    assert_eq!(*effects.status(), ExecutionStatus::Success);
    ...
    assert_eq!(GasCoin::try_from(&gas_object)?.value(), 5002000 - 500 - 1500 - gas_used,);
...
async fn test_pay_all_sui_success_one_input_coin() -> anyhow::Result<()> {
    ...
    assert_eq!(effects.mutated()[0].1.get_address_owner_address().unwrap(), recipient);
...
async fn execute_pay_sui(
    ...
    let handles: Vec<_> = input_coin_objects.into_iter().map(|obj| authority_state.insert_genesis_object(obj)).collect();
...
async fn execute_pay_all_sui(
    ...
    let network_config = sui_swarm_config::network_config_builder::ConfigBuilder::new(&dir)
        ...
        .with_objects(input_coin_objects.clone().into_iter().map(ToOwned::to_owned))
        .build();
    ...
    for coin in input_coin_objects {
        let id = coin.id();
        let object_ref = genesis.objects().iter().find(|o| o.id() == id).unwrap().compute_object_reference();
