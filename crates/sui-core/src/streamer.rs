...
{
    pub fn spawn(buffer: usize, metrics: Arc<SubscriptionMetrics>, metrics_label: &'static str) -> Self {
        let channel_label = format!("streamer_{}", metrics_label);
        let gauge = if let Some(metrics) = mysten_metrics::get_metrics() {
            metrics.channel_inflight.with_label_values(&[&channel_label])
        } else {
            ...
            mysten_metrics::get_metrics().unwrap().channel_inflight.with_label_values(&[&channel_label])
        };
        ...
        spawn_monitored_task!(async move {
            while let Some(data) = rx.recv().await {
                Self::send_to_all_subscribers(subscribers.clone(), data, metrics.clone(), metrics_label).await;
            }
        });
        streamer
    }
    ...
    async fn send_to_all_subscribers(
        ...
        let success_counter = metrics.streaming_success.with_label_values(&[metrics_label]);
        let failure_counter = metrics.streaming_failure.with_label_values(&[metrics_label]);
        let subscriber_count = metrics.streaming_active_subscriber_number.with_label_values(&[metrics_label]);
        ...
        let to_remove = {
            ...
            for (id, (subscriber, filter)) in subscribers_snapshot.iter() {
                ...
                match subscriber.try_send(data.into()) {
                    ...
                    Err(e) => {
                        warn!(subscription_id = id, "Error when streaming data, removing subscriber. Error: {e}");
