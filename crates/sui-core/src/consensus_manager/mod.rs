...
use consensus_core::{Clock, CommitConsumerArgs, CommitConsumerMonitor, CommitIndex, ConsensusAuthority, NetworkType};
...
use crate::{
    ...
    consensus_handler::{ConsensusBlockHandler, ConsensusHandlerInitializer, MysticetiConsensusHandler},
...
impl ConsensusManager {
    ...
    pub async fn start(
        ...
        *running = Running::True(epoch, protocol_config.version);
        ...
        info!("Starting up consensus for epoch {epoch:?} & protocol version {:?}", protocol_config.version);

        self.consensus_client.set(self.client.clone());

        let consensus_config = node_config.consensus_config().expect("consensus_config should exist");
        ...
        let last_processed_commit_index = consensus_handler.last_processed_subdag_index() as CommitIndex;
        let replay_after_commit_index = last_processed_commit_index.saturating_sub(num_prior_commits);
...
impl ReplayWaiter {
    pub(crate) fn new(consumer_monitor_receiver: broadcast::Receiver<Arc<CommitConsumerMonitor>>) -> Self {
