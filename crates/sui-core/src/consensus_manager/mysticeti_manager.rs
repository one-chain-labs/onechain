...
use consensus_core::{Clock, CommitConsumer, CommitConsumerMonitor, CommitIndex, ConsensusAuthority};
...
impl ConsensusManagerTrait for MysticetiManager {
    async fn start(
        ...
        let (commit_consumer, commit_receiver, block_receiver) = CommitConsumer::new(starting_commit);
