...
impl OverloadTracker {
    pub(crate) fn new() -> Self {
        Self { object_waiting_queue: RwLock::new(HashMap::new()) }
    }

    pub(crate) fn add_pending_certificate(&self, tx_data: &SenderSignedData) {
        let tx_digest = tx_data.digest();
        let exclusively_accessed_shared_objects = Self::get_exclusively_accessed_shared_objects(tx_data);
    ...
    fn get_exclusively_accessed_shared_objects(tx_data: &SenderSignedData) -> Vec<FullObjectID> {
        tx_data
            ...
            .into_iter()
            .filter_map(|r| {
                r.is_accessed_exclusively().then_some(FullObjectID::new(r.id, Some(r.initial_shared_version)))
            })
            .collect()
    }
    ...
    fn objects_queue_len_and_age(&self, keys: Vec<FullObjectID>) -> Vec<(FullObjectID, usize, Option<Duration>)> {
