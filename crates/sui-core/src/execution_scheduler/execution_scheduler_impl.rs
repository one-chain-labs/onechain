        ...
        if !settlement_txns.is_empty() {
            ...
            spawn_monitored_task!(epoch_store.clone().within_alive_epoch(async move {
                ...
                while let Some((txns, env)) = futures.next().await {
                    let txns = txns.into_iter().map(|tx| (tx, env.clone())).collect::<Vec<_>>();
...
mod test {
    ...
    async fn execution_scheduler_object_dependency() {
        ...
        let shared_object_arg_read =
            ObjectArg::SharedObject { id: shared_object.id(), initial_shared_version, mutable: false };
        ...
        let shared_object_arg_default =
            ObjectArg::SharedObject { id: shared_object.id(), initial_shared_version, mutable: true };
        ...
        execution_scheduler.enqueue_transactions(
            vec![
                (
                    ...
                    ExecutionEnv::new()
                        .with_assigned_versions(AssignedVersions::non_withdraw(tx_read_0_assigned_versions)),
                ),
                (
                    transaction_read_1.clone(),
                    ExecutionEnv::new()
                        .with_assigned_versions(AssignedVersions::non_withdraw(tx_read_1_assigned_versions)),
                ),
                (
                    transaction_default.clone(),
                    ExecutionEnv::new()
                        .with_assigned_versions(AssignedVersions::non_withdraw(tx_default_assigned_versions)),
                ),
                (
                    transaction_read_2.clone(),
                    ExecutionEnv::new()
                        .with_assigned_versions(AssignedVersions::non_withdraw(tx_read_2_assigned_versions)),
                ),
            ],
            &state.epoch_store_for_testing(),
        );
    ...
    async fn execution_scheduler_with_cancelled_transactions() {
        ...
        execution_scheduler.enqueue_transactions(
            vec![(
                ...
                ExecutionEnv::new().with_assigned_versions(AssignedVersions::non_withdraw(assigned_versions)),
            )],
            &state.epoch_store_for_testing(),
        );
