...
impl BarrierDependencyBuilder {
    ...
    pub fn process_tx(&mut self, tx_digest: TransactionDigest, tx: &TransactionData) -> BTreeSet<TransactionDigest> {
...
impl<'a> PendingGuard<'a> {
    pub fn new(scheduler: &'a ExecutionScheduler, cert: &'a VerifiedExecutableTransaction) -> Self {
        scheduler.metrics.transaction_manager_num_pending_certificates.inc();
        scheduler.overload_tracker.add_pending_certificate(cert.data());
        Self { scheduler, cert }
    }
}
...
impl Drop for PendingGuard<'_> {
    fn drop(&mut self) {
        self.scheduler.metrics.transaction_manager_num_pending_certificates.dec();
        self.scheduler.overload_tracker.remove_pending_certificate(self.cert.data());
    }
}
...
impl ExecutionScheduler {
    pub fn new(
        ...
        let balance_withdraw_scheduler = Arc::new(Mutex::new(Self::initialize_balance_withdraw_scheduler(
            epoch_store,
            &object_cache_read,
    ...
    fn initialize_balance_withdraw_scheduler(
        ...
        let withdraw_scheduler_enabled = epoch_store.is_validator() && epoch_store.accumulators_enabled();
        ...
        let starting_accumulator_version = object_cache_read
            ...
            .version();
        Some(BalanceWithdrawScheduler::new(Arc::new(child_object_resolver), starting_accumulator_version))
    }
    ...
    async fn schedule_transaction(
        ...
        let input_object_keys: Vec<_> = epoch_store
            .get_input_object_keys(&cert.key(), &input_object_kinds, &execution_env.assigned_versions)
            .into_iter()
            .collect();
        ...
        let receiving_object_keys: HashSet<_> = tx_data
            ...
            .collect();
        let input_and_receiving_keys = [input_object_keys, receiving_object_keys.iter().cloned().collect()].concat();

        let epoch = epoch_store.epoch();
        debug!(?tx_digest, "Scheduled transaction, waiting for input objects: {:?}", input_and_receiving_keys,);

        let availability = self.object_cache_read.multi_input_objects_available_cache_only(&input_and_receiving_keys);
        ...
        if !execution_env.barrier_dependencies.is_empty() {
            debug!("waiting for barrier dependencies to be executed: {:?}", execution_env.barrier_dependencies);
    ...
    fn send_transaction_for_execution(
        ...
        let pending_cert = PendingCertificate {
            ...
            execution_env,
            stats: PendingCertificateStats { enqueue_time, ready_time: Some(Instant::now()) },
            executing_guard: Some(ExecutingGuard::new(
                self.metrics.transaction_manager_num_executing_certificates.clone(),
            )),
        };
    ...
    fn schedule_balance_withdraws(
        ...
        for (cert, env) in &certs {
            let tx_withdraws = cert.transaction_data().process_funds_withdrawals_for_execution();
            ...
            withdraws
                ...
                .or_insert(Vec::new())
                .push(TxBalanceWithdraw { tx_digest, reservations: tx_withdraws });
        }
        let mut receivers = FuturesUnordered::new();
        {
            let guard = self.balance_withdraw_scheduler.lock();
            let withdraw_scheduler =
                guard.as_ref().expect("Balance withdraw scheduler must be enabled if there are withdraws");
                ...
                match result {
                    Ok(result) => match result.status {
                        ScheduleStatus::InsufficientBalance => {
                            ...
                            debug!(?tx_digest, "Balance withdraw scheduling result: Insufficient balance");
        ...
        if !settlement_txns.is_empty() {
            ...
            spawn_monitored_task!(epoch_store.clone().within_alive_epoch(async move {
                let mut futures: FuturesUnordered<_> = settlement_txns
                    ...
                    .map(|(key, env)| {
                        let epoch_store = epoch_store.clone();
                        async move { (epoch_store.wait_for_settlement_transactions(key).await, env) }
                    })
                    .collect();
    ...
    fn schedule_tx_keys(
        ...
        spawn_monitored_task!(epoch_store.clone().within_alive_epoch(async move {
            ...
            let digests = epoch_store.notify_read_tx_key_to_digest(&tx_keys).await.expect("db error");
    ...
    fn assert_cert_not_executed_previous_epochs(&self, cert: &VerifiedExecutableTransaction) {
        ...
        let digests = [digest];
        let executed = self.transaction_cache_read.multi_get_executed_effects(&digests).pop().unwrap();
...
impl ExecutionScheduler {
    pub fn enqueue(&self, certs: Vec<(Schedulable, ExecutionEnv)>, epoch_store: &Arc<AuthorityPerEpochStore>) {
    ...
    pub fn enqueue_transactions(
        ...
        let pending_certs = certs.into_iter().zip(executed).filter_map(|((cert, execution_env), executed)| {
            if executed.is_none() {
        ...
        for (cert, execution_env) in pending_certs {
            let scheduler = self.clone();
            let epoch_store = epoch_store.clone();
            spawn_monitored_task!(epoch_store.within_alive_epoch(scheduler.schedule_transaction(
                cert,
                execution_env,
                &epoch_store,
            )));
    ...
    pub fn check_execution_overload(
        ...
        self.overload_tracker.check_execution_overload(overload_config, tx_data, inflight_queue_len)
    }

    pub fn num_pending_certificates(&self) -> usize {
        (self.metrics.transaction_manager_num_pending_certificates.get()
            + self.metrics.transaction_manager_num_executing_certificates.get()) as usize
    }
...
mod test {
    ...
    use sui_types::{
        base_types::{random_object_ref, ObjectID, SequenceNumber, SuiAddress},
    ...
    use crate::{
        authority::{
            authority_tests::init_state_with_objects,
    ...
    fn make_execution_scheduler(state: &AuthorityState) -> (ExecutionScheduler, UnboundedReceiver<PendingCertificate>) {
    ...
    fn make_transaction(gas_object: Object, input: Vec<CallArg>) -> VerifiedExecutableTransaction {
        ...
        let transaction = TestTransactionBuilder::new(sender, gas_object.compute_object_reference(), rgp)
    ...
    async fn execution_scheduler_basics() {
        ...
        assert!(rx_ready_certificates.try_recv().is_err_and(|err| err == TryRecvError::Empty));
        ...
        execution_scheduler.enqueue_transactions(
            vec![(transaction.clone(), ExecutionEnv::new().with_scheduling_source(SchedulingSource::NonFastPath))],
            &state.epoch_store_for_testing(),
        );
        ...
        assert!(pending_certificate.stats.ready_time.unwrap() >= pending_certificate.stats.enqueue_time);
        ...
        let gas_object_new =
            Object::with_id_owner_version_for_testing(ObjectID::random(), 0.into(), Owner::AddressOwner(owner));
        ...
        state.get_cache_writer().write_object_entry_for_test(gas_object_new);
        // scheduler should output the transaction eventually.
        ...
        assert_eq!(pending_certificate.certificate.digest(), pending_certificate2.certificate.digest());
    ...
    async fn execution_scheduler_object_dependency() {
        ...
        let state = init_state_with_objects(
            [gas_objects.clone(), vec![shared_object.clone(), shared_object_2.clone()]].concat(),
        )
        .await;
        ...
        let transaction_read_0 = make_transaction(gas_objects[0].clone(), vec![CallArg::Object(shared_object_arg_read)]);
        let transaction_read_1 = make_transaction(gas_objects[1].clone(), vec![CallArg::Object(shared_object_arg_read)]);
        let tx_read_0_assigned_versions =
            vec![((shared_object.id(), shared_object.owner().start_version().unwrap()), shared_version)];
        let tx_read_1_assigned_versions =
            vec![((shared_object.id(), shared_object.owner().start_version().unwrap()), shared_version)];
        ...
        let transaction_default =
            make_transaction(gas_objects[2].clone(), vec![CallArg::Object(shared_object_arg_default)]);
        let tx_default_assigned_versions =
            vec![((shared_object.id(), shared_object.owner().start_version().unwrap()), shared_version)];
        ...
        let transaction_read_2 = make_transaction(gas_objects[3].clone(), vec![
        ...
        execution_scheduler.enqueue_transactions(
            vec![
                (
                    ...
                    ExecutionEnv::new().with_assigned_versions(AssignedVersions::new(tx_read_0_assigned_versions, None)),
                ),
                (
                    transaction_read_1.clone(),
                    ExecutionEnv::new().with_assigned_versions(AssignedVersions::new(tx_read_1_assigned_versions, None)),
                ),
                (
                    transaction_default.clone(),
                    ExecutionEnv::new()
                        .with_assigned_versions(AssignedVersions::new(tx_default_assigned_versions, None)),
                ),
                (
                    transaction_read_2.clone(),
                    ExecutionEnv::new().with_assigned_versions(AssignedVersions::new(tx_read_2_assigned_versions, None)),
                ),
            ],
            &state.epoch_store_for_testing(),
        );
        ...
        new_shared_object.data.try_as_move_mut().unwrap().increment_version_to(shared_version_2);
        state.get_cache_writer().write_object_entry_for_test(new_shared_object);
        ...
        {
            let mut want_digests =
                vec![transaction_read_0.digest(), transaction_read_1.digest(), transaction_default.digest()];
            want_digests.sort();
        ...
        new_shared_object_2.data.try_as_move_mut().unwrap().increment_version_to(shared_version_2);
        state.get_cache_writer().write_object_entry_for_test(new_shared_object_2);
    ...
    async fn execution_scheduler_receiving_notify_commit() {
        ...
        let object_arguments: Vec<_> = (0 .. 10)
            .map(|i| {
                let object = Object::with_id_owner_version_for_testing(obj_id, i.into(), Owner::AddressOwner(owner));
        ...
        for (i, (_, txn)) in object_arguments.iter().enumerate() {
            ...
            execution_scheduler.enqueue_transactions(
                vec![(txn.clone(), ExecutionEnv::new().with_scheduling_source(SchedulingSource::NonFastPath))],
                &state.epoch_store_for_testing(),
            );
            sleep(Duration::from_secs(1)).await;
            assert!(rx_ready_certificates.try_recv().is_err());
        ...
        for (i, (object, txn)) in object_arguments.into_iter().enumerate() {
            ...
            state.get_cache_writer().write_object_entry_for_test(object.clone());
    ...
    async fn execution_scheduler_receiving_object_ready_notifications() {
        ...
        let receiving_object_arg0 = ObjectArg::Receiving(receiving_object_new0.compute_object_reference());
        let receive_object_transaction0 =
            make_transaction(gas_objects[0].clone(), vec![CallArg::Object(receiving_object_arg0)]);

        let receiving_object_arg1 = ObjectArg::Receiving(receiving_object_new1.compute_object_reference());
        let receive_object_transaction1 =
            make_transaction(gas_objects[0].clone(), vec![CallArg::Object(receiving_object_arg1)]);
        ...
        state.get_cache_writer().write_object_entry_for_test(receiving_object_new0.clone());
        ...
        // Notify scheduler that the receiving object 0 is available.
        state.get_cache_writer().write_object_entry_for_test(receiving_object_new1.clone());
    ...
    async fn execution_scheduler_receiving_object_ready_notifications_multiple_of_same_receiving() {
        ...
        let receive_object_transaction01 =
            make_transaction(gas_objects[1].clone(), vec![CallArg::Object(receiving_object_arg0)]);
        ...
        let tx1 = make_transaction(gas_objects[0].clone(), vec![CallArg::Object(gas_receiving_arg)]);
        ...
        execution_scheduler.enqueue_transactions(
            vec![(tx1.clone(), ExecutionEnv::new().with_scheduling_source(SchedulingSource::NonFastPath))],
            &state.epoch_store_for_testing(),
        );
        sleep(Duration::from_secs(1)).await;
        rx_ready_certificates.recv().await.unwrap();
    ...
    async fn execution_scheduler_receiving_object_ready_if_current_version_greater() {
        ...
        let receiving_object =
            Object::with_id_owner_version_for_testing(ObjectID::random(), 10.into(), Owner::AddressOwner(owner));
        ...
        let receiving_object_new0 =
            Object::with_id_owner_version_for_testing(receiving_object.id(), 0.into(), Owner::AddressOwner(owner));
        let receiving_object_new1 =
            Object::with_id_owner_version_for_testing(receiving_object.id(), 1.into(), Owner::AddressOwner(owner));
        let receiving_object_arg0 = ObjectArg::Receiving(receiving_object_new0.compute_object_reference());
        let receive_object_transaction0 =
            make_transaction(gas_objects[0].clone(), vec![CallArg::Object(receiving_object_arg0)]);
    ...
    async fn execution_scheduler_with_cancelled_transactions() {
        ...
        let cancelled_transaction = make_transaction(gas_object.clone(), vec![
        ...
        execution_scheduler.enqueue_transactions(
            vec![(
                ...
                ExecutionEnv::new().with_assigned_versions(AssignedVersions::new(assigned_versions, None)),
            )],
            &state.epoch_store_for_testing(),
        );
        ...
        new_owned_object.data.try_as_move_mut().unwrap().increment_version_to(owned_version);
        state.get_cache_writer().write_object_entry_for_test(new_owned_object);
    ...
    fn test_barrier_dependency_builder() {
        let make_transaction = |non_exclusive_writes: Vec<u32>, exclusive_writes: Vec<u32>| {
            assert!(non_exclusive_writes.iter().all(|id| !exclusive_writes.contains(id)));
            assert!(exclusive_writes.iter().all(|id| !non_exclusive_writes.contains(id)));

            let non_exclusive_writes = non_exclusive_writes.into_iter().map(|id| ObjectID::from_single_byte(id as u8));
            let exclusive_writes = exclusive_writes.into_iter().map(|id| ObjectID::from_single_byte(id as u8));
            ...
            let tx_data = TransactionData::new(tx, SuiAddress::default(), random_object_ref(), 1, 1);
        ...
        // One non-exclusive write, one exclusive write.
        {
            ...
            let tx1_deps = barrier_dependency_builder.process_tx(*tx1.digest(), tx1.transaction_data());
            let tx2_deps = barrier_dependency_builder.process_tx(*tx2.digest(), tx2.transaction_data());
        ...
        // a dependency of two barriers
        {
            ...
            let tx1_deps = barrier_dependency_builder.process_tx(*tx1.digest(), tx1.transaction_data());
            let tx2_deps = barrier_dependency_builder.process_tx(*tx2.digest(), tx2.transaction_data());
            let tx3_deps = barrier_dependency_builder.process_tx(*tx3.digest(), tx3.transaction_data());
            assert!(tx1_deps.is_empty());
            assert_eq!(Vec::from_iter(tx2_deps), vec![*tx1.digest()]);
