...
impl ExecutionScheduler {
    ...
    async fn schedule_transaction(
        ...
        let input_object_keys: Vec<_> = match epoch_store.get_input_object_keys(&cert.key(), &input_object_kinds) {
            Ok(keys) => keys,
            Err(_) => {
                ...
                assert!(self.transaction_cache_read.is_tx_already_executed(cert.digest()));
                self.metrics
        ...
        tracing::trace!(?digests, "Waiting for input objects: {:?}", input_and_receiving_keys);
...
impl ExecutionSchedulerAPI for ExecutionScheduler {
    fn enqueue_impl(
        ...
        let pending_certs = certs.into_iter().zip(executed).filter_map(|((cert, expected_effects_digest), executed)| {
            if executed.is_none() {
...
mod test {
    ...
    fn make_execution_scheduler(
    ...
    ) -> (ExecutionSchedulerWrapper, UnboundedReceiver<PendingCertificate>) {
        ...
        let execution_scheduler = ExecutionSchedulerWrapper::ExecutionScheduler(ExecutionScheduler::new(
    ...
    async fn execution_scheduler_receiving_object_ready_notifications() {
        ...
        execution_scheduler.enqueue(vec![receive_object_transaction0.clone()], &state.epoch_store_for_testing());
        sleep(Duration::from_secs(1)).await;
        assert!(rx_ready_certificates.try_recv().is_err());
        assert_eq!(execution_scheduler.num_pending_certificates(), 1);

        // scheduler should output no transaction yet since waiting on receiving object.
        execution_scheduler.enqueue(vec![receive_object_transaction1.clone()], &state.epoch_store_for_testing());
        sleep(Duration::from_secs(1)).await;
        assert!(rx_ready_certificates.try_recv().is_err());
        assert_eq!(execution_scheduler.num_pending_certificates(), 2);
    ...
    async fn execution_scheduler_receiving_object_ready_notifications_multiple_of_same_receiving() {
        ...
        execution_scheduler.enqueue(vec![receive_object_transaction01.clone()], &state.epoch_store_for_testing());
        sleep(Duration::from_secs(1)).await;
        assert!(rx_ready_certificates.try_recv().is_err());
        assert_eq!(execution_scheduler.num_pending_certificates(), 3);
    ...
    async fn execution_scheduler_with_cancelled_transactions() {
        ...
        state
            ...
            .set_shared_object_versions_for_testing(cancelled_transaction.digest(), &[
                (
                    (shared_object_1.id(), shared_object_1.owner().start_version().unwrap()),
        ...
        execution_scheduler.enqueue(vec![cancelled_transaction.clone()], &state.epoch_store_for_testing());
