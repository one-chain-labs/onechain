...
impl AccountBalanceRead for Arc<dyn ChildObjectResolver + Send + Sync> {
    fn get_account_balance(&self, account_id: &AccumulatorObjId, accumulator_version: SequenceNumber) -> u128 {
        let value: U128 = AccumulatorValue::load_by_id(self.as_ref(), Some(accumulator_version), *account_id)
    ...
    pub(crate) fn new(init_version: SequenceNumber, init_balances: BTreeMap<ObjectID, u128>) -> Self {
        let balances = init_balances
            ...
            .map(|(account_id, balance)| {
                (AccumulatorObjId::new_unchecked(*account_id), BTreeMap::from_iter([(init_version, *balance)]))
            })
...
impl MockBalanceReadInner {
    fn settle_balance_changes(&mut self, balance_changes: BTreeMap<AccumulatorObjId, i128>) {
        ...
        for (account_id, balance_change) in balance_changes {
            ...
            self.balances.entry(account_id).or_default().insert(new_accumulator_version, new_balance as u128);
        }
    }
    ...
    fn get_account_balance(&self, account_id: &AccumulatorObjId, accumulator_version: SequenceNumber) -> u128 {
        ...
        account_balances.range(..= accumulator_version).last().map(|(_, balance)| *balance).unwrap_or_default()
    }
}
