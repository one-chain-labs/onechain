...
use sui_types::{accumulator_root::AccumulatorObjId, base_types::SequenceNumber, digests::TransactionDigest};
...
impl EagerBalanceWithdrawScheduler {
    pub fn new(balance_read: Arc<dyn AccountBalanceRead>, starting_accumulator_version: SequenceNumber) -> Arc<Self> {
        Arc::new(Self {
            balance_read,
...
impl BalanceWithdrawSchedulerTrait for EagerBalanceWithdrawScheduler {
    ...
    async fn schedule_withdraws(&self, withdraws: WithdrawReservations) {
        ...
        for (withdraw, sender) in withdraws.withdraws.into_iter().zip(withdraws.senders) {
            ...
            inner_state.pending_settlements.entry(withdraws.accumulator_version).or_default().extend(&accounts);
            let pending_withdraw = PendingWithdraw::new(withdraws.accumulator_version, withdraw, sender);
            for account_id in accounts {
                let account_state = inner_state.tracked_accounts.entry(account_id).or_insert_with(|| {
                    ...
                    AccountState::new(self.balance_read.as_ref(), account_id, cur_accumulator_version)
                });
                let has_blocking_reservations = !account_state.pending_reservations.is_empty();
                let result =
                    account_state.try_reserve(cur_accumulator_version, &pending_withdraw, has_blocking_reservations);
                if matches!(result, TryReserveResult::Pending) {
                    account_state.pending_reservations.push_back(pending_withdraw.clone());
                }
            }
        }
    }
    ...
    async fn settle_balances(&self, settlement: BalanceSettlement) {
        ...
        let mut inner_state = self.inner_state.lock();
        assert_eq!(next_accumulator_version, inner_state.accumulator_version.next());
        ...
        let mut affected_accounts = inner_state.pending_settlements.remove(&cleanup_version).unwrap_or_default();
        ...
        if let Some(current_version_accounts) = inner_state.pending_settlements.get(&next_accumulator_version) {
            affected_accounts.extend(current_version_accounts);
        }
        affected_accounts.extend(settlement.balance_changes.keys().cloned());

        debug!("Processing withdraws affecting accounts: {:?}", affected_accounts,);
        for object_id in affected_accounts {
            ...
            let reserved = account_state.reserved_balance.remove(&cleanup_version).unwrap_or_default() as i128;
            let settled = settlement.balance_changes.get(&object_id).copied().unwrap_or_default();
            ...
            while !account_state.pending_reservations.is_empty() {
                ...
                let result = account_state.try_reserve(next_accumulator_version, &pending_withdraw, false);
                if matches!(result, TryReserveResult::Pending) {
                    account_state.pending_reservations.push_front(pending_withdraw);
                    break;
                }
            }

            if in_test_configuration() {
                account_state.debug_check_account_state_invariants(self.balance_read.as_ref(), next_accumulator_version);
            }
...
impl AccountState {
    ...
    fn commit_reservation(&mut self, pending_withdraw: &Arc<PendingWithdraw>) {
        ...
        *self.reserved_balance.entry(pending_withdraw.accumulator_version).or_default() += to_reserve;
        debug!(
    ...
    fn debug_check_account_state_invariants(
        ...
        let actual_balance = balance_read.get_account_balance(&self.account_id, last_settled_version);
...
impl PendingWithdraw {
    ...
    fn notify_insufficient_balance(&self) {
        ...
        if let Some(sender) = sender_guard.take() {
            ...
            let _ =
                sender.send(ScheduleResult { tx_digest: self.tx_digest, status: ScheduleStatus::InsufficientBalance });
        }
    }
}
