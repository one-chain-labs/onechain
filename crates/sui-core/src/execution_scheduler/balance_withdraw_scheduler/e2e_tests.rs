...
use sui_types::{
    accumulator_root::{update_account_balance_for_testing, AccumulatorObjId, AccumulatorValue},
    balance::Balance,
...
use crate::{
    ...
    execution_scheduler::{balance_withdraw_scheduler::BalanceSettlement, ExecutionScheduler, PendingCertificate},
};
...
async fn create_test_env(init_balances: BTreeMap<TypeTag, u64>) -> TestEnv {
    ...
    TestEnv { sender, sender_key, gas_object, account_objects, rx_ready_certificates, scheduler, state }
}
...
impl TestEnv {
    fn create_transactions(&self, amounts: Vec<u64>) -> Vec<VerifiedExecutableTransaction> {
        amounts
            ...
            .enumerate()
            .map(|(idx, amount)| {
                let withdraw = FundsWithdrawalArg::balance_from_sender(amount, GAS::type_tag().into());
    ...
    fn get_accumulator_object(&self) -> Object {
        self.state.get_object_cache_reader().get_object(&SUI_ACCUMULATOR_ROOT_OBJECT_ID).unwrap()
    }
    ...
    async fn receive_certificate(&mut self) -> Option<PendingCertificate> {
        timeout(Duration::from_secs(1), self.rx_ready_certificates.recv()).await.ok()?
    }

    async fn expect_withdraw_results(&mut self, expected_results: BTreeMap<TransactionDigest, BalanceWithdrawStatus>) {
        let mut results = BTreeMap::new();
        while results.len() < expected_results.len() {
            let cert = self.receive_certificate().await.unwrap();
            results.insert(*cert.certificate.digest(), cert.execution_env.withdraw_status);
        }
    ...
    fn settle_balances(&mut self, balance_changes: BTreeMap<ObjectID, i128>) {
        let balance_changes: BTreeMap<AccumulatorObjId, i128> = balance_changes
            .into_iter()
            .map(|(object_id, balance_change)| (AccumulatorObjId::new_unchecked(object_id), balance_change))
            .collect();
        ...
        for (object_id, balance_change) in balance_changes {
            let mut account_object = self.state.get_object_cache_reader().get_object(object_id.inner()).unwrap();
            update_account_balance_for_testing(&mut account_object, balance_change);
            account_object.data.try_as_move_mut().unwrap().increment_version_to(next_version);
            self.state.get_cache_writer().write_object_entry_for_test(account_object);
        }
        accumulator_object.data.try_as_move_mut().unwrap().increment_version_to(next_version);
        self.state.get_cache_writer().write_object_entry_for_test(accumulator_object);
    }
}
...
async fn test_withdraw_schedule_e2e() {
    ...
    test_env
        .expect_withdraw_results(BTreeMap::from([
            (*transactions[0].digest(), BalanceWithdrawStatus::SufficientBalance),
            (*transactions[1].digest(), BalanceWithdrawStatus::SufficientBalance),
            (*transactions[2].digest(), BalanceWithdrawStatus::InsufficientBalance),
        ]))
        .await;
    ...
    test_env
        .expect_withdraw_results(BTreeMap::from([
            (*transactions[0].digest(), BalanceWithdrawStatus::SufficientBalance),
            (*transactions[1].digest(), BalanceWithdrawStatus::InsufficientBalance),
        ]))
        .await;
