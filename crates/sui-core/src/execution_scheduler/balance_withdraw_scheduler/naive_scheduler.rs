...
impl NaiveBalanceWithdrawScheduler {
    pub fn new(balance_read: Arc<dyn AccountBalanceRead>, cur_accumulator_version: SequenceNumber) -> Arc<Self> {
        let (accumulator_version_sender, accumulator_version_receiver) = watch::channel(cur_accumulator_version);
        Arc::new(Self { balance_read, accumulator_version_sender, accumulator_version_receiver })
    }
}
