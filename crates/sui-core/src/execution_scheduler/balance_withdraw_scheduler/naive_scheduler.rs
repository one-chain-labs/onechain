...
impl NaiveBalanceWithdrawScheduler {
    pub fn new(
        ...
        Arc::new(Self { balance_read, last_settled_version_sender, last_settled_version_receiver })
    }
}
...
impl BalanceWithdrawSchedulerTrait for NaiveBalanceWithdrawScheduler {
    async fn schedule_withdraws(&self, withdraws: WithdrawReservations) {
        ...
        while *receiver.borrow_and_update() < withdraws.accumulator_version {
            debug!("Waiting for accumulator version {:?} to be settled", withdraws.accumulator_version);
            if receiver.changed().await.is_err() {
                return;
            }
        }
        if *receiver.borrow() > withdraws.accumulator_version {
            debug!("Accumulator version {:?} is already settled", withdraws.accumulator_version);
            for (withdraw, sender) in withdraws.withdraws.into_iter().zip(withdraws.senders) {
                let _ = sender
                    .send(ScheduleResult { tx_digest: withdraw.tx_digest, status: ScheduleStatus::AlreadyExecuted });
            }
            return;
        }
        ...
        for (withdraw, sender) in withdraws.withdraws.into_iter().zip(withdraws.senders) {
            ...
            for (object_id, reservation) in &withdraw.reservations {
                let entry = cur_balances
                    .entry(*object_id)
                    .or_insert_with(|| self.balance_read.get_account_balance(object_id, withdraws.accumulator_version));
            ...
            if success {
                ...
                let _ = sender
                    .send(ScheduleResult { tx_digest: withdraw.tx_digest, status: ScheduleStatus::SufficientBalance });
            } else {
                let _ = sender
                    .send(ScheduleResult { tx_digest: withdraw.tx_digest, status: ScheduleStatus::InsufficientBalance });
            }
        }
    }
