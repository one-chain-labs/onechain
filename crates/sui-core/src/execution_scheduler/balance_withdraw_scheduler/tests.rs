...
impl TestScheduler {
    fn new(init_version: SequenceNumber, init_balances: BTreeMap<ObjectID, u128>) -> Self {
        ...
        Self { mock_read, schedulers: BTreeMap::from([("naive_scheduler".to_string(), naive_scheduler)]) }
    }
    ...
    fn settle_balance_changes(&self, next_accumulator_version: SequenceNumber, changes: BTreeMap<ObjectID, i128>) {
        ...
        self.mock_read.settle_balance_changes(accumulator_changes.clone());
...
async fn test_schedule_wait_for_settlement() {
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw.tx_digest, ScheduleStatus::SufficientBalance)])).await;
}
...
async fn test_schedules_and_settles() {
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw0.tx_digest, ScheduleStatus::SufficientBalance)])).await;
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw1.tx_digest, ScheduleStatus::InsufficientBalance)])).await;
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw2.tx_digest, ScheduleStatus::SufficientBalance)])).await;
}
