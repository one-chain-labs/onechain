...
impl TestScheduler {
    ...
    fn settle_balance_changes(&self, next_accumulator_version: SequenceNumber, changes: BTreeMap<ObjectID, i128>) {
        let accumulator_changes: BTreeMap<_, _> =
            changes.iter().map(|(id, value)| (AccumulatorObjId::new_unchecked(*id), *value)).collect();
        self.mock_read.settle_balance_changes(accumulator_changes.clone(), next_accumulator_version);
...
async fn test_schedule_wait_for_settlement() {
    ...
    assert!(wait_for_results(receivers, BTreeMap::from([(withdraw.tx_digest, ScheduleStatus::SufficientBalance)]),)
        .await
        .is_err());
}
...
async fn test_schedules_and_settles() {
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw0.tx_digest, ScheduleStatus::SufficientBalance)]))
        .await
        .unwrap();
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw1.tx_digest, ScheduleStatus::InsufficientBalance)]))
        .await
        .unwrap();
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw2.tx_digest, ScheduleStatus::SufficientBalance)]))
        .await
        .unwrap();
}
...
async fn test_already_executed() {
    ...
    let test = TestScheduler::new(init_version, BTreeMap::from([(account1, 100), (account2, 200)]));
    ...
    let receivers = test.schedule_withdraws(init_version, vec![withdraw1.clone(), withdraw2.clone()]);
    wait_for_results(
        receivers,
        BTreeMap::from([
...
async fn test_multiple_withdraws_same_version() {
    ...
    let receivers = test.schedule_withdraws(init_version, vec![withdraw1.clone(), withdraw2.clone(), withdraw3.clone()]);
    wait_for_results(
        receivers,
        BTreeMap::from([
...
async fn test_multiple_withdraws_multiple_accounts_same_version() {
    ...
    let account1 = ObjectID::random();
    let account2 = ObjectID::random();
    let test = TestScheduler::new(init_version, BTreeMap::from([(account1, 100), (account2, 100)]));
...
async fn test_withdraw_already_settled_account_object() {
    ...
    let mock_read = Arc::new(MockBalanceRead::new(v1, BTreeMap::from([(account, 100u128)])));
    let scheduler = TestScheduler::new_with_mock_read(mock_read.clone());

    let withdraw =
        TxBalanceWithdraw { tx_digest: TransactionDigest::random(), reservations: BTreeMap::from([(account_id, 60)]) };

    let receivers = scheduler.schedule_withdraws(v0, vec![withdraw.clone()]);
    wait_for_results(receivers, BTreeMap::from([(withdraw.tx_digest, ScheduleStatus::SkipSchedule)])).await.unwrap();
    ...
    wait_for_results(receivers, BTreeMap::from([(withdraw.tx_digest, ScheduleStatus::SufficientBalance)]))
        .await
        .unwrap();
}
...
async fn test_settle_just_updated_account_object() {
    ...
    let account1 = ObjectID::random();
    let account2 = ObjectID::random();
    let mock_read = Arc::new(MockBalanceRead::new(v0, BTreeMap::from([(account1, 100u128), (account2, 100u128)])));

    let scheduler = TestScheduler::new_with_mock_read(mock_read.clone());
    // Bump underlying account object versions to v1.
    mock_read.settle_balance_changes(
        BTreeMap::from([(AccumulatorObjId::new_unchecked(account1), 0), (AccumulatorObjId::new_unchecked(account2), 0)]),
        v1,
    );
    ...
    scheduler
        .scheduler
        .settle_balances(BalanceSettlement { next_accumulator_version: v1, balance_changes: BTreeMap::new() });
    scheduler.wait_for_accumulator_version(v1).await;
    assert!(wait_for_results(receivers1, BTreeMap::from([(withdraw1.tx_digest, ScheduleStatus::InsufficientBalance)]),)
        .await
        .is_err());
    ...
    wait_for_results(receivers2, BTreeMap::from([(withdraw2.tx_digest, ScheduleStatus::InsufficientBalance)]))
        .await
        .unwrap();
}
...
async fn test_withdraw_settle_and_deleted_account() {
    ...
    // the underlying store has already observed a newer version of the account object through
    // the execution of settlement transactions.
    let mock_read = Arc::new(MockBalanceRead::new(v0, BTreeMap::from([(account, 100u128)])));
    let scheduler = TestScheduler::new_with_mock_read(mock_read.clone());
    ...
    let withdraw =
        TxBalanceWithdraw { tx_digest: TransactionDigest::random(), reservations: BTreeMap::from([(account_id, 100)]) };
...
impl StressTestEnv {
    fn new(num_accounts: usize, num_transactions: usize) -> Self {
        ...
        let accounts = (0 .. num_accounts).map(|_| ObjectID::random()).collect::<Vec<_>>();
        let mut rng = rand::thread_rng();
        let init_balances = accounts
            .iter()
            .filter_map(|account_id| if rng.gen_bool(0.7) { Some((*account_id, rng.gen_range(0 .. 20))) } else { None })
        ...
        for idx in 0 .. num_transactions {
            ...
            let account_ids = accounts.choose_multiple(&mut rng, num_reservation_accounts).cloned().collect::<Vec<_>>();
            let reservations = account_ids
                .iter()
                .map(|account_id| (AccumulatorObjId::new_unchecked(*account_id), rng.gen_range(1 .. 10)))
                .collect::<BTreeMap<_, _>>();
            cur_reservations.push(TxBalanceWithdraw { tx_digest: TransactionDigest::random(), reservations });
        ...
        Self { init_balances, accounts, withdraws }
    }
}
...
async fn balance_withdraw_scheduler_stress_test() {
    ...
    info!("Running stress test with num_accounts: {:?}, num_transactions: {:?}", num_accounts, num_transactions);

    let StressTestEnv { init_balances, accounts, withdraws } = StressTestEnv::new(num_accounts, num_transactions);
    ...
    for test_run in 0 .. 50 {
        ...
        let results = tokio::time::timeout(Duration::from_secs(30), async {
            ...
            let (schedule_results_tx, mut schedule_results_rx) =
                unbounded_channel::<BTreeMap<AccumulatorObjId, u64>>("test");
                ...
                while let Some(reserved_amounts) = schedule_results_rx.recv().await {
                    if test_run == 0 {
                        ...
                        let balance_changes = accounts
                            ...
                            .map(|account_id| {
                                let withdraws = if let Some(reserved_amount) =
                                    reserved_amounts.get(&AccumulatorObjId::new_unchecked(*account_id))
                                {
            ...
            for (version, receivers, withdraws) in all_receivers {
                debug!(
                    "Test instance waiting for results from version {:?}, receiver count: {}",
                    version,
                    receivers.len()
                );
                for result in receivers {
                    ...
                    debug!(
                        "Test instance received result for tx {:?} with status {:?} at version {:?}",
                        result.tx_digest, result.status, version
                    );
                    results.insert(result.tx_digest, result.status);
                }
        ...
        if let Some(expected_results) = &expected_results {
            ...
            for (tx_digest, status) in results {
                assert_eq!(&status, expected_results.get(&tx_digest).unwrap(), "Tx digest: {:?}", tx_digest);
            }
        } else {
