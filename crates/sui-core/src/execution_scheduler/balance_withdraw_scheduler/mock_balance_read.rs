...
impl MockBalanceRead {
    pub(crate) fn new(init_version: SequenceNumber, init_balances: BTreeMap<ObjectID, u128>) -> Self {
        let balances = init_balances
            .iter()
            .map(|(account_id, balance)| {
                (AccumulatorObjId::new_unchecked(*account_id), BTreeMap::from_iter([(init_version, Some(*balance))]))
            })
            .collect::<BTreeMap<_, _>>();
        Self { inner: Arc::new(RwLock::new(MockBalanceReadInner { cur_version: init_version, balances })) }
    }
...
impl MockBalanceReadInner {
    fn settle_balance_changes(
        ...
        debug!(?next_accumulator_version, "Updating balance states in MockBalanceRead: {:?}", balance_changes,);
        ...
        for (account_id, balance_change) in balance_changes {
            let balance = self.get_account_balance(&account_id, self.cur_version).unwrap_or_default();
            let new_balance = balance as i128 + balance_change;
            assert!(new_balance >= 0);
            let new_entry = if new_balance == 0 { None } else { Some(new_balance as u128) };
            self.balances.entry(account_id).or_default().insert(new_accumulator_version, new_entry);
        }
    }

    fn get_account_balance(&self, account_id: &AccumulatorObjId, accumulator_version: SequenceNumber) -> Option<u128> {
        let account_balances = self.balances.get(account_id)?;
        account_balances.range(..= accumulator_version).last().and_then(|(_, balance)| *balance)
    }
...
impl AccountBalanceRead for MockBalanceRead {
    ...
    fn get_account_balance(&self, account_id: &AccumulatorObjId, accumulator_version: SequenceNumber) -> u128 {
        let inner = self.inner.read();
        inner.get_account_balance(account_id, accumulator_version).unwrap_or_default()
    }

    fn get_latest_account_balance(&self, account_id: &AccumulatorObjId) -> u128 {
        let inner = self.inner.read();
        inner.get_latest_account_balance(account_id).unwrap_or_default()
    }
}
