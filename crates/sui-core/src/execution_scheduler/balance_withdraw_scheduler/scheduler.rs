...
impl WithdrawReservations {
    pub fn new(
        ...
        let (senders, receivers) = (0 .. withdraws.len())
            ...
            .unzip();
        (Self { accumulator_version, withdraws, senders }, receivers)
    }

    pub fn notify_skip_schedule(self) {
        debug!("Withdraws at accumulator version {:?} are already settled", self.accumulator_version);
        for (withdraw, sender) in self.withdraws.into_iter().zip(self.senders) {
            let _ = sender.send(ScheduleResult { tx_digest: withdraw.tx_digest, status: ScheduleStatus::SkipSchedule });
        }
    }
}

impl BalanceWithdrawScheduler {
    pub fn new(balance_read: Arc<dyn AccountBalanceRead>, starting_accumulator_version: SequenceNumber) -> Self {
        ...
        let innards = BTreeMap::from([
            (
                "naive".to_string(),
                NaiveBalanceWithdrawScheduler::new(balance_read.clone(), starting_accumulator_version)
        ...
        let (withdraw_sender, withdraw_receiver) = unbounded_channel("withdraw_scheduler_withdraws");
        let (settlement_sender, settlement_receiver) = unbounded_channel("withdraw_scheduler_settlements");
        let scheduler = Self { innards, withdraw_sender, settlement_sender };
        tokio::spawn(scheduler.clone().process_withdraw_task(withdraw_receiver));
        tokio::spawn(scheduler.clone().process_settlement_task(settlement_receiver));
        scheduler
    }
    ...
    pub fn get_current_accumulator_version(&self) -> SequenceNumber {
        let versions: Vec<_> =
            self.innards.iter().map(|(name, scheduler)| (name, scheduler.get_current_accumulator_version())).collect();
    ...
    async fn process_withdraw_task(self, mut withdraw_receiver: UnboundedReceiver<WithdrawReservations>) {
    ...
    fn aggregate_results(results: Vec<ScheduleResult>) -> ScheduleResult {
        ...
        let non_skip_results: Vec<_> = results.iter().filter(|r| r.status != ScheduleStatus::SkipSchedule).collect();

        if non_skip_results.is_empty() {
            // All schedulers returned SkipSchedule
            return ScheduleResult { tx_digest, status: ScheduleStatus::SkipSchedule };
        }
        ...
        ScheduleResult { tx_digest, status: first_status }
    }

    async fn process_settlement_task(self, mut settlement_receiver: UnboundedReceiver<BalanceSettlement>) {
