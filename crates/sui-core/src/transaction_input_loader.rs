...
use crate::{
    authority::{authority_per_epoch_store::CertLockGuard, shared_object_version_manager::AssignedVersions},
...
impl TransactionInputLoader {
    ...
    pub fn read_objects_for_execution(
        ...
        let assigned_shared_versions: BTreeMap<_, _> = assigned_shared_object_versions
            .iter()
            .map(|((id, initial_shared_version), version)| ((*id, *initial_shared_version), *version))
            .collect();
