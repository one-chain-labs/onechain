...
impl TryFrom<RawWaitForEffectsRequest> for WaitForEffectsRequest {
    ...
    fn try_from(value: RawWaitForEffectsRequest) -> Result<Self, Self::Error> {
        let transaction_digest =
            bcs::from_bytes(&value.transaction_digest).map_err(|err| SuiError::GrpcMessageDeserializeError {
                type_info: "RawWaitForEffectsRequest.transaction_digest".to_string(),
                error: err.to_string(),
            })?;
        let transaction_position =
            bcs::from_bytes(&value.transaction_position).map_err(|err| SuiError::GrpcMessageDeserializeError {
                type_info: "RawWaitForEffectsRequest.transaction_position".to_string(),
                error: err.to_string(),
            })?;
        Ok(Self { epoch: value.epoch, transaction_digest, transaction_position, include_details: value.include_details })
    }
}
    ...
    fn try_from(value: RawWaitForEffectsResponse) -> Result<Self, Self::Error> {
        match value.inner {
            Some(RawValidatorTransactionStatus::Executed(executed)) => {
                let effects_digest =
                    bcs::from_bytes(&executed.effects_digest).map_err(|err| SuiError::GrpcMessageDeserializeError {
                        type_info: "RawWaitForEffectsResponse.effects_digest".to_string(),
                        error: err.to_string(),
                    })?;
                let details = if let Some(details) = executed.details {
                    let effects =
                        bcs::from_bytes(&details.effects).map_err(|err| SuiError::GrpcMessageDeserializeError {
                            type_info: "RawWaitForEffectsResponse.details.effects".to_string(),
                            error: err.to_string(),
                        })?;
                    let events = if let Some(events) = details.events {
                        Some(bcs::from_bytes(&events).map_err(|err| SuiError::GrpcMessageDeserializeError {
                ...
                Ok(Self::Executed { effects_digest, details })
            }
            Some(RawValidatorTransactionStatus::Rejected(rejected)) => {
                let raw_reason =
                    RawRejectReason::try_from(rejected.reason).map_err(|err| SuiError::GrpcMessageDeserializeError {
                ...
                let reason = match raw_reason {
                    ...
                    RawRejectReason::LockConflict => RejectReason::LockConflict(rejected.message.unwrap_or_default()),
...
impl TryFrom<WaitForEffectsResponse> for RawWaitForEffectsResponse {
    ...
    fn try_from(value: WaitForEffectsResponse) -> Result<Self, Self::Error> {
        let inner = match value {
            WaitForEffectsResponse::Executed { effects_digest, details } => {
                ...
                let details = if let Some(details) = details {
                    ...
                    Some(RawExecutedData { effects, events, input_objects, output_objects })
                } else {
                    None
                };
                RawValidatorTransactionStatus::Executed(RawExecutedStatus { effects_digest, details })
            }
            WaitForEffectsResponse::Rejected { reason } => {
                let (reason, message) = match reason {
                    ...
                    RejectReason::PackageVerification(message) => (RawRejectReason::PackageVerification, Some(message)),
                ...
                RawValidatorTransactionStatus::Rejected(RawRejectedStatus { reason: reason as i32, message })
            }
