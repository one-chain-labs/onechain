...
impl RpcIndexes for RpcIndexStore {
    ...
    fn get_transaction_info(
        ...
        self.get_transaction_info(digest).map_err(StorageError::custom)
    }
        ...
        let iter = self.owner_iter(owner, object_type, cursor)?.map(|result| {
            result.map(
                |(
                    ...
                    OwnerIndexInfo { version, digest, start_version },
                )| {
                    OwnedObjectInfo {
                        owner,
                        start_version,
    ...
    fn dynamic_field_iter(
        ...
        Box<dyn Iterator<Item = Result<(DynamicFieldKey, DynamicFieldIndexInfo), TypedStoreError>> + '_>,
    > {
