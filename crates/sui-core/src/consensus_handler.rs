...
use lru::LruCache;
use mysten_common::{assert_reachable, assert_sometimes, debug_fatal, random_util::randomize_cache_capacity_in_tests};
use mysten_metrics::{
    monitored_future,
...
use sui_types::{
    ...
    base_types::{AuthorityName, ConciseableName, ConsensusObjectSequenceKey, SequenceNumber, TransactionDigest},
...
use crate::{
    ...
    checkpoints::{CheckpointService, CheckpointServiceNotify, PendingCheckpoint, PendingCheckpointInfo},
...
impl ConsensusHandlerInitializer {
    ...
    #[cfg(test)]
    pub(crate) fn new_for_testing(state: Arc<AuthorityState>, checkpoint_service: Arc<CheckpointService>) -> Self {
        ...
        let consensus_adapter = make_consensus_adapter_for_test(state.clone(), HashSet::new(), false, vec![]);
        Self {
            state: state.clone(),
            checkpoint_service,
            ...
            throughput_calculator: Arc::new(ConsensusThroughputCalculator::new(None, state.metrics.clone())),
            backpressure_manager,
        }
    }
...
mod additional_consensus_state {
    ...
    impl AdditionalConsensusState {
        pub fn new(additional_consensus_state_window_size: u32) -> Self {
            Self { commit_interval_observer: CommitIntervalObserver::new(additional_consensus_state_window_size) }
        }
        ...
        pub(crate) fn observe_commit(
            ...
            self.commit_info_impl(epoch_start_time, consensus_commit, Some(estimated_commit_period))
        }
    ...
    impl ConsensusCommitInfo {
        ...
        fn consensus_commit_prologue_transaction(&self, epoch: u64) -> VerifiedExecutableTransaction {
            let transaction = VerifiedTransaction::new_consensus_commit_prologue(epoch, self.round, self.timestamp);
            VerifiedExecutableTransaction::new_system(transaction, epoch)
        }

        fn consensus_commit_prologue_v2_transaction(&self, epoch: u64) -> VerifiedExecutableTransaction {
        ...
        pub fn create_consensus_commit_prologue_transaction(
            ...
            let version_assignments = if protocol_config.record_consensus_determined_version_assignments_in_prologue_v2()
            {
                Some(ConsensusDeterminedVersionAssignments::CancelledTransactionsV2(cancelled_txn_version_assignment))
            } else if protocol_config.record_consensus_determined_version_assignments_in_prologue() {
                Some(ConsensusDeterminedVersionAssignments::CancelledTransactions(
                    cancelled_txn_version_assignment
                        .into_iter()
                        .map(|(tx_digest, versions)| {
                            (tx_digest, versions.into_iter().map(|(id, v)| (id.0, v)).collect())
                        })
                        .collect(),
                ))
            } else {
            ...
            if protocol_config.record_additional_state_digest_in_prologue() {
                let additional_state_digest = if protocol_config.additional_consensus_digest_indirect_state() {
    ...
    impl IndirectStateObserver {
        ...
        pub fn fold_with(self, d1: AdditionalConsensusStateDigest) -> AdditionalConsensusStateDigest {
    ...
    fn test_additional_consensus_state() {
        ...
        fn observe(state: &mut AdditionalConsensusState, round: u64, timestamp: u64) {
            ...
            state.observe_commit(&protocol_config, 100, &TestConsensusCommit::empty(round, timestamp, 0));
        }
...
impl<C> ConsensusHandler<C> {
    pub(crate) fn new(
        ...
        let mut last_consensus_stats =
            epoch_store.get_last_consensus_stats().expect("Should be able to read last consensus index");
        ...
        let execution_scheduler_sender = ExecutionSchedulerSender::start(execution_scheduler, epoch_store.clone());
        ...
        Self {
            ...
            consensus_adapter,
            throughput_calculator,
            additional_consensus_state: AdditionalConsensusState::new(commit_rate_estimate_window_size),
...
impl CommitHandlerState {
    ...
    fn init_randomness<'a, 'epoch>(
        ...
        let mut randomness_manager = epoch_store
            .randomness_manager
            .get()
...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    fn handle_prior_consensus_commit(&mut self, consensus_commit: impl ConsensusCommitAPI) {
        assert!(self.epoch_store.protocol_config().record_additional_state_digest_in_prologue());
        ...
        self.additional_consensus_state.observe_commit(protocol_config, epoch_start_time, &consensus_commit);
    }

    #[cfg(test)]
    pub(crate) async fn handle_consensus_commit_for_test(&mut self, consensus_commit: impl ConsensusCommitAPI) {
    ...
    pub(crate) async fn handle_consensus_commit(&mut self, consensus_commit: impl ConsensusCommitAPI) {
        let protocol_config = self.epoch_store.protocol_config();
        ...
        if let Some(consensus_tx_status_cache) = self.epoch_store.consensus_tx_status_cache.as_ref() {
            consensus_tx_status_cache.update_last_committed_leader_round(last_committed_round as u32).await;
        }
        ...
        let (timestamp, leader_author, commit_sub_dag_index) = self.gather_commit_metadata(&consensus_commit);
        ...
        self.metrics.consensus_committed_subdags.with_label_values(&[&leader_author.to_string()]).inc();
        ...
        let transactions =
            self.filter_consensus_txns(state.initial_reconfig_state.clone(), &commit_info, &consensus_commit);
        ...
        let (should_accept_tx, lock, final_round) = self.handle_eop(&mut state, end_of_publish_transactions);
        ...
        self.create_pending_checkpoints(&mut state, &commit_info, &schedulables, &randomness_schedulables, final_round);
        ...
        self.throughput_calculator.add_transactions(timestamp, schedulables.len() as u64);
        ...
        self.execution_scheduler_sender.send(schedulables, assigned_versions, SchedulingSource::NonFastPath);
    ...
    fn handle_eop(
        ...
        let collected_eop = self.process_end_of_publish_transactions(state, end_of_publish_transactions);
        if collected_eop {
    ...
    fn record_deferral_deletion(&self, state: &mut CommitHandlerState) {
        let mut deferred_transactions = self.epoch_store.consensus_output_cache.deferred_transactions_v2.lock();
    ...
    fn create_pending_checkpoints(
        ...
        let checkpoint_height = self.epoch_store.calculate_pending_checkpoint_height(commit_info.round);
        ...
        let should_write_random_checkpoint =
            state.randomness_round.is_some() || (state.dkg_failed && !randomness_schedulables.is_empty());
        ...
        info!("Written pending checkpoint: {:?}", pending_checkpoint.details,);
    ...
    fn process_transactions(
        ...
        let mut shared_object_congestion_tracker = self.init_congestion_tracker(commit_info, false, &ordered_txns);
        ...
        let randomness_state_update_transaction =
            state.randomness_round.map(|round| Schedulable::RandomnessStateUpdate(epoch, round));
        debug!(
            "Randomness state update transaction: {:?}",
            randomness_state_update_transaction.as_ref().map(|t| t.key())
        );

        let mut transactions_to_schedule = Vec::with_capacity(ordered_txns.len());
        let mut randomness_transactions_to_schedule = Vec::with_capacity(ordered_randomness_txns.len());
        ...
        for transaction in ordered_randomness_txns {
            if state.dkg_failed {
                ...
                cancelled_txns.insert(*transaction.digest(), CancelConsensusCertificateReason::DkgFailed);
        ...
        self.metrics.consensus_handler_deferred_transactions.inc_by(total_deferred_txns as u64);
        self.metrics.consensus_handler_cancelled_transactions.inc_by(cancelled_txns.len() as u64);
        self.metrics
        ...
        let randomness_object_debts = shared_object_using_randomness_congestion_tracker.accumulated_debts(commit_info);
        if let Some(tx_object_debts) = self.epoch_store.tx_object_debts.get()
            && let Err(e) = tx_object_debts
                .try_send(object_debts.iter().chain(randomness_object_debts.iter()).map(|(id, _)| *id).collect())
        {
            info!("failed to send updated object debts to ExecutionTimeObserver: {e:?}");
        }

        state.output.set_congestion_control_object_debts(object_debts);
        state.output.set_congestion_control_randomness_object_debts(randomness_object_debts);
        ...
        if self.epoch_store.accumulators_enabled() {
            ...
            if state.randomness_round.is_some() {
                randomness_settlement = Some(Schedulable::AccumulatorSettlement(epoch, checkpoint_height + 1));
            }
        }

        let consensus_commit_prologue = (!commit_info.skip_consensus_commit_prologue_in_test).then_some(
            Schedulable::ConsensusCommitPrologue(epoch, commit_info.round, commit_info.consensus_commit_ref.index),
        );

        let schedulables: Vec<_> = itertools::chain!(
            consensus_commit_prologue.into_iter(),
            authenticator_state_update_transaction.into_iter().map(Schedulable::Transaction),
        ...
        let randomness_schedulables: Vec<_> = randomness_state_update_transaction
            .into_iter()
            .chain(randomness_transactions_to_schedule.into_iter().map(Schedulable::Transaction))
        ...
        if let Some(consensus_commit_prologue) = consensus_commit_prologue {
            assert!(matches!(schedulables[0], Schedulable::ConsensusCommitPrologue(..)));
            assert!(matches!(assigned_versions.0[0].0, TransactionKey::ConsensusCommitPrologue(..)));
        ...
        self.epoch_store.process_user_signatures(schedulables.iter().chain(randomness_schedulables.iter()));
    ...
    fn add_consensus_commit_prologue_transaction<'a>(
        ...
        for (txn_key, assigned_versions) in assigned_versions.0.iter() {
            ...
            if assigned_versions.shared_object_versions.iter().any(|(_, version)| version.is_cancelled()) {
                assert_reachable!("cancelled transactions");
                cancelled_txn_version_assignment.push((*d, assigned_versions.shared_object_versions.clone()));
            }
        }
    ...
    fn merge_and_reorder_transactions(
        ...
        commit_info: &ConsensusCommitInfo,
        user_transactions: Vec<VerifiedExecutableTransaction>,
    ) -> (Vec<VerifiedExecutableTransaction>, Vec<VerifiedExecutableTransaction>, HashMap<TransactionDigest, DeferralKey>)
    {
        let protocol_config = self.epoch_store.protocol_config();
        ...
        PostConsensusTxReorder::reorder(&mut txns, protocol_config.consensus_transaction_ordering());
        PostConsensusTxReorder::reorder(&mut randomness_txns, protocol_config.consensus_transaction_ordering());
    ...
    ) -> (Vec<VerifiedExecutableTransaction>, Vec<VerifiedExecutableTransaction>, HashMap<TransactionDigest, DeferralKey>)
    {
        ...
        let deferred_txs: Vec<_> = self
            .epoch_store
            .load_deferred_transactions_for_up_to_consensus_round_v2(&mut state.output, commit_info.round)
            .expect("db error")
            .into_iter()
            ...
            .collect();
        trace!("loading deferred transactions: {:?}", deferred_txs.iter().map(|tx| tx.digest()));
        ...
        let deferred_randomness_txs = if state.dkg_failed || state.randomness_round.is_some() {
            let txns: Vec<_> = self
                ...
                .collect();
            trace!("loading deferred randomness transactions: {:?}", txns.iter().map(|tx| tx.digest()));
            txns
        } else {
            vec![]
        };

        (deferred_txs, deferred_randomness_txs, previously_deferred_tx_digests)
    }
    ...
    fn init_congestion_tracker(
        ...
        let mut ret = SharedObjectCongestionTracker::from_protocol_config(
            self.epoch_store
                .consensus_quarantine
                .read()
                .load_initial_object_debts(&self.epoch_store, commit_info.round, for_randomness, txns)
                .expect("db error"),
            self.epoch_store.protocol_config(),
            for_randomness,
        );
    ...
    fn process_jwks(
        ...
        for (authority_name, jwk_id, jwk) in new_jwks {
            self.epoch_store.record_jwk_vote(&mut state.output, commit_info.round, authority_name, &jwk_id, &jwk);
        }
    }

    fn process_capability_notifications(&self, capability_notifications: Vec<AuthorityCapabilitiesV2>) {
        for capabilities in capability_notifications {
            self.epoch_store.record_capabilities_v2(&capabilities).expect("db error");
        }
    }
    ...
    fn process_execution_time_observations(
        ...
        for ExecutionTimeObservation { authority, generation, estimates } in execution_time_observations {
            ...
            let authority_index = self.epoch_store.committee().authority_index(&authority).unwrap();
            estimator.process_observations_from_consensus(authority_index, Some(generation), &estimates);
            state.output.insert_execution_time_observation(authority_index, generation, estimates);
        }
    }

    fn process_checkpoint_signature_messages(&self, checkpoint_signature_messages: Vec<CheckpointSignatureMessage>) {
    ...
    async fn process_dkg_updates(
        ...
        let randomness_manager = randomness_manager.expect("randomness manager should exist if randomness is enabled");

        let randomness_dkg_updates = self.process_randomness_dkg_messages(randomness_manager, randomness_dkg_messages);

        let randomness_dkg_confirmation_updates =
            self.process_randomness_dkg_confirmations(state, randomness_manager, randomness_dkg_confirmations);

        if randomness_dkg_updates || randomness_dkg_confirmation_updates {
            randomness_manager.advance_dkg(&mut state.output, commit_info.round).await.expect("epoch ended");
        }
    }
    ...
    fn process_randomness_dkg_messages(
        ...
        for (authority, bytes) in randomness_dkg_messages {
            match bcs::from_bytes(&bytes) {
                ...
                Err(e) => {
                    warn!("Failed to deserialize RandomnessDkgMessage from {:?}: {e:?}", authority.concise(),);
                }
            }
        }
    ...
    fn process_randomness_dkg_confirmations(
        ...
        for (authority, bytes) in randomness_dkg_confirmations {
            match bcs::from_bytes(&bytes) {
                ...
                Err(e) => {
                    warn!("Failed to deserialize RandomnessDkgConfirmation from {:?}: {e:?}", authority.concise(),);
                }
            }
        }
    ...
    fn process_end_of_publish_transactions(
        ...
        let mut eop_aggregator = self
            .epoch_store
            .end_of_publish
            .try_lock()
            .expect("No contention on end_of_publish as it is only accessed from consensus handler");
    ...
    fn advance_eop_state_machine(
        ...
        let previous_commits_have_deferred_txns = !self.epoch_store.deferred_transactions_empty_v2();
    ...
    fn gather_commit_metadata(&self, consensus_commit: &impl ConsensusCommitAPI) -> (u64, AuthorityIndex, u64) {
        let timestamp = consensus_commit.commit_timestamp_ms();
        let leader_author = consensus_commit.leader_author_index();
        let commit_sub_dag_index = consensus_commit.commit_sub_dag_index();

        let system_time_ms = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as i64;
        ...
        self.metrics.consensus_timestamp_bias.observe(consensus_timestamp_bias_seconds);
    ...
    fn create_authenticator_state_update(
        ...
        let new_jwks =
            self.epoch_store.get_new_jwks(last_committed_round).expect("Unrecoverable error in consensus handler");

        if !new_jwks.is_empty() {
            let authenticator_state_update_transaction =
                authenticator_state_update_transaction(&self.epoch_store, commit_info.round, new_jwks);
            debug!(
    ...
    fn filter_consensus_txns(
        ...
        for (block, parsed_transactions) in consensus_commit.transactions() {
            ...
            self.epoch_store
                .set_consensus_tx_status(ConsensusPosition::ping(epoch, block), ConsensusTxStatus::Finalized);

            for (tx_index, parsed) in parsed_transactions.into_iter().enumerate() {
                let position = ConsensusPosition { epoch, block, index: tx_index as TransactionIndex };
                        ...
                        self.epoch_store.submitted_transaction_cache.increment_submission_count(digest)
                    {
                        if let Some(ref traffic_controller) = self.traffic_controller {
                            ...
                            for addr in submitter_client_addrs {
                                traffic_controller.tally(TrafficTally::new(Some(addr), None, None, spam_weight.clone()));
                            }
                        } else {
                            warn!(
                ...
                if matches!(parsed.transaction.kind, ConsensusTransactionKind::UserTransaction(_)) {
                    self.epoch_store.set_consensus_tx_status(position, ConsensusTxStatus::Finalized);
                    num_finalized_user_transactions[author] += 1;
                }
                let kind = classify(&parsed.transaction);
                self.metrics.consensus_handler_processed.with_label_values(&[kind]).inc();
                self.metrics
                ...
                if matches!(
                    ...
                    ConsensusTransactionKind::CertifiedTransaction(_) | ConsensusTransactionKind::UserTransaction(_)
                ) {
                ...
                if parsed.transaction.is_mfp_transaction() && !self.epoch_store.protocol_config().mysticeti_fastpath() {
                    debug!("Ignoring MFP transaction {:?} because MFP is disabled", parsed.transaction.key());
                    continue;
                }

                if let ConsensusTransactionKind::CertifiedTransaction(certificate) = &parsed.transaction.kind
                ...
                if matches!(
                    &parsed.transaction.kind,
                    ConsensusTransactionKind::UserTransaction(_) | ConsensusTransactionKind::CertifiedTransaction(_)
                ) {
                    let author_name = self.epoch_store.committee().authority_by_index(author as u32).unwrap();
        ...
        for (i, authority) in self.committee.authorities() {
            ...
            self.metrics
                ...
                .with_label_values(&[hostname])
                .set(self.last_consensus_stats.stats.get_num_user_transactions(i.value()) as i64);
            self.metrics
    ...
    fn deduplicate_consensus_txns(
        ...
        for (seq, (transaction, cert_origin)) in transactions.into_iter().enumerate() {
            ...
            let certificate_author = *self.epoch_store.committee().authority_by_index(cert_origin).unwrap();
            ...
            let Some(verified_transaction) = self.epoch_store.verify_consensus_transaction(sequenced_transaction) else {
                continue;
            };
            ...
            if let Some(tx_digest) = key.user_transaction_digest() {
                self.epoch_store.cache_recently_finalized_transaction(tx_digest);
            }
                ...
                SequencedConsensusTransactionKind::External(consensus_transaction) => {
                    match consensus_transaction.kind {
                        ...
                        ConsensusTransactionKind::CertifiedTransaction(cert) => {
                            ...
                            let transaction = VerifiedExecutableTransaction::new_from_certificate(cert);
                        ...
                        ConsensusTransactionKind::UserTransaction(tx) => {
                            ...
                            let transaction = VerifiedExecutableTransaction::new_from_consensus(tx, epoch);
                            commit_handler_input.user_transactions.push(transaction);
                        }

                        // === State machines ===
                        ConsensusTransactionKind::EndOfPublish(authority_public_key_bytes) => {
                            commit_handler_input.end_of_publish_transactions.push(authority_public_key_bytes);
                        }
                        ConsensusTransactionKind::NewJWKFetched(authority_public_key_bytes, jwk_id, jwk) => {
                            commit_handler_input.new_jwks.push((authority_public_key_bytes, jwk_id, jwk));
                        }
                        ConsensusTransactionKind::RandomnessDkgMessage(authority_public_key_bytes, items) => {
                            commit_handler_input.randomness_dkg_messages.push((authority_public_key_bytes, items));
                        }
                        ConsensusTransactionKind::RandomnessDkgConfirmation(authority_public_key_bytes, items) => {
                            commit_handler_input.randomness_dkg_confirmations.push((authority_public_key_bytes, items));
                        }
                        ConsensusTransactionKind::CapabilityNotificationV2(authority_capabilities_v2) => {
                            commit_handler_input.capability_notifications.push(authority_capabilities_v2);
                        }
                        ConsensusTransactionKind::ExecutionTimeObservation(execution_time_observation) => {
                            commit_handler_input.execution_time_observations.push(execution_time_observation);
                        }
                        ConsensusTransactionKind::CheckpointSignatureV2(checkpoint_signature_message) => {
                            commit_handler_input.checkpoint_signature_messages.push(*checkpoint_signature_message);
                        }
    ...
    async fn send_end_of_publish_if_needed(&self) {
        ...
        if let Err(err) = self.consensus_adapter.submit(end_of_publish, None, &self.epoch_store, None, None) {
...
pub(crate) struct ExecutionSchedulerSender {
    ...
    sender: monitored_mpsc::UnboundedSender<(Vec<Schedulable>, AssignedTxAndVersions, SchedulingSource)>,
}

impl ExecutionSchedulerSender {
    fn start(execution_scheduler: Arc<ExecutionScheduler>, epoch_store: Arc<AuthorityPerEpochStore>) -> Self {
    ...
    async fn run(
        mut recv: monitored_mpsc::UnboundedReceiver<(Vec<Schedulable>, AssignedTxAndVersions, SchedulingSource)>,
        execution_scheduler: Arc<ExecutionScheduler>,
        epoch_store: Arc<AuthorityPerEpochStore>,
    ) {
                ...
                .map(|txn| {
                    ...
                    (
                        ...
                        ExecutionEnv::new()
                            ...
                            .with_assigned_versions(assigned_versions.get(&key).cloned().unwrap_or_default()),
                    )
                })
                .collect();
...
impl<'de> Deserialize<'de> for SequencedConsensusTransactionKind {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let serializable = SerializableSequencedConsensusTransactionKind::deserialize(deserializer)?;
...
impl SequencedConsensusTransactionKind {
    pub fn key(&self) -> SequencedConsensusTransactionKey {
        match self {
            SequencedConsensusTransactionKind::External(ext) => SequencedConsensusTransactionKey::External(ext.key()),
            SequencedConsensusTransactionKind::System(txn) => SequencedConsensusTransactionKey::System(*txn.digest()),
        }
    }
...
impl SequencedConsensusTransaction {
    ...
    pub fn as_consensus_txn(&self) -> Option<&SenderSignedData> {
        match &self.transaction {
            ...
            SequencedConsensusTransactionKind::System(txn) if txn.is_consensus_tx() => Some(txn.data()),
            _ => None,
        }
    }
}
    ...
    async fn handle_certified_blocks(&self, blocks_output: CertifiedBlocksOutput) {
        ...
        let parsed_transactions = blocks_output
            ...
            .map(|certified_block| {
                ...
                let transactions = parse_block_transactions(&certified_block.block, &certified_block.rejected);
        ...
        for (block, transactions) in parsed_transactions.into_iter() {
            ...
            self.epoch_store
                .set_consensus_tx_status(ConsensusPosition::ping(epoch, block), ConsensusTxStatus::FastpathCertified);

            for (txn_idx, parsed) in transactions.into_iter().enumerate() {
                let position = ConsensusPosition { epoch, block, index: txn_idx as TransactionIndex };
                ...
                if parsed.rejected {
                    ...
                    self.epoch_store.set_consensus_tx_status(position, ConsensusTxStatus::Rejected);
                    self.metrics.consensus_block_handler_txn_processed.with_label_values(&["rejected"]).inc();
                    continue;
                }

                self.metrics.consensus_block_handler_txn_processed.with_label_values(&["certified"]).inc();
                ...
                if let ConsensusTransactionKind::UserTransaction(tx) = parsed.transaction.kind {
                    ...
                    self.epoch_store.set_consensus_tx_status(position, ConsensusTxStatus::FastpathCertified);
                    let tx = VerifiedTransaction::new_unchecked(*tx);
                    executable_transactions.push(Schedulable::Transaction(
                        VerifiedExecutableTransaction::new_from_consensus(tx, self.epoch_store.epoch()),
                    ));
                }
            }
        }
        ...
        self.metrics.consensus_block_handler_fastpath_executions.inc_by(executable_transactions.len() as u64);
...
mod tests {
    ...
    use sui_types::{
        base_types::{random_object_ref, AuthorityName, ExecutionDigests, FullObjectRef, ObjectID, SuiAddress},
        committee::Committee,
        ...
        object::Object,
        transaction::{CertifiedTransaction, SenderSignedData, TransactionData, TransactionDataAPI},
    };

    use super::*;
    use crate::{
        authority::{authority_per_epoch_store::ConsensusStatsAPI, test_authority_builder::TestAuthorityBuilder},
        checkpoints::CheckpointServiceNoop,
        consensus_adapter::consensus_tests::{test_certificates_with_gas_objects, test_user_transaction},
    ...
    async fn test_consensus_commit_handler() {
        ...
        // 12 gas objects.
        let gas_objects: Vec<Object> =
            (0 .. 12).map(|_| Object::with_id_owner_for_testing(ObjectID::random(), sender)).collect();
        // 4 owned objects.
        let owned_objects: Vec<Object> =
            (0 .. 4).map(|_| Object::with_id_owner_for_testing(ObjectID::random(), sender)).collect();
        // 6 shared objects.
        let shared_objects: Vec<Object> = (0 .. 6).map(|_| Object::shared_for_testing()).collect::<Vec<_>>();
        let mut all_objects = gas_objects.clone();
        all_objects.extend(owned_objects.clone());
        all_objects.extend(shared_objects.clone());

        let network_config = sui_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
        ...
        for (i, gas_object) in gas_objects[0 .. 8].iter().enumerate() {
            ...
            let transaction =
                test_user_transaction(&state, sender, &keypair, gas_object.clone(), vec![input_object]).await;
        ...
        let certified_transactions = [
            test_certificates_with_gas_objects(&state, &gas_objects[8 .. 10], shared_objects[4].clone()).await,
            test_certificates_with_gas_objects(&state, &gas_objects[10 .. 12], shared_objects[5].clone()).await,
        ]
        .concat();
        ...
        for (i, consensus_transaction) in user_transactions
            .iter()
            .map(|t| ConsensusTransaction::new_user_transaction_message(&state.name, t.inner().clone()))
            .chain(
        ...
        // AND process the consensus commit once
        {
            ...
            tokio::time::timeout(std::time::Duration::from_secs(5), &mut waiter).await.unwrap_err();

            // lift backpressure
            backpressure_manager.set_backpressure(false);

            // waiter completes now.
            tokio::time::timeout(std::time::Duration::from_secs(100), waiter).await.unwrap();
        }
        ...
        assert_eq!(last_consensus_stats_1.index.transaction_index, num_transactions as u64);
        ...
        assert_eq!(last_consensus_stats_1.stats.get_num_messages(0), num_blocks as u64);
        assert_eq!(last_consensus_stats_1.stats.get_num_user_transactions(0), num_transactions as u64);

        // THEN check for execution status of user transactions.
        for (i, t) in user_transactions.iter().enumerate() {
            let digest = t.digest();
            if let Ok(Ok(_)) =
                tokio::time::timeout(std::time::Duration::from_secs(10), state.notify_read_effects("", *digest)).await
            {
    ...
    async fn test_consensus_block_handler() {
        ...
        // 8 gas objects.
        let gas_objects: Vec<Object> =
            (0 .. 8).map(|_| Object::with_id_owner_for_testing(ObjectID::random(), sender)).collect();
        // 4 owned objects.
        let owned_objects: Vec<Object> =
            (0 .. 4).map(|_| Object::with_id_owner_for_testing(ObjectID::random(), sender)).collect();
        // 4 shared objects.
        let shared_objects: Vec<Object> = (0 .. 4).map(|_| Object::shared_for_testing()).collect::<Vec<_>>();
        let mut all_objects = gas_objects.clone();
        all_objects.extend(owned_objects.clone());
        all_objects.extend(shared_objects.clone());
        ...
        let execution_scheduler_sender =
            ExecutionSchedulerSender::start(state.execution_scheduler().clone(), epoch_store.clone());
        ...
        let serialized_transactions: Vec<_> = transactions
            ...
            .map(|t| {
                Transaction::new(
                    bcs::to_bytes(&ConsensusTransaction::new_user_transaction_message(&state.name, t.inner().clone()))
                        .unwrap(),
                )
            })
            .collect();
        ...
        let block = VerifiedBlock::new_for_test(
            TestBlock::new(100, 1).set_transactions(serialized_transactions.clone()).build(),
        );
        ...
        block_handler
            .handle_certified_blocks(CertifiedBlocksOutput {
                blocks: vec![CertifiedBlock { block: block.clone(), rejected: rejected_transactions.clone() }],
            })
            .await;
        ...
        for txn_idx in 0 .. transactions.len() {
            ...
            if rejected_transactions.contains(&(txn_idx as TransactionIndex)) {
                ...
                assert_eq!(consensus_tx_status_cache.get_transaction_status(&position), None,);
            }
        }
        ...
        for (i, t) in transactions.iter().enumerate() {
            ...
            if tokio::time::timeout(
                ...
                state.get_transaction_cache_reader().notify_read_fastpath_transaction_outputs(&[*digest]),
            )
            .await
            .is_err()
            {
    ...
    fn test_order_by_gas_price() {
        ...
        assert_eq!(to_short_strings(v), vec!["transaction(100)".to_string(), "transaction(42)".to_string(),]);

        let mut v = vec![user_txn(1200), user_txn(12), user_txn(1000), user_txn(42), user_txn(100), user_txn(1000)];
        PostConsensusTxReorder::reorder(&mut v, ConsensusTransactionOrdering::ByGasPrice);
    ...
    async fn test_checkpoint_signature_dedup() {
        ...
        let v2_a =
            ConsensusTransaction::new_checkpoint_signature_message_v2(CheckpointSignatureMessage { summary: v2_s1 });

        let v2_s2 = make_signed();
        let v2_digest_b = v2_s2.data().digest();
        let v2_b =
            ConsensusTransaction::new_checkpoint_signature_message_v2(CheckpointSignatureMessage { summary: v2_s2 });
        ...
        let v2_dup = ConsensusTransaction::new_checkpoint_signature_message_v2(CheckpointSignatureMessage {
            summary: v2_s1_clone,
        });

        let to_tx = |ct: &ConsensusTransaction| Transaction::new(bcs::to_bytes(ct).unwrap());
        let block = VerifiedBlock::new_for_test(
            TestBlock::new(100, 0).set_transactions(vec![to_tx(&v2_a), to_tx(&v2_b), to_tx(&v2_dup)]).build(),
        );
        ...
        assert!(epoch_store.is_consensus_message_processed(&v2_key_a).unwrap());
        assert!(epoch_store.is_consensus_message_processed(&v2_key_b).unwrap());
    }
    ...
    async fn test_verify_consensus_transaction_filters_mismatched_authorities() {
        ...
        let valid_checkpoint_signed = SignedCheckpointSummary::new(epoch, summary, &*state.secret, state.name);
        let valid_checkpoint_digest = valid_checkpoint_signed.data().digest();
        let valid_checkpoint = ConsensusTransaction::new_checkpoint_signature_message_v2(CheckpointSignatureMessage {
        ...
        assert!(
            epoch_store.is_consensus_message_processed(&valid_eop_key).unwrap(),
            "Valid EndOfPublish should have been processed"
        );

        let valid_checkpoint_key = SK::External(CK::CheckpointSignatureV2(state.name, 42, valid_checkpoint_digest));
        assert!(
            epoch_store.is_consensus_message_processed(&valid_checkpoint_key).unwrap(),
        ...
        let mismatched_checkpoint_key =
            SK::External(CK::CheckpointSignatureV2(wrong_authority, 42, mismatched_checkpoint_digest));
        assert!(
    ...
    fn to_short_strings(v: Vec<VerifiedExecutableTransaction>) -> Vec<String> {
        v.into_iter().map(|t| format!("transaction({})", t.transaction_data().gas_price())).collect()
    }
