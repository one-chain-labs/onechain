...
impl ProcessTransactionState {
    pub fn record_conflicting_transaction_if_any(
        ...
        if let SuiErrorKind::ObjectLockConflict { obj_ref, pending_transaction: transaction } = err.as_inner() {
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(&self, validity_threshold: StakeUnit) -> bool {
        ...
        let invalid_sig_stake: StakeUnit = self
            ...
            .filter_map(|(e, _, stake)| {
                if matches!(e.as_inner(), SuiErrorKind::FailedToVerifyTxCertWithExecutedEffects { .. }) {
                    Some(stake)
                } else {
                    None
                }
    ...
    fn handle_transaction_response_with_signed(
        ...
        match state.tx_signatures.insert(plain_tx.clone()) {
            InsertResult::NotEnoughVotes { bad_votes, bad_authorities } => {
                ...
                if bad_votes > 0 {
                    state.errors.push((
                        SuiErrorKind::InvalidSignature { error: "Individual signature verification failed".to_string() }
                            .into(),
                        bad_authorities,
                        bad_votes,
                    ));
