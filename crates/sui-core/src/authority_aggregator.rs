...
use sui_authority_aggregation::{quorum_map_then_reduce_with_timeout, ReduceOutput};
use sui_config::genesis::Genesis;
use sui_network::{default_mysten_network_config, DEFAULT_CONNECT_TIMEOUT_SEC, DEFAULT_REQUEST_TIMEOUT_SEC};
use sui_swarm_config::network_config::NetworkConfig;
use sui_types::{
    base_types::*,
    committee::{Committee, CommitteeWithNetworkMetadata, StakeUnit},
...
impl Default for TimeoutConfig {
    fn default() -> Self {
        Self { pre_quorum_timeout: Duration::from_secs(60), post_quorum_timeout: Duration::from_secs(7) }
    }
}
...
impl<A: Clone> AuthorityAggregator<A> {
    ...
    pub fn get_display_name(&self, name: &AuthorityName) -> String {
        self.validator_display_names.get(name).cloned().unwrap_or_else(|| name.concise().to_string())
    }
}
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
                    ...
                    match result {
                        Ok(object_info) => {
                            ...
                            if state
                                ...
                                .as_ref()
                                .is_none_or(|latest| object_info.object.version() > latest.version())
                            {
    ...
    fn handle_process_transaction_error(&self, state: ProcessTransactionState) -> AggregatorProcessTransactionError {
...
impl<'a> AuthorityAggregatorBuilder<'a> {
    ...
    fn get_network_committee(&self) -> CommitteeWithNetworkMetadata {
        self.get_genesis().unwrap_or_else(|| panic!("need either NetworkConfig or Genesis.")).committee_with_network()
    }

    fn get_committee_authority_names_to_hostnames(&self) -> HashMap<AuthorityName, String> {
        if let Some(genesis) = self.get_genesis() {
            let state = genesis.sui_system_object().into_genesis_version_for_tooling();
            state
                .validators
                .active_validators
                .iter()
    ...
    fn get_reference_gas_price(&self) -> u64 {
        self.reference_gas_price.unwrap_or_else(|| self.get_genesis().map(|g| g.reference_gas_price()).unwrap_or(1000))
    }
    ...
    pub fn build_mock_authority_aggregator(self) -> AuthorityAggregator<MockAuthorityApi> {
        ...
        let clients = committee
            .names()
            .map(|name| (*name, MockAuthorityApi::new(Duration::from_millis(100), Arc::new(std::sync::Mutex::new(30)))))
            .collect();
