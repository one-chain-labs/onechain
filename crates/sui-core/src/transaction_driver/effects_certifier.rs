...
use crate::{
    ...
    transaction_driver::{
        error::{aggregate_request_errors, AggregatedEffectsDigests, TransactionDriverError, TransactionRequestError},
...
impl EffectsCertifier {
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        let (consensus_position, full_effects) = match submit_txn_result {
            ...
            SubmitTxResult::Rejected { error } => {
                return Err(TransactionDriverError::ClientInternal {
                    error: format!("Unexpected submission error in get_certified_finalized_effects(): {}", error),
                });
            }
        };

        let mut retrier = RequestRetrier::new(authority_aggregator, client_monitor, tx_type, vec![]);
        ...
        let (acknowledgments_result, mut full_effects_result) = join!(
            ...
            async {
                ...
                let (name, client) = retrier.next_target().expect("there should be at least 1 target");
                current_target = name;
                full_effects_start_time = Some(Instant::now());
                self.get_full_effects(client, tx_digest, consensus_position, options, ping).await
            },
        );
        ...
        loop {
            ...
            match full_effects_result {
                Ok((effects_digest, executed_data, _fast_path)) => {
                    if effects_digest != certified_digest {
                        ...
                        return Ok(self.get_quorum_transaction_response(effects_digest, executed_data));
                    }
                }
                Err(e) => {
            ...
            current_target = name;
            full_effects_start_time = Some(Instant::now());
            full_effects_result = self.get_full_effects(client, tx_digest, consensus_position, options, ping).await;
        }
    }
    ...
    {
        let request =
            WaitForEffectsRequest { transaction_digest: tx_digest, consensus_position, include_details: true, ping };

        match timeout(WAIT_FOR_EFFECTS_TIMEOUT, client.wait_for_effects(request, options.forwarded_client_addr)).await {
            Ok(Ok(response)) => match response {
                WaitForEffectsResponse::Executed { effects_digest, details, fast_path } => {
                    if let Some(details) = details {
                        tracing::Span::current().record("ret_effects_digest", format!("{:?}", effects_digest));
                        Ok((effects_digest, details, fast_path))
                    } else {
                        tracing::debug!("Execution data not found, retrying...");
                        Err(TransactionRequestError::ValidatorInternal("Execution data not found".to_string()))
                    }
                }
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        self.metrics.certified_effects_ack_attempts.with_label_values(&[tx_type.as_str(), ping_label]).inc();
        ...
        let mut effects_digest_aggregators: BTreeMap<TransactionEffectsDigest, StatusAggregator<()>> = BTreeMap::new();
        ...
        let mut non_retriable_errors_aggregator = StatusAggregator::<TransactionRequestError>::new(committee.clone());
        // Collect responses from validators which observed the transaction getting rejected,
        // and rejected the transaction with errors retriable with new transaction submissions.
        let mut retriable_errors_aggregator = StatusAggregator::<TransactionRequestError>::new(committee.clone());
        // Collect responses from validators which observed the transaction getting rejected,
        ...
        while let Some((name, response)) = futures.next().await {
            match response {
                Ok(WaitForEffectsResponse::Executed { effects_digest, details: _, fast_path }) => {
                    ...
                    if aggregator.reached_quorum_threshold() {
                        ...
                        if fast_path_aggregator.reached_quorum_threshold() {
                            ...
                            let display_name = authority_aggregator.get_display_name(&submitted_tx_to_validator);
                ...
                Ok(WaitForEffectsResponse::Rejected { error }) => {
                    ...
                    self.metrics.rejection_acks.with_label_values(&[tx_type.as_str(), ping_label]).inc();
                }
                Ok(WaitForEffectsResponse::Expired { epoch, round }) => {
                    ...
                    retriable_errors_aggregator.insert(name, error);
                    self.metrics.expiration_acks.with_label_values(&[tx_type.as_str(), ping_label]).inc();
                }
                Err(error) => {
            ...
            let executed_weight: u64 = effects_digest_aggregators.values().map(|agg| agg.total_votes()).sum();
            ...
            if total_weight >= committee.quorum_threshold() {
                ...
                if non_retriable_errors_aggregator.total_votes() + remaining_weight < committee.validity_threshold()
                    && retriable_errors_aggregator.total_votes() + non_retriable_errors_aggregator.total_votes()
                        >= committee.validity_threshold()
                {
                    ...
                    return Err(TransactionDriverError::Aborted {
                        ...
                        observed_effects_digests: AggregatedEffectsDigests { digests: observed_effects_digests },
                    });
                }
            }
        }
        ...
        let retriable_weight = retriable_errors_aggregator.total_votes() + reason_not_found_aggregator.total_votes();
        ...
        let mut observed_effects_digests = Vec::<(TransactionEffectsDigest, Vec<AuthorityName>, StakeUnit)>::new();
        for (effects_digest, aggregator) in effects_digest_aggregators {
            ...
            observed_effects_digests.push((effects_digest, aggregator.authorities(), aggregator.total_votes()));
        }
        if submission_retriable {
            Err(TransactionDriverError::Aborted {
                ...
                submission_retriable_errors: aggregate_request_errors(retriable_errors_aggregator.status_by_authority()),
                observed_effects_digests: AggregatedEffectsDigests { digests: observed_effects_digests },
            })
        } else {
            if observed_effects_digests.len() <= 1 {
                debug_fatal!("Expect at least 2 effects digests, but got {:?}", observed_effects_digests);
            }
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        let backoff = ExponentialBackoff::from_millis(100).max_delay(Duration::from_secs(2)).map(jitter);
        ...
        for (attempt, delay) in backoff.enumerate() {
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    tracing::trace!(?name, "Wait for effects acknowledgement (attempt {attempt}): rpc error: {:?}", e);
                }
            };
            sleep(delay).await;
        }
