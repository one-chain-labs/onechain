...
impl EffectsCertifier {
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        let mut retrier = RequestRetrier::new(authority_aggregator, client_monitor, tx_type, vec![], vec![]);
        ...
        loop {
            ...
            current_target = name;
            full_effects_start_time = Some(Instant::now());
            full_effects_result = self.get_full_effects(client, tx_digest, tx_type, consensus_position, options).await;
        }
    }
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        let backoff = ExponentialBackoff::new(Duration::from_millis(100), MAX_WAIT_FOR_EFFECTS_RETRY_DELAY);
