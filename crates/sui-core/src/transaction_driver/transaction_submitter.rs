...
impl TransactionSubmitter {
        ...
        A: AuthorityAPI + Send + Sync + 'static + Clone,
    {
        ...
        self.metrics.submit_amplification_factor.observe(amplification_factor as f64);

        let mut retrier =
            RequestRetrier::new(authority_aggregator, client_monitor, tx_type, options.allowed_validators.clone());
            ...
            match request_rpcs.next().await {
                Some((name, display_name, Ok(result))) => {
                    self.metrics
                        ...
                        .with_label_values(&[&display_name, tx_type.as_str(), ping_label])
                        .inc();
                    self.metrics.submit_transaction_retries.observe(retries as f64);
                ...
                Some((name, display_name, Err(e))) => {
                    let error_type = if e.is_submission_retriable() { "retriable" } else { "non_retriable" };
                    self.metrics
                        .validator_submit_transaction_errors
                        .with_label_values(&[&display_name, error_type, tx_type.as_str(), ping_label])
                        .inc();
