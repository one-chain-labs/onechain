...
impl MockAuthority {
    ...
    fn set_ack_response(&self, tx_digest: TransactionDigest, response: WaitForEffectsResponse) {
        self.ack_responses.lock().unwrap().insert(tx_digest, response);
    }

    fn set_full_response(&self, tx_digest: TransactionDigest, response: WaitForEffectsResponse) {
        self.full_responses.lock().unwrap().insert(tx_digest, response);
    }
}
...
impl AuthorityAPI for MockAuthority {
    ...
    async fn wait_for_effects(
        ...
        let responses = if request.include_details { &self.full_responses } else { &self.ack_responses };
        ...
        if let Some(response) = maybe_response {
            ...
            Err(SuiErrorKind::TransactionNotFound { digest: request.transaction_digest.unwrap() }.into())
        }
    }
...
async fn test_successful_certified_effects() {
    ...
    let client_monitor = Arc::new(ValidatorClientMonitor::new_for_test(authority_aggregator.clone()));
    ...
    let executed_response_ack = WaitForEffectsResponse::Executed { effects_digest, details: None, fast_path: false };
    ...
    let consensus_position = ConsensusPosition { block: BlockRef::MIN, index: 0, epoch };
    let name = authority_aggregator.authority_clients.keys().next().unwrap();
    ...
    let submit_tx_result =
        SubmitTxResult::Executed { effects_digest, details: Some(Box::new(executed_data.clone())), fast_path: false };
    let result = certifier
        .get_certified_finalized_effects(
            &authority_aggregator,
            &client_monitor,
...
async fn test_transaction_rejected_non_retriable() {
    ...
    let epoch = 0;
    let consensus_position = ConsensusPosition { epoch, block: BlockRef::MIN, index: 0 };
    let options = SubmitTransactionOptions::default();
...
async fn test_transaction_rejected_retriable() {
    ...
    let retriable_rejected_response = WaitForEffectsResponse::Rejected {
        error: Some(
            SuiErrorKind::UserInputError {
                error: UserInputError::ObjectNotFound { object_id: random_object_ref().0, version: None },
            }
            .into(),
        ),
    };
...
async fn test_transaction_expired() {
    ...
    let expired_response = WaitForEffectsResponse::Expired { epoch: 42, round: Some(100) };
...
async fn test_mixed_rejected_and_expired() {
    ...
    for (i, authority_name) in authorities.iter().enumerate() {
        let client = authority_aggregator.authority_clients.get(authority_name).unwrap().authority_client();
        if i % 2 == 0 {
            client.set_ack_response(tx_digest, non_retriable_rejected_response.clone());
            client.set_full_response(tx_digest, non_retriable_rejected_response.clone());
        } else {
...
async fn test_mixed_rejected_reasons() {
    ...
    {
        tracing::debug!("Case #2: Test 1 retriable, 1 not found, and 2 non-retriable reasons that arrive later");
        let authority_aggregator = Arc::new(create_test_authority_aggregator());
        let authorities: Vec<_> = authority_aggregator.authority_clients.keys().collect();
        for (i, authority_name) in authorities.iter().enumerate() {
            let client = authority_aggregator.authority_clients.get(authority_name).unwrap().authority_client();
...
async fn test_forked_execution() {
    ...
    for (i, authority_name) in authorities.iter().enumerate() {
        let client = authority_aggregator.authority_clients.get(authority_name).unwrap().authority_client();
        let digest = if i % 2 == 0 { effects_digest_1 } else { effects_digest_2 };
        let response = WaitForEffectsResponse::Executed { effects_digest: digest, details: None, fast_path: false };
    ...
    for (i, authority_name) in authorities.iter().enumerate() {
        ...
        let response = match i {
            ...
            2 => WaitForEffectsResponse::Rejected {
                error: Some(SuiErrorKind::ValidatorOverloadedRetryAfter { retry_after_secs: 5 }.into()),
            },
...
async fn test_full_effects_digest_mismatch() {
    ...
    // Set up successful acknowledgments from all authorities
    let executed_response_ack =
        WaitForEffectsResponse::Executed { effects_digest: certified_digest, details: None, fast_path: false };
