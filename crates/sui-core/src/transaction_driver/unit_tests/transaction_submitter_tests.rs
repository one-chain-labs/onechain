...
impl MockAuthority {
    ...
    fn set_submit_response(&self, tx_digest: TransactionDigest, response: Result<SubmitTxResult, SuiError>) {
        self.submit_responses.lock().unwrap().insert(tx_digest, response);
    }
...
impl AuthorityAPI for MockAuthority {
    async fn submit_transaction(
        ...
        let maybe_response = match raw_request.transactions.first() {
            Some(tx_bytes) => {
                let tx: Transaction = bcs::from_bytes(tx_bytes).map_err(|e| SuiErrorKind::GenericAuthorityError {
        ...
        if let Some(response) = maybe_response {
            match response {
                Ok(result) => Ok(SubmitTxResponse { results: vec![result] }),
                Err(e) => Err(e),
            }
        } else {
            // Default response
            let consensus_position = ConsensusPosition { block: BlockRef::MIN, index: 0, epoch: 0 };
            let result = SubmitTxResult::Submitted { consensus_position };
            Ok(SubmitTxResponse { results: vec![result] })
        }
    }
...
fn create_test_authority_aggregator_with_rgp(
    ...
    let mut aggregator = AuthorityAggregatorBuilder::from_committee(committee).build_custom_clients(authority_clients);
...
fn create_test_submit_request(gas_price: u64) -> SubmitTxRequest {
    ...
    let tx_data = TestTransactionBuilder::new(sender, gas_object_ref, gas_price).transfer_oct(None, sender).build();
...
async fn test_submit_transaction_with_amplification() {
    ...
    let (authority_aggregator, mock_authorities) = create_test_authority_aggregator_with_rgp(reference_gas_price);
    ...
    // Test 1: Transaction with 1x RGP (amplification factor = 1)
    {
        ...
        for mock_authority in &mock_authorities {
            mock_authority.set_submit_response(
                ...
                Ok(SubmitTxResult::Submitted {
                    consensus_position: ConsensusPosition { block: BlockRef::MIN, index: 0, epoch: 0 },
                }),
            );
        }
        ...
        let total_submissions: usize = mock_authorities.iter().map(|auth| auth.get_submission_count()).sum();
    ...
    // Test 3: Transaction with high amplification factor still works.
    {
        ...
        let total_submissions: usize = mock_authorities.iter().map(|auth| auth.get_submission_count()).sum();
        assert_eq!(total_submissions, 4, "Expected 4 submissions (all validators), got {}", total_submissions);
    }
    ...
    // Test 4: Transaction with errors in submission.
    {
        ...
        for (i, mock_authority) in mock_authorities.iter().enumerate() {
            if i < 2 {
                mock_authority.set_submit_response(
                    tx_digest,
                    Err(SuiErrorKind::ValidatorOverloadedRetryAfter { retry_after_secs: 1 }.into()),
                );
            } else {
                mock_authority.set_submit_response(
                    tx_digest,
