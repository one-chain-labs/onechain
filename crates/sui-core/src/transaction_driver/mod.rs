    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn submit_transaction_once(
        ...
        let raw_request = request.into_raw().map_err(TransactionDriverError::SerializationError)?;
        ...
        let response =
            timeout(Duration::from_secs(2), client.submit_transaction(raw_request, options.forwarded_client_addr))
                .await
                .map_err(|_| TransactionDriverError::TimeoutBeforeFinality)?
                .map_err(|e| TransactionDriverError::RpcFailure(name.concise().to_string(), e.to_string()))?;
