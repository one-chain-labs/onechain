...
use crate::{
    ...
    validator_client_monitor::{OperationFeedback, OperationType, ValidatorClientMetrics, ValidatorClientMonitor},
};
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    pub fn new(
        ...
        let monitor_config = node_config.and_then(|nc| nc.validator_client_monitor_config.clone()).unwrap_or_default();
        let client_monitor = ValidatorClientMonitor::new(monitor_config, client_metrics, shared_swap.clone());
    ...
    async fn run_latency_checks(self: Arc<Self>) {
        ...
        loop {
            ...
            for tx_type in [TxType::SingleWriter, TxType::SharedObject] {
                Self::ping_for_tx_type(self.clone(), &mut tasks, tx_type, MAX_JITTER, PING_REQUEST_TIMEOUT);
            }
    ...
    fn ping_for_tx_type(
        ...
        self.metrics.latency_check_runs.with_label_values(&[tx_type.as_str()]).inc();
        ...
        for name in validators {
            ...
            let task = async move {
                ...
                let ping = if tx_type == TxType::SingleWriter { PingType::FastPath } else { PingType::Consensus };

                // Now send a ping transaction to the chosen validator for the provided tx type
                match self_clone
                    .drive_transaction(
                        SubmitTxRequest::new_ping(ping),
                        SubmitTransactionOptions { allowed_validators: vec![name], ..Default::default() },
                        Some(ping_timeout),
                    )
                    .await
                {
    ...
    pub async fn drive_transaction(
        ...
        let amplification_factor = if request.ping.is_some() {
            ...
            let gas_price = request.transaction.as_ref().unwrap().transaction_data().gas_price();
            ...
            if amplification_factor == 0 {
                return Err(TransactionDriverError::ValidationFailed {
                    error: UserInputError::GasPriceUnderRGP { gas_price, reference_gas_price }.to_string(),
                });
            }
        ...
        let ping_label = if request.ping.is_some() { "true" } else { "false" };
        let timer = Instant::now();

        self.metrics.total_transactions_submitted.with_label_values(&[tx_type.as_str(), ping_label]).inc();
        ...
        let retry_loop = async {
            loop {
                ...
                match self.drive_transaction_once(amplification_factor, request.clone(), &options).await {
                    Ok(resp) => {
                    ...
                    Err(e) => {
                        ...
                        tracing::info!("Failed to finalize transaction (attempt {}): {}. Retrying ...", attempts, e);
        ...
        match timeout_duration {
            Some(duration) => {
                tokio::time::timeout(duration, retry_loop).await.unwrap_or_else(|_| {
                    ...
                    tracing::info!("Transaction timed out after {} attempts. Last error: {}", attempts, e);
                    Err(e)
                })
            }
    ...
    async fn drive_transaction_once(
        ...
        let amplification_factor = amplification_factor.min(auth_agg.committee.num_members() as u64);
        let start_time = Instant::now();
        ...
        let (name, submit_txn_result) = self
            .submitter
            .submit_transaction(&auth_agg, &self.client_monitor, tx_type, amplification_factor, request, options)
            .await?;
    ...
    fn enable_reconfig(self: &Arc<Self>, reconfig_observer: Arc<dyn ReconfigObserver<A> + Sync + Send>) {
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    fn update_authority_aggregator(&self, new_authorities: Arc<AuthorityAggregator<A>>) {
        tracing::info!("Transaction Driver updating AuthorityAggregator with committee {}", new_authorities.committee);
...
pub fn choose_transaction_driver_percentage(chain_id: Option<sui_types::digests::ChainIdentifier>) -> u8 {
