    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn drive_transaction(
        ...
        let mut backoff = ExponentialBackoff::new(Duration::from_millis(100), MAX_DRIVE_TRANSACTION_RETRY_DELAY);
        let mut attempts = 0;
                ...
                match self.drive_transaction_once(amplification_factor, request.clone(), &options).await {
                    ...
                    Err(e) => {
                        self.metrics
                            ...
                            .with_label_values(&[e.categorize().into(), tx_type.as_str(), ping_label])
                            .inc();
        ...
        match timeout_duration {
            Some(duration) => {
                tokio::time::timeout(duration, retry_loop).await.unwrap_or_else(|_| {
                    ...
                    if request.transaction.is_some() {
                        tracing::info!("User transaction timed out after {} attempts. Last error: {}", attempts, e);
                    }
                    Err(e)
                })
            }
    ...
    fn ping_for_tx_type(
        ...
        for name in validators {
            ...
            let task = async move {
                ...
                let ping_type = if tx_type == TxType::SingleWriter { PingType::FastPath } else { PingType::Consensus };
