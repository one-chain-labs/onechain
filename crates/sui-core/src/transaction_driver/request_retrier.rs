...
impl<A: Clone> RequestRetrier<A> {
    pub(crate) fn new(
        ...
        let ranked_validators =
            client_monitor.select_shuffled_preferred_validators(&auth_agg.committee, tx_type, SELECT_LATENCY_DELTA);
        let ranked_clients = ranked_validators
            .into_iter()
            .map(|name| (name, auth_agg.get_display_name(&name)))
            .filter(|(_name, display_name)| allowed_validators.is_empty() || allowed_validators.contains(display_name))
            .filter(|(_name, display_name)| blocked_validators.is_empty() || !blocked_validators.contains(display_name))
    ...
    pub(crate) fn next_target(&mut self) -> Result<(AuthorityName, Arc<SafeClient<A>>), TransactionDriverError> {
...
mod tests {
    ...
    pub(crate) fn get_authority_aggregator(committee_size: usize) -> AuthorityAggregator<MockAuthorityApi> {
    ...
    async fn test_next_target() {
        ...
        let mut retrier = RequestRetrier::new(&auth_agg, &client_monitor, TxType::SingleWriter, vec![], vec![]);

        for name in auth_agg.committee.names() {
            retrier.next_target().unwrap();
            retrier.add_error(*name, TransactionRequestError::TimedOutSubmittingTransaction).unwrap();
        }
    ...
    async fn test_allowed_validators() {
        ...
        println!("Case 1. Mix of unknown validators and one known validator");
        {
            ...
            let retrier =
                RequestRetrier::new(&auth_agg, &client_monitor, TxType::SingleWriter, allowed_validators, vec![]);
        ...
        println!("Case 2. Only unknown validators are provided");
        {
            let allowed_validators =
                vec![unknown_validator1.concise().to_string(), unknown_validator2.concise().to_string()];
    ...
    async fn test_blocked_validators() {
        ...
        let blocked_validators = auth_agg.committee.names().take(3).copied().collect::<Vec<_>>();
        let blocked_display_names =
            blocked_validators.iter().map(|name| auth_agg.get_display_name(name)).collect::<Vec<_>>();

        // Only the last validator will be picked up.
        let allowed_validator = auth_agg.committee.names().nth(3).unwrap();

        let mut retrier =
            RequestRetrier::new(&auth_agg, &client_monitor, TxType::SingleWriter, vec![], blocked_display_names);
    ...
    async fn test_add_error() {
        ...
        // Add retriable errors.
        {
            ...
            // 25% stake.
            retrier.add_error(authorities[0], TransactionRequestError::TimedOutSubmittingTransaction).unwrap();
            // 50% stake.
            retrier.add_error(authorities[1], TransactionRequestError::TimedOutSubmittingTransaction).unwrap();
            // 75% stake.
            retrier.add_error(authorities[1], TransactionRequestError::TimedOutSubmittingTransaction).unwrap();
            // 100% stake.
            retrier.add_error(authorities[1], TransactionRequestError::TimedOutSubmittingTransaction).unwrap();
            ...
            retrier
                .add_error(
                    ...
                    TransactionRequestError::RejectedAtValidator(
                        SuiErrorKind::UserInputError { error: UserInputError::EmptyCommandInput }.into(),
                    ),
                )
                .unwrap();
