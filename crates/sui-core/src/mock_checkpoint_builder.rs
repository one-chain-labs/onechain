...
impl MockCheckpointBuilder {
    ...
    pub fn get_next_checkpoint_number(&self) -> u64 {
        self.previous_checkpoint.as_ref().map(|c| c.sequence_number + 1).unwrap_or_default()
    }
    ...
    pub fn get_settlement_txns(&self, protocol_config: &ProtocolConfig) -> Vec<Transaction> {
        let effects: Vec<_> = self.transactions.iter().map(|e| e.effects.clone()).collect();
        ...
        let (settlement_txns, barrier_tx) =
            builder.build_tx(protocol_config, self.epoch, OBJECT_START_VERSION, checkpoint_height, checkpoint_seq);
