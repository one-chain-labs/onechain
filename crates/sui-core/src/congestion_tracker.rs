...
impl CongestionTracker {
    pub fn new() -> Self {
        Self { congestion_clearing_prices: Cache::new(10_000) }
    }
    ...
    pub fn process_checkpoint_effects(
        ...
        for effect in effects {
            let gas_price = transaction_cache_reader
                ...
                .transaction_data()
                .gas_price();
            if let Some(CongestedObjects(congested_objects)) = effect.status().get_congested_objects() {
        ...
        self.process_per_checkpoint_events(checkpoint.timestamp_ms, &congestion_events, &cleared_events);
    }
...
impl CongestionTracker {
    fn process_per_checkpoint_events(
        ...
        let congestion_info_map = self.compute_per_checkpoint_congestion_info(now, congestion_events, cleared_events);
        self.process_checkpoint_congestion(congestion_info_map);
    }

    fn get_suggested_gas_price_for_objects(&self, objects: impl Iterator<Item = ObjectID>) -> Option<u64> {
        let mut clearing_price = None;
        for object_id in objects {
            if let Some(info) = self.get_congestion_info(object_id) {
                let clearing_price_for_object = match info.last_success_time.cmp(&Some(info.last_cancellation_time)) {
    ...
    fn process_checkpoint_congestion(&self, congestion_info_map: HashMap<ObjectID, CongestionInfo>) {
        for (object_id, info) in congestion_info_map {
            self.congestion_clearing_prices.entry(object_id).and_compute_with(|maybe_entry| {
...
mod tests {
    ...
    fn test_process_events_new_congestion() {
        ...
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj1].into_iter()), Some(100));
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj2].into_iter()), Some(200));
    }
    ...
    fn test_process_events_congestion_then_success() {
        ...
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj].into_iter()), Some(100));

        // No cancellations in last checkpoint, so no congestion
        tracker.process_per_checkpoint_events(2000, &[], &[(150, vec![obj])]);
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj].into_iter()), None,);

        // next checkpoint has cancellations and successes, so the lowest success price is used.
        tracker.process_per_checkpoint_events(3000, &[(100, vec![obj])], &[(175, vec![obj]), (125, vec![obj])]);
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj].into_iter()), Some(125));
    }
    ...
    fn test_get_suggested_gas_price_multiple_objects() {
        ...
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj1, obj2].into_iter()), Some(200));

        // Process different congestion events
        tracker.process_per_checkpoint_events(2000, &[(100, vec![obj1]), (200, vec![obj2])], &[
            (100, vec![obj1]),
            (150, vec![obj2]),
        ]);
        // Should suggest the highest lowest success price
        assert_eq!(tracker.get_suggested_gas_price_for_objects(vec![obj1, obj2].into_iter()), Some(150));
    }
}
