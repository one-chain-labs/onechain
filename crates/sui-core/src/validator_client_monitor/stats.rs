...
use sui_types::{base_types::AuthorityName, committee::Committee, messages_grpc::TxType};
use tracing::debug;
...
impl ValidatorClientStats {
    pub fn new(init_reliability: f64, reliability_moving_window_size: usize, latency_moving_window_size: usize) -> Self {
        Self {
...
impl ClientObservedStats {
    ...
    pub fn record_interaction_result(&mut self, feedback: OperationFeedback) {
        let validator_stats = self.validator_stats.entry(feedback.authority_name).or_insert_with(|| {
    ...
    pub fn get_all_validator_stats(&self, committee: &Committee, tx_type: TxType) -> HashMap<AuthorityName, Duration> {
        committee
            .names()
            .map(|validator| {
