...
use sui_types::{
    ...
    committee::Committee,
    crypto::{get_key_pair, AuthorityKeyPair, KeypairTraits},
};
...
mod client_stats_tests {
    ...
    async fn test_client_stats_record_success() {
        ...
        let submit_latency = validator_stats.average_latencies.get(&OperationType::Submit).unwrap();
    ...
    async fn test_validator_stats_update_latency() {
        ...
        assert_eq!(stats.average_latencies.get(&OperationType::Submit).unwrap().get(), Duration::from_millis(100));
        ...
        let latency = stats.average_latencies.get(&OperationType::Submit).unwrap().get();
    ...
    async fn test_latency_calculation_with_missing_operations() {
        ...
        let committee =
            Committee::new_for_testing_with_normalized_voting_power(0, vec![(validator, 1)].into_iter().collect());
    ...
    async fn test_reliability_decay() {
        ...
        let initial_reliability = stats.validator_stats.get(&validator).unwrap().reliability.get();
        ...
        let new_reliability = stats.validator_stats.get(&validator).unwrap().reliability.get();
    ...
    async fn test_moving_window_average_differences() {
        ...
        let validator_latency =
            stats.validator_stats.get(&validator).unwrap().average_latencies.get(&OperationType::Submit).unwrap().get();
    ...
    async fn test_calculate_client_latency() {
        ...
        let committee =
            Committee::new_for_testing_with_normalized_voting_power(0, validators.iter().map(|v| (*v, 1)).collect());
        ...
        println!("Case 2: Good validator with FastPath operation");
        {
            ...
            assert_eq!(*latency.get(&validator1).unwrap(), Duration::from_millis(100));
        }
        ...
        println!("Case 3: Good validator with Consensus operation");
        {
            ...
            assert_eq!(*latency_shared.get(&validator1).unwrap(), Duration::from_millis(200));
        }
        ...
        println!("Case 4: Validator with reduced reliability");
        {
            ...
            assert!((validator2_latency.as_secs_f64() - 3.433).abs() < 0.001, "{}", validator2_latency.as_secs_f64());
        }
    ...
    async fn test_reliability_weight() {
        ...
        {
            let config_half_weight = ValidatorClientMonitorConfig { reliability_weight: 1.0, ..Default::default() };
        ...
        {
            let config_zero_weight = ValidatorClientMonitorConfig { reliability_weight: 0.0, ..Default::default() };
...
mod client_monitor_tests {
    ...
    fn get_authority_aggregator(committee_size: usize) -> Arc<AuthorityAggregator<MockAuthorityApi>> {
        Arc::new(AuthorityAggregatorBuilder::from_committee_size(committee_size).build_mock_authority_aggregator())
    }
    ...
    async fn test_validator_selection_top_k_basic() {
        ...
        let selected = monitor.select_shuffled_preferred_validators(&committee, TxType::SingleWriter, 1.0);
    ...
    async fn test_validator_selection_with_failures() {
        ...
        for (i, validator) in validators.iter().enumerate() {
            monitor.record_interaction_result(OperationFeedback {
                ...
                operation: OperationType::FastPath,
                ping_type: None,
                result: if i < 2 { Ok(Duration::from_millis((i as u64 + 1) * 50)) } else { Err(()) }, // First 2 validators succeed, others fail
            });
        }
        ...
        let selected = monitor.select_shuffled_preferred_validators(&committee, TxType::SingleWriter, 2.0);
    ...
    async fn test_validator_selection_empty_committee() {
        ...
        for validator in &validators {
            for op in [OperationType::Submit, OperationType::Effects, OperationType::HealthCheck] {
                monitor.record_interaction_result(OperationFeedback {
                    authority_name: *validator,
                    display_name: auth_agg.get_display_name(validator),
                    operation: op,
        ...
        let selected = monitor.select_shuffled_preferred_validators(&other_committee, TxType::SingleWriter, 1.0);
    ...
    async fn test_validator_selection_more_k_than_validators() {
        ...
        let selected = monitor.select_shuffled_preferred_validators(&committee, TxType::SingleWriter, 1000.0);
    ...
    async fn test_validator_selection_shared_object_tx_type() {
        ...
        let selected = monitor.select_shuffled_preferred_validators(&committee, TxType::SharedObject, 1.0);
        assert_eq!(selected.len(), 4); // Should return all 4 validators from committee
    ...
    async fn test_stats_cleanup_on_authority_aggregator_change() {
        ...
        let config =
            ValidatorClientMonitorConfig { health_check_interval: Duration::from_millis(50), ..Default::default() };
        ...
        let new_auth_agg = Arc::new(
            AuthorityAggregatorBuilder::from_committee(Committee::new_for_testing_with_normalized_voting_power(
                0, // Same epoch
        ...
        assert!(check_result.is_ok(), "Stats cleanup did not happen within timeout");
    ...
    async fn test_new_validators_added_after_committee_change() {
        ...
        let mut new_committee_members = initial_validators.iter().map(|v| (*v, 1)).collect::<Vec<_>>();
        ...
        let new_auth_agg =
            Arc::new(AuthorityAggregatorBuilder::from_committee(new_committee).build_mock_authority_aggregator());
        ...
        let check_result = timeout(Duration::from_secs(2), async {
            loop {
                ...
                if monitor.get_client_stats_len() == 3 && monitor.has_validator_stats(&new_validator) {
                    break;
                }
            }
        })
        ...
        assert!(check_result.is_ok(), "New validator stats were not added within timeout");
