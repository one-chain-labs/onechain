...
use sui_types::{
    ...
    committee::Committee,
    messages_grpc::{TxType, ValidatorHealthRequest},
};
use tokio::{
    task::JoinSet,
        ...
        loop {
            ...
            for (name, safe_client) in authority_agg.authority_clients.iter() {
                ...
                tasks.spawn(async move {
                    ...
                    match timeout(timeout_duration, client.validator_health(ValidatorHealthRequest {})).await {
...
impl<A: Clone> ValidatorClientMonitor<A> {
    ...
    fn update_cached_latencies(&self, authority_agg: &AuthorityAggregator<A>) {
        ...
        for tx_type in TxType::iter() {
            let latencies_map = self.client_stats.read().get_all_validator_stats(committee, tx_type);

            for (validator, latency) in latencies_map.iter() {
                debug!("Validator {}, tx type {}: latency {}", validator, tx_type.as_str(), latency.as_secs_f64());
    ...
    pub fn record_interaction_result(&self, feedback: OperationFeedback) {
        ...
        match feedback.result {
            Ok(latency) => {
                ...
                self.metrics.operation_success.with_label_values(&[&feedback.display_name, operation_str]).inc();
            }
            Err(()) => {
                self.metrics.operation_failure.with_label_values(&[&feedback.display_name, operation_str]).inc();
            }
        }
    ...
    pub fn select_shuffled_preferred_validators(
        ...
        let mut validator_with_latencies: Vec<_> =
            committee.names().map(|v| (*v, cached_latencies.get(v).cloned().unwrap_or(Duration::ZERO))).collect();
        ...
        self.metrics.shuffled_validators.with_label_values(&[tx_type.as_str()]).observe(k as f64);
