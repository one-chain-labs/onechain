...
impl SuiRuntimes {
    pub fn new(_confg: &NodeConfig) -> Self {
        let sui_node =
            tokio::runtime::Builder::new_multi_thread().thread_name("sui-node-runtime").enable_all().build().unwrap();
        ...
        let worker_thread = env::var("RPC_WORKER_THREAD")
            .ok()
            .and_then(|o| usize::from_str(&o).tap_err(|e| warn!("Cannot parse RPC_WORKER_THREAD to usize: {e}")).ok())
        ...
        let json_rpc = tokio::runtime::Builder::new_multi_thread()
            ...
            .build()
            .unwrap();
        Self { sui_node, metrics, json_rpc }
    }
}
