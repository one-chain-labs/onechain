...
impl AuthorityAPI for NetworkAuthorityClient {
    ...
    async fn submit_transaction(
        ...
        self.client()?.submit_transaction(request).await.map(tonic::Response::into_inner).map_err(Into::into)
    }
    ...
    async fn wait_for_effects(
        ...
        self.client()?.wait_for_effects(request).await.map(tonic::Response::into_inner).map_err(Into::into)
    }
...
pub fn make_network_authority_clients_with_network_config(
    ...
    for (name, (_state, network_metadata)) in committee.validators() {
        ...
        let maybe_channel = tls_config
            .and_then(|tls_config| {
                network_config.connect_lazy(&address, Some(tls_config)).map_err(|e| e.to_string().into())
            })
            .tap_err(|e| {
                tracing::error!(
                    address = %address,
