...
impl SuiTxValidator {
    pub fn new(
        ...
        info!("SuiTxValidator constructed for epoch {}", epoch_store.epoch());
        ...
        Self { authority_state, consensus_overload_checker, checkpoint_service, metrics }
    }
    ...
    fn validate_transactions(&self, txs: &[ConsensusTransactionKind]) -> Result<(), SuiError> {
        ...
        for tx in txs.iter() {
            match tx {
                ...
                ConsensusTransactionKind::CheckpointSignatureV2(signature) => {
                    if !epoch_store.protocol_config().consensus_checkpoint_signature_key_includes_digest() {
                        return Err(SuiErrorKind::UnexpectedMessage(
                            "ConsensusTransactionKind::CheckpointSignatureV2 is unsupported".to_string(),
                        )
                        .into());
                    }
                    ckpt_messages.push(signature.as_ref());
                ...
                ConsensusTransactionKind::ExecutionTimeObservation(obs) => {
                    ...
                    if obs.estimates.len()
                        > epoch_store.protocol_config().max_programmable_tx_commands().try_into().unwrap()
                    {
        ...
        self.metrics.certificate_signatures_verified.inc_by(cert_count as u64);
        self.metrics.checkpoint_signatures_verified.inc_by(ckpt_count as u64);
        Ok(())
    }
    ...
    fn vote_transactions(&self, block_ref: &BlockRef, txs: Vec<ConsensusTransactionKind>) -> Vec<TransactionIndex> {
        ...
        for (i, tx) in txs.into_iter().enumerate() {
            ...
            if let Err(error) = self.vote_transaction(&epoch_store, tx) {
                ...
                self.metrics.transaction_reject_votes.with_label_values(&[error.to_variant_name()]).inc();
                ...
                epoch_store.set_rejection_vote_reason(
                    ConsensusPosition { epoch: epoch_store.epoch(), block: *block_ref, index: i as TransactionIndex },
                    &error,
                );
            } else {
    ...
    fn vote_transaction(&self, epoch_store: &Arc<AuthorityPerEpochStore>, tx: Box<Transaction>) -> SuiResult<()> {
...
fn tx_kind_from_bytes(tx: &[u8]) -> Result<ConsensusTransactionKind, ValidationError> {
    bcs::from_bytes::<ConsensusTransaction>(tx)
        .map_err(|e| ValidationError::InvalidTransaction(format!("Failed to parse transaction bytes: {:?}", e)))
        .map(|tx| tx.kind)
}
...
impl TransactionVerifier for SuiTxValidator {
    fn verify_batch(&self, batch: &[&[u8]]) -> Result<(), ValidationError> {
        ...
        let txs: Vec<_> = batch.iter().map(|tx| tx_kind_from_bytes(tx)).collect::<Result<Vec<_>, _>>()?;

        self.validate_transactions(&txs).map_err(|e| ValidationError::InvalidTransaction(e.to_string()))
    }
...
mod tests {
    ...
    use sui_types::{
        ...
        crypto::{deterministic_random_account_key, Ed25519SuiSignature},
        ...
        messages_consensus::{ConsensusPosition, ConsensusTransaction},
        object::Object,
    ...
    use crate::{
        ...
        consensus_adapter::consensus_tests::{test_certificates, test_gas_objects, test_user_transaction},
    ...
    async fn accept_valid_transaction() {
        ...
        let first_transaction_bytes: Vec<u8> =
            bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, first_transaction)).unwrap();

        let metrics = SuiTxValidatorMetrics::new(&Default::default());
        let validator = SuiTxValidator::new(state.clone(), Arc::new(CheckpointServiceNoop {}), metrics);
        ...
        let transaction_bytes: Vec<_> = certificates
            .clone()
            .into_iter()
            .map(|cert| bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap())
            .collect();
        ...
        let batch: Vec<_> = bogus_transaction_bytes.iter().map(|t| t.as_slice()).collect();
        let res_batch = validator.verify_batch(&batch);
    ...
    async fn test_verify_and_vote_batch() {
        ...
        let owned_objects: Vec<Object> =
            (0 .. 2).map(|_| Object::with_id_owner_for_testing(ObjectID::random(), sender)).collect();
        ...
        let transaction_deny_config = TransactionDenyConfigBuilder::new().add_denied_object(denied_object.id()).build();
        ...
        let valid_transaction =
            test_user_transaction(&state, sender, &keypair, gas_objects[0].clone(), vec![owned_objects[0].clone()])
                .await;

        // An invalid transaction where the input object is denied
        let invalid_transaction =
            test_user_transaction(&state, sender, &keypair, gas_objects[1].clone(), vec![denied_object.clone()]).await;
        ...
        let serialized_transactions: Vec<_> = transactions
            ...
            .collect();
        let batch: Vec<_> = serialized_transactions.iter().map(|t| t.as_slice()).collect();
        ...
        // WHEN
        let rejected_transactions = validator.verify_and_vote_batch(&BlockRef::MAX, &batch).unwrap();
        ...
        assert_eq!(reason, SuiErrorKind::UserInputError {
            error: UserInputError::TransactionDenied {
                error: format!("Access to input object {:?} is temporarily disabled", denied_object.id())
            }
        });
    }
    ...
    async fn reject_checkpoint_signature_v2_when_flag_disabled() {
        ...
        let disabled_cfg = ProtocolConfig::get_for_version(ProtocolVersion::new(92), Chain::Unknown);
        let state = TestAuthorityBuilder::new()
            .with_network_config(&network_config, 0)
        ...
        let signed = SignedCheckpointSummary::new(epoch_store.epoch(), checkpoint_summary, keypair, authority);
    ...
    async fn accept_already_executed_transaction() {
        ...
        let transaction =
            test_user_transaction(&state, sender, &keypair, gas_object.clone(), vec![owned_object.clone()]).await;
        let tx_digest = *transaction.digest();
        let cert = VerifiedExecutableTransaction::new_from_quorum_execution(transaction.clone(), 0);
        let (executed_effects, _) =
            state.try_execute_immediately(&cert, ExecutionEnv::new(), &state.epoch_store_for_testing()).await.unwrap();
        ...
        let rejected_transactions =
            validator.verify_and_vote_batch(&BlockRef::MAX, &[&serialized_tx]).expect("Verify and vote should succeed");
