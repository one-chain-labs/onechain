        ...
        T: Clone,
    {
        self.statuses
            ...
            .map(|(name, status)| {
                (
                    *name,
                    self.committee.stake_by_index(self.committee.authority_index(name).unwrap()).unwrap(),
                    status.clone(),
                )
            })
            .collect()
...
mod tests {
    ...
    fn test_insert() {
        ...
        assert_eq!(aggregator.statuses().get(&authorities[0]), Some(&initial_status));
        ...
        assert_eq!(aggregator.total_votes(), stake_per_authority);
        assert_eq!(aggregator.statuses().get(&authorities[0]), Some(&different_status));
        ...
        assert_eq!(aggregator.statuses().get(&authorities[1]), Some(&new_status));
        ...
        assert_eq!(aggregator.statuses().get(&authorities[2]), Some(&new_status));
