...
fn extract_struct_info(input: ItemStruct) -> ExtractedStructInfo {
    ...
    ExtractedStructInfo { field_names, cf_names, inner_types, derived_table_options: options, deprecated_cfs }
}
...
#[proc_macro_derive(DBMapUtils, attributes(default_options_override_fn, rename, tidehunter))]
pub fn derive_dbmap_utils_general(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as ItemStruct);
    let name = &input.ident;
    let is_tidehunter = input.attrs.iter().any(|attr| attr.path.is_ident("tidehunter"));
    let generics = &input.generics;
    let generics_names = extract_generics_names(generics);

    // TODO: use `parse_quote` over `parse()`
    let ExtractedStructInfo { field_names, cf_names, inner_types, derived_table_options, deprecated_cfs } =
