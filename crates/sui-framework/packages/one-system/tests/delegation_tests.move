...
module one_system::stake_tests {
    ...
    fun test_split_join_staked_sui() {
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            assert!(amount1 == 20 * MIST_PER_OCT || amount1 == 40 * MIST_PER_OCT);
            assert!(amount2 == 20 * MIST_PER_OCT || amount2 == 40 * MIST_PER_OCT);
            assert!(amount1 + amount2 == 60 * MIST_PER_OCT);

            part1.join(part2);
            assert!(part1.amount() == 60 * MIST_PER_OCT);
    ...
    fun test_add_remove_stake_flow() {
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            system_state_mut_ref.request_add_stake(
                coin::mint_for_testing(60 * MIST_PER_OCT, ctx), VALIDATOR_ADDR_1, ctx
            );

            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * MIST_PER_OCT);
            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * MIST_PER_OCT);
        ...
        {
            let mut system_state = scenario.take_shared<SuiSystemState>();
            assert!(system_state.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * MIST_PER_OCT);
            test_scenario::return_shared(system_state);
        };
        scenario_val.end();
    }
    ...
    fun test_remove_stake_post_active_flow(should_distribute_rewards: bool) {
        ...
        assert_validator_total_stake_amounts(
            ...
            vector[200 * MIST_PER_OCT, 100 * MIST_PER_OCT],
            scenario
        );
