...
public fun build(builder: TestRunnerBuilder): TestRunner {
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<OCT>(sui_supply_amount.destroy_or!(1000) * MIST_PER_OCT), // sui_supply
    ...
    one_system::create(
        ...
        validators,
        balance::create_for_testing<OCT>(storage_fund_amount.destroy_or!(0) * MIST_PER_OCT), // storage_fund
...
public fun staking_rewards_balance(runner: &mut TestRunner): u64 {
    ...
    let total_balance = scenario.ids_for_sender<StakedOct>().fold!(0, |mut sum, staked_oct_id| {
        ...
        let rewards = system.calculate_rewards(&staked_oct, current_epoch);
