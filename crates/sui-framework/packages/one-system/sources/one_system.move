...
public fun calculate_rewards(self: &mut SuiSystemState, staked_oct: &StakedOct, epoch: u64): u64 {
    let system_state = self.load_system_state_mut();
    let validator_address = system_state.validator_address_by_pool_id(&staked_oct.pool_id());

    system_state
        .active_validator_by_address(validator_address)
        .get_staking_pool_ref()
        .calculate_rewards(staked_oct, epoch)
}
