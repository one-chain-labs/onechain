...
fun create_system_objects() {
    ...
    let clock = scenario.take_shared<one::clock::Clock>();
    test_scenario::return_shared(clock);

    let random = scenario.take_shared<one::random::Random>();
    test_scenario::return_shared(random);

    let deny_list = scenario.take_shared<one::deny_list::DenyList>();
