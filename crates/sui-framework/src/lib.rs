...
use move_binary_format::{binary_config::BinaryConfig, compatibility::Compatibility, normalized, CompiledModule};
...
impl BuiltInFramework {
    pub fn iter_system_package_metadata() -> impl Iterator<Item = &'static SystemPackageMetadata> {
        ...
        define_system_package_metadata!([
            ...
            (SUI_FRAMEWORK_PACKAGE_ID, "One", "one-framework", [MOVE_STDLIB_PACKAGE_ID]),
            (SUI_SYSTEM_PACKAGE_ID, "OneSystem", "one-system", [MOVE_STDLIB_PACKAGE_ID, SUI_FRAMEWORK_PACKAGE_ID]),
            (DEEPBOOK_PACKAGE_ID, "DeepBook", "deepbook", [MOVE_STDLIB_PACKAGE_ID, SUI_FRAMEWORK_PACKAGE_ID]),
            (BRIDGE_PACKAGE_ID, "Bridge", "bridge", [
                MOVE_STDLIB_PACKAGE_ID,
                SUI_FRAMEWORK_PACKAGE_ID,
                SUI_SYSTEM_PACKAGE_ID
            ])
...
pub async fn compare_system_package<S: ObjectStore>(
    ...
    let mut new_normalized = new_pkg.normalize(pool, binary_config, /* include code */ false).ok()?;
