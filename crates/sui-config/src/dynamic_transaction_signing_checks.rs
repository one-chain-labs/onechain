...
pub enum DynamicCheckRunnerError {
    ...
    #[error("Failed to execute Starlark program -- invalid return type expected a bool but got {0}")]
...
impl DynamicCheckRunnerContext {
    ...
    pub fn new(starlark_program: String) -> Result<Self, DynamicCheckRunnerError> {
        ...
        Ok(Self { module, globals, loaded_program: starlark_program })
    }
    ...
    pub fn run_predicate(
        ...
        let tx_data_json =
            serde_json::to_value(tx_data).map_err(|e| DynamicCheckRunnerError::JSONSerializationError(e.to_string()))?;
    ...
    fn json_to_starlark<'v>(value: &JsonValue, heap: &'v Heap) -> Result<Value<'v>, DynamicCheckRunnerError> {
        Ok(match value {
            ...
            JsonValue::Number(n) => {
                if let Some(i) = n.as_u64() {
                    ...
                    return Err(DynamicCheckRunnerError::UnsupportedNumberFormat(n.to_string()));
                }
            }
            JsonValue::String(s) => heap.alloc(s.as_str()),
            JsonValue::Array(arr) => {
                let list: Vec<_> = arr.iter().map(|v| Self::json_to_starlark(v, heap)).collect::<Result<_, _>>()?;
    ...
    D: Deserializer<'de>,
{
    ...
    match path_opt {
        Some(p) => Ok(Some(DynamicCheckRunnerContext::new(p).map_err(de::Error::custom)?)),
        None => Ok(None),
    }
}
...
{
    match value {
        Some(DynamicCheckRunnerContext { loaded_program, .. }) => serializer.serialize_some(&loaded_program),
