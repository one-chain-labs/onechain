...
pub use antithesis_sdk::{
    assert_reachable as assert_reachable_antithesis,
    assert_sometimes as assert_sometimes_antithesis,
    assert_unreachable as assert_unreachable_antithesis,
};
pub use serde_json::json;

#[inline(always)]
pub fn crash_on_debug() -> bool {
    static CRASH_ON_DEBUG: Lazy<bool> =
        Lazy::new(|| in_test_configuration() || std::env::var("SUI_ENABLE_DEBUG_ASSERTIONS").is_ok());
...
pub mod intercept_debug_fatal {
    ...
    pub fn register_callback(message: &str, f: impl Fn() + Send + Sync + 'static) {
        INTERCEPT_DEBUG_FATAL.with(|m| {
            *m.lock().unwrap() = Some(DebugFatalCallback { pattern: message.to_string(), callback: Arc::new(f) });
        });
    }
