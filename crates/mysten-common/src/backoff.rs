...
impl ExponentialBackoff {
    ...
    pub fn new(initial_delay: Duration, max_delay: Duration) -> ExponentialBackoff {
        ExponentialBackoff { next: initial_delay, factor: 1.2, max_delay, max_jitter: initial_delay }
    }
...
impl Iterator for ExponentialBackoff {
    ...
    fn next(&mut self) -> Option<Duration> {
        ...
        let jitter = if self.max_jitter.is_zero() {
            ...
            Duration::from_secs_f64(rand::thread_rng().gen_range(0.0 .. self.max_jitter.as_secs_f64()))
        };
        self.next = current.mul_f64(self.factor).min(self.max_delay).saturating_add(jitter);
