...
pub const CHECKPOINT_BUILDER_NOTIFY_READ_TASK_NAME: &str = "CheckpointBuilder::notify_read_executed_effects";
...
impl<K: Eq + Hash + Clone + Unpin + std::fmt::Debug + Send + Sync + 'static, V: Clone + Unpin> NotifyRead<K, V> {
    pub async fn read(&self, task_name: &'static str, keys: &[K], fetch: impl FnOnce(&[K]) -> Vec<Option<V>>) -> Vec<V> {
        ...
        let _log_handle_guard = if has_waiting_keys {
            ...
            let handle = spawn_monitored_task!(async move {
                ...
                let mut interval = interval_at(start, Duration::from_secs(LONG_WAIT_LOG_INTERVAL_SECS));
        ...
        let results = results.into_iter().zip(registrations).zip(keys.iter()).map(|((a, r), key)| match a {
