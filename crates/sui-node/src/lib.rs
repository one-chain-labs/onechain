...
impl SuiNode {
    ...
    pub async fn start_async(
        ...
        let perpetual_tables_options = AuthorityPerpetualTablesOptions { enable_write_stall, compaction_filter: None };
    ...
    async fn start_checkpoint_service(
        config: &NodeConfig,
        consensus_adapter: Arc<ConsensusAdapter>,
        checkpoint_store: Arc<CheckpointStore>,
        epoch_store: Arc<AuthorityPerEpochStore>,
        ...
        let tasks = checkpoint_service.spawn().await;

        (checkpoint_service, tasks)
    }
...
pub async fn build_http_server(
    ...
    let json_rpc_router = {
        ...
        let name_service_config = if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
            ...
            NameServiceConfig::new(package_address, registry_id, reverse_registry_id)
        } else {
            match CHAIN_IDENTIFIER.get().expect("chain_id should be initialized").chain() {
                Chain::Mainnet => NameServiceConfig::mainnet(),
                Chain::Testnet => NameServiceConfig::testnet(),
                Chain::Unknown => NameServiceConfig::default(),
            }
        };
