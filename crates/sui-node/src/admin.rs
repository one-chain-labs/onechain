...
pub async fn run_admin_server(node: Arc<SuiNode>, port: u16, tracing_handle: TracingHandle) {
    ...
    let app = Router::new()
        ...
        .route(DUMP_CONSENSUS_TX_COST_ESTIMATES_ROUTE, get(dump_consensus_tx_cost_estimates))
...
async fn force_close_epoch(State(state): State<Arc<AppState>>, epoch: Query<Epoch>) -> (StatusCode, String) {
    ...
    if actual_epoch != expected_epoch {
        let err = SuiErrorKind::WrongEpoch { expected_epoch, actual_epoch };
...
async fn get_tx_cost(State(state): State<Arc<AppState>>, args: Query<GetTxCost>) -> (StatusCode, String) {
    let Query(GetTxCost { tx_digest }) = args;
    let tx_digest = TransactionDigest::from_str(tx_digest.as_str()).unwrap();

    let Some(transaction) = state.node.state().get_transaction_cache_reader().get_transaction_block(&tx_digest) else {
...
async fn dump_consensus_tx_cost_estimates(State(state): State<Arc<AppState>>) -> (StatusCode, String) {
    let epoch_store = state.node.state().load_epoch_store_one_call_per_task();
