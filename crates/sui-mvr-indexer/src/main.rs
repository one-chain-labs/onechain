...
use sui_mvr_indexer::{
    ...
    db::{
        ...
        check_prunable_tables_valid,
        reset_database,
        run_migrations,
        setup_postgres::clear_database,
    },
    indexer::Indexer,
...
async fn main() -> anyhow::Result<()> {
    ...
    match opts.command {
        ...
        Command::JsonRpcService(json_rpc_config) => {
            ...
            Indexer::start_reader(&json_rpc_config, &registry, pool, CancellationToken::new()).await?;
        }
        Command::ResetDatabase { force, skip_migrations } => {
            if !force {
