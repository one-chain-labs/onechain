...
async fn test_simple() {
    ...
    let Response { result: Page { data, has_next_page, .. } } = owned_objects(
        &cluster,
        owner,
        json!({
    ...
    let actual = data.iter().map(|o| ObjectID::from_str(&o.data.object_id)).collect::<Result<BTreeSet<_>, _>>().unwrap();
...
async fn test_multi_fetch() {
    let mut cluster = setup_cluster(ObjectsConfig { filter_scan_size: 6, ..Default::default() }).await;
...
async fn test_next_cursor() {
    let mut cluster = setup_cluster(ObjectsConfig { filter_scan_size: 5, ..Default::default() }).await;
    ...
    loop {
        ...
        actual.extend(result.data.into_iter().map(|o| ObjectID::from_str(&o.data.object_id).unwrap()));
...
async fn setup_cluster(config: ObjectsConfig) -> FullCluster {
    FullCluster::new_with_configs(
        ...
        OffchainClusterConfig {
            jsonrpc_config: JsonRpcConfig { objects: config, ..Default::default() },
            ..Default::default()
        },
...
fn create_coin(cluster: &mut FullCluster, owner: SuiAddress, amount: u64) -> ObjectID {
    ...
    find::address_owned(&fx).expect("Failed to find created coin").0
}
...
fn create_bag(cluster: &mut FullCluster, owner: SuiAddress, size: u64) -> ObjectID {
    ...
    find::address_owned(&fx).expect("Failed to find created bag").0
}
