...
use sui_indexer_alt_framework::{
    ...
    ingestion::{ingestion_client::IngestionClientArgs, streaming_client::StreamingClientArgs, ClientArgs},
    pipeline::{
...
impl Processor for TxCounts {
    ...
    async fn process(&self, checkpoint: &std::sync::Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        let user_tx_count = checkpoint.transactions.iter().filter(|tx| !tx.transaction.is_system_tx()).count();
...
async fn transfer_coin(wallet: &mut sui_sdk::wallet_context::WalletContext) -> sui_types::base_types::TransactionDigest {
    let sender = wallet.active_address().unwrap();
    let gas_objs = wallet.gas_for_owner_budget(sender, 5000, Default::default()).await.unwrap();
    let gas_obj = gas_objs.1.object_ref();

    let tx_data = TestTransactionBuilder::new(sender, gas_obj, 1000).transfer_oct(Some(1000), sender).build();
...
async fn test_indexer_cluster_with_grpc_streaming() {
    ...
    let client_args = ClientArgs {
        ...
        streaming: StreamingClientArgs { streaming_url: Some(test_cluster.rpc_url().parse().unwrap()) },
    };
    ...
    let mut indexer =
        IndexerCluster::builder().with_database_url(url.clone()).with_client_args(client_args).build().await.unwrap();
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        while num_txns < 5 {
            ...
            num_txns = tx_counts::table.select(tx_counts::count).load::<i64>(&mut conn).await.unwrap().iter().sum();
        }
    })
    .await
