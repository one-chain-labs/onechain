...
use sui_types::{
    ...
    transaction::{Argument, Command, GasData, ObjectArg, Transaction, TransactionData, TransactionKind},
    TypeTag,
...
async fn test_objects_by_type_pagination() {
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::table::Table" }), 5, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();

    let backward = objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::table::Table" }), 5, false, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::table::Table<u8, u8>" }), 5, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();

    let backward = objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::table::Table<u8, u8>" }), 5, false, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward =
        objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::bag" }), 5, true, |v| v.pointer("/data/objects"))
            .await
            .unwrap();

    let backward =
        objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::bag" }), 5, false, |v| v.pointer("/data/objects"))
            .await
            .unwrap();
    ...
    let all_tu8s = objects(&cluster, OBJECTS_QUERY, json!({"type": "0x2::table::Table<u8, u8>"}), 10, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward = objects(&cluster, CHECKPOINT_QUERY, json!({ "type": "0x2::table::Table" }), 5, true, |v| {
        v.pointer("/data/checkpoint/query/objects")
    })
    .await
    .unwrap();

    let backward = objects(&cluster, CHECKPOINT_QUERY, json!({ "type": "0x2::table::Table" }), 5, false, |v| {
        v.pointer("/data/checkpoint/query/objects")
    })
    .await
    .unwrap();
...
async fn test_objects_by_address_owner() {
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "ADDRESS", "owner": a }), 3, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();

    let backward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "ADDRESS", "owner": a }), 3, false, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "ADDRESS", "owner": b }), 5, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "ADDRESS", "owner": a }), 5, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let forward = objects(&cluster, CHECKPOINT_QUERY, json!({ "ownerKind": "ADDRESS", "owner": a }), 10, true, |v| {
        v.pointer("/data/checkpoint/query/objects")
    })
    .await
    .unwrap();
    ...
    let all_as = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "ADDRESS", "owner": a }), 10, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
...
async fn test_objects_by_object_owner() {
    ...
    let forward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "OBJECT", "owner": table.0 }), 3, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();

    let backward = objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "OBJECT", "owner": table.0 }), 3, false, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    assert_eq!(forward.iter().map(|t| t.1).collect::<Vec<_>>(), backward_refs);

    // Test 2: Time-travel - Query table's objects at checkpoint 1 (original state)
    let checkpoint1_objects =
        objects(&cluster, CHECKPOINT_QUERY, json!({ "ownerKind": "OBJECT", "owner": table.0 }), 10, true, |v| {
            v.pointer("/data/checkpoint/query/objects")
        })
        .await
        .unwrap();
...
async fn test_shared_and_immutable_objects() {
    ...
    let forward =
        objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "SHARED", "type": "0x2::table::Table" }), 2, true, |v| {
            v.pointer("/data/objects")
        })
        .await
        .unwrap();
    ...
    let backward =
        objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "SHARED", "type": "0x2::table::Table" }), 2, false, |v| {
            v.pointer("/data/objects")
        })
        .await
        .unwrap();
    ...
    let tables = objects(&cluster, OBJECTS_QUERY, json!({ "type": "0x2::table::Table" }), 20, true, |v| {
        v.pointer("/data/objects")
    })
    .await
    .unwrap();
    ...
    let all_shared =
        objects(&cluster, OBJECTS_QUERY, json!({ "ownerKind": "SHARED", "type": "0x2::table::Table" }), 10, true, |v| {
            v.pointer("/data/objects")
        })
        .await
        .unwrap();
...
fn destroy_coin(
    ...
    let (sponsor, sponsor_kp, gas) = cluster.funded_account(DEFAULT_GAS_BUDGET).expect("Failed to fund sponsor account");

    let mut builder = ProgrammableTransactionBuilder::new();
    let coin_arg = builder.obj(ObjectArg::ImmOrOwnedObject(coin)).expect("Failed to add coin object");
    builder.command(Command::MergeCoins(Argument::GasCoin, vec![coin_arg]));

    let gas_data =
        GasData { payment: vec![gas], owner: sponsor, price: cluster.reference_gas_price(), budget: DEFAULT_GAS_BUDGET };
    ...
    let (fx, _) = cluster
        .execute_transaction(Transaction::from_data_and_signer(data, vec![&sponsor_kp, owner_kp]))
        .expect("Failed to execute transaction");
...
fn create_shared_table(cluster: &mut FullCluster, ty: TypeTag, size: u64) -> ObjectRef {
    ...
    assert!(fx.status().is_ok(), "create shared table transaction failed");
    fx.created()
        .into_iter()
...
fn create_immutable_table(cluster: &mut FullCluster, ty: TypeTag, size: u64) -> ObjectRef {
    ...
    assert!(fx.status().is_ok(), "create immutable table transaction failed");
    fx.created()
        .into_iter()
        .find_map(|((obj_id, version, digest), owner)| match owner {
...
fn transfer_object(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let obj_arg = builder.obj(ObjectArg::ImmOrOwnedObject(object)).expect("Failed to add object");
    ...
    let data =
        TransactionData::new_with_gas_data(TransactionKind::ProgrammableTransaction(builder.finish()), owner, gas_data);
