...
async fn create_graphql_test_cluster(validator_cluster: &TestCluster) -> GraphQlTestCluster {
    ...
    let url = Url::parse(&format!("http://{}/graphql", graphql_listen_address)).expect("Failed to parse GraphQL URL");

    GraphQlTestCluster { url, handle: graphql_handle, cancel }
}
...
async fn test_execute_transaction_mutation_schema() {
    ...
    let effects: TransactionEffects =
        serde_json::from_value(result.pointer("/data/executeTransaction/effects").unwrap().clone()).unwrap();
...
async fn test_execute_transaction_input_validation() {
    let validator_cluster = TestClusterBuilder::new().with_num_validators(1).build().await;
    let graphql_cluster = create_graphql_test_cluster(&validator_cluster).await;
...
async fn test_execute_transaction_with_events() {
    let validator_cluster = TestClusterBuilder::new().with_num_validators(1).enable_fullnode_events().build().await;
    let graphql_cluster = create_graphql_test_cluster(&validator_cluster).await;
    ...
    assert!(!events.nodes.is_empty(), "Package publish should emit events from init function");

    let sender_address = validator_cluster.get_address_0();
    for event_node in &events.nodes {
        assert!(!event_node.event_bcs.is_empty(), "Event should have eventBcs");
        assert_eq!(
...
async fn test_execute_transaction_grpc_errors() {
    ...
    let signed_tx1 = make_transfer_oct_transaction(&validator_cluster.wallet, Some(recipient1), Some(1_000_000)).await;
    let signed_tx2 = make_transfer_oct_transaction(&validator_cluster.wallet, Some(recipient2), Some(2_000_000)).await;
    ...
    assert_eq!(effects, Some(&serde_json::Value::Null), "Should have null effects on gRPC error");
...
async fn test_execute_transaction_unchanged_consensus_objects() {
    ...
    let tx_data = validator_cluster.test_transaction_builder().await.programmable(ptb.finish()).build();
    let signed_tx = validator_cluster.sign_transaction(&tx_data).await;
    let (tx_bytes, signatures) = signed_tx.to_tx_bytes_and_signatures();
    ...
    assert!(!consensus_objects.edges.is_empty(), "Clock read should create unchanged consensus objects");
...
async fn test_execute_transaction_object_changes_input_output() {
    ...
    let object_changes_value = result.pointer("/data/executeTransaction/effects/objectChanges/nodes").unwrap();
