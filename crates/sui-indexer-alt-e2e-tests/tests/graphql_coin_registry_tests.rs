...
use serde_json::json;
use sui_indexer_alt_e2e_tests::{find_address_mutated, find_address_owned_by, find_immutable, find_shared, FullCluster};
...
async fn test_sui() {
    ...
    let metadata = query_metadata(&cluster, "0x2::oct::OCT").await;
...
async fn test_dynamic() {
    ...
    builder
        .move_call(package, ident_str!("dynamic").to_owned(), ident_str!("new_currency").to_owned(), vec![], vec![
    ...
    let (_fx, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![&kp])).expect("new_currency failed");
...
async fn test_burn_only() {
    ...
    burn_from_currency(&mut cluster, sender, &kp, package, "burn", coin, 100_000_000, currency, gas).await;
...
async fn test_unknown() {
    ...
    let fx =
        burn_from_treasury(&mut cluster, sender, &kp, package, "unknown", coin, 200_000_000, treasury_cap, gas).await;
    let gas = fx.gas_object().0;
    ...
    let fx = hide_treasury_cap(&mut cluster, sender, &kp, package, "unknown", treasury_cap, gas).await;
    let gas = fx.gas_object().0;
    ...
    let fx =
        burn_from_treasury(&mut cluster, sender, &kp, package, "unknown", coin, 100_000_000, treasury_cap, gas).await;
    let gas = fx.gas_object().0;
    ...
    show_treasury_cap(&mut cluster, sender, &kp, package, "unknown", treasury_cap, gas).await;
...
async fn test_legacy() {
    ...
    let fx = migrate(&mut cluster, sender, &kp, package, "legacy", coin_metadata, gas).await;
...
/// transaction.
async fn publish(cluster: &mut FullCluster, package: &str) -> (SuiAddress, Ed25519KeyPair, TransactionEffects) {
    // Compile the package
    ...
    builder.publish_immutable(pkg.get_package_bytes(with_unpublished_deps), pkg.get_dependency_storage_package_ids());
    ...
    let (fx, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![&kp])).expect("Publish failed");
...
async fn burn_from_currency(
    ...
    let currency = builder
        .obj(ObjectArg::SharedObject { id: currency.0, initial_shared_version: currency.1, mutable: true })
        .unwrap();
    ...
    let (fx, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![kp])).expect("Burn failed");
...
async fn burn_from_treasury(
    ...
    let treasury = builder
        .obj(ObjectArg::SharedObject { id: treasury.0, initial_shared_version: treasury.1, mutable: true })
        .unwrap();
    ...
    let (fx, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![kp])).expect("Treasury burn failed");
...
async fn migrate(
    ...
    let (fx, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![kp])).expect("Migration failed");
...
async fn delete_migrated_legacy_metadata(
    ...
    let treasury_cap = builder
        .obj(ObjectArg::SharedObject { id: treasury_cap.0, initial_shared_version: treasury_cap.1, mutable: false })
        .unwrap();
