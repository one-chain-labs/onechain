...
async fn test_burn_only() {
    ...
    let fx = coin_registry::finalize(&mut cluster, sender, &kp, package, "burn", currency, gas).await;
    ...
    coin_registry::burn_from_currency(&mut cluster, sender, &kp, package, "burn", coin, 100_000_000, currency, gas)
        .await;

    cluster.create_checkpoint().await;
    assert_eq!(query_metadata(&cluster, &format!("{package}::burn::BURN")).await, CoinMetadataResponse {
...
async fn test_unknown() {
    ...
    let fx = coin_registry::finalize(&mut cluster, sender, &kp, package, "unknown", currency, gas).await;
    ...
    assert_eq!(query_metadata(&cluster, &format!("{package}::unknown::UNKNOWN")).await, CoinMetadataResponse {
        supply: Some("800000000".to_string()),
        ..metadata.clone()
    });

    // Hide the treasury cap (move it to dynamic object field)
    let fx = coin_registry::hide_treasury_cap(&mut cluster, sender, &kp, package, "unknown", treasury_cap, gas).await;
    ...
    coin_registry::show_treasury_cap(&mut cluster, sender, &kp, package, "unknown", treasury_cap, gas).await;
...
async fn test_legacy_regulated_migrate_deny_cap() {
    ...
    let migrated = query_metadata(&cluster, &outputs.coin_type.to_canonical_string(true)).await;
    assert_eq!(migrated, CoinMetadataResponse { allow_global_pause: Some(false), ..metadata.clone() });
}
...
async fn test_legacy_regulated_migrate_regulated_metadata() {
    ...
    let regulated_metadata = fx
        ...
        .into_iter()
        .find_map(|(oref, o)| {
            matches!(o, Owner::Immutable if oref.0 != outputs.coin_type.address.into()).then_some(oref)
        })
        .unwrap();
    let gas = fx.gas_object().0;
    ...
    coin_registry::migrate_regulated_metadata(&mut cluster, sender, &kp, &outputs, currency, regulated_metadata, gas)
        .await;
...
async fn query_owned_outputs(cluster: &FullCluster, owner: SuiAddress) -> LegacyCoinOutputs {
    ...
    let (type_, treasury) = objects
        .iter()
        .find_map(|(type_, obj)| TreasuryCap::is_treasury_with_coin_type(type_).map(|type_| (type_.clone(), *obj)))
        .unwrap();

    let metadata = objects.iter().find_map(|(type_, obj)| CoinMetadata::is_coin_metadata(type_).then_some(*obj));

    let deny = objects.iter().find_map(|(type_, obj)| DenyCapV2::is_deny_cap_v2(type_).then_some(*obj));

    LegacyCoinOutputs { coin_type: type_, treasury, metadata, deny }
}
...
async fn query_objects(cluster: &FullCluster, owner: SuiAddress) -> Vec<(StructTag, ObjectRef)> {
    ...
    loop {
        ...
        for node in nodes {
            ...
            let type_: StructTag = node.contents.pointer("/type/repr").unwrap().as_str().unwrap().parse().unwrap();
