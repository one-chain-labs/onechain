...
macro_rules! assert_resolved {
    ($target:expr, $resp:expr) => {
        ...
        assert_eq!(
            $target,
            resp["result"].as_str().expect("result should be string").parse().expect("failed to parse result address"),
...
macro_rules! assert_invalid_params {
    ($resp:expr) => {
        ...
        assert!(resp["result"].is_null(), "Expected null result, got {resp:#?}");

        assert_eq!(
            INVALID_PARAMS_CODE as i64,
            resp["error"]["code"].as_i64().expect("error code should be a number"),
...
macro_rules! assert_no_reverse {
    ($resp:expr) => {
        ...
        assert_eq!(Some(&vec![]), data, "Expected successful response with empty data from RPC, got {resp:#?}",);
    };
}
...
async fn test_resolve_domain() {
    ...
    c.add_domain(nft, &["sui", "foo"], Some(target), 1000).await.expect("Failed to add domain");
...
async fn test_resolve_domain_no_target() {
    ...
    let nft = ObjectID::random();
    c.add_domain(nft, &["sui", "foo"], None, 1000).await.expect("Failed to add domain");
...
async fn test_resolve_domain_expiry() {
    ...
    let target = SuiAddress::random_for_testing_only();
    let expiry_ms = 1000;
    c.add_domain(nft, &["sui", "foo"], Some(target), expiry_ms).await.expect("Failed to add domain");
...
async fn test_resolve_subdomain() {
    ...
    c.add_domain(nft, &["sui", "foo"], None, 1000).await.expect("Failed to add parent domain");

    c.add_domain(nft, &["sui", "foo", "bar"], Some(target), 0).await.expect("Failed to add subdomain");
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    c.add_domain(nft, &["sui", "foo"], None, expiry_ms).await.expect("Failed to add parent domain");
...
async fn test_resolve_subdomain_expiry() {
    ...
    c.add_domain(parent_nft, &["sui", "foo"], None, parent_expiry_ms).await.expect("Failed to add parent domain");

    c.add_domain(nft, &["sui", "foo", "bar"], Some(target), expiry_ms).await.expect("Failed to add subdomain");
...
async fn test_resolve_subdomain_bad_parent() {
    ...
    c.add_domain(nft0, &["sui", "foo"], None, 1000).await.expect("Failed to add parent domain");

    c.add_domain(nft1, &["sui", "foo", "bar"], Some(target), 0).await.expect("Failed to add subdomain");
...
impl SuiNSCluster {
    ...
    async fn new() -> Self {
        ...
        let pkg = BuildConfig::new_for_testing().build(&path).expect("Failed to compile package");

        // (3) Create an address and fund it to be able to run transactions.
        let (sender, kp, gas) = sim.funded_account(DEFAULT_GAS_BUDGET * 3).expect("Failed to get account");

        // (4) Publish the mock SuiNS package.
        let mut builder = ProgrammableTransactionBuilder::new();
        let with_unpublished_deps = false;
        builder
            .publish_immutable(pkg.get_package_bytes(with_unpublished_deps), pkg.get_dependency_storage_package_ids());
        ...
        let (fx, _) =
            sim.execute_transaction(Transaction::from_data_and_signer(data, vec![&kp])).expect("Publish failed");
        ...
        let forward_registry =
            ObjectArg::SharedObject { id: registry_id, initial_shared_version: fx.lamport_version(), mutable: true };
        ...
        let config = NameServiceConfig { package_address: package_address.into(), registry_id, reverse_registry_id };

        let jsonrpc_config = JsonRpcConfig { name_service: config.clone(), ..Default::default() };

        // (8) Spin up the rest of the cluster.
        let cluster = FullCluster::new_with_configs(
            sim,
            OffchainClusterConfig { jsonrpc_config, ..Default::default() },
        ...
        Self { cluster, config, forward_registry, reverse_registry, client: Client::new() }
    }
    ...
    async fn add_domain(
        ...
        let (sender, kp, gas) = self.cluster.funded_account(DEFAULT_GAS_BUDGET).expect("failed to get account");
        ...
        builder.programmable_move_call(
            ...
            vec![],
            vec![forward_registry, reverse_registry, nft_id, labels, target, expiration_timestamp_ms],
        );
