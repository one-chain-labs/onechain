...
use sui_indexer_alt_e2e_tests::{local_ingestion_client_args, write_checkpoint, OffchainCluster, OffchainClusterConfig};
...
async fn safe_mode_system_state_v1() -> Result<(), anyhow::Error> {
    ...
    let SafeModeEpoch {
        safe_mode:
            SafeMode {
                enabled,
                gas_summary: GasSummary { computation_cost, storage_cost, storage_rebate, non_refundable_storage_fee },
            },
    } = test_graphql(SAFE_MODE_QUERY, sui_system_state.clone(), AdvanceEpochConfig {
...
"#;
    ...
    let LiveObjectsSetDigestEpoch { live_object_set_digest: actual_digest } =
        test_graphql(LIVE_OBJECT_SET_DIGEST_QUERY, sui_system_state.clone(), AdvanceEpochConfig {
            output_objects: mock::system_state_output_objects(sui_system_state),
            epoch_commitments: vec![CheckpointCommitment::ECMHLiveObjectSetDigest(ECMHLiveObjectSetDigest {
    ...
    let offchain = OffchainCluster::new(
        ...
        OffchainClusterConfig {
            ...
            bootstrap_genesis: Some(BootstrapGenesis {
                stored_genesis: StoredGenesis { genesis_digest: [1u8; 32].to_vec(), initial_protocol_version: 0 },
