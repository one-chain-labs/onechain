...
use sui_indexer_alt_jsonrpc::{args::SystemPackageTaskArgs, config::RpcConfig, start_rpc, NodeArgs, RpcArgs};
...
impl FnDelegationTestCluster {
    ...
    async fn new() -> anyhow::Result<Self> {
        ...
        let rpc_listen_address = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), get_available_port());
        let rpc_url = Url::parse(&format!("http://{}/", rpc_listen_address)).expect("Failed to parse RPC URL");
        ...
        let rpc_handle = start_rpc(
            ...
            rpc_args,
            NodeArgs { fullnode_rpc_url: Some(fullnode_rpc_url) },
        ...
        Ok(Self { onchain_cluster, rpc_url, rpc_handle, client: Client::new(), cancel })
    }
    ...
    async fn transfer_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        ...
        let tx = self.onchain_cluster.test_transaction_builder().await.transfer_oct(Some(1_000), recipient).build();
    ...
    async fn privileged_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        let tx: sui_types::transaction::TransactionData =
            self.onchain_cluster.test_transaction_builder().await.call_request_remove_validator().build();
        let tx_digest = tx.digest().to_string();
        let signed_tx = self.onchain_cluster.wallet.sign_transaction(&tx).await;
        let (tx_bytes, sigs) = signed_tx.to_tx_bytes_and_signatures();
        let tx_bytes = tx_bytes.encoded();
    ...
    async fn execute_jsonrpc(&self, method: String, params: Value) -> anyhow::Result<Value> {
        ...
        let body: Value = response.json().await.context("Failed to parse JSON-RPC response")?;
...
async fn test_execution() {
    ...
    let test_cluster = FnDelegationTestCluster::new().await.expect("Failed to create test cluster");
...
async fn test_execution_with_deprecated_mode() {
    ...
    assert_eq!(response["error"]["message"], "Invalid Params: WaitForLocalExecution mode is deprecated");
...
async fn test_execution_with_no_sigs() {
    ...
    assert!(response["error"]["data"].as_str().unwrap().starts_with("missing field `signatures`"));
...
async fn test_execution_with_empty_sigs() {
    ...
    assert_eq!(response["error"]["message"], "Invalid user signature: Expect 1 signer signatures but got 0");
...
async fn test_dry_run_with_invalid_tx() {
    ...
    assert_eq!(response["error"]["code"], -32602);
    assert_eq!(response["error"]["message"], "Invalid params");
    assert!(response["error"]["data"].as_str().unwrap().starts_with("Invalid value was given to the function"));
    test_cluster.stopped().await;
}
...
async fn test_get_all_balances() {
    ...
    let response = test_cluster
        .execute_jsonrpc("suix_getAllBalances".to_string(), json!({ "owner": address.to_string().as_str()}))
        .await
        .unwrap();
...
async fn test_get_all_balances_with_invalid_address() {
    ...
    let response = test_cluster
        .execute_jsonrpc("suix_getAllBalances".to_string(), json!({ "owner": invalid_address }))
        .await
        .unwrap();

    assert_eq!(response["error"]["code"], -32602);
    assert_eq!(response["error"]["message"], "Invalid params");
    assert!(response["error"]["data"].as_str().unwrap().contains("Deserialization failed"));
...
async fn test_get_stakes_and_by_ids() {
    ...
    wallet.execute_transaction_must_succeed(staking_transaction).await;

    // Get the stake by owner.
    let get_stakes_response = test_cluster
        .execute_jsonrpc("suix_getStakes".to_string(), json!({ "owner": stake_owner_address }))
        .await
        .unwrap();

    assert_eq!(get_stakes_response["result"][0]["validatorAddress"], validator_address.to_string().as_str());
    assert!(get_stakes_response["result"][0]["stakes"][0]["stakedOctId"].is_string());
    let stake_id = get_stakes_response["result"][0]["stakes"][0]["stakedOctId"].as_str().unwrap();

    // Now get the stake by id.
    let get_stakes_by_ids_response = test_cluster
        .execute_jsonrpc("suix_getStakesByIds".to_string(), json!({ "staked_oct_ids": [stake_id] }))
        .await
        .unwrap();
...
async fn test_get_stakes_invalid_params() {
    ...
    let response =
        test_cluster.execute_jsonrpc("suix_getStakes".to_string(), json!({ "owner": "invalid_address" })).await.unwrap();
    ...
    let response = test_cluster
        .execute_jsonrpc("suix_getStakesByIds".to_string(), json!({ "staked_oct_ids": ["invalid_stake_id"] }))
        .await
        .unwrap();

    assert_eq!(response["error"]["code"], -32602);
    assert_eq!(response["error"]["message"], "Invalid params");
    assert!(response["error"]["data"].as_str().unwrap().contains("AccountAddressParseError"));
...
async fn test_get_validators_apy() {
    ...
    let response = test_cluster.execute_jsonrpc("suix_getValidatorsApy".to_string(), json!({})).await.unwrap();

    assert_eq!(response["result"]["apys"][0]["address"], validator_address.to_string());
    test_cluster.stopped().await;
}
...
async fn test_get_balance() {
    ...
    let execution_result = wallet.execute_transaction_must_succeed(publish_transaction).await;
    ...
    let coin_type = sui_types::parse_sui_struct_tag(&format!("{}::my_coin::MY_COIN", package_id)).unwrap().to_string();
    let response = test_cluster
        .execute_jsonrpc(
    ...
    let response = test_cluster
        .execute_jsonrpc("suix_getBalance".to_string(), json!({ "owner": owner_address.to_string().as_str()}))
        .await
        .unwrap();
    ...
    let response = test_cluster
        ...
        .unwrap();
    assert_eq!(response["error"]["code"], -32602);
    assert!(response["error"]["message"].as_str().unwrap().contains("Invalid struct type: invalid_coin_type"));

    // Test out the invalid address.
    let response = test_cluster
        .execute_jsonrpc("suix_getBalance".to_string(), json!({ "owner": "invalid_address", "coinType": coin_type}))
        .await
        .unwrap();
    assert_eq!(response["error"]["code"], -32602);
