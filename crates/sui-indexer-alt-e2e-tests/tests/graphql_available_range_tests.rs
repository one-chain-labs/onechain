...
async fn test_available_range_with_pipelines() {
    ...
    cluster.wait_for_pruner("tx_affected_addresses", 1, Duration::from_secs(10)).await.unwrap();
    ...
    cluster.wait_for_pruner("tx_digests", 1, Duration::from_secs(10)).await.unwrap();

    cluster.wait_for_pruner("tx_affected_addresses", 6, Duration::from_secs(10)).await.unwrap();
...
async fn test_transaction_pagination_pruning() {
    ...
    cluster.wait_for_pruner("tx_affected_addresses", 2, Duration::from_secs(10)).await.unwrap();
    ...
    cluster.wait_for_pruner("tx_digests", 2, Duration::from_secs(10)).await.unwrap();

    cluster.wait_for_pruner("tx_affected_addresses", 7, Duration::from_secs(10)).await.unwrap();
...
async fn test_events_pagination_pruning() {
    ...
    cluster.wait_for_pruner("ev_emit_mod", 4, Duration::from_secs(10)).await.unwrap();
...
async fn test_checkpoint_pagination_pruning() {
    ...
    cluster.wait_for_pruner("cp_sequence_numbers", 4, Duration::from_secs(10)).await.unwrap();

    let checkpoints_in_range = execute_graphql_query(&cluster, CHECKPOINT_QUERY, None).await;
    let checkpoints = checkpoints_in_range["data"]["checkpoints"]["nodes"].as_array().unwrap();

    assert_eq!(checkpoints[0]["sequenceNumber"].as_u64().unwrap(), cp_sequence_numbers[4]);
    assert_eq!(checkpoints[3]["sequenceNumber"].as_u64().unwrap(), cp_sequence_numbers[7]);
...
async fn cluster_with_pipelines(pipeline: PipelineLayer) -> FullCluster {
    FullCluster::new_with_configs(
        ...
        OffchainClusterConfig {
            indexer_config: IndexerConfig { pipeline, ..IndexerConfig::for_test() },
            graphql_config: GraphQlConfig {
                watermark: WatermarkConfig { watermark_polling_interval: Duration::from_millis(50) },
                ..Default::default()
            },
            ..Default::default()
        },
...
fn collect_digests(resp: &Value) -> Vec<TransactionDigest> {
    const RESPONSE_PATHS: &[(&str, &str)] =
        &[("/data/events/nodes", "/transaction/digest"), ("/data/transactions/nodes", "/digest")];

    let (nodes, digest_path) = RESPONSE_PATHS
        .iter()
        .find_map(|(nodes_path, digest_path)| resp.pointer(nodes_path).map(|nodes| (nodes, *digest_path)))
    ...
    nodes
        ...
        .iter()
        .map(|node| {
            let digest = node.pointer(digest_path).and_then(Value::as_str).expect("node must contain digest");
...
fn collect_sequence_numbers(resp: &Value) -> (u64, u64) {
    ...
    (range["first"]["sequenceNumber"].as_u64().unwrap(), range["last"]["sequenceNumber"].as_u64().unwrap())
}

async fn publish_emit_event(cluster: &mut FullCluster) -> ObjectID {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("packages/event/emit_test_event");
    ...
    let (fx, _) = cluster
        .execute_transaction(Transaction::from_data_and_signer(
            TestTransactionBuilder::new(sender, gas, 1000).with_gas_budget(DEFAULT_GAS_BUDGET).publish(path).build(),
            vec![&kp],
        ))
...
async fn emit_test_event(cluster: &mut FullCluster, pkg: &ObjectID) -> TransactionDigest {
    ...
    let (effects, error) =
        cluster.execute_transaction(Transaction::from_data_and_signer(data, vec![&kp])).expect("Emit failed");
...
fn concurrent_pipeline(retention: u64) -> ConcurrentLayer {
    ConcurrentLayer {
        pruner: Some(PrunerLayer { retention: Some(retention), ..Default::default() }),
        ..Default::default()
    }
}
...
fn transfer_dust(
    ...
    let fx = cluster.request_gas(sender, DEFAULT_GAS_BUDGET + 1).expect("Failed to request gas");
...
async fn execute_graphql_query(cluster: &FullCluster, query: &str, variables: Option<Value>) -> serde_json::Value {
    Client::new()
...
async fn query_available_range(cluster: &FullCluster, field: &str, filters: Option<&[&str]>) -> Value {
    let filters = filters.unwrap_or_default().iter().map(|&s| s.to_string()).collect::<Vec<_>>();
