...
use sui_indexer_alt_e2e_tests::{find_address_owned, find_immutable, FullCluster, OffchainClusterConfig};
...
async fn test_within_limits() {
    ...
    assert!(result["result"]["data"]["content"].is_object(), "Result: {result:#?}");
...
async fn test_type_argument_depth() {
    let mut c = TypeLimitCluster::new(PackageResolverLayer { max_type_argument_depth: 3, ..Default::default() }).await;
...
async fn test_type_argument_width() {
    let mut c = TypeLimitCluster::new(PackageResolverLayer { max_type_argument_width: 3, ..Default::default() }).await;
...
async fn test_type_nodes() {
    let mut c = TypeLimitCluster::new(PackageResolverLayer { max_type_nodes: 3, ..Default::default() }).await;
    ...
    assert!(err.contains("More than 3 struct definitions required to resolve type"), "{err}");
...
async fn test_value_depth() {
    let mut c = TypeLimitCluster::new(PackageResolverLayer { max_move_value_depth: 3, ..Default::default() }).await;
    ...
    assert!(err.contains("Type layout nesting exceeded limit of 3"), "{err}");
    ...
    async fn new(package_resolver: PackageResolverLayer) -> Self {
        ...
        let mut cluster = FullCluster::new_with_configs(
            ...
            OffchainClusterConfig {
                ...
                jsonrpc_config: JsonRpcConfig { package_resolver: package_resolver.finish(), ..Default::default() },
                ..Default::default()
            },
        ...
        let (sender, kp, gas) = cluster.funded_account(DEFAULT_GAS_BUDGET).expect("Failed to get account");
        ...
        Self { cluster, package_id, client: Client::new() }
    }
