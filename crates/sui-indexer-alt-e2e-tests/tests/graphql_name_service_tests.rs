...
macro_rules! assert_resolved {
    ($target:expr, $resp:expr) => {
        let resp = $resp;
        let address = resp["data"]["suinsName"]["address"].as_str().expect("result should be string");
...
macro_rules! assert_not_resolved {
    ($resp:expr) => {
        let resp = $resp;
        assert!(resp["data"]["suinsName"].is_null(), "Expected null result for expired/invalid domain, got {resp:#?}");
    };
}
...
macro_rules! assert_reverse {
    ($target:expr, $resp:expr) => {
        let resp = $resp;
        let name = resp["data"]["address"]["defaultSuinsName"].as_str().expect("defaultSuinsName should be a string");
...
impl SuiNSCluster {
    ...
    async fn new() -> Self {
        ...
        let graphql_config = RpcConfig { name_service: config.clone(), ..Default::default() };

        // (8) Spin up the rest of the cluster.
        let cluster = FullCluster::new_with_configs(
            sim,
            OffchainClusterConfig { graphql_config, ..Default::default() },
