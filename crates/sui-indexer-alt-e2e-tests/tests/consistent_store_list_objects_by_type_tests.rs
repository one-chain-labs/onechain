...
async fn test_type_filter() {
    ...
    assert_eq!(
        list_objects_by_type(&cluster, "0x2::bag", None, Some(50)).await.unwrap(),
        (bags.values().map(repr).collect(), None)
    );

    // All tables, by fully-qualified name filter
    assert_eq!(
        list_objects_by_type(&cluster, "0x2::table::Table", None, Some(50)).await.unwrap(),
        (tu8s.values().chain(tu64s.values()).map(repr).collect(), None)
    );

    // All Table<u64, u64>s
    assert_eq!(
        list_objects_by_type(&cluster, "0x2::table::Table<u64, u64>", None, Some(50)).await.unwrap(),
        (tu64s.values().map(repr).collect(), None)
    );

    // Try to paginate Table<u8, u64>s -- none should exist.
    assert_eq!(
        list_objects_by_type(&cluster, "0x2::table::Table<u8, u64>", None, Some(50)).await.unwrap(),
        (vec![], None)
    );
    ...
    let mut results = vec![];
    loop {
        let (page, next) = list_objects_by_type(&cluster, "0x2::table", after.clone(), Some(5)).await.unwrap();
    ...
    assert_eq!(results, tu8s.values().chain(tu64s.values()).map(repr).collect::<Vec<_>>());

    // Pass in a bad type filter (malformed).
    let err = list_objects_by_type(&cluster, "not::a::valid::type<>", None, Some(20)).await.unwrap_err();

    assert_eq!(err.code(), tonic::Code::InvalidArgument);
    assert_eq!(err.message(), "Bad 'object_type' filter, expected: package[::module[::name[<type, ...>]]]");

    // Missing type filter
    let err = list_objects_by_type(&cluster, "", None, Some(20)).await.unwrap_err();
