...
async fn test_address_owner() {
    ...
    async fn list_owned_objects(
        ...
        let mut request = tonic::Request::new(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Address.into()), address: Some(owner.to_string()) }),
            page_size,
        ...
        let after_token =
            response.has_next_page().then(|| response.objects.last().map(|o| o.page_token().to_owned())).flatten();
    ...
    let coins: BTreeMap<_, _> = (0 .. 4).map(|i| (i, create_coin(&mut cluster, a, i))).collect();
    ...
    assert_eq!(
        list_owned_objects(&cluster, a, None, None, Some(4)).await.unwrap(),
        (coins.values().rev().map(repr).collect(), None),
    );

    // Bags don't have a balance, so they are returned in order of ID.
    assert_eq!(
        list_owned_objects(&cluster, b, None, None, Some(4)).await.unwrap(),
        (bags.values().map(repr).collect(), None),
    );

    // C doesn't own anything yet.
    assert_eq!(list_owned_objects(&cluster, c, None, None, Some(4)).await.unwrap(), (vec![], None),);

    // Explicitly supplying the checkpoint also works.
    assert_eq!(
        list_owned_objects(&cluster, a, Some(1), None, Some(4)).await.unwrap(),
        (coins.values().rev().map(repr).collect(), None),
    );

    // Requesting a checkpoint that we don't have a snapshot for returns an error.
    assert_eq!(
        list_owned_objects(&cluster, a, Some(2), None, Some(4)).await.unwrap_err().code(),
        tonic::Code::OutOfRange,
    );

    // In the second checkpoint, A and B get funded, so they can sign transactions.
    let mut a_gas =
        find::address_owned(&cluster.request_gas(a, DEFAULT_GAS_BUDGET * 5).unwrap()).expect("Failed to find gas for A");
    let mut b_gas =
        find::address_owned(&cluster.request_gas(b, DEFAULT_GAS_BUDGET * 5).unwrap()).expect("Failed to find gas for B");
    cluster.create_checkpoint().await;

    // A now owns an extra coin, it has the biggest balance so it should appear first.
    assert_eq!(
        list_owned_objects(&cluster, a, None, None, Some(5)).await.unwrap(),
        (iter::once(&a_gas).chain(coins.values().rev()).map(repr).collect(), None),
    );

    // B also owns a coin now, objects are grouped and sorted by type, so it should appear last
    // (after the bags).
    assert_eq!(
        list_owned_objects(&cluster, b, None, None, Some(5)).await.unwrap(),
        (bags.values().chain(iter::once(&b_gas)).map(repr).collect(), None),
    );
    ...
    assert_eq!(list_owned_objects(&cluster, a, None, None, Some(5)).await.unwrap(), (vec![repr(&a_gas)], None),);

    // Same for B
    assert_eq!(list_owned_objects(&cluster, b, None, None, Some(5)).await.unwrap(), (vec![repr(&b_gas)], None),);
    ...
    let mut results = vec![];
    loop {
        let (page, next) = list_owned_objects(&cluster, c, None, after.clone(), Some(2)).await.unwrap();
...
async fn test_missing_address() {
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Address.into()), address: None }),
            ..Default::default()
        })
        .await;
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Object.into()), address: None }),
            ..Default::default()
        })
        .await;
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Address.into()), address: Some("".to_string()) }),
            ..Default::default()
        })
        .await;
...
async fn test_unexpected_address() {
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Shared.into()), address: Some("0x1".to_string()) }),
            ..Default::default()
        })
        .await;
    ...
    let response = client
        .list_owned_objects(ListOwnedObjectsRequest {
            owner: Some(Owner { kind: Some(OwnerKind::Immutable.into()), address: Some("0x2".to_string()) }),
            ..Default::default()
        })
        .await;
...
async fn test_type_filters() {
    ...
    assert_eq!(
        list_owned_objects(&cluster, a, "0x2", None, Some(20)).await.unwrap(),
        (bags.values().chain(coins.values().rev()).chain(tu8s.values()).chain(tu64s.values()).map(repr).collect(), None)
    );

    // Fetch all bags using a package and module filter
    assert_eq!(
        list_owned_objects(&cluster, a, "0x2::bag", None, Some(20)).await.unwrap(),
        (bags.values().map(repr).collect(), None)
    );

    // Fetch all tables using an unqualified type filter (no type parameters).
    assert_eq!(
        list_owned_objects(&cluster, a, "0x2::table::Table", None, Some(20)).await.unwrap(),
        (tu8s.values().chain(tu64s.values()).map(repr).collect(), None)
    );

    // Fetch all Table<u64, u64>s using a fully qualified type filter.
    assert_eq!(
        list_owned_objects(&cluster, a, "0x2::table::Table<u64, u64>", None, Some(20)).await.unwrap(),
        (tu64s.values().map(repr).collect(), None)
    );

    // Fetch a type that we don't have any of, like `0x2::clock::Clock`.
    assert_eq!(list_owned_objects(&cluster, a, "0x2::clock::Clock", None, Some(20)).await.unwrap(), (vec![], None));

    // Pass in a bad type filter (malformed).
    let err = list_owned_objects(&cluster, a, "not::a::valid::type<>", None, Some(20)).await.unwrap_err();

    assert_eq!(err.code(), tonic::Code::InvalidArgument);
    assert_eq!(err.message(), "Bad 'object_type' filter, expected: package[::module[::name[<type, ...>]]]");
}
...
async fn test_shared_immutable_filters() {
    ...
    async fn list_owned_objects(
        ...
        let response = client
            .list_owned_objects(ListOwnedObjectsRequest {
                owner: Some(Owner { kind: Some(kind.into()), address: address.map(|a| a.to_string()) }),
                object_type: Some(object_type.to_string()),
                page_size,
                after_token: after_token.map(Into::into),
                ..Default::default()
    ...
    assert_eq!(
        list_owned_objects(&cluster, OwnerKind::Shared, None, "0x2::coin", None, Some(20)).await.unwrap(),
        (shared.values().rev().map(repr).collect(), None),
    );

    // Fetching all the frozen coins, filtering by package, module and type name.
    assert_eq!(
        list_owned_objects(&cluster, OwnerKind::Immutable, None, "0x2::coin::Coin", None, Some(20)).await.unwrap(),
...
async fn test_coin_balance_change_cleanup() {
    ...
    builder.transfer_oct(a, Some(500));
    ...
    assert!(matches!(fx, TransactionEffects::V1(_)), "Test must run with Effects V1 to validate the fix");
    ...
    let objects = list_owned_objects(&cluster, a).await.unwrap();
    assert_eq!(objects.len(), 2, "A should own exactly 2 objects (no duplicates)");

    // Verify the objects are the new versions
    let object_ids: Vec<_> = objects.iter().map(|(id, ver, _)| (id.as_str(), *ver)).collect();
    assert!(
...
fn repr((i, v, d): &ObjectRef) -> (String, u64, String) {
    (i.to_canonical_string(/* with_prefix */ true), v.value(), d.base58_encode())
}
...
fn create_bag(cluster: &mut FullCluster, owner: SuiAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster.funded_account(DEFAULT_GAS_BUDGET).expect("Failed to fund account");
...
fn transfer_object(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_object(recipient, FullObjectRef::from_fastpath_ref(object)).unwrap();
