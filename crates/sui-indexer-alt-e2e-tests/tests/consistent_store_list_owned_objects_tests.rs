...
use sui_indexer_alt_e2e_tests::{find_address_mutated, find_address_owned, find_immutable, find_shared, FullCluster};
use sui_types::{
...
async fn test_address_owner() {
    ...
    let mut a_gas =
        find_address_owned(&cluster.request_gas(a, DEFAULT_GAS_BUDGET * 5).unwrap()).expect("Failed to find gas for A");
    let mut b_gas =
        find_address_owned(&cluster.request_gas(b, DEFAULT_GAS_BUDGET * 5).unwrap()).expect("Failed to find gas for B");
    cluster.create_checkpoint().await;
