...
async fn test_filter_pruned() {
    ...
    cluster.wait_for_pruner("tx_affected_addresses", 5, Duration::from_secs(10)).await.unwrap();
    ...
    cluster.wait_for_pruner("tx_digests", 5, Duration::from_secs(10)).await.unwrap();

    cluster.wait_for_pruner("tx_affected_addresses", 10, Duration::from_secs(10)).await.unwrap();
...
async fn test_digests_pruned() {
    ...
    cluster.wait_for_pruner("tx_digests", 10, Duration::from_secs(10)).await.unwrap();
...
async fn query_transactions(
    ...
    let response = client.post(cluster.jsonrpc_url()).json(&query).send().await.expect("Failed to send request");

    let body: Value = response.json().await.expect("Failed to parse JSON-RPC response");
