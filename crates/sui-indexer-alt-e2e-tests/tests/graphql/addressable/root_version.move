...
//> 0: one::bag::add<u64, one::bag::Bag>(Input(2), Input(3), Input(4));
//> 1: one::bag::add<u64, one::bag::Bag>(Input(2), Input(5), Input(6));
//> 2: one::bag::add<u64, one::bag::Bag>(Input(0), Input(1), Input(2));

//# programmable --sender A --inputs object(5,0) 400u64 500u64
//> 0: SplitCoins(Gas, [Input(1), Input(2)]);
//> 1: one::object_bag::add<u64, one::coin::Coin<one::oct::OCT>>(Input(0), Input(1), NestedResult(0,0));
//> 2: one::object_bag::add<u64, one::coin::Coin<one::oct::OCT>>(Input(0), Input(2), NestedResult(0,1));
