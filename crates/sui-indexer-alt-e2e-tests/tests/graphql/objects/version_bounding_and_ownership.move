...
//> 1: one::object::id_address<one::coin::Coin<one::oct::OCT>>(Gas);
...
//> 1: one::object::id_address<one::coin::Coin<one::oct::OCT>>(NestedResult(0,0));
//> 2: TransferObjects([NestedResult(0,1)], Result(1));
//> 3: one::object::id_address<one::coin::Coin<one::oct::OCT>>(Gas);
...
{ # ... and for balance point-lookups
  object(address: "@{obj_0_0}", version: 2) {
    balance(coinType: "0x2::oct::OCT") { totalBalance }
  }
}
...
{ # ... and balance multi-gets
  object(address: "@{obj_0_0}", version: 2) {
    multiGetBalances(keys: ["0x2::oct::OCT"]) { totalBalance }
  }
}
...
fragment Owned on IAddressable {
  ...
  sui: balance(coinType: "0x2::oct::OCT") { totalBalance }
