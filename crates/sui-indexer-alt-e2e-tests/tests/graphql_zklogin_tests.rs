...
impl FullCluster {
    async fn new() -> anyhow::Result<Self> {
        ...
        let offchain = OffchainCluster::new(
            ClientArgs {
                ingestion: IngestionClientArgs { local_ingestion_path: Some(ingestion_dir), ..Default::default() },
                ..Default::default()
            },
            OffchainClusterConfig {
                graphql_config: GraphQlConfig {
                    zklogin: ZkLoginConfig { env: ZkLoginEnv::Test, ..Default::default() },
                    ..Default::default()
                },
                ..Default::default()
            },
        ...
        Ok(Self { onchain, offchain, temp_dir })
    }
    ...
    async fn verify_zklogin(
        ...
        if let Some(errors) = response.get("errors").and_then(|es| es.as_array()) {
            let errors: Vec<_> = errors.iter().map(|e| e.get("message").unwrap().as_str().unwrap().to_owned()).collect();
...
async fn test_verify_transaction() {
    ...
    let (kp, pk, inputs) = &load_test_vectors("../sui-types/src/unit_tests/zklogin_test_vectors.json")[1];

    let addr = pk.into();
    let rgp = cluster.onchain.get_reference_gas_price().await;
    let gas = cluster.onchain.fund_address_and_return_gas(rgp, Some(1000000), addr).await;

    let tx = TestTransactionBuilder::new(addr, gas, rgp).transfer_oct(None, SuiAddress::ZERO).build();
    ...
    let result = cluster
        .verify_zklogin(bcs::to_bytes(&tx).unwrap(), signature.as_ref().to_owned(), "TRANSACTION_DATA", addr)
        .await
        .unwrap();

    assert_eq!(result, ZkLoginResult { success: true, error: None });
}
...
async fn test_verify_personal_message() {
    ...
    let message = IntentMessage::new(Intent::personal_message(), PersonalMessage { message: personal.clone() });
    ...
    let result =
        cluster.verify_zklogin(personal, signature.as_ref().to_owned(), "PERSONAL_MESSAGE", addr).await.unwrap();
...
async fn test_verify_invalid_scope() {
    ...
    let result =
        cluster.verify_zklogin(personal, signature.as_ref().to_owned(), "TRANSACTION_DATA", addr).await.unwrap_err();
...
async fn test_verify_invalid_transaction() {
    ...
    let result = cluster
        .verify_zklogin(invalid_tx_bytes, signature.as_ref().to_owned(), "TRANSACTION_DATA", addr)
        .await
        .unwrap_err();
...
async fn test_verify_wrong_address() {
    ...
    assert_eq!(result, ZkLoginResult { success: false, error: Some("Invalid address".to_string()) });
}
...
async fn test_verify_invalid_signature() {
    ...
    let result = cluster.verify_zklogin(personal, invalid_signature, "PERSONAL_MESSAGE", addr).await.unwrap_err();
...
async fn test_verify_not_zklogin_signature() {
    ...
    let keypair = SuiKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::rngs::StdRng::from_seed([1; 32])));
    ...
    let result =
        cluster.verify_zklogin(personal, signature.as_ref().to_owned(), "PERSONAL_MESSAGE", addr).await.unwrap_err();
