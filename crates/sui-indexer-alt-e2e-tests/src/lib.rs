...
use sui_indexer_alt_reader::{bigtable_reader::BigtableArgs, system_package_task::SystemPackageTaskArgs};
use sui_pg_db::{
...
impl FullCluster {
    ...
    pub fn funded_account(&mut self, amount: u64) -> anyhow::Result<(SuiAddress, AccountKeyPair, ObjectRef)> {
    ...
    pub fn request_gas(&mut self, address: SuiAddress, amount: u64) -> anyhow::Result<TransactionEffects> {
...
impl OffchainCluster {
    ...
    pub async fn latest_checkpoint(&self) -> anyhow::Result<Option<u64>> {
        ...
        let mut conn = self.db.connect().await.context("Failed to connect to database")?;
    ...
    pub async fn latest_pruner_checkpoint(&self, pipeline: &str) -> anyhow::Result<Option<u64>> {
        ...
        let latest: Option<i64> =
            w::watermarks.select(w::reader_lo).filter(w::pipeline.eq(pipeline)).first(&mut conn).await.optional()?;
    ...
    pub async fn latest_graphql_checkpoint(&self) -> anyhow::Result<u64> {
        ...
        let body: Response = response.json().await.context("Failed to parse GraphQL response")?;

        ensure!(body.data.checkpoint.sequence_number != i64::MAX, "Indexer has not started yet",);
