...
use sui_types::{
    ...
    transaction::{CallArg, Command, ObjectArg, SharedObjectMutability, Transaction, TransactionData},
    Identifier,
    TypeTag,
...
/// transaction.
pub async fn publish(cluster: &mut FullCluster, package: &str) -> (SuiAddress, Ed25519KeyPair, TransactionEffects) {
    // Compile the package
...
pub async fn migrate(
    ...
    let metadata = builder.obj(ObjectArg::ImmOrOwnedObject(outputs.metadata.unwrap())).unwrap();
...
pub async fn migrate_deny_cap(
    ...
    let deny_cap = builder.obj(ObjectArg::ImmOrOwnedObject(outputs.deny.unwrap())).unwrap();
...
pub async fn migrate_regulated_metadata(
    ...
    let regulated_metadata = builder.obj(ObjectArg::ImmOrOwnedObject(regulated_metadata)).unwrap();
