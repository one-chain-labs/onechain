...
use sui_types::{
    ...
    error::{ExecutionError, SuiError, SuiErrorKind, SuiResult},
...
impl Storage {
    ...
    pub fn all_objects(&self) -> Vec<Object> {
        self.live_objects_store
            ...
            .values()
            .cloned()
            .chain(self.package_cache.lock().expect("Unable to lock").values().cloned())
            .chain(self.object_version_cache.lock().expect("Unable to lock").values().cloned())
            .collect::<Vec<_>>()
    }
}
...
impl LocalExec {
    ...
    pub async fn execute_all_in_checkpoints(
        ...
        for tx in txs {
            match self
                .execute_transaction(&tx, expensive_safety_check_config.clone(), use_authority, None, None, None)
                .await
    ...
    async fn resolve_download_input_objects(
        ...
        tx_info
            ...
            .map(|kind| match kind {
                ...
                InputObjectKind::SharedMoveObject { id, initial_shared_version: _, mutability: _ }
