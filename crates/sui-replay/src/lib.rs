...
static DEFAULT_SANDBOX_BASE_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/sandbox_snapshots");
...
pub enum ReplayToolCommand {
    ...
    ReplayBatch {
        ...
        #[arg(long, short, default_value = "16", help = "Number of tasks to run in parallel")]
        num_tasks: u64,
        #[arg(
            long,
    ...
    BatchReplayFromSandbox {
        #[arg(help = "The path to the directory that contains many JSON files, each representing a persisted sandbox.\
            These files are typically generated by running the ReplayBatch command with --persist-path specified.")]
        path: String,
        #[arg(long, short, default_value = "64", help = "Number of tasks to run in parallel")]
        num_tasks: usize,
    },
...
pub async fn execute_replay_command(
    ...
    let safety =
        if safety_checks { ExpensiveSafetyCheckConfig::new_enable_all() } else { ExpensiveSafetyCheckConfig::default() };
    Ok(match cmd {
        ReplayToolCommand::ReplaySandbox { path } => {
            ...
            let sandbox_state = LocalExec::certificate_execute_with_sandbox_state(&sandbox_state).await?;
            sandbox_state.check_effects()?;
            info!("Execution finished successfully. Local and on-chain effects match.");
            None
        }
        ReplayToolCommand::PersistSandbox { tx_digest, base_path } => {
        ...
        ReplayToolCommand::Fuzz { start, num_mutations_per_base, num_base_transactions } => {
            ...
            let fuzzer = ReplayFuzzer::new(get_rpc_url(rpc_url, cfg_path, chain)?, config).await.unwrap();
        ...
        ReplayToolCommand::ReplayBatch { path, terminate_early, num_tasks, persist_path } => {
        ...
        ReplayToolCommand::BatchReplayFromSandbox { path, num_tasks } => {
            ...
            let tasks = chunks.into_iter().map(|chunk| async move {
                for file in chunk {
                    ...
                    let sandbox_state: ExecutionSandboxState = serde_json::from_str(&contents).unwrap();
                    let sandbox_state = LocalExec::certificate_execute_with_sandbox_state(&sandbox_state).await.unwrap();
        ...
        ReplayToolCommand::Report => {
            let mut lx = LocalExec::new_from_fn_url(&rpc_url.expect("Url must be provided")).await?;
        ...
        ReplayToolCommand::ReplayCheckpoints { start, end, terminate_early, max_tasks } => {
            ...
            futures::future::join_all(handles).await.into_iter().for_each(|x| match x {
        ...
        ReplayToolCommand::ReplayEpoch { epoch, terminate_early, max_tasks } => {
            let lx = LocalExec::new_from_fn_url(&rpc_url.clone().expect("Url must be provided")).await?;

            let (start, end) = lx.checkpoints_for_epoch(epoch).await?;

            info!("Executing epoch {} (checkpoint range {}-{}) with at most {} tasks", epoch, start, end, max_tasks);
            ...
            match status {
                Ok(Some((succeeded, total))) => {
                    info!("Epoch {} replay finished {} out of {} TXs", epoch, succeeded, total);
...
fn parse_configs_versions(configs_and_versions: Option<Vec<String>>) -> Option<Vec<(ObjectID, SequenceNumber)>> {
    ...
    Some(
        configs_and_versions
            .chunks_exact(2)
            .map(|chunk| {
                let object_id = ObjectID::from_str(&chunk[0]).expect("Invalid object id for config");
                let object_version =
                    SequenceNumber::from_u64(chunk[1].parse::<u64>().expect("Invalid object version for config"));
