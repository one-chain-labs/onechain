...
fn display_info(f: &mut Formatter<'_>, sui_gas_status: &GasStatusV2) -> std::fmt::Result {
    ...
    builder.push_record(vec![format!("Reference Gas Price: {}", sui_gas_status.reference_gas_price())]);
    builder.push_record(vec![format!("Gas Price: {}", sui_gas_status.gas_status.gas_price())]);
    ...
    builder.push_record(vec![format!("Max Gas Stack Size: {}", sui_gas_status.gas_status.stack_size_high_water_mark())]);
