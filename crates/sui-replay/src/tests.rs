...
async fn execute_replay(url: &str, tx: &TransactionDigest) -> Result<(), ReplayEngineError> {
    LocalExec::new_from_fn_url(url)
        ...
        .init_for_execution()
        .await?
        .execute_transaction(tx, ExpensiveSafetyCheckConfig::default(), true, None, None, None)
        .await?
        .check_effects()?;
    tokio::task::yield_now().await;
    LocalExec::new_from_fn_url(url)
        ...
        .init_for_execution()
        .await?
        .execute_transaction(tx, ExpensiveSafetyCheckConfig::default(), false, None, None, None)
        .await?
        .check_effects()?;
    tokio::task::yield_now().await;
