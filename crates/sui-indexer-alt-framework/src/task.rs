...
pub async fn with_slow_future_monitor<F, C>(future: F, threshold: Duration, callback: C) -> F::Output
where
    F: Future,
    C: FnOnce(),
{
...
mod tests {
    ...
    async fn slow_monitor_stuck_future_detection() {
        ...
        let monitored = with_slow_future_monitor(StuckFuture, Duration::from_millis(200), || c.increment());
