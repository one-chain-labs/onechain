...
impl Connection for MockConnection<'_> {
    ...
    async fn set_reader_watermark(&mut self, _pipeline: &'static str, reader_lo: u64) -> anyhow::Result<bool> {
        ...
        *curr = Some(MockWatermark {
            reader_lo,
            pruner_timestamp: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64,
    ...
    async fn set_pruner_watermark(&mut self, _pipeline: &'static str, pruner_hi: u64) -> anyhow::Result<bool> {
        let mut curr = self.0.watermark.lock().unwrap();
        *curr = Some(MockWatermark { pruner_hi, ..curr.as_ref().unwrap().clone() });
        Ok(true)
    }
}
...
impl MockStore {
    ...
    pub async fn commit_data(&self, values: std::collections::HashMap<u64, Vec<u64>>) -> anyhow::Result<usize> {
        ...
        let mut total_count = 0;
        {
            ...
            for (checkpoint, checkpoint_values) in values {
                ...
                data.entry(checkpoint).or_default().extend(checkpoint_values);
            }
        }
    ...
    pub async fn wait_for_watermark(&self, checkpoint: u64, timeout_duration: Duration) -> MockWatermark {
        let start = std::time::Instant::now();
        while start.elapsed() < timeout_duration {
