...
use crate::store::{CommitterWatermark, Connection, PrunerWatermark, ReaderWatermark, Store, TransactionalStore};
...
impl Connection for MockConnection<'_> {
    async fn committer_watermark(&mut self, pipeline_task: &str) -> Result<Option<CommitterWatermark>, anyhow::Error> {
    ...
    async fn reader_watermark(&mut self, pipeline: &'static str) -> Result<Option<ReaderWatermark>, anyhow::Error> {
        let watermark = self.0.watermarks.get(pipeline);
        Ok(watermark
            .map(|w| ReaderWatermark { checkpoint_hi_inclusive: w.checkpoint_hi_inclusive, reader_lo: w.reader_lo }))
    }
    ...
    async fn pruner_watermark(
        ...
        Ok(watermark.map(|w| {
            let elapsed_ms =
                w.pruner_timestamp as i64 - SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as i64;
    ...
    async fn set_reader_watermark(&mut self, pipeline: &'static str, reader_lo: u64) -> anyhow::Result<bool> {
        ...
        curr.pruner_timestamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;
        Ok(true)
    }

    async fn set_pruner_watermark(&mut self, pipeline: &'static str, pruner_hi: u64) -> anyhow::Result<bool> {
        let mut curr = self.0.watermarks.get_mut(pipeline).unwrap();
...
impl MockStore {
    ...
    pub async fn commit_data(&self, pipeline: &'static str, checkpoint: u64, values: Vec<u64>) -> anyhow::Result<usize> {
        // Apply commit delay if configured
        if self.commit_delay_ms > 0 {
            tokio::time::sleep(Duration::from_millis(self.commit_delay_ms)).await;
        }
    ...
    pub fn prune_data(&self, pipeline: &'static str, from: u64, to_exclusive: u64) -> anyhow::Result<usize> {
    ...
    pub fn with_commit_watermark_failures(mut self, failures: usize) -> Self {
        self.commit_watermark_failures = Arc::new(Failures { failures, attempts: AtomicUsize::new(0) });
        self
    }
    ...
    pub fn with_prune_failures(self, from: u64, to_exclusive: u64, failures: usize) -> Self {
        self.prune_failure_attempts.insert((from, to_exclusive), Failures { failures, attempts: AtomicUsize::new(0) });
        self
    }
    ...
    pub fn with_data(self, pipeline_task: &str, data: std::collections::HashMap<u64, Vec<u64>>) -> Self {
    ...
    pub async fn wait_for_prune_attempts(&self, from: u64, to_exclusive: u64, expected: usize, timeout: Duration) {
        tokio::time::timeout(timeout, async {
            loop {
    ...
    pub async fn wait_for_data(&self, pipeline_task: &str, checkpoint: u64, timeout_duration: Duration) -> Vec<u64> {
        let start = std::time::Instant::now();
        while start.elapsed() < timeout_duration {
            {
    ...
    pub async fn wait_for_watermark(
        ...
        panic!("Timeout waiting for watermark on pipeline {} to reach {}", pipeline_task, checkpoint);
    }
}
