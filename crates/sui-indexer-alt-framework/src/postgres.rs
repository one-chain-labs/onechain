...
impl Indexer<Db> {
    ...
    pub async fn new_from_pg(
        ...
        store.run_migrations(migrations).await.context("Failed to run pending migrations")?;

        registry.register(Box::new(DbConnectionStatsCollector::new(Some("indexer_db"), store.clone())))?;

        Indexer::new(store, indexer_args, client_args, ingestion_config, registry, cancel).await
    }
    ...
    pub async fn new_for_testing(migrations: &'static EmbeddedMigrations) -> (Indexer<Db>, TempDb) {
        ...
        let store = Db::for_write(temp_db.database().url().clone(), DbArgs::default()).await.unwrap();
...
pub mod tests {
    ...
    async fn test_add_existing_pipeline() {
        let (mut indexer, _temp_db) = Indexer::new_for_testing(&MIGRATIONS).await;
        {
            ...
            assert!(conn.set_committer_watermark(ConcurrentPipeline1::NAME, watermark).await.unwrap());
        }
        indexer.concurrent_pipeline(ConcurrentPipeline1, ConcurrentConfig::default()).await.unwrap();
    ...
    async fn test_add_multiple_pipelines() {
        let (mut indexer, _temp_db) = Indexer::new_for_testing(&MIGRATIONS).await;
        {
            ...
            let mut conn = indexer.store().connect().await.unwrap();
            assert!(conn.set_committer_watermark(ConcurrentPipeline1::NAME, watermark1).await.unwrap());
            let watermark2 = CommitterWatermark::new_for_testing(20);
            assert!(conn.set_committer_watermark(ConcurrentPipeline2::NAME, watermark2).await.unwrap());
        }
    ...
    async fn test_add_multiple_pipelines_pruning_requires_processed_values() {
...

        {
            ...
            let mut conn = indexer.store().connect().await.unwrap();
            assert!(conn.set_committer_watermark(ConcurrentPipeline3::NAME, watermark3).await.unwrap());
            assert!(conn.set_pruner_watermark(ConcurrentPipeline3::NAME, 5).await.unwrap());
        }
