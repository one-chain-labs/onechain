...
use crate::{metrics::IndexerMetrics, store::Store, types::full_checkpoint_content::CheckpointData, FieldCount};
...
impl<H: Handler> BatchedRows<H> {
    ...
    fn is_full(&self) -> bool {
        self.values.len() >= max_chunk_rows::<H>() || self.watermark.len() >= H::MAX_WATERMARK_UPDATES
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        pipeline::Processor,
        types::{full_checkpoint_content::CheckpointData, test_checkpoint_data_builder::TestCheckpointDataBuilder},
        FieldCount,
    };
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(values: &[Self::Value], conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
    ...
    async fn test_out_of_order_processing() {
        ...
        let _watermark = setup.store.wait_for_watermark(4, Duration::from_secs(5)).await;
