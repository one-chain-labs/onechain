...
use self::{
    ...
    committer::committer,
    main_reader_lo::track_main_reader_lo,
    pruner::pruner,
    reader_watermark::reader_watermark,
};
use super::{processor::processor, CommitterConfig, Processor, WatermarkPart, PIPELINE_BUFFER};
use crate::{metrics::IndexerMetrics, store::Store, types::full_checkpoint_content::Checkpoint, Task};
    ...
    H: Handler<Batch = Vec<V>, Value = V>,
{
    ...
    pub fn from_vec(batch: Vec<V>, watermark: Vec<WatermarkPart>) -> Self {
        ...
        Self { batch, batch_len, watermark }
    }
}
...
pub(crate) fn pipeline<H: Handler + Send + Sync + 'static>(
    ...
    let reader_watermark =
        reader_watermark::<H>(pruner_config.clone(), store.clone(), metrics.clone(), pruner_cancel.clone());

    let pruner = pruner(handler, pruner_config, store, metrics, pruner_cancel.clone());
...
mod tests {
    ...
    use crate::{
        ...
        pipeline::Processor,
        types::{full_checkpoint_content::Checkpoint, test_checkpoint_data_builder::TestCheckpointBuilder},
        FieldCount,
    };
    ...
    impl Processor for DataPipeline {
        ...
        async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
            ...
            Ok(vec![TestValue { checkpoint: cp_num, data: cp_num * 10 + 1 }, TestValue {
                checkpoint: cp_num,
                data: cp_num * 10 + 2,
            }])
        }
    ...
    impl Handler for DataPipeline {
        ...
        // High value to disable eager batching
        const MAX_PENDING_ROWS: usize = 4;
        // Small value to trigger back pressure quickly
        const MAX_WATERMARK_UPDATES: usize = 1;
        const MIN_EAGER_ROWS: usize = 1000;

        // Each batch will have 1 checkpoint for an ease of testing.
        ...
        async fn commit<'a>(&self, batch: &Self::Batch, conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            // Group values by checkpoint
            let mut grouped: std::collections::HashMap<u64, Vec<u64>> = std::collections::HashMap::new();
            for value in batch {
                grouped.entry(value.checkpoint).or_default().push(value.data);
            }
        ...
        async fn prune<'a>(&self, from: u64, to_exclusive: u64, conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
    ...
    impl TestSetup {
        async fn new(config: ConcurrentConfig, store: MockStore, next_checkpoint: u64) -> Self {
            ...
            Self { store, checkpoint_tx, pipeline_handle, cancel }
        }
        ...
        async fn send_checkpoint_with_timeout(&self, checkpoint: u64, timeout_duration: Duration) -> anyhow::Result<()> {
            timeout(timeout_duration, self.send_checkpoint(checkpoint)).await?
        }

        async fn send_checkpoint_expect_timeout(&self, checkpoint: u64, timeout_duration: Duration) {
            timeout(timeout_duration, self.send_checkpoint(checkpoint)).await.unwrap_err();
            // Panics if send succeeds
        }
    }
    ...
    async fn test_e2e_pipeline() {
        ...
        for i in 0 .. 3 {
            setup.send_checkpoint_with_timeout(i, Duration::from_millis(200)).await.unwrap();
        }

        // Verify all initial data is available (before any pruning)
        for i in 0 .. 3 {
            let data = setup.store.wait_for_data(DataPipeline::NAME, i, TEST_TIMEOUT).await;
        ...
        for i in 0 .. 6 {
            let data = setup.store.wait_for_data(DataPipeline::NAME, i, Duration::from_secs(1)).await;
            assert_eq!(data, vec![i * 10 + 1, i * 10 + 2]);
        }
    ...
    async fn test_e2e_pipeline_without_pruning() {
        let config = ConcurrentConfig { pruner: None, ..Default::default() };
        let store = MockStore::default();
        let setup = TestSetup::new(config, store, 0).await;
        ...
        let watermark = setup.store.wait_for_watermark(DataPipeline::NAME, 9, TEST_TIMEOUT).await;
    ...
    async fn test_out_of_order_processing() {
        ...
        for cp in checkpoints {
            setup.send_checkpoint_with_timeout(cp, Duration::from_millis(200)).await.unwrap();
        }

        // Wait for all data to be processed
        setup.store.wait_for_watermark(DataPipeline::NAME, 4, Duration::from_secs(5)).await;
    ...
    async fn test_watermark_progression_with_gaps() {
        ...
        setup.send_checkpoint_with_timeout(2, Duration::from_millis(200)).await.unwrap();

        // Now watermark should progress to 4
        let watermark = setup.store.wait_for_watermark(DataPipeline::NAME, 4, TEST_TIMEOUT).await;
    ...
    async fn test_back_pressure_collector_max_pending_rows() {
        ...
        setup.send_checkpoint_expect_timeout(14, Duration::from_millis(200)).await;

        // Allow pipeline to drain by sending the blocked checkpoint with longer timeout
        setup.send_checkpoint_with_timeout(14, TEST_TIMEOUT).await.unwrap();

        // Verify data was processed correctly
        let data = setup.store.wait_for_data(DataPipeline::NAME, 0, TEST_TIMEOUT).await;
    ...
    async fn test_back_pressure_committer_slow_commits() {
        ...
        for i in 0 .. 19 {
            setup.send_checkpoint_with_timeout(i, Duration::from_millis(100)).await.unwrap();
        }
        ...
        for checkpoint in 19 .. 22 {
            if setup.send_checkpoint_with_timeout(checkpoint, Duration::from_millis(100)).await.is_err() {
                back_pressure_checkpoint = Some(checkpoint);
                break;
            }
        }
        assert!(back_pressure_checkpoint.is_some(), "Back pressure should occur between checkpoints 19-21");

        // Verify that some data has been processed (pipeline is working)
        setup.store.wait_for_any_data(DataPipeline::NAME, TEST_TIMEOUT).await;

        // Allow pipeline to drain by sending the blocked checkpoint with longer timeout
        setup.send_checkpoint_with_timeout(back_pressure_checkpoint.unwrap(), TEST_TIMEOUT).await.unwrap();
    ...
    async fn test_commit_failure_retry() {
        ...
        // Send a checkpoint
        setup.send_checkpoint_with_timeout(0, Duration::from_millis(200)).await.unwrap();

        // Should eventually succeed despite initial commit failures
        setup.store.wait_for_watermark(DataPipeline::NAME, 0, TEST_TIMEOUT).await;

        // Verify data was eventually committed
        let data = setup.store.wait_for_data(DataPipeline::NAME, 0, Duration::from_secs(1)).await;
        assert_eq!(data, vec![1, 2]);
    ...
    async fn test_prune_failure_retry() {
        ...
        setup.store.wait_for_prune_attempts(0, 2, 1, TEST_TIMEOUT).await;
        {
            let data = setup.store.data.get(DataPipeline::NAME).unwrap();
            for i in 0 .. 4 {
        ...
        setup.store.wait_for_prune_attempts(0, 2, 2, TEST_TIMEOUT).await;
        {
            let data = setup.store.data.get(DataPipeline::NAME).unwrap();
            // Verify recent checkpoints are still available
            assert!(data.contains_key(&2));
    ...
    async fn test_reader_watermark_failure_retry() {
        ...
        setup.store.wait_for_watermark(DataPipeline::NAME, 5, TEST_TIMEOUT).await;
