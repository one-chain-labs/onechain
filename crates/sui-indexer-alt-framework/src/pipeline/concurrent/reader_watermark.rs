...
mod tests {
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(_values: &[Self::Value], _conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            Ok(0)
        }
    }
    ...
    async fn setup_test(
        ...
        let store = MockStore {
            ...
            set_reader_watermark_failure_attempts: Arc::new(Mutex::new(set_reader_watermark_failure_attempts)),
        ...
        let handle = reader_watermark::<DataPipeline>(Some(config), store_clone, metrics, cancel_clone);

        TestSetup { store, handle, cancel }
    }
    ...
    async fn test_reader_watermark_does_not_update_smaller_reader_lo() {
        ...
        {
            let watermarks = setup.store.watermark().unwrap();
            assert_eq!(watermarks.reader_lo, 7, "Reader watermark should not be updated when new value is smaller");
        }
    ...
    async fn test_reader_watermark_retry_update_after_connection_failure() {
        ...
        setup.store.wait_for_connection_attempts(1, TEST_TIMEOUT).await;

        // Verify state before retry succeeds
        let watermark = setup.store.watermark().unwrap();
        assert_eq!(watermark.reader_lo, 0, "Reader watermark should not be updated due to DB connection failure");

        // Wait for second connection attempt (which should succeed)
        setup.store.wait_for_connection_attempts(2, TEST_TIMEOUT).await;
        ...
        let watermark = setup.store.watermark().unwrap();
        assert_eq!(watermark.reader_lo, 6, "Reader watermark should be updated after retry succeeds");
