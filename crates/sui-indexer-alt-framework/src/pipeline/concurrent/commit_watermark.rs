...
use crate::{
    ...
    store::{pipeline_task, Connection, Store},
};
...
pub(super) fn commit_watermark<H: Handler + 'static>(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let guard = metrics.watermark_gather_latency.with_label_values(&[H::NAME]).start_timer();
            ...
            if let Some(watermark) = watermark {
                ...
                metrics.watermark_checkpoint.with_label_values(&[H::NAME]).set(watermark.checkpoint_hi_inclusive as i64);

                metrics.watermark_transaction.with_label_values(&[H::NAME]).set(watermark.tx_hi as i64);
                ...
                match conn.set_committer_watermark(&pipeline_task, watermark).await {
                    ...
                    Ok(true) => {
                        ...
                        metrics.watermark_transaction_in_db.with_label_values(&[H::NAME]).set(watermark.tx_hi as i64);
...
mod tests {
    ...
    use crate::{
        ...
        mocks::store::*,
        pipeline::{concurrent::BatchStatus, CommitterConfig, Processor, WatermarkPart},
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(&self, _batch: &Self::Batch, _conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            Ok(0)
        }
    }
    ...
    fn setup_test<H: Handler<Store = MockStore> + 'static>(
        ...
        let commit_watermark_handle =
            commit_watermark::<H>(next_checkpoint, config, watermark_rx, store_clone, None, metrics, cancel_clone);

        TestSetup { store, watermark_tx, commit_watermark_handle, cancel }
    }

    fn create_watermark_part_for_checkpoint(checkpoint: u64) -> WatermarkPart {
        WatermarkPart {
            watermark: CommitterWatermark { checkpoint_hi_inclusive: checkpoint, ..Default::default() },
            batch_rows: 1,
            total_rows: 1,
        }
    }
    ...
    async fn test_out_of_order_watermarks() {
        ...
        setup.watermark_tx.send(vec![create_watermark_part_for_checkpoint(3)]).await.unwrap();
    ...
    async fn test_committer_retries_on_commit_watermark_failure() {
        ...
        let part = WatermarkPart {
            watermark: CommitterWatermark { checkpoint_hi_inclusive: 10, ..Default::default() },
            batch_rows: 1,
            total_rows: 1,
        };
        setup.watermark_tx.send(vec![part]).await.unwrap();
    ...
    async fn test_committer_retries_on_commit_watermark_failure_advances() {
        ...
        let part = WatermarkPart {
            watermark: CommitterWatermark { checkpoint_hi_inclusive: 11, ..Default::default() },
            batch_rows: 1,
            total_rows: 1,
        };
        setup.watermark_tx.send(vec![part]).await.unwrap();
    ...
    async fn test_incomplete_watermark() {
        ...
        let part = WatermarkPart {
            watermark: CommitterWatermark { checkpoint_hi_inclusive: 1, ..Default::default() },
            batch_rows: 1,
            total_rows: 3,
        };
        ...
        setup.watermark_tx.send(vec![part.clone(), part.clone()]).await.unwrap();
    ...
    async fn test_no_initial_watermark() {
        ...
        setup.watermark_tx.send(vec![create_watermark_part_for_checkpoint(1)]).await.unwrap();
        ...
        setup.watermark_tx.send(vec![create_watermark_part_for_checkpoint(0)]).await.unwrap();
    ...
    async fn test_final_watermark_sync_on_shutdown() {
        ...
        setup.watermark_tx.send(vec![create_watermark_part_for_checkpoint(10)]).await.unwrap();
        setup.watermark_tx.send(vec![create_watermark_part_for_checkpoint(11)]).await.unwrap();
