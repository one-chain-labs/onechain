...
mod tests {
    ...
    use crate::{
        metrics::IndexerMetrics,
        mocks::store::*,
        pipeline::{concurrent::BatchStatus, Processor},
        FieldCount,
    };
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(&self, batch: &Self::Batch, _conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            Ok(batch.len())
        }
    ...
    async fn test_pruner() {
        ...
        let store =
            MockStore::new().with_watermark(DataPipeline::NAME, watermark).with_data(DataPipeline::NAME, test_data);
    ...
    async fn test_pruner_watermark_update_with_retries() {
        ...
        let test_data = HashMap::from([(1, vec![1, 2]), (2, vec![3, 4]), (3, vec![5, 6]), (4, vec![7, 8])]);
        ...
        tokio::time::sleep(Duration::from_millis(500)).await;
        {
            ...
            assert_eq!(watermarks.pruner_hi, 1, "Pruner watermark should remain at 1 because range [1,2) failed");
        ...
        tokio::time::sleep(Duration::from_millis(3000)).await;
        {
            ...
            assert_eq!(watermarks.pruner_hi, 4, "Pruner watermark should advance to 4 after all ranges complete");
            assert!(!data.contains_key(&1), "Checkpoint 1 should be pruned");
            assert!(!data.contains_key(&2), "Checkpoint 2 should be pruned");
            assert!(!data.contains_key(&3), "Checkpoint 3 should be pruned");
            assert!(data.contains_key(&4), "Checkpoint 4 should be preserved");
