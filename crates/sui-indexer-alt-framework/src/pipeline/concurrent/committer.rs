...
pub(super) fn committer<H: Handler + 'static>(
    ...
    tokio::spawn(async move {
        ...
        match ReceiverStream::new(rx)
            .try_for_each_spawned(config.write_concurrency, |BatchedRows { batch, batch_len, watermark }| {
...
mod tests {
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(&self, batch: &Self::Batch, conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            for value in batch {
                ...
                conn.0
                    .commit_data(DataPipeline::NAME, value.cp_sequence_number, value.tx_sequence_numbers.clone())
                    .await?;
            }
            Ok(batch.len())
        }
    ...
    async fn setup_test(store: MockStore) -> TestSetup {
        ...
        let committer_handle = tokio::spawn(async move {
            let _ = committer(handler, config, batch_rx, watermark_tx, store_clone, metrics, cancel).await;
        });
    ...
    async fn test_concurrent_batch_processing() {
        ...
        let batch2 = BatchedRows::from_vec(
            vec![StoredData { cp_sequence_number: 3, tx_sequence_numbers: vec![7, 8, 9], ..Default::default() }],
            vec![WatermarkPart {
                watermark: CommitterWatermark {
                    epoch_hi_inclusive: 0,
    ...
    async fn test_commit_with_retries_for_commit_failure() {
        ...
        // Verify state before retry succeeds
        {
            ...
            assert!(data.is_none(), "Data should not be committed before retry succeeds");
        }
    ...
    async fn test_commit_with_retries_for_connection_failure() {
        ...
        let batch = BatchedRows::from_vec(
            vec![StoredData { cp_sequence_number: 1, tx_sequence_numbers: vec![1, 2, 3], ..Default::default() }],
            vec![WatermarkPart {
                watermark: CommitterWatermark {
                    epoch_hi_inclusive: 0,
                    checkpoint_hi_inclusive: 1,
    ...
    async fn test_empty_batch_handling() {
        ...
        {
            let data = setup.store.data.get(DataPipeline::NAME);
            assert!(data.is_none(), "No data should be committed for empty batch");
        }
