...
mod tests {
    ...
    impl Handler for DataPipeline {
        ...
        async fn commit<'a>(values: &[StoredData], conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            for value in values {
                ...
                {
                    let remaining = value.commit_failure_remaining.fetch_sub(1, Ordering::Relaxed);
                    ensure!(remaining == 0, "Commit failed, remaining failures: {}", remaining - 1);
                }
    ...
    async fn setup_test(store: MockStore, skip_watermark: bool) -> TestSetup {
        ...
        let committer_handle = tokio::spawn(async move {
            let _ = committer(config, skip_watermark, batch_rx, watermark_tx, store_clone, metrics, cancel).await;
        });

        TestSetup { store, batch_tx, watermark_rx, committer_handle }
    }
    ...
    async fn test_concurrent_batch_processing() {
        ...
        let batch1 = BatchedRows {
            values: vec![
                StoredData { cp_sequence_number: 1, tx_sequence_numbers: vec![1, 2, 3], ..Default::default() },
                StoredData { cp_sequence_number: 2, tx_sequence_numbers: vec![4, 5, 6], ..Default::default() },
            ],
            watermark: vec![
                WatermarkPart {
        ...
        let batch2 = BatchedRows {
            values: vec![StoredData { cp_sequence_number: 3, tx_sequence_numbers: vec![7, 8, 9], ..Default::default() }],
        ...
        {
            let data: std::sync::MutexGuard<'_, HashMap<u64, Vec<u64>>> = setup.store.data.lock().unwrap();
    ...
    async fn test_commit_with_retries_for_commit_failure() {
        ...
        // Verify state before retry succeeds
        {
            ...
            assert!(data.is_empty(), "Data should not be committed before retry succeeds");
        }
        assert!(setup.watermark_rx.try_recv().is_err(), "No watermark should be received before retry succeeds");
    ...
    async fn test_commit_with_retries_for_connection_failure() {
        ...
        let batch = BatchedRows {
            values: vec![StoredData { cp_sequence_number: 1, tx_sequence_numbers: vec![1, 2, 3], ..Default::default() }],
            watermark: vec![WatermarkPart {
                watermark: CommitterWatermark {
                    epoch_hi_inclusive: 0,
                    checkpoint_hi_inclusive: 1,
    ...
    async fn test_empty_batch_handling() {
        ...
        {
            let data = setup.store.data.lock().unwrap();
            assert!(data.is_empty(), "No data should be committed for empty batch");
        }
    ...
    async fn test_skip_watermark_mode() {
        ...
        assert!(setup.watermark_rx.try_recv().is_err(), "No watermark should be sent in skip_watermark mode");
    ...
    async fn test_watermark_channel_closed() {
        ...
        assert!(result.is_ok(), "Committer should terminate gracefully when watermark channel is closed");
    }
}
