...
mod tests {
    ...
    impl Processor for DataPipeline {
        ...
        async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
            Ok(vec![StoredData { value: checkpoint.summary.sequence_number * 10 + 1 }, StoredData {
    ...
    async fn test_processor_process_checkpoints() {
        ...
        let indexed1 = indexed_rx.recv().await.expect("Should receive first IndexedCheckpoint");
        ...
        let indexed2 = indexed_rx.recv().await.expect("Should receive second IndexedCheckpoint");
        ...
        assert!(timeout_result.is_err(), "Should timeout waiting for more checkpoints");
    ...
    async fn test_processor_does_not_process_checkpoint_after_cancellation() {
        // Build two checkpoints using the test builder
        let checkpoint1: Arc<Checkpoint> = Arc::new(TestCheckpointBuilder::new(1).build_checkpoint());
        let checkpoint2: Arc<Checkpoint> = Arc::new(TestCheckpointBuilder::new(2).build_checkpoint());
        ...
        assert!(next_result.is_none(), "Channel should be closed after cancellation");
    ...
    async fn test_processor_error_retry_behavior() {
        ...
        let processor = Arc::new(RetryTestPipeline { attempt_count: attempt_count.clone() });
        ...
        let indexed2 = indexed_rx.recv().await.expect("Should receive second IndexedCheckpoint after retries");
        assert_eq!(indexed2.watermark.checkpoint_hi_inclusive, 2);
    ...
    async fn test_processor_concurrency() {
        ...
        impl Processor for SlowProcessor {
            ...
            async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
                ...
                Ok(vec![StoredData { value: checkpoint.summary.sequence_number }])
            }
        }

        // Set up test data
        let checkpoints: Vec<Arc<Checkpoint>> =
            (0 .. 5).map(|i| Arc::new(TestCheckpointBuilder::new(i).build_checkpoint())).collect();
