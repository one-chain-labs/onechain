...
mod tests {
    ...
    impl Processor for DataPipeline {
        ...
        fn process(&self, checkpoint: &Arc<CheckpointData>) -> anyhow::Result<Vec<Self::Value>> {
            Ok(vec![StoredData { value: checkpoint.checkpoint_summary.sequence_number * 10 + 1 }, StoredData {
    ...
    async fn test_processor_error_failed_to_process_checkpoint() {
        ...
        let next_result = indexed_rx.recv().await;
        assert!(next_result.is_none(), "Channel should be closed after processing error");
    ...
    async fn test_processor_concurrency() {
        ...
        impl Processor for SlowProcessor {
            ...
            fn process(&self, checkpoint: &Arc<CheckpointData>) -> anyhow::Result<Vec<Self::Value>> {
                ...
                Ok(vec![StoredData { value: checkpoint.checkpoint_summary.sequence_number }])
            }
        }

        // Set up test data
        let checkpoints: Vec<_> =
            (0 .. 5).map(|i| Arc::new(TestCheckpointDataBuilder::new(i).build_checkpoint())).collect();
