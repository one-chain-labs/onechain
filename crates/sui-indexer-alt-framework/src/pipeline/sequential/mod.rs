...
pub trait Handler: Processor {
    ...
    /// affected.
    async fn commit<'a>(batch: &Self::Batch, conn: &mut <Self::Store as Store>::Connection<'a>)
        -> anyhow::Result<usize>;
}
...
pub(crate) fn pipeline<H: Handler + Send + Sync + 'static>(
    ...
    let processor = processor(Arc::new(handler), checkpoint_rx, processor_tx, metrics.clone(), cancel.clone());

    let committer =
        committer::<H>(config, initial_watermark, committer_rx, watermark_tx, db, metrics.clone(), cancel.clone());
