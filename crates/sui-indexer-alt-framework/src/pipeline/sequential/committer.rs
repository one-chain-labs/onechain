...
mod tests {
    ...
    fn setup_test(next_checkpoint: u64, config: SequentialConfig, store: MockStore) -> TestSetup {
        ...
        let committer_handle =
            committer(handler, config, next_checkpoint, checkpoint_rx, commit_hi_tx, store_clone, metrics, cancel);

        TestSetup { store, checkpoint_tx, commit_hi_rx, committer_handle }
    }
    ...
    async fn test_committer_commit_up_to_max_batch_checkpoints() {
        ...
        assert_eq!(commit_hi1.1, 3, "First commit_hi should be 3 (checkpoint 2 + 1, highest processed of first batch)");
    ...
    async fn test_committer_retries_on_transaction_failure() {
        ...
        let commit_hi = setup.commit_hi_rx.recv().await.unwrap();
        assert_eq!(commit_hi.0, "test", "Pipeline name should be 'test'");
        assert_eq!(commit_hi.1, 11, "commit_hi should be 11 (checkpoint 10 + 1)");
