...
mod tests {
    ...
    impl super::Handler for TestHandler {
        ...
        const MAX_BATCH_CHECKPOINTS: usize = 3;
        // Using small max value for testing.
        const MIN_EAGER_ROWS: usize = 4;

        // Using small eager value for testing.

        fn batch(batch: &mut Self::Batch, values: Vec<Self::Value>) {
            batch.extend(values);
        }

        async fn commit<'a>(batch: &Self::Batch, conn: &mut MockConnection<'a>) -> anyhow::Result<usize> {
            if !batch.is_empty() {
    ...
    fn setup_test(next_checkpoint: u64, config: SequentialConfig, store: MockStore) -> TestSetup {
        ...
        let committer_handle =
            committer(config, next_checkpoint, checkpoint_rx, watermark_tx, store_clone, metrics, cancel);

        TestSetup { store, checkpoint_tx, watermark_rx, committer_handle }
    }

    async fn send_checkpoint(setup: &mut TestSetup, checkpoint: u64) {
        setup.checkpoint_tx.send(create_checkpoint(checkpoint)).await.unwrap();
    }
    ...
    async fn test_committer_commit_up_to_max_batch_checkpoints() {
        ...
        assert_eq!(watermark1.1, 2, "First watermark should be at checkpoint 2 (highest processed of first batch)");

        let watermark2 = setup.watermark_rx.recv().await.unwrap();
        assert_eq!(watermark2.1, 3, "Second watermark should be at checkpoint 3 (highest processed of second batch)");
