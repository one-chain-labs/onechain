...
impl IndexerClusterBuilder {
    ...
    pub async fn build(self) -> Result<IndexerCluster> {
        ...
        Ok(IndexerCluster { indexer, metrics, cancel })
    }
}
...
mod tests {
    ...
    impl concurrent::Handler for TxCounts {
        ...
        async fn commit<'a>(values: &[Self::Value], conn: &mut Connection<'a>) -> anyhow::Result<usize> {
            Ok(diesel::insert_into(tx_counts::table).values(values).on_conflict_do_nothing().execute(conn).await?)
        }
    }
    ...
    async fn test_indexer_cluster() {
        ...
        let args = Args {
            ...
            indexer_args: IndexerArgs { first_checkpoint: Some(0), last_checkpoint: Some(9), ..Default::default() },
            metrics_args: MetricsArgs { metrics_address },
        };

        let mut indexer =
            IndexerCluster::builder().with_database_url(url.clone()).with_args(args).build().await.unwrap();

        indexer.concurrent_pipeline(TxCounts, ConcurrentConfig::default()).await.unwrap();
        ...
        // Check that the results were all written out.
        {
            ...
            let counts: Vec<StoredTxCount> =
                tx_counts::table.order_by(tx_counts::cp_sequence_number).load(&mut conn).await.unwrap();
        ...
        macro_rules! assert_pipeline_metric {
            ($name:ident, $value:expr) => {
                assert_eq!(metrics.$name.get_metric_with_label_values(&["tx_counts"]).unwrap().get(), $value);
            };
        }
    ...
    fn test_individual_methods_override_bundled_args() {
        let builder = IndexerClusterBuilder::new()
            .with_args(Args {
                indexer_args: IndexerArgs { first_checkpoint: Some(100), ..Default::default() },
                client_args: Some(ClientArgs { local_ingestion_path: Some("/bundled".into()), ..Default::default() }),
                metrics_args: MetricsArgs { metrics_address: "127.0.0.1:8080".parse().unwrap() },
            })
            .with_indexer_args(IndexerArgs { first_checkpoint: Some(200), ..Default::default() })
            .with_client_args(ClientArgs { local_ingestion_path: Some("/individual".into()), ..Default::default() })
            .with_metrics_args(MetricsArgs { metrics_address: "127.0.0.1:9090".parse().unwrap() });

        assert_eq!(builder.args.indexer_args.first_checkpoint, Some(200));
        assert_eq!(builder.args.client_args.unwrap().local_ingestion_path.unwrap().to_string_lossy(), "/individual");
        assert_eq!(builder.args.metrics_args.metrics_address.to_string(), "127.0.0.1:9090");
    }
    ...
    fn test_bundled_args_override_individual_methods() {
        ...
        assert_eq!(builder.args.client_args.unwrap().local_ingestion_path.unwrap().to_string_lossy(), "/bundled");
        assert_eq!(builder.args.metrics_args.metrics_address.to_string(), "127.0.0.1:8080");
    }
}
