...
mod tests {
    ...
    use crate::{
        ...
        pipeline::{concurrent::ConcurrentConfig, Processor},
        postgres::{
    ...
    async fn test_indexer_cluster() {
        ...
        macro_rules! assert_pipeline_metric {
            ($name:ident, $value:expr) => {
                assert_eq!(indexer_metrics.$name.get_metric_with_label_values(&["tx_counts"]).unwrap().get(), $value);
            };
        }
    ...
    fn test_individual_methods_override_bundled_args() {
        ...
        assert_eq!(
            builder.args.client_args.unwrap().ingestion.local_ingestion_path.unwrap().to_string_lossy(),
            "/individual"
        );
