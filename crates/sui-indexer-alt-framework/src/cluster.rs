...
impl IndexerCluster {
    ...
    pub async fn new(database_url: Url, args: Args, migrations: Option<&'static EmbeddedMigrations>) -> Result<Self> {
        Self::new_with_configs(database_url, DbArgs::default(), args, IngestionConfig::default(), migrations, None).await
    }
    ...
    pub async fn new_with_configs(
        ...
        let registry = Registry::new_custom(metric_label, None).context("Failed to create Prometheus registry.")?;
        ...
        Ok(Self { indexer, metrics, cancel })
    }
