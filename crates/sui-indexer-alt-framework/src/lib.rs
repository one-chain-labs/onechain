...
use futures::future;
use ingestion::{ingestion_client::IngestionClient, ClientArgs, IngestionConfig, IngestionService};
...
use sui_indexer_alt_framework_store_traits::{pipeline_task, Connection, Store, TransactionalStore};
...
pub struct TaskArgs {
    ...
    /// pipeline’s pruner watermark. Requires `--reader_interval_ms`.
...
impl TaskArgs {
    ...
    fn into_task(self) -> Option<Task> {
        Some(Task { task: self.task?, reader_interval: Duration::from_millis(self.reader_interval_ms?) })
    }
}
...
impl<S: Store> Indexer<S> {
    ...
    pub async fn new(
        ...
        let ingestion_service =
            IngestionService::new(client_args, ingestion_config, metrics_prefix, registry, cancel.clone())?;
    ...
    pub fn pipelines(&self) -> impl Iterator<Item = &'static str> + '_ {
        self.added_pipelines.iter().copied().filter(|p| self.enabled_pipelines.as_ref().is_none_or(|e| e.contains(*p)))
    }
    ...
    pub async fn concurrent_pipeline<H>(&mut self, handler: H, config: ConcurrentConfig) -> Result<()>
    where
    ...
    async fn add_pipeline<P: Processor + 'static>(&mut self) -> Result<Option<u64>> {
        ...
        let mut conn = self.store.connect().await.context("Failed to establish connection to store")?;

        let pipeline_task = pipeline_task::<S>(P::NAME, self.task.as_ref().map(|t| t.task.as_str()))?;
        ...
        let next_checkpoint =
            watermark.as_ref().map(|w| w.checkpoint_hi_inclusive + 1).unwrap_or(self.default_next_checkpoint);
...
impl<T: TransactionalStore> Indexer<T> {
    ...
    pub async fn sequential_pipeline<H>(&mut self, handler: H, config: SequentialConfig) -> Result<()>
    where
        H: Handler<Store = T> + Send + Sync + 'static,
    {
        ...
        self.next_sequential_checkpoint =
            Some(self.next_sequential_checkpoint.map_or(next_checkpoint, |n| n.min(next_checkpoint)));
...
mod tests {
    ...
    use crate::{
        ...
        pipeline::{concurrent::ConcurrentConfig, CommitterConfig, Processor},
    ...
    impl Processor for ControllableHandler {
        ...
        async fn process(
            ...
            self.process_below.clone().wait_for(|&limit| cp_num <= limit).await.ok();
    ...
    impl crate::pipeline::concurrent::Handler for ControllableHandler {
        ...
        async fn commit<'a>(
            ...
            for value in batch {
                conn.0.commit_data(Self::NAME, value.0, vec![value.0]).await?;
            }
            Ok(batch.len())
        }
    }
    ...
    async fn test_first_ingestion_checkpoint_all_pipelines_have_watermarks() {
        ...
        conn.set_committer_watermark(A::NAME, CommitterWatermark { checkpoint_hi_inclusive: 100, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(B::NAME, CommitterWatermark { checkpoint_hi_inclusive: 10, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(C::NAME, CommitterWatermark { checkpoint_hi_inclusive: 1, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(D::NAME, CommitterWatermark { checkpoint_hi_inclusive: 50, ..Default::default() })
            .await
            .unwrap();
        ...
        let mut indexer =
            Indexer::new(store, indexer_args, client_args, ingestion_config, None, &registry, cancel).await.unwrap();

        indexer.concurrent_pipeline::<A>(A, ConcurrentConfig::default()).await.unwrap();
        indexer.concurrent_pipeline::<B>(B, ConcurrentConfig::default()).await.unwrap();
        indexer.sequential_pipeline::<C>(C, SequentialConfig::default()).await.unwrap();
        indexer.sequential_pipeline::<D>(D, SequentialConfig::default()).await.unwrap();
    ...
    async fn test_first_ingestion_checkpoint_smallest_is_0() {
        ...
        conn.set_committer_watermark(C::NAME, CommitterWatermark { checkpoint_hi_inclusive: 1, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(D::NAME, CommitterWatermark::default()).await.unwrap();
    ...
    async fn test_first_ingestion_checkpoint_first_checkpoint_and_no_watermark() {
        ...
        let mut conn = store.connect().await.unwrap();
        conn.set_committer_watermark(B::NAME, CommitterWatermark { checkpoint_hi_inclusive: 50, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(C::NAME, CommitterWatermark { checkpoint_hi_inclusive: 10, ..Default::default() })
            .await
            .unwrap();

        let indexer_args = IndexerArgs { first_checkpoint: Some(5), ..Default::default() };
        let temp_dir = tempfile::tempdir().unwrap();
        let client_args = ClientArgs {
            ingestion: IngestionClientArgs {
                local_ingestion_path: Some(temp_dir.path().to_owned()),
    ...
    async fn test_first_ingestion_checkpoint_large_first_checkpoint() {
        ...
        let indexer_args = IndexerArgs { first_checkpoint: Some(24), ..Default::default() };
        let temp_dir = tempfile::tempdir().unwrap();
        let client_args = ClientArgs {
            ingestion: IngestionClientArgs {
                local_ingestion_path: Some(temp_dir.path().to_owned()),
    ...
    async fn test_indexer_ingestion_existing_watermarks_no_first_checkpoint() {
        ...
        let mut conn = store.connect().await.unwrap();
        conn.set_committer_watermark(A::NAME, CommitterWatermark { checkpoint_hi_inclusive: 5, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(B::NAME, CommitterWatermark { checkpoint_hi_inclusive: 10, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(C::NAME, CommitterWatermark { checkpoint_hi_inclusive: 15, ..Default::default() })
            .await
            .unwrap();
        conn.set_committer_watermark(D::NAME, CommitterWatermark { checkpoint_hi_inclusive: 20, ..Default::default() })
            .await
            .unwrap();
        ...
        let indexer_args = IndexerArgs { last_checkpoint: Some(29), ..Default::default() };
        ...
        let mut indexer =
            Indexer::new(store.clone(), indexer_args, client_args, ingestion_config, None, &registry, cancel.clone())
                .await
                .unwrap();
        ...
        assert_eq!(
            indexer_metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[A::NAME]).unwrap().get(),
            0
        );
        assert_eq!(
            indexer_metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[B::NAME]).unwrap().get(),
            5
        );
        assert_eq!(
            indexer_metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[C::NAME]).unwrap().get(),
            10
        );
        assert_eq!(
            indexer_metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[D::NAME]).unwrap().get(),
            15
        );
    }
    ...
    async fn test_indexer_ingestion_existing_watermarks_ignore_first_checkpoint() {
        ...
        let indexer_args = IndexerArgs { first_checkpoint: Some(3), last_checkpoint: Some(29), ..Default::default() };
        ...
        assert_eq!(ingestion_metrics.total_ingested_checkpoints.get(), 24);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[A::NAME]).unwrap().get(), 0);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[B::NAME]).unwrap().get(), 5);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[C::NAME]).unwrap().get(), 10);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[D::NAME]).unwrap().get(), 15);
    }
    ...
    async fn test_indexer_ingestion_missing_watermarks_no_first_checkpoint() {
        ...
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[A::NAME]).unwrap().get(), 0);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[B::NAME]).unwrap().get(), 11);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[C::NAME]).unwrap().get(), 16);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[D::NAME]).unwrap().get(), 21);
    }
    ...
    async fn test_indexer_ingestion_use_first_checkpoint() {
        ...
        let indexer_args = IndexerArgs { first_checkpoint: Some(10), last_checkpoint: Some(29), ..Default::default() };
        ...
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[A::NAME]).unwrap().get(), 0);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[B::NAME]).unwrap().get(), 1);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[C::NAME]).unwrap().get(), 6);
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&[D::NAME]).unwrap().get(), 11);
    }
    ...
    async fn test_multiple_sequential_pipelines_next_checkpoint() {
        ...
        conn.set_committer_watermark(MockHandler::NAME, CommitterWatermark {
        ...
        conn.set_committer_watermark(SequentialHandler::NAME, CommitterWatermark {
            epoch_hi_inclusive: 0,
        ...
        let indexer_args =
            IndexerArgs { first_checkpoint: None, last_checkpoint: Some(19), pipeline: vec![], ..Default::default() };
        ...
        indexer.sequential_pipeline(MockHandler, pipeline::sequential::SequentialConfig::default()).await.unwrap();

        // Verify next_sequential_checkpoint is set correctly (10 + 1 = 11)
        assert_eq!(indexer.next_sequential_checkpoint(), Some(11), "next_sequential_checkpoint should be 11");

        // Add second sequential pipeline
        indexer.sequential_pipeline(SequentialHandler, pipeline::sequential::SequentialConfig::default()).await.unwrap();

        // Should change to 6 (minimum of 6 and 11)
        assert_eq!(indexer.next_sequential_checkpoint(), Some(6), "next_sequential_checkpoint should still be 6");
        ...
        let watermark2 = conn.committer_watermark(SequentialHandler::NAME).await.unwrap();
    ...
    async fn test_tasked_pipelines_ignore_below_main_reader_lo() {
        ...
        let mut conn = store.connect().await.unwrap();
        conn.set_committer_watermark(MockCheckpointSequenceNumberHandler::NAME, CommitterWatermark {
            checkpoint_hi_inclusive: 10,
            ..Default::default()
        })
        .await
        .unwrap();
        conn.set_reader_watermark(MockCheckpointSequenceNumberHandler::NAME, 7).await.unwrap();
        ...
        let mut tasked_indexer =
            Indexer::new(store.clone(), indexer_args, client_args, ingestion_config, None, &registry, cancel)
                .await
                .unwrap();

        let _ =
            tasked_indexer.concurrent_pipeline(MockCheckpointSequenceNumberHandler, ConcurrentConfig::default()).await;
        ...
        let data = store.data.get(MockCheckpointSequenceNumberHandler::NAME).unwrap();
    ...
    async fn test_tasked_pipelines_surpass_main_pipeline_committer_hi() {
        ...
        let mut conn = store.connect().await.unwrap();
        conn.set_committer_watermark("test", CommitterWatermark { checkpoint_hi_inclusive: 10, ..Default::default() })
            .await
            .unwrap();
        ...
        assert_eq!(metrics.total_watermarks_out_of_order.get_metric_with_label_values(&["test"]).unwrap().get(), 0);
        assert_eq!(
            metrics
                .total_collector_skipped_checkpoints
                .get_metric_with_label_values(&[MockCheckpointSequenceNumberHandler::NAME])
    ...
    async fn test_tasked_pipelines_skip_checkpoints_trailing_main_reader_lo() {
        ...
        let _ = tasked_indexer
            .concurrent_pipeline(controllable_handler, ConcurrentConfig {
                committer: CommitterConfig { collect_interval_ms: 10, watermark_interval_ms: 10, ..Default::default() },
                ..Default::default()
            })
            .await;
        ...
        conn.set_committer_watermark(ControllableHandler::NAME, CommitterWatermark {
            checkpoint_hi_inclusive: 300,
            ..Default::default()
        })
        .await
        .unwrap();
        conn.set_reader_watermark(ControllableHandler::NAME, 250).await.unwrap();
        ...
        assert_eq!(lt_250, 11);
        assert_eq!(
            conn.committer_watermark(&pipeline_task::<MockStore>(ControllableHandler::NAME, Some("task")).unwrap())
                .await
                .unwrap()
                .unwrap()
