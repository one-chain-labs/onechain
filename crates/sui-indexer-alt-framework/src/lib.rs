...
use sui_indexer_alt_framework_store_traits::{CommitterWatermark, Connection, Store, TransactionalStore};
...
impl<S: Store> Indexer<S> {
    ...
    pub async fn new(
        ...
        let IndexerArgs { first_checkpoint, last_checkpoint, pipeline, skip_watermark } = indexer_args;
    ...
    async fn add_pipeline<P: Processor + 'static>(&mut self) -> Result<Option<Option<CommitterWatermark>>> {
        ...
        let expected_first_checkpoint = watermark.as_ref().map(|w| w.checkpoint_hi_inclusive + 1).unwrap_or_default();

        self.first_checkpoint_from_watermark = expected_first_checkpoint.min(self.first_checkpoint_from_watermark);
...
mod tests {
    ...
    async fn test_first_checkpoint_from_watermark() {
        ...
        conn.set_committer_watermark("test_processor", CommitterWatermark {
        ...
        let indexer_args =
            IndexerArgs { first_checkpoint: Some(50), last_checkpoint: None, pipeline: vec![], skip_watermark: false };
        let temp_dir = tempfile::tempdir().unwrap();
        let client_args = ClientArgs { local_ingestion_path: Some(temp_dir.path().to_owned()), ..Default::default() };
        ...
        indexer.concurrent_pipeline::<MockHandler>(MockHandler, ConcurrentConfig::default()).await.unwrap();
    ...
    async fn test_indexer_concurrent_pipeline_disallow_inconsistent_first_checkpoint() {
        ...
        let indexer_args =
            IndexerArgs { first_checkpoint: Some(1001), last_checkpoint: None, pipeline: vec![], skip_watermark: false };
        let temp_dir = tempfile::tempdir().unwrap();
        let client_args = ClientArgs { local_ingestion_path: Some(temp_dir.path().to_owned()), ..Default::default() };
        ...
        let result = indexer.concurrent_pipeline::<MockHandler>(MockHandler, ConcurrentConfig::default()).await;
    ...
    async fn test_indexer_concurrent_pipeline_allow_inconsistent_first_checkpoint_with_skip_watermark() {
        ...
        let indexer_args =
            IndexerArgs { first_checkpoint: Some(1001), last_checkpoint: None, pipeline: vec![], skip_watermark: true };
        let temp_dir = tempfile::tempdir().unwrap();
        let client_args = ClientArgs { local_ingestion_path: Some(temp_dir.path().to_owned()), ..Default::default() };
    ...
    async fn test_indexer_sequential_pipeline_disallow_inconsistent_first_checkpoint() {
        ...
        let result = indexer.sequential_pipeline::<MockHandler>(MockHandler, SequentialConfig::default()).await;
    ...
    async fn test_indexer_sequential_pipeline_always_resume_from_watermark() {
        ...
        let _ = indexer.sequential_pipeline::<MockHandler>(MockHandler, SequentialConfig::default()).await;

        let metrics = indexer.metrics().clone();

        indexer.run().await.unwrap().await.unwrap();

        assert_eq!(metrics.total_ingested_checkpoints.get(), num_ingested_checkpoints);
        assert_eq!(
            metrics.total_watermarks_out_of_order.get_metric_with_label_values(&["test_processor"]).unwrap().get(),
    ...
    async fn test_indexer_concurrent_pipeline_always_resume_from_first_checkpoint() {
        ...
        let _ = indexer.concurrent_pipeline::<MockHandler>(MockHandler, ConcurrentConfig::default()).await;
