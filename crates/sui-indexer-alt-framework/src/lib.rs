...
impl Indexer {
    ...
    pub async fn new(
        ...
        let db = Db::for_write(db_args).await.context("Failed to connect to database")?;

        // At indexer initialization, we ensure that the DB schema is up-to-date.
        db.run_migrations(Self::migrations(migrations)).await.context("Failed to run pending migrations")?;

        let metrics = IndexerMetrics::new(registry);
        registry.register(Box::new(DbConnectionStatsCollector::new(Some("indexer_db"), db.clone())))?;
    ...
    pub async fn new_for_testing(migrations: &'static EmbeddedMigrations) -> (Self, TempDb) {
        ...
        let indexer = Indexer::new(
            db_args,
            IndexerArgs::default(),
            ClientArgs { remote_store_url: None, local_ingestion_path: Some(tempdir().unwrap().into_path()) },
    ...
    async fn add_pipeline<P: Processor + 'static>(
        ...
        let expected_first_checkpoint = if start_from_pruner_watermark {
            ...
            watermark.as_ref().map(|w| w.checkpoint_hi_inclusive as u64 + 1).unwrap_or_default()
        };
...
mod tests {
    ...
    async fn test_add_new_pipeline() {
        ...
        indexer.concurrent_pipeline(ConcurrentPipeline1, ConcurrentConfig::default()).await.unwrap();
    ...
    async fn test_add_existing_pipeline() {
        ...
        watermark.update(&mut indexer.db().connect().await.unwrap()).await.unwrap();
        indexer.concurrent_pipeline(ConcurrentPipeline1, ConcurrentConfig::default()).await.unwrap();
    ...
    async fn test_add_multiple_pipelines() {
        ...
        watermark1.update(&mut indexer.db().connect().await.unwrap()).await.unwrap();
        let watermark2 = CommitterWatermark::new_for_testing(ConcurrentPipeline2::NAME, 20);
        watermark2.update(&mut indexer.db().connect().await.unwrap()).await.unwrap();

        indexer.concurrent_pipeline(ConcurrentPipeline2, ConcurrentConfig::default()).await.unwrap();
    ...
    async fn test_add_multiple_pipelines_pruning_requires_processed_values() {
        ...
        watermark3.update(&mut indexer.db().connect().await.unwrap()).await.unwrap();
        let pruner_watermark = PrunerWatermark::new_for_testing(ConcurrentPipeline3::NAME, 5);
        assert!(pruner_watermark.update(&mut indexer.db().connect().await.unwrap()).await.unwrap());
        indexer.concurrent_pipeline(ConcurrentPipeline3, ConcurrentConfig::default()).await.unwrap();
