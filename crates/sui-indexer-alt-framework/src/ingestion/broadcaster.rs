...
use super::{ingestion_client::IngestionClient, IngestionConfig};
use crate::{
    ...
    S: CheckpointStreamingClient,
{
    ...
    let mut handle_streaming_fallback = |reason: &str| {
        ...
        warn!(checkpoint_hi, ingestion_end, "{reason}, falling back to ingestion");
        metrics.total_streaming_connection_failures.inc();
        *streaming_backoff_batch_size = (backoff_batch_size * 2).min(config.streaming_backoff_max_batch_size as u64);
    ...
    *streaming_backoff_batch_size = config.streaming_backoff_initial_batch_size as u64;
    ...
    if network_latest_cp > checkpoint_hi + config.checkpoint_buffer_size as u64 {
        info!(network_latest_cp, checkpoint_hi, "Outside buffer size, delaying streaming start");
        return (noop_streaming_task(ingestion_end), ingestion_end);
    }

    info!(network_latest_cp, checkpoint_hi, "Within buffer size, starting streaming");
...
mod tests {
    ...
    use crate::{
        ingestion::{
            ingestion_client::FetchData,
            streaming_client::test_utils::MockStreamingClient,
    ...
    async fn recv_set(rx: &mut mpsc::Receiver<Arc<Checkpoint>>, count: usize) -> BTreeSet<u64> {
        ...
        for _ in 0 .. count {
            ...
            assert!(inserted, "Received duplicate checkpoint {}", checkpoint.summary.sequence_number());
        }
        result
    }
    ...
    async fn finite_list_of_checkpoints() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 5).await, BTreeSet::from_iter(0 .. 5));
    ...
    async fn halted() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 4).await, BTreeSet::from_iter(0 .. 4));
    ...
    async fn resumption() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 2).await, BTreeSet::from_iter(0 .. 2));

        // Regulator stopped because of watermark, but resumes when that watermark is updated.
        expect_timeout(&mut subscriber_rx).await;
        hi_tx.send(("test", 4)).unwrap();

        assert_eq!(recv_set(&mut subscriber_rx, 2).await, BTreeSet::from_iter(2 .. 4));
    ...
    async fn multiple_physical_subscribers() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx1, 3).await, BTreeSet::from_iter(0 .. 3));
        assert_eq!(recv_set(&mut subscriber_rx2, 3).await, BTreeSet::from_iter(0 .. 3));
    ...
    async fn start_from_non_zero() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 5).await, BTreeSet::from_iter(1000 .. 1005));
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 5).await, BTreeSet::from_iter(1005 .. 1010));
    ...
    async fn streaming_with_transition() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 60).await, BTreeSet::from_iter(0 .. 60));
    ...
    async fn streaming_beyond_end_checkpoint() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 30).await, BTreeSet::from_iter(0 .. 30));
    ...
    async fn streaming_before_start_checkpoint() {
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 70).await, Vec::from_iter(30 .. 100));
    ...
    async fn streaming_behind_watermark_skips_duplicates() {
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 20).await, Vec::from_iter(0 .. 20));
    ...
    async fn streaming_ahead_of_watermark_recovery() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 7).await, BTreeSet::from_iter(3 .. 10));
    ...
    async fn streaming_with_backpressure() {
        ...
        let config = IngestionConfig { checkpoint_buffer_size: 5, ..test_config() };
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 10).await, Vec::from_iter(0 .. 10));
        assert_eq!(metrics.latest_streamed_checkpoint.get(), 9);
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 10).await, Vec::from_iter(10 .. 20));
        assert_eq!(metrics.latest_streamed_checkpoint.get(), 19);
    ...
    async fn streaming_error_during_streaming() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 10).await, BTreeSet::from_iter(5 .. 15));
    ...
    async fn streaming_start_failure_fallback_to_ingestion() {
        ...
        let metrics = test_ingestion_metrics();
        let config = IngestionConfig { streaming_backoff_initial_batch_size: 5, ..test_config() };
        let h_broadcaster = broadcaster(
            0 .. 20,
            None,
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 15).await, Vec::from_iter(5 .. 20));
    ...
    async fn streaming_connection_retry_with_backoff() {
        ...
        assert_eq!(recv_set(&mut subscriber_rx, 50).await, BTreeSet::from_iter(0 .. 50));
    ...
    async fn streaming_connection_failure_backoff_reset() {
        ...
        assert_eq!(recv_vec(&mut subscriber_rx, 10).await, Vec::from_iter(30 .. 40));

        // Then fallback to ingestion for the next 2 checkpoints, since backoff should have reset
        assert_eq!(recv_set(&mut subscriber_rx, 2).await, BTreeSet::from_iter(40 .. 42));

        // Finally stream the last 8 checkpoints
        assert_eq!(recv_vec(&mut subscriber_rx, 8).await, Vec::from_iter(42 .. 50));
