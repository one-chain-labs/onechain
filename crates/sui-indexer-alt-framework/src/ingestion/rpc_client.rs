...
impl IngestionClientTrait for RpcClient {
    async fn fetch(&self, checkpoint: u64) -> FetchResult {
        let data = self.get_full_checkpoint(checkpoint).await.map_err(|status| match status.code() {
            Code::NotFound => FetchError::NotFound,
            _ => FetchError::Transient { reason: "get_full_checkpoint", error: anyhow!(status) },
        })?;
