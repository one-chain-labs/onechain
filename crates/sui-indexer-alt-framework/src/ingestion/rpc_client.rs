...
impl IngestionClientTrait for RpcClient {
    async fn fetch(&self, checkpoint: u64) -> FetchResult {
        let request: GetCheckpointRequest =
            GetCheckpointRequest::by_sequence_number(checkpoint).with_read_mask(FieldMask::from_paths([
                "summary.bcs",
                "signature",
                "contents.bcs",
        ...
        let response = self
            ...
            .map_err(|status| match status.code() {
                ...
                _ => FetchError::Transient { reason: "get_checkpoint", error: anyhow!(status) },
            })?
            .into_inner();

        let checkpoint = Checkpoint::try_from(response.checkpoint())
            .map_err(|e| FetchError::Permanent { reason: "proto_conversion", error: e.into() })?;
