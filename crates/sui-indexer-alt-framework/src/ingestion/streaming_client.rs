...
impl CheckpointStreamingClient for GrpcStreamingClient {
    async fn connect(&mut self) -> Result<CheckpointStream> {
        ...
        let stream = client.subscribe_checkpoints(request).await.map_err(Error::RpcClientError)?.into_inner();
...
pub mod test_utils {
    ...
    impl MockStreamingClient {
        ...
        pub fn insert_error(&mut self) {
            self.checkpoints.lock().unwrap().push(Err(Error::StreamingError(anyhow::anyhow!("Mock streaming error"))));
        }
    ...
    impl CheckpointStreamingClient for MockStreamingClient {
        async fn connect(&mut self) -> Result<CheckpointStream> {
            if self.connection_failures_remaining > 0 {
                ...
                return Err(Error::StreamingError(anyhow::anyhow!("Mock connection failure")));
            }
            let stream = MockStreamState { checkpoints: Arc::clone(&self.checkpoints) };
