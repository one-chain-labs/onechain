...
use sui_rpc::{client::HeadersInterceptor, Client};
...
use crate::{
    ...
    metrics::{CheckpointLagMetricReporter, IngestionMetrics},
    task::with_slow_future_monitor,
    types::full_checkpoint_content::{Checkpoint, CheckpointData},
};
...
impl IngestionClient {
    ...
    pub fn new(args: IngestionClientArgs, metrics: Arc<IngestionMetrics>) -> IngestionResult<Self> {
        ...
        let client = if let Some(url) = args.remote_store_url.as_ref() {
            ...
            IngestionClient::new_rpc(rpc_api_url.clone(), args.rpc_username, args.rpc_password, metrics.clone())?
        } else {
    ...
    pub fn new_rpc(
        ...
        let client = if let Some(username) = username {
            ...
            Client::new(url.to_string())?.with_max_decoding_message_size(MAX_GRPC_MESSAGE_SIZE_BYTES)
        };
        Ok(Self::new_impl(Arc::new(client), metrics))
    }

    pub(crate) fn new_impl(client: Arc<dyn IngestionClientTrait>, metrics: Arc<IngestionMetrics>) -> Self {
    ...
    pub async fn wait_for(&self, checkpoint: u64, retry_interval: Duration) -> IngestionResult<Arc<Checkpoint>> {
            ...
            async move {
                ...
                .map_err(|err| match err {
                    ...
                    FetchError::Permanent { reason, error } => {
                        ...
                        self.metrics.total_ingested_permanent_errors.with_label_values(&[reason]).inc();
...
mod tests {
    ...
    impl IngestionClientTrait for MockIngestionClient {
        async fn fetch(&self, checkpoint: u64) -> FetchResult {
            ...
            self.checkpoints.get(&checkpoint).as_deref().cloned().ok_or(FetchError::NotFound)
        }
    }
    ...
    async fn test_fetch_checkpoint_success() {
        ...
        mock.checkpoints.insert(1, FetchData::Checkpoint(checkpoint.into()));
    ...
    async fn test_fetch_transient_error_with_retry() {
        ...
        mock.checkpoints.insert(1, FetchData::Checkpoint(checkpoint.clone().into()));
        ...
        let retries = client.metrics.total_ingested_transient_retries.with_label_values(&["mock_transient_error"]).get();
        assert_eq!(retries, 2);
    }
    ...
    async fn test_wait_for_permanent_deserialization_error() {
        ...
        mock.checkpoints.insert(1, FetchData::Raw(Bytes::from("invalid data")));

        // wait_for should keep retrying on deserialization errors and timeout
        timeout(Duration::from_secs(1), client.wait_for(1, Duration::from_millis(50))).await.unwrap_err();
    }
    ...
    async fn test_fetch_non_retryable_error() {
        ...
        let errors = client.metrics.total_ingested_permanent_errors.with_label_values(&["mock_permanent_error"]).get();
        assert_eq!(errors, 1);
    }
}
