...
use crate::{
    ingestion::{ingestion_client::IngestionClientArgs, ClientArgs, IngestionConfig},
    Indexer,
    IndexerArgs,
};
...
impl Indexer<Db> {
    ...
    pub async fn new_from_pg(
        ...
        Indexer::new(store, indexer_args, client_args, ingestion_config, metrics_prefix, registry, cancel).await
    }
    ...
    macro_rules! define_test_concurrent_pipeline {
        ($name:ident) => {
            ...
            impl handler::Handler for $name {
                async fn commit<'a>(_values: &[Self::Value], _conn: &mut Connection<'a>) -> anyhow::Result<usize> {
                    todo!()
                }
            }
        };
