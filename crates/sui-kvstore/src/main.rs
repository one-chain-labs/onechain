...
use sui_types::{base_types::ObjectID, digests::TransactionDigest, storage::ObjectKey};
...
async fn main() -> Result<()> {
    ...
    match app.command {
        Some(Command::Ingestion { network }) => {
            let client = BigTableClient::new_remote(app.instance_id, false, None, "ingestion".to_string(), None).await?;
        ...
        Some(Command::Fetch { entry }) => {
            let mut client = BigTableClient::new_remote(app.instance_id, true, None, "cli".to_string(), None).await?;
            let result = match entry {
                Entry::Epoch { id } => client.get_epoch(id).await?.map(|e| bcs::to_bytes(&e)),
                Entry::Object { id, version } => {
                    let objects = client.get_objects(&[ObjectKey(ObjectID::from_str(&id)?, version.into())]).await?;
                ...
                Entry::Transaction { id } => {
                    let transactions = client.get_transactions(&[TransactionDigest::from_str(&id)?]).await?;
