...
async fn main() -> Result<()> {
    ...
    match app.command {
        Some(Command::Ingestion { network }) => {
            let client =
                BigTableClient::new_remote(app.instance_id, false, None, "ingestion".to_string(), None, None).await?;
        ...
        Some(Command::Fetch { entry }) => {
            let mut client =
                BigTableClient::new_remote(app.instance_id, true, None, "cli".to_string(), None, None).await?;
