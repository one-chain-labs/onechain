...
impl KeyValueStoreWriter for BigTableClient {
    async fn save_objects(&mut self, objects: &[&Object], timestamp_ms: TimestampMs) -> Result<()> {
        ...
        self.multi_set(OBJECTS_TABLE, items, Some(timestamp_ms)).await
    }
    ...
    async fn save_transactions(&mut self, transactions: &[TransactionData]) -> Result<()> {
        ...
        self.multi_set(TRANSACTIONS_TABLE, items, timestamp_ms).await
    }
    ...
    async fn save_checkpoint(&mut self, checkpoint: &CheckpointData) -> Result<()> {
        ...
        self.multi_set(CHECKPOINTS_TABLE, [(key.clone(), cells)], Some(timestamp)).await?;
        self.multi_set(
            CHECKPOINTS_BY_DIGEST_TABLE,
            [(checkpoint.checkpoint_summary.digest().inner().to_vec(), vec![(DEFAULT_COLUMN_QUALIFIER, key)])],
            Some(timestamp),
        )
        .await
    }
    ...
    async fn save_watermark(&mut self, watermark: CheckpointSequenceNumber) -> Result<()> {
        ...
        self.multi_set(
            WATERMARK_ALT_TABLE,
            [(vec![0], vec![(DEFAULT_COLUMN_QUALIFIER, watermark_bytes.clone())])],
            Some(watermark),
        )
        .await?;
        self.multi_set(WATERMARK_TABLE, [(watermark_bytes, vec![(DEFAULT_COLUMN_QUALIFIER, vec![])])], None).await
    }
    ...
    async fn save_epoch(&mut self, epoch: EpochInfo) -> Result<()> {
        ...
        self.multi_set(
            EPOCHS_TABLE,
            [(key, vec![(DEFAULT_COLUMN_QUALIFIER, bcs::to_bytes(&epoch)?)])],
...
impl KeyValueStoreReader for BigTableClient {
    ...
    async fn get_checkpoint_by_digest(&mut self, digest: CheckpointDigest) -> Result<Option<Checkpoint>> {
        ...
        let mut response = self.multi_get(CHECKPOINTS_BY_DIGEST_TABLE, vec![key], None).await?;
    ...
    async fn get_latest_checkpoint(&mut self) -> Result<CheckpointSequenceNumber> {
        match self.multi_get(WATERMARK_ALT_TABLE, vec![vec![0]], None).await?.pop().and_then(|(_, mut row)| row.pop()) {
    ...
    async fn get_epoch(&mut self, epoch_id: EpochId) -> Result<Option<EpochInfo>> {
        ...
        Ok(match self.multi_get(EPOCHS_TABLE, vec![key], None).await?.pop() {
            Some((_, mut row)) => row.pop().map(|value| bcs::from_bytes(&value.1)).transpose()?,
            None => None,
        })
    }
    ...
    async fn get_latest_epoch(&mut self) -> Result<Option<EpochInfo>> {
        ...
        Ok(match self.reversed_scan(EPOCHS_TABLE, upper_limit).await?.pop() {
            Some((_, mut row)) => row.pop().map(|value| bcs::from_bytes(&value.1)).transpose()?,
            None => None,
        })
    }
    ...
    async fn get_events_for_transactions(
        ...
        let query = self.multi_get(
            TRANSACTIONS_TABLE,
            transaction_digests.iter().map(|tx| tx.inner().to_vec()).collect(),
            Some(RowFilter {
                filter: Some(Filter::ColumnQualifierRegexFilter(
        ...
        for (key, row) in query.await? {
            ...
            let events = transaction_events.context("events field is missing")?.map(|e| e.data).unwrap_or_default();

            let transaction_digest =
                TransactionDigest::try_from(key).context("Failed to deserialize transaction digest")?;

            results.push((transaction_digest, TransactionEventsData { events, timestamp_ms }));
        }
...
impl BigTableClient {
    ...
    fn report_bt_stats(&self, request_stats: &Option<RequestStats>, table_name: &str) {
        ...
        if let Some(StatsView::FullReadStatsView(view)) = request_stats.as_ref().and_then(|r| r.stats_view.as_ref()) {
            if let Some(latency) = view.request_latency_stats.as_ref().and_then(|s| s.frontend_server_latency) {
                if latency.seconds < 0 || latency.nanos < 0 {
                    return;
                }
                let duration = Duration::new(latency.seconds as u64, latency.nanos as u32);
                metrics.kv_bt_chunk_latency_ms.with_label_values(&labels).observe(duration.as_millis() as f64);
            }
    ...
    async fn multi_set(
        ...
        for chunk in values.into_iter().collect::<Vec<_>>().chunks(50_000) {
            self.multi_set_internal(table_name, chunk.iter().cloned(), timestamp_ms).await?;
        }
        Ok(())
    }
    ...
    async fn multi_set_internal(
        ...
        for (row_key, cells) in values {
            let mutations = cells
                ...
                .collect();
            entries.push(Entry { row_key, mutations, idempotency: None });
        }
        ...
        while let Some(part) = response.message().await? {
            for entry in part.entries {
                if let Some(status) = entry.status
                    && status.code != 0
                {
                    return Err(anyhow!("bigtable write failed {} {}", status.code, status.message));
                }
            }
        }
        Ok(())
    ...
    pub async fn multi_get(
        ...
        if num_keys_requested > rows.len() {
            metrics.kv_get_not_found.with_label_values(&labels).inc_by((num_keys_requested - rows.len()) as u64);
        }
        ...
        if num_keys_requested > 0 {
            metrics.kv_get_latency_ms_per_key.with_label_values(&labels).observe(elapsed_ms / num_keys_requested as f64);
        }
    ...
    pub async fn multi_get_internal(
        ...
        let filter = Some(match filter {
            Some(filter) => RowFilter { filter: Some(Filter::Chain(Chain { filters: vec![filter, version_filter] })) },
            None => version_filter,
        });
