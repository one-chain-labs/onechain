...
impl DbConnectionStatsCollector {
    pub fn new(prefix: Option<&str>, db: Db) -> Self {
        ...
        let desc = vec![
            (MetricType::GAUGE, desc(name("connections"), "Number of connections currently being managed by the pool")),
            (MetricType::GAUGE, desc(name("idle_connections"), "Number of idle connections in the pool")),
            (MetricType::COUNTER, desc(name("connect_direct"), "Connections that did not have to wait")),
            (MetricType::SUMMARY, desc(name("connect_waited"), "Connections that had to wait")),
            (
                MetricType::COUNTER,
                desc(name("connect_timed_out"), "Connections that timed out waiting for a connection"),
            ),
            (MetricType::COUNTER, desc(name("connections_created"), "Connections that have been created in the pool")),
            (
                MetricType::COUNTER,
                desc_with_labels(name("connections_closed"), "Total connections that were closed", &["reason"]),
            ),
        ];
...
fn desc_with_labels(name: String, help: &str, labels: &[&str]) -> Desc {
    Desc::new(name, help.to_string(), labels.iter().map(|s| s.to_string()).collect(), Default::default())
