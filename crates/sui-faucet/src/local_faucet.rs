...
use sui_sdk::{
    rpc_types::{SuiTransactionBlockEffectsAPI, SuiTransactionBlockResponse, SuiTransactionBlockResponseOptions},
    types::{
...
use crate::{CoinInfo, FaucetConfig, FaucetError};
...
impl LocalFaucet {
    pub async fn new(mut wallet: WalletContext, config: FaucetConfig) -> Result<Arc<Self>, FaucetError> {
    ...
    pub async fn local_request_execute_tx(&self, recipient: SuiAddress) -> Result<Vec<CoinInfo>, FaucetError> {
        let gas_price = self
            .wallet
        ...
        ptb.pay_oct(recipients, amounts).map_err(FaucetError::internal)?;
        ...
        let tx_data =
            TransactionData::new_programmable(self.active_address, vec![coin_id_ref], ptb, GAS_BUDGET, gas_price);

        let tx = self.execute_txn_with_retries(tx_data, *coin_id, NUM_RETRIES).await.map_err(FaucetError::internal)?;

        let Some(ref effects) = tx.effects else {
            return Err(FaucetError::internal("Failed to get coin id from response".to_string()));
        };
...
async fn find_gas_coins_and_address(
    ...
    for address in std::iter::once(active_address).chain(wallet.get_addresses().into_iter()) {
        let coins: Vec<_> = wallet
            ...
            .iter()
            .filter_map(|(balance, obj)| if *balance >= config.amount { GasCoin::try_from(obj).ok() } else { None })
            .collect();
...
mod tests {
    ...
    async fn test_local_faucet_execute_txn() {
        ...
        let coins = client.coin_read_api().get_coins(recipient, None, None, None).await.unwrap();
