...
static FAUCET_WEB_APP_URL: Lazy<String> =
    Lazy::new(|| std::env::var("FAUCET_WEB_APP_URL").ok().unwrap_or_else(|| DEFAULT_FAUCET_WEB_APP_URL.to_string()));

static CLOUDFLARE_TURNSTILE_URL: Lazy<Option<String>> = Lazy::new(|| std::env::var("CLOUDFLARE_TURNSTILE_URL").ok());

static TURNSTILE_SECRET_KEY: Lazy<Option<String>> = Lazy::new(|| std::env::var("TURNSTILE_SECRET_KEY").ok());

static DISCORD_BOT_PWD: Lazy<String> = Lazy::new(|| std::env::var("DISCORD_BOT_PWD").unwrap_or_else(|_| "".to_string()));
...
impl RequestsManager {
    ...
    async fn validate_turnstile_token(&self, addr: SocketAddr, token: &str) -> Result<(), (StatusCode, FaucetError)> {
        let ip = addr.ip();
        let req = reqwest::Client::new();
        let params =
            [("secret", self.turnstile_secret_key.as_str()), ("response", token), ("remoteip", &ip.to_string())];

        // Make the POST request
        let resp = match req.post(&self.cloudflare_turnstile_url).form(&params).send().await {
            Ok(resp) => resp,
            Err(e) => {
                error!("Cloudflare turnstile request failed: {:?}", e);
                return Err((StatusCode::INTERNAL_SERVER_ERROR, FaucetError::Internal(e.to_string())));
            }
        };

        // Check if the request was successful.
        if !resp.status().is_success() {
            return Err((StatusCode::INTERNAL_SERVER_ERROR, FaucetError::Internal("Verification failed".to_string())));
        }
        ...
        match self.data.entry(ip) {
            Entry::Vacant(entry) => {
                entry.insert(RequestInfo { timestamp: Instant::now(), requests_used: 1 });
            }
    ...
    fn cleanup_expired_tokens(&self) {
        ...
        self.data.retain(|_, info| info.timestamp.elapsed() < self.reset_time_interval);
    }
}
...
pub async fn start_faucet(
    ...
    let (cloudflare_turnstile_url, turnstile_secret_key) = if app_state.config.authenticated {
        ...
        (CLOUDFLARE_TURNSTILE_URL.as_ref().unwrap().to_string(), TURNSTILE_SECRET_KEY.as_ref().unwrap().to_string())
    } else {
        ("".to_string(), "".to_string())
    };

    // TODO: restrict access if needed
    let cors = CorsLayer::new().allow_methods(vec![Method::GET, Method::POST]).allow_headers(Any).allow_origin(Any);
    ...
    let global_limited_routes = Router::new()
        ...
        .layer(GovernorLayer { config: governor_cfg.clone() });
    ...
    // Combine all routes
    let app = Router::new().merge(global_limited_routes).merge(unrestricted_routes).merge(faucet_web_routes).layer(
        ServiceBuilder::new()
...
async fn batch_faucet_discord(
    ...
    if state.config.authenticated {
        let Some(agent_value) = headers.get(reqwest::header::USER_AGENT).and_then(|v| v.to_str().ok()) else {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let FaucetRequest::FixedAmountRequest(request) = payload else {
        return (
            StatusCode::BAD_REQUEST,
            Json(BatchFaucetResponse::from(FaucetError::Internal("Input Error.".to_string()))),
        );
    };
...
async fn batch_faucet_web_gas(
    ...
    if state.config.authenticated {
        let Some(token) = headers.get("X-Turnstile-Token").and_then(|v| v.to_str().ok()) else {
            return (StatusCode::BAD_REQUEST, Json(BatchFaucetResponse::from(FaucetError::MissingTurnstileTokenHeader)));
        };
...
async fn batch_request_spawn_task(
    ...
    let result = spawn_monitored_task!(async move {
        state
            .faucet
            .batch_send(Uuid::new_v4(), request.recipient, &vec![state.config.amount; state.config.num_coins])
            .await
    })
    .await
    .unwrap();
    match result {
        ...
        Err(v) => (StatusCode::INTERNAL_SERVER_ERROR, Json(BatchFaucetResponse::from(v))),
    }
}
...
async fn batch_request_gas(
    ...
    if state.config.batch_enabled {
        ...
        let result = spawn_monitored_task!(async move {
            state.faucet.send(id, request.recipient, &vec![state.config.amount; state.config.num_coins]).await
        })
        .await
        .unwrap();
        ...
        match result {
            ...
            Err(v) => {
                ...
                (StatusCode::INTERNAL_SERVER_ERROR, Json(BatchFaucetResponse::from(v)))
            }
        }
    }
}
...
async fn request_status(Extension(state): Extension<Arc<AppState>>, Path(id): Path<String>) -> impl IntoResponse {
    match Uuid::parse_str(&id) {
        Ok(task_id) => {
            let result = state.faucet.get_batch_send_status(task_id).await;
            match result {
                Ok(v) => (StatusCode::CREATED, Json(BatchStatusFaucetResponse::from(v))),
                Err(v) => (StatusCode::INTERNAL_SERVER_ERROR, Json(BatchStatusFaucetResponse::from(v))),
            }
        }
        Err(e) => (StatusCode::BAD_REQUEST, Json(BatchStatusFaucetResponse::from(FaucetError::Internal(e.to_string())))),
    }
}
...
async fn request_gas(
    ...
    let result = match payload {
        FaucetRequest::FixedAmountRequest(requests) => {
            ...
            spawn_monitored_task!(async move {
                state.faucet.send(id, requests.recipient, &vec![state.config.amount; state.config.num_coins]).await
            })
            .await
            .unwrap()
        }
        _ => {
            return (
                StatusCode::BAD_REQUEST,
                Json(FaucetResponse::from(FaucetError::Internal("Input Error.".to_string()))),
            )
        }
    };
    match result {
        ...
        Err(v) => {
            warn!(uuid =?id, "Failed to request gas: {:?}", v);
            (StatusCode::INTERNAL_SERVER_ERROR, Json(FaucetResponse::from(v)))
        }
    }
}

pub fn create_wallet_context(timeout_secs: u64, config_dir: PathBuf) -> Result<WalletContext, anyhow::Error> {
    let wallet_conf = config_dir.join(SUI_CLIENT_CONFIG);
    info!("Initialize wallet from config path: {:?}", wallet_conf);
    WalletContext::new(&wallet_conf, Some(Duration::from_secs(timeout_secs)), Some(1000))
}

async fn handle_error(error: BoxError) -> impl IntoResponse {
    if error.is::<tower::load_shed::error::Overloaded>() {
        return (StatusCode::SERVICE_UNAVAILABLE, Cow::from("service is overloaded, please try again later"));
    }
...
mod tests {
    ...
    async fn setup_mock_cloudflare() -> MockServer {
        ...
        Mock::given(method("POST"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({ "success": true, "error-codes": [] })))
            .mount(&mock_server)
            .await;
    ...
    async fn test_token_validation_and_limits() {
        ...
        let manager =
            RequestsManager::new(MAX_REQUESTS_PER_IP, RESET_TIME_INTERVAL, mock_server.uri(), "test_secret".to_string());
    ...
    async fn test_concurrent_ip_requests() {
        ...
        let ips: Vec<SocketAddr> =
            (0 .. 10).map(|i| SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, i as u8)), 8080)).collect();
        ...
        for result in all_results {
            ...
            for (idx, _) in results.iter().enumerate().take(MAX_REQUESTS_PER_IP as usize) {
                assert!(results[idx].is_ok(), "Request {} for IP {} should succeed", idx, ip);
            }
