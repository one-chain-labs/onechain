...
use crate::{types::*, AppState, FaucetConfig, FaucetError, FaucetRequest};
...
async fn request_local_gas(
    ...
    let request = state.faucet.local_request_execute_tx(request.recipient).await;

    if let Err(e) = request {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(FaucetResponse { status: RequestStatus::Failure(e), coins_sent: None }),
        );
    }
    ...
    (StatusCode::OK, Json(FaucetResponse { status: RequestStatus::Success, coins_sent: Some(coins) }))
}
...
pub fn create_wallet_context(timeout_secs: u64, config_dir: PathBuf) -> Result<WalletContext, anyhow::Error> {
    ...
    WalletContext::new(&wallet_conf)
        .map(|ctx| ctx.with_request_timeout(Duration::from_secs(timeout_secs)).with_max_concurrent_requests(1000))
}
...
mod tests {
    ...
    async fn test_v2_gas_endpoint() {
        ...
        let port = 9090;
        let config = FaucetConfig { host_ip: "127.0.0.1".parse().unwrap(), port, ..Default::default() };
        let local_faucet = LocalFaucet::new(cluster.wallet, config.clone()).await.unwrap();

        let app_state = Arc::new(AppState { faucet: local_faucet, config });

        // Spawn the faucet in a background task
        let handle = tokio::spawn(async move {
            start_faucet(app_state).await.expect("Failed to start faucet");
        });
        ...
        let response = client.post(format!("http://127.0.0.1:{port}/v2/gas",)).json(&req).send().await.unwrap();
