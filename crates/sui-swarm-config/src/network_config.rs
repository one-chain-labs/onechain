...
use sui_types::{committee::CommitteeWithNetworkMetadata, crypto::AccountKeyPair, multiaddr::Multiaddr};
...
impl NetworkConfig {
    ...
    pub fn net_addresses(&self) -> Vec<Multiaddr> {
        self.genesis.committee_with_network().validators().values().map(|(_, n)| n.network_address.clone()).collect()
    }
