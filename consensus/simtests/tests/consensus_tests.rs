...
mod consensus_tests {
    ...
    use consensus_core::{BlockAPI, BlockStatus, NoopTransactionVerifier, TransactionVerifier, ValidationError};
    ...
    fn test_config() -> SimConfig {
        env_config(uniform_latency_ms(10 .. 20), [
            ("regional_high_variance", bimodal_latency_ms(30 .. 40, 300 .. 800, 0.01)),
    ...
    async fn test_committee_start_simple() {
        ...
        let _handle = tokio::spawn(async move {
            ...
            for i in 0 .. NUM_TRANSACTIONS {
                ...
                transaction_clients_clone[i as usize % transaction_clients_clone.len()].submit(vec![txn]).await.unwrap();
            }
        });
    ...
    async fn test_committee_fast_path() {
        ...
        for (authority_index, _authority_info) in committee.authorities() {
            ...
            let config = Config {
                ...
                protocol_config: protocol_config.clone(),
                clock_drift: clock_drifts[authority_index.value() as usize],
                transaction_verifier: Arc::new(RandomizedTransactionVerifier::new(REJECTION_PROBABILITY)),
            };
            let node = AuthorityNode::new(config);
            node.start().await.unwrap();
            node.spawn_committed_subdag_consumer().unwrap();
        ...
        loop {
            ...
            let transaction_indexes_range = transaction_index .. transaction_index + num_of_transactions;
            ...
            let index = (transaction_index - num_of_transactions) as usize % transaction_clients.len();
            let (_block_ref, _indexes, status_waiter) = transaction_clients[index].submit(transactions).await.unwrap();
                ...
                async move {
                    match status_waiter.await {
                        Ok(BlockStatus::Sequenced(_)) => {
                            ...
                            total_sequenced_transactions.fetch_add(num_of_transactions as u64, Ordering::SeqCst);
                        }
                        Ok(BlockStatus::GarbageCollected(_)) => {
                            total_garbage_collected_transactions.fetch_add(num_of_transactions as u64, Ordering::SeqCst);
                        }
                        Err(e) => {
                            panic!("Transactions in range {:?} failed with error: {e}", transaction_indexes_range);
                        }
                    }
                }
            });
        ...
        loop {
            ...
            for authority_index in 0 .. NUM_OF_AUTHORITIES {
                ...
                if let Some(sub_dag) =
                    timeout(Duration::from_secs(90), commit_consumer_receivers[authority_index].recv())
                        .await
            ...
            tracing::info!("Received {} sub dags for commit {:?}", sub_dags.len(), last_sub_dag_commit_ref.unwrap());
            ...
            for sub_dag in sub_dags.iter().skip(1) {
                assert_eq!(first_sub_dag.rejected_transactions_by_block, sub_dag.rejected_transactions_by_block);
            }

            // Now pick the first sub dag and count all the transactions included in it.
            transaction_count += first_sub_dag.blocks.iter().map(|block| block.transactions().len()).sum::<usize>();
        ...
        tracing::info!("Total sequenced transactions: {}", total_sequenced_transactions.load(Ordering::SeqCst));
        tracing::info!(
            "Total garbage collected transactions: {}",
            total_garbage_collected_transactions.load(Ordering::SeqCst)
        );
        tracing::info!("Total rejected transactions: {}", total_rejected_transactions);
    }
