...
impl AuthorityNode {
    pub fn new(config: Config) -> Self {
        Self { inner: Default::default(), config, commit_consumer_receiver: Mutex::new(None) }
    }
    ...
    pub fn spawn_committed_subdag_consumer(&self) -> Result<()> {
        ...
        if let Some(inner) = inner.as_ref() {
            let (commit_sender, commit_receiver) = unbounded_channel("consensus_commit_output_simtests");
    ...
    pub fn commit_consumer_receiver(&self) -> UnboundedReceiver<CommittedSubDag> {
        let mut commit_consumer_receiver_lock = self.commit_consumer_receiver.lock();
        commit_consumer_receiver_lock.take().expect("No commit consumer receiver found")
    }
...
impl AuthorityNodeInner {
    ...
    pub async fn spawn(config: Config) -> Self {
        ...
        let Ok((consensus_authority, commit_receiver, commit_consumer_monitor)) = Arc::try_unwrap(init_tuple) else {
