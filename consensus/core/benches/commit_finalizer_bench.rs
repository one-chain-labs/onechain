...
impl BenchFixture {
    fn new(num_authorities: usize) -> BenchFixture {
        ...
        let (commit_sender, _commit_receiver) = monitored_mpsc::unbounded_channel("consensus_commit_output");
    ...
    fn populate_commits(
        ...
        self.transaction_certifier
            .add_voted_blocks(dag_builder.all_blocks().iter().map(|b| (b.clone(), vec![])).collect());

        let leaders =
            dag_builder.leader_blocks(1 ..= num_commits as u32).into_iter().map(Option::unwrap).collect::<Vec<_>>();
                    ...
                    async move {
                        ...
                        for sub_dag in fixture.workload_commits {
                            ...
                            for result in results {
                                ...
                                run_stats.rejected +=
                                    result.rejected_transactions_by_block.values().map(|txns| txns.len()).sum::<usize>();
                            }
                        }
...
criterion_group!(commit_finalizer_benches, process_commit_direct, process_commit_indirect);
