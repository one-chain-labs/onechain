        ...
        match block.round.cmp(&self.round) {
            ...
            Ordering::Equal => {
                ...
                if self.aggregator.add(block.author, &self.context.committee) {
                    ...
                    self.quorum_ts = now;
                    debug!("ThresholdClock advanced to round {} with block {} completing quorum", self.round, block);
                    return true;
                }
            ...
            Ordering::Greater => {
                ...
                debug!("ThresholdClock advanced to round {} with block {} catching up round", self.round, block);
                true
            }
        }
    }
...
mod tests {
    ...
    async fn test_threshold_clock_add_block_min_committee() {
        ...
        assert!(!aggregator.add_block(BlockRef::new(9, AuthorityIndex::new_for_test(0), BlockDigest::default(),)));
        assert_eq!(aggregator.get_round(), 10);

        // Adding one block is enough to complete the quorum.
        assert!(aggregator.add_block(BlockRef::new(10, AuthorityIndex::new_for_test(0), BlockDigest::default(),)));
        assert_eq!(aggregator.get_round(), 11);

        // Adding a catch up block should both advance the round and complete the quorum.
        aggregator.add_block(BlockRef::new(20, AuthorityIndex::new_for_test(0), BlockDigest::default()));
