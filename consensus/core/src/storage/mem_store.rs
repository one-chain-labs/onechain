...
struct Inner {
    ...
    finalized_commits: BTreeMap<(CommitIndex, CommitDigest), BTreeMap<BlockRef, Vec<TransactionIndex>>>,
}
...
impl Store for MemStore {
    fn write(&self, write_batch: WriteBatch) -> ConsensusResult<()> {
        ...
        for block in write_batch.blocks {
            ...
            inner.blocks.insert((block_ref.round, block_ref.author, block_ref.digest), block.clone());
            inner.digests_by_authorities.insert((block_ref.author, block_ref.round, block_ref.digest));
        ...
        for (commit_ref, rejected_transactions) in write_batch.finalized_commits {
            inner.finalized_commits.insert((commit_ref.index, commit_ref.digest), rejected_transactions);
        }
    ...
    fn read_blocks(&self, refs: &[BlockRef]) -> ConsensusResult<Vec<Option<VerifiedBlock>>> {
        ...
        let blocks = refs.iter().map(|r| inner.blocks.get(&(r.round, r.author, r.digest)).cloned()).collect();
        Ok(blocks)
    }
    ...
    fn scan_blocks_by_author(&self, author: AuthorityIndex, start_round: Round) -> ConsensusResult<Vec<VerifiedBlock>> {
        let inner = self.inner.read();
        let mut refs = vec![];
        for &(author, round, digest) in inner
            .digests_by_authorities
            .range((Included((author, start_round, BlockDigest::MIN)), Included((author, Round::MAX, BlockDigest::MAX))))
        {
    ...
    fn scan_commits(&self, range: CommitRange) -> ConsensusResult<Vec<TrustedCommit>> {
        ...
        for (_, commit) in inner
            .commits
            .range((Included((range.start(), CommitDigest::MIN)), Included((range.end(), CommitDigest::MAX))))
        {
    ...
    fn scan_finalized_commits(
        ...
        for (commit, rejected_transactions) in inner
            .finalized_commits
            .range((Included((range.start(), CommitDigest::MIN)), Included((range.end(), CommitDigest::MAX))))
        {
            finalized_commits.push((CommitRef::new(commit.0, commit.1), rejected_transactions.clone()));
        }
