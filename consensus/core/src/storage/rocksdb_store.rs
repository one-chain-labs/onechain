...
pub struct RocksDBStore {
    ...
    finalized_commits: DBMap<(CommitIndex, CommitDigest), BTreeMap<BlockRef, Vec<TransactionIndex>>>,
}
...
impl RocksDBStore {
    ...
    pub fn new(path: &str) -> Self {
        ...
        Self::open_tables_read_write(path.into(), metrics_conf, Some(db_options.options), Some(column_family_options))
    }

    #[cfg(tidehunter)]
    pub fn new(path: &str) -> Self {
        tracing::warn!("Consensus store using tidehunter");
        use typed_store::tidehunter_util::{default_mutex_count, KeyIndexing, KeySpaceConfig, KeyType, ThConfig};
        ...
        Self::open_tables_read_write(
            path.into(),
            MetricConf::new("consensus").with_sampling(SamplingInterval::new(Duration::from_secs(60), 0)),
...
impl Store for RocksDBStore {
    fn write(&self, write_batch: WriteBatch) -> ConsensusResult<()> {
        ...
        for commit in write_batch.commits {
            batch
                .insert_batch(&self.commits, [((commit.index(), commit.digest()), commit.serialized())])
                .map_err(ConsensusError::RocksDBFailure)?;
        }

        for (commit_ref, commit_info) in write_batch.commit_info {
            batch
                .insert_batch(&self.commit_info, [((commit_ref.index, commit_ref.digest), commit_info)])
                .map_err(ConsensusError::RocksDBFailure)?;
        }

        for (commit_ref, rejected_transactions) in write_batch.finalized_commits {
            batch
                .insert_batch(&self.finalized_commits, [((commit_ref.index, commit_ref.digest), rejected_transactions)])
                .map_err(ConsensusError::RocksDBFailure)?;
        }
    ...
    fn read_blocks(&self, refs: &[BlockRef]) -> ConsensusResult<Vec<Option<VerifiedBlock>>> {
        ...
        for (key, serialized) in refs.iter().zip(serialized) {
            if let Some(serialized) = serialized {
                let signed_block: SignedBlock = bcs::from_bytes(&serialized).map_err(ConsensusError::MalformedBlock)?;
    ...
    fn scan_blocks_by_author(&self, author: AuthorityIndex, start_round: Round) -> ConsensusResult<Vec<VerifiedBlock>> {
        let mut refs = vec![];
        ...
        for (r, block) in refs.into_iter().zip(results.into_iter()) {
            blocks.push(block.unwrap_or_else(|| panic!("Storage inconsistency: block {:?} not found!", r)));
        }
        Ok(blocks)
    }
    ...
    fn scan_commits(&self, range: CommitRange) -> ConsensusResult<Vec<TrustedCommit>> {
        ...
        for result in self
            .commits
            .safe_range_iter((Included((range.start(), CommitDigest::MIN)), Included((range.end(), CommitDigest::MAX))))
        {
            let ((_index, digest), serialized) = result?;
            let commit = TrustedCommit::new_trusted(
                bcs::from_bytes(&serialized).map_err(ConsensusError::MalformedCommit)?,
    ...
    fn read_last_finalized_commit(&self) -> ConsensusResult<Option<CommitRef>> {
        let Some(result) = self.finalized_commits.reversed_safe_iter_with_bounds(None, None)?.next() else {
            return Ok(None);
        };
    ...
    fn scan_finalized_commits(
        ...
        for result in self
            ...
            .safe_range_iter((Included((range.start(), CommitDigest::MIN)), Included((range.end(), CommitDigest::MAX))))
        {
            let ((index, digest), rejected_transactions) = result.map_err(ConsensusError::RocksDBFailure)?;
