...
impl RocksDBStore {
    ...
    pub(crate) fn new(path: &str) -> Self {
        ...
        let configs = vec![
            (
                Self::BLOCKS_CF.to_string(),
                ThConfig::new_with_indexing(index_index_digest_key.clone(), MUTEXES, u32_prefix.clone()),
            ),
            (
                Self::DIGESTS_BY_AUTHORITIES_CF.to_string(),
                ThConfig::new_with_indexing(index_index_digest_key.clone(), MUTEXES, u64_prefix.clone()),
            ),
            (
                Self::COMMITS_CF.to_string(),
                ThConfig::new_with_indexing(index_digest_key.clone(), MUTEXES, u32_prefix.clone()),
            ),
            (Self::COMMIT_VOTES_CF.to_string(), ThConfig::new_with_indexing(commit_vote_key, 1024, u32_prefix.clone())),
            (
