...
impl CommitConsumer {
    pub fn new(
        ...
        (Self { commit_sender, block_sender, last_processed_commit_index, monitor }, commit_receiver, block_receiver)
    }
...
impl CommitConsumerMonitor {
    ...
    pub fn set_highest_observed_commit_at_startup(&self, highest_observed_commit_at_startup: CommitIndex) {
        ...
        assert!(*commit == 0, "highest_known_commit_at_startup can only be set once");
        *commit = highest_observed_commit_at_startup;

        info!("Highest observed commit at startup set to {}", highest_observed_commit_at_startup);
    }
