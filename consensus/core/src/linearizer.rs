...
impl Linearizer {
    pub(crate) fn new(
        ...
        Self { dag_state, leader_schedule, context }
    }
    ...
    fn collect_sub_dag_and_commit(
        ...
        let (to_commit, rejected_transactions) =
            Self::linearize_sub_dag(&self.context, leader_block.clone(), last_committed_rounds, &mut dag_state);
...
mod tests {
    ...
    async fn test_handle_commit_with_schedule_update() {
        ...
        let mut linearizer = Linearizer::new(context.clone(), dag_state.clone(), leader_schedule.clone());
    ...
    async fn test_handle_already_committed() {
        ...
        context.protocol_config.set_consensus_linearize_subdag_v2_for_testing(false);
