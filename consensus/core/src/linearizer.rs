...
impl BlockStoreAPI for parking_lot::lock_api::RwLockWriteGuard<'_, parking_lot::RawRwLock, DagState> {
    fn get_blocks(&self, refs: &[BlockRef]) -> Vec<Option<VerifiedBlock>> {
...
impl Linearizer {
    ...
    fn collect_sub_dag_and_commit(&mut self, leader_block: VerifiedBlock) -> (CommittedSubDag, TrustedCommit) {
        let _s = self
            .context
            .metrics
            .node_metrics
        ...
        let timestamp_ms =
            Self::calculate_commit_timestamp(&self.context, &mut dag_state, &leader_block, last_commit_timestamp_ms);
        ...
        let sub_dag = CommittedSubDag::new(leader_block.reference(), to_commit, timestamp_ms, commit.reference());
    ...
    pub(crate) fn calculate_commit_timestamp(
        ...
        let timestamp_ms = if context.protocol_config.consensus_median_based_commit_timestamp() {
            ...
            median_timestamp_by_stake(context, blocks).unwrap_or_else(|e| {
                panic!("Cannot compute median timestamp for leader block {:?} ancestors: {}", leader_block, e)
            })
        } else {
    ...
    fn update_blocks_pruned_metric(&self, sub_dag: &CommittedSubDag) {
        ...
        for block_ref in sub_dag
            ...
            .filter(
                |ancestor_ref| {
                    ancestor_ref.round <= gc_round && last_committed_rounds[ancestor_ref.author] != ancestor_ref.round
            ...
            .unique()
        {
            ...
            self.context.metrics.node_metrics.blocks_pruned_on_commit.with_label_values(label_values).inc();
        }
    }
}
...
pub(crate) fn median_timestamp_by_stake(
    ...
    if total_stake < context.committee.quorum_threshold() {
        return Err(format!("Total stake {} < quorum threshold {}", total_stake, context.committee.quorum_threshold())
            .to_string());
    }
...
mod tests {
    ...
    async fn test_handle_commit(#[values(true, false)] consensus_median_timestamp: bool) {
        ...
        context.protocol_config.set_consensus_median_based_commit_timestamp_for_testing(consensus_median_timestamp);
    ...
    #[rstest]
    #[tokio::test]
    async fn test_handle_already_committed(#[values(true, false)] consensus_median_timestamp: bool) {
        telemetry_subscribers::init_for_testing();
        let num_authorities = 4;
    ...
    #[case(3, true)]
    #[tokio::test]
    async fn test_handle_commit_with_gc_simple(#[case] gc_depth: u32, #[case] consensus_median_timestamp: bool) {
        telemetry_subscribers::init_for_testing();
            ...
            if idx == 0 {
                ...
                assert!(
                    subdag
                        .blocks
                        .iter()
                        .any(|block| block.round() == 2 && block.author() == AuthorityIndex::new_for_test(3)),
                ...
                assert!(
                    subdag
                        .blocks
                        .iter()
                        .any(|block| block.round() == 1 && block.author() == AuthorityIndex::new_for_test(3)),
    ...
    async fn test_calculate_commit_timestamp(
        ...
        // WHEN
        let timestamp =
            Linearizer::calculate_commit_timestamp(&context, &mut dag_state, &leader_block, last_commit_timestamp_ms);
        ...
        let leader_block = VerifiedBlock::new_for_test(
            TestBlock::new(5, 0)
                .set_timestamp_ms(5_000)
                .set_ancestors(ancestors.iter().skip(1).map(|block| block.reference()).collect::<Vec<_>>())
                .build(),
        );
        ...
        let leader_block = VerifiedBlock::new_for_test(
            TestBlock::new(5, 0)
                .set_timestamp_ms(5_000)
                .set_ancestors(ancestors.iter().take(1).map(|block| block.reference()).collect::<Vec<_>>())
                .build(),
        );
        let last_commit_timestamp_ms = 0;
        let timestamp =
