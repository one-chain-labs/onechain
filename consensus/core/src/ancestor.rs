...
use crate::{context::Context, dag_state::DagState, leader_scoring::ReputationScores, round_tracker::QuorumRound};
...
impl AncestorStateManager {
    ...
    pub(crate) fn new(context: Arc<Context>, dag_state: Arc<RwLock<DagState>>) -> Self {
        ...
        let excluded_nodes_stake_threshold =
            (excluded_nodes_stake_threshold_percentage * context.committee.total_stake()) / 100 as Stake;
    ...
    pub(crate) fn update_all_ancestors_state(&mut self, accepted_quorum_rounds: &[QuorumRound]) {
        ...
        let network_high_quorum_round = self.calculate_network_high_quorum_round(accepted_quorum_rounds);

        let current_clock_round = self.dag_state.read().threshold_clock_round();
        let low_score_threshold =
            (self.propagation_scores.highest_score() * Self::SCORE_EXCLUSION_THRESHOLD_PERCENTAGE) / 100;
        ...
        for (idx, score) in self.propagation_scores.scores_per_authority.iter().enumerate() {
            let authority_id = self.context.committee.to_authority_index(idx).expect("Index should be valid");
            ...
            match ancestor_info.state {
                ...
                AncestorState::Exclude(_) => {
                    if *score > low_score_threshold || authority_high_quorum_round >= network_high_quorum_round {
    ...
    fn apply_state_change(&mut self, transition: StateTransition, new_state: AncestorState, current_clock_round: u32) {
        let block_hostname = &self.context.committee.authority(transition.authority_id).hostname;
        ...
        match (ancestor_info.state, new_state) {
            (AncestorState::Exclude(_), AncestorState::Include) => {
                self.total_excluded_stake = self.total_excluded_stake.checked_sub(transition.stake).expect(
                    "total_excluded_stake underflow - trying to subtract more stake than we're tracking as excluded",
                );
            }
        ...
        self.context
            ...
            .with_label_values(&[block_hostname, match new_state {
...
mod test {
    use super::*;
    use crate::{leader_scoring::ReputationScores, storage::mem_store::MemStore, test_dag_builder::DagBuilder};
    ...
    async fn test_calculate_network_high_accepted_quorum_round() {
        ...
        let network_high_quorum_round = ancestor_state_manager.calculate_network_high_quorum_round(&[]);
    ...
    async fn test_update_all_ancestor_state_using_accepted_rounds() {
        ...
        let accepted_quorum_rounds = vec![(225, 229), (225, 229), (229, 300), (229, 300), (229, 300)];
        ...
        let accepted_quorum_rounds = vec![(225, 229), (229, 300), (229, 300), (229, 300), (229, 300)];
        ancestor_state_manager.update_all_ancestors_state(&accepted_quorum_rounds);
        ...
        let accepted_quorum_rounds = vec![(229, 300), (229, 300), (229, 300), (229, 300), (229, 300)];
        ancestor_state_manager.update_all_ancestors_state(&accepted_quorum_rounds);
