...
mod test {
    ...
    async fn test_verify_block() {
        ...
        // Block at genesis round.
        {
            ...
            let signed_block = SignedBlock::new(block, author_protocol_keypair).unwrap();
            assert!(matches!(verifier.verify_block(&signed_block), Err(ConsensusError::UnexpectedGenesisBlock)));
        }
        ...
        // Block without signature.
        {
            ...
            assert!(matches!(verifier.verify_block(&signed_block), Err(ConsensusError::MalformedSignature(_))));
        }
        ...
        // Block with too many ancestors.
        {
            let block = test_block
                ...
                .build();
            let signed_block = SignedBlock::new(block, author_protocol_keypair).unwrap();
            assert!(matches!(verifier.verify_block(&signed_block), Err(ConsensusError::TooManyAncestors(_, _))));
        }
