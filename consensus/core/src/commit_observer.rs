...
impl CommitObserver {
    ...
    pub(crate) fn handle_commit(
        ...
        if schedule_updated {
            let reputation_scores_desc = self.leader_schedule.leader_swap_table.read().reputation_scores_desc.clone();
    ...
    async fn recover_and_send_commits(&mut self, commit_consumer: &CommitConsumerArgs) {
        ...
        let last_commit = self.store.read_last_commit().expect("Reading the last commit should not fail");
        ...
        info!("Recovering commit observer in the range [{}..={last_commit_index}]", replay_after_commit_index + 1,);
        ...
        for start_index in
            (replay_after_commit_index + 1 ..= last_commit_index).step_by(COMMIT_RECOVERY_BATCH_SIZE as usize)
        {
            let end_index = start_index.saturating_add(COMMIT_RECOVERY_BATCH_SIZE - 1).min(last_commit_index);

            let unsent_commits =
                self.store.scan_commits((start_index ..= end_index).into()).expect("Scanning commits should not fail");
            assert_eq!(
            ...
            info!("Recovering {} unsent commits in range [{start_index}..={end_index}]", unsent_commits.len());
            ...
            for commit in unsent_commits.into_iter() {
                ...
                let committed_sub_dag = load_committed_subdag_from_store(self.store.as_ref(), commit, reputation_scores);

                if !committed_sub_dag.recovered_rejected_transactions && !seen_unfinalized_commit {
                    info!("Starting to recover unfinalized commit from {}", committed_sub_dag.commit_ref);
                ...
                if seen_unfinalized_commit {
                    ...
                    self.transaction_certifier.recover_and_vote_on_blocks(committed_sub_dag.blocks.clone());
                }
    ...
    fn report_metrics(&self, committed: &[CommittedSubDag]) {
        ...
        for commit in committed {
            ...
            metrics.last_committed_leader_round.set(commit.leader.round as i64);
            metrics.last_commit_index.set(commit.commit_ref.index as i64);
            metrics.blocks_per_commit_count.observe(commit.blocks.len() as f64);

            for block in &commit.blocks {
                let latency_ms = utc_now.checked_sub(block.timestamp_ms()).unwrap_or_default();
                metrics.block_commit_latency.observe(Duration::from_millis(latency_ms).as_secs_f64());
            }
        }

        self.context.metrics.node_metrics.sub_dags_per_commit_count.observe(committed.len() as f64);
    }
}
...
mod tests {
    ...
    async fn test_handle_commit() {
        ...
        transaction_certifier.add_voted_blocks(builder.all_blocks().iter().map(|b| (b.clone(), vec![])).collect());
        ...
        for (idx, subdag) in commits.iter().enumerate() {
            ...
            let expected_ts = if idx == 0 { expected_ts } else { expected_ts.max(commits[idx - 1].timestamp_ms) };
    ...
    async fn test_recover_and_send_commits() {
        ...
        let mut commits = observer.handle_commit(leaders[.. expected_last_processed_index].to_vec(), true).unwrap();
        ...
        commits.append(&mut observer.handle_commit(leaders[expected_last_processed_index ..].to_vec(), true).unwrap());
        ...
        // Replay commits after index 2. And use last processed index by consumer at 10, which is the last persisted commit.
        {
            ...
            let dag_state = Arc::new(RwLock::new(DagState::new(context.clone(), mem_store.clone())));
            let (commit_consumer, mut commit_receiver, _transaction_receiver) =
                CommitConsumerArgs::new(replay_after_commit_index, consumer_last_processed_commit_index);
            ...
            while let Ok(Some(mut subdag)) = timeout(Duration::from_secs(1), commit_receiver.recv()).await {
        ...
        // Replay commits after index 2. And use last processed index by consumer at 4, less than the last persisted commit.
        {
            ...
            assert_eq!(processed_subdag_index, expected_last_sent_index as CommitIndex);
