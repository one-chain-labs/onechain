    ...
    fn recover_and_send_commits(&mut self, last_processed_commit_index: CommitIndex) {
        ...
        for (index, commit) in unsent_commits.into_iter().enumerate() {
            ...
            self.sender
                ...
                .unwrap_or_else(|e| panic!("Failed to send commit during recovery, probably due to shutdown: {:?}", e));
...
mod tests {
    ...
    async fn test_handle_commit() {
        ...
        let mut observer =
            CommitObserver::new(context.clone(), commit_consumer, dag_state.clone(), mem_store.clone(), leader_schedule);
        ...
        for (idx, subdag) in commits.iter().enumerate() {
            ...
            let expected_ts = if idx == 0 {
                ...
                leaders[idx].timestamp_ms().max(commits[idx - 1].timestamp_ms)
            };
    ...
    async fn test_recover_and_send_commits() {
        ...
        let mut commits = observer
            .handle_commit(leaders.clone().into_iter().take(expected_last_processed_index).collect::<Vec<_>>())
            .unwrap();
        ...
        commits.append(
            &mut observer
                .handle_commit(leaders.clone().into_iter().skip(expected_last_processed_index).collect::<Vec<_>>())
                .unwrap(),
        );
