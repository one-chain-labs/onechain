...
impl CommitObserver {
    pub(crate) async fn new(
        ...
        let mut observer =
            Self { context, dag_state, store, leader_schedule, commit_interpreter, commit_finalizer_handle };
    ...
    async fn recover_and_send_commits(&mut self, commit_consumer: &CommitConsumerArgs) {
        ...
        let last_finalized_commit_index = if self.context.protocol_config.mysticeti_fastpath() {
            self.store.read_last_finalized_commit().unwrap().map(|commit_ref| commit_ref.index).unwrap_or(0)
        } else {
            last_commit_index
        };
            ...
            (replay_after_commit_index + 1 ..= last_commit_index).step_by(COMMIT_RECOVERY_BATCH_SIZE as usize)
        {
            ...
            info!("Recovered {} unsent commits in range [{start_index}..={end_index}]", unsent_commits.len());
