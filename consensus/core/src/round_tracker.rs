...
impl PeerRoundTracker {
    ...
    pub(crate) fn update_from_verified_block(&mut self, extended_block: &ExtendedBlock) {
        ...
        self.block_accepted_rounds[author][author] = self.block_accepted_rounds[author][author].max(block.round());
        ...
        for excluded_ancestor in excluded_ancestors {
            self.block_accepted_rounds[author][excluded_ancestor.author] =
                self.block_accepted_rounds[author][excluded_ancestor.author].max(excluded_ancestor.round);
        }
    }

    /// Update accepted & received rounds based on probing results
    pub(crate) fn update_from_probe(&mut self, accepted_rounds: Vec<Vec<Round>>, received_rounds: Vec<Vec<Round>>) {
    ...
    pub(crate) fn calculate_propagation_delay(&self, last_proposed_round: Round) -> Round {
        ...
        node_metrics.round_tracker_propagation_delays.observe(propagation_delay as f64);
        node_metrics.round_tracker_last_propagation_delay.set(propagation_delay as i64);
    ...
    fn compute_received_quorum_rounds(&self) -> Vec<QuorumRound> {
        let received_quorum_rounds = self
            ...
            .committee
            .authorities()
            .map(|(peer, _)| compute_quorum_round(&self.context.committee, peer, &self.probed_received_rounds))
            .collect::<Vec<_>>();
...
mod test {
    ...
    async fn test_compute_accepted_quorum_round() {
        ...
        // Observe latest rounds from peers.
        let highest_accepted_rounds = vec![vec![10, 11, 12, 13], vec![5, 2, 7, 4], vec![0, 0, 0, 0], vec![3, 4, 5, 6]];
        ...
        let test_block = TestBlock::new(7, 2)
            .set_ancestors(vec![BlockRef::new(6, AuthorityIndex::new_for_test(3), BlockDigest::MIN)])
            .build();
        let block = VerifiedBlock::new_for_test(test_block);
        round_tracker.update_from_verified_block(&ExtendedBlock {
            block,
            excluded_ancestors: vec![BlockRef::new(8, AuthorityIndex::new_for_test(1), BlockDigest::MIN)],
        });
    ...
    async fn test_quorum_round_manager() {
        ...
        for authority in 0 .. NUM_AUTHORITIES {
            ...
            let block = VerifiedBlock::new_for_test(TestBlock::new(round, authority as u32).build());
            round_tracker.update_from_verified_block(&ExtendedBlock { block, excluded_ancestors: vec![] });
        }
