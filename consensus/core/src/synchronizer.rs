...
impl<C: NetworkClient, V: BlockVerifier, D: CoreThreadDispatcher> Synchronizer<C, V, D> {
    ...
    async fn process_fetched_blocks(
        ...
        let blocks = Handle::current()
            .spawn_blocking({
                ...
                move || {
                    Self::verify_blocks(serialized_blocks, block_verifier, transaction_certifier, &context, peer_index)
                }
            })
            .await
    ...
    fn verify_blocks(
        ...
        for serialized_block in serialized_blocks {
            ...
            if drift > 0 {
                let peer_hostname = &context.committee.authority(verified_block.author()).hostname;
                context
                    .metrics
                    .node_metrics
                ...
                if context.protocol_config.consensus_median_based_commit_timestamp() {
    ...
    async fn start_fetch_missing_blocks_task(&mut self) -> ConsensusResult<()> {
        let missing_blocks = self.core_dispatcher.get_missing_blocks().await.map_err(|_err| ConsensusError::Shutdown)?;
        ...
        self.fetch_blocks_scheduler_task.spawn(monitored_future!(async move {
            ...
            let results = if context.protocol_config.consensus_batched_block_sync() {
                ...
                Self::fetch_blocks_from_authorities_old(
                    context.clone(),
                    blocks_to_fetch.clone(),
                    network_client,
                    missing_blocks,
            ...
            for (blocks_guard, fetched_blocks, peer) in results {
                ...
                .await
                {
                    ...
                    context
                        ...
                        .with_label_values(&[&context.committee.authority(peer).hostname, "periodic"])
                        .inc();
                }
            }
    ...
    async fn fetch_blocks_from_authorities(
        ...
        let num_peers = authorities.len().div_ceil((context.committee.size() - 1).div_ceil(MAX_PERIODIC_SYNC_PEERS));
...
mod tests {
    ...
    use crate::{
        ...
        synchronizer::{InflightBlocksMap, Synchronizer, FETCH_BLOCKS_CONCURRENCY, FETCH_REQUEST_TIMEOUT},
        transaction_certifier::TransactionCertifier,
        CommitDigest,
        CommitIndex,
    };
    ...
    impl NetworkClient for MockNetworkClient {
        ...
        async fn fetch_blocks(
            ...
            let response = lock.remove(&(block_refs.clone(), peer)).unwrap_or_else(|| {
                panic!("Unexpected fetch blocks request made: {:?} {}. Current lock: {:?}", block_refs, peer, lock);
            });
    ...
    async fn synchronizer_periodic_task_when_commit_lagging_gets_disabled() {
        ...
        let stub_blocks = (sync_missing_block_round_threshold * 2 .. sync_missing_block_round_threshold * 3)
            .map(|round| VerifiedBlock::new_for_test(TestBlock::new(round, 0).build()))
            .collect::<Vec<_>>();
        let missing_blocks = stub_blocks.iter().map(|block| block.reference()).collect::<BTreeSet<_>>();
        core_dispatcher.stub_missing_blocks(missing_blocks.clone()).await;
        ...
        let mut expected_blocks =
            stub_blocks.iter().take(context.parameters.max_blocks_per_sync).cloned().collect::<Vec<_>>();
        network_client
            .stub_fetch_blocks(expected_blocks.clone(), AuthorityIndex::new_for_test(1), Some(FETCH_REQUEST_TIMEOUT))
            .await;
        network_client.stub_fetch_blocks(expected_blocks.clone(), AuthorityIndex::new_for_test(2), None).await;
    ...
    async fn test_process_fetched_blocks() {
        ...
        let dag_state = Arc::new(RwLock::new(DagState::new(context.clone(), store)));
        let transaction_certifier = TransactionCertifier::new(context.clone(), dag_state.clone(), blocks_sender);
        let (commands_sender, _commands_receiver) = monitored_mpsc::channel("consensus_synchronizer_commands", 1000);
        ...
        expected_blocks.extend((30 ..= 60).map(|round| VerifiedBlock::new_for_test(TestBlock::new(round, 1).build())));
        assert_eq!(expected_blocks.len(), context.parameters.max_blocks_per_sync);

        let expected_serialized_blocks = expected_blocks.iter().map(|b| b.serialized().clone()).collect::<Vec<_>>();

        let expected_block_refs = expected_blocks.iter().map(|b| b.reference()).collect::<BTreeSet<_>>();
        ...
        let inflight_blocks_map = InflightBlocksMap::new();
        let blocks_guard =
            inflight_blocks_map.lock_blocks(expected_block_refs.clone(), peer_index).expect("Failed to lock blocks");
        ...
        let added_blocks = core_dispatcher.get_add_blocks().await;
        assert_eq!(added_blocks.iter().map(|b| b.reference()).collect::<BTreeSet<_>>(), expected_block_refs,);
