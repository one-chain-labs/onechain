...
impl CommitFinalizer {
    ...
    pub(crate) fn start(
        ...
        let _handle = spawn_logged_monitored_task!(processor.run(receiver), "consensus_commit_finalizer");
    ...
    async fn run(mut self, mut receiver: UnboundedReceiver<CommittedSubDag>) {
        while let Some(committed_sub_dag) = receiver.recv().await {
            let already_finalized =
                !self.context.protocol_config.mysticeti_fastpath() || committed_sub_dag.recovered_rejected_transactions;
            let finalized_commits =
                if !already_finalized { self.process_commit(committed_sub_dag).await } else { vec![committed_sub_dag] };
            if !finalized_commits.is_empty() {
                ...
                if !already_finalized {
                    ...
                    for commit in &finalized_commits {
                        dag_state.add_finalized_commit(commit.commit_ref, commit.rejected_transactions_by_block.clone());
                    }
                }
    ...
    pub async fn process_commit(&mut self, committed_sub_dag: CommittedSubDag) -> Vec<CommittedSubDag> {
        ...
        for i in 0 .. self.pending_commits.len() {
            ...
            if !commit_state.commit.decided_with_local_blocks {
                ...
                if commit_state.commit.leader.round + DEFAULT_WAVE_LENGTH > last_commit_state.commit.leader.round {
                    break;
                }
            }
        ...
        self.context.metrics.node_metrics.finalizer_buffered_commits.set(self.pending_commits.len() as i64);
    ...
    fn try_direct_finalize_commit(&mut self, index: usize) {
        ...
        for (block_ref, num_transactions) in pending_blocks {
            ...
            metrics.finalizer_reject_votes.with_label_values(&[hostname]).inc_by(reject_votes.len() as u64);
            ...
            for (transaction_index, stake) in reject_votes {
                ...
                let entry = if stake < self.context.committee.quorum_threshold() {
                    ...
                    metrics.finalizer_transaction_status.with_label_values(&["direct_reject"]).inc();
    ...
    fn link_blocks_in_last_commit(&mut self) {
        let commit_state = self.pending_commits.back_mut().unwrap_or_else(|| panic!("No pending commit."));
        ...
        for block in blocks {
            ...
            blocks_map
                .entry(block_ref)
                .or_insert_with(|| RwLock::new(BlockState::new(block, commit_state.commit.commit_ref.index)));
        }
    }
    ...
    fn append_origin_descendants_from_last_commit(&mut self) {
        ...
        for committed_block in committed_blocks {
            ...
            let mut origin_ancestor_ref =
                *blocks_map.get(&committed_block_ref).unwrap().read().block.ancestors().first().unwrap();
            while origin_ancestor_ref.author == committed_block_ref.author {
                ...
                origin_ancestor_ref = *origin_ancestor_block.read().block.ancestors().first().unwrap();
            }
        }
    }
    ...
    fn check_pending_transactions_in_first_commit(&mut self) {
        ...
        for (block_ref, rejected_transactions) in all_rejected_transactions {
            ...
            curr_commit_state.rejected_transactions.entry(block_ref).or_default().extend(rejected_transactions);
        }
    }

    async fn try_indirect_finalize_pending_transactions_in_first_commit(&mut self) {
        let _scope = monitored_scope("CommitFinalizer::try_indirect_finalize_pending_transactions_in_first_commit");

        let pending_blocks: Vec<_> =
            self.pending_commits[0].pending_transactions.iter().map(|(k, v)| (*k, v.clone())).collect();

        let gc_rounds = self
            .pending_commits
            .iter()
            .map(|c| (c.commit.commit_ref.index, self.dag_state.read().calculate_gc_round(c.commit.leader.round)))
            .collect::<Vec<_>>();
        ...
        for (block_ref, finalized_transactions) in all_finalized_transactions {
            ...
            self.pending_commits[0].remove_pending_transactions(&block_ref, &finalized_transactions);
        }
    }
    ...
    fn try_indirect_reject_pending_transactions_in_first_commit(&mut self) {
        ...
        if curr_leader_round + INDIRECT_REJECT_DEPTH <= last_commit_leader_round {
            ...
            for (block_ref, pending_transactions) in pending_transactions {
                ...
                curr_commit_state.rejected_transactions.entry(block_ref).or_default().extend(pending_transactions);
            }
        }
    }
    ...
    fn try_indirect_finalize_pending_transactions_in_block(
        ...
        let mut accept_votes: BTreeMap<TransactionIndex, StakeAggregator<QuorumThreshold>> = pending_transactions
            .into_iter()
        ...
        while let Some(curr_block_ref) = to_visit_blocks.pop_first() {
            ...
            let curr_block_state = blocks_map
                .get(&curr_block_ref)
                .unwrap_or_else(|| {
                    panic!("Block {curr_block_ref} is either incorrectly gc'ed or failed to be recovered after crash.")
                })
                .read();
            ...
            if ignored.insert(curr_block_ref) {
                ...
                if context.protocol_config.consensus_skip_gced_accept_votes() && votes_gced {
                    ...
                    context.metrics.node_metrics.finalizer_skipped_voting_blocks.with_label_values(&[hostname]).inc();
                    continue;
                }
                // Get reject votes from current block to the pending block.
                let curr_block_reject_votes =
                    curr_block_state.reject_votes.get(&pending_block_ref).cloned().unwrap_or_default();
            ...
            to_visit_blocks.extend(curr_block_state.children.iter().filter(|b| !visited.contains(*b)));
        }
    ...
    fn gced_transaction_votes_for_pending_block(
        ...
        let (commit_index, gc_round) =
            *gc_rounds.get((current_commit_index - 1 - pending_commit_index) as usize).unwrap();
        assert_eq!(
            commit_index,
    ...
    fn pop_finalized_commits(&mut self) -> Vec<CommittedSubDag> {
        ...
        while let Some(commit_state) = self.pending_commits.front() {
            if !commit_state.pending_blocks.is_empty() || !commit_state.pending_transactions.is_empty() {
            ...
            for (block_ref, rejected_transactions) in commit_state.rejected_transactions {
                commit.rejected_transactions_by_block.insert(block_ref, rejected_transactions.into_iter().collect());
            }
            ...
            self.context.metrics.node_metrics.finalizer_round_delay.observe(round_delay as f64);
...
impl CommitState {
    fn new(commit: CommittedSubDag) -> Self {
        ...
        Self { commit, pending_blocks, pending_transactions: BTreeMap::new(), rejected_transactions: BTreeMap::new() }
    }

    fn remove_pending_transactions(&mut self, block_ref: &BlockRef, transactions: &[TransactionIndex]) {
...
impl BlockState {
    fn new(block: VerifiedBlock, commit_index: CommitIndex) -> Self {
        let reject_votes: BTreeMap<_, _> =
            block.transaction_votes().iter().map(|v| (v.block_ref, v.rejects.clone().into_iter().collect())).collect();
        ...
        Self { block, children: BTreeSet::new(), reject_votes, origin_descendants, commit_index }
    }
}
...
mod tests {
    ...
    fn create_commit_finalizer_fixture() -> Fixture {
        ...
        context.protocol_config.set_consensus_gc_depth_for_testing(5);
        context.protocol_config.set_consensus_skip_gced_accept_votes_for_testing(true);
        ...
        let commit_finalizer =
            CommitFinalizer::new(context.clone(), dag_state.clone(), transaction_certifier.clone(), commit_sender);
        Fixture { context, dag_state, transaction_certifier, linearizer, commit_finalizer }
    }
    ...
    fn create_block(
        ...
        let i = ancestors
            ...
            .unwrap_or_else(|| panic!("Authority {authority} (round {round}) not found in {ancestors:?}"));
    ...
    async fn test_direct_finalize_no_reject_votes() {
        ...
        dag_builder.layers(1 ..= 4).num_transactions(10).build().persist_layers(fixture.dag_state.clone());
        let blocks = dag_builder.all_blocks();
        fixture.transaction_certifier.add_voted_blocks(blocks.iter().map(|b| (b.clone(), vec![])).collect());
        ...
        let finalized_commits = fixture.commit_finalizer.process_commit(committed_sub_dag.clone()).await;
    ...
    async fn test_direct_finalize_with_reject_votes() {
        ...
        let mut dag_builder = DagBuilder::new(fixture.context.clone());
        dag_builder.layer(1).num_transactions(10).build().persist_layers(fixture.dag_state.clone());
        let round_1_blocks = dag_builder.all_blocks();
        fixture.transaction_certifier.add_voted_blocks(
            round_1_blocks
                .iter()
                .map(|b| if b.author().value() != 3 { (b.clone(), vec![]) } else { (b.clone(), vec![0, 3]) })
                .collect(),
        );

        // Select the block with rejected transaction.
        let block_with_rejected_txn = round_1_blocks[3].clone();
        let reject_vote = BlockTransactionVotes { block_ref: block_with_rejected_txn.reference(), rejects: vec![0, 3] };
        ...
        let round_2_blocks = vec![
            create_block(2, 0, round_1_blocks.iter().map(|b| b.reference()).collect(), 10, vec![reject_vote.clone()]),
        ...
        let round_3_blocks = vec![
            ...
            create_block(
                3,
                3,
                std::iter::once(round_1_blocks[3].reference()).chain(ancestors.clone()).collect(),
                0,
        ...
        assert_eq!(
            finalized_commit.rejected_transactions_by_block.get(&block_with_rejected_txn.reference()).unwrap().clone(),
            vec![0, 3],
        );
    ...
    async fn test_indirect_finalize_with_reject_votes() {
        ...
        // Leader links to block_with_rejected_txn, but other blocks do not.
        let round_2_blocks = vec![
            create_block(2, 0, round_1_blocks.iter().map(|b| b.reference()).collect(), 10, vec![
                BlockTransactionVotes { block_ref: block_with_rejected_txn.reference(), rejects: vec![1, 4] },
            ]),
        ...
        let round_3_blocks = vec![
            create_block(3, 0, ancestors.clone(), 0, vec![]),
            create_block(3, 1, ancestors.clone(), 0, vec![BlockTransactionVotes {
        ...
        let round_4_blocks = vec![
            ...
            create_block(
                4,
                2,
                std::iter::once(round_2_blocks[2].reference()).chain(ancestors.clone()).collect(),
                0,
                vec![BlockTransactionVotes { block_ref: block_with_rejected_txn.reference(), rejects: vec![1] }],
            ),
            create_block(4, 3, ancestors.clone(), 0, vec![]),
        ];
        fixture.add_blocks(round_4_blocks.clone());
        ...
        for r in 5 ..= 7 {
            let ancestors: Vec<BlockRef> = last_round_blocks.iter().map(|b| b.reference()).collect();
            let round_blocks: Vec<_> = (0 .. 4).map(|i| create_block(r, i, ancestors.clone(), 0, vec![])).collect();
        ...
        for commit in committed_sub_dags.iter().take(3) {
            let finalized_commits = fixture.commit_finalizer.process_commit(commit.clone()).await;
            assert_eq!(finalized_commits.len(), 0);
        }

        // Buffering the 4th commit should finalize all commits.
        let finalized_commits = fixture.commit_finalizer.process_commit(committed_sub_dags[3].clone()).await;
        ...
        assert_eq!(rejected_transactions.get(&block_with_rejected_txn.reference()).unwrap(), &vec![1, 4]);
    ...
    async fn test_indirect_reject_with_gc() {
        ...
        dag_builder.layer(1).num_transactions(10).build().persist_layers(fixture.dag_state.clone());
        let round_1_blocks = dag_builder.all_blocks();
        fixture.transaction_certifier.add_voted_blocks(round_1_blocks.iter().map(|b| (b.clone(), vec![])).collect());
        ...
        let round_2_blocks = vec![
            ...
            create_block(2, 1, ancestors.clone(), 0, vec![BlockTransactionVotes {
                block_ref: block_with_rejected_txn.reference(),
                rejects: vec![1],
            }]),
        ...
        let mut last_round_blocks: Vec<VerifiedBlock> =
            round_2_blocks.iter().enumerate().filter_map(|(i, b)| if i != 2 { Some(b.clone()) } else { None }).collect();
        for r in 3 ..= 6 {
            let ancestors: Vec<BlockRef> = last_round_blocks.iter().map(|b| b.reference()).collect();
            last_round_blocks = [0, 1, 3].map(|i| create_block(r, i, ancestors.clone(), 0, vec![])).to_vec();
        ...
        let mut leaders = vec![];
        for r in 7 ..= 10 {
            let mut ancestors: Vec<BlockRef> = last_round_blocks.iter().map(|b| b.reference()).collect();
        ...
        let rejected_transactions = finalized_commits[0].rejected_transactions_by_block.clone();
        assert_eq!(rejected_transactions.len(), 1);
        assert_eq!(rejected_transactions.get(&block_with_rejected_txn.reference()).unwrap(), &vec![1]);
    ...
    async fn test_finalize_remote_commits_with_reject_votes() {
        ...
        async fn add_blocks_and_process_commit(
            ...
            fixture.commit_finalizer.process_commit(remote_commit.clone()).await
        }

        // Add commit 1-3 as remote commits. There should be no finalized commits.
        for i in 0 .. 3 {
            let finalized_commits = add_blocks_and_process_commit(&mut fixture, &leaders, &all_blocks, i, false).await;
            assert!(finalized_commits.is_empty());
        }

        // Buffer round 4 commit as a remote commit. This should finalize the 1st commit at round 1.
        let finalized_commits = add_blocks_and_process_commit(&mut fixture, &leaders, &all_blocks, 3, false).await;
        assert_eq!(finalized_commits.len(), 1);
        ...
        let finalized_commits = add_blocks_and_process_commit(&mut fixture, &leaders, &all_blocks, 4, true).await;
        ...
        let finalized_commits = add_blocks_and_process_commit(&mut fixture, &leaders, &all_blocks, 5, true).await;
