...
impl CommitFinalizer {
    ...
    fn link_blocks_in_last_commit(&mut self) {
        ...
        for block in blocks {
            ...
            for votes in block.transaction_votes() {
                block_state.reject_votes.entry(votes.block_ref).or_default().extend(votes.rejects.iter());
            }
    ...
    fn inherit_reject_votes_in_last_commit(&mut self) {
        ...
        for block in blocks {
            ...
            let Some(own_ancestor_rejects) = self.blocks.get(&own_ancestor).map(|b| b.reject_votes.clone()) else {
            ...
            for (block_ref, reject_votes) in own_ancestor_rejects {
                block_state.reject_votes.entry(block_ref).or_default().extend(reject_votes.iter());
            }
        }
    }
    ...
    fn try_indirect_finalize_pending_transactions_in_first_commit(&mut self) {
        ...
        for (block_ref, pending_transactions) in &self.pending_commits[0].pending_transactions {
            let finalized_transactions =
                self.try_indirect_finalize_pending_transactions_in_block(*block_ref, pending_transactions.clone());
    ...
    fn try_indirect_finalize_pending_transactions_in_block(
        ...
        let mut to_visit_blocks = self.blocks.get(&pending_block_ref).unwrap().children.clone();
        ...
        while let Some(curr_block_ref) = to_visit_blocks.pop_first() {
            ...
            let curr_block_state = self.blocks.get(&curr_block_ref).unwrap_or_else(|| {
...
mod tests {
    ...
    async fn test_direct_finalize_no_reject_votes() {
        ...
        let finalized_commits = fixture.commit_finalizer.process_commit(committed_sub_dag.clone());
    ...
    async fn test_indirect_finalize_with_reject_votes() {
        ...
        let finalized_commits = fixture.commit_finalizer.process_commit(committed_sub_dags[3].clone());
    ...
    async fn test_finalize_remote_commits_with_reject_votes() {
        ...
        let mut add_blocks_and_process_commit = |index: usize, local: bool| -> Vec<CommittedSubDag> {
            ...
            fixture.commit_finalizer.process_commit(remote_commit.clone())
        };
