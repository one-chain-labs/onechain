...
impl CommitFinalizer {
    ...
    pub async fn process_commit(&mut self, committed_sub_dag: CommittedSubDag) -> Vec<CommittedSubDag> {
        ...
        if let Some(last_commit) = finalized_commits.last() {
            let gc_round = self.dag_state.read().calculate_gc_round(last_commit.leader.round);
    ...
    async fn try_indirect_finalize_pending_transactions_in_first_commit(&mut self) {
        ...
        let pending_blocks: Vec<(BlockRef, BTreeSet<TransactionIndex>)> =
    ...
    fn try_indirect_finalize_pending_transactions_in_block(
        ...
        let mut to_visit_blocks = blocks_map.get(&pending_block_ref).unwrap().read().children.clone();
