...
async fn indirect_commit() {
    ...
    let leader_wave2 = committer.elect_leader(leader_round_wave2).expect("there should be a leader for wave 2");
    ...
    let leader_status_wave1_indirect = committer.try_indirect_decide(leader, [leader_status_wave_2].iter());
...
async fn indirect_skip() {
    ...
    let leader_wave2 = committer.elect_leader(leader_round_wave_2).expect("there should be a leader for wave 2");
...
async fn test_equivocating_direct_commit() {
    ...
    let leader_wave_1 = committer.elect_leader(leader_round_wave_1).expect("there should be a leader for wave 1");
    ...
    let block_refs_round_3: Vec<_> = dag_builder.blocks(3u32 ..= 3).iter().map(|b| b.reference()).collect();

    // Authority B index is 1
    let b4_votes_all = VerifiedBlock::new_for_test(
        TestBlock::new(4, 1).set_ancestors(block_refs_round_3).set_timestamp_ms(4 * 1000 + 1_u64).build(),
    );
