...
impl BlockManager {
    ...
    pub(crate) fn try_accept_blocks(&mut self, blocks: Vec<VerifiedBlock>) -> (Vec<VerifiedBlock>, BTreeSet<BlockRef>) {
    ...
    pub(crate) fn try_accept_committed_blocks(&mut self, blocks: Vec<VerifiedBlock>) -> Vec<VerifiedBlock> {
        ...
        assert!(missing_blocks.is_empty(), "No missing blocks should be returned for committed blocks");
    ...
    fn try_accept_blocks_internal(
        ...
        for block in blocks {
            ...
            let blocks_to_accept = self.verify_block_timestamps_and_accept(to_verify_timestamps_and_accept);
    ...
    pub(crate) fn try_find_blocks(&mut self, block_refs: Vec<BlockRef>) -> BTreeSet<BlockRef> {
        ...
        let mut block_refs = block_refs.into_iter().filter(|block_ref| block_ref.round > gc_round).collect::<Vec<_>>();
        ...
        debug!("Trying to find blocks: {}", block_refs.iter().map(|b| b.to_string()).join(","));

        let mut missing_blocks = BTreeSet::new();

        for (found, block_ref) in
            self.dag_state.read().contains_blocks(block_refs.clone()).into_iter().zip(block_refs.iter())
        {
    ...
    pub(crate) fn try_unsuspend_blocks_for_latest_gc_round(&mut self) {
        ...
        while let Some((block_ref, _children_refs)) = self.missing_ancestors.first_key_value() {
            ...
            self.context.metrics.node_metrics.block_manager_gced_blocks.with_label_values(&[hostname]).inc();
...
mod tests {
    ...
    async fn unsuspend_blocks_for_latest_gc_round(#[values(5, 10, 14)] gc_depth: u32) {
        ...
        for seed in 0 .. 100u8 {
            ...
            let non_existing_refs = (1 ..= 3)
                .map(|round| BlockRef::new(round, AuthorityIndex::new_for_test(0), BlockDigest::MIN))
                .collect::<Vec<_>>();
    ...
    async fn try_find_blocks() {
        ...
        let missing_block_refs = round_2_blocks.first().unwrap().ancestors();
        let missing_block_refs_from_accept = missing_block_refs.iter().cloned().collect::<BTreeSet<_>>();
        ...
        let missing_block_refs_from_find = block_manager
            .try_find_blocks(round_2_blocks.iter().map(|b| b.reference()).chain(round_3_blocks.into_iter()).collect());
