...
impl DagBuilder {
    ...
    pub(crate) fn get_sub_dag_and_commits(
        ...
        for leader_block in self.leader_blocks(last_leader_round + 1 ..= *leader_rounds.end()).into_iter().flatten() {
            ...
            let sub_dag = CommittedSubDag::new(leader_block_ref, to_commit, last_timestamp_ms, commit.reference());
...
impl<'a> LayerBuilder<'a> {
    ...
    pub fn rejected_transactions_pct(mut self, pct: u8, seed: Option<u64>) -> Self {
        ...
        self.rejected_transactions_seed = if let Some(seed) = seed { seed } else { thread_rng().r#gen() };
        self
    }
    ...
    pub fn configure_min_parent_links(&mut self, round: Round) -> Vec<(AuthorityIndex, Vec<BlockRef>)> {
            ...
            .map(|authority| {
                ...
                (
                    *authority,
                    self.ancestors
                        .iter()
                        .filter(|a| leaders.contains(&a.author) || min_ancestors.contains(&a.author) || a.round != round)
                        .cloned()
    ...
    fn configure_fully_linked_ancestors(&mut self, round: Round) -> Vec<(AuthorityIndex, Vec<BlockRef>)> {
        self.dag_builder
            .context
            .committee
            .authorities()
            .map(|authority| {
                (
                    ...
                    self.ancestors.clone().into_iter().filter(|a| a.round != round).collect::<Vec<_>>(),
                )
            })
            .collect::<Vec<_>>()
    }
    ...
    fn create_blocks(&mut self, round: Round, connections: Vec<(AuthorityIndex, Vec<BlockRef>)>) {
        let mut references = Vec::new();
        let mut reject_rng = StdRng::seed_from_u64(self.rejected_transactions_seed ^ (round as u64));
        for (authority, ancestors) in connections {
            if self.should_skip_block(round, authority) {
                continue;
            };
            let transactions =
                (0 .. self.num_transactions).map(|_| Transaction::new(vec![1_u8; 16])).collect::<Vec<_>>();
    ...
    fn block_timestamp(&self, authority: AuthorityIndex, round: Round, num_block: u32) -> BlockTimestampMs {
