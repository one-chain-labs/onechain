...
impl TransactionCertifier {
    pub fn new(
        ...
        Self { certifier_state: Arc::new(RwLock::new(CertifierState::new(context))), dag_state, certified_blocks_sender }
    }
    ...
    pub(crate) fn recover(&self, block_verifier: &impl BlockVerifier, last_processed_commit_index: CommitIndex) {
        ...
        let last_processed_commit =
            store.scan_commits((last_processed_commit_index ..= last_processed_commit_index).into()).unwrap().pop();
        ...
        let authorities = certifier_state.context.committee.authorities().map(|(index, _)| index).collect::<Vec<_>>();
        for authority_index in authorities {
            let blocks = store.scan_blocks_by_author(authority_index, recovery_start_round).unwrap();
            info!(
                ...
                .map(|b| {
                    if b.round() <= dag_state_gc_round || dag_state.is_hard_linked(&b.reference()) {
                        ...
                        let reject_transaction_votes = block_verifier
                            .vote(&b)
                            .unwrap_or_else(|e| panic!("Failed to vote on block during recovery: {}", e));
    ...
    fn send_certified_blocks(&self, certified_blocks: Vec<CertifiedBlock>) {
        ...
        if let Err(e) = self.certified_blocks_sender.send(CertifiedBlocksOutput { blocks: certified_blocks }) {
    ...
    pub(crate) fn get_own_votes(&self, block_refs: Vec<BlockRef>) -> Vec<BlockTransactionVotes> {
        ...
        for block_ref in block_refs {
            ...
            let vote_info = certifier_state
                .votes
                .get(&block_ref)
                .unwrap_or_else(|| panic!("Ancestor block {} not found in certifier state", block_ref));
            if !vote_info.own_reject_txn_votes.is_empty() {
                votes.push(BlockTransactionVotes { block_ref, rejects: vote_info.own_reject_txn_votes.clone() });
            }
        }
        votes
    }
    ...
    pub(crate) fn get_reject_votes(&self, block_ref: &BlockRef) -> Option<Vec<(TransactionIndex, Stake)>> {
...
impl CertifierState {
    ...
    fn add_voted_blocks(&mut self, voted_blocks: Vec<(VerifiedBlock, Vec<TransactionIndex>)>) -> Vec<CertifiedBlock> {
    ...
    fn add_voted_block(
        ...
        let peer_hostname = &self.context.committee.authority(voted_block.author()).hostname;
        self.context
            .metrics
            .node_metrics
        ...
        for block_votes in voted_block.transaction_votes() {
            ...
            if let Some(certified_block) = vote_info.take_certified_output(&self.context) {
                let authority_name = self.context.committee.authority(certified_block.block.author()).hostname.clone();
                self.context.metrics.node_metrics.certifier_block_latency.with_label_values(&[&authority_name]).observe(
                    Duration::from_millis(now.saturating_sub(certified_block.block.timestamp_ms())).as_secs_f64(),
                );
    ...
    fn add_proposed_block(&mut self, proposed_block: VerifiedBlock) -> Vec<CertifiedBlock> {
        ...
        for voting_ancestor in proposed_block.ancestors() {
            ...
            for target_ancestor in voting_block.ancestors() {
                ...
                target_vote_info.accept_block_votes.add_unique(voting_block.author(), &self.context.committee);
                ...
                if let Some(certified_block) = target_vote_info.take_certified_output(&self.context) {
                    let authority_name =
                        self.context.committee.authority(certified_block.block.author()).hostname.clone();
                    self.context
                        .metrics
                        .node_metrics
    ...
    fn update_gc_round(&mut self, gc_round: Round) {
        ...
        self.context.metrics.node_metrics.certifier_gc_round.set(self.gc_round as i64);
    }
}
    ...
    fn take_certified_output(&mut self, context: &Context) -> Option<CertifiedBlock> {
        ...
        for (idx, reject_txn_votes) in &self.reject_txn_votes {
            ...
            if reject_txn_votes.reached_threshold(committee) {
                context.metrics.node_metrics.certifier_rejected_transactions.with_label_values(&[peer_hostname]).inc();
                rejected.push(*idx);
                continue;
            }
            ...
            if self.accept_block_votes.stake().saturating_sub(reject_txn_votes.stake()) < committee.quorum_threshold() {
                return None;
            }
        }
...
mod test {
    ...
    use super::*;
    use crate::{block::BlockTransactionVotes, context::Context, test_dag_builder::DagBuilder, TestBlock, Transaction};
    ...
    async fn test_vote_info_basic() {
        ...
        // Accept votes but not enough.
        {
            ...
            vote_info.block = Some(block.clone());
            for i in 0 .. 4 {
                vote_info.accept_block_votes.add_unique(AuthorityIndex::new_for_test(i), committee);
            }
        ...
        // A quorum of accept votes and block exists.
        {
            ...
            vote_info.accept_block_votes.add_unique(AuthorityIndex::new_for_test(4), committee);
        ...
        // A quorum of accept and reject votes.
        {
            ...
            let block = VerifiedBlock::new_for_test(
                TestBlock::new(1, 1).set_transactions(vec![Transaction::new(vec![4; 8]); 7]).build(),
            );
            vote_info.block = Some(block.clone());
            ...
            for reject_tx_idx in 3 .. 8 {
                vote_info.reject_txn_votes.insert(reject_tx_idx, StakeAggregator::new());
        ...
        // A transaction in the block is neither rejected nor certified.
        {
            ...
            let block = VerifiedBlock::new_for_test(
                TestBlock::new(1, 1).set_transactions(vec![Transaction::new(vec![4; 8]); 6]).build(),
            );
            vote_info.block = Some(block.clone());
            // Add 5 accept votes which form a quorum.
            for i in 0 .. 5 {
            ...
            vote_info.reject_txn_votes.get_mut(&5).unwrap().add_unique(AuthorityIndex::new_for_test(4), committee);
    ...
    async fn test_certify_basic() {
        ...
        let certified_blocks = certifier.add_voted_blocks(round_1_blocks.iter().map(|b| (b.clone(), vec![])).collect());
        ...
        let transactions = (0 .. 4).map(|_| Transaction::new(vec![0_u8; 16])).collect::<Vec<_>>();
        let ancestors = round_1_blocks
            .iter()
            .filter_map(|b| if b.author().value() < 3 { Some(b.reference()) } else { None })
            .collect::<Vec<_>>();
        for author in 0 .. 3 {
            let mut block =
                TestBlock::new(2, author).set_ancestors(ancestors.clone()).set_transactions(transactions.clone());
            let mut votes = vec![];
            for i in 0 .. (3 - author) {
                let j = author + i;
                if j == 0 {
        ...
        let mut certifier = CertifierState::new(context.clone());
        let certified_blocks = certifier.add_voted_blocks(all_blocks.iter().map(|b| (b.clone(), vec![])).collect());
        assert!(certified_blocks.is_empty());
        ...
        for author in 0 .. 4 {
            let block =
                TestBlock::new(3, author).set_ancestors(ancestors.clone()).set_transactions(transactions.clone());
        ...
        let mut certified_blocks = certifier.add_voted_blocks(vec![(proposed_block.clone(), vec![])]);
        certified_blocks.extend(certifier.add_proposed_block(proposed_block));
        assert_eq!(
            certified_blocks.len(),
            2,
            "Certified blocks {:?}",
            certified_blocks.iter().map(|b| b.block.reference().to_string()).join(",")
        );
        assert_eq!(certified_blocks[0].block.reference(), round_1_blocks[0].reference());
        assert!(certified_blocks[0].rejected.is_empty());
        assert_eq!(certified_blocks[1].block.reference(), round_1_blocks[2].reference());
    ...
    async fn test_certify_randomized() {
        ...
        let num_rounds = 50;
        let mut dag_builder = DagBuilder::new(context.clone());
        dag_builder.layers(1 ..= num_rounds).min_ancestor_links(false, None).build();
        ...
        for _ in 0 .. 100 {
            ...
            let mut actual_certified_blocks =
                certifier.add_voted_blocks(all_blocks.iter().map(|b| (b.clone(), vec![])).collect());
            actual_certified_blocks.sort_by_key(|b| b.block.reference());

            // Ensure the certified blocks are the expected ones.
            assert_eq!(actual_certified_blocks.len(), expected_certified_blocks.len());
            for (actual, expected) in actual_certified_blocks.iter().zip(expected_certified_blocks.iter()) {
