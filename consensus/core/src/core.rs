...
impl Core {
    ...
    fn recover(mut self) -> Self {
        ...
        let max_ancestor_timestamp = ancestor_blocks.iter().fold(0, |ts, (b, _)| ts.max(b.timestamp_ms()));
        ...
        if self.context.protocol_config.consensus_median_based_commit_timestamp() {
            ...
            warn!("Waiting for {} ms while recovering ancestors from storage", wait_ms);
    ...
    pub(crate) fn add_blocks(&mut self, blocks: Vec<VerifiedBlock>) -> ConsensusResult<BTreeSet<BlockRef>> {
        ...
        if !accepted_blocks.is_empty() {
            debug!("Accepted blocks: {}", accepted_blocks.iter().map(|b| b.reference().to_string()).join(","));
...
impl CoreTextFixture {
    fn new(
        ...
        let (commit_consumer, commit_output_receiver, blocks_output_receiver) = CommitConsumer::new(0);
...
mod test {
    ...
    async fn test_core_propose_once_receiving_a_quorum() {
        ...
        let mut core_fixture =
            CoreTextFixture::new(context.clone(), vec![1, 1, 1, 1], AuthorityIndex::new_for_test(0), false);
