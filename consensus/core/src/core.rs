...
use crate::{
    ...
    block::{Block, BlockAPI, BlockV1, BlockV2, ExtendedBlock, SignedBlock, Slot, VerifiedBlock, GENESIS_ROUND},
    block_manager::BlockManager,
    commit::{CertifiedCommit, CertifiedCommits, CommitAPI, CommittedSubDag, DecidedLeader, Decision},
...
impl Core {
    pub(crate) fn new(
        ...
        let number_of_leaders = context.protocol_config.mysticeti_num_leaders_per_round().unwrap_or(1);
        let committer = UniversalCommitterBuilder::new(context.clone(), leader_schedule.clone(), dag_state.clone())
        ...
        let mut ancestor_state_manager = AncestorStateManager::new(context.clone(), dag_state.clone());
    ...
    fn recover(mut self) -> Self {
        let _s =
            self.context.metrics.node_metrics.scope_processing_time.with_label_values(&["Core::recover"]).start_timer();
        ...
        let last_proposed_block = if let Some(last_proposed_block) = self.try_propose(true).unwrap() {
            ...
            self.signals
                .new_block(ExtendedBlock { block: last_proposed_block.clone(), excluded_ancestors: vec![] })
                .unwrap();
            last_proposed_block
        };
    ...
    pub(crate) fn add_blocks(&mut self, blocks: Vec<VerifiedBlock>) -> ConsensusResult<BTreeSet<BlockRef>> {
        ...
        let _s = self
            ...
            .start_timer();
        self.context.metrics.node_metrics.core_add_blocks_batch_size.observe(blocks.len() as f64);

        let (accepted_blocks, missing_block_refs) = self.block_manager.try_accept_blocks(blocks);

        if !accepted_blocks.is_empty() {
            trace!("Accepted blocks: {}", accepted_blocks.iter().map(|b| b.reference().to_string()).join(","));
        ...
        if !missing_block_refs.is_empty() {
            trace!("Missing block refs: {}", missing_block_refs.iter().map(|b| b.to_string()).join(", "));
        }
    ...
    pub(crate) fn add_certified_commits(
        ...
        let commits =
            self.filter_new_commits(certified_commits.commits().to_vec()).expect("Certified commits validation failed");
    ...
    pub(crate) fn check_block_refs(&mut self, block_refs: Vec<BlockRef>) -> ConsensusResult<BTreeSet<BlockRef>> {
        ...
        let _s = self
            ...
            .start_timer();
        self.context.metrics.node_metrics.core_check_block_refs_batch_size.observe(block_refs.len() as f64);
    ...
    fn try_signal_new_round(&mut self) {
        ...
        self.context.metrics.node_metrics.threshold_clock_round.set(new_clock_round as i64);
    }
    ...
    pub(crate) fn new_block(&mut self, round: Round, force: bool) -> ConsensusResult<Option<VerifiedBlock>> {
        let _scope = monitored_scope("Core::new_block");
        if self.last_proposed_round() < round {
            self.context.metrics.node_metrics.leader_timeout_total.with_label_values(&[&format!("{force}")]).inc();
    ...
    fn filter_new_commits(&mut self, commits: Vec<CertifiedCommit>) -> ConsensusResult<Vec<CertifiedCommit>> {
    ...
    fn try_new_block(&mut self, force: bool) -> Option<ExtendedBlock> {
        ...
        let (ancestors, excluded_and_equivocating_ancestors) = self.smart_ancestors_to_propose(clock_round, !force);
        ...
        if ancestors.is_empty() {
            ...
            debug!("Skipping block proposal for round {} because no good ancestor is found", clock_round,);
            return None;
        }
        ...
        let excluded_ancestors =
            excluded_and_equivocating_ancestors.into_iter().take(excluded_ancestors_limit).collect();
        ...
        let leader_authority = &self.context.committee.authority(self.first_leader(quorum_round)).hostname;
        self.context.metrics.node_metrics.block_proposal_leader_wait_ms.with_label_values(&[leader_authority]).inc_by(
            Instant::now().saturating_duration_since(self.dag_state.read().threshold_clock_quorum_ts()).as_millis()
                as u64,
        );
        self.context.metrics.node_metrics.block_proposal_leader_wait_count.with_label_values(&[leader_authority]).inc();

        self.context.metrics.node_metrics.proposed_block_ancestors.observe(ancestors.len() as f64);
        for ancestor in &ancestors {
        ...
        self.context.metrics.node_metrics.proposed_block_transactions.observe(transactions.len() as f64);
        ...
        let transaction_votes = if self.context.protocol_config.mysticeti_fastpath() {
            let hard_linked_ancestors = {
                ...
                ancestors.iter().flat_map(|ancestor| dag_state.link_causal_history(ancestor.reference())).collect()
            };
            self.transaction_certifier.get_own_votes(hard_linked_ancestors)
        } else {
            vec![]
        };
        ...
        let signed_block = SignedBlock::new(block, &self.block_signer).expect("Block signing failed.");
        let serialized = signed_block.serialize().expect("Block serialization failed.");
        self.context.metrics.node_metrics.proposed_block_size.observe(serialized.len() as f64);
        ...
        if last_proposed_block.round() > 0 {
            self.context.metrics.node_metrics.block_proposal_interval.observe(
                Duration::from_millis(verified_block.timestamp_ms().saturating_sub(last_proposed_block.timestamp_ms()))
                    .as_secs_f64(),
            );
        }

        // Accept the block into BlockManager and DagState.
        let (accepted_blocks, missing) = self.block_manager.try_accept_blocks(vec![verified_block.clone()]);
        ...
        self.context.metrics.node_metrics.proposed_blocks.with_label_values(&[&force.to_string()]).inc();
    ...
    fn try_commit(&mut self, mut certified_commits: Vec<CertifiedCommit>) -> ConsensusResult<Vec<CommittedSubDag>> {
        let _s = self
            .context
            .metrics
            .node_metrics
        ...
        loop {
            ...
            if commits_until_update == 0 {
                ...
                tracing::info!("Leader schedule change triggered at commit index {last_commit_index}");

                self.leader_schedule.update_leader_schedule_v2(&self.dag_state);

                let propagation_scores = self.leader_schedule.leader_swap_table.read().reputation_scores.clone();
                self.ancestor_state_manager.set_propagation_scores(propagation_scores);

                commits_until_update = self.leader_schedule.commits_until_leader_schedule_update(self.dag_state.clone());
            ...
            let (certified_leaders, decided_certified_commits): (Vec<DecidedLeader>, Vec<CertifiedCommit>) =
                self.try_select_certified_leaders(&mut certified_commits, commits_until_update).into_iter().unzip();
            ...
            let blocks = decided_certified_commits.iter().flat_map(|c| c.blocks()).cloned().collect::<Vec<_>>();
            ...
            self.context.metrics.node_metrics.last_decided_leader_round.set(self.last_decided_leader.round as i64);

            let sequenced_leaders =
                decided_leaders.into_iter().filter_map(|leader| leader.into_committed_block()).collect::<Vec<_>>();
        ...
        let committed_block_refs = committed_sub_dags
            ...
            .filter_map(|block| (block.author() == self.context.own_index).then_some(block.reference()))
            .collect::<Vec<_>>();
        self.transaction_consumer.notify_own_blocks_status(committed_block_refs, self.dag_state.read().gc_round());
    ...
    pub(crate) fn should_propose(&self) -> bool {
        ...
        if !self.subscriber_exists {
            ...
            core_skipped_proposals.with_label_values(&["no_subscriber"]).inc();
            return false;
        }

        if self.propagation_delay > self.context.parameters.propagation_delay_stop_proposal_threshold {
            debug!(
                "Skip proposing for round {clock_round}, high propagation delay {} > {}.",
                self.propagation_delay, self.context.parameters.propagation_delay_stop_proposal_threshold
            );
            core_skipped_proposals.with_label_values(&["high_propagation_delay"]).inc();
            return false;
        }
        ...
        let Some(last_known_proposed_round) = self.last_known_proposed_round else {
            ...
            core_skipped_proposals.with_label_values(&["no_last_known_proposed_round"]).inc();
            return false;
        };
        if clock_round <= last_known_proposed_round {
            ...
            core_skipped_proposals.with_label_values(&["higher_last_known_proposed_round"]).inc();
            return false;
        }
        ...
        let sequenced_leaders = to_commit
            ...
            .map(|commit| {
                ...
                assert_eq!(
                    leader.reference(),
                    commit.leader(),
                    "Last block of the committed sub dag should have the same digest as the leader of the commit"
                );
    ...
    fn smart_ancestors_to_propose(
        ...
        let _s =
            node_metrics.scope_processing_time.with_label_values(&["Core::smart_ancestors_to_propose"]).start_timer();
        ...
        for (score, ancestor) in excluded_ancestors.iter() {
            ...
            let last_included_round =
                self.last_included_ancestors[excluded_author].map(|block_ref| block_ref.round).unwrap_or(GENESIS_ROUND);
            ...
            if last_included_round >= accepted_low_quorum_round {
                ...
                node_metrics.excluded_proposal_ancestors_count_by_authority.with_label_values(&[block_hostname]).inc();
                continue;
            }
    ...
    fn leaders(&self, round: Round) -> Vec<Slot> {
        self.committer.get_leaders(round).into_iter().map(|authority_index| Slot::new(round, authority_index)).collect()
    }
...
impl CoreSignals {
    pub fn new(context: Arc<Context>) -> (Self, CoreSignalsReceivers) {
        ...
        let (tx_block_broadcast, rx_block_broadcast) =
            broadcast::channel::<ExtendedBlock>(context.parameters.dag_state_cached_rounds as usize);
    ...
    pub(crate) fn new_block(&self, extended_block: ExtendedBlock) -> ConsensusResult<()> {
        ...
        if self.context.committee.size() > 1 {
            ...
            debug!("Did not broadcast block {extended_block:?} to receivers as committee size is <= 1");
        }
        Ok(())
    }
...
#[cfg(test)]
pub(crate) async fn create_cores(context: Context, authorities: Vec<Stake>) -> Vec<CoreTextFixture> {
    let mut cores = Vec::new();

    for index in 0 .. authorities.len() {
        let own_index = AuthorityIndex::new_for_test(index as u32);
        let core = CoreTextFixture::new(context.clone(), authorities.clone(), own_index, false).await;
        cores.push(core);
    }
    cores
}
...
impl CoreTextFixture {
    async fn new(
        ...
        let leader_schedule =
            Arc::new(LeaderSchedule::from_store(context.clone(), dag_state.clone()).with_num_commits_per_schedule(10));
        let (_transaction_client, tx_receiver) = TransactionClient::new(context.clone());
        let transaction_consumer = TransactionConsumer::new(tx_receiver, context.clone());
        let (blocks_sender, _blocks_receiver) =
            mysten_metrics::monitored_mpsc::unbounded_channel("consensus_block_output");
        let transaction_certifier = TransactionCertifier::new(context.clone(), dag_state.clone(), blocks_sender);
        ...
        let (commit_consumer, commit_output_receiver, blocks_output_receiver) = CommitConsumerArgs::new(0, 0);
    ...
    pub(crate) fn add_blocks(&mut self, blocks: Vec<VerifiedBlock>) -> ConsensusResult<BTreeSet<BlockRef>> {
        self.transaction_certifier.add_voted_blocks(blocks.iter().map(|b| (b.clone(), vec![])).collect());
...
mod test {
    ...
    async fn test_core_recover_from_store_for_full_round() {
        ...
        for round in 1 ..= 4 {
            ...
            for (index, _authority) in context.committee.authorities() {
                ...
                if round == 1 && index == context.own_index {
                    let subscription = transaction_consumer.subscribe_for_block_status_testing(block.reference());
        ...
        // write them in store
        store.write(WriteBatch::default().blocks(all_blocks)).expect("Storage error");
        ...
        let (blocks_sender, _blocks_receiver) = monitored_mpsc::unbounded_channel("consensus_block_output");
        let transaction_certifier = TransactionCertifier::new(context.clone(), dag_state.clone(), blocks_sender);
        ...
        let proposed_block = block_receiver.recv().await.expect("A block should have been created");
        ...
        let last_commit = store.read_last_commit().unwrap().expect("last commit should be set");
    ...
    async fn test_core_propose_after_genesis() {
        ...
        loop {
            ...
            index += 1;
            let _w = transaction_client.submit_no_wait(vec![transaction]).await.unwrap();
        ...
        let extended_block = block_receiver.recv().await.expect("A new block should have been created");
    ...
    async fn test_core_propose_once_receiving_a_quorum() {
        ...
        let mut core_fixture =
            CoreTextFixture::new(context.clone(), vec![1, 1, 1, 1], AuthorityIndex::new_for_test(0), false).await;
    ...
    async fn test_commit_and_notify_for_block_status() {
        ...
        // write them in store
        store.write(WriteBatch::default().blocks(dag_builder.blocks(1 ..= 8))).expect("Storage error");
    ...
    async fn test_core_set_min_propose_round() {
        ...
        assert_eq!(core.last_proposed_round(), GENESIS_ROUND, "No block should have been created other than genesis");
        ...
        core.round_tracker.write().update_from_probe(
            vec![vec![10, 10, 10, 10], vec![10, 10, 10, 10], vec![10, 10, 10, 10], vec![10, 10, 10, 10]],
            vec![vec![10, 10, 10, 10], vec![10, 10, 10, 10], vec![10, 10, 10, 10], vec![10, 10, 10, 10]],
        );
    ...
    async fn test_core_try_new_block_leader_timeout() {
                ...
                if let Some(r) = last_round_blocks.first().map(|b| b.round()) {
                    ...
                    if core_fixture.core.last_proposed_round() == r {
                        ...
                        core_fixture
                            ...
                            .try_propose(true)
                            .unwrap()
                            .unwrap_or_else(|| panic!("Block should have been proposed for round {}", round));
                    }
                }
        ...
        for core_fixture in cores.iter_mut() {
            ...
            let last_commit = core_fixture.store.read_last_commit().unwrap().expect("last commit should be set");
            // There are 1 leader rounds with rounds completed up to and including
            // round 4
            assert_eq!(last_commit.index(), 1);
            let all_stored_commits = core_fixture.store.scan_commits((0 ..= CommitIndex::MAX).into()).unwrap();
    ...
    async fn test_smart_ancestor_selection() {
        ...
        let mut builder = DagBuilder::new(context.clone());
        builder.layers(1 ..= 12).authorities(vec![AuthorityIndex::new_for_test(1)]).skip_block().build();
        ...
        builder.layers(13 ..= 14).authorities(vec![AuthorityIndex::new_for_test(0)]).skip_block().build();
        ...
        let authority_1_excluded_block_reference =
            blocks.iter().find(|block| block.author() == AuthorityIndex::new_for_test(1)).unwrap().reference();
        // Wait for min round delay to allow blocks to be proposed.
        sleep(context.parameters.min_round_delay).await;
        ...
        builder
            ...
            .build();
        let blocks = builder.blocks(15 ..= 15);
        let round_15_ancestors: Vec<BlockRef> =
            blocks.iter().filter(|block| block.round() == 15).map(|block| block.reference()).collect();
        ...
        let extended_block = loop {
            let extended_block =
                tokio::time::timeout(Duration::from_secs(1), block_receiver.recv()).await.unwrap().unwrap();
        ...
        assert_eq!(extended_block.excluded_ancestors[0], authority_1_excluded_block_reference);
        ...
        // Check that a new block has been proposed & signaled.
        let extended_block = tokio::time::timeout(Duration::from_secs(1), block_receiver.recv()).await.unwrap().unwrap();
        ...
        // unlock
        builder.layers(17 ..= 22).authorities(vec![AuthorityIndex::new_for_test(0)]).skip_block().build();
    ...
    async fn test_excluded_ancestor_limit() {
        ...
        builder.layer(4).authorities(vec![AuthorityIndex::new_for_test(1)]).equivocate(9).build();
    ...
    async fn test_core_set_propagation_delay_per_authority() {
        ...
        round_tracker.write().update_from_probe(
            vec![vec![0, 0, 0, 0], vec![0, 0, 0, 0], vec![0, 0, 0, 0], vec![0, 0, 0, 0]],
            vec![vec![0, 0, 0, 0], vec![0, 0, 0, 0], vec![0, 0, 0, 0], vec![0, 0, 0, 0]],
        );
        ...
        round_tracker.write().update_from_probe(
            vec![vec![1000, 1000, 1000, 1000], vec![1000, 1000, 1000, 1000], vec![1000, 1000, 1000, 1000], vec![
                1000, 1000, 1000, 1000,
            ]],
            vec![vec![1000, 1000, 1000, 1000], vec![1000, 1000, 1000, 1000], vec![1000, 1000, 1000, 1000], vec![
                1000, 1000, 1000, 1000,
    ...
    async fn test_leader_schedule_change() {
        ...
        for round in 1 ..= 30 {
            ...
            for core_fixture in &mut cores {
                ...
                let new_round =
                    receive(Duration::from_secs(1), core_fixture.signal_receivers.new_round_receiver()).await;
                assert_eq!(new_round, round);

                // Check that a new block has been proposed.
                let extended_block = tokio::time::timeout(Duration::from_secs(1), core_fixture.block_receiver.recv())
                    .await
                    .unwrap()
                    .unwrap();
                assert_eq!(extended_block.block.round(), round);
                assert_eq!(extended_block.block.author(), core_fixture.core.context.own_index);
                ...
                assert_eq!(block.ancestors().len(), core_fixture.core.context.committee.size());
                for ancestor in block.ancestors() {
                    if block.round() > 1 {
                        // don't bother with round 1 block which just contains the genesis blocks.
                        assert!(
                            last_round_blocks.iter().any(|block| block.reference() == *ancestor),
        ...
        for core_fixture in cores {
            ...
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().bad_nodes.len(), 1);
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().good_nodes.len(), 1);
            let expected_reputation_scores = ReputationScores::new((11 ..= 20).into(), vec![29, 29, 29, 29]);
            assert_eq!(
                core_fixture.core.leader_schedule.leader_swap_table.read().reputation_scores,
    ...
    async fn test_filter_new_commits() {
        ...
        let certified_commits = sub_dags_and_commits.iter().take(4).map(|(_, c)| c).cloned().collect::<Vec<_>>();
        assert!(
            certified_commits.last().unwrap().index() <= committed_sub_dags.last().unwrap().commit_ref.index,
        ...
        // Highest certified commit should be for leader of round 4.
        let certified_commits = sub_dags_and_commits.iter().take(5).map(|(_, c)| c.clone()).collect::<Vec<_>>();
        ...
        // Highest certified commit should be for leader of round 4.
        let certified_commits = sub_dags_and_commits.iter().skip(5).take(1).map(|(_, c)| c.clone()).collect::<Vec<_>>();

        let err = core.filter_new_commits(certified_commits.clone()).unwrap_err();
        match err {
            ConsensusError::UnexpectedCertifiedCommitIndex { expected_commit_index: 5, commit_index: 6 } => (),
    ...
    async fn test_add_certified_commits() {
        ...
        let certified_commits = sub_dags_and_commits.iter().skip(3).take(5).map(|(_, c)| c.clone()).collect::<Vec<_>>();
        ...
        core.add_certified_commits(CertifiedCommits::new(certified_commits.clone(), vec![])).expect("Should not fail");
    ...
    async fn try_commit_with_certified_commits_gced_blocks() {
        ...
        let (_sub_dags, certified_commits): (Vec<_>, Vec<_>) =
            dag_builder.get_sub_dag_and_certified_commits(1 ..= 5).into_iter().unzip();
    ...
    async fn parameterized_test_commit_on_leader_schedule_change_boundary(num_leaders_per_round: Option<usize>) {
        telemetry_subscribers::init_for_testing();
        let default_params = Parameters::default();

        let (mut context, _) = Context::new_for_test(6);
        context.protocol_config.set_mysticeti_num_leaders_per_round_for_testing(num_leaders_per_round);
        // create the cores and their signals for all the authorities
        ...
        for core_fixture in cores {
            ...
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().bad_nodes.len(), 1);
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().good_nodes.len(), 1);
            let expected_reputation_scores = ReputationScores::new((21 ..= 30).into(), vec![43, 43, 43, 43, 43, 43]);
            assert_eq!(
                core_fixture.core.leader_schedule.leader_swap_table.read().reputation_scores,
                expected_reputation_scores
            );
    ...
    async fn try_select_certified_leaders() {
        ...
        let mut certified_commits = sub_dags_and_commits.into_iter().map(|(_, commit)| commit).collect::<Vec<_>>();
