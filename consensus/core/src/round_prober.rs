...
impl<C: NetworkClient> RoundProber<C> {
    ...
    pub(crate) async fn probe(&self) -> Round {
        ...
        self.round_tracker.write().update_from_probe(highest_accepted_rounds, highest_received_rounds);
        let propagation_delay = self.round_tracker.read().calculate_propagation_delay(last_proposed_round);
