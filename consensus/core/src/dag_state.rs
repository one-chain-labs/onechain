...
impl DagState {
    ...
    fn flush_internal(&mut self, finalized_commit_index: CommitIndex, ensure_exists: bool) {
        ...
        let finalized_commit_info = std::mem::replace(&mut self.commit_info_to_write, unfinalized_commit_info);
        ...
        debug!(
            ...
            finalized_commit_info.iter().map(|(commit_ref, _)| commit_ref.to_string()).join(","),
        );
        self.store
            .write(WriteBatch::new(blocks, finalized_commits, finalized_commit_info))
...
mod test {
    ...
    async fn test_flush_and_recovery() {
        ...
        assert_eq!(store_commits.last().unwrap().index(), LAST_FINALIZED_COMMIT_INDEX);
        assert_eq!(store_commits.last().unwrap().round(), LAST_FINALIZED_COMMIT_ROUND);
    ...
    async fn test_accept_block_not_panics_when_timestamp_is_ahead_and_median_timestamp() {
        // GIVEN
        let (mut context, _) = Context::new_for_test(4);
        context.protocol_config.set_consensus_median_based_commit_timestamp_for_testing(true);
