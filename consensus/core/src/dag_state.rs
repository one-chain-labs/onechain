...
impl DagState {
    ...
    pub(crate) fn has_been_included(&self, block_ref: &BlockRef) -> bool {
        self.recent_blocks
            .get(block_ref)
            .unwrap_or_else(|| {
                panic!("Attempted to query for inclusion status for a block not in cached data {}", block_ref)
            })
            .included
    }
...
mod test {
    ...
    async fn test_last_finalized_commit() {
        ...
        let stored_rejected_transactions = store.read_rejected_transactions(commit_ref).unwrap().unwrap();
