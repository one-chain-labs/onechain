...
trait ManagerBuilder {
    fn build(&self, context: Arc<Context>, network_keypair: NetworkKeyPair) -> impl NetworkManager<Mutex<TestService>>;
}
...
fn block_for_round(round: Round) -> ExtendedSerializedBlock {
    ExtendedSerializedBlock { block: Bytes::from(vec![round as u8; 16]), excluded_ancestors: vec![] }
}
...
fn service_with_own_blocks() -> Arc<Mutex<TestService>> {
    let service = Arc::new(Mutex::new(TestService::new()));
    {
        ...
        let own_blocks = (0 ..= 100u8).map(|i| block_for_round(i as Round)).collect::<Vec<_>>();
...
#[rstest]
#[tokio::test]
async fn send_and_receive_blocks_with_auth(#[values(TonicManagerBuilder {})] manager_builder: impl ManagerBuilder) {
    let (context, keys) = Context::new_for_test(4);

    let context_0 = Arc::new(context.clone().with_authority_index(context.committee.to_authority_index(0).unwrap()));
    ...
    let context_1 = Arc::new(context.clone().with_authority_index(context.committee.to_authority_index(1).unwrap()));
    ...
    client_0
        .send_block(context.committee.to_authority_index(1).unwrap(), &test_block_0, Duration::from_secs(5))
        .await
        .unwrap();
    let test_block_1 = VerifiedBlock::new_for_test(TestBlock::new(9, 1).build());
    client_1
        .send_block(context.committee.to_authority_index(0).unwrap(), &test_block_1, Duration::from_secs(5))
        .await
        .unwrap();

    assert_eq!(service_0.lock().handle_send_block.len(), 1);
    assert_eq!(service_0.lock().handle_send_block[0].0.value(), 1);
    assert_eq!(service_0.lock().handle_send_block[0].1, ExtendedSerializedBlock {
    ...
    assert_eq!(service_1.lock().handle_send_block[0].1, ExtendedSerializedBlock {
    ...
    let context_4 = Arc::new(context_4.clone().with_authority_index(context_4.committee.to_authority_index(4).unwrap()));
    ...
    assert!(client_4
        .send_block(context.committee.to_authority_index(0).unwrap(), &test_block_2, Duration::from_secs(5),)
        .await
        .is_err());
    let test_block_3 = VerifiedBlock::new_for_test(TestBlock::new(9, 3).build());
    assert!(client_4
        .send_block(context.committee.to_authority_index(1).unwrap(), &test_block_3, Duration::from_secs(5),)
        .await
        .is_err());
}
...
async fn subscribe_and_receive_blocks(
    ...
    let receive_stream_0 = client_0
        .subscribe_blocks(context_0.committee.to_authority_index(1).unwrap(), client_0_round, Duration::from_secs(5))
        .await
        .unwrap();
    ...
    let mut receive_stream_1 = client_1
        .subscribe_blocks(context_1.committee.to_authority_index(0).unwrap(), client_1_round, Duration::from_secs(5))
        .await
        .unwrap();
