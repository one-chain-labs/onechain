    ...
    N: NetworkManager<AuthorityService<ChannelCoreThreadDispatcher>>,
{
    pub(crate) async fn start(
        ...
        info!(
            ...
            own_index, own_hostname, protocol_config.version, epoch_start_timestamp_ms, boot_counter
        );
        info!(
        ...
        context.metrics.node_metrics.protocol_version.set(context.protocol_config.version.as_u64() as i64);
        ...
        let transaction_certifier =
            TransactionCertifier::new(context.clone(), dag_state.clone(), commit_consumer.block_sender.clone());
        transaction_certifier.recover(block_verifier.as_ref(), commit_consumer.last_processed_commit_index);
...
mod tests {
    ...
    use mysten_metrics::{monitored_mpsc::UnboundedReceiver, RegistryService};
    use prometheus::Registry;
    use rstest::rstest;
    ...
    async fn test_amnesia_recovery_success(#[values(5, 10)] gc_depth: u32) {
        ...
        'outer: while let Some(result) = timeout(Duration::from_secs(10), commit_receivers[index_1].recv())
            .await
    ...
    async fn make_authority(
        ...
        boot_counter: u64,
        protocol_config: ProtocolConfig,
    ) -> (ConsensusAuthority, UnboundedReceiver<CommittedSubDag>, UnboundedReceiver<CertifiedBlocksOutput>) {
        let registry = Registry::new();
