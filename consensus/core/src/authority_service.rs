...
impl<C: CoreThreadDispatcher> AuthorityService<C> {
    ...
    fn parse_excluded_ancestors(
        ...
        let excluded_ancestors = excluded_ancestors
            .into_iter()
            .map(|serialized| {
                let block_ref: BlockRef = bcs::from_bytes(&serialized).map_err(ConsensusError::MalformedBlock)?;
        ...
        for excluded_ancestor in &excluded_ancestors {
            let excluded_ancestor_hostname = &self.context.committee.authority(excluded_ancestor.author).hostname;
            self.context
                .metrics
                .node_metrics
...
impl<C: CoreThreadDispatcher> NetworkService for AuthorityService<C> {
    async fn handle_send_block(
        ...
        let (verified_block, reject_txn_votes) =
            self.block_verifier.verify_and_vote(signed_block, serialized_block.block).tap_err(|e| {
                self.context
                    .metrics
                    .node_metrics
                    .invalid_blocks
        ...
        if last_commit_index + self.context.parameters.commit_sync_batch_size * COMMIT_LAG_MULTIPLIER
            < quorum_commit_index
        {
            self.context.metrics.node_metrics.rejected_blocks.with_label_values(&["commit_lagging"]).inc();
            debug!(
        ...
        self.context.metrics.node_metrics.verified_blocks.with_label_values(&[peer_hostname]).inc();
        ...
        if self.context.protocol_config.mysticeti_fastpath() {
            self.transaction_certifier.add_voted_blocks(vec![(verified_block.clone(), reject_txn_votes)]);
        }

        // Try to accept the block into the DAG.
        let missing_ancestors =
            self.core_dispatcher.add_blocks(vec![verified_block.clone()]).await.map_err(|_| ConsensusError::Shutdown)?;
        ...
        let excluded_ancestors =
            self.parse_excluded_ancestors(peer, &verified_block, serialized_block.excluded_ancestors).tap_err(|e| {
        ...
        self.round_tracker.write().update_from_verified_block(&ExtendedBlock {
            block: verified_block,
            excluded_ancestors: excluded_ancestors.clone(),
        });

        let missing_excluded_ancestors =
            self.core_dispatcher.check_block_refs(excluded_ancestors).await.map_err(|_| ConsensusError::Shutdown)?;
    ...
    async fn handle_subscribe_blocks(&self, peer: AuthorityIndex, last_received: Round) -> ConsensusResult<BlockStream> {
        ...
        let missed_blocks = stream::iter(
            dag_state
                ...
                .into_iter()
                .map(|block| ExtendedSerializedBlock { block: block.serialized().clone(), excluded_ancestors: vec![] }),
        );

        let broadcasted_blocks =
            BroadcastedBlockStream::new(peer, self.rx_block_broadcast.resubscribe(), self.subscription_counter.clone());
    ...
    async fn handle_fetch_blocks(
        ...
        if !highest_accepted_rounds.is_empty() && highest_accepted_rounds.len() != self.context.committee.size() {
        ...
        let blocks = if !highest_accepted_rounds.is_empty() {
            block_refs.sort();
            block_refs.dedup();
            let mut blocks = self.dag_state.read().get_blocks(&block_refs).into_iter().flatten().collect::<Vec<_>>();
            ...
            if breadth_first {
                ...
                for block_ref in blocks.iter().map(|b| b.reference()) {
                    let entry = lowest_missing_rounds.entry(block_ref.author).or_insert(block_ref.round);
    ...
    async fn handle_fetch_commits(
        ...
        let inclusive_end = commit_range
            .end()
            .min(commit_range.start() + self.context.parameters.commit_sync_batch_size as CommitIndex - 1);
        ...
        'commit: while let Some(c) = commits.last() {
            ...
            if stake_aggregator.reached_threshold(&self.context.committee) {
                ...
                self.context.metrics.node_metrics.commit_sync_fetch_commits_handler_uncertified_skipped.inc();
                commits.pop();
            }
        }
        let certifier_blocks = self.store.read_blocks(&certifier_block_refs)?.into_iter().flatten().collect();
    ...
    async fn handle_get_latest_rounds(&self, _peer: AuthorityIndex) -> ConsensusResult<(Vec<Round>, Vec<Round>)> {
        ...
        let highest_accepted_rounds = blocks.into_iter().map(|(block, _)| block.round()).collect::<Vec<_>>();

        // Own blocks do not go through the core dispatcher, so they need to be set separately.
        highest_received_rounds[self.context.own_index] = highest_accepted_rounds[self.context.own_index];
...
impl SubscriptionCounter {
    fn new(context: Arc<Context>) -> Self {
        ...
        for (_, authority) in context.committee.authorities() {
            context.metrics.node_metrics.subscribed_by.with_label_values(&[authority.hostname.as_str()]).set(0);
        }
    ...
    fn increment(&self, peer: AuthorityIndex) -> Result<(), ConsensusError> {
        ...
        let peer_hostname = &self.context.committee.authority(peer).hostname;
        self.context.metrics.node_metrics.subscribed_by.with_label_values(&[peer_hostname]).set(1);
    ...
    fn decrement(&self, peer: AuthorityIndex) -> Result<(), ConsensusError> {
        ...
        if counter.subscriptions_by_authority[peer] == 0 {
            let peer_hostname = &self.context.committee.authority(peer).hostname;
            self.context.metrics.node_metrics.subscribed_by.with_label_values(&[peer_hostname]).set(0);
        }
...
struct BroadcastStream<T> {
    ...
    inner: ReusableBoxFuture<'static, (Result<T, broadcast::error::RecvError>, broadcast::Receiver<T>)>,
...
mod tests {
    ...
    impl CoreThreadDispatcher for FakeCoreThreadDispatcher {
        async fn add_blocks(&self, blocks: Vec<VerifiedBlock>) -> Result<BTreeSet<BlockRef>, CoreError> {
    ...
    async fn test_handle_send_block() {
        ...
        let transaction_certifier =
            TransactionCertifier::new(context.clone(), block_verifier.clone(), dag_state.clone(), blocks_sender);
        ...
        let input_block = VerifiedBlock::new_for_test(
            TestBlock::new(9, 0).set_timestamp_ms(now + max_drift.as_millis() as u64).build(),
        );

        let service = authority_service.clone();
        let serialized = ExtendedSerializedBlock { block: input_block.serialized().clone(), excluded_ancestors: vec![] };

        tokio::spawn({
            let service = service.clone();
            let context = context.clone();
            async move {
                service.handle_send_block(context.committee.to_authority_index(0).unwrap(), serialized).await.unwrap();
            }
        });
        ...
        // Test invalid block.
        let invalid_block = VerifiedBlock::new_for_test(TestBlock::new(10, 1000).set_timestamp_ms(10).build());
        let extended_block =
            ExtendedSerializedBlock { block: invalid_block.serialized().clone(), excluded_ancestors: vec![] };
        service.handle_send_block(context.committee.to_authority_index(0).unwrap(), extended_block).await.unwrap_err();

        // Test invalid excluded ancestors.
        let invalid_excluded_ancestors = vec![
            bcs::to_bytes(&BlockRef::new(10, AuthorityIndex::new_for_test(1000), BlockDigest::MIN)).unwrap(),
            vec![3u8; 40],
    ...
    async fn test_handle_fetch_blocks() {
        ...
        dag_builder.layers(1 ..= (NUM_ROUNDS as u32)).build().persist_layers(dag_state.clone());
        let all_blocks = dag_builder.all_blocks();

        // WHEN: Request 2 blocks from round 40, get ancestors breadth first.
        let missing_block_refs: Vec<BlockRef> = all_blocks.iter().rev().take(2).map(|b| b.reference()).collect();
        ...
        let num_missing_ancestors = blocks.keys().filter(|b| b.round == NUM_ROUNDS as Round - 1).count();
        assert_eq!(num_missing_ancestors, context.parameters.max_blocks_per_sync - missing_block_refs.len());

        // WHEN: Request 2 blocks from round 37, get ancestors depth first.
        let missing_round = NUM_ROUNDS as Round - 3;
        let missing_block_refs: Vec<BlockRef> =
            all_blocks.iter().filter(|b| b.reference().round == missing_round).map(|b| b.reference()).take(2).collect();
        ...
        let missing_block_refs: Vec<BlockRef> = all_blocks
            ...
            .collect();
        let results = authority_service
            .handle_fetch_blocks(AuthorityIndex::new_for_test(0), missing_block_refs.clone(), vec![], false)
            .await
            .unwrap();
    ...
    async fn test_handle_fetch_latest_blocks() {
        ...
        let results =
            authority_service.handle_fetch_latest_blocks(AuthorityIndex::new_for_test(1), authorities_to_request).await;
