...
impl<C: CoreThreadDispatcher> AuthorityService<C> {
    pub(crate) fn new(
        ...
        let subscription_counter = Arc::new(SubscriptionCounter::new(context.clone(), core_dispatcher.clone()));
        Self {
            context,
            block_verifier,
            commit_vote_monitor,
...
impl<C: CoreThreadDispatcher> NetworkService for AuthorityService<C> {
    async fn handle_send_block(
        ...
        let reject_txn_votes = self.block_verifier.verify_and_vote(&signed_block).tap_err(|e| {
            self.context
                .metrics
                .node_metrics
                .invalid_blocks
        ...
        if !self.context.protocol_config.consensus_median_based_commit_timestamp() {
            ...
            if forward_time_drift > self.context.parameters.max_forward_time_drift {
                self.context.metrics.node_metrics.rejected_future_blocks.with_label_values(&[peer_hostname]).inc();
                debug!(
    ...
    async fn handle_fetch_blocks(
        ...
        if !self.context.protocol_config.consensus_batched_block_sync() {
            return self.handle_fetch_blocks_old(peer, block_refs, highest_accepted_rounds).await;
        }
...
impl SubscriptionCounter {
    ...
    fn increment(&self, peer: AuthorityIndex) -> Result<(), ConsensusError> {
        ...
        if counter.count == 1 {
            self.dispatcher.set_subscriber_exists(true).map_err(|_| ConsensusError::Shutdown)?;
        }
        Ok(())
    }
    ...
    fn decrement(&self, peer: AuthorityIndex) -> Result<(), ConsensusError> {
        ...
        if counter.count == 0 {
            self.dispatcher.set_subscriber_exists(false).map_err(|_| ConsensusError::Shutdown)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_handle_fetch_blocks() {
        ...
        context.protocol_config.set_consensus_batched_block_sync_for_testing(true);
        let context = Arc::new(context);
        let block_verifier = Arc::new(crate::block_verifier::NoopBlockVerifier {});
        let commit_vote_monitor = Arc::new(CommitVoteMonitor::new(context.clone()));
        let core_dispatcher = Arc::new(FakeCoreThreadDispatcher::new());
