...
### Installing OneChain Binaries {/* {#installing-onechain-binaries} */}
...
##### Additional prerequisites by operating system {/* {#additional-prerequisites-by-operating-system} */}
...
###### All Linux prerequisites {/* {#all-linux-prerequisites} */}
...
###### libpq-dev (optional) {/* {#libpq-dev--optional-} */}
...
###### All macOS prerequisites {/* {#all-macos-prerequisites} */}
...
#### How to build OneChain binaries {/* {#how-to-build-onechain-binaries} */}
...
### Connect to OneChain {/* {#connect-to-onechain} */}
...
#### Configure OneChain client {/* {#configure-onechain-client} */}
...
### Get your OneChain address {/* {#get-your-onechain-address} */}
...
#### OneChain Wallet {/* {#onechain-wallet} */}
...
## Your First dApp {/* {#your-first-dapp} */}
...
### Write a Move Package {/* {#write-a-move-package} */}
...
[dependencies]
One = { git = "https://github.com/one-chain-labs/onechain.git", subdir = "crates/sui-framework/packages/sui-framework", rev = "main" }
...
- **[dependencies]:** Lists the other packages that your package depends on to run. By default, the `one_chain move new` command lists the `One` package on GitHub as the lone dependency.
...
### Build and Test Packages {/* {#build-and-test-packages} */}
...
Use a filter string to run only a matching subset of the unit tests. With a filter string provided, the `one_chain move test` checks the fully qualified (`<address>::<module_name>::<fn_name>`) name for a match.
...
While the `one_chain move` command does not support publishing explicitly, you can still test module initializers using the testing framework by dedicating the first transaction to executing the initializer function.
...
```sh
$ one_chain client publish --gas-budget 5000000
```
...
#### Interact with the package {/* {#interact-with-the-package} */}

Now that the package is on chain, you can call its functions to interact with the package. You can use the `one_chain client call` command to make individual calls to package functions, or you can construct more advanced blocks of transactions using the `one_chain client ptb` command. The `ptb` part of the command stands for [programmable transaction blocks](../../../concepts/transactions/prog-txn-blocks.mdx). In basic terms, PTBs allow you to group commands together in a single transaction for more efficient and cost-effective network activity.

```mermaid
flowchart TB
    my_module["my_module::new_sword(&Forge, strength, magic)"]

    one_client["one_chain client"]
...
After the transaction executes, you can check the status of the `Sword` object by using the `one_chain client objects` command again. Provided you used your address as the `<TO-ADDRESS>`, you should now see a total of four objects:
...
#### Using debug in my_module {/* {#using-debug-in-my_module} */}
...
### Shared versus Owned Objects {/* {#shared-versus-owned-objects} */}
...
For more information on the types of objects that OneChain supports, see [Object Ownership](/concepts/object-ownership.mdx).

#### Example: Escrow {/* {#example--escrow} */}
...
##### `Locked<T>` and `Key` {/* {#-locked-t---and--key-} */}
...
If you don't want to run a custom indexer, you can poll the OneChain network to query for emitted events instead. This approach typically includes a database to store the data retrieved from these calls. The [Poll events](#poll-events) section provides an example of using this method.

#### Move event structure {/* {#move-event-structure} */}
...
#### Emit events in Move {/* {#emit-events-in-move} */}
...
##### Query events with RPC {/* {#query-events-with-rpc} */}

The OneChain RPC provides a [queryEvents](/sui-api-ref#suix_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the Deepbook package on Mainnet for a specific type of event:
...
##### Query events in Rust {/* {#query-events-in-rust} */}
...
- Incorporate a [custom indexer](../advanced/custom-indexer.mdx) to take advantage of OneChain's micro-data ingestion framework.
...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](../app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the OneChain network. The database is populated from polling the network to retrieve emitted events.
...
### Access On-Chain Time {/* {#access-on-chain-time} */}
...
#### The sui::clock::Clock module {/* {#the-sui--clock--clock-module} */}
...
```sh
$ one_chain client call --package <EXAMPLE> --module 'clock' --function 'access' --args '0x6' --gas-budget <GAS-AMOUNT>
```
...
When using the [OneChain CLI](/references/cli.mdx) for the first time, it creates a local file in `~/.one/keystore` on your machine with a list of private keys (encoded as Base64 encoded `flag || 32-byte-private-key`). You can use any key to sign transactions by specifying its address. Use `one_chain keytool list` to see a list of addresses.

There are three ways to initialize a key:

```sh
# generate randomly.
one_chain client new-address ed25519
one_chain client new-address secp256k1
one_chain client new-address secp256r1

# import the 32-byte private key to keystore.
one_chain keytool import "0xd463e11c7915945e86ac2b72d88b8190cfad8ff7b48e7eb892c275a5cf0a3e82" ed25519
one_chain keytool import "0xd463e11c7915945e86ac2b72d88b8190cfad8ff7b48e7eb892c275a5cf0a3e82" secp256k1
one_chain keytool import "0xd463e11c7915945e86ac2b72d88b8190cfad8ff7b48e7eb892c275a5cf0a3e82" secp256r1
...
one_chain keytool import "$MNEMONICS" ed25519
one_chain keytool import "$MNEMONICS" secp256k1
one_chain keytool import "$MNEMONICS" secp256r1
```
...
```
```sh
$ one_chain client transfer-oct --to $ONE_ADDRESS --coin-object-id $GAS_COIN_ID --gas-budget $GAS_BUDGET --serialize-unsigned-transaction
```
```sh
$ one_chain keytool sign --address $ONE_ADDRESS --data $TX_BYTES
```
```sh
$ one_chain client execute-signed-tx --tx-bytes $TX_BYTES --signatures $SERIALIZED_SIGNATURE
```
...
#### Roles in sponsored transactions {/* {#roles-in-sponsored-transactions} */}
...
##### App-specific sponsorship {/* {#app-specific-sponsorship} */}
...
##### Wildcard sponsorship {/* {#wildcard-sponsorship} */}
...
#### Sponsored transaction flow {/* {#sponsored-transaction-flow} */}
...
##### Client equivocation risk {/* {#client-equivocation-risk} */}
...
##### Censorship risk {/* {#censorship-risk} */}
...
### Avoiding Equivocation {/* {#avoiding-equivocation} */}
...
### Working with PTBs {/* {#working-with-ptbs} */}

#### Building Programmable Transaction Blocks {/* {#building-programmable-transaction-blocks} */}

This guide explores creating a programmable transaction block (PTB) on OneChain using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the OneChain TypeScript SDK, follow the [install instructions](https://sdk.mystenlabs.com/typescript/install) on the OneChain TypeScript SDK site.
...
##### Passing transaction results as arguments {/* {#passing-transaction-results-as-arguments} */}
...
##### Use the gas coin {/* {#use-the-gas-coin} */}
...
###### dApp / Wallet integration {/* {#dapp---wallet-integration} */}
...
#### Simulating References {/* {#simulating-references} */}
