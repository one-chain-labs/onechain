...
- **[OneChain Client CLI](#onechain-client-cli):** Use the `one client` commands to interact with the OneChain network.
- **[OneChain Client PTB CLI](#onechain-client-ptb-cli):** Use the `one client ptb` command to build and execute PTBs.
- **[OneChain Console CLI](#onechain-client-console-cli):** Use `one console` to open an interactive console with the currently active network.
- **[OneChain Keytool CLI](#onechain-keytool-cli):** Use the `one keytool` commands to access cryptography utilities.
- **[OneChain Move CLI](#onechain-move-cli):** Use the `one move` commands to work with the Move programming language.
- **[OneChain Validator CLI](#onechain-validator-cli):** Use the `one validator` commands to access tools useful for OneChain validators.
...
Beginning with the OneChain [release](https://github.com/one-chain-labs/onechain/releases), the `--gas-budget` option is no longer required for CLI commands.
...
Use the `one client replay-transaction --tx-digest <TRANSACTION-DIGEST>` to re-execute a transaction locally and show the transaction effects.
...
Use `one client replay-batch --path <FILEPATH>` to replay several transactions listed in a newline-separated file. This
...
Use the `one client profile-transaction --tx-digest <TRANSACTION-DIGEST>` command to re-execute a transaction locally
...
```sh
$ cargo install --locked --git https://github.com/one-chain-labs/onechain.git --branch <BRANCH-NAME> --features tracing one
```
...
1. Open a terminal or console to the root of your local OneChain repository and navigate to the `move_tutorial` example.
    ...
    ```
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `one client envs` for a list of your defined networks and their aliases).
    ```sh
    $ one client switch --env devnet
    ```
1. Use `one move build` to build the package. You must run this command at the same level as the package manifest file ([Move.toml](https://move-book.com/concepts/manifest.html)).
    ```sh
    $ one move build
    ```
    ...
    ```
1. Use `one move test` to run the unit tests.
    ```sh
    $ one move test
    ```
    ...
    Test result: OK. Total tests: 2; passed: 2; failed: 0
     ```
1. Use the `one client verify-bytecode-meter` to check if the module passes the bytecode meter.
    ```sh
    ...
    ╰────────┴────────────┴────────────╯
    ```
1. Use `one client gas` to verify that the active address has a gas coin for paying gas.
    ```sh
    $ one client gas
    ```
    ...
    ```
1. If you need coins, use `one client faucet` (not available for Mainnet). For more information on getting gas tokens, see [Get OneChain Tokens](#request-a-oct-coin-from-faucet).
    ```sh
    $ one client faucet
    ```
1. Use `one client gas` to verify the current active address received the coins.
    ```sh
    $ one client gas
    ```
    ```sh
...
For all available commands, consult the [OneChain Client CLI docs](#onechain-client-cli). To start the OneChain Console, type `one console`, which displays some information similar to the following:
...
SuiValidatorSummary {
	...
	project_url: "https://staked.kh/",
	net_address: "/dns/one-mainnet.prod-eks-eu-west-1.staked.cloud/tcp/8080/http",
	p2p_address: "/dns/one-mainnet-udp.prod-eks-eu-west-1.staked.cloud/udp/8084",
	primary_address: "/dns/one-mainnet-udp.prod-eks-eu-west-1.staked.cloud/udp/8081",
	worker_address: "/dns/one-mainnet-udp.prod-eks-eu-west-1.staked.cloud/udp/8082",
...
<table className="w-100">
	...
	<tbody>
		...
		<tr>
			<td className="w-2/3">`one keytool generate ed25519`</td>
			<td className="w-1/3">Generate a new keypair with ED25519 scheme and save it to file</td>
		</tr>
		<tr>
      <td className="w-2/3">`one keytool import INPUT KEY_SCHEME`</td>
		...
		<tr>
			<td className="w-2/3">`one keytool update-alias OLD_ALIAS NEW_ALIAS`</td>
...
<table className="w-100">
	...
	<tbody>
		<tr>
			<td className="w-2/3">`one client active-env`</td>
			<td className="w-1/3">Get the active environment</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client envs`</td>
			<td className="w-1/3">List defined environments</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client new-env --rpc URL --alias ALIAS`</td>
			<td className="w-1/3">Create a new environment with URL and alias</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client switch --env ENV_ALIAS`</td>
			<td className="w-1/3">Switch to the given environment</td>
		</tr>
		<tr>
			<td className="w-2/3">`one genesis`</td>
			<td className="w-1/3">Bootstrap and initialize a new OneChain network</td>
		</tr>
		<tr>
			<td className="w-2/3">`one start`</td>
			<td className="w-1/3">Start the local OneChain network</td>
		</tr>
		<tr>
...
<table className="w-100">
	...
	<tbody>
		<tr>
			<td className="w-2/3">`one client call \`<br/>&nbsp;&nbsp;`--package PACKAGE \`<br/>&nbsp;&nbsp;`--module MODULE \`<br/>&nbsp;&nbsp;`--function FUNCTION`</td>
			<td className="w-1/3">Call a Move package</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client merge-coin \`<br/>&nbsp;&nbsp;`--primary-coin COIN_ID \`<br/>&nbsp;&nbsp;`--coin-to-merge COIN_ID`</td>
			<td className="w-1/3">Merge two coins</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client split-coin \`<br/>&nbsp;&nbsp;`--coin-id COIN_ID \`<br/>&nbsp;&nbsp;`--amounts 1000`</td>
			<td className="w-1/3">Split a coin into two coins: one with 1000 MIST and the rest</td>
		</tr>
		<tr>
			<td className="w-2/3">
				`one client pay-oct \`<br/>&nbsp;&nbsp;`--input-coins COIN_ID \`<br/>&nbsp;&nbsp;`--recipients ADDRESS \`<br/>&nbsp;&nbsp;`--amounts 100000000`
			</td>
			<td className="w-1/3">Transfer 0.1 OCT to an address and use the same coin for gas</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client transfer-oct \`<br/>&nbsp;&nbsp;`--coin-object-id COIN_ID \`<br/>&nbsp;&nbsp;`--to ADDRESS`</td>
...
<table className="w-100">
	...
	<tbody>
		<tr>
			<td className="w-2/3">`one client ptb --move-call p::m::f "<type>" args`</td>
			<td className="w-1/3">Call a Move function from a package and module</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client ptb --make-move-vec "<u64>" "[1000,2000]"`</td>
			<td className="w-1/3">Make a Move vector with two elements of type u64</td>
		</tr>
		<tr>
			<td className="w-2/3">
				`one client ptb \`<br/>&nbsp;&nbsp;`--split-coins gas "[1000]" \`<br/>&nbsp;&nbsp;`--assign new_coins \`<br/>&nbsp;&nbsp;`--transfer-objects
				"[new_coins]" ADDRESS`
			</td>
			<td className="w-1/3">Split a gas coin and transfer it to address</td>
		</tr>
		<tr>
			<td className="w-2/3">`one client ptb --transfer-objects "[object_id]" ADDRESS`</td>
			<td className="w-1/3">
		...
		<tr>
			<td className="w-2/3">
				`one client ptb \`<br/>&nbsp;&nbsp;`--move-call one::tx_context::sender \`<br/>&nbsp;&nbsp;`--assign sender \`<br/>&nbsp;&nbsp;`--publish "." \`<br/>&nbsp;&nbsp;`--assign
