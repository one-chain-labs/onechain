...
1.  Download the docker-compose.yaml file:
    ```sh
    $ wget https://github.com/one-chain-labs/onechain/blob/branch-name/docker/fullnode/docker-compose.yaml
    ```
1.  Download the fullnode-template.yaml file:
    ```sh
    $ wget https://github.com/one-chain-labs/onechain/raw/branch-name/crates/sui-config/data/fullnode-template.yaml
    ```
1.  Download the genesis.blob file:
    ```sh
    $ wget https://github.com/one-chain-labs/onechain-genesis/raw/main/branch-name/genesis.blob
    ```
...
You can also track the balance for an address by subscribing to all of the events emitted from it. Use a filter to include only the events related to OCT coins, such as when the address acquires a coin or pays for a gas fee.
...
## OCT Balance transfer {#sui-balance-transfer}

To transfer a specific amount of SUI between addresses, you need a OCT token object with that specific value. In Sui, everything is an object, including OCT tokens. The amount of SUI in each OCT token object varies. For example, an address could own 3 OCT tokens with different values: one of 0.1 SUI, a second of 1.0 SUI, and a third with 0.005 SUI. The total balance for the address equals the sum of the values of the individual OCT token objects, in this case, 1.105 SUI.

You can merge and split OCT token objects to create token objects with specific values. To create a OCT token worth .6 SUI, split the token worth 1 SUI into two token objects worth .6 SUI and .4 SUI.

To transfer a specific amount of SUI, you need a OCT token worth that specific amount. To get a OCT token with that specific value, you might need to split or merge existing OCT tokens. Sui supports several methods to accomplish this, including some that do not require you to manually split or merge coins.
...
- [sui_transferObject](/sui-api-ref#sui_transferObject)
  Because OCT tokens are objects, you can transfer OCT tokens just like any other object. This method requires a gas token, and is useful in niche cases only.

- [sui_payAllOct](/sui-api-ref#sui_payAllOct)
  This method accepts an array of OCT token IDs. It merges all existing tokens into one, deducts the gas fee, then sends the merged token to the recipient address.
...
- [sui_payOct](/sui-api-ref#sui_payOct)
  This operation accepts an array of OCT token IDs, an array of amounts, and an array of recipient addresses.
  ...
  The benefits of this method include: no gas fees for merging or splitting tokens, and the abstracted token merge and split. The `sui_payOct` operation is a native function, so the merge and split operations are not considered Sui transactions. The gas fees for them match typical transactions on Sui.You can use this operation to split coins in your own address by setting the recipient as your own address. Note that the total value of the input coins must be greater than the total value of the amounts to send.

- [sui_pay](/sui-api-ref#sui_pay)
  This method is similar to sui_payOct, but it accepts any kind of coin or token instead of only SUI. You must include a gas token, and all of the coins or tokens must be the same type.

- [sui_transferOct](/sui-api-ref#sui_transferOct)
  This method accepts only one OCT token object and an amount to send to the recipient. It uses the same token for gas fees, so the amount to transfer must be strictly less than the value of the OCT token used.

## Signing Transactions {#signing-transactions}

Please refer to Sui Signatures for more details on signature validity requirements.

## OCT Staking {#sui-staking}

The Sui blockchain uses a Delegated Proof-of-Stake mechanism (DPoS). This allows OCT token holders to stake their OCT tokens to any validator of their choice. When someone stakes their OCT tokens, it means those tokens are locked for the entire epoch. Users can withdraw their stake at any time, but new staking requests become active only at the start of the next epoch.
...
Sui supports the following API operations related to staking. You can find the source code in the [sui_system](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-framework/packages/one-system/sources/sui_system.move) module.
...
- `request_add_stake_with_locked_coin`
  Add user stake to a validator's staking pool using a locked OCT coin.

```move
public fun request_add_stake_with_locked_coin(
   self: &mut SuiSystemState,
   stake: LockedCoin<OCT>,
   validator_address: address,
   ctx: &mut TxContext,
) {
