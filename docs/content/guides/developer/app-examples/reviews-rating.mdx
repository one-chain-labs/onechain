...
The pool of `OCT` tokens to be distributed to reviewers is stored in the `reward_pool` field, and the amount of `OCT` tokens awarded to each participant is configured in `reward` field.
...
<RelatedLink href="https://github.com/one-chain-labs/onechain/tree/main/examples/move/reviews_rating" label="Reviews Rating repository" desc="Repo with the Revies Rating codebase." />
