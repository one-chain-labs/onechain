...
The test starts with a helper function for creating an object. The object type does not matter, as long as it has the `store` ability. The test uses `Coin<OCT>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that do not need to simulate multiple transactions but still need access to a `TxContext` can use `one::tx_context::dummy` to create a test context instead.
...
- [Full source code](https://github.com/one-chain-labs/onechain/blob/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/contracts/escrow/sources/shared.move)
...
<ImportContent source="examples/trading/contracts/escrow/sources/shared.move" mode="code" dep="one::event"  />
...
- [Full source code](https://github.com/one-chain-labs/onechain/tree/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/api)
...
Before being able to publish your package to Testnet, you need Testnet OCT tokens. To get some, run the following command: 
...
The full source code of the demo bear smart contract is available at [Trading Contracts Demo directory](https://github.com/one-chain-labs/onechain/tree/main/examples/trading/contracts/demo)
