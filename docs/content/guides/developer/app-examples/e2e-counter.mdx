...
[Example source code](https://github.com/one-chain-labs/onechain/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)
...
Your smart contract is complete. You should be able to run the `one move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `one move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
