...
Then use the `USDC` type just as you would use the `OCT` type when accepting a `Coin<OCT>` parameter.

{@inject: examples/move/usdc_usage/sources/example.move#fun=buy_sword_with_usdc}

The following example demonstrates how to use the USDC stablecoin in a Move package and how it relates to using the `OCT` type as well as any generic coin types.
