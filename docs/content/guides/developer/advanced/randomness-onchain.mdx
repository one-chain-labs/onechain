...
entry fun insecure_play(r: &Random, payment: Coin<OCT>, ...) {
  ...
...
- For a complete list of the current limits defined in the Protocol Config, see [this](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-protocol-config/src/lib.rs#L731).
...
See [this](https://github.com/one-chain-labs/onechain/blob/main/examples/move/random/random_nft/sources/example.move#L117-L142) for example implementation.
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<OCT>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<OCT>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
...
- [Raffle example](https://github.com/one-chain-labs/onechain/blob/main/examples/move/random/raffles)
