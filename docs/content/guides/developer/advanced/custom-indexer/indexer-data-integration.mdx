...
For a complete working example of BYOS with [ClickHouse](https://clickhouse.com/) (a high-performance columnar database for analytics), see the [example project in the Sui repo](https://github.com/one-chain-labs/onechain/tree/main/examples/rust/clickhouse-sui-indexer).
...
When Move smart contracts execute on Sui, they can emit events using the `one::event` module. These events are stored in checkpoints as **BCS-serialized bytes** that your indexer needs to deserialize to extract meaningful data.
