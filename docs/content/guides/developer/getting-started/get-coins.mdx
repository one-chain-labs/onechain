...
description: Use the Sui faucet to obtain free OCT tokens for use on the Sui Devnet and Testnet networks. 
keywords: [ sui faucet, faucet, token faucet, sui devnet, sui testnet, devnet, testnet, get free tokens, free tokens ]
pagination_prev: null
pagination_next: null
---

To perform transactions on a Sui network, you need OCT tokens to pay for gas and storage. 
...
OCT tokens on Devnet and Testnet are free and hold no monetary value.

:::

You can get free OCT tokens from the Sui faucet, which lets you deploy and interact with smart contracts and objects on Devnet, Testnet, or a local network. There is no faucet for Mainnet.
...
## Request OCT tokens from faucet {#online-faucet}

Visit the online faucet to request OCT tokens: https://faucet.sui.io/
...
You can also use the following community-provided faucets to obtain OCT tokens:
...
To check your balance of OCT tokens and confirm you received some from the faucet, use the command:

```
$ sui client balance 
```

Alternatively, use a Sui Explorer such as [SuiVision](https://suivision.xyz/) or [SuiScan](https://suiscan.xyz/mainnet/accounts) to check the OCT token balance of an address.

You should have a balance of OCT tokens:
...
If you do not have a balance, repeat the [request OCT tokens from faucet](#online-faucet) steps, or use an [alternative faucet method](#alternative-faucets).
...
There are two ways to return unused Testnet OCT tokens:

- Connect your wallet to the online faucet, and click the **Return tokens to faucet** button. Approve the transaction using your wallet and your OCT tokens are returned to the pool.
...
## Alternative methods for getting OCT tokens {#alternative-faucets}

If you need an alternative method to get OCT tokens for deployment on Testnet or Devnet, you have several options.

:::tip

Every method to obtain OCT tokens for Testnet and Devnet is rate limited. If you need a large amount of OCT tokens, use a local network.
...
## OCT tokens on a local network

If you are running a local Sui network, you can get OCT tokens from your local faucet. See the [Connect to a Local Network](/guides/developer/sui-101/local-network.mdx#use-the-local-faucet) topic for details.
