...
- `address.coins` and `address.stakedOcts` are being deprecated in favor of `address.objects` with the corresponding `type` filter.
...
Return all `Coin<0x2::one::SUI>` objects that an address owns.
...
query {
  address(address: "0x5094652429957619e6efa79a404a6714d1126e63f551f4b6c7fb76440f8118c9") {
    objects(
      ...
      filter: { type: "0x2::coin::Coin<0x2::one::SUI>" }
    ) {
      nodes {
        address
      }
