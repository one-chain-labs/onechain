...
```
$ git clone \
  https://github.com/one-chain-labs/onechain-stack-hello-world.git
...
[Learn more](https://github.com/one-chain-labs/onechain/blob/main/external-crates/move/move-execution/v1/crates/move-bytecode-verifier/README.md) about the Move Bytecode Verifier.
...
Then, check your balance of OCT tokens to confirm you have enough to publish to Testnet:
...
When you publish a Move package to the network, the network uploads and stores the bytecode as a Move package with a unique package ID and version number. The network consumes OCT tokens as gas and processes the transaction on-chain.
