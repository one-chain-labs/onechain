...
keywords: [ integrate sui, integrate with OCT token, sui tokens, SUI, integrate sui into exchange, cryptocurrency exchange, decentralized exchange, dex, integrate with dex, sui balance transfer, staking functions, track balance, checkpoints, staking functions ]
---
...
Sui supports the following API operations related to transferring SUI between addresses:
 ...
 - [`sui_transferObject`](/sui-api-ref#sui_transferObject): Because OCT tokens are objects, you can transfer OCT tokens just like any other object. This method requires a gas token, and is useful in niche cases only.

 - [`sui_payAllSui`](/sui-api-ref#sui_payAllSui): This method accepts an array of OCT token IDs. It merges all existing tokens into one, deducts the gas fee, then sends the merged token to the recipient address.
 ...
 - [`sui_paySui`](/sui-api-ref#sui_paySui): This operation accepts an array of OCT token IDs, an array of amounts, and an array of recipient addresses.
 ...
 - [`sui_transferSui`](/sui-api-ref#sui_transferSui): This method accepts only one OCT token object and an amount to send to the recipient. It uses the same token for gas fees, so the amount to transfer must be strictly less than the value of the OCT token used.
...
Sui supports the following API operations related to staking. You can find the source code in the [sui_system](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-framework/packages/one-system/sources/sui_system.move) module.
 ...
 - `request_add_stake_with_locked_coin`: Add user stake to a validator's staking pool using a locked OCT coin.
