...
Protocol buffers define the gRPC interface. You can find the relevant `.proto` files at [sui-apis on Github](https://github.com/one-chain-labs/onechain-apis/tree/main/proto), which apart from the gRPC messages (request and response payloads) include the following services:
...
```shell
$ grpcurl -d '{ "coin_type": "0x2::one::SUI" }' <full node URL:port> sui.rpc.v2.StateService/GetCoinInfo
```
...
For Rust, consider using the [Sui-Rust-Sdk](https://github.com/one-chain-labs/onechain-rust-sdk).
...
Download all the `sui/rpc/v2` proto files from [Github](https://github.com/one-chain-labs/onechain-apis/tree/main/proto) in the same folder.
...
Assuming you have the `v2` proto files from [Github](https://github.com/one-chain-labs/onechain-apis/tree/main/proto), run:
...
- Generate client code from the official `.proto` definitions in [sui-apis](https://github.com/one-chain-labs/onechain-apis/tree/main/proto) to ensure compatibility and type safety.
