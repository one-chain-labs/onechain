...
`Bag` is a heterogeneous map, so it can hold key-value pairs of arbitrary types (they do not need to match each other). The `Bag` type does not have any type parameters for this reason. Like `Table`, `Bag` is also an object, so creating one with `one::bag::new` requires supplying a `&mut TxContext` to generate an ID.
