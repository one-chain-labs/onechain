...
module examples::dummy_rule {
    ...
    ///
    /// This example also illustrates that rules can add Coin<OCT>
...
The `TransferPolicy` module allows removing any rule at any time, as guaranteed by the constraints on the rule's configuration. This example module has no configuration and accepts a `Coin<OCT>` of any value.
...
The `one::transfer_policy` module provides public functions to access these fields: `paid()`, `item()`, and `from()`.
