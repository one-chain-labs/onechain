...
The pipeline composition, concurrency, and deployment grouping is configured via a TOML config file. A built-in [`GenerateConfig` command](https://github.com/one-chain-labs/onechain/blob/58eabc882671ceb6266473a5531253a90b22a66f/crates/sui-indexer-alt/src/main.rs#L109) is provided to output sample configuration files for different deployment setups. The configuration generated by this command includes all pipelines in a single indexer deployment.
...
<RelatedLink href="https://github.com/one-chain-labs/onechain/tree/main/crates/sui-indexer-alt" label="Sui Indexer Alt" desc="The `sui-indexer-alt` crate in the Sui repo." />
