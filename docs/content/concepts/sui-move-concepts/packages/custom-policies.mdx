...
You can change the current policy by calling one of the functions in `one::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `one::package::only_dep_upgrades` to restrict the policy to become dependency-only, calling `one::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
<ImportContent source="crates/sui-framework/packages/one-framework/sources/package.move" mode="" fun="authorize_upgrade,commit_upgrade" noComments />
...
<ImportContent source="crates/sui-framework/packages/one-framework/sources/package.move" mode="code" struct="UpgradeCap" noComments />
...
<ImportContent source="crates/sui-framework/packages/one-framework/sources/package.move" mode="code" struct="UpgradeTicket" noComments />
...
Refer to the [implementation for digest calculation](https://github.com/one-chain-labs/onechain/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/sui-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build, when passing the `--dump-bytecode-as-base64` flag:
...
<ImportContent source="crates/sui-framework/packages/one-framework/sources/package.move" mode="code" struct="UpgradeReceipt" noComments />
...
│  │ Sender: 0x65437300e280695a40df8cf524c7bca6ad62574cac3a52d3b085ad628c797241                    │
│  │ Owner: Account Address ( 0x65437300e280695a40df8cf524c7bca6ad62574cac3a52d3b085ad628c797241 ) │
│  │ ObjectType: 0x2::coin::Coin<0x2::one::SUI>                                                    │
│  │ Version: 149516399                                                                            │
│  │ Digest: F4zEiy3eS1h9V2EfrV3WXWe8bktA28sbKxzNCqtTRK2T                                          │
│  └──                                                                                             │
│ Published Objects:                                                                               │
...
- `OCT`: the location of the `sui` CLI binary.
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::one::SUI>"),
        "objectId": String("<GAS>"),
        "version": String("12"),
