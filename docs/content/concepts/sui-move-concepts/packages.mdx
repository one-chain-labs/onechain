...
You can call the Move function `one::package::make_immutable` to destroy a package’s `UpgradeCap` and make the package permanently immutable. This eliminates single-key risk but also prevents all future bug fixes and feature additions.
