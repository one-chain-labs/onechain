...
- The `commands` value is a vector of commands, `[Command]`. The possible commands are:
  ...
  - [`SplitCoins`](#splitcoins) splits off one or more coins from a single coin. It can be any `one::coin::Coin<_>` object.

  - [`MergeCoins`](#mergecoins) merges one or more coins into a single coin. Any `one::coin::Coin<_>` objects can be merged, as long as they are all of the same type.
  ...
  - [`Upgrade`](#upgrade) upgrades an existing package. The upgrade is gated by the `one::package::UpgradeCap` for that package.
...
- If the object is owned by another object, as in it was sent to an object's ID through the `TransferObjects` command or the `one::transfer::transfer` function, then use `ObjectArg::Receiving(ObjectID, SequenceNumber, ObjectDigest)`. The object data is the same as for the `ImmOrOwnedObject` case.

For pure inputs, the only data provided is the [BCS](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-framework/docs/std/bcs.md) bytes, which are deserialized to construct Move values. Not all Move values can be constructed from BCS bytes. The following types are allowed to be used with pure values:
...
The most important thing to note at this stage is the effects on the gas coin. At the beginning of execution, the maximum gas budget (in terms of `OCT`) is withdrawn from the gas coin. Any unused gas is returned to the gas coin at the end of execution, even if the coin has changed owners.
...
- `CoinArg: Argument`: Coin of type `one::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `AmountArgs: [Argument]`: `u64` values for split amounts. Taken by value (copied).

**Returns:** Vector of coins `one::coin::Coin<T>` matching the number of amounts.

**Signature:** `<T: key + store>(coin: &mut one::coin::Coin<T>, amounts: vector<u64>): vector<one::coin::Coin<T>>`
...
- `CoinArg: Argument`: Target coin of type `one::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `ToMergeArgs: [Argument]`: Coins of type `one::coin::Coin<T>` to merge. Taken by value (moved).

**Returns:** Empty result vector.

**Signature:** `<T: key + store>(coin: &mut one::coin::Coin<T>, to_merge: vector<one::coin::Coin<T>>): ()`
...
**Returns:** Single result of type `one::package::UpgradeCap` for the newly published package.
...
- `UpgradeTicket: one::package::UpgradeTicket`: Upgrade ticket generated from `one::package::UpgradeCap`. Taken by value (moved).

**Returns:** Single result of type `one::package::UpgradeReceipt` providing proof of upgrade.
...
The inputs include the friend's address, the marketplace object, and the coin split value. The commands split off the coin, call the marketplace function, send the gas coin and one object, grab your address (through `one::tx_context::sender`), and send the remaining object to yourself.

#### Initial state

```rust
Gas Coin: Coin<OCT> { id: gas_coin, balance: 1_000_000u64 }
...
```rust
Gas Coin: Coin<OCT> { id: gas_coin, balance: 500_000u64 }
```
...
**Changes:** Gas coin balance decreases to `499_900u64`. New result: `[Coin<OCT> { id: new_coin, value: 100u64 }]`
