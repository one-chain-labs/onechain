...
- The coin argument `CoinArg: Argument` must be a coin of type `one::coin::Coin<T>` where `T` is the type of the coin being split. It can be any coin type and is not limited to `OCT` coins.
...
- The coin argument `CoinArg: Argument` must be a coin of type `one::coin::Coin<T>` where `T` is the type of the coin being merged. It can be any coin type and is not limited to `OCT` coins.
