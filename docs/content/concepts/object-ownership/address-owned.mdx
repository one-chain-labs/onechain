...
Use these [transfer module](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-framework/packages/one-framework/sources/transfer.move) functions to create address-owned objects:
...
Use the `one::transfer::transfer` function if you are defining [custom transfer rules](/concepts/transfers/custom-rules.mdx) for the object. Use the `one::transfer::public_transfer` function to create an address-owned object if the object has the `store` capability.
