...
`Coin`s can be party objects, including `Coin<OCT>`. However, you can't use a party object `Coin<OCT>` for gas payment. To use a party object `Coin<OCT>` for gas, you must first transfer it back to address-owned.
