...
You can see this function use in one of the [color_object example module](https://github.com/one-chain-labs/onechain/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new (owned) `ColorObject`, then calls `public_freeze_object` to turn it into an immutable object.
