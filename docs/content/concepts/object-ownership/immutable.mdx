...
To make an object immutable, call the `public_freeze_object` function from the [transfer module](https://github.com/one-chain-labs/onechain/blob/main/crates/sui-framework/packages/one-framework/sources/transfer.move):
