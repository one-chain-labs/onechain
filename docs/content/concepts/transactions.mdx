...
- **Gas input:** An object reference pointing to the object that will be used to pay for this transaction's execution and storage. This object must be owned by the user and must be of type `one::coin::Coin<OCT>` (i.e., the Sui native currency).
...
- Object B with 2 OCT coins that belongs to John

Tom decides to send 1 OCT coin to Alice. In this case, Object A is the input to this transaction and 1 OCT coin is debited from this object. The output of the transaction is two objects: 
- Object A with 4 OCT coins that still belongs to Tom
- new created Object C with 1 OCT coin that belongs now to Alice

At the same time, John decides to send 2 OCT coins to Anna. Because the relationship between objects and transactions is written in a directed acyclic graph (DAG), and both transactions interact with different objects, this transaction executes in parallel with the transaction that sends coins from Tom to Alice. This transaction changes only the owner of Object B from John to Anna.

After receiving 2 OCT coins, Anna sent them immediately to Tom. Now Tom has 6 OCT coins (4 from Object A and 2 from Object B).

Finally, Tom sends all of his OCT coins to John. For this transaction, the input is actually two objects (Object A and Object B). Object B is destroyed, and its value is added to Object A. As a result, the transaction's output is only Object A with a value of 6 SUI.
