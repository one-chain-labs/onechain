...
- **Gas input:** A reference pointing to the object used to pay for the transaction's execution and storage. The sender address must own the object and it must be of type `one::coin::Coin<OCT>`.
...
- Object A contains 5 OCT coins and belongs to Tom.
- Object B contains 2 OCT coins and belongs to John.

Tom decides to send 1 OCT coin to Alice. In this case, Object A is the input to this transaction and 1 OCT coin is debited from this object. The output of the transaction is: 

- Object A with 4 OCT coins that still belongs to Tom.
- New Object C that contains 1 OCT coin and belongs to Alice.

At the same time, John decides to send 2 OCT coins to Anna. The output of this transaction is:

- Object B that contains 2 OCT coins and now belongs to Alice. 
...
After receiving 2 OCT coins, Anna sends them to Tom. Now Tom has 6 OCT coins, 4 from Object A and 2 from Object B.

Finally, Tom sends all of his OCT coins to John. For this transaction, the input is both Object A and Object B. Object B is destroyed, and its value is added to Object A. As a result, the transaction's output is only Object A with a value of 6 SUI.
```mermaid
flowchart LR
    id1(Object A
    fa:fa-coins 5 SUI
