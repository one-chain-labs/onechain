...
Coin creates tokens using `coin::create_currency` while Currency uses `one::coin_registry`.
...
The Coin Standard is a technical standard used by Move smart contracts for creating currency on Sui. The `one::coin` module provides the logic that defines the standard.
