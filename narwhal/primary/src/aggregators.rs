...
impl VotesAggregator {
    pub fn new(protocol_config: &ProtocolConfig, metrics: Arc<PrimaryMetrics>) -> Self {
        ...
        Self { protocol_config: protocol_config.clone(), weight: 0, votes: Vec::new(), used: HashSet::new(), metrics }
    }

    pub fn append(&mut self, vote: Vote, committee: &Committee, header: &Header) -> DagResult<Option<Certificate>> {
        ...
        if self.weight >= committee.quorum_threshold() {
            let mut cert =
                Certificate::new_unverified(&self.protocol_config, committee, header.clone(), self.votes.clone())?;
            let (_, pks) = cert.signed_by(committee);

            let certificate_digest: Digest<{ crypto::DIGEST_LENGTH }> = Digest::from(cert.digest());
            match AggregateSignature::try_from(cert.aggregated_signature().ok_or(DagError::InvalidSignature)?)
                .map_err(|_| DagError::InvalidSignature)?
                .verify_secure(&to_intent_message(certificate_digest), &pks[..])
            {
                Err(err) => {
                    warn!("Failed to verify aggregated sig on certificate: {} error: {}", certificate_digest, err);
...
impl CertificatesAggregator {
    ...
    pub fn append(&mut self, certificate: Certificate, committee: &Committee) -> Option<Vec<Certificate>> {
