    ...
    while let Some(x) = buffer.pop() {
        ...
        for parent in x.header().parents() {
            let (digest, certificate) =
                match state.dag.get(&(x.round() - 1)).and_then(|x| x.values().find(|(x, _)| x == parent)) {
                    Some(x) => x,
            ...
            skip |= state.last_committed.get(&certificate.origin()).map_or_else(|| false, |r| &certificate.round() <= r);
            if !skip {
