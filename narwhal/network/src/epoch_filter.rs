...
impl AuthorizeRequest for AllowedEpoch {
    fn authorize(&self, request: &mut Request<Bytes>) -> Result<(), Response<Bytes>> {
        ...
        let epoch = request
            .headers()
            .get(EPOCH_HEADER_KEY)
            .ok_or_else(|| Status::new_with_message(StatusCode::BadRequest, "missing epoch header").into_response())?;
        ...
        if self.allowed_epoch == *epoch {
            ...
            Err(Status::new_with_message(
                ...
                format!("request from epoch {:?} does not match current epoch {:?}", epoch, self.allowed_epoch),
            )
            .into_response())
        }
    }
...
mod tests {
    ...
    async fn authorize_request_by_epoch() {
        ...
        let response = svc.ready().await.unwrap().call(Request::new(Bytes::from("foobar"))).await.unwrap();
