...
async fn synchronize() {
    ...
    let message = WorkerSynchronizeMessage { digests: vec![digest], target: target_primary.id(), is_certified: false };

    let mut mock_server = MockWorkerToWorker::new();
    let mock_batch_response = batch.clone();
    mock_server.expect_request_batches().withf(move |request| request.body().batch_digests == vec![digest]).return_once(
        move |_| {
...
async fn synchronize_versioned_batches() {
    ...
    let message_v1 =
        WorkerSynchronizeMessage { digests: vec![digest_v1], target: target_primary.id(), is_certified: false };

    let batch_v2 = test_utils::batch(latest_protocol_config);
    let digest_v2 = batch_v2.digest();
    let message_v2 =
        WorkerSynchronizeMessage { digests: vec![digest_v2], target: target_primary.id(), is_certified: false };
...
async fn synchronize_when_batch_exists() {
    ...
    let target_primary = fixture.authorities().nth(1).unwrap();
    let message =
        WorkerSynchronizeMessage { digests: missing.clone(), target: target_primary.id(), is_certified: false };
