    ...
    pub fn reopen<Path: AsRef<std::path::Path> + Send>(
        ...
        let column_family_options = vec![
            ...
            (
                ...
                default_db_options().optimize_for_write_throughput().optimize_for_large_values_no_scan(1 << 10).options,
            ),
        ...
        let rocksdb = open_cf_opts(store_path, Some(db_options.options), metrics_conf, &column_family_options)
        ...
        let consensus_store = Arc::new(ConsensusStore::new(last_committed_map, committed_sub_dag_map));

        Self { proposer_store, vote_digest_store, certificate_store, payload_store, batch_store, consensus_store }
    }
}
