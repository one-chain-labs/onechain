...
do
    pr_body=$(gh api -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" /repos/one-chain-labs/onechain/pulls/"${pr_number}" --jq ".body")
    release_notes="${pr_body#*### Release notes}"
    echo -e "\nhttps://github.com/one-chain-labs/onechain/pull/${pr_number}: ${release_notes}"
done
