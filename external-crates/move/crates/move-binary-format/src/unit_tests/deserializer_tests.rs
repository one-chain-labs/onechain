...
fn malformed_simple_versioned_test(version: u32) {
    ...
    assert_eq!(res.expect_err("Expected bad uleb").major_status(), StatusCode::MALFORMED);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected no table count").major_status(), StatusCode::MALFORMED);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected no table header").major_status(), StatusCode::MALFORMED);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected no table content").major_status(), StatusCode::MALFORMED);
    ...
    binary.push(10); // table length
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected bad table offset").major_status(), StatusCode::BAD_HEADER_TABLE);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected bad table content").major_status(), StatusCode::MALFORMED);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected unknown table").major_status(), StatusCode::UNKNOWN_TABLE_TYPE);
    ...
    binary.resize(binary.len() + 5000, 0);
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected table offset overflow").major_status(), StatusCode::DUPLICATE_TABLE);
}
...
fn malformed_simple() {
    ...
    let binary = vec![];
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected malformed binary").major_status(), StatusCode::BAD_MAGIC);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected bad magic").major_status(), StatusCode::BAD_MAGIC);

    // only magic
    let binary = BinaryConstants::MOVE_MAGIC.to_vec();
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected malformed binary").major_status(), StatusCode::MALFORMED);
    ...
    let res = CompiledModule::deserialize_with_defaults(&binary);
    assert_eq!(res.expect_err("Expected unknown version").major_status(), StatusCode::UNKNOWN_VERSION);
...
fn deserialize_trailing_bytes() {
    ...
    let test = |bytes| {
        // ok with flag false
        CompiledModule::deserialize_with_config(bytes, &BinaryConfig::with_extraneous_bytes_check(false)).unwrap();
        // error with flag true
        let status_code =
            CompiledModule::deserialize_with_config(bytes, &BinaryConfig::with_extraneous_bytes_check(true))
                .unwrap_err()
                .major_status();
...
fn no_metadata() {
    ...
    module.metadata.push(Metadata { key: vec![], value: vec![] });
    let test2 = {
        let mut v = vec![];
        module.serialize(&mut v).unwrap();
        v
    ...
    let mut module = basic_test_module();
    module.metadata.push(Metadata { key: metadata_bytes.clone(), value: metadata_bytes.clone() });
    module.metadata.push(Metadata { key: metadata_bytes.clone(), value: metadata_bytes.clone() });
    let test2 = {
        let mut v = vec![];
        module.serialize(&mut v).unwrap();
...
fn deserialize_below_min_version() {
    ...
    let bytes = {
        let mut v = vec![];
        module.serialize_with_version(module.version, &mut v).unwrap();
        v
    };

    let res = CompiledModule::deserialize_with_config(&bytes, &BinaryConfig::legacy(VERSION_MAX, VERSION_MAX, true))
        .unwrap_err()
        .major_status();
...
fn enum_version_lie() {
    ...
    module.function_defs[0].code = Some(CodeUnit {
        ...
        code: vec![Bytecode::VariantSwitch(VariantJumpTableIndex::new(0)), Bytecode::Ret],
        jump_tables: vec![],
    });
    let mut m_bytes = {
        let mut v = vec![];
...
fn deserialize_empty_enum_fails() {
    ...
    CompiledModule::deserialize_with_config(&bin, &BinaryConfig::with_extraneous_bytes_check(true)).unwrap_err();
}
