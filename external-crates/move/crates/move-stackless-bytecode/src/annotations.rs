...
impl Data {
    fn new<T: Any + Clone>(x: T) -> Self {
        let clone_fun = Rc::new(|x: &Box<dyn Any>| -> Box<dyn Any> { Box::new(x.downcast_ref::<T>().unwrap().clone()) });
        Self { value: Box::new(x), clone_fun }
    }
}

impl Clone for Data {
    fn clone(&self) -> Self {
        Self { value: (self.clone_fun)(&self.value), clone_fun: self.clone_fun.clone() }
    }
}

impl Debug for Annotations {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        write!(f, "annotations{{{}}}", self.map.keys().map(|t| format!("{:?}", t)).join(", "))
    }
}
...
impl Annotations {
    ...
    pub fn get<T: Any>(&self) -> Option<&T> {
        let id = TypeId::of::<T>();
        self.map.get(&id).and_then(|(d, _)| d.value.downcast_ref::<T>())
    }
    ...
    pub fn get_or_default_mut<T: Any + Default + Clone>(&mut self, fixedpoint: bool) -> &mut T {
        let id = TypeId::of::<T>();
        let (data, _) = self.map.entry(id).or_insert_with(|| (Data::new(T::default()), fixedpoint));
    ...
    pub fn remove<T: Any>(&mut self) -> Option<Box<T>> {
        let id = TypeId::of::<T>();
        self.map.remove(&id).and_then(|(d, _)| d.value.downcast::<T>().ok())
    }
