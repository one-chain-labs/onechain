...
pub trait DataflowAnalysis: TransferFunctions {
    fn analyze_function(
        ...
        state_map.insert(cfg.entry_block(), BlockState { pre: initial_state.clone(), post: initial_state.clone() });
            ...
            for next_block_id in cfg.successors(block_id) {
                match state_map.get_mut(next_block_id) {
                    ...
                    None => {
                        ...
                        state_map.insert(*next_block_id, BlockState { pre: post.clone(), post: initial_state.clone() });
                        work_list.push_back(*next_block_id);
                    }
                }
            }
