...
fn generate_module_with_struct(resource: bool) -> CompiledModule {
    ...
    for i in 0..num_fields {
        module.identifiers.push(Identifier::new(format!("string{}", i)).unwrap());
        let str_pool_idx = IdentifierIndex::new(i + 1);
        fields.push(FieldDefinition { name: str_pool_idx, signature: TypeSignature(SignatureToken::Bool) });
    }
    ...
    module.datatype_handles = vec![DatatypeHandle {
        ...
        abilities: if resource { AbilitySet::EMPTY | Ability::Key | Ability::Store } else { AbilitySet::PRIMITIVES },
        type_parameters: vec![],
    }];
    module
}
...
fn create_struct_value(module: &CompiledModule) -> (AbstractValue, Vec<SignatureToken>) {
    ...
    let tokens: Vec<SignatureToken> =
        struct_def.fields().into_iter().flatten().map(|field| field.signature.0.clone()).collect();
    ...
    let abilities =
        AbilitySet::polymorphic_abilities(struct_abilities, declared_phantom_parameters, type_argument_abilities)
            .unwrap();
    (AbstractValue::new_struct(SignatureToken::Datatype(struct_def.struct_handle), abilities), tokens)
}
...
fn bytecode_pack_signature_not_satisfied() {
    ...
    let state1 = AbstractState::from_locals(module, HashMap::new(), vec![], vec![], CallGraph::new(0));
...
fn bytecode_pack() {
    let module = generate_module_with_struct(false);
    let mut state1 = AbstractState::from_locals(module, HashMap::new(), vec![], vec![], CallGraph::new(0));
    let (struct_value1, tokens) = create_struct_value(&state1.module.module);
    for token in tokens {
        let abstract_value =
            AbstractValue { token: token.clone(), abilities: abilities(&state1.module.module, &token, &[]) };
        state1.stack_push(abstract_value);
    }
    let (state2, _) = common::run_instruction(Bytecode::Pack(StructDefinitionIndex::new(0)), state1);
    let struct_value2 = state2.stack_peek(0).expect("struct not added to stack");
    assert_eq!(struct_value1, struct_value2, "stack type postcondition not met");
}
...
fn bytecode_unpack() {
    ...
    let (state2, _) = common::run_instruction(Bytecode::Unpack(StructDefinitionIndex::new(0)), state1);
    assert_eq!(state2.stack_len(), tokens.len(), "stack type postcondition not met");
}
...
fn bytecode_mutborrowfield() {
    ...
    module.field_handles.push(FieldHandle { owner: struct_def_idx, field: 0 });
    let field_handle_idx = FieldHandleIndex((module.field_handles.len() - 1) as u16);
    let field_signature = get_field_signature(&module, &module.field_handles[field_handle_idx.0 as usize]).clone();
    ...
    assert_eq!(
        state2.stack_peek(0),
        Some(AbstractValue { token: SignatureToken::MutableReference(Box::new(field_signature)), abilities }),
