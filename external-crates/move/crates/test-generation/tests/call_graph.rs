...
fn call_graph_simple_recursive_call() {
    ...
    assert!(!can_call1.iter().any(|fh| *fh == FunctionHandleIndex(0) || *fh == FunctionHandleIndex(1)));
}
...
fn call_graph_call_graph_depth() {
    ...
    assert!(call_graph.call_depth(FunctionHandleIndex(0), FunctionHandleIndex(1)).unwrap() == 3);
}
