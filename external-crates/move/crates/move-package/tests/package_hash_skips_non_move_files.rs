...
fn package_hash_skips_non_move_files() {
    ...
    let pkg1 = BuildConfig { install_dir: Some(tempdir().unwrap().path().to_path_buf()), ..Default::default() }
        .resolution_graph_for_package(path, None, &mut Vec::new())
        .unwrap();

    let dummy_path = path.join("deps_only/other_dep/sources/dummy_text");
    std::fs::File::create(&dummy_path).unwrap().write_all("hello".as_bytes()).unwrap();

    let pkg2 = BuildConfig { install_dir: Some(tempdir().unwrap().path().to_path_buf()), ..Default::default() }
        .resolution_graph_for_package(path, None, &mut Vec::new())
        .unwrap();
    ...
    for (pkg, res_pkg) in pkg1.package_table {
        ...
        assert_eq!(res_pkg.source_digest, other_res_pkg.source_digest, "source digests differ for {}", pkg)
    }
}
