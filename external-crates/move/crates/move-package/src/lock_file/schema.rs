...
impl ToolchainVersion {
    ...
    pub fn read(lock: &mut impl Read) -> Result<Option<ToolchainVersion>> {
        ...
        let Schema { move_: value } =
            toml::de::from_str::<Schema<TV>>(&contents).context("Deserializing toolchain version")?;
...
impl ManagedPackage {
    pub fn read(lock: &mut impl Read) -> Result<HashMap<String, ManagedPackage>> {
        ...
        let Lookup { env } =
            toml::de::from_str::<Lookup>(&contents).context("Deserializing managed package in environment")?;
        Ok(env)
    }
}
...
impl Header {
    ...
    fn from_str(contents: &str) -> Result<Header> {
        ...
        if header.version != VERSION {
            bail!("Lock file format mismatch, expected version {}, found {}", VERSION, header.version);
        }
...
pub(crate) fn write_prologue(file: &mut NamedTempFile, manifest_digest: String, deps_digest: String) -> Result<()> {
    writeln!(file, "# @generated by Move, please check-in and do not edit manually.\n")?;

    let prologue = toml::ser::to_string(&Schema { move_: Header { version: VERSION, manifest_digest, deps_digest } })?;
...
pub fn update_compiler_toolchain(
    ...
    let toolchain_version = toml::Value::try_from(ToolchainVersion { compiler_version, edition, flavor })?;
...
pub enum ManagedAddressUpdate {
    Published { original_id: String, chain_id: String },
...
pub fn update_managed_address(
    ...
    match managed_address_update {
        ManagedAddressUpdate::Published { original_id, chain_id } => {
