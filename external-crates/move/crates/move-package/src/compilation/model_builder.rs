...
impl ModelBuilder {
    pub fn create(resolution_graph: ResolvedGraph, model_config: ModelConfig) -> Self {
        Self { resolution_graph, model_config }
    }
    ...
    pub fn build_model(&self) -> Result<GlobalEnv> {
        ...
        let deps_source_info = self
            ...
            .filter_map(|(nm, pkg)| {
                ...
                let mut dep_source_paths = pkg.get_sources(&self.resolution_graph.build_options).unwrap();
        ...
        let (target, deps) = make_source_and_deps_for_compiler(&self.resolution_graph, &root_package, deps_source_info)?;
        ...
        let (all_targets, all_deps) = match &self.model_config.target_filter {
            Some(filter) => {
                ...
                for PackagePaths { name, paths, named_address_map } in all_targets {
                    ...
                    if !false_targets.is_empty() {
                        new_deps.push(PackagePaths { name: name.clone(), paths: false_targets, named_address_map })
                    }
                }
                (new_targets, new_deps)
            }
            None => (all_targets, all_deps.into_iter().map(|(p, _)| p).collect_vec()),
        };
