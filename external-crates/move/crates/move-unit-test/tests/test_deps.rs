...
fn test_deps_arent_tested() {
    let mut testing_config =
        UnitTestingConfig::default_with_bound(None).with_named_addresses(move_stdlib::move_stdlib_named_addresses());
    ...
    let expected_mod_id = ModuleId::new(AccountAddress::from_hex_literal("0x6").unwrap(), Identifier::new("B").unwrap());
