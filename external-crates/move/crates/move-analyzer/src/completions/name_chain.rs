...
use crate::{
    completions::utils::{call_completion_item, completion_item, mod_defs, PRIMITIVE_TYPE_COMPLETIONS},
    symbols::{
...
pub fn name_chain_completions(
    ...
    let Some(ChainInfo { chain, kind: chain_kind, inside_use }) = cursor.find_access_chain() else {
        eprintln!("no access chain");
        return (completions, completion_finalized);
    };

    let (leading_name, path_entries) = match &chain.value {
        P::NameAccessChain_::Single(entry) => (sp(entry.name.loc, P::LeadingNameAccess_::Name(entry.name)), vec![]),
        P::NameAccessChain_::Path(name_path) => {
            (name_path.root.name, name_path.entries.iter().map(|e| e.name).collect::<Vec<_>>())
        }
    };
    ...
    if leading_name.loc.contains(&cursor.loc) {
        ...
        //
        completions
            .extend(all_packages(symbols, &info).iter().map(|n| completion_item(n.as_str(), CompletionItemKind::UNIT)));
        ...
        if let P::LeadingNameAccess_::Name(_) = &leading_name.value {
            completions.extend(info.modules.keys().map(|n| completion_item(n.as_str(), CompletionItemKind::MODULE)));
            completions.extend(all_single_name_member_completions(symbols, cursor, &info.members, chain_kind));
            if matches!(chain_kind, ChainCompletionKind::Type) {
                completions.extend(PRIMITIVE_TYPE_COMPLETIONS.clone());
                completions.extend(
                    info.type_params.iter().map(|t| completion_item(t.as_str(), CompletionItemKind::TYPE_PARAMETER)),
                );
            }
        }
...
pub fn use_decl_completions(symbols: &Symbols, cursor: &CursorContext) -> (Vec<CompletionItem>, bool) {
    ...
    match use_ {
        P::Use::ModuleUse(sp!(_, mod_ident), mod_use) => {
            if mod_ident.address.loc.contains(&cursor.loc) {
                ...
                completions.extend(
                    all_packages(symbols, &info).iter().map(|n| completion_item(n.as_str(), CompletionItemKind::UNIT)),
                );
                ...
                && cursor.loc.end() <= mod_ident.module.loc().end()
            {
                ...
                for ident in pkg_mod_identifiers(symbols, &info, &mod_ident.address) {
                    completions.push(completion_item(ident.value.module.value().as_str(), CompletionItemKind::MODULE));
                }
            } else {
        ...
        P::Use::NestedModuleUses(leading_name, uses) => {
            if leading_name.loc.contains(&cursor.loc) {
                ...
                if let Some((first_name, _)) = uses.first() {
                    if cursor.loc.start() > leading_name.loc.end() && cursor.loc.end() <= first_name.loc().start() {
                        ...
                        for ident in pkg_mod_identifiers(symbols, &info, &leading_name) {
                            completions
                                .push(completion_item(ident.value.module.value().as_str(), CompletionItemKind::MODULE));
                        }
                ...
                for (mod_name, mod_use) in &uses {
                    ...
                    completions.extend(module_use_completions(symbols, cursor, &info, mod_use, &leading_name, mod_name));
                }
            }
        }
        P::Use::Fun { .. } => (), // already handled as part of name chain completion
        P::Use::Partial { package, colon_colon, opening_brace: _ } => {
...
fn datatype_completion(
    ...
    let current_mod_ident_str = expansion_mod_ident_to_map_key(&cursor.module.as_ref().unwrap().value);
    ...
    let fields_list = field_names
        .iter()
        .enumerate()
        .map(|(idx, name)| if named_fields { format!("${{{}:{}}}", idx + 1, name) } else { format!("${{{}}}", idx + 1) })
        .collect::<Vec<_>>()
        .join(separator);
...
fn module_member_completions(
    ...
    if matches!(chain_kind, CT::Function) || matches!(chain_kind, CT::All) {
        let fun_completions = mod_defs
            .functions
            .iter()
            .filter_map(|(fname, fdef)| symbols.def_info(&fdef.name_loc).map(|def_info| (fname, def_info)))
            .filter(|(_, def_info)| {
            ...
            .filter_map(|(fname, def_info)| {
                if let DefInfo::Function(_, _, fun_type, _, type_args, arg_names, arg_types, ret_type, _) = def_info {
    ...
    if matches!(chain_kind, CT::Type) || matches!(chain_kind, CT::All) {
        completions.extend(
            mod_defs
                .structs
                .iter()
                .flat_map(|(sname, member_def)| struct_completion(cursor, &mod_defs.ident, *sname, member_def)),
        );
        completions.extend(mod_defs.enums.keys().map(|ename| completion_item(ename, CompletionItemKind::ENUM)));
    }

    if matches!(chain_kind, CT::All) && same_module {
        completions.extend(mod_defs.constants.keys().map(|cname| completion_item(cname, CompletionItemKind::CONSTANT)));
    }
...
fn struct_completion(
    ...
    let MemberDef { info: MemberDefInfo::Struct { field_defs, positional }, .. } = member_def else {
...
fn single_name_member_completion(
    ...
    if mod_defs.enums.get(member_name).is_some() {
        ...
        return vec![completion_item(member_alias.as_str(), CompletionItemKind::ENUM)];
    }
    ...
    if mod_defs.constants.get(member_name).is_some() {
        ...
        return vec![completion_item(member_alias.as_str(), CompletionItemKind::CONSTANT)];
    }
...
fn is_pkg_mod_ident(mod_ident: &ModuleIdent_, leading_name: &P::LeadingNameAccess) -> bool {
    match mod_ident.address {
        ...
        Address::Numerical { name, value, name_conflict: _ } => match leading_name.value {
            P::LeadingNameAccess_::AnonymousAddress(addr) if addr == value.value => true,
            P::LeadingNameAccess_::Name(addr_name) | P::LeadingNameAccess_::GlobalAddress(addr_name)
...
fn variant_completion(
    ...
    let Some(DefInfo::Variant(_, _, _, positional, field_names, ..)) = symbols.def_info.get(&vinfo.name.loc) else {
        return vec![completion_item(vinfo.name.value.as_str(), CompletionItemKind::ENUM_MEMBER)];
    };
...
fn all_variant_completions(
    ...
    variants.iter().flat_map(|vinfo| variant_completion(symbols, cursor, &mod_defs.ident, vinfo)).collect_vec()
}
    ...
    match prev_kind {
        ChainComponentKind::Package(leading_name) => {
            for mod_ident in pkg_mod_identifiers(symbols, info, &leading_name) {
                completions.push(completion_item(mod_ident.value.module.value().as_str(), CompletionItemKind::MODULE));
            }
        }
        ChainComponentKind::Module(mod_ident) => {
            completions.extend(module_member_completions(symbols, cursor, &mod_ident, chain_kind, inside_use));
        }
        ChainComponentKind::Member(mod_ident, member_name) => {
            completions.extend(all_variant_completions(symbols, cursor, &mod_ident, member_name));
        }
    }
}
...
fn next_name_chain_component_kind(
    ...
    match prev_kind {
        ChainComponentKind::Package(leading_name) => pkg_mod_identifiers(symbols, info, &leading_name)
            .into_iter()
            .find(|mod_ident| mod_ident.value.module.value() == component_name.value)
            .map(ChainComponentKind::Module),
        ChainComponentKind::Module(mod_ident) => Some(ChainComponentKind::Member(mod_ident, component_name.value)),
...
fn completions_for_name_chain_entry(
    ...
    let ChainComponentInfo { loc: prev_loc, kind: prev_kind } = prev_info;
    ...
    if at_colon_colon || path_entries[path_index].loc.contains(&cursor.loc) {
        name_chain_entry_completions(symbols, cursor, info, prev_kind, chain_kind, inside_use, completions);
    } else {
        let component_name = path_entries[path_index];
        if let Some(next_kind) = next_name_chain_component_kind(symbols, info, prev_kind, component_name) {
            completions_for_name_chain_entry(
                symbols,
                cursor,
                info,
...
fn is_package_address(symbols: &Symbols, info: &AliasAutocompleteInfo, pkg_addr: NumericalAddress) -> bool {
...
fn is_package_name(symbols: &Symbols, info: &AliasAutocompleteInfo, pkg_name: Name) -> bool {
    ...
    symbols.file_mods.values().flatten().map(|mdef| &mdef.ident).any(|mod_ident| match &mod_ident.address {
        Address::NamedUnassigned(name) if name == &pkg_name => true,
        Address::Numerical { name: Some(name), .. } if name == &pkg_name => true,
        _ => false,
    })
}
...
fn all_packages(symbols: &Symbols, info: &AliasAutocompleteInfo) -> BTreeSet<String> {
    ...
    symbols.file_mods.values().flatten().map(|mdef| &mdef.ident).for_each(|mod_ident| match &mod_ident.address {
...
fn first_name_chain_component_kind(
    ...
    match leading_name.value {
        P::LeadingNameAccess_::Name(n) => {
            ...
            } else if let Some((mod_ident, member_name)) = info.members.iter().find_map(
                |(alias_name, mod_ident, member_name)| {
...
fn module_use_completions(
    ...
    let Some(mod_ident) =
        pkg_mod_identifiers(symbols, info, package).into_iter().find(|mod_ident| &mod_ident.value.module == mod_name)
    else {
        return completions;
    };
    ...
    match mod_use {
        ...
        MU::Members(members) => {
            if let Some((first_name, _)) = members.first() {
                if cursor.loc.start() > mod_name.loc().end() && cursor.loc.end() <= first_name.loc.start() {
        ...
        MU::Partial { colon_colon, opening_brace: _ } => {
            if let Some(colon_colon_loc) = colon_colon {
                if cursor.loc.start() >= colon_colon_loc.start() {
                    // cursor is on or past `::`
                    completions.extend(module_member_completions(
