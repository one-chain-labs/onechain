...
use lsp_types::{CompletionItem, CompletionItemKind, CompletionItemLabelDetails, InsertTextFormat, Position};
use move_compiler::{
...
pub fn dot_completions(symbols: &Symbols, use_fpath: &Path, position: &Position) -> (Vec<CompletionItem>, bool) {
    ...
    for AutocompleteMethod { method_name, target_function: (mod_ident, function_name) } in &info.methods {
        let call_completion =
            if let Some(DefInfo::Function(.., fun_type, _, type_args, arg_names, arg_types, ret_type, _)) =
