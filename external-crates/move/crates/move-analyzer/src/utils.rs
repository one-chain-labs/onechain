...
use move_compiler::{shared::files::MappedFiles, unit_test::filter_test_members::UNIT_TEST_POISON_FUN_NAME};
...
pub fn offset_to_lsp_position(files: &MappedFiles, file_hash: &FileHash, offset: ByteIndex) -> Option<Position> {
    let loc_posn = files.byte_index_to_position_opt(file_hash, offset)?;
    let result = Position { line: loc_posn.line_offset() as u32, character: loc_posn.column_offset() as u32 };
    Some(result)
}
...
pub fn loc_start_to_lsp_position_opt(files: &MappedFiles, loc: &Loc) -> Option<Position> {
    ...
    let result =
        Position { line: start_loc_posn.line_offset() as u32, character: start_loc_posn.column_offset() as u32 };
    Some(result)
}

pub fn loc_end_to_lsp_position_opt(files: &MappedFiles, loc: &Loc) -> Option<Position> {
    let end_loc_posn = files.end_position_opt(loc)?;
    let result = Position { line: end_loc_posn.line_offset() as u32, character: end_loc_posn.column_offset() as u32 };
    Some(result)
}

pub fn lsp_position_to_loc(files: &MappedFiles, file_hash: FileHash, pos: &Position) -> Option<Loc> {
