                    ...
                    Some(
                        labels
                            ...
                            .filter_map(|(lloc, lmsg)| {
                                ...
                                let lpos = Location::new(Url::from_file_path(lpath).unwrap(), Range::new(lstart, lend));
                                Some(DiagnosticRelatedInformation { location: lpos, message: lmsg.to_string() })
                            })
                            .chain(notes.iter().map(|note| {
                                ...
                                DiagnosticRelatedInformation { location: fpos, message: format!("Note: {note}") }
                            }))
                            .collect(),
                    )
                };
                lsp_diagnostics.entry(fpath.to_path_buf()).or_insert_with(Vec::new).push(Diagnostic::new(
                    range,
                    Some(severity(*s)),
                    None,
                    None,
...
pub fn lsp_empty_diagnostics(file_name_mapping: &BTreeMap<FileHash, PathBuf>) -> BTreeMap<PathBuf, Vec<Diagnostic>> {
    let mut lsp_diagnostics = BTreeMap::new();
