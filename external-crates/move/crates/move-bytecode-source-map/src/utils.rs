...
pub fn source_map_from_file(file_path: &Path) -> Result<SourceMap> {
    ...
    bcs::from_bytes::<SourceMap>(&bytes).map_err(|_| format_err!("Error deserializing into source map"))
}
...
pub fn serialize_to_json_file(map: &SourceMap, file_path: &Path) -> Result<()> {
    let json = serde_json::to_string_pretty(map).map_err(|e| format_err!("Error serializing to json: {}", e))?;
    let mut f = std::fs::File::create(file_path).map_err(|e| format_err!("Error creating file: {}", e))?;
    f.write_all(json.as_bytes()).map_err(|e| format_err!("Error writing to file: {}", e))?;
    Ok(())
}
...
pub fn deserialize_from_json(file_path: &Path) -> Result<SourceMap> {
    ...
    file.read_to_string(&mut json).map_err(|e| format_err!("Error reading file: {}", e))?;
