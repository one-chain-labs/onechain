...
use simplelog::{CombinedLogger, Config, ConfigBuilder, LevelPadding, SimpleLogger, TermLogger, TerminalMode};
...
impl Options {
    ...
    pub fn create_from_args(args: &[String]) -> anyhow::Result<Options> {
        ...
        let is_number = |s: &str| s.parse::<usize>().map(|_| ()).map_err(|_| "expected number".to_string());
        let cli = Command::new("mvp")
            .version("0.1.0")
            .about("The Move Prover")
            .author("The Diem Core Contributors")
            .arg(Arg::new("config").short('c').long("config").num_args(1).value_name("TOML_FILE").help(
            ...
            .arg(
                Arg::new("vector-theory")
                    .long("vector-theory")
                    .num_args(1)
                    .value_parser(["BoogieArray", "BoogieArrayIntern", "SmtArray", "SmtArrayExt", "SmtSeq"])
            ...
            .arg(Arg::new("cores").long("cores").num_args(1).value_name("NUMBER").value_parser(is_number).help(
            ...
            .arg(
                Arg::new("timeout")
                    ...
                    .value_parser(is_number)
                    .help(
                        "sets a timeout (in seconds) for each \
            ...
            .arg(
                Arg::new("ignore-pragma-opaque-when-possible")
                    ...
                    .action(clap::ArgAction::SetTrue)
                    .help(
                        "Ignore the \"opaque\" pragma on specs of \
            ...
            .arg(
                Arg::new("simplification-pipeline")
                    ...
                    .number_of_values(1)
                    .help(
                        "Specify one simplification pass to run on the specifications. \
                    This option May be specified multiple times to compose a pipeline",
                    ),
            )
            .arg(Arg::new("docgen").long("docgen").action(clap::ArgAction::SetTrue).help(
                "runs the documentation generator instead of the prover. \
            ...
            .arg(Arg::new("abigen").long("abigen").action(clap::ArgAction::SetTrue).help(
                "runs the ABI generator instead of the prover. \
            ...
            .arg(
                Arg::new("bench-repeat")
                    ...
                    .value_name("COUNT")
                    .value_parser(is_number)
                    .help("for benchmarking: how many times to call the backend on the verification problem"),
            )
            .arg(
                Arg::new("mutation")
                    .long("mutation")
                    .action(clap::ArgAction::SetTrue)
                    .help("Specifies to use the mutation pass"),
            )
            .arg(
            ...
            .arg(Arg::new("stable-test-output").long("stable-test-output").action(clap::ArgAction::SetTrue).help(
                "instruct the prover to produce output in diagnosis which is stable \
            ...
            .arg(
                Arg::new("unconditional-abort-as-inconsistency")
                    ...
                    .action(clap::ArgAction::SetTrue)
                    .help(
                        "treat functions that do not return (i.e., abort unconditionally) \
                    as inconsistency violations",
                    ),
            )
            .arg(Arg::new("verify-only").long("verify-only").num_args(1).value_name("FUNCTION_NAME").help(
            ...
            .arg(Arg::new("z3-trace").long("z3-trace").num_args(1).value_name("FUNCTION_NAME").help(
            ...
            .arg(Arg::new("script-reach").long("script-reach").action(clap::ArgAction::SetTrue).help(
                "For each script function which is verification target, \
            ...
            .after_help(
                "More options available via `--config file` or `--config-str str`. \
        ...
        if matches.contains_id("severity") {
            options.prover.report_severity = match matches.get_one::<String>("severity").unwrap().as_str() {
                "bug" => Severity::Bug,
        ...
        if matches.contains_id("mutation-add-sub") {
            options.prover.mutation_add_sub = matches.get_one::<String>("mutation-add-sub").unwrap().parse::<usize>()?;
        }
        if matches.contains_id("mutation-sub-add") {
            options.prover.mutation_sub_add = matches.get_one::<String>("mutation-sub-add").unwrap().parse::<usize>()?;
        }
        if matches.contains_id("mutation-mul-div") {
            options.prover.mutation_mul_div = matches.get_one::<String>("mutation-mul-div").unwrap().parse::<usize>()?;
        }
        if matches.contains_id("mutation-div-mul") {
            options.prover.mutation_div_mul = matches.get_one::<String>("mutation-div-mul").unwrap().parse::<usize>()?;
        }
        ...
        if matches.contains_id("docgen-template") {
            options.run_docgen = true;
            options.docgen.root_doc_templates =
                vec![matches.get_one::<String>("docgen-template").map(|s| s.to_string()).unwrap()]
        }
    ...
    pub fn setup_logging(&self) {
        if LOGGER_CONFIGURED.compare_exchange(false, true, Ordering::Relaxed, Ordering::Relaxed).is_err() {
            return;
        }
        let config =
            ConfigBuilder::new().set_time_level(LevelFilter::Debug).set_level_padding(LevelPadding::Off).build();
        let logger = if std::io::stderr().is_terminal() && std::io::stdout().is_terminal() {
            CombinedLogger::init(vec![TermLogger::new(self.verbosity_level, config, TerminalMode::Mixed)])
        } else {
    ...
    pub fn setup_logging_for_test(&self) {
        ...
        SimpleLogger::init(self.verbosity_level, Config::default()).expect("UnexpectedSimpleLogger failure");
    }
...
pub fn named_addresses_for_options(named_address_values: &BTreeMap<String, NumericalAddress>) -> Vec<String> {
    named_address_values.iter().map(|(name, addr)| format!("{}={}", name, addr)).collect()
}
