...
use move_core_types::{
    ...
    identifier::Identifier as MoveCoreIdentifier,
...
impl AnnotatedCompiledModule {
    pub fn module_ident(&self) -> ModuleIdent {
        ...
        sp(self.loc, ModuleIdent_::new(address, ModuleName(sp(self.module_name_loc, self.named_module.name))))
    }
    ...
    pub fn verify(&self) -> Diagnostics {
        let AnnotatedCompiledModule { loc, named_module: NamedCompiledModule { module, source_map, .. }, .. } = self;
...
impl NamedCompiledModule {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        self.module.serialize_with_version(self.module.version, &mut serialized).unwrap();
        serialized
    }
...
fn bytecode_verifier_mismatch_bug(
    ...
    let loc = match e.offsets().first() {
        Some((fdef_idx, offset)) if &location == e.location() => sm.get_code_location(*fdef_idx, *offset).unwrap_or(loc),
        _ => loc,
    };
    Diagnostics::from(vec![diag!(Bug::BytecodeVerification, (loc, format!("ICE failed bytecode verifier: {:#?}", e)),)])
}
...
fn verify_module(sm: &SourceMap, loc: Loc, cm: &F::CompiledModule) -> Diagnostics {
    match move_bytecode_verifier::verifier::verify_module_unmetered(cm) {
        ...
        Err(e) => bytecode_verifier_mismatch_bug(sm, loc, move_binary_format::errors::Location::Module(cm.self_id()), e),
    }
}
