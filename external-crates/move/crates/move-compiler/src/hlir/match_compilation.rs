...
pub(super) fn compile_match(
    ...
    let (mut initial_binders, init_subject, match_subject) = make_initial_fringe(context, subject, loc);
    ...
    let mut resolution_context =
        ResolutionContext { hlir_context: context, output_type: result_type, arms: &arms, arms_loc: loc };
...
fn make_initial_fringe(
    ...
    let subject_entry = FringeEntry { var: subject_var, ty: subject.ty.clone() };
    let subject_borrow_rhs = make_var_ref(subject_entry.clone());

    let match_entry = FringeEntry { var: match_var, ty: subject_borrow_rhs.ty.clone() };

    let subject_binder = {
        let lhs_loc = subject_loc;
        let lhs_lvalue = make_lvalue(subject_var, Mutability::Imm, subject.ty.clone());
        let binder =
            T::SequenceItem_::Bind(sp(lhs_loc, vec![lhs_lvalue]), vec![Some(subject.ty.clone())], Box::new(subject));
        sp(lhs_loc, binder)
    };
    ...
    (VecDeque::from([subject_binder, subject_borrow]), subject_entry, match_entry)
}

#[growing_stack]
fn build_match_tree(context: &mut Context, mut fringe: VecDeque<FringeEntry>, mut matrix: PatternMatrix) -> MatchTree {
    debug_print!(
    ...
    if subject.ty.value.unfold_to_builtin_type_name().is_some() {
        ...
        if context.info.is_struct(&mident, &datatype_name) {
            compile_match_struct(context, subject, tyargs, fringe, matrix, mident, datatype_name)
        } else {
            compile_variant_switch(context, subject, tyargs, fringe, matrix, mident, datatype_name)
        }
    }
}
...
fn compile_match_literal(
    ...
    for lit in lits {
        ...
        arms.insert(lit, Box::new(build_match_tree(context, fringe.clone(), inner_matrix)));
    }
    ...
    MatchTree::LiteralSwitch { subject, subject_binders, arms, default: default_result }
}
...
fn compile_match_struct(
    ...
    let (subject_binders, unpack) = if let Some((ploc, arg_types)) = matrix.first_struct_ctors() {
        ...
        let unpack =
            StructUnpack::Unpack(fringe_binders, Box::new(build_match_tree(context, inner_fringe, inner_matrix)));
        (subject_binders, unpack)
    } else {
        let (subject_binders, default_matrix) = matrix.specialize_default();
        let unpack = StructUnpack::Default(Box::new(build_match_tree(context, fringe, default_matrix)));
        (subject_binders, unpack)
    };

    MatchTree::StructUnpack { subject, subject_binders, tyargs, unpack }
}
...
fn compile_variant_switch(
    ...
    let mut unmatched_variants =
        context.info.enum_variants(&mident, &datatype_name).into_iter().collect::<BTreeSet<_>>();
    ...
    for (ctor, (ploc, arg_types)) in ctors {
        ...
        let decl_fields = context.info.enum_variant_fields(&mident, &datatype_name, &ctor).unwrap();
        let fringe_binders = context.make_imm_ref_match_binders(decl_fields, ploc, arg_types);
        let fringe_exps = make_fringe_entries(&fringe_binders);
        let inner_fringe = fringe_exps.into_iter().chain(fringe.clone()).collect();
        ...
        arms.insert(ctor, (fringe_binders, Box::new(build_match_tree(context, inner_fringe, inner_matrix))));
    }

    let (mut new_binders, default_matrix) = if unmatched_variants.is_empty() {
        let empty_pattern = PatternMatrix { tys: vec![], loc: matrix.loc, patterns: vec![] };
        (vec![], empty_pattern)
    } else {
...
fn make_fringe_entries(binders: &[(Field, Var, Type)]) -> VecDeque<FringeEntry> {
    binders.iter().map(|(_, var, ty)| FringeEntry { var: *var, ty: ty.clone() }).collect::<VecDeque<_>>()
}
...
#[growing_stack]
fn match_tree_to_exp(context: &mut ResolutionContext, init_subject: &FringeEntry, result: MatchTree) -> T::Exp {
    match result {
        ...
        MatchTree::Failure => {
            ...
            T::exp(context.output_type(), sp(context.arms_loc, T::UnannotatedExp_::UnresolvedError))
        }
        MatchTree::VariantSwitch { subject, subject_binders, tyargs, mut arms, default } => {
            let (m, e) = subject.ty.value.unfold_to_type_name().and_then(|sp!(_, name)| name.datatype_name()).unwrap();
            // Bindings in the arm are always immutable
            let bindings =
                subject_binders.into_iter().map(|(_mut, binder)| (binder, (Mutability::Imm, subject.clone()))).collect();
            ...
            for v in sorted_variants {
                if let Some((unpack_fields, next)) = arms.remove(&v) {
                    ...
                    let unpack_block =
                        make_match_variant_unpack(m, e, v, tyargs.clone(), unpack_fields, subject.clone(), rest_result);
        ...
        MatchTree::StructUnpack { subject, subject_binders, tyargs, unpack } => {
            let (m, s) = subject.ty.value.unfold_to_type_name().and_then(|sp!(_, name)| name.datatype_name()).unwrap();
            // Bindings in the arm are always immutable
            let bindings =
                subject_binders.into_iter().map(|(_mut, binder)| (binder, (Mutability::Imm, subject.clone()))).collect();
            let unpack_exp = match unpack {
                ...
                StructUnpack::Unpack(unpack_fields, next) => {
                    let rest_result = match_tree_to_exp(context, init_subject, *next);
                    make_match_struct_unpack(m, s, tyargs.clone(), unpack_fields, subject.clone(), rest_result)
                }
            };
            make_copy_bindings(bindings, unpack_exp)
        }
        MatchTree::LiteralSwitch { subject, subject_binders, mut arms, default: _ }
            if matches!(subject.ty.value.unfold_to_builtin_type_name(), Some(sp!(_, BuiltinTypeName_::Bool)))
                && arms.len() == 2 =>
        {
            // Bindings in the arm are always immutable
            let bindings =
            ...
            let true_arm = arms.remove(&sp(Loc::invalid(), Value_::Bool(true))).unwrap();
            let false_arm = arms.remove(&sp(Loc::invalid(), Value_::Bool(false))).unwrap();
            ...
            make_copy_bindings(bindings, make_if_else(lit_subject, true_arm, false_arm, result_ty))
        }
        MatchTree::LiteralSwitch { subject, subject_binders, arms: map, default } => {
            // Bindings in the arm are always immutable
            let bindings =
                subject_binders.into_iter().map(|(_mut, binder)| (binder, (Mutability::Imm, subject.clone()))).collect();
            let lit_subject = make_match_lit(subject.clone());
...
fn make_leaf(context: &mut ResolutionContext, subject: &FringeEntry, mut leaf: Vec<ArmResult>) -> T::Exp {
    ...
    let mut out_exp = make_copy_bindings(last.bindings, make_arm(context, subject.clone(), last.arm));
    let out_ty = out_exp.ty.clone();
    while let Some(arm) = leaf.pop() {
        ice_assert!(context.hlir_context.reporter, arm.guard.is_some(), arm.loc, "Expected a guard");
...
fn make_guard_exp(
    ...
    let ArmResult { loc: _, bindings, guard, arm } = arm;
    // Bindings in the guard are always immutable
    let bindings = bindings.into_iter().map(|(x, (_mut, entry))| (x, (Mutability::Imm, entry))).collect();
...
fn make_arm(context: &mut ResolutionContext, subject: FringeEntry, arm: Arm) -> T::Exp {
    ...
    make_arm_unpack(context, subject, arm.orig_pattern, &arm.rhs_binders, arm_exp)
}
    ...
    while let Some((entry, pat)) = queue.pop_front() {
        ...
        match pat.pat.value {
            TP::Variant(m, e, v, tys, fs) => {
                let Some((queue_entries, unpack)) = arm_variant_unpack(context, None, ploc, m, e, tys, v, fs, entry)
                else {
                    context.hlir_context.add_diag(ice!((ploc, "Did not build an arm unpack for a value variant")));
                    continue;
                };
            ...
            TP::Struct(m, s, tys, fs) => {
                let Some((queue_entries, unpack)) = arm_struct_unpack(context, None, ploc, m, s, tys, fs, entry) else {
                    context.hlir_context.add_diag(ice!((ploc, "Did not build an arm unpack for a value struct")));
                    continue;
                };
                for entry in queue_entries.into_iter().rev() {
                    queue.push_front(entry);
            ...
            TP::BorrowStruct(mut_, m, s, tys, fs) => {
                let Some((queue_entries, unpack)) = arm_struct_unpack(context, Some(mut_), ploc, m, s, tys, fs, entry)
                else {
                    continue;
                };
                for entry in queue_entries.into_iter().rev() {
...
fn match_pattern_has_binders(pat: &T::MatchPattern, rhs_binders: &BTreeSet<Var>) -> bool {
    match &pat.pat.value {
        ...
        TP::At(x, inner) => rhs_binders.contains(x) || match_pattern_has_binders(inner, rhs_binders),
        TP::Variant(_, _, _, _, fields) | TP::BorrowVariant(_, _, _, _, _, fields) => {
            fields.iter().any(|(_, _, (_, (_, pat)))| match_pattern_has_binders(pat, rhs_binders))
        }
        TP::Struct(_, _, _, fields) | TP::BorrowStruct(_, _, _, _, fields) => {
            fields.iter().any(|(_, _, (_, (_, pat)))| match_pattern_has_binders(pat, rhs_binders))
        }
        TP::Literal(_) => false,
        TP::Wildcard => false,
...
fn arm_variant_unpack(
    ...
    let all_wild = fields.iter().all(|(_, _, (_, (_, pat)))| matches!(pat.pat.value, TP::Wildcard)) || fields.is_empty();
...
fn arm_struct_unpack(
    ...
    let (queue_entries, fields) = make_arm_struct_unpack_fields(context, mut_ref, pat_loc, mident, struct_, fields);
...
fn make_arm_variant_unpack_fields(
    ...
    let decl_fields = context.hlir_context.info.enum_variant_fields(&mident, &enum_, &variant).unwrap();

    let field_tys = {
        let field_tys = fields.map(|_key, (ndx, (ty, _))| (ndx, ty));
        if let Some(mut_) = mut_ref {
            field_tys.map(|_field, (ndx, sp!(loc, ty))| {
                (ndx, sp(loc, N::Type_::Ref(mut_, Box::new(sp(loc, ty.base_type_())))))
            })
        } else {
            field_tys
        }
    };
    let fringe_binders = context.hlir_context.make_unpack_binders(decl_fields.clone(), pat_loc, field_tys);
    let fringe_exps = make_fringe_entries(&fringe_binders);
    ...
    let queue_entries =
        fringe_exps.into_iter().zip(ordered_pats.into_iter().map(|(_, _, ordered_pat)| ordered_pat)).collect::<Vec<_>>();
...
fn make_arm_struct_unpack_fields(
    ...
    let field_pats = fields.clone().map(|_key, (ndx, (_, pat))| (ndx, pat));
    let decl_fields = context.hlir_context.info.struct_fields(&mident, &struct_).unwrap();
...
fn make_match_variant_unpack(
    ...
    for (ndx, (field_name, var, ty)) in fields.into_iter().enumerate() {
        ...
        lvalue_fields.add(field_name, (ndx, (lhs_ty, var_lvalue))).unwrap();
    }

    let unpack_lvalue =
        sp(rhs_loc, T::LValue_::BorrowUnpackVariant(false, mident, enum_, variant, tyargs, lvalue_fields));
...
fn make_match_struct_unpack(
    ...
    let unpack_lvalue = sp(rhs_loc, T::LValue_::BorrowUnpack(false, mident, struct_, tyargs, lvalue_fields));
...
fn make_arm_variant_unpack_stmt(
    ...
    let binder = T::SequenceItem_::Bind(sp(rhs_loc, vec![sp(rhs_loc, unpack_lvalue_)]), vec![Some(rhs_ty)], rhs);
    sp(rhs_loc, binder)
}
...
fn make_lvalue(lhs: Var, mut_: Mutability, ty: Type) -> T::LValue {
    let lhs_loc = lhs.loc;
    let lhs_var = T::LValue_::Var { var: lhs, ty: Box::new(ty.clone()), mut_: Some(mut_), unused_binding: false };
    sp(lhs_loc, lhs_var)
}
...
fn make_move_binding(lhs: Var, mut_: Mutability, ty: Type, rhs: FringeEntry) -> T::SequenceItem {
    ...
    let binder = T::SequenceItem_::Bind(sp(lhs_loc, vec![lhs_lvalue]), vec![Some(ty)], Box::new(rhs.into_move_exp()));
    sp(lhs_loc, binder)
}
...
fn make_lit_test(lit_exp: T::Exp, value: Value) -> T::Exp {
    let loc = value.loc;
    let value_exp = T::exp(lit_exp.ty.clone(), sp(loc, T::UnannotatedExp_::Value(value)));
...
fn make_if_else(test: T::Exp, conseq: T::Exp, alt: T::Exp, result_ty: Type) -> T::Exp {
    ...
    T::exp(result_ty, sp(loc, T::UnannotatedExp_::IfElse(Box::new(test), Box::new(conseq), Some(Box::new(alt)))))
}

fn make_copy_exp(ty: Type, loc: Loc, var: Var) -> T::Exp {
    let exp_ = T::UnannotatedExp_::Copy { var, from_user: false };
...
impl AstDebug for PatternMatrix {
    fn ast_debug(&self, w: &mut AstWriter) {
        for arm in &self.patterns {
            let PatternArm { pats: pat, guard, arm } = arm;
            w.write("    { ");
