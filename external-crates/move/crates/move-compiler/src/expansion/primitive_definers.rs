...
pub fn modules(
    ...
    for (mident, m) in modules.key_cloned_iter() {
        check_prim_definer(&reporter, /* allow shadowing */ false, &mut definers, mident, m)
    }
    if let Some(pre_compiled_lib) = pre_compiled_lib_opt {
        for (mident, m) in pre_compiled_lib.expansion.modules.key_cloned_iter() {
            check_prim_definer(&reporter, /* allow shadowing */ true, &mut definers, mident, m)
        }
    }
...
fn check_prim_definer(
    ...
    let defines_prim_attr = m.attributes.get_(&KnownAttribute::DefinesPrimitive(DefinesPrimitive));
    let Some(sp!(attr_loc, attr_)) = defines_prim_attr else {
        return;
    };
    let Attribute_::Parameterized(_, params) = attr_ else {
        let msg =
            format!("Expected a primitive type parameterization, e.g. '{}(<type>)'", DefinesPrimitive::DEFINES_PRIM);
    ...
    if let Some(prev) = definers.get(&prim) {
        if !allow_shadowing {
            ...
            reporter.add_diag(diag!(Attributes::InvalidUsage, (*attr_loc, msg), (prev.loc, "Previously declared here")));
        }
    } else {
