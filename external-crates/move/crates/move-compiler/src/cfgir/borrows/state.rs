...
impl BorrowState {
    ...
    pub fn initial<T>(locals: &UniqueMap<Var, T>, mutably_used: RefExpInfoMap, prev_had_errors: bool) -> Self {
        let mut new_state = BorrowState {
            ...
            prev_had_errors,
            mutably_used,
            next_eid: ExpBasedID { block: None, command: 0, count: 0 },
    ...
    fn borrow_error<F: Fn() -> String>(
        ...
        for (borrower, rloc) in full_borrows {
            ...
            diag.add_secondary_label((*rloc, format!("It is still being {}borrowed by this reference", adj)))
        }
        for (field_lbl, borrowers) in field_borrows {
            for (borrower, rloc) in borrowers {
                ...
                diag.add_secondary_label((
                    *rloc,
                    format!("Field '{}' is still being {}borrowed by this reference", field, adj),
                ))
            }
        }
    ...
    pub fn start_command(&mut self, block: H::Label, command: usize) {
        self.next_eid = ExpBasedID { block: Some(block), command, count: 0 };
    }

    fn set_param_name(&mut self, id: RefID, name: Var) {
        let exps = self.id_to_exp.get(&id).unwrap();
        assert_eq!(exps.len(), 1);
        let infos: &mut BTreeMap<ExpBasedID, RefExpInfo> = &mut RefCell::borrow_mut(&self.mutably_used);
    ...
    fn new_exp(&mut self, loc: Loc, is_mut: bool) -> ExpBasedID {
        ...
        infos.entry(eid).or_insert_with(|| RefExpInfo { loc, is_mut, used_mutably: false, param_name: None });
    ...
    fn add_field_borrow(&mut self, loc: Loc, parent: RefID, field: Field, child: RefID) {
        self.borrows.add_strong_field_borrow(loc, parent, Self::field_label(&field), child)
    }

    fn add_local_borrow(&mut self, loc: Loc, local: &Var, id: RefID) {
        self.borrows.add_strong_field_borrow(loc, Self::LOCAL_ROOT, Self::local_label(local), id)
    }
    ...
    fn writable<F: Fn() -> String>(&self, loc: Loc, msg: F, id: RefID) -> Diagnostics {
        ...
        Self::borrow_error(&self.borrows, loc, &full_borrows, &field_borrows, ReferenceSafety::Dangling, msg).into()
    }
    ...
    fn freezable<F: Fn() -> String>(
        ...
        let mut_filter_set = |s: BTreeMap<RefID, Loc>| {
            s.into_iter().filter(|(id, _loc)| self.borrows.is_mutable(*id)).collect::<BTreeMap<_, _>>()
        };
        ...
        let mut_field_borrows = field_borrows
            ...
            .collect();
        Self::borrow_error(&self.borrows, loc, &mut_full_borrows, &mut_field_borrows, code, msg).into()
    }
    ...
    fn divergent_control_flow(&mut self) {
        *self = Self::initial(&self.locals, self.mutably_used.clone(), self.prev_had_errors);
    }
    ...
    fn local_borrowed_by(&self, local: &Var) -> BTreeMap<RefID, Loc> {
        ...
        field_borrows.remove(&Self::local_label(local)).unwrap_or_default()
    }
    ...
    fn check_use_borrowed_by(
        ...
        Self::borrow_error(borrows, loc, full_borrows, &BTreeMap::new(), code, move || {
            match display_var(local.value()) {
                DisplayVar::Tmp => panic!("ICE invalid use of tmp local {} with borrows {:#?}", local.value(), borrows),
    ...
    pub fn mutate(&mut self, loc: Loc, rvalue: Value) -> Diagnostics {
        let id = match rvalue {
            Value::NonRef => {
                assert!(self.prev_had_errors, "ICE borrow checking failed {:#?}", loc);
        ...
        for rvalue in rvalues {
            match rvalue {
                Value::Ref(id) if self.borrows.is_mutable(id) => {
                    ...
                    let ds =
                        Self::borrow_error(&self.borrows, loc, &fulls, &fields, ReferenceSafety::InvalidTransfer, msg);
                    diags.add_opt(ds);
                }
                _ => (),
            }
    ...
    pub fn move_local(&mut self, loc: Loc, local: &Var, last_usage_inferred: bool) -> (Diagnostics, Value) {
        let old_value = self.locals.remove(local).unwrap();
        ...
        match old_value {
            ...
            Value::NonRef if last_usage_inferred => {
                ...
                diag_opt.iter_mut().for_each(|diag| {
                    ...
                    let tip = format!("Try an explicit annotation, e.g. 'move {v}' or 'copy {v}'", v = vstr);
    ...
    pub fn copy_local(&mut self, loc: Loc, local: &Var) -> (Diagnostics, Value) {
        match self.locals.get(local).unwrap() {
            ...
            Value::NonRef => {
                ...
                let mut_borrows = borrowed_by.into_iter().filter(|(id, _loc)| borrows.is_mutable(*id)).collect();
    ...
    pub fn borrow_local(&mut self, loc: Loc, mut_: bool, local: &Var) -> (Diagnostics, Value) {
        ...
        let diags = if !mut_ {
            ...
            // check that it is 'readable'
            let mut_borrows = borrowed_by.into_iter().filter(|(id, _loc)| borrows.is_mutable(*id)).collect();
            Self::check_use_borrowed_by(borrows, loc, local, &mut_borrows, ReferenceSafety::RefTrans, "borrow").into()
        } else {
            Diagnostics::new()
        };
    ...
    pub fn freeze(&mut self, loc: Loc, rvalue: Value) -> (Diagnostics, Value) {
        ...
        let diags = self.freezable(loc, ReferenceSafety::MutOwns, || "Invalid freeze.".into(), id, None);
    ...
    pub fn dereference(&mut self, loc: Loc, rvalue: Value) -> (Diagnostics, Value) {
        ...
        let diags = self.readable(loc, ReferenceSafety::MutOwns, || "Invalid dereference.".into(), id, None);
        self.release(id);
        (diags, Value::NonRef)
    }
    ...
    pub fn borrow_field(
        ...
        let diags = if mut_ {
            ...
            Self::borrow_error(&self.borrows, loc, &full_borrows, &BTreeMap::new(), ReferenceSafety::MutOwns, msg).into()
        } else {
            let msg = || format!("Invalid immutable borrow at field '{}'.", field);
            self.readable(loc, ReferenceSafety::RefTrans, msg, id, Some(field))
        };
        let copy_parent = if from_unpack.is_some() { Some(id) } else { None };
    ...
    pub fn borrow_variant_fields(
        ...
        let id = match rvalue {
            Value::NonRef => {
                assert!(self.prev_had_errors, "ICE borrow checking failed {:#?}", loc);
                return (Diagnostics::new(), fields.iter().map(|_| Value::NonRef).collect::<Vec<_>>());
            }
            Value::Ref(id) => id,
        };
    ...
    pub fn call(&mut self, loc: Loc, args: Values, return_ty: &Type) -> (Diagnostics, Values) {
        ...
        args.iter().filter_map(|arg| arg.as_vref().filter(|id| self.borrows.is_mutable(*id))).for_each(|mut_id| {
            ...
            let ds = Self::borrow_error(&self.borrows, loc, &fulls, &fields, ReferenceSafety::InvalidTransfer, msg);
            diags.add_opt(ds);
        });
        ...
        for value in &values {
            if let Value::Ref(id) = value {
                ...
                self.mark_mutably_used(*id);
                let parents = if self.borrows.is_mutable(*id) { &mut_parents } else { &all_parents };
    ...
    pub fn canonicalize_locals(&mut self, local_numbers: &UniqueMap<Var, usize>) {
        ...
        self.locals.iter_mut().for_each(|(_, _, v)| v.remap_refs(&id_map));
    ...
    pub fn join_(mut self, mut other: Self) -> Self {
        ...
        for (local, self_value) in self.locals.key_cloned_iter() {
            let joined_value = match (self_value, other.locals.get(&local).unwrap()) {
                ...
                (Value::NonRef, Value::Ref(released_id)) | (Value::Ref(released_id), Value::NonRef) => {
    ...
    fn leq(&self, other: &Self) -> bool {
        ...
        assert!(self_prev_had_errors == other_prev_had_errors, "ICE previous errors flag changed");
        self_locals == other_locals
            && self_borrows.leq(other_borrows)
            && other_id_to_exp.iter().all(|(id, other_eids)| {
                self_id_to_exp.get(id).map(|self_eids| other_eids.is_subset(self_eids)).unwrap_or(false)
            })
    }
}
