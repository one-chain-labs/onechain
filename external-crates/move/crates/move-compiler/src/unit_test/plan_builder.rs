...
use crate::{
    ...
    unit_test::{ExpectedFailure, ExpectedMoveError, ModuleTestPlan, MoveErrorType, TestArgument, TestCase},
};
use move_core_types::{
...
impl<'env> Context<'env> {
    fn new(compilation_env: &'env CompilationEnv, prog: &G::Program) -> Self {
        ...
        Self { env: compilation_env, reporter, constants }
    }
    ...
    fn constants(&self) -> &UniqueMap<ModuleIdent, UniqueMap<ConstantName, (Loc, Option<u64>, Attributes)>> {
        &self.constants
    }
}
    ...
    Some(
        prog.modules
            ...
            .flat_map(|(module_ident, module_def)| {
                ...
                let plan = construct_module_test_plan(&mut context, package_filter, module_ident, module_def);
...
fn construct_module_test_plan(
    ...
    let tests: BTreeMap<_, _> = module
        ...
        .filter_map(|(loc, fn_name, func)| {
            ...
            let info = build_test_info(context, loc, fn_name, func).map(|test_case| (fn_name.to_string(), test_case));
            context.pop_warning_filter_scope();
            info
        })
        .collect();
...
fn build_test_info<'func>(
    ...
    let get_attrs = |attr: TestingAttribute| -> Option<&'func E::Attribute> { function.attributes.get_(&attr.into()) };
    ...
    let (test_attribute, is_random_test) = match (test_attribute_opt, random_test_attribute_opt) {
        (None, None) => {
            ...
            if let Some(abort_attribute) = abort_attribute_opt {
                ...
                context.add_diag(diag!(Attributes::InvalidUsage, (fn_loc, fn_msg), (abort_attribute.loc, abort_msg),))
            }
            return None;
        }
            ...
            None if is_random_test => {
                let generated_type = match convert_builtin_type_to_typetag(&s_type.value) {
                    ...
                    None => {
                        ...
                        let mut diag = diag!(Attributes::InvalidTest, (s_type.loc, msg), (fn_loc, IN_THIS_TEST_MSG),);
                        diag.add_note(
    ...
    if is_random_test && arguments.is_empty() {
        ...
        context.add_diag(diag!(Attributes::InvalidTest, (test_attribute.loc, msg), (fn_loc, IN_THIS_TEST_MSG),));
        return None;
    }
    ...
    Some(TestCase { test_name: fn_name.to_string(), arguments, expected_failure })
}
...
fn parse_test_attribute(
    ...
    match test_attribute {
        EA::Name(_) | EA::Parameterized(_, _) if depth > 0 => {
            context
                .add_diag(diag!(Attributes::InvalidTest, (*aloc, "Unexpected nested attribute in test declaration"),));
            BTreeMap::new()
        }
        EA::Name(nm) => {
            assert!(check_attribute_name(nm.value.as_str()), "ICE: We should only be parsing a raw test attribute");
            BTreeMap::new()
        }
        ...
        EA::Parameterized(nm, attributes) => {
            assert!(check_attribute_name(nm.value.as_str()), "ICE: We should only be parsing a raw test attribute");
            attributes.iter().flat_map(|(_, _, attr)| parse_test_attribute(context, attr, depth + 1)).collect()
        }
    }
}
...
fn parse_failure_attribute(context: &mut Context, sp!(aloc, expected_attr): &E::Attribute) -> Option<ExpectedFailure> {
    use E::Attribute_ as EA;
    match expected_attr {
        EA::Name(nm) => {
            assert!(
        ...
        EA::Assigned(_, value) => {
            ...
            let expected_msg = "Expect an #[expected_failure(...)] attribute for error specification";
            context.add_diag(diag!(
                Attributes::InvalidValue,
        ...
        EA::Parameterized(sp!(_, nm), attrs) => {
            ...
            let mut attrs: BTreeMap<String, (Loc, Attribute)> =
                attrs.key_cloned_iter().map(|(sp!(kloc, k_), v)| (k_.to_string(), (kloc, v.clone()))).collect();
            ...
            let (expected_failure_kind, (attr_loc, attr)) = expected_failure_kind_vec.pop().unwrap();
            let (status_code, sub_status_code, location) = match expected_failure_kind.as_str() {
                TestingAttribute::ABORT_CODE_NAME => {
                    let (value_name_loc, attr_value) =
                        get_assigned_attribute(context, TestingAttribute::ABORT_CODE_NAME, attr_loc, attr)?;
                    let (value_loc, const_location_opt, u) =
                        convert_constant_value_u64_constant_or_value(context, value_name_loc, &attr_value)?;
                ...
                TestingAttribute::ARITHMETIC_ERROR_NAME => {
                    check_attribute_unassigned(context, TestingAttribute::ARITHMETIC_ERROR_NAME, attr_loc, attr)?;
                    let (location_loc, location_attr) =
                        check_location(context, attr_loc, TestingAttribute::ARITHMETIC_ERROR_NAME, location_opt)?;
                ...
                TestingAttribute::OUT_OF_GAS_NAME => {
                    check_attribute_unassigned(context, TestingAttribute::OUT_OF_GAS_NAME, attr_loc, attr)?;
                    let (location_loc, location_attr) =
                        check_location(context, attr_loc, TestingAttribute::OUT_OF_GAS_NAME, location_opt)?;
                    let location = convert_location(context, location_loc, location_attr)?;
                    (StatusCode::OUT_OF_GAS, None, location)
                }
                TestingAttribute::VECTOR_ERROR_NAME => {
                    check_attribute_unassigned(context, TestingAttribute::VECTOR_ERROR_NAME, attr_loc, attr)?;
                    ...
                    let minor_status = if let Some((minor_loc, minor_attr)) = minor_attr_opt {
                        let (minor_value_loc, minor_value) =
                            get_assigned_attribute(context, TestingAttribute::MINOR_STATUS_NAME, minor_loc, minor_attr)?;
                        let (_, _, minor_status) =
                            convert_constant_value_u64_constant_or_value(context, minor_value_loc, &minor_value)?;
                        Some(minor_status)
                    } else {
                        None
                    };
                    let (location_loc, location_attr) =
                        check_location(context, attr_loc, TestingAttribute::VECTOR_ERROR_NAME, location_opt)?;
                    let location = convert_location(context, location_loc, location_attr)?;
                    (StatusCode::VECTOR_OPERATION_ERROR, minor_status, location)
                }
                TestingAttribute::MAJOR_STATUS_NAME => {
                    let (value_name_loc, attr_value) =
                        get_assigned_attribute(context, TestingAttribute::MAJOR_STATUS_NAME, attr_loc, attr)?;
                    ...
                    let Some(major_status) = major_status_u64.and_then(|x| StatusCode::try_from(x).ok()) else {
                        let bad_value = format!("Invalid value for '{}'", TestingAttribute::MAJOR_STATUS_NAME,);
                        let no_code = format!("No status code associated with value '{move_error_type}'");
                        context.add_diag(diag!(
                            Attributes::InvalidValue,
                    ...
                    let (location_loc, location_attr) =
                        check_location(context, attr_loc, TestingAttribute::MAJOR_STATUS_NAME, location_opt)?;
                    let location = convert_location(context, location_loc, location_attr)?;
            ...
            for (_, (loc, _)) in attrs {
                let msg = format!("Unused attribute for {}", TestingAttribute::ExpectedFailure.name());
...
fn check_attribute_unassigned(context: &mut Context, kind: &str, attr_loc: Loc, attr: Attribute) -> Option<()> {
    use E::Attribute_ as EA;
    match attr {
        ...
        sp!(loc, _) => {
            let msg = format!("Expected no assigned value, e.g. '{}', for expected failure attribute", kind);
            context.add_diag(diag!(
                Attributes::InvalidValue,
...
fn get_assigned_attribute(
    ...
    match attr {
        ...
        sp!(loc, _) => {
            let msg = format!("Expected assigned value, e.g. '{}=...', for expected failure attribute", kind);
            context.add_diag(diag!(
                Attributes::InvalidValue,
                (attr_loc, "Unsupported attribute in this location"),
                (loc, msg)
...
fn convert_location(context: &mut Context, attr_loc: Loc, attr: Attribute) -> Option<ModuleId> {
    ...
    let (loc, value) = get_assigned_attribute(context, TestingAttribute::ERROR_LOCATION, attr_loc, attr)?;
    match value {
...
fn convert_constant_value_u64_constant_or_value(
    ...
    let (vloc, module, member) = match value {
        sp!(vloc, EAV::ModuleAccess(sp!(_, ModuleAccess_::ModuleAccess(m, n)))) => (*vloc, m, n),
        _ => {
    ...
    let constant = match modules_constants.get_(&member.value) {
        None => {
            context.add_diag(diag!(
                Attributes::InvalidValue,
                (vloc, INVALID_VALUE),
                (module.loc, format!("Unbound constant '{member}' in module '{module}'")),
            ));
            return None;
        }
        Some(c) => c,
    ...
    match constant {
        ...
        (cloc, None, _) => {
            ...
            context.add_diag(diag!(Attributes::InvalidValue, (vloc, INVALID_VALUE), (*cloc, msg),));
            None
        }
...
fn convert_module_id(context: &mut Context, vloc: Loc, module: &ModuleIdent) -> Option<ModuleId> {
    ...
    let addr = match address {
        Address::Numerical { value: sp!(_, a), .. } => a.into_inner(),
...
fn convert_attribute_value_u64(context: &mut Context, loc: Loc, value: &AttributeValue) -> Option<(Loc, u64)> {
...
fn convert_attribute_value_to_move_value(context: &mut Context, value: &E::AttributeValue_) -> Option<MoveValue> {
    use E::{AttributeValue_ as EAV, Value_ as EV};
    match value {
        // Only addresses are allowed
        EAV::Value(sp!(_, EV::Address(a))) => {
            Some(MoveValue::Address(MoveAddress::new(context.resolve_address(a).into_bytes())))
        }
        _ => None,
    }
}

fn check_location<T>(context: &mut Context, loc: Loc, attr: &str, location: Option<T>) -> Option<T> {
    if location.is_none() {
        let msg = format!("Expected '{}' following '{attr}'", TestingAttribute::ERROR_LOCATION);
    ...
    fn get_builtin_type_inner(bt: &HA::BaseType) -> Option<TypeTag> {
        match &bt.value {
            HA::BaseType_::Apply(_, sp!(_, HA::TypeName_::Builtin(b)), bts) => {
                let mut tts = bts.iter().map(get_builtin_type_inner).collect::<Option<Vec<_>>>()?;
            ...
            HA::BaseType_::Param(_) | HA::BaseType_::Unreachable | HA::BaseType_::UnresolvedError => None,
        }
    }
    match s_type {
