...
provide one. By default we will use the current chain ID from `one client` (which we will keep
...
4. If there are multiple environments `e1`, `e2` in the manifest having chain ID `i`, we produce an
   ...
   `testnet`.
    ...
    > available. Run `one move build --build-env e1` or `one move build --build-env e2`
...
6. If there is no `Pub.e.toml` file, we produce an error, depending on the command:
   For build:
    ...
    >
    >   one move build --build-env testnet
    >
    > Note: adding local networks to `Move.toml` is discouraged; see `one client test-publish --help`
...
If the user runs `one move update-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `one move update-deps d1 d2` we rerun these steps only for the
