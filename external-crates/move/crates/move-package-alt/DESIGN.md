...
# Document status
 ...
 - 4/25/25
     - TODO:
         ...
         - one move build/test now use the default dependencies unless you ask otherwise
...
# pinned transitive dependencies from most recent `one move upgrade-deps`
...
1. The user explicitly asks for it by running `one move update-deps`. This command will repin all
   dependencies (or you could ask to update a single dependency using `one move update-deps
   dep-name`).
...
To the best of our ability, we maintain the following invariants between `Move.toml` and `Move.lock`s:
 ...
 - unpublished.dependencies.unpinned matches manifest.dependencies (likewise for dep-replacements).
   If we discover a mismatch, we upgrade the mismatched dependency (as if the user ran `one move
   upgrade-deps foo`
...
best-effort on that. We may provide an additional tool to help fix things up (e.g. `one move
...
If the user runs `one move upgrade-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `one move upgrade-deps d1 d2` we rerun these steps only for the
