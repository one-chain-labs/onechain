...
use crate::{
    ...
    annotated_visitor::{self, NullTraversal, StructDriver, Traversal, ValueDriver, VariantDriver, VecDriver, Visitor},
...
impl<'b, 'l> Traversal<'b, 'l> for CountingTraversal {
    ...
    fn traverse_u8(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: u8) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u16(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: u16) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u32(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: u32) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u64(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: u64) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u128(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: u128) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u256(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: U256) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_bool(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: bool) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }
    ...
    fn traverse_signer(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, _value: AccountAddress) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }
    ...
    fn traverse_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<(), Self::Error> {
        self.0 += 1;
        while driver.next_field(self)?.is_some() {}
        Ok(())
    }

    fn traverse_variant(&mut self, driver: &mut VariantDriver<'_, 'b, 'l>) -> Result<(), Self::Error> {
        self.0 += 1;
        while driver.next_field(self)?.is_some() {}
        Ok(())
    }
...
impl<'b, 'l> Visitor<'b, 'l> for PrintVisitor {
    ...
    fn visit_u8(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: u8) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u8", self.depth).unwrap();
        Ok(MoveValue::U8(value))
    }

    fn visit_u16(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: u16) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u16", self.depth).unwrap();
        Ok(MoveValue::U16(value))
    }

    fn visit_u32(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: u32) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u32", self.depth).unwrap();
        Ok(MoveValue::U32(value))
    }

    fn visit_u64(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: u64) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u64", self.depth).unwrap();
        Ok(MoveValue::U64(value))
    }

    fn visit_u128(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: u128) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u128", self.depth).unwrap();
        Ok(MoveValue::U128(value))
    }

    fn visit_u256(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: U256) -> Result<Self::Value, Self::Error> {
        write!(self.output, "\n[{}] {value}: u256", self.depth).unwrap();
        Ok(MoveValue::U256(value))
    }

    fn visit_bool(&mut self, _driver: &ValueDriver<'_, 'b, 'l>, value: bool) -> Result<Self::Value, Self::Error> {
    ...
    fn visit_vector(&mut self, driver: &mut VecDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        let layout = driver.element_layout();
        write!(self.output, "\n[{}] vector<{layout:#}>", self.depth).unwrap();

        let mut elems = vec![];
        let mut elem_visitor = Self { depth: self.depth + 1, output: std::mem::take(&mut self.output) };
    ...
    fn visit_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
        let layout = driver.struct_layout();
        write!(self.output, "\n[{}] {layout:#}", self.depth).unwrap();

        let mut fields = vec![];
        let mut field_visitor = Self { depth: self.depth + 1, output: std::mem::take(&mut self.output) };
    ...
    fn visit_variant(&mut self, driver: &mut VariantDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
...
fn traversal() {
    ...
    let type_layout = struct_layout_("0x0::foo::Bar", vec![
        ("a", T::U8),
        ("b", T::U16),
        ("c", T::U32),
        ("d", T::U64),
        ...
        ("m", enum_layout_("0x0::foo::Qux", vec![("n", vec![("o", T::U8)])])),
    ]);
    ...
    let value = struct_value_("0x0::foo::Bar", vec![
        ...
        ("m", variant_value_("0x0::foo::Qux", "n", 0, vec![("o", V::U8(11))])),
    ]);
...
fn unexpected_eof() {
    ...
    assert_eq!(
        ...
        MoveValue::visit_deserialize(&bytes, &type_layout, &mut NullTraversal).unwrap_err().to_string(),
    );

    assert_eq!(
        "unexpected end of input",
        MoveStruct::visit_deserialize(&bytes, struct_layout, &mut NullTraversal).unwrap_err().to_string(),
    );
}
...
fn no_enum_tag() {
    ...
    assert_eq!(
        ...
        MoveValue::visit_deserialize(&bytes, &layout, &mut NullTraversal).unwrap_err().to_string(),
    );
}
...
fn bad_bool_byte() {
    ...
    assert_eq!(
        "unexpected byte: 42",
        MoveValue::visit_deserialize(&[42], &T::Bool, &mut NullTraversal).unwrap_err().to_string(),
    );
}
...
fn trailing_bytes() {
    ...
    assert_eq!(
        ...
        MoveValue::visit_deserialize(&[42, 42], &T::U8, &mut NullTraversal).unwrap_err().to_string(),
    );
}
...
fn nested_datatype_visit() {
    ...
    let type_layout = struct_layout_("0x0::foo::Bar", vec![
        ("inner", struct_layout_("0x0::baz::Qux", vec![("f", T::U64), ("g", T::Vector(Box::new(T::U32)))])),
        ("last", enum_layout_("0x0::foo::Baz", vec![("e", vec![("h", T::U64)])])),
    ]);
    ...
    let value = struct_value_("0x0::foo::Bar", vec![
        (
            "inner",
            struct_value_("0x0::baz::Qux", vec![
                ("f", V::U64(7)),
                ("g", V::Vector(vec![V::U32(1), V::U32(2), V::U32(3)])),
            ]),
        ),
        ("last", variant_value_("0x0::foo::Baz", "e", 0, vec![("h", V::U64(4))])),
    ]);
    ...
    let from_struct = MoveStruct::visit_deserialize(&bytes, struct_layout, &mut struct_visitor).unwrap();
...
fn peek_field_test() {
    ...
    impl<'b, 'l, 'f> Visitor<'b, 'l> for PeekU64Visitor<'f> {
        ...
        fn visit_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
            ...
            while let Some(layout) = driver.peek_field() {
                if layout.name.as_str() == *field {
                    return driver.next_field(&mut Self { fields }).map(|value| value.and_then(|(_, v)| v));
                } else {
                    driver.skip_field()?;
                }
            }
        ...
        fn visit_u16(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: u16) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u32(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: u32) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u128(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: u128) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u256(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: U256) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_bool(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: bool) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_address(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_signer(&mut self, _: &ValueDriver<'_, 'b, 'l>, _: AccountAddress) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        /// Field specifier doesn't support vectors, so we know we won't find the field we want
        /// under here.
        fn visit_vector(&mut self, _: &mut VecDriver<'_, 'b, 'l>) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }
    }
    ...
    let type_layout = struct_layout_("0x0::foo::Bar", vec![
        ...
        ("f", enum_layout_("0x0::foo::Qux", vec![("g", vec![("h", T::U64)])])),
    ]);
    ...
    let value = struct_value_("0x0::foo::Bar", vec![
        ...
        ("f", variant_value_("0x0::foo::Qux", "g", 0, vec![("h", V::U64(46))])),
    ]);

    let bytes = serialize(value);

    let visit_value =
        |fields| MoveValue::visit_deserialize(&bytes, &type_layout, &mut PeekU64Visitor { fields }).unwrap();

    let visit_struct =
        |fields| MoveStruct::visit_deserialize(&bytes, struct_layout, &mut PeekU64Visitor { fields }).unwrap();
...
fn byte_offset_test() {
    ...
    impl<'b, 'l> Traversal<'b, 'l> for ByteOffsetVisitor {
        ...
        fn traverse_u8(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: u8) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u8", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_u16(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: u16) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u16", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_u32(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: u32) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u32", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_u64(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: u64) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u64", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_u128(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: u128) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u128", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_u256(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: U256) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: u256", driver.start(), driver.position()).unwrap();
            Ok(())
        }

        fn traverse_bool(&mut self, driver: &ValueDriver<'_, 'b, 'l>, value: bool) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {value}: bool", driver.start(), driver.position()).unwrap();
            Ok(())
        }
        ...
        fn traverse_struct(&mut self, driver: &mut StructDriver<'_, 'b, 'l>) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {:#}", driver.start(), driver.position(), driver.struct_layout(),)
                .unwrap();
        ...
        fn traverse_variant(&mut self, driver: &mut VariantDriver<'_, 'b, 'l>) -> Result<(), Self::Error> {
            write!(&mut self.0, "\n[{:>3} .. {:>3}] {:#}", driver.start(), driver.position(), driver.enum_layout(),)
                .unwrap();
    ...
    let bytes = serialize(struct_value_("0x0::foo::Bar", vec![
        (
            "inner",
            struct_value_("0x0::baz::Qux", vec![
                ("f", V::U64(7)),
...
pub(crate) fn struct_value_(rep: &str, fields: Vec<(&str, MoveValue)>) -> MoveValue {
    ...
    let fields = fields.into_iter().map(|(name, value)| (Identifier::new(name).unwrap(), value)).collect();
...
pub(crate) fn struct_layout_(rep: &str, fields: Vec<(&str, MoveTypeLayout)>) -> MoveTypeLayout {
    let type_ = StructTag::from_str(rep).unwrap();
    let fields =
        fields.into_iter().map(|(name, layout)| MoveFieldLayout::new(Identifier::new(name).unwrap(), layout)).collect();

    MoveTypeLayout::Struct(Box::new(MoveStructLayout { type_, fields: Box::new(fields) }))
}

/// Create a variant value for test purposes.
pub(crate) fn variant_value_(rep: &str, name: &str, tag: u16, fields: Vec<(&str, MoveValue)>) -> MoveValue {
    let type_ = StructTag::from_str(rep).unwrap();
    let fields = fields.into_iter().map(|(name, value)| (Identifier::new(name).unwrap(), value)).collect();

    MoveValue::Variant(MoveVariant { type_, variant_name: Identifier::new(name).unwrap(), tag, fields })
}

/// Create an enum layout for test purposes.
pub(crate) fn enum_layout_(rep: &str, variants: Vec<(&str, Vec<(&str, MoveTypeLayout)>)>) -> MoveTypeLayout {
    let type_ = StructTag::from_str(rep).unwrap();
