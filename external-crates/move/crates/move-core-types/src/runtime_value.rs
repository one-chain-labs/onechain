...
use crate::{account_address::AccountAddress, annotated_value as A, fmt_list, u256, VARIANT_COUNT_MAX};
...
impl MoveValue {
    ...
    pub fn vec_to_vec_u8(vec: Vec<MoveValue>) -> AResult<Vec<u8>> {
        ...
        for byte in vec {
            match byte {
                ...
                _ => {
                    return Err(
                        anyhow!("Expected inner MoveValue in Vec<MoveValue> to be a MoveValue::U8".to_string(),),
                    );
                }
            }
        }
    ...
    pub fn decorate(self, layout: &A::MoveTypeLayout) -> A::MoveValue {
        match (self, layout) {
            (MoveValue::Struct(s), A::MoveTypeLayout::Struct(l)) => A::MoveValue::Struct(s.decorate(l)),
            (MoveValue::Variant(s), A::MoveTypeLayout::Enum(l)) => A::MoveValue::Variant(s.decorate(l)),
...
impl MoveStruct {
    ...
    pub fn decorate(self, layout: &A::MoveStructLayout) -> A::MoveStruct {
        ...
        A::MoveStruct {
            type_: type_.clone(),
            fields: vals.into_iter().zip(fields.iter()).map(|(v, l)| (l.name.clone(), v.decorate(&l.layout))).collect(),
        }
    }
...
impl MoveVariant {
    ...
    pub fn decorate(self, layout: &A::MoveEnumLayout) -> A::MoveVariant {
        ...
        let ((v_name, _), v_layout) = variants.iter().find(|((_, v_tag), _)| *v_tag == tag).unwrap();
        A::MoveVariant {
            type_: type_.clone(),
            tag,
            fields: fields
...
impl<'d> serde::de::DeserializeSeed<'d> for &MoveStructLayout {
    ...
    fn deserialize<D: serde::de::Deserializer<'d>>(self, deserializer: D) -> Result<Self::Value, D::Error> {
        Ok(MoveStruct(deserializer.deserialize_tuple(self.0.len(), StructFieldVisitor(&self.0))?))
    }
}
...
impl fmt::Display for MoveVariant {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt_list(f, &format!("variant(tag = {})[", self.tag), &self.fields, "]")
    }
}
