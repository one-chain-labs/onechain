...
pub(crate) fn parse<'a, Tok: Token, R>(
    ...
    let tokens: Vec<_> = Tok::tokenize(s)?.into_iter().filter(|(tok, _)| !tok.is_whitespace()).collect();
...
impl<'a, Tok: Token, I: Iterator<Item = (Tok, &'a str)>> Parser<'a, Tok, I> {
    pub fn new<T: IntoIterator<Item = (Tok, &'a str), IntoIter = I>>(v: T) -> Self {
        Self { count: 0, it: v.into_iter().peekable() }
    }
    ...
    pub fn parse_value<Extra: ParsableValue>(&mut self) -> Result<ParsedValue<Extra>> {
        ...
        Ok(match tok {
            ...
            ValueToken::ByteString => {
                let contents = contents.strip_prefix("b\"").unwrap().strip_suffix('\"').unwrap();
                ParsedValue::Vector(contents.as_bytes().iter().copied().map(ParsedValue::U8).collect())
            }
            ValueToken::HexString => {
                let contents = contents.strip_prefix("x\"").unwrap().strip_suffix('\"').unwrap().to_ascii_lowercase();
                ParsedValue::Vector(hex::decode(contents).unwrap().into_iter().map(ParsedValue::U8).collect())
            }
            ...
            ValueToken::Ident if contents == "vector" => {
                ...
                let values =
                    self.parse_list(|parser| parser.parse_value(), ValueToken::Comma, ValueToken::RBracket, true)?;
            ...
            ValueToken::Ident if contents == "struct" => {
                ...
                let values =
                    self.parse_list(|parser| parser.parse_value(), ValueToken::Comma, ValueToken::RParen, true)?;
    ...
    Ok(match tok {
        ValueToken::Number => ParsedAddress::Numerical(
            NumericalAddress::parse_str(contents)
                .map_err(|s| anyhow!("Failed to parse numerical address '{}'. Got error: {}", contents, s))?,
        ),
