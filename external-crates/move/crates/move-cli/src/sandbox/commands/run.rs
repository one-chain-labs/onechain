...
pub fn run(
    ...
    assert!(
        is_bytecode_file(module_file) && (state.is_module_path(module_file) || !contains_module(module_file)),
    ...
    let signer_addresses =
        signers.iter().map(|s| AccountAddress::from_hex_literal(s)).collect::<Result<Vec<AccountAddress>, _>>()?;
    ...
    let script_type_arguments = vm_type_tags.iter().map(|tag| session.load_type(tag)).collect::<Result<Vec<_>, _>>()?;

    // TODO rethink move-cli arguments for executing functions
    let vm_args = signer_addresses
        .iter()
        .map(|a| MoveValue::Signer(*a).simple_serialize().expect("transaction arguments must serialize"))
        .chain(vm_args)
        .collect();
    let res = {
