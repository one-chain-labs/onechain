...
impl CompiledState {
    pub fn resolve_named_address(&self, s: &str) -> AccountAddress {
        if let Some(addr) = self.named_address_mapping.get(s).or(self.default_named_address_mapping.as_ref()) {
    ...
    pub fn resolve_args<Extra: ParsableValue>(
        ...
        args.into_iter().map(|arg| arg.into_concrete_value(&|s| Some(self.resolve_named_address(s)))).collect()
    }

    pub fn resolve_type_args(&self, type_args: Vec<ParsedType>) -> Result<Vec<TypeTag>> {
        type_args.into_iter().map(|arg| arg.into_type_tag(&|s| Some(self.resolve_named_address(s)))).collect()
    }
}
...
pub trait MoveTestAdapter<'a>: Sized + Send {
    ...
    async fn handle_subcommand(&mut self, subcommand: TaskInput<Self::Subcommand>) -> Result<Option<String>>;
        ...
        match command {
            ...
            TaskCommand::PrintBytecode(PrintBytecodeCommand { syntax }) => {
                ...
                let output = modules.into_iter().fold(output, |output, m| {
                    let MaybeNamedCompiledModule { module, source_map, .. } = m;
                    let source_mapping = match source_map {
                        Some(m) => SourceMapping::new(m, &module),
                        None => SourceMapping::new_without_source_map(&module, Spanned::unsafe_no_loc(()).loc)
            ...
            TaskCommand::Run(RunCommand { signers, args, type_args, gas_budget, syntax, name: None }, extra_args) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
                ...
                let (ret_output, return_values) = self
                    .call_function(&module_id, name.as_ident_str(), type_args, signers, args, gas_budget, extra_args)
                    .await?;
            ...
            TaskCommand::Run(
                RunCommand { signers, args, type_args, gas_budget, syntax, name: Some((raw_addr, module_name, name)) },
                extra_args,
            ) => {
...
fn single_entry_function(modules: &[MaybeNamedCompiledModule]) -> anyhow::Result<(ModuleId, Identifier)> {
    anyhow::ensure!(modules.len() == 1, "Expected exactly one module");
    let module = &modules[0].module;
    let entry_funs: Vec<_> = module.function_defs().iter().filter(|def| def.is_entry).collect();
...
fn display_return_values(return_values: SerializedReturnValues) -> Option<String> {
    let SerializedReturnValues { mutable_reference_outputs, return_values } = return_values;
    let mut output = vec![];
    if !mutable_reference_outputs.is_empty() {
        let values = mutable_reference_outputs
            .iter()
            .map(|(idx, bytes, layout)| {
                let value = move_vm_types::values::Value::simple_deserialize(bytes, layout).unwrap();
                (idx, value)
            })
            .collect::<Vec<_>>();
        let printed = values
    ...
    if !return_values.is_empty() {
        let values = return_values
            .iter()
            .map(|(bytes, layout)| move_vm_types::values::Value::simple_deserialize(bytes, layout).unwrap())
            .collect::<Vec<_>>();
        let printed = values
            .iter()
            .map(|v| {
...
impl CompiledState {
    pub fn new(
        ...
        let pre_compiled_ids = match pre_compiled_deps.clone() {
            ...
            Some(pre_compiled) => pre_compiled
                ...
                .modules
                .key_cloned_iter()
                .map(|(ident, _)| (ident.value.address.into_addr_bytes().into_inner(), ident.value.module.to_string()))
                .collect(),
        };
        let mut state = Self {
        ...
        if let Some(pcd) = pre_compiled_deps {
            for unit in &pcd.compiled {
                ...
                state.add_precompiled(named_addr_opt.map(|n| n.value), unit.named_module.module.clone());
            }
        }
        state
    }
    ...
    pub fn add_with_source_file(
        ...
        assert!(prev.is_none());
        for m in modules {
            let MaybeNamedCompiledModule { named_address: named_addr_opt, module, .. } = m;
            let id = module.self_id();
            self.check_not_precompiled(&id);
            if let Some(named_addr) = named_addr_opt {
                self.compiled_module_named_address_mapping.insert(id.clone(), named_addr);
            }
    ...
    pub fn add_and_generate_interface_file(&mut self, module: CompiledModule) {
        ...
        .unwrap();
        interface_file.reopen().unwrap().write_all(interface_text.as_bytes()).unwrap();
...
pub async fn compile_any<'state, 'adapter: 'result, 'result, F, A, R>(
    ...
    handler: F,
) -> Result<(Option<String>, Option<String>, NamedTempFile, Vec<MaybeNamedCompiledModule>)>
where
    ...
    R: Future<Output = Result<(Option<String>, Vec<MaybeNamedCompiledModule>)>> + 'result,
{
    ...
    let (modules, warnings_opt) = match syntax {
        SyntaxChoice::Source => {
            ...
            let modules = units
                ...
                .map(|unit| {
                    ...
                    MaybeNamedCompiledModule { named_address: named_addr_opt, module, source_map }
                })
                .collect();
        ...
        SyntaxChoice::IR => {
            ...
            (vec![MaybeNamedCompiledModule { named_address: None, module, source_map: None }], None)
        }
    };
...
pub fn store_modules<'a, A: MoveTestAdapter<'a>>(
    ...
    match syntax {
        SyntaxChoice::Source => {
            ...
            test_adapter.compiled_state().add_with_source_file(modules, (path, data))
        }
        SyntaxChoice::IR => {
            let module = modules.pop().unwrap().module;
            test_adapter.compiled_state().add_and_generate_interface_file(module);
        }
    }
}
...
pub fn compile_source_units(
    ...
    fn rendered_diags(files: &MappedFiles, diags: Diagnostics) -> Option<String> {
        ...
        let error_buffer =
            move_compiler::diagnostics::report_diagnostics_to_buffer_with_mapped_files(files, diags, ansi_color);
    ...
    .set_default_config(PackageConfig { edition: state.edition, flavor: state.flavor, ..PackageConfig::default() })
    .run::<PASS_COMPILATION>()?;
    let units_or_diags = comments_and_compiler_res.map(|(_comments, move_compiler)| move_compiler.into_compiled_units());
...
pub fn compile_ir_module(state: &CompiledState, file_name: impl AsRef<Path>) -> Result<CompiledModule> {
    use move_ir_compiler::Compiler as IRCompiler;
    let code = std::fs::read_to_string(file_name).unwrap();
    let named_addresses =
        state.named_address_mapping.iter().map(|(name, addr)| (name.clone(), addr.into_inner())).collect();
    IRCompiler::new(state.dep_modules().collect()).with_named_addresses(named_addresses).into_compiled_module(&code)
}
    ...
    Adapter::Subcommand: Debug,
{
    ...
    writeln!(&mut output, "processed {} task{}", num_tasks, if num_tasks > 1 { "s" } else { "" }).unwrap();
    ...
    let (mut adapter, result_opt) = Adapter::init(default_syntax, fully_compiled_program_opt, init_opt, path).await;
...
async fn handle_known_task<'a, Adapter: MoveTestAdapter<'a>>(
    ...
    let task_text = adapter.render_command_input(&task).unwrap_or_else(|| task.task_text.clone());
...
fn handle_expected_output(test_path: &Path, output: impl AsRef<str>) -> Result<()> {
    ...
    let expected_output = std::fs::read_to_string(&exp_path).unwrap().replace("\r\n", "\n").replace('\r', "\n");
    if output != expected_output {
        let msg = format!("Expected errors differ from actual errors:\n{}", format_diff(expected_output, output),);
