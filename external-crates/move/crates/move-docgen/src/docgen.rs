...
impl<'env> Docgen<'env> {
    ...
    pub fn gen(mut self) -> Vec<(String, String)> {
        ...
        let root_templates = self
            ...
            .filter_map(|file_name| {
                let root_out_name =
                    PathBuf::from(file_name).file_name().unwrap().to_str().unwrap().replace("_template", "");
                match self.parse_root_template(file_name) {
                    Ok(elements) => Some((root_out_name, elements)),
                    Err(_) => {
                        self.env.error(&self.env.unknown_loc(), &format!("cannot read root template `{}`", file_name));
                        None
                    }
                }
            })
        ...
        if let Some(fname) = &self.options.references_file {
            ...
            if File::open(fname).and_then(|mut file| file.read_to_string(&mut content)).is_ok() {
                ...
                self.env.error(&self.env.unknown_loc(), &format!("cannot read references file `{}`", fname));
            }
        }
    ...
    fn expand_root_template(&mut self, output_file_name: &str, elements: Vec<TemplateElement>) {
        ...
        *self.label_counter.borrow_mut() = 0;
        ...
        for elem in elements {
            match elem {
                ...
                TemplateElement::IncludeModule(name) => {
                    if let Some(module_env) = self.env.find_module_by_name(self.env.symbol_pool().make(&name)) {
                        let info = self.infos.get(&module_env.get_id()).expect("module defined").clone();
        ...
        self.output.push((self.make_file_in_out_dir(output_file_name), self.writer.extract_result()));
    }
    ...
    fn compute_module_infos(&mut self, templates: &[(String, Vec<TemplateElement>)]) {
        ...
        let log = |m: &ModuleEnv<'_>, i: &ModuleInfo| {
            info!(
                ...
                out_dir,
                i.target_file,
                if m.is_target() { "is target" } else { "is a dependency" }
            );
        };
        ...
        for m in self.env.get_modules() {
            if !included.contains(&m.get_id()) {
                if let Some(file_name) = self.compute_output_file(&m) {
                    let info =
                        ModuleInfo { target_file: file_name, label: self.make_label_for_module(&m), is_included: false };
                    log(&m, &info);
    ...
    fn compute_output_file(&self, module_env: &ModuleEnv<'env>) -> Option<String> {
        ...
        if !module_env.is_target() {
            ...
            self.options.doc_path.iter().find_map(|dir| {
                ...
                path.push(&file_name);
                if path.exists() {
                    Some(self.path_relative_to(&path, &output_path).to_string_lossy().to_string())
                } else {
                    ...
                    package_name.map(|package_name| {
                        format!("dependencies/{}/{}", package_name.to_string_lossy(), file_name.to_string_lossy())
                    })
                }
            })
        } else {
    ...
    fn gen_module(&mut self, module_env: &ModuleEnv<'env>, info: &ModuleInfo) {
        ...
        let toc_label = if !info.is_included { Some(self.gen_toc_header()) } else { None };
        ...
        let used_modules = module_env
            ...
            .map(|id| module_env.env.get_module(*id).get_name().display_full(module_env.symbol_pool()).to_string())
            .sorted();
        ...
        if !module_env.get_structs().count() > 0 {
            for s in module_env.get_structs().sorted_by(|a, b| Ord::cmp(&a.get_loc(), &b.get_loc())) {
                self.gen_struct(&s);
            }
        }

        if !module_env.get_enums().count() > 0 {
            for s in module_env.get_enums().sorted_by(|a, b| Ord::cmp(&a.get_loc(), &b.get_loc())) {
                self.gen_enum(&s);
            }
        }
    ...
    fn gen_call_diagram(&self, fun_id: QualifiedId<FunId>, is_forward: bool) {
        ...
        while let Some(id) = queue.pop_front() {
            ...
            let next_list = if is_forward { curr_env.get_called_functions() } else { curr_env.get_calling_functions() };

            if fun_env.module_env.get_id() == curr_env.module_env.get_id() {
                dot_src_lines.push(format!("\t{}", curr_name));
            } else {
                let module_name = curr_env.module_env.get_name().display(curr_env.module_env.symbol_pool());
                dot_src_lines.push(format!("\tsubgraph cluster_{} {{", module_name));
                dot_src_lines.push(format!("\t\tlabel = \"{}\";", module_name));
                dot_src_lines.push(format!("\t\t{}[label=\"{}\"]", curr_name, curr_env.get_simple_name_string()));
        ...
        let out_file_path = PathBuf::from(&self.options.output_directory).join("img").join(format!(
            "{}_{}_call_graph.svg",
    ...
    fn gen_dependency_diagram(&self, module_id: ModuleId, is_forward: bool) {
        ...
        while let Some(id) = queue.pop_front() {
            ...
            let dep_list = if is_forward { mod_env.get_used_modules() } else { mod_env.get_using_modules() };
    ...
    fn gen_svg_file(&self, out_file_path: &Path, dot_src: &str) {
        if let Err(e) = fs::create_dir_all(out_file_path.parent().unwrap()) {
            self.env.error(&self.env.unknown_loc(), &format!("cannot create a directory for images ({})", e));
            return;
        }
            ...
            .spawn()
        {
            Ok(c) => c,
            Err(e) => {
                self.env.error(&self.env.unknown_loc(), &format!("The Graphviz tool \"dot\" is not available. {}", e));
                return;
            }
        };
            ...
            Ok(output) => {
                if !output.status.success() {
                    self.env.error(
                        ...
                        &format!("dot failed to generate {}\n{}", out_file_path.to_str().unwrap(), dot_src),
                    );
                }
            }
            Err(e) => {
    ...
    fn gen_toc(&mut self, label: CodeWriterLabel) {
        ...
        {
            let mut level = 0;
            for (nest, entry) in self.toc.borrow().iter().filter(|(n, _)| *n > 0 && *n <= self.options.toc_depth) {
                let n = *nest - 1;
                while level < n {
                    self.begin_items();
                    self.writer.indent();
            ...
            self.writer.process_result(|s| writer.insert_at_label(label, s));
        }
        self.writer = writer;
    }
    ...
    fn gen_index(&self) {
        ...
        let sorted_infos = self.infos.iter().sorted_by(|(id1, _), (id2, _)| {
            let name = |id: ModuleId| self.env.symbol_pool().string(self.env.get_module(id).get_name().name());
    ...
    fn gen_struct(&self, struct_env: &StructEnv<'_>) {
        ...
        self.section_header(&self.struct_title(struct_env), &self.label_for_module_item(&struct_env.module_env, name));
        self.increment_section_nest();
    ...
    fn struct_title(&self, struct_env: &StructEnv<'_>) -> String {
        ...
        format!(
            "{} `{}`",
            if struct_env.has_memory() { "Resource" } else { "Struct" },
    ...
    fn named_constant_display(&self, const_env: &NamedConstantEnv<'_>) -> String {
        ...
        let is_error_const = const_env
            .get_attributes()
            .iter()
            .any(|attr| matches!(attr, Attribute::Apply(_, sym, _) if self.name_string(*sym).to_string() == *"error"));
        ...
        format!(
            ...
            error_const_annot,
            name,
            const_env.get_type().display(&TypeDisplayContext::WithEnv { env: self.env, type_param_names: None }),
            rendered_value,
        )
    }
    ...
    fn struct_header_display(&self, struct_env: &StructEnv<'_>) -> String {
        ...
        if ability_tokens.is_empty() {
            ...
            format!("struct {}{} has {}", name, type_params, ability_tokens.join(", "))
        }
    }
    ...
    fn gen_struct_fields(&self, struct_env: &StructEnv<'_>) {
        ...
        for field in struct_env.get_fields() {
            self.definition_text(
                &format!("`{}: {}`", self.name_string(field.get_name()), field.get_type().display(&tctx)),
                field.get_doc(),
            );
        }
    ...
    fn enum_header_display(&self, enum_env: &EnumEnv<'_>) -> String {
        ...
        if ability_tokens.is_empty() {
            ...
            format!("public enum {}{} has {}", name, type_params, ability_tokens.join(", "))
        }
    }
    ...
    fn gen_enum_variants(&self, enum_env: &EnumEnv<'_>) {
        let tctx = {
            let type_param_names =
                Some(enum_env.get_named_type_parameters().iter().map(|TypeParameter(name, _)| *name).collect_vec());
            TypeDisplayContext::WithEnv { env: self.env, type_param_names }
        };
        self.begin_definitions();
    ...
    fn gen_function(&self, func_env: &FunctionEnv<'_>) {
        ...
        if self.options.include_call_diagrams {
            ...
            self.begin_collapsed(&format!("Show all the functions that \"{}\" calls", &func_name));
            self.image(&format!(
            ...
            self.begin_collapsed(&format!("Show all the functions that call \"{}\"", &func_name));
            self.image(&format!(
    ...
    fn function_header_display(&self, func_env: &FunctionEnv<'_>) -> String {
        ...
        let return_str = match return_types.len() {
            ...
            _ => format!(": ({})", return_types.iter().map(|ty| ty.display(tctx)).join(", ")),
        };
    ...
    fn type_display_context_for_fun(&self, func_env: &FunctionEnv<'_>) -> TypeDisplayContext<'_> {
        let type_param_names =
            Some(func_env.get_named_type_parameters().iter().map(|TypeParameter(name, _)| *name).collect_vec());
        TypeDisplayContext::WithEnv { env: self.env, type_param_names }
    }

    /// Creates a type display context for a struct.
    fn type_display_context_for_struct(&self, struct_env: &StructEnv<'_>) -> TypeDisplayContext<'_> {
        let type_param_names =
            Some(struct_env.get_named_type_parameters().iter().map(|TypeParameter(name, _)| *name).collect_vec());
        TypeDisplayContext::WithEnv { env: self.env, type_param_names }
    }
    ...
    fn section_header(&self, s: &str, label: &str) {
        ...
        if !label.is_empty() {
            self.label(label);
            let entry = TocEntry { title: s.to_owned(), label: label.to_string() };
            self.toc.borrow_mut().push((level, entry));
        }
        emitln!(self.writer, "{} {}", self.repeat_str("#", self.options.section_level_start + level), s,);
        emitln!(self.writer);
    }
            ...
            if chr == '`' {
                ...
                if is_start_of_code_block {
                    ...
                    assert!(
                        ...
                        self.current_module.as_ref().unwrap().get_name().display_full(self.env.symbol_pool()),
                        text,
                    );

                    write!(&mut decorated_text, "<code>{}</code>", self.decorate_code(&code)).unwrap()
                }
            } else {
    ...
    fn decorate_code(&self, code: &str) -> String {
        static REX: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"(?P<ident>(\b\w+\b\s*::\s*)*\b\w+\b)(?P<call>\s*[(<])?|(?P<lt><)|(?P<gt>>)").unwrap()
        });
        ...
        while let Some(cap) = REX.captures(&code[at..]) {
            let replacement = {
                if cap.name("lt").is_some() {
                    ...
                    let s = m.as_str();
                    if KEYWORDS.contains(&s) || CONTEXTUAL_KEYWORDS.contains(&s) || BUILTINS.contains(&s) {
    ...
    fn resolve_to_label(&self, mut s: &str, is_followed_by_open: bool) -> Option<String> {
        ...
        let try_func_struct_or_const = |module: &ModuleEnv<'_>, name: Symbol, is_qualified: bool| {
            ...
            if module.find_struct(name).is_some()
                ...
                || self.declared_schemas.get(&module.get_id()).map(|s| s.contains(&name)).unwrap_or(false)
        ...
        let parts_sym = parts.iter().map(|p| self.env.symbol_pool().make(p)).collect_vec();
    ...
    fn make_label_for_module(&self, module_env: &ModuleEnv<'_>) -> String {
        module_env.get_name().display_full(self.env.symbol_pool()).to_string().replace("::", "_")
    }
    ...
    fn ref_for_module(&self, module_env: &ModuleEnv<'_>) -> String {
        if let Some(info) = self.infos.get(&module_env.get_id()) {
            let extension =
                if !self.current_module.as_ref().map(|x| x.is_target()).unwrap_or(true) { "../../" } else { "" };
    ...
    fn ref_for_module_item(&self, module_env: &ModuleEnv<'_>, item: Symbol) -> String {
        format!("{}_{}", self.ref_for_module(module_env), item.display(self.env.symbol_pool()))
    }
    ...
    fn type_parameter_list_display(&self, tps: &[TypeParameter]) -> String {
        if tps.is_empty() {
            ...
            format!("<{}>", tps.iter().map(|tp| self.type_parameter_display(tp)).join(", "))
        }
    }
    ...
    fn get_source_with_indent(&self, loc: &Loc) -> String {
        if let Ok(source) = self.env.get_source(loc) {
            ...
            let source_before = self
                .env
                .get_source(&Loc::new(loc.file_id(), Span::new(ByteIndex(peek_start), loc.span().start())))
                .unwrap_or("");
