...
impl<'a> Resolver<'a> {
    ...
    pub(crate) fn instantiate_generic_type(
        ...
        Ok(Type::DatatypeInstantiation(Box::new((
            struct_inst.def,
            instantiation.iter().map(|ty| self.subst(ty, ty_args)).collect::<PartialVMResult<_>>()?,
        ))))
    }
...
impl Function {
    fn new(
        ...
        let (code, locals_len) = match &def.code {
            Some(code) => (code.code.clone(), parameters_len + module.signature_at(code.locals).0.len()),
            None => (vec![], 0),
        };
