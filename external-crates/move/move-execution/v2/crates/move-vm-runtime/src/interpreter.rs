...
use move_vm_profiler::{profile_close_frame, profile_close_instr, profile_open_frame, profile_open_instr};
use move_vm_types::{
    data_store::DataStore,
    gas::{GasMeter, SimpleInstruction},
    loaded_data::runtime_types::Type,
    values::{self, IntegerValue, Locals, Reference, Struct, StructRef, VMValueCast, Value, Vector, VectorRef},
    views::TypeView,
};
...
impl Interpreter {
    ...
    pub(crate) fn entrypoint(
        ...
        if function.is_native() {
            ...
            interpreter.execute_main(loader, data_store, gas_meter, extensions, function, ty_args, args)
        }
    }
...
impl Frame {
    ...
    fn execute_code(
        ...
        self.execute_code_impl(resolver, interpreter, gas_meter).map_err(|e| {
            let e = if resolver.loader().vm_config().error_execution_state {
