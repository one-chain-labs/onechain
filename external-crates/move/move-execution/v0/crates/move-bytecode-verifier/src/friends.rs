...
fn verify_module_impl(module: &CompiledModule) -> PartialVMResult<()> {
    ...
    if module.friend_decls().contains(self_handle) {
        return Err(PartialVMError::new(StatusCode::INVALID_FRIEND_DECL_WITH_SELF));
    }
    ...
    let self_address = module.address_identifier_at(module.module_handle_at(module.self_handle_idx()).address);
    let has_external_friend =
        module.friend_decls().iter().any(|handle| module.address_identifier_at(handle.address) != self_address);
    if has_external_friend {
        return Err(PartialVMError::new(StatusCode::INVALID_FRIEND_DECL_WITH_MODULES_OUTSIDE_ACCOUNT_ADDRESS));
    }
