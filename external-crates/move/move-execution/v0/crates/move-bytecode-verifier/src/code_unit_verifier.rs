...
impl<'a> CodeUnitVerifier<'a> {
    ...
    fn verify_module_impl(
        ...
        for (idx, function_definition) in module.function_defs().iter().enumerate() {
            ...
            let num_back_edges =
                Self::verify_function(verifier_config, index, function_definition, module, &name_def_map, meter)
    ...
    fn verify_function(
        ...
        let resolver = module;
        // verify
        let code_unit_verifier = CodeUnitVerifier { resolver, function_context, name_def_map };
    ...
    fn verify_common(&self, verifier_config: &VerifierConfig, meter: &mut (impl Meter + ?Sized)) -> PartialVMResult<()> {
        StackUsageVerifier::verify(verifier_config, self.resolver, &self.function_context, meter)?;
        ...
        reference_safety::verify(self.resolver, &self.function_context, self.name_def_map, meter)
    }
}
