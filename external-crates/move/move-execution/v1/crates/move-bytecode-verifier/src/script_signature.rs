...
fn verify_module_function_signature(
    ...
    verify_main_signature_impl(resolver, fdef.is_entry, parameters, Some(return_), check_signature)
...
pub fn legacy_script_signature_checks(
    ...
    let return_types = return_idx.map(|idx| &resolver.signature_at(idx).0).unwrap_or(empty_vec);
