...
impl AbstractState {
    ...
    fn add_field_borrow(&mut self, parent: RefID, field: FieldHandleIndex, child: RefID) {
        self.borrow_graph.add_strong_field_borrow((), parent, Label::Field(field), child)
    }
    ...
    pub fn copy_loc(&mut self, offset: CodeOffset, local: LocalIndex) -> PartialVMResult<AbstractValue> {
    ...
    pub fn move_loc(&mut self, offset: CodeOffset, local: LocalIndex) -> PartialVMResult<AbstractValue> {
        let old_value =
    ...
    pub fn st_loc(&mut self, offset: CodeOffset, local: LocalIndex, new_value: AbstractValue) -> PartialVMResult<()> {
    ...
    pub fn borrow_loc(&mut self, offset: CodeOffset, mut_: bool, local: LocalIndex) -> PartialVMResult<AbstractValue> {
    ...
    pub fn move_from(&mut self, offset: CodeOffset, resource: StructDefinitionIndex) -> PartialVMResult<AbstractValue> {
    ...
    pub fn vector_op(&mut self, offset: CodeOffset, vector: AbstractValue, mut_: bool) -> PartialVMResult<()> {
    ...
    pub fn call(
        ...
        meter.add_items(Scope::Function, CALL_PER_ACQUIRES_COST, acquired_resources.len())?;
        // Check acquires
    ...
    pub fn join_(&self, other: &Self) -> Self {
        ...
        Self { current_function, locals, borrow_graph, next_id }
    }
}
...
impl AbstractDomain for AbstractState {
    ...
    fn join(&mut self, state: &AbstractState, meter: &mut (impl Meter + ?Sized)) -> PartialVMResult<JoinResult> {
        ...
        meter.add_items(Scope::Function, JOIN_PER_GRAPH_ITEM_COST, self.borrow_graph.graph_size())?;
