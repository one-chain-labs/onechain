...
The SDK will be published to [npm registry](https://www.npmjs.com/package/hocjs) with the same
...
Note that the `latest` tag for the [published SDK](https://www.npmjs.com/package/hocjs) might
...
```bash
npm install hocjs@experimental
```
...
[current release of the SDK](https://www.npmjs.com/package/hocjs) at
...
```
pnpm --filter hocjs test:unit
```

To run E2E tests against local network

```
pnpm --filter hocjs prepare:e2e

// This will run all e2e tests
pnpm --filter hocjs test:e2e
...
```
VITE_FAUCET_URL='https://faucet-devnet.onelabs.cc:443/gas' VITE_FULLNODE_URL='https://fullnode.devnet.sui.io' pnpm --filter hocjs exec vitest e2e
```
...
```typescript
import { getFullnodeUrl, SuiClient } from 'hocjs/client';
...
```typescript
import { getFaucetHost, requestSuiFromFaucetV0 } from 'hocjs/faucet';
...
```typescript
import { getFullnodeUrl, SuiClient } from 'hocjs/client';
import { Ed25519Keypair } from 'hocjs/keypairs/ed25519';
import { Transaction } from 'hocjs/transactions';
