...
#### Adding non-OCT coins to the link

You can add non-OCT coins to the link by calling `link.addClaimableBalance()`. This method takes the
following params:

- **`coinType`** (required) - The coin type of the coin to add to the link (e.g. `0x2::oct::OCT`).
